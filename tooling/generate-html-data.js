...
/** @typedef {{ source: { idl: string, elements: string }, boolean: AttributeScopes, url: AttributeScopes, integer: AttributeScopes, signedInteger: string[], tokenList: AttributeScopes, eventHandler: string[] }} ReflectTables */
...
const distill = (idl, elements) => {
	...
	// §8.1.7.2.1 event handler content attributes: the `EventHandler` members of
	// the mixin every HTML element includes. `WindowEventHandlers` is left out —
	// those are content attributes on `<body>` / `<frameset>` alone, and an
	// `onunload=""` anywhere else is an ordinary attribute nothing may drop.
	const eventHandler = [
		...(bodies.get("GlobalEventHandlers") || "").matchAll(
			/attribute\s+EventHandler\??\s+(\w+)\s*;/g
		)
	]
		.map((member) => member[1])
		.sort();
	// Sorted, so re-fetching a spec that did not change this table produces no
	// diff — the IDL's own member order is not stable across editions.
	/**
...
// Derived too: an event handler compiled from an empty string is a function
// body that does nothing, which is what no handler at all does.
for (const name of reflect.eventHandler) {
	if (!emptyRemovable.includes(name)) emptyRemovable.push(name);
}
emptyRemovable.sort();
...
// cspell:ignore ${CSPELL_IGNORE} ${reflect.eventHandler.join(" ")}
