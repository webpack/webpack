...
// without `--write` and fails when the checked-in file no longer matches — on a
// moved table alone, since no version of a source is written into it.
//
...
/**
 * The datasets this run read, each with its version, for the log line alone.
 * They are reported rather than emitted, so a bump moving no table moves no
 * byte of `lib/css/data.js`.
...
// WHY: The version a browser that never shipped a construct is given below, and
// the one a spelling still prefixed today is unprefixed at. Finite, a plain
// number and below 2**31, so both version tables are \`Uint32Array\`s of small
// integers; far past any real version, and past the one Safari TP is read as.
