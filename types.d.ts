 ...
 */
declare interface CliConfigOptions {
	/**
	...
	/**
	 * Enable/Disable colors on console
	 */
	color?: boolean;

	/**
	 * Merge two or more configurations using webpack-merge e.g. -c ./webpack.config.js -c ./webpack.test.config.js --merge
	 */
	merge?: boolean;

	/**
	 * Print compilation progress during build
	 */
	progress?: boolean | string;

	/**
	 * Outputs list of supported flags
	 */
	help?: boolean;
	...
	/**
	 * Sets the build target e.g. node
	 */
	target?: string[];
	...
	/**
	 * Determine source maps to use
	 */
	devtool?: string;

	/**
	 * Prefetch this request
	 */
	prefetch?: string;

	/**
	 * Prints result as JSON or store it in a file
	 */
	json?: string | boolean;

	/**
	 * Defines the mode to pass to webpack
	 */
	mode?: Mode;
	...
	/**
	 * You can pass `false` to disable AMD support.
	 */
	amd?: boolean;
	...
	/**
	 * Enable in memory caching. Disable caching.
	 */
	cache?: boolean;

	/**
	 * In memory caching. Filesystem caching.
	 */
	cacheType?: string;
	 ...
	 */
	cacheCacheDirectory?: string;

	/**
	 * Locations for the cache (defaults to cacheDirectory / name).
	 */
	cacheCacheLocation?: string;
	 ...
	 */
	cacheIdleTimeoutForInitialStore?: number;

	/**
	 * A path to a immutable directory (usually a package manager cache directory).
	 */
	cacheImmutablePaths?: string[];

	/**
	 * Clear all items provided in configuration. List of paths that are managed by a package manager and contain a version or hash in its path so all files are immutable.
	 */
	cacheImmutablePathsReset?: boolean;

	/**
	 * A path to a managed directory (usually a node_modules directory).
	 */
	cacheManagedPaths?: string[];

	/**
	 * Clear all items provided in configuration. List of paths that are managed by a package manager and can be trusted to not be modified otherwise.
	 */
	cacheManagedPathsReset?: boolean;
	...
	/**
	 * References to another configuration to depend on.
	 */
	dependencies?: string[];

	/**
	 * Clear all items provided in configuration. References to other configurations to depend on.
	 */
	dependenciesReset?: boolean;

	/**
	 * Clear all items provided in configuration. All modules are loaded upon startup. The last one is exported.
	 */
	entryReset?: boolean;
	 ...
	 */
	experimentsAsset?: boolean;

	/**
	 * Support WebAssembly as asynchronous EcmaScript Module.
	 */
	experimentsAsyncWebAssembly?: boolean;

	/**
	 * Allow output javascript files as module source type.
	 */
	experimentsOutputModule?: boolean;

	/**
	 * Support WebAssembly as synchronous EcmaScript Module (outdated).
	 */
	experimentsSyncWebAssembly?: boolean;

	/**
	 * Allow using top-level-await in EcmaScript Modules.
	 */
	experimentsTopLevelAwait?: boolean;
	...
	/**
	 * Clear all items provided in configuration. Specify dependencies that shouldn't be resolved by webpack, but should become dependencies of the resulting bundle. The kind of the dependency depends on `output.libraryTarget`.
	 */
	externalsReset?: boolean;

	/**
	 * Treat common electron built-in modules in main and preload context like 'electron', 'ipc' or 'shell' as external and load them via require() when used.
	 */
	externalsPresetsElectron?: boolean;

	/**
	 * Treat electron built-in modules in the main context like 'app', 'ipc-main' or 'shell' as external and load them via require() when used.
	 */
	externalsPresetsElectronMain?: boolean;

	/**
	 * Treat electron built-in modules in the preload context like 'web-frame', 'ipc-renderer' or 'shell' as external and load them via require() when used.
	 */
	externalsPresetsElectronPreload?: boolean;
	 ...
	 */
	externalsPresetsElectronRenderer?: boolean;

	/**
	 * Treat node.js built-in modules like fs, path or vm as external and load them via require() when used.
	 */
	externalsPresetsNode?: boolean;

	/**
	 * Treat NW.js legacy nw.gui module as external and load it via require() when used.
	 */
	externalsPresetsNwjs?: boolean;
	 ...
	 */
	externalsPresetsWebAsync?: boolean;
	 ...
	 * A RegExp to select the origin file for the warning.
	 */
	ignoreWarningsFile?: string[];

	/**
	 * A RegExp to select the warning message.
	 */
	ignoreWarningsMessage?: string[];

	/**
	 * A RegExp to select the origin module for the warning.
	 */
	ignoreWarningsModule?: string[];

	/**
	 * Clear all items provided in configuration. Ignore specific warnings.
	 */
	ignoreWarningsReset?: boolean;

	/**
	 * Enable/Disable debug logging for all loggers. Enable debug logging for specific loggers.
	 */
	infrastructureLoggingDebug?: string[];

	/**
	 * Clear all items provided in configuration. Enable debug logging for specific loggers.
	 */
	infrastructureLoggingDebugReset?: boolean;

	/**
	 * Log level.
	 */
	infrastructureLoggingLevel?: string;

	/**
	 * Enable warnings for full dynamic dependencies.
	 */
	moduleExprContextCritical?: boolean;

	/**
	 * Enable recursive directory lookup for full dynamic dependencies.
	 */
	moduleExprContextRecursive?: boolean;

	/**
	 * Sets the default regular expression for full dynamic dependencies.
	 */
	moduleExprContextRegExp?: string;

	/**
	 * Set the default request for full dynamic dependencies.
	 */
	moduleExprContextRequest?: string;
	 ...
	 */
	moduleNoParse?: string[];

	/**
	 * Clear all items provided in configuration. Don't parse files matching. It's matched against the full resolved request.
	 */
	moduleNoParseReset?: boolean;

	/**
	 * Match the child compiler name.
	 */
	moduleRulesCompiler?: string[];

	/**
	 * Match dependency type.
	 */
	moduleRulesDependency?: string[];

	/**
	 * Enforce this rule as pre or post step.
	 */
	moduleRulesEnforce?: string[];

	/**
	 * Shortcut for resource.exclude.
	 */
	moduleRulesExclude?: string[];

	/**
	 * Shortcut for resource.include.
	 */
	moduleRulesInclude?: string[];

	/**
	 * Match the issuer of the module (The module pointing to this module).
	 */
	moduleRulesIssuer?: string[];

	/**
	 * A loader request.
	 */
	moduleRulesLoader?: string[];

	/**
	 * Match module mimetype when load from Data URI.
	 */
	moduleRulesMimetype?: string[];

	/**
	 * Match the real resource path of the module.
	 */
	moduleRulesRealResource?: string[];

	/**
	 * Match the resource path of the module.
	 */
	moduleRulesResource?: string[];

	/**
	 * Match the resource fragment of the module.
	 */
	moduleRulesResourceFragment?: string[];

	/**
	 * Match the resource query of the module.
	 */
	moduleRulesResourceQuery?: string[];

	/**
	 * Flags a module as with or without side effects.
	 */
	moduleRulesSideEffects?: boolean[];

	/**
	 * Shortcut for resource.test.
	 */
	moduleRulesTest?: string[];

	/**
	 * Module type to use for the module.
	 */
	moduleRulesType?: string[];

	/**
	 * Unique loader options identifier.
	 */
	moduleRulesUseIdent?: string[];

	/**
	 * A loader request.
	 */
	moduleRulesUseLoader?: string[];

	/**
	 * Options passed to a loader.
	 */
	moduleRulesUseOptions?: string[];

	/**
	 * A loader request.
	 */
	moduleRulesUse?: string[];

	/**
	 * Clear all items provided in configuration. A list of rules.
	 */
	moduleRulesReset?: boolean;

	/**
	 * Emit errors instead of warnings when imported names don't exist in imported module.
	 */
	moduleStrictExportPresence?: boolean;

	/**
	 * Handle the this context correctly according to the spec for namespace objects.
	 */
	moduleStrictThisContextOnImports?: boolean;

	/**
	 * Enable warnings when using the require function in a not statically analyse-able way.
	 */
	moduleUnknownContextCritical?: boolean;

	/**
	 * Enable recursive directory lookup when using the require function in a not statically analyse-able way.
	 */
	moduleUnknownContextRecursive?: boolean;

	/**
	 * Sets the regular expression when using the require function in a not statically analyse-able way.
	 */
	moduleUnknownContextRegExp?: string;

	/**
	 * Sets the request when using the require function in a not statically analyse-able way.
	 */
	moduleUnknownContextRequest?: string;

	/**
	 * Cache the resolving of module requests.
	 */
	moduleUnsafeCache?: boolean;

	/**
	 * Enable warnings for partial dynamic dependencies.
	 */
	moduleWrappedContextCritical?: boolean;

	/**
	 * Enable recursive directory lookup for partial dynamic dependencies.
	 */
	moduleWrappedContextRecursive?: boolean;

	/**
	 * Set the inner regular expression for partial dynamic dependencies.
	 */
	moduleWrappedContextRegExp?: string;
	 ...
	 */
	optimizationMangleExports?: string;

	/**
	 * Reduce size of WASM by changing imports to shorter strings.
	 */
	optimizationMangleWasmImports?: boolean;

	/**
	 * Merge chunks which contain the same modules.
	 */
	optimizationMergeDuplicateChunks?: boolean;

	/**
	 * Enable minimizing the output. Uses optimization.minimizer.
	 */
	optimizationMinimize?: boolean;

	/**
	 * Define the algorithm to choose module ids (natural: numeric ids in order of usage, named: readable ids for better debugging, hashed: (deprecated) short hashes as ids for better long term caching, deterministic: numeric hash ids for better long term caching, size: numeric ids focused on minimal initial download size, false: no algorithm used, as custom one can be provided via plugin).
	 */
	optimizationModuleIds?: string;

	/**
	 * Set process.env.NODE_ENV to a specific value.
	 */
	optimizationNodeEnv?: string;

	/**
	 * Generate records with relative paths to be able to move the context folder.
	 */
	optimizationPortableRecords?: boolean;

	/**
	 * Figure out which exports are provided by modules to generate more efficient code.
	 */
	optimizationProvidedExports?: boolean;

	/**
	 * Use real [contenthash] based on final content of the assets.
	 */
	optimizationRealContentHash?: boolean;

	/**
	 * Removes modules from chunks when these modules are already included in all parents.
	 */
	optimizationRemoveAvailableModules?: boolean;

	/**
	 * Remove chunks which are empty.
	 */
	optimizationRemoveEmptyChunks?: boolean;

	/**
	 * Create an additional chunk which contains only the webpack runtime and chunk hash maps.
	 */
	optimizationRuntimeChunk?: string;

	/**
	 * The name or name factory for the runtime chunks.
	 */
	optimizationRuntimeChunkName?: string;

	/**
	 * Skip over modules which contain no side effects when exports are not used (false: disabled, 'flag': only use manually placed side effects flag, true: also analyse source code for side effects).
	 */
	optimizationSideEffects?: string;

	/**
	 * Optimize duplication and caching by splitting chunks by shared modules and cache group.
	 */
	optimizationSplitChunks?: boolean;

	/**
	 * Sets the name delimiter for created chunks.
	 */
	optimizationSplitChunksAutomaticNameDelimiter?: string;

	/**
	 * Select chunks for determining shared modules (defaults to "async", "initial" and "all" requires adding these chunks to the HTML).
	 */
	optimizationSplitChunksChunks?: string;

	/**
	 * Size type, like 'javascript', 'webassembly'.
	 */
	optimizationSplitChunksDefaultSizeTypes?: string[];

	/**
	 * Clear all items provided in configuration. Sets the size types which are used when a number is used for sizes.
	 */
	optimizationSplitChunksDefaultSizeTypesReset?: boolean;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksEnforceSizeThreshold?: number;

	/**
	 * Sets the name delimiter for created chunks.
	 */
	optimizationSplitChunksFallbackCacheGroupAutomaticNameDelimiter?: string;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksFallbackCacheGroupMaxAsyncSize?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksFallbackCacheGroupMaxInitialSize?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksFallbackCacheGroupMaxSize?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksFallbackCacheGroupMinSize?: number;

	/**
	 * Sets the template for the filename for created chunks.
	 */
	optimizationSplitChunksFilename?: string;

	/**
	 * Prevents exposing path info when creating names for parts splitted by maxSize.
	 */
	optimizationSplitChunksHidePathInfo?: boolean;

	/**
	 * Maximum number of requests which are accepted for on-demand loading.
	 */
	optimizationSplitChunksMaxAsyncRequests?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksMaxAsyncSize?: number;

	/**
	 * Maximum number of initial chunks which are accepted for an entry point.
	 */
	optimizationSplitChunksMaxInitialRequests?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksMaxInitialSize?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksMaxSize?: number;

	/**
	 * Minimum number of times a module has to be duplicated until it's considered for splitting.
	 */
	optimizationSplitChunksMinChunks?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksMinRemainingSize?: number;

	/**
	 * Size of the javascript part of the chunk.
	 */
	optimizationSplitChunksMinSize?: number;

	/**
	 * Give chunks created a name (chunks with equal name are merged).
	 */
	optimizationSplitChunksName?: string;

	/**
	 * Compare used exports when checking common modules. Modules will only be put in the same chunk when exports are equal.
	 */
	optimizationSplitChunksUsedExports?: boolean;
	 ...
	 */
	optimizationUsedExports?: string;

	/**
	 * The filename of asset modules as relative path inside the `output.path` directory.
	 */
	outputAssetModuleFilename?: string;

	/**
	 * Add charset attribute for script tag.
	 */
	outputCharset?: boolean;
	 ...
	 */
	outputChunkFormat?: string;

	/**
	 * Number of milliseconds before chunk request expires.
	 */
	outputChunkLoadTimeout?: number;
	 ...
	 */
	outputCompareBeforeEmit?: boolean;

	/**
	 * This option enables cross-origin loading of chunks.
	 */
	outputCrossOriginLoading?: string;

	/**
	 * Similar to `output.devtoolModuleFilenameTemplate`, but used in the case of duplicate module identifiers.
	 */
	outputDevtoolFallbackModuleFilenameTemplate?: string;

	/**
	 * Filename template string of function for the sources array in a generated SourceMap.
	 */
	outputDevtoolModuleFilenameTemplate?: string;

	/**
	 * Module namespace to use when interpolating filename template string for the sources array in a generated SourceMap. Defaults to `output.library` if not set. It's useful for avoiding runtime collisions in sourcemaps from multiple webpack projects built as libraries.
	 */
	outputDevtoolNamespace?: string;

	/**
	 * The method of loading chunks (methods included by default are 'jsonp' (web), 'importScripts' (WebWorker), 'require' (sync node.js), 'async-node' (async node.js), but others might be added by plugins).
	 */
	outputEnabledChunkLoadingTypes?: string[];

	/**
	 * Clear all items provided in configuration. List of chunk loading types enabled for use by entry points.
	 */
	outputEnabledChunkLoadingTypesReset?: boolean;
	 ...
	 */
	outputEnabledLibraryTypes?: string[];

	/**
	 * Clear all items provided in configuration. List of library types enabled for use by entry points.
	 */
	outputEnabledLibraryTypesReset?: boolean;

	/**
	 * The method of loading WebAssembly Modules (methods included by default are 'fetch' (web/WebWorker), 'async-node' (node.js), but others might be added by plugins).
	 */
	outputEnabledWasmLoadingTypes?: string[];

	/**
	 * Clear all items provided in configuration. List of wasm loading types enabled for use by entry points.
	 */
	outputEnabledWasmLoadingTypesReset?: boolean;

	/**
	 * The environment supports arrow functions ('() => { ... }').
	 */
	outputEnvironmentArrowFunction?: boolean;

	/**
	 * The environment supports BigInt as literal (123n).
	 */
	outputEnvironmentBigIntLiteral?: boolean;

	/**
	 * The environment supports const and let for variable declarations.
	 */
	outputEnvironmentConst?: boolean;

	/**
	 * The environment supports destructuring ('{ a, b } = obj').
	 */
	outputEnvironmentDestructuring?: boolean;

	/**
	 * The environment supports an async import() function to import EcmaScript modules.
	 */
	outputEnvironmentDynamicImport?: boolean;

	/**
	 * The environment supports 'for of' iteration ('for (const x of array) { ... }').
	 */
	outputEnvironmentForOf?: boolean;

	/**
	 * The environment supports EcmaScript Module syntax to import EcmaScript modules (import ... from '...').
	 */
	outputEnvironmentModule?: boolean;
	 ...
	 */
	outputGlobalObject?: string;

	/**
	 * Digest type used for the hash.
	 */
	outputHashDigest?: string;

	/**
	 * Number of chars which are used for the hash.
	 */
	outputHashDigestLength?: number;

	/**
	 * Algorithm used for generation the hash (see node.js crypto package).
	 */
	outputHashFunction?: string;
	 ...
	 */
	outputHotUpdateChunkFilename?: string;

	/**
	 * The global variable used by webpack for loading of hot update chunks.
	 */
	outputHotUpdateGlobal?: string;

	/**
	 * The filename of the Hot Update Main File. It is inside the `output.path` directory.
	 */
	outputHotUpdateMainFilename?: string;
	 ...
	 */
	outputImportFunctionName?: string;

	/**
	 * The name of the native import.meta object (can be exchanged for a polyfill).
	 */
	outputImportMetaName?: string;
	...
	/**
	 * Clear all items provided in configuration. The name of the library (some types allow unnamed libraries too).
	 */
	outputLibraryReset?: boolean;

	/**
	 * Name of the exposed AMD library in the UMD.
	 */
	outputLibraryAmd?: string;

	/**
	 * Name of the exposed commonjs export in the UMD.
	 */
	outputLibraryCommonjs?: string;

	/**
	 * Part of the name of the property exposed globally by a UMD library.
	 */
	outputLibraryRoot?: string[];

	/**
	 * Clear all items provided in configuration. Name of the property exposed globally by a UMD library.
	 */
	outputLibraryRootReset?: boolean;

	/**
	 * Append the same comment above each import style.
	 */
	outputLibraryAuxiliaryComment?: string;

	/**
	 * Set comment for `amd` section in UMD.
	 */
	outputLibraryAuxiliaryCommentAmd?: string;

	/**
	 * Set comment for `commonjs` (exports) section in UMD.
	 */
	outputLibraryAuxiliaryCommentCommonjs?: string;

	/**
	 * Set comment for `commonjs2` (module.exports) section in UMD.
	 */
	outputLibraryAuxiliaryCommentCommonjs2?: string;

	/**
	 * Set comment for `root` (global variable) section in UMD.
	 */
	outputLibraryAuxiliaryCommentRoot?: string;

	/**
	 * Part of the export that should be exposed as library.
	 */
	outputLibraryExport?: string[];

	/**
	 * Clear all items provided in configuration. Specify which export should be exposed as library.
	 */
	outputLibraryExportReset?: boolean;
	 ...
	 * Clear all items provided in configuration. The name of the library (some types allow unnamed libraries too).
	 */
	outputLibraryNameReset?: boolean;

	/**
	 * Name of the exposed AMD library in the UMD.
	 */
	outputLibraryNameAmd?: string;

	/**
	 * Name of the exposed commonjs export in the UMD.
	 */
	outputLibraryNameCommonjs?: string;

	/**
	 * Part of the name of the property exposed globally by a UMD library.
	 */
	outputLibraryNameRoot?: string[];

	/**
	 * Clear all items provided in configuration. Name of the property exposed globally by a UMD library.
	 */
	outputLibraryNameRootReset?: boolean;

	/**
	 * Type of library (types included by default are 'var', 'module', 'assign', 'this', 'window', 'self', 'global', 'commonjs', 'commonjs2', 'commonjs-module', 'amd', 'amd-require', 'umd', 'umd2', 'jsonp', 'system', but others might be added by plugins).
	 */
	outputLibraryType?: string;

	/**
	 * If `output.libraryTarget` is set to umd and `output.library` is set, setting this to true will name the AMD module.
	 */
	outputLibraryUmdNamedDefine?: boolean;
	 ...
	 */
	outputSourcePrefix?: string;

	/**
	 * Handles exceptions in module loading correctly at a performance cost.
	 */
	outputStrictModuleExceptionHandling?: boolean;
	 ...
	 * The method of loading WebAssembly Modules (methods included by default are 'fetch' (web/WebWorker), 'async-node' (node.js), but others might be added by plugins).
	 */
	outputWasmLoading?: string;

	/**
	 * The filename of WebAssembly modules as relative path inside the `output.path` directory.
	 */
	outputWebassemblyModuleFilename?: string;

	/**
	 * The method of loading chunks (methods included by default are 'jsonp' (web), 'importScripts' (WebWorker), 'require' (sync node.js), 'async-node' (async node.js), but others might be added by plugins).
	 */
	outputWorkerChunkLoading?: string;

	/**
	 * The method of loading WebAssembly Modules (methods included by default are 'fetch' (web/WebWorker), 'async-node' (node.js), but others might be added by plugins).
	 */
	outputWorkerWasmLoading?: string;
	 ...
	 */
	performanceMaxAssetSize?: number;

	/**
	 * Total size of an entry point (in bytes).
	 */
	performanceMaxEntrypointSize?: number;
	...
	/**
	 * Ignore request (replace with empty module). New request.
	 */
	resolveAliasAlias?: string[];
	 ...
	 */
	resolveAliasOnlyModule?: boolean[];

	/**
	 * Clear all items provided in configuration. Redirect module requests.
	 */
	resolveAliasReset?: boolean;

	/**
	 * Field in the description file (usually package.json) which are used to redirect requests inside the module.
	 */
	resolveAliasFields?: string[];

	/**
	 * Clear all items provided in configuration. Fields in the description file (usually package.json) which are used to redirect requests inside the module.
	 */
	resolveAliasFieldsReset?: boolean;
	 ...
	 */
	resolveCacheWithContext?: boolean;

	/**
	 * Condition names for exports field entry point.
	 */
	resolveConditionNames?: string[];

	/**
	 * Clear all items provided in configuration. Condition names for exports field entry point.
	 */
	resolveConditionNamesReset?: boolean;

	/**
	 * Filename used to find a description file (like a package.json).
	 */
	resolveDescriptionFiles?: string[];

	/**
	 * Clear all items provided in configuration. Filenames used to find a description file (like a package.json).
	 */
	resolveDescriptionFilesReset?: boolean;
	 ...
	 */
	resolveEnforceExtension?: boolean;

	/**
	 * Field name from the description file (usually package.json) which is used to provide entry points of a package.
	 */
	resolveExportsFields?: string[];

	/**
	 * Clear all items provided in configuration. Field names from the description file (usually package.json) which are used to provide entry points of a package.
	 */
	resolveExportsFieldsReset?: boolean;
	...
	/**
	 * Clear all items provided in configuration. Extensions added to the request when trying to find the file.
	 */
	resolveExtensionsReset?: boolean;

	/**
	 * Ignore request (replace with empty module). New request.
	 */
	resolveFallbackAlias?: string[];

	/**
	 * Request to be redirected.
	 */
	resolveFallbackName?: string[];

	/**
	 * Redirect only exact matching request.
	 */
	resolveFallbackOnlyModule?: boolean[];

	/**
	 * Clear all items provided in configuration. Redirect module requests.
	 */
	resolveFallbackReset?: boolean;
	 ...
	 */
	resolveImportsFields?: string[];

	/**
	 * Clear all items provided in configuration. Field names from the description file (usually package.json) which are used to provide internal request of a package (requests starting with # are considered as internal).
	 */
	resolveImportsFieldsReset?: boolean;
	...
	/**
	 * Clear all items provided in configuration. Field names from the description file (package.json) which are used to find the default entry point.
	 */
	resolveMainFieldsReset?: boolean;

	/**
	 * Filename used to find the default entry point if there is no description file or main field.
	 */
	resolveMainFiles?: string[];

	/**
	 * Clear all items provided in configuration. Filenames used to find the default entry point if there is no description file or main field.
	 */
	resolveMainFilesReset?: boolean;
	...
	/**
	 * Clear all items provided in configuration. Folder names or directory paths where to find modules.
	 */
	resolveModulesReset?: boolean;

	/**
	 * Prefer to resolve module requests as relative request and fallback to resolving as module.
	 */
	resolvePreferRelative?: boolean;

	/**
	 * Resolve restriction. Resolve result must fulfill this restriction.
	 */
	resolveRestrictions?: string[];

	/**
	 * Clear all items provided in configuration. A list of resolve restrictions. Resolve results must fulfill all of these restrictions to resolve successfully. Other resolve paths are taken when restrictions are not met.
	 */
	resolveRestrictionsReset?: boolean;
	...
	/**
	 * Clear all items provided in configuration. A list of directories in which requests that are server-relative URLs (starting with '/') are resolved. On non-windows system these requests are tried to resolve as absolute path first.
	 */
	resolveRootsReset?: boolean;
	 ...
	 */
	resolveUnsafeCache?: boolean;

	/**
	 * Use synchronous filesystem calls for the resolver.
	 */
	resolveUseSyncFileSystemCalls?: boolean;

	/**
	 * Ignore request (replace with empty module). New request.
	 */
	resolveLoaderAliasAlias?: string[];

	/**
	 * Request to be redirected.
	 */
	resolveLoaderAliasName?: string[];

	/**
	 * Redirect only exact matching request.
	 */
	resolveLoaderAliasOnlyModule?: boolean[];

	/**
	 * Clear all items provided in configuration. Redirect module requests.
	 */
	resolveLoaderAliasReset?: boolean;

	/**
	 * Field in the description file (usually package.json) which are used to redirect requests inside the module.
	 */
	resolveLoaderAliasFields?: string[];

	/**
	 * Clear all items provided in configuration. Fields in the description file (usually package.json) which are used to redirect requests inside the module.
	 */
	resolveLoaderAliasFieldsReset?: boolean;

	/**
	 * Enable caching of successfully resolved requests (cache entries are revalidated).
	 */
	resolveLoaderCache?: boolean;

	/**
	 * Include the context information in the cache identifier when caching.
	 */
	resolveLoaderCacheWithContext?: boolean;

	/**
	 * Condition names for exports field entry point.
	 */
	resolveLoaderConditionNames?: string[];

	/**
	 * Clear all items provided in configuration. Condition names for exports field entry point.
	 */
	resolveLoaderConditionNamesReset?: boolean;

	/**
	 * Filename used to find a description file (like a package.json).
	 */
	resolveLoaderDescriptionFiles?: string[];

	/**
	 * Clear all items provided in configuration. Filenames used to find a description file (like a package.json).
	 */
	resolveLoaderDescriptionFilesReset?: boolean;

	/**
	 * Enforce the resolver to use one of the extensions from the extensions option (User must specify requests without extension).
	 */
	resolveLoaderEnforceExtension?: boolean;

	/**
	 * Field name from the description file (usually package.json) which is used to provide entry points of a package.
	 */
	resolveLoaderExportsFields?: string[];

	/**
	 * Clear all items provided in configuration. Field names from the description file (usually package.json) which are used to provide entry points of a package.
	 */
	resolveLoaderExportsFieldsReset?: boolean;

	/**
	 * Extension added to the request when trying to find the file.
	 */
	resolveLoaderExtensions?: string[];

	/**
	 * Clear all items provided in configuration. Extensions added to the request when trying to find the file.
	 */
	resolveLoaderExtensionsReset?: boolean;

	/**
	 * Ignore request (replace with empty module). New request.
	 */
	resolveLoaderFallbackAlias?: string[];

	/**
	 * Request to be redirected.
	 */
	resolveLoaderFallbackName?: string[];

	/**
	 * Redirect only exact matching request.
	 */
	resolveLoaderFallbackOnlyModule?: boolean[];

	/**
	 * Clear all items provided in configuration. Redirect module requests.
	 */
	resolveLoaderFallbackReset?: boolean;

	/**
	 * Treats the request specified by the user as fully specified, meaning no extensions are added and the mainFiles in directories are not resolved (This doesn't affect requests from mainFields, aliasFields or aliases).
	 */
	resolveLoaderFullySpecified?: boolean;

	/**
	 * Field name from the description file (usually package.json) which is used to provide internal request of a package (requests starting with # are considered as internal).
	 */
	resolveLoaderImportsFields?: string[];

	/**
	 * Clear all items provided in configuration. Field names from the description file (usually package.json) which are used to provide internal request of a package (requests starting with # are considered as internal).
	 */
	resolveLoaderImportsFieldsReset?: boolean;

	/**
	 * Field name from the description file (package.json) which are used to find the default entry point.
	 */
	resolveLoaderMainFields?: string[];

	/**
	 * Clear all items provided in configuration. Field names from the description file (package.json) which are used to find the default entry point.
	 */
	resolveLoaderMainFieldsReset?: boolean;

	/**
	 * Filename used to find the default entry point if there is no description file or main field.
	 */
	resolveLoaderMainFiles?: string[];

	/**
	 * Clear all items provided in configuration. Filenames used to find the default entry point if there is no description file or main field.
	 */
	resolveLoaderMainFilesReset?: boolean;
	 ...
	 * Clear all items provided in configuration. Folder names or directory paths where to find modules.
	 */
	resolveLoaderModulesReset?: boolean;

	/**
	 * Prefer to resolve module requests as relative request and fallback to resolving as module.
	 */
	resolveLoaderPreferRelative?: boolean;

	/**
	 * Resolve restriction. Resolve result must fulfill this restriction.
	 */
	resolveLoaderRestrictions?: string[];

	/**
	 * Clear all items provided in configuration. A list of resolve restrictions. Resolve results must fulfill all of these restrictions to resolve successfully. Other resolve paths are taken when restrictions are not met.
	 */
	resolveLoaderRestrictionsReset?: boolean;

	/**
	 * Directory in which requests that are server-relative URLs (starting with '/') are resolved.
	 */
	resolveLoaderRoots?: string[];

	/**
	 * Clear all items provided in configuration. A list of directories in which requests that are server-relative URLs (starting with '/') are resolved. On non-windows system these requests are tried to resolve as absolute path first.
	 */
	resolveLoaderRootsReset?: boolean;

	/**
	 * Enable resolving symlinks to the original location.
	 */
	resolveLoaderSymlinks?: boolean;

	/**
	 * Enable caching of successfully resolved requests (cache entries are not revalidated).
	 */
	resolveLoaderUnsafeCache?: boolean;

	/**
	 * Use synchronous filesystem calls for the resolver.
	 */
	resolveLoaderUseSyncFileSystemCalls?: boolean;

	/**
	 * Use hashes of the content of the files/directories to determine invalidation.
	 */
	snapshotBuildDependenciesHash?: boolean;

	/**
	 * Use timestamps of the files/directories to determine invalidation.
	 */
	snapshotBuildDependenciesTimestamp?: boolean;

	/**
	 * A path to a immutable directory (usually a package manager cache directory).
	 */
	snapshotImmutablePaths?: string[];

	/**
	 * Clear all items provided in configuration. List of paths that are managed by a package manager and contain a version or hash in its path so all files are immutable.
	 */
	snapshotImmutablePathsReset?: boolean;

	/**
	 * A path to a managed directory (usually a node_modules directory).
	 */
	snapshotManagedPaths?: string[];

	/**
	 * Clear all items provided in configuration. List of paths that are managed by a package manager and can be trusted to not be modified otherwise.
	 */
	snapshotManagedPathsReset?: boolean;

	/**
	 * Use hashes of the content of the files/directories to determine invalidation.
	 */
	snapshotModuleHash?: boolean;

	/**
	 * Use timestamps of the files/directories to determine invalidation.
	 */
	snapshotModuleTimestamp?: boolean;

	/**
	 * Use hashes of the content of the files/directories to determine invalidation.
	 */
	snapshotResolveHash?: boolean;

	/**
	 * Use timestamps of the files/directories to determine invalidation.
	 */
	snapshotResolveTimestamp?: boolean;

	/**
	 * Use hashes of the content of the files/directories to determine invalidation.
	 */
	snapshotResolveBuildDependenciesHash?: boolean;

	/**
	 * Use timestamps of the files/directories to determine invalidation.
	 */
	snapshotResolveBuildDependenciesTimestamp?: boolean;
	 ...
	 */
	statsCachedAssets?: boolean;

	/**
	 * Add information about cached (not built) modules.
	 */
	statsCachedModules?: boolean;

	/**
	 * Add children information.
	 */
	statsChildren?: boolean;

	/**
	 * Display auxiliary assets in chunk groups.
	 */
	statsChunkGroupAuxiliary?: boolean;

	/**
	 * Display children of chunk groups.
	 */
	statsChunkGroupChildren?: boolean;

	/**
	 * Limit of assets displayed in chunk groups.
	 */
	statsChunkGroupMaxAssets?: number;

	/**
	 * Display all chunk groups with the corresponding bundles.
	 */
	statsChunkGroups?: boolean;

	/**
	 * Add built modules information to chunk information.
	 */
	statsChunkModules?: boolean;

	/**
	 * Add the origins of chunks and chunk merging info.
	 */
	statsChunkOrigins?: boolean;

	/**
	 * Add information about parent, children and sibling chunks to chunk information.
	 */
	statsChunkRelations?: boolean;
	 ...
	 */
	statsColorsBold?: string;

	/**
	 * Custom color for cyan text.
	 */
	statsColorsCyan?: string;

	/**
	 * Custom color for green text.
	 */
	statsColorsGreen?: string;

	/**
	 * Custom color for magenta text.
	 */
	statsColorsMagenta?: string;

	/**
	 * Custom color for red text.
	 */
	statsColorsRed?: string;

	/**
	 * Custom color for yellow text.
	 */
	statsColorsYellow?: string;
	 ...
	 */
	statsErrorDetails?: boolean;

	/**
	 * Add internal stack trace to errors.
	 */
	statsErrorStack?: boolean;
	 ...
	 * Add errors count.
	 */
	statsErrorsCount?: boolean;

	/**
	 * Suppress assets that match the specified filters. Filters can be Strings, RegExps or Functions.
	 */
	statsExcludeAssets?: string[];

	/**
	 * Clear all items provided in configuration. Suppress assets that match the specified filters. Filters can be Strings, RegExps or Functions.
	 */
	statsExcludeAssetsReset?: boolean;

	/**
	 * Suppress modules that match the specified filters. Filters can be Strings, RegExps, Booleans or Functions.
	 */
	statsExcludeModules?: string[];

	/**
	 * Clear all items provided in configuration. Suppress modules that match the specified filters. Filters can be Strings, RegExps, Booleans or Functions.
	 */
	statsExcludeModulesReset?: boolean;

	/**
	 * Group assets by how their are related to chunks.
	 */
	statsGroupAssetsByChunk?: boolean;

	/**
	 * Group assets by their status (emitted, compared for emit or cached).
	 */
	statsGroupAssetsByEmitStatus?: boolean;

	/**
	 * Group assets by their extension.
	 */
	statsGroupAssetsByExtension?: boolean;

	/**
	 * Group assets by their asset info (immutable, development, hotModuleReplacement, etc).
	 */
	statsGroupAssetsByInfo?: boolean;

	/**
	 * Group assets by their path.
	 */
	statsGroupAssetsByPath?: boolean;

	/**
	 * Group modules by their attributes (errors, warnings, assets, optional, orphan, or dependent).
	 */
	statsGroupModulesByAttributes?: boolean;

	/**
	 * Group modules by their status (cached or built and cacheable).
	 */
	statsGroupModulesByCacheStatus?: boolean;

	/**
	 * Group modules by their extension.
	 */
	statsGroupModulesByExtension?: boolean;

	/**
	 * Group modules by their path.
	 */
	statsGroupModulesByPath?: boolean;
	 ...
	 */
	statsLoggingDebug?: string[];

	/**
	 * Clear all items provided in configuration. Include debug logging of specified loggers (i. e. for plugins or loaders). Filters can be Strings, RegExps or Functions.
	 */
	statsLoggingDebugReset?: boolean;

	/**
	 * Add stack traces to logging output.
	 */
	statsLoggingTrace?: boolean;

	/**
	 * Add information about assets inside modules.
	 */
	statsModuleAssets?: boolean;

	/**
	 * Add dependencies and origin of warnings/errors.
	 */
	statsModuleTrace?: boolean;

	/**
	 * Add built modules information.
	 */
	statsModules?: boolean;

	/**
	 * Sort the modules by that field.
	 */
	statsModulesSort?: string;

	/**
	 * Space to display modules (groups will be collapsed to fit this space, values is in number of modules/groups).
	 */
	statsModulesSpace?: number;

	/**
	 * Add information about modules nested in other modules (like with module concatenation).
	 */
	statsNestedModules?: boolean;

	/**
	 * Show reasons why optimization bailed out for modules.
	 */
	statsOptimizationBailout?: boolean;

	/**
	 * Add information about orphan modules.
	 */
	statsOrphanModules?: boolean;

	/**
	 * Add output path information.
	 */
	statsOutputPath?: boolean;

	/**
	 * Add performance hint flags.
	 */
	statsPerformance?: boolean;
	 ...
	 */
	statsProvidedExports?: boolean;

	/**
	 * Add public path information.
	 */
	statsPublicPath?: boolean;

	/**
	 * Add information about the reasons why modules are included.
	 */
	statsReasons?: boolean;

	/**
	 * Add information about assets that are related to other assets (like SourceMaps for assets).
	 */
	statsRelatedAssets?: boolean;

	/**
	 * Add information about runtime modules.
	 */
	statsRuntimeModules?: boolean;
	 ...
	 */
	statsTimings?: boolean;

	/**
	 * Show exports used by modules.
	 */
	statsUsedExports?: boolean;

	/**
	 * Add webpack version information.
	 */
	statsVersion?: boolean;

	/**
	 * Add warnings.
	 */
	statsWarnings?: boolean;

	/**
	 * Add warnings count.
	 */
	statsWarningsCount?: boolean;
	 ...
	 */
	statsWarningsFilter?: string[];

	/**
	 * Clear all items provided in configuration. Suppress listing warnings that match the specified filters (they will still be counted). Filters can be Strings, RegExps or Functions.
	 */
	statsWarningsFilterReset?: boolean;

	/**
	 * Clear all items provided in configuration. Environment to build for. An array of environments to build for all of them when possible.
	 */
	targetReset?: boolean;
	 ...
	 */
	watchOptionsAggregateTimeout?: number;

	/**
	 * Resolve symlinks and watch symlink and real file. This is usually not needed as webpack already resolves symlinks ('resolve.symlinks').
	 */
	watchOptionsFollowSymlinks?: boolean;

	/**
	 * A glob pattern for files that should be ignored from watching. Ignore some files from watching (glob pattern or regexp).
	 */
	watchOptionsIgnored?: string[];

	/**
	 * Clear all items provided in configuration. Ignore some files from watching (glob pattern or regexp).
	 */
	watchOptionsIgnoredReset?: boolean;

	/**
	 * `number`: use polling with specified interval. `true`: use polling.
	 */
	watchOptionsPoll?: string;
