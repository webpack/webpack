...
declare interface OutputHtmlOptions {
	...
	/**
	 * Inline the content of matching chunks directly into the HTML instead of emitting a separate `<script>`/`<link>` tag. `true` inlines every chunk; `"script"` inlines only JavaScript, `"style"` only CSS; an array of `RegExp` patterns matches against the chunk name and the name it is emitted under. A chunk another entry loads its runtime from by url stays a separate file.
	 * @since 5.109.0
	 */
