...
- **Usage**: Git submodule — initialize with `git submodule update --init test/css-parsing-tests`. Test runner: `test/cssParsing.spectest.js` (`yarn test:css-parsing`). Asserts token roundtrip and that every entry point terminates without throwing (the suite tracks an older draft, so AST equality is not asserted).
