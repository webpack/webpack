...
- `lib/` — Main source code (CommonJS only; types declared via JSDoc `@typedef`).
  ...
  - `lib/css/` — CSS Modules, CSS parsing and generation; `mathPrimitives.js` holds the arithmetic the generated math-function descriptors bind to.
...
- `lib/css/data.js` — every table the CSS minifier looks a name up in: derived from `mdn-data` + `color-name` (box shorthands, color-argument and math functions, named colors) plus the generator's `SUPPLEMENT` of spec-prose tables, by `tooling/generate-css-data.js` — which also holds the value-definition-syntax parser those grammars are read with, and runs the generation only as the entry point so its tests can require it.
