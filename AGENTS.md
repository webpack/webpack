...
- **Removing information** — a dead rule, a duplicate declaration, a longhand its shorthand implies, whitespace, a comment — shrinks raw, and normally shrinks the compressed columns with it, so it needs no routine compressed check. Not a guarantee: deleting a run a later match pointed back to can cost a compressed byte or two, which is worth a look only where the removal is small and the compressed column moves the other way.
