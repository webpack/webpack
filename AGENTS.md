...
| `yarn lint:all`                                                      | The same stages, but every one runs and a summary names those that failed. Use when a stage trips on drift.       |
...
| `yarn test:case "<category> <case>"`                                 | Run one integration case across every suite that carries it, `ConfigCacheTestCases` included. Add `-u` to update. |
...
**When CI is red, read its log — do not reproduce the whole job locally.** The run already holds the answer, and re-running `yarn lint` or a whole suite to rediscover one line costs minutes and a great many tokens. Filter the run to its failing jobs (`list_workflow_jobs`, or `gh run view <run-id> --json jobs --jq '.jobs[] | select(.conclusion=="failure")'`), read that job's log (`get_job_logs` with `return_content` — its tail is mostly `Post job cleanup`, so ask for enough lines to clear that — or `gh run view --job <id> --log-failed`). Reach for the GitHub tools first: `gh` is not installed in every agent sandbox, and discovering that costs a round trip, then reproduce **only the case it names**: `yarn test:base --testPathPatterns="<file>"`, `yarn test:basic --testNamePattern="<category> <case>"`, or `npx eslint <file>`.
