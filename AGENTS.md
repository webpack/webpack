...
- `lib/` — Main source code (CommonJS only; types declared via JSDoc `@typedef`).
  ...
  - `lib/javascript/` — JavaScript parsing (webpack's own ECMAScript parser, ported from acorn), generation, exports analysis. `syntax.js` serves every production a build reaches, so `grammar.js` (the full grammar) and `regexp.js` (the pattern validator) are installed onto the parser's prototype only when something asks for one — keep it that way and never `require` either from a path a build takes. Minification lives here too: `jsMinify.js` is the `minify` function the default minimizer dispatches JavaScript to, and it minifies through `terser.js` rather than through terser's published entry point — that loader reads terser's own sources, so a phase webpack implements itself (today `terserMangle.js`, which drops the rescan of a scope's enclosed definitions per candidate name) can replace the method terser installs. A phase states what it reads with `supports`, and a terser that moved any of it, or a runtime that cannot import those sources, keeps terser's own. Add a phase to the `PHASES` list in `terser.js` and nowhere else, and hold it to writing byte-for-byte what it replaced.
