...
- `yarn test:terser` — webpack's JS printer vs terser as published, byte for byte, over terser's own `test/compress` corpus.
...
- `test/` — all suites (`unitCases/`, `cases/`, `configCases/`, `specCases/`, `watchCases/`, `hotCases/`, `statsCases/`, `typesCases/`, `benchmarkCases/`, `memoryLimitCases/`, …).
  ...
  - `external/` — what webpack doesn't maintain: every git submodule (today the four spec corpora below and terser's own corpus) — upstream's to change, ours only to pin. `external/wpt/` (web-platform-tests), checked out one commit deep by `parser (html)` and `syntax-equivalence`, holds the HTML tree-construction corpus since html5lib-tests dropped it.
...
- `test/external/terser` — [terser/terser](https://github.com/terser/terser), pinned to the installed `terser`'s version; fetched by `parser (js-printer)`
