...
`webpack/comment-length` reports it, so `yarn lint:code` is where you see it — over whole files rather than a diff, and it reads comment tokens, so comment-shaped lines inside a string are not comments.
