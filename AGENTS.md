...
- `lib/` — main source (CommonJS; JSDoc types). Its **root is the core** — what a build is made of (`Compilation`, `Compiler`, `Dependency`, `MultiCompiler`) and what publishes it (`webpack.js`, `index.js`) — plus re-export shims keeping old `webpack/lib/<Name>` paths open for the ecosystem. **A plugin never belongs in the root**: it goes in the directory for what it acts on (asset set → `lib/output/`, module graph → `lib/optimize/`, entry → `lib/entry/`); if none fits, add a directory, with its bullet here, in the same commit. A file leaving the root owes a shim at its old path only once it has shipped there; `yarn find-deep-imports --check` and `deepPathShims.unittest.js` decide.
  ...
  - `lib/dependencies/` — concrete `Dependency` subclasses and their templates (ESMImport, CommonJsRequire, RequireContext, …); the base `Dependency` is in `lib/graph/`, `DependencyTemplate` in `lib/template/`. Its root holds what every language shares (`NullDependency`, `ModuleDependency`, `ConstDependency`, both `ModuleDependencyTemplateAs*`, the helpers several families read); a family owning a set of them gets its own directory: `amd/` (with the `LocalModule` a named `define` records, `RequireJsStuffPlugin`, and the generically named `UnsupportedDependency`, which only AMD emits), `commonjs/` (with `require.resolve`, `require.ensure`, `require.include` and `ModuleDecoratorDependency`), `context/`, `css/`, `esm/` (with `TopLevelAwaitDependency`), `html/`, `hmr/`, `import/`, `system/`, `wasm/`, `worker/` (with the `CreateScriptUrlDependency` only it constructs). Two group by what a request asks for rather than by syntax: `context/` (the `ContextDependency` base every family's context variant extends, plus `require.context`, `import.meta.webpackContext` and the glob form) and `hmr/` (the `module.hot` and `import.meta.webpackHot` accept/decline pairs). What decides the directory is which plugin applies it, not the language it names: `CreateRequireParserPlugin` reads a CommonJS `require` out of an ESM module, so it sits in `esm/`, because `ESMModulesPlugin` applies it. `import.meta` is not `esm/` for the same reason: `WebpackOptionsApply` applies `ImportMetaPlugin` unconditionally, it taps `javascript/auto` as well as `javascript/esm`, and `lib/node/`, `lib/url/`, `lib/hmr/` and `worker/` read its hooks, so it stays at the root and its context and hot dependencies follow their applier. `import()` sits in `import/`, not `esm/`, on the same evidence: `ImportPlugin` is applied by `WebpackOptionsApply` and taps `javascript/dynamic` too, since the expression is legal in a script. Its `ImportPhase` enum stays at the root, read by `graph/`, `optimize/`, `externals/` and more. A dependency a single feature owns is not here at all but with that feature, by the same rule: `LoaderDependency` in `lib/loaders/`, `JsonExportsDependency` in `lib/json/`, `DllEntryDependency` in `lib/dll/`, the `ExternalModule*` three in `lib/node/`. A class leaving this root moves its `makeSerializable` request with it and holds the old one open with `registerLegacyRequest`, or a pre-move cache pack stops loading — one that moved twice keeps a registration per old request, which is why the `esm/` classes carry both an `ESM` and a `Harmony` one. `ContextElementDependency` also keeps a re-export at its old path, the one moved path a published webpack 5 plugin imports.
...
- `tooling/` — repo-internal scripts:
  ...
  - `compare-css-tools.js` / `compare-html-tools.js` / `compare-js-tools.js` (`yarn benchmark:css-tools` / `:html-tools` / `:js-tools`, sharing `compare-tools-harness.js`) — no arguments or source reading needed. Each runs webpack's implementation for the language and ecosystem equivalents over popular framework stylesheets, real documents and shipped JS bundles.
    ...
    - **Invariants.** Each comparison first holds webpack's implementation to five relations (CSS six) over its own input — `test/**/*.css` / `*.html` / `*.js`, its own fixtures, whatever the cache holds — in seconds, before any install. `--invariants` runs only that and exits non-zero on any finding; the `invariants` job gates every PR on it (`yarn test:invariants`, or `:css` / `:html` / `:js`). A relation is cheap to hold, so a regression fails rather than reports. `FIXTURE=`, `RELATION=`, `PRESET=`, `SPELLING=` narrow a run.
      ...
      6. CSS printer, `readable` (`tooling/css-readability.js`): what the printer writes for a browserslist target, every browser in it reads — asked per browser, since one engine dropping a rule is that rule gone for its users. Three things are owed: nothing unreadable the source didn't already ask for, unless that browser read nothing the source said there either (a shorthand over longhands it read is a loss; one over longhands it couldn't read isn't); no list the printer joined around a selector one browser drops, which drops the rest with it; and no construct left that a lowering the printer states for that target (nesting where it reads `:is()`, `light-dark()`, `:not()`/`:lang()` lists, media ranges, the `text-decoration` shorthand) takes out. Readability comes from `lib/css/data.js`'s tables; only the reading of them is independent, so it catches how the printer applies a table, not a wrong table. Asked under every preset naming browsers plus two older targets closing every gate `SUPPORTED_FROM` names, over the corpus and **stylesheets generated from those tables** — every pseudo alone, in a list, nested beside a sibling, after a declaration and in an at-rule, and every gated spelling alone and nested. Framework CSS reaches few of these shapes (a nested vendor-prefixed rule was in none, which is how webpack/webpack#22308's bug shipped), and `CompareCssReadability.unittest.js` fails when a new table row isn't reached. Each finding names its shape, so a known decline is one `EXPECTED` entry with a `what`.
