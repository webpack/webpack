...
| `yarn test:test262` / `yarn test:html5lib` / `yarn test:css-parsing` / `yarn test:xmlconf` | Spec-conformance suites.                                                                                             |
...
- `lib/` — Main source code (CommonJS only; types declared via JSDoc `@typedef`).
  ...
  - `lib/html/` — Experimental HTML support. `syntax.js` names its two halves the
    ...
    `parseHtml(…, { xml: true })` reads XML 1.0 instead, through `tokenizeXml` and
    one flat tree builder: case-sensitive names, no insertion modes, and the
    internal DTD subset's entities and attribute defaults applied; the printer then
    writes XML back, never HTML, and offers the renderer only an SVG or XHTML
    element's `<style>` and `style=""`, the namespaces where they are CSS.
    `xmlMinify` is what `optimization.minimizer` hands `.svg` and `.xml` assets
    under `experiments.futureDefaults` (or an `optimization.minimizeOptions.xml`
    object), with the CSS options and browsers a `.css` asset gets.
...
| `test/external/xml-conformance-suite` | [lddubeau/xml-conformance-suite](https://github.com/lddubeau/xml-conformance-suite) (the W3C XML Conformance Test Suite) | `parser (xml)`                                   |
...
Keep `--depth 1`: `wpt` alone is ~161k files. `--remote` changes the commit the repo records for each path, so `git status` shows the five `test/…` paths modified — commit that only once CI is green on the new commits, or run `git submodule update` to return to the pinned ones.
...
Swap `ConfigTestCases` for `StatsTestCases`, `HotTestCases`, `WatchTestCases`, … (full matrix in [TESTING_DOCS.md](TESTING_DOCS.md)). The `test262`, `html5lib`, `syntax-equivalence`, `css-parsing` and `xmlconf` suites use git submodules — run `git submodule update --init --depth 1 test/external/test262-cases test/external/html5lib-tests test/external/wpt test/external/css-parsing-tests test/external/xml-conformance-suite` first, or they fail confusingly — except `xmlconf`, which registers a no-op and passes, so a green run without its submodule says nothing about conformance.
