...
`webpack/comment-length` reports it, so `yarn lint:code` is where you see it — over whole files rather than a diff, and it reads comment tokens, so comment-shaped lines inside a string are not comments. It is switched on for `test/`, `tooling/` and `hot/` only: `lib/` has never been swept, so until the TODO in `eslint.config.mjs` goes, the limit still binds there but nothing reports it for you.
