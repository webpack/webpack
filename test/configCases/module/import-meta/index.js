...
const url = pathToFileURL(
	path.resolve("./test/configCases/module/import-meta/index.js")
).toString();
...
it("should keep import.meta.UNKNOWN_PROPERTY", () => {
	try {
		...
		const {
			UNKNOWN_PROPERTY: UNKNOWN_PROPERTY_2,
			webpack: WEBPACK_PROPERTY_2
		} = import.meta;
...
it("should support destructuring assignment", async () => {
	...
	({ webpack: version2 } = await ({ url: url3 } =
		{ d } =
		{ UNKNOWN_PROPERTY: unknown2 } =
			await import.meta));
