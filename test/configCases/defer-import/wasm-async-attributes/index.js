...
// The exact module bytes `wast-loader` produces for `module.wat`, run through
// Node's platform `WebAssembly.instantiate` as the reference — the test VM
// intercepts `import("./x.wasm")`, and the platform API shares v8's wasm engine.
...
// Side-effect-counter deferral verification does not work for async wasm: a JS
// wrapper importing one becomes top-level-async, which both blocks `it()`
// registration and leaves wasm exports `undefined` after evaluation.
