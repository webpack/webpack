...
//  - two `alterHtml` taps prove the waterfall (the 2nd sees the 1st's output),
//  - `alterHtml` injects a CSP `<meta>` (a common real transform),
...
const hooksPlugin = {
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap("HooksPlugin", (compilation) => {
			...
			hooks.alterHtml.tapPromise("csp", async (html) =>
				html.replace(
					"</head>",
			...
			hooks.alterHtml.tapPromise("marker", async (html, { outputName }) => {
