...
describe("htmlMinify — inline script", () => {
	...
	it("parses a module as a module, so its top level can be mangled", async () => {
		// A classic script's top level is the global scope, where a name may be read
		// from outside; a module's is its own, so terser may rename it.
		expect(
	...
	it("escapes a `</script>` the minified output would otherwise carry", async () => {
		// The element ends at the first `</script>`, so an unescaped one in a string
		// would cut the script short and spill the rest into the document.
	...
	it("escapes it even when the build asked terser not to", async () => {
		// `inline_script` is a preference for a `.js` asset, but a requirement of
		// this embedding — the HTML would not survive without it.
	...
	it("leaves a data block alone — the browser never executes it", async () => {
		// Only an absent / empty type, a JavaScript MIME essence, or `module` is
		// script text. Everything else is data, whatever it looks like.
	...
	it("keeps a body terser cannot parse", async () => {
		// A JS type over something that is not JavaScript is the author's problem,
		// but dropping or mangling it would be ours.
