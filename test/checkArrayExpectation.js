...
const normalizeString = (str, testDirectory) => {
	...
	str = str
		...
		.replace(/\n{3,}/g, "\n\n")
		.trim();
	// Normalize engine-specific `JSON.parse` error text — V8's "Unexpected token …"
	// against JSC/Bun's "JSON Parse error: …" — so JSON type-import snapshots match
	// across runtimes. The acorn "(line:col)" errors are engine-independent.
	str = str.replace(
