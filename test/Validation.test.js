		...
		msg =>
			...
			"Invalid configuration object. Webpack has been initialized using a configuration object that does not match the API schema.
			 - configuration.devtool should be one of these:
			   false | \\"eval\\" | string (should match pattern \\"^(inline-|hidden-|eval-)?(nosources-)?(cheap-(module-)?)?source-map(-debugids)?$\\") | object { type, fallbackModuleFilenameTemplate?, moduleFilenameTemplate?, namespace?, sourceMapFilename? }
			    ...
			    * configuration.devtool should be an object:
			      object { type, fallbackModuleFilenameTemplate?, moduleFilenameTemplate?, namespace?, sourceMapFilename? }"
		`)
	);
