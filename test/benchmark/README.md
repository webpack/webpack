...
Use `createRuntimeBench` instead to measure **the emitted bundle** rather than
the build. The case is compiled once outside the measured region; each iteration
then instantiates the output (runtime bootstrap plus every module factory that
runs at import time) and calls the entry's exported `run`. The entry must export
`run(seed)`, do its work with that opaque seed and return the result, or the
compiler folds the workload away (both are checked and fail the run).

```js
export default {
	name: "e2e/many-modules-interop-runtime",
	benches: [
...
A `development`/`production` pair shows what scope hoisting and minification are
worth at runtime.
