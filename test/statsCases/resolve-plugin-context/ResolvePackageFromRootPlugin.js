...
module.exports = class ResolvePackageFromRootPlugin {
	...
	apply(resolver) {
		resolver.hooks.resolved.tapAsync(
			"ResolvePackageFromRootPlugin",
			(originalResolved, _, callback) => {
				if (
					!nestedNodeModuleRegex.test(originalResolved.path) ||
				...
				resolver.doResolve(
					...
					`resolve issuer of ${originalResolved.path}`,
					{},
					(err, issuer) => {
						if (err) {
							return callback(null, originalResolved);
						...
						const moduleRequestPath = originalResolved.path.replace(
							replaceNodeModuleRegex,
							""
						);
						...
						resolver.doResolve(
							...
							`resolve ${moduleRequestPath} in ${this.rootPath}`,
							{},
							(err, resolvedInParentContext) => {
								if (err) {
									return callback(null, originalResolved);
								}
								...
								const wantedVersion =
									issuer.descriptionFileData.dependencies[packageName];
