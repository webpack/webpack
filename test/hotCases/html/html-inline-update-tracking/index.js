...
it("should report the HTML module + the removed inline data-URI modules when inline <style> / <script> change", (done) => {
	NEXT((err) => {
		...
		// as `data:text/css,…` / `data:text/javascript;base64,…` virtual
		// modules; when the body changes the data-URI identifier changes
		...
		module.hot
			...
			.then((updatedModules) => {
				...
				// The OLD inline-style data URI; the body is URL-encoded so we
				// can match the original CSS text directly.
				expect(
					ids.some(
						(id) =>
							id.startsWith("data:text/css,") &&
							decodeURIComponent(id.slice("data:text/css,".length)).includes(
								"color: red"
							)
					)
				).toBe(true);
