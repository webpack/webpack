...
// Only the scripts this page authored carry a `type`. The runtime appends its own
// `<script src>` for a hot-update chunk and removes it once run, so whether that
// one is still here is a race the assertion must not depend on.
const scriptTypes = () =>
...
it("should patch a removed <script> that never ran and reload for the rest", (done) => {
	...
	// Stand in for the runtime's own hot-update loader, which lands in the head with
	// no `type`. It is in neither authored head, so the delta must leave it alone, and
	// pinning one here keeps the assertions off whether the real one was cleaned up.
