...
it("should DOM-patch document.body and document.title on hot update of an extracted HTML module", (done) => {
	// Initial: simulate the browser having rendered the extracted page. The HMR shim's
	// DOM-patch branch is guarded behind `module.hot.data`, so it does not run on the
	// first evaluation and the document stays whatever the test sets here.
	...
	NEXT(
		require("../../update")(done, true, () => {
			// After the hot update the shim's dispose handler ran on the previous instance, so
			// `module.hot.data` is set on this evaluation and the DOM-patch branch swaps in the
			// new body and title without anyone calling into the importer.
