 ...
 * The special `byLayer` key on object externals isn't a request — when its
 * value is itself an object of `{ [layer]: ExternalItemObject }`, the nested
 * request keys are collected so layered externals are reserved across layers.
 ...
 */
const collectStaticExternalRequests = (externals) => {
	/** @type {Set<string>} */
		...
		for (const [request, value] of Object.entries(item)) {
			if (request === "byLayer") {
				if (value && typeof value === "object") {
					for (const perLayer of Object.values(value)) visit(perLayer);
				}
				continue;
			}
...
class LazyCompilationProxyModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		if (this.active) {
			...
			const requests = collectStaticExternalRequests(options.externals);
