 ...
 *
 * Resolution order matches `ExternalModuleFactoryPlugin.resolveLayer`: for
 * object externals, layer-specific overrides from `byLayer[layer]` are merged
 * on top of the base map before request keys are collected. This means a
 * layer override of `{ react: false }` cancels a base entry of `{ react: ... }`
 * for that layer — neither gets reserved. With a `null` layer the `byLayer`
 * key is skipped entirely.
 *
 * Object entries whose effective value is `false` are also skipped — `false`
 * explicitly disables externalization for that key.
...
const collectStaticExternalRequests = (externals, layer) => {
	...
	const visit = (item) => {
		...
		if (
			layer !== null &&
			byLayer &&
			typeof byLayer === "object" &&
			!Array.isArray(byLayer)
		) {
			const forLayer = /** @type {Record<string, unknown>} */ (byLayer)[layer];
			if (forLayer && typeof forLayer === "object") {
				Object.assign(effective, forLayer);
			}
		}
		for (const [request, value] of Object.entries(effective)) {
