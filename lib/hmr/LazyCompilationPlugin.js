...
class LazyCompilationProxyModule extends Module {
	...
	codeGeneration({ runtimeTemplate, chunkGraph, moduleGraph }) {
		...
		if (block) {
			...
			// Get external dependencies if any
			/** @type {Array<string | number>} */
			const externalModuleIds = [];
			const hasExternals =
				this.originalModule && this.originalModule.dependencies
					? this.originalModule.dependencies.some(dep => {
							if (dep.module && moduleGraph.getModule(dep)) {
								const moduleId = chunkGraph.getModuleId(dep.module);
								if (moduleId !== null) {
			...
			if (hasExternals) {
				...
				source = Template.asString([
					...
					`${RuntimeGlobals.ensureChunkHandlers}.ensureExternals = ${RuntimeGlobals.ensureChunkHandlers}.ensureExternals || (function(promises) { return Promise.all(promises); });`,
					`${RuntimeGlobals.ensureChunkHandlers}.ensureExternals([${externalModuleIds.map(id => `${RuntimeGlobals.ensureChunk}(${JSON.stringify(id)})`).join(", ")}]).then(function() {`,
					Template.indent([
						"if (module.hot) {",
						Template.indent([
							"module.hot.accept();",
