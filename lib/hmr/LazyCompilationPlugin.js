...
const { JAVASCRIPT_TYPES } = require("../module/ModuleSourceTypeConstants");
...
class LazyCompilationProxyModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		if (this.active) {
			...
			// Reserve the statically declared externals on the inactive proxy, so the
			// entry chunk's library wrapper already exposes their closure identifiers
			// and they resolve rather than throw once the proxy activates.
							...
							const isReferringToDynamicImport = originModule.blocks.some(
								(block) =>
									block.dependencies.some(
										(dep) =>
											dep.type === "import()" &&
											/** @type {ESMImportDependency} */ (dep).request ===
												hmrDep.request
									)
							);
