 ...
 * Object entries whose value is `false` are skipped — `false` explicitly
 * disables externalization for that key. The special `byLayer` key holds
 * layer-specific external maps; if `layer` is supplied, the matching
 * `byLayer[layer]` map is also collected, otherwise `byLayer` is skipped
 * entirely (its entries would only apply to modules in other layers).
