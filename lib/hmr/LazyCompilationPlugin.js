 ...
 * (e.g. `__WEBPACK_EXTERNAL_MODULE_react__`). External modules added via HMR
 * land in hot update chunks that live outside that closure, so their factory
 * bodies cannot resolve the closure identifier and throw at runtime. The fix
 * is to reload on the first activation so the freshly initialized chunk has
 * the externals baked into its wrapper.
 * @param {import("../../declarations/WebpackOptions").LibraryOptions | undefined} library normalized library option
 * @returns {boolean} true when activating a lazy proxy needs a full reload
 */
const libraryRequiresReloadOnActivation = (library) => {
	if (!library || !library.type) return false;
	return CLOSURE_LIBRARY_TYPES.has(library.type);
};
...
class LazyCompilationProxyModule extends Module {
	 ...
	 */
	codeGeneration({ runtimeTemplate, chunkGraph, moduleGraph, compilation }) {
		/** @type {Sources} */
		...
		const needsReloadOnActivation =
			compilation !== undefined &&
			libraryRequiresReloadOnActivation(compilation.options.output.library);
		/** @type {string} */
		let source;
		if (block) {
			const dep = block.dependencies[0];
			const module = /** @type {Module} */ (moduleGraph.getModule(dep));
			const reloadOnActivation = needsReloadOnActivation
				? Template.asString([
						'if (module.hot && module.hot.data && module.hot.data.wasInactive && typeof window !== "undefined" && window.location && typeof window.location.reload === "function") {',
						Template.indent(["window.location.reload();", "return;"]),
						"}"
					])
				: "";
			...
			const inactiveDispose = needsReloadOnActivation
				? "module.hot.dispose(function(data) { data.resolveSelf = resolveSelf; data.wasInactive = true; dispose(data); });"
