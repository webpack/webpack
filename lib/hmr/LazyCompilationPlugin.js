...
class LazyCompilationPlugin {
	 ...
	 * @param {(function(string): boolean)} options.skipModule additional filter for lazy compiled entrypoint modules
	 */
	constructor({ backend, client, entries, imports, skipModule }) {
				...
				normalModuleFactory.hooks.module.tap(
					...
					(originalModule, createData, resolveData) => {
						if (
							...
							!(
								this.skipModule &&
								this.skipModule(originalModule.nameForCondition())
							)
						) {
