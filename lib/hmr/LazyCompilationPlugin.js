 ...
 * The special `byLayer` key on object externals is also skipped — it holds
 * layer-specific external maps, not a request name.
	...
	const visit = (item) => {
		if (typeof item === "string") {
			...
			for (const request of Object.keys(item)) {
				if (request === "byLayer") continue;
				requests.add(request);
			}
		}
		// RegExp and function externals can't be enumerated statically.
	};
