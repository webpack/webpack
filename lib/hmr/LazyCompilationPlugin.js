 ...
 * @returns {boolean} true when the library wrapper passes externals as closure arguments
 */
const isClosureLibrary = (library) => {
 ...
 * The special `byLayer` key on object externals holds layer-specific external
 * maps rather than a request name. If `layer` is supplied, the matching
 * `byLayer[layer]` map is also collected; otherwise `byLayer` is skipped
 * entirely (its entries would only apply to modules in other layers and would
 * fail to resolve as ordinary requests).
 * @param {import("../../declarations/WebpackOptions").Externals | undefined} externals normalized externals option
 * @param {string | null} layer issuer layer for which to resolve `byLayer`, or `null` for no layer
		...
		for (const [request, value] of Object.entries(item)) {
			if (request === "byLayer") {
				if (
					...
					const forLayer = /** @type {Record<string, unknown>} */ (value)[
						layer
					];
					if (forLayer) {
						visit(
							/** @type {import("../../declarations/WebpackOptions").ExternalItem} */ (
								forLayer
							)
						);
					}
...
class LazyCompilationProxyModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		if (this.active) {
			...
			this.addBlock(block);
		} else if (isClosureLibrary(compilation.options.output.library)) {
