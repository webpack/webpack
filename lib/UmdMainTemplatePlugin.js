...
class UmdMainTemplatePlugin {
	...
	apply(compilation) {
		...
		compilation.templatesPlugin("render-with-entry", function (source, chunk, hash) {
			...
			var defaultImports = ['require', 'exports'];
			...
			function externalsDepsArray(modules) {
				const moduleNames = replaceKeys(defaultImports.map(m=>`"${m}"`)
					.concat(modules
			...
			function externalsRequireArray(type) {
				return replaceKeys(defaultImports
			...
			function externalsArguments(modules) {
				return defaultImports.concat(modules.map(m => Template.toIdentifier(`__WEBPACK_EXTERNAL_MODULE_${m.id}__`))).join(", ");
			}
