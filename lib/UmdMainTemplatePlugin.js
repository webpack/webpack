...
const { chunksIterator } = require("./util/templatePlugin");
...
const normalizeNameInfo = (context, name) => {
	if (typeof name === "object" && !Array.isArray(name)) {
		context.name = name.root || name.amd || name.commonjs;
		context.names = name;
	} else {
		context.name = name;
		context.names = {
...
const resolveNamesWithChunk = (context, chunk) => {
	let ret = {};
	if (context.nameFunction) {
		const name = context.nameFunction({ chunk });
		normalizeNameInfo(ret, name);
		return ret;
	}
	ret = {
		name: context.name,
		names: Object.assign({}, context.names)
	};
	const evalName = (obj, name) => {
		if (isFunction(obj[name])) {
			obj[name] = obj[name]({ chunk });
		}
	};
	if (isFunction(ret.name)) {
		evalName(ret, "name");
	}
	Object.keys(ret.names).forEach(type => {
		if (isFunction(ret.names[type])) {
			evalName(ret.names, type);
		}
	});
	return ret;
};

const updateHashPath = (compilation, context, paths) => {
	chunksIterator(compilation, chunk => {
		const { names } = resolveNamesWithChunk(context, chunk);
		if (names.root) paths = paths.concat(names.root);
		if (names.amd) paths = paths.concat(names.amd);
		if (names.commonjs) paths = paths.concat(names.commonjs);
	});
};

/** @typedef {string | string[] | LibraryCustomUmdObject | Function} UmdMainTemplatePluginName */
...
class UmdMainTemplatePlugin {
	...
	apply(compilation) {
		...
		const replaceKeysFactory = (hash, chunk) => {
			return str => {
		...
		const libraryNameFactory = replaceKeys => {
			return library => {
		...
		const onRenderWithEntry = (source, chunk, hash) => {
			...
			const { names } = resolveNamesWithChunk(this, chunk);
			const replaceKeys = replaceKeysFactory(hash, chunk);
			...
			const libraryName = libraryNameFactory(replaceKeys);
			...
			return new ConcatSource(
				new OriginalSource(
					"(function webpackUniversalModuleDefinition(root, factory) {\n" +
						...
						(requiredExternals.length > 0
							? names.amd && this.namedDefine === true
								? "		define(" +
		...
		mainTemplate.hooks.globalHashPaths.tap("UmdMainTemplatePlugin", paths => {
			updateHashPath(compilation, this, paths);
			return paths;
		});
		...
		mainTemplate.hooks.hashForChunk.tap(
			...
			(hash, chunk) => {
				...
				const libraryName = libraryNameFactory(replaceKeys);
				hash.update("umd");
				hash.update(`${libraryName(names.root)}`);
				hash.update(`${libraryName(names.amd)}`);
				hash.update(`${libraryName(names.commonjs)}`);
			}
		);
	}
}
