...
class UmdMainTemplatePlugin {
	...
	apply(compilation) {
		...
		compilation.templatesPlugin("render-with-entry", function (source, chunk, hash) {
			...
			var defaultImports = [{ in: 'require', out: 'parentRequire' }, 'exports'];

			var defaultImportsIn = defaultImports.map(m => typeof m === 'object' ? m.in : m);
			var defaultImportsOut = defaultImports.map(m => typeof m === 'object' ? m.out : m);
			...
			function externalsDepsArray(modules) {
				const moduleNames = replaceKeys(defaultImportsIn.map(m => `"${m}"`)
					.concat(modules
						.map(m => JSON.stringify(typeof m.request === "object" ? m.request.amd : m.request))).join(", "));
				return `[${moduleNames}]`;
			}
			...
			function externalsRequireArray(type) {
				return replaceKeys(defaultImportsIn
					.concat(externals.map(m => {
						let expr;
						let request = m.request;
			...
			function externalsArguments(modules) {
				return defaultImportsOut.concat(modules.map(m => Template.toIdentifier(`__WEBPACK_EXTERNAL_MODULE_${m.id}__`))).join(", ");
			}
			...
			return new ConcatSource(new OriginalSource(
				...
				(this.name ?
					...
					(externals.length > 0 ?
						...
						"		var a = factory(undefined, {});\n"
					) +
					"		for(var i in a) (typeof exports === 'object' ? exports : root)[i] = a[i];\n" +
					"	}\n"
				) +
				"})(this, function(" + externalsArguments(externals) + ") {\nvar result= ", "webpack/universalModuleDefinition"), source, ";\n" +
				"\tfor(var prop in result){\n\t\t exports[prop] = result[prop];\n\t}\n\treturn exports;\n})");
		}.bind(this));
