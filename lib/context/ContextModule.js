...
const {
	ImportPhase,
	ImportPhaseUtils,
	NO_MODULE_SOURCE_SUFFIX
} = require("../dependencies/ImportPhase");
 ...
 */
/** @import Chunk, { ChunkId, ChunkName } from "../graph/Chunk" */
/** @import ChunkGraph, { ModuleId } from "../graph/ChunkGraph" */
/** @import { RawChunkGroupOptions } from "../graph/ChunkGroup" */
/** @import Compilation, { FileSystemDependencies } from "../Compilation" */
/** @import Dependency, { RawReferencedExports } from "../graph/Dependency" */
/** @import { Snapshot } from "../fs/FileSystemInfo" */
/**
 * @import {
 * 	BuildCallback,
 * 	BuildInfo,
...
class ContextModule extends Module {
	...
	getReturnModuleObjectSource(
		...
		if (asyncDeps) {
			...
			// The bound third argument absorbs the promise's resolved value, which
			// would otherwise land in the export-names parameter.
			return `${asyncDeps} ? ${asyncDeps}.length ? ${RuntimeGlobals.deferredModuleAsyncTransitiveDependencies}(${asyncDeps}).then(${RuntimeGlobals.makeDeferredNamespaceObject}.bind(${RuntimeGlobals.require}, id, ${type} ^ 1, undefined)) : ${RuntimeGlobals.makeDeferredNamespaceObject}(id, ${type} ^ 1 | 16) : ${source}`;
		}
		return source;
	}
...
module.exports = webpackEmptyAsyncContext;`;
	...
	getSourceString(asyncMode, phase, context) {
		const source = this.options.patterns
			? this.getGlobSourceString(asyncMode, phase, context)
			: this.getContextSourceString(asyncMode, phase, context);
		if (!ImportPhaseUtils.isSource(phase)) return source;
		return source + this.getSourcePhaseGuard(context);
	}

	/**
	 * Adapts a context to the source phase, which binds the module source object
	 * a member carries — and rejects for one that carries none, as the spec's
	 * GetModuleSource does.
	 * @param {CodeGenerationContext} context context info
	 * @returns {string} source code appended to the context
	 */
	getSourcePhaseGuard({ chunkGraph, runtimeTemplate }) {
		/** @type {Record<string, 1>} */
		const noModuleSource = Object.create(null);
		let anyMissing = false;
		for (const dependency of this.getAllDependencies()) {
			const module = chunkGraph.moduleGraph.getModule(dependency);
			if (module && module.hasModuleSource()) continue;
			noModuleSource[dependency.userRequest] = 1;
			anyMissing = true;
		}
		const cst = runtimeTemplate.renderConst();
		// Coercing the request is observable and may itself throw, so it happens
		// inside the promise the spec rejects rather than at the call.
		const body = anyMissing
			? `Promise.resolve().then(${runtimeTemplate.basicFunction("", [
					`if(${RuntimeGlobals.hasOwnProperty}(webpackNoModuleSource, req)) {`,
					Template.indent([
						`throw new SyntaxError("Module '" + req + "'${NO_MODULE_SOURCE_SUFFIX}");`
					]),
					"}",
					"return webpackSourcePhaseInner(req);"
				])})`
			: "webpackSourcePhaseInner(req)";
		const noModuleSourceInit = anyMissing
			? `\n${cst} webpackNoModuleSource = ${JSON.stringify(noModuleSource, null, "\t")};`
			: "";
		const unwrap = runtimeTemplate.returningFunction('m["default"]', "m");
		return `
${cst} webpackSourcePhaseInner = module.exports;${noModuleSourceInit}
function webpackSourcePhaseContext(req) {
	return ${body}.then(${unwrap});
}
webpackSourcePhaseContext.keys = webpackSourcePhaseInner.keys;
webpackSourcePhaseContext.resolve = webpackSourcePhaseInner.resolve;
webpackSourcePhaseContext.id = webpackSourcePhaseInner.id;
module.exports = webpackSourcePhaseContext;
`;
	}

	/**
	 * Every element dependency of this context, whichever mode put it in a block.
	 * @returns {ContextElementDependency[]} dependencies
	 */
	getAllDependencies() {
		const all =
			/** @type {ContextElementDependency[]} */
		...
		for (const block of this.blocks) {
			for (const dependency of block.dependencies) {
				all.push(/** @type {ContextElementDependency} */ (dependency));
			}
		}
		return all;
	}
	...
	codeGeneration(context) {
		...
		if (allDeps.length > 0) {
			...
			if (
				...
				set.add(RuntimeGlobals.hasSynthesizedDeferredNamespace);
			}
		}
		return {
			sources,
...
makeSerializable(ContextModule, "webpack/lib/context/ContextModule");

// TODO in the next major release: remove
// Restores caches written before this moved into lib/context/
registerLegacyRequest(ContextModule, "webpack/lib/ContextModule");
