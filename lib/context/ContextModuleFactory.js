 ...
 */
/** @import ResolverFactory from "../resolve/ResolverFactory" */
/** @import ContextDependency from "../dependencies/context/ContextDependency" */
 ...
 */
/** @import { IStats, InputFileSystem } from "../fs/fs" */
						...
						if (!exclude || !exclude.test(subResource)) {
							fs.stat(subResource, (err, _stat) => {
								...
								if (stat.isDirectory()) {
									...
									// Collected for one batched `alternativeRequests` call per
									// directory: calling the hook per file would pay its overhead
									// across the whole context, the bulk of a rebuild.
									fileObjs.push({
										context: ctx,
