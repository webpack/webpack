...
class StrictModeChecksError extends WebpackError {
	/**
	...
	constructor(message, loc) {
		super(message);
		this.name = "StrictModeChecksError";
		this.loc = loc;
	}
}

class StrictModeChecksWarning extends WebpackError {
	/**
	 * @param {string} message message
	 * @param {DependencyLocation | undefined} loc loc
	 */
	constructor(message, loc) {
		super(message);
		this.name = "StrictModeChecksWarning";
		this.loc = loc;
	}
}

const PLUGIN_NAME = "StrictModeChecksPlugin";

class StrictModeChecksPlugin {
	/**
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser, parserOptions) => {
					...
					const mode =
						parserOptions &&
						(parserOptions.strictModeChecks ||
							parserOptions.strictCompatibility);
					if (!mode) return;
					...
					parser.hooks.program.tap(PLUGIN_NAME, (ast) => {
						...
						const reportError = (msg, loc) => {
							if (mode === "error") {
								mod.addError(new StrictModeChecksError(msg, loc));
							} else {
								mod.addWarning(new StrictModeChecksWarning(msg, loc));
							}
						};
						/**
						 * @param {string} msg message
							...
							switch (node.type) {
								...
								case "AssignmentExpression":
									...
									if (node.left && node.left.type === "Identifier") {
										...
										// Only check if the variable is defined (otherwise already reported as undeclared)
										if (
											parser.isVariableDefined(n) &&
											(n === "undefined" || n === "Infinity" || n === "NaN")
										) {
											reportError(
...
module.exports = StrictModeChecksPlugin;
