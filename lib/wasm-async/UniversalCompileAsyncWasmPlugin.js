		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			const generateBeforeLoadBinaryCode = (path) =>
				Template.asString([
					...
					"var useFetch = !isDeno && (typeof document !== 'undefined' || typeof self !== 'undefined');",
					`var wasmUrl = ${path};`
				]);
			/**
			 * Generates the runtime expression that fetches the binary in browsers,
			 * reads it via the Deno API in Deno, or from the filesystem in Node.js.
			...
			const generateLoadBinaryCode = () =>
				Template.asString([
					...
					Template.indent([": isDeno"]),
					Template.indent([
						`? Deno.readFile(new URL(wasmUrl, ${compilation.outputOptions.importMetaName}.url)).then(function (buffer) { return { arrayBuffer() { return buffer; } }; })`
					]),
					Template.indent([
