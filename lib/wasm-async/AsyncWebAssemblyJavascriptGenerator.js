...
class AsyncWebAssemblyJavascriptGenerator extends Generator {
	...
	getConcatenationBailoutReason(module, context) {
		// Source-phase imports resolve to `WebAssembly.Module` via `compileWasm`
		// and use a different async pattern (`define`-style getters) that
		// doesn't trivially inline into a concatenated body.
		if (/** @type {AsyncWasmModule} */ (module).phase === "source") {
			return "Module Concatenation is not implemented for source phase wasm imports";
		}
		return undefined;
	}
	...
	generate(module, generateContext) {
		...
		if (concatenationScope) {
			// The outer ConcatenatedModule emits the `__webpack_require__.a`
			// wrapper and exposes `__webpack_handle_async_dependencies__` in
			// scope. Inline the wasm instantiation: build a fresh namespace
			// object, optionally route async JS imports through the outer
			// handle, then await the wasm to populate the namespace.
			const namespaceVar = ConcatenationScope.NAMESPACE_OBJECT_EXPORT;
			concatenationScope.registerNamespaceExport(namespaceVar);

			const concatInstantiateCall = `${RuntimeGlobals.instantiateWasm}(${namespaceVar}, ${JSON.stringify(
			...
			const body =
				promises.length > 0
					? Template.asString([
							...
							`var [${promises.join(", ")}] = __webpack_async_dependencies__.then ? (await __webpack_async_dependencies__)() : __webpack_async_dependencies__;`,
							`${importsCompatCode}await ${concatInstantiateCall};`
						])
					: Template.asString([
							`var ${namespaceVar} = {};`,
							importsCode,
