...
class AsyncWebAssemblyJavascriptGenerator extends Generator {
	 ...
	 * @param {OutputOptions["wasmImportObject"]} wasmImportObject Controls the memory configuration for the `importObj` parameter in `WebAssembly.instantiate`
	 */
	constructor(filenameTemplate, wasmImportObject) {
		super();
	...
	generate(module, generateContext) {
		...
		if (this.wasmImportObject !== "plain-object") {
			for (const dep of module.presentationalDependencies) {
				if (
					dep instanceof WebAssemblyImportDependency &&
					dep.description.type === "Memory"
				) {
					importObjRequestItems.push(
						Template.asString([
							`${JSON.stringify(dep.request)}: {`,
							Template.indent(
								[
									// @ts-expect-error
									`"${dep.name}": new WebAssembly.Memory({ initial: ${dep.description.limits.min}, maximum: ${dep.description.limits.max || "65536"}, shared: ${this.wasmImportObject === "SharedArrayBuffer" ? "true" : "false"} })`
								].join(",\n")
							),
							"}"
						])
