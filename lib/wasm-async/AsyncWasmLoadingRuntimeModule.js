...
/**
 * Callbacks used to customize how async WebAssembly binaries are requested and
 * instantiated for a particular environment.
...
/**
 * Runtime module that downloads and instantiates asynchronous WebAssembly
 * modules, optionally using `WebAssembly.instantiateStreaming`.
 */
class AsyncWasmLoadingRuntimeModule extends RuntimeModule {
	/**
	 * Stores the environment-specific loading hooks used to fetch and
	 * instantiate emitted WebAssembly binaries at runtime.
	...
	/**
	 * Generates the runtime helper that loads a WebAssembly binary, falls back
	 * to `WebAssembly.instantiate` when necessary, and copies the resulting
	 * exports into the provided runtime export object.
	...
	generate() {
		...
		const getStreaming = () => {
			/**
			 * Joins generated code fragments into a single emitted expression.
