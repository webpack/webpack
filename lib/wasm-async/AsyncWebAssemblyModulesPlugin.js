			...
			(compilation, { normalModuleFactory }) => {
				...
				normalModuleFactory.hooks.createGenerator
					...
					.tap(PLUGIN_NAME, () => {
						...
						const jsGenerator = new AsyncWebAssemblyJavascriptGenerator();
						const generator = Generator.byType({
							javascript: jsGenerator,
							webassembly: new AsyncWebAssemblyGenerator(this.options)
						});
						// Concatenation only embeds the JS-typed portion of a
						// module, so route the bailout question to the JS
						// generator — otherwise `ByTypeGenerator`'s inherited
						// default ("Module Concatenation is not implemented for
						// ByTypeGenerator") would block async wasm even after
						// `AsyncWebAssemblyJavascriptGenerator` opts in.
						generator.getConcatenationBailoutReason = (module, context) =>
							jsGenerator.getConcatenationBailoutReason(module, context);
						return generator;
					});
