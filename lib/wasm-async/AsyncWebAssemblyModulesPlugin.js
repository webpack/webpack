		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				normalModuleFactory.hooks.afterResolve.tap(
					PLUGIN_NAME,
					(resolveData) => {
						const { dependencies, createData } = resolveData;
						if (
							!createData ||
							createData.type !== WEBASSEMBLY_MODULE_TYPE_ASYNC
						) {
							return;
						}
						...
						if (
							dep &&
							"phase" in dep &&
							ImportPhaseUtils.isSource(
								/** @type {import("../dependencies/ImportPhase").ImportPhaseType} */ (
									dep.phase
								)
							)
						) {
							createData.sourcePhase = true;
						}
					}
				);
