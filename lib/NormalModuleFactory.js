...
class NormalModuleFactory extends ModuleFactory {
	...
	resolveResource(
		...
		if (resolver.options.alias && unresolvedResource) {
			const target = resolver.options.alias.find(alias => {
				if (!/^[^a-zA-Z0-9]+$/.test(alias.name)) return false;
				return unresolvedResource.startsWith(alias.name);
			});
			if (target) {
				const aliasPath = path.join(
					target.alias.toString(),
					unresolvedResource.slice(target.name.length)
				);
				if (fs.existsSync(aliasPath)) {
					resolver.options.alias.push({
						alias: aliasPath,
						name: unresolvedResource,
						onlyModule: target.onlyModule
					});
				}
			}
		}
