...
/**
 * Adds every item of `otherSet` into `set`, supporting Set, LazySet and the
 * write-only dependency sets carried on a resolve context.
 * @template T
 * @param {LazySet<T> | { add: (item: T) => void }} set set to add items to
 * @param {LazySet<T> | Iterable<T>} otherSet set to add items from
 * @returns {void}
 */
...
class NormalModuleFactory extends ModuleFactory {
	...
	constructor({
		...
		/** @type {Map<string, { loader: string, query: string | undefined, type: string | undefined, fileDependencies: LazySet<string>, contextDependencies: LazySet<string>, missingDependencies: LazySet<string> }>} */
...
If changing the source code is not an option there is also a resolve options called 'preferRelative' which tries to resolve these kind of requests in the current directory too.`
	...
	resolveRequestArray(
		...
		asyncLib.map(
			 ...
			 */
			(item, callback) => {
				// loaders resolve identically for a given (context, request); memoize the
				// result and the resolve's deps so repeated loaders skip the real resolve.
				// yield-based resolves can produce multiple results — never cache those.
				const canCache = typeof resolveContext.yield !== "function";
					...
					if (cached !== undefined) {
						if (resolveContext.fileDependencies) {
							addAllToSet(
								...
								cached.fileDependencies
							);
							addAllToSet(
								/** @type {NonNullable<ResolveContext["contextDependencies"]>} */
								(resolveContext.contextDependencies),
								cached.contextDependencies
							);
							addAllToSet(
								/** @type {NonNullable<ResolveContext["missingDependencies"]>} */
				...
				// capture the resolve's deps separately so a future hit can replay them
				const captureContext = canCache
					? {
							...resolveContext,
				...
				resolver.resolve(
					...
					(err, result, resolveRequest) => {
						if (canCache && resolveContext.fileDependencies) {
							addAllToSet(
								resolveContext.fileDependencies,
								/** @type {LazySet<string>} */
								(captureContext.fileDependencies)
							);
							addAllToSet(
								/** @type {NonNullable<ResolveContext["contextDependencies"]>} */
								(resolveContext.contextDependencies),
								/** @type {LazySet<string>} */
								(captureContext.contextDependencies)
							);
							addAllToSet(
								/** @type {NonNullable<ResolveContext["missingDependencies"]>} */
								(resolveContext.missingDependencies),
								/** @type {LazySet<string>} */
								(captureContext.missingDependencies)
							);
						}
						if (
							err &&
