...
${hints.join("\n\n")}`;
	...
	_resolveResourceErrorHints(
		...
		const suggestClosestEntry = (callback) => {
			...
			if (ABSOLUTE_PATH_REGEXP.test(requestPath)) {
				absoluteRequests = [requestPath];
			} else if (RELATIVE_REQUEST_REGEXP.test(requestPath)) {
				absoluteRequests = [join(this.fs, context, requestPath)];
			} else {
				absoluteRequests = moduleLookupDirectories(
					this.fs,
					context,
					resolver.options.modules
				).map((directory) => join(this.fs, directory, requestPath));
			}
			...
			const suggest = (
				...
				if (directoryCorrected && entries.includes(requestedName)) {
					const suggestion = prefix + requestedName;
					return callback(`Did you mean '${suggestion}'?
'${suggestion}' exists and differs from the request only in the casing of a directory name. Case-sensitive filesystems (most Linux ones) fail to resolve it, even when the same build succeeds on a case-insensitive one.`);
				}
				...
				const suggestion = prefix + closest.entry;
				callback(
					closest.distance === 0
