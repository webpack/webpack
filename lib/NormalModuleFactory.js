	...
	this.plugin("resolver", function() {
		return function(data, callback) {
			...
			async.parallel([
				...
				if(noAutoLoaders) {
					async.parallel([
						this.resolveRequestArray.bind(this, context, noPostAutoLoaders ? [] : this.postLoaders.match(resource, context), this.resolvers.loader),
						this.resolveRequestArray.bind(this, context, this.preLoaders.match(resource, context), this.resolvers.loader)
					], function(err, results) {
						if(err) return callback(err);
					...
					async.parallel([
						this.resolveRequestArray.bind(this, context, noPostAutoLoaders ? [] : this.postLoaders.match(resource, context), this.resolvers.loader),
						this.resolveRequestArray.bind(this, context, this.loaders.match(resource, context), this.resolvers.loader),
						this.resolveRequestArray.bind(this, context, this.preLoaders.match(resource, context), this.resolvers.loader)
					], function(err, results) {
						if(err) return callback(err);
