...
/** @typedef {new (createData: CreateData, resolveData: ResolveData) => Module} ModuleClassConstructor */
...
class NormalModuleFactory extends ModuleFactory {
	...
	constructor({
		 ...
		 * @type {Map<string, ModuleClassConstructor>}
		 */
						...
						this.hooks.createModule.callAsync(
							...
							(err, createdModule) => {
								if (!createdModule) {
									...
									// TODO webpack 6 require a module class for every type and remove the NormalModule fallback
									createdModule = this.hooks.createModuleClass
										.for(createData.settings.type)
										.call(createData, resolveData);

									if (!createdModule) {
										const ModuleClass = this.moduleClasses.get(
											/** @type {string} */ (createData.settings.type)
										);
										createdModule = ModuleClass
											? new ModuleClass(createData, resolveData)
											: /** @type {Module} */ (new NormalModule(createData));
									}
								}
