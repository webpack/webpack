...
class NormalModuleFactory extends ModuleFactory {
	...
	getParser(type, parserOptions = EMPTY_RESOLVE_OPTIONS) {
		if (!this.parserCache.has(type)) {
			this.parserCache.set(type, new WeakMap());
		}

		const cache = this.parserCache.get(type);

		if (cache.has(parserOptions)) {
			return cache.get(parserOptions);
		}

		return cache
			.set(parserOptions, this.createParser(type, parserOptions))
			.get(parserOptions);
	}
	...
	getGenerator(type, generatorOptions = EMPTY_RESOLVE_OPTIONS) {
		if (!this.generatorCache.has(type)) {
			this.generatorCache.set(type, new WeakMap());
		}

		const cache = this.generatorCache.get(type);

		if (cache.has(generatorOptions)) {
			return cache.get(generatorOptions);
		}

		return cache
			.set(generatorOptions, this.createGenerator(type, generatorOptions))
