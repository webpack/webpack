...
class NormalModuleFactory extends ModuleFactory {
	...
	constructor({
		...
		// canonical install per "name@version"; redirects non-hoisted duplicate
		// installs with identical content to a single module instance
		/** @type {Map<string, { root: string, descriptionFileData: object }>} */
		this._packageDedupeRoots = new Map();
		...
		this.hooks.afterResolve.tapAsync(
			{
				name: "NormalModuleFactory",
				stage: 100
			},
			(resolveData, callback) => {
				this._deduplicatePackageCopy(
					/** @type {CreateData} */ (resolveData.createData),
					callback
				);
			}
		);
	...
	/**
	 * Redirects a resolved resource to a single canonical copy when the same
	 * package "name@version" is installed at multiple paths (non-hoisted
	 * duplicates) with byte-identical content, so singleton packages stay a
	 * single module instance without any user configuration.
	 * @param {CreateData} createData create data, mutated in place on a redirect
	 * @param {(err?: (Error | null)) => void} callback callback
	 * @returns {void}
	 */
	_deduplicatePackageCopy(createData, callback) {
		const data = createData.resourceResolveData;
		// only plain package files: skip loaders `!=!` overrides, schemes and queries
		if (
			createData.matchResource !== undefined ||
			!data ||
			data.query ||
			data.fragment ||
			!data.descriptionFileRoot ||
			!data.descriptionFileData
		) {
			return callback();
		}
		const root = data.descriptionFileRoot;
		const descriptionFileData = data.descriptionFileData;
		...
		const key = `${/** @type {string} */ (descriptionFileData.name)}@${/** @type {string} */ (descriptionFileData.version)}`;
		const canonical = this._packageDedupeRoots.get(key);
		if (canonical === undefined) {
			this._packageDedupeRoots.set(key, { root, descriptionFileData });
			return callback();
		}
		if (canonical.root === root) return callback();
		// require identical package.json too, so the surviving module keeps
		// consistent metadata (e.g. `sideEffects`) whichever copy wins the merge
		if (
			JSON.stringify(canonical.descriptionFileData) !==
			JSON.stringify(descriptionFileData)
		) {
			return callback();
		}
		const canonicalResource = canonical.root + resource.slice(root.length);
		if (canonicalResource === resource) return callback();
		// merge only byte-identical copies; a patched/forked copy stays separate
		this.fs.readFile(resource, (err1, buf1) => {
			if (err1 || !buf1) return callback();
			this.fs.readFile(canonicalResource, (err2, buf2) => {
				if (
					err2 ||
					!buf2 ||
					!(/** @type {Buffer} */ (buf1)).equals(/** @type {Buffer} */ (buf2))
				) {
					return callback();
				}
				createData.resource = canonicalResource;
				createData.context = getContext(canonicalResource);
				createData.request = stringifyLoadersAndResource(
					createData.loaders,
					canonicalResource
				);
				createData.userRequest = createData.userRequest.replace(
					resource,
					canonicalResource
				);
				callback();
