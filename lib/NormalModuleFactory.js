...
function iterationMapOfCallback(array, fn) {
...
function NormalModuleFactory(context, resolvers, options) {
	...
	this.plugin("resolver", function() {
		...
		return function(data, callback) {
			...
			var noAutoLoaders = regexp.noAutoLoaders.test(request);
			var noPrePostAutoLoaders = regexp.noPrePostAutoLoaders.test(request);
			var noPostAutoLoaders = regexp.noPostAutoLoaders.test(request);
			var elements = request.replace(regexp.request1, "").replace(regexp.request2, "!").split("!");
			var resource = elements.pop();
			elements = iterationMapOfCallback(elements, identToLoaderRequest);
			...
			async.parallel([
				...
				try {
					for(var indexLoader = 0; indexLoader < loaders.length; indexLoader++) {
						var loaderItem = loaders[indexLoader];

						if(typeof loaderItem.options === "string" && regexp.translateOption.test(loaderItem.options)) {
							loaderItem.options = _this.ruleSet.findOptionsByIdent(loaderItem.options.substr(1));
						}
					}
				} catch(e) {
					return callback(e);
				...
				var userRequest = iterationMapOfCallback(loaders, loaderToIdent).concat([resource]).join("!");
				...
				function onDoneResolving() {
					callback(null, {
						context: context,
						request: iterationMapOfCallback(loaders, loaderToIdent).concat([resource]).join("!"),
...
NormalModuleFactory.prototype.resolveRequestArray = function resolveRequestArray(contextInfo, context, array, resolver, callback) {
	...
	async.map(array, function(item, callback) {
		resolver.resolve(contextInfo, context, item.loader, function(err, result) {
			if(err && regexp.errorModule.test(item.loader) && !regexp.isLoader.test(item.loader)) {
