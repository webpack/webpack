...
class Cache {
	...
	constructor() {
		this.hooks = {
			/** @type {AsyncSeriesBailHook<[string, Etag | null, GotHandler<unknown>[]], Data>} */
	...
	get(identifier, etag, callback) {
		/** @type {GotHandler<T>[]} */
		const gotHandlers = [];
		this.hooks.get.callAsync(
			identifier,
			etag,
			/** @type {GotHandler<unknown>[]} */ (gotHandlers),
			(err, _result) => {
				if (err) {
				...
				const result =
					_result === null ? undefined : /** @type {T | undefined} */ (_result);
				if (gotHandlers.length > 1) {
					const innerCallback = needCalls(gotHandlers.length, () =>
						callback(null, result)
					);
					for (const gotHandler of gotHandlers) {
						gotHandler(/** @type {T} */ (result), innerCallback);
					}
				} else if (gotHandlers.length === 1) {
					gotHandlers[0](/** @type {T} */ (result), () =>
						callback(null, result)
					);
				} else {
					callback(null, result);
