...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		if (typeof options.mode !== "string") new WarnNoModeSetPlugin().apply(compiler);

		new EnsureChunkConditionsPlugin().apply(compiler);
		if (options.optimization.removeAvailableModules) new RemoveParentModulesPlugin().apply(compiler);
		...
		if (options.optimization.flagIncludedChunks) new FlagIncludedChunksPlugin().apply(compiler);
		if (options.optimization.occurrenceOrder) new OccurrenceOrderPlugin(true).apply(compiler);
		...
		if (options.optimization.usedExports) new FlagDependencyUsagePlugin().apply(compiler);
		...
		if (options.optimization.runtimeChunk) new RuntimeChunkPlugin(options.optimization.runtimeChunk).apply(compiler);
		if (options.optimization.noEmitOnErrors) new NoEmitOnErrorsPlugin().apply(compiler);
		if (options.optimization.namedModules) new NamedModulesPlugin().apply(compiler);
		if (options.optimization.namedChunks) new NamedChunksPlugin().apply(compiler);
