...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		switch (options.stats) {
			case "verbose":
				new StatsPlugin.VerboseStatsPreset().apply(compiler);
				break;
			case "detailed":
				new StatsPlugin.DetailedStatsPreset().apply(compiler);
				break;
			case "minimal":
				new StatsPlugin.MinimalStatsPreset().apply(compiler);
				break;
			case "errors-only":
				new StatsPlugin.ErrorOnlyStatsPreset().apply(compiler);
				break;
			case "none":
				new StatsPlugin.NoStatsPreset().apply(compiler);
				break;
			default:
				new StatsPlugin.AssetsStatsPlugin().apply(compiler);
				new StatsPlugin.ModuleStatsPlugin().apply(compiler);
				new StatsPlugin.ChunksStatsPlugin().apply(compiler);
				new StatsPlugin.AdditionalStatsPlugin().apply(compiler);
				break;
		}
		new StatsPlugin.AssetsStatsPlugin().apply(compiler);
