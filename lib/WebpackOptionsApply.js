...
/** @typedef {import("./SourceMapDevToolModuleOptionsPlugin").SourceMapDevToolPluginOptions} SourceMapDevToolPluginOptions */
/** @typedef {import("./EvalDevToolModulePlugin").EvalDevToolModulePluginOptions} EvalDevToolModulePluginOptions */
...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		if (options.devtool) {
			...
			const useSourcemap = devtool.includes("source-map");
			const useEval = devtool.includes("eval");
			 ...
			 * @returns {(SourceMapDevToolPluginOptions | EvalDevToolModulePluginOptions)=} options
			 */
			const getOptions = (devtool) => {
			...
			const devtoolsByType = options.experiments.css
				? [
						{
							type: CSS_TYPE,
							plugin: SourceMapDevToolPlugin,
							options: useEval ? getOptions("source-map") : getOptions(devtool)
						},
						{
							...
							options: getOptions(devtool)
						}
					]
				: [
						{
							type: JAVASCRIPT_AND_CSS_TYPES,
							plugin: useSourcemap
								? useEval
									? EvalSourceMapDevToolPlugin
									: SourceMapDevToolPlugin
			...
			for (const item of devtoolsByType) {
				const test =
					item.type === JAVASCRIPT_AND_CSS_TYPES
						? /\.((c|m)?js|css)($|\?)/i
						: item.type === CSS_TYPE
							? /\.(css)($|\?)/i
							: /\.((c|m)?js)($|\?)/i;
