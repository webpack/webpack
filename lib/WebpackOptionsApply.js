...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.performance) {
			...
			if (options.performance.circularDependencies) {
				const CircularDependenciesPlugin = require("./performance/CircularDependenciesPlugin");

				new CircularDependenciesPlugin(options.performance).apply(compiler);
			}
			...
			if (options.performance.unsplitDynamicImports) {
				const UnsplitDynamicImportsPlugin = require("./performance/UnsplitDynamicImportsPlugin");

				new UnsplitDynamicImportsPlugin(options.performance).apply(compiler);
			}
