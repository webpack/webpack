...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		// Not gated on `module.rules` being non-empty: plugins may still add rules
		// before the `NormalModuleFactory` compiles them.
		if (options.module.unusedRules) {
			const UnusedRuleWarningPlugin = require("./rules/UnusedRuleWarningPlugin");

			new UnusedRuleWarningPlugin(options.module.unusedRules).apply(compiler);
		}
