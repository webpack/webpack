...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.externalsPresets.nodeModules) {
			...
			// Relative/absolute requests are never installed packages.
			const RELATIVE_OR_ABSOLUTE = /^(?:\.\.?(?:\/|$)|\/|[A-Za-z]:[\\/])/;
			const NODE_MODULES = /[\\/]node_modules[\\/]/i;
			// CSS/url references are handled by their own presets, never as package externals.
			...
			new ExternalsPlugin(
				...
				({ request, context, dependencyType, getResolve }, callback) => {
					if (
						RELATIVE_OR_ABSOLUTE.test(request) ||
					...
					// Externalize the package only when it resolves into a node_modules directory.
					resolve(context, request, (err, result) => {
						callback(
							null,
							!err && result && NODE_MODULES.test(result) ? true : undefined
						);
					});
				}
			).apply(compiler);
