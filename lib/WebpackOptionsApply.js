 ...
 */
/** @import { LibraryType } from "../declarations/WebpackOptions" */
...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (!options.output.module) {
			// The defaults turn `output.module` on for these types, so reaching here
			// means it was turned back off by hand.
			for (const type of ["module", "modern-module"]) {
				if (enabledLibraryTypes.includes(/** @type {LibraryType} */ (type))) {
					throw new Error(
						`library type "${type}" is only allowed when 'output.module' is enabled`
					);
				}
			}
		}
