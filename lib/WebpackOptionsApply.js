...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.performance) {
			// First in the block: interception only reaches a tap registered after
			// it, so every plugin wired below is one this one can still see.
			if (options.performance.slowPlugins) {
				const SlowPluginsPlugin = require("./performance/SlowPluginsPlugin");

				new SlowPluginsPlugin(options.performance).apply(compiler);
			}
