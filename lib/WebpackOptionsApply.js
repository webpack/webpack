...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.performance) {
			...
			if (options.performance.constReassignment) {
				const ConstReassignmentPlugin = require("./performance/ConstReassignmentPlugin");

				new ConstReassignmentPlugin(options.performance).apply(compiler);
			}

			if (options.performance.namespaceCall) {
				const NamespaceCallPlugin = require("./performance/NamespaceCallPlugin");

				new NamespaceCallPlugin(options.performance).apply(compiler);
			}
			...
			if (options.performance.loadersOnNodeModules) {
				const LoadersOnNodeModulesPlugin = require("./performance/LoadersOnNodeModulesPlugin");

				new LoadersOnNodeModulesPlugin(options.performance).apply(compiler);
			}
