...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
			...
			new ExternalsPlugin(
				...
				({ request, context, dependencyType, getResolve }, callback) => {
					...
					resolve(context, request, (err, result) => {
						...
						const resolvedPath = result.split(NODE_MODULES).pop();
		...
		// CSS and HTML that reach the bundle inside a JS string literal — every
		// `exportType` but `"link"`, and an HTML module imported from JS. Driven by
		// `optimization.minimize` rather than by membership in `optimization.minimizer`:
		// a minimizer, webpack's own or a replacement, matches asset filenames and so
		// can never reach this text, which would otherwise leave nobody to minify it.
		const { minimize } = options.optimization;
		if (minimize) {
			const outputEnvironment = options.output.environment || {};
			// The spellings the minifier may reach for are the ones the target
			// supports, matching what the asset-level minifier is given.
			const environment = {
			...
			const cssOptions = typeof minimize.css === "object" ? minimize.css : {};
			const htmlOptions =
				typeof minimize.html === "object" ? minimize.html : {};
			const css = Boolean(options.experiments.css) && minimize.css !== false;
			const html = Boolean(options.experiments.html) && minimize.html !== false;
			if (css || html) {
				const MinifyEmbeddedSourcePlugin = require("./optimize/MinifyEmbeddedSourcePlugin");

				new MinifyEmbeddedSourcePlugin({
					css: css ? { environment, ...cssOptions } : undefined,
					html: html
						? { environment, ...cssOptions, ...htmlOptions }
						: undefined
				}).apply(compiler);
			}
		}
