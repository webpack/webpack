...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.output.module) {
			// Rewrites post-hash placeholders baked into analyzable ESM literals
			// (hashed chunk filenames / templated publicPath) to real paths.
			const AnalyzableEsmPlaceholderPlugin = require("./esm/AnalyzableEsmPlaceholderPlugin");

			new AnalyzableEsmPlaceholderPlugin().apply(compiler);
		}
