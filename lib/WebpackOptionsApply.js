...
/** @type {Map<string, typeof JavascriptParser>} */
const importPhaseParserCache = new Map();
...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.experiments.deferImport || options.experiments.sourceImport) {
			...
			const source = Boolean(options.experiments.sourceImport);
			const defer = Boolean(options.experiments.deferImport);
			const cacheKey = `source:${source},defer:${defer}`;

			if (!importPhaseParserCache.has(cacheKey)) {
				JavascriptParser.extend(importPhases({ source, defer }));
				importPhaseParserCache.set(cacheKey, JavascriptParser);
			}
		}
