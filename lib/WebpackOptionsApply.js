...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		if (typeof options.target === "string") {
			const TargetPlugin = require("./target/TargetPlugin");
			const TargetPluginClass = TargetPlugin.getPluginClass(options.target);
			new TargetPluginClass(options).apply(compiler);
		} else if (typeof options.target === "function") {
			options.target(compiler);
		} else {
			options.target.apply(compiler);
		}
