...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		if (options.devtool) {
			// TODO webpack 6 remove following options
			// `output.sourceMapFilename`
			// `output.devtoolModuleFilenameTemplate`
			// `output.devtoolFallbackModuleFilenameTemplate`
			// `output.devtoolNamespace`
			const filename = options.output.sourceMapFilename;
			const moduleFilenameTemplate =
				options.output.devtoolModuleFilenameTemplate;
			const fallbackModuleFilenameTemplate =
				options.output.devtoolFallbackModuleFilenameTemplate;
			const namespace = options.output.devtoolNamespace;

			if (typeof options.devtool === "object") {
				const evalWrapped = options.devtool.eval;
				...
				new Plugin({
					...
					namespace,
					// SourceMap related options defined in the `output` configuration will be overwritten.
					...options.devtool
				}).apply(compiler);
