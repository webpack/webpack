...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.experiments.parallelBuild) {
			const WorkerModuleBuilder = require("./build/WorkerModuleBuilder");
			const workerCount =
				typeof options.experiments.parallelBuild === "number"
					? options.experiments.parallelBuild
					: undefined;
			const builder = new WorkerModuleBuilder({ workers: workerCount });
			compiler._workerModuleBuilder = builder;
			// Start initialization eagerly but don't block compilation.
			// WorkerModuleBuilder.build() will await init internally if
			// called before workers are ready.
			builder.initialize();
			compiler.hooks.shutdown.tapPromise(
				"WorkerModuleBuilder",
				async () => {
					await builder.terminate();
					compiler._workerModuleBuilder = undefined;
				}
			);
		}
