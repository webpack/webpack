...
class Watching {
		...
		this.watcher =
			...
			(this.compiler.watchFileSystem).watch(
				buildDependencyFiles !== undefined && buildDependencyFiles.size > 0
					? concatIterables(files, buildDependencyFiles)
					: files,
				dirs,
				missing,
	 ...
	 * @param {ReadonlySet<string>} buildDependencyFiles watched build dependency files
	 * @param {ReadonlySet<string>=} changedFiles changed files
	 * @param {ReadonlySet<string>=} removedFiles removed files
	...
	_handleBuildDependenciesChanged(
		...
		for (const files of [changedFiles, removedFiles]) {
			if (!files) continue;
			for (const file of files) {
				if (buildDependencyFiles.has(file)) changed.add(file);
			}
		}
