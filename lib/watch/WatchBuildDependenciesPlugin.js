...
const toLoadedTimestamp = (err, entry, loadedAt) => {
	...
	const { timestamp, safeTime } = entry;
	// within the filesystem's timestamp accuracy of loading counts as after it
	return timestamp !== undefined && safeTime <= loadedAt
		? timestamp
		: undefined;
};
...
class WatchBuildDependenciesPlugin {
	...
	apply(compiler) {
		...
		// applied right after the configuration was loaded, callers knowing better set it themselves
		if (compiler.buildDependenciesStartTime === undefined) {
			compiler.buildDependenciesStartTime = Date.now();
		}
						...
						fileSystemInfo.getFileTimestamp(file, (err, entry) => {
							timestamps.set(
								...
								toLoadedTimestamp(
									err,
									entry,
									/** @type {number} */ (compiler.buildDependenciesStartTime)
								)
							);
							done();
						});
