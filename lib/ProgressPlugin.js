...
const InsightsPlugin = require("./Progress/InsightsPlugin");
...
// Create and wire InsightsPlugin so it receives every progress update.
// Use try/catch to avoid breaking builds if something goes wrong.
let __insightsInstance = null;
try {
	__insightsInstance = new InsightsPlugin({
		baselineCount: 10,
		showPhases: true
	});
	// Give the plugin a chance to hook in (it will attach its own ProgressPlugin)
	if (__insightsInstance && typeof __insightsInstance.apply === "function") {
		// Apply it to the same compiler so it sees lifecycle hooks
		try {
			__insightsInstance.apply && __insightsInstance.apply(compiler);
		} catch (e) {
			// swallow; we don't want progress reporting to break compilation
		}
	}
} catch (e) {
	__insightsInstance = null;
}

// Wrap the original handler so we call insights.updateProgress after each handler call.
// Keep this wrapper light and defensive.
const originalHandler = handler;
handler = (percentage, msg, ...args) => {
	try {
		originalHandler(percentage, msg, ...args);
	} catch (e) {
		// keep original behavior even if the handler throws
		try {
			compiler.getInfrastructureLogger("webpack.Progress").error(
				"ProgressPlugin handler error",
				e && e.stack ? e.stack : e
			);
		} catch {}
	}
	try {
		if (__insightsInstance && typeof __insightsInstance.updateProgress === "function") {
			// updateProgress expects numeric pct (0..1), message and optional args
			__insightsInstance.updateProgress(percentage, msg, ...args);
		}
	} catch (e) {
		// swallow any error from insights to avoid breaking the build
		try {
			compiler.getInfrastructureLogger("webpack.Progress").debug(
				"InsightsPlugin updateProgress error",
				e && e.stack ? e.stack : e
			);
		} catch {}
	}
