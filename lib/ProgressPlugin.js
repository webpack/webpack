			...
			for (let i = 0; i < len; i++) {
				...
				if (lastStateItem) {
					if (stateItem !== lastStateItem.value) {
						const { alreadyReported } = lastStateItem;
...
class ProgressPlugin {
	...
	_applyOnCompiler(compiler, handler) {
		const { showEntries } = this;
		const { showModules } = this;
		const { showDependencies } = this;
		const { showActiveModules } = this;
		...
		interceptHook(compiler.hooks.watchRun, 0.02, "run");
		interceptHook(compiler.hooks.beforeCompile, 0.03, "before compile");
