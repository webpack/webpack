...
const createReportBar = (name, color, width) => {
	...
	return (percentage) => {
		const w = Math.round(percentage * width);
		const filled = BLOCK_CHAR.repeat(w);
		const empty = BLOCK_CHAR.repeat(width - w);
		const colorFn =
			color in c ? c[/** @type {keyof Colors} */ (color)] : c.green;

		return `${[BULLET_ICON, name, filled].map(colorFn).join(" ")}${c.white(
			empty
		)}`;
	};
};
...
const createDefaultHandler = (profile, logger, progressBar, timing = {}) => {
	...
	const defaultHandler = (percentage, msg, ...args) => {
		...
		if (progressBar) {
			const reportBar = createReportBar(
				progressBar.name,
				progressBar.color,
				progressBar.width
			);
...
class ProgressPlugin {
	...
	constructor(options = {}) {
		...
		this.progressBar = progressBar
			? { name: "Build", color: "green", width: BAR_LENGTH, ...progressBar }
			: false;
		/** @type {boolean} */
