		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			compilation.hooks.moduleIds.tap(PLUGIN_NAME, () => {
				...
				const context = this.options.context
					...
					: compiler.context;
				const maxLength =
					this.options.maxLength !== undefined ? this.options.maxLength : 3;
				const failOnConflict =
					this.options.failOnConflict !== undefined
						? this.options.failOnConflict
						: false;
				const fixedLength =
					this.options.fixedLength !== undefined
						? this.options.fixedLength
						: false;
				const salt = this.options.salt !== undefined ? this.options.salt : 0;
				let conflicts = 0;
