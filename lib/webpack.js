...
/**
 * Whether core should auto-enable progress for interactive terminals (webpack@6
 * default, opt-in today via `experiments.futureDefaults`). Like other bundlers,
 * progress shows in a TTY and stays silent in CI / when logging is off.
...
const isDefaultProgressEnabled = (options) => {
	if (!options.experiments || !options.experiments.futureDefaults) return false;
...
const createMultiCompiler = (childOptions, options) => {
	...
	if (
		firstOptions &&
		isDefaultProgressEnabled(firstOptions) &&
		!compilers.some((c) =>
...
const createCompiler = (rawOptions, compilerIndex) => {
	...
	// A MultiCompiler applies one aggregated progress reporter itself (see below).
	if (
		compilerIndex === undefined &&
		isDefaultProgressEnabled(
