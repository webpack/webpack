...
module.exports = class Template extends Tapable {
	...
	renderChunkModules(chunk, moduleTemplate, dependencyTemplates, prefix) {
		...
		if(bounds) {
			...
			// Render an object
			source.add("{\n");
			var modulesSorting = Object.keys(modulesId).sort(function(a, b) {
				var aId = a + "";
				var bId = b + "";
				if(aId < bId) return -1;
				if(aId > bId) return 1;
			...
			for(var indexSorting = 0; indexSorting < modulesSorting.length; indexSorting++) {
				var moduleSorting = modulesId[modulesSorting[indexSorting]];
				if(indexSorting !== 0) source.add(",\n");

				source.add("\n/***/ " + JSON.stringify(moduleSorting.id) + ":\n");
				source.add(moduleSorting.source);
			}
			source.add("\n\n" + prefix + "}");
		}
		return source;
