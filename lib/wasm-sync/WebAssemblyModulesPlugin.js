...
const getWebAssemblyGenerator = lazyModule(() =>
	require("./WebAssemblyGenerator")
);
const getWebAssemblyJavascriptGenerator = lazyModule(() =>
	require("./WebAssemblyJavascriptGenerator")
);
const getWebAssemblyParser = lazyModule(() => require("./WebAssemblyParser"));
const getSyncWasmModule = lazyModule(() => require("./SyncWasmModule"));
			...
			(compilation, { normalModuleFactory }) => {
				...
				preloadModuleType(normalModuleFactory, PLUGIN_NAME, [
					[
						...
						[
							getSyncWasmModule,
							getWebAssemblyParser,
							getWebAssemblyJavascriptGenerator,
							getWebAssemblyGenerator
						]
					]
				]);
				...
				normalModuleFactory.hooks.createModuleClass
					...
					.tap(PLUGIN_NAME, (createData, _resolveData) => {
						const SyncWasmModule = getSyncWasmModule.loaded();
				...
				normalModuleFactory.hooks.createParser
					.for(WEBASSEMBLY_MODULE_TYPE_SYNC)
					.tap(PLUGIN_NAME, () => {
						const WebAssemblyParser = getWebAssemblyParser.loaded();
				...
				normalModuleFactory.hooks.createGenerator
					...
					.tap(PLUGIN_NAME, () => {
						const WebAssemblyJavascriptGenerator =
							getWebAssemblyJavascriptGenerator.loaded();
						const WebAssemblyGenerator = getWebAssemblyGenerator.loaded();
