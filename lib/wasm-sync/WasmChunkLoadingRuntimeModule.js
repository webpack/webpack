...
/**
 * Collects every asynchronous WebAssembly module reachable from the chunk.
 * The modules are returned in deterministic identifier order per async chunk.
...
/**
 * Generates the runtime factory that builds the WebAssembly import object for
 * a specific wasm module.
 * The generated code wires JS dependencies, deferred imports, and instance
 * lookups into the shape expected by `WebAssembly.instantiate`.
 * @param {ChunkGraph} chunkGraph the chunk graph
...
/**
 * Options that control how the wasm chunk loading runtime module fetches,
 * instantiates, and exposes WebAssembly modules at runtime.
...
/**
 * Runtime module that wires chunk loading to WebAssembly instantiation for
 * synchronous wasm modules.
 */
class WasmChunkLoadingRuntimeModule extends RuntimeModule {
	/**
	 * Creates the runtime module responsible for loading synchronous wasm chunks.
	...
	/**
	 * Generates the runtime that fetches wasm binaries, creates import objects,
	 * instantiates modules, and stores the resulting exports in the runtime
	 * instance cache.
	...
	generate() {
		...
		/**
		 * Wraps generated import bindings in the module namespace expected by the
		 * wasm loader when import names are mangled.
