 ...
 * @param {WasmImportObject} wasmImportObject Controls the memory configuration for the `importObj` parameter in `WebAssembly.instantiate`.
...
const generateImportObject = (
	...
	const usedWasmPresentationalDependencies =
		WebAssemblyUtils.getUsedPresentationalDependencies(
			module,
			mangle,
			usedWasmDependencies.length
		);
	if (wasmImportObject !== "plain-object") {
		for (const usedDep of usedWasmPresentationalDependencies) {
			const module = usedDep.module;
			const name = usedDep.name;
			properties.push({
				module,
				name,
				// @ts-expect-error
				value: `new WebAssembly.Memory({ initial: ${usedDep.dependency.description.limits.min}, maximum: ${usedDep.dependency.description.limits.max || "65536"}, shared: ${wasmImportObject === "SharedArrayBuffer" ? "true" : "false"} })`
			});
		}
	}
...
class WasmChunkLoadingRuntimeModule extends RuntimeModule {
	...
	constructor({
		...
		this.wasmImportObject = wasmImportObject;
	}
	...
	generate() {
		...
		const { mangleImports, wasmImportObject } = this;
		/** @type {string[]} */
		const declarations = [];
