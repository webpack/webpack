...
const generateImportObject = (
	...
	if (waitForInstances.size === 1) {
		...
		return Template.asString([
			`${moduleIdStringified}: function() {`,
			Template.indent([
				`return promiseResolve().then(function() { return ${RuntimeGlobals.promise}.all([${promises}]); }).then(function(array) {`,
...
class WasmChunkLoadingRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		return Template.asString([
			...
			`function promiseResolve() { return ${RuntimeGlobals.promise}.resolve(); }`,
			"",
				...
				Template.indent([
					...
					Template.indent([
						...
						this.supportsStreaming
							? Template.asString([
									`if(importObject instanceof ${RuntimeGlobals.promise} && typeof WebAssembly.compileStreaming === 'function') {`,
									Template.indent([
										`promise = ${RuntimeGlobals.promise}.all([WebAssembly.compileStreaming(req), importObject]).then(function(items) {`,
										Template.indent([
							...
							: Template.asString([
									`if(importObject instanceof ${RuntimeGlobals.promise}) {`,
									Template.indent([
										"var bytesPromise = req.then(function(x) { return x.arrayBuffer(); });",
										`promise = ${RuntimeGlobals.promise}.all([`,
										Template.indent([
