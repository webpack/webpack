...
class WebAssemblyJavascriptGenerator extends Generator {
	...
	generate(module, generateContext) {
		...
		const initParams = [];
		let index = 0;
		// In concat scope, the wasm JS wrapper is inlined into another module, so
		// `module.exportsArgument` is not available. Re-export assignments instead
		// route through the namespace object that this module registers.
		const exportsTarget = concatenationScope
			...
			if (mod) {
				...
				if (dep instanceof WebAssemblyExportImportedDependency) {
					...
					if (usedName) {
						...
						const exportProp = `${exportsTarget}[${JSON.stringify(usedName)}]`;
		...
		if (concatenationScope) {
			const moduleIdExpr = JSON.stringify(chunkGraph.getModuleId(module));
			...
			const source = new RawSource(
				[
					"// Inline WebAssembly module exports namespace",
					`var ${namespaceVar} = {};`,
					`var wasmExports = ${RuntimeGlobals.wasmInstances}[${moduleIdExpr}];`,
					// Copy named exports onto the namespace, skipping the start
					// function entry (its key is the empty string).
					`for(var name in wasmExports) if(name) ${namespaceVar}[name] = wasmExports[name];`,
