...
class IgnoringWatchFileSystem {
	...
	watch(files, dirs, missing, startTime, options, callback, callbackUndelayed) {
		...
		const onWfsWatchCallback = (err, filesModified, dirsModified, missingModified, fileTimestamps, dirTimestamps) => {
		...
		this.wfs.watch(files.filter(notIgnored), dirs.filter(notIgnored), missing, startTime, options, onWfsWatchCallback, callbackUndelayed);
	}
}
