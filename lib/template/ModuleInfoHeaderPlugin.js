 ...
 * @param {Set<InstanceType<ExportInfo>>} alreadyPrinted deduplication set
 * @returns {void}
 */
...
// `generateHeader`'s shape, as it reads back. A header names the module of the
// build that wrote it, so a source webpack reads its own output from sheds the
// one it arrives with and is headed again — see `stripHeaders`.
const HEADER_REGEXP =
	/^\/\*!\*+!\*\\\r?\n {2}!\*\*\* .* \*\*\*!\r?\n {2}\\\*+\/\r?\n/;
...
class ModuleInfoHeaderPlugin {
	 ...
	 */
	/**
	 * Drop the headers a build wrote at the top of a source that is being read
	 * back in, so this build's own header heads the module rather than the last
	 * build's — a page whose inline `<style>` webpack rewrites is read again on
	 * every rebuild, and a header left in place would be headed once more.
	 * @param {string} source source that may have been written by a build
	 * @returns {string} the source with any header of ours taken off the front
	 */
	static stripHeaders(source) {
		let stripped = source;
		// A page built before this stripped may carry a stack of them.
		while (HEADER_REGEXP.test(stripped)) {
			stripped = stripped.replace(HEADER_REGEXP, "");
		}
		return stripped;
	}
