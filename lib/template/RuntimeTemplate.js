...
const getExternalModule = memoize(() => require("../externals/ExternalModule"));
 ...
 */
/** @import { PublicPath, WasmLoading } from "../../declarations/WebpackOptions.d.ts" */
...
/** @import Chunk, { ChunkFilenameTemplate, ChunkId } from "../graph/Chunk" */
...
class RuntimeTemplate {
	 ...
	 * @returns {void}
	 */
	analyzableBailout(module, reason) {
		this._analyzableBailout(module, reason, null);
	}
	 ...
	 * @returns {string} the annotation, or an empty string
	 */
	sideEffectsAnnotation() {
		return this.outputOptions.module ? "/*#__PURE__*/" : "";
	}
	 ...
	 * @param {Module} module the instantiated module
	 * @param {ModuleGraph} moduleGraph the module graph
	 * @returns {string} the annotation, or an empty string when it may have side effects
	 */
	moduleSideEffectsAnnotation(module, moduleGraph) {
		return module.getSideEffectsConnectionState(moduleGraph) === false
			? this.sideEffectsAnnotation()
			: "";
	}
	...
	importStatement({
		...
		if (isModuleDeferred) {
			...
			importContent = `/* deferred harmony import */ ${optDeclaration}${importVar} = ${this.sideEffectsAnnotation()}${getOptimizedDeferredModule(
				moduleId,
				exportsType,
		...
		importContent = `/* harmony import */ ${optDeclaration}${importVar} = ${this.moduleSideEffectsAnnotation(
			module,
			moduleGraph
	...
	/**
	 * @param {object} options options
	 * @param {Module} options.module the module the body belongs to
	 * @param {boolean} options.needModule whether the body reads `module`
	 * @param {boolean} options.needExports whether the body reads `exports`
	 * @param {boolean} options.needRequire whether the body reads `__webpack_require__`
	 * @param {boolean} options.needThisAsExports whether the body reads `this` as exports
	 * @param {boolean=} options.shorthand render as a method shorthand, without the `function` keyword
	 * @returns {string} the header, ending with the opening brace
	 */
	moduleFactoryFnHeader({
		module,
		needModule,
		needExports,
		needRequire,
		...
		if (shorthand) return `(${parameters}) {`;
		if (!needThisAsExports && this.supportsArrowFunction()) {
			return `(${parameters}) => {`;
		}
