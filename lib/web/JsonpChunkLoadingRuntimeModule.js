...
/**
 * Compilation hooks that let plugins customize the `<link>` tags emitted for
 * JSONP-driven preload and prefetch resource hints.
...
/**
 * Runtime module that implements browser-side JavaScript chunk loading through
 * JSONP, including preload, prefetch, base URI, and HMR support.
 */
class JsonpChunkLoadingRuntimeModule extends RuntimeModule {
	/**
	 * Returns the per-compilation hook set used to customize preload and
	 * prefetch link generation for JSONP chunk loading.
	...
	/**
	 * Stores the runtime requirements that control which JSONP loading features
	 * should be emitted for the current chunk.
	...
	/**
	 * Returns the runtime statement that initializes `RuntimeGlobals.baseURI`
	 * from the entry configuration or the current document location.
	 * @private
	...
	/**
	 * Generates the JSONP chunk-loading runtime, including chunk state tracking,
	 * script loading, preload and prefetch handlers, update-manifest fetching,
	 * hot update loading, and the global JSONP callback.
