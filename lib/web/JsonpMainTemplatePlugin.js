	...
	apply(mainTemplate) {
		...
		mainTemplate.hooks.jsonpScript.tap(
			...
			(_, chunk, hash) => {
				...
				const alternatePublicPath = mainTemplate.hooks.alternatePublicPath.call(
					""
				);
				return Template.asString([
					...
					"}",
					"script.src = reloadUrl || jsonpScriptSrc(chunkId);",
					crossOriginLoading
						? Template.asString([
						...
						Template.indent([
							...
							alternatePublicPath
								? Template.indent([
										"if(!reloadUrl) {",
										Template.indent([
											`reloadUrl = ${JSON.stringify(
												alternatePublicPath
											)} + ${getScriptSrcPath(hash, chunk, "chunkId")};`,
											`${mainTemplate.requireFn}.e(chunkId, reloadUrl);`,
											"return ;"
										]),
										"}"
								  ])
			...
			(source, chunk, hash) => {
				return Template.asString([
					...
					Template.indent([
						...
						"if(!reloadUrl && installedChunkData) {",
