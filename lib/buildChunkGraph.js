...
const visitModules = (
	...
	const getBlockModules = (block, runtime) => {
		...
		if (memCache !== undefined) {
			// an optimization (e.g. side effects) can flip a connection's active
			// state without the module changing — key the cache by the states too
			let stateEtag = "";
			for (const connection of moduleGraph.getOutgoingConnections(module)) {
				if (!connection.dependency || !connection.module || connection.weak) {
					continue;
				}
				const state = connection.getActiveState(runtime);
				stateEtag += state === true ? "T" : state === false ? "F" : "O";
			}
			/** @type {BlockModulesMap} */
