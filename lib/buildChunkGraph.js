...
import { addConnectionStates } from "./ModuleGraphConnection.js";
import AsyncDependencyToInitialChunkError from "./errors/AsyncDependencyToInitialChunkError.js";
import { getEntryRuntime, mergeRuntime } from "./util/runtime.js";
...
const getActiveStateOfConnections = (connections, runtime) => {
	...
	for (let i = 1; i < connections.length; i++) {
		...
		merged = addConnectionStates(merged, c.getActiveState(runtime));
		if (merged === true) return true;
	}
	return merged;
};
			...
			if (indexMap === undefined) {
				...
				for (; i < length; i += 3) {
					if (modules[i] === m) {
						...
						modules[i + 1] = addConnectionStates(merged, state);
						continue outer;
					}
				}
				modules[length] = m;
				...
				if (idx !== undefined) {
					...
					modules[idx] = addConnectionStates(merged, state);
				} else {
					modules[length] = m;
					length++;
					modules[length] = state;
...
export default buildChunkGraph;

export { buildChunkGraph as "module.exports" };
