		...
		for (const info of outdatedChunkGroupInfo) {
			...
			if (info.skippedItems !== undefined) {
				for (const module of info.skippedItems) {
					if (
						!cachedMinAvailableModules.has(module) &&
						!cachedMinAvailableModules.plus.has(module)
					) {
						queue.push({
 ...
 */
const handleDependOn = compilation => {
	for (const [
		name,
		{
			options: { dependOn = [] }
		...
		const modulesToDeleteMap = new Map();
		const child = compilation.entrypoints.get(name);

		if (dependOn.length === 1) {
			const parent = compilation.entrypoints.get(dependOn[0]);
			child.setRuntimeChunk(parent.runtimeChunk);
		}

		for (const dependName of dependOn) {
			const parent = compilation.entrypoints.get(dependName);

			connectChunkGroupParentAndChild(parent, child);

			for (const chunk of parent.chunks) {
				const modules = compilation.chunkGraph.getChunkModules(chunk);
				for (const module of modules) {
					let moduleChunks = modulesToDeleteMap.get(module);
					if (!moduleChunks) {
						modulesToDeleteMap.set(module, (moduleChunks = new Set()));
					}
		...
		if (dependOn.length) {
			const chunksToPush = new Set();

			for (const chunk of child.chunks) {
				for (const [module, parentChunks] of modulesToDeleteMap) {
					if (compilation.chunkGraph.isModuleInChunk(module, chunk)) {
						// get the first chunk with this module
						const parentChunk = parentChunks.values().next().value;

						compilation.chunkGraph.disconnectChunkAndModule(chunk, module);
						chunksToPush.add(parentChunk);
					}
				}
			}

			// push to a child group only these chunks
			// that includes the modules, that child entrypoint are requiring
			for (const chunk of chunksToPush) {
				connectChunkGroupAndChunk(child, chunk);
			}
		}
	}
};
...
const buildChunkGraph = (compilation, inputChunkGroups) => {
	...
	logger.time("handleDependOn");
	handleDependOn(compilation);
	logger.timeEnd("handleDependOn");
