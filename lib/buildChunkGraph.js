	...
	const processBlock = block => {
		...
		if (blockModules !== undefined) {
			...
			for (let i = 0, len = blockModules.length; i < len; i += 3) {
				...
				if (activeState !== true) {
					const connections = /** @type {ModuleGraphConnection[]} */ (blockModules[i + 2]);
				...
				// 🔥 FIX: Ensure CSS modules are enqueued first to preserve order
				console.log(refModule); // Check available properties in console

const moduleIdentifier = typeof refModule.identifier === "function"
    ? refModule.identifier()
    : "";

const isCSSModule = moduleIdentifier.endsWith(".css");


				if (isCSSModule) {
					// Push CSS modules first to avoid reordering issues
					queueBuffer.unshift({
						action: ADD_AND_ENTER_MODULE,
						block: refModule,
						module: refModule,
