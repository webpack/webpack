	...
	const iteratorBlock = (b) => {
		...
		if (cgi === undefined) {
			...
			if (entryOptions) {
				...
				!chunkGroupInfo.asyncChunks ||
				!chunkGroupInfo.chunkLoading ||
				// `new URL()` blocks without entryOptions stay in the parent chunk
				// (ordinary assets); entryOptions promote CSS/etc. to async entries.
				(b.groupOptions && b.groupOptions.url)
			) {
