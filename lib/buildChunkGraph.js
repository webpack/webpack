			...
			if (entryOptions) {
				...
				if (!cgi) {
					...
					for (const key_ of Object.keys(entryOptions)) {
						...
						if (existingOptions[key] === entryOptions[key]) continue;
						if (
							Array.isArray(existingOptions[key]) &&
							Array.isArray(entryOptions[key]) &&
							arrayEquals(existingOptions[key], entryOptions[key])
						) {
							continue;
						}
						if (existingOptions[key] === undefined) {
							...
							compilation.errors.push(
								new WebpackError(
									`Conflicting entry option ${key} = ${existingOptions[key]} vs ${entryOptions[key]} for entrypoint ${entrypoint.name}`
								)
							);
						}
					}
