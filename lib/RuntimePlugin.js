		...
		compiler.hooks.compilation.tap("RuntimePlugin", compilation => {
			...
			compilation.hooks.runtimeRequirementInTree
				...
				.tap("RuntimePlugin", (chunk, set, { chunkGraph }) => {
					...
					const entries = Array.from(chunk.groupsIterable).filter(
						group => group instanceof Entrypoint
					);
					for (const entry of entries) {
						for (const dep of entry.dependencies) {
							const depChunks = compilation.chunkGraph.getModuleChunks(dep);
							for (const c of depChunks) {
								chunkGraph.connectChunkAndEntryModule(chunk, c);
							}
						}
					}
					compilation.addRuntimeModule(
						...
						new GetChunkFilenameRuntimeModule(
							...
							chunk => {
								const hasJs = getJavascriptModulesPlugin().chunkHasJs(
									chunk,
									chunkGraph
								);
								const hasEntryDeps = chunk.hasEntryModule(chunkGraph);
								return (
									hasJs &&
									(hasEntryDeps || chunk.canBeInitial()) && // Include entry dependencies
									/** @type {TemplatePath} */ (
			...
			compilation.hooks.runtimeRequirementInTree
				...
				.tap("RuntimePlugin", (chunk, set) => {
					...
					let hasEntryDependencies = false;
					for (const group of chunk.groupsIterable) {
						if (group instanceof Entrypoint) {
							const deps = group.options.dependOn;
							if (Array.isArray(deps) && deps.length > 0) {
								hasEntryDependencies = true;
								break;
							}
						}
					}
