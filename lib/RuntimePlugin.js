...
const MakeModuleSourceRuntimeModule = require("./runtime/MakeModuleSourceRuntimeModule");
...
const GLOBALS_ON_REQUIRE = [
	...
	RuntimeGlobals.makeModuleSource
];
...
class RuntimePlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.runtimeRequirementInTree
				.for(RuntimeGlobals.makeModuleSource)
				.tap(PLUGIN_NAME, (chunk) => {
					compilation.addRuntimeModule(
						chunk,
						new MakeModuleSourceRuntimeModule()
					);
					return true;
				});
			compilation.hooks.runtimeRequirementInTree
