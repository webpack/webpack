...
/** @typedef {import("./optimize/InlineExports").InlinedUsedName} InlinedUsedName */
...
// Initial sentinel `HasInfo` must be distinct from `Yes`/`No` so that the
// dependency condition stays neutral until at least one consumer speaks.
const CanInlineUse = Object.freeze({
	HasInfo: /** @type {0} */ (0),
	Yes: /** @type {1} */ (1),
	No: /** @type {2} */ (2)
});

/** @typedef {typeof CanInlineUse[keyof typeof CanInlineUse]} CanInlineUseType */
	...
	getUsedName(name, runtime) {
		if (Array.isArray(name)) {
			...
			if (x instanceof InlinedUsedName) {
				// Inlined: tail ids become the suffix property access
				if (name.length === 1) return x;
				return new InlinedUsedName(x.value, [...x.suffix, ...name.slice(1)]);
			}
			const arr =
				/** @type {ExportInfoName[]} */
			...
			if (
				...
				// Nested inlined: return as-is (its suffix already includes the rest)
				if (nested instanceof InlinedUsedName) return nested;
...
class ExportInfo {
	...
	constructor(name, initFrom) {
		 ...
		 */
		this.canInlineProvide = initFrom ? initFrom.canInlineProvide : undefined;
		/**
		 * true: it can be mangled
		 * false: is can not be mangled
		 * undefined: it was not determined if it can be mangled
		 ...
		 * @type {CanInlineUseType | undefined}
		 */
		this.canInlineUse = initFrom ? initFrom.canInlineUse : undefined;
		/** @type {boolean} */
	 ...
	 */
	canInline() {
		return this.canInlineProvide !== undefined &&
			this.canInlineUse === CanInlineUse.Yes
	...
	setUsedInUnknownWay(runtime) {
		...
		if (this.canInlineUse !== CanInlineUse.No) {
			this.canInlineUse = CanInlineUse.No;
			changed = true;
		}
		return changed;
	}
	...
	setHasUseInfo() {
		...
		if (this.canInlineUse === undefined) {
			this.canInlineUse = CanInlineUse.HasInfo;
		}
...
module.exports.CanInlineUse = CanInlineUse;
