...
class ExportInfo {
	 ...
	 */
	getTarget(moduleGraph, alreadyVisited = new Set()) {
		/**
		...
		const resolveTarget = (target, alreadyVisited) => {
			...
			const newTarget = exportInfo.getTarget(moduleGraph, alreadyVisited);
			if (newTarget) {
				...
				const newAlreadyVisited = new Set([...alreadyVisited, exportInfo]);
				return resolveTarget(target, newAlreadyVisited);
			}
			return target;
		};
