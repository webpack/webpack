...
class ExportsInfo {
	 ...
	 * @param {RuntimeSpec} runtime check usage for this runtime only
	 * @param {boolean | undefined} isGlobalUsedExports whether usedExports is configured 'global'
	 * @returns {string | string[] | false} the used name
	 */
	getUsedName(name, runtime, isGlobalUsedExports = false) {
		if (Array.isArray(name)) {
			// TODO improve this
			if (name.length === 0) {
			...
			let info = this.getReadOnlyExportInfo(name[0]);
			const x = info.getUsedName(name[0], runtime, isGlobalUsedExports);
			...
			if (
				...
				const nested = info.exportsInfo.getUsedName(
					name.slice(1),
					runtime,
					isGlobalUsedExports
				);
			...
			let info = this.getReadOnlyExportInfo(name);
			const usedName = info.getUsedName(name, runtime, isGlobalUsedExports);
			return usedName;
		}
	}
...
class ExportInfo {
	 ...
	 */
	getUsedName(fallbackName, runtime, isGlobalUsedExports = false) {
		if (this._hasUseInRuntimeInfo && !isGlobalUsedExports) {
			if (this._globalUsed !== undefined) {
