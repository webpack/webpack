...
class ChunkPrefetchPreloadPlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			compilation.hooks.additionalChunkRuntimeRequirements.tap(
				...
				(chunk, set, { chunkGraph }) => {
					...
					if (startupChildChunks) {
						...
						// This is the one startup callback that is queued behind a priority.
			...
			compilation.hooks.additionalTreeRuntimeRequirements.tap(
				...
				(chunk, set, { chunkGraph }) => {
					...
					if (chunkMap.cssPreload) {
						// A CSS-only preload reaches the CSS handler itself, so it asks for
						// the handlers alone: nothing here calls the function fanning out
						// across them.
			...
			// The handlers object carries the runtime module, so a chunk asking only for
			// it ships no fan-out function. That is read here rather than in `create`,
			// which runs once the requirement pass is over.
