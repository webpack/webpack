...
/**
 * Yields URL-asset deps on a module, including those under `new URL()` blocks.
 * @param {Module} module module
 * @returns {IterableIterator<URLDependency | CssUrlDependency | HtmlSourceDependency>} dependencies
 */
function* iterateModuleUrlAssetDeps(module) {
	const deps = module.dependencies;
	if (deps) {
		for (const dep of deps) {
			if (isUrlAssetDep(dep)) yield dep;
		}
	}
...
function* iterateHintedUrlAssets(compilation, chunks) {
	...
	for (const chunk of chunks) {
		for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
			for (const dep of iterateModuleUrlAssetDeps(module)) {
