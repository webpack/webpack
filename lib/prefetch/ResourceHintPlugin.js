...
const collectEntrypointHints = (compilation, entryName, hints) => {
	...
	/** @type {EntrypointHint[]} */
	const out = [];
	/** @type {Map<string, EntrypointHint>} */
	const byKey = new Map();
	const push = (/** @type {EntrypointHint} */ h) => {
		const key = `${h.rel}\0${h.href}`;
		const existing = byKey.get(key);
		if (existing) {
			// Same asset reachable from several chunks — merge their names so the
			// callback / manifest sees every referencing chunk (Vite's `hostId`).
			if (h.hostChunks && existing.hostChunks) {
				for (const c of h.hostChunks) {
					if (!existing.hostChunks.includes(c)) existing.hostChunks.push(c);
				}
			}
			return;
		}
	...
	const publicPath =
		...
		compilation.outputOptions.publicPath !== "auto"
			...
			: "";
	// `output.autoPreconnect`: warm the connection to a cross-origin publicPath
	// (the origin every chunk / asset is fetched from), emitted first.
	if (compilation.outputOptions.autoPreconnect) {
...
class ResourceHintPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			const manifestPath = compilation.outputOptions.resourceHintsManifest;
			if (manifestPath) {
