...
const WorkerDependency = require("../dependencies/worker/WorkerDependency");
 ...
 */
/** @import { HtmlResourceHint } from "../dependencies/html/HtmlEntryDependency" */
 ...
 * @param {import("../graph/Dependency")} dep dependency
 ...
 * @param {Compilation} compilation compilation
 * @param {Iterable<import("../graph/Chunk")>} chunks chunks to scan
 * @returns {IterableIterator<{ dep: URLDependency | CssUrlDependency | HtmlSourceDependency, target: Module, chunk: import("../graph/Chunk") }>} hinted assets
 */
 ...
 * schema entry.
 * @param {import("../graph/Entrypoint")} entrypoint entrypoint
...
const collectEntrypointHints = (compilation, entryName, hints) => {
	...
	// `hostType` is `"html"` only where the entrypoint has an extracted page, that
	// is where some HTML module's script dependency names this entry. An SSR
	// framework reading the stats sees `"js"`.
...
const collectStartupAssetHintLines = (
	...
	for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
		const deps =
			/** @type {{ dependencies?: import("../graph/Dependency")[] }} */
			(module).dependencies;
		if (!deps) continue;
		for (const dep of deps) {
			if (!isUrlAssetDep(dep)) continue;
...
class ResourceHintPlugin {
	 ...
	 * @param {import("../module/NormalModule")} module module for emitting warnings on invalid comments
	 * @param {import("../graph/Dependency").DependencyLocation} loc loc for warnings
	 * @returns {void}
	 */
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			// WHY: computed on first access. One walk of every HTML module's
			// `HtmlEntryDependency`s — resolving each named entry and collecting every
			// URL asset dep reachable from its ordered chunks — yields both views: a
			// per-entry list, which `HtmlEntryDependency.Template` emits the `<link>`
			// tags from, and a global `WeakSet`, which `StartupAssetHintRuntimeModule`
			// reads to skip the JS runtime `<link>` for the same asset. Deterministic,
			// the module graph being fixed by seal time, and cheap, since it only fires
			// when a JS chunk actually produces hints.
			...
			const build = () => {
				...
				for (const module of compilation.modules) {
					...
					for (const dep of presDeps) {
						...
						// Mirrors `getEntrypointChunksInLoadOrder`: the entry, runtime and
						// every initial sibling. An async `import()` chunk stays on the
						// on-demand runtime and is never HTML-hinted.
						/** @type {Set<import("../graph/Chunk")>} */
			...
			// The SSR manifest serializes each entrypoint's resolved hints to a JSON
			// asset, the same descriptors the stats carry, so an SSR server can inject
			// the tags itself. `"none"` yields empty lists.
