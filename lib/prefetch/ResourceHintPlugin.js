...
class ResourceHintPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			// Computed on first access: one walk of every HTML module's entry
			// dependencies yields both a per-entry list, which emits the `<link>` tags,
			// and a global set, which keeps the JS runtime from repeating them.
