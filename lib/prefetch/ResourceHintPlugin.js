	...
	if (hints === true || hints === "prefetch" || typeof hints === "function") {
		...
		const rel = prefetch
			...
			: isModuleOutput
				? "modulepreload"
				: "preload";
		// These land in extracted HTML, where the preload scanner sees them and the
		// `fetchpriority` is honored — unlike a runtime-injected `modulepreload` link,
		// on which browsers ignore the attribute.
		const autoFetchPriority =
			outputResourceHints && outputResourceHints.fetchPriority;
		...
		for (const chunk of entrypoint.chunks) {
			...
			for (const file of chunk.files) {
				...
				if (autoFetchPriority) h.fetchPriority = autoFetchPriority;
				push(h);
			}
		}
	}
