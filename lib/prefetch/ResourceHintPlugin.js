...
const getResourceHintRuntimeModule = memoize(() =>
	require("./ResourceHintRuntimeModule")
);
const getStartupAssetHintRuntimeModule = memoize(() =>
...
const collectEntrypointHints = (compilation, entryName, hints) => {
	...
	for (const { dep, target, chunk } of iterateHintedUrlAssets(
		...
		const asAttribute =
			dep.asAttribute ||
			getResourceHintRuntimeModule().guessAsAttribute(dep.request);
		/** @type {EntrypointHint} */
		const h = {
				...
				(chunk, set, { chunkGraph }) => {
					...
					if (
						...
						compilation.addRuntimeModule(
							chunk,
							new (getStartupAssetHintRuntimeModule())()
						);
					}
				}
			);
			for (const [rel, runtimeGlobal] of /** @type {const} */ ([
				...
				compilation.hooks.runtimeRequirementInTree
					...
					.tap(PLUGIN_NAME, (chunk) => {
						compilation.addRuntimeModule(
							chunk,
							new (getResourceHintRuntimeModule())(rel)
						);
					});
			}
