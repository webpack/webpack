...
const collectStartupAssetHintLines = (
	...
	// Dedupe per asset module: if the same asset is referenced twice (e.g. a CSS rule
	// says `prefetch` and a JS magic comment says `preload`), pick `preload` and keep
	// the most recently seen explicit attribute overrides. Avoids emitting a duplicate
	// `<link>` and ordering-dependent winners.
	...
	for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
		...
		if (!deps) continue;
		for (const dep of deps) {
			if (!(dep instanceof URLDependency) && !isCssOrHtmlUrlAssetDep(dep)) {
				continue;
			}
			if (!dep.prefetch && !dep.preload) continue;
	...
	for (const entry of perAsset.values()) {
		...
		let href;
		// An asset with a javascript wrapper exposes its url through it; one without
		// (referenced only from css, or from a `new URL()` that names the file itself)
		// has no `__webpack_modules__` entry to require.
		if (assetModule.getSourceTypes().has(JAVASCRIPT_TYPE)) {
			...
			// A generator `publicPath` (e.g. a CDN override) makes the asset's own url
			// differ from `output.publicPath` + filename, so prefer it. Only the two
			// placeholders below are left for `output.publicPath`, and they are resolved
			// after this runs — everything else is already a usable literal.
			const templated =
			...
			if (typeof resolved === "string" && !templated) {
				...
				// The same literal the `new URL()` call site bakes, asked of the same
				// module, so the two agree on whether one is spellable.
				const specifier = runtimeTemplate.supportsAnalyzable(
					"url",
					chunkGraph,
					entry.originModule
...
class ResourceHintPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			// The `<link>` calls are rendered here rather than in the runtime module:
			// runtime requirements are still open at this point, so an href that comes
			// out a literal costs neither `.p` nor `.u`. Attached to *this* chunk
			// (covering async chunks too) — `runtimeRequirementInTree` alone would only
			// reach the runtime chunk, so a hinted `new URL(...)` in an async chunk
			// would never fire its `<link>` injection.
