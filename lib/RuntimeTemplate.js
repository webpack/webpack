...
module.exports = class RuntimeTemplate {
	 ...
	 */
	importStatement({ update, module, request, importVar, originModule, chunk }) {
		if (!module) {
		...
		// if amd import will be handled via require
		if (
			module.externalType === "amd" &&
			!module.getChunks().some(c => c.entryModule) &&
			chunk.getModules().some(m => m.id === module.id)
		) {
			return "";
		}
