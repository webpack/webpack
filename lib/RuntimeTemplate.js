...
class RuntimeTemplate {
	...
	_analyzableBailout(module, reason, answer) {
		if (module !== undefined && this.outputOptions.module) {
			...
			if (!bailouts.includes(text)) {
				bailouts.push(text);
				if (this._analyzableBailoutSink !== undefined) {
					this._analyzableBailoutSink.push(reason);
				}
			}
		}
		return answer;
	 ...
	 * @template T
	 * @param {Module} module the module being generated, which the reasons are recorded on
	 * @param {() => T} generate the code generation
	 * @returns {[T, string[]]} its result and the reasons recorded during it
	 */
	collectAnalyzableBailouts(module, generate) {
		const outerModule = this._analyzableGeneratingModule;
		const outerSink = this._analyzableBailoutSink;
		/** @type {string[]} */
		const sink = [];
		this._analyzableGeneratingModule = module;
		this._analyzableBailoutSink = sink;
		try {
			const result = generate();
			return [result, sink];
		} finally {
			this._analyzableGeneratingModule = outerModule;
			this._analyzableBailoutSink = outerSink;
		}
	}
