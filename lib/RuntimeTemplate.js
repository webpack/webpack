...
class RuntimeTemplate {
	...
	_foldAnalyzableNames(chunk, hash) {
		...
		// What a stand-in in this chunk can name: `analyzableCssChunkUrls` reaches an
		// initial stylesheet that no reference does.
		...
		for (const referenced of reachable) {
			...
			for (const [contentHashType, naming] of CHUNK_ASSET_NAMING) {
				...
				// The very call the fill makes, so what is folded in is what lands there —
				// a filename function included, which resolves to a name here either way.
				hash.update(
					compilation.getPath(naming.template(referenced, outputOptions), {
						chunk: referenced,
