...
class RuntimeTemplate {
	...
	getBuiltinModule(request, access = "") {
		...
		const requireFallback = `(${this.basicFunction("", [
			"try {",
			Template.indent(`return require(${request})${access};`),
			"} catch (e) {",
			Template.indent("return false;"),
			"}"
		])})()`;
		return `typeof process !== "undefined" && (typeof process.getBuiltinModule === "function" ? ${getter} : ${requireFallback})`;
	}
