...
const { isAbsoluteBaseUri } = require("./runtime/baseUri");
...
class RuntimeTemplate {
	...
	_getAnalyzableChunkSpecifier(
		...
		// Baking makes this chunk's content depend on the referenced one's hash. A pair
		// that reaches back would have no fixed point if both baked, so only the lower
		// id does: every dependency then points one way and the hashes settle in order.
		const cycles =
			deferred &&
