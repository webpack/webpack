...
const getModuleUndoPath = memoize(() =>
	require("./dependencies/getModuleUndoPath")
);
...
class RuntimeTemplate {
	...
	supportsAnalyzableEsm() {
		...
		const devtool = this.compilation && this.compilation.options.devtool;
		return (
			this.isModule() &&
	...
	/**
	 * For ESM module output, load a single statically-named chunk with an analyzable
	 * `import("./chunk.js")` (literal specifier) plus the existing `installChunk`, so
	 * other bundlers and webpack itself can follow the dynamic import. Returns `null`
	 * to fall back to the runtime `ensureChunk` form.
	...
	_analyzableChunkImport(
		...
		// assets needs the runtime's dedup and per-type loaders, so keep the runtime form.
		...
		// Relative to the consuming chunk (the `import()` resolves against its own
		// `import.meta.url`).
		const base = getModuleUndoPath()(
			originModule,
			chunkGraph,
			this.compilation
		);
		...
		const filename = this.compilation.getPath(filenameTemplate, {
			chunk,
			contentHashType: JAVASCRIPT_TYPE
		});
		runtimeRequirements.add(RuntimeGlobals.externalInstallChunk);
		// Keep `getChunkScriptFilename` available: coexisting prefetch/preload handlers
		// (`__webpack_require__.F.j`) still reference `__webpack_require__.u`.
		runtimeRequirements.add(RuntimeGlobals.getChunkScriptFilename);
		return `import(${comment}${JSON.stringify(base + filename)}).then(${
