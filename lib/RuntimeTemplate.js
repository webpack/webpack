...
const {
	CHUNKHASH_IN_FILENAME,
	CONTENTHASH_IN_FILENAME,
	FULLHASH_IN_FILENAME,
	HASH_IN_FILENAME,
	HASH_IN_FILENAME_GLOBAL,
	PUBLIC_PATH_PLACEHOLDER,
	chunkFilenamePlaceholder,
	isFullHashOnlyTemplate
} = require("./esm/analyzableEsmPlaceholders");
...
class RuntimeTemplate {
	...
	_analyzableChunkImport(
		...
		// For a post-hash publicPath placeholder, test the template's static prefix.
		const testable = specifier.startsWith(`"${PUBLIC_PATH_PLACEHOLDER}`)
			? `"${/** @type {string} */ (outputOptions.publicPath).split("[", 1)[0]}`
			: specifier;
	...
	_getAnalyzableChunkSpecifier(
		...
		if (HASH_IN_FILENAME.test(filenameTemplate)) {
			...
			// AnalyzableEsmPlaceholderPlugin substitutes once hashed assets exist.
		...
		if (typeof publicPath === "string") {
			...
			// A publicPath resolvable from the compilation hash alone also gets the
			// post-hash placeholder.
			if (isFullHashOnlyTemplate(publicPath) && getEmbedMode() !== 0) {
