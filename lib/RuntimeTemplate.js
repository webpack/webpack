...
// or `:<digest>[:<length>]` suffix. Its value is unknown during code generation.
...
// Post-hash placeholders: a hashed chunk filename or a `[fullhash]` publicPath is unknown
// during code generation, so the analyzable ESM literal is baked with one of these tokens
// and `substituteAnalyzableEsmPlaceholders` rewrites it at JS render time (after hashing,
// before assets are emitted). The consuming chunk is made to depend on the resolved hash
// (a referenced chunk hash via `updateHashForAnalyzableEsmReferences`, the full hash via the
// `analyzableEsmFullHash` runtime module) so it re-renders when that hash changes. The
// charset is string-literal-safe and survives minification.
const PLACEHOLDER_PREFIX = "___WEBPACK_ANALYZABLE_";
const CHUNK_PLACEHOLDER_PREFIX = `${PLACEHOLDER_PREFIX}CHUNK_`;
...
/**
 * Placeholder for a chunk's final (post-hash) filename. Unique per chunk id, so the
 * substitution maps tokens back by re-encoding each chunk's id (`.`/`-` replace the
 * base64 chars that would collide with the delimiter or the regex).
 * @param {ChunkId} chunkId the referenced chunk's id
 * @returns {string} placeholder token
 */
const chunkFilenamePlaceholder = (chunkId) =>
	`${CHUNK_PLACEHOLDER_PREFIX}${Buffer.from(JSON.stringify(chunkId))
		.toString("base64")
		.replace(/\+/g, "-")
		.replace(/\//g, ".")
		.replace(/[=]+$/, "")}___`;
 ...
 * (`[fullhash]` / legacy `[hash]`): such a value can be baked after hashing even though
 * it is unknown during code generation.
...
const isFullHashOnlyTemplate = (str) => {
	...
	for (const token of tokens) {
		if (!FULLHASH_TOKEN_REGEXP.test(token.slice(1, -1))) return false;
	}
	return true;
};
...
class RuntimeTemplate {
	...
	constructor(compilation, outputOptions, requestShortener) {
		...
		/** @type {Map<Module, Module> | undefined} */
		this._concatenationRootByInner = undefined;
		/** @type {Map<string, Chunk> | undefined} */
		this._analyzableChunkByToken = undefined;
		/** @type {boolean | undefined} */
		this._mayHaveAnalyzablePlaceholders = undefined;
	}

	/**
	 * Whether any analyzable ESM placeholder can exist in this compilation — only when a
	 * chunk filename template carries a hash or the publicPath is a full-hash template.
	 * Lets the per-asset / per-chunk analyzable passes skip cheaply on builds that use no
	 * hashed filenames. Cached, since it scans output options and per-chunk overrides once.
	 * @returns {boolean} true when placeholders may have been baked
	 */
	_mayHavePlaceholders() {
		let value = this._mayHaveAnalyzablePlaceholders;
		...
		value =
			...
			publicPath.includes("[") &&
			isFullHashOnlyTemplate(publicPath);
		if (!value) {
			for (const template of [
				outputOptions.filename,
				outputOptions.chunkFilename,
				outputOptions.workerChunkFilename
			]) {
				if (typeof template === "string" && HASH_IN_FILENAME.test(template)) {
					value = true;
					break;
				}
			}
		...
		if (!value) {
			for (const chunk of this.compilation.chunks) {
				if (
					typeof chunk.filenameTemplate === "string" &&
					HASH_IN_FILENAME.test(chunk.filenameTemplate)
				) {
					value = true;
					break;
				}
		...
		return (this._mayHaveAnalyzablePlaceholders = value);
	}

	/**
	 * The chunk-bearing module for `module`: itself, or the ConcatenatedModule
	 * hosting it — an inner concatenated module is disconnected from all chunks.
	 * @param {Module} module the (possibly inner) module
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {Module} a module that is connected to chunks
	 */
	_getChunkBearingModule(module, chunkGraph) {
		if (chunkGraph.getNumberOfModuleChunks(module) > 0) return module;
		let map = this._concatenationRootByInner;
		if (map === undefined) {
			map = this._concatenationRootByInner = new Map();
			const ConcatenatedModule = getConcatenatedModule();
			for (const m of this.compilation.modules) {
				if (m instanceof ConcatenatedModule) {
					for (const inner of m.modules) map.set(inner, m);
				}
			}
		}
		const root = map.get(module);
		return root === undefined ? module : root;
	}
	...
	_analyzableChunkImport(
		...
		// Also require the consuming chunk to carry the runtime (the `.ei` helper): when
		// `optimization.runtimeChunk` splits it out, the import runs in a chunk that only
		// links the runtime, and installing the target through that boundary is unreliable —
		// keep the runtime form, which loads via the shared runtime chunk.
		originModule = this._getChunkBearingModule(originModule, chunkGraph);
		...
		for (const originChunk of chunkGraph.getModuleChunksIterable(
			...
			if (originChunk.hasRuntime()) hasRuntime = true;
		}
		if (!hasRuntime) {
			return null;
		}
		...
		// keep the runtime form for them. (A chunk shared across chunk groups is fine: the
		// `.ei` helper de-duplicates by id, resolving an already-installed/in-flight load.)
		if (
		...
		for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
			...
			// Only pure-JS chunks: css/wasm/other assets need the runtime's per-type loaders.
		...
		// absolute publicPath prefix. Resolve without side effects; the full-hash marker
		// is added below only once the specifier is confirmed usable.
		...
		// For a full-hash publicPath placeholder, test the publicPath's static prefix.
		const testable = specifier.startsWith(`"${PUBLIC_PATH_PLACEHOLDER}`)
			? `"${String(this.outputOptions.publicPath).split("[", 1)[0]}`
			: specifier;
		if (!/^"(?:\.{0,2}\/|[a-zA-Z][\w+.-]*:)/.test(testable)) {
			return null;
		}
		if (specifier.includes(PUBLIC_PATH_PLACEHOLDER)) {
			runtimeRequirements.add(RuntimeGlobals.analyzableEsmFullHash);
		}
	...
	_getModuleUndoPath(module, chunkGraph) {
		...
		let result = null;
		let found = false;
		module = this._getChunkBearingModule(module, chunkGraph);
	...
	/**
	 * Whether a post-hash placeholder may be embedded into the consuming module's
	 * chunks. The consuming chunk's hash picks up the referenced hash
	 * (`updateHashForAnalyzableEsmReferences`), so a `[chunkhash]`/`[contenthash]`/
	 * `[fullhash]` or hash-free name all stay cache-correct; only a function filename
	 * template (which we can't reason about) blocks embedding.
	 * @param {Module} consumingModule the module the reference is emitted into
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {boolean} true when the reference may be baked as a placeholder
	 */
	_canEmbedPostHash(consumingModule, chunkGraph) {
		consumingModule = this._getChunkBearingModule(consumingModule, chunkGraph);
		// A module in no chunk (e.g. an unmapped inner module) can't be reasoned about.
		if (chunkGraph.getNumberOfModuleChunks(consumingModule) === 0) return false;
		for (const chunk of chunkGraph.getModuleChunksIterable(consumingModule)) {
			const template = getJavascriptModulesPlugin().getChunkFilenameTemplate(
				chunk,
				this.outputOptions
			);
	...
	/**
	 * The chunk's rank in `createHash`'s 4-category hashing order (lower hashes first):
	 * async 0, initial 1, runtime 2, entry 3. A reference is ordering-safe only when the
	 * referenced chunk hashes before the consumer, so its rank picks up in the consumer.
	 * @param {Chunk} chunk the chunk
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {0 | 1 | 2 | 3} the hashing-order rank
	 */
	_chunkHashRank(chunk, chunkGraph) {
	...
	/**
	 * True when `referencedChunk` is guaranteed to be hashed before every chunk of the
	 * consuming module (see `_chunkHashRank`). Same-rank is only safe for runtime chunks,
	 * which `createHash` topologically orders so a referenced runtime chunk hashes first;
	 * async/initial/entry share a rank but are id-sorted, so their order isn't guaranteed.
	 * @param {Chunk} referencedChunk the chunk being referenced
	 * @param {Module} consumingModule the module the reference is emitted into
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {boolean} true when the reference is ordering-safe
	 */
	_isReferencedHashedFirst(referencedChunk, consumingModule, chunkGraph) {
		const refRank = this._chunkHashRank(referencedChunk, chunkGraph);
		consumingModule = this._getChunkBearingModule(consumingModule, chunkGraph);
		for (const chunk of chunkGraph.getModuleChunksIterable(consumingModule)) {
			const rank = this._chunkHashRank(chunk, chunkGraph);
			if (rank > refRank) continue;
			if (rank === refRank && rank === 2) continue;
			return false;
		}
		return true;
	}
	 ...
	 * or `import(<here>)` pointing at `chunk`'s JS file, or `null` when it can't be
	 * emitted — a hashed filename or templated publicPath that post-hash placeholder
	 * substitution can't embed cache-correctly, or a dynamic publicPath.
	 ...
	 * @param {Module} consumingModule the module the reference is emitted into
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @param {RuntimeRequirements=} runtimeRequirements filled with the full-hash marker when a `[fullhash]` publicPath is baked; omit during hashing (read-only)
	...
	_getAnalyzableChunkSpecifier(
		...
		let filename;
		if (HASH_IN_FILENAME.test(filenameTemplate)) {
			// The hash is unknown during code generation: bake a placeholder that
			// `substituteAnalyzableEsmPlaceholders` resolves at render time.
			if (
				chunk.id === null ||
				!this._canEmbedPostHash(consumingModule, chunkGraph)
			) {
				return null;
			}
			// The consuming chunk's hash must pick up the referenced chunk's hash so it
			// re-renders when the target filename changes (updateHashForAnalyzableEsmReferences).
			// That only works if the referenced chunk is hashed first — otherwise keep the
			// runtime form, whose `.u` map carries the dependency safely.
			if (!this._isReferencedHashedFirst(chunk, consumingModule, chunkGraph)) {
				return null;
			}
			filename = chunkFilenamePlaceholder(chunk.id);
		} else {
			filename = compilation.getPath(filenameTemplate, {
				chunk,
				contentHashType: JAVASCRIPT_TYPE
			});
		}
		return this._applyAnalyzablePublicPath(
			overridePublicPath,
			filename,
			consumingModule,
			chunkGraph,
	...
	/**
	 * Prefixes `filename` with output.publicPath (or a per-dependency override),
	 * returning the quoted JS literal: `auto` resolves relative to the consuming chunk
	 * (`import.meta.url` base), a static publicPath is a plain prefix, and a `[fullhash]`
	 * publicPath bakes the post-hash placeholder (adding the full-hash marker requirement
	 * when `runtimeRequirements` is given). Returns `null` for a bare/dynamic publicPath.
	 * @param {string | undefined} overridePublicPath per-dependency public path (wins over output.publicPath)
	 * @param {string} filename the resolved chunk/asset filename (or its post-hash placeholder)
	 * @param {Module} consumingModule the module the reference is emitted into
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @param {RuntimeRequirements=} runtimeRequirements filled with the full-hash marker for a `[fullhash]` publicPath; omit during hashing (read-only)
	 * @returns {string | null} a quoted JS string literal, or `null` to fall back
	 */
	_applyAnalyzablePublicPath(
		overridePublicPath,
		filename,
		consumingModule,
		chunkGraph,
		...
		const { publicPath } = this.compilation.outputOptions;
		if (publicPath === "auto") {
		...
		if (typeof publicPath === "string") {
			...
			// A publicPath resolvable from the full hash alone bakes the post-hash
			// placeholder; the full-hash marker makes the chunk re-render when it changes.
			if (isFullHashOnlyTemplate(publicPath)) {
				if (runtimeRequirements) {
					runtimeRequirements.add(RuntimeGlobals.analyzableEsmFullHash);
				}
				return toJsStringLiteral(PUBLIC_PATH_PLACEHOLDER + filename);
			}
		}
		return null;
	}
	 ...
	 * @param {Source} source the rendered chunk source
	 * @param {Chunk} chunk the chunk being rendered
	 * @returns {Source} the source with placeholders resolved
	 */
	substituteAnalyzableEsmPlaceholders(source, chunk) {
		const { compilation } = this;
		const { outputOptions } = compilation;
		// Skip the source materialization when no placeholder could have been baked.
		if (!outputOptions.module || !this._mayHavePlaceholders()) return source;
		const content = source.source();
		if (typeof content !== "string" || !content.includes(PLACEHOLDER_PREFIX)) {
			return source;
		}
		const { publicPath } = outputOptions;
		let chunkByToken = this._analyzableChunkByToken;
		if (chunkByToken === undefined) {
			chunkByToken = this._analyzableChunkByToken = new Map();
			for (const c of compilation.chunks) {
				if (c.id !== null) {
					chunkByToken.set(chunkFilenamePlaceholder(c.id), c);
				}
			}
		}
		...
		while ((match = PLACEHOLDER_REGEXP.exec(content)) !== null) {
			...
			if (token === PUBLIC_PATH_PLACEHOLDER) {
				// Emitted only for a `[fullhash]`-templated string publicPath.
				if (typeof publicPath === "string" && publicPath.includes("[")) {
					path = compilation.getPath(publicPath, { hash: compilation.hash });
				}
			} else {
				const referenced = chunkByToken.get(token);
				if (referenced !== undefined) {
					path = compilation.getPath(
						/** @type {string} */ (
							getJavascriptModulesPlugin().getChunkFilenameTemplate(
								referenced,
								outputOptions
			...
			// The token sits inside a double-quoted literal, so escape the fragment.
			newSource.replace(
				match.index,
		...
		return replaced ? newSource : source;
	}
	 ...
	 * @param {Hash} hash the chunk hash being built
	 * @param {Chunk} chunk the chunk being hashed
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {void}
	 */
	updateHashForAnalyzableEsmReferences(hash, chunk, chunkGraph) {
		const { outputOptions } = this;
		// No hashed chunk filename ⇒ no placeholder ⇒ no cross-chunk hash dependency.
		if (!outputOptions.module || !this._mayHavePlaceholders()) return;
		for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
			// Async blocks nest (an `import()` inside an `import()`), so walk the tree.
			...
			for (let i = 0; i < blocks.length; i++) {
				...
				for (const referenced of group.chunks) {
					if (referenced === chunk || referenced.renderedHash === undefined) {
						continue;
					}
					// Only a hashed target's filename changes independently of its content
					// in the literal, so only it needs the dependency.
					const template =
						getJavascriptModulesPlugin().getChunkFilenameTemplate(
							referenced,
							outputOptions
					...
					// Mirror emission: mix only when the reference is actually baked as a
					// chunk-filename placeholder (embeddable, resolvable, not a rejected
					// cycle). No `runtimeRequirements` — this is the read-only hashing phase.
					const specifier = this._getAnalyzableChunkSpecifier(
						undefined,
						referenced,
						module,
					...
					if (
						specifier !== null &&
						specifier.includes(CHUNK_PLACEHOLDER_PREFIX)
					) {
						hash.update(referenced.renderedHash);
					}
				}
			}
		}
