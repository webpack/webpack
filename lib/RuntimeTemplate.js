...
class RuntimeTemplate {
	...
	/**
	 * @param {string} filename the file name of the resource
	 * @returns {string} the resultant public path expression
	 */
	generatePublicPath(filename) {
		const publicPath = `${RuntimeGlobals.dynamicPublicPath} ? ${RuntimeGlobals.dynamicPublicPath}("${filename}", ${RuntimeGlobals.publicPath}) : (${this.concatenation({ expr: RuntimeGlobals.publicPath }, filename)})`;
		return publicPath;
	}
