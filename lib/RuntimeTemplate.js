...
class RuntimeTemplate {
	 ...
	 * @returns {boolean} true when the analyzable form is emitted
	 */
	supportsAnalyzableWasm() {
		const { publicPath, webassemblyModuleFilename } = this.outputOptions;
		// Only `auto` has a base every wasm loading backend agrees on: the node backends
		// resolve against `import.meta.url` and ignore `output.publicPath`.
		return (
			publicPath === "auto" &&
