...
class RuntimeTemplate {
	...
	_hashesBefore(chunk, target) {
		...
		// Once the marks are made, a chunk settling after the compilation hash is read
		// only by one settling later still. Before then — during code generation — the
		// answer is the one the marking will bring about, since baking a name here is
		// what settles the chunk it lands in late in turn.
	...
	_markChunksSettlingLate() {
		...
		// A name taken after the compilation hash takes the name it is baked into after
		// it too, and that one may be baked somewhere further up — so this runs to a
	...
	_foldAnalyzableNames(chunk, hash) {
		...
		if (folded === undefined) {
			...
			// Everything a stand-in in this chunk can name. An async entrypoint — a worker's
			// chunk — is reached by neither of the other two walks. Only a runtime chunk
			// writes out names it does not reference (`analyzableCssChunkUrls` reaches an
			// initial stylesheet), so only it pays for that third walk.
			const reachable = new Set(chunk.getAllReferencedChunks());
			for (const entrypoint of chunk.getAllReferencedAsyncEntrypoints()) {
				for (const referenced of entrypoint.chunks) reachable.add(referenced);
			}
			if (chunk.hasRuntime()) {
	...
	_analyzablePathPrefix(module, chunkGraph, chunks, relativeBase) {
		...
		if (climb === null) {
			// They disagree, but each of them knows its own answer, so the deferred pass
			// asks per asset — where each has one to give, and where nothing else has to go
			// between the two shapes the answer picks from.
