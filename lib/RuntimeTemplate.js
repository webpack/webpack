...
class RuntimeTemplate {
	...
	constructor(compilation, outputOptions, requestShortener) {
		...
		/** @type {boolean | undefined} */
		this._buildBakesFullHashText = undefined;
	...
	_partsBakeFullHash(parts) {
		for (const [kind, value] of parts) {
			if (kind === "publicPath" || kind === "template") return true;
	...
	getAnalyzableAssetUrl(module, chunkGraph, filename, runtime) {
		...
		if (base !== undefined) {
			...
			if (!isAbsoluteBaseUri(base)) {
				this._analyzableBailout(
					module,
					"the entry baseUri is protocol-relative, so its scheme is only known where the chunk is loaded"
				);
				return null;
			}
	...
	_hashesBefore(chunk, target) {
		...
		// A runtime chunk carrying full-hash modules is re-hashed after every round, and
		// only a runtime chunk can carry one, so no other hash may be read from it.
		if (chunk.hasRuntime()) return false;
		// One that bakes the compilation hash is settled after every round too. Asked of
		// the shape of the build rather than of the marks, which are not made until code
		// generation is over — so both phases get the same answer.
		if (
			this._buildBakesFullHash() &&
			!this._chunkNameIndependentOfContent(chunk)
		) {
			return false;
		}
	...
	/**
	 * Whether anything in this build can put the compilation hash into a chunk's bytes —
	 * a public path or a wasm filename built from it. Config alone, so it answers the
	 * same during code generation and during hashing.
	 * @returns {boolean} true when a stand-in may carry the compilation hash
	 */
	_buildBakesFullHash() {
		if (this._buildBakesFullHashText === undefined) {
			const { publicPath, webassemblyModuleFilename } = this.outputOptions;
			/**
			 * @param {string | undefined} path a configured path
			 * @returns {boolean} true when it names the compilation hash
			 */
			const carries = (path) =>
				typeof path === "string" &&
				getTemplatedPathPlugin().getPresentKinds(path).has("fullhash");
			this._buildBakesFullHashText =
				typeof publicPath === "function"
					? // One whose answer moves with the hash is called again by the fill, and
						// what it answers there is the compilation hash by another name.
						this._publicPathShape() !== undefined &&
						this._resolveHashIndependent(
							publicPath,
							this._publicPathShape()
						) === null
					: carries(publicPath) || carries(webassemblyModuleFilename);
		}
		return this._buildBakesFullHashText;
	}
	 ...
	 * @returns {void}
	 */
	_markChunksBakingFullHash() {
		this._chunksBakingFullHash = new Set();
		if (!this._buildBakesFullHash()) return;
		...
		for (const chunk of compilation.chunks) {
			...
			for (const module of modules) {
				...
				const source = results.getSource(
					module,
					chunk.runtime,
					JAVASCRIPT_TYPE
				);
				...
				if (
					...
					!this._sourceBakesFullHash(content)
				) {
					continue;
				}
	...
	/**
	 * Whether generated code will have the compilation hash written into it — either as a
	 * stand-in of its own, or inside one whose recipe is resolved against `output` once
	 * that hash exists.
	 * @param {string} content generated code
	 * @returns {boolean} true when the fill will put that hash here
	 */
	_sourceBakesFullHash(content) {
		if (this._hasReservedFullHash(content)) return true;
	...
	_foldAnalyzableNames(chunk, hash) {
		...
		let folded = bakesFullHash
			? undefined
			: this._foldedAnalyzableNames.get(chunk);
		if (folded === undefined) {
			const { compilation } = this;
			const { outputOptions } = compilation;
			// Everything a stand-in in this chunk can name. Only a runtime chunk writes
			// out names it does not reference — `analyzableCssChunkUrls` reaches an
			// initial stylesheet — so only it pays for that second walk.
			const reachable = chunk.hasRuntime()
				? new Set([
			...
			folded = names.join("\n");
			if (!bakesFullHash) this._foldedAnalyzableNames.set(chunk, folded);
		}
	...
	_getAnalyzableChunkSpecifier(
		...
		// A consuming chunk named by its own content bakes anyway when this one's hash is
		// settled first: `_foldAnalyzableNames` puts the name into that hash before it is
		// taken, so the two move together. Only a name built from this chunk alone —
		// `_foldsWholeName` below — is covered, the rest still needing a repair after.
	...
	_analyzablePathPrefix(module, chunkGraph, chunks, relativeBase) {
		...
		// What a chunk the loader fetched has to climb back out of. A public path of no
		// depth of its own — `auto`, and every relative one that stays at the output root
		// — leaves nothing to climb, and then it does not matter which way the asset was
		// served: both answers put the same text in front.
	...
	static fillReservedNames(compiler) {
		compiler.hooks.compilation.tap(PASS_NAME, (compilation) => {
			...
			compilation.hooks.beforeHash.tap(PASS_NAME, () => {
				compilation.runtimeTemplate._markChunksBakingFullHash();
			});
