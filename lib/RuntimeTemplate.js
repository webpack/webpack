...
class RuntimeTemplate {
		...
		for (const originChunk of chunkGraph.getModuleChunksIterable(
			...
			const chunkLoading =
				entryOptions.chunkLoading !== undefined
					...
					: outputOptions.workerChunkLoading;
			if (chunkLoading !== "import") {
				return null;
			}
		}
	...
	_getAnalyzableChunkSpecifier(
		...
		if (publicPath === "auto") {
			...
			// Chunks at different depths hold the consumer, so no single relative literal
			// works. A `new URL(...)` caller can still name the file behind the runtime
			// base; `import()` needs the whole specifier static, so it gets nothing.
