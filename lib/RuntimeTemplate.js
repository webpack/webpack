...
class RuntimeTemplate {
	 ...
	 * and webpack itself can follow, wrapped to keep `ensureChunk` timing and dedup.
	...
	_analyzableChunkImport(
		...
		// Drop-in for `ensureChunk(id)`: the literal `import()` is statically followable,
		// the helper keeps webpack's install timing and dedup (see the runtime module).
		return `${RuntimeGlobals.analyzableChunkImport}(${JSON.stringify(
			chunk.id
		)}, () => import(${comment}${JSON.stringify(base + filename)}))`;
	}
