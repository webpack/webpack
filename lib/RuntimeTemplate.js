...
class RuntimeTemplate {
	...
	constructor(compilation, outputOptions, requestShortener) {
		...
		/** @type {boolean} */
		this._analyzableRepairNeeded = false;
		...
		this._hotUpdateUndoText = undefined;
	...
	needsAnalyzableRepair() {
		return this._analyzableRepairNeeded;
	}
	...
	_hotUpdateKeepsDepth(chunks) {
		if (this._hotUpdateUndoText === undefined) {
			const { outputOptions } = this;
			// Its hashes are neutralized as a chunk's are; the id is the chunk's own.
			this._hotUpdateUndoText = getUndoPath(
		...
		for (const chunk of chunks) {
			const { undo } = this._chunkPlacement(chunk);
			if (undo !== null && undo !== this._hotUpdateUndoText) return false;
		}
		return true;
	}
		...
		compiler.hooks.compilation.tap(PASS_NAME, (compilation) => {
			...
			compilation.hooks.processAssets.tapPromise(
				...
				async () => {
					...
					// Whether any stand-in landed in a chunk whose hashed name has to be
					// brought back in line, which `RealContentHashPlugin` then does.
					...
					compilation.runtimeTemplate._analyzableRepairNeeded = repairNeeded;
