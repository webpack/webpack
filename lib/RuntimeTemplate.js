...
class RuntimeTemplate {
	...
	templateString(strings, runtimeValues) {
		const lastIndex = strings.length - 1;
		return this.supportTemplateLiteral()
			? `\`${strings
					.map((str, i) =>
						i === lastIndex ? str : `${str}\${${runtimeValues[i]}}`
					)
					.join("")}\``
			: this._es5String(strings, runtimeValues);
	}

	_es5String(strings, runtimeValues) {
		let str = "";
		// for cases like `${"num"}` where `num` is type number at runtime
		let hasString = false;
		const lastIndex = strings.length - 1;

		for (let i = 0; i < lastIndex; i++) {
			const _str = strings[i];
			if (_str.length) {
				hasString = true;
				str += `${str.length === 0 ? "" : " + "}${JSON.stringify(_str)}`;
			}

			str += `${str.length === 0 ? "" : " + "}${runtimeValues[i]}`;
		}

		const _str = strings[strings.length - 1];
		if (_str.length || !hasString)
			str += `${str.length === 0 ? "" : " + "}${JSON.stringify(_str)}`;
