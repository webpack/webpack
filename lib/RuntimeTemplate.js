 ...
 * 	SpecifierPart,
 * 	SpecifierPartKind
 * } from "./esm/AnalyzableChunkHashPlugin"
 */
...
class RuntimeTemplate {
	...
	analyzableBailout(module, reason) {
		this._analyzableBailout(module, reason);
	}
	...
	getAnalyzableAssetUrl(module, chunkGraph, filename, runtime) {
		...
		if (base !== undefined) {
			...
			const text =
				spelled !== null &&
				!getAnalyzableChunkHashPlugin().hasReservedFullHash(spelled)
					? spelled
					: null;
			/** @type {URL} */
			let resolved;
	...
	importStatement({
		...
		if (isModuleDeferred) {
			...
			importContent = `/* deferred harmony import */ ${optDeclaration}${importVar} = /*#__PURE__*/${getOptimizedDeferredModule(
				moduleId,
				exportsType,
	...
	/**
	 * Whether this chunk's name sorts before every chunk a stand-in would land in, the
	 * tie-break that lets one direction of a cycle bake: ids are unique and settled
	 * here, so the order is total and the same in every build.
	...
	_namesBeforeAll(chunk, targets) {
		...
		for (const target of targets) {
			if (target.id === null || String(target.id) <= key) return false;
		}
		return true;
	}
	...
	_resolvePublicPathPrefix(publicPath, module, chunks) {
		if (typeof publicPath === "function") {
			...
			if (this._canDeferAnalyzableName(chunks)) return [["publicPath", ""]];
			this._analyzableBailout(module, DEFER_BAILOUT);
			return null;
		}
