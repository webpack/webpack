...
/**
 * Tarjan strongly-connected components over the chunk-group graph (children edges);
 * groups on a common cycle share a component id. Used to reject mutual post-hash
 * embeds that would make realContentHash's hash-dependency graph circular.
 * @param {ChunkGroup[]} chunkGroups all chunk groups of the compilation
 * @returns {Map<ChunkGroup, number>} group → component id
 */
const computeChunkGroupScc = (chunkGroups) => {
	/** @type {Map<ChunkGroup, number>} */
	const index = new Map();
	/** @type {Map<ChunkGroup, number>} */
	const low = new Map();
	/** @type {Map<ChunkGroup, number>} */
	...
	let nextIndex = 0;
	let nextComponent = 0;
	for (const root of chunkGroups) {
		if (index.has(root)) continue;
		/** @type {[ChunkGroup, Iterator<ChunkGroup>][]} */
		const frames = [[root, root.childrenIterable[Symbol.iterator]()]];
		...
		while (frames.length > 0) {
			const [group, iterator] = frames[frames.length - 1];
			...
			if (!step.done) {
				...
				if (childIndex === undefined) {
					...
					stack.push(child);
					onStack.add(child);
					frames.push([child, child.childrenIterable[Symbol.iterator]()]);
				} else if (
					onStack.has(child) &&
					childIndex < /** @type {number} */ (low.get(group))
				) {
					low.set(group, childIndex);
				}
			} else {
				frames.pop();
				const lowGroup = /** @type {number} */ (low.get(group));
				if (frames.length > 0) {
					const parent = frames[frames.length - 1][0];
					if (lowGroup < /** @type {number} */ (low.get(parent))) {
						low.set(parent, lowGroup);
					}
				}
				if (lowGroup === index.get(group)) {
					const component = nextComponent++;
					/** @type {ChunkGroup} */
					let member;
					do {
						member = /** @type {ChunkGroup} */ (stack.pop());
...
class RuntimeTemplate {
	...
	constructor(compilation, outputOptions, requestShortener) {
		...
		/** @type {Map<ChunkGroup, number> | undefined} */
		this._chunkGroupScc = undefined;
	...
	/**
	 * Whether content substituted after hashing (a post-hash placeholder) may be
	 * embedded into the consuming module's chunks without breaking long-term caching:
	 * every containing chunk's filename must either carry no hash, change with the
	 * full hash, or be re-hashed from emitted content (`[contenthash]` +
	 * `optimization.realContentHash`). A `[chunkhash]` name would stay stable while
	 * the embedded path changes, so it blocks embedding.
	 * @param {Module} consumingModule the module the reference is emitted into
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {0 | 1 | 2} 0 = not embeddable, 1 = embeddable, 2 = embeddable and a containing chunk is `[contenthash]`-named
	 */
	_getPostHashEmbedMode(consumingModule, chunkGraph) {
		const { compilation } = this;
		const { outputOptions } = compilation;
		/** @type {0 | 1 | 2} */
		let mode = 1;
		...
		if (chunkGraph.getNumberOfModuleChunks(consumingModule) === 0) return 0;
		for (const chunk of chunkGraph.getModuleChunksIterable(consumingModule)) {
			...
			if (!HASH_IN_FILENAME.test(template)) continue;
			if (FULLHASH_IN_FILENAME.test(template)) continue;
			if (CHUNKHASH_IN_FILENAME.test(template)) return 0;
			if (!compilation.options.optimization.realContentHash) return 0;
			mode = 2;
		}
		return mode;
	}
	...
	/**
	 * True when `chunk` shares a chunk-group cycle with a chunk of the consuming
	 * module: mutual `[contenthash]` embeds on such a cycle would make
	 * realContentHash's hash-dependency graph circular (a build error).
	 * @param {Chunk} chunk the referenced chunk
	 * @param {Module} consumingModule the module the reference is emitted into
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {boolean} true when on a common cycle
	 */
	_isOnChunkGroupCycle(chunk, consumingModule, chunkGraph) {
		let scc = this._chunkGroupScc;
		if (scc === undefined) {
			scc = this._chunkGroupScc = computeChunkGroupScc(
				this.compilation.chunkGroups
			);
		}
		consumingModule = this._getChunkBearingModule(consumingModule, chunkGraph);
		for (const group of chunk.groupsIterable) {
			const component = scc.get(group);
			for (const originChunk of chunkGraph.getModuleChunksIterable(
				consumingModule
			)) {
				for (const originGroup of originChunk.groupsIterable) {
					if (scc.get(originGroup) === component) return true;
				}
			}
		}
		return false;
	...
	_getAnalyzableChunkSpecifier(
		...
		// Lazily computed — only placeholder emission needs it.
		/** @type {-1 | 0 | 1 | 2} */
		let embedMode = -1;
		const getEmbedMode = () =>
			embedMode === -1
				? (embedMode = this._getPostHashEmbedMode(consumingModule, chunkGraph))
				: embedMode;
		let filename;
		if (HASH_IN_FILENAME.test(filenameTemplate)) {
			...
			if (chunk.id === null || getEmbedMode() === 0) return null;
			if (
				getEmbedMode() === 2 &&
				CONTENTHASH_IN_FILENAME.test(filenameTemplate) &&
				this._isOnChunkGroupCycle(chunk, consumingModule, chunkGraph)
			) {
				return null;
			}
	...
	substituteAnalyzableEsmPlaceholders(source, chunk) {
		const { compilation } = this;
		const { outputOptions } = compilation;
		if (!outputOptions.module) return source;
	...
	updateHashForAnalyzableEsmReferences(hash, chunk, chunkGraph) {
		const { outputOptions } = this;
		if (!outputOptions.module) return;
		for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
