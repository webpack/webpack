...
// Source types a chunk carries that nothing loads through an `ensureChunkHandlers`
// entry: javascript and runtime ride the chunk itself, and the rest are rendered into
// assets of their own that no `.f` handler ever fetches. Everything else may install
// one (`css`, sync `webassembly`, `remote`, `consume-shared`, or a plugin's own type),
// so the map has to exist for it.
/** @type {Set<string>} */
...
class RuntimeTemplate {
	...
	_canDeferAnalyzableName(chunks) {
		...
		for (const chunk of chunks) {
			...
			// A function template could name a chunk anything, so it counts as one that
			// takes its name from its own content.
			if (typeof template !== "string") return false;
			const kinds = templatedPathPlugin.getPresentKinds(template);
			...
			if (kinds.has("chunkhash")) return false;
			if (
