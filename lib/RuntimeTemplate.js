...
class RuntimeTemplate {
	...
	getBuiltinModule(request, access = "") {
		...
		const importMetaName = this.outputOptions.importMetaName;
		const moduleId = this.renderNodePrefixForCoreModule("module");
		const createRequire = `require(${moduleId}).createRequire(${importMetaName}.url)(${request})${access}`;
		const body = `try { return ${getter}; } catch (e) {} try { return ${createRequire}; } catch (e) {}`;
		const fn = this.supportsArrowFunction()
			? `() => { ${body} }`
			: `function () { ${body} }`;
		return `typeof process !== "undefined" && (${fn})()`;
	}
