...
class RuntimeTemplate {
	...
	constructor(compilation, outputOptions, requestShortener) {
		...
		/** @type {boolean} */
		this.specNamespaceObjects = Boolean(
			compilation &&
			compilation.options.experiments &&
			compilation.options.experiments.specNamespaceObjects
		);
	...
	supportsAnalyzable(form, chunkGraph, module, runtime) {
		...
		if (form === "import") {
			...
			if (outputOptions.chunkFormat !== "module") {
				return this._analyzableBailout(
					module,
					`output.chunkFormat is ${JSON.stringify(
						outputOptions.chunkFormat
					)}, so chunks are not read through a native import()`,
					false
				);
			}
			if (outputOptions.importFunctionName !== "import") {
				return this._analyzableBailout(
					module,
					`output.importFunctionName is ${JSON.stringify(
						outputOptions.importFunctionName
					)}, so the call site is not a native import()`,
					false
				);
			}
			...
			for (const originChunk of /** @type {ChunkGraph} */ (
				...
				if (entryOptions.chunkLoading !== "import") {
					return this._analyzableBailout(
						module,
						`this worker loads its chunks with ${JSON.stringify(
		...
		if (
			...
			return this._analyzableBailout(
				...
				`devtool ${JSON.stringify(
					devtool
				)} wraps the module in eval(), where import.meta does not parse`,
				false
			);
		}
	...
	getAnalyzableAssetUrl(module, chunkGraph, filename, runtime) {
		...
		if (!isAbsoluteBaseUri(base)) {
			return this._analyzableBailout(
				module,
				`an entry sets a baseUri of ${JSON.stringify(
					base
				)}, whose scheme names no base anything can be resolved against`,
				null
			);
		}
		...
		try {
			...
			return this._analyzableBailout(
				...
				`an entry sets a baseUri of ${JSON.stringify(
					base
				)}, which is not absolute, so no url can be resolved against it here`,
				null
			);
		}
	...
	moduleNamespace({
		...
		switch (exportsType) {
			case "namespace": {
				...
				if (!this.specNamespaceObjects) return raw;
	...
	exportFromImport({
		...
		if (
			...
			if (materialized !== undefined) {
				if (!this.specNamespaceObjects || exportName.length > 0) {
					return materialized;
				}
				runtimeRequirements.add(RuntimeGlobals.specNamespaceObject);
				return `${RuntimeGlobals.specNamespaceObject}(${materialized})`;
			}
		}
