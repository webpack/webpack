	...
	concatenation(...args) {
		...
		if (len === 1)
			return typeof args[0] === "string"
				...
				: this.supportTemplateLiteral()
				? `\`\${${args[0].expr}}\``
				: `"" + ${args[0].expr}`;
		if (!this.supportTemplateLiteral()) return this._es5Concatenation(args);

		const numberOfExpressions = args.reduce(
			(acc, arg) => (typeof arg === "string" ? acc : acc + 1),
			0
		);

		if (numberOfExpressions >= Math.round(len / 2))
			return this._es5Concatenation(args);
