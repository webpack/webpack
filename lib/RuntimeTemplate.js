...
class RuntimeTemplate {
	...
	supportsAnalyzable(form, chunkGraph, module, runtime) {
		...
		if (
			getTemplatedPathPlugin()
				...
				.has("fullhash") &&
			!this._canDeferOrBail(undefined, module)
		) {
			return false;
		}
		return true;
	...
	/**
	 * Whether everything in front of a chunk's filename is text the consuming chunk
	 * already carries, so folding that filename in accounts for the whole stand-in. A
	 * public path, a template or an `undo` resolves against the compilation hash or the
	 * consuming asset's own name, neither of which exists while that chunk is hashed.
	...
	_foldAnalyzableNames(chunk, hash) {
		...
		let folded = this._foldedAnalyzableNames.get(chunk);
		if (folded === undefined) {
			...
			folded = names.join("\n");
			this._foldedAnalyzableNames.set(chunk, folded);
		}
	...
	_getAnalyzableChunkSpecifier(
		...
		const specifier = (prefix) => {
			...
			if (!canReserve && !(canFold && this._foldsWholeName(prefix))) {
	...
	_resolvePublicPathPrefix(publicPath, module, chunks) {
		if (typeof publicPath === "function") {
			...
			return this._canDeferOrBail(chunks, module) ? [["publicPath", ""]] : null;
		}
		...
		return [
			[
				"literal",
				this.compilation.getPath(publicPath, this._deferredFullHashPathData())
			]
		];
	}
