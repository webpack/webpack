...
const getAnalyzableChunkNaming = memoize(() =>
	require("./esm/analyzableChunkNaming")
);
 ...
 * 	SpecifierPart,
 * 	SpecifierPartKind
 * } from "./esm/analyzableChunkNaming"
 */
...
class RuntimeTemplate {
	...
	getAnalyzableAssetUrl(module, chunkGraph, filename, runtime) {
		...
		if (base !== undefined) {
			...
			const text =
				spelled !== null &&
				!getAnalyzableChunkNaming().hasReservedFullHash(spelled)
					? spelled
					: null;
			/** @type {URL} */
			let resolved;
			...
			return toJsStringLiteral(
				getAnalyzableChunkNaming().reserveSpecifier([
					["base", base],
					...parts,
					["literal", filename]
				])
	...
	_resolvePublicPathPrefix(publicPath, module, chunks) {
		...
		const analyzableChunkNaming = getAnalyzableChunkNaming();
		if (!analyzableChunkNaming.canDeferFullHash(publicPath)) {
		...
		return [
			[
				...
				this.compilation.getPath(
					publicPath,
					analyzableChunkNaming.DEFERRED_FULL_HASH_PATH_DATA
				)
			]
		];
	}
	...
	_getAnalyzableFileSpecifier(
		...
		const specifier = (parts) => {
			...
			return toJsStringLiteral(
				getAnalyzableChunkNaming().reserveSpecifier(parts)
			);
		};
		...
		return this.importMetaUrl(
			`${RuntimeGlobals.publicPath} + ${toJsStringLiteral(
				compilation.getPath(template, {
					...
					runtime,
					chunkGraph,
					...getAnalyzableChunkNaming().DEFERRED_FULL_HASH_PATH_DATA
				})
			)}`
		);
	}
