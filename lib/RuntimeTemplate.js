...
class RuntimeTemplate {
	...
	supportsPromise() {
		return this.outputOptions.environment.promise;
	}
	...
	missingModulePromise({ request }) {
		...
		}.resolve().then(${this.throwMissingModuleErrorFunction({
			request
		})})`;
	}
	...
	weakError({ module, chunkGraph, request, idExpr, type }) {
		...
		switch (type) {
			...
			case "promise":
				return `${RuntimeGlobals.promise}.resolve().then(${this.basicFunction(
					"",
					errorStatements
				)})`;
			case "expression":
	...
	moduleNamespacePromise({
		...
		return `${promise || `${RuntimeGlobals.promise}.resolve()`}${appending}`;
	}
	...
	blockPromise({ block, message, chunkGraph, runtimeRequirements }) {
		if (!block) {
			...
			runtimeRequirements.add(RuntimeGlobals.promise);
			return `${RuntimeGlobals.promise}.resolve(${comment.trim()})`;
		}
		...
		if (chunks.length === 1) {
			...
			return `${RuntimeGlobals.promise}.all(${comment.trim()}[${chunks
				.map(requireChunkId)
				.join(", ")}])`;
		} else {
			runtimeRequirements.add(RuntimeGlobals.promise);
	...
	asyncModuleFactory({ block, chunkGraph, runtimeRequirements, request }) {
		...
		return this.returningFunction(
			ensureChunk.startsWith(`${RuntimeGlobals.promise}.resolve(`)
				? `${factory}`
