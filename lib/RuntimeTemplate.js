...
// Post-hash placeholder: a hashed chunk filename is unknown during code generation, so
// the analyzable ESM literal is baked with this token and `substituteAnalyzableEsmPlaceholders`
// rewrites it at JS render time (after hashing, before assets are emitted). The consuming
// chunk mixes the referenced chunk's hash into its own (see `updateHashForAnalyzableEsmReferences`),
// so it re-renders when the target's filename changes. The charset is string-literal-safe.
const PLACEHOLDER_PREFIX = "___WEBPACK_ANALYZABLE_CHUNK_";
// The chunk payload charset excludes `_`, keeping the `___` delimiters unambiguous.
const PLACEHOLDER_REGEXP = /___WEBPACK_ANALYZABLE_CHUNK_[A-Za-z0-9.-]+___/g;
...
const chunkFilenamePlaceholder = (chunkId) =>
	`${PLACEHOLDER_PREFIX}${Buffer.from(JSON.stringify(chunkId))
		.toString("base64")
		.replace(/\+/g, "-")
		.replace(/\//g, ".")
		.replace(/[=]+$/, "")}___`;
...
class RuntimeTemplate {
	 ...
	 * returning the quoted JS literal: `auto` resolves relative to the consuming
	 * chunk (`import.meta.url` base), a static publicPath is a plain prefix. Returns
	 * `null` for a bare/templated/dynamic publicPath the static literal can't carry.
	 * @param {string | undefined} overridePublicPath per-dependency public path (wins over output.publicPath)
		...
		for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
			...
			for (let i = 0; i < blocks.length; i++) {
				...
				for (const referenced of group.chunks) {
					...
					// placeholder (embeddable, resolvable, not a rejected cycle).
					const specifier = this._getAnalyzableChunkSpecifier(
						undefined,
						referenced,
						module,
					...
					if (specifier !== null && specifier.includes(PLACEHOLDER_PREFIX)) {
