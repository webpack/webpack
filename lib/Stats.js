...
const SizeFormatHelpers_1 = require("./SizeFormatHelpers");
class Stats {
	...
	static jsonToString(obj, useColors) {
				...
				t.push([
					{
						...
						value: SizeFormatHelpers_1.formatSize(asset.size),
		...
		function processModuleAttributes(module) {
			colors.normal(" ");
			colors.normal(SizeFormatHelpers_1.formatSize(module.size));
			if(module.chunks) {
		...
		if(obj.chunks) {
			obj.chunks.forEach(chunk => {
				...
				colors.normal(" ");
				colors.normal(SizeFormatHelpers_1.formatSize(chunk.size));
