				...
				for (const asset of chunk.files) {
					if (assetsByFile[asset]) {
						...
						if (chunk.name) {
							...
							if (obj.assetsByChunkName[chunk.name]) obj.assetsByChunkName[chunk.name] = [].concat(obj.assetsByChunkName[chunk.name]).concat([asset]);
		...
		const processModuleContent = (module, prefix) => {
			...
			if (module.usedExports !== undefined) {
				if (module.usedExports !== true) {
					...
					else if (Array.isArray(module.usedExports) && module.usedExports.length === 0) colors.cyan("[no exports used]");
					else if (Array.isArray(module.usedExports)) colors.cyan(`[only some exports used: ${module.usedExports.join(", ")}]`);
					newline();
				}
			}
