...
class Stats {
	...
	toJson(options, forToString) {
		...
		const excludeModules = [].concat(optionOrFallback(options.exclude, [])).map(testAgainstGivenOption);
		const excludeAssets = [].concat(optionOrFallback(options.excludeAssets, [])).map(testAgainstGivenOption);
		...
		if(obj.assets && obj.assets.length > 0) {
			...
			if(obj.filteredAssets > 0) {
				...
				if(obj.assets.length > 0)
					filteredAssets += "+ ";
				filteredAssets += obj.filteredAssets;
				if(obj.assets.length > 0)
					filteredAssets += " hidden";
				filteredAssets += obj.filteredAssets !== 1 ? " assets" : " asset";

				t.push([{
					value: filteredAssets,
					color: colors.normal
				}, emtpyCell, emtpyCell, emtpyCell, emtpyCell, emtpyCell]);
			}
			table(t, "rrrlll");
		}
		if(obj.entrypoints) {
