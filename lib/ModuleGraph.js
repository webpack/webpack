...
class ModuleGraph {
	constructor() {
		...
		/**
		 * @type {{ oldModule?: Module, newModule: Module, movedOut?: ModuleGraphConnection[] | undefined, movedIn?: ModuleGraphConnection[] | undefined, copied?: ModuleGraphConnection[], retargetDependency?: Dependency, oldConnection?: ModuleGraphConnection, retargetConnection?: ModuleGraphConnection, oldConditional?: boolean, oldActive?: boolean, oldMetaSymbols?: [symbol, EXPECTED_ANY][], oldSourceOrder?: DependencySourceOrder }[] | undefined}
		 * @private
		 */
	...
	updateModule(dependency, module) {
		...
		if (this._mutationJournal) {
			// seal-time retarget (SideEffectsFlagPlugin): record everything the
			// paired updateParent/setIds calls will mutate, so the durable graph
			// can be restored (meta holds the pre-setIds ids override)
			const meta = this._metaMap.get(dependency);
			this._mutationJournal.push({
				newModule: module,
				retargetDependency: dependency,
				oldConnection: connection,
				retargetConnection: newConnection,
				oldConditional: connection.conditional,
				oldActive: connection._active,
				oldMetaSymbols: meta
					? Object.getOwnPropertySymbols(meta).map((s) => [s, meta[s]])
					: undefined,
				oldSourceOrder: this._dependencySourceOrderMap.get(dependency)
			});
		}
	...
	/**
	 * Removes the connection of `dependency` from both sides, tolerating a
	 * null origin (entry/seed dependencies). Used before re-factorizing an
	 * existing dependency so the reused graph doesn't accumulate a connection
	 * per rebuild.
	 * @param {Dependency} dependency the dependency about to be re-factorized
	 * @returns {void}
	 */
	_removeConnectionLoose(dependency) {
		const connection = this.getConnection(dependency);
		// delete (not null): null is getConnection's negative cache and would
		// hide the re-factorized connection from the lazy by-parent flush. A miss
		// leaves one behind, so clear it on that path too.
		if (connection === undefined) {
			this._dependencyMap.delete(dependency);
			return;
		}
		if (connection.module) {
			this._getModuleGraphModule(connection.module).incomingConnections.delete(
				connection
			);
		}
		if (connection.originModule) {
			const outgoing = this._getModuleGraphModule(
				connection.originModule
			).outgoingConnections;
			if (outgoing !== undefined) outgoing.delete(connection);
		}
		this._dependencyMap.delete(dependency);
	}

	/**
	 * Removes null-origin incoming connections of `module` created by a previous
	 * seeding of the same dependency kind (prefetch chains re-seeded each make),
	 * keeping `keepDependency`'s connection. Prevents unbounded accumulation on a
	 * reused graph.
	 * @param {Module} module the re-seeded module
	 * @param {Dependency[]} keepDependencies this make's seeding dependencies
	 * @returns {void}
	 */
	_removeStaleSeedConnections(module, keepDependencies) {
		const mgm = this._getModuleGraphModule(module);
		for (const connection of mgm.incomingConnections) {
			const dependency = connection.dependency;
			if (
				(connection.originModule === null ||
					connection.originModule === undefined) &&
				dependency !== null &&
				dependency !== undefined &&
				dependency.constructor === keepDependencies[0].constructor &&
				!keepDependencies.includes(dependency)
			) {
				mgm.incomingConnections.delete(connection);
				this._dependencyMap.set(dependency, null);
			}
		}
	}
	...
	_restoreFromMutationJournal() {
		...
		for (let i = journal.length - 1; i >= 0; i--) {
			...
			const retargetDependency = journal[i].retargetDependency;
			if (retargetDependency !== undefined) {
				const entry = journal[i];
				const retargetConnection = /** @type {ModuleGraphConnection} */ (
					entry.retargetConnection
				);
				const oldConnection = /** @type {ModuleGraphConnection} */ (
					entry.oldConnection
				);
				if (retargetConnection.module) {
					this._getModuleGraphModule(
						retargetConnection.module
					).incomingConnections.delete(retargetConnection);
				}
				if (retargetConnection.originModule) {
					const outgoing = this._getModuleGraphModule(
						retargetConnection.originModule
					).outgoingConnections;
					if (outgoing !== undefined) outgoing.delete(retargetConnection);
				}
				this._dependencyMap.set(retargetDependency, oldConnection);
				oldConnection.conditional = /** @type {boolean} */ (
					entry.oldConditional
				);
				oldConnection._active = /** @type {boolean} */ (entry.oldActive);
				const meta = this._metaMap.get(retargetDependency);
				if (meta) {
					for (const symbol of Object.getOwnPropertySymbols(meta)) {
						delete meta[symbol];
					}
					if (entry.oldMetaSymbols) {
						for (const [symbol, value] of entry.oldMetaSymbols) {
							meta[symbol] = value;
						}
					}
				}
				if (entry.oldSourceOrder === undefined) {
					this._dependencySourceOrderMap.delete(retargetDependency);
				} else {
					this._dependencySourceOrderMap.set(
						retargetDependency,
						entry.oldSourceOrder
					);
				}
				continue;
			}
