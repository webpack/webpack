...
class ModuleGraph {
	constructor() {
		...
		// experiments.incremental: when set, moveModuleConnections records its moves
		// here so seal-time graph mutations (concatenation) can be reversed, keeping
		// this durable graph reusable across rebuilds
		/**
		 * @type {{ oldModule: Module, newModule: Module, movedOut: ModuleGraphConnection[] | undefined, movedIn: ModuleGraphConnection[] | undefined }[] | undefined}
		 * @private
		 */
	 ...
	 * @returns {void}
	 */
	restoreFromMutationJournal() {
		const journal = this._mutationJournal;
		if (journal === undefined) return;
		for (let i = journal.length - 1; i >= 0; i--) {
			const { oldModule, newModule, movedOut, movedIn } = journal[i];
			const oldMgm = this._getModuleGraphModule(oldModule);
			const newMgm = this._getModuleGraphModule(newModule);
			if (movedOut !== undefined) {
				...
				for (const connection of movedOut) {
					connection.originModule = oldModule;
