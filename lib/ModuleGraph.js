...
// Defer this require() because it causes a cycle
let CommonJsSelfReferenceDependency = () => {
	CommonJsSelfReferenceDependency = () =>
		require("./dependencies/CommonJsSelfReferenceDependency");
	return CommonJsSelfReferenceDependency();
};
...
class ModuleGraph {
	...
	isDeferred(module) {
		...
		for (const connection of connections) {
			if (connection.dependency instanceof CommonJsSelfReferenceDependency())
				continue;
			if (!connection.dependency.defer) return false;
		}
		return true;
	}
