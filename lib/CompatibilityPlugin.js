				...
				const handler = (parser) => {
					...
					parser.hooks.expression
						...
						.tap(PLUGIN_NAME, (expr) => {
							const tagData = /** @type {CompatibilitySettings} */ (
								parser.currentTagData
							);
							const { name, declaration } = tagData;

							const exprRange = /** @type {Range} */ (expr.range);
							const { options: commentOptions } = parser.parseCommentOptions([
								Math.max(0, exprRange[0] - 50),
								exprRange[0]
							]);

							if (commentOptions && commentOptions.webpackNoRename === true) {
								return;
							}
