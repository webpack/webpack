		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser) => {
							...
							if (
								...
								parser.tagVariable(
									...
									nestedWebpackIdentifierTag,
									{
										...
										declaration: {
											updated: false,
											loc: parser.getLocation(statement.id),
					...
					parser.hooks.pattern
						...
						.tap(PLUGIN_NAME, (pattern) => {
							...
							parser.tagVariable(pattern.name, nestedWebpackIdentifierTag, {
								...
								declaration: {
									updated: false,
									loc: parser.getLocation(pattern),
					 ...
					 */
					const declaratorHandler = (declarator) => {
						if (
					...
					parser.hooks.declarator.tap(PLUGIN_NAME, declaratorHandler);
					// mirrors the handler's bail conditions on the columns
					parser.registerColumnGate(
						parser.hooks.declarator,
						declaratorHandler,
						(_parser, store, did) => {
							const patId = store.kid0[did];
							if (store.types[patId] !== SoaAst.TYPE_IDENTIFIER) return false;
							const name = parser._soaIdentName(store, patId);
							return (
								(name === RuntimeGlobals.exports ||
									name === RuntimeGlobals.require) &&
								parser.getTagData(name, nestedWebpackIdentifierTag) !==
									undefined
							);
						}
					);
