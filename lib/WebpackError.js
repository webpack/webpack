...
class WebpackError extends Error {
	...
	/**
	 * @param {ObjectSerializerContext} context Serialization context
	 */
	serialize({ write }) {
		write(this.name);
		write(this.message);
	...
	/**
	 * @param {ObjectDeserializerContext} context Deserialization context
	 */
	deserialize({ read }) {
		this.name = read();
		this.message = read();
		...
		if (errorMessages) {
			this.errors = errorMessages.map(message => new Error(message));
		}
	}
}
