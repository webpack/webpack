...
const EntryDependency = require("../entry/EntryDependency");
const RawModule = require("../module/RawModule");

/** @import Compiler from "../Compiler" */
/** @import { ResolveData } from "../module/NormalModuleFactory" */
/** @import { BeforeContextResolveData } from "../context/ContextModuleFactory" */

/**
 * @inline
 * @tsType import('./IgnorePlugin').CheckResourceFn
 * @typedef {(resource: string, context: string) => boolean} CheckResourceFn
 */

/**
 * @inline
 * @typedef {object} IgnorePluginPatternOptions
 * @property {RegExp=} contextRegExp A RegExp to test the context (directory) against.
 * @property {RegExp} resourceRegExp A RegExp to test the request against.
 */

/**
 * @inline
 * @typedef {object} IgnorePluginCheckOptions
 * @property {CheckResourceFn} checkResource A filter function for resource and context.
 */

/**
 * @schema plugins/IgnorePlugin
 * @typedef {IgnorePluginPatternOptions | IgnorePluginCheckOptions} IgnorePluginOptions
 */
	...
	apply(compiler) {
		compiler.hooks.validate.tap(PLUGIN_NAME, () => {
			compiler.validate(
				...
				(options) =>
					require("../../schemas/plugins/IgnorePlugin.check")(options)
			);
		});
