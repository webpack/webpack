	...
	apply(mainTemplate) {
		...
		mainTemplate.plugin("jsonp-script", function(_, chunk, hash) {
            ...
            return this.asString([
                ...
                `xhr.timeout = ${chunkLoadTimeout};`,
                "function sendError() {",
                "if(chunk !== 0) {",
                this.indent([
                    "if(chunk) {",
                    this.indent("chunk[1](new Error('Loading chunk ' + chunkId + ' failed, code: ' + event.target.status));"),
                    "}",
                ...
                "(event.target.status === 200) ? (script.textContent = event.target.response) : sendError();",
                "};",
                ...
                "sendError();",
                "};",
                `xhr.open('get', ${this.requireFn}.p + ${scriptSrcPath}, true);`,
                "xhr.send();",
            ]);
		});
