		...
		mainTemplate.plugin("bootstrap", function(source, chunk, hash) {
			if(chunk.chunks.length > 0) {
				...
				return this.asString([
					...
					`var parentJsonpFunction = self[${JSON.stringify(jsonpFunction)}];`,
					`self[${JSON.stringify(jsonpFunction)}] = function webpackJsonpCallback(chunkIds, moreModules, executeModules) {`,
					this.indent([
...
self[${JSON.stringify(hotUpdateFunction)}] = ${runtimeSource}`;
		});
