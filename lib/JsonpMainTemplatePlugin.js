...
JsonpMainTemplatePlugin.prototype.apply = function(mainTemplate) {
	...
	mainTemplate.plugin("require-ensure", function(_, chunk, hash) {
		...
		return this.asString([
			...
			"",
			"// signal for \"error on load\"",
			"if(chunkLoadErrors[chunkId] !== undefined)",
			this.indent("return callback.call(null, " + this.requireFn + ", chunkLoadErrors[chunkId]);"),
			"",
				...
				this.indent([
					...
					this.indent([
						...
						this.indent([
							...
							"",
							"if (callbacks) for (var i = 0; callbacks.length > i; i += 1) {",
							this.indent([
								"callbacks[i].call(null, " + this.requireFn + ", chunkLoadErrors[chunkId]);"
							]),
							"}"
						]),
						"}",
