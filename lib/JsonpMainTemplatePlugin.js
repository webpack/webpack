...
class JsonpMainTemplatePlugin {
	...
	apply(mainTemplate) {
		const needChunkLoadingCode = chunk => {
			var otherChunksInEntry = chunk.entrypoints.some(entrypoint => entrypoint.chunks.length > 1);
		...
		mainTemplate.plugin("jsonp-script", (_, chunk, hash) => {
			...
			const scriptSrcPath = mainTemplate.applyPluginsWaterfall("asset-path", JSON.stringify(chunkFilename), {
		...
		mainTemplate.plugin("hot-bootstrap", (source, chunk, hash) => {
			...
			const currentHotUpdateChunkFilename = mainTemplate.applyPluginsWaterfall("asset-path", JSON.stringify(hotUpdateChunkFilename), {
				hash: `" + ${mainTemplate.renderCurrentHashCode(hash)} + "`,
				hashWithLength: length => `" + ${mainTemplate.renderCurrentHashCode(hash, length)} + "`,
				chunk: {
					id: "\" + chunkId + \""
			...
			const currentHotUpdateMainFilename = mainTemplate.applyPluginsWaterfall("asset-path", JSON.stringify(hotUpdateMainFilename), {
				hash: `" + ${mainTemplate.renderCurrentHashCode(hash)} + "`,
				hashWithLength: length => `" + ${mainTemplate.renderCurrentHashCode(hash, length)} + "`
			});
