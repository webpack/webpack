...
class ContainerEntryModule extends Module {
	 ...
	 * @param {[string][]} remotes list of remotes
	 */
	constructor(name, exposes, shareScope, remotes) {
	...
	codeGeneration({ moduleGraph, chunkGraph, runtimeTemplate }) {
		...
		console.log(this._remotes);

		const exposesHost = [];
		this._remotes.forEach(remoteKey => {
			exposesHost.push(`
			Object.keys(shareScope).forEach((shareScopeKey) => {
				if(!shareScopeKey.includes('webpack/container/reference/${remoteKey}')) return;
				${RuntimeGlobals.moduleFactories}[shareScopeKey.replace('reference', 'remote')] = module => {
					module.exports = shareScope[shareScopeKey][0].get()()
				};
			});`);
		});
		...
		const source = Template.asString([
			...
			`var init = ${runtimeTemplate.basicFunction("shareScope, initScope", [
				...
				"// when its already loaded by entrypoint, this will be already initialized, and shared references are global",
				`if(oldScope) return oldScope`,
				exposesHost.join(",\n"),
