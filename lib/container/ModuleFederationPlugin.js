...
class ModuleFederationPlugin {
	...
	apply(compiler) {
		...
		const type = options.type || compiler.options.output.library.type;
		const library = options.library || { type, name: options.name };

		const remoteType =
			options.remoteType || (library ? library.type : "script");
