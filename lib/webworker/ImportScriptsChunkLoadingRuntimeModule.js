...
class ImportScriptsChunkLoadingRuntimeModule extends RuntimeModule {
	...
	getInstalledChunkHandlers() {
		...
		if (runtimeRequirements.has(RuntimeGlobals.ensureChunkHandlers)) {
			handlers.push([RuntimeGlobals.ensureChunkHandlers, "i"]);
		}
		...
		if (runtimeRequirements.has(RuntimeGlobals.hmrDownloadUpdateHandlers)) {
			handlers.push([RuntimeGlobals.ensureChunkHandlers, "importScriptsHmr"]);
		}
		return handlers;
	}
	...
	_generateBaseUri(chunk) {
		...
		const rootOutputDir = compilation.runtimeTemplate.chunkRootOutputDir(
			chunk,
			false
		);
		return renderBaseUri(
			options ? options.baseUri : undefined,
			`self.location + ${JSON.stringify(rootOutputDir ? `/../${rootOutputDir}` : "")}`
		);
	}
	...
	generate() {
		...
		// Only the install/loading pair below reads the table. A chunk asking only for
		// `.b` gets this module for the base uri alone, and then has nothing to look up.
		const withInstalledChunks = withCallback || withLoading || withHmr;
