 ...
 * @param {string|(() => string)} unknownAsiStateExpression unknown asi state expression
 * @returns {string} asi safe expression
 */
exports.asiSafeExpression = (
	parser,
	node,
	expression,
	unknownAsiStateExpression
) => {
	const asiSafe = !parser.isAsiPosition(node.range[0]);
	...
	switch (asiSafe) {
		...
		case false:
			return `;${expression}`;
		default:
			return typeof unknownAsiStateExpression === "string"
				? unknownAsiStateExpression
				: unknownAsiStateExpression();
	}
};
