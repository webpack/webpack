...
/** @typedef {import("estree").SpreadElement} SpreadElementNode */
 ...
 * @returns {(expr: ExpressionNode|SpreadElementNode) => void|MemberExpressionNode} parse callback
 */
exports.parseMetaProperty = (parser, expression) => {
	/** @type {string[]} */
	const members = [];
	/** @type {string} */
	let meta;
...

	{
		...
		meta = expression.slice(0, i);
		property = expression.slice(++i, (i = expression.indexOf(".", i)));
		let j = i + 1;
		i = expression.indexOf(".", j);

		while (i !== -1) {
			members.push(expression.slice(j, i));
			j = i + 1;
			i = expression.indexOf(".", j);
		}

		members.push(expression.slice(j));
	}
	...
	return expr => {
		...
		if (
			chain.members.length !== len ||
			chain.object.type !== "MetaProperty" ||
			chain.object.meta.name !== meta ||
			chain.object.property.name !== property
		)
			return;

		for (let i = 0; i < len; i++) if (chain.members[i] !== members[i]) return;
