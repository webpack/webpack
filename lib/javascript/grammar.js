...
// cspell:ignore binop prec Prec stanceof iget iset sget sset

// The grammar the parser installs on first use. `lib/javascript/syntax.js`
// serves every production a webpack build reaches, so a build never loads it.
...
/**
 * The productions `install` copies onto the parser's prototype. It extends
...
/**
 * Copy the grammar onto the parser's prototype, leaving alone any name the
...
const install = (target) => {
	const keys = [
		...Object.getOwnPropertyNames(Grammar.prototype),
		...Object.getOwnPropertySymbols(Grammar.prototype)
	];
	for (const key of keys) {
		if (key === "constructor") continue;
		if (Object.prototype.hasOwnProperty.call(target.prototype, key)) continue;
		/** @type {EXPECTED_ANY} */
		(target.prototype)[key] = /** @type {EXPECTED_ANY} */ (Grammar.prototype)[
			key
		];
	}
};
