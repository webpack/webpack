...
/**
 * Swaps a scope's shared empty stand-in for a list of its own, on the first
 * binding it receives.
 * @param {Scope} scope the scope about to declare a name
 * @returns {void}
 */
const openBindings = (scope) => {
	if (scope.variables === NO_VARIABLE_LIST) scope.variables = [];
};

/**
 * Keeps a scope's index in step with its last binding, building one once the
 * scope has outgrown a scan.
 * @param {Scope} scope the scope that just declared a name
 * @param {Variable} variable the binding it declared
 * @returns {void}
 */
const indexBinding = (scope, variable) => {
