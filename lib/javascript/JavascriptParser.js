 ...
 * @property {SourcePositions=} sourcePositions offset to line/column mapping of the parsed code
 */
...
class JavascriptParser extends Parser {
	...
	constructor(sourceType = "auto", options = {}) {
		...
		// Offset → line/column mapping of the source being parsed.
		/** @type {SourcePositions | undefined} */
		this._sourcePositions = undefined;
	...
	parse(source, state) {
		...
		if (typeof source === "object") {
			...
			// no source text for a preparsed AST — getLocation falls back to
			// whatever `loc` the AST provides
		} else {
			({ ast, comments, semicolons, sourcePositions } = JavascriptParser._parse(
				source,
				{
		...
		const oldSourcePositions = this._sourcePositions;
		...
		// release — the mapping retains the whole source string
		this._sourcePositions = oldSourcePositions;
		return state;
	}
	 ...
	 */
	getLocation(node) {
		const positions = this._sourcePositions;
	...
	static _parse(code, options, customParse) {
		...
		const internalParse = (code, options) => {
			if (typeof customParse === "function") {
				const result = customParse(code, options);
				// custom parsers may not support locations at all — getLocation
				// derives them from node offsets via this mapping instead
				if (result && result.sourcePositions === undefined) {
					result.sourcePositions = new SourcePositions(code);
				}
				return result;
			}
			...
			return { ast, comments, semicolons, sourcePositions };
		};
		...
		let threw = false;
		try {
			({ ast, comments, semicolons, sourcePositions } = internalParse(
				code,
				parserOptions
			));
		} catch (err) {
