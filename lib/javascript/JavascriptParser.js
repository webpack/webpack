 ...
 * @property {SourcePositions=} lazySourcePositions internal: serve loc/range lazily instead of allocating them during parsing
...
// Snippet ASTs for `evaluate()`: plugins like DefinePlugin evaluate the same
// definition strings once per usage per module, so reparsing dominates.
// Consumers only read the nodes, which makes sharing safe. Keyed weakly by
// the compilation so the ASTs are released together with it.
/** @type {WeakMap<Compilation, Map<string, Program>>} */
const evaluateAstCaches = new WeakMap();
// The WeakMap only bounds lifetime; the inner map is strong while the
// compilation lives, so cap it in case a plugin evaluates per-module strings.
...
class JavascriptParser extends Parser {
	...
	static _parse(code, options, customParse) {
		...
		const internalParse = (code, options) => {
			...
			// When both loc and range are wanted (the main parse path), have the
			// parser plugin serve them on demand instead of eagerly.
			const sourcePositions =
				options.locations && options.ranges
