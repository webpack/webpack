			...
			switch (typeof expr.value) {
				case "number":
					return new BasicEvaluatedExpression()
						...
						.setRange(expr.range)
						.setFalsy(expr.value === 0);
				case "bigint":
					return new BasicEvaluatedExpression()
						.setBigInt(expr.value)
						.setRange(expr.range)
						.setFalsy(expr.value === BigInt(0));
				case "string":
					return new BasicEvaluatedExpression()
		...
		this.hooks.evaluate
			...
			.tap("JavascriptParser", _expr => {
				...
				if (expr.operator === "&&") {
					...
					returnRight = leftAsNullish === true;
				} else return;
				const right = this.evaluateExpression(expr.right);
				if (!right) return;
				if (left.couldHaveSideEffects()) right.setSideEffects();
				if (returnRight) return right.setRange(expr.range);
