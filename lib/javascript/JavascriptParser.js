...
class JavascriptParser extends Parser {
	...
	_walkStatementId(store, id) {
		...
		switch (store.types[id]) {
			...
			case T_BLOCK_STATEMENT:
				this._walkBlockStatementId(store, id, facade);
				break;
	 ...
	 * @param {EXPECTED_ANY} facade block facade
	 */
	_walkBlockStatementId(store, id, facade) {
		this.inBlockScope(() => {
			const prev = this.prevStatement;
			this.blockPreWalkStatements(facade.body);
	...
	_walkLoopBodyId(store, bodyId) {
		if (store.types[bodyId] === T_BLOCK_STATEMENT) {
			const body = /** @type {EXPECTED_ANY} */ (store.nodeAt(bodyId)).body;
			const prev = this.prevStatement;
	...
	_walkCatchClauseId(store, id) {
		this.inBlockScope(() => {
			...
			const bodyFacade = /** @type {EXPECTED_ANY} */ (store.nodeAt(bodyId));
			const prev = this.prevStatement;
			this.blockPreWalkStatement(bodyFacade);
			this.prevStatement = prev;
	...
	_walkFunctionId(store, id, facade, isExpression) {
		...
		this.inFunctionScope(true, scopeParams, () => {
			...
			const prev = this.prevStatement;
			this.preWalkStatement(/** @type {EXPECTED_ANY} */ (bodyFacade));
			this.prevStatement = prev;
			this._walkStatementId(store, bodyId);
		});
		this.scope.topLevelScope = wasTopLevel;
	 ...
	 * materialized facade; the remaining object/computed-property descent reads
	 * child ids from the columns.
	 * @param {SoaAst} store column store
	...
	_walkCallExpressionId(store, id) {
		...
		if (
			...
			if (
				...
				// a defined, untagged callee applies no call hooks — walk callee and
				// arguments directly, skipping the hook dispatch
