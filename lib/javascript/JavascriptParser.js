...
class JavascriptParser extends Parser {
	 ...
	 */
	modulePreWalkDeclarations(declarations) {
	...
	preWalkScope(node) {
		...
		// BACKWARD-COMPAT END
		this.preWalkHoistedDeclarations(
	 ...
	 */
	preWalkHoistedDeclarations(hoisted) {
	...
	// BACKWARD-COMPAT START
	/**
	 * Walk every statement of a scope to declare what hoists to it. The parser
	 * records that now, so `preWalkScope` reads it instead of walking for it.
	 * @deprecated TODO webpack 6: remove, with the statement walk below it
	 ...
	 * @deprecated TODO webpack 6: remove, see `preWalkStatements`
	 ...
	 * @deprecated TODO webpack 6: remove, call `modulePreWalkDeclarations`
	 * @param {(Statement | ModuleDeclaration)[]} statements statements
	 */
	modulePreWalkStatements(statements) {
		...
		this.modulePreWalkDeclarations(declarations);
	}
	// BACKWARD-COMPAT END
	...
	walkFunctionDeclaration(statement) {
		...
		this.preWalkScope(statement.body);
		this.prevStatement = prev;
	...
	walkFunctionExpression(expression) {
		...
		this.preWalkScope(expression.body);
		this.prevStatement = prev;
	...
	_walkIIFE(functionExpression, options, currentThis) {
		...
		if (functionExpression.body.type === "BlockStatement") {
			...
			const prev = this.prevStatement;
			this.preWalkScope(functionExpression.body);
			this.prevStatement = prev;
	...
	parse(source, state) {
		...
		if (this.hooks.program.call(ast, comments) === undefined) {
			...
			this.modulePreWalkDeclarations(
				moduleDeclarationsOf(ast, this._hasParserRecord)
			);
			this.prevStatement = undefined;
			this.preWalkScope(ast);
			this.prevStatement = undefined;
