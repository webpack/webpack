...
class JavascriptParser extends Parser {
	...
	constructor(sourceType = "auto", options = {}) {
		...
		this.hooks = Object.freeze({
			...
			/** @type {HookMap<SyncBailHook<[UpdateExpression], boolean | void>>} */
			update: new HookMap(() => new SyncBailHook(["expression"])),
			/** @type {HookMap<SyncBailHook<[UpdateExpression, Members], boolean | void>>} */
			updateMemberChain: new HookMap(
	...
	walkUpdateExpression(expression) {
		if (expression.argument.type === "Identifier") {
			...
			// An update reads and writes its target, so a tap that cares about the
			// write gets its own hook: `assign` carries an AssignmentExpression.
			if (
