...
/** @typedef {ModuleDeclaration[]} ModuleDeclarations the program's import and re-export declarations, in source order */
...
const moduleDeclarationsOf = (ast, trusted) => {
	...
	for (let index = 0, len = body.length; index < len; index++) {
		...
		if (MODULE_STATEMENT_TYPES.has(statement.type)) {
			...
			declarations.push(/** @type {ModuleDeclaration} */ (statement));
		}
	}
	return declarations;
};
...
class JavascriptParser extends Parser {
	...
	modulePreWalkDeclarations(declarations) {
		for (let index = 0, len = declarations.length; index < len; index++) {
			const statement = declarations[index];
			/** @type {StatementPath} */
		...
		this.modulePreWalkDeclarations(
			...
			(
				statements.filter((statement) =>
					MODULE_STATEMENT_TYPES.has(statement.type)
				)
			)
		);
	}
