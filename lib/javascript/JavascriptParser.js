 ...
 * @property {boolean=} soaAst internal: emit the AST into the SoA column store, serving facade nodes (SOA_MIGRATION_PLAN.md)
 */
...
class JavascriptParser extends Parser {
	...
	preWalkStatement(statement) {
		...
		if (id !== undefined) {
			this._preWalkStatementId(
				/** @type {EXPECTED_ANY} */ (statement)[SOA_KEY_STORE],
				id
			);
			return;
		}
	...
	_preWalkStatementId(store, id) {
		...
		/** @type {StatementPath} */
		(this.statementPath).push(facade);
		const { preStatement, preStatementByType } = this.hooks;
		let bail =
			preStatement.taps.length !== 0 ? preStatement.call(facade) : undefined;
		if (!bail) {
			const typed = preStatementByType.get(facade.type);
			if (typed !== undefined) bail = typed.call(facade);
		}
		if (!bail) {
			switch (store.types[id]) {
				...
				case T_FOR_STATEMENT:
					...
					if (store.kid0[id] === 0) {
						const init = /** @type {EXPECTED_ANY} */ (facade).init;
				...
				case T_SWITCH_STATEMENT:
					this._preWalkSwitchCasesId(store, id, facade);
					break;
				...
				case T_FOR_OF_STATEMENT:
					this._preWalkForInOfStatementId(store, id, facade);
					break;
				case T_DO_WHILE_STATEMENT:
				case T_LABELED_STATEMENT:
					this._preWalkStatementId(store, store.kid0[id]);
					break;
				// every other column type pre-walks nothing; foreign statement
				// types (class/import/export) never carry an id
			}
		}
		this.prevStatement =
			/** @type {StatementPath} */
	 ...
	 * @param {EXPECTED_ANY} facade loop facade
	 */
	_preWalkForInOfStatementId(store, id, facade) {
		if (
			store.types[id] === T_FOR_OF_STATEMENT &&
			facade.await &&
			this.scope.topLevelScope === true
		) {
			this.hooks.topLevelAwait.call(facade);
		}
		// a for-head declaration is acorn-built (foreign) — pre-walk the facade
		const left = facade.left;
		if (left.type === "VariableDeclaration") {
			this.preWalkVariableDeclaration(left);
		}
	 ...
	 * @param {EXPECTED_ANY} facade switch facade
	 */
	_preWalkSwitchCasesId(store, id, facade) {
		const len = store.listLens[id];
		if (len === 0) {
			// empty or foreign-pinned case list
			this.preWalkSwitchCases(facade.cases);
			return;
		}
		const flat = store.flat;
		const start = store.listStarts[id];
		for (let i = 0; i < len; i++) {
			// born-unfinished SwitchCase: `consequent` lives on the facade
			this.preWalkStatements(
				/** @type {EXPECTED_ANY} */ (store.nodeAt(flat[start + i])).consequent
			);
		}
	}
	...
	_blockPreWalkStatementId(store, id) {
		...
		let bail =
			blockPreStatement.taps.length !== 0
				? blockPreStatement.call(facade)
				: undefined;
		if (!bail) {
			const typed = blockPreStatementByType.get(facade.type);
			if (typed !== undefined) bail = typed.call(facade);
		}
		if (!bail) {
			switch (store.types[id]) {
	 ...
	 * @param {EXPECTED_ANY} facade declaration facade
	 * @param {HookMap<SyncBailHook<[Identifier], boolean | void>>} hookMap map of hooks
	 */
	_preWalkVariableDeclarationId(store, id, facade, hookMap) {
	...
	_walkStatementId(store, id) {
		...
		if (
			statementHook.taps.length !== 0 &&
			statementHook.call(facade) !== undefined
		) {
			this.prevStatement =
				/** @type {StatementPath} */
				(this.statementPath).pop();
		...
		switch (store.types[id]) {
			...
			case T_THROW_STATEMENT:
				this._walkTerminatingStatementId(store, id, facade);
				break;
			case T_IF_STATEMENT:
				this._walkIfStatementId(store, id, facade);
				break;
			case T_WHILE_STATEMENT:
			...
			case T_DO_WHILE_STATEMENT:
				...
				break;
			case T_FUNCTION_DECLARATION:
				this._walkFunctionId(store, id, facade, false);
				break;
			case T_VARIABLE_DECLARATION:
				this._walkVariableDeclarationId(store, id, facade);
				break;
			case T_FOR_STATEMENT:
			...
			case T_FOR_IN_STATEMENT:
			case T_FOR_OF_STATEMENT:
				this._walkForInOfStatementId(store, id, facade);
				break;
			case T_SWITCH_STATEMENT:
				this._walkSwitchStatementId(store, id, facade);
				break;
			case T_TRY_STATEMENT:
				this._walkTryStatementId(store, id);
				break;
			case T_LABELED_STATEMENT:
				this._walkLabeledStatementId(store, id, facade);
				break;
			case T_WITH_STATEMENT:
				this._walkWithStatementId(store, id, facade);
				break;
			...
			case T_CONTINUE_STATEMENT:
				break;
			default:
				this._dispatchWalkStatement(facade);
		}
		this.prevStatement =
			/** @type {StatementPath} */
			(this.statementPath).pop();
	 ...
	 * @param {SoaAst} store column store
	 * @param {number} id statement node id
	 * @param {EXPECTED_ANY} facade statement facade
	 */
	_walkTerminatingStatementId(store, id, facade) {
		...
		if (this.hooks.terminate.call(facade)) {
	 ...
	 * @param {EXPECTED_ANY} facade if facade
	 */
	_walkIfStatementId(store, id, facade) {
		const result = this.hooks.statementIf.call(facade);
		const consequentId = store.kid1[id];
		const alternateId = store.kid2[id];
		if (result === undefined) {
			const guard = this.hooks.collectGuards.call(facade.test);
	 ...
	 * @param {EXPECTED_ANY} facade loop facade
	 */
	_walkForInOfStatementId(store, id, facade) {
		this.inBlockScope(() => {
			// a for-head declaration is acorn-built (foreign), and binding
			// patterns stay on the object walker — both walk off the facade
			const left = facade.left;
			if (left.type === "VariableDeclaration") {
				this.blockPreWalkVariableDeclaration(left);
				this.walkVariableDeclaration(left);
			} else {
				this.walkPattern(left);
			}
	...
	_walkSwitchStatementId(store, id, facade) {
		...
		this.inBlockScope(() => {
			...
			// reference a later case's binding — see `walkSwitchCases`)
			for (let i = 0; i < len; i++) {
			...
			for (let i = 0; i < len; i++) {
				...
				// born-unfinished SwitchCase: `consequent` is filled through the
				// facade setter (never a column span), so the object walker owns it
				const consequent =
					/** @type {EXPECTED_ANY} */
					(store.nodeAt(caseId)).consequent;
				if (consequent.length > 0) {
	 ...
	 * @param {EXPECTED_ANY} facade labeled statement facade
	 */
	_walkLabeledStatementId(store, id, facade) {
	 ...
	 * @param {EXPECTED_ANY} facade with facade
	 */
	_walkWithStatementId(store, id, facade) {
		if (this._strictInModuleOutput) {
	 ...
	 * @param {EXPECTED_ANY} facade declaration facade
	 */
	_walkVariableDeclarationId(store, id, facade) {
		...
		for (let i = 0; i < len; i++) {
			...
			if (!this.hooks.declarator.call(declarator, facade)) {
	...
	_walkExpressionId(store, id) {
		switch (store.types[id]) {
			...
			case T_FUNCTION_EXPRESSION:
				this._walkFunctionId(store, id, store.nodeAt(id), true);
				break;
			case T_ARROW_FUNCTION_EXPRESSION:
				this._walkArrowFunctionId(store, id, store.nodeAt(id));
				break;
	...
	_soaIdentName(store, id) {
		...
		return sideName === undefined
			? store.source.slice(store.starts[id], store.ends[id])
			: sideName;
	}
	 ...
	 * the body (a block) is walked on the columns, params/patterns stay on the
	 * object walker.
	 * @param {SoaAst} store column store
	 * @param {number} id function node id
	 * @param {EXPECTED_ANY} facade function facade
	 * @param {boolean} isExpression whether this is a function expression (adds
	 * the self-name to the scope for recursion)
	 */
	_walkFunctionId(store, id, facade, isExpression) {
		const wasTopLevel = this.scope.topLevelScope;
		this.scope.topLevelScope = false;
		...
		this.inFunctionScope(true, scopeParams, () => {
			...
			const bodyId = store.kid1[id];
			const bodyFacade = store.nodeAt(bodyId);
			this.detectMode(/** @type {EXPECTED_ANY} */ (bodyFacade).body);
			const prev = this.prevStatement;
	 ...
	 * @param {EXPECTED_ANY} facade arrow facade
	 */
	_walkArrowFunctionId(store, id, facade) {
		const wasTopLevel = this.scope.topLevelScope;
	...
	_walkMemberExpressionId(store, id) {
		if (this._soaMemberChainHasNoInfo(store, id)) {
	...
	_walkCallExpressionId(store, id) {
		...
		if (
			...
			if (expression.callee.type === "MemberExpression") {
				// callMemberChainOfCallMemberChain only applies to call-rooted chains
				if (
	...
	_walkSequenceExpressionId(store, id) {
		const expression = /** @type {EXPECTED_ANY} */ (store.nodeAt(id));
		const path = /** @type {StatementPath} */ (this.statementPath);
		const currentStatement = path[path.length - 1];
		if (
	...
	walkExpression(expression) {
		...
		if (id !== undefined) {
			this._walkExpressionId(
				/** @type {EXPECTED_ANY} */ (expression)[SOA_KEY_STORE],
				id
			);
			return;
		}
	...
	parse(source, state) {
		...
		if (typeof source === "object") {
			...
			({ ast, comments } = JavascriptParser._parse(
				source,
				{
					...
					soaAst: this.options.soaAst === true
				},
				this.options.parse
			));
		}
		...
		if (this.hooks.program.call(ast, comments) === undefined) {
			...
			this.prevStatement = undefined;
			// with a SoA-backed AST the walk pass runs on the column store
			// (id-based dispatch), materializing facades only at escape points.
			// The root Program itself is always an acorn-built object (its body
			// entries are the facades), so the store is taken from the first
			// facade in the body rather than the Program node.
			const body = ast.body;
			let soaStore;
			for (let i = 0, len = body.length; i < len; i++) {
				const st = /** @type {EXPECTED_ANY} */ (body[i])[SOA_KEY_STORE];
				if (st !== undefined) {
					soaStore = st;
					break;
				}
			...
			if (soaStore !== undefined) {
				this._walkStatementsIdList(soaStore, body);
			} else {
