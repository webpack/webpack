...
// Consumers only read the nodes, which makes sharing safe.
/** @type {Map<string, Program>} */
const evaluateAstCache = new Map();
...
class JavascriptParser extends Parser {
	...
	evaluate(source) {
		...
		let ast =
			cacheKey === undefined ? undefined : evaluateAstCache.get(cacheKey);
		if (ast === undefined) {
			...
			if (cacheKey !== undefined) {
				if (evaluateAstCache.size >= EVALUATE_AST_CACHE_LIMIT) {
					evaluateAstCache.clear();
				}
				evaluateAstCache.set(cacheKey, ast);
			}
		}
