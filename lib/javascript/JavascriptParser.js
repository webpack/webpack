...
// Assignment targets that still exist at runtime in strict ESM output — the
// CommonJS wrapper bindings and well-known runtime globals — so no diagnostic.
const STRICT_MODE_SAFE_ASSIGN_TARGETS = new Set([
	"module",
	"exports",
	"global",
	"globalThis",
...
class JavascriptParser extends Parser {
	 ...
	 * @param {{ parse?: ParseFunction, typescript?: boolean, importPhases?: boolean }=} options parser options
	 */
	...
	walkUpdateExpression(expression) {
		if (
			...
			this._checkStrictModeUndeclaredAssign(
				expression.argument.name,
				expression.loc
			);
		}
	...
	/**
	 * Reports an assignment or update whose target is declared nowhere in the
	 * module: sloppy mode creates a global, strict-mode ESM output throws a
	 * ReferenceError (a TypeError for read-only globals) instead.
	 * @param {string} name target identifier name
	 * @param {SourceLocation | null | undefined} loc source location
	 */
	_checkStrictModeUndeclaredAssign(name, loc) {
		if (
			// `eval` / `arguments` targets are reported by _checkStrictModeAssignTarget
			name === "eval" ||
			name === "arguments" ||
			this.scope.isStrict ||
			STRICT_MODE_SAFE_ASSIGN_TARGETS.has(name) ||
			this.isVariableDefined(name) ||
			// a tapped identifier is rewritten by its plugin (`__webpack_nonce__`,
			// ProvidePlugin/DefinePlugin names), so the assignment target survives
			this.hooks.expression.get(name) !== undefined
		) {
			return;
		}
		...
		this._reportStrictModeViolation(
			`Assigning to the undeclared variable "${name}" is not allowed`,
			loc
		);
	}

	/**
	 * Reports `arguments.callee` / `arguments.caller` in a member chain — both
	 * throw a TypeError once the module runs in strict-mode ESM output.
	 * @param {MemberExpression} expression outermost member expression
	 */
	_checkStrictModeArgumentsMember(expression) {
		let node = expression;
		while (node.object.type === "MemberExpression") {
			node = /** @type {MemberExpression} */ (node.object);
		}
		if (
			node.computed ||
		...
		if (name !== "callee" && name !== "caller") return;
		if (this.scope.isStrict || this.isVariableDefined("arguments")) return;
		this._reportStrictModeViolation(
		...
		if (expression.left.type === "Identifier") {
			...
			this.enterPattern(expression.left, (name, _decl) => {
				if (!this.callHooksForName(this.hooks.assign, name, expression)) {
					// only unhandled targets — a tapped name is rewritten by its plugin
					if (this._strictInModuleOutput) {
						this._checkStrictModeUndeclaredAssign(name, expression.loc);
					}
					this.walkExpression(
	...
	walkCallExpression(expression) {
		if (
			...
			// a member callee is consumed without its own member walk, so
			// `arguments.callee()` is checked here
			this._checkStrictModeArgumentsMember(expression.callee);
		}
		if (
			expression.callee.type === "MemberExpression" &&
	...
	walkMemberExpression(expression) {
		if (this._strictInModuleOutput) {
			this._checkStrictModeArgumentsMember(expression);
		}
		const exprInfo = this.getMemberExpressionInfo(
			expression,
