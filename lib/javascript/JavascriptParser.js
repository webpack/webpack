...
class JavascriptParser extends Parser {
	...
	_initializeEvaluating() {
		...
		this.hooks.evaluate.for("SequenceExpression").tap(CLASS_NAME, (_expr) => {
			...
			// `parser.evaluate(source)` parses without `ranges: true`, so AST
			// nodes from that path don't carry range info. Bail out instead of
			// throwing into `evaluateExpression`'s catch block (which would
			// otherwise emit a `console.warn`).
			...
			// Only return the folded value when the tail is a compile-time
			// primitive (string/number/bool/...). Folding identifiers or member
			// chains would conflate idiomatic indirect-call patterns like
			// `(0, eval)("...")` with their direct-call counterparts and
			// trigger the wrong call-hooks (e.g. the `eval()` concatenation
			// bailout in `JavascriptMetaInfoPlugin`).
	...
	walkNewExpression(expression) {
		...
		// and the callee to keep them valid.
		if (expression.callee.type === "ImportExpression") {
			const source =
			...
			const between = sourceText && sourceText.slice(newStart, calleeStart);
			// Strip block and line comments before looking for `(`, so e.g.
			// `new /* ( */ import.defer(...)` is still rejected.
			const betweenStripped =
				between !== undefined
					? between.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "")
					: undefined;
			if (betweenStripped !== undefined && !betweenStripped.includes("(")) {
				const err =
