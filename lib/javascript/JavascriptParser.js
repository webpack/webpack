 ...
 * @param {HoistedDeclarations} hoisted where they are collected
 * @returns {void}
 */
const collectHoistedDeclarations = (statements, hoisted) => {
	for (let index = 0, len = statements.length; index < len; index++) {
		collectHoistedDeclaration(statements[index], hoisted);
	}
};
 ...
 * @param {HoistedDeclarations} hoisted where declarations are collected
 * @returns {void}
 */
const collectHoistedDeclaration = (statement, hoisted) => {
	switch (statement.type) {
		case "VariableDeclaration":
			if (statement.kind === "var") hoisted.push(statement);
			break;
		...
		case "ClassDeclaration":
			...
			break;
		case "BlockStatement":
			collectHoistedDeclarations(statement.body, hoisted);
			break;
		case "IfStatement":
			collectHoistedDeclaration(statement.consequent, hoisted);
			if (statement.alternate) {
				collectHoistedDeclaration(statement.alternate, hoisted);
			}
			break;
		case "ForStatement":
			if (statement.init && statement.init.type === "VariableDeclaration") {
				collectHoistedDeclaration(statement.init, hoisted);
			}
			collectHoistedDeclaration(statement.body, hoisted);
			break;
		case "ForInStatement":
		case "ForOfStatement":
			if (statement.left.type === "VariableDeclaration") {
				collectHoistedDeclaration(statement.left, hoisted);
			}
			collectHoistedDeclaration(statement.body, hoisted);
			break;
		case "WhileStatement":
		...
		case "WithStatement":
			...
			break;
		case "TryStatement":
			collectHoistedDeclaration(statement.block, hoisted);
			if (statement.handler) {
				collectHoistedDeclaration(statement.handler.body, hoisted);
			}
			if (statement.finalizer) {
				collectHoistedDeclaration(statement.finalizer, hoisted);
			}
			break;
		case "SwitchStatement": {
			const cases = statement.cases;
			for (let index = 0, len = cases.length; index < len; index++) {
				collectHoistedDeclarations(cases[index].consequent, hoisted);
			}
			break;
		}
	}
 ...
 */
const moduleDeclarationsOf = (ast) => {
 ...
 */
const hoistedDeclarationsOf = (node) => {
	const recorded = /** @type {EXPECTED_ANY} */ (node)[HOISTED_DECLARATIONS];
	if (recorded !== undefined) return recorded;
	if (node.body.length === 0) return EMPTY_HOISTED_DECLARATIONS;
	/** @type {HoistedDeclarations} */
	const hoisted = [];
	collectHoistedDeclarations(node.body, hoisted);
	return hoisted;
};
...
class JavascriptParser extends Parser {
	...
	walkFunctionDeclaration(statement) {
		...
		this.preWalkHoistedDeclarations(hoistedDeclarationsOf(statement.body));
		this.prevStatement = prev;
	...
	walkFunctionExpression(expression) {
		...
		this.preWalkHoistedDeclarations(hoistedDeclarationsOf(expression.body));
		this.prevStatement = prev;
	...
	_walkIIFE(functionExpression, options, currentThis) {
		...
		if (functionExpression.body.type === "BlockStatement") {
			...
			this.preWalkHoistedDeclarations(
				hoistedDeclarationsOf(functionExpression.body)
			);
			this.prevStatement = prev;
	...
	parse(source, state) {
		...
		if (this.hooks.program.call(ast, comments) === undefined) {
			...
			this.modulePreWalkDeclarations(moduleDeclarationsOf(ast));
			this.prevStatement = undefined;
			this.preWalkHoistedDeclarations(hoistedDeclarationsOf(ast));
			this.prevStatement = undefined;
