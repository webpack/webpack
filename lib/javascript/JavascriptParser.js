...
const SOA_EVAL_INERT_CANDIDATES = [
	...
	{
		type: SOA_TYPE.ConditionalExpression,
...
class JavascriptParser extends Parser {
	...
	_walkStatementId(store, id) {
		...
		switch (store.types[id]) {
			...
			// every other column statement type has a case above, and foreign
			// statement types never carry an id
	...
	_detectModeId(store, blockId) {
		if (store.facades[blockId] === undefined) {
			// an unregistered block with no list span is empty
	 ...
	 * identifier params (and the self-name) from the columns, then runs `fn`.
	 * Answers false — entering nothing — when the object path must serve the
	 * walk: pattern-shaped params, a registered (possibly mutated) facade, or
	 * the strict-mode diagnostics pass, which reports on param facades.
	 * @param {SoaAst} store column store
	...
	_inFunctionScopeIds(store, id, hasThis, selfNameId, fn) {
		...
		const flat = store.flat;
		for (let i = 0; i < len; i++) {
			// a foreign param would pin the facade (caught above) — every ref
			// is real, and only plain identifier params stay column-native
			if (types[flat[start + i]] !== T_IDENTIFIER) return false;
		}
		...
		for (let i = 0; i < len; i++) {
			this._enterIdentifierId(store, flat[start + i]);
		}
		if (selfNameId !== 0) {
	...
	_walkCallExpressionId(store, id) {
		...
		if (calleeId !== 0 && store.types[calleeId] === T_MEMBER_EXPRESSION) {
			...
			if (
				...
				// no info: no call/member hook can match and the callee can never
				// evaluate to an identifier — walk the call context (object,
				// computed property) and arguments straight from the columns
	...
	_walkAssignmentExpressionId(store, id) {
		...
		if (store.facades[id] === undefined && leftId !== 0 && rightId !== 0) {
			...
			if (leftType === T_IDENTIFIER) {
				...
				// an info-free member target cannot reach assignMemberChain
				this._walkExpressionId(store, rightId);
				this._walkExpressionId(store, leftId);
				return;
			}
	...
	parse(source, state) {
		...
		if (this.hooks.program.call(ast, comments) === undefined) {
			...
			// with a SoA-backed AST the pre-walk and walk passes run on the
			// column store (id-based dispatch), materializing facades only at
			// escape points. The root Program itself is always an acorn-built
			// object (its body entries are the facades), so the store is taken
			// from the first facade in the body rather than the Program node.
			// It is held on the parser so pending `statementPath` id entries
			// can materialize on access.
			const body = ast.body;
			let soaStore;
