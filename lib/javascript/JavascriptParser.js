...
class JavascriptParser extends Parser {
	...
	_initializeEvaluating() {
		...
		this.hooks.evaluate.for("SequenceExpression").tap(CLASS_NAME, (_expr) => {
			...
			// transforms (e.g. ImportDependency) would replace the whole
			// range and silently drop those side effects.
	...
	walkNewExpression(expression) {
		// Per spec, `ImportCall` is a `CallExpression`, never a
		// `MemberExpression`, so `new import.defer(...)` /
		// `new import.source(...)` is a SyntaxError. Acorn rejects bare
		// `new import(...)`, but `acorn-import-phases` currently accepts the
		// phased variants, so we reject them here. Wrapping the call in
		// parentheses (`new (import.defer(...))`) is valid and produces the
		// same AST shape, so we look at the source between `new` and the
		// callee to tell them apart.
		if (expression.callee.type === "ImportExpression") {
			...
			const isParenthesized = between !== undefined && /\(/.test(between);
			if (!isParenthesized) {
				const err =
