...
const SOA_OP_NOT = /** @type {number} */ (SoaAst.OPERATOR_IDS.get("!"));
const SOA_OP_PLUS = /** @type {number} */ (SoaAst.OPERATOR_IDS.get("+"));
...
/**
 * Column predicate paired with a hook tap via `registerColumnGate`: receives
 * the node's row (and its owning declaration's row for declarator hooks) and
 * returns false when the tap would provably bail without observing the node.
 * @typedef {(parser: JavascriptParser, store: SoaAst, id: number, ownerId: number) => boolean} ColumnGate
 */

/** @typedef {(...args: EXPECTED_ANY[]) => EXPECTED_ANY} TapFn */
 ...
 * @property {boolean=} soaAst emit the AST into the SoA column store, serving facade nodes (SOA_MIGRATION_PLAN.md); on by default
 */
...
const defaultParserOptions = {
	...
	allowHashBang: true,
	// direct `_parse` callers analyze with own-key traversals (eslint-scope),
	// so the static default stays on object nodes; the instance parse path
	// passes the parser-level default explicitly
	soaAst: false
};
...
class JavascriptParser extends Parser {
	 ...
	 * @param {{ parse?: ParseFunction, typescript?: boolean, importPhases?: boolean, strictModeViolations?: "error" | "warn" | false, soaAst?: boolean }=} options parser options
	 */
	constructor(sourceType = "auto", options = {}) {
		...
		// per-hook column gates: hook -> (tap fn -> predicate); see registerColumnGate
		/** @type {WeakMap<EXPECTED_OBJECT, WeakMap<TapFn, ColumnGate>>} */
		this._columnGates = new WeakMap();
	...
	_modulePreWalkStatementsId(store, ownerId) {
		const len = store.listLens[ownerId];
		...
		const start = store.listStarts[ownerId];
		for (let i = 0; i < len; i++) {
		...
		if (!bail) {
			switch (store.types[id]) {
				...
				case T_FOR_STATEMENT:
					// a column-owned init is an expression (for-head declarations
					// are acorn-built) — only a declaration init pre-walks
					if (store.kid0[id] === 0) {
	...
	_preWalkForInOfStatementId(store, id) {
		...
		// an owned left is an expression/pattern and never pre-walks; a
		// foreign (acorn-built) left is always a for-head declaration
		if (store.kid0[id] === 0) {
	...
	_preWalkVariableDeclarationId(store, id, hookMap) {
		...
		const flat = store.flat;
		const start = store.listStarts[id];
		const { preDeclarator: preDeclaratorHook } = this.hooks;
		for (let i = 0; i < len; i++) {
			const did = flat[start + i];
			const patId = store.kid0[did];
			if (
				store.types[patId] !== T_IDENTIFIER ||
				this._soaHookNeeded(preDeclaratorHook, store, did, id)
			) {
	 ...
	 * @param {EXPECTED_OBJECT} hook the tapped hook
	 * @param {TapFn} fn the tap function the gate covers
	 * @param {ColumnGate} gate column predicate for the tap's bail conditions
	 */
	registerColumnGate(hook, fn, gate) {
		let gates = this._columnGates.get(hook);
		if (gates === undefined) {
			gates = new WeakMap();
			this._columnGates.set(hook, gates);
		}
		gates.set(fn, gate);
	}

	/**
	 * Whether a hook call needs a materialized node: any interceptor or
	 * tap without a gate forces it, otherwise the registered gates decide.
	 * @param {EXPECTED_ANY} hook the hook about to be called
	 * @param {SoaAst} store column store
	 * @param {number} id node id passed to the gates
	 * @param {number} ownerId owning node id passed to the gates (0 if none)
	 * @returns {boolean} true when the node must materialize and the hook fire
	 */
	_soaHookNeeded(hook, store, id, ownerId) {
		const taps = hook.taps;
		const len = taps.length;
		if (len === 0) return false;
		if (hook.interceptors.length !== 0) return true;
		const gates = this._columnGates.get(hook);
		if (gates === undefined) return true;
		for (let i = 0; i < len; i++) {
			const gate = gates.get(taps[i].fn);
			if (gate === undefined || gate(this, store, id, ownerId)) return true;
		}
		return false;
	}
	...
	_walkStatementId(store, id) {
		...
		const { statement: statementHook } = this.hooks;
		if (
			this._soaHookNeeded(statementHook, store, id, 0) &&
			statementHook.call(
				/** @type {Statement} */ (
					/** @type {EXPECTED_ANY} */ (store.nodeAt(id))
				)
	...
	_walkIfStatementId(store, id) {
		...
		const result = this._soaHookNeeded(statementIf, store, id, 0)
			? statementIf.call(
					/** @type {EXPECTED_ANY} */ (
						/** @type {EXPECTED_ANY} */ (store.nodeAt(id))
					)
		...
		const alternateId = store.kid2[id];
		if (result === undefined) {
			const guard = this._soaHookNeeded(collectGuards, store, id, 0)
				? collectGuards.call(
	...
	_walkForStatementId(store, id) {
		this.inBlockScope(() => {
			...
			if (initId !== 0) {
				// for-head declarations are acorn-built (`parseForStatement`
				// finishes the node itself), so an owned init is an expression
	...
	_walkForInOfStatementId(store, id) {
		this.inBlockScope(() => {
			// a foreign (acorn-built) left is always a for-head declaration;
			// an owned left is a pattern, walked on the facade (a plain
	...
	_walkVariableDeclarationId(store, id) {
		...
		const flat = store.flat;
		const start = store.listStarts[id];
		const { declarator: declaratorHook } = this.hooks;
		const types = store.types;
		for (let i = 0; i < len; i++) {
			const did = flat[start + i];
			const declaratorTapped = this._soaHookNeeded(
				declaratorHook,
				store,
				did,
				id
	...
	_walkExprChildId(store, parentId, childId, prop) {
		if (childId !== 0) {
			...
			const child = /** @type {EXPECTED_ANY} */ (store.nodeAt(parentId))[prop];
	...
	_walkArrayExpressionId(store, id) {
		const len = store.listLens[id];
		if (len === 0) {
			const elements = /** @type {EXPECTED_ANY} */ (store.nodeAt(id)).elements;
	...
	/**
	 * Whether evaluating this row can only fall through to an unknown value
	 * (no static bool/nullish verdict): shapes rooted in plain defined
	 * variables while only the parser's own evaluators are tapped.
	 * Conservative — false routes to the facade path.
	 * @param {SoaAst} store column store
	 * @param {number} id expression row (0 for a foreign operand)
	 * @param {number=} depth remaining recursion budget
	 * @returns {boolean} true when the evaluation provably stays unknown
	 */
	_soaEvalUnknowable(store, id, depth = 6) {
		if (id === 0 || depth === 0) return false;
		switch (store.types[id]) {
			case T_IDENTIFIER: {
				if (!this._evalIdentOwnTaps) return false;
				const name = this._soaIdentName(store, id);
			...
			case T_MEMBER_EXPRESSION: {
				// a static chain on a plain defined root matches no name-keyed
				// evaluator (those key free/tagged roots) and never folds
				if (!this._evalMemberOwnTaps) return false;
				const rootId = this._soaMemberRootId(store, id);
				if (rootId === 0 || store.types[rootId] !== T_IDENTIFIER) {
					return false;
				}
				const name = this._soaIdentName(store, rootId);
				return name !== "undefined" && this._isDefinedPlainVariable(name);
			}
			case T_LOGICAL_EXPRESSION:
				// the right operand alone can fold (`unknown && falsy`,
				// `unknown || truthy`) — both sides must stay unknown
				return (
					this._evalLogicalOwnTaps &&
					this._soaEvalUnknowable(store, store.kid0[id], depth - 1) &&
					this._soaEvalUnknowable(store, store.kid1[id], depth - 1)
				);
			case T_UNARY_EXPRESSION:
				// `!x` stays unknown for an unknown operand; `typeof x` folds by
				// type even for unknown operands, so only `!` recurses
				return (
					this._soaEvalInertTypes[T_UNARY_EXPRESSION] === 1 &&
					store.aux[id] === SOA_OP_NOT &&
					this._soaEvalUnknowable(store, store.kid0[id], depth - 1)
				);
			case T_BINARY_EXPRESSION: {
				if (this._soaEvalInertTypes[T_BINARY_EXPRESSION] !== 1) return false;
				const leftUnknown = this._soaEvalUnknowable(
					store,
					store.kid0[id],
					depth - 1
				);
				// `+` wraps known string ends around an unknown side and the
				// wrapped value still answers `asBool` — both must be unknown
				if (store.aux[id] === SOA_OP_PLUS) {
					return (
						leftUnknown &&
						this._soaEvalUnknowable(store, store.kid1[id], depth - 1)
					);
				}
				// every other binary fold needs both operand values
				return (
					leftUnknown ||
					this._soaEvalUnknowable(store, store.kid1[id], depth - 1)
				);
			...
			case T_CALL_EXPRESSION: {
				// only member callees (string methods) and tagged callees fold
				if (!this._evalCallOwnTaps) return false;
				const calleeId = store.kid0[id];
				return (
					calleeId !== 0 &&
					store.types[calleeId] === T_IDENTIFIER &&
					this._isDefinedPlainVariable(this._soaIdentName(store, calleeId))
				);
			}
			default:
				return false;
	...
	_walkLogicalExpressionId(store, id) {
		// gated-out taps cannot short-circuit the walk, so an unregistered
		// node descends without materializing the hook argument
		if (
			store.facades[id] === undefined &&
			!this._soaHookNeeded(this.hooks.expressionLogicalOperator, store, id, 0)
		) {
			this._walkExprChildId(store, id, store.kid0[id], "left");
			this._walkExprChildId(store, id, store.kid1[id], "right");
			return;
	...
	_walkConditionalExpressionId(store, id) {
		// with no live operator/guard taps both branches always walk — descend
		// on the columns without materializing the hook argument
		if (
			store.facades[id] === undefined &&
			!this._soaHookNeeded(
				this.hooks.expressionConditionalOperator,
				store,
				id,
				0
			) &&
			!this._soaHookNeeded(this.hooks.collectGuards, store, id, 0)
		) {
			this._walkExprChildId(store, id, store.kid0[id], "test");
		...
		if (result === undefined) {
			const guard = this._soaHookNeeded(this.hooks.collectGuards, store, id, 0)
				? this.hooks.collectGuards.call(expression.test)
				: undefined;
			this._walkExprChildId(store, id, store.kid0[id], "test");
			this.walkGuardedBranch(guard ? guard.consequent : undefined, () =>
				this._walkExprChildId(store, id, store.kid1[id], "consequent")
		...
		if (typeof source === "object") {
			...
			({ ast, comments } = JavascriptParser._parse(
				source,
				{
					...
					soaAst: this.options.soaAst !== false
				},
				this.options.parse
			));
		}
