...
class JavascriptParser extends Parser {
	...
	blockPreWalkExportNamedDeclaration(statement) {
		...
		// If there's a source (exporting from another module), call exportImport hook
		if (statement.source) {
			...
			for (
				...
				if (specifier.type === "ExportSpecifier") {
					const localName =
						...
						/** @type {string} */ (specifier.local).value;
					const name =
						/** @type {Identifier} */ (specifier.exported).name ||
					...
					} else if (!this.alreadyExported(name)) {
						// Ensure export isn't repeated
	 ...
	 * @param {ExportNamedDeclaration} statement The export declaration to walk
	 */
	walkExportNamedDeclaration(statement) {
		// Walk through the declaration if it exists
		...
		if (statement.specifiers && statement.specifiers.length > 0) {
			for (
				...
				if (specifier.type === "ExportSpecifier") {
					...
					// Trigger export specifier hooks with proper index
					this.hooks.exportSpecifier.call(
						statement,
						localName,
						exportedName,
	 ...
	 * @param {ExportDefaultDeclaration} statement The export default statement.
	 */
	blockPreWalkExportDefaultDeclaration(statement) {
		...
		if (
			...
			// Check if the declaration has an id
			if (statement.declaration.id) {
				// Register the export specifier for the named default export
				this.hooks.exportSpecifier.call(
					statement,
			...
			// Handle expressions differently - they don't need preWalk handling
			// But add any required expression handling here
		}
	}
	...
	walkExportDefaultDeclaration(statement) {
		// Trigger the export hook for this statement
		this.hooks.export.call(statement);
		...
		if (
			...
			// If the declaration is not handled by exportDeclaration hook, walk through it
			if (!this.hooks.exportDeclaration.call(statement, declaration)) {
				this.walkStatement(declaration);
			}
		} else {
			// Handle anonymous functions/classes as expressions
			if (
				statement.declaration.type === "FunctionDeclaration" ||
				statement.declaration.type === "ClassDeclaration"
			) {
			...
			// If the exportExpression hook does not handle it, trigger exportSpecifier
			if (
	 ...
	 * @param {ExportAllDeclaration} statement The export all statement.
	 */
	blockPreWalkExportAllDeclaration(statement) {
		// Extract the source module being re-exported
		const source = /** @type {ImportSource} */ (statement.source.value);

		// Extract the exported name (if present), otherwise it's null
		const name = statement.exported
			? /** @type {Identifier} */ (statement.exported).name ||
				/** @type {string} */ (
		...
		// Call the exportImportSpecifier hook to register the export mapping
