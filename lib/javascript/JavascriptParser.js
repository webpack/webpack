...
/** @typedef {import("./BasicEvaluatedExpression").Guard} Guard */
...
class JavascriptParser extends Parser {
	...
	maybeGuarded(fn) {
		this.scope.guards.push(new Set());
		fn();
		this.scope.guards.pop();
	}

	inBlockScope(fn) {
		const oldScope = this.scope;
		const guardsLength = oldScope.guards.length;
		this.scope = {
			topLevelScope: oldScope.topLevelScope,
			guards: oldScope.guards,
		...
		oldScope.guards.length = guardsLength;
		this.scope = oldScope;
	}
	...
	/**
	 * @param {any} tagInfoData current tagInfo data
	 * @returns {boolean} is guarded or not
	 */
	inGuard(tagInfoData) {
		const guards = this.scope.guards;
		if (guards.length === 0) return false;

		for (let i = guards.length - 1; i >= 0; i--)
			if (guards[i].has(tagInfoData)) return true;

		return false;
	}

	/**
	 * @param {Guard|undefined} guard guard
	 */
	guardBy(guard) {
		if (!guard) return;
		const current = this.scope.guards[this.scope.guards.length - 1];
		if (!current) return;
		this.scope.guards[this.scope.guards.length - 1] =
			BasicEvaluatedExpression.mergeGuards(guard, current);
	}
