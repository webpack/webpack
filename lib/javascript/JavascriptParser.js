...
class JavascriptParser extends Parser {
	 ...
	 * `node.loc` is read only on the violation branch — lazy nodes materialize
	 * line/column on access, so the common no-violation path stays free.
	 ...
	 * `node.loc` is read only on the violation branches — lazy nodes materialize
	 * line/column on access, so the common no-violation path stays free.
