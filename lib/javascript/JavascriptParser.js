...
class JavascriptParser extends Parser {
	...
	walkIfStatement(statement) {
		...
		if (result === undefined) {
			...
			this.scope.terminated =
				consequentTerminated && (statement.alternate ? alternateTerminated : true)
					? this.scope.terminated
					: undefined;
		} else {
			this.inStatementScope(() => {
	 ...
	 * @returns {void}
	 */
	inStatementScope(fn, inExecutedPath = false) {
		const oldScope = this.scope;
		this.scope = {
			topLevelScope: oldScope.topLevelScope,
		...
		if (
			inExecutedPath &&
			((this.scope.inTry && terminated === SCOPE_INFO_TERMINATED_THROW) ||
				terminated === SCOPE_INFO_TERMINATED_RETURN)
		) {
