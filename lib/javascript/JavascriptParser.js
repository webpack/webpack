...
class JavascriptParser extends Parser {
	 ...
	 */
	isAsiPosition(pos) {
		const len = this.statementPath.length;

		if (len === 0) throw new Error("Not in statement");

		const currentStatement = this.statementPath[len - 1];

		// check for statements with one child statement
		// that current statement is a child
		if (len !== 1) {
			const prevStatementInPath = this.statementPath[len - 2];
			switch (prevStatementInPath.type) {
				case "IfStatement":
					if (
						prevStatementInPath.consequent === currentStatement ||
						prevStatementInPath.alternate === currentStatement
					)
						return false;
					break;
				case "ThrowStatement":
					if (prevStatementInPath.argument === currentStatement) return false;
					break;
				case "ForStatement":
				case "ForInStatement":
				case "ForOfStatement":
				...
				case "DoWhileStatement":
				case "WithStatement":
					if (prevStatementInPath.body === currentStatement) return false;
					break;
			}
		}
