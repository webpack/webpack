...
class JavascriptParser extends Parser {
	...
	inBlockScope(fn, inExecutedPath = false) {
		...
		loop: for (let i = this.statementPath.length - 1; i >= 0; i--) {
			const statementPath = this.statementPath[i];
		...
		// check if this block is from TryStatement,
		// if so, we should not pass `throw` terminated state to old scope
		// as there might be a catch clause
		if (inExecutedPath && (!enterTry || terminated !== "throw")) {
