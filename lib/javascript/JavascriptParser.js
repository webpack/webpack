...
class JavascriptParser {
	...
	prewalkStatement(statement) {
		...
		if (isFunction(this[`prewalk${statement.type}`])) {
			this[`prewalk${statement.type}`](statement);
		}
	}

	blockPrewalkStatement(statement) {
		if (this.hooks.blockPreStatement.call(statement)) return;
		if (isFunction(this[`blockPrewalk${statement.type}`])) {
			this[`blockPrewalk${statement.type}`](statement);
		}
	}
	...
	walkStatement(statement) {
		...
		if (isFunction(this[`walk${statement.type}`])) {
	...
	walkPattern(pattern) {
		if (isFunction(this[`walk${pattern.type}`])) {
	...
	walkExpression(expression) {
		if (isFunction(this[`walk${expression.type}`])) {
