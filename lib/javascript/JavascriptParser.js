...
const T_VARIABLE_DECLARATOR = SOA_TYPE.VariableDeclarator;
...
class JavascriptParser extends Parser {
	 ...
	 * patterns and the rename/`declarator`-hook analysis run on the facade. A
	 * foreign-pinned declaration list falls back to the object walker.
	 * @param {SoaAst} store column store
	...
	_walkVariableDeclarationId(store, id, facade) {
		...
		if (len === 0) {
			this.walkVariableDeclaration(facade);
			return;
		}
		const flat = store.flat;
		...
		for (let i = 0; i < len; i++) {
			...
			if (store.types[did] !== T_VARIABLE_DECLARATOR) continue;
	...
	_walkObjectExpressionId(store, id) {
		const len = store.listLens[id];
		if (len === 0) {
			const properties = /** @type {EXPECTED_ANY} */ (store.nodeAt(id))
				.properties;
			for (let i = 0, l = properties.length; i < l; i++) {
				this.walkProperty(properties[i]);
			}
			return;
		}
		const flat = store.flat;
