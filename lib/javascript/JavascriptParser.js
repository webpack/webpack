...
/** @typedef {import("estree").ObjectPattern} ObjectPatternNode */
/** @typedef {import("estree").AssignmentProperty} AssignmentPropertyNode */
...
class JavascriptParser extends Parser {
	...
	constructor(sourceType = "auto") {
		...
		/** @type {VariableDeclaratorNode|undefined} */
		this.currentVariableDeclarator = undefined;
	...
	walkVariableDeclaration(statement) {
		for (const declarator of statement.declarations) {
			switch (declarator.type) {
				case "VariableDeclarator": {
					this.currentVariableDeclarator = declarator;
	...
	/**
	 * @returns {string[]|undefined} undefined for unknown "used" properties list
	 */
	getUsedPropertiesIfAny() {
		if (!this.currentVariableDeclarator) return;

		let used;

		switch (this.currentVariableDeclarator.id.type) {
			case "ObjectPattern":
				used = this.getUsedObjectProperties(this.currentVariableDeclarator.id);
				break;
		}

		if (!used) return;

		let maybeAssignExpression = this.currentVariableDeclarator.init;

		while (maybeAssignExpression.type === "AssignmentExpression") {
			if (maybeAssignExpression.left.type !== "ObjectPattern") return;
			used = this.getUsedObjectProperties(maybeAssignExpression.left, used);
			if (!used) return;
			maybeAssignExpression = maybeAssignExpression.right;
		}
	...
	/**
	 * @param {ObjectPatternNode} pattern pattern
	 * @param {Set<string>} used used
	 * @returns {Set<string>|undefined} undefined for unknown "used" properties list
	 */
	getUsedObjectProperties(pattern, used = new Set()) {
		const properties = pattern.properties;
		const len = properties.length;
		if (len === 0) return used;
		if (properties[len - 1].type === "RestElement") return;

		for (let i = 0; i < len; i++) {
			const property = /** @type {AssignmentPropertyNode} */ (properties[i]);
			if (property.computed) {
				// computed must be false for PrivateIdentifier
				// https://github.com/estree/estree/blob/91f49977f1f05dea86c112a070a703bb37f5722d/es2022.md#methoddefinition
				const evaluated = this.evaluateExpression(
					/** @type {ExpressionNode} */ (property.key)
				);
				if (!evaluated) return;
				const str = evaluated.asString();
				if (typeof str !== "string") return;
				used.add(str);
			} else {
				used.add(/** @type {IdentifierNode} */ (property.key).name);
			}
		}
