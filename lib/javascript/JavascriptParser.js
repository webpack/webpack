...
class JavascriptParser extends Parser {
	...
	_initializeEvaluating() {
		...
		tapEvaluateWithVariableInfo("Identifier", expr => {
			let info = this.getVariableInfo(/** @type {Identifier} */ (expr).name);
			if (this.isVariableDefined(/** @type {Identifier} */ (expr).name)) {
				info = /** @type {Identifier} */ (expr).name;
			}
			if (
				typeof info === "string" ||
	...
	getMemberExpressionInfo(expression, allowedTypes) {
		...
		switch (object.type) {
			...
			case "ThisExpression": {
				...
				if (
					result === undefined &&
					this.isVariableDefined(rootName) &&
					members.length === 0
				) {
					return {
						type: "expression",
						name: objectAndMembersToName(rootName, members),
						rootInfo: rootName,
						getMembers: memoize(() => members.reverse()),
						getMembersOptionals: memoize(() => membersOptionals.reverse()),
						getMemberRanges: memoize(() => memberRanges.reverse())
