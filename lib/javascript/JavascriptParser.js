...
class JavascriptParser extends Parser {
	...
	isPure(expr, commentsStartPos) {
		...
		switch (expr.type) {
			...
			case "UnaryExpression":
				// `typeof`, `void` and `!` produce their result without running user
				// code on the operand, so each is pure where its operand is. The rest
				// fall through to the evaluator, which still reads literal cases.
				if (
