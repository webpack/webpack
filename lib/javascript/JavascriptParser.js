 ...
 * @property {Set<number>=} semicolons positions of inserted semicolons; when absent they are derived from statement offsets and the source text
 */
...
// Statement types acorn terminates via `semicolon()` — the only statements
// whose end can be an ASI position. Everything else (blocks, declarations
// ending in `}`, do-while, empty statements) never reports an insertion.
const ASI_STATEMENT_TYPES = new Set([
	"ExpressionStatement",
	"VariableDeclaration",
	"ReturnStatement",
	"ThrowStatement",
...
/**
 * The statement whose last token sits at the enclosing statement's end — the
 * one acorn ran `semicolon()` for. `if (a) b` ends where its consequent ends.
 * @param {Statement | ModuleDeclaration | Expression | MaybeNamedFunctionDeclaration | MaybeNamedClassDeclaration} statement statement
 * @returns {Statement | ModuleDeclaration | Expression | MaybeNamedFunctionDeclaration | MaybeNamedClassDeclaration} trailing statement
 */
const trailingStatement = (statement) => {
	for (;;) {
		switch (statement.type) {
			...
			case "ForOfStatement":
			case "WithStatement":
				statement = /** @type {Statement} */ (statement.body);
				break;
			case "IfStatement":
...
class JavascriptParser extends Parser {
	...
	constructor(sourceType = "auto", options = {}) {
		...
		// set/unsetAsiPosition adjustments when no semicolons set is provided
		/** @type {Map<number, boolean> | undefined} */
		this._asiOverrides = undefined;
	...
	parse(source, state) {
		...
		if (typeof source === "object") {
			...
			if (source.semicolons) {
				...
				semicolons = new Set(source.semicolons);
			}
		} else {
			({ ast, comments, semicolons } = JavascriptParser._parse(
				source,
				{
					...
					// line/column locations and inserted semicolons are derived
					// from node offsets and the source text — parsers never need
					// to track either
					locations: false,
					ranges: true,
					comments: true,
		...
		const oldAsiOverrides = this._asiOverrides;
		...
		this._asiOverrides = oldAsiOverrides;
		return state;
	}
	...
	/**
	 * Whether a semicolon was inserted at the end of the statement. Uses the
	 * parser-provided semicolons set when there is one; derives the answer
	 * from the statement type and the source text otherwise, so parsers
	 * without an `onInsertedSemicolon` API are fully supported.
	 * @param {Statement | ModuleDeclaration | Expression | MaybeNamedFunctionDeclaration | MaybeNamedClassDeclaration} statement statement ending at `end`
	 * @param {number} end end position of the statement
	 * @returns {boolean} true when a semicolon was inserted there
	 */
	_hasInsertedSemicolonAt(statement, end) {
		const overrides = this._asiOverrides;
		if (overrides !== undefined) {
			const override = overrides.get(end);
			if (override !== undefined) return override;
		}
		const semicolons = this.semicolons;
		if (semicolons !== undefined) return semicolons.has(end);
		const source = this._source;
		// an explicit semicolon is always the statement's last character
		if (source === undefined || source.charCodeAt(end - 1) === 59) {
			return false;
		}
		const trailing = trailingStatement(statement);
		switch (trailing.type) {
			case "ExportNamedDeclaration":
				// acorn records the insertion for a declaration form via the
				// nested declaration's own `semicolon()` call
				return trailing.declaration
					? trailing.declaration.type === "VariableDeclaration"
					: true;
			case "ExportDefaultDeclaration": {
				const declarationType = trailing.declaration.type;
				return (
			...
			default:
				return ASI_STATEMENT_TYPES.has(trailing.type);
		}
	}
	...
	isAsiPosition(pos) {
		...
		return (
			...
			(range[1] === pos &&
				this._hasInsertedSemicolonAt(currentStatement, pos)) ||
	...
	setAsiPosition(pos) {
		...
		if (semicolons !== undefined) {
			...
			(this._asiOverrides || (this._asiOverrides = new Map())).set(pos, true);
		}
	}
	...
	unsetAsiPosition(pos) {
		const semicolons = this.semicolons;
		if (semicolons !== undefined) {
			semicolons.delete(pos);
		} else {
			(this._asiOverrides || (this._asiOverrides = new Map())).set(pos, false);
		}
	}
	...
	static _parse(code, options, customParse) {
		...
		const internalParse = (code, options) => {
			...
			if (options.semicolons) {
				const set = (semicolons = new Set());
				/** @type {AcornOptions} */
				(options).onInsertedSemicolon = (pos) => set.add(pos);
			}
