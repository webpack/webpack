...
/** @typedef {(Statement | ModuleDeclaration | undefined)[]} BlockDeclarations what one statement list declares into its block scope, each followed by the statement written before it */
...
const EMPTY_BLOCK_DECLARATIONS = [];
...
const blockDeclarationsOf = (node, statements, trusted) => {
	...
	/** @type {BlockDeclarations} */
	let declarations = EMPTY_BLOCK_DECLARATIONS;
	for (let index = 0, len = statements.length; index < len; index++) {
		const statement = statements[index];
		let declares = BLOCK_DECLARATION_TYPES.has(statement.type);
		if (!declares && statement.type === "ExpressionStatement") {
		...
		if (declares) {
			if (declarations === EMPTY_BLOCK_DECLARATIONS) declarations = [];
			declarations.push(
				statement,
...
class JavascriptParser extends Parser {
	...
	/**
	 * Declare what the recorded statements bind, each with the statement written
	 * before it so a tap reading `prevStatement` sees what the walk would show.
	 * @param {BlockDeclarations} declarations what the list declares
	 */
	_blockPreWalkDeclarations(declarations) {
		for (let index = 0, len = declarations.length; index < len; index += 2) {
			// the first statement of the list has none, and leaves whatever the
			// caller set standing, exactly as walking the whole list did
			const previous = declarations[index + 1];
			if (previous !== undefined) this.prevStatement = previous;
