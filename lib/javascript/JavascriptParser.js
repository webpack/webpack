...
/**
 * What a statement list declares into its own block scope: the parser's own
 * record, or a scan of the list for an AST some other parser built.
 ...
 * @param {boolean} trusted whether the parser's own record may be read
 * @returns {BlockDeclarations} the declarations, in source order
 */
const blockDeclarationsOf = (node, statements, trusted) => {
	if (trusted) {
		const recorded = /** @type {EXPECTED_ANY} */ (node)[BLOCK_DECLARATIONS];
		if (recorded !== undefined) return recorded;
	}
	let mask = 0;
...
class JavascriptParser extends Parser {
	...
	_blockPreWalkScope(node, statements) {
		...
		// BACKWARD-COMPAT END
		this._blockPreWalkDeclarations(
			statements,
			blockDeclarationsOf(node, statements, this._hasParserRecord)
		);
	}
