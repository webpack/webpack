...
const { Parser: BaseParser, tokContexts, tokTypes } = require("acorn");
 ...
 * parseExprList: (close: TokenType, allowTrailingComma: boolean, allowEmpty: boolean, refDestructuringErrors: DestructuringErrorsShim) => Expression[],
...
const createDestructuringErrors = () =>
	DestructuringErrorsClass !== null
		? new DestructuringErrorsClass()
		: {
				shorthandAssign: -1,
				trailingComma: -1,
...
class WebpackParser extends BaseParser {
	...
	parseVar(node, isFor, kind, allowMissingInitializer) {
		const sourcePositions = this[kSourcePositions];
		if (!sourcePositions) {
			return base.parseVar.call(this, node, isFor, kind, allowMissingInitializer);
		}
		/** @type {Node[]} */
		const declarations = [];
		const target = /** @type {Node & { declarations: Node[], kind: string }} */ (
			node
		);
		...
		for (;;) {
			...
			if (this.eat(tokTypes.eq)) {
				...
				!allowMissingInitializer &&
				usingKind &&
				/** @type {number} */ (this.options.ecmaVersion) >= 17 &&
				this.type !== tokTypes._in &&
				!this.isContextual("of")
			) {
	...
	parseBlock(createNewLexicalScope, node, exitStrict) {
		const sourcePositions = this[kSourcePositions];
		if (!sourcePositions) {
			return base.parseBlock.call(this, createNewLexicalScope, node, exitStrict);
		}
	...
	parseNew() {
		...
		if (this.eat(tokTypes.parenL)) {
			args = this.parseExprList(tokTypes.parenR, true, false);
		} else {
			args = EMPTY_NEW_ARGS;
		}
		return /** @type {Expression} */ (
	...
	parseMaybeUnary(refDestructuringErrors, sawUnary, incDec, forInit) {
		...
		if (this.isContextual("await") && this.canAwait) {
			...
			if (update) {
				...
				this.raiseRecoverable(startPos, "Deleting local variable in strict mode");
