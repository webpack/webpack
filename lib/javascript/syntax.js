...
/** @typedef {EXPECTED_ANY[]} BlockDeclarations what one statement list declares into its block scope, each followed by the statement written before it */
...
// What one statement list declares into its own block scope, read before any
// of it is walked — same sparse-slot reasoning as the two lists above. Each
// entry is followed by the statement written before it in that list.
const BLOCK_DECLARATIONS = Symbol("blockDeclarations");

/** @type {BlockDeclarations} */
const NO_BLOCK_DECLARATIONS = [];
	...
	parseTopLevel(node) {
		...
		while (this.type !== tokTypes.eof) {
			...
			if (declaresIntoBlock(statement)) {
				if (blockDeclarations === undefined) blockDeclarations = [];
				blockDeclarations.push(statement, this._previousTopLevelStatement);
			}
		...
		program[BLOCK_DECLARATIONS] =
			blockDeclarations === undefined
				? NO_BLOCK_DECLARATIONS
				: blockDeclarations;
		...
		while (this.type !== tokTypes.braceR) {
			...
			if (declaresIntoBlock(statement)) {
				...
				blockDeclarations.push(
					statement,
					count === 0 ? undefined : scratch[count - 1]
				);
			}
			scratch[count++] = statement;
		}
	...
	_parseSwitchStatementAt(start) {
		...
		for (let sawDefault = false; this.type !== tokTypes.braceR;) {
			if (this.type === tokTypes._case || this.type === tokTypes._default) {
				...
				const statement = this.parseStatement(null);
				if (declaresIntoBlock(statement)) {
					if (caseDeclarations === undefined) caseDeclarations = [];
			...
			if (declaresIntoBlock(statement)) {
				...
				blockDeclarations.push(
					...
					target.body.length === 0
						? undefined
						: target.body[target.body.length - 1]
				);
			}
