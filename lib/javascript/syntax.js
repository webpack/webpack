...
class SoaAst {
	...
	_packFacades() {
		...
		/** @type {(EXPECTED_OBJECT | undefined)[]} */
		const packed = Array.from({ length: this.count });
...
class WebpackParser extends BaseParser {
	/**
	 * @param {Options & { lazyNodes?: boolean, lazyComments?: CollectedComment[], importPhases?: boolean, moduleFallback?: boolean, soaAst?: boolean, transientAst?: boolean }} options options
	 * @param {string} input source code
	...
	parse() {
		...
		try {
			...
			// (untouched slack is only virtual, so transient stores skip it)
