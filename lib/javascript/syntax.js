 ...
 * _arrDepth: number,
 * _acquireScratch: () => any[],
 * _releaseScratch: (scratch: any[], count: number) => any[],
...
class WebpackParser extends BaseParser {
	...
	/**
	 * @param {any[]} scratch scratch array from `_acquireScratch`
	 * @param {number} count number of elements written
	 * @returns {any[]} exactly-sized copy of the first `count` entries
	 * @this {ParserInternals}
	 */
	...
	_readRadixNumber(radix) {
		...
		if (this._ecmaVersion >= 11 && input.charCodeAt(this.pos) === 110) {
			val = /** @type {any} */ (stringToBigInt(input.slice(start, this.pos)));
			++this.pos;
	...
	_readNumberCold(startsWithDot) {
		...
		if (!octal && !startsWithDot && this._ecmaVersion >= 11 && next === 110) {
			...
			return this.finishToken(tokTypes.num, /** @type {any} */ (val));
		}
