 ...
 * parseImportAttribute: () => EXPECTED_ANY,
...
/**
 * webpack's single extension over acorn — the one place parser behavior is
 * customized and the staging ground for incrementally taking over parsing.
 * Covers lazy `loc`/`range` (via the internal `lazySourcePositions` option),
 * import attributes (`with { ... }` and legacy `assert { ... }`) and import
 * phases (via the `importPhases` option). Unlike the former
 * `acorn-import-phases` package, acorn's `!forNew` guard is kept, so
 * `new import.defer(...)` is a SyntaxError just like `new import(...)`.
 * @type {(BaseParser: AcornParser) => AcornParser}
 */
const webpackSyntax = (Parser) => {
	// internal methods are absent from acorn's types
...
module.exports.webpackSyntax = webpackSyntax;
