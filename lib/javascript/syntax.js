...
// calls one of these module-level emitters. With `_soaStore` unset (the
// default) each emitter mirrors its node class constructor exactly; a
// `soaAst` parse installs a column store and the emitters fill columns and
// return identity-stable facades instead (phase C1). -----

/**
 * Active column store while a `soaAst` parse runs; `null` keeps the
 ...
 */
const _emitPropertyStart = (start) =>
	_soaStore === null ? new PropertyNode(start) : _soaPropertyStart(start);
 ...
 */
const _emitSwitchCaseStart = (start) =>
	_soaStore === null ? new SwitchCaseNode(start) : _soaSwitchCaseStart(start);
...
const UNSET = /** @type {EXPECTED_ANY} */ (undefined);
...
class SoaAst {
	...
	_grow(need) {
		...
		this.kid2 = /** @type {Int32Array} */ (grow(this.kid2, false));
		this.aux = /** @type {Int32Array} */ (grow(this.aux, false));
	...
	trim() {
		...
		// the copies are themselves allocation churn, so only trim stores
		// wasting more than half their capacity
		if (this.capacity - count > count) {
			this.types = this.types.slice(0, count);
			this.starts = this.starts.slice(0, count);
			this.ends = this.ends.slice(0, count);
			this.kid0 = this.kid0.slice(0, count);
			this.kid1 = this.kid1.slice(0, count);
			this.kid2 = this.kid2.slice(0, count);
			this.aux = this.aux.slice(0, count);
			this.flags = this.flags.slice(0, count);
			this.listStarts = this.listStarts.slice(0, count);
			this.listLens = this.listLens.slice(0, count);
			this.capacity = count;
		}
		const flatTop = this.flatTop;
		if (this.flat.length - flatTop > flatTop) {
...
class FacadeBase {
	...
	constructor(ast, id, type) {
		...
		this[kSoaStore] = ast;
		this[kSoaId] = id;
		// memo slots are pre-declared so every instance keeps one stable
		// shape and the memos stay in-object (no PropertyArray spill)
		this[kSoaMemoB] = UNSET;
	}
...
class MemberExpressionFacade extends FacadeBase {
	...
	constructor(ast, id) {
		...
		this[kSoaMemoA] = UNSET;
		this[kSoaMemoC] = UNSET;
	}
...
/**
 * Allocates a row plus its transient facade (the grammar flows nodes, so
 * an object is still handed back, but it is not registered — `nodeAt`
 * rebuilds an identical facade from the columns on demand). Scalar
 * columns (`flags`, `aux`) are written before the facade is built so
 * eager fields see them; child columns may be wired afterwards (child
 * getters are lazy).
 * @param {number} type numeric node type
 * @param {number} start start offset
 * @param {number} end end offset
 ...
 * @returns {FacadeBase} transient facade
 */
const _soaAlloc = (type, start, end, flags, aux) => {
	const ast = /** @type {SoaAst} */ (_soaStore);
	const id = ast.allocNode(type, start, end);
	if (flags !== 0) ast.flags[id] = flags;
	if (aux !== 0) ast.aux[id] = aux;
	return new FACADE_CLASSES[type](ast, id);
};

/**
 * Wires a fixed child into `kid0` when it lives in the current store;
 * a foreign child memoizes onto the facade instead (`null` stays ref 0).
 * @param {FacadeBase} facade owning facade
 * @param {EXPECTED_OBJECT | null | undefined} child child node
 * @param {symbol} memoKey facade memo slot serving this child
 * @returns {void}
 */
const _soaKid0 = (facade, child, memoKey) => {
	if (child === null) return;
	const ast = /** @type {SoaAst} */ (facade[kSoaStore]);
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (c[kSoaStore] === ast) {
		ast.kid0[/** @type {number} */ (facade[kSoaId])] = c[kSoaId];
	} else {
		/** @type {EXPECTED_ANY} */ (facade)[memoKey] = child;
		_soaPin(facade);
	}
};

/**
 * `_soaKid0` for the `kid1` column.
 * @param {FacadeBase} facade owning facade
 * @param {EXPECTED_OBJECT | null | undefined} child child node
 * @param {symbol} memoKey facade memo slot serving this child
 * @returns {void}
 */
const _soaKid1 = (facade, child, memoKey) => {
	if (child === null) return;
	const ast = /** @type {SoaAst} */ (facade[kSoaStore]);
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (c[kSoaStore] === ast) {
		ast.kid1[/** @type {number} */ (facade[kSoaId])] = c[kSoaId];
	} else {
		/** @type {EXPECTED_ANY} */ (facade)[memoKey] = child;
		_soaPin(facade);
	}
...
/**
 * `_soaKid0` for the `kid2` column.
 * @param {FacadeBase} facade owning facade
 * @param {EXPECTED_OBJECT | null | undefined} child child node
 * @param {symbol} memoKey facade memo slot serving this child
 * @returns {void}
 */
const _soaKid2 = (facade, child, memoKey) => {
	if (child === null) return;
	const ast = /** @type {SoaAst} */ (facade[kSoaStore]);
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (c[kSoaStore] === ast) {
		ast.kid2[/** @type {number} */ (facade[kSoaId])] = c[kSoaId];
	} else {
		/** @type {EXPECTED_ANY} */ (facade)[memoKey] = child;
		_soaPin(facade);
	}
...
/**
 * `_soaKid0` for the `aux` overflow column (`ForStatement.body`).
 * @param {FacadeBase} facade owning facade
 * @param {EXPECTED_OBJECT | null | undefined} child child node
 * @param {symbol} memoKey facade memo slot serving this child
 * @returns {void}
 */
const _soaKidAux = (facade, child, memoKey) => {
	if (child === null) return;
	const ast = /** @type {SoaAst} */ (facade[kSoaStore]);
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (c[kSoaStore] === ast) {
		ast.aux[/** @type {number} */ (facade[kSoaId])] = c[kSoaId];
	} else {
		/** @type {EXPECTED_ANY} */ (facade)[memoKey] = child;
		_soaPin(facade);
	}
...
/**
 * Seals a child list into the flat buffer when every element lives in the
 * current store (holes are ref 0); a foreign element pins the array itself
 * onto the facade, which then serves it ahead of the columns.
 * @param {FacadeBase} facade owning facade
 * @param {(EXPECTED_OBJECT | null)[]} list children
 * @param {symbol} memoKey facade memo slot serving this list
 * @returns {void}
 */
const _soaList = (facade, list, memoKey) => {
	const ast = /** @type {SoaAst} */ (facade[kSoaStore]);
	const len = list.length;
	for (let i = 0; i < len; i++) {
		const el = /** @type {EXPECTED_ANY} */ (list[i]);
		if (el !== null && el[kSoaStore] !== ast) {
			/** @type {EXPECTED_ANY} */ (facade)[memoKey] = list;
			_soaPin(facade);
			return;
		}
	}
	const id = /** @type {number} */ (facade[kSoaId]);
	const top = ast.flatTop;
	if (top + len > ast.flat.length) ast._growFlat(top + len);
	const flat = ast.flat;
	for (let i = 0; i < len; i++) {
		const el = /** @type {EXPECTED_ANY} */ (list[i]);
		flat[top + i] = el === null ? 0 : el[kSoaId];
	}
...
/**
 * SoA twin of `_emitIdentifier`.
 * @param {number} start start offset
 * @param {number} end end offset
 * @param {string} name identifier name
 * @returns {IdentifierNode} facade shaped like the node
 */
const _soaIdentifier = (start, end, name) => {
	const ast = /** @type {SoaAst} */ (_soaStore);
	const id = ast.allocNode(TYPE_IDENTIFIER, start, end);
	// escapes cook strictly shorter, so length equality means slice === name
	if (name.length !== end - start) ast.values[id] = name;
	const facade = new IdentifierFacade(ast, id);
	return /** @type {IdentifierNode} */ (/** @type {unknown} */ (facade));
};
				...
				: t === "boolean"
					...
					: value === null
						...
						: t === "bigint"
							...
							: LITERAL_REGEXP;
	ast.values[id] = value;
	const facade = new LiteralFacade(ast, id);
	return /** @type {LiteralNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaMemberExpression = (
	...
	_soaKid0(facade, object, kSoaMemoA);
	_soaKid1(facade, property, kSoaMemoC);
	return /** @type {MemberExpressionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaCallExpression = (start, end, callee, args, optional) => {
	...
	_soaKid0(facade, callee, kSoaMemoA);
	_soaList(facade, args, kSoaMemoC);
	return /** @type {CallExpressionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaThis = (start, end) =>
	/** @type {ThisNode} */ (
		/** @type {unknown} */ (_soaAlloc(TYPE_THIS_EXPRESSION, start, end, 0, 0))
	);
...
const _soaBinary = (start, end, type, left, operator, right) => {
	...
	_soaKid0(facade, left, kSoaMemoA);
	_soaKid1(facade, right, kSoaMemoC);
	return /** @type {BinaryNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaAssignment = (start, end, operator, left, right) => {
	...
	_soaKid0(facade, left, kSoaMemoA);
	_soaKid1(facade, right, kSoaMemoC);
	return /** @type {AssignmentNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaUnary = (start, end, type, operator, prefix, argument) => {
	...
	_soaKid0(facade, argument, kSoaMemoA);
	return /** @type {UnaryNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaVariableDeclaration = (start, end, declarations, kind) => {
	...
	_soaList(facade, declarations, kSoaMemoA);
	return /** @type {VariableDeclarationNode} */ (
		/** @type {unknown} */ (facade)
	);
};
...
const _soaVariableDeclarator = (start, end, id, init) => {
	const facade = _soaAlloc(TYPE_VARIABLE_DECLARATOR, start, end, 0, 0);
	_soaKid0(facade, id, kSoaMemoA);
	_soaKid1(facade, init, kSoaMemoC);
	return /** @type {VariableDeclaratorNode} */ (
		/** @type {unknown} */ (facade)
	);
};
...
const _soaExpressionStatement = (start, end, expression) => {
	const facade = _soaAlloc(TYPE_EXPRESSION_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, expression, kSoaMemoA);
	return /** @type {ExpressionStatementNode} */ (
		/** @type {unknown} */ (facade)
	);
};
...
const _soaBlockStatement = (start, end, body) => {
	const facade = _soaAlloc(TYPE_BLOCK_STATEMENT, start, end, 0, 0);
	_soaList(facade, body, kSoaMemoA);
	return /** @type {BlockStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaIfStatement = (start, end, test, consequent, alternate) => {
	const facade = _soaAlloc(TYPE_IF_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, test, kSoaMemoA);
	_soaKid1(facade, consequent, kSoaMemoC);
	_soaKid2(facade, alternate, kSoaMemoD);
	return /** @type {IfStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaReturnStatement = (start, end, argument) => {
	const facade = _soaAlloc(TYPE_RETURN_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, argument, kSoaMemoA);
	return /** @type {ReturnStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaConditionalExpression = (start, end, test, consequent, alternate) => {
	const facade = _soaAlloc(TYPE_CONDITIONAL_EXPRESSION, start, end, 0, 0);
	_soaKid0(facade, test, kSoaMemoA);
	_soaKid1(facade, consequent, kSoaMemoC);
	_soaKid2(facade, alternate, kSoaMemoD);
...
const _soaNewExpression = (start, end, callee, args) => {
	const facade = _soaAlloc(TYPE_NEW_EXPRESSION, start, end, 0, 0);
	_soaKid0(facade, callee, kSoaMemoA);
	_soaList(facade, args, kSoaMemoC);
	return /** @type {NewExpressionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaArrayExpression = (start, end, elements) => {
	const facade = _soaAlloc(TYPE_ARRAY_EXPRESSION, start, end, 0, 0);
	_soaList(facade, elements, kSoaMemoA);
	return /** @type {ArrayExpressionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaTemplateLiteral = (start, end, expressions, quasis) => {
	const facade = _soaAlloc(TYPE_TEMPLATE_LITERAL, start, end, 0, 0);
	const ast = /** @type {SoaAst} */ (_soaStore);
	const eLen = expressions.length;
	const qLen = quasis.length;
	let owned = qLen === eLen + 1;
	for (let i = 0; owned && i < qLen; i++) {
		const el = /** @type {EXPECTED_ANY} */ (quasis[i]);
		if (el === null || el[kSoaStore] !== ast) owned = false;
	}
	...
	if (!owned) {
		/** @type {EXPECTED_ANY} */ (facade)[kSoaMemoA] = expressions;
		/** @type {EXPECTED_ANY} */ (facade)[kSoaMemoC] = quasis;
		_soaPin(facade);
		return /** @type {TemplateLiteralNode} */ (/** @type {unknown} */ (facade));
	}
	const id = /** @type {number} */ (facade[kSoaId]);
	const len = eLen + qLen;
	const top = ast.flatTop;
	...
	for (let i = 0; i < eLen; i++) {
		flat[top + 2 * i] = /** @type {EXPECTED_ANY} */ (quasis[i])[kSoaId];
		flat[top + 2 * i + 1] = /** @type {EXPECTED_ANY} */ (expressions[i])[
			kSoaId
		];
	}
	flat[top + len - 1] = /** @type {EXPECTED_ANY} */ (quasis[eLen])[kSoaId];
	ast.listStarts[id] = top;
	ast.listLens[id] = len;
	ast.flatTop = top + len;
	return /** @type {TemplateLiteralNode} */ (/** @type {unknown} */ (facade));
...
const _soaTemplateElement = (start, end, value, tail) => {
	...
	ast.values[id] = value;
	const facade = new TemplateElementFacade(ast, id);
	return /** @type {TemplateElementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaObject = (start, end, type, properties) => {
	...
	_soaList(facade, properties, kSoaMemoA);
	return /** @type {ObjectNode} */ (/** @type {unknown} */ (facade));
};

/**
 * SoA twin of `_emitPropertyStart`.
 * @param {number} start start offset
 * @returns {PropertyNode} facade shaped like the node
 */
const _soaPropertyStart = (start) => {
	const facade = /** @type {PropertyFacade} */ (
		_soaAlloc(TYPE_PROPERTY, start, 0, 0, 0)
	);
	// born unfinished like `PropertyNode`: `finishNode` stamps the real
	// type/end, `parsePropertyName`/`parsePropertyValue` fill the rest —
	// state the columns never learn, so the facade stays pinned
	facade.type = "";
	facade.kind = "";
	_soaPin(facade);
	return /** @type {PropertyNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaRestSpread = (start, end, type, argument) => {
	...
	_soaKid0(facade, argument, kSoaMemoA);
	return /** @type {RestSpreadNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaFunction = (
	...
	_soaKid0(facade, id, kSoaMemoA);
	_soaList(facade, params, kSoaMemoC);
	_soaKid1(facade, body, kSoaMemoD);
	return /** @type {FunctionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaSequenceExpression = (start, end, expressions) => {
	const facade = _soaAlloc(TYPE_SEQUENCE_EXPRESSION, start, end, 0, 0);
	_soaList(facade, expressions, kSoaMemoA);
	return /** @type {SequenceExpressionNode} */ (
		/** @type {unknown} */ (facade)
	);
};
...
const _soaChainExpression = (start, end, expression) => {
	const facade = _soaAlloc(TYPE_CHAIN_EXPRESSION, start, end, 0, 0);
	_soaKid0(facade, expression, kSoaMemoA);
	return /** @type {ChainExpressionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaYieldExpression = (start, end, delegate, argument) => {
	...
	_soaKid0(facade, argument, kSoaMemoA);
	return /** @type {YieldExpressionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaAwaitExpression = (start, end, argument) => {
	const facade = _soaAlloc(TYPE_AWAIT_EXPRESSION, start, end, 0, 0);
	_soaKid0(facade, argument, kSoaMemoA);
	return /** @type {AwaitExpressionNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaExportSpecifier = (start, end, local, exported) => {
	const facade = _soaAlloc(TYPE_EXPORT_SPECIFIER, start, end, 0, 0);
	_soaKid0(facade, local, kSoaMemoA);
	_soaKid1(facade, exported, kSoaMemoC);
	return /** @type {ExportSpecifierNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaForStatement = (start, end, init, test, update, body) => {
	const facade = _soaAlloc(TYPE_FOR_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, init, kSoaMemoA);
	_soaKid1(facade, test, kSoaMemoC);
	_soaKid2(facade, update, kSoaMemoD);
	_soaKidAux(facade, body, kSoaMemoE);
	return /** @type {ForStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaForIn = (start, end, type, left, right, body) => {
	...
	const facade = _soaAlloc(TYPE_FOR_IN_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, left, kSoaMemoA);
	_soaKid1(facade, right, kSoaMemoC);
	_soaKid2(facade, body, kSoaMemoD);
	return /** @type {ForInNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaForOf = (start, end, isAwait, left, right, body) => {
	...
	_soaKid1(facade, right, kSoaMemoC);
	_soaKid2(facade, body, kSoaMemoD);
	return /** @type {ForOfNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaWhileStatement = (start, end, test, body) => {
	const facade = _soaAlloc(TYPE_WHILE_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, test, kSoaMemoA);
	_soaKid1(facade, body, kSoaMemoC);
	return /** @type {WhileStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaDoWhileStatement = (start, end, body, test) => {
	const facade = _soaAlloc(TYPE_DO_WHILE_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, body, kSoaMemoA);
	_soaKid1(facade, test, kSoaMemoC);
	return /** @type {DoWhileStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaSwitchStatement = (start, end, discriminant, cases) => {
	const facade = _soaAlloc(TYPE_SWITCH_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, discriminant, kSoaMemoA);
	_soaList(facade, cases, kSoaMemoC);
	return /** @type {SwitchStatementNode} */ (/** @type {unknown} */ (facade));
};

/**
 * SoA twin of `_emitSwitchCaseStart`.
 * @param {number} start start offset
 * @returns {SwitchCaseNode} facade shaped like the node
 */
const _soaSwitchCaseStart = (start) => {
	const facade = _soaAlloc(TYPE_SWITCH_CASE, start, 0, 0, 0);
	// born unfinished like `SwitchCaseNode`: the parser pushes into
	// `consequent` and fills `test`/`end` while the clause parses —
	// state the columns never learn, so the facade stays pinned
	/** @type {EXPECTED_ANY} */ (facade)[kSoaMemoA] = [];
	_soaPin(facade);
	return /** @type {SwitchCaseNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaTryStatement = (start, end, block, handler, finalizer) => {
	const facade = _soaAlloc(TYPE_TRY_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, block, kSoaMemoA);
	_soaKid1(facade, handler, kSoaMemoC);
	_soaKid2(facade, finalizer, kSoaMemoD);
	return /** @type {TryStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaCatchClause = (start, end, param, body) => {
	const facade = _soaAlloc(TYPE_CATCH_CLAUSE, start, end, 0, 0);
	_soaKid0(facade, param, kSoaMemoA);
	_soaKid1(facade, body, kSoaMemoC);
	return /** @type {CatchClauseNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaThrowStatement = (start, end, argument) => {
	const facade = _soaAlloc(TYPE_THROW_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, argument, kSoaMemoA);
	return /** @type {ThrowStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaLabeledStatement = (start, end, body, label) => {
	const facade = _soaAlloc(TYPE_LABELED_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, body, kSoaMemoA);
	_soaKid1(facade, label, kSoaMemoC);
	return /** @type {LabeledStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaBreakContinue = (start, end, type, label) => {
	...
	_soaKid0(facade, label, kSoaMemoA);
	return /** @type {BreakContinueNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaWithStatement = (start, end, object, body) => {
	const facade = _soaAlloc(TYPE_WITH_STATEMENT, start, end, 0, 0);
	_soaKid0(facade, object, kSoaMemoA);
	_soaKid1(facade, body, kSoaMemoC);
	return /** @type {WithStatementNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaArrayPattern = (start, end, elements) => {
	const facade = _soaAlloc(TYPE_ARRAY_PATTERN, start, end, 0, 0);
	_soaList(facade, elements, kSoaMemoA);
	return /** @type {ArrayPatternNode} */ (/** @type {unknown} */ (facade));
};
...
const _soaAssignmentPattern = (start, end, left, right) => {
	const facade = _soaAlloc(TYPE_ASSIGNMENT_PATTERN, start, end, 0, 0);
	_soaKid0(facade, left, kSoaMemoA);
	_soaKid1(facade, right, kSoaMemoC);
	return /** @type {AssignmentPatternNode} */ (/** @type {unknown} */ (facade));
};
...
class WebpackParser extends BaseParser {
	/**
	 * @param {Options & { lazyNodes?: boolean, lazyComments?: CollectedComment[], importPhases?: boolean, moduleFallback?: boolean, soaAst?: boolean }} options options
	 * @param {string} input source code
	...
	constructor(options, input, startPos) {
		...
		this._lazy = lazy;
		// SoA emission (phase C1, opt-in): the `_emit*` seam fills the column
		// store and returns facades instead of single-shape nodes
		this._soaAst = lazy && options.soaAst === true;
	 ...
	 * slot for the duration when `soaAst` is on. Parsing is synchronous and
	...
	parse() {
		...
		try {
			...
			// parsing is done growing — release the columns' allocation slack
			store.trim();
			return program;
		} finally {
			_soaStore = previous;
	...
	_parseFunctionBody(fnStart, id, params, isArrowFunction, isMethod, forInit) {
		...
		if (isExpression) {
			...
			const store = /** @type {EXPECTED_ANY} */ (body)[kSoaStore];
			// a pinned list (foreign statements) already holds the facades, so
			// the object path is free there; owned blocks scan the columns
			// without materializing the body
			if (
				store !== undefined &&
				/** @type {EXPECTED_ANY} */ (body)[kSoaMemoA] === undefined
			) {
				_soaAdaptDirectivePrologue(
					store,
					/** @type {number} */ (/** @type {EXPECTED_ANY} */ (body)[kSoaId])
				);
			} else {
