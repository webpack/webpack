...
class WebpackParser extends BaseParser {
	...
	parseParenAndDistinguishExpression(canBeArrow, forInit) {
		...
		if (exprList.length > 1) {
			val = /** @type {Expression} */ (
				/** @type {unknown} */ (
					new SequenceExpressionNode(innerStartPos, innerEndPos, exprList)
				)
			);
		} else {
			val = exprList[0];
	...
	parseExpression(forInit, refDestructuringErrors) {
		...
		if (this.type === tokTypes.comma) {
			...
			return /** @type {Expression} */ (
				/** @type {unknown} */ (
					new SequenceExpressionNode(startPos, this.lastTokEnd, expressions)
				)
			);
		}
		return expr;
		...
		for (;;) {
			...
			if (element === baseExpr || element.type === "ArrowFunctionExpression") {
				if (optionalChained) {
					return /** @type {Expression} */ (
						/** @type {unknown} */ (
							new ChainExpressionNode(startPos, this.lastTokEnd, element)
						)
					);
				}
				return element;
