...
const _soaKid1Id = (ast, id, child, memoKey) => {
	// no `kid1` consumer passes null children, so no hole guard
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (typeof c === "number") {
		ast.kid1[id] = c;
	} else if (c[kSoaStore] === ast) {
...
const _soaKid2Id = (ast, id, child, memoKey) => {
	// only the conditional's `alternate` lands here: always present, and a
	// non-ref can only be foreign
...
const _soaListId = (ast, id, list, memoKey) => {
	...
	for (let i = 0; i < len; i++) {
		...
		if (el !== null && typeof el !== "number") {
			// a node-object element (foreign) pins the list onto the owner's
			// facade; the memoized array is served verbatim, so raw refs
			// materialize first
			for (let j = 0; j < len; j++) {
	...
	const flat = ast.flat;
	for (let i = 0; i < len; i++) {
		// elements here are raw refs or holes — node objects took the memo path
		const el = /** @type {EXPECTED_ANY} */ (list[i]);
		flat[top + i] = el === null ? 0 : el;
	}
...
const _soaKid0 = (facade, child, memoKey) => {
	...
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (typeof c === "number") {
		ast.kid0[/** @type {number} */ (facade[kSoaId])] = c;
	} else if (c[kSoaStore] === ast) {
...
const _soaKid1 = (facade, child, memoKey) => {
	...
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (typeof c === "number") {
		ast.kid1[/** @type {number} */ (facade[kSoaId])] = c;
	} else if (c[kSoaStore] === ast) {
...
const _soaKid2 = (facade, child, memoKey) => {
	...
	const c = /** @type {EXPECTED_ANY} */ (child);
	if (typeof c === "number") {
		ast.kid2[/** @type {number} */ (facade[kSoaId])] = c;
	} else if (c[kSoaStore] === ast) {
...
const _soaFunction = (
	...
	// declarations flow into statement lists, whose top-level consumers
	// (the walk's entry) still expect node objects
	return /** @type {FunctionNode} */ (
		/** @type {unknown} */ (declaration ? ast.nodeAt(ref) : ref)
	);
};
