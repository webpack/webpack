 ...
 * operator or a keyword it opens a regexp.
 * @param {string} source source code
...
/**
 * Reads a `'`/`"` string starting at `i`. A specifier never carries an escape,
 * so one there rejects the literal rather than growing a decoder. Templates are
 * not specifiers here — the ast this replaces only matched `Literal` nodes.
 * @param {string} source source code
 * @param {number} i offset of the opening quote
 * @returns {{ value: string, end: number } | undefined} the literal
 */
const readPlainString = (source, i) => {
	const quote = source.charCodeAt(i);
	if (quote !== 34 && quote !== 39) return;
	for (let j = i + 1; j < source.length; j++) {
		const code = source.charCodeAt(j);
		if (code === 92 || code === 10 || code === 13) return;
...
const collectCjsRequireSpecifiers = (source) => {
	...
	for (let i = 0; i < source.length; i++) {
		...
		const literal = readPlainString(source, j);
