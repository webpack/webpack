...
// WORD_CACHE's regime applied to literal text, where a large input holds some
// ten times more number literals than distinct raws, so one shared string cuts
...
class WebpackParser extends BaseParser {
	...
	constructor(options, input, startPos) {
		...
		// the owned per-token loop also serves the public non-lazy tokenizer():
		// locations off, no onToken, and no plugin override of the acorn methods it
		// inlines — its shortcuts bypass them even when the full path below is off
		/** @type {boolean} */
		this._tokenFastPath =
			lazy ||
			(this._noLocations &&
