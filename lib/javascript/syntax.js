...
// Classification memo in front of `WORD_CACHE`: word identity plus the id of
// the keyword set that classified it, so a slot written by another option set
// misses instead of mis-classifying. One shared table (not one per keyword
// set) keeps the footprint constant; it is smaller than `WORD_CACHE` because
// only the few classified-as-keyword words repeat often enough to matter.
const WORD_TYPE_MASK = 0x7ff;
/** @type {(string | null)[]} */
const WORD_TYPE_WORDS = Array.from({ length: WORD_TYPE_MASK + 1 }, () => null);
/** @type {(TokenType | null)[]} */
const WORD_TYPES = Array.from({ length: WORD_TYPE_MASK + 1 }, () => null);
const WORD_TYPE_OWNERS = new Int32Array(WORD_TYPE_MASK + 1);
...
class WebpackParser extends BaseParser {
	...
	readWord() {
		...
		if (wordLen >= 2 && wordLen <= WORD_CACHE_MAX_LEN) {
			const slot = hash & WORD_CACHE_MASK;
			const typeSlot = hash & WORD_TYPE_MASK;
			...
			if (
				...
				word = cached;
				if (
					WORD_TYPE_WORDS[typeSlot] === cached &&
					WORD_TYPE_OWNERS[typeSlot] === owner
				) {
					type = /** @type {TokenType} */ (WORD_TYPES[typeSlot]);
				} else {
					type = classifyWord(cached, lookups.keywords);
					WORD_TYPE_WORDS[typeSlot] = cached;
					WORD_TYPES[typeSlot] = type;
					WORD_TYPE_OWNERS[typeSlot] = owner;
				}
			} else {
