...
// AST nodes. `kSlowContext` is the per-TokContext "step aside" flag, cached on
// the context so the hot guard reads one slot rather than two.
...
const CLS_NEWLINE = 7;
const CLS_SLASH = 8;
// Covers the whole `charCodeAt` range, so the scan loop needs no `code > 127`
...
const ensureParserCaches = () => {
	...
	WORD_CACHE = Array.from(
		...
		(_, i) => (i % WORD_CACHE_STRIDE === 2 ? 0 : null)
	);
...
class WebpackParser extends BaseParser {
	...
	readWord1() {
		...
		if (wordLen >= 2 && wordLen <= WORD_CACHE_MAX_LEN) {
			...
			const cached = /** @type {string | null} */ (WORD_CACHE[slot]);
			if (cached !== null && wordMatches(input, start, cached, wordLen)) {
				return cached;
			}
	...
	readWord() {
		...
		if (wordLen >= 2 && wordLen <= WORD_CACHE_MAX_LEN) {
			...
			if (cached !== null && wordMatches(input, start, cached, wordLen)) {
				word = cached;
				if (WORD_CACHE[slot + 2] === owner) {
					type = /** @type {TokenType} */ (WORD_CACHE[slot + 1]);
					this._wordKind = /** @type {number} */ (WORD_CACHE[slot + 3]);
				} else {
					type = classifyWord(cached, lookups.keywords);
					WORD_CACHE[slot + 1] = type;
					WORD_CACHE[slot + 2] = owner;
					this._wordKind = WORD_CACHE[slot + 3] = wordKind(cached, lookups);
				}
			} else {
		...
		// plain name and can be neither `of` nor `yield`, nor reserved.
		this._wordKind = 0;
