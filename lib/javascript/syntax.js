...
const { Parser: AcornParser, tokContexts, tokTypes } = require("acorn");
...
/** @typedef {TokenType & { beforeExpr: boolean, updateContext?: (prevType: TokenType) => void }} TokenTypeInternal acorn's internal TokenType fields, absent from its public types */
...
const SIMPLE_PUNCT = new Array(128);
...
class MemberExpressionNode {
	 ...
	 * @param {number} start start offset
	 * @param {number} end end offset
	 * @param {AcornExpression} object object expression
	 * @param {AcornNode} property property node
	 * @param {boolean} computed whether the access is computed (`a[b]`)
	 * @param {boolean} optional whether the access is optional (`a?.b`)
	 */
	constructor(sourcePositions, start, end, object, property, computed, optional) {
...
class CallExpressionNode {
	 ...
	 * @param {number} start start offset
	 * @param {number} end end offset
	 * @param {AcornExpression} callee callee expression
	 * @param {AcornNode[]} args call arguments
...
class BinaryNode {
	 ...
	 * @param {AcornExpression} left left operand
	 * @param {string} operator operator text
	 * @param {AcornExpression} right right operand
	 */
 ...
 * parseLiteral: (value: unknown) => AcornNode,
 * awaitIdentPos: number,
 * lastTokStart: number,
 * yieldPos: number,
 * awaitPos: number,
 * parseExpression: () => AcornExpression,
 * parseExprList: (close: TokenType, allowTrailingComma: boolean, allowEmpty: boolean, refDestructuringErrors: DestructuringErrorsShim) => AcornExpression[],
 * parsePrivateIdent: () => AcornNode,
 * parseTemplate: (opts: { isTagged: boolean }) => AcornNode,
 * shouldParseAsyncArrow: () => boolean,
 * parseSubscriptAsyncArrow: (startPos: number, startLoc: AcornPosition | undefined, exprList: AcornExpression[], forInit: boolean | string) => AcornExpression,
 ...
 * checkExpressionErrors: (refDestructuringErrors: DestructuringErrorsShim, andThrow?: boolean) => boolean,
 * parseSubscript: (base: AcornExpression, startPos: number, startLoc: AcornPosition | undefined, noCalls: boolean | undefined, maybeAsyncArrow: boolean, optionalChained: boolean, forInit: boolean | string) => AcornExpression,
 * parseExprAtom: (refDestructuringErrors?: DestructuringErrorsShim | null, forInit?: boolean | string, forNew?: boolean) => AcornExpression,
 * buildBinary: (startPos: number, startLoc: AcornPosition | undefined, left: AcornExpression, right: AcornExpression, op: string, logical: boolean) => AcornExpression,
 * potentialArrowAt: number,
 * potentialArrowInForAwait: boolean,
 * overrideContext: (tokenCtx: unknown) => void,
 * parseFunction: (node: AcornNode, statement: number, allowExpressionBody?: boolean, isAsync?: boolean, forInit?: boolean | string) => AcornExpression,
 * parseArrowExpression: (node: AcornNode, params: AcornNode[], isAsync: boolean, forInit?: boolean | string) => AcornExpression,
...
/**
 * Field-compatible stand-in for acorn's internal `DestructuringErrors` class
 * (not exported); acorn only ever reads/writes these five offsets.
...
class WebpackParser extends AcornParser {
	 ...
	 * @returns {AcornIdentifier} identifier node
	 * @this {ParserInternals}
	 */
	parseIdent(liberal) {
		...
		const node = /** @type {AcornIdentifier} */ (
			/** @type {unknown} */ (
				new IdentifierNode(
					sourcePositions,
					this.start,
	 ...
	 * @param {AcornExpression} baseExpr subscript base
	 * @param {number} startPos expression start offset
	 * @param {AcornPosition | undefined} startLoc expression start position
	 ...
	 * @returns {AcornExpression} subscript element or `baseExpr` when done
	 * @this {ParserInternals}
	 */
	parseSubscript(
		baseExpr,
		...
		if (
			...
			} else if (this.type === tokTypes.privateId && baseExpr.type !== "Super") {
			...
			/** @type {AcornNode & { tag?: AcornExpression, quasi?: AcornNode }} */
			(node).tag = baseExpr;
			/** @type {AcornNode & { tag?: AcornExpression, quasi?: AcornNode }} */
	 ...
	 * @returns {AcornExpression} binary or logical expression node
	 * @this {ParserInternals}
	 */
	 ...
	 * @returns {AcornExpression} expression atom
	 * @this {ParserInternals}
	 */
	parseExprAtom(refDestructuringErrors, forInit, forNew) {
		...
		if (
			...
			this.next();
			return /** @type {AcornExpression} */ (/** @type {unknown} */ (node));
		}
		...
		return base.parseExprAtom.call(this, refDestructuringErrors, forInit, forNew);
	}
	 ...
	 * @param {unknown} value literal value
	 * @returns {AcornNode} literal node
	 * @this {ParserInternals}
	 */
	parseLiteral(value) {
		const sourcePositions = this[kSourcePositions];
		...
		this.next();
		return /** @type {AcornNode} */ (/** @type {unknown} */ (node));
	}
