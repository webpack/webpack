 ...
 * @property {(string | null)[]} typeWords `WORD_CACHE`-slot-parallel memo keys (interned word per slot)
 * @property {(TokenType | null)[]} types memoized token type per slot, valid when `typeWords[slot]` matches
 */
...
const getWordLookups = (parser) => {
	...
	const lookups = {
		...
		typeWords: Array.from({ length: WORD_CACHE_MASK + 1 }, () => null),
		types: Array.from({ length: WORD_CACHE_MASK + 1 }, () => null)
	};
	...
	readWord() {
		...
		if (wordLen >= 2 && wordLen <= WORD_CACHE_MAX_LEN) {
			...
			if (
				...
				// memo is per keyword set and keyed by string identity, so a slot
				// reused by another option set can only miss, never mis-classify
				if (lookups.typeWords[slot] === cached) {
					return this.finishToken(
						/** @type {TokenType} */ (lookups.types[slot]),
						cached
					);
				}
				const type = classifyWord(cached, lookups.keywords);
				lookups.typeWords[slot] = cached;
				lookups.types[slot] = type;
				return this.finishToken(type, cached);
			}
			const word = input.slice(start, pos);
			WORD_CACHE[slot] = word;
			const type = classifyWord(word, lookups.keywords);
			lookups.typeWords[slot] = word;
			lookups.types[slot] = type;
