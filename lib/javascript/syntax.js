...
class WebpackParser extends BaseParser {
	...
	parseTopLevel(node) {
		...
		if (!this._lazy || this.options.program) {
			return baseProto().parseTopLevel.call(this, node);
		}
	...
	parseVarStatement(node, kind, allowMissingInitializer) {
		if (!this._lazy || !this._varIdOwn) {
			return baseProto().parseVarStatement.call(
				this,
				node,
				kind,
	...
	parseFunctionBody(node, isArrowFunction, isMethod, forInit) {
		if (!this._funcFastPath) {
			return baseProto().parseFunctionBody.call(
				this,
				node,
				isArrowFunction,
				isMethod,
	...
	parseClass(node, isStatement) {
		if (!this._classFastPath) {
			return baseProto().parseClass.call(this, node, isStatement);
		}
		const classNode =
