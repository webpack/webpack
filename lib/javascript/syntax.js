...
/**
 * Estimates the node count of a source: separator density in a mid-file
 * sample (past any license banner) tracks nodes-per-byte within ~15%
 * across formatting styles, from minified (~2.6 B/node) to commented
 * (~11 B/node) — a flat divisor either re-copies every column mid-parse
 * on dense sources or over-allocates loose ones ~2x.
 * @param {string} source source code
 * @returns {number} estimated node capacity
 */
const estimateCapacity = (source) => {
	const len = source.length;
	const mid = len > 20000 ? len >> 1 : 0;
	const window = Math.min(8192, len - mid);
	let punct = 0;
	for (let i = mid, end = mid + window; i < end; i++) {
	...
	// ~1 node per separator, 15% headroom — undershoot grows every column
	return window === 0
		? MIN_CAPACITY
		: Math.max(MIN_CAPACITY, Math.ceil((len * punct * 1.15) / window));
};

/**
 * Moves a pre-sized side store (values, facades) onto a larger array so
 * writes at grown node ids stay in-bounds fast-elements stores.
 * @param {EXPECTED_ANY[]} store current side store
 * @param {number} capacity new node capacity
 * @returns {EXPECTED_ANY[]} moved side store
 */
const growSideStore = (store, capacity) => {
	// eslint-disable-next-line unicorn/no-new-array
	const next = new Array(capacity);
	for (let i = store.length - 1; i >= 1; i--) next[i] = store[i];
	return next;
};
...
class SoaAst {
	 ...
	 */
	constructor(source) {
		const capacity = estimateCapacity(source);
		this.source = source;
		...
		const buffer = new ArrayBuffer(
			(capacity * 7 + flatCapacity) * 4 + capacity * 2
		);
		let offset = 0;
		/** @type {Int32Array} */
		...
		offset += capacity * 4;
		/**
		 * @type {Int32Array} shared child-list buffer: each `listStarts`
		 * span is `[length, ...child refs]`; slot 0 stays 0 so a node
		 * without a list (`listStarts` 0) reads length 0
		 */
		...
		// slot 0 is the reserved zero-length read for list-less nodes
		this.flatTop = 1;
		// literal values and identifier names, sparse by node id; pre-sized so
		// the ~50%-density writes stay in-bounds — progressive growth re-pays
		// backing-store copies (~5x per write at 1M nodes)
		/** @type {EXPECTED_ANY[]} */
		// eslint-disable-next-line unicorn/no-new-array
		this.values = new Array(capacity);
		// identity-stable facades; filled lazily on materialization — the
		// first pin sizes the array (`nodeAt`) so probe reads stay on fast
		// holey elements instead of a dictionary
	 ...
	 * @param {number} consumed source bytes consumed so far
	 * @returns {void}
	 */
	_grow(need, consumed) {
		...
		// project the final count from the density seen so far — doubling
		// alone re-copies every column again on dense minified sources
		if (consumed > 0) {
			const projected =
				Math.ceil((need * this.source.length) / consumed) + (need >> 4) + 16;
			if (projected > capacity) capacity = projected;
		}
		this._reallocColumns(capacity, this.count);
		// re-size the side stores too, so value/pin writes stay in-bounds
		this.values = growSideStore(this.values, capacity);
		if (this.facades.length !== 0) {
			this.facades = growSideStore(this.facades, capacity);
		}
	}
	...
	_reallocColumns(capacity, used) {
		const buffer = new ArrayBuffer(capacity * 30);
		let offset = 0;
		/**
	...
	allocNode(type, start, end) {
		const id = this.count++;
		if (id >= this.capacity) this._grow(id + 1, start);
		this.types[id] = type;
	...
	setList(id, childRefs) {
		...
		if (top + len + 1 > this.flat.length) this._growFlat(top + len + 1);
		const flat = this.flat;
		flat[top] = len;
		for (let i = 0; i < len; i++) flat[top + 1 + i] = childRefs[i];
		this.listStarts[id] = top;
		this.flatTop = top + len + 1;
	}
	...
	trim() {
		...
		// right-trim the pre-sized side store's hole tail
		if (this.values.length > count) this.values.length = count;
	}
	...
	nodeAt(id) {
		...
		// the first pin sizes the memo — an out-of-bounds write would demote
		// the bare array to dictionary elements (hash probes ever after)
		if (this.facades.length === 0) {
			// eslint-disable-next-line unicorn/no-new-array
			this.facades = new Array(this.capacity);
		}
	 ...
	 */
	listAt(id) {
		const base = this.listStarts[id];
		const flat = this.flat;
		const len = flat[base];
		/** @type {(EXPECTED_OBJECT | null)[]} */
		const out = Array.from({ length: len });
		for (let i = 0; i < len; i++) out[i] = this.nodeAt(flat[base + 1 + i]);
		return out;
	}
}
...
class TemplateLiteralFacade extends FacadeBase {
	...
	get expressions() {
		...
		const start = ast.listStarts[id] + 1;
		const len = ast.flat[start - 1];
		const out = [];
...
const _soaAdaptDirectivePrologue = (ast, blockId) => {
	const start = ast.listStarts[blockId] + 1;
	const { flat, types, kid0, aux, starts, source } = ast;
	const len = flat[start - 1];
	for (let i = 0; i < len; i++) {
...
/**
 * Declarator count of a grammar-flowing `VariableDeclaration` (declarator
 * refs always land in the flat list, so its length prefix is authoritative).
 * @param {EXPECTED_ANY} n flowing declaration
 * @returns {number} declarator count
 */
const _refDeclarationSize = (n) => {
	if (typeof n !== "number") return n.declarations.length;
	const ast = /** @type {SoaAst} */ (_soaStore);
	return ast.flat[ast.listStarts[n]];
};

/**
 * `kind` of a grammar-flowing `VariableDeclaration`.
 * @param {EXPECTED_ANY} n flowing declaration
 * @returns {string} declaration kind
 */
const _refDeclarationKind = (n) =>
	typeof n === "number"
		? DECLARATION_KINDS[/** @type {SoaAst} */ (_soaStore).aux[n]]
		: n.kind;

/**
 * First declarator of a grammar-flowing `VariableDeclaration`, materialized
 * for the object-shaped for-in/of head checks (single-facade cost on those
 * heads only — three-clause `for` heads never read a declarator).
 * @param {EXPECTED_ANY} n flowing declaration
 * @returns {EXPECTED_ANY} first declarator node
 */
const _refFirstDeclarator = (n) => {
	if (typeof n !== "number") return n.declarations[0];
	const ast = /** @type {SoaAst} */ (_soaStore);
	return ast.nodeAt(ast.flat[ast.listStarts[n] + 1]);
};
...
const _soaListId = (ast, id, list, memoKey) => {
	...
	if (len === 0) return;
	// one scan proves the common all-refs shape (branchless copy below) and
	// catches foreign elements, which pin the list onto the owner's facade
	let mixed = false;
	for (let i = 0; i < len; i++) {
		const el = /** @type {EXPECTED_ANY} */ (list[i]);
		if (typeof el === "number") continue;
		if (el !== null && el[kSoaStore] !== ast) {
			// the memoized array is served verbatim, so raw refs materialize
			for (let j = 0; j < len; j++) {
	...
	if (top + len + 1 > ast.flat.length) ast._growFlat(top + len + 1);
	const flat = ast.flat;
	flat[top] = len;
	if (mixed) {
		for (let i = 0; i < len; i++) {
			// holes or store-backed node objects (adopted statements)
			const el = /** @type {EXPECTED_ANY} */ (list[i]);
			flat[top + 1 + i] =
				el === null ? 0 : typeof el === "number" ? el : el[kSoaId];
		}
	} else {
		for (let i = 0; i < len; i++) {
			flat[top + 1 + i] = /** @type {number} */ (
				/** @type {unknown} */ (list[i])
			);
		}
	}
	ast.listStarts[id] = top;
	ast.flatTop = top + len + 1;
};
...
const _soaTemplateLiteral = (start, end, expressions, quasis) => {
	...
	const base = top + 1;
	for (let i = 0; i < eLen; i++) {
		flat[base + 2 * i] = /** @type {number} */ (
			/** @type {unknown} */ (quasis[i])
		);
		flat[base + 2 * i + 1] = /** @type {number} */ (
			/** @type {unknown} */ (expressions[i])
		);
	}
	flat[base + len - 1] = /** @type {number} */ (
		/** @type {unknown} */ (quasis[eLen])
	);
	ast.listStarts[id] = top;
	ast.flatTop = base + len;
	return /** @type {TemplateLiteralNode} */ (/** @type {unknown} */ (id));
};
...
class WebpackParser extends BaseParser {
	...
	parse() {
		...
		try {
			...
			// allocation slack (untouched slack is only virtual and the
			// facade memo is pre-sized, so the transient walk arm skips it)
			if (!this._soaTransient) store.trim();
			return program;
		} finally {
			_soaStore = previous;
		}
	...
	parseForStatement(node) {
		...
		if (this.type === tokTypes._var || this.type === tokTypes._const || isLet) {
			// the head declaration goes through the emitter seam (a raw ref in
			// SoA mode) — the classic three-clause head then stays facade-free
			const declStart = this.start;
			const kind = isLet ? "let" : /** @type {string} */ (this.value);
			this.next();
			/** @type {Node[]} */
			const declarations = [];
			this._parseVarInto(declarations, true, kind);
		...
		const usingKind = this.isUsing(true)
			...
			: this.isAwaitUsing(true)
				...
				: null;
		if (usingKind) {
			// `using` heads stay acorn-built — the id walkers rely on owned
			// head declarations being var/let/const only
	...
	parseForAfterInit(node, init, awaitAt) {
		...
		if (
			...
			_refDeclarationSize(init) === 1
		) {
			if (this.type === tokTypes._in) {
				const kind = _refDeclarationKind(init);
				if (
					(kind === "using" || kind === "await using") &&
					!(
						/** @type {Node & { init: Node | null }} */ (
							_refFirstDeclarator(init)
						).init
					)
				) {
					this.raise(
	...
	parseForIn(node, init) {
		...
		this.next();
		if (
			_refIs(init, TYPE_VARIABLE_DECLARATION, "VariableDeclaration") &&
			/** @type {Node & { init: Node | null, id: Node }} */ (
				_refFirstDeclarator(init)
			).init !== null &&
			(!isForIn ||
				this._ecmaVersion < 8 ||
				this.strict ||
				_refDeclarationKind(init) !== "var" ||
				/** @type {Node & { init: Node | null, id: Node }} */ (
					_refFirstDeclarator(init)
				).id.type !== "Identifier")
		) {
		...
		if (this._ecmaVersion >= 6 && node) {
			...
			if (typeof node === "number") {
				...
				if (t === TYPE_OBJECT_EXPRESSION) {
					...
					const start = ast.listStarts[node] + 1;
					const len = ast.flat[start - 1];
					for (let i = 0; i < len; i++) {
	...
	checkLValPattern(expr, bindingType, checkClashes) {
		...
		if (typeof expr === "number") {
			...
			if (t === TYPE_OBJECT_PATTERN) {
				const start = ast.listStarts[expr] + 1;
				const len = ast.flat[start - 1];
				for (let i = 0; i < len; i++) {
