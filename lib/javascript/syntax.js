...
const _soaProperty = (
	...
	// serve the children from the transient while it lives (`checkPropClash`,
	// pattern conversion read them right after), keeping `nodeAt` out of parse
	/** @type {EXPECTED_ANY} */ (facade)[kSoaMemoA] = key;
	/** @type {EXPECTED_ANY} */ (facade)[kSoaMemoC] = value;
...
const _soaSwitchCase = (start, end, test, consequent) => {
	const facade = _soaAlloc(TYPE_SWITCH_CASE, start, end, 0, 0);
	_soaKid0(
		facade,
		/** @type {EXPECTED_OBJECT | null} */ (/** @type {unknown} */ (test)),
		kSoaMemoC
	);
	_soaList(facade, consequent, kSoaMemoA);
