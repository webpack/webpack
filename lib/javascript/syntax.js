...
/**
 * Allocates a row plus its registered facade. Scalar columns (`flags`,
 * `aux`) are written before the facade is built so eager fields see them;
 * child columns may be wired afterwards (child getters are lazy).
 * @param {number} type numeric node type
 * @param {number} start start offset
 * @param {number} end end offset
 * @param {number} flags packed flag bits
 * @param {number} aux operator/kind id
 * @returns {FacadeBase} registered facade
 */
const _soaAlloc = (type, start, end, flags, aux) => {
	...
	const facade = new FACADE_CLASSES[type](ast, id);
	ast.facades[id] = facade;
	return facade;
};
