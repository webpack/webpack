...
// cspell:ignore yuku binop prec Prec Unsyntactic
...
class WebpackParser extends BaseParser {
	...
	parseParenAndDistinguishExpression(canBeArrow, forInit) {
		...
		if (exprList.length > 1) {
			const seq = this.startNodeAt(innerStartPos, undefined);
			/** @type {Node & { expressions?: Expression[] }} */ (seq).expressions =
				exprList;
			val = /** @type {Expression} */ (
				this.finishNodeAt(seq, "SequenceExpression", innerEndPos, undefined)
			);
		} else {
			val = exprList[0];
		}
	...
	parseForIn(node, init) {
		...
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new ForOfNode(node.start, this.lastTokEnd, isAwait, init, right, body)
			)
		);
	}
	...
	parseWhileStatement(node) {
		...
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new WhileStatementNode(node.start, this.lastTokEnd, test, body)
			)
		);
	}
	...
	parseDoStatement(node) {
		...
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new DoWhileStatementNode(node.start, this.lastTokEnd, body, test)
			)
		);
	}
	...
	parseThrowStatement(node) {
		...
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new ThrowStatementNode(node.start, this.lastTokEnd, argument)
			)
		);
	}
	...
	parseWithStatement(node) {
		...
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new WithStatementNode(node.start, this.lastTokEnd, object, body)
			)
		);
	}
	...
	parseEmptyStatement(node) {
		...
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new BareStatementNode(node.start, this.lastTokEnd, "EmptyStatement")
			)
		);
	}
	...
	parseDebuggerStatement(node) {
		...
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new BareStatementNode(node.start, this.lastTokEnd, "DebuggerStatement")
			)
		);
	}
		...
		if (this._ecmaVersion >= 6) {
			switch (this.type) {
				case tokTypes.bracketL: {
					...
					return /** @type {Node} */ (
						/** @type {unknown} */ (
							new ArrayPatternNode(start, this.lastTokEnd, elements)
						)
					);
				}
				case tokTypes.braceL:
