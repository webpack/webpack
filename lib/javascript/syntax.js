...
class WebpackParser extends BaseParser {
	 ...
	 * Hex/unicode/octal escapes (`\x`, `\u`, `\0`-`\9`) restart acorn's
	 * implementation, which owns their readers and strict-mode errors; so do old
	 * ecmaVersions (LS/PS terminate strings there) and location tracking.
	 * @this {ParserInternals}
	...
	readString(quote) {
		...
		for (;;) {
			...
			if (ch === 92) {
				...
				// hex/unicode/octal escapes (and a trailing `\`) need acorn's readers
				// and strict-mode errors; restart from the quote (this.pos never moved)
				if (
					pos + 1 >= len ||
					esc === 120 ||
					esc === 117 ||
