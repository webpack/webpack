...
import { Parser as BaseParser, tokTypes } from "acorn";

// acorn exports these at runtime but leaves them out of its public types
const { keywordTypes, tokContexts } = /** @type {EXPECTED_ANY} */ (acorn);
...
class WebpackParser extends BaseParser {
	...
	parseTemplateElement(opts) {
		...
		if (this.type === tokTypes.invalidTemplate) {
			...
			value = {
				raw: /** @type {string} */ (this.value).replaceAll(/\r\n?/g, "\n"),
				cooked: null
			};
		} else {
			value = {
				raw: this.input.slice(this.start, this.end).replaceAll(/\r\n?/g, "\n"),
	...
	parseLiteral(value) {
		...
		if (raw.charCodeAt(raw.length - 1) === 110) {
			// trailing "n" marks a bigint literal; acorn always sets `value` for it
	...
	declareName(name, bindingType, pos) {
		...
		if (bindingType === BIND_LEXICAL) {
			...
			redeclared = this.treatFunctionsAsVar
				...
				: (scope.lexical !== undefined && scope.lexical.has(name)) ||
					(scope.var !== undefined && scope.var.has(name));
			(scope.functions ||= new Set()).add(name);
		} else {
	...
	parseWithClause() {
		...
		while (!this.eat(tokTypes.braceR)) {
			...
			if (Object.hasOwn(attributeKeys, keyName)) {
				this.raiseRecoverable(
					attr.key.start,
...
export { LEGACY_ASSERT_ATTRIBUTES };
export { SourcePositions };
export { WebpackParser };
