 ...
 * @property {string=} moduleConcatenationRootBailout what rules the module out as a concatenation root, while it may still be an inner module
 ...
 * @property {Set<string>=} stableExports names of own exports the language pins to one value, so an importer may read them into a local binding
 * @property {Map<string, [number, number][]>=} classHeritageRanges per class name, the ranges of the heritage clauses parsed in the enclosing scope, where a reference to that name resolves to the class at runtime but to the outer name here
