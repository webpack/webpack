...
class JavascriptModulesPlugin {
	...
	constructor(options = {}) {
		...
		/** @type {WeakMap<Source, { source: Source, needModule:boolean, needExports: boolean, needRequire: boolean, needThisAsExports: boolean, needStrict: boolean | undefined, needRequireGlobal: boolean, renderShorthand: boolean }>} */
	...
	renderModule(module, renderContext, hooks) {
		...
		try {
			...
			if (factory) {
				...
				const needRequireGlobal = runtimeRequirements.has(
					RuntimeGlobals.requireGlobal
				);
				...
				if (
					...
					cacheEntry.needRequireGlobal === needRequireGlobal
				) {
					...
					if (needRequireGlobal) args.push(RuntimeGlobals.requireGlobal);
	...
	renderBootstrap(renderContext, hooks) {
		...
		if (runtimeRequirements.has(RuntimeGlobals.requireGlobal)) {
			result.allowInlineStartup = false;
			startup.push("// require global is used so entry inlining is disabled");
		}
	...
	renderRequire(renderContext, hooks) {
		...
		const moduleExecution = runtimeRequirements.has(
			...
			? Template.asString([
					`var execOptions = { id: moduleId, module: module, factory: __webpack_modules__[moduleId], require: ${RuntimeGlobals.require}${needRequireGlobal ? `, ${RuntimeGlobals.requireGlobal}: ${RuntimeGlobals.require}` : ""} };`,
					`${RuntimeGlobals.interceptModuleExecution}.forEach(function(handler) { handler(execOptions); });`,
					"module = execOptions.module;",
					`execOptions.factory.call(module.exports, module, module.exports, execOptions.require${needRequireGlobal ? `, execOptions.${RuntimeGlobals.requireGlobal}` : ""});`
				])
			: runtimeRequirements.has(RuntimeGlobals.thisAsExports)
				? Template.asString([
						`__webpack_modules__[moduleId].call(module.exports, module, module.exports, ${RuntimeGlobals.require}${needRequireGlobal ? `, ${RuntimeGlobals.require}` : ""});`
					])
				: Template.asString([
						`__webpack_modules__[moduleId](module, module.exports, ${RuntimeGlobals.require}${needRequireGlobal ? `, ${RuntimeGlobals.require}` : ""});`
					]);
