...
class JavascriptModulesPlugin {
	constructor(options = {}) {
		...
		/** @type {WeakMap<Source, Map<string, Source>>} */
	...
	renderModule(module, renderContext, hooks) {
		...
		try {
			...
			if (factory) {
				...
				// The entry-IIFE rename renders a module a second time and needs a
				// standalone function, so one source has several factory shapes: a
				// variant each keeps the two renders from evicting one another.
				const cacheKey = `${needModule}|${needExports}|${needRequire}|${needThisAsExports}|${needStrict}|${renderShorthand}`;
				let factoryCache = this._moduleFactoryCache.get(
					moduleSourcePostContent
				);
				if (factoryCache === undefined) {
					this._moduleFactoryCache.set(
						moduleSourcePostContent,
						(factoryCache = new Map())
					);
				}
				/** @type {Source | undefined} */
				let source = factoryCache.get(cacheKey);
				if (source === undefined) {
					...
					factoryCache.set(cacheKey, source);
				}
	...
	_getRenamedInlineModule(
		...
		for (const m of allModules) {
			...
			// The render hooks add free names of their own, so only the source they
			// left behind can be analysed. Keying the names on it spares the parse:
			// an unchanged module renders to the very same source on a rebuild.
