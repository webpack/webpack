...
class JavascriptModulesPlugin {
	...
	renderMain(renderContext, hooks, compilation) {
		...
		if (inlinedModules) {
			...
			const entryIife = compilation.options.optimization.entryIife;
	...
	getRenamedInlineModule(
		...
		const innerStrict = !allStrict && allModules.every(m => m.buildInfo.strict); // only the single entry for now.
		const singleEntryWithModules = inlinedModules.size === 1 && hasChunkModules;
		// TODO: We can more accurately judge when we need to rename and rename only when we need to rename.

		if (singleEntryWithModules && !innerStrict) {
			return false;
		}
