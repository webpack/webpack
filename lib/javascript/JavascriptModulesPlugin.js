	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// A chunk with an analyzable `import()`/`new Worker` literal to a hashed
				// chunk must re-hash when that target's filename changes — the analyzable
				// form drops the `__webpack_require__.u` module that normally carries this.
				// Tapping the JS `chunkHash` feeds `chunk.contentHash.javascript`, the asset
				// cache key and `[contenthash]` value, so the target's change re-renders it.
				hooks.chunkHash.tap(PLUGIN_NAME, (chunk, hash, { chunkGraph }) => {
					compilation.runtimeTemplate.updateHashForAnalyzableEsmReferences(
						hash,
						chunk,
						chunkGraph
					);
				...
				// Resolve the post-hash chunk-filename placeholders baked during code
				// generation, via the shared `renderChunkAsset` hook (the same entry point
				// CSS/HTML use). It runs after hashing and before the asset is cached or
				// emitted, so realContentHash still sees the substituted hash string and no
				// processAssets consumer ever sees a placeholder.
				compilation.hooks.renderChunkAsset.tap(
					PLUGIN_NAME,
					(source, { chunk }) =>
						compilation.runtimeTemplate.substituteAnalyzableEsmPlaceholders(
							source,
							chunk
						)
				);
