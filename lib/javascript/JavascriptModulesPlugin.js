	...
	renderBootstrap(renderContext, hooks) {
		...
		if (!runtimeRequirements.has(RuntimeGlobals.startupNoDefault)) {
			if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
				...
				// // `await` is only legal at the top level of an ESM chunk, never in
				// // the startup function rendered below, and only where the target
				// // can parse it (`target: "node14"` reads ESM but not top-level await).
				// const canAwaitEntry =
				// 	runtimeTemplate.isModule() &&
				...
				for (const [entryModule, entrypoint] of jsEntries) {
					...
					if (chunks.length > 0) {
						...
						// Held back for the patch release, reapply with `output.environment.topLevelAwait`:
						// an async entry nobody awaits settles after the bundle did, which
						// orphans its rejection.
						// const awaitEntry =
