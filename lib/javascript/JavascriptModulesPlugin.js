			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.hooks.additionalTreeRuntimeRequirements.tap(
					...
					(chunk, set, { chunkGraph }) => {
						...
						if (
							dependentChunks ||
							mergedEntryModules(chunk, chunkGraph) === undefined
						) {
							return;
						}
						set.add(RuntimeGlobals.mergeExports);
						set.add(RuntimeGlobals.exports);
					}
				);
				compilation.hooks.afterChunks.tap(PLUGIN_NAME, (chunks) => {
					...
					for (const chunk of chunks) {
						...
						if (
							...
							compilation.errors.push(new UnmergeableEntryExportsError(chunk));
						}
					}
				});
	...
	renderBootstrap(renderContext, hooks) {
		...
		if (!runtimeRequirements.has(RuntimeGlobals.startupNoDefault)) {
			if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
				...
				const merge = mergedEntryModules(chunk, chunkGraph) !== undefined;
				...
				/** @type {string[]} */
				const mergedEntries = [];
				// An async entry webpack does not await here hands back a promise,
				// which only resolves to the exports the merge needs.
				let mergeAwaitsEntries = false;
				...
				for (const [entryModule, entrypoint] of jsEntries) {
					...
					if (merge) mergedEntries.push(target);
					if (chunks.length > 0) {
						const awaitEntry =
						...
						if (awaitEntry) {
							awaitedEntries.push(target);
						} else if (merge && moduleGraph.isAsync(entryModule)) {
							mergeAwaitsEntries = true;
						}
					} else {
						if (i === 0 || merge) {
							buf2.push(`${exportsDecl} ${target} = {};`);
				...
				// Every entry has run by now, so the merge sees the final exports of
				// each one — behind one promise where an entry is still settling.
				if (merge) {
					const list = `[${mergedEntries.join(", ")}]`;
					buf2.push(
						`${runtimeTemplate.renderLet()} ${RuntimeGlobals.exports} = ${
							mergeAwaitsEntries
								? `Promise.all(${list}).then(${RuntimeGlobals.mergeExports})`
								: `${RuntimeGlobals.mergeExports}(${list})`
						};`
					);
				}
				if (
