			...
			if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
				...
				for (const [entryModule, entrypoint] of jsEntries) {
					...
					if (chunks.length > 0) {
						...
						// The `entryModuleId` form assigns inside the argument, which the
						// annotation would license a minifier to drop along with the call.
						const pure = runtimeRequirements.has(RuntimeGlobals.entryModuleId)
							? ""
						...
						buf2.push(
							`${
								i === 0 ? `${exportsDecl} ${RuntimeGlobals.exports} = ` : ""
							}${pure}${RuntimeGlobals.require}(${moduleIdExpr});`
						);
					} else {
						if (i === 0) {
