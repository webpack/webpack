	...
	renderBootstrap(renderContext, hooks) {
		...
		if (!runtimeRequirements.has(RuntimeGlobals.startupNoDefault)) {
			if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
				...
				// the startup function rendered below.
				const canAwaitEntry =
					runtimeTemplate.isModule() &&
				...
				// One `Promise.all` so an entry rejecting first still leaves the
				// others observed, and they all start before anything is awaited.
				if (awaitedEntries.length > 0) {
					buf2.push(
						`[${awaitedEntries.join(", ")}] = await Promise.all([${awaitedEntries.join(", ")}]);`
					);
				}
				if (
					runtimeRequirements.has(RuntimeGlobals.startup) ||
