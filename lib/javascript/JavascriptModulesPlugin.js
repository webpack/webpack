			...
			(compilation, { normalModuleFactory }) => {
				...
				for (const type of [
					...
					NormalModule.getCompilationHooks(compilation).processResult.tap(
						...
						(result, module) => {
							if (module.type === type) {
								let [source, sourceMap, ...rest] = result;

								const resource = module.getResource();

								if (
									/** @type {JavascriptParser} */
									(module.parser).options.typescript &&
									resource &&
									(/^.*?\.(ts|mts|cts)$/i.test(resource) ||
										/^data:(text|application|)\/typescript/i.test(resource))
								) {
									// eslint-disable-next-line n/no-unsupported-features/node-builtins
									if (typeof mod.stripTypeScriptTypes !== "function") {
										throw new Error(
											"Strip types feature is not supported by your version of Node.js"
										);
									}

									const needSourceMap =
										module.useSourceMap || module.useSimpleSourceMap;
									const originalSource = Buffer.isBuffer(source)
										? source.toString("utf8")
										: source;

									// eslint-disable-next-line n/no-unsupported-features/node-builtins
									source = mod.stripTypeScriptTypes(originalSource, {
										mode: "transform",
									...
									if (needSourceMap) {
										const { sourceMappingURL, replacementString } =
											getExtractSourceMap().getSourceMappingURL(source);

										const decodedSourceMap =
											getDataURL().decodeDataURI(sourceMappingURL);

										if (decodedSourceMap) {
											source = source.replace(replacementString, "");
											sourceMap =
												/** @type {RawSourceMap} */
												(JSON.parse(decodedSourceMap.toString("utf8")));
											sourceMap.sourcesContent = [originalSource];
										}
									}
								}

								return [removeBOM(source), sourceMap, ...rest];
							}
