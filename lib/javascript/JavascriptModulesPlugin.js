	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// generation. Render runs after hashing and before assets are emitted, so
				// realContentHash still sees the substituted hash string and no
				// processAssets consumer ever sees a placeholder.
				hooks.render.tap(PLUGIN_NAME, (source, renderContext) =>
					renderContext.runtimeTemplate.substituteAnalyzableEsmPlaceholders(
						source,
						renderContext.chunk
					)
				);
