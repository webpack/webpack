 ...
 * } from "../Compilation"
 */
const ConflictingEntryExportsWarning = require("../errors/ConflictingEntryExportsWarning");
const { tryRunOrWebpackError } = require("../errors/HookWebpackError");
const UnmergeableEntryExportsError = require("../errors/UnmergeableEntryExportsError");
...
/** @import ChunkGraph, { EntryModuleWithChunkGroup } from "../graph/ChunkGraph" */
/** @import CodeGenerationResults from "../module/CodeGenerationResults" */
/** @import Compiler from "../Compiler" */
/** @import DependencyTemplates from "../template/DependencyTemplates" */
/** @import Entrypoint from "../graph/Entrypoint" */
/** @import Module, { BuildInfo, CodeGenerationResultData } from "../module/Module" */
 ...
 * @property {Map<string, string>=} renamedDeclarations the inlined entry's top-level declarations renamed to avoid a collision, original to new name
 */
 ...
 * @param {Set<number>} shorthandIdentifierStarts where each shorthand property's identifier starts
 ...
 * @param {Map<string, string>} renamedDeclarations the declarations renamed so far, original to new name (mutated)
 * @returns {void}
 */
const renameInlinedModuleVariable = (
	source,
	shorthandIdentifierStarts,
	variable,
	moduleIdentifier,
	readableIdentifier,
	allUsedNames,
	...
	if (allUsedNames.has(name) || usedNames.has(name)) {
		...
		renamedDeclarations.set(name, newName);
		for (const identifier of allIdentifiers) {
			const r = /** @type {Range} */ (identifier.range);
			if (shorthandIdentifierStarts.has(r[0])) {
...
/** @typedef {{ source: Source, needModule: boolean, needExports: boolean, needRequire: boolean, needThisAsExports: boolean, needStrict: boolean | undefined, renderShorthand: boolean, next: ModuleFactoryCacheEntry | undefined }} ModuleFactoryCacheEntry */

/**
 * @typedef {object} RenamedInlinedModules
 * @property {Map<Module, Source>} sources each inlined module's rendered source, renamed where it collided
 * @property {Map<Module, Map<string, string>>} renamedDeclarations each renamed module's declarations, original to new name
 */
...
class JavascriptModulesPlugin {
	constructor(options = {}) {
		...
		this._renderedFreeNamesCache = new WeakMap();
	}
			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.hooks.additionalTreeRuntimeRequirements.tap(
					...
					(chunk, set, { chunkGraph }) => {
						if (set.has(RuntimeGlobals.startupNoDefault)) return;
						...
						const merged = mergedEntryModules(chunk, chunkGraph, compilation);
						if (merged === undefined) return;
						const { exports, namespace } = getMergedExports(
							merged,
							compilation.moduleGraph,
							chunk.runtime
						);
						set.add(RuntimeGlobals.exports);
						if (namespace) set.add(RuntimeGlobals.makeNamespaceObject);
						if (exports.length > 0) {
							set.add(RuntimeGlobals.definePropertyGetters);
							set.add(RuntimeGlobals.definePropertyGettersFromArray);
						}
					}
				);
				compilation.hooks.afterChunks.tap(PLUGIN_NAME, (chunks) => {
					if (!compilationMergesEntryExports(compilation)) return;
					const { chunkGraph, moduleGraph, runtimeTemplate } = compilation;
					for (const chunk of chunks) {
						const merged = mergedEntryModules(chunk, chunkGraph, compilation);
						if (merged === undefined) continue;
						// Merging reads each entry back right after it ran, which
						// only the bootstrap of the runtime-carrying chunk can do.
						if (
							!chunk.hasRuntime() ||
							chunkGraph.hasChunkEntryDependentChunks(chunk) ||
							entryStartupAwaitsChunks(chunkGraph, chunk)
						) {
							compilation.errors.push(
								new UnmergeableEntryExportsError(
									chunk,
									"they don't all run when this chunk starts up"
								)
							);
							continue;
						}
						// An async entry hands back a promise the merged object would
						// read names off, so it has to be awaited before the merge.
						if (
							!(
								runtimeTemplate.isModule() &&
								runtimeTemplate.supportsTopLevelAwait()
							) &&
							merged.some((module) => moduleGraph.isAsync(module))
						) {
							compilation.errors.push(
								new UnmergeableEntryExportsError(
									chunk,
									"one of them is async and this output can't await it at the top level (see 'output.environment.topLevelAwait')"
								)
							);
							continue;
						}
						const { conflicting, unknownExports } = getMergedExports(
							merged,
							moduleGraph,
							chunk.runtime
						);
						if (unknownExports.length > 0) {
							compilation.errors.push(
								new UnmergeableEntryExportsError(
									chunk,
									`webpack can't tell what ${unknownExports
										.map((module) =>
						...
						if (conflicting.length > 0) {
							compilation.warnings.push(
								new ConflictingEntryExportsWarning(chunk, conflicting)
							);
						}
					}
				});
	...
	renderModule(module, renderContext, hooks) {
		...
		try {
			...
			if (factory) {
				...
				let source;
				// The chunk renders a method shorthand where the entry-IIFE rename
				// needs a standalone function, so a source can have both shapes.
				for (let entry = cacheEntry; entry; entry = entry.next) {
					if (
						entry.needModule === needModule &&
						entry.needExports === needExports &&
						entry.needRequire === needRequire &&
						entry.needThisAsExports === needThisAsExports &&
						entry.needStrict === needStrict &&
						entry.renderShorthand === renderShorthand
					) {
						source = entry.source;
						break;
					}
				...
				if (source === undefined) {
					...
					const header = runtimeTemplate.moduleFactoryFnHeader({
						module,
						needModule,
						needExports,
						needRequire,
						needThisAsExports,
						shorthand: renderShorthand
					});
					factorySource.add(
						renderShorthand ? `${header}\n\n` : `/***/ (${header}\n\n`
					);
	...
	renderChunk(renderContext, hooks) {
		...
		const inlinedEntryModule =
			JavascriptModulesPlugin._getEsmEntryModuleToInline(renderContext);
	 ...
	 *
	 * Static so that a plugin can decide before the chunk is rendered what it may
	 * leave out of the inlined entry's source and put back in the registry.
	 * @param {Pick<RenderContext, "chunk" | "chunkGraph" | "moduleGraph" | "runtimeTemplate" | "codeGenerationResults">} context chunk, graphs and code generation results
	 * @returns {Module | undefined} the entry module to inline, if any
	 */
	static _getEsmEntryModuleToInline(context) {
		const {
			chunk,
			chunkGraph,
			moduleGraph,
	...
	renderMain(renderContext, hooks, compilation) {
		...
		if (inlinedModules) {
			...
			/** @type {RenamedInlinedModules | false} */
			...
			for (const m of inlinedModules) {
				...
				const renderedModule = renamedInlinedModule
					? renamedInlinedModule.sources.get(m)
					: this.renderModule(
							m,
							{
								...chunkRenderContext,
			...
			const startupRenderContext = {
				...
				renamedDeclarations: renamedInlinedModule
					? renamedInlinedModule.renamedDeclarations.get(lastInlinedModule)
					: undefined
			};
	...
	renderBootstrap(renderContext, hooks) {
		...
		if (runtimeRequirements.has(RuntimeGlobals.makeDeferredNamespaceObject)) {
			// The deferred namespace drops its proxy handlers once the module has
			// initialized, which needs every deferred import of it to reach the exports
			// object before evaluation.
			buf.push("// The deferred module cache");
			buf.push(`${bootstrapCst} __webpack_module_deferred_exports__ = {};`);
			// Every defer-import site for one module must yield the same Deferred
			// Module Namespace, distinct from any eager one, so the proxy is cached
			// here for call sites in different files to share.
		...
		if (!runtimeRequirements.has(RuntimeGlobals.startupNoDefault)) {
			if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
				...
				const mergedModules = mergedEntryModulesOfLibrary(
					library,
					chunk,
					chunkGraph
				);
				const merge = mergedModules !== undefined;
				// A module library awaits the entry itself, in `ModuleLibraryPlugin`
				// — but it only sees the merged object, so merging awaits them here.
				const awaitedByLibrary =
					!merge && library !== undefined && library.type === "module";
				// `await` is legal only at an ESM chunk's top level, never in the
				// startup function below, and only where the target parses it.
				const canAwaitEntry =
					runtimeTemplate.isModule() &&
					runtimeTemplate.supportsTopLevelAwait() &&
					!awaitedByLibrary &&
				...
				if (merge && result.allowInlineStartup) {
					buf2.push(
						"// The exports of the entry modules are merged so they can't be inlined"
					);
					result.allowInlineStartup = false;
				}
				/** @type {string[]} */
				...
				for (const [entryModule, entrypoint] of jsEntries) {
					...
					if (
						...
						// Its exports are then read off the exports object, and a property
						// read is not something a consumer of the bundle can drop.
						runtimeTemplate.analyzableBailout(
							entryModule,
							"the entry is referenced by other modules, so its exports are read off the exports object and a consumer cannot drop them"
						);
						result.allowInlineStartup = false;
					}
					...
					// `onChunksLoaded` wrapper, a top-level `await` bridge or other
					// codegen, so it is never declared as a constant.
					const exportsDecl = runtimeTemplate.renderLet();
					// Merging reads every entry back, so each one needs a variable of
					// its own; without it only the last entry (`i === 0`) is exported.
					const target =
						i === 0 && !merge
							? RuntimeGlobals.exports
							: `${RuntimeGlobals.exports}_${i}`;
					if (merge) mergedTargets.set(entryModule, target);
					if (chunks.length > 0) {
						const awaitEntry =
							canAwaitEntry && moduleGraph.isAsync(entryModule);
						// The wrapper hands back nothing until the flush below runs the
						// entry, so a captured one is caught by the entry call itself.
						const capture = awaitEntry || merge;
						if (capture && target !== RuntimeGlobals.exports) {
							buf2.push(`${exportsDecl} ${target};`);
						}
						...
						buf2.push(
							`${
								i === 0 && !merge
									...
									: ""
							}${RuntimeGlobals.onChunksLoaded}(undefined, ${JSON.stringify(
								chunks.map((c) => c.id)
							)}, ${runtimeTemplate.returningFunction(
								capture ? `(${target} = ${entryCall})` : entryCall
							)})`
						);
						...
						const awaitEntry =
							canAwaitEntry && moduleGraph.isAsync(entryModule);
						const needsDecl = i === 0 || awaitEntry || merge;
						buf2.push(
						...
						if (
							...
							const exportsArg = i === 0 || merge ? target : "{}";
								...
								(() => {
									if (needThisAsExports) {
										...
										return `__webpack_modules__[${moduleIdExpr}].call(${
											merge ? target : RuntimeGlobals.exports
										}${comma}${args.join(",")});`;
									}
				...
				// Every entry has run by now, and the getters below read through to
				// whichever one provides each name.
				if (merge) {
					const { exports, namespace } = getMergedExports(
						/** @type {Module[]} */ (mergedModules),
						moduleGraph,
						chunk.runtime
					);
					buf2.push(
					...
					if (namespace) {
						buf2.push(
							`${RuntimeGlobals.makeNamespaceObject}(${RuntimeGlobals.exports});`
						);
					}
					if (exports.length > 0) {
						const bindings = exports.map(
							({ name, module, usedName }) =>
								`${JSON.stringify(name)}, ${runtimeTemplate.returningFunction(
									`${mergedTargets.get(module)}${propertyAccess([usedName])}`
								)}`
						);
						buf2.push(
	...
	renderRequire(renderContext, hooks) {
		...
		// Both deferred-namespace runtimes re-invoke `__webpack_require__` on access,
		// so an evaluation error must be retained and re-thrown. The optimized one
		// skips the deferred exports object, hence the separate flag.
	 ...
	 * @returns {RenamedInlinedModules | false} renamed inlined modules
	 */
	_getRenamedInlineModule(
		...
		// Runs before the IIFE reason is calculated, and only for the cases whose
		// reason it can remove: several inlined entries, or one beside other chunk
		// modules. A non-standard exports name is not handled here.
		...
		/** @type {Map<Module, Map<string, string>>} */
		const renamedDeclarations = new Map();
		const { runtimeTemplate } = renderContext;

		/** @typedef {{ source: Source, module: Module, shorthandIdentifierStarts: Set<number>, variables: Set<Variable>, through: Set<Reference>, usedInNonInlined: Set<Variable>, moduleScope: Scope }} Info */
		...
		for (const m of allModules) {
			...
			// The render hooks add free names, so only the source they left behind
			// can be analysed; keying on it lets a rebuild skip the parse.
			let freeNames = this._renderedFreeNamesCache.get(moduleSource);

			if (freeNames === undefined) {
				...
				for (const ref of analysis.unresolvedReferences) {
					freeNames.add(ref.identifier.name);
				}
				this._renderedFreeNamesCache.set(moduleSource, freeNames);
			}

			for (const name of freeNames) {
				nonInlinedModuleThroughIdentifiers.add(name);
			}
		}

		// Codegen reports each inlined entry's rendered top-level declarations and
		// free names, so where nothing collides there is no renaming to do and the
		// entries need not be parsed at all.
		fastPath: {
			/** @type {Set<string>} */
		...
		for (const [m, moduleSource] of inlinedModuleSources) {
			...
			inlinedModulesToInfo.set(m, {
				source: moduleSource,
				shorthandIdentifierStarts: analysis.shorthandIdentifierStarts,
				module: m,
		...
		// Several inlined entries share the chunk's startup scope, so any top-level
		// declaration colliding with another's declaration, any module's free
		// reference or a reserved name is renamed, sparing them a per-entry IIFE.
		if (isMultipleEntries) {
			...
			for (const [m, moduleInfo] of inlinedModulesToInfo) {
				const { shorthandIdentifierStarts, source: _source } = moduleInfo;
				...
				if (renamed.size > 0) renamedDeclarations.set(m, renamed);
			}
			return {
				sources: renamedInlinedModules,
				renamedDeclarations
