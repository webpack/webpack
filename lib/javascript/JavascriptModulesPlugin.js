			...
			if (chunkGraph.getNumberOfEntryModules(chunk) > 0) {
				...
				for (const [entryModule, entrypoint] of jsEntries) {
					...
					if (chunks.length > 0) {
						...
						// Only an ESM bundle can await its own entry. Without this an
						// async entry's promise is nobody's, so the bundle settles
						// before the entry did and its rejection is orphaned.
						if (
							i === 0 &&
							runtimeTemplate.isModule() &&
							moduleGraph.isAsync(entryModule)
						) {
							buf2.push(
