...
// cspell:ignore DEFMETHOD, Defun, defun, fnames, funarg, classnames, mangleable, unmangleable, NOINLINE, privatename, argnames, thedef, argname, bcatch, bfinally, MANGLEPROP, nlb, punc, Funarg, propmangle, preferrably, nondeferred, fargs
...
const scopeFits = ({ ast, parse, utils }) => {
	...
	if (
		...
		typeof utils.defaults !== "function" ||
		typeof utils.push_uniq !== "function" ||
		typeof utils.string_template !== "function"
	) {
		return false;
	}
	...
	return (
		createHash("xxhash64")
			.update(original.toString().replace(/\s+/g, " "))
			.digest("hex") === TERSER_FIGURE_OUT_SCOPE
	);
};
 ...
 * @returns {void}
 */
const installScope = ({ ast, parse, utils }) => {
	const {
		AST_Call,
		AST_Class,
		AST_DefClass,
	...
	const { defaults, push_uniq: pushUnique, string_template: template } = utils;
	...
	function figureOutScope(options, context = {}) {
		...
		let scope = (this.parent_scope = parentScope);
		...
		const resolveIn = (node) => {
			...
			if (kind & SCOPE_REFERENCE) {
				...
				let symbol;
				if (
					(parent() instanceof AST_NameMapping && parent(1).module_name) ||
			...
			if (
				...
				for (let outer = node.scope; outer; outer = outer.parent_scope) {
					pushUnique(outer.enclosed, definition);
		...
		if (options.safari10) {
			for (const forScope of forScopes) {
				for (const definition of forScope.parent_scope.variables.values()) {
					pushUnique(forScope.enclosed, definition);
				}
			}
		}
	}
...
const unusedFits = ({ ast, common, flags, inference, scope, utils }) => {
	...
	if (
		...
		typeof utils.keep_name !== "function" ||
		typeof utils.make_node !== "function" ||
		typeof utils.map_add !== "function" ||
		typeof utils.remove !== "function" ||
...
// The xxhash64 of terser's `lib/minify.js`, each run of whitespace read as one
// space: its driver is not exported, so the file is what the phase pins.
const TERSER_MINIFY = "e8f2206a8a4f9e0f";
...
const minifyFits = (modules) => {
	const { compress, directory, propmangle, sourcemap, utils } = modules;
	if (
		typeof directory !== "string" ||
		!compress ||
		typeof compress.Compressor !== "function" ||
		!propmangle ||
		typeof propmangle.mangle_private_properties !== "function" ||
		!sourcemap ||
		typeof sourcemap.SourceMap !== "function" ||
		typeof utils.map_from_object !== "function" ||
		typeof utils.map_to_object !== "function" ||
		typeof utils.HOP !== "function"
	) {
		return false;
	}
	...
	let source;
	try {
		source = fs.readFileSync(path.join(directory, "lib", "minify.js"), "utf8");
	} catch (_err) {
		return false;
	}
	return (
...
const installMinify = (modules) => {
	const { ast, parse: parseModule, output, utils } = modules;
	const { AST_Node, AST_Scope, AST_Toplevel, walk } = ast;
	const { Compressor } = modules.compress;
	const {
		find_annotated_props: findAnnotatedProperties,
		mangle_private_properties: manglePrivateProperties,
		mangle_properties: mangleProperties,
		reserve_quoted_keys: reserveQuotedKeys
	} = modules.propmangle;
	const { SourceMap } = modules.sourcemap;
	const { base54 } = modules.scope;
	const {
		defaults,
		HOP: hasOwn,
		map_from_object: mapFromObject,
		map_to_object: mapToObject
	} = utils;
	...
	const namesNoPrivateMember = (files) => {
		if (files instanceof AST_Toplevel) return WITHOUT_PRIVATE_NAMES.has(files);
...
const createTerserTree = ({ ast, parse }) => {
	...
	// read is at an offset or index this source set, so the rest is inert.
	// By offset, how many tokens start at or before it.
	let tokensThroughBuffer = new Int32Array(0);
	// Each token's offsets and kind, by index; a source holds a fraction as
	// many tokens as characters, so these start at a quarter and double.
	...
	return (source, options) => {
		...
		if (startBuffer.length <= source.length >> 2) {
			const capacity = (source.length >> 2) + 16;
			startBuffer = new Int32Array(capacity);
			endBuffer = new Int32Array(capacity);
			kindBuffer = new Int8Array(capacity);
		}
		// Read through these: a source too large to keep releases the ones above
		 ...
		 */
		const tokenStart = (offset) => tokensThrough[offset];
		/**
		 * @param {number} offset where a token ends
		 * @returns {number} that token's index, plus one
		 */
		const tokenEnd = (offset) => tokensThrough[offset - 1];
			...
			program = parseSource(source, {
				...
				onToken: (token) => {
					...
					if (count === starts.length) {
						...
						grownEnds.set(ends);
						const grownKinds = new Int8Array(capacity);
						grownKinds.set(kinds);
						starts = startBuffer = grownStarts;
						ends = endBuffer = grownEnds;
						kinds = kindBuffer = grownKinds;
					}
		...
		if (source.length > 1 << 23) {
			tokensThroughBuffer = new Int32Array(0);
		 ...
		 */
		const startOf = (node) => tokenAt(tokenStart(node.start) - 1);
		/**
		 * @param {EstreeNode} node a node
		 * @returns {Token} the token it ends with
		 */
		const endOf = (node) => tokenAt(tokenEnd(node.end) - 1);
		/**
		 * @param {EstreeNode} node a node
		 * @returns {Token} the token after it
		 */
		const afterOf = (node) => tokenAt(tokenEnd(node.end));
		...
		const statement = (node) => {
			...
			converted.start = firstReadAt(tokenStart(node.start) - 1);
			...
			for (let i = 0; i < body.length; i++) {
				...
				if (
					...
					if (
						...
						tokenStart(next.start) - 1 === tokenEnd(node.end)
					) {
		...
		const binding = (node, Type) => {
			if (node.type === "ArrayPattern") {
				let cursor = tokenStart(node.start);
				const names = [];
				const { elements } = node;
				...
				for (const property of node.properties) {
					...
					if (property.shorthand && startOf(property.key).type === "name") {
						...
						keyValue = new A.AST_ObjectKeyVal({
							start: tokenAt(tokenStart(property.key.start) - 2),
							key: value.name,
							value,
							end: value.end
						});
		...
		const elements = (list, convert) => {
			...
			let cursor = tokenStart(list.start);
				...
				if (
					...
					(keyName(node.key) === "async" ||
						((keyName(node.key) === "get" || keyName(node.key) === "set") &&
							values[tokenEnd(node.end)] === "*" &&
							kinds[tokenEnd(node.end)] === OTHER))
				) {
		...
		const parenthesized = (node) => {
			...
			if (inner.type === "SequenceExpression") {
				...
				const endIndex = Math.min(tokenEnd(node.end) + 1, count);
				ownGap(endIndex + 1);
		...
		const importAttributes = (node) => {
			const after = tokenEnd(node.source.end);
			if (
				after >= count ||
				after >= tokenEnd(node.end) ||
			...
			let closeIndex = tokenEnd(node.end) - 1;
		...
		const importDeclaration = (node) => {
			...
			for (const specifier of node.specifiers) {
				if (specifier.type === "ImportDefaultSpecifier") {
					...
					importedNames = [
						mapAsterisk(true, local, tokenEnd(specifier.end) - 1)
					];
				} else {
			...
			if (
				...
				const index = tokenStart(node.source.start) - 3;
			...
			return new A.AST_Import({
				...
				end: attributes ? tokenAt(tokenEnd(node.end) - 1) : afterOf(node.source)
			});
		};
		...
		const exportDeclaration = (node) => {
			const start = tokenAt(tokenStart(node.start));
			if (node.type === "ExportAllDeclaration") {
				...
				const named = node.exported
					...
					: undefined;
				const lastIndex = tokenStart(node.source.start) - 3;
		...
		const anonymousDefaultEnds = (declaration) => {
			const next = tokenEnd(declaration.end);
		...
		const endOfExport = (node) => {
			const last = tokenEnd(node.end) - 1;
			return tokenAt(
		...
		const from = (node) => {
			switch (node.type) {
				...
				case "ArrowFunctionExpression": {
					...
					const startIndex = tokenStart(node.start) - 1;
					const firstIndex =
						params.length === 1 ? tokenStart(params[0].start) - 1 : -1;
					const lone =
						params.length === 1 &&
				...
				case "SequenceExpression":
					...
					return new A.AST_Sequence({
						start: firstReadAt(tokenStart(node.start) - 1),
						expressions: list(node.expressions),
						end: tokenAt(Math.min(tokenEnd(node.end) + 1, count))
					});
				case "SpreadElement":
					return new A.AST_Expansion({
						start: startOf(node),
				...
				case "TryStatement": {
					...
					return new A.AST_Try({
						...
						bfinally: finalizer
							? new A.AST_Finally({
									start: tokenAt(tokenStart(finalizer.start) - 2),
				...
				case "TemplateLiteral": {
					...
					for (let i = 0; i < quasis.length; i++) {
						...
						const token = tokenAt(tokenStart(quasi.start - 1) - 1);
						segments.push(
						...
						properties: body.map((/** @type {EstreeNode} */ element) =>
							element.type === "StaticBlock"
								? new A.AST_ClassStaticBlock({
										start: tokenAt(tokenStart(element.start)),
				...
				case "ImportExpression":
					...
					if (
						values[tokenEnd(node.end) - 2] === "," &&
						kinds[tokenEnd(node.end) - 2] === OTHER
					) {
						throw DECLINE;
					}
...
const loadSources = async () => {
	...
	const propmangle = await at("propmangle.js");
	const sourcemap = await at("sourcemap.js");
