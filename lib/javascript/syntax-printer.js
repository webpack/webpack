...
/**
 * terser's sources as webpack carries them: the classes behind `minify`, which
 * a phase has to reach and terser's own entry point does not export.
 * @returns {TerserModules & { minify: typeof import("./terser").minify }} the modules
 */
const loadSources = () => {
	const ast = require("./terser/ast");

	// Read for its effect: it installs `transform` on every node class.
	require("./terser/transform");

	const scope = require("./terser/scope");
	const parse = require("./terser/parse");
	// Read for its effect too: it installs the code generators on every node.
	const output = require("./terser/output");
	const unicode = require("./terser/unicode");
	const utils = require("./terser/utils/index");

	// Read for its effect: it installs the ESTree conversions `minify` reaches.
	require("./terser/mozilla-ast");
 ...
 * The answer is cached, so a worker installs them once. The phases go into the
 * one copy of terser there is, so `terser` below carries them from then on too.
 * @returns {Promise<Terser>} terser and the phases installed into it
 */
const load = () => {
	if (loading !== undefined) return loading;
	loading = Promise.resolve().then(() => {
		const modules = loadSources();
		/** @type {string[]} */
		const phases = [];
...
module.exports = {
	...
	createTerserTree,
	/**
	 * terser as it was released, which a worker reaches through `webpack` itself:
	 * `minimizer-webpack-plugin` hands a minify function over as source, where a
	 * path relative to this file means nothing.
	 * @returns {typeof import("./terser")} its entry point
	 */
	get terser() {
