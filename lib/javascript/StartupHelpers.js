 ...
 * (`require.main`, `import.meta.main`) asked for that global itself. A split runtime
 * chunk carries the tree requirements, so the chunk being rendered has none to read.
...
const entryModuleIdExpression = (chunkGraph, chunk, moduleIdExpression) => {
	...
	const runtimeChunk = firstEntry
		? /** @type {Entrypoint} */ (firstEntry[1]).getRuntimeChunk()
		: undefined;
	return chunkGraph
		.getTreeRuntimeRequirements(runtimeChunk || chunk)
		.has(RuntimeGlobals.entryModuleId)
...
module.exports.generateEntryStartup = (
	...
	/** @type {Map<ModuleId, string>} entry module id -> its side-effects annotation */
	const pureByModuleId = new Map();
	// `__webpack_exec__` assigns `entryModuleId` on the way in, which the annotation
	// would license a minifier to drop along with the call.
	const canAnnotate = !chunkGraph
	 ...
	 */
	const runModule = (id) =>
		`${pureByModuleId.get(id) || ""}__webpack_exec__(${JSON.stringify(id)})`;
	/**
	 * Output combination.
