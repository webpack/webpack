...
const getModuleFederationPlugin = memoize(() =>
	require("../container/ModuleFederationPlugin")
);
...
/**
 * @typedef {object} StartupChunkDependencyParams
 * @property {(string|number)[]} chunkIds
 * @property {Chunk} chunk
 * @property {string} fn
 * @property {boolean} final
 * @property {boolean} passive
 * @property {string[]} runtime
 * @property {string} EXPORT_PREFIX
 */
...
module.exports.generateEntryStartup = (
	...
	const outputCombination = (chunks, moduleIds, final) => {
		...
		const handled = hooks.generateFederationStartup.call(params);

		if (handled) {
			// Federation plugin handled the case
