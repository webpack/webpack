...
const jsMinify = async (
	...
	let terser;
	try {
		// Through webpack's loader rather than `require("terser")`: it installs the
		// phases webpack implements itself, and falls back to terser's own.
		const webpack =
		...
		terser = await webpack.javascript.terser.load();
	} catch (err) {
