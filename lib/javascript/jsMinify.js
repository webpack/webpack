...
// The terser release `./terser` is, as every file there names it.
 ...
 * @param {(MinifyOptions & { as?: JavascriptProduction, printer?: boolean })=} minimizerOptions `optimization.minimize.javascript`, as terser's own options; `as` names the production the source is written in, which is the source's own rather than the configuration's, and `printer` minifies through webpack's printer rather than terser as published
...
const jsMinify = async (
	...
	const { as, printer, ...options } = minimizerOptions || {};
	/** @type {{ minify: typeof import("./terser").minify }} */
	let terser;
	try {
		// terser is carried by webpack, and reached through webpack itself: a path
		// relative to this file means nothing in the worker this runs in.
		const webpack =
		...
		// Through webpack's own printer, which installs the phases webpack
		// implements itself and leaves terser's where one does not fit.
		terser = printer
			? await webpack.javascript.syntax.printer.load()
			: webpack.javascript.syntax.printer.terser;
	} catch (err) {
...
/**
 * The version minified output is cached against: the terser release webpack
 * carries, which is what every file of `./terser` names. Read as a constant, so
 * asking does not load terser into the process that only wants to know.
 * @returns {string} the release
 */
jsMinify.getMinimizerVersion = () => TERSER_VERSION;
