			...
			compilation => {
					...
					(modules, renderContext) => {
						...
						if (hotUpdateChunk) {
							const hotUpdateGlobal =
								typeof runtimeTemplate.outputOptions.hotUpdateGlobal ===
								"string"
									? runtimeTemplate.outputOptions.hotUpdateGlobal
									: runtimeTemplate.outputOptions.hotUpdateGlobal(chunk);
				...
				hooks.chunkHash.tap(
					...
					(chunk, hash, { chunkGraph, runtimeTemplate }) => {
						if (chunk.hasRuntime()) return;
						const hotUpdateGlobal =
							typeof runtimeTemplate.outputOptions.hotUpdateGlobal === "string"
								? runtimeTemplate.outputOptions.hotUpdateGlobal
								: runtimeTemplate.outputOptions.hotUpdateGlobal(chunk);
						hash.update(
							`ArrayPushCallbackChunkFormatPlugin1${runtimeTemplate.outputOptions.chunkLoadingGlobal}${hotUpdateGlobal}${runtimeTemplate.globalObject}`
						);
						const entries = Array.from(
							chunkGraph.getChunkEntryModulesWithChunkGroupIterable(chunk)
						);
