	...
	apply(compiler) {
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			hooks.renderChunk.tap(PLUGIN_NAME, (modules, renderContext) => {
				...
				if (hotUpdateChunk) {
					const hotUpdateGlobalOption =
						compilation.outputOptions.hotUpdateGlobal;
					const hotUpdateGlobal =
						typeof hotUpdateGlobalOption === "function"
							? hotUpdateGlobalOption({ chunk, contentHashType: "javascript" })
							: /** @type {string} */ (hotUpdateGlobalOption);
				...
				(chunk, hash, { chunkGraph, runtimeTemplate }) => {
					...
					const hotUpdateGlobal =
						typeof hotUpdateGlobalOption === "function"
							...
							: /** @type {string} */ (hotUpdateGlobalOption);
					hash.update(
						`${PLUGIN_NAME}1${runtimeTemplate.outputOptions.chunkLoadingGlobal}${hotUpdateGlobal}${runtimeTemplate.globalObject}`
					);
					/** @type {EntryModuleWithChunkGroup[]} */
					const entries = [
						...chunkGraph.getChunkEntryModulesWithChunkGroupIterable(chunk)
