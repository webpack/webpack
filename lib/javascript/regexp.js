...
// Which comparison last marked a disjunction, so that a walk's marks are
// told apart from the previous walk's without clearing them.
let branchWalk = 0;
...
class BranchID {
	...
	constructor(parent, base) {
		...
		/** @type {BranchID} */
		this.base = base || this;
		// which walk last marked this disjunction, and with which of its branches
		this.walk = 0;
		/** @type {BranchID | null} */
		this.taken = null;
	}
	 ...
	 */
	separatedFrom(alt) {
		// WHY: a chain holds at most one branch per disjunction — siblings share
		// a parent, so a chain takes one of them — so marking each disjunction
		// this chain passed answers the whole comparison in one walk of the
		// other. acorn walks one chain per node of the other, which is quadratic
		// in the nesting depth and re-walked per same-named group (acorn#1445).
		const walk = ++branchWalk;
		/** @type {BranchID | null} */
		let self = this;
		while (self) {
			self.base.walk = walk;
			self.base.taken = self;
			self = self.parent;
		}
		/** @type {BranchID | null} */
		let other = alt;
		while (other) {
			const disjunction = /** @type {BranchID} */ (other.base);
			if (disjunction.walk === walk && disjunction.taken !== other) {
				return true;
			}
			other = other.parent;
		}
