...
const pushComment = (options, array) =>
	...
	function pushOneComment(block, text, start, end, startLoc, endLoc) {
		...
		if (options.locations) {
			comment.loc = new ParserSourceLocation(this, startLoc, endLoc);
		}
...
/** @type {typeof import("./grammar") | undefined} */
let grammarModule;

/**
 * The methods `Parser` defines only as stubs that load the grammar on first
 * use; the grammar's own take their slots once it is installed.
 * @type {(string | symbol)[]}
 */
const GRAMMAR_STUBS = ["parse", "parseExpression", "getToken", Symbol.iterator];
...
const installGrammar = () => {
	if (grammarModule !== undefined) return grammarModule;

	// held here so a stub never pays the module loader's lookup per token
	const loaded = require("./grammar");

	loaded.install(Parser);
	// a stub left in place would still delegate on every token
	for (const name of GRAMMAR_STUBS) {
		/** @type {EXPECTED_ANY} */ (Parser.prototype)[name] =
			/** @type {EXPECTED_ANY} */ (loaded.Grammar.prototype)[name];
	}
	grammarModule = loaded;
	return loaded;
};
...
class Scope {
	...
	constructor(flags) {
		...
		// WHY: the declarations that hoist to this scope, in source order, kept
		// for the consumer that would otherwise re-walk every statement to find
		// them again. Only a var scope fills it; `exitScope` hands it to the
		// node that owns the scope and clears the slot before pooling.
		/** @type {(NodeLike | number)[] | undefined} */
		this.hoisted = undefined;
	}
}
...
class ParserNode {
	...
	constructor(parser, pos, loc) {
		...
		if (parser.options.locations) {
			this.loc = new ParserSourceLocation(
				parser,
				/** @type {Position} */ (loc),
...
class Token {
	...
	constructor(p) {
		...
		if (p.options.locations) {
			this.loc = new ParserSourceLocation(p, p.startLoc, p.endLoc);
		}
...
class Parser {
	 ...
	 */
	parse() {
		return installGrammar().Grammar.prototype.parse.call(this);
	}
	...
	parseExpression(forInit, refDestructuringErrors) {
		return installGrammar().Grammar.prototype.parseExpression.call(
			this,
			forInit,
	...
	getToken() {
		return installGrammar().Grammar.prototype.getToken.call(this);
	}

	/** @returns {Iterator<Token>} the remaining tokens */
	[Symbol.iterator]() {
		return installGrammar().Grammar.prototype[Symbol.iterator].call(this);
	}
...
// The base class, typed as the parser with its grammar installed — which is
// what every instance sees, since `baseProto` installs it before use.
/** @type {typeof import("./grammar").Grammar} */
const BaseParser = /** @type {EXPECTED_ANY} */ (Parser);

/** @typedef {EXPECTED_ANY[]} HoistedDeclarations the declarations one scope hoists, in source order */
/** @typedef {EXPECTED_ANY[]} ModuleDeclarations the program's import and re-export declarations in source order, each followed by the statement written before it */
/** @typedef {number | number[]} BlockDeclarations which statements of one list declare into its block scope: a mask of their indices, or the indices themselves once one passes `MAX_MASKED_INDEX` */
/** @typedef {{ token: string, isExpr: boolean, preserveSpace?: boolean, generator?: boolean, override?: unknown }} TokContextShim the TokContext fields the owned tokenizer reads */

// the token contexts used by the inlined finishToken context updates
...
/** @typedef {NodeLike} Node */
/** @typedef {ImportSpecifier | ImportDefaultSpecifier | ImportNamespaceSpecifier} AnyImportSpecifier */
/** @typedef {TokenType & { beforeExpr: boolean, isAssign?: boolean, prefix?: boolean, postfix?: boolean, startsExpr?: boolean, isLoop?: boolean, binop: number | null, updateContext: ((prevType: TokenType) => void) | null }} TokenTypeInternal the internal TokenType fields the owned tokenizer reads */
...
/** @typedef {typeof Parser} ParserClass the parser class itself, as a plugin receives and returns it */
...
// Symbol-keyed so they stay out of for-in, Object.keys and JSON.stringify over
// AST nodes.
...
// Single-char punctuators acorn reads as just `++pos; finishToken(type)`, with
// no value and no operator state machine. Dispatching them from `nextToken`'s
// char table skips a call and a switch for the commonest tokens in JS.
...
// Characters starting only a token that cannot continue an expression, so an
// atom directly followed by one is the whole expression. `}` is included: it
// only ever closes a block, an object or a template substitution.
...
// load steers the skip loop, the token dispatch and `readWord`'s scan. Both
// loops leave on one compare, which is what the ordering below is for.

// an identifier continues with these two, so `readWord` scans while `cls <= 1`
const CLS_DIGIT = 0;
const CLS_IDENT = 1;
const CLS_PUNCT = 2;
const CLS_DOT = 3;
...
// no token opens with these, so the skip loop stops while `cls < CLS_SPACE`
const CLS_SPACE = 7;
const CLS_NEWLINE = 8;
const CLS_SLASH = 9;
// Covers the whole `charCodeAt` range, so the scan loops need no `code > 127`
// branch: non-ASCII classifies as CLS_UNICODE, which sorts below CLS_SPACE and
// so exits on the same compare, then delegates to acorn's unicode paths.
const CHAR_CLASS = new Uint8Array(0x10000).fill(CLS_UNICODE, 128);
CHAR_CLASS.fill(CLS_OTHER, 0, 128);
for (let i = 0; i < 128; i++) {
	if (IDENT_START[i] === 1) CHAR_CLASS[i] = CLS_IDENT;
	// IDENT_CHAR minus IDENT_START is exactly 0-9
	else if (IDENT_CHAR[i] === 1) CHAR_CLASS[i] = CLS_DIGIT;
	else if (SIMPLE_PUNCT[i] !== undefined) CHAR_CLASS[i] = CLS_PUNCT;
}
CHAR_CLASS[46] = CLS_DOT;
CHAR_CLASS[61] = CLS_EQ;
// WHY: a `\` opens an identifier written with a `\u` escape, which only the
// cold readers handle — the same answer CLS_UNICODE already carries, so it
// shares that class rather than adding one the two scan loops must test for.
// `_skipSpaceCold` consumes nothing here and `readToken` sends 92 to `readWord`.
CHAR_CLASS[92] = CLS_UNICODE;
...
// WHY: a sparse symbol slot, not a field: the list is webpack's own record of
// the scope, and a named key would land in every consumer's view of the AST —
// `JSON.stringify` included, where it duplicates each declaration's subtree.
const HOISTED_DECLARATIONS = Symbol("hoistedDeclarations");

// The slot of a scope that declared nothing. It has to differ from the empty
// slot of a node some other parser built, which has no record to read.
/** @type {HoistedDeclarations} */
const NO_HOISTED_DECLARATIONS = [];

// The program's own import and re-export declarations, read before anything
// else — same sparse-slot reasoning as the hoisted list above.
const MODULE_DECLARATIONS = Symbol("moduleDeclarations");

/** @type {ModuleDeclarations} */
const NO_MODULE_DECLARATIONS = [];

// Which statements of one list declare into its own block scope, read before
// any of it is walked — same sparse-slot reasoning as the two lists above. It
// holds their indices, so `undefined` alone means no record was kept.
const BLOCK_DECLARATIONS = Symbol("blockDeclarations");

// WHY: the indices live in one number rather than an array because 59.4% of the
// lists that declare anything declare exactly once, and a 4-element array costs
// ~200 bytes of V8 overhead — 2.33 MB of an 84 MB AST measured over the
// `js-parser-unit` sources. Only an index past this one needs the array.
const MAX_MASKED_INDEX = 30;

/**
 * Turn a mask that can no longer hold every index into the list it falls back
 * to, holding what the mask held plus the index that overflowed it.
 * @param {number} mask the indices recorded so far
 * @param {number} index the index that does not fit
 * @returns {number[]} every index, in source order
 */
const overflowFrom = (mask, index) => {
	/** @type {number[]} */
	const indices = [];
	for (let bits = mask; bits !== 0; bits &= bits - 1) {
		indices.push(31 - Math.clz32(bits & -bits));
	}
	indices.push(index);
	return indices;
};
...
/**
 * Whether a statement declares anything into the block scope holding it, which
 * is what the consumer reads the list for.
 * @param {EXPECTED_ANY} statement the finished statement
 * @returns {boolean} whether it belongs in the list
 */
const declaresIntoBlock = (statement) => {
	...
	// WHY: only an assignment to an object pattern is read here, for the
	// properties it names. Keeping every expression statement would carry the
	// most common statement there is: 10390 of three-long's 22758, against 1.
	if (type !== "ExpressionStatement") return false;
...
class BlockStatementNode {
	...
	constructor(start, end, body) {
		...
		this.end = end;
		this.body = body;
		// filled for a function body only: what hoists to the scope it opens
		/** @type {HoistedDeclarations | undefined} */
		this[HOISTED_DECLARATIONS] = undefined;
		/** @type {BlockDeclarations | undefined} */
		this[BLOCK_DECLARATIONS] = undefined;
	}
}
...
class ProgramNode {
	...
	constructor(start, end, body, sourceType) {
		...
		/** @type {HoistedDeclarations | undefined} */
		this[HOISTED_DECLARATIONS] = undefined;
		/** @type {ModuleDeclarations | undefined} */
		this[MODULE_DECLARATIONS] = undefined;
		/** @type {BlockDeclarations | undefined} */
		this[BLOCK_DECLARATIONS] = undefined;
	}
}
...
/**
 * The base parser plus `WebpackParser`'s own fields, so the constructor can
 * write them before the instance is one.
 * @typedef {import("./grammar").Grammar & {
 ...
 * _readWordIntoCache: (start: number, end: number, wordLen: number, slot: number) => void,
 * _readWordUncacheable: (start: number, end: number) => void,
 * _updateContext: (internal: TokenTypeInternal, prevType: TokenType, value?: EXPECTED_ANY) => void,
 * _skipSpaceCold: () => void,
 * _getUnknownOrPrivate: (code: number) => void,
 * lineStart: number,
 * _scopePool: Scope[],
 * _setPool: Set<string>[],
 * _exportHeadDepth: number,
 * _exitedHoisted: HoistedDeclarations | undefined,
 * _moduleDeclarations: ModuleDeclarations | undefined,
 * _previousTopLevelStatement: Node | undefined,
 * _takeSet: () => Set<string>,
...
const base = /** @type {import("./grammar").Grammar} */ (
	/** @type {unknown} */ (BaseParser.prototype)
);

/**
 * The base prototype with the grammar installed. Every delegation below reads
 * it through here, so the productions load the first time one is needed.
 * @returns {import("./grammar").Grammar} the base prototype
 */
const baseProto = () => {
	installGrammar();
	return base;
...
/**
 * The parser's destructuring-errors record. Owned methods must create records
 * with the same hidden class the rest of the expression parser reads, or every
 * record field access there turns polymorphic.
...
/**
 * @returns {DestructuringErrorsShim} fresh destructuring-errors record
 */
const createDestructuringErrors = () => new DestructuringErrors();
...
// Direct-mapped identifier cache for `readWord1`: one slot per hash, verified by
// char compare, overwritten on collision. Shared across parses, since a hit is
// content-checked. Only lengths V8 stores flat, never source-retaining slices.
const WORD_CACHE_MASK = 0xfff;
const WORD_CACHE_MAX_LEN = 12;

// Four entries per slot (word, token type, id of the keyword set that classified
// it, padding), so a probe touches one line; another set's id retypes the word.
// An empty slot holds "", which no word matches, so a probe needs no null check.
/** @type {(string | TokenType | number | null)[]} */
let WORD_CACHE = NO_CACHE;
const WORD_CACHE_STRIDE = 4;
let nextWordLookupsId = 1;

// WHY: a direct-mapped cache for short non-word slices — WORD_CACHE's regime
// applied to literal text. Number raws repeat heavily, a typical large input
// holding some ten times more number literals than distinct raws, and short
// string values and raws repeat too, so serving them from one shared string cuts
// both allocation churn and retained AST strings. Same rules as WORD_CACHE:
// content-checked hits, and only the lengths V8 stores flat, never a slice
...
const ensureParserCaches = () => {
	...
	WORD_CACHE = Array.from(
		{ length: (WORD_CACHE_MASK + 1) * WORD_CACHE_STRIDE },
		(_, i) =>
			i % WORD_CACHE_STRIDE === 0 ? "" : i % WORD_CACHE_STRIDE === 2 ? 0 : null
	);
	SLICE_CACHE = Array.from({ length: SLICE_CACHE_MASK + 1 }, () => "");
};
...
const getShortSlice = (input, start, end) => {
	...
	const cached = /** @type {string} */ (SLICE_CACHE[slot]);
	if (cached.length === end - start && input.startsWith(cached, start)) {
		return cached;
	}
...
/**
 * Whether the word at `start` is `cached`, whose hash already matched: a
 * char loop beats a builtin call for words this short.
 * @param {string} input source code
 * @param {number} start where the word starts
 * @param {string} cached the cached word
 * @param {number} wordLen the word's length
 * @returns {boolean} true when they are the same word
 */
const wordMatches = (input, start, cached, wordLen) => {
	if (cached.length !== wordLen) return false;
	for (let i = 0; i < wordLen; i++) {
		if (cached.charCodeAt(i) !== input.charCodeAt(start + i)) return false;
	}
	return true;
};

/**
 * The reserved kind `checkUnreserved` acts on for a word, 0 when it has nothing
 * to check: pure in (word, lookups), so it is memoized beside the token type.
 * @param {string} word word
 * @param {WordLookups} lookups the parser's keyword sets
 * @returns {number} the reserved kind, 4 for the names with their own checks
 */
const wordKind = (word, lookups) => {
	const kind = lookups.reservedKinds.get(word);
	if (kind !== undefined) return kind;
	return word === "arguments" || word === "await" || word === "yield" ? 4 : 0;
};
...
// One-entry identity memo in front of the string-keyed cache. `wordsRegexp`
// interns its regexps, so identity captures the whole word set, and a build
// constructs thousands of parsers from one option set.
 ...
 * @property {ResolvedOptions} options normalized options template — never handed to an instance, see `cloneNormalizedOptions`
 ...
 * @param {ResolvedOptions} template normalized options template of the shape
 * @returns {ResolvedOptions} per-instance options object
 */
const cloneNormalizedOptions = (template) => {
	...
	return /** @type {ResolvedOptions} */ (/** @type {unknown} */ (options));
};
...
class WebpackParser extends BaseParser {
	...
	constructor(options, input, startPos) {
		ensureParserCaches();
		// fast construction for the gated hot-path shape, skipping acorn's per-parse
		// getOptions/wordsRegexp work. A derived constructor may return an object
		// without super(), and a plugin's body then runs on it just the same.
		const shape = getFastConstructionShape(options, input, startPos);
		if (shape !== null) {
			// write order below is load-bearing: acorn's constructor fields first, in
			// dist order, then WebpackParser's in slow-constructor order, so fast- and
			...
			self._scopePool = [];
			self._setPool = [];
			self._exportHeadDepth = -1;
			self._exitedHoisted = undefined;
			self._moduleDeclarations = undefined;
			self._previousTopLevelStatement = undefined;
			self._labelsStack = [];
			self._labelsDepth = 0;
			self._scanGapNewline = false;
			self._validRegexpFlags = shape.validRegexpFlags;
			self._varIdOwn = self.parseVarId === proto.parseVarId;
			self._stmtFastPath =
				self._varIdOwn &&
			...
			self._funcFastPath =
				shape.ecmaVersion >= 9 &&
				self.initFunction === proto.initFunction &&
				self.isSimpleParamList === proto.isSimpleParamList;
			self._funcStmtOwn =
				self._funcFastPath &&
				self.parseFunctionStatement === proto.parseFunctionStatement &&
				self.parseFunction === proto.parseFunction;
			self._stmt2FastPath =
				shape.ecmaVersion >= 9 &&
				self._varIdOwn &&
				self.parseForStatement === proto.parseForStatement &&
				self.parseFor === proto.parseFor &&
				self.parseForIn === proto.parseForIn &&
				self.parseForAfterInit === proto.parseForAfterInit &&
				self.parseVar === proto.parseVar &&
				self.parseWhileStatement === proto.parseWhileStatement &&
				self.parseSwitchStatement === proto.parseSwitchStatement &&
				self.parseThrowStatement === proto.parseThrowStatement &&
				self.parseTryStatement === proto.parseTryStatement &&
				self.parseBreakContinueStatement ===
					proto.parseBreakContinueStatement &&
				self.parseDoStatement === proto.parseDoStatement &&
				self.parseWithStatement === proto.parseWithStatement &&
				self.parseEmptyStatement === proto.parseEmptyStatement &&
				self.parseDebuggerStatement === proto.parseDebuggerStatement;
			self._exprFastPath =
		...
		// WHY: the owned per-token loop (nextToken/finishToken/next) also serves the
		...
		// `nextToken`'s punct/dot/eq shortcuts bypass `getTokenFromCode`, `finishOp`,
		// `readToken_dot` and `readToken_eq_excl` even when the full token path below
		// is off, so their overrides must gate this loop too.
		/** @type {boolean} */
		this._tokenFastPath =
			lazy ||
			(this._noLocations &&
				...
				self.updateContext === proto.updateContext &&
				self.readToken_dot === proto.readToken_dot &&
				self.readToken_eq_excl === proto.readToken_eq_excl);
		...
		this._fullTokenFastPath =
			...
			(lazy ||
				(this._tokenFastPath &&
					self.readToken_slash === proto.readToken_slash &&
					self.readToken_mult_modulo_exp === proto.readToken_mult_modulo_exp &&
					self.readToken_pipe_amp === proto.readToken_pipe_amp &&
					self.readToken_caret === proto.readToken_caret &&
					self.readToken_plus_min === proto.readToken_plus_min &&
					self.readToken_lt_gt === proto.readToken_lt_gt &&
					self.readToken_question === proto.readToken_question &&
					self.readToken_numberSign === proto.readToken_numberSign &&
					self.readRadixNumber === proto.readRadixNumber));
		...
		/** @type {Set<string>[]} */
		this._setPool = [];
		// WHY: a declaration directly under an `export` head is reported by the
		// export statement that holds it, never by the scope, so recording is
		// held back for exactly the one scope depth the head opened at.
		/** @type {number} */
		this._exportHeadDepth = -1;
		// what the scope `exitScope` just closed had recorded, for the caller
		// that owns that scope's node and reads it right after the call
		/** @type {HoistedDeclarations | undefined} */
		this._exitedHoisted = undefined;
		// the program's import and re-export declarations, in source order, each
		// followed by the top-level statement written before it
		/** @type {ModuleDeclarations | undefined} */
		this._moduleDeclarations = undefined;
		// WHY: what precedes a module declaration decides whether removing it
		// joins two statements through ASI, so the consumer needs it and cannot
		// read it back off a list holding declarations alone.
		/** @type {Node | undefined} */
		this._previousTopLevelStatement = undefined;
		...
		/** @type {boolean} */
		this._varIdOwn = self.parseVarId === proto.parseVarId;
		...
		// last arrow finished by parseArrowExpression, so `expr === this._lastArrow`
		// replaces the megamorphic type probes on the expression spine: arrows are
		// created in one place and never backtracked, so identity captures the test
		/** @type {Expression | null} */
		...
		this._funcFastPath =
			lazy &&
			this._ecmaVersion >= 9 &&
			internals.initFunction === proto.initFunction &&
			internals.isSimpleParamList === proto.isSimpleParamList;
		// the owned parseStatement inlines parseFunctionStatement's body
		/** @type {boolean} */
		this._funcStmtOwn =
			this._funcFastPath &&
			internals.parseFunctionStatement === proto.parseFunctionStatement &&
		...
		this._stmt2FastPath =
			...
			this._ecmaVersion >= 9 &&
			this._varIdOwn &&
			internals.parseForStatement === proto.parseForStatement &&
			internals.parseFor === proto.parseFor &&
			internals.parseForIn === proto.parseForIn &&
			internals.parseForAfterInit === proto.parseForAfterInit &&
			this.parseVar === proto.parseVar &&
			internals.parseWhileStatement === proto.parseWhileStatement &&
			internals.parseSwitchStatement === proto.parseSwitchStatement &&
			internals.parseThrowStatement === proto.parseThrowStatement &&
			internals.parseTryStatement === proto.parseTryStatement &&
			internals.parseBreakContinueStatement ===
				proto.parseBreakContinueStatement &&
			internals.parseDoStatement === proto.parseDoStatement &&
			internals.parseWithStatement === proto.parseWithStatement &&
			internals.parseEmptyStatement === proto.parseEmptyStatement &&
			internals.parseDebuggerStatement === proto.parseDebuggerStatement;
	...
	parseExportDeclaration(node) {
		return this._inExportHead(() => this.parseStatement(null));
	}
	...
	finishNodeAt(node, type, pos, loc) {
		if (!this._lazy) {
			return baseProto().finishNodeAt.call(this, node, type, pos, loc);
		}
		node.type = type;
		node.end = pos;
		return node;
	...
	nextToken() {
		...
		if (!this._tokenFastPath || curContext === undefined) {
			this._newlineBefore = 2;
			return baseProto().nextToken.call(this);
		}
		// WHY: read the two fields rather than memoize their answer onto the
		// context. acorn's TokContext constructor sets both, so two loads off one
		// shape beat a symbol-keyed probe — and the memo added a property to the
		// contexts that reached it, splitting them across shapes and making every
		// other read of a context polymorphic.
		if (curContext.preserveSpace || curContext.override) {
			this._newlineBefore = 2;
			...
			if (override !== undefined && curContext.preserveSpace) {
				...
				return /** @type {(parser: EXPECTED_ANY) => void} */ (override)(this);
			}
			return baseProto().nextToken.call(this);
		}
		const input = this.input;
	...
	finishToken(type, value) {
		if (!this._tokenFastPath) {
			return baseProto().finishToken.call(this, type, value);
		}
		this.end = this.pos;
		const prevType = this.type;
		this.type = type;
		...
		// WHY: the types without a context hook are the majority and are answered
		// here, the rest in `_updateContext`. Keeping the two apart is what holds
		// this method under V8's inlining budget — inlined at its ~30 call sites it
		// costs no call at all, and whole it was over the limit and inlined nowhere.
		if (internal.updateContext === null) {
			// keyword-after-dot forbids an expression, else `beforeExpr` says whether
			// a `/` next opens a regexp or divides. `name` has a hook, so it is not
			// here; acorn gives every keyword-after-dot case one too.
			this.exprAllowed =
		...
		this._updateContext(internal, prevType, value);
	}

	/**
	 * `finishToken`'s tail for the token types carrying a context hook: acorn's
	 * `updateContext`, inlined per type.
	 * @param {TokenTypeInternal} internal the token's kind
	 * @param {TokenType} prevType the kind before it
	 * @param {EXPECTED_ANY=} value what it carries
	 * @returns {void}
	 */
	_updateContext(internal, prevType, value) {
		const type = /** @type {TokenType} */ (internal);
		if (type === tokTypes.name) {
			...
			const update =
				/** @type {(prevType: TokenType) => void} */
				(internal.updateContext);
			if (type === tokTypes.parenR || type === tokTypes.braceR) {
				// parenR/braceR.updateContext, inlined
				const context = this.context;
				if (context.length === 1) {
					this.exprAllowed = true;
	...
	next(ignoreEscapeSequenceInKeyword) {
		if (!this._tokenFastPath) {
			return baseProto().next.call(this, ignoreEscapeSequenceInKeyword);
		}
		const type = this.type;
	...
	readWord1() {
		...
		while (pos < len) {
			const ch = input.charCodeAt(pos);
			cls = CHAR_CLASS[ch];
			// CLS_DIGIT and CLS_IDENT are the two identifier-continue classes
			if (cls > CLS_IDENT) break;
			hash = (Math.imul(hash, 33) + ch) | 0;
			pos++;
		}
		if (cls === CLS_UNICODE) {
			// an escape or a non-ASCII char: restart cold, where those rules live
		...
		if (wordLen >= 2 && wordLen <= WORD_CACHE_MAX_LEN) {
			const slot = (hash & WORD_CACHE_MASK) * WORD_CACHE_STRIDE;
			const cached = /** @type {string} */ (WORD_CACHE[slot]);
			if (wordMatches(input, start, cached, wordLen)) return cached;
	...
	readString(quote) {
		if (!this._noLocations || this._ecmaVersion < 10) {
			return baseProto().readString.call(this, quote);
		}
		const input = this.input;
		const len = input.length;
		...
		for (;;) {
			...
			if (ch === quote) break;
			if (ch === 92) {
				// any escape: restart in the cold reader (this.pos is still on the
				// quote), which cooks them all. Keeping none of that here is what keeps
				// this method small enough to inline, and an escaped string is rare.
				this.pos = quotePos;
		...
		this.pos = pos + 1;
		const valueLen = pos - start;
		this.finishToken(
			tokTypes.string,
			valueLen >= 2 && valueLen <= SLICE_CACHE_MAX_LEN
	...
	/** @returns {void} */
	readTmplToken() {
		if (!this._noLocations) return baseProto().readTmplToken.call(this);
		const input = this.input;
		const start = this.pos;
		const len = input.length;
		let pos = start;
	...
	/** @returns {void} */
	skipSpace() {
		if (!this._noLocations) return baseProto().skipSpace.call(this);
		const input = this.input;
		const len = input.length;
		let pos = this.pos;
		while (pos < len) {
	...
	// WHY: this method, `readString` and `finishToken` are each held under V8's
	// callee bytecode limit (460 at the time of writing) so TurboFan inlines them
	// — `readWord` into `nextToken`, `finishToken` at its call sites. That is
	// where this file's tokenizer speed comes from, and it is invisible: adding
	// one argument here once cost 1.4% by pushing it to 465. Before growing any
	// of the three, read the size back with
	// `node --print-bytecode --print-bytecode-filter=readWord` and keep the cold
	// arms (`_readWordIntoCache`, `_readWordUncacheable`, `_readStringCold`,
	// `_updateContext`) carrying whatever does not fit.
	/** @returns {void} */
	readWord() {
		const input = this.input;
		const start = this.pos;
		...
		const wordLen = pos - start;
		if (wordLen < 2 || wordLen > WORD_CACHE_MAX_LEN) {
			return this._readWordUncacheable(start, pos);
		}
		const slot = (hash & WORD_CACHE_MASK) * WORD_CACHE_STRIDE;
		const cached = /** @type {string} */ (WORD_CACHE[slot]);
		// the keyword-set check comes before the char compare: it is one load, and
		// the arm below re-checks the text either way
		if (
			WORD_CACHE[slot + 2] !== this._wordLookups.id ||
			!wordMatches(input, start, cached, wordLen)
		) {
			return this._readWordIntoCache(start, pos, wordLen, slot);
		}
		const type = /** @type {TokenType} */ (WORD_CACHE[slot + 1]);
		this._wordKind = /** @type {number} */ (WORD_CACHE[slot + 3]);
		if (type !== tokTypes.name || !this._inlineFinish) {
			return this.finishToken(type, cached);
		}
		...
		this.value = cached;
		this.exprAllowed =
			((cached === "of" && !this.exprAllowed) ||
				(cached === "yield" && this.inGeneratorContext())) &&
			prevType !== tokTypes.dot &&
			this._ecmaVersion >= 6;
	}

	/**
	 * `readWord`'s miss arm: classify the word, record it against the current
	 * keyword set and finish the token. A stale keyword set lands here too, and
	 * keeps the cached text when it still matches.
	 * @param {number} start where the word starts
	 * @param {number} end where it ends
	 * @param {number} wordLen its length
	 * @param {number} slot its base index in WORD_CACHE
	 * @returns {void}
	 */
	_readWordIntoCache(start, end, wordLen, slot) {
		const input = this.input;
		const lookups = this._wordLookups;
		const cached = /** @type {string} */ (WORD_CACHE[slot]);
		const word = wordMatches(input, start, cached, wordLen)
			? cached
			: /** @type {string} */ (WORD_CACHE[slot] = input.slice(start, end));
		const type = classifyWord(word, lookups.keywords);
		WORD_CACHE[slot + 1] = type;
		WORD_CACHE[slot + 2] = lookups.id;
		this._wordKind = WORD_CACHE[slot + 3] = wordKind(word, lookups);
		this.finishToken(type, word);
	}

	/**
	 * `readWord`'s tail for the lengths WORD_CACHE does not hold. Every keyword is
	 * 2-10 chars, so such a word is a plain name: neither `of` nor `yield`, nor
	 * reserved.
	 * @param {number} start where the word starts
	 * @param {number} end where it ends
	 * @returns {void}
	 */
	_readWordUncacheable(start, end) {
		const word = this.input.slice(start, end);
		this._wordKind = 0;
		if (!this._inlineFinish) return this._finishWordSlow(word);
		this.end = end;
	...
	checkUnreserved(ref) {
		...
		// `yield`, `await` and `arguments` all pass it
		const lookups = this._wordLookups;
	...
	checkLValSimple(expr, bindingType, checkClashes) {
		...
		if (type === "Identifier") {
			...
			if (isBind) {
				...
				if (checkClashes) {
					// the owned checkParams tracks names in a Set; acorn's passes an object
					if (checkClashes instanceof Set) {
	...
	skipLineComment(startSkip) {
		...
		if (
			...
			return baseProto().skipLineComment.call(this, startSkip);
		}
		const input = this.input;
		const start = this.pos;
		const len = input.length;
	...
	skipBlockComment() {
		...
		if (
			...
			return baseProto().skipBlockComment.call(this);
		}
		const start = this.pos;
	...
	startNode() {
		if (!this._lazy) return baseProto().startNode.call(this);
	 ...
	 */
	startNodeAt(pos, loc) {
		if (!this._lazy) return baseProto().startNodeAt.call(this, pos, loc);
	...
	finishNode(node, type) {
		if (!this._lazy) return baseProto().finishNode.call(this, node, type);
		node.type = type;
	 ...
	 */
	parseIdent(liberal) {
		if (!this._lazy) return baseProto().parseIdent.call(this, liberal);
		/** @type {string} */
		let name;
		const type = this.type;
		if (type === tokTypes.name) {
			...
			return baseProto().parseIdent.call(this, liberal);
		}
		const node = /** @type {Identifier} */ (
			/** @type {unknown} */ (new IdentifierNode(this.start, this.end, name))
		);
		// the reserved kind readWord memoized for this token, before next() replaces it
		const kind = this._wordKind;
		this.next(Boolean(liberal));
		if (!liberal) {
			if (kind !== 0) this.checkUnreserved(node);
	...
	parseSubscripts(baseExpr, startPos, startLoc, noCalls, forInit) {
		if (!this._subscriptFastPath || !this._arrowFastPath) {
			return baseProto().parseSubscripts.call(
				this,
				baseExpr,
				startPos,
				startLoc,
	...
	parseSubscript(
		...
		if (!this._subscriptFastPath) {
			return baseProto().parseSubscript.call(
				this,
				baseExpr,
				startPos,
				startLoc,
	...
	parseTopLevel(node) {
		...
		if (!this._lazy || this.options.program) {
			const program = baseProto().parseTopLevel.call(this, node);
			/** @type {EXPECTED_ANY} */
			(program)[HOISTED_DECLARATIONS] = this._takeHoisted(this.scopeStack[0]);
			// acorn's own loop reports no statement order, so what precedes each
			// module declaration is unknown and the record is not kept
			this._moduleDeclarations = undefined;
			return program;
		}
		...
		while (this.type !== tokTypes.eof) {
			...
			if (declaresIntoBlock(statement)) {
				if (blockOverflow !== undefined) blockOverflow.push(count);
				else if (count <= MAX_MASKED_INDEX) blockMask |= 1 << count;
				else blockOverflow = overflowFrom(blockMask, count);
			}
			scratch[count++] = statement;
			this._previousTopLevelStatement = statement;
		}
		...
		program[HOISTED_DECLARATIONS] = this._takeHoisted(this.scopeStack[0]);
		program[MODULE_DECLARATIONS] = this._takeModuleDeclarations();
		program[BLOCK_DECLARATIONS] =
			blockOverflow === undefined ? blockMask : blockOverflow;
		return /** @type {Node} */ (/** @type {unknown} */ (program));
	}
	...
	parseStatement(context, topLevel, exports) {
		if (!this._stmtFastPath) {
			return baseProto().parseStatement.call(this, context, topLevel, exports);
		}
		...
		switch (startType) {
			...
			case tokTypes._function: {
				if (!this._funcStmtOwn) {
					return baseProto().parseStatement.call(
						this,
						context,
						topLevel,
						exports
	...
	parseVarStatement(node, kind, allowMissingInitializer) {
		if (!this._lazy || !this._varIdOwn) {
			const declaration = baseProto().parseVarStatement.call(
				this,
				node,
				kind,
				allowMissingInitializer
			);
			if (kind === "var") this._recordHoisted(declaration);
			return declaration;
		}
	...
	_parseVarStatementAt(start, kind, allowMissingInitializer) {
		...
		if (kind === "var") this._recordHoisted(node);
		return node;
	}
	...
	parseVar(node, isFor, kind, allowMissingInitializer) {
		if (!this._lazy || !this._varIdOwn) {
			return baseProto().parseVar.call(
				this,
				node,
				isFor,
				kind,
	...
	parseExpressionStatement(node, expr) {
		if (!this._lazy) {
			return baseProto().parseExpressionStatement.call(this, node, expr);
		}
	...
	parseBlock(createNewLexicalScope, node, exitStrict) {
		if (!this._lazy) {
			return baseProto().parseBlock.call(
				this,
		...
		const block = new BlockStatementNode(
			start,
			this.lastTokEnd,
		...
		block[BLOCK_DECLARATIONS] =
			blockOverflow === undefined ? blockMask : blockOverflow;
		return /** @type {Node} */ (/** @type {unknown} */ (block));
	}
	...
	parseIfStatement(node) {
		if (!this._lazy) return baseProto().parseIfStatement.call(this, node);
	...
	parseReturnStatement(node) {
		if (!this._lazy) {
			...
			return baseProto().parseReturnStatement.call(this, node);
		}
	...
	_parseForStatementAt(start) {
		...
		if (this.type === tokTypes._var || this.type === tokTypes._const || isLet) {
			...
			if (kind === "var") this._recordHoisted(init);
	...
	_parseSwitchStatementAt(start) {
		...
		for (let sawDefault = false; this.type !== tokTypes.braceR;) {
			if (this.type === tokTypes._case || this.type === tokTypes._default) {
				...
				if (cur) {
					cur[BLOCK_DECLARATIONS] =
						caseOverflow === undefined ? caseMask : caseOverflow;
					this.finishNode(/** @type {Node} */ (cur), "SwitchCase");
				}
				caseMask = 0;
				caseOverflow = undefined;
				...
				const statement = this.parseStatement(null);
				if (declaresIntoBlock(statement)) {
					const index = consequent.length;
					if (caseOverflow !== undefined) caseOverflow.push(index);
					else if (index <= MAX_MASKED_INDEX) caseMask |= 1 << index;
					else caseOverflow = overflowFrom(caseMask, index);
				}
	...
	parseLabeledStatement(node, maybeName, expr, context) {
		if (!this._lazy) {
			return baseProto().parseLabeledStatement.call(
				this,
				node,
				maybeName,
				expr,
	...
	parseSpread(refDestructuringErrors) {
		if (!this._lazy) {
			return baseProto().parseSpread.call(this, refDestructuringErrors);
		}
		const start = this.start;
		this.next();
	...
	parseMaybeConditional(forInit, refDestructuringErrors) {
		if (!this._lazy) {
			return baseProto().parseMaybeConditional.call(
				this,
				forInit,
				refDestructuringErrors
			);
	...
	parseArrowExpression(node, params, isAsync, forInit) {
		if (!this._funcFastPath) {
			const result = baseProto().parseArrowExpression.call(
				this,
				node,
				params,
				isAsync,
	...
	parseFunction(node, statement, allowExpressionBody, isAsync, forInit) {
		if (!this._funcFastPath) {
			return baseProto().parseFunction.call(
				this,
				node,
				statement,
				allowExpressionBody,
	...
	_parseFunctionAt(start, statement, allowExpressionBody, isAsync, forInit) {
		...
		if (statement & FUNC_STATEMENT) this._recordHoisted(finished);
		return /** @type {Expression} */ (/** @type {unknown} */ (finished));
	}
	...
	parseFunctionBody(node, isArrowFunction, isMethod, forInit) {
		if (!this._funcFastPath) {
			baseProto().parseFunctionBody.call(
				this,
				node,
				isArrowFunction,
				isMethod,
			...
			const body = /** @type {Node & { body: Node }} */ (node).body;
			if (body.type === "BlockStatement") {
				/** @type {EXPECTED_ANY} */
				(body)[HOISTED_DECLARATIONS] = !this._recordsHoisted()
					? undefined
					: this._exitedHoisted === undefined
						? NO_HOISTED_DECLARATIONS
						: this._exitedHoisted;
			}
			return;
		}
		...
		if (isExpression) {
			...
			/** @type {EXPECTED_ANY} */
			(target.body)[HOISTED_DECLARATIONS] = this._takeHoisted(
				this.currentScope()
			);
			this._exitedHoisted = undefined;
		}
	...
	checkParams(node, allowDuplicates) {
		...
		const names =
			...
			(params.length === 1 && params[0].type === "Identifier")
				...
				: this._takeSet();
		for (let i = 0; i < params.length; i++) {
			this.checkLValInnerPattern(params[i], BIND_VAR, names);
		}
		if (names !== null) {
			names.clear();
			this._setPool.push(names);
		}
	}
	...
	toAssignable(node, isBinding, refDestructuringErrors) {
		if (!this._lazy) {
			return baseProto().toAssignable.call(
				this,
				node,
				isBinding,
				refDestructuringErrors
	...
	toAssignableList(exprList, isBinding) {
		if (!this._lazy) {
			return baseProto().toAssignableList.call(this, exprList, isBinding);
		}
	...
	parseMaybeDefault(startPos, startLoc, left) {
		if (!this._lazy) {
			return baseProto().parseMaybeDefault.call(this, startPos, startLoc, left);
		}
		const target = left || /** @type {Node} */ (this.parseBindingAtom());
		if (this._ecmaVersion < 6 || !this.eat(tokTypes.eq)) return target;
		const right = this.parseMaybeAssign();
		return /** @type {Node} */ (
			/** @type {unknown} */ (
				new AssignmentPatternNode(startPos, this.lastTokEnd, target, right)
			)
		);
	}
	...
	/** @returns {EXPECTED_ANY} the finished element */
	parseRestBinding() {
		if (!this._lazy) return baseProto().parseRestBinding.call(this);
		const start = this.start;
		this.next();
	...
	parseBindingAtom() {
		if (!this._lazy || this._ecmaVersion < 6) {
			return baseProto().parseBindingAtom.call(this);
		}
		const type = this.type;
	...
	parseExprSubscripts(refDestructuringErrors, forInit) {
		if (!this._arrowFastPath) {
			return baseProto().parseExprSubscripts.call(
				this,
				refDestructuringErrors,
				forInit
			);
	...
	parseExprList(close, allowTrailingComma, allowEmpty, refDestructuringErrors) {
		if (!this._lazy) {
			return baseProto().parseExprList.call(
				this,
				close,
				allowTrailingComma,
				allowEmpty,
	...
	parseBindingList(close, allowEmpty, allowTrailingComma, allowModifiers) {
		if (!this._lazy) {
			return baseProto().parseBindingList.call(
				this,
				close,
				allowEmpty,
				allowTrailingComma,
	...
	parseParenAndDistinguishExpression(canBeArrow, forInit) {
		if (!this._lazy || this._ecmaVersion < 6) {
			return baseProto().parseParenAndDistinguishExpression.call(
				this,
				canBeArrow,
				forInit
			);
	...
	parseExpression(forInit, refDestructuringErrors) {
		if (!this._lazy) {
			return baseProto().parseExpression.call(
				this,
				forInit,
				refDestructuringErrors
			);
	 ...
	 */
	parseNew() {
		if (!this._lazy) return baseProto().parseNew.call(this);
		if (this.containsEsc) {
	...
	parseTemplateElement(opts) {
		if (!this._lazy) return baseProto().parseTemplateElement.call(this, opts);
		const start = this.start;
	...
	parseTemplate(opts) {
		if (!this._lazy) {
			return baseProto().parseTemplate.call(
				this,
	...
	parseMaybeUnary(refDestructuringErrors, sawUnary, incDec, forInit) {
		if (!this._lazy) {
			return baseProto().parseMaybeUnary.call(
				this,
				refDestructuringErrors,
				sawUnary,
				incDec,
	...
	parseMaybeAssign(forInit, refDestructuringErrors, afterLeftParse) {
		if (!this._lazy) {
			return baseProto().parseMaybeAssign.call(
				this,
				forInit,
				refDestructuringErrors,
				afterLeftParse
	...
	parseExprOps(forInit, refDestructuringErrors) {
		if (!this._lazy) {
			return baseProto().parseExprOps.call(
				this,
				forInit,
				refDestructuringErrors
			);
	...
	parseExprOp(left, leftStartPos, leftStartLoc, minPrec, forInit) {
		if (!this._lazy) {
			return baseProto().parseExprOp.call(
				this,
				left,
				leftStartPos,
				leftStartLoc,
	...
	buildBinary(startPos, startLoc, left, right, op, logical) {
		if (!this._lazy) {
			return baseProto().buildBinary.call(
				this,
				startPos,
				startLoc,
				left,
	...
	parseExprAtom(refDestructuringErrors, forInit, forNew) {
		if (!this._lazy) {
			return baseProto().parseExprAtom.call(
				this,
				refDestructuringErrors,
				forInit,
				forNew
	...
	parseObj(isPattern, refDestructuringErrors) {
		if (!this._subscriptFastPath) {
			return baseProto().parseObj.call(this, isPattern, refDestructuringErrors);
		}
		const start = this.start;
		let first = true;
	...
	parseProperty(isPattern, refDestructuringErrors) {
		if (!this._subscriptFastPath) {
			return baseProto().parseProperty.call(
				this,
				isPattern,
				refDestructuringErrors
			);
	...
	parseClass(node, isStatement) {
		if (!this._classFastPath) {
			const parsed = baseProto().parseClass.call(this, node, isStatement);
			if (isStatement) this._recordHoisted(parsed);
			return parsed;
		}
		const classNode =
		...
		if (isStatement) this._recordHoisted(finished);
		return finished;
	}
	...
	parseClassElement(constructorAllowsSuper) {
		if (!this._classFastPath) {
			return baseProto().parseClassElement.call(this, constructorAllowsSuper);
		}
	...
	parseClassElementName(element) {
		if (!this._classFastPath) {
			return baseProto().parseClassElementName.call(this, element);
		}
		const target =
	...
	parseClassMethod(method, isGenerator, isAsync, allowsDirectSuper) {
		if (!this._classFastPath) {
			return baseProto().parseClassMethod.call(
				this,
				method,
				isGenerator,
				isAsync,
	...
	parseClassField(field) {
		if (!this._classFastPath) {
			return baseProto().parseClassField.call(this, field);
		}
		const target =
	...
	parseClassStaticBlock(node) {
		if (!this._classFastPath) {
			return baseProto().parseClassStaticBlock.call(this, node);
		}
		...
		while (this.type !== tokTypes.braceR) {
			const statement = this.parseStatement(null);
			if (declaresIntoBlock(statement)) {
				const index = target.body.length;
				if (blockOverflow !== undefined) blockOverflow.push(index);
				else if (index <= MAX_MASKED_INDEX) blockMask |= 1 << index;
				else blockOverflow = overflowFrom(blockMask, index);
			}
			target.body.push(statement);
		}
		/** @type {EXPECTED_ANY} */
		(target)[BLOCK_DECLARATIONS] =
			blockOverflow === undefined ? blockMask : blockOverflow;
		this.next();
		this.exitScope();
		this.labels = oldLabels;
	...
	parseClassId(node, isStatement) {
		if (!this._classFastPath) {
			return baseProto().parseClassId.call(this, node, isStatement);
		}
	...
	parseClassSuper(node) {
		if (!this._classFastPath) {
			return baseProto().parseClassSuper.call(this, node);
		}
	...
	enterClassBody() {
		if (!this._classFastPath) return baseProto().enterClassBody.call(this);
	...
	/** @returns {void} */
	exitClassBody() {
		if (!this._classFastPath) return baseProto().exitClassBody.call(this);
	...
	parseMethod(isGenerator, isAsync, allowDirectSuper) {
		if (!this._funcFastPath) {
			return baseProto().parseMethod.call(
				this,
				isGenerator,
				isAsync,
				allowDirectSuper
		...
		const oldAwaitPos = this.awaitPos;
		const oldAwaitIdentPos = this.awaitIdentPos;
		node.generator = /** @type {boolean} */ (isGenerator);
	...
	parsePropertyName(prop) {
		if (!this._subscriptFastPath) {
			return baseProto().parsePropertyName.call(this, prop);
		}
		const target =
	...
	parsePropertyValue(
		...
		if (!this._subscriptFastPath) {
			return baseProto().parsePropertyValue.call(
				this,
				prop,
				isPattern,
				isGenerator,
	...
	isAsyncProp(prop) {
		if (!this._subscriptFastPath) {
			return baseProto().isAsyncProp.call(this, prop);
		}
	...
	checkPropClash(prop, propHash, refDestructuringErrors) {
		if (!this._subscriptFastPath) {
			return baseProto().checkPropClash.call(
				this,
				prop,
				propHash,
				refDestructuringErrors
	 ...
	 */
	parseLiteral(value) {
		if (!this._lazy) return baseProto().parseLiteral.call(this, value);
		const start = this.start;
		const end = this.end;
		const len = end - start;
		const raw =
	...
	readRegexp() {
		...
		try {
			...
			this.pos = start;
			return baseProto().readRegexp.call(this);
		}
	...
	validateRegExpPattern(state) {
		try {
			...
			baseProto().validateRegExpPattern.call(this, state);
		}
	}
	...
	exitScope() {
		...
		this._exitedHoisted = scope.hoisted;
		scope.hoisted = undefined;
		// a recorded list is the consumer's now, and an empty one is still a
		// record — what must never reach it is the list of some earlier scope
	...
	/**
	 * Record a declaration statement on the var scope it hoists to, in source
	 * order. A declaration under an `export` head belongs to that statement.
	 * @param {EXPECTED_ANY} node the declaration statement
	 * @returns {void}
	 */
	_recordHoisted(node) {
		if (!this._recordsHoisted()) return;
		if (this.scopeStack.length === this._exportHeadDepth) return;
		const scope = this.currentVarScope();
		if (scope.hoisted === undefined) scope.hoisted = [node];
		else scope.hoisted.push(node);
	}

	/**
	 * Hand the scope's recorded declarations to the node that owns it, leaving
	 * the slot empty for the pool. Call it before the matching `exitScope`.
	 * @param {Scope} scope the scope being closed
	 * @returns {HoistedDeclarations | undefined} the declarations, in source order
	 */
	_takeHoisted(scope) {
		if (!this._recordsHoisted()) return undefined;
		const hoisted = scope.hoisted;
		if (hoisted === undefined) return NO_HOISTED_DECLARATIONS;
		scope.hoisted = undefined;
		return hoisted;
	}

	/**
	 * Whether the declarations of each scope are recorded at all: only when every
	 * production that reaches a recording site is this class's own, since an
	 * override of any of them leaves the record silently short.
	 * @returns {boolean} whether the record is kept
	 */
	_recordsHoisted() {
		return (
			this._lazy &&
			...
			this._funcStmtOwn &&
			this._stmt2FastPath &&
			this.parseClass === WebpackParser.prototype.parseClass
		);
	}

	/**
	 * Record an import or re-export the program states directly. One nested
	 * deeper belongs to no program, so only the top-level list is kept.
	 * @param {EXPECTED_ANY} node the finished declaration
	 * @returns {void}
	 */
	_recordModuleDeclaration(node) {
		...
		const previous = this._previousTopLevelStatement;
		if (this._moduleDeclarations === undefined) {
			this._moduleDeclarations = [node, previous];
		} else {
			this._moduleDeclarations.push(node, previous);
		}
	}

	/**
	 * Hand the program its module declarations, leaving the slot empty.
	 * @returns {ModuleDeclarations | undefined} them, in source order
	 */
	_takeModuleDeclarations() {
		if (!this._recordsModuleDeclarations()) return undefined;
		const declarations = this._moduleDeclarations;
		if (declarations === undefined) return NO_MODULE_DECLARATIONS;
		this._moduleDeclarations = undefined;
		return declarations;
	}

	/**
	 * Whether imports and re-exports are recorded at all: only when this class
	 * reads them, since an override of either is what would go unrecorded.
	 * @returns {boolean} whether the record is kept
	 */
	_recordsModuleDeclarations() {
		const proto = WebpackParser.prototype;
		return (
			this._lazy &&
			this.parseImport === proto.parseImport &&
			this.parseExport === proto.parseExport
		);
	}

	/**
	 * Run `parse` with declaration recording held back for the scope depth it
	 * opens at, which is how an `export` head keeps its own declaration.
	 * @template T
	 * @param {() => T} parse what reads the declaration
	 * @returns {T} whatever it read
	 */
	_inExportHead(parse) {
		const previous = this._exportHeadDepth;
		this._exportHeadDepth = this.scopeStack.length;
		const declaration = parse();
		this._exportHeadDepth = previous;
		return declaration;
	}
	...
	parseImportAttribute() {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseImportAttribute.call(this);
		}
		const start = this.start;
		const key =
			this.type === tokTypes.string
	...
	parseModuleExportName() {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseModuleExportName.call(this);
		}
		if (this.type === tokTypes.string) {
	 ...
	 */
	parseImport(node) {
		const declaration = this._parseImport(node);
		this._recordModuleDeclaration(declaration);
		return declaration;
	}
	...
	_parseImport(node) {
		...
		this._importPhase = null;
		if (!this._moduleDeclFastPath) {
			const result = baseProto().parseImport.call(this, node);
	...
	parseImportSpecifier() {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseImportSpecifier.call(this);
		}
		const start = this.start;
	...
	parseImportDefaultSpecifier() {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseImportDefaultSpecifier.call(this);
		}
	...
	parseImportNamespaceSpecifier() {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseImportNamespaceSpecifier.call(this);
		}
		const start = this.start;
		this.next();
	...
	_parseImportSpecifiersBase() {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseImportSpecifiers.call(this);
		}
		const scratch = this._acquireScratch();
		let count = 0;
		let first = true;
	...
	parseExport(node, exports) {
		const declaration = this._parseExport(node, exports);
		this._recordModuleDeclaration(declaration);
		return declaration;
	}
	 ...
	 * @returns {EXPECTED_ANY} the finished export
	 */
	_parseExport(node, exports) {
		this._moduleSyntaxSeen = true;
		if (!this._moduleDeclFastPath) {
			return baseProto().parseExport.call(this, node, exports);
		}
		this.next();
		// export * from '...'
		...
		if (this.shouldParseExportStatement()) {
			declaration = /** @type {Node} */ (this.parseExportDeclaration(node));
	...
	parseExportAllDeclaration(node, exports) {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseExportAllDeclaration.call(this, node, exports);
		}
		/** @type {Node | null} */
		let exported;
		if (this.eatContextual("as")) {
	...
	parseExportDefaultDeclaration() {
		return this._inExportHead(() => this._parseExportDefaultDeclaration());
	}

	/** @returns {EXPECTED_ANY} what `export default` exports */
	_parseExportDefaultDeclaration() {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseExportDefaultDeclaration.call(this);
		}
		let isAsync = false;
		if (
	...
	parseExportSpecifier(exports) {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseExportSpecifier.call(this, exports);
		}
		const start = this.start;
	...
	parseExportSpecifiers(exports) {
		if (!this._moduleDeclFastPath) {
			return baseProto().parseExportSpecifiers.call(this, exports);
		}
		const scratch = this._acquireScratch();
		let count = 0;
		let first = true;
	...
	parseAwait(forInit) {
		...
		if (!this._lazy) return baseProto().parseAwait.call(this, forInit);
	 ...
	 */
	parseYield(forInit) {
		if (!this._lazy) return baseProto().parseYield.call(this, forInit);
	...
	parsePrivateIdent() {
		if (!this._lazy) return baseProto().parsePrivateIdent.call(this);
	...
	parseExprImport(forNew) {
		...
		// does not check escapes, so the keyword's own check is here
		if (this.containsEsc) {
		...
		if (this.type === tokTypes.parenL && !forNew) {
			node = this.parseDynamicImport(start);
		} else if (this.type === tokTypes.dot) {
			const meta = this.startNodeAt(
				start.start,
				start.loc ? start.loc.start : undefined
			);
			meta.name = "import";
			start.meta = this.finishNode(meta, "Identifier");
			node = this.parseImportMeta(start);
		} else {
	...
	parseImportMeta(node) {
		if (!this._importPhasesEnabled) {
			// only `import.meta` is accepted here, and it is module-only
			this._moduleSyntaxSeen = true;
			this.next();
			const onlyEsc = this.containsEsc;
			const onlyProperty = this.parseIdent(true);
			node.property = onlyProperty;
			if (onlyProperty.name !== "meta") {
				this.raiseRecoverable(
					onlyProperty.start,
...
/** @import { SourcePosition } from "../graph/Dependency" */
...
module.exports.ParserSourceLocation = ParserSourceLocation;
...
module.exports.BLOCK_DECLARATIONS = BLOCK_DECLARATIONS;
module.exports.MAX_MASKED_INDEX = MAX_MASKED_INDEX;
module.exports.HOISTED_DECLARATIONS = HOISTED_DECLARATIONS;
module.exports.MODULE_DECLARATIONS = MODULE_DECLARATIONS;
...
module.exports.declaresIntoBlock = declaresIntoBlock;
module.exports.overflowFrom = overflowFrom;
