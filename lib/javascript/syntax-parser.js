...
class Parser {
	...
	startNode() {
		return new ParserNode(this, this.start, this.startLoc);
	}
	 ...
	 */
	startNodeAt(pos, loc) {
		return new ParserNode(this, pos, /** @type {EXPECTED_ANY} */ (loc));
	}
	...
	copyNode(node) {
		const newNode = /** @type {EXPECTED_ANY} */ (
			new ParserNode(this, node.start, this.startLoc)
		);
...
// cspell:ignore yuku binop prec Prec stanceof iget iset sget sset
...
class WebpackParser extends BaseParser {
	...
	finishNodeAt(node, type, pos, loc) {
		if (!this._lazy) {
			return base.finishNodeAt.call(this, node, type, pos, loc);
		}
		node.type = type;
		node.end = pos;
		return node;
	...
	parseTopLevel(node) {
		...
		if (!this._lazy || this.options.program) {
			const program = base.parseTopLevel.call(this, node);
			/** @type {EXPECTED_ANY} */
	...
	parseVarStatement(node, kind, allowMissingInitializer) {
		if (!this._lazy || !this._varIdOwn) {
			const declaration = base.parseVarStatement.call(
				this,
				node,
				kind,
				allowMissingInitializer
	...
	parseClass(node, isStatement) {
		if (!this._classFastPath) {
			const parsed = base.parseClass.call(this, node, isStatement);
	...
	/**
	 * Whether the `from` under the cursor names the imported binding of
	 * `import source from from "…"` rather than opening the FromClause.
	 * @returns {boolean} whether it names the binding
	 */
	_isPhaseBindingNamedFrom() {
		if (!this.isContextual("from")) return false;
		const input = this.input;
		const next = this._scanGap(this.pos);
		if (!input.startsWith("from", next)) return false;
		if (next + 4 === input.length) return true;
		const after = this.fullCharCodeAt(next + 4);
		return !(isIdentifierChar(after, true) || after === 92);
	}
	...
	parseImportSpecifiers() {
		...
		// `import source from from "…"` binds a variable named `from`, so only a
		// single `from` here means the phase word was the default import name.
		const bindsFrom = phase === "source" && this._isPhaseBindingNamedFrom();
		if (
			(!bindsFrom && this.isContextual("from")) ||
			this.type === tokTypes.comma
		) {
...
const collectCjsRequireSpecifiers = (source) => {
	...
	for (let i = 0; i < source.length; i++) {
		...
		if (i > 0) {
			...
			// a dot is a member access, except as the last of a `...` spread
			if (before === 46 && !source.startsWith("...", i - 3)) continue;
		}
