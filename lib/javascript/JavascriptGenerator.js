...
const ESMCompatibilityDependency = require("../dependencies/esm/ESMCompatibilityDependency");
	...
	getConcatenationBailoutReason(module, context) {
		...
		const isESM =
			...
			presentationalDependencies.some(
				(d) => d instanceof ESMCompatibilityDependency
			);
			...
			get importBindings() {
				if (!importBindings) {
					const data =
						/** @type {NonNullable<GenerateContext["getData"]>} */
						(generateContext.getData)();
					importBindings = data.get("importBindings");
					if (!importBindings) {
						importBindings = new Map();
						data.set("importBindings", importBindings);
					}
				}
