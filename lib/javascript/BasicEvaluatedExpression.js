...
/** @typedef {import("./JavascriptParser").TagInfo} TagInfo */
/** @typedef {TagInfo["data"]} TagInfoData */
/** @typedef {Set<TagInfoData>} Guard */
...
class BasicEvaluatedExpression {
	constructor() {
		...
		/** @type {Set<TagInfoData> | undefined} */
		this._tagInfoData = undefined;
	}
	...
	setIdentifier(identifier, rootInfo, getMembers) {
		...
		if (rootInfo.tagInfo) {
			this.clear();
			this.add(rootInfo.tagInfo.data);
		}
		return this;
	}
	...
	/**
	 * @param {TagInfoData} data TagInfo data
	 */
	add(data) {
		if (!this._tagInfoData) this._tagInfoData = new Set();
		this._tagInfoData.add(data);
	}

	/**
	 * @param {TagInfoData} data TagInfo data
	 * @returns {boolean} in guard or not
	 */
	inGuard(data) {
		if (!this._tagInfoData) return false;
		return this._tagInfoData.has(data);
	}

	getGuard() {
		return this._tagInfoData;
	}
...
/**
 * merge 1 to 2
 * @param {Guard} guard1 expression
 * @param {Guard} guard2 guard
 * @returns {Guard}  guard
 */
BasicEvaluatedExpression.mergeGuards = (guard1, guard2) => {
	if (guard1.size === 0) return guard2;
	if (guard2.size === 0) return new Set(guard1);

	for (const data of guard1) guard2.add(data);
