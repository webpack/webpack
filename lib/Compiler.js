...
class Compiler {
	...
	constructor(context, options = /** @type {WebpackOptions} */ ({})) {
		this.hooks = Object.freeze({
			...
			/** @type {AsyncSeriesHook<[SchemaValidationFunction]>} */
			validate: new AsyncSeriesHook(["validate"])
		});
...
${other}`);
	 ...
	 */
	compile(callback) {
		// Validate hook is called in compile to ensure both watch and run modes can run validate.
		// It's only called once per compiler instance for performance.
		if (this.options.validate && !this._validated) {
			this._validated = true;
			this.hooks.validate.callAsync(
				this.validate,
