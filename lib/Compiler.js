...
class Compiler {
	...
	constructor(context) {
		this.hooks = Object.freeze({
			...
			/** @type {SyncBailHook<[string, object | undefined, string, any[]], true>} */
	...
	getInfrastructureLogger(name, options) {
		...
		return new Logger(
			(type, args) => {
				...
				if (
					this.hooks.infrastructureLog.call(name, options, type, args) ===
					undefined
				) {
					if (this.infrastructureLogger !== undefined) {
						this.infrastructureLogger(name, options, type, args);
					}
				}
			},
			childName => {
