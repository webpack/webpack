...
class Compiler extends Tapable {
	...
	run(callback) {
		...
		const onCompiled = (err, compilation) => {
			...
			process.stdout.write("emitAssets\n");
			...
			this.emitAssets(compilation, err => {
				...
				if (compilation.hooks.needAdditionalPass.call()) {
					...
					this.hooks.done.callAsync(stats, err => {
						...
						this.hooks.additionalPass.callAsync(err => {
							if (err) return finalCallback(err);
							process.stdout.write("additional pass\n");
				...
				this.emitRecords(err => {
					...
					this.hooks.done.callAsync(stats, err => {
						process.stdout.write("finalCallback\n");
	...
	runAsChild(callback) {
		process.stdout.write("runAsChild\n");		
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			this.hooks.make.callAsync(compilation, err => {
				process.stdout.write("make done\n");
				if (err) return callback(err);
				...
				compilation.seal(err => {
					if (err) return callback(err);
					process.stdout.write("afterCompile\n");

					this.hooks.afterCompile.callAsync(compilation, err => {
						if (err) return callback(err);
						process.stdout.write("compile done\n");
