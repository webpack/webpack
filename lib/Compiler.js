...
class Compiler {
	...
	constructor(context, options = /** @type {WebpackOptions} */ ({})) {
		...
		// sticky: set once any compilation ran build-time execution
		// (importModule); disables incremental module-graph reuse for the rest
		// of the watch session, since execution mutates the durable graph in
		// ways a reused make cannot untangle
		/** @type {boolean} */
	...
	_cleanupLastCompilation() {
		if (this._lastCompilation !== undefined) {
			// incremental rebuilds: keep the module graph and modules alive so the
					...
					process.nextTick(() => {
						...
						compilation.finish((err) => {
							...
							compilation.seal((err) => {
								...
								// incremental rebuilds: release the parsers of reused
