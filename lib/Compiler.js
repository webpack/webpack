...
const getWatching = memoize(() => require("./Watching"));
...
/** @import { AssetSymlink, AssetTimestamps } from "./Compilation" */
/**
 * @import {
 * 	EntryNormalized as Entry,
 * 	OutputNormalized as OutputOptions,
 * 	WatchOptions,
 * 	WebpackOptionsNormalized as WebpackOptions,
 * 	Plugins
 * } from "../declarations/WebpackOptions"
 */
/** @import { WebpackPluginFunction, ErrorCallback } from "./webpack" */
 ...
 * 	HotIndex
 * } from "./HotModuleReplacementPlugin"
 */
/** @import Module, { BuildInfo } from "./Module" */
/** @import Watching from "./Watching" */
/** @import { RecordsChunks, RecordsModules } from "./RecordIdsPlugin" */
/** @import { PlatformTargetProperties } from "./config/target" */
/** @import { LoggingFunction } from "./logging/createConsoleLogger" */
/** @import { SplitData } from "./optimize/AggressiveSplittingPlugin" */
/**
 * @import {
 * 	IStats,
 * 	InputFileSystem,
 ...
 * } from "./util/fs"
 */
/** @import { Schema, ValidationErrorConfiguration } from "schema-utils" */
...
// only reached by `watch()`, so a one-shot build never loads the watcher
...
${other}`);
						...
						const doWrite = (content) => {
							...
							(this.outputFileSystem).writeFile(targetPath, content, (err) => {
								...
								// stamped on what was just written, so a file the comparison
								// above left alone keeps the times it already carries
								this._setAssetTimestamps(targetPath, info.timestamps, (err) => {
									if (err) return callback(err);
						...
						/**
						 * Keeps the file already on disk, stamping it with the times the asset
						 * carries — the build which wrote it may not have set them. Only the
						 * modification time decides, as reading a file moves its access time.
						 * @param {IStats} stats stats of the file on disk
						 * @returns {void}
						 */
						const keepExistingFile = (stats) => {
							if (!info.timestamps || stats.mtimeMs === info.timestamps.mtime) {
	...
	/**
	 * Stamps the file an asset was written to with the times it carries. A
	 * symbolic link is left alone, as setting them follows it to its target.
	 * @private
	 * @param {string} targetPath absolute path of the file
	 * @param {AssetTimestamps | undefined} timestamps times the asset carries, when it carries any
	 * @param {ErrorCallback} callback signals when the call finishes
	 * @returns {void}
	 */
	_setAssetTimestamps(targetPath, timestamps, callback) {
		if (!timestamps) return callback(null);
		const fs = /** @type {OutputFileSystem} */ (this.outputFileSystem);
		if (!fs.utimes) {
			return callback(
				new WebpackError(
