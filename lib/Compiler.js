...
class Compiler {
	...
	constructor(context) {
		this.hooks = Object.freeze({
			...
			/** @type {SyncBailHook<[string, string, any[], object | undefined], true>} */
	...
	getInfrastructureLogger(name, options) {
		...
		return new Logger(
			(type, args) => {
				...
				if (
					this.hooks.infrastructureLog.call(name, type, args, options) ===
					undefined
				) {
					if (this.infrastructureLogger !== undefined) {
						this.infrastructureLogger(name, type, args, options);
					}
				}
			},
			childName => {
