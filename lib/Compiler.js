...
class Compiler {
	...
	constructor(context, options = /** @type {WebpackOptions} */ ({})) {
		...
		// sticky for the compiler's lifetime: build-time execution (importModule)
		// mutates the durable graph, so it disables incremental reuse from then on
		/** @type {boolean} */
		this._executeModuleSeen = false;
		// entry deps added inside the make-hook window (config entry plugins);
		// only these may be reconciled away on rebuilds — finishMake-driven deps
		// (html pages, sharing includes) are re-added only when their originator
	...
	_cleanupLastCompilation() {
		if (this._lastCompilation !== undefined) {
			// keep graph and modules alive for reuse; children are never persisted
			const incremental = this.watchMode && !this._executeModuleSeen;
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, (err) => {
					...
					process.nextTick(() => {
						...
						compilation.finish((err) => {
							...
							compilation.seal((err) => {
								...
								// release reused parsers — their tapped hooks capture the
								// compilation and would keep every previous one alive
								if (
									this.watchMode &&
									!this._executeModuleSeen &&
			...
			if (compilation._incrementalRebuild) {
				// make seeding stays O(seed): entry re-adds dedupe and unchanged
				// modules short-circuit; then rebuild only what changed
				compilation._incrementalNormalModuleFactory =
					params.normalModuleFactory;
				// pre-make source-module count (runtime modules are seal artifacts)
				let preMakeCount = 0;
				...
				compilation._inMakeHookWindow = true;
				this.hooks.make.callAsync(compilation, (err) => {
					compilation._inMakeHookWindow = false;
