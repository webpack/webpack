...
${other}`);
						...
						/**
						 * @returns {Partial<IStats> | undefined} stats
						 */
						const getStats = () => info.stats;
						 ...
						 * @param {Partial<IStats> | undefined} stats stats
						 * @returns {void}
						 */
						const doWrite = (content, stats) => {
							const options = {};
						...
						/**
						 * @param {IStats} existingStats stats
						 * @returns {void}
						 */
						const processExistingFile = existingStats => {
							...
							if (
								content.length === existingStats.size &&
								(stats && stats.mode ? existingStats.mode === stats.mode : true)
							) {
