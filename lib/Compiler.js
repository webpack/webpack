...
${other}`);
	 ...
	 * @param {NormalModuleFactory=} normalModuleFactory the factory of the same compilation, whose rule set decides whether contexts are issuer-sensitive
	 * @returns {ContextModuleFactory} the context module factory
	 */
	createContextModuleFactory(normalModuleFactory) {
		const contextModuleFactory = new ContextModuleFactory(
			this.resolverFactory,
			normalModuleFactory && normalModuleFactory.ruleSet
		);
	...
	newCompilationParams() {
		const normalModuleFactory = this.createNormalModuleFactory();
		const params = {
			normalModuleFactory,
			contextModuleFactory: this.createContextModuleFactory(normalModuleFactory)
		};
		return params;
	}
