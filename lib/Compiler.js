...
Watching.prototype._done = function(err, compilation) {
	...
	let stats = this._getStats(compilation);
	if(err) {
...
Compiler.prototype.watch = function(watchOptions, handler) {
	this.fileTimestamps = {};
	this.contextTimestamps = {};
	let watching = new Watching(this, watchOptions, handler);
	return watching;
};
...
Compiler.prototype.runAsChild = function(callback) {
	this.compile(function(err, compilation) {
		...
		let entries = Object.keys(compilation.entrypoints).map(function(name) {
	...
	function emitFiles(err) {
		...
		require("async").forEach(Object.keys(compilation.assets), function(file, callback) {
			...
			function writeOut(err) {
				if(err) return callback(err);
				let targetPath = this.outputFileSystem.join(outputPath, targetFile);
...
Compiler.prototype.emitRecords = function emitRecords(callback) {
	...
	let idx1 = this.recordsOutputPath.lastIndexOf("/");
	let idx2 = this.recordsOutputPath.lastIndexOf("\\");
...
Compiler.prototype.createChildCompiler = function(compilation, compilerName, outputOptions, plugins) {
	let childCompiler = new Compiler();
...
Compiler.prototype.newCompilation = function(params) {
	let compilation = this.createCompilation();
...
Compiler.prototype.createNormalModuleFactory = function() {
	let normalModuleFactory = new NormalModuleFactory(this.options.context, this.resolvers, this.options.module || {});
	this.applyPlugins("normal-module-factory", normalModuleFactory);
	return normalModuleFactory;
};

Compiler.prototype.createContextModuleFactory = function() {
	let contextModuleFactory = new ContextModuleFactory(this.resolvers, this.inputFileSystem);
