...
/** @typedef {import('estree').VariableDeclarator} DeclaratorNode */
...
/** @typedef {LiteralNode | string | null | undefined} Source */
...
class JavascriptParser {
	constructor(options, sourceType = "auto") {
		this.hooks = Object.freeze({
			/** @type {HookMap<SyncBailHook<[ExpressionNode]>>} */
			evaluateTypeof: new HookMap(() => new SyncBailHook(["expression"])),
			/** @type {HookMap<SyncBailHook<[ExpressionNode]>>} */
			evaluate: new HookMap(() => new SyncBailHook(["expression"])),
			/** @type {HookMap<SyncBailHook<[IdentifierNode]>>} */
			evaluateIdentifier: new HookMap(() => new SyncBailHook(["expression"])),
			/** @type {HookMap<SyncBailHook<[ExpressionNode]>>} */
			evaluateDefinedIdentifier: new HookMap(
				() => new SyncBailHook(["expression"])
			),
			/** @type {HookMap<SyncBailHook<[CallExpressionNode, BasicEvaluatedExpression | undefined]>>} */
			evaluateCallExpressionMember: new HookMap(
				() => new SyncBailHook(["expression", "param"])
			),
			/** @type {SyncBailHook<[StatementNode]>} */
			preStatement: new SyncBailHook(["statement"]),

			/** @type {SyncBailHook<[import('estree').ModuleDeclaration | DeclarationNode]>} */
			blockPreStatement: new SyncBailHook(["declaration"]),
			/** @type {SyncBailHook<[StatementNode]>} */
			statement: new SyncBailHook(["statement"]),
			/** @type {SyncBailHook<[StatementNode]>} */
			statementIf: new SyncBailHook(["statement"]),
			/** @type {HookMap<SyncBailHook<[StatementNode]>>} */
			label: new HookMap(() => new SyncBailHook(["statement"])),
			/** @type {SyncBailHook<[StatementNode, Source]>} */
			import: new SyncBailHook(["statement", "source"]),
			/** @type {SyncBailHook<[StatementNode, Source, string, string]>} */
			...
			/** @type {SyncBailHook<[StatementNode, DeclarationNode]>} */
			exportDeclaration: new SyncBailHook(["statement", "declaration"]),
			/** @type {SyncBailHook<[StatementNode, DeclarationNode]>} */
			exportExpression: new SyncBailHook(["statement", "declaration"]),
			/** @type {SyncBailHook<[StatementNode, string, string, number | undefined]>} */
			...
			/** @type {SyncBailHook<[StatementNode, Source, string, string, number | undefined]>} */
			...
			/** @type {SyncBailHook<[DeclaratorNode, StatementNode]>} */
			preDeclarator: new SyncBailHook(["declarator", "statement"]),
			/** @type {SyncBailHook<[DeclaratorNode, StatementNode]>} */
			declarator: new SyncBailHook(["declarator", "statement"]),
			/** @type {HookMap<SyncBailHook<[DeclarationNode]>>} */
			...
			/** @type {HookMap<SyncBailHook<[import('estree').AssignmentExpression]>>} */
			assign: new HookMap(() => new SyncBailHook(["expression"])),
			/** @type {HookMap<SyncBailHook<[ExpressionNode]>>} */
			typeof: new HookMap(() => new SyncBailHook(["expression"])),
			/** @type {SyncBailHook<[ExpressionNode]>} */
			importCall: new SyncBailHook(["expression"]),
			/** @type {SyncBailHook<[ExpressionNode]>} */
			topLevelAwait: new SyncBailHook(["expression"]),
			/** @type {HookMap<SyncBailHook<[ExpressionNode]>>} */
			call: new HookMap(() => new SyncBailHook(["expression"])),
			/** @type {HookMap<SyncBailHook<[ExpressionNode, any[]]>>} */
			...
			/** @type {SyncBailHook<[import('estree').Program, CommentNode[]]>} */
	...
	initializeEvaluating() {
		...
		this.hooks.evaluate.for("UnaryExpression").tap("JavascriptParser", expr => {
			if (expr.type !== "UnaryExpression") return;
		...
		this.hooks.evaluate.for("ThisExpression").tap("JavascriptParser", expr => {
			if (expr.type !== "ThisExpression") return;
	...
	/**
	 * @param {ExpressionNode | import('estree').SpreadElement} expression expression node
	 * @returns {BasicEvaluatedExpression | undefined} evaluation result
	 */
	evaluateExpression(expression) {
		// Since there is no taps on SpreadElement we can skip it
