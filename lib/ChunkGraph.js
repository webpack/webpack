...
class ChunkGraph {
	...
	connectChunkAndEntryModule(chunk, module, entrypoint) {
		...
		const dependentChunks = this.getChunkEntryDependentChunksIterable(chunk);
		for (const depChunk of dependentChunks) {
			// Cannot connect chunk to chunk - need to connect modules from dependent chunks instead
			for (const module of this.getChunkModulesIterable(depChunk)) {
...
Caller might not support runtime-dependent code generation (opt-out via optimization.usedExports: "global").`
	...
	/**
	 * @param {Chunk} chunk the chunk
	 * @returns {Chunk[]} entry-dependent chunks
	 */
	static getChunkEntryDependentChunks(chunk) {
		const chunkGraph = ChunkGraph.getChunkGraphForChunk(
			chunk,
			"ChunkGraph.getChunkEntryDependentChunks",
			"DEP_WEBPACK_CHUNK_GRAPH_GET_CHUNK_ENTRY_DEPENDENT_CHUNKS"
		);
		...
		for (const entryModule of chunkGraph.getChunkEntryModulesIterable(chunk)) {
			for (const dep of entryModule.dependencies) {
				if (dep.module) {
					const depChunks = chunkGraph.getModuleChunks(dep.module);
		...
		return Array.from(dependentChunks);
	}
