...
class ChunkGraph {
	 ...
	 * @returns {void}
	 */
	_clearRuntimeModuleHashes(chunk) {
		const cgc = this._getChunkGraphChunk(chunk);
		if (cgc.fullHashModules !== undefined) {
			cgc.fullHashModules.clear();
		}
		if (cgc.dependentHashModules !== undefined) {
			cgc.dependentHashModules.clear();
		}
	}

	/**
	 * Drops per-seal module state cached on a reused chunk graph: graph hashes
	 * reflect the previous seal's exports/connection states, and runtime
	 * requirements only ever merge, so dropped ones would stick.
	 * @param {Module} module the module
	 * @returns {void}
	 */
	_clearModuleSealState(module) {
		const cgm = this._getChunkGraphModule(module);
		cgm.graphHashes = undefined;
		cgm.graphHashesWithConnections = undefined;
		cgm.runtimeRequirements = undefined;
	}

	/**
	 * Same for per-chunk runtime requirements.
	 * @param {Chunk} chunk the chunk
	 * @returns {void}
	 */
	_clearChunkSealState(chunk) {
		const cgc = this._getChunkGraphChunk(chunk);
		cgc.runtimeRequirements = undefined;
		cgc.runtimeRequirementsInTree = new Set();
	}
