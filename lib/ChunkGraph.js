...
class ChunkGraph {
	...
	constructor(moduleGraph, hashFunction = DEFAULTS.HASH_FUNCTION) {
		...
		// build-time execution (`Compilation.executeModule`) runs modules in a
		// CommonJS-style vm wrapper, so templates must not emit ESM-only syntax
		/** @type {boolean} */
		this.buildTimeExecution = false;
