...
class LibraryTemplatePlugin {
	/**
	 * @param {string|string[]|LibraryCustomUmdObject|Function} name name of library
	 ...
	 * @param {string|string[]|Function} exportProperty which export should be exposed as library
	 */
			...
			switch (this.target) {
				case "var":
					...
					new SetVarMainTemplatePlugin(
						this._withName(
							name => `var ${accessorAccess(undefined, name, "root")}`
						),
						false
					).apply(compilation);
					break;
				case "assign":
					new SetVarMainTemplatePlugin(
						this._withName(name => accessorAccess(undefined, name, "root")),
						false
					).apply(compilation);
					break;
				case "this":
				...
				case "window":
					if (this.name) {
						new SetVarMainTemplatePlugin(
							this._withName(name => accessorAccess(this.target, name, "root")),
							false
						).apply(compilation);
					} else {
				...
				case "commonjs":
					if (this.name) {
						new SetVarMainTemplatePlugin(
							this._withName(name =>
								accessorAccess("exports", name, "commonjs")
							),
							false
						).apply(compilation);
					} else {
				...
				case "amd-require": {
					...
					if (
						...
						throw new Error(
							"library name must be a string or function for amd target"
						);
					}
	...
	_withName(fn) {
		if (typeof this.name === "function") {
			// workaround types
			const nameFn = /** @type {Function} */ (this.name);
			return data => fn(nameFn(data));
		}
		return fn(this.name);
	}
}
