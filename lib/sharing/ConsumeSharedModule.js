...
const ConsumeSharedDependenciesBlock = require("./ConsumeSharedDependenciesBlock");
...
class ConsumeSharedModule extends Module {
	...
	readableIdentifier(requestShortener) {
		...
		return `shared module (${shareScope}) ${shareKey}@${versionToString(
			requiredVersion
		)}${strictVersion ? " (strict)" : ""}${request ? ` -> ${request}` : ""}`;
	}
	...
	build(options, compilation, resolver, fs, callback) {
		...
		if (this.options.import) {
			const block = new ConsumeSharedDependenciesBlock();
	...
	codeGeneration({ chunkGraph, moduleGraph, runtimeTemplate }) {
		...
		let fallbackCode;
		if (request) {
			const fallbackBlock = this.blocks[0];
			const fallbackDep = fallbackBlock.dependencies[0];
			const fallbackModule = moduleGraph.getModule(fallbackDep);
			...
			fallbackCode = runtimeTemplate.returningFunction(
				ensureChunk.startsWith("Promise.resolve(")
					? `${factory}`
					: `${ensureChunk}.then(${runtimeTemplate.returningFunction(factory)})`
			);
		}
		let fn = "load";
