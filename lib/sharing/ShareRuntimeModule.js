				...
				[
					...
					`var register = ${runtimeTemplate.basicFunction(
						...
						[
							"version = version || [];",
							"currentName = name;",
							`var versionConflict = ${runtimeTemplate.basicFunction("v", [
								'return warn("Version conflict for shared modules: " + name + " " + (v && v.slice(0,3).join(".") + (v.length>3 ? "-"+v.slice(3).join(".") : "")) + " <=> " + (version && version.slice(0,3).join(".") + (version.length>3 ? "-"+version.slice(3).join(".") : "")));'
							])};`,
							`var registerCurrent = ${runtimeTemplate.basicFunction("", [
								...
								Template.indent([
									...
									// Never replace a normal version with a prerelease version
									"if (v.length <= 3 && version.length > 3) return;",
									"for(var i = 0; i < version.length && i < v.length; i++) {",
									Template.indent([
										"if(v[i] != version[i]) { // loose equal is intentional to match string and number",
										Template.indent([
											// For prerelease versions, numeric identifiers come before string identifiers
											'if (typeof v[i] === "number" && typeof version[i] === "string") return;',
											'if (typeof v[i] === "string" && typeof version[i] === "number") { i = -1; break; }',
									...
									"}",
									// For prerelease versions, a smaller set of identifiers comes before a larger set
							...
							"if (version.length > 3) {",
							Template.indent([
								// Register at the global level in case this is the only version as a singleton
								"registerCurrent();",
								'currentName += "`" + version.join("`")',
								"registerCurrent();",
								"return;"
							]),
							"}",
