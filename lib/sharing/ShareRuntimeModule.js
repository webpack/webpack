		...
		return Template.asString([
			...
			`${RuntimeGlobals.initializeSharing} = ${runtimeTemplate.basicFunction(
				...
				[
					...
					// using eval due that CompatibilityPlugin is renaming the variable dynamically
					"// get parent shared modules if are availables",
					`if(eval('typeof ${RuntimeGlobals.require} !== "undefined"') && eval('${RuntimeGlobals.shareScopeMap}') && eval('${RuntimeGlobals.shareScopeMap}')[name]) ${RuntimeGlobals.shareScopeMap}[name] = eval('${RuntimeGlobals.shareScopeMap}')[name]`,
					...
					`var initExternal = ${runtimeTemplate.basicFunction("id", [
						...
						Template.indent([
							...
							"if(module.then) return module.then(initFn, handleError);",
							"var initResult = initFn(module);",
							"if(initResult && initResult.then) return initResult['catch'](handleError);"
						]),
