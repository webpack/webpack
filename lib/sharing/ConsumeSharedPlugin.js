				...
				const createConsumeSharedModule = (context, request, config) => {
					...
					return Promise.all([
						...
						new Promise(
							 ...
							 */
							(resolve) => {
								const explicitRequiredVersion = config.requiredVersion;
								if (explicitRequiredVersion === false) {
									resolve(false);
									return;
								}
								...
								if (packageName === undefined) {
									if (/^(?:\/|[A-Z]:|\\\\)/i.test(request)) {
										...
										if (explicitRequiredVersion !== undefined) {
											resolve(
												/** @type {SemVerRange} */ (explicitRequiredVersion)
											);
										} else {
											resolve();
										}
