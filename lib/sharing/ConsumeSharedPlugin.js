...
const { parseRange, satisfy } = require("../util/semver");
			...
			(compilation, { normalModuleFactory }) => {
				...
				const createConsumeSharedModule = (context, request, config) => {
					...
					return Promise.all([
						...
						// Check if consumption is excluded by requiredVersion
						...
						// Memoize this so that lookups are fast
						if (config.exclusionCriteria && config.exclusionCriteria.version) {
							// Check the version of the package against exclusion range
