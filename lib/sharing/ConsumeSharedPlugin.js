	...
	constructor(options) {
		...
		this._consumes = parseOptions(
			...
			(item, key) => {
				...
				try {
					result =
						item === key || !(typeof item === "string" || Array.isArray(item))
							...
							: // key is a request/key
							  // item is a version, either a semver string or already parsed as a list
							  {
									import: key,
									shareScope: options.shareScope || "default",
									shareKey: key,
					...
					throw new Error(
						`Error creating share entry for ${key}: ${e.message}`
					);
				}
				return result;
			},
						...
						new Promise(resolve => {
							...
							getDescriptionFile(
								...
								(err, result) => {
									...
									let parsedRequiredVersion;
									try {
										parsedRequiredVersion = parseRequiredVersion(
											requiredVersion
										);
									} catch (e) {
										requiredVersionWarning(
											`Required version in description file (${requiredVersion}) is unsupported: ${e.message}`
										);
										return resolve();
									}
									resolve(parsedRequiredVersion);
								}
							);
						})
