...
exports.parseRequiredVersion = range => {
	...
	if (r.set.length !== 1) {
		throw new Error(`Cannot handle version range ${range}: contains more than one clause`);
	}

	let s = r.set[0];

	// Handle the case '>=x.y.z'
	if (s.length === 1 && s[0].operator === '>=') {
		let v = s[0].semver;
		if (!(v.prerelease.length === 0 && v.build.length === 0)) {
			throw new Error(`Cannot handle version range ${range}: includes prerelease and/or build`);
		}
		return [v.major, v.minor, v.patch];
	}

	if (!(s.length === 2 && s[0].operator === '>=' && s[1].operator === '<')) {
		// TODO: is the order fixed, or could s[0] be '<' and s[1] be '>='?
		throw new Error(`Cannot handle version range ${range}: simplifies to more complex than one simple range`);
	}
	...
	if (!(v1.prerelease.length === 0 
		...
		&& v2.prerelease[0] === 0
		&& v2.build.length === 0)) {
		throw new Error(`Cannot handle version range ${range}: each end of the range is not simple enough`);
	}
	...
	for (let level of ['major', 'minor', 'patch']) {
		if (exact === true) {
			if (v1[level] === v2[level]) {
				...
				throw new Error(`Cannot handle version range ${range}: exact levels do not match up`);
			}
		} else {
			if (v2[level] === 0) {
				result.push(v1[level])
			} else {
				throw new Error(`Cannot handle version range ${range}: fuzzy levels do not match up`);
			}
		}
	}
	return result;
...
exports.isRequiredVersion = str => {
	try {
		exports.parseRequiredVersion(str);
	} catch (e) {
		return false;
	}
	return true;
