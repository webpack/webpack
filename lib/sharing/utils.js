...
const { dirname, join, readJson } = require("../fs/fs");
const { parseRange, satisfy } = require("./semver");

/** @import { InputFileSystem, JsonObject, JsonPrimitive } from "../fs/fs" */
/** @import { SharedModuleFilter } from "../../declarations/plugins/sharing/SharePlugin" */
...
/**
 * Tests the request remainder under a prefix key against `include`/`exclude` request filters.
 * @param {string} remainder request after the prefix
 * @param {SharedModuleFilter=} include only share matching requests
 * @param {SharedModuleFilter=} exclude don't share matching requests
 * @returns {boolean} true, when the request should be shared
 */
const matchesRequestFilters = (remainder, include, exclude) => {
	/**
	 * @param {string | RegExp} pattern request filter
	 * @returns {boolean} true, when the remainder matches
	 */
	const matches = (pattern) => {
		if (typeof pattern === "string") return remainder === pattern;
		// a global or sticky RegExp would resume from the previous match
		pattern.lastIndex = 0;
		return pattern.test(remainder);
	};
	if (include && include.request !== undefined && !matches(include.request)) {
		return false;
	}
	return !(
		exclude &&
		exclude.request !== undefined &&
		matches(exclude.request)
	);
};

module.exports.matchesRequestFilters = matchesRequestFilters;

/**
 * Tests a module version against `include`/`exclude` version filters; an unknown version fails `include`.
 * @param {string | false | undefined} version version of the module
 * @param {SharedModuleFilter=} include only share matching versions
 * @param {SharedModuleFilter=} exclude don't share matching versions
 * @returns {boolean} true, when the version should be shared
 */
const matchesVersionFilters = (version, include, exclude) => {
	if (
		include &&
		include.version !== undefined &&
		(!version || !satisfy(parseRange(include.version), version))
	) {
		return false;
	}
	return !(
		exclude &&
		exclude.version !== undefined &&
		version &&
		satisfy(parseRange(exclude.version), version)
	);
};

module.exports.matchesVersionFilters = matchesVersionFilters;
