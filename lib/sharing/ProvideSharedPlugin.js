	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				let unresolvedProvides;
				/** @type {Map<string, ProvideOptions>} */
				let resolvedProvides;
				/** @type {Map<string, ProvideOptions>} */
				let prefixedProvides;
				const promise = resolveMatchedConfigs(compilation, this._provides).then(
					({ resolved, unresolved, prefixed }) => {
						resolvedProvides = resolved;
						unresolvedProvides = unresolved;
						prefixedProvides = prefixed;
					}
				);
				const data = {
					provided: new Map()
				...
				normalModuleFactory.hooks.module.tap(
					...
					(module, { resource, resourceResolveData }, { request }) => {
						if (data.provided.has(resource)) {
							return module;
						}
						{
							const config = unresolvedProvides.get(request);
							if (config !== undefined) {
								provideModule(request, config, resource, resourceResolveData);
							}
						}
						{
							const config = resolvedProvides.get(resource);
							if (config !== undefined) {
								provideModule(resource, config, resource, resourceResolveData);
							}
						}
						for (const [prefix, config] of prefixedProvides) {
