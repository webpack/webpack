 ...
 * @property {((string|RegExp)[] | string | RegExp | Function)=} exclude
 * @property {(string | RegExp | (string|RegExp)[])=} prefer
		...
		this._provides = parseOptions(
			...
			item => {
				...
				const result = {
					...
					exclude:
						(options.provides &&
							options.provides[item] &&
							options.provides[item].exclude) ||
						undefined,
				...
				normalModuleFactory.hooks.module.tap(
					...
					(module, { resource, resourceResolveData }, resolveData) => {
						...
						const { request } = resolveData;
						{
							...
							if (config !== undefined) {
								// Check if request is made from a context which is excluded
								if (
