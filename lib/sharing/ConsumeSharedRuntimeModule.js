	...
	generate() {
		...
		return Template.asString([
			...
			`var findVersion = ${runtimeTemplate.basicFunction(
				"scope, key, version, strict",
				[
					"version = version || [];",
					"var currentName = key;",
					`var versions = version.map(${runtimeTemplate.returningFunction(
						'currentName += "`" + v',
						"v"
					)});`,
					"var lastVersion;",
					...
					Template.indent([
						`if(${RuntimeGlobals.hasOwnProperty}(scope, currentName) && !invalidVersion(lastVersion = scope[currentName].version || [], version)) return scope[currentName];`
					]),
					"}",
					'var msg = "Unsatisfied version of shared module " + key + "@" + (lastVersion && lastVersion.join(".")) + ", but required " + version.join(".");',
