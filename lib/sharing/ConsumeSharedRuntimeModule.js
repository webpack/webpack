	...
	generate() {
		...
		return Template.asString([
			...
			`var satisfies = ${runtimeTemplate.basicFunction(
				...
				[
					'console.log("checking if "+JSON.stringify(version)+" satisfies "+JSON.stringify(requiredVersion));',
					// A prerelease version never satisfies a non-prerelease requiredVersion
					"if (version.length > 3 && requiredVersion.length <= 4) return;",
					// Any non-prerelease version satisfies requiredVersion [] (i.e., '*')
					"if (requiredVersion.length === 0) return 1;",
					"var fixed = requiredVersion[0]",
					// A prerelease version must match the major/minor/patch components of requiredVersion
					"if (version.length > 3 && fixed < 3) fixed = 3;",
					"for(var i = 0, ri = 1; ri < requiredVersion.length; i++, ri++) {",
					Template.indent([
						"if (i < fixed) {",
						Template.indent([
							"if (version[i] !== requiredVersion[ri]) return;"
						]),
						"} else {",
						Template.indent([
							// For prerelease versions, numeric identifiers < non-numeric identifiers.
							'if (i >= 3 && typeof version[i] === "number" && typeof requiredVersion[ri] === "string") return;',
							"if (version[i] < requiredVersion[ri]) return;"
						]),
						"}"
					]),
			...
			/*
				// all false
				[
					satisfies([1,2,3,0], [2,1,2,3]),
					satisfies([1,2,3,0], [2,1,2,3,'0']),
					satisfies([1,2,4,10], [2,1,2,3,0]),
					satisfies([1,2,4,10], [2,1,2,3]),
					satisfies([1,2,3,10], [4,1,2,3,0]),
					satisfies([1,2,3,0], []),
					satisfies([], [0,1,2,3])
				]
				// all true
				[
					satisfies([1,2,4], [2,1,2,3,0]),
					satisfies([1,2,3,'a'], [2,1,2,3,0]),
					satisfies([1,2,3,'a'], [2,1,2,3,0]),
					satisfies([1,2,3,0,1], [4,1,2,3,0]),
					satisfies([1,2,3], []),
					satisfies([], [])
				]
			*/
			`var formatVersion = ${runtimeTemplate.basicFunction("v", [
				'return v.slice(0,3).join(".") + (v.length > 3 ? "-"+v.slice(3).join(".") : "")'
			])};`,
			`var formatVersionRange = ${runtimeTemplate.basicFunction(
				...
				[
					...
					'return (fixed < v.length ? ">=" : "")+formatVersion(v)+((fixed===0 || fixed === v.length) ? "" : " <" + formatVersion(v.slice(0,3).map((val, i) => (i<fixed-1) ? val : (i===fixed-1) ? (val + 1) : 0)));'
				]
			)};`,
			...
			`var findVersion = ${runtimeTemplate.basicFunction(
				...
				[
					...
					// On the other hand, for error messages, it would be useful to know the closest version, which involves iterating through the prefixed versions.
					"requiredVersion = requiredVersion || [];",
					"var currentName = key;",
					"var fixed = requiredVersion.length > 0 ? requiredVersion[0] : 0",
					// append the fixed version components to the name
					'if (fixed > 0) {currentName += "`" + requiredVersion.slice(1, fixed+1).join("`")}',
					'console.log("Finding version "+currentName + " in "+JSON.stringify(Array.from(Object.keys(scope))));',
					`if(${RuntimeGlobals.hasOwnProperty}(scope, currentName) && satisfies(scope[currentName].version || [], requiredVersion)) return scope[currentName];`,
					// We do not have a version that satisfies the required range. Find some registered versions by relaxing the fixed version components.
					"currentName = key;",
					// Use an object for deduplication
					"versions = [];",
					`if (${RuntimeGlobals.hasOwnProperty}(scope, currentName)) { versions.push(formatVersion(scope[currentName].version || []))}`,
					"for (var i = 1; i < fixed+1; i++) {",
					Template.indent([
						'currentName += "`" + requiredVersion[i]',
						`if (${RuntimeGlobals.hasOwnProperty}(scope, currentName)) { var v = formatVersion(scope[currentName].version || []); if (versions[0] !== v) versions.unshift(v); }`
					]),
					"}",
					// TODO: message isn't very clear
					'var msg = "Unsatisfied version of shared module " + key + "@[" + versions.join(", ") + "] (required " + key + "@" + formatVersionRange(requiredVersion) + ")";',
					"if(strict) throw new Error(msg);",
					'typeof console !== "undefined" && console.warn && console.warn(msg);'
				]
			)};`,
