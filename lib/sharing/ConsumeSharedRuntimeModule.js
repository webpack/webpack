	...
	generate() {
		...
		return Template.asString([
			...
			`var getInvalidSingletonVersionMessage = ${runtimeTemplate.basicFunction(
				"key, version, requiredVersion, from",
				[
					`return "Unsatisfied version " + version + (from ? " from " + from: '') + " of shared singleton module " + key + " (required " + rangeToString(requiredVersion) + ")"`
				]
			)};`,
			`var getSingletonVersion = ${runtimeTemplate.basicFunction(
				...
				[
					...
					"if (!satisfy(requiredVersion, version)) " +
						'typeof console !== "undefined" && console.warn && console.warn(getInvalidSingletonVersionMessage(key, version, requiredVersion, scope[key][version].from));',
			...
			`var getStrictSingletonVersion = ${runtimeTemplate.basicFunction(
				...
				[
					"var version = findSingletonVersionKey(scope, key);",
					"if (!satisfy(requiredVersion, version)) " +
						"throw new Error(getInvalidSingletonVersionMessage(key, version, requiredVersion, scope[key][version].from));",
					"return get(scope[key][version]);"
				]
			)};`,
