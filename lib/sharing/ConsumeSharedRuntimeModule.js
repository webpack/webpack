	...
	generate() {
		...
		return Template.asString([
			...
			Template.indent([
				...
				"return scope[key].l = 1, scope[key].g();"
			]),
			"}",
			...
			Template.indent([
				`${RuntimeGlobals.initializeSharing}(scopeName);`,
				`var scope = ${RuntimeGlobals.shareScopeMap}[scopeName];`,
				`return scope && ${RuntimeGlobals.hasOwnProperty}(scope, key) ? (scope[key].l = 1, scope[key].g()) : fallback();`
			]),
			"}",
			...
			Template.indent([
				`${RuntimeGlobals.initializeSharing}(scopeName);`,
				`var scope = ${RuntimeGlobals.shareScopeMap}[scopeName];`,
				`return scope && ${RuntimeGlobals.hasOwnProperty}(scope, key) && !checkVersion(scope, key, version) ? (scope[key].l = 1, scope[key].g()) : fallback();`
			]),
			"}",
