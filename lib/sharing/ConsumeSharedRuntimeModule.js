...
class ConsumeSharedRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		return Template.asString([
			// If we only have a prerelease version available, it won't show up here since the prerelease has a completely fixed version number and is not promoted to the general scope. This is because we don't want a prerelease to override a normal release in general (i.e., ^1.1.0 should prefer 1.1.0 over 1.2.0-beta.1). However, we have a problem if there is *only* one prerelease available - we can't test for its existence.
			// TODO: for singletons, we can handle the key being a prerelease, but if *any* other non-prerelease version becomes available, it takes precedence, since it satisfies more ranges.
			...
			// TODO: extend satisfies() to handle prerelease versions. In particular, a prerelease satisfies a range *only* if the first three components match exactly, so [1,2,3,'alpha',0] satisfies [3,1,2,3] but not [0,1,2,2].
