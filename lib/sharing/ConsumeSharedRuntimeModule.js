...
class ConsumeSharedRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		return Template.asString([
			...
			`var loadSharedModule = ${runtimeTemplate.basicFunction("promises, id", [
			...
			initialConsumes.length > 0
				? Template.asString([
						...
						`${RuntimeGlobals.startup} = ${runtimeTemplate.basicFunction("", [
							...
							`initialConsumes.forEach(loadSharedModule.bind(null, promises));`,
							"return promises.length ? Promise.all(promises).then(next) : next();"
						])};`
				  ])
				: "// no consumes in initial chunks",
			this._runtimeRequirements.has(RuntimeGlobals.ensureChunkHandlers) &&
			Object.keys(chunkToModuleMapping).length > 0
				? Template.asString([
						...
						`${
							...
							Template.indent([
								`chunkMapping[chunkId].forEach(loadSharedModule.bind(null, promises));`
							]),
							"}"
						])}`
				  ])
