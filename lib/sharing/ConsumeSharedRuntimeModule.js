			...
			*/
			...
			`var findVersion = ${runtimeTemplate.basicFunction(
				...
				[
					...
					"var lastVersion;",
						// We do not have a version that satisfies the required range. Find a registered versions by relaxing the fixed version components.
						"currentName = key;",
						`var versions = [currentName].concat(requiredVersion.slice(1, fixed).map(${runtimeTemplate.returningFunction(
							'currentName += "`" + v',
							"v"
						)}));`,
						"versions.unshift(key)",
						// Iterate from most restrictive range to least restrictive range
						"while(currentName = versions.pop()) {",
						Template.indent([
							`if (${RuntimeGlobals.hasOwnProperty}(scope, currentName)) { lastVersion = scope[currentName].version; break; }`
						]),
						"}",
						"if (lastVersion === undefined) { lastVersion = ['unavailable']; }"
					// TODO: message isn't very clear
					'var msg = "Unsatisfied version of shared module " + key + "@" + (formatVersion(lastVersion)) + " (required " + key + "@" + formatVersionRange(requiredVersion) + ")";',
					"if(strict) throw new Error(msg);",
					'typeof console !== "undefined" && console.warn && console.warn(msg);'
				]
			)};`,
