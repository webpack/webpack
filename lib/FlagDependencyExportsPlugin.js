		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.finishModules.tapAsync(
				...
				(modules, callback) => {
					...
					// On a full build usage is still undetermined here (FlagDependencyUsage
					// runs later, at optimizeDependencies). Against the persisted graph the
					// previous build's usage survives, so getMode() can answer "unused" and
					// getExports() throws on it. Reset now to match the full-build state.
					if (compilation._incrementalRebuild) {
					...
					asyncLib.each(
						...
						(module, callback) => {
							...
							if (
								...
								if (
									compilation._incrementalRebuild &&
									compilation.builtModules.has(module)
								) {
