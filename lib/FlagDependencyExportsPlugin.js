...
/**
 * Cache key for a module's provided exports. For a lazy barrel it folds in the
 * request keys of the re-export targets still deferred, so a run whose set of
 * built targets differs (e.g. a re-export later imported and un-deferred) gets a
 * distinct key instead of restoring stale exports under the unchanged build hash.
 * @param {Module} module the module
 * @param {HashFunction} hashFunction the hash function
 * @returns {string} the cache etag
 */
const getCacheEtag = (module, hashFunction) => {
	const buildInfo = /** @type {BuildInfo} */ (module.buildInfo);
	const hash = /** @type {string} */ (buildInfo.hash);
	const deferred = buildInfo.lazyDependencies;
	if (deferred === undefined || deferred.size === 0) return hash;
	const etagHash = createHash(hashFunction);
	etagHash.update(hash);
	for (const requestKey of [...deferred].sort()) {
		etagHash.update("|");
		etagHash.update(requestKey);
	}
	return /** @type {string} */ (etagHash.digest("hex"));
};
				...
				(modules, callback) => {
					...
					asyncLib.each(
						...
						(module, callback) => {
							...
							cache.get(
								module.identifier(),
								getCacheEtag(module, hashFunction),
								(err, result) => {
