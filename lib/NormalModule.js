...
class NormalModule extends Module {
	...
	_createLoaderContext(resolver, options, compilation, fs, hooks) {
		...
		const utils = {
			/**
			 * @param {string | URL} context context
		...
		const loaderContext = {
			...
			addBuildDependency: (dep) => {
				...
				buildInfo.buildDependencies.add(maybeFileURLToPath(dep));
			},
			utils,
			...
			const handleBuildDone = () => {
				...
				const checkDependencies = (deps) => {
					for (const dep of deps) {
						// resolve a file URL instance to an absolute path
						const resolved = maybeFileURLToPath(
							/** @type {string | URL} */ (dep)
						);
						if (resolved !== dep) {
							deps.delete(dep);
