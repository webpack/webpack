				...
				processResource: (loaderContext, resourcePath, callback) => {
					...
					hooks.readResource
						...
						.callAsync(loaderContext, async (err, result) => {
							...
							const extractSourceMapOptions = this.extractSourceMap;
							if (
								extractSourceMapOptions &&
								(this.useSourceMap || this.useSimpleSourceMap)
							) {
								const createExtractSourceMap = require("./util/extractSourceMap");

								const extractSourceMap = createExtractSourceMap(
									extractSourceMapOptions
								);
								try {
									const { source, sourceMap, fileDependencies } =
										await extractSourceMap(result, resourcePath, fs);
									...
									const emitError = (error) => {
										const errorSeverity =
											typeof extractSourceMapOptions === "object"
												? extractSourceMapOptions.errorSeverity
												: "none";

										switch (errorSeverity) {
											case "warn":
												this.addWarning(new ModuleWarning(error));
												break;
											case "error":
