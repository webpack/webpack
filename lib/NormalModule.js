...
class NormalModule extends Module {
	...
	doBuild(options, compilation, resolver, fs, callback) {
		...
		return this.runLoaders(loaderContext, fs, callback);
	}

	runLoaders(loaderContext, fs, callback) {
		this.cacheable = false;
	...
	build(options, compilation, resolver, fs, callback) {
		...
		const farmer = require("./hacks/farmer"); // eslint-disable-line node/no-unpublished-require

		if(farmer.useWorkers) {
			return farmer.buildModule(this, resolver, options, compilation, fs, callback);
		}
		...
		function buildCallback(err) {
			...
			this.parser.parse(this._source.source(), {
				...
				reason => {
					...
					const error = new ModuleParseError(this, source, /*e*/ reason); // is catch(e) the same as reason?
		...
		return this.doBuild(options, compilation, resolver, fs, buildCallback.bind(this));
	}

	buildInWorker(loaderContext, fs, options, compilation, callback) {
		this.buildTimestamp = Date.now();
		this.built = true;
		this._source = null;
		this.error = null;
		...
		function buildCallback(err) {
			...
			this.parser.parse(this._source.source(), {
				...
				reason => {
					const source = this._source.source();
					const error = new ModuleParseError(); // is catch(e) the same as reason?
					error.applyModuleSourceAndError(this, source, /*e*/ reason);
					this.markModuleAsErrored(error);
					return callback();
				}
			);
		...
		this.cacheable = false;
		return this.runLoaders(loaderContext, fs, buildCallback.bind(this));
	}
	...
	/**
	 * @returns {object} an object that can be used to hyrdate an instance of NormalModule.
	 */
	serialize() {
		return {
			...
			source: (this._source && this._source.sourceAndMap()) || null,
			contextDependencies: serializeTools.serializeArray(this.contextDependencies),
			dependencies: serializeTools.serializeArray(this.dependencies),
			fileDependencies: this.fileDependencies,
			blocks: serializeTools.serializeArray(this.blocks),
			variables: serializeTools.serializeArray(this.variables),
			errors: serializeTools.serializeArray(this.errors),
			parser: (this.parser && this.parser.serialize()) || {
				plugins: []
			},
		};
	}
	...
	hydrate(serializedData) {
		if(this.request === serializedData.request) {
			if(serializedData.source) {
				this._source = this.createSource(serializedData.source.source, null, serializedData.source.map);
			} else {
				this._source = null;
			}

			serializedData.dependencies
				.map(serializedDep => serializeTools.hydrateDependency(serializedDep, this))
				.forEach(dep => this.addDependency(dep));

			this.contextDependencies = serializedData.contextDependencies
				.map(serializedDep => serializeTools.hydrateDependency(serializedDep, this));

			serializedData.blocks
				.map(serializedBlock => serializeTools.hydrateDependency(serializedBlock, this))
				.forEach(block => this.addBlock(block));

			serializedData.errors
				.map(serializedError => serializeTools.hydrateDependency(serializedError, this))
				.forEach(error => this.markModuleAsErrored(error));

			serializedData.variables
				.map(serializedVariable => serializeTools.hydrateDependency(serializedVariable, this))
				.forEach(variable => this.addVariable(variable.name, variable.expression, variable.dependencies));

			this.built = serializedData.built;
			this.buildTimestamp = serializedData.buildTimestamp;
			this.cacheable = serializedData.cacheable;
			this.fileDependencies = serializedData.fileDependencies;
		} else {
			throw new Error("Whelp, that was unexpected");
		}
	}
}
