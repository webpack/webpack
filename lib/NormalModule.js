...
const InvalidDependenciesModuleWarningModule = require("./errors/InvalidDependenciesModuleWarning");
		...
		const loaderContext = {
			...
			getOptions: (/** @type {EXPECTED_ANY} */ schema = undefined) => {
				...
				if (schema && compilation.options.validate) {
					...
					validate(schema, /** @type {EXPECTED_OBJECT} */ (options), {
						name,
						baseDataPath
					});
				}
				...
				processResource: async (loaderContext, resourcePath, callback) => {
					...
					try {
						...
						if (
							...
							try {
								const { source, sourceMap } = await extractSourceMap(
									result,
									resourcePath,
									/** @type {ReadResource} */
									(resourcePath) =>
			...
			const handleBuildDone = () => {
				...
				if (nonAbsoluteDependencies !== undefined) {
					const InvalidDependenciesModuleWarning =
						InvalidDependenciesModuleWarningModule;
					this.addWarning(
