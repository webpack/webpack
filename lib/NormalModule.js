...
NormalModule.prototype.checkForDuplicatedLoaders = function() {
	var loadersUsage = this.loaders.reduce(function(acc, loaderData) {
		var loaderPath = loaderData.loader;

		acc[loaderPath] = acc[loaderPath] || 0;
		acc[loaderPath] += 1;
		return acc;
	}, {});

	for(var loaderPath in loadersUsage) {
		if(loadersUsage[loaderPath] > 1) {
			this.warnings.push(new ModuleWarning(this, "Loader " + loaderPath +
				" was applied " + loadersUsage[loaderPath] + " times."));
		}
	}
}
...
NormalModule.prototype.build = function build(options, compilation, resolver, fs, callback) {
	...
	_this.checkForDuplicatedLoaders();
