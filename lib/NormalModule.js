...
function hydrate(module, serializedData) {
	if(serializedData.source) {
		module._source = module.createSource(serializedData.source.source, null, serializedData.source.map);
	} else {
		module._soruce = null;
	}
	...
	serializedData.dependencies.forEach(dep => {
		try {
			dep.options.forEach((opt, index) => {
				if(opt === "SELF_MODULE_REFERENCE") {
					dep.options[index] = module;
					...
					dep.options[index] = rehydratedRegExp;
				}
			});
			let ClassDef = require(dep.path);
			const dependency = newCall(ClassDef, dep.options);
			module.addDependency(dependency);
		} catch(e) {
			/**
			 * TODO - handle this better
			 */
			console.log(`Failure to hydrate ${dep.path} with ${dep.options}`);
			console.log(e);
			console.log(require(dep.path) instanceof Function);
		}
	});
}
 ...
 * @param {Module} module - The module to serialize.
 * @returns {array} - An array of serialized dependencies.
 */
function serializeDependencies(module) {
...
class NormalModule extends Module {
	...
	serialize() {
		...
		return {
			...
			dependencies: serializeDependencies(this),
	...
	hydrate(serializedData) {
		if(this.request === serializedData.request) {
			return hydrate(this, serializedData);
		} else {
