...
/**
 * @template T
 * @typedef {import("../types/LoaderContext").NormalModuleLoaderContext<T>} NormalModuleLoaderContext
 */
...
class NormalModule extends Module {
	 ...
	 * @returns {import("../types/LoaderContext").LoaderContext<T>} loader context
	 */
	_createLoaderContext(resolver, options, compilation, fs, hooks) {
		...
		const loaderContext = {
			...
			getResolve(options) {
				...
				return /** @type {ReturnType<import("../types/LoaderContext").NormalModuleLoaderContext<T>["getResolve"]>} */ (
			...
			hooks.processResult.callAsync(
				...
				(hookErr, hookResult) => {
					if (hookErr) {
						...
						return callback(
							new (getModuleBuildError())(
								hookErr instanceof Error
									? hookErr
									: new (getNonErrorEmittedError())(hookErr),
	...
	build(options, compilation, resolver, fs, callback) {
		...
		this.buildInfo = {
			...
			stableExports: undefined,
			classHeritageRanges: undefined,
		...
		for (const type of sourceTypes || chunkGraph.getModuleSourceTypes(this)) {
			...
			const source = this.error
				? generator.generateError
					...
					: new RawSource(getGenerator().throwBuildErrorCode(this.error))
