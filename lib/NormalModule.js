...
class NormalModule extends Module {
	...
	doBuild(options, compilation, resolver, fs, callback) {
		...
		runLoaders(
			...
			(err, result) => {
				...
				this.buildInfo.fileDependencies = new PathSet(result.fileDependencies);
				this.buildInfo.contextDependencies = new PathSet(
					result.contextDependencies
				);
				this.buildInfo.missingDependencies = new PathSet(
	...
	serialize(context) {
		const { write, writePath } = context;
		// constructor
		write(this.type);
		writePath(this.resource);
		// deserialize
		write(this._source);
