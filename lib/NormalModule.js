 ...
 * @param {ModuleGraph} moduleGraph the module graph
 * @param {any} SideEffectDep `HarmonyImportSideEffectDependency` constructor, resolved once at the entry
 ...
 * @property {string | null | undefined | Record<string, any>} options
...
/** @typedef {LoaderContext<any>} AnyLoaderContext */
...
class NormalModule extends Module {
	...
	_createLoaderContext(resolver, options, compilation, fs, hooks) {
		...
		const loaderContext = {
			version: 2,
			/** @type {LoaderContext<any>["getOptions"]} */
			getOptions: (/** @type {any} */ schema = undefined) => {
				...
				if (schema && compilation.options.validate) {
					...
					getValidate()(schema, /** @type {object} */ (options), {
						name,
						baseDataPath
					});
				}
			...
			/** @type {LoaderContext<any>["emitWarning"]} */
			emitWarning: (warning) => {
			...
			/** @type {LoaderContext<any>["emitError"]} */
			emitError: (error) => {
			...
			/** @type {LoaderContext<any>["getLogger"]} */
			getLogger: (name) => {
				const currentLoader = this.getCurrentLoader(
					/** @type {AnyLoaderContext} */
					(loaderContext)
			...
			/** @type {LoaderContext<any>["resolve"]} */
			resolve(context, request, callback) {
				resolver.resolve({}, context, request, getResolveContext(), callback);
			},
			/** @type {LoaderContext<any>["getResolve"]} */
			getResolve(options) {
			...
			/** @type {LoaderContext<any>["emitFile"]} */
	...
	/**
	 * @param {Exclude<NoParse, any[]>} rule rule
	...
	static deserialize(context) {
		...
		const obj = new this({
			...
			extractSourceMap: /** @type {any} */ (null)
		});
