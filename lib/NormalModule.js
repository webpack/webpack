 ...
 * @property {HookMap<AsyncSeriesBailHook<[string], string | Buffer | null>>} readResourceFromURL
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				...
				/**
				 * Unlike readResource hook, this only returns resource content based on URL without side effects
				 * like setting module.buildInfo, addDependency, etc.
				 */
				readResourceFromURL: new HookMap(
					() => new AsyncSeriesBailHook(["resourcePath"])
				),
		...
		runLoaders(
			{
				...
				processResource: async (loaderContext, resourcePath, callback) => {
					...
					/** @type {ReadResource} */
					const readResourceFromURL = (resourcePath) => {
						const scheme = getScheme(resourcePath);
						return new Promise((resolve, reject) => {
							hooks.readResourceFromURL
								.for(scheme)
								.callAsync(resourcePath, (err, result) => {
									if (err) {
										reject(err);
									} else {
										if (typeof result !== "string" && !result) {
					...
					try {
						const result = await readResource(resourcePath);
						if (
