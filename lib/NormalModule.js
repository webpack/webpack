...
class NormalModule extends Module {
	...
	splitVariablesInUniqueNamedChunks(vars) {
		const chunkCollection = vars.reduce((chunkCol, variable) => {
			const nameStore = chunkCol.names;
			// if we dont know this variable name yet, add it to the tempChunk and names list
			if(nameStore.length === 0 || nameStore.indexOf(variable.name) === -1) {
				chunkCol.tempChunk.push(variable);
				chunkCol.names.push(variable.name);
			} else {
				/**
				 * We know this variable name already.
				 * Add current tempChunk to chunks and start a new tempChunk/names list
				 */
				chunkCol.chunks.push(chunkCol.tempChunk);
				chunkCol.tempChunk = [variable];
				chunkCol.names = [variable.name];
			}
			return chunkCol;
		}, {
			chunks: [],
		...
		const finalChunks = chunkCollection.chunks;
		if(chunkCollection.tempChunk.length !== 0) {
			finalChunks.push(chunkCollection.tempChunk);
		}
		return finalChunks;
	}
	...
	sourceBlock(block, availableVars, dependencyTemplates, source, outputOptions, requestShortener) {
		...
		if(vars.length > 0) {
			 ...
			 * [foo, bar, baz, foo, wurst, suppe]
			 * we can not inject "foo" twice, therefore we just make two IIFEs like so:
			 * (function(foo, bar, baz){
			 *   (function(foo, wurst, suppe){
			 *     ...
			 *   }(...));
			 * }(...));
			 *
