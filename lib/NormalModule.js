...
class NormalModule extends Module {
	...
	createLoaderContext(resolver, options, compilation, fs) {
		const addErrorFrom = (error) => {
			if(!error.from) {
				const currentLoaderObject = loaderContext.loaders && loaderContext.loaders.length ?
					loaderContext.loaders[loaderContext.loaderIndex] :
					null;
				if(currentLoaderObject) {
					const loaderName = currentLoaderObject.origin || currentLoaderObject.path;
					error.from = /\//.test(loaderName) ?
		...
		const loaderContext = {
			...
			emitError: (error) => {
				...
				addErrorFrom(error);
				this.errors.push(new ModuleBuildError(this, error));
			},
			exec: (code, filename) => {
