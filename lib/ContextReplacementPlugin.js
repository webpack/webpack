...
class ContextReplacementPlugin {
	...
	apply(compiler) {
		...
		const { newContentCallback } = this;
		const { newContentResource } = this;
		const { newContentRecursive } = this;
		const { newContentRegExp } = this;
		const { newContentCreateContextMap } = this;
