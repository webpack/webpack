...
const { dirname, maybeFileURLToPath, mkdirp } = require("../util/fs");
 ...
 * @property {string | URL=} context Context of requests in the manifest file (defaults to the webpack context).
 ...
 * @property {string | URL} path Absolute path to the manifest json file (output).
				...
				asyncLib.each(
					...
					(chunk, callback) => {
						...
						const targetPath = compilation.getPath(
							maybeFileURLToPath(this.options.path),
							{
								chunk
							}
						);
						...
						for (const module of chunkGraph.getOrderedChunkModulesIterable(
							...
							const ident = module.libIdent({
								context:
									maybeFileURLToPath(this.options.context) || compiler.context,
