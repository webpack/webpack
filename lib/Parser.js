	...
	initializeEvaluating() {
		...
		const getSimplifiedTemplateResult = (kind, templateLiteralExpr) => {
			...
			for (let i = 0; i < templateLiteralExpr.quasis.length; i++) {
				...
				quasis.push(
					// expression is meaningless here
					// as it may correspond to multiple expressions
					new BasicEvaluatedExpression()
						.setString(quasiExpr.value[kind])
				...
				if (i > 0) {
					const prevExpr = quasis[quasis.length - 2],
						lastExpr = quasis[quasis.length - 1];
		...
		this.hooks.evaluate.for("TemplateLiteral").tap("Parser", node => {
			const { quasis, parts } = getSimplifiedTemplateResult.call(
				this,
				"cooked",
				node
			);
			if (quasis.length === 1) {
				return quasis[0].setRange(node.range);
			}
			return new BasicEvaluatedExpression()
				.setTemplateString(quasis, parts)
				.setRange(node.range);
		});
