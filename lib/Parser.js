...
function babylonWrapper(source, opts) {
	return babylon.parse(source, opts).program;
}

const AST_PLUGINS = ["dynamicImport", "estree", "importMeta"];
...
class Parser extends Tapable {
		...
		for(let i = 0, len = POSSIBLE_AST_OPTIONS.length; i < len; i++) {
			if(!ast) {
				try {
					...
					ast = babylonWrapper(source, POSSIBLE_AST_OPTIONS[i]);
				} catch(e) {
					// ignore the error
				}
			}
	...
	evaluate(source) {
		const ast = babylonWrapper("(" + source + ")", {
			ranges: true,
			locations: true,
			ecmaVersion: ECMA_VERSION,
			sourceType: "module",
