	...
	["substr", "substring"].forEach(function(fn) {
		...
		this.plugin("evaluate TemplateLiteral", function(node) {
			...
			for (i = 0; i < quasis.length; i++) {
				exprs.push(new BasicEvaluatedExpression().setString(quasis[i].value.cooked).setRange(quasis[i].range));

				if (i > 0) {
					var prevExpr = exprs[exprs.length - 2], lastExpr = exprs[exprs.length - 1];
					var expr = this.evaluateExpression(node.expressions[i-1]);
		...
		this.plugin("evaluate TaggedTemplateExpression", function(node) {
			...
			for (i = 0; i < quasis.length; i++) {
				exprs.push(new BasicEvaluatedExpression().setString(quasis[i].value.raw).setRange(quasis[i].range));

				if (i > 0) {
					var prevExpr = exprs[exprs.length - 2], lastExpr = exprs[exprs.length - 1];
					var expr = this.evaluateExpression(node.quasi.expressions[i-1]);
					if (!(expr.isString() || expr.isNumber())) continue;
