...
class Parser extends Tapable {
	constructor(options, sourceType = "auto") {
		...
		this.hooks = {
			...
			commentOptions: new SyncBailHook(["comment", "context"])
		};
	...
	getCommentOptions(range) {
		...
		if (comments.length !== 0) {
			const optionsList = comments.map(comment => {
				const context = {};
				if (this.hooks.commentOptions.call(comment, context) !== undefined) {
					return {};
				}
				try {
					...
					if (comment.loc) {
						e.message += ` (${comment.loc.start.line}:${
			...
			options = optionsList.reduce((o, i) => Object.assign(o, i), {});
		}
		if (errors.length > 0 && current) {
			current.warnings.push(
				new ModuleWarning(
					current,
					new Error(
						["Some errors occurred while transforming comments to options:"]
							.concat(errors.map(e => `\n${e.message}`))
							.join("")
					)
				)
