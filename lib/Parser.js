...
const originalPlugin = Parser.prototype.plugin;
Parser.prototype.plugin = function pluginIntoParser(inHook, potentialDef, opts) {
	// console.log('called with', arguments);
	const hooks = inHook instanceof Array ? inHook : [inHook];
	return hooks.map(hook => {
		if(potentialDef.path) {
			this._plugins.push(arguments);
			// This is really ugly, but parser plugins don't have a static api, they might have more or less arguments.
			return originalPlugin.call(this, hook, function(expr, param1, param2, param3, param4) {
				if(typeof param4 !== "undefined") {
					return require(potentialDef.path)[potentialDef.fnName](expr, param1, param2, param3, param4, opts, this);
				}
				if(typeof param3 !== "undefined" || hook === "export specifier") { // extra gross
					return require(potentialDef.path)[potentialDef.fnName](expr, param1, param2, param3, opts, this);
				}
				if(typeof param2 !== "undefined") {
					return require(potentialDef.path)[potentialDef.fnName](expr, param1, param2, opts, this);
				}
				if(typeof param1 !== "undefined") {
					return require(potentialDef.path)[potentialDef.fnName](expr, param1, opts, this);
				}
				return require(potentialDef.path)[potentialDef.fnName](expr, opts, this);
			});
		} else {
			/**
			 * Simple catch all for anything using Tapable's .plugin api outside of this file.
			 */
			// try {
			// 	throw new Error("This is here for sanity checking parser.plugin api changes.");
			// } catch(e) {
			// 	const firstLine = e.stack.split("\n")[3];
			// 	if(!firstLine.includes(__filename)) {
			// 		console.log(e.stack);
			// 	}
			// }
		}
...
Parser.prototype.serialize = function serialize() {
	return {
		plugins: this._plugins,
	};
};

Parser.prototype.hydrate = function(serializedData) {
	this._plugins = serializedData.plugins;
};
