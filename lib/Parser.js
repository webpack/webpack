...
Parser.prototype.walkVariableDeclarators = function walkVariableDeclarators(declarators) {
	declarators.forEach(function(declarator) {
		...
		case "VariableDeclarator":
			...
			if(initToIdentifier && declarator.id.type === "Identifier" && !this.applyPluginsBailResult("rename " + declarator.init.name, declarator)) {
...
Parser.prototype.walkExpression = function walkExpression(expression) {
	...
	case "AssignmentExpression":
		if(expression.left.type === "Identifier" && expression.right.type === "Identifier" && !this.applyPluginsBailResult("rename " + expression.right.name, expression.right)) {
			...
			if(!this.applyPluginsBailResult("assign " + (this.scope.renames["$"+expression.left.name] || expression.left.name, expression), expression)) {
				this.walkExpression(expression.left);
			}
			if(!this.applyPluginsBailResult("assigned " + (this.scope.renames["$"+expression.left.name] || expression.left.name, expression))) {
				this.walkExpression(expression.right);
			}
		} else {
			this.walkExpression(expression.left);
