...
class URLParserPlugin {
	 ...
	 * @param {((request: string) => ResolvedResourceHints)=} resolveResourceHints request → defaults resolver from `output.resourceHints`
	 */
	constructor(options, resolveResourceHints) {
		/** @type {JavascriptParserOptions} */
		this.options = options;
		/** @type {(request: string) => ResolvedResourceHints} */
		this._resolveResourceHints =
			resolveResourceHints || (() => /** @type {ResolvedResourceHints} */ ({}));
	}
	...
	apply(parser) {
		...
		parser.hooks.new.for("URL").tap(PLUGIN_NAME, (_expr) => {
			...
			if ((request = evaluatedExpr.asString())) {
				...
				// `output.resourceHints` provides project-wide defaults
				// (resolved per-request, with `test` / `include` / `exclude`
				// matching); a per-URL magic comment for the same hint wins
				// when both are set.
				const defaults = this._resolveResourceHints(request);
				dep.prefetch = hints.prefetch || defaults.prefetch || undefined;
				dep.preload = hints.preload || defaults.preload || undefined;
				dep.fetchPriority =
					hints.fetchPriority ||
					(defaults.fetchPriority !== false
						? defaults.fetchPriority
						: undefined);
				dep.asAttribute = hints.as;
				dep.typeAttribute = hints.type;
				dep.mediaAttribute = hints.media;
