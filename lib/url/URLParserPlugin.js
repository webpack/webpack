...
/**
 * Like {@link keepNewURL}, but warns that the target won't be bundled.
 * @param {JavascriptParser} parser parser
 * @param {NewExpressionNode} expr expression
 * @param {string} reason why the target won't be bundled
 * @returns {void}
 */
const keepNewURLWithWarning = (parser, expr, reason) => {
	parser.state.module.addWarning(
		new UnsupportedFeatureWarning(
			`${reason} — keeping \`new URL(...)\` for runtime evaluation. Add /* webpackIgnore: true */ to silence this warning.`,
...
class URLParserPlugin {
	...
	apply(parser) {
		...
		const evalOnly = this.options.importMetaUrl === "eval-only";
		...
		parser.hooks.new.for("URL").tap(PLUGIN_NAME, (_expr) => {
			...
			if ((request = evaluatedExpr.asString())) {
				...
				if (isDirectoryRequest(request)) {
					if (evalOnly) {
						keepNewURLWithWarning(
							parser,
							expr,
							`new URL('${request}', import.meta.url): '${request}' is a directory, not an asset`
						);
					} else {
						keepNewURL(parser, expr);
					}
				...
				if (evalOnly && getScheme(request)) {
					keepNewURLWithWarning(
						parser,
						expr,
						`new URL('${request}', import.meta.url): '${request}' is not a local asset`
					);
					return true;
				}
				...
				dep.loc = /** @type {DependencyLocation} */ (expr.loc);
				// eval-only: a request that fails to resolve warns and stays a runtime URL
				dep.optional = evalOnly;
			...
			if (this.options.dynamicUrl === false) {
				if (evalOnly) {
					keepNewURLWithWarning(
						parser,
						expr,
						"new URL(..., import.meta.url): the request can't be statically analyzed"
					);
					return true;
				}
				return;
