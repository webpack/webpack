	...
	apply(parser) {
		...
		parser.hooks.new.for("URL").tap(PLUGIN_NAME, (_expr) => {
			...
			if ((request = evaluatedExpr.asString())) {
				...
				dep.loc = parser.getLocation(expr);
				// `parser.javascript.urlHints` gives the project-wide defaults,
				// resolved per request; a per-URL magic comment for the same hint
				...
				// Fixed URLDependenciesBlock shape: promote() sets entryOptions
				// later in finishModules once resolve knows the target type.
				const block = new URLDependenciesBlock({
					name: entryOptions && entryOptions.name
				});
				block.loc = parser.getLocation(expr);
				block.addDependency(dep);
				parser.state.module.addBlock(block);
				// The ESM template embeds the asset's resolved filename, so the asset
				// must be code-generated first. Added in that mode only: elsewhere the
				// extra code-gen dependency can deadlock a watch rebuild.
