...
/** @typedef {import("../AsyncDependenciesBlock").BlockPromotion} BlockPromotion */
 ...
 * @param {AsyncDependenciesBlock} block the URL async block
 * @param {ModuleGraph} moduleGraph module graph
 * @param {URLEntryPluginHooks} hooks compilation hooks
 * @returns {void}
 */
const reconcileUrlBlock = (block, moduleGraph, hooks) => {
	if (!block.groupOptions || !block.groupOptions.url) return;
			...
			compilation.hooks.finishModules.tap(PLUGIN_NAME, (modules) => {
				...
				for (const module of modules) {
					...
					for (const block of module.blocks) {
						if (!(block instanceof AsyncDependenciesBlock)) continue;
