 ...
 * @property {string} requestKey grouping key (target resource) of the request
...
class LazyBarrelInfo {
	...
	addLazy(requestKey, dependency, factory, context) {
		...
		if (group === undefined) {
			group = { factory, dependencies: [], requestKey, context };
...
class LazyBarrelController {
	...
	/**
	 * Removes the just-built (un-lazied) dependencies from the barrel's recorded
	 * deferred set on `buildInfo`. `FlagDependencyExportsPlugin` folds that set
	 * into its provided-exports cache key, so a barrel whose built targets differ
	 * from a cached run does not reuse that run's stale exports.
	 * @param {Module} module the barrel module
	 * @param {DependencyGroup[]} groups the groups un-lazied this call
	 */
	_unlazyDeferred(module, groups) {
		const buildInfo = module.buildInfo;
		const deferred = buildInfo && buildInfo.lazyDependencies;
		if (!deferred) return;
		for (const group of groups) deferred.delete(group.requestKey);
	}
	...
	classify(module) {
		...
		// request keys of the targets kept deferred; folded into the provided-exports
		// cache key so a differently-deferred cached run is not reused
		...
		for (const dep of dependencies) {
			...
			if (dep.isLazy()) {
				...
				if (deferred === undefined) deferred = new Set();
				deferred.add(requestKey);
			}
		...
		if (module.buildInfo !== undefined) {
			module.buildInfo.lazyDependencies = deferred;
		}
		if (state !== undefined) {
			...
			const groups = info.request(state.forwardedIds);
			this._unlazyDeferred(module, groups);
	...
	request(module, dependencies) {
		...
		// targets just built: shrink the recorded deferred set so the cache key reflects it
		this._unlazyDeferred(module, groups);
