...
/**
 * Whether a dependency answers the lazy barrel protocol. A foreign type may
 * carry one of these names as a plain flag rather than a method, so presence
 * alone does not say the walk may call it.
 * @param {Dependency} dep the dependency to ask
 * @returns {boolean} true when every method the walk calls is callable
 */
const answersLazyBarrel = (dep) =>
	typeof dep.getLazyUntil === "function" &&
	typeof dep.isLazy === "function" &&
	typeof dep.getLazyName === "function" &&
	typeof dep.setLazy === "function";
...
class LazyBarrelController {
	...
	classify(module) {
		...
		for (const dep of dependencies) {
			...
			if (!answersLazyBarrel(dep)) continue;
...
module.exports.answersLazyBarrel = answersLazyBarrel;
