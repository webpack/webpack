...
class PresetProcessor {
	 ...
	 * @returns {WebpackOptions} webpackConfig object with presets applied
	 */
	recursiveMerge(webpackConfig) {
		if (!webpackConfig.presets) return webpackConfig;
		if (webpackConfig.presets.length === 0) return webpackConfig;
		return webpackMerge.merge([
			...webpackConfig.presets.map(preset => this.recursiveMerge(preset)),
			webpackConfig
		]);
	}
	...
	process(options) {
		...
		return this.recursiveMerge(options);
	}
}
