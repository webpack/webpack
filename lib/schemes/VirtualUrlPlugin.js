...
const { isAbsolute, join } = require("../fs/fs");
 ...
 * @jsonType boolean
 * @inline
 * @typedef {true} VirtualModuleVersion
 */

/**
 * A virtual module definition.
 * @typedef {object} VirtualModule
 * @property {string=} context The context for the virtual module. A string path. Defaults to 'auto', which will try to resolve the context from the module id.
 * @property {SourceFn} source The source function that provides the virtual content.
 * @property {string=} type The module type.
 * @property {VirtualModuleVersion | string | VersionFn=} version Optional version function or value for cache invalidation.
 */

/**
 * A virtual module can be a string, a function, or a VirtualModule object.
 * @typedef {string | SourceFn | VirtualModule} VirtualModuleContent
 */

/**
 * Options for building virtual resources.
 * @typedef {object} VirtualUrlOptions
 * @property {string=} context The default context for virtual modules. A string path. Defaults to 'auto', which will try to resolve the context from the module id.
 * @property {{ [key: string]: VirtualModuleContent }} modules The virtual modules configuration.
 * @property {string=} scheme The URL scheme to use for virtual resources.
 */

/**
 * @schema plugins/schemes/VirtualUrlPlugin
 * @typedef {VirtualUrlOptions} VirtualUrlPluginOptions
 */

/**
 * @inline
 * @tsType import('./VirtualUrlPlugin').SourceFn
 * @typedef {(loaderContext: LoaderContext<EXPECTED_ANY>) => Promise<string | Buffer> | string | Buffer} SourceFn
 */
/**
 * @inline
 * @tsType import('./VirtualUrlPlugin').VersionFn
 * @typedef {() => string} VersionFn
 */
/** @typedef {{ [key: string]: VirtualModuleContent }} VirtualModules */
 ...
 * @param {VirtualModuleContent} virtualConfig The virtual module to normalize
...
/** @typedef {{ [key: string]: VirtualModule }} NormalizedModules */
...
class VirtualUrlPlugin {
	 ...
	 * @param {string} id The module id
	 * @returns {VirtualModule} The virtual module config
	 */
	findVirtualModuleConfigById(id) {
		const config = this.modules[id];
		if (!config) {
			throw new (getModuleNotFoundError())(
				null,
