...
const { dirname, join, mkdirp } = require("../fs/fs");
...
/** @import { ResourceDataWithData } from "../module/NormalModuleFactory" */
 ...
 * Allowed URI pattern.
 * @inline
 * @typedef {RegExp} HttpUriOptionsAllowedUrisItemBranch1
 */

/**
 * Allowed URI (resp. the beginning of it).
 * @inline
 * @typedef {HttpUrl} HttpUriOptionsAllowedUrisItemBranch2
 */

/**
 * Allowed URI filter function.
 * @inline
 * @typedef {import("./HttpUriPlugin").AllowedUriFn} HttpUriOptionsAllowedUrisItemBranch3
 */

/**
 * List of allowed URIs (resp. the beginning of them).
 * @inline
 * @typedef {HttpUriOptionsAllowedUrisItemBranch1 | HttpUriOptionsAllowedUrisItemBranch2 | HttpUriOptionsAllowedUrisItemBranch3} HttpUriOptionsAllowedUrisItem
 */

/**
 * Options for building http resources.
 * @typedef {object} HttpUriOptions
 * @property {HttpUriOptionsAllowedUris} allowedUris
 * @property {false | AbsolutePath=} cacheLocation Location where resource content is stored for lockfile entries. It's also possible to disable storing by passing false.
 * @property {boolean=} frozen When set, anything that would lead to a modification of the lockfile or any resource content, will result in an error.
 * @property {AbsolutePath=} lockfileLocation Location of the lockfile.
 * @property {string=} proxy Proxy configuration, which can be used to specify a proxy server to use for HTTP requests.
 * @property {boolean=} upgrade When set, resources of existing lockfile entries will be fetched and entries will be upgraded when resource content has changed.
 */

/**
 * List of allowed URIs (resp. the beginning of them).
 * @typedef {HttpUriOptionsAllowedUrisItem[]} HttpUriOptionsAllowedUris
 */

/**
 * @schema plugins/schemes/HttpUriPlugin
 * @typedef {HttpUriOptions} HttpUriPluginOptions
 */
...
/**
 * Parses key value pairs. Keys are lower-cased because HTTP directive names are
 * case-insensitive; values are kept verbatim (they may be quoted field lists).
 ...
 * @property {InstanceType<Snapshot>} snapshot snapshot
 */
...
/**
 * @inline
 * @tsType import('./HttpUriPlugin').AllowedUriFn
							...
							intermediateFs.readFile(lockfileLocation, (err, buffer) => {
								...
								compilation.fileSystemInfo.createSnapshot(
									...
									(err, s) => {
										...
										lockfileCache = {
											lockfile,
											snapshot: /** @type {InstanceType<Snapshot>} */ (s)
										};
				...
				for (const { scheme, fetch } of schemes) {
					...
					const fetchContentRaw = (url, cachedResult, callback) => {
						...
						fetch(new URL(url), { headers }, (res) => {
							...
							// content codings are case-insensitive tokens
							...
							if (stream !== res) {
								// a decompressor error never reaches the request's handler
