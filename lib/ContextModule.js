 ...
 * @property {string[]=} globPatterns glob patterns for import.meta.glob
 * @property {string=} globContext importer context for import.meta.glob
 * @property {string=} globImport glob import selection
 * @property {boolean=} globExhaustive glob exhaustive mode
 */
...
class ContextModule extends Module {
	...
	_createIdentifier() {
		...
		} else if (this.options.globPatterns) {
			identifier += `|glob: ${this.options.globPatterns.join(", ")}`;
		}
		...
		if (this.options.globImport) {
			identifier += `|globImport: ${this.options.globImport}`;
		}
		if (this.options.globExhaustive) {
			identifier += "|globExhaustive";
		}
...
module.exports = webpackEmptyAsyncContext;`;
	...
	/**
	 * @returns {string | undefined} glob import property access
	 */
	getGlobImportPropertyAccess() {
		const globImport = this.options.globImport;
		if (!globImport || globImport === "*") return;
		return propertyAccess([globImport]);
	}
	...
	getGlobLazySource(
		...
		const moduleGraph = chunkGraph.moduleGraph;
		const globImportAccess = this.getGlobImportPropertyAccess();
		/** @type {RuntimeRequirements} */
	...
	getGlobSyncSource(dependencies, chunkGraph, runtimeTemplate) {
		...
		const entries = [];
		let index = 0;
		for (const [userRequest, moduleId] of Object.entries(map).sort(
			([a], [b]) => (a < b ? -1 : a > b ? 1 : 0)
		)) {
			let moduleExpr;
			if (moduleId !== undefined) {
				...
				moduleExpr = `undefined /* ${JSON.stringify(userRequest)} */`;
			}
			this.appendGlobMapEntry(index++, userRequest, moduleExpr, entries);
		}
