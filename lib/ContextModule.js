...
function webpackAsyncContextResolve(req) {
	...
	return ${RuntimeGlobals.promise}.resolve().then(${
			arrow ? "() =>" : "function()"
		} {
		if(!${RuntimeGlobals.hasOwnProperty}(map, req)) {
			var e = new Error("Cannot find module '" + req + "'");
...
module.exports = webpackAsyncContext;`;
	...
	getLazySource(blocks, id, { chunkGraph, runtimeTemplate }) {
		...
		const requestPrefix = hasNoChunk
			? `${RuntimeGlobals.promise}.resolve()`
			: hasMultipleOrNoChunks
			? `${RuntimeGlobals.promise}.all(ids.slice(${chunksStartPosition}).map(${RuntimeGlobals.ensureChunk}))`
		...
		const webpackAsyncContext =
			requestPrefix === `${RuntimeGlobals.promise}.resolve()`
				? `
function webpackAsyncContext(req) {
	return ${RuntimeGlobals.promise}.resolve().then(${
						arrow ? "() =>" : "function()"
...
module.exports = webpackEmptyAsyncContext;`;
	...
	codeGeneration(context) {
		...
		if (allDeps.length > 0) {
			...
			set.add(RuntimeGlobals.promise);
			if (asyncMode === "weak") {
