...
class ContextModule extends Module {
	...
	getReturnModuleObjectSource(
		...
		if (asyncDeps) {
			...
			return `${asyncDeps} ? ${asyncDeps}.length ? ${RuntimeGlobals.deferredModuleAsyncTransitiveDependencies}(${asyncDeps}).then(${RuntimeGlobals.makeDeferredNamespaceObject}.bind(${RuntimeGlobals.require}, id, ${type} ^ 1 | 16)) : ${RuntimeGlobals.makeDeferredNamespaceObject}(id, ${type} ^ 1 | 16) : ${source}`;
		}
		return source;
	}
...
module.exports = webpackEmptyAsyncContext;`;
	...
	codeGeneration(context) {
		...
		sources.set(
			JAVASCRIPT_TYPE,
			this.getSource(
				this.getSourceString(this.options.mode, this.options.phase, context),
				compilation
			)
		);
		...
		if (allDeps.length > 0) {
			...
			if (ImportPhaseUtils.isDefer(this.options.phase)) {
