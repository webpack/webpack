 ...
 * @property {string=} issuer module that requested the context, only kept when a rule matches on `issuer`
...
class ContextModule extends Module {
	...
	_createIdentifier() {
		...
		if (this.options.issuer) {
			identifier += `|issuer: ${this.options.issuer}`;
		}
		return identifier;
	}
	 ...
	 * @returns {NameForCondition | null} absolute path which should be used as issuer for condition matching
	 */
	nameForIssuerCondition() {
		return this.options.issuer || super.nameForIssuerCondition();
	}
	...
	readableIdentifier(requestShortener) {
		...
		if (this.options.issuer) {
			identifier += ` issuer: ${requestShortener.shorten(this.options.issuer)}`;
		}
