...
class ContextModule extends Module {
	constructor(resolveDependencies, options) {
		...
		/* resource, recursive, regExp, addon, asyncMode, chunkName, include, exclude */
	...
	build(options, compilation, resolver, fs, callback) {
		...
		this.resolveDependencies(fs, this.options, (err, dependencies) => {
			...
			if(!this.options.async || this.options.async === "eager") {

				// if we have an sync or eager context
				// just add all dependencies and continue
				this.dependencies = dependencies;

			} else if(this.options.async === "lazy-once") {
			...
			} else if(this.options.async === "weak" || this.options.async === "async-weak") {
...
webpackEmptyAsyncContext.id = ${JSON.stringify(id)};`;
	...
	source(dependencyTemplates, outputOptions, requestShortener) {
		return this.getSource(
			this.getSourceString(this.options.async, outputOptions, requestShortener)
		);
	}
