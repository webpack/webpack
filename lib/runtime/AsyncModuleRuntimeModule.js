		...
		return Template.asString([
						...
						`${RuntimeGlobals.deferredModuleAsyncTransitiveDependencies} = ${runtimeTemplate.basicFunction(
							...
							[
								Template.indent([
									...
									Template.indent([
										`return ({ then(onFulfilled, onRejected) { return ${RuntimeGlobals.promise}.all(asyncDeps.map(__webpack_require__)).then(onFulfilled, onRejected) } })`
									]),
									"}"
								])
							]
			...
			`${fn} = ${runtimeTemplate.basicFunction(
				...
				[
					...
					`${lt} outerResolve;`,
					`${lt} reject;`,
					`${cst} promise = new ${RuntimeGlobals.promise}(${runtimeTemplate.basicFunction(
						"resolve, rej",
