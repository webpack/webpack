	...
	generate() {
		...
		return Template.asString([
			...
			`var completeQueue = ${runtimeTemplate.basicFunction("queue", [
				"if(queue) {",
				Template.indent([
					`queue.forEach(${runtimeTemplate.basicFunction("fn", "fn.r--;")});`,
					`queue.forEach(${runtimeTemplate.basicFunction(
						"fn",
						"fn.r-- ? fn.r++ : fn();"
					)});`
				]),
			...
			`var completeFunction = ${runtimeTemplate.basicFunction(
				"fn",
				"!--fn.r && fn()"
			)};`,
			`var queueFunction = ${runtimeTemplate.basicFunction(
				"queue, fn",
				...
				`deps.map(${runtimeTemplate.basicFunction("dep", [
					...
					Template.indent([
						...
						Template.indent([
							...
							`var obj = { [webpackThen]: ${runtimeTemplate.basicFunction(
								"fn, reject",
					...
					"}",
					`return { [webpackThen]: ${runtimeTemplate.basicFunction(
						"fn",
						"completeFunction(fn);"
					)}, [webpackExports]: dep };`
				])})`,
