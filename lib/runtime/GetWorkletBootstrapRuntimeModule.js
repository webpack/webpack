 ...
 * script. A worklet global scope has no `self`, no `location` and no
 * `importScripts`; this script sets them up before the emitted chunks — which
 * assume a worker-like scope — are added via `addModule`. Deduping by base URI
 * avoids leaking a new object URL per worklet.
 */
		...
		const getter = runtimeTemplate.basicFunction("", [
			...
			Template.indent([
				...
				Template.indent([
					...
					// auto public-path feature-detects a worker-like scope via `importScripts`;
					// worklets lack it, so define one (throwing, as it must never be called).
					'"globalThis.importScripts = () => { throw new Error(\\"importScripts is not available in worklets\\"); };"'
				]),
				'].join("\\n");',
