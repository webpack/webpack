...
class DefinePropertyGettersRuntimeModule extends HelperRuntimeModule {
	...
	generate() {
		...
		// Only webpack emits the array form, so a build whose emitters all stayed
		// silent cannot receive one; the object form stays either way.
		const withArray = this.runtimeRequirements.has(
			RuntimeGlobals.definePropertyGettersFromArray
		);
		// TODO in the next major release: remove object format support. Internal code (e.g. ConcatenatedModule)
