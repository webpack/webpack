		...
		return Template.asString([
			...
			`${fn} = ${runtimeTemplate.basicFunction("url, done, key", [
						...
						Template.asString([
							"return " +
								runtimeTemplate.basicFunction(
									"event",
									Template.asString([
										`createOnScriptComplete = ${runtimeTemplate.basicFunction("", "")}`,
				...
				`var timeout = setTimeout(${runtimeTemplate.basicFunction(
					"",
					"script.onload && script.onload({ type: 'timeout', target: script })"
				)}, ${loadTimeout});`,
				"script.onerror = script.onload = createOnScriptComplete(script.onload);",
