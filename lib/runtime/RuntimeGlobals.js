...
/**
 * Drives an async module body written as a generator function (used when the
 * target environment supports generators but not `async`/`await`).
 * It runs the generator, resolving each `yield`ed dependency promise, so the
 * body reads like a plain `async` function without emitting `async`/`await`.
 * Signature: function(body: GeneratorFunction) => (handleDependencies, result) => void
 */
module.exports.asyncModuleGenerator = "__webpack_require__.aG";
...
/**
 * wrap a concatenated module body as a lazy accessor, returning its exports
 */
module.exports.concatenationWrap = "__webpack_require__.cw";

/**
 * apply the `new` return-value rule to a concatenated module's exports
 */
module.exports.constructRequire = "__webpack_require__.cr";
...
/**
 * the object url of the worklet bootstrap script (sets up `self`, `location` and
 * a throwing `importScripts` before worklet chunks are added)
 */
module.exports.getWorkletBootstrap = "__webpack_require__.wb";
...
// TODO in the next major release: rename to `esmModuleDecorator`
/**
...
/**
 * a flag when a server render may already have put the stylesheets in the
 * document, so the css runtime has to wait for one that has not applied yet
 */
module.exports.hasServerRenderedStylesheets = "has server rendered stylesheets";
...
/**
 * the asset prefetch function: injects a
 * `<link rel="prefetch">` for an asset module URL
 */
...
/**
 * the asset preload function: injects a
 * `<link rel="preload">` for an asset module URL
 */
...
/**
 * marker requirement that adds the startup runtime module which fires
 * `<link rel="prefetch|preload">` for `new URL(..., import.meta.url)`
 * assets with `webpackPrefetch` / `webpackPreload` hints, before any
 * chunk module evaluates
 */
module.exports.startupAssetHints = "__webpack_require__.SAH";
