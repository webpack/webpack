...
const {
	getPresentKinds,
	usesFullHashDigest
} = require("../template/TemplatedPathPlugin");
const StringXor = require("../util/StringXor");
const lazyModule = require("../util/lazyModule");
const RuntimeGlobals = require("./RuntimeGlobals");

/** @import { ChunkLoading, LibraryOptions } from "../../declarations/WebpackOptions.d.ts" */
/** @import Chunk from "../graph/Chunk" */
/** @import { RuntimeRequirements } from "../module/Module" */
/** @import Compiler from "../Compiler" */
/** @import RuntimeModule from "./RuntimeModule" */
/** @import { ScriptLoader } from "./LoadScriptRuntimeModule" */
...
const getMakeNamespaceObjectRuntimeModule = lazyModule(() =>
	require("./MakeNamespaceObjectRuntimeModule")
);
const getNonceRuntimeModule = lazyModule(() => require("./NonceRuntimeModule"));
const getOnChunksLoadedRuntimeModule = lazyModule(() =>
	require("./OnChunksLoadedRuntimeModule")
);
const getPublicPathRuntimeModule = lazyModule(() =>
	require("./PublicPathRuntimeModule")
);
const getRelativeUrlRuntimeModule = lazyModule(() =>
	require("./RelativeUrlRuntimeModule")
);
const getRuntimeIdRuntimeModule = lazyModule(() =>
	require("./RuntimeIdRuntimeModule")
);
const getSetAnonymousDefaultNameRuntimeModule = lazyModule(() =>
	require("./SetAnonymousDefaultNameRuntimeModule")
);
const getSystemContextRuntimeModule = lazyModule(() =>
	require("./SystemContextRuntimeModule")
);
const getToBinaryRuntimeModule = lazyModule(() =>
	require("./ToBinaryRuntimeModule")
);
const getWorkerRuntimeModule = lazyModule(() =>
	require("./WorkerRuntimeModule")
);
...
const GLOBALS_ON_REQUIRE = [
	...
	// TODO in the next major release: rename to nonce, because we use it for CSS too
...
class RuntimePlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			/**
			 * Resolves the chunk loading of the chunk's entry, falling back to the global one.
			 * @param {Chunk} chunk chunk
			 * @returns {ChunkLoading} chunk loading of the chunk
			 */
			const getChunkLoadingForChunk = (chunk) => {
				const options = chunk.getEntryOptions();
				return options && options.chunkLoading !== undefined
			...
			const isChunkLoadingDisabledForChunk = (chunk) =>
				getChunkLoadingForChunk(chunk) === false;
			...
			compilation.hooks.runtimeRequirementInTree
				...
				.tap(PLUGIN_NAME, (chunk, set) => {
					...
					const entryOptions = chunk.getEntryOptions();
					const importScripts =
						getChunkLoadingForChunk(chunk) === "import-scripts";
					// a worker has no `document`; one importing its chunks as modules
					// can't call `importScripts` either, so it gets a clear error
					/** @type {ScriptLoader} */
					const loader = importScripts
						? "import-scripts"
						: entryOptions && entryOptions.worker
							? "unavailable"
							: "script";
					compilation.addLazyRuntimeModule(
						chunk,
						getLoadScriptRuntimeModule,
						(Ctor) => new Ctor(withCreateScriptUrl, withFetchPriority, loader)
					);
					return true;
				});
			compilation.hooks.runtimeRequirementInTree
			...
			// TODO in the next major release: remove CompatRuntimeModule
