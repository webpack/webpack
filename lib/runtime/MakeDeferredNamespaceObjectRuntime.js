	...
	generate() {
		...
		return `${fn} = ${runtimeTemplate.basicFunction(
			...
			[
				...
				// sites must yield the same object, so it is cached per moduleId.

				// Bit 16 marks a Promise-like return, which a deferred namespace never
				// is, so stripping it lets static defer and `await import.defer` share
				// one object. The key stays `(moduleId, mode)` to separate the rest.
				"mode &= ~16;",
