			...
			[
				...
				Template.indent([
					...
					`${runtimeTemplate.method("get", "_, name", [
						...
						"if (exportNames) return exportNames.indexOf(name) < 0 ? undefined : ns[name];",
						"return ns[name];"
					])},`,
					`${runtimeTemplate.method("has", "_, name", [
						...
						init,
						"if (exportNames) return exportNames.indexOf(name) >= 0;",
						"return name in ns;"
					])},`,
					...
					`${runtimeTemplate.method("getOwnPropertyDescriptor", "_, name", [
						...
						"if (exportNames) return exportNames.indexOf(name) < 0 ? undefined : { value: ns[name], writable: true, enumerable: true, configurable: false };",
 ...
 * Names are never mangled here: materializing a namespace marks the exports
 * used in an unknown way, which clears `canMangleUse`.
 * @param {ModuleGraph} moduleGraph the module graph
 * @param {Module} module the deferred module
 * @returns {string[] | null} sorted export names, or null when unknown
 */
const getKnownExportNames = (moduleGraph, module) => {
