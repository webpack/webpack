		...
		return `${fn} = ${runtimeTemplate.basicFunction("moduleId, mode", [
			...
			`var init = ${runtimeTemplate.basicFunction("", [
				...
				Template.indent([
					...
					// Mirror the synthetic exports from the fake namespace (created
					// by createFakeNamespaceObject) onto the proxy target so that
					// proxy invariants hold for callers that structurally
					// introspect via `Object.keys` / `Object.getOwnPropertyNames`:
					// when our `getOwnPropertyDescriptor` trap reports
					// `configurable: false` for a key, the target must actually
					// have that key with a matching descriptor. `Symbol.toStringTag`
					// is skipped because the trap synthesizes "Deferred Module"
					// independently of what the underlying namespace exposes.
					"var keys = Reflect.ownKeys(ns);",
					"for (var i = 0; i < keys.length; i++) {",
					Template.indent([
						"var k = keys[i];",
						"if (k === Symbol.toStringTag) continue;",
			...
			"",
			"var ns_target = __webpack_module_deferred_exports__[moduleId] || (__webpack_module_deferred_exports__[moduleId] = { __proto__: null });",
			"var ns = ns_target;",
			"var handler = {",
			Template.indent([
				"__proto__: null,",
