...
/**
 * Converts a module exports type into the bitmask understood by webpack's
 * deferred namespace-object helpers.
...
/**
 * Returns the runtime call that creates an optimized deferred namespace
...
/**
 * Runtime helper that produces lightweight deferred namespace wrappers for
 * modules whose exports shape is already known.
 */
class MakeOptimizedDeferredNamespaceObjectRuntimeModule extends HelperRuntimeModule {
	/**
	 * Creates the optimized deferred-namespace runtime helper.
	...
	/**
	 * Generates the optimized helper that defers module evaluation until an
	 * export is accessed while avoiding the full Proxy-based implementation.
...
/**
 * Runtime helper that creates Proxy-based deferred namespace objects for
 * modules that should not be evaluated until their exports are accessed.
 */
class MakeDeferredNamespaceObjectRuntimeModule extends HelperRuntimeModule {
	/**
	 * Creates the general deferred-namespace runtime helper.
	 * @param {boolean} hasAsyncRuntime if async module is used.
	 */
	constructor(hasAsyncRuntime) {
	...
	/**
	 * Generates the Proxy-based deferred namespace implementation used when
	 * webpack cannot rely on the optimized path.
	 * @returns {string | null} runtime code
	 */
	generate() {
		if (!this.compilation) return null;
