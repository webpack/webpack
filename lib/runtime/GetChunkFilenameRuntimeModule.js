...
/**
 * Runtime module that resolves chunk ids to the filenames webpack should load
 * for them at runtime.
 */
class GetChunkFilenameRuntimeModule extends RuntimeModule {
	/**
	 * Creates the runtime module that computes filenames for referenced chunks
	...
	/**
	 * Generates the runtime function that maps chunk ids to either statically
	 * known filenames or a templated fallback expression.
	...
	generate() {
		...
		/**
		 * Registers a chunk and its filename template in the local lookup tables.
		...
		/**
		 * Pre-computes a concrete URL for a chunk whose filename can be resolved
		 * without deferring to the dynamic template path.
		 * @param {Chunk} c the chunk
		...
		const addStaticUrl = (c, chunkFilename) => {
			/**
			 * Serializes a chunk-related value for interpolation into a quoted
			 * template string without surrounding quotes.
			...
			/**
			 * Creates a truncated variant of `unquotedStringify` for hash values
		...
		/**
		 * Builds a compact runtime expression that maps dynamic chunk ids to the
		...
		/**
		 * Wraps a chunk-id map expression so it can be embedded directly inside a
		...
		/**
		 * Produces a truncated variant of `mapExpr` for hashes that support
		 * length-specific interpolation.
		 * @param {(chunk: Chunk) => string | number} fn function from chunk to value
