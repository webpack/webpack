...
class GetChunkFilenameRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		// `[fullhash:<digest>]`/`[hash:<digest>]` resolve to a runtime `getFullHash()`
		// expression here, which cannot be re-encoded — fail loudly instead of 404ing.
		/** @type {HashWithDigestFunction} */
		const fullHashWithDigest = (digest) => {
			throw new Error(
				`An inline hash digest on [fullhash]/[hash] (here [fullhash:${digest}]) is not supported in a chunk filename for dynamically-loaded chunks: the runtime builds it from ${RuntimeGlobals.getFullHash}() and cannot re-encode that expression. Use [chunkhash:${digest}] or [contenthash:${digest}], or remove the inline digest.`
			);
		};
