...
class SpecNamespaceObjectRuntimeModule extends HelperRuntimeModule {
	...
	generate() {
		...
		return Template.asString([
			`${fn} = ${runtimeTemplate.basicFunction("exports, record, mode", [
				// One namespace per module: a module's own exports object identifies it,
				// while a synthesized one is fresh per site, so its record identifies it.
				`${cst} owner = record || exports;`,
				`${lt} slots = ${fn}.c.get(owner);`,
				`if (slots === undefined) ${fn}.c.set(owner, (slots = { __proto__: null }));`,
				// A synthesized namespace carries different names per mode, so one
				// module can owe its importers more than one namespace.
				`${cst} cached = slots[mode];`,
				"if (cached) return cached;",
				// The exports object keeps the live bindings; the sealed target
				// carries only their names, so the traps stay invariant-safe.
				`${cst} names = Object.getOwnPropertyNames(exports).filter(${runtimeTemplate.returningFunction(
					"name !== '__esModule'",
					"name"
				)}).sort();`,
				...
				`${cst} own = ${runtimeTemplate.returningFunction(
			...
			// The same owner must always yield the same namespace.
			`${fn}.c = new WeakMap();`
		]);
	}
}
