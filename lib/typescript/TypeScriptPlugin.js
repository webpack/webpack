...
/** @import NormalModuleType, { Result } from "../module/NormalModule" */
 ...
 * `TypeScript ...` errors as `getModuleBuildError()` so they surface as
...
const stripTypes = (input) => {
	try {
		// Only `mode`: `sourceUrl` would emit a `//# sourceURL=` pragma, and
		// `sourceMap: true` is rejected in strip mode, so the map is built by hand.
		// eslint-disable-next-line n/no-unsupported-features/node-builtins
		return mod.stripTypeScriptTypes(input, { mode: "strip" });
	} catch (err) {
		throw new (getModuleBuildError())(/** @type {Error} */ (err));
	}
};
...
class TypeScriptPlugin {
	...
	_processResult(result, module) {
		...
		if (TSX_RESOURCE_RE.test(resource)) {
			throw new (getModuleBuildError())(new Error(TSX_NOT_SUPPORTED));
		}
