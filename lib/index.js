...
// The two halves are separate modules so that reading a color does not
// parse the options schema; `webpack.cli` is the surface webpack-cli reads.
const getCli = memoize(() => {
	const cliArguments = require("./config/cli");
	const terminalColors = require("./util/terminalColors");

	return { ...cliArguments, ...terminalColors };
});
