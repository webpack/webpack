...
const cli = require("../util/terminalColors");
...
/** @import { Colors } from "../util/terminalColors" */
...
/**
 * @inline
 * @typedef {object} ProgressPluginOptionsProgressBarBranch3Object
 * @property {string=} color Color used for the filled portion of the bar.
 * @property {string=} name Name shown before the progress bar.
 * @property {PositiveNumber=} width Width of the progress bar in characters. Default: 25.
 */

/**
 * Function that executes for every progress step.
 * @typedef {import("./ProgressPlugin").HandlerFn} HandlerFunction
 */

/**
 * Options object for the ProgressPlugin.
 * @typedef {object} ProgressPluginOptions
 * @property {boolean=} activeModules Show active modules count and one active module in progress message.
 * @property {boolean=} dependencies Show dependencies count in progress message.
 * @property {number=} dependenciesCount Minimum dependencies count to start with. For better progress calculation. Default: 10000.
 * @property {boolean=} entries Show entries count in progress message.
 * @property {boolean=} estimatedTime Show estimated time remaining based on build progress. Default: false.
 * @property {HandlerFunction=} handler
 * @property {boolean=} modules Show modules count in progress message.
 * @property {number=} modulesCount Minimum modules count to start with. For better progress calculation. Default: 5000.
 * @property {"entries" | "modules" | "dependencies" | null=} percentBy Collect percent algorithm. By default it calculates by a median from modules, entries and dependencies percent.
 * @property {boolean=} phaseTimings Show a timing breakdown for each build phase when the build completes. Default: false.
 * @property {true | false | null=} profile Collect profile data for progress steps. Default: false.
 * @property {"auto" | boolean | ProgressPluginOptionsProgressBarBranch3Object=} progressBar Generate progress bar. `"auto"` enables it only for interactive terminals. Default: false.
 */

/**
 * @schema plugins/ProgressPlugin
 * @typedef {ProgressPluginOptions | HandlerFunction} ProgressPluginArgument
 */
...
/**
 * @inline
 * @tsType import('./ProgressPlugin').HandlerFn
...
class ProgressPlugin {
	...
	apply(compiler) {
		...
		// the next major release (opt-in today via `experiments.futureDefaults`) turns an unset
	...
	_applyOnCompiler(compiler, handler) {
		compiler.hooks.validate.tap(PLUGIN_NAME, () => {
			compiler.validate(
				...
				(options) =>
					require("../../schemas/plugins/ProgressPlugin.check")(options)
			);
		});
