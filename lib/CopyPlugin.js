 ...
 * @returns {Promise<string[]>} entries of the directory, empty when it does not exist
 */
...
/**
 * @param {string} directory an absolute normalized directory
 * @returns {string} it without a trailing separator, unless it names a root
 */
const trimTrailingSeparator = (directory) =>
	directory.endsWith(path.sep) &&
	directory.length > path.parse(directory).root.length
		? directory.slice(0, -1)
		: directory;
 ...
 * @param {Set<string>} visited real paths of the directories already walked, guarding against symlink cycles
...
const walkDirectory = async (fs, directory, visited, maxDepth) => {
	const realPath = await realpath(fs, directory);
	if (visited.has(realPath)) return [];
	visited.add(realPath);
	const entries = (await readDirectory(fs, directory)).sort();
	const results = await mapLimit(
		entries,
		DIRECTORY_CONCURRENCY,
		async (name) => {
			const entry = join(fs, directory, name);
			const stats = await stat(fs, entry);
			// the entry is gone between the readdir and the stat
			if (stats === undefined) return [];
			if (stats.isDirectory()) {
				return maxDepth > 0
					? walkDirectory(fs, entry, visited, maxDepth - 1)
					: [];
			}
...
class CopyPlugin {
	...
	async _copyPattern(compiler, compilation, logger, cache, item, index) {
		...
		const base = trimTrailingSeparator(
			path.normalize(unescapeGlobPath(escapedBase))
		);
		...
		const entries =
			typeOfFrom === "file"
				? [absoluteFrom]
				: (await walkDirectory(fs, base, new Set(), maxDepth)).filter(
						(filePath) =>
							globMatchWithOptions(
								glob,
