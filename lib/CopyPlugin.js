 ...
 */
const walkDirectory = async (fs, directory, visited) => {
	...
	const results = await mapLimit(
		...
		async (name) => {
			...
			if (stats.isDirectory()) {
				return walkDirectory(fs, entry, visited);
			}
...
class CopyPlugin {
	...
	async _copyPattern(compiler, compilation, logger, cache, item, index) {
		...
		const base = unescapeGlobPath(extractGlobBaseDir(glob));
		logger.debug(`'${from}' is a ${typeOfFrom}, globbing '${glob}'`);

		const entries =
			typeOfFrom === "file"
				? [absoluteFrom]
				: (await walkDirectory(fs, base, new Set())).filter((filePath) =>
	 ...
	 * @param {string} base literal base directory of the glob
	...
	async _addPatternDependency(
		...
		if (checkExistence && !(await stat(fs, directory))) {
			compilation.missingDependencies.add(directory);
			return;
		}
