 ...
 * @property {boolean} followSymlinks whether a symlink is walked and copied at all
...
const walkDirectory = async (options, directory, visited, maxDepth) => {
	const { fs, followSymlinks, ignore } = options;
...
class CopyPlugin {
	...
	async _resolveFrom(
		...
		compilation,
		logger,
		{ from, context, globOptions, matchOptions, ignore }
	) {
		...
		const entries =
			typeOfFrom === "file"
				? ignore.file(normalizePathSeparatorsForPath(absoluteFrom))
					...
					: [absoluteFrom]
				: (
						await walkBase({ fs, followSymlinks, ignore }, base, maxDepth)
					).filter((filePath) => {
	...
	async _copyFile(
		...
		const assetFilename = pattern.to
			? path.posix.join(
