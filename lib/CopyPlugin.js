 ...
 * @param {boolean} followSymlinks whether a symlink is walked and copied at all
...
const walkDirectory = async (
	...
	const results = await mapLimit(
		...
		async (entry) => {
			...
			return walkDirectory(fs, filePath, visited, maxDepth - 1, followSymlinks);
		}
	);
	/** @type {string[]} */
	const files = [];
 ...
 * @returns {Promise<string[]>} absolute paths of the files below it, in a stable order
 */
const walkBase = async (fs, base, maxDepth, followSymlinks) =>
	walkDirectory(
		fs,
		base,
		// the base counts as walked, so a symlink pointing back at it ends there
		new Set(followSymlinks ? [await realpath(fs, base)] : []),
		maxDepth,
		followSymlinks
	);
...
class CopyPlugin {
	...
	async _copyPattern(compiler, compilation, logger, cache, item, index) {
		...
		const { from } = pattern;
		const followSymlinks = pattern.followSymlinks !== false;
		...
		const matchOptions = { caseSensitive: pattern.caseSensitive };
		const entries =
			typeOfFrom === "file"
				? [absoluteFrom]
				: (await walkBase(fs, base, maxDepth, followSymlinks)).filter(
						(filePath) =>
							globMatchWithOptions(
								glob,
