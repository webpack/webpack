...
module.exports = class RuleSet {
	...
	static normalizeRule(rule, refs, ident) {
		...
		if (typeof loader === "string" && !rule.options && !rule.query) {
			...
			newRule.use = RuleSet.normalizeUse(loader.split("!"), refs, ident);
			...
			newRule.use = RuleSet.normalizeUse(loader, refs, ident);
		...
		if (rule.use) {
			checkUseSource("use");
			newRule.use = RuleSet.normalizeUse(rule.use, refs, ident);
		}
	...
	static normalizeUse(use, refs, ident) {
		if (typeof use === "function") {
			return data => RuleSet.normalizeUse(use(data), refs, ident);
		}
		if (Array.isArray(use)) {
			return use
				.map((item, idx) => RuleSet.normalizeUse(item, refs, `${ident}-${idx}`))
	...
	static normalizeUseItem(item, refs, ident) {
		...
		if (newItem.ident) {
			refs[newItem.ident] = newItem.options;
		}
