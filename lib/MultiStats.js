...
/** @typedef {{ version: boolean, hash: boolean, errorsCount: boolean, warningsCount: boolean, errors: boolean, warnings: boolean, hints: boolean, hintsCount: boolean, children: NormalizedStatsOptions[] }} ChildOptions */
...
class MultiStats {
	...
	_createChildOptions(options, context) {
		...
		return {
			...
			hints: children.every((o) => o.hints),
			hintsCount: children.every((o) => o.hintsCount),
			children
		};
	}
	...
	toJson(options) {
		...
		if (childOptions.hints) {
			obj.hints = [];
			for (const j of obj.children) {
				const hints =
					/** @type {NonNullable<KnownStatsCompilation["hints"]>} */
					(j.hints);
				for (const i of hints) {
					obj.hints.push(mapError(j, i));
				}
			}
		}
		...
		if (childOptions.hintsCount) {
			obj.hintsCount = 0;
			for (const j of obj.children) {
				obj.hintsCount += /** @type {number} */ (j.hintsCount);
			}
		}
		return obj;
	}
