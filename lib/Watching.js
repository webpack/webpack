...
/**
 * Returns whether a path contains glob characters.
 * @param {string} pattern path or glob pattern
 * @returns {boolean} true when the value looks like a glob pattern
 */
const isGlob = (pattern) => GLOB_CHARS_RE.test(pattern);

/**
 * Returns the longest leading path of a glob pattern that contains no glob
 * characters. Used as the "context" directory that the watcher must monitor
 * so that newly-added or removed entries matching the glob trigger a rebuild.
 * @param {string} pattern glob pattern
 * @returns {string | undefined} base directory or undefined if there is none
 */
const getGlobBase = (pattern) => {
	...
	const joined = baseSegments.join(path.sep);
...
class Watching {
	...
	watch(files, dirs, missing) {
		...
		const { additional } = this.watchOptions;
		if (additional) {
			const additionalPaths =
				typeof additional === "string" ? [additional] : additional;
			if (additionalPaths.length > 0) {
				const inputFileSystem =
					/** @type {InputFileSystem} */
					(this.compiler.inputFileSystem);
				const augmentedFiles = new Set(files);
				const augmentedDirs = new Set(dirs);
				const augmentedMissing = new Set(missing);
				asyncLib.each(
					additionalPaths,
					(additionalPath, callback) => {
						if (isGlob(additionalPath)) {
							// eslint-disable-next-line n/no-unsupported-features/node-builtins
							if (typeof fs.glob !== "function") {
								return callback(
									new Error(
										`watchOptions.additional glob patterns require Node.js >= 22 (got ${process.version})`
									)
								);
							}
							const globBase = getGlobBase(additionalPath);
							if (globBase) augmentedDirs.add(globBase);
							// eslint-disable-next-line n/no-unsupported-features/node-builtins
							fs.glob(
								additionalPath,
								{ withFileTypes: true },
								(err, matches) => {
									...
									for (const match of /** @type {fs.Dirent[]} */ (matches)) {
										const fullPath = path.join(match.parentPath, match.name);
										if (match.isDirectory()) {
											augmentedDirs.add(fullPath);
										} else {
											augmentedFiles.add(fullPath);
										}
									}
									callback();
								}
						...
						inputFileSystem.stat(additionalPath, (err, stats) => {
							if (err || !stats) {
								augmentedMissing.add(additionalPath);
							} else if (stats.isDirectory()) {
								augmentedDirs.add(additionalPath);
							} else {
								augmentedFiles.add(additionalPath);
							}
							callback();
						});
					},
		...
		this._startWatching(files, dirs, missing);
	}
	 ...
	 * @returns {void}
	 */
	_startWatching(files, dirs, missing) {
		this.watcher =
