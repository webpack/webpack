...
"use strict";
/** @import { ObjectDeserializerContext, ObjectSerializerContext } from "./ObjectMiddleware" */

/** @typedef {ObjectSerializerContext<EXPECTED_ANY>["write"]} CacheAssoc */

/**
 * Defines the shared type used by this module.
 * @type {WeakMap<CacheAssoc, ObjectStructure<PropertyKey>>}
 */
...
class ObjectStructure {
	constructor() {
		/** @type {undefined | K[]} */
		this.keys = undefined;
		/** @type {undefined | Map<K, ObjectStructure<K>>} */
	 ...
	 * @param {K} key key
	 * @returns {ObjectStructure<K>} object structure
	 */
	key(key) {
		if (this.children === undefined) this.children = new Map();
		const child = this.children.get(key);
		if (child !== undefined) return child;
		/** @type {ObjectStructure<K>} */
...
const getCachedKeys = (keys, cacheAssoc) => {
	let root = /** @type {ObjectStructure<K> | undefined} */ (
		cache.get(cacheAssoc)
	);
	if (root === undefined) {
...
class PlainObjectSerializer {
	 ...
	 * @param {ObjectSerializerContext<((keyof T)[] | keyof T | null | T[keyof T])[]>} context context
	 */
	 ...
	 * @template {object} T
	 * @param {ObjectDeserializerContext<((keyof T)[] | keyof T | null | T[keyof T])[]>} context context
