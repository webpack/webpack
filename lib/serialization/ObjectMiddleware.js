...
class ObjectMiddleware extends SerializerMiddleware {
	...
	/**
	 * Gets serializer for.
	 * @param {unknown} object for serialization
					...
					if (typeof item === "object" && item !== null) {
						if (item.constructor) {
							...
							if (item.constructor === Map) {
								return `Map { ${/** @type {Map<unknown, unknown>} */ (item).size} items }`;
							}
							if (item.constructor === Array) {
								return `Array { ${/** @type {unknown[]} */ (item).length} items }`;
							}
							if (item.constructor === Set) {
								return `Set { ${/** @type {Set<unknown>} */ (item).size} items }`;
							}
