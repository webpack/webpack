...
/** @import { LazyOptions } from "./SerializerMiddleware" */
/**
 * @import {
...
// In a sandboxed environment such as jest, this escapes the sandbox to register
// the real Object and Array too, which reach us through a structured clone.
...
class ObjectMiddleware extends SerializerMiddleware {
	...
	/**
	 * Accepts a historical request/name as an extra deserialization key for an
	 * already registered constructor, so caches written before a move restore.
	 * @param {Constructor} Constructor the constructor, already registered
	 * @param {string} request the historical request
	 * @param {(string | null)=} name the historical name
	 * @returns {void}
	 */
	static registerLegacyRequest(Constructor, request, name = null) {
		const config = serializers.get(Constructor);

		if (config === undefined || config === NOT_SERIALIZABLE) {
			throw new Error(
				`ObjectMiddleware.registerLegacyRequest: serializer for ${Constructor.name} is not registered`
			);
		}
		...
		if (serializerInversed.has(key)) {
			throw new Error(
				`ObjectMiddleware.registerLegacyRequest: serializer for ${key} is already registered`
			);
		}

		serializerInversed.set(
			key,
			/** @type {SerializerConfigWithSerializer} */ (config).serializer
		);
	}
	...
	serialize(data, context) {
		...
		try {
			...
			// Drop these references, which would otherwise leak: v8 optimizes the
			// generated code for this `ctx.write`, so an inline cache on
			// `Dependency.prototype.serialize` keeps pointing at it.
			data =
				result =
				referenceable =
				bufferDedupeMap =
	...
	deserialize(data, context) {
		...
		try {
			...
			// Drop these references, which would otherwise leak: v8 optimizes the
			// generated code for this `ctx.read`, so an inline cache on
			// `Dependency.prototype.deserialize` keeps pointing at it.
			result =
				referenceable =
				data =
				objectTypeLookup =
...
ObjectMiddleware.NOT_SERIALIZABLE = NOT_SERIALIZABLE;
