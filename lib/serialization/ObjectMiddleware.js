...
/** @typedef {object | string} ReferenceableItem */

/**
 * First tuple element; element type for an open array; `any` once exhausted.
 * @template {readonly any[]} T
 * @typedef {T extends readonly [infer H, ...any[]] ? H : T extends readonly [] ? any : T extends readonly (infer E)[] ? E : never} Head
 */

/**
 * Tuple without its first element; unchanged when `T` is an open array.
 * @template {readonly any[]} T
 * @typedef {T extends readonly [any, ...infer R] ? R : T} Tail
 */
 ...
 * @template {readonly any[]} [T=readonly any[]]
 ...
 * @property {((item: any | (() => any)) => void)=} writeLazy
 * @property {((item: (any | (() => any)), obj: LazyOptions | undefined) => import("./SerializerMiddleware").LazyFunction<any, any, any, LazyOptions>)=} writeSeparate
 */
 ...
 * @property {(value: any, context: ObjectSerializerContext<any>) => void} serialize
 * @property {(context: ObjectDeserializerContext<any>) => any} deserialize
 */
...
/** @typedef {(context: ObjectSerializerContext<any> | ObjectDeserializerContext<any>) => void} ExtendContext */
...
class ObjectMiddleware extends SerializerMiddleware {
	...
	/**
	 * Gets serializer for.
	 * @param {any} object for serialization
	...
	serialize(data, context) {
					...
					if (typeof item === "object" && item !== null) {
						if (item.constructor) {
							...
							if (item.constructor === Map) {
								return `Map { ${/** @type {Map<any, any>} */ (item).size} items }`;
							}
							if (item.constructor === Array) {
								return `Array { ${/** @type {any[]} */ (item).length} items }`;
							}
							if (item.constructor === Set) {
								return `Set { ${/** @type {Set<any>} */ (item).size} items }`;
							}
		...
		/** @type {ObjectSerializerContext<any>} */
		let ctx = {
			write(value) {
				try {
	...
	deserialize(data, context) {
		...
		let result = [];
		/** @type {ObjectDeserializerContext<any>} */
		let ctx = {
			read() {
				return /** @type {any} */ (decodeValue());
			},
