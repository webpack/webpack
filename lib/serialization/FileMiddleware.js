...
class FileMiddleware extends SerializerMiddleware {
	...
	serialize(data, context) {
		...
		const indexFiles = new Set([filename, filename + ".old"]);
		return new Promise((resolve, reject) => {
			mkdirp(this.fs, dirname(this.fs, filename), err => {
				...
				const deleteStaleFiles = async () => {
					// We grab all the files in the current cache location
					const cacheFiles = await new Promise((resolve, reject) => {
						this.fs.readdir(cacheLocation, (error, data) => {
							if (error) return reject(error);
							resolve(data);
						});
					});

					// We mark for deletion, all files in the cache folder that were not
					// generated for this cache, and are also not the index and index.old
					// files.
					const filesToDelete = new Set();
					cacheFiles.forEach(fileName => {
						const normalizedFiles = cacheLocation + "/" + fileName;
						if (
							!allWrittenFiles.has(normalizedFiles) &&
							!indexFiles.has(normalizedFiles)
						) {
							filesToDelete.add(fileName);
						}
					});

					// We iterate over the files mark for deletion and delete them
					await Promise.all(
						Array.from(
							filesToDelete,
							file =>
				...
				resolve(
					serialize(this, data, false, writeFile).then(
						async ({ backgroundJob }) => {
							...
							// If specified, we purge previous cache files
							if (purgeStaleCache) {
								await deleteStaleFiles();
							}
