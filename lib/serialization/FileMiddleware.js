...
const serialize = async (middleware, data, name, writeFile) => {
	...
	const resolvedData = (
		...
		buf.writeBigUInt64LE(BigInt(item.size), 0);
...
const deserialize = async (middleware, name, readFile) => {
	...
	const readSlice = (l, clone) => {
		...
		contentPosition += l;
		return clone ? Buffer.from(result) : result;
	};
	...
	const result = lengths.map(length => {
		if (length < 0) {
			...
			const clone = readSlice(-length, true);
			const size = Number(clone.readBigUInt64LE(0));
			const nameBuffer = clone.slice(8);
