...
/**
 * Note directory should have only files
 * @param {IntermediateFileSystem} fs a file system
 * @param {string} directory directory
 * @param {Set<string>} files file names
 * @param {number} maxAge max age since last modify when file is valid
 * @param {(err?: Error) => void} callback callback
 * @returns {void}
 */
const deleteRestFiles = (fs, directory, files, maxAge, callback) => {
	...
	fs.readdir(directory, (err, allFiles) => {
		...
		for (const file of allFiles) {
			// without options file is string
			const fileString = /** @type {string} */ (file);
			if (!files.has(fileString)) {
				const path = join(fs, directory, fileString);
				batchedJobs.push(
					new Promise((resolve, reject) => {
						fs.stat(path, (err, stats) => {
							...
							if (mtime.valueOf() + maxAge < Date.now()) {
								fs.unlink(path, err => {
									if (err) reject(err);
									else resolve();
								});
...
class FileMiddleware extends SerializerMiddleware {
	...
	serialize(data, context) {
		const { cacheLocation, filename, extension = "", maxAge, logger } = context;
		const files = new Set();
		return new Promise((resolve, reject) => {
			mkdirp(this.fs, dirname(this.fs, filename), err => {
				...
				const writeFile = async (name, content) => {
					const filenameInDir = name
						? `${name}${extension}`
						: filename.slice(cacheLocation.length + 1);
					const filePath = name
						? join(this.fs, filename, `../${filenameInDir}`)
						: filename;
					if (files.has(filenameInDir)) {
						throw new Error(
							`[FileMiddleware] writing same file ${JSON.stringify(name)}`
						);
					} else {
						files.add(filenameInDir);
					}
					await new Promise((resolve, reject) => {
						let stream = this.fs.createWriteStream(filePath + "_");
						let compression;
					...
					if (name) allWrittenFiles.add(filePath);
				};
			...
			logger.time("delete rest files");
			if (files.size > 0) {
				return new Promise(resolve => {
					deleteRestFiles(this.fs, cacheLocation, files, maxAge, err => {
						logger.timeEnd("delete rest files");
						if (err)
							logger.warn(`Error while deleting other packs for ${filename}`);
						resolve(data);
					});
				});
			}
