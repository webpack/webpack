...
const I32_SIZE = 4;
const F64_SIZE = 8;
const NUMBERS_SIZES_BY_TYPE = [I8_SIZE, I32_SIZE, F64_SIZE];
const NUMBERS_HEADERS_BY_TYPE = [I8_HEADER, I32_HEADER, F64_HEADER];

/**
 * @param {number} type type
 * @param {Buffer} buffer buffer
 * @returns {(num: number, offset: number) => number} write method
 */
function getWriteNumberMethod(type, buffer) {
	switch (type) {
		case 0:
			return buffer.writeInt8;
		case 1:
			return buffer.writeInt32LE;
		default:
			return buffer.writeDoubleLE;
	}
}
 ...
 * @param {number} type type
 * @param {Buffer} buffer buffer
 * @returns {(offset: number) => number} read method
 */
function getReadNumberMethod(type, buffer) {
	switch (type) {
		case 0:
...
class BinaryMiddleware extends SerializerMiddleware {
			...
			for (let i = 0; i < data.length; i++) {
				...
				switch (typeof thing) {
					...
					case "number": {
						...
						const size = NUMBERS_SIZES_BY_TYPE[type];
						const writeNumber = getWriteNumberMethod(type, currentBuffer);

						allocate(HEADER_SIZE + size * n);
						writeU8(NUMBERS_HEADERS_BY_TYPE[type] | (n - 1));
	...
	_deserialize(data, context) {
		...
		const readNumbers = (type, header) => {
			const size = NUMBERS_SIZES_BY_TYPE[type];
			const len = (header & NUMBERS_COUNT_MASK) + 1;
			const needBytes = size * len;
			const readNum = getReadNumberMethod(
				type,
				/** @type {Buffer} */ (currentBuffer)
			);

			if (isInCurrentBuffer(needBytes)) {
				for (let i = 0; i < len; i++) {
					result.push(readNum.call(currentBuffer, currentPosition));
					currentPosition += size;
				}
				checkOverflow();
			} else {
				const buf = read(needBytes);
				for (let i = 0; i < len; i++) {
					result.push(readNum.call(buf, i * size));
				}
			}
		};
