...
class ChunkGroup {
	...
	constructor(options) {
		...
		/** @type {WeakMap<ChunkGroup, Set<string>>} */
		this._childrenConnectionType = new WeakMap();
		/** @type {SortableSet<ChunkGroup>} */
	 ...
	 * @param {Set<string>=} connectionType connection type. mutable set e.g. Set(["prefetchOrder"])
	 * @returns {boolean} returns true if chunk group was added
	 */
	addChild(group, connectionType) {
		const size = this._children.size;
		this._children.add(group);
		if (connectionType) {
			// if there is no connectionType
			// always assume chunks are connected by type N
			this._childrenConnectionType.set(group, connectionType);
		}
		return size !== this._children.size;
	}

	/**
	 * @param {ChunkGroup} child chunk group
	 * @returns {Set<string>|undefined} returns set of connection types, if undefined is returned there is no info about connection
	 */
	getConnectionType(child) {
		return this._childrenConnectionType.get(child);
	}
	...
	removeChild(group) {
		...
		this._childrenConnectionType.delete(group);
	...
	remove() {
		...
		for (const parentChunkGroup of this._parents) {
			...
			for (const chunkGroup of this._children) {
				...
				parentChunkGroup.addChild(
					chunkGroup,
					this._childrenConnectionType.get(chunkGroup)
				);
			}
		}
	...
	getChildrenByOrders(moduleGraph, chunkGraph) {
		...
		for (const childGroup of this._children) {
			const connectionType = this._childrenConnectionType.get(childGroup);
			for (const key of Object.keys(childGroup.options)) {
				if (connectionType && !connectionType.has(key)) continue;
				if (key.endsWith("Order")) {
