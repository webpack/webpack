...
/**
 * Condition for `setUsedConditionally`, hoisted so no closure is made per call.
 * @param {UsageStateType} used usage state
 * @returns {boolean} true, when the export is unused
 */
const isUnused = (used) => used === UsageState.Unused;

/**
 * Condition for `setUsedConditionally`, hoisted so no closure is made per call.
 * @param {UsageStateType} used usage state
 * @returns {boolean} true, when the usage is less than unknown
 */
const isBelowUnknown = (used) => used < UsageState.Unknown;

/**
 * Sets the usage of one runtime in a per-runtime usage map.
 * @param {UsedInRuntime} usedInRuntime usage per runtime
 * @param {string} runtime the runtime
 * @param {UsageStateType} newValue new usage state
 * @param {((condition: UsageStateType) => boolean) | undefined} condition only set when it holds for the old value
 * @returns {boolean} true, when the usage changed
 */
const setUsedInRuntime = (usedInRuntime, runtime, newValue, condition) => {
	/** @type {UsageStateType | undefined} */
	let oldValue = usedInRuntime.get(runtime);
	if (oldValue === undefined) oldValue = UsageState.Unused;
	if (newValue === oldValue) return false;
	if (condition !== undefined && !condition(oldValue)) return false;
...
/**
 * Creates a per-runtime usage map holding one usage state for every runtime.
 * @param {Exclude<RuntimeSpec, undefined>} runtime the runtime
 * @param {RuntimeUsageStateType} value usage state
 * @returns {UsedInRuntime} usage per runtime
 */
const createUsedInRuntime = (runtime, value) => {
	/** @type {UsedInRuntime} */
	const usedInRuntime = new Map();
	if (typeof runtime === "string") {
		usedInRuntime.set(runtime, value);
	} else {
		for (const name of runtime) usedInRuntime.set(name, value);
	}
	return usedInRuntime;
};
...
// Restores caches written before this moved into lib/graph/
...
class ExportsInfo {
	 ...
	 * @param {ExportInfoName} name export name
	 * @returns {InstanceType<ExportInfo>} export info for this name
	 */
...
class ExportInfo {
	...
	setUsedInUnknownWay(runtime) {
		let changed = false;
		if (
			this.setUsedConditionally(isBelowUnknown, UsageState.Unknown, runtime)
		) {
			changed = true;
		}
	...
	setUsedConditionally(condition, newValue, runtime) {
		if (runtime === undefined) {
			...
			if (newValue !== UsageState.Unused && condition(UsageState.Unused)) {
				this._usedInRuntime = createUsedInRuntime(runtime, newValue);
				return true;
			}
		} else {
			const usedInRuntime = this._usedInRuntime;
			let changed = false;
			if (typeof runtime === "string") {
				changed = setUsedInRuntime(usedInRuntime, runtime, newValue, condition);
			} else {
				for (const name of runtime) {
					if (setUsedInRuntime(usedInRuntime, name, newValue, condition)) {
						changed = true;
					}
				}
			}
	...
	setUsed(newValue, runtime) {
		if (runtime === undefined) {
			...
			let changed = false;
			if (typeof runtime === "string") {
				changed = setUsedInRuntime(usedInRuntime, runtime, newValue, undefined);
			} else {
				for (const name of runtime) {
					if (setUsedInRuntime(usedInRuntime, name, newValue, undefined)) {
						changed = true;
					}
				}
			}
		...
		let target = {
			...
			deferred: Boolean(
				...
				ImportPhaseUtils.isDefer(
					/** @type {ESMImportDependency} */ (rawTarget.connection.dependency)
						.phase
				)
			)
		};
...
ExportsInfo.ExportInfo = ExportInfo;
ExportsInfo.RestoreProvidedData = RestoreProvidedData;
ExportsInfo.UsageState = UsageState;
