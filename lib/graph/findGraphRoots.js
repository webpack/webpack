...
module.exports = (items, getDependencies) => {
				...
				if (topOfStack.openEdges.length > 0) {
					...
					switch (depSCC.marker) {
						...
						case IN_PROGRESS_MARKER: {
							// A back-edge to an SCC still on the stack merges every SCC
							// above it — in `A -> B -> C -> D` with `D -> B`, reaching `B`
							// from `D` puts `B`, `C` and `D` into one SCC.
							/** @type {Set<SCC<T>>} */
					...
					stack.pop();
					// An SCC is DONE only once the popped node is its last one on
					// the stack. In `A -> B -> C -> D` with `D -> B`, popping `B` onto a
					// stack topped by `A` — a different SCC — finalizes B's.
					if (
						stack.length &&
	...
	for (const scc of rootSCCs) {
		if (scc.nodes.size === 1) {
			rootNodes.add(/** @type {Node<T>} */ (first(scc.nodes)).item);
			continue;
		}
		let max = 0;
		/** @type {Nodes<T>} */
