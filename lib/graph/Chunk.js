...
const { first, intersect } = require("../util/SetHelpers");
...
/** @typedef {import("../template/TemplatedPathPlugin").TemplatePathFn<PathDataChunk>} ChunkFilenameTemplateFn */
...
class Chunk {
	...
	/**
	 * Gets the chunks shared by every group of this chunk.
	 * @private
	 * @returns {Chunks} a new set of the chunks in all groups
	 */
	_getChunksInAllGroups() {
		if (this._groups.size === 1) {
			return new Set(/** @type {ChunkGroup} */ (first(this._groups)).chunks);
		}
		return intersect(Array.from(this._groups, (g) => new Set(g.chunks)));
	}
	...
	getAllAsyncChunks() {
		...
		const initialChunks = this._getChunksInAllGroups();
