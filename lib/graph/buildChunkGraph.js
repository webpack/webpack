...
const URLDependenciesBlock = require("../url/URLDependenciesBlock");
const memoize = require("../util/memoize");

const { getEntryRuntime, mergeRuntime } = require("../util/runtime");
const ModuleGraphConnection = require("./ModuleGraphConnection");

const getAsyncDependencyToInitialChunkError = memoize(() =>
	require("../errors/AsyncDependencyToInitialChunkError")
);
...
/** @import Entrypoint, { EntryOptions } from "./Entrypoint" */
...
const visitModules = (
	...
	const iteratorBlock = (b) => {
		...
		const entryOptions =
			b instanceof URLDependenciesBlock
				? b.promotion && b.promotion.entryOptions
				: b.groupOptions && b.groupOptions.entryOptions;
		if (cgi === undefined) {
			const chunkName =
				(b.groupOptions && b.groupOptions.name) ||
				(entryOptions && entryOptions.name) ||
				b.chunkName;
			if (entryOptions) {
				...
				if (!cgi) {
					...
					// Fill in only the options still unset: blocks deduping to one
					// entrypoint may carry different values, so the first to create it
					// wins.

					// `name` is excluded, being the entrypoint's identity and fixed at
					// creation. Back-filling it would make codegen order-dependent, which
					// breaks persistent caching.
					...
					// Backfill may set standalone after creation; keep preventSplitting in sync.
					if (existingOptions.standalone) {
				...
				!chunkGroupInfo.asyncChunks ||
				!chunkGroupInfo.chunkLoading ||
				// URL blocks without entryOptions stay in the parent chunk
				// (ordinary assets); promote() elevates CSS/etc. to async entries.
				b instanceof URLDependenciesBlock
			) {
				...
				if (!cgi) {
					...
					if (c.isInitial()) {
						compilation.errors.push(
							new (getAsyncDependencyToInitialChunkError())(
...
const connectChunkGroups = (
	...
	for (const [block, connections] of blockConnections) {
		// Skip the whole set when no dependency needs connecting. They cannot be
		// filtered one by one without inconsistent connections, since a module can
		// only carry one version.
