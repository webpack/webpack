...
const findGraphRoots = require("./findGraphRoots");
...
/** @type {(a: RuntimeModule, b: RuntimeModule) => -1 | 0 | 1} */
const compareRuntimeModulesByStage = concatComparators(
	compareSelect(
		(module) => /** @type {RuntimeModule} */ (module).stage,
		compareIds
	),
	compareModulesByIdentifier
);
...
class ChunkGraph {
	...
	getChunkModuleIdMap(chunk, filterFn, includeAllChunks = false) {
		...
		const comparator = compareModulesById(this);
	...
	getChunkRuntimeModulesInOrder(chunk) {
		...
		array.sort(compareRuntimeModulesByStage);
		return array;
	}
	...
	_getModuleGraphHashWithConnections(cgm, module, runtime) {
		...
		return cgm.graphHashesWithConnections.provide(runtime, () => {
			...
			if (runtime === undefined || typeof runtime === "string") {
				...
				// cspell:word Tnamespace
				// the runtime is sorted once, so every connection reads it in the same order
				runtime.sort();
				for (const connection of connections) {
					...
					let sameState = true;
					let stateInfo = "";
					for (const runtimeName of runtime) {
						const state = connection.getActiveState(runtimeName);
						if (firstState === undefined) {
							firstState = state;
						} else if (state !== firstState) {
							sameState = false;
						}
						stateInfo += activeStateToString(state) + runtimeName;
					}
					if (sameState) {
						if (firstState === false) continue;
