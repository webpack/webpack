			...
			(compilation, { normalModuleFactory }) => {
				...
				const walkDefinitionsForKeys = definitions => {
					for (const key of Object.keys(definitions)) {
						...
						const nested = splitted.slice(0, -1).join(".");
						const final = splitted[splitted.length - 1];
						if (finalByNestedKey.has(nested)) {
							/** @type {Set<string>} */
							(finalByNestedKey.get(nested)).add(final);
						} else {
							finalByNestedKey.set(nested, new Set([final]));
						}
						if (nestedByFinalKey.has(final)) {
							/** @type {Set<string>} */
							(nestedByFinalKey.get(final)).add(nested);
						} else {
							nestedByFinalKey.set(final, new Set([nested]));
						}
					}
				};
