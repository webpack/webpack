			...
			(compilation, { normalModuleFactory }) => {
				...
				const walkDefinitionsForValues = (definitions, prefix) => {
					for (const key of Object.keys(definitions)) {
						...
						} else if (declaredKeys !== undefined) {
							// Leaves only: an object key counts as used whenever any of its
							// members is, which would report the parent as unused.
...
/**
 * Marks a plugin instance as webpack's own, so the keys it declares are left out
 * of what `performance.unusedDefines` reports.
...
/**
 * The define keys the configuration declared, webpack's own injected ones excluded.
 * @param {Compilation} compilation the compilation
 * @returns {Set<string> | undefined} the keys, or undefined when nothing declared any
 */
