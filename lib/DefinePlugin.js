...
/** @typedef {(value: { module?: NormalModule, key: string, readonly version: ValueCacheVersion }) => CodeValuePrimitive | Promise<CodeValuePrimitive>} GeneratorFn */
...
class RuntimeValue {
	...
	constructor(fn, options) {
		/** @type {GeneratorFn} */
		this.fn = fn;
		/** @type {boolean} true when `fn` is declared `async`, resolved before parsing */
	...
	exec(parser, valueCacheVersions, key) {
		...
		if (this.isAsync) {
			...
			const values =
				compilation === undefined
					? undefined
					: asyncRuntimeValueResults.get(compilation);
			const entry = values === undefined ? undefined : values.get(this);
			if (entry !== undefined) {
				if ("value" in entry) return entry.value;
				throw entry.error;
			}
			throw new WebpackError(
				`${PLUGIN_NAME}\nAsync runtime value '${key}' was not resolved before parsing.`
			);
		}
		...
		if (
			typeof value === "object" &&
			value !== null &&
			typeof (/** @type {Promise<CodeValuePrimitive>} */ (value).then) ===
				"function"
		) {
			throw new WebpackError(
				`${PLUGIN_NAME}\nRuntime value '${key}' returned a Promise, but its generator is not an async function.`
			);
		}
		return /** @type {CodeValuePrimitive} */ (value);
	}
...
/**
 * The async runtime values a compile cycle resolved, per compilation. The sync
 * parser reads these during parsing; each entry carries the resolved value or
 * the error its generator threw.
 * @type {WeakMap<Compilation, Map<RuntimeValue, { value: CodeValuePrimitive } | { error: Error }>>}
 */
const asyncRuntimeValueResults = new WeakMap();

/**
 * Collects every async runtime value in a definitions map. Each instance is
 * resolved once per compile, with the key of its first occurrence.
 * @param {Definitions} definitions the definitions map
 * @param {string} prefix the key prefix of the current nesting level
 * @param {Set<RuntimeValue>} seen visited instances (mutated)
 * @param {{ runtimeValue: RuntimeValue, key: string }[]} results collected instances (mutated)
 * @returns {void}
 */
const collectAsyncRuntimeValues = (definitions, prefix, seen, results) => {
	for (const key of Object.keys(definitions)) {
		const code = definitions[key];
		if (code instanceof RuntimeValue) {
			if (!code.isAsync || seen.has(code)) continue;
			seen.add(code);
			results.push({ runtimeValue: code, key: prefix + key });
		} else if (isObjectDefinition(code)) {
...
class DefinePlugin {
	 ...
	 */
	apply(compiler) {
		/** @type {Map<RuntimeValue, { value: CodeValuePrimitive } | { error: Error }>} */
		const resolvedAsyncValues = new Map();
		/** @type {{ runtimeValue: RuntimeValue, key: string }[]} */
		const asyncRuntimeValues = [];
		collectAsyncRuntimeValues(
			this.definitions,
			"",
			new Set(),
		...
		if (asyncRuntimeValues.length > 0) {
			/**
			 * Resolves the async runtime values, so the sync parser can read them
			 * from the results map. Runs before every compile; a watch rebuild
			 * regenerates the values like a sync generator would.
			 * @returns {Promise<void>} resolves when every value is resolved
			 */
			const resolveAsyncRuntimeValues = async () => {
				await Promise.all(
					asyncRuntimeValues.map(async ({ runtimeValue, key }) => {
						try {
							resolvedAsyncValues.set(runtimeValue, {
								value: await runtimeValue.fn({
									module: undefined,
									key,
									version: /** @type {ValueCacheVersion} */ (
										runtimeValue.getCacheVersion()
									)
								})
							});
						} catch (err) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				if (asyncRuntimeValues.length > 0) {
					let values = asyncRuntimeValueResults.get(compilation);
					if (values === undefined) {
						values = new Map();
						asyncRuntimeValueResults.set(compilation, values);
					}
					for (const [runtimeValue, entry] of resolvedAsyncValues) {
						values.set(runtimeValue, entry);
					}
				}
