			...
			params.normalModuleFactory.plugin("parser", (parser) => {
				...
				function applyDefine(key, code) {
					...
					if(!isTypeof) {
						...
						parser.evaluateIdentifierPlugin.plugin(key, (expr) => {
							/**
					...
					parser.evaluateTypeOfPlugin.plugin("evaluate typeof " + key, (expr) => {
						/**
						 * this is needed in case there is a recursion in the DefinePlugin
						 * to prevent an endless recursion
						 * e.g.: new DefinePlugin({
				...
				function applyObjectDefine(key, obj) {
					...
					parser.plugin("can-rename " + key, ParserHelpers.approve);
					parser.evaluateIdentifierPlugin.plugin(key, (expr) => new BasicEvaluatedExpression().setRange(expr.range));
					parser.evaluateTypeOfPlugin.plugin("evaluate typeof " + key, ParserHelpers.evaluateToString("object"));
					parser.plugin("expression " + key, ParserHelpers.toConstantDependency(code));
