		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				if (this.type === "javascript") {
					...
					/**
					 * @param {EXPECTED_ANY} parser Parser
					 * @returns {void}
					 */
					const textHandler = (parser) => {
						parser.hooks.program.tap(
							PLUGIN_NAME,
							(
								/** @type {string} */ source,
								/** @type {import("./Parser").ParserState} */ state
							) => {
								...
								const processDefinitions = (defs, prefix) => {
									for (const key of Object.keys(defs)) {
										...
										const replacementCode = toCode(
											code,
											/** @type {JavascriptParser} */ (parser),
											compilation.valueCacheVersions,
											fullKey,
											runtimeTemplate,
											logger,
										...
										replacements.push([fullKey, replacementCode]);
									}
								};
								...
								const claimed = /** @type {[number, number][]} */ ([]);
								for (const [key, replacementCode] of replacements) {
									let startIndex = 0;
									while (
										(startIndex = source.indexOf(key, startIndex)) !== -1
									) {
										const endIndex = startIndex + key.length;
										let overlaps = false;
										for (const [cs, ce] of claimed) {
											if (startIndex < ce && endIndex > cs) {
												overlaps = true;
												break;
											}
										}
										if (!overlaps) {
											const dep = new ConstDependency(replacementCode, [
												startIndex,
												endIndex
											]);
											parserState.module.addPresentationalDependency(dep);
											claimed.push([startIndex, endIndex]);
										}
										startIndex = endIndex;
									}
								}
					...
					for (const moduleType of moduleTypes) {
						normalModuleFactory.hooks.parser.for(moduleType).tap(
							PLUGIN_NAME,
							/** @type {EXPECTED_ANY} */
							(textHandler)
						);
