...
// Keys webpack injects itself are not the user's to remove, so `performance.unusedDefines`
// never sees them.
...
/**
 * @typedef {object} DefinePluginOptions
 * @property {"javascript" | "html" | "css"=} type module type to apply definitions to
 */
...
class DefinePlugin {
	 ...
	 * @param {DefinePluginOptions=} options options
	 */
	constructor(definitions, options = {}) {
		/** @type {Definitions} */
		this.definitions = definitions;
		/** @type {"javascript" | "html" | "css"} */
		this.type = options.type || "javascript";
	}
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// Only `performance.unusedDefines` reads these, so a build without it
				// collects nothing.
				const collectDeclared =
					performance !== false &&
					performance.unusedDefines === true &&
				...
				// Only JavaScript defines belong in the merged tree: it is what JS
				// object expansion reads, so an `html`/`css` instance sharing a
				// prefix with a JS one would otherwise inline its keys into JS.
				...
				if (this.type === "javascript") {
					...
					// Substituting into the source before it is parsed, rather than
					// recording replacements from a parse hook: the parser has to see
					// the substituted text, or `%BASE_URL%favicon.ico` in an `src`
					// resolves verbatim and the parser's own dependency ranges overlap
					// these.
					NormalModule.getCompilationHooks(compilation).processResult.tap(
						{ name: PLUGIN_NAME, stage: -10 },
						(result, module) => {
							if (!moduleTypes.has(module.type)) return result;
							...
							// Hash over every definition, so adding or removing one
							// invalidates this module — a per-key entry cannot, since
							// `needBuild` only compares keys the cached map already holds.
							...
							// `RuntimeValue#exec` reads `parser.state.module`, and neither
							// text parser has a `state` — the module is what it wants.
							const moduleParser =
								/** @type {JavascriptParser} */
								(/** @type {unknown} */ ({ state: { module } }));

							const replacements = /** @type {[string, string][]} */ ([]);
							/**
							 * @param {Record<string, CodeValue>} defs definitions
							 * @param {string} prefix prefix
							 */
							const processDefinitions = (defs, prefix) => {
								for (const key of Object.keys(defs)) {
									const code = defs[key];
									if (isObjectDefinition(code)) {
										processDefinitions(code, `${prefix + key}.`);
										continue;
									}
									const fullKey = prefix + key;
									valueDependencies.set(
										VALUE_DEP_PREFIX + fullKey,
										/** @type {ValueCacheVersion} */
										(
											compilation.valueCacheVersions.get(
												VALUE_DEP_PREFIX + fullKey
											)
										)
									);
							...
							// longest key first — specific match wins over prefix
							replacements.sort((a, b) => b[0].length - a[0].length);

							/** @type {[number, number, string][]} */
							const edits = [];
							for (const [key, replacementCode] of replacements) {
								let startIndex = 0;
								while ((startIndex = text.indexOf(key, startIndex)) !== -1) {
									const endIndex = startIndex + key.length;
									let overlaps = false;
									for (const [from, to] of edits) {
										if (startIndex < to && endIndex > from) {
											overlaps = true;
											break;
										}
									}
									if (!overlaps) {
										edits.push([startIndex, endIndex, replacementCode]);
									}
									startIndex = endIndex;
								}
							}
							...
							let out = "";
							let last = 0;
							for (const [from, to, replacementCode] of edits) {
...
/**
 * Marks an instance as webpack's own, so `performance.unusedDefines` never
...
module.exports.getDeclaredKeys = getDeclaredKeys;
...
module.exports.markInternal = markInternal;
