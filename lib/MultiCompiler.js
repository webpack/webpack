...
const { MultiHook, SyncBailHook, SyncHook } = require("tapable");

const WebpackError = require("./errors/WebpackError");
const MultiStats = require("./stats/MultiStats");
const ArrayQueue = require("./util/ArrayQueue");
const asyncLib = require("./util/async");
const memoize = require("./util/memoize");
const MultiWatching = require("./watch/MultiWatching");
...
module.exports = class MultiCompiler {
	...
	constructor(compilers, options) {
		...
		this.hooks = Object.freeze({
			 ...
			 * @type {SyncBailHook<[ReadonlySet<string>], true | void>}
			 * @since 5.112.0
			 */
			buildDependenciesChanged: new SyncBailHook(["changedFiles"])
		});
		/** @type {Compiler[]} */
		...
		/** @type {Map<string, { timestamp: number | null | undefined, handled: boolean }>} */
		this._reportedBuildDependencies = new Map();
		...
		for (let index = 0; index < this.compilers.length; index++) {
			...
			// before plugins tapping the child, like this hook does for the MultiCompiler
			compiler.hooks.buildDependenciesChanged.tap(
				{ name: CLASS_NAME, stage: -100 },
				(changedFiles) => {
					if (!this.hooks.buildDependenciesChanged.isUsed()) return;
					return this._reportBuildDependenciesChanged(compiler, changedFiles);
				}
			);
			...
			// Release this child's Compilation fields once it is done. The
			// `stage: Infinity` tap runs last, so a plugin watching in afterDone still
			// sees it intact, and only fields Stats never reads are dropped.
	...
	_runGraph(setup, run, callback) {
		...
		// A node starts blocked and reaches `starting` once its parents are done or
		// the queue reaches it, then `running`, then `done`. Invalidation sends it
		// back to `blocked`, or to `running-outdated` if it is mid-compilation.
	...
	/**
	 * Reports a child compiler's changed build dependencies, once for a change several children report.
	 * @param {Compiler} compiler the child compiler reporting
	 * @param {ReadonlySet<string>} changedFiles its changed build dependencies
	 * @returns {true | void} true when a plugin took over
	 */
	_reportBuildDependenciesChanged(compiler, changedFiles) {
		const reported = this._reportedBuildDependencies;
		const timestamps = compiler.buildDependencyFiles;
		// the reporting child already recorded the timestamps it saw, the same ones mean the same change
		let sameChange = timestamps !== undefined;
		for (const file of changedFiles) {
			if (!sameChange) break;
			const timestamp = /** @type {Map<string, number | null | undefined>} */ (
				timestamps
			).get(file);
			const previous = reported.get(file);
			sameChange =
				timestamp !== undefined &&
				previous !== undefined &&
		...
		if (sameChange) {
			for (const file of changedFiles) {
				if (
					!(/** @type {{ handled: boolean }} */ (reported.get(file)).handled)
				) {
					return;
				}
			}
		...
		const result = this.hooks.buildDependenciesChanged.call(changedFiles);
		for (const file of changedFiles) {
			reported.set(file, {
				timestamp: timestamps && timestamps.get(file),
				handled: result === true
			});
		}
		return result;
