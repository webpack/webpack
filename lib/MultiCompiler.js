...
module.exports = class MultiCompiler extends Tapable {
	...
	validateDependencies(callback) {
		...
		const stack = this.compilers.filter((c) => !edges.find((e) => e.target === c));
		while(stack.length > 0) {
			const current = stack.pop();
			const incoming = edges.filter((e) => e.source === current);
		...
		if(edges.length > 0) {
			const lines = edges.sort(sortEdges).map(edge => `${edge.source.name} -> ${edge.target.name}`);
