...
const { register, registerLegacyRequest } = require("../util/serialization");
const extractUrlAndGlobal = require("./extractUrlAndGlobal");
...
/** @import ExportsInfo from "../graph/ExportsInfo" */
...
register(
	...
	"webpack/lib/externals/ExternalModule",
	"ModuleExternalInitFragment",
	{
		serialize(obj, { write }) {
			write(obj._request);
...
class ExternalModule extends Module {
	...
	getSourceTypes() {
		...
		// TODO in the next major release: drop "css-url" once the alias is removed
	...
	hasModuleSource() {
		...
		// An external reaches the host in the phase it was requested, so whether it
		// has one is the host's answer to give, never webpack's.
		return ImportPhaseUtils.isSource(meta && meta.phase);
	}
	...
	codeGeneration({
		...
		switch (externalType) {
			...
			// TODO in the next major release: remove "css-url" alias
			case "css-url":
			case "asset-url": {
				/** @type {Sources} */
				const sources = new Map();
...
makeSerializable(ExternalModule, "webpack/lib/externals/ExternalModule");

// TODO in the next major release: remove
// Restores caches written before these moved into lib/externals/
registerLegacyRequest(ExternalModule, "webpack/lib/ExternalModule");
