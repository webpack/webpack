	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				const outputModule = Boolean(compiler.options.experiments.outputModule);

				const { skipMakeNamespaceObject } =
					getConcatenatedModule().getCompilationHooks(compilation);
				...
				// injecting __esModule. Only in outputModule mode to preserve CJS interop.
				// https://tc39.es/proposal-json-modules/#sec-create-default-export-synthetic-module
				skipMakeNamespaceObject.tap(PLUGIN_NAME, (module) => {
					if (
						module.type === ASSET_MODULE_TYPE_SOURCE &&
				...
				normalModuleFactory.hooks.createParser
					...
					.tap(
						PLUGIN_NAME,
						/**
						 * @param {import("../../declarations/WebpackOptions").AssetSourceParserOptions} parserOptions parser options
						 * @returns {AssetSourceParser} the parser
						 */
						(parserOptions) => {
							...
							return new AssetSourceParser({
								exportsType:
									parserOptions.exportsType ||
									(outputModule ? "namespace" : "default")
							});
						}
					);
				normalModuleFactory.hooks.createParser
