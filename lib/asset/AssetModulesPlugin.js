		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				for (const type of [
					...
					// uses the hook instead of `moduleClasses`: `sideEffectFree` comes from plugin options
					normalModuleFactory.hooks.createModuleClass.for(type).tap(
						PLUGIN_NAME,
						(createData, _resolveData) =>
							new AssetModule({
								...createData,
								sideEffectFree: this.options.sideEffectFree
							})
					);
				}
