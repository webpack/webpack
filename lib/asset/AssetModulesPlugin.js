		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.hooks.executeModule.tap(PLUGIN_NAME, (options, context) => {
					const { codeGenerationResult } = options;
					// An asset consumed only via `new URL()` in analyzable ESM output has
					// no `module.exports = …` wrapper, so requiring it at build time would
					// yield `{}` — emulate the wrapper's exports instead.
