	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				const outputModule = Boolean(compiler.options.output.module);
				if (outputModule) {
					const { skipMakeNamespaceObject } =
						getConcatenatedModule().getCompilationHooks(compilation);
					// When output.module is enabled, skip __webpack_require__.r() for
					// asset/source modules to avoid injecting __esModule into the namespace.
					...
					skipMakeNamespaceObject.tap(
						PLUGIN_NAME,
						(module) => module.type === ASSET_MODULE_TYPE_SOURCE
					);
				}
