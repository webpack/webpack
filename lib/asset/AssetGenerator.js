...
const URL_DEPENDENCY_TYPE = "new URL()";
const CSS_URL_DEPENDENCY_TYPE = "css url()";
...
class AssetGenerator extends Generator {
	...
	getTypes(module) {
		...
		// Concatenation can absorb a css module into the javascript one that imported
		// it, which makes its `url()` look like a javascript reader of the wrapper.
		// Only worth asking where a css module can exist at all — without one the loop
		// below keeps the cheaper path of not reading a dependency type per connection.
		const hasCss = Boolean(
			this._compilation && this._compilation.options.experiments.css
		);
		...
		for (const connection of connections) {
			...
			if (originType === ASSET_MODULE_TYPE_WEBMANIFEST) {
				...
				const dependency = (isModule || hasCss) && connection.dependency;
				const dependencyType = dependency && dependency.type;
				if (dependencyType === CSS_URL_DEPENDENCY_TYPE) {
					// Concatenation absorbs a css module into the javascript one that
					// imported it, so the origin type alone would misread it as a reader
					// of the wrapper — it is still a css url() and reads the asset-url.
					hasUrl = true;
				} else if (
					isModule &&
					dependencyType === URL_DEPENDENCY_TYPE &&
