...
class AssetGenerator extends Generator {
	 ...
	 * @param {OutputOptions=} outputOptions the output options (`getTypes` has no generate context, so the analyzable-output decision needs `output.module`/`output.publicPath` here)
	 */
	constructor(
		moduleGraph,
		dataUrlOptions,
	...
	_hasAbsolutePublicPath() {
		const publicPath =
			this.publicPath !== undefined
				? this.publicPath
				: this._outputOptions && this._outputOptions.publicPath;
		return (
	...
	getTypes(module) {
		...
		const jsAsAssetUrl =
			Boolean(this._outputOptions && this._outputOptions.module) &&
			hasUrlJs &&
			!hasOtherJs &&
