...
class AssetGenerator extends Generator {
	...
	static getAssetPathWithInfo(
		...
		if (generatorOptions.publicPath !== undefined && type === JAVASCRIPT_TYPE) {
			...
			// Only under `auto`, where `.p` is this asset's own directory url, does the
			// absolute `href` spell the same string the concatenation would have.
			const analyzable =
				runtimeTemplate.outputOptions.publicPath === "auto" &&
				runtimeTemplate.supportsAnalyzable("url", chunkGraph, module)
					...
					: null;
			if (analyzable !== null) {
				assetPath = `${runtimeTemplate.importMetaUrl(analyzable)}.href`;
			} else {
