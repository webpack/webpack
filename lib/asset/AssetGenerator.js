...
class AssetGenerator extends Generator {
	...
	getTypes(module) {
		...
		for (const connection of connections) {
			...
			hasOrigin = true;
			// Concatenation re-points the connection at the javascript module that
			// absorbed the one holding the reference, so the origin no longer says what
			// kind of reference it is. The module the dependency belongs to still does.
			const dependency = connection.dependency;
			const owner = dependency
				? this._moduleGraph.getParentModule(dependency)
				: undefined;
			const originType = (owner || originModule).type;
			// Javascript is the only consumer that can read the url back out of the
			// module wrapper; css, html, a manifest and anything added later embed a
			// bare url string, which is what `asset-url` renders. Asked that way round
			// so a type nobody here has heard of resolves rather than falling through.
			if (typePrefixEquals(originType, JAVASCRIPT_TYPE)) {
