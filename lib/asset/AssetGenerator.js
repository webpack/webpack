...
const {
	ASSET_MODULE_TYPE,
	CSS_MODULES,
	JAVASCRIPT_LIKE_MODULES
} = require("../ModuleTypeConstants");
const RuntimeGlobals = require("../RuntimeGlobals");
const CssUrlDependency = require("../dependencies/CssUrlDependency");
const { hasIntersection } = require("../util/SetHelpers");
...
class AssetGenerator extends Generator {
	...
	getTypes(module) {
		...
		for (const connection of connections) {
			...
			sourceTypes.add(connection.originModule.type);
		}

		if ((module.buildInfo && module.buildInfo.dataUrl) || this.emit === false) {
			if (sourceTypes.size > 0) {
				const hasJs = hasIntersection(sourceTypes, JAVASCRIPT_LIKE_MODULES);
				const hasCss = hasIntersection(sourceTypes, CSS_MODULES);
				if (hasJs && hasCss) {
