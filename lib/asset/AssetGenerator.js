...
class AssetGenerator extends Generator {
	 ...
	 * @param {boolean=} outputModule whether the output is an ESM module (enables the analyzable `new URL` form)
	 * @param {RawPublicPath=} outputPublicPath the global `output.publicPath`
	 */
	constructor(
		moduleGraph,
		dataUrlOptions,
		...
		this.emit = emit;
		/** @type {boolean} */
		this._outputModule = outputModule || false;
		/** @type {RawPublicPath | undefined} */
		this._outputPublicPath = outputPublicPath;
		/** @type {ModuleGraph} */
	...
	_hasAbsolutePublicPath() {
		const publicPath =
			this.publicPath !== undefined ? this.publicPath : this._outputPublicPath;
		return (
	...
	getTypes(module) {
		...
		// A JS module that references the asset only through `new URL(…, import.meta.url)`
		// doesn't need the `module.exports = …` wrapper in ESM output — it is consumed
		// like a CSS/HTML asset (`asset-url`), which keeps the wrapper out of the bundle.
		let hasUrlJs = false;
		...
		for (const connection of connections) {
			...
			const type = connection.originModule.type.split("/")[0];
			if (type === JAVASCRIPT_TYPE) {
				if (
					connection.dependency &&
					connection.dependency.type === "new URL()"
				) {
					hasUrlJs = true;
				} else {
					hasOtherJs = true;
		...
		const jsAsAssetUrl =
			...
			hasUrlJs &&
			!hasOtherJs &&
			this._hasAbsolutePublicPath();
		const wantJs = sourceTypes.has(JAVASCRIPT_TYPE) && !jsAsAssetUrl;
		const wantAssetUrl =
			sourceTypes.has(CSS_TYPE) || sourceTypes.has(HTML_TYPE) || jsAsAssetUrl;
		...
		if (
			...
			if (sourceTypes.size > 0) {
				if (wantJs && wantAssetUrl) {
					return JAVASCRIPT_AND_ASSET_URL_TYPES;
				} else if (wantAssetUrl) {
