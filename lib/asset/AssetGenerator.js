...
const { ASSET_MODULE_TYPE } = require("../module/ModuleTypeConstants");
...
/** @import Compilation, { AssetInfo } from "../Compilation" */
...
class AssetGenerator extends Generator {
	...
	static getSourceFileName(module, runtimeTemplate) {
		return makePathsRelative(
			...
			AssetGenerator.getAssetResource(module),
			runtimeTemplate.compilation.compiler.root
		).replace(/^\.\//, "");
	}

	/**
	 * The resource this asset is named after, which something that re-encoded it
	 * may have changed. Its emitted name and inlined media type both read this.
	 * @param {NormalModule} module module
	 * @returns {string} the resource
	 */
	static getAssetResource(module) {
		const buildInfo = /** @type {AssetModuleBuildInfo | undefined} */ (
			module.buildInfo
		);

		return /** @type {string} */ (
			(buildInfo && buildInfo.assetResource) || module.getResource()
		);
	}

	/**
	 * Gets the source file name as seen by a filename template. With
	 * `experiments.futureDefaults` (the next major release's default) `[path]` and `[file]` of a
	 * module outside of `context` behave like `[containedpath]`/`[containedfile]`,
	 * so they can't emit the asset outside of `output.path`.
	 * @param {NormalModule} module module
	 * @param {RuntimeTemplate} runtimeTemplate runtime template
	 * @returns {string} source file name for the filename template
	 */
	static getTemplateSourceFileName(module, runtimeTemplate) {
		return toTemplateSourceFileName(
			AssetGenerator.getSourceFileName(module, runtimeTemplate),
	...
	static getFilenameWithInfo(
		...
		// Both templates decide where the asset is written, so they see the
		// contained path.
		const templateSourceFilename = AssetGenerator.getTemplateSourceFileName(
			module,
			runtimeTemplate
		);
		let { path: filename, info: assetInfo } =
			runtimeTemplate.compilation.getAssetPathWithInfo(assetModuleFilename, {
				module,
				runtime,
				filename: templateSourceFilename,
				chunkGraph,
				contentHash,
	...
	static getAssetPathWithInfo(
		...
		// A public path is a url prefix, it doesn't decide where the asset is
		// written, so `[path]` stays the real path of the source file here.
		...
		if (generatorOptions.publicPath !== undefined && type === JAVASCRIPT_TYPE) {
			...
			// `auto` reads `.p` as this asset's own directory url, which the absolute
			// `href` spells too; a complete url of its own is already the whole string.
			const constant = runtimeTemplate.constantPublicPath();
			const whole =
				constant !== undefined &&
				(constant.startsWith("//") || getScheme(constant) !== undefined);
			const analyzable =
				(runtimeTemplate.outputOptions.publicPath === "auto" || whole) &&
			...
			if (analyzable !== null) {
				assetPath = whole
					? analyzable
					: `${runtimeTemplate.importMetaUrl(analyzable)}.href`;
			} else {
	...
	getMimeType(module) {
		...
		if (mimeType === undefined) {
			// The resource as it now is, minus the query and fragment a request
			// carries into it.
			const resource = AssetGenerator.getAssetResource(module);
			const ext = path.extname(parseResource(resource).path);
			// `resourceResolveData` describes what was read; once something
			// re-encoded it, only the new extension says what the bytes are.
			if (
	...
	/**
	 * Offer the payload to `renderEmbeddedSource` before it is encoded, so the
	 * encoding covers what came back. Declines what names no language, and a
	 * `dataUrl` function, which is handed the source itself.
	 * @param {NormalModule} module the module holding the payload
	 * @param {GenerateContext["runtimeTemplate"]} runtimeTemplate runtime template
	 * @returns {Source | undefined} the rendered payload, when one came back
	 */
	_renderEmbedded(module, runtimeTemplate) {
		if (!runtimeTemplate || typeof this.dataUrlOptions === "function") {
			return undefined;
		}
		// Untapped, nothing can change: no mime lookup and no copy of the payload.
		const { compilation } = runtimeTemplate;
		if (compilation.hooks.renderEmbeddedSource.taps.length === 0) {
			return undefined;
		}
		// A `data:` URI written as text with a raw `#` ends where a browser reads
		// its fragment, so its payload is not the one this module holds.
		const resolved = module.resourceResolveData;
		if (
			resolved &&
			resolved.encoding === false &&
			typeof resolved.encodedContent === "string" &&
			resolved.encodedContent.includes("#")
		) {
			return undefined;
		}
		const type = languageOfMediaType(this.getMimeType(module));
		if (type === undefined) return undefined;
		// An asset's source holds bytes; the hook is offered text, and the language
		// above is only ever one written as text.
		const content = /** @type {Source} */ (module.originalSource()).source();
		return compilation._resolveEmbeddedSource(
			new RawSource(
				typeof content === "string" ? content : content.toString("utf8")
			),
			{ type, hostType: JAVASCRIPT_TYPE, module }
		);
	}
	 ...
	 * Generates data uri.
	 * @param {NormalModule} module module for which the code should be generated
	 * @param {Source=} renderedSource the payload to encode, when a hook rendered it
	 * @returns {string} DataURI
	 */
	generateDataUri(module, renderedSource) {
		const source =
			renderedSource || /** @type {Source} */ (module.originalSource());
		...
		if (typeof this.dataUrlOptions === "function") {
			...
			if (
				...
				encoding === false &&
				module.resourceResolveData &&
				module.resourceResolveData.encoding === false
			) {
				// A rendered payload of a `data:` URI written as text is written back
				// as text: percent-encoding all of it outgrows what minifying saved.
				encodedContent = encodeDataURIPayload(
	...
	generate(module, generateContext) {
		...
		if (
			...
			// WHY: CSS and HTML serializers offer the payload in the `url()` or
			// attribute it lands in, but the `asset-url` value is also what an
			// analyzable `new URL` copies into JavaScript, which nothing re-reads.
			const encodedSource = this.generateDataUri(
				module,
				this._renderEmbedded(module, runtimeTemplate)
			);
			content =
			...
			// TODO in the next major release: remove the back-compat writes to module.buildInfo
			/** @type {AssetModuleBuildInfo} */
	...
	getTypes(module) {
		...
		// WHY: only analyzable ESM output can drop a bare `new URL()` to an asset
		// url; without it every JS consumer needs the wrapper, so the per-connection
		// dependency probe is skipped entirely and the cheap path stands. This must
		// match the condition `URLDependency` emits the literal under, or the wrapper
		// is dropped while the call site still requires the module — except where the
		// fallback spells the name itself (`.p + <file>`), which needs no wrapper to
		// read it out of. Build-time execution emulates the wrapper instead
		// (`AssetModulesPlugin`).
		const isModule = Boolean(
			this._compilation &&
			this._compilation.runtimeTemplate.supportsAnalyzable(
				emits ? "url-inline" : "url"
			)
		);
		...
		for (const connection of connections) {
			...
			hasOrigin = true;
			// The dependency says how the asset is read, so a reference added later answers
			// for itself; concatenation re-points the origin module, so it cannot.
			const dependency = connection.dependency;
			if (dependency && dependency.referencedSourceType === ASSET_URL_TYPE) {
				hasUrl = true;
			} else if (
				// Only a bare `new URL()` becomes the analyzable literal; a relative ref
				// keeps the runtime form and its wrapper. A prefetch or preload ref needs
				// none: its `<link>` resolves the href at chunk startup.
				isModule &&
				dependency instanceof URLDependency &&
				!dependency.relative
			) {
				hasUrlJs = true;
			} else {
		...
		switch (type) {
			...
			default:
				if (
					...
					// Rough data-url size: 4/3 for the base64 encoding, plus about 34 bytes
					// of header, footer and rounding.
	...
	updateHash(hash, updateHashContext) {
		...
		if (
			...
			// Hash what generation emits: the filename template sees the contained
			// path, unlike the public path above.
			const { path: filename, info } =
				runtimeTemplate.compilation.getAssetPathWithInfo(assetModuleFilename, {
					module,
					runtime,
					filename: AssetGenerator.getTemplateSourceFileName(
						module,
						runtimeTemplate
					),
					chunkGraph,
