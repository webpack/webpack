...
const { isImportMetaContextEnabled, isImportMetaGlobEnabled } =
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { contextModuleFactory, normalModuleFactory }) => {
				 ...
				 * @param {import("../../declarations/WebpackOptions").JavascriptParserOptions} parserOptions parserOptions
				 * @returns {void}
				 */
				const handler = (parser, parserOptions) => {
					if (
						!isImportMetaContextEnabled(parserOptions) &&
						!isImportMetaGlobEnabled(parserOptions)
					) {
						return;
					}

					new ImportMetaContextDependencyParserPlugin().apply(
						parser,
						parserOptions
					);
				};
