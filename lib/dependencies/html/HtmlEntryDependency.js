...
// only read when a page carries stylesheets, and it drags the css subsystem in
...
const getEntrypointChunksInLoadOrder = (entrypoint) => {
	...
	/** @type {Set<import("../../graph/ChunkGroup")>} */
	const visitedGroups = new Set();
	/**
	 * Collects the chunks an ancestor entry's own tags already load.
	 * @param {import("../../graph/ChunkGroup")} group chunk group to walk up from
	 * @returns {void}
	 */
	const walk = (group) => {
...
// WHY: an ES5 `<link rel="modulepreload">` polyfill (Safari <17, Firefox <115,
// …). On a browser without native support, each modulepreload target is fetched
// with the tag's own credentials and integrity, so the pending module import
// reuses it. The `.ep` guard makes that idempotent and the observer catches any
// later-inserted link. Emitted only where the target environment lacks native
// support.
...
HtmlEntryDependency.Template = class HtmlEntryDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// A `type: "html"` link points at another emitted page rather than a chunk,
		// so rewrite the attribute to that page's filename and emit no siblings:
		// a page link loads only itself.
		...
		// Mirrors `output.crossOriginLoading` onto every injected tag, as webpack's
		// runtime does for chunk-loading scripts. Empty when the option is off or
		// the originating tag set `crossorigin` itself, since the author wins.
		const crossOrigin =
			crossOriginLoading && !dep.hasOwnCrossOrigin
				? ` crossorigin="${crossOriginLoading}"`
				: "";

		// Per-chunk SRI emits an `integrity` sentinel resolved once the chunks'
		// final bytes exist. One module can back pages with different `integrity`
		// options, so every page emits them and strips its own on emit.
		...
		const isChunkInlined = (chunk, contentType) => {
			...
			if (!getHtmlGenerator().canInlineChunk(compilation, chunk)) {
				return false;
			}
			if (dep.forceInline) return true;
			return getHtmlGenerator().isChunkInlined(htmlOption, chunk, contentType);
		};
		...
		const isExtractedHtml = chunkGraph
			.getModuleSourceTypes(
				/** @type {import("../../module/Module")} */ (templateContext.module)
			)
			.has("html");
		const entryInlined =
			isExtractedHtml &&
		...
		if (inject !== false) {
			...
			const buildSibling = (chunk, kind) => {
				...
				const integrityAttr = integrityOn
					...
					: "";
				if (kind === "css") {
					// A CSS chunk always loads through `<link rel="stylesheet">`. Clone
					// only when the entry tag is itself one, so `media` carries over;
					// anything else gets a freshly synthesized `<link>`.
			...
			// With the entry tag already inside `<head>`, inserting at `headClose`
			// would place siblings after it and reverse runtime-before-entry order
			// for deferred scripts. Insert before the entry tag instead.
			const headAt =
				headClose < 0
					? -1
			...
			let cssAt = -1;
			if (isStylesheet) {
				// WHY: a `<link rel="stylesheet">` entry is CSS, but a shared or runtime
				// JS-only chunk — `optimization.runtimeChunk`, say — can still sit in the
				// entrypoint, so the `<link>` is cloned only for siblings that actually
				// produce a `.css` asset, mirroring the JS branch below. Otherwise it
				// points at a `.css` file that does not exist: a 404, or an empty
				// `<style>` when inlined. Attributes such as `media` still carry over.
				...
				// WHY: splitChunks inserts each new chunk before the entry chunk, so
				// `entrypoint.chunks` hands back split CSS siblings in reverse of the
				// order they were processed — the html-webpack-plugin#1838 and
				// mini-css-extract#959 symptom. The order is re-derived from the
				// entrypoint's module post-order index, which follows the dependency
				// walk and so the source's import order.
				/** @type {Chunk[]} */
				const cssChunks = [];
				/** @type {Chunk[]} */
				const jsChunks = [];
				for (let i = 0; i < orderedChunks.length - 1; i++) {
					...
					// Anything not CSS-only stays on the JS lane in `orderedChunks` order,
					// preserving the runtime-first invariant. A chunk emitting neither
					// `.js` nor `.css` still gets a clone, as it always has.
					if (hasJs || !hasCss) jsChunks.push(chunk);
				}
				// When the entry chunk carries CSS itself, fold it into the same
				// ordering so its `<link>` lands in the right cascade position
				// relative to the sibling CSS chunks.
				if (chunkHasCss(entryChunk, chunkGraph)) {
					cssChunks.push(entryChunk);
				}
				const cssChunkOrder = getCssModulesPlugin().getCssChunksInCascadeOrder(
					cssChunks,
					entrypoint,
					chunkGraph
				);
				// Stylesheets land in `<head>`. A classic blocking script pins them
				// ahead of itself, so it never awaits later CSS; with only `defer` and
				// module scripts they go last, which starts the script fetches earlier.
				cssAt =
					dep.headAnchor >= 0
		...
		// WHY: resource hints are opt-in, the initial chunks already loading
		// through parallel `<script>` tags, so there is no default waterfall to
		// prevent. `output.resourceHints` requests them: `true` preloads the entry's
		// initial dependency chunks (runtime, vendor, split), while an array or a
		// function adds custom ones and a function may spread the auto
		// `defaultHints`. Extracted pages only — a JS-string HTML export would embed
		// the sentinels in the JS chunk. A URL-referenced asset carrying
		// `webpackPrefetch` / `webpackPreload`, by magic comment or a
		// `parser.<type>.urlHints` rule, emits here too.
		...
		if (
			...
			if (resourceHints !== undefined && resourceHints !== false) {
				// WHY: descriptors for the auto initial-graph preload, carrying their
				// exact tag and built only when needed — `true` or `"prefetch"` emits
				// them, and a function may spread them. The entry chunk is skipped, being
				// the `<script src>` already. ESM uses `modulepreload`, which fetches and
				// parses; classic output `preload as="script"`; and `"prefetch"` opts into
				// `<link rel="prefetch">`, an idle-time fetch with no `as` and no
				// `integrity`, which the spec does not list. An async `import()` chunk is
				// not covered here: that is
				// `module.parser.javascript.dynamicImport*`, which routes through
				// webpack's on-demand chunk-load runtime.
					...
					for (const chunk of orderedChunks) {
						...
						const integrityAttr =
							integrityOn && !prefetchMode
								...
								: "";
						// Mirror the entry tag's `nonce`, `crossorigin` and `referrerpolicy`,
						// so the request matches the sibling `<script>`'s and the browser
						// reuses it rather than fetching twice.
			...
			// The plugin already walked the entrypoint once and cached this entry's
			// asset deps, so consume that list rather than repeat it. URLs stay
			// sentinels here: the asset module may not be codegen'd yet.
			...
			for (const { module, dep: md } of resourceHintResolver.getHtmlHinted(
				...
				const asAttr =
					...
					ResourceHintRuntimeModule.guessAsAttribute(
						/** @type {import("../url/URLDependency")} */ (md).request || ""
					);
			...
			if (hints.length > 0) {
				...
				// Without native `modulepreload` the hints warm no cache, so append the
				// polyfill once per page. It defaults from the target's support, and
				// `false` opts out, which a strict CSP needs.
				if (
					rhOptions &&
				...
				// Insert just after the `<head>` open tag, so the hints are the first
				// fetches. A page with no written `<head>` falls back to the first
				// blocking script, else the entry tag.
				const hintAt =
					dep.headOpenEnd >= 0
						? dep.headOpenEnd
						: dep.cssAnchor === -1
...
makeSerializable(
	HtmlEntryDependency,
	"webpack/lib/dependencies/html/HtmlEntryDependency"
);
