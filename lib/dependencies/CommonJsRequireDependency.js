...
class CommonJsRequireDependency extends ModuleDependency {
	...
	/**
	 * Whether codegen replaces the whole `require(...)` call with `0`.
	 * @param {ModuleGraph} moduleGraph module graph
	 * @param {RuntimeSpec} runtime the runtime
	 * @returns {boolean} true when the call is dropped
	 */
	isCallDropped(moduleGraph, runtime) {
		if (this.valueRange === undefined || !this.isEvaluationOnly()) return false;
		const connection = moduleGraph.getConnection(this);
		if (!connection || connection.isTargetActive(runtime)) return false;
		const guards = this.branchGuards;
		// A dead branch keeps the call (and its header) as unreachable code.
		return (
			guards === undefined ||
