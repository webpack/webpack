...
import RequireContextDependencyParserPlugin from "./RequireContextDependencyParserPlugin.js";
						...
						for (const item of items) {
							...
							for (const modulesItems of finalResolveOptions.modules) {
								if (Array.isArray(modulesItems)) {
									...
									const dir = modulesItems.replaceAll("\\", "/");
									const fullPath =
										item.context.replaceAll("\\", "/") + item.request.slice(1);
...
export default RequireContextPlugin;

export { RequireContextPlugin as "module.exports" };
