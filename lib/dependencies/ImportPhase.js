...
// Kept apart from the message so runtime code can concatenate a request it only
// knows at runtime without restating the sentence.
const NO_MODULE_SOURCE_SUFFIX =
	" has no module source, so it cannot be imported in the source phase";

/**
 * Message for a module that carries no module source object. Every module
 * webpack does not build as a source representation is one.
 * @param {string} request the request that was imported
 * @returns {string} the message
 */
const noModuleSourceMessage = (request) =>
	`Module '${request}'${NO_MODULE_SOURCE_SUFFIX}`;
