...
/**
 * Returns true if the imported module is a JavaScript source-text module that
 * a source-phase import binds to an opaque ModuleSource reflection (per the
 * TC39 source-phase imports proposal). Returns false for module types that
 * provide a richer source-phase representation (e.g. async WebAssembly, where
 * the source phase exposes a `WebAssembly.Module` through the default export).
 * @param {Module | undefined | null} module imported module
 * @returns {boolean} whether the module is a JavaScript source-text module
 */
...
HarmonyImportDependency.Template = class HarmonyImportDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Source-phase imports of JavaScript modules bind the import variable
		// directly to a ModuleSource reflection (per the TC39 source-phase
		// imports proposal). The imported module is referenced for bundling
		// purposes but is not evaluated by this import.
		if (
			ImportPhaseUtils.isSource(dep.phase) &&
			isJavaScriptModule(referencedModule)
		) {
			templateContext.runtimeRequirements.add(RuntimeGlobals.makeModuleSource);
			const importVar = dep.getImportVar(templateContext.moduleGraph);
			const moduleId = templateContext.runtimeTemplate.moduleId({
			...
			templateContext.initFragments.push(
				new ConditionalInitFragment(
					`/* source-phase harmony import */ var ${importVar} = ${RuntimeGlobals.makeModuleSource}(${moduleId});\n`,
