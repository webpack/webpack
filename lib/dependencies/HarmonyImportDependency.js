...
/**
 * Export names a lazy barrel still defers, so they are missing from its provided
 * exports without being absent from the module.
 * @param {Module} module the imported module
 * @returns {Set<string> | true | undefined} deferred names, true when a deferred star re-export may provide any name, undefined when none
 */
const getDeferredExports = (module) => {
	...
	for (const dep of module.dependencies) {
		...
		if (until !== Dependency.LAZY_UNTIL_ID) continue;
		if (names === undefined) names = new Set();
		names.add(/** @type {string} */ (dep.getLazyName()));
	}
	return names;
};
...
class HarmonyImportDependency extends ModuleDependency {
	 ...
	 */
	getSourcePhaseErrors(moduleGraph) {
		...
		if (importedModule.hasModuleSource()) {
			return;
		}
		return [new HarmonyLinkingError(noModuleSourceMessage(this.userRequest))];
	}
	...
	getLinkingErrors(moduleGraph, ids, additionalMessage) {
		...
		if (exportsType === "namespace" || exportsType === "default-with-named") {
			...
			if (
				...
				const deferredExports = getDeferredExports(importedModule);

				if (
					deferredExports !== true &&
					(deferredExports === undefined || !deferredExports.has(ids[0]))
				) {
					...
					while (pos < ids.length && exportsInfo) {
						...
						if (exportInfo.provided === false) {
							// We are sure that it's not provided
							let providedExports = exportsInfo.getProvidedExports();
							// only the first level names the imported module itself
							if (
								pos === 1 &&
								deferredExports !== undefined &&
								Array.isArray(providedExports)
							) {
								providedExports = [...providedExports, ...deferredExports];
							}
...
HarmonyImportDependency.ExportPresenceModes = ExportPresenceModes;
HarmonyImportDependency.getNonOptionalPart = getNonOptionalPart;
