...
HarmonyImportDependency.Template = class HarmonyImportDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Source-phase imports of JavaScript modules are unsupported per the
		// TC39 source-phase imports proposal: GetModuleSource of a SourceTextModule
		// throws a SyntaxError. WebAssembly modules do support this phase, so only
		// throw when the imported module is a JavaScript module type.
		if (
			ImportPhaseUtils.isSource(dep.phase) &&
			referencedModule &&
			...
			templateContext.initFragments.push(
				new ConditionalInitFragment(
					`throw new SyntaxError(${JSON.stringify(
						`Source phase imports of JavaScript modules are not supported (importing '${dep.userRequest}').`
					)});\n`,
