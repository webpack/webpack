...
/**
 * Export names a side-effect-free barrel still defers (lazy barrel optimization),
 * `true` when a deferred star re-export may provide any name. Its provided
 * exports only cover the names something asked for, so these are missing from
 * them without being absent from the module.
 * @param {Module} module the imported module
 * @returns {Set<string> | true | undefined} deferred names, undefined when none
 */
const getDeferredExports = (module) => {
	...
	for (const dep of module.dependencies) {
		...
		if (!("getLazyUntil" in dep) || !dep.isLazy()) continue;
