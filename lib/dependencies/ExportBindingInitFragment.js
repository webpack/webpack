...
/**
 * Offered the rendered `d()` call and where it sits, and answers whether it took it
 * over. A consumer that does emits the call itself where it is actually read — or
 * leaves it out. Value bindings read the declarations above them, so one placed at
 * the end cannot be moved before the module body.
...
class ExportBindingInitFragment extends InitFragment {
	...
	_generateDefinition({ runtimeTemplate, runtimeRequirements }) {
		...
		return this.onDemand !== undefined &&
			this.onDemand(definition, this.placeAtEnd)
			? ""
			: definition;
	}
