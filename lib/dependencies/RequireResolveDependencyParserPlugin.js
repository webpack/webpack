...
class RequireResolveDependencyParserPlugin {
	...
	apply(parser) {
		...
		const process = (expr, weak) => {
			...
			if (param.isConditional()) {
				param.options.forEach((option, idx) => {
					const subParamExpr =
						idx === 0
							? expr.arguments[0].consequent
							: expr.arguments[0].alternate;
					const result = processItem(expr, option, weak);
					if (result === undefined) {
						processContext(expr, option, subParamExpr, weak);
					}
				});
				...
				if (result === undefined) {
					processContext(expr, param, expr.arguments[0], weak);
				}
				const dep = new RequireResolveHeaderDependency(expr.callee.range);
				dep.loc = expr.loc;
				parser.state.current.addDependency(dep);
		...
		const processContext = (expr, param, paramExpr, weak) => {
