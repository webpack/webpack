...
const collectImportMetaEnvDefinitions = (compilation) => {
	...
	return { stringify: `{${pairs.join(",")}}`, env };
};
				...
				const parserHandler = (parser, { importMeta }) => {
					...
					parser.hooks.expression
						...
						.tap(PLUGIN_NAME, (expr, rawExpr) => {
							...
							if (
								rawExpr &&
								rawExpr.type === "MemberExpression" &&
								/** @type {Identifier} */ (rawExpr.property).name !== "env"
							) {
								const name = /** @type {Identifier} */ (rawExpr.property).name;
