	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				 ...
				 * @returns {void}
				 */
				const parserHandler = (parser, { importMeta, importMetaResolve }) => {
					if (importMeta === false) {
								...
								switch (prop.id) {
									...
									case "resolve":
										if (importMetaResolve) {
											str += "resolve: import.meta.resolve,";
											break;
										}
									// falls through
									default:
					...
					if (importMetaResolve) {
						...
						parser.hooks.evaluateIdentifier.for("import.meta.resolve").tap(
							PLUGIN_NAME,
							evaluateToIdentifier(
								"import.meta.resolve",
								"import.meta",
						...
						parser.hooks.call
							...
							.tap(PLUGIN_NAME, (expr) => {
								...
								const evaluated = parser.evaluateExpression(arg1);
								const request = evaluated && evaluated.asString();

								if (!request) {
									if (compilation.outputOptions.module) return true;
									return;
								}
