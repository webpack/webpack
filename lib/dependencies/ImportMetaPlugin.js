...
const BuildTimeConstDependency = require("./BuildTimeConstDependency");
const ConstDependency = require("./ConstDependency");
const ImportMetaMainDependency = require("./ImportMetaMainDependency");
const ModuleInitFragmentDependency = require("./ModuleInitFragmentDependency");
const ImportMetaResolveDependency = require("./url/ImportMetaResolveDependency");
...
/** @import { MergedDefinitionNode } from "../define/DefinePlugin" */
/** @import { ValueCacheVersion } from "../module/Module" */
/** @import NormalModule, { NormalModuleBuildInfo } from "../module/NormalModule" */
/**
 * @import {
 ...
 */
/** @import { RawRuntimeRequirements } from "./NullDependency" */
...
// The `import.meta` accesses that exist only in ES modules, and so mark one.
// `dirname` and `filename` are resolved by NodeStuffPlugin. The webpack pragmas
// are excluded: they work in CommonJS too and must not force ESM.
...
// WHY: the ordering contract for the `import.meta` parser hooks across plugins.
// ESM detection (HarmonyDetectionParserPlugin, non-bailing) must observe every
// access before any bailing replacement, and the merged replacements here must
// run before DefinePlugin's object defines (default stage 0) and its nested
// destructuring handler (stage 100).
...
/**
 * Renders the `import.meta.main` comparison, which reads the module argument a
 * scope-hoisted module does not have. A caller that can answer the expression
 * statically for an inner module rules out only the concatenation root.
 * @param {Parser} parser the parser
 * @param {boolean} staticWhenInner whether an inner concatenated module answers it without the comparison
 * @returns {string} the comparison expression
 */
const renderImportMetaMain = (parser, staticWhenInner) => {
	const currentModule = parser.state.module;
	const buildInfo =
		/** @type {JavascriptModuleBuildInfo} */
		(currentModule.buildInfo);
	if (staticWhenInner) {
		buildInfo.moduleConcatenationRootBailout = IMPORT_META_MAIN;
	} else {
		buildInfo.moduleConcatenationBailout = IMPORT_META_MAIN;
	}
	return `${RuntimeGlobals.moduleCache}[${RuntimeGlobals.entryModuleId}] === ${currentModule.moduleArgument}`;
};

const IMPORT_META_VAR = "__webpack_import_meta__";

/**
 * The name of the module's synthesized `import.meta` binding, stepped aside when
 * the source already spells it. Re-bundling webpack output is the case that
 * matters: the earlier binding would shadow this one and make it initialize
 * itself.
 * @param {Parser} parser the parser
 * @returns {string} a name no declaration in this module takes
 */
const importMetaVarName = (parser) => {
	const { source } = parser.state;
	if (!source || !source.includes(IMPORT_META_VAR)) return IMPORT_META_VAR;
	let index = 2;
	while (source.includes(`__webpack_import_meta_${index}__`)) index++;
	return `__webpack_import_meta_${index}__`;
};
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.dependencyTemplates.set(
					...
					new BuildTimeConstDependency.Template()
				);
				compilation.dependencyTemplates.set(
					ImportMetaMainDependency,
					new ImportMetaMainDependency.Template()
				);
				...
				const parserHandler = (parser, { importMeta, url }) => {
					...
					/**
					 * Import meta runtime property for build-time execution.
					 * @param {string} field field name
					 * @returns {string} generated property
					 */
					const importMetaBuildTimeProperty = (field) =>
						`[${JSON.stringify(field)}]: ${Template.toNormalComment(
					...
					parser.hooks.expression
						...
						.tap(PLUGIN_NAME, (metaProperty) => {
							...
							if (!referencedPropertiesInDestructuring) {
								const varName = importMetaVarName(parser);
								const knownProps = [];
								...
								if (isImportMetaFieldEnabled(importMeta, "main")) {
									knownProps.push(
										`main: ${renderImportMetaMain(parser, false)}`
									);
								}
								if (isImportMetaFieldEnabled(importMeta, "env")) {
								...
								const plainInitCode = `var ${varName} = {${knownProps.join(
									", "
								)}};\n`;
								// Each module owns its `import.meta`: it copies the bundle's one
								// and inherits from it, so later host writes stay visible.
								const initCode = preserveUnknown(importMeta)
									? `var ${varName} = Object.assign(Object.create(import.meta), import.meta, {${knownProps.join(
											", "
										)}});\n`
									: plainInitCode;
								/** @type {RawRuntimeRequirements} */
								...
								const initDep = new ModuleInitFragmentDependency(
									...
									varName,
									// native `import.meta` does not parse in the build-time vm
									initCode === plainInitCode ? undefined : plainInitCode
								);
							...
							let strBuildTime = "";
							/**
							 * Appends a property shared by both variants.
							 * @param {string} code generated property
							 */
							const append = (code) => {
								str += code;
								strBuildTime += code;
							...
							/**
							 * Appends a property reading native `import.meta`, which
							 * does not parse in the build-time vm.
							 * @param {string} field field name
							 */
							const appendRuntimeProperty = (field) => {
								str += importMetaRuntimeProperty(field);
								strBuildTime += importMetaBuildTimeProperty(field);
							};
							for (const prop of referencedPropertiesInDestructuring) {
								...
								switch (prop.id) {
									case "url":
										if (fieldEnabled) {
											append(`url: ${importMetaUrl()},`);
										} else {
											appendRuntimeProperty(prop.id);
										}
										break;
									case "webpack":
										if (fieldEnabled) {
											append(`webpack: ${importMetaWebpackVersion()},`);
										} else {
											appendRuntimeProperty(prop.id);
										}
										break;
									case "main":
										if (fieldEnabled) {
											append(`main: ${renderImportMetaMain(parser, false)},`);
									...
									default: {
										...
										if (node !== undefined) {
											addDefinitionValueDependencies();
											append(
												`${DefinePlugin.toPropertyKey(prop.id)}: ${renderDefinition(
													node,
													`${IMPORT_META}.${prop.id}`
												)},`
											);
					...
					if (isImportMetaFieldEnabled(importMeta, "main")) {
						parser.hooks.expression
							...
							.tap(PLUGIN_NAME, (expr) => {
								const dep = new ImportMetaMainDependency(
									renderImportMetaMain(parser, true),
									"false",
									/** @type {Range} */ (expr.range),
									[
										RuntimeGlobals.moduleCache,
					...
					parser.hooks.unhandledExpressionMemberChain
						...
						.tap(PLUGIN_NAME, (expr, members) => {
							...
							if (
								...
								// keep the native access verbatim for emitted output, but
								// native `import.meta` does not parse in the build-time vm
								const dep = new BuildTimeConstDependency(
									`${IMPORT_META}${propertyAccess(members, 0)}`,
...
ImportMetaPlugin.IMPORT_META_DIRNAME = IMPORT_META_DIRNAME;
ImportMetaPlugin.IMPORT_META_FILENAME = IMPORT_META_FILENAME;
ImportMetaPlugin.IMPORT_META_NAMES = IMPORT_META_NAMES;
ImportMetaPlugin.IMPORT_META_STAGE_ESM_DETECTION =
	IMPORT_META_STAGE_ESM_DETECTION;
ImportMetaPlugin.isImportMetaFieldEnabled = isImportMetaFieldEnabled;
