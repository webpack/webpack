 ...
 */
const createRequireCallHandlerFactory = (parser, options, getContext) => {
	/**
...
class CommonJsImportsParserPlugin {
	...
	apply(parser) {
		 ...
		 */
		const createRequireCallHandler = createRequireCallHandlerFactory(
			parser,
			options,
			getContext
		);
		...
		parser.hooks.new
			.for("require")
			.tap(PLUGIN_NAME, createRequireCallHandler(true));
		parser.hooks.call
			.for("module.require")
			.tap(PLUGIN_NAME, createRequireCallHandler(false));
		parser.hooks.new
