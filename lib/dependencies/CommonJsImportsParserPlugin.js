...
class CommonJsImportsParserPlugin {
	...
	apply(parser) {
		 ...
		 */
		const preDeclaratorHandler = (declarator, statement) => {
		...
		parser.registerColumnGate(
			...
			(_parser, store, did, declId) => {
				// the pre-walk only gates plain identifier bindings
				if (store.aux[declId] !== DECL_KIND_CONST) return false;
				...
				return (
					callee !== 0 &&
					store.types[callee] === SoaAst.TYPE_IDENTIFIER &&
					parser._soaIdentName(store, callee) === "require"
				);
			}
		);
