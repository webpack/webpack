...
class CommonJsImportsParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.expression
			.for("require")
			.tap("CommonJsImportsParserPlugin", requireHandler);
		parser.hooks.expression
			.for(createdRequireIdentifierTag)
			.tap("CommonJsImportsParserPlugin", requireHandler);
		//#endregion
		...
		parser.hooks.import.tap(
			...
			(statement, source) => {
				if (
					...
					statement.specifiers[0].imported.name !== "createRequire"
				)
					return;
		...
		parser.hooks.importSpecifier.tap(
			...
			(statement, source, id, name) => {
				if (source !== "module" || id !== "createRequire") return;
