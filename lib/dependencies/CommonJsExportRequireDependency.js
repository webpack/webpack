...
class CommonJsExportRequireDependency extends ModuleDependency {
	...
	getExports(moduleGraph) {
		...
		// FlagDependencyExportsPlugin yet (no owned exports), so its
		...
		// across runtimes; defer — the `from.module` dependency re-queues us.
		if (
			importedModule &&
			importedModule.getExportsType(moduleGraph, false) === "namespace" &&
			moduleGraph.getExportsInfo(importedModule).ownedExports.next().done
		) {
