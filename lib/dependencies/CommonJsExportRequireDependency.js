...
class CommonJsExportRequireDependency extends ModuleDependency {
	...
	getExports(moduleGraph) {
		...
		// The imported ESM module's `"module.exports"` eligibility isn't settled
		// yet (its export set is being determined). Star-reexporting its namespace
		// now would leak names the monotonic exports merge can't retract once it
		// turns out to unwrap, making the result order-dependent across runtimes;
		// defer until the imported module's exports are known.
		if (
			importedModule &&
			importedModule.getExportsType(moduleGraph, false) === "namespace" &&
			moduleGraph
				.getExportsInfo(importedModule)
				.getReadOnlyExportInfo(ESM_MODULE_EXPORTS_NAME).provided === null
		) {
