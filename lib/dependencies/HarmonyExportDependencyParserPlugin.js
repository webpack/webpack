...
const { graphNodeSymbol } = require("../optimize/InnerGraphPlugin");
	...
	apply(parser) {
		...
		parser.hooks.exportExpression.tap(
			...
			(statement, expr) => {
				...
				if (graphNode && topLevelSymbol) {
					topLevelSymbol.addLink(graphNode);
				}
				return true;
			}
		);
		parser.hooks.exportSpecifier.tap(
			...
			(statement, id, name, idx) => {
				...
				const topLevelSymbol = parser.getTagData(id, graphNodeSymbol);
				const graphNode = InnerGraph.getNode(
					parser.state,
					statement,
					name,
