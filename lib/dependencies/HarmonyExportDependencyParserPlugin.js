...
/**
 * Records an export whose binding the language pins to a single value, so an
 * importer may read it once into a local variable.
 * @param {JavascriptParser} parser the parser
 * @param {string} name the exported name
 * @returns {void}
 */
const markStableExport = (parser, name) => {
	const buildInfo =
		/** @type {JavascriptModuleBuildInfo | undefined} */
		(parser.state.module.buildInfo);
	if (buildInfo === undefined) return;
	if (buildInfo.stableExports === undefined) {
		buildInfo.stableExports = new Set();
	}
	buildInfo.stableExports.add(name);
};
...
module.exports = class HarmonyExportDependencyParserPlugin {
	 ...
	 * @param {boolean} localImportBindings whether stable exports are recorded
	 */
	constructor(options, localImportBindings) {
		/** @type {JavascriptParserOptions} */
		this.options = options;
		this.localImportBindings = localImportBindings;
	 ...
	 */
	apply(parser) {
		const { exportPresenceMode, localImportBindings } = this;
		...
		parser.hooks.exportExpression.tap(PLUGIN_NAME, (statement, node) => {
			...
			// A named declaration keeps a binding the module body may reassign;
			// every other form lands in a const or a name only webpack writes.
			if (
				localImportBindings &&
				!(
					node.type.endsWith("Declaration") &&
				...
				markStableExport(parser, "default");
			}
			dep.setLocWithIndex(parser.getLocation(statement), -1);
			parser.state.current.addDependency(dep);
			getInnerGraphUtils(parser.state.compilation).addVariableUsage(
		...
		parser.hooks.exportSpecifier.tap(
			...
			(statement, id, name, idx) => {
				...
				// Only a top-level `const` carries the tag, and no later statement
				// of the module can point that binding somewhere else.
				if (localImportBindings && tagData) markStableExport(parser, name);
				// lazy barrel: defer the re-export at parse time when the importing module is side-effect-free
				const moduleFactoryMeta = parser.state.module.factoryMeta;
				if (
					settings &&
