...
ContextDependencyHelpers.create = function(Dep, range, param, expr, options, contextOptions) {
	...
	if(param.isTemplateString()) {
		...
		dep = new Dep(context, options.wrappedContextRecursive, regExp, range, valueRange, contextOptions);
		dep.loc = expr.loc;
		dep.replaces = [{
			range: prefixRange,
			value: prefix
		...
		dep = new Dep(options.exprContextRequest, options.exprContextRecursive, options.exprContextRegExp, range, param.range, contextOptions);
		dep.loc = expr.loc;
