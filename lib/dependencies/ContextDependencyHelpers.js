...
ContextDependencyHelpers.create = function(Dep, range, param, expr, options, chunkName, filter) {
	let dep;
	let prefix;
	let postfix;
	let prefixRange;
	...
	if(param.isTemplateString()) {
		...
		// If filter exists, don't attempt to merge prefix/postfix with filter. we've already pulled out the context
		// If there are more than two quasis, maybe the generated RegExp can be more precise?
		regExp = filter || new RegExp(`^${quotemeta(prefix)}${options.wrappedContextRegExp.source}${quotemeta(postfix)}$`);
