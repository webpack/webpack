...
class HtmlInlineStyleDependency extends ModuleDependency {
	...
	updateHash(hash, context) {
		// The dependency template substitutes the referenced CSS module's
		// rendered text (with `url(...)` already rewritten to hashed asset
		// filenames by `CssUrlDependency`) into the HTML at code-generation
		// time. The inline CSS body itself rides on the HTML source —
		// already covered by the HTML module's own `buildInfo.hash` — so
		// what we actually need to propagate here is changes that ride on
		// the CSS module's *dependencies* (e.g. an asset referenced via
		// `url(...)` whose bytes changed). Those changes don't move the
		// CSS module's `buildInfo.hash`; they move its module hash via
		// `CssUrlDependency.updateHash`. Recurse into the CSS module's
		// full `updateHash` so the same chain folds up into the HTML.
