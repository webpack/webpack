...
/** @typedef {import("../serialization/ObjectMiddleware").ObjectDeserializerContext<[Range, Range, string, DeclarationId | undefined, boolean, InlinedValue | undefined, string | undefined]>} ObjectDeserializerContext */
/** @typedef {import("../serialization/ObjectMiddleware").ObjectSerializerContext<[Range, Range, string, DeclarationId | undefined, boolean, InlinedValue | undefined, string | undefined]>} ObjectSerializerContext */
...
class HarmonyExportExpressionDependency extends NullDependency {
	...
	constructor(range, rangeStatement, prefix, declarationId, constValue) {
		...
		this._defaultExportLocalName = undefined;
	}
	...
	getExports(moduleGraph) {
		...
		// Nested property exports for compound component / object literal pattern
		const objectExportProperties =
			module && module.buildInfo && module.buildInfo.objectExportProperties;
		const localName =
			typeof this.declarationId === "string"
				? this.declarationId
				: this._defaultExportLocalName;
		if (objectExportProperties && localName) {
			const propNames = objectExportProperties.get(localName);
			if (propNames) {
	...
	serialize(context) {
		context
			...
			.write(this._defaultExportLocalName);
	...
	deserialize(context) {
		...
		const c6 = c5.rest;
		this._defaultExportLocalName = c6.read();
