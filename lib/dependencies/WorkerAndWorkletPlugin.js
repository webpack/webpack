...
const { WORKLET_MODULE } = require("./WorkletDependency");
				...
				const parserPlugin = (parser, parserOptions) => {
					...
					const handleNewWorklet = (expr) => {
						...
						const block = new AsyncDependenciesBlock({
							...
							circular: false,
							entryOptions: {
								// worklets can't load chunks at runtime; the bootstrap
								// pre-loads every chunk via `addModule`, so chunks use the
								// import-scripts format to self-register.
						...
						block.loc = expr.loc;
						// The dependency inserts the bootstrap wrapper before the call.
						...
						// Replace the argument list `(<url>)` with `(<module>); })`,
						// forwarding the wrapper param and closing the wrapper function
						// and the bootstrap call (opened by WorkletDependency).
						const callDep = new ConstDependency(`(${WORKLET_MODULE}); })`, [
							/** @type {Range} */ (expr.callee.range)[1],
							/** @type {Range} */ (expr.range)[1]
						]);
						callDep.loc = /** @type {DependencyLocation} */ (expr.loc);
						parser.state.module.addPresentationalDependency(callDep);
