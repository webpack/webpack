...
const { WORKLET_ARGS } = require("./WorkletDependency");
			...
			(compilation, { normalModuleFactory }) => {
				...
				const parserPlugin = (parser, parserOptions) => {
					...
					const handleNewWorklet = (expr) => {
						...
						// Replace `(<url>` with `(...args)` so the wrapper forwards the
						// original `addModule` arguments to the real call.
						const callDep = new ConstDependency(`(...${WORKLET_ARGS})`, [
						...
						for (let i = 1; i < expr.arguments.length; i++) {
							parser.walkExpression(
								/** @type {Expression} */ (expr.arguments[i])
							);
						}
