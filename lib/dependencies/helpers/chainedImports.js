...
module.exports.getTrimmedIdsAndRange = (
	...
	if (trimmedIds.length !== untrimmedIds.length) {
		// `dep.idRanges` is right-aligned with `dep.names`: the two may differ in
		// length, but their last elements correspond, which is what turns the count
		// of trimmed ids into the replacement range.
		const idx =
			ranges === undefined
		...
		if (idx < 0 || idx >= /** @type {Range[]} */ (ranges).length) {
			...
			// TODO in the next major release: remove the "trimmedIds = ids" above and uncomment the following line instead.
