...
const { ImportPhaseUtils, noModuleSourceMessage } = require("./ImportPhase");
...
ImportDependency.Template = class ImportDependencyTemplate extends (
	...
	/**
	 * A source phase import binds the module source object, so a module carrying
	 * none rejects with the `SyntaxError` the spec's GetModuleSource throws.
	 * @param {ImportDependency} dep the dependency
	 * @param {ModuleGraph} moduleGraph the module graph
	 * @returns {string | undefined} the rejecting expression, or undefined when the phase can be served
	 */
	getSourcePhaseRejection(dep, moduleGraph) {
		if (!ImportPhaseUtils.isSource(dep.phase)) return;
		const module = moduleGraph.getModule(dep);
		if (!module || module.hasModuleSource()) return;
		return `Promise.reject(new SyntaxError(${JSON.stringify(
			noModuleSourceMessage(dep.userRequest)
		)}))`;
	}
	...
	apply(
		...
		// The module is never loaded, let alone evaluated, when the phase it is
		// asked for cannot be served.
		let content = this.getSourcePhaseRejection(dep, moduleGraph);
		if (content === undefined) {
			content = runtimeTemplate.moduleNamespacePromise({
				chunkGraph,
				block,
			...
			// The source phase binds the module source object, which webpack
			// delivers as the module's default export.
