...
/** @typedef {Set<string> | Map<string, string[]> | false} UsedByExports */
/** @typedef {import("../serialization/ObjectMiddleware").ObjectDeserializerContext<[Range, UsedByExports]>} ObjectDeserializerContext */
/** @typedef {import("../serialization/ObjectMiddleware").ObjectSerializerContext<[Range, UsedByExports]>} ObjectSerializerContext */
	...
	_getRuntimeCondition(moduleGraph, runtime) {
		...
		if (usedByExports !== false) {
			...
			const runtimeCondition = filterRuntime(runtime, (runtime) => {
				if (usedByExports instanceof Map) {
					for (const [exportName, properties] of usedByExports) {
						const used = exportsInfo.getUsed(exportName, runtime);
						if (used === UsageState.Unused) continue;
						if (used !== UsageState.OnlyPropertiesUsed) return true;
						const exportInfo = exportsInfo.getReadOnlyExportInfo(exportName);
						const nestedInfo = exportInfo.getNestedExportsInfo();
						if (!nestedInfo) return true;
						for (const prop of properties) {
							if (nestedInfo.getUsed(prop, runtime) !== UsageState.Unused) {
								return true;
							}
						}
					}
