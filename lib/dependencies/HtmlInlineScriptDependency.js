...
const { makeHtmlChunkUrlSentinel } = require("../html/htmlChunkUrl");
...
HtmlInlineScriptDependency.Template = class HtmlInlineScriptDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		if (entrypoint) {
			...
			// Don't resolve the chunk filename here — code generation
			// runs before `chunk.hash` / `chunk.contentHash` are set, so
			// `[contenthash]` / `[chunkhash]` / `[fullhash]` placeholders
			// in `output.chunkFilename` would fail. Emit a sentinel that
			// `HtmlModulesPlugin#renderManifest` swaps for the resolved
			// `${PUBLIC_PATH_AUTO}<filename>` once chunk hashes are known
			// — done *before* the HTML's own `[contenthash]` is computed,
			// so the HTML invalidates whenever the inline-script entry
			// chunk's URL would change.
			url = makeHtmlChunkUrlSentinel(chunk, "javascript");
		}
