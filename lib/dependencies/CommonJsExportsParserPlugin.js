...
/** @type {Map<string, string[]>} per facade type: accessor keys serving child nodes */
const facadeChildKeysByType = new Map();
 ...
 * type is seen. SoA facades keep children behind non-enumerable prototype
 * getters, so their key set is collected from the prototype chain instead
 * (cached separately — both backends can serve the same type string).
 * @param {Node} node node
 * @returns {string[]} candidate child keys
 */
const getChildKeys = (node) => {
	if (SoaAst.isFacade(node)) {
		let keys = facadeChildKeysByType.get(node.type);
		if (keys === undefined) {
			...
			facadeChildKeysByType.set(node.type, keys);
		}
		return keys;
	}
...
const T_THIS_EXPRESSION = SoaAst.TYPE.ThisExpression;
const T_FUNCTION_EXPRESSION = SoaAst.TYPE.FunctionExpression;
const T_FUNCTION_DECLARATION = SoaAst.TYPE.FunctionDeclaration;
const T_STATIC_BLOCK = SoaAst.TYPE.StaticBlock;
const T_PROPERTY_DEFINITION = SoaAst.TYPE.PropertyDefinition;
const T_FOR_STATEMENT = SoaAst.TYPE.ForStatement;
 ...
 * `this`-scope boundary is enforced when an entry is popped, so both branches
 * may enumerate any child without crossing it. SoA-backed subtrees descend on
 * the column refs (the kid slots and the list span are exactly the owned
 * children), materializing only registered nodes — whose children may be
 * foreign or mutated — property definitions and the `this` result itself.
...
const findOwnThisExpression = (initialNodes) => {
	...
	let store;
	/**
	 * All facades of one parse share one store, so bare ids on the stack are
	 * unambiguous — the first store-backed node seen adopts it.
	 * @param {Node} node child node
	 */
	const push = (node) => {
		const nodeStore = /** @type {EXPECTED_ANY} */ (node)[SOA_KEY_STORE];
		if (
			nodeStore !== undefined &&
			(store === undefined || nodeStore === store)
		) {
			store = nodeStore;
			stack.push(/** @type {EXPECTED_ANY} */ (node)[SOA_KEY_ID]);
		} else {
			stack.push(node);
		}
	};
	for (const node of initialNodes) push(node);
	/** @type {Node | number | undefined} */
	let entry;
	while ((entry = stack.pop()) !== undefined) {
		if (typeof entry === "number") {
			const type = store.types[entry];
			if (type === T_THIS_EXPRESSION) {
				return /** @type {ThisExpression} */ (store.nodeAt(entry));
			}
			if (
				type === T_FUNCTION_EXPRESSION ||
			...
			if (
				type === T_PROPERTY_DEFINITION ||
				store.facades[entry] !== undefined
			) {
				stack.push(/** @type {Node} */ (store.nodeAt(entry)));
				continue;
			}
			const kid0 = store.kid0[entry];
			if (kid0 !== 0) stack.push(kid0);
			const kid1 = store.kid1[entry];
			if (kid1 !== 0) stack.push(kid1);
			const kid2 = store.kid2[entry];
			if (kid2 !== 0) stack.push(kid2);
			if (type === T_FOR_STATEMENT) {
				// the only 4-fixed-children type: `body` overflows into `aux`
				const bodyId = store.aux[entry];
				if (bodyId !== 0) stack.push(bodyId);
			}
			const len = store.listLens[entry];
			if (len !== 0) {
				const start = store.listStarts[entry];
				for (let i = 0; i < len; i++) {
					const cid = store.flat[start + i];
					if (cid !== 0) stack.push(cid);
				}
			}
			continue;
		}
		...
		switch (node.type) {
			...
			case "PropertyDefinition":
				if (node.computed) push(/** @type {EXPECTED_ANY} */ (node).key);
				break;
			default: {
				const children =
