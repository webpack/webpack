...
const getAnalyzableUrlSpecifier = (dep, templateContext) => {
	...
	if (urlData && jsUrl === undefined) {
		...
		// placeholder only those assets are rendered with — reaching javascript it would
		// never resolve, so the name below is what the literal is built from instead.
		if (
...
class URLDependency extends ModuleDependency {
	...
	updateHash(hash, context) {
		...
		if (runtimeTemplate === undefined) return;
		const base = runtimeTemplate.entryBaseUri(runtime);
		// Entries that disagree keep the runtime form, which is different code again from
		// the base-less literal `undefined` bakes — so the two must not hash alike.
		// Tagged apart so an empty base cannot spell the same thing as no answer at all.
