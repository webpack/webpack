...
URLDependency.Template = class URLDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Build-time execution keeps the runtime form too: its vm wrapper is a script,
		// where `import.meta` is a syntax error, and the code is never emitted.
		if (
			!dep.relative &&
		...
		// A wrapper-less asset (`asset-url` only) has no code to run at build
		// time; its exports are emulated from `__webpack_require__.p` by
		// `AssetModulesPlugin`, so make sure the publicPath runtime is there.
		if (
			chunkGraph.buildTimeExecution &&
			module &&
		...
		// the time user code reaches this line. They are browser markup, so
		// build-time execution skips them (no DOM, no sealed codegen results).
