...
/** @typedef {import("../serialization/ObjectMiddleware").ObjectDeserializerContext<[Range, boolean, EntryOptions | undefined, UsedByExports | undefined, true | undefined, true | undefined, ("high" | "low" | "auto" | undefined), string | undefined, string | undefined, string | undefined]>} ObjectDeserializerContext */
/** @typedef {import("../serialization/ObjectMiddleware").ObjectSerializerContext<[Range, boolean, EntryOptions | undefined, UsedByExports | undefined, true | undefined, true | undefined, ("high" | "low" | "auto" | undefined), string | undefined, string | undefined, string | undefined]>} ObjectSerializerContext */
...
class URLDependency extends ModuleDependency {
	...
	updateHash(hash, context) {
		...
		// Entry vs asset picks different runtime helpers; promote() is
		// per-compilation and must still reach the module hash.
		if (runtimeTemplate !== undefined && runtimeTemplate.compilation) {
			const parentBlock =
				runtimeTemplate.compilation.moduleGraph.getParentBlock(this);
			if (
				parentBlock instanceof URLDependenciesBlock &&
				parentBlock.promotion
			) {
				hash.update(
					parentBlock.promotion.chunkFilenameGlobal ||
	...
	deserialize(context) {
		...
		const c4 = c3.rest;
		this.prefetch = c4.read();
		const c5 = c4.rest;
		this.preload = c5.read();
		const c6 = c5.rest;
		this.fetchPriority = c6.read();
		const c7 = c6.rest;
		this.asAttribute = c7.read();
		const c8 = c7.rest;
		this.typeAttribute = c8.read();
		const c9 = c8.rest;
		this.mediaAttribute = c9.read();
...
URLDependency.Template = class URLDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Entry-block mode: promotion is compilation-local and not serialized.
		const parentBlock = moduleGraph.getParentBlock(dep);
		const urlBlock =
			parentBlock instanceof URLDependenciesBlock ? parentBlock : undefined;
		const isEntryUrl = Boolean(urlBlock && urlBlock.promotion);
		...
		// Entry URLs keep the runtime form so the chunk filename helper is used.
		if (
			!isEntryUrl &&
			!dep.relative &&
		...
		/** @type {string} */
		let urlExpr;
		if (isEntryUrl && urlBlock) {
			const entrypoint =
				/** @type {import("../graph/Entrypoint")} */
				(chunkGraph.getBlockChunkGroup(urlBlock));
			const chunk = entrypoint.getEntrypointChunk();
			const getChunkFilename =
				(urlBlock.promotion && urlBlock.promotion.chunkFilenameGlobal) ||
				RuntimeGlobals.getChunkScriptFilename;
