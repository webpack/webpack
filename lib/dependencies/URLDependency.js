...
const getAnalyzableUrlSpecifier = (dep, templateContext) => {
	...
	if (typeof jsUrl === "string") {
		...
		if (jsUrl.includes(RuntimeGlobals.require)) {
			...
			// The asset filename is already final (its content hash is known at code
			// generation); the publicPath prefix (static, or a baked `[fullhash]`) yields
			// the analyzable literal.
			return runtimeTemplate._applyAnalyzablePublicPath(
				undefined,
				filename,
				consumingModule,
				chunkGraph,
