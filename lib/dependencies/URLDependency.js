	...
	if (typeof jsUrl === "string") {
		...
		if (jsUrl.includes(RuntimeGlobals.require)) {
			...
			if (typeof publicPath === "string") {
				...
				// A publicPath resolvable from the compilation hash alone: bake the
				// post-hash placeholder, substituted by AnalyzableEsmPlaceholderPlugin.
				if (
					isFullHashOnlyTemplate(publicPath) &&
					runtimeTemplate._getPostHashEmbedMode(consumingModule, chunkGraph) !==
						0
				) {
