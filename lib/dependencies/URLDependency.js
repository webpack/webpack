...
class URLDependency extends ModuleDependency {
	...
	updateHash(hash, context) {
		...
		// Entry vs asset picks different runtime helpers; promotion is
		...
		if (runtimeTemplate !== undefined && runtimeTemplate.compilation) {
			...
			if (promotion) {
				hash.update(
					promotion.chunkFilenameGlobal || RuntimeGlobals.getChunkScriptFilename
				);
			} else if (this.entryChunkFilenameGlobal !== undefined) {
...
URLDependency.Template = class URLDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Entry-block mode: parent URLDependenciesBlock.promotion for this compilation.
		const parentBlock = moduleGraph.getParentBlock(dep);
		const promotion =
			parentBlock instanceof URLDependenciesBlock
				? parentBlock.promotion
				: undefined;
		const isEntryUrl = Boolean(promotion);
		...
		/** @type {string} */
		let urlExpr;
		if (isEntryUrl && parentBlock instanceof URLDependenciesBlock) {
			const entrypoint =
				/** @type {import("../Entrypoint")} */
				(chunkGraph.getBlockChunkGroup(parentBlock));
			const chunk = entrypoint.getEntrypointChunk();
			const getChunkFilename =
				(promotion && promotion.chunkFilenameGlobal) ||
