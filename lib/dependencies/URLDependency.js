...
class URLDependency extends ModuleDependency {
	...
	updateHash(hash, context) {
		...
		// A relative reference keeps the runtime form, which reads `.b` when it runs and
		// so is the same text under every base. Asked of the template rather than
		// restated, so a base that cannot reach the code cannot reach the hash — and
		// cost every build setting one its code generation cache.
		if (
