...
CommonJsSelfReferenceDependency.Template = class CommonJsSelfReferenceDependencyTemplate extends (
	...
	apply(
		dependency,
		source,
		{ module, moduleGraph, runtime, runtimeRequirements, runtimeTemplate }
	) {
		const dep = /** @type {CommonJsSelfReferenceDependency} */ (dependency);
		const isGlobalUsedExports =
			runtimeTemplate.compilation.options.optimization.usedExports === "global";
		let used;
		if (dep.names.length === 0) {
			...
			used = moduleGraph
				...
				.getUsedName(dep.names, runtime, isGlobalUsedExports);
		}
		if (!used) {
			throw new Error(
