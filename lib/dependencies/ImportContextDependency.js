...
ImportContextDependency.Template = class ImportContextDependencyTemplate extends (
	...
	replaceCall(dep, source, moduleExports, templateContext) {
		...
		// The spec evaluates the request, then the options, then checks them, so
		// both stay as arguments in that order. Their own source is left in
		// place, so nested dependencies in them still get their replacements.
