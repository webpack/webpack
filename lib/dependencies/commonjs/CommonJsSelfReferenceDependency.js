...
makeSerializable(
	...
	"webpack/lib/dependencies/commonjs/CommonJsSelfReferenceDependency"
);
...
CommonJsSelfReferenceDependency.Template = class CommonJsSelfReferenceDependencyTemplate extends (
	...
	apply(
		...
		if (base === dep.base && equals(used, dep.names)) {
			// Nothing has to change, and no replacement is emitted: plugins that
			// update `module._source`, which they should not, depend on that.
			return;
		}
