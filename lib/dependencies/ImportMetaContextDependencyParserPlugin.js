	...
	apply(parser) {
		...
		parser.hooks.call
			...
			.tap(PLUGIN_NAME, (expr) => {
				...
				const dep = ImportMetaContextDependency.webpackContext(
					{
						request,
						include,
						exclude,
		...
		parser.hooks.call.for("import.meta.glob").tap(PLUGIN_NAME, (expr) => {
			const {
				...
				errors,
				warnings
			} = parseImportMetaGlobCall(expr, parser);
			...
			const dep = ImportMetaContextDependency.glob(
				{
					request: `${globOptions.baseDir}${globOptions.globQuery}`,
					recursive: globOptions.recursive,
					regExp: false,
					globPatterns: globOptions.globPatterns,
					globContext: globOptions.context,
					globImport: globOptions.globImport,
					globExhaustive: globOptions.globExhaustive,
