...
const {
	isDeclarableBindingName,
	isStableImport
} = require("../optimize/LocalImportBindings");
...
/** @import { GetConditionFn, ReferencedExports, UpdateHashContext } from "../Dependency" */
...
HarmonyImportSpecifierDependency.Template = class HarmonyImportSpecifierDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		if (dep.shorthand) {
			// A local binding renders as the name already written there, so the
			// shorthand stays shorthand.
			if (exportExpr !== dep.name) {
	...
	_getCodeForIds(dep, source, templateContext, ids, connection) {
		...
		if (
			...
			const localBinding = this._getLocalBinding(
				dep,
				templateContext,
				ids,
				connection
			...
			// Only a bare reference reads exactly what the binding names.
			if (
				dep.directImport === true &&
				isImportBindingScopesEnabled(moduleGraph)
			) {
				// A call site drops the `this` a member access would bind, which is
				// a statement's concern rather than the binding's value.
				const value = dep.call
				...
				if (!value.startsWith(";")) {
					...
					(templateContext.importBindings).set(dep.name, value);
				}
			}
		}
		return exportExpr;
	...
	/**
	 * Declares the imported value under the name the source gave it and returns
	 * that name, for the references where a local variable is observationally
	 * equal to the live binding. Returns undefined to leave the reference reading
	 * through the namespace.
	 * @param {HarmonyImportSpecifierDependency} dep dependency
	 * @param {DependencyTemplateContext} templateContext context
	 * @param {Ids} ids ids
	 * @param {ModuleGraphConnection | undefined} connection the resolved connection for dep
	 * @returns {string | undefined} the local name, or undefined when not applicable
	 */
	_getLocalBinding(dep, templateContext, ids, connection) {
		const {
			moduleGraph,
			module,
			runtime,
		...
		// Only a reference that names the binding on its own can become the
		// binding; a member access off it reads a path this never declares.
		if (dep.directImport !== true || dep.weak) return;
		if (
			ImportPhaseUtils.isDefer(dep.phase) ||
		...
		if (!isDeclarableBindingName(dep.name)) return;
		// Inside a heritage clause the class' own binding shadows this name and is
		// in its temporal dead zone, so only a reference outside one can be local.
		const originBuildInfo =
			/** @type {JavascriptModuleBuildInfo | undefined} */
			(/** @type {Module} */ (module).buildInfo);
		if (originBuildInfo !== undefined && dep.range) {
			const heritageRanges =
				originBuildInfo.classHeritageRanges !== undefined
					? originBuildInfo.classHeritageRanges.get(dep.name)
					: undefined;
			if (heritageRanges !== undefined) {
				const [start, end] = dep.range;
				for (const [heritageStart, heritageEnd] of heritageRanges) {
					if (start >= heritageStart && end <= heritageEnd) return;
				}
			}
		}
		const importedModule = connection && connection.module;
		...
		// An inlined export renders as its literal, and holding it in a binding
		// would keep alive the import that inlining exists to drop.
		if (
			isInlineExportsEnabled(moduleGraph) &&
			isExportInlined(moduleGraph, importedModule, ids, runtime)
		) {
			return;
		}

		const value = runtimeTemplate.exportFromImport({
			moduleGraph,
			module: importedModule,
			chunkGraph,
			request: dep.request,
			exportName: ids,
			originModule: /** @type {Module} */ (module),
			asiSafe: true,
			isCall: false,
			callContext: false,
			defaultInterop: true,
			...
			dependency: dep,
			mangleableNamespace:
				ids.length === 0 && !dep.referencedPropertiesInDestructuring
		});
		initFragments.push(
			new ConditionalInitFragment(
				`/* harmony import binding */ var ${dep.name} = ${value};\n`,
				InitFragment.STAGE_HARMONY_IMPORT_BINDINGS,
				/** @type {number} */ (dep.sourceOrder),
				`harmony import binding ${dep.name}`,
				filterRuntime(runtime, (r) => connection.isTargetActive(r))
			)
		);
		// The name now resolves on its own, so a map need not say what it reads.
		if (isImportBindingScopesEnabled(moduleGraph)) {
			/** @type {Map<string, string>} */
			(templateContext.importBindings).delete(dep.name);
		}
		return dep.name;
	}
};
