...
class HarmonyImportSpecifierDependency extends HarmonyImportDependency {
	...
	getReferencedExports(moduleGraph, runtime) {
		// Source-phase imports of JavaScript modules bind to a ModuleSource
		// reflection without evaluating the imported module, so they do not
...
HarmonyImportSpecifierDependency.Template = class HarmonyImportSpecifierDependencyTemplate extends (
	...
	_getCodeForIds(dep, source, templateContext, ids) {
		...
		// `import source X from "./js.js"` binds X directly to the ModuleSource
		// reflection emitted by HarmonyImportDependency.Template. The leading
		// "default" id only reflects the surface syntax (ImportDefaultSpecifier);
		// for source-phase JS imports the binding is the import variable itself,
		// so strip that leading id and emit a member chain on the import var.
		if (
			...
			const memberIds = ids[0] === "default" ? ids.slice(1) : ids;
			return memberIds.length === 0
				? importVar
				: `${importVar}${propertyAccess(memberIds)}`;
		}
