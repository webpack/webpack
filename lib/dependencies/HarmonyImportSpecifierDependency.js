...
class HarmonyImportSpecifierDependency extends HarmonyImportDependency {
	constructor(
		...
		this.moduleScopeAnalyser = undefined; // for deep scope analysis
	}
	...
	getReference() {
		...
		const { usedExports } = this.originModule;
		if (
			usedExports &&
			this.moduleScopeAnalyser &&
			this._id &&
			!this.namespaceObjectAsContext &&
			Array.isArray(usedExports)
		) {
			const exportInfo = this.moduleScopeAnalyser.generateExportInfo(
				usedExports
			);
			if (this.request in exportInfo) {
				const names = exportInfo[this.request];
				if (names.indexOf(this._id) >= 0) {
					return new DependencyReference(this._module, [this._id], false);
				} else {
					return null;
				}
			} else {
