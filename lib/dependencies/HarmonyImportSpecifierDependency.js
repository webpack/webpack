...
HarmonyImportSpecifierDependency.Template = class HarmonyImportSpecifierDependencyTemplate extends (
	...
	_getLocalBinding(dep, templateContext, ids, connection) {
		...
		// A class heritage clause reads the class' own binding, which is in its
		// temporal dead zone there, so only a name user code cannot bind survives.
		const originBuildInfo =
			/** @type {JavascriptModuleBuildInfo | undefined} */
			(/** @type {Module} */ (module).buildInfo);
		if (
			originBuildInfo !== undefined &&
			originBuildInfo.classHeritageNames !== undefined &&
			originBuildInfo.classHeritageNames.has(dep.name)
		) {
			return;
		}
