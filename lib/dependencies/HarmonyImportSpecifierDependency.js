...
class HarmonyImportSpecifierDependency extends HarmonyImportDependency {
	...
	getCondition(moduleGraph) {
		...
		return (connection, runtime) => {
			...
			const { isExportInlined } = getInlineExports();
			if (
				ids.length > 0 &&
...
HarmonyImportSpecifierDependency.Template = class HarmonyImportSpecifierDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		const ids = dep.getIds(moduleGraph);
		const { InlinedUsedName, isInlineEnabled } = getInlineExports();

		// Only render declaration for import specifier when the dependency is conditional.
		// Inlined imports also produce inactive connections — render the literal first.
		if (connection && !connection.isTargetActive(runtime)) {
			if (ids.length > 0 && isInlineEnabled(connection.module)) {
				const used = moduleGraph
					.getExportsInfo(connection.module)
					.getUsedName(ids, runtime);
				if (used instanceof InlinedUsedName) {
					const inlinedExpr = used.render(
					...
					if (dep.shorthand) {
						source.insert(trimmedRangeEnd, `: ${inlinedExpr}`);
					} else {
						source.replace(trimmedRangeStart, trimmedRangeEnd - 1, inlinedExpr);
					}
					return;
				}
			}
