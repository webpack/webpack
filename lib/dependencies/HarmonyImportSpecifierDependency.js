	...
	_getCodeForIds(dep, source, templateContext, ids, connection) {
		...
		if (
			...
			if (ids.length === 0) {
				...
				if (dep.assignment) {
					// the written name, which for a namespace member is the member
					const name = ids.length === 0 ? dep.name : ids[ids.length - 1];
					const holder = `{ get ${name}() { return ${exportExpr}; } }`;
					exportExpr = `${dep.asiSafe ? "" : ";"}(${holder}).${name}`;
				}
			}
		} else {
