...
class HarmonyImportSpecifierDependency extends HarmonyImportDependency {
	...
	_getReferencedExportsInDestructuring(ids) {
		if (this.referencedPropertiesInDestructuring) {
			/** @type {string[][]} */
			const refs = [];
			for (const idsInDestructuring of this._getReferencedIdsInDestructuring(
				this.referencedPropertiesInDestructuring
			)) {
				refs.push(ids ? ids.concat(idsInDestructuring) : idsInDestructuring);
			}
			return refs;
		} else {
	 ...
	 * @returns {string[][]} all the referenced ids
	 */
	_getReferencedIdsInDestructuring(properties) {
		/** @type {string[][]} */
		const allIds = [];
...
HarmonyImportSpecifierDependency.Template = class HarmonyImportSpecifierDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		if (dep.referencedPropertiesInDestructuring) {
			for (let {
				...
				shorthand,
				range
			} of this._getReplacementsInDestructuring(
				dep.referencedPropertiesInDestructuring
			)) {
				const concatedIds = ids.concat(idsInDestructuring);
	 ...
	 * @returns {{ ids: string[], range: Range | undefined, shorthand: boolean | string }[]} replacements
	 */
	_getReplacementsInDestructuring(properties) {
