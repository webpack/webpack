...
const getLocalIdent = (local, module, chunkGraph, runtimeTemplate) => {
	...
	// When a CSS module is a concatenation inner module, chunkGraph.getModuleId()
	// may return null during code generation. Fall back to a plain object with
	// readableIdentifier as id so [id] in localIdentName can be resolved.
	const moduleForInterpolate =
		chunkGraph.getModuleId(module) !== null
			? module
			: /** @type {ModulePathData} */ ({
					id: module.readableIdentifier(runtimeTemplate.requestShortener),
					hash:
						module.buildInfo && module.buildInfo.hash
