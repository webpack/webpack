...
const getLocalIdent = (local, module, chunkGraph, runtimeTemplate) => {
	...
	let localIdent = interpolate(localIdentName, {
		prepareId: (id) => {
			...
			return (
				id
					.replace(/^([.-]|[^a-z0-9_-])+/i, "")
					// We keep the `@` symbol because it can be used in the package name (e.g. `@company/package`) and it we will replace it on `_` a class conflict may occur.
	...
	if (typeof localIdentName === "string" && /\[local\]/.test(localIdentName)) {
		localIdent = localIdent.replace(/\[local\]/g, local);
	}

	if (
		typeof localIdentName === "string" &&
		/\[uniqueName\]/.test(localIdentName)
	) {
