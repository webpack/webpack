...
CssIcssExportDependency.Template = class CssIcssExportDependencyTemplate extends (
	 ...
	 * @returns {string[] | undefined} final names
	 */
	static resolveReferences(name, templateContext) {
		const { module, moduleGraph, runtime } = templateContext;
		const importDeps =
			/** @type {CssIcssImportDependency[]} */
			(
				module.dependencies.filter(
					(d) => d instanceof CssIcssImportDependency && d.value === name
				)
			);

		if (importDeps.length === 0) return undefined;

		/** @type {string[]} */
		const references = [];

		for (const importDep of importDeps) {
			const connection = moduleGraph.getConnection(importDep);
			if (!connection || !connection.module) return undefined;

			const exportsInfo = moduleGraph.getExportsInfo(connection.module);
			const exportInfo = exportsInfo.getExportInfo(importDep.name);
			const usedName = exportInfo.getUsedName(importDep.name, runtime);

			if (usedName) {
				const deepReferences = this.resolveReferences(usedName, {
					...templateContext,
					module: connection.module
				});

				if (deepReferences && deepReferences.length > 0) {
					references.push(...deepReferences);
				} else {
					references.push(
						CssIcssExportDependency.Template.getIdentifier(
							usedName,
							importDep,
							{
								...templateContext,
		...
		if (
			...
			for (const used of allNames) {
				if (dep.exportMode === CssIcssExportDependency.EXPORT_MODE.ONCE) {
					...
					let final = `${originalValue ? `${originalValue}${newValue ? " " : ""}` : ""}${newValue}`;

					const reference = CssIcssExportDependencyTemplate.resolveReferences(
						resolvedValue,
						templateContext
					);

					if (reference) {
						final = `${final} ${reference.join(" ")}`;
					}

					cssData.exports.set(used, final);
				}
			}
		} else if (
			dep.range &&
