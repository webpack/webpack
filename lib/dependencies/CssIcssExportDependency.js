...
/**
 * Ambiguous names can be exported by both a custom (dashed) identifier and a
 * plain one that rename differently; a requester resolves against its own
 * family, falling back to all matches when that family is absent.
 * @param {CssExportEntry[]} matches all entries with the requested name
 * @param {boolean} wantCustom whether the requester is a custom (dashed) identifier
 * @returns {CssExportEntry[]} entries the requester may resolve to
 */
const selectEntriesForRequester = (matches, wantCustom) => {
	// a single match resolves the same either way — skip the copy
	if (matches.length === 1) return matches;
	/** @type {CssExportEntry[] | undefined} */
	let selected;
	for (let i = 0; i < matches.length; i++) {
		const isCustom = matches[i].exportType === EXPORT_TYPE.CUSTOM_VARIABLE;
		if (isCustom === wantCustom) {
			if (selected === undefined) selected = [];
			selected.push(matches[i]);
		}
	}
	return selected === undefined ? matches : selected;
};
...
CssIcssExportDependency.Template = class CssIcssExportDependencyTemplate extends (
	 ...
	 * @param {boolean=} wantCustom prefer custom (dashed) entries over plain ones
	...
	static resolve(
		...
		const exportEntry =
			matches && selectEntriesForRequester(matches, wantCustom)[0];
	...
	static resolveReferences(entry, templateContext, seen, cache) {
		...
		if (Array.isArray(entry.value)) {
			...
			let matches = getExportIndex(cache, module).get(name);
			if (matches) {
				matches = selectEntriesForRequester(
					matches,
					entry.exportType === EXPORT_TYPE.CUSTOM_VARIABLE
				);
