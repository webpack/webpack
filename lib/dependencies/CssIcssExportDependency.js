...
/**
 * Get (or lazily build) the `name -> entries` index for `module`, merged across
 * every `CssIcssExportDependency` it owns, so composes resolution does a single
 * Map lookup instead of re-scanning `module.dependencies` per reference. Each
 * export dep's own index is cached on the dep, so a module composed/imported by
 * many others builds its index once per build rather than once per importer.
 * @param {ResolutionCache} cache resolution cache
...
const getExportIndex = (cache, module) => {
	...
	if (map === undefined) {
		/** @type {CssIcssExportDependency | undefined} */
		let single;
		/** @type {CssIcssExportDependency[] | undefined} */
		let many;
		for (const dep of module.dependencies) {
			if (!(dep instanceof CssIcssExportDependency)) continue;
			if (single === undefined) single = dep;
			else (many || (many = [single])).push(dep);
		}
		if (many === undefined) {
			// The usual case: one consolidated export dep per module — reuse its
			// cached index directly (callers only read it).
			map = single === undefined ? EMPTY_INDEX : single.getEntriesByName();
		} else {
			map = new Map();
			for (const dep of many) {
				for (const [name, entries] of dep.getEntriesByName()) {
