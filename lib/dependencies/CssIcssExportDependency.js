...
class CssIcssExportDependency extends NullDependency {
	...
	updateHash(hash, { chunkGraph }) {
		if (this._hashUpdate === undefined) {
			...
			this._hashUpdate = `exportsConvention|${JSON.stringify(
				names
			)}|localIdentName|${JSON.stringify(
				generator.options.localIdentName
			)}|value|${JSON.stringify(this.value)}|range|${JSON.stringify(
				this.range
			)}|interpolate|${this.interpolate}|exportMode|${
				this.exportMode
...
CssIcssExportDependency.Template = class CssIcssExportDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		if (
			...
			// Union of used (mangled) names and convention names — used names
			// first to keep emission order. Built without the intermediate
			// map/filter/spread arrays since this runs per export on a hot path.
			/** @type {Set<string>} */
			...
			for (const used of allNames) {
				if (dep.exportMode === CssIcssExportDependency.EXPORT_MODE.ONCE) {
					...
					cssData.exports.set(
						used,
						`${
							originalValue ? `${originalValue}${unescaped ? " " : ""}` : ""
						}${unescaped}`
					);
...
// Shared with the consolidated `CssExportsDependency`.
module.exports.computeInterpolatedIdentifier = computeInterpolatedIdentifier;
