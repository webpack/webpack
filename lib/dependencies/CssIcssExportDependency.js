...
const getLocalIdent = (local, module, chunkGraph, runtimeTemplate) => {
	...
	if (/\[(contenthash|hash)\]/.test(localIdentName)) {
		...
		const hashDigestLength =
			generator.options.localIdentHashDigestLength ||
			runtimeTemplate.outputOptions.hashDigestLength;

		const { hashFunction } = runtimeTemplate.outputOptions;

		// Reduces collisions - ported from `css-loader`
	...
	let localIdent = runtimeTemplate.compilation
		...
		// Remove the first `_` from module id
		.replace(/^_/, "");
	...
	return (
		localIdent
			...
			.replace(/^((-?[0-9])|--)/, "_$1")
			// Avoid unssuported characters at all
			.replace(FILENAME_RESERVED_REGEX, "-")
	);
};
...
CssIcssExportDependency.Template = class CssIcssExportDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		if (templateContext.type === JAVASCRIPT_TYPE) {
