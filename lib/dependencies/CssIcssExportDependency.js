...
CssIcssExportDependency.Template = class CssIcssExportDependencyTemplate extends (
	 ...
	 */
	static resolve(localName, importName, templateContext) {
		...
		if (Array.isArray(value)) {
			return this.resolve(value[0], value[1], {
				...templateContext,
				module
			});
		}
	 ...
	 */
	static resolveReferences(dep, templateContext) {
		/** @type {string[]} */
		const references = [];
				...
				if (d instanceof CssIcssExportDependency && d.name === dep.value[1]) {
					if (Array.isArray(d.value)) {
						const deepReferences =
							CssIcssExportDependencyTemplate.resolveReferences(d, {
								...templateContext,
								module
							});
	...
	apply(dependency, source, templateContext) {
		...
		if (
			...
			if (dep.exportType === CssIcssExportDependency.EXPORT_TYPE.COMPOSES) {
				const references = CssIcssExportDependencyTemplate.resolveReferences(
					dep,
					templateContext
				);
