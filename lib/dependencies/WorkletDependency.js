...
WorkletDependency.Template = class WorkletDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// chunks pushed before it (see ImportScriptsChunkLoadingRuntimeModule).
		...
		if (runtimeChunk && runtimeChunk.id !== null) {
			chunkIds.push(runtimeChunk.id);
		}
		...
		const code = Template.asString([
			"(/* worklet bootstrap */ async function(__webpack_worklet_add__, __webpack_worklet_opts__) {",
			Template.indent([
				`await __webpack_worklet_add__(${RuntimeGlobals.getWorkletBootstrap}(), __webpack_worklet_opts__);`,
				`var __webpack_worklet_files__ = [${files}];`,
				"for (var __webpack_worklet_i__ = 0; __webpack_worklet_i__ < __webpack_worklet_files__.length; __webpack_worklet_i__++) {",
				Template.indent([
					`await __webpack_worklet_add__(new URL(__webpack_worklet_files__[__webpack_worklet_i__], ${RuntimeGlobals.baseURI}), __webpack_worklet_opts__);`
				]),
				"}"
			]),
			`})((...${WORKLET_ARGS}) => `
