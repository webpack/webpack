 ...
 */
const staticOptionKeyFromPropName = (propName, parser) => {
 ...
 */
const staticGlobPatternsFromExpr = (expr, parser) => {
	...
	for (const elem of expr.elements) {
		if (!elem || elem.type === "SpreadElement") return;
...
const parseImportMetaGlobOptionsObject = (globOptions, parser, warnings) => {
	...
	for (const prop of globOptions.properties) {
		...
		const key = staticOptionKeyFromPropName(prop.key, parser);
		if (key === undefined) {
			warnings.push(
				createImportMetaGlobWarning(
...
const parseImportMetaGlobCall = (expr, parser) => {
	...
	const { resolveContextModuleGlobPattern } = require("../util/globUtils");
 ...
 * @param {CallExpression} expr call expression
 * @param {JavascriptParser} parser parser
 * @returns {ImportMetaGlobDependencyOptions | undefined} options
 */
const createImportMetaGlobDependencyOptions = (expr, parser) =>
	parseImportMetaGlobCall(expr, parser).options;

module.exports = {
	createImportMetaGlobDependencyOptions,
	parseImportMetaGlobCall,
	staticGlobPatternsFromExpr
};
