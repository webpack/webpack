	...
	apply(dependency, source, templateContext) {
		...
		const promises = new Set(
			dep.dependencies
				.filter((dependency) => {
					...
					return connection ? moduleGraph.isAsync(connection.module) : false;
				})
				.map((dependency) => dependency.getImportVar(moduleGraph))
		);
		...
		if (dep.hasCallback) {
			if (runtimeTemplate.supportsArrowFunction()) {
				source.insert(
					dep.range[0],
					`${optAsync}__WEBPACK_OUTDATED_DEPENDENCIES__ => { ${content}(`
				);
				source.insert(dep.range[1], ")(__WEBPACK_OUTDATED_DEPENDENCIES__); }");
			} else {
				source.insert(
					dep.range[0],
					`${optAsync}function(__WEBPACK_OUTDATED_DEPENDENCIES__) { ${content}(`
				);
				source.insert(
					dep.range[1],
