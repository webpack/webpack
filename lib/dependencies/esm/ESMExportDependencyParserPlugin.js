...
const { ExportPresenceModes } = require("./ESMImportDependency");
...
/** @import { CompatibilitySettings } from "../../javascript/CompatibilityPlugin" */

const { HarmonyStarExportsList } = ESMExportImportedSpecifierDependency;

const PLUGIN_NAME = "HarmonyExportDependencyParserPlugin";

// TODO in the next major release: rename to `ESMExportDependencyParserPlugin`
module.exports = class HarmonyExportDependencyParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.export.tap(PLUGIN_NAME, (statement) => {
			const dep = new ESMExportHeaderDependency(
		...
		parser.hooks.exportImport.tap(PLUGIN_NAME, (statement, source) => {
			...
			const sideEffectDep = new ESMImportSideEffectDependency(
				/** @type {string} */ (source),
				parser.state.lastHarmonyImportOrder,
				phase,
		...
		// `export { ns as default }`, so it becomes the same re-export. Stage -10
		// beats the inner-graph tap, whose guard would outlive the removed statement.
		parser.hooks.statement.tap(
			...
			(statement) => {
				...
				const dep = new ESMExportImportedSpecifierDependency(
					settings.source,
				...
				const headerDep = new ESMExportHeaderDependency(
					undefined,
					/** @type {Range} */ (statement.range)
				);
		...
		parser.hooks.exportExpression.tap(PLUGIN_NAME, (statement, node) => {
			...
			const dep = new ESMExportExpressionDependency(
				exprRange,
				statementRange,
				comments
					.map((c) => {
		...
		parser.hooks.exportSpecifier.tap(
			...
			(statement, id, name, idx) => {
				...
				const dep = settings
					? new ESMExportImportedSpecifierDependency(
							settings.source,
							settings.sourceOrder,
							settings.ids,
							name,
					...
					: new ESMExportSpecifierDependency(
							id,
							name,
