...
const ConditionalInitFragment = require("../../template/ConditionalInitFragment");
...
const ESMExportInitFragment = require("./ESMExportInitFragment");
const ESMImportDependency = require("./ESMImportDependency");
const ESMLinkingError = require("./ESMLinkingError");

const { ExportBindingType } = ESMExportInitFragment;
...
/** @import ExportsInfo, { ExportInfo, UsedName } from "../../graph/ExportsInfo" */
...
/** @import { ExportMap } from "./ESMExportInitFragment" */
 ...
 */
/** @import ConcatenationScope from "../../optimize/ConcatenationScope" */
...
class NormalReexportItem {
	 ...
	 * @param {InstanceType<ExportInfo>} exportInfo export info from other module
 ...
 * @param {InstanceType<ExportInfo>} exportInfo export info on the imported module
...
const isStarReexportBackToParent = (moduleGraph, exportInfo, parentModule) => {
	...
	/** @type {Set<InstanceType<ExportInfo>> | undefined} */
	let visited;
	for (;;) {
...
/**
 * Two `export *` sources are ambiguous for a name when they resolve to
 * different bindings; a diamond re-export of the same binding is not.
 * @param {ModuleGraph} moduleGraph module graph
 * @param {InstanceType<ExportInfo>} previous export info of the star source that first provided the name
 * @param {InstanceType<ExportInfo>} exportInfo export info of a later star source providing the same name
 * @returns {boolean} true when the two sources provide different bindings
 */
const isAmbiguousWith = (moduleGraph, previous, exportInfo) => {
	const previousBinding = previous.getTerminalBinding(moduleGraph);
	if (!previousBinding) return false;
	const binding = exportInfo.getTerminalBinding(moduleGraph);
	if (!binding) return false;
	return previousBinding !== binding;
};
 ...
 * @returns {{ names: ExportInfoName[], dependencyIndices: DependencyIndices, conflicting: Set<ExportInfoName> | undefined }} result
 */
const determineExportAssignments = (
	...
	/** @type {Map<ExportInfoName, InstanceType<ExportInfo>>} */
	const firstProvider = new Map();
	/** @type {Set<ExportInfoName> | undefined} */
	let conflicting;
	...
	for (const dep of dependencies) {
		...
		if (otherImportedModule) {
			...
			for (const exportInfo of exportsInfo.exports) {
				if (exportInfo.provided !== true || exportInfo.name === "default") {
					continue;
				}
				if (names.has(exportInfo.name)) {
					const previous =
						/** @type {InstanceType<ExportInfo>} */
						(firstProvider.get(exportInfo.name));
					if (
						isAmbiguousWith(moduleGraph, previous, exportInfo) &&
						// A cyclic contribution resolves to nothing, so it never competes
						// with the binding a sibling `export *` provides.
						!(
							parentModule &&
							isStarReexportBackToParent(moduleGraph, exportInfo, parentModule)
						)
					) {
						if (conflicting === undefined) conflicting = new Set();
						conflicting.add(exportInfo.name);
					}
					continue;
				}
				if (
				...
				firstProvider.set(exportInfo.name, exportInfo);
				dependencyIndices[i] = names.size;
			}
		}
	}
	...
	return { names: [...names], dependencyIndices, conflicting };
};
...
// TODO in the next major release: rename to `ESMExportImportedSpecifierDependency`
class HarmonyExportImportedSpecifierDependency extends ESMImportDependency {
	/**
	...
	getStarReexports(
		...
		// An ambiguous name resolves to no binding at all, so no `export *` in this
		// module may contribute it — not even the first one.
		const conflicting = this._getConflictingStarExportNames(moduleGraph);
		if (conflicting !== undefined) {
			for (const name of conflicting) ignoredExports.add(name);
		}
	...
	/**
	 * Names this module's `export *` sources provide with conflicting bindings.
	 * @param {ModuleGraph} moduleGraph the module graph
	 * @returns {Set<ExportInfoName> | undefined} ambiguous names, or nothing when there are none
	 */
	_getConflictingStarExportNames(moduleGraph) {
		// Every `export *` carries the module's full star list; a lone one has
		// nothing to be ambiguous with, so skip the walk.
		if (!this.allStarExports) return;
		const { dependencies } = this.allStarExports;
		if (dependencies.length < 2) return;
		return moduleGraph.cached(determineExportAssignments, dependencies)
			.conflicting;
	}
	...
	getWarnings(moduleGraph) {
		...
		const circular =
			this._getCircularReexportPresenceLevel() === ExportPresenceModes.WARN
				? this._getCircularReexportErrors(moduleGraph)
				: undefined;
	...
	/**
	 * Reporting level for a circular reexport chain. It skips the promotion to
	 * an error that AUTO gives a strict harmony module — a cycle warns by
	 * default — but an explicit setting still says whether and how to report.
	 * @returns {ExportPresenceMode} effective mode
	 */
	_getCircularReexportPresenceLevel() {
		return this.exportPresenceMode === ExportPresenceModes.AUTO
			? ExportPresenceModes.WARN
			: this.exportPresenceMode;
	}

	/**
	 * Returns errors for a name resolving through a circular reexport chain.
	 * @param {ModuleGraph} moduleGraph module graph
	 * @returns {WebpackError[] | undefined} errors
	 */
	_getCircularReexportErrors(moduleGraph) {
	...
	getErrors(moduleGraph) {
		const exportsPresence = this._getEffectiveExportPresenceLevel(moduleGraph);
		const errors =
			exportsPresence === ExportPresenceModes.ERROR
				? this._getErrors(moduleGraph)
				: null;
		if (
			this._getCircularReexportPresenceLevel() !== ExportPresenceModes.ERROR
		) {
			return errors;
		}
		const circular = this._getCircularReexportErrors(moduleGraph);
		if (!circular) return errors;
		return errors ? [...errors, ...circular] : circular;
	}
...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// a wrapped module has no shared-scope symbols to redirect reexports to
		...
		if (mode.type !== "unused" && mode.type !== "empty-star") {
			const importedModule = moduleGraph.getModule(dep);
			// an in-scope target has no standalone module left to import from
			if (
	 ...
	 * @param {ConcatenationScope=} concatenationScope concatenation scope of a wrapped module
	 * @returns {void}
	 */
	_addExportFragments(
		initFragments,
		...
		// an in-scope target has no import statement, read it through a reference
		const inScope =
			concatenationScope !== undefined &&
			concatenationScope.isModuleInScope(importedModule);
		const importVar = inScope
			? /** @type {ConcatenationScope} */ (
					concatenationScope
				).createModuleReference(importedModule, { asiSafe: true })
			: dep.getImportVar(moduleGraph);

		const defaultImportVar = inScope
			? /** @type {ConcatenationScope} */ (
					concatenationScope
				).createModuleReference(importedModule, {
					ids: ["default"],
					asiSafe: true
				})
			: dep.getImportVar(moduleGraph);

		// whole-namespace modes need the original export names, not mangled ones
		const namespaceImportVar = inScope
			? /** @type {ConcatenationScope} */ (
					concatenationScope
				).createModuleReference(importedModule, {
					asiSafe: true,
					mangleableNamespace: true
				})
			: importVar;
		const isDeferred =
		...
		switch (mode.type) {
			...
			case "reexport-fake-namespace-object":
				// `${name}_namespace_cache` needs an identifier to suffix, and an in-scope
				// target is an expression. The concatenation already memoizes the same
				// object, its interop namespace taking the mode `fakeType` mirrors.
				if (inScope) {
					initFragments.push(
						this.getReexportFragment(
							module,
							"reexport fake namespace object from non-harmony",
							moduleGraph
								.getExportsInfo(module)
								.getUsedName(/** @type {string} */ (mode.name), runtime),
							/** @type {ConcatenationScope} */ (
			...
			case "reexport-namespace-object": {
				let value = namespaceImportVar;

				// The re-exported binding is the namespace itself, so it has to be
				// the same object every importer of that module is handed.
				if (
					importedModule.buildInfo &&
					importedModule.buildInfo.specNamespaceObject
				) {
					runtimeRequirements.add(RuntimeGlobals.specNamespaceObject);
					value = `${RuntimeGlobals.specNamespaceObject}(${value})`;
				}
			...
			case "dynamic-reexport": {
				...
				let content =
					...
					`/* harmony reexport (unknown) */ for(${runtimeTemplate.renderConst()} __WEBPACK_IMPORT_KEY__ in ${namespaceImportVar}) `;
				...
				content +=
					...
					runtimeTemplate.supportsConst()
						? `() => ${namespaceImportVar}[__WEBPACK_IMPORT_KEY__]`
						: `function(key) { return ${namespaceImportVar}[key]; }.bind(0, __WEBPACK_IMPORT_KEY__)`;
	 ...
	 * @returns {ESMExportInitFragment} harmony export init fragment
	 */
	getReexportFragment(
		module,
		comment,
		...
		map.set(key, {
			...
			type: ExportBindingType.Getter
		});

		return new ESMExportInitFragment(module.exportsArgument, map);
	}
	 ...
	 * @param {RuntimeRequirements} runtimeRequirements runtime requirements
	 * @returns {[InitFragment<GenerateContext>, ESMExportInitFragment]} init fragments
	 */
	getReexportFakeNamespaceObjectFragments(
		...
		const map = new Map();
		map.set(key, {
			value: `/* reexport fake namespace object from non-harmony */ ${name}_namespace_cache || (${name}_namespace_cache = ${
		...
		return [
			...
			new ESMExportInitFragment(module.exportsArgument, map)
		];
	}
	...
	getReexportDeferredNamespaceObjectFragments(
		...
		map.set(key, {
			value: `/* reexport deferred namespace object */ ${name}_deferred_namespace_cache || (${name}_deferred_namespace_cache = ${RuntimeGlobals.makeDeferredNamespaceObject}(${moduleId}, ${mode}${names ? `, ${JSON.stringify(names)}` : ""}))`,
			type: ExportBindingType.Getter
		});
...
// TODO in the next major release: rename to `ESMStarExportsList`
...
HarmonyExportImportedSpecifierDependency.HarmonyStarExportsList =
	HarmonyStarExportsList;
HarmonyExportImportedSpecifierDependency.idsSymbol = idsSymbol;
