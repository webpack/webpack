...
/** @import CommonJsFullRequireDependency from "../commonjs/CommonJsFullRequireDependency" */
/** @import CommonJsRequireDependency from "../commonjs/CommonJsRequireDependency" */
/** @import ESMEvaluatedImportSpecifierDependency from "./ESMEvaluatedImportSpecifierDependency" */
/** @import ESMImportSpecifierDependency from "./ESMImportSpecifierDependency" */
/** @import ImportDependency from "../import/ImportDependency" */
...
/** @typedef {["p", ESMEvaluatedImportSpecifierDependency]} GuardPresenceAtom */
...
/** @typedef {ESMImportSpecifierDependency | CommonJsRequireDependency | CommonJsFullRequireDependency | ImportDependency} GuardableDependency */
 ...
 * @param {Map<number, ESMImportSpecifierDependency>} depByRangeStart import-specifier deps in the test, keyed by range start
...
const evalLivenessFormula = (formula, moduleGraph, runtime) => {
	switch (formula[0]) {
		case "v": {
			const dep = /** @type {ESMImportSpecifierDependency} */ (formula[1]);
...
const buildFrameLivenessFormula = (parser, frame) => {
	...
	/** @type {Map<number, ESMImportSpecifierDependency>} */
	const depByRangeStart = new Map();
	for (let i = /** @type {number} */ (frame.depStart); i < deps.length; i++) {
		const dep = /** @type {ESMEvaluatedImportSpecifierDependency} */ (deps[i]);
		// Exclude `"x" in ns` evaluated deps; those feed the presence formula only.
		if (
			dep.isHarmonyImportSpecifier === true &&
			dep.isHarmonyEvaluatedImportSpecifier !== true &&
			dep.range
		) {
 ...
 * @param {JavascriptParser} parser the parser
 * @param {Expression} test conditional test expression
 * @param {Map<number, ESMEvaluatedImportSpecifierDependency>} depByRangeStart in-operator deps in the test, keyed by range start
...
const evalPresenceFormula = (formula, name, member, needTruthy) => {
	switch (formula[0]) {
		case "p": {
			const dep = /** @type {ESMEvaluatedImportSpecifierDependency} */ (
				formula[1]
			);
			return (
				needTruthy &&
...
const buildFramePresenceFormula = (parser, frame) => {
	const deps = /** @type {Module} */ (parser.state.module).dependencies;
	/** @type {Map<number, ESMEvaluatedImportSpecifierDependency>} */
	const depByRangeStart = new Map();
	for (let i = /** @type {number} */ (frame.depStart); i < deps.length; i++) {
		const dep = /** @type {ESMEvaluatedImportSpecifierDependency} */ (deps[i]);
		if (dep.isHarmonyEvaluatedImportSpecifier === true && dep.range) {
			depByRangeStart.set(dep.range[0], dep);
		}
	}
