...
/** @import { ExportsSpec, LazyUntil } from "../../graph/Dependency" */
...
/** @import { UnusedExports, ExportMap } from "./ESMExportInitFragment" */
/**
 * @import {
...
// TODO in the next major release: rename to `ESMExportSpecifierDependency`
...
makeSerializable(
	...
	"webpack/lib/dependencies/esm/ESMExportSpecifierDependency"
);
registerLegacyRequest(
	HarmonyExportSpecifierDependency,
	"webpack/lib/dependencies/ESMExportSpecifierDependency"
);
registerLegacyRequest(
	HarmonyExportSpecifierDependency,
...
HarmonyExportSpecifierDependency.Template = class HarmonyExportSpecifierDependencyTemplate extends (
	...
	apply(
		...
		if (!used) {
			...
			initFragments.push(
				new ESMExportInitFragment(module.exportsArgument, undefined, set)
			);
			return;
		}
		...
		const buildInfo =
			/** @type {import("../../module/Module").BuildInfo} */
			(module.buildInfo);
		const valueFormat =
			dep.constValue !== undefined && buildInfo.isCircular === false;

		const meta = moduleGraph.getMetaIfExisting(module);

		const onDemand =
			!concatenationScope &&
			meta !== undefined &&
			meta.onDemandExports === true;

		/** @type {ExportMap} */
		const map = new Map();
		map.set(used, {
			value: `/* binding */ ${dep.id}`,
			type: valueFormat ? ExportBindingType.Value : ExportBindingType.Getter,
			finalName: dep.id
		});
		initFragments.push(
			new ESMExportInitFragment(
