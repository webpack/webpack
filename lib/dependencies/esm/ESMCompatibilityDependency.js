...
// Required here rather than at the top: the deferred-namespace runtime imports
...
// TODO in the next major release: rename to `ESMCompatibilityDependency`
...
makeSerializable(
	...
	"webpack/lib/dependencies/esm/ESMCompatibilityDependency"
);
registerLegacyRequest(
	HarmonyCompatibilityDependency,
	"webpack/lib/dependencies/ESMCompatibilityDependency"
);
registerLegacyRequest(
	HarmonyCompatibilityDependency,
...
HarmonyCompatibilityDependency.Template = class HarmonyExportDependencyTemplate extends (
	...
	apply(
		...
		if (
			...
			if (!concatenationScope && meta && meta.onDemandExports && getData) {
				const data = getData();
				const before = data.get("exportsSource");
				data.set("exportsSource", before ? `${before}${content}` : content);
			} else {
				initFragments.push(
					new InitFragment(
						content,
