	...
	apply(parser) {
		...
		const parseCreateRequireArguments = (expr) => {
			...
			if (!evaluated.isString()) {
				...
				err.loc = parser.getLocation(arg);
				parser.state.module.addWarning(err);
				return;
			}
		...
		/**
		 * @param {VariableDeclarator} declarator declarator
		 * @returns {boolean | void} true when the binding was tagged
		 */
		const preDeclaratorHandler = (declarator) => {
			if (
		...
		parser.hooks.preDeclarator.tap(PLUGIN_NAME, preDeclaratorHandler);
		// mirrors the handler's bail conditions on the columns
		parser.registerColumnGate(
			parser.hooks.preDeclarator,
			preDeclaratorHandler,
			(_parser, store, did) => {
				// the pre-walk only gates plain identifier bindings
				const init = store.kid1[did];
				if (init === 0 || store.types[init] !== SoaAst.TYPE_CALL_EXPRESSION) {
					return false;
				}
				const callee = store.kid0[init];
				if (callee === 0 || store.types[callee] !== SoaAst.TYPE_IDENTIFIER) {
					return false;
				}
				const variableInfo = parser.getVariableInfo(
					parser._soaIdentName(store, callee)
				);
				return (
					variableInfo instanceof VariableInfo &&
					variableInfo.tagInfo !== undefined &&
					variableInfo.tagInfo.tag === createRequireSpecifierTag
				);
			}
		);
		...
		parser.hooks.expression
			...
			.tap(PLUGIN_NAME, (expr) => {
				...
				clearDep.loc = parser.getLocation(expr);
				parser.state.module.addPresentationalDependency(clearDep);
				return true;
			});
		parser.hooks.call
