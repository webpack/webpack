...
class AMDRequireDependenciesBlockParserPlugin {
	...
	processArray(parser, expr, param, paramExpr) {
		if (param.isArray()) {
			param.items.forEach((p, idx) => {
				const subParamExpr = paramExpr.elements[idx];
				const result = this.processItem(parser, expr, p, subParamExpr);
				if (result === undefined) {
					this.processContext(parser, expr, p, subParamExpr);
				}
			});
			return true;
	...
	processItem(parser, expr, param, paramExpr) {
		if (param.isConditional()) {
			param.options.forEach((p, idx) => {
				const subParamExpr =
	...
	processCallRequire(parser, expr) {
		...
		if (expr.arguments.length === 1) {
			parser.inScope([], () => {
				result = this.processArray(parser, expr, param, expr.arguments[0]);
			});
