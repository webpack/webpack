...
class AMDRequireDependenciesBlockParserPlugin {
	...
	/**
	 * @param {JavascriptParser} parser the parser
	 * @param {AMDRequireDependenciesBlock} depBlock dependency block
	 * @param {CallExpression} expr call expression
	 * @param {BasicEvaluatedExpression} param param
	 * @returns {boolean | undefined} result
	 */
	processArray(parser, depBlock, expr, param) {
		if (param.isArray()) {
			for (const p of /** @type {BasicEvaluatedExpression[]} */ (param.items)) {
				const result = this.processItem(parser, depBlock, expr, p);
				if (result === undefined) {
					this.processContext(parser, depBlock, expr, p);
				}
			}
			return true;
	 ...
	 * @returns {boolean | undefined} result
	 */
	processItem(parser, depBlock, expr, param) {
	 ...
	 * @returns {boolean | undefined} result
	 */
	processContext(parser, depBlock, expr, param) {
	...
	processCallRequire(parser, expr) {
		...
		let param;
		/** @type {AMDRequireDependenciesBlock | undefined} */
		let depBlock;
