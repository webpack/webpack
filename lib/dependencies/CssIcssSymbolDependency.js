			...
			if (item instanceof CssIcssImportDependency) {
				...
				for (let i = module.dependencies.length - 1; i >= 0; i--) {
					...
					if (
						...
						if (nestedDep.reexportName) {
							return this.findReference(nestedDep.reexportName, {
								...templateContext,
								module
							});
						}

						// @ts-expect-error TODO refactor me
						return nestedDep.constructor.Template.getIdentifier(nestedDep, {
							...templateContext,
							module
						});
					}
	...
	apply(dependency, source, templateContext) {
		if (templateContext.type === "css") {
			...
			const value = dep.isReference
				? CssIcssSymbolDependencyTemplate.findReference(
						dep.symbol,
						templateContext
					)
				: dep.symbol;
