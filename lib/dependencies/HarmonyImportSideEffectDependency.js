...
/**
 * Export names a module keeps deferred (lazy barrel optimization).
 * @param {Module} module the module
 * @returns {Set<string> | true | undefined} deferred names, true for any name, undefined when nothing is deferred
 */
const getDeferredNames = (module) => {
	/** @type {Set<string> | undefined} */
	let names;
	for (const dependency of module.dependencies) {
		// TODO remove in webpack 6
		// They may be missing on custom dependency types not extending the base Dependency
		if (!("isLazy" in dependency) || !("getLazyUntil" in dependency)) continue;
		if (!dependency.isLazy()) continue;
		const until = dependency.getLazyUntil();
		// a deferred star re-export may provide any name
		...
		names.add(/** @type {string} */ (dependency.getLazyName()));
	}
	return names;
};
...
class HarmonyImportSideEffectDependency extends HarmonyImportDependency {
	...
	_getUnusedSpecifierErrors(moduleGraph) {
		...
		// A lazy barrel has not built the targets nothing requested, so the names
		// they would provide are missing from its exports.
		const deferredNames = importedModule
			? getDeferredNames(importedModule)
			: undefined;
		if (deferredNames === true) return;
		...
		for (const [ids, name] of specifiers) {
			if (deferredNames !== undefined && deferredNames.has(ids[0])) continue;
