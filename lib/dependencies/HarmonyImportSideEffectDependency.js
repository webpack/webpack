...
class HarmonyImportSideEffectDependency extends HarmonyImportDependency {
	...
	constructor(request, sourceOrder, phase, attributes) {
		...
		this.declaredSpecifiers = undefined;
	}
	...
	serialize(context) {
		...
		context.write(this.declaredSpecifiers);
	...
	deserialize(context) {
		...
		this.declaredSpecifiers = context.read();
...
HarmonyImportSideEffectDependency.Template = class HarmonyImportSideEffectDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		this._recordImportBindings(
			/** @type {HarmonyImportSideEffectDependency} */ (dependency),
			templateContext,
			module
		);
	}
	...
	/**
	 * Records what each binding this import declares reads in the generated code,
	 * for the source map `scopes` field. Only a module whose exports are a
	 * namespace is answered directly by the import variable; every interop shape
	 * is left to the reference sites, which know which one they built.
	 * @param {HarmonyImportSideEffectDependency} dep dependency
	 * @param {DependencyTemplateContext} templateContext the context object
	 * @param {Module} importedModule the imported module
	 * @returns {void}
	 */
	_recordImportBindings(dep, templateContext, importedModule) {
		const { moduleGraph, runtime, module } = templateContext;
		const declared = dep.declaredSpecifiers;
		if (
			declared === undefined ||
			!importedModule ||
		...
		const bindings =
			...
			(templateContext.importBindings);
		for (const [ids, name] of declared) {
			if (ids.length === 0) {
				bindings.set(name, importVar);
				continue;
			}
			const used = exportsInfo.getUsedName(ids, runtime);
			// A name the build dropped, or replaced by a literal, is not a place a
			// debugger can read the binding from.
			if (!used || !Array.isArray(used)) continue;
			bindings.set(name, importVar + propertyAccess(used));
		}
	}
};
