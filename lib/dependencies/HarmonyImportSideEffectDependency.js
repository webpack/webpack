...
/**
 * Whether a module keeps a re-export target deferred (lazy barrel optimization).
 * @param {Module} module the module
 * @returns {boolean} true, when a target stays deferred
 */
const hasDeferredTarget = (module) => {
	for (const dependency of module.dependencies) {
		// TODO remove in webpack 6
		// It may be missing on custom dependency types not extending the base Dependency
		if ("isLazy" in dependency && dependency.isLazy()) return true;
	}
	return false;
};
...
class HarmonyImportSideEffectDependency extends HarmonyImportDependency {
	...
	_getUnusedSpecifierErrors(moduleGraph) {
		...
		// A lazy barrel never builds the re-export targets nothing requested, so
		// its provided exports stay incomplete and a name it forwards reads as
		// missing.
		if (importedModule !== null && hasDeferredTarget(importedModule)) {
			return;
		}
