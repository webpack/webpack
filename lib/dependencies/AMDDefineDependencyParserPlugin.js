...
class AMDDefineDependencyParserPlugin {
	...
	processArray(parser, expr, param, paramExpr, identifiers, namedModule) {
		if (param.isArray()) {
			param.items.forEach((param, idx) => {
				...
				const subParamExpr = paramExpr.elements[idx];
				...
				if (result === undefined) {
					this.processContext(parser, expr, param, subParamExpr);
				}
			});
			return true;
	...
	processItem(parser, expr, param, paramExpr, namedModule) {
		if (param.isConditional()) {
			param.options.forEach((param, idx) => {
				const subParamExpr =
					idx === 0 ? paramExpr.consequent : paramExpr.alternate;
				const result = this.processItem(parser, expr, param, subParamExpr);
				if (result === undefined) {
					this.processContext(parser, expr, param, subParamExpr);
				}
			});
	...
	processContext(parser, expr, param, paramExpr) {
