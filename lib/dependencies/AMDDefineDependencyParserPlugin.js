		...
		parser.plugin("call define", (expr) => {
			...
			if(fn && fn.type === "FunctionExpression") {
				...
				parser.inScope(fnParams, () => {
					...
					parser.traverseNodeInCurrentScope(fn.body);
				});
				...
				parser.inScope(fn.callee.object.params.filter((i) => ["require", "module", "exports"].indexOf(i.name) < 0), () => {
					parser.scope.renames = fnRenames;
					parser.scope.inTry = inTry;
					parser.traverseNodeInCurrentScope(fn.callee.object.body);
				});
				if(fn.arguments)
					parser.traverseNodesInCurrentScope(fn.arguments);
			} else if(fn || obj) {
				parser.traverseNodeInCurrentScope(fn || obj);
			}
