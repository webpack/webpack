...
const ESMImportGuard = require("./ESMImportGuard");
const { ImportPhaseUtils } = require("./ImportPhase");
const { getTrimmedIdsAndRange } = require("./chainedImports");
const traverseDestructuringAssignmentProperties = require("./traverseDestructuringAssignmentProperties");

/** @import { ReplaceSource } from "webpack-sources" */
/** @import { GetConditionFn, ReferencedExports, UpdateHashContext } from "../graph/Dependency" */
/** @import { DependencyTemplateContext } from "../template/DependencyTemplate" */
/** @import Module, { BuildMeta } from "../module/Module" */
...
/** @import { IdRanges } from "./chainedImports" */
/** @import { ExportPresenceMode } from "./ESMImportDependency" */
/** @typedef {ESMImportDependency.Ids} Ids */
/** @import { ImportPhaseType } from "./ImportPhase" */
/** @import { DependencyGuard } from "./ESMImportGuard" */
/**
 * @import {
...
// TODO in the next major release: rename to `ESMImportSpecifierDependency`
class HarmonyImportSpecifierDependency extends ESMImportDependency {
	// read by `ESMImportGuard` in place of `instanceof`, which would cycle
	...
	constructor(
		...
		phase,
		attributes,
		idRanges // TODO in the next major release: make this non-optional. It must always be set to properly trim ids.
	) {
		...
		/** @type {undefined | boolean} this reference is an assignment target */
	...
	getCondition(moduleGraph) {
		...
		const condition = (connection, runtime) => {
			...
			if (
				guards !== undefined &&
				ESMImportGuard.isDeadByGuards(guards, moduleGraph, runtime)
			) {
				return false;
			}
			return true;
			...
			? [
					{
						...
						// An inlined literal cannot be an assignment target at all.
						canInline: !this.assignment && !ImportPhaseUtils.isDefer(this.phase)
					}
				]
...
makeSerializable(
	...
	"webpack/lib/dependencies/ESMImportSpecifierDependency"
);
	...
	_getCodeForIds(dep, source, templateContext, ids, connection) {
		...
		if (
			...
			if (ids.length === 0) {
				...
				// Concatenation merges the binding into a plain local a write would
				// succeed against. Routing the write at a getter-only property of a
				// throwaway object restores the TypeError the namespace gave it.
				if (dep.assignment) {
					// the local binding, the one name here an export is free not to
					// spell as an identifier: `import { "a-b" as ab }`
					const holder = `{ get ${dep.name}() { return ${exportExpr}; } }`;
					exportExpr = `${dep.asiSafe ? "" : ";"}(${holder}).${dep.name}`;
				}
			}
		} else {
