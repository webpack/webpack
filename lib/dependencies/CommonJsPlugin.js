...
const SelfModuleFactory = require("../module/SelfModuleFactory");
...
const TopLevelThisParserPlugin = require("./TopLevelThisParserPlugin");
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { contextModuleFactory, normalModuleFactory }) => {
				...
				const handler = (parser, parserOptions) => {
					// what `this` means does not depend on CommonJS syntax being parsed,
					// so this one is applied before the `commonjs` option is read
					if (parserOptions.topLevelThis === "global") {
						new TopLevelThisParserPlugin(
					...
					new CommonJsExportsParserPlugin(parserOptions.topLevelThis).apply(
						parser
					);
				};
...
// TODO in the next major release: rename to `ESMModuleDecoratorRuntimeModule`
