 ...
 * @param {Output["crossOriginLoading"]} crossOriginLoading the `output.crossOriginLoading` option
 * @returns {string} the sibling `<link>` tag's HTML
 */
const buildStylesheetLink = (extra, href, crossOriginLoading) => {
	const safeHref = href.replace(/"/g, "&quot;");
	return `<link rel="stylesheet" href="${safeHref}"${extra}${crossOriginAttr(crossOriginLoading, extra)}>`;
};
 ...
 */
const buildScriptTag = (extra, src, isModule, crossOriginLoading) => {
	const safeSrc = src.replace(/"/g, "&quot;");
	return `<script${isModule ? ' type="module"' : ""} src="${safeSrc}"${extra}${crossOriginAttr(crossOriginLoading, extra)}></script>`;
};

/**
 * `crossorigin` attribute to append to an injected tag, derived from
 * `output.crossOriginLoading`. Returns `""` when the option is off or the tag
 * already carries a `crossorigin` (an author-set value wins). Mirrors webpack's
 * runtime, which sets `crossOrigin` on chunk-loading scripts, and matches Vite,
 * which emits `crossorigin` on every injected script/stylesheet.
 * @param {Output["crossOriginLoading"]} crossOriginLoading the output option
 * @param {string} existing tag/attribute text already present, to avoid overriding an author value
 * @returns {string} ` crossorigin="…"` or `""`
 */
const crossOriginAttr = (crossOriginLoading, existing) =>
	crossOriginLoading && !/\scrossorigin/i.test(existing)
...
const cloneTagWithUrl = (
	...
	// Add `crossorigin` from `output.crossOriginLoading` unless the clone
	// already carries one (author value wins), inserted right after the tag name.
	const crossOrigin = crossOriginAttr(crossOriginLoading, body);
	if (crossOrigin) {
		body = body.replace(/^<[a-zA-Z][^\s/>]*/, (name) => name + crossOrigin);
	}
...
HtmlScriptSrcDependency.Template = class HtmlScriptSrcDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Mirror `output.crossOriginLoading` onto the entry tag too (siblings get
		// it via the builders below). A native tag whose author already set
		// `crossorigin` keeps that value; the attribute goes right after the tag
		// name to sit alongside any `type="module"` the parser injected.
		if (
			tagIsNative &&
			crossOriginLoading &&
			!/\scrossorigin/i.test(originalTag)
		) {
			const nameMatch = /^<[a-zA-Z][^\s/>]*/.exec(originalTag);
			if (nameMatch) {
				source.insert(
					dep.tagStart + nameMatch[0].length,
					` crossorigin="${crossOriginLoading}"`
				);
			}
		}
