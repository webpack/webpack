...
class HarmonyExportSpecifierDependency extends NullDependency {
	...
	getExports(moduleGraph) {
		...
		/** @type {import("../Dependency").ExportSpec} */
		const spec = {
			name: this.name,
			isPure: isPure || undefined,
			inlined: this.constValue || undefined
		};
		// Nested property exports for compound component pattern
		const objectExportProperties =
			module && module.buildInfo && module.buildInfo.objectExportProperties;
		if (objectExportProperties) {
			const propNames = objectExportProperties.get(this.id);
			if (propNames) {
				// Runtime keys stay Form.Item; no member-access rewrite → never mangle.
				spec.exports = propNames.map((/** @type {string} */ name) => ({
					name,
					canMangle: false
				}));
				spec.partialExports = true;
			}
		}
		return {
			exports: [spec],
