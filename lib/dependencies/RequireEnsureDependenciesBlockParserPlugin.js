				...
				case 2:
					{
						...
						if(successExpression) {
							parser.traverseAstInCurrentScope(successExpression.expressions);
						}
						if(errorExpression) {
							parser.traverseAstInCurrentScope(errorExpression.expressions);
						}
						...
						try {
							...
							if(successExpression) {
								parser.traverseAstInCurrentScope(successExpression.fn.body);
							}
							old.addBlock(dep);
						} finally {
						...
						if(!successExpression) {
							parser.traverseAstInCurrentScope(successExpressionArg);
						}
						if(errorExpression) {
							parser.traverseAstInCurrentScope(errorExpression.fn.body);
						} else if(errorExpressionArg) {
							parser.traverseAstInCurrentScope(errorExpressionArg);
						}
						return true;
					}
			}
