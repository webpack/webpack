	...
	apply(parser) {
		...
		parser.hooks.importCall.tap(PLUGIN_NAME, (expr, importThen) => {
			...
			if (importOptions) {
				...
				// `webpackEntryOptions` only applies to workers, not to `import()`
				if (importOptions.webpackEntryOptions !== undefined) {
					parser.state.module.addWarning(
						new UnsupportedFeatureWarning(
							"`webpackEntryOptions` is not supported in `import()`, it only applies to workers (e.g. `new Worker(new URL(...))`).",
							/** @type {DependencyLocation} */ (expr.loc)
						)
					);
				}
