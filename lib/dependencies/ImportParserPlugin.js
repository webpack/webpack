...
class ImportParserPlugin {
	...
	apply(parser) {
		 ...
		 */
		const preDeclaratorHandler = (decl) => {
			if (
				decl.init &&
		...
		parser.registerColumnGate(
			...
			(_parser, store, did) => {
				...
				if (init === 0 || store.types[init] !== SoaAst.TYPE_AWAIT_EXPRESSION) {
					return false;
				}
				const arg = store.kid0[init];
				// dynamic imports are foreign (ref 0) — those must materialize
				return arg === 0 || store.types[arg] === SoaAst.TYPE_IMPORT_EXPRESSION;
			}
		);
