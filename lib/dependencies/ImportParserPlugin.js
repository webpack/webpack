 ...
 */
function tagDynamicImportVariable(parser, importCall, variableName) {
...
/**
 * @param {CallExpression} importCallThen import().then() call
 * @returns {Identifier | ObjectPattern | undefined} the dynamic imported namespace obj
 */
function getFulfilledCallbackNamespaceObj(importCallThen) {
	const fulfilledCallback = importCallThen.arguments[0];
	if (
		fulfilledCallback &&
 ...
 */
function walkImportCallThenFulfilledCallback(
	parser,
	importCall,
	fulfilledCallback,
	namespaceObjArg
	...
	parser.inFunctionScope(!arrow, scopeParams, () => {
		if (namespaceObjArg.type === "Identifier") {
			tagDynamicImportVariable(parser, importCall, namespaceObjArg.name);
		} else {
			...
			if (referencedPropertiesInDestructuring) {
				const state = getState(parser);
				const references = /** @type {RawReferencedExports} */ (state.get(importCall));
	...
	apply(parser) {
		...
		parser.hooks.preDeclarator.tap(PLUGIN_NAME, (decl) => {
			if (
				...
				tagDynamicImportVariable(parser, decl.init.argument, decl.id.name);
			}
		});
		...
		parser.hooks.importCall.tap(PLUGIN_NAME, (expr, importCallThen) => {
			...
			const fulfilledNamespaceObj =
				importCallThen && getFulfilledCallbackNamespaceObj(importCallThen);
			if (
			...
			if (fulfilledNamespaceObj) {
				walkImportCallThenFulfilledCallback(
					parser,
					expr,
					/** @type {ArrowFunctionExpression | FunctionExpression} */
					(importCallThen.arguments[0]),
					fulfilledNamespaceObj
				);
				parser.walkExpressions(importCallThen.arguments.slice(1));
			}
