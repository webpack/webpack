...
class ImportParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.newImportCall.tap(PLUGIN_NAME, (expression) => {
			const source = /** @type {{ source?: string | Buffer }} */ (parser.state)
				.source;
			const sourceText =
		...
		// Fold a comma expression `(a, b, c)` to its tail `c` when the leading
		// expressions are side-effect free, so e.g. `import((1, 0, "./x.js"))`
		// is treated as `import("./x.js")`. We bail out otherwise because the
		// dependency template replaces the whole expression range and would
		// silently drop the leading side effects.
		...
		parser.hooks.importCall.tap(PLUGIN_NAME, (expr, importThen) => {
			...
			if (param.isString()) {
				...
				// If `import(spec, options)` carries an options expression
				// that we couldn't statically lift to attributes, capture its
				// source range so the dependency template can preserve the
				// expression's side effects (e.g. `yield`) by wrapping the
				// runtime require chain in an IIFE.
				const dynamicOptionsRange =
					expr.options && !attributes
						? /** @type {Range} */ (expr.options.range)
						: undefined;
