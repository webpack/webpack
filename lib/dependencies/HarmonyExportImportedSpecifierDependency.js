...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate extends (
	...
	_addExportFragments(
		...
		// Check if the target is an external module with ESM library output format
		const isExternalModuleTarget =
			importedModule && importedModule.constructor.name === "ExternalModule";
		const isEsmLibraryOutput =
			runtimeTemplate.outputOptions.library &&
			runtimeTemplate.outputOptions.library.type === "module";
		const isStarReexport = mode.type === "dynamic-reexport";

		// Special handling for ESM external modules - allows direct native ESM syntax
		// when targeting module library type with external modules
		if (isExternalModuleTarget && isEsmLibraryOutput) {
			const externalRequest = /** @type {import("../ExternalModule")} */ (
				importedModule
			).request;

			// For star exports, directly emit 'export * from "module"' syntax
			if (isStarReexport) {
				initFragments.push(
					new InitFragment(
						`export * from ${JSON.stringify(externalRequest)};\n`,
			...
			// For named exports from external modules, use native ESM named export syntax when possible
			if (
				mode.type === "normal-reexport" &&
				mode.items &&
				mode.items.length > 0
			) {
				// Check if all exports are simple references (not nested paths or hidden exports)
				const canUseNativeNamedExports = mode.items.every(
					item => item.ids && item.ids.length === 1 && !item.hidden
				);

				if (canUseNativeNamedExports) {
					// Generate native ESM named export statements
					const namedExportStatements = mode.items
						.map(
							item =>
								`export { ${item.ids[0]} as ${item.name} } from ${JSON.stringify(externalRequest)};`
						)
						.join("\n");
