...
HarmonyExportImportedSpecifierDependency.prototype.getReference = function() {
	...
	if(!this.name) {
		...
		if(Array.isArray(this.originModule.usedExports)) {
			...
			if(Array.isArray(m.providedExports)) {
				return {
					...
					importedNames: this.originModule.usedExports.filter(function(id) {
						return activeExports.indexOf(id) < 0 && m.providedExports.indexOf(id) >= 0;
					}, this)
				}
			} else {
				return {
			...
			return {
				module: m,
				importedNames: m.providedExports
			}
		} else {
			return {
				module: m,
...
HarmonyExportImportedSpecifierDependency.prototype.getExports = function() {
	...
	if(this.importDependency.module && Array.isArray(this.importDependency.module.providedExports)) {
		return {
			exports: this.importDependency.module.providedExports,
...
HarmonyExportImportedSpecifierDependency.Template.prototype.apply = function(dep, source, outputOptions, requestShortener) {
	...
	if(!used) { // we want to rexport something, but the export isn't used
		...
		content = "/* harmony reexport (std) */ " + getReexportStatement(JSON.stringify(used), JSON.stringify(idUsed)) + "\n";
		...
		if(items.length > 0) {
			content = "/* harmony namespace reexport (by used) */ " + items.map(function(item) {
		...
		if(items.length > 0) {
			content = "/* harmony namespace reexport (by provided) */ " + items.map(function(item) {
				return getReexportStatement(JSON.stringify(item[0]), JSON.stringify(item[1]));
			}).join(" ") + "\n";
