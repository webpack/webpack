...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate extends HarmonyImportDependency.Template {
	...
	apply(dependency, source, templateContext) {
		const { moduleGraph, concatenationScope, runtime } = templateContext;
		...
		if (concatenationScope) {
			...
			if (exportName) {
				concatenationScope.registerReexport(
					exportName,
					importedModule,
					importNames
				);
				...
				if (providedExports) {
					for (const name of providedExports) {
						concatenationScope.registerReexport(name, importedModule, [name]);
					}
				}
			}
			return;
