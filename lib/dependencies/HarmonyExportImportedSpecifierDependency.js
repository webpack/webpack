...
/** @import ExportsInfo, { ExportInfo, UsedName } from "../ExportsInfo" */
...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate extends (
	...
	_addExportFragments(
		...
		switch (mode.type) {
			...
			case "reexport-dynamic-default": {
				// `${importVar}_default` names the interop variable that the import
				// statement declares. An in-scope target has no import statement, so
				// reference its default export and let the concatenation resolve it.
				const defaultAccess = inScope
					? /** @type {ConcatenationScope} */ (
							concatenationScope
						).createModuleReference(importedModule, {
							ids: ["default"],
			...
			case "reexport-fake-namespace-object":
				// `${name}_namespace_cache` needs an identifier to suffix, but an
				// in-scope target is an expression. The concatenation already builds
				// the same object, memoized: its interop namespace picks the mode from
				// the exports type, which is what `fakeType` mirrors.
				if (inScope) {
					initFragments.push(
						this.getReexportFragment(
							module,
			...
			case "reexport-namespace-object": {
				// The re-export has to hand out the same object a direct
				// `import * as ns` does, or the two compare unequal.
				let namespaceValue = namespaceImportVar;
				if (runtimeTemplate.specNamespaceObjects) {
					runtimeRequirements.add(RuntimeGlobals.specNamespaceObject);
					namespaceValue = `${RuntimeGlobals.specNamespaceObject}(${namespaceImportVar})`;
				}
				initFragments.push(
					this.getReexportFragment(
						module,
	...
	getReexportDeferredNamespaceObjectFragments(
		...
		map.set(
			key,
			`/* reexport deferred namespace object */ ${name}_deferred_namespace_cache || (${name}_deferred_namespace_cache = ${RuntimeGlobals.makeDeferredNamespaceObject}(${moduleId}, ${mode}${names ? `, ${JSON.stringify(names)}` : ""}))`
		);
