...
class HarmonyExportImportedSpecifierDependency extends HarmonyImportDependency {
	...
	/**
	 * @return {{ module: string; weak?: boolean; importedNames?: (boolean | string[]) }}
	 */
	getReference() {
...
HarmonyExportImportedSpecifierDependency.Template = class HarmonyExportImportedSpecifierDependencyTemplate extends HarmonyImportDependency.Template {
	harmonyInit(dep, source, runtime, dependencyTemplates) {
		// @ts-ignore Expected 0-3 arguments, but got 4.
	...
	getContent(dep) {
		...
		switch (mode.type) {
			...
			case "dynamic-reexport": {
				...
				// @ts-ignore TODO: Map has no length
				if (activeExports.length > 0)
					content +=
						"if(" +
						// @ts-ignore activeExports is not an Array
