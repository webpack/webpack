...
class CSSModuleDefineDependency extends NullDependency {
	 ...
	 * @param {Range} identifierRange range
	 * @param {string=} prefix prefix
	 */
	constructor(name, range, identifierRange, prefix = "") {
		super();
		this.name = name;
		this.range = range;
...
CSSModuleDefineDependency.Template = class CSSModuleDefineDependencyTemplate extends (
	...
	apply(
		...
		// should not generate output if css module is not used
		if (!used) {
		...
		const name = moduleId + "-" + used;

		const identifier = prefix + name;

		const content = escapeCssIdentifier(identifier, dep.prefix);

		source.replace(dep.identifierRange[0], dep.identifierRange[1] - 1, content);
