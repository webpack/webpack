...
CssModuleDefineDependency.Template = class CssModuleDefineDependencyTemplate extends (
	 ...
	 */
	isStyleBlockUsed(dependency, { module, moduleGraph, runtime }) {
	...
	apply(dependency, source, templateContext) {
		...
		while (parent) {
			if (!this.isStyleBlockUsed(parent, templateContext)) {
		...
		if (!this.isStyleBlockUsed(dep, templateContext) || !areAllParentsUsed) {
		...
		for (let i = 0; i < identifiers.length; i++) {
			...
			// if identifier is not used, remove it and continue to next identifier
			if (!used) {
