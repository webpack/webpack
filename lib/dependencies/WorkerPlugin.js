	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// Name chunks that belong only to a worker (its entry chunk and any
				// chunks reachable solely from worker entrypoints) with
				// `output.workerChunkFilename`. Chunks also reachable from an initial
				// entrypoint stay on `output.chunkFilename`. A per-worker
				// `webpackEntryOptions.filename` already on the chunk wins.
				compilation.hooks.afterOptimizeChunks.tap(PLUGIN_NAME, () => {
					const { workerChunkFilename, chunkFilename } =
						compilation.outputOptions;
					if (
						workerChunkFilename === chunkFilename ||
						compilation.asyncEntrypoints.length === 0
					) {
						return;
					}
					const mainChunks = new Set();
					...
					for (const entrypoint of compilation.asyncEntrypoints) {
						for (const chunk of entrypoint.chunks) {
							for (const c of chunk.getAllReferencedChunks()) {
								if (c.filenameTemplate === undefined && !mainChunks.has(c)) {
									c.filenameTemplate = workerChunkFilename;
								}
							}
						}
					}
