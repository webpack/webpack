	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// Apply `output.workerChunkFilename` from outputOptions (kept out of
				// the serialized block so a function value doesn't break the cache);
				// a per-worker `webpackEntryOptions.filename` already on the chunk wins.
				if (this._workerChunkFilename) {
					compilation.hooks.afterChunks.tap(PLUGIN_NAME, () => {
						for (const entrypoint of compilation.asyncEntrypoints) {
							const chunk = entrypoint.getEntrypointChunk();
							if (chunk.filenameTemplate === undefined) {
								chunk.filenameTemplate = this._workerChunkFilename;
							}
						}
					});
				}
