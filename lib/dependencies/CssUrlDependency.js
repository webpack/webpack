...
/**
 * Scan `content` for `PUBLIC_PATH_FULL_HASH` placeholders (emitted when a
 * `url()` public path needs `[fullhash]` but the hash is not yet known) and
 * invoke `onMatch` once per well-formed occurrence. The placeholder spans the
 * half-open range `[start, end)` and encodes the requested hash length
 * (`0` means the full hash). Callers substitute the build-time hash or a
 * runtime expression depending on context.
 * @param {string} content text to scan
 * @param {(start: number, end: number, length: number) => void} onMatch placeholder callback
 * @returns {void}
 */
CssUrlDependency.walkFullHashPlaceholders = (content, onMatch) => {
	const prefix = CssUrlDependency.PUBLIC_PATH_FULL_HASH;
