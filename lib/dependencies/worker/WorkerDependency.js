...
WorkerDependency.Template = class WorkerDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// `new URL("./worker.<name>.js", import.meta.url)` form — a literal specifier,
		// no runtime helpers — so other bundlers and webpack itself can statically
		// follow the worker. A resource hint does not block it: the `<link>` is
		// emitted at chunk startup instead of wrapping the href, which would turn the
		// specifier into a non-analyzable expression.
...
makeSerializable(
	WorkerDependency,
	"webpack/lib/dependencies/worker/WorkerDependency"
);
