...
/** @import { ReferencedExports, UpdateHashContext } from "../../graph/Dependency" */
...
WorkletDependency.Template = class WorkletDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// ESM output emits the analyzable `new URL("./worklet.<name>.js",
		// import.meta.url)`, which another bundler can follow statically. Only the
		// entry chunk is named, its splits being linked by native `import`.
		...
		// A worklet loads no chunks at runtime, so every reachable one is pre-added
		// from the calling scope. The runtime chunk goes last, installing what came
		// before it and running startup with every module present.
		/** @type {(string | number)[]} */
		const restChunkIds = [];
...
makeSerializable(
	WorkletDependency,
	"webpack/lib/dependencies/worker/WorkletDependency"
);
...
WorkletDependency.WORKLET_MODULE = WORKLET_MODULE;
