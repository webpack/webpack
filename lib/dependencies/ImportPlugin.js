...
class ImportPlugin {
	...
	apply(compiler) {
		...
		compiler.plugin("compilation", (compilation, params) => {
			...
			contextModuleFactory.plugin("context-module-files", (files, contextDependency) => {
				if(contextDependency instanceof ImportContextDependency) {
					if(contextDependency.filter) {
						files = files.filter(file => !!file.match(contextDependency.filter));
					}
					if(contextDependency.exclude) {
						files = files.filter(file => !file.match(contextDependency.exclude));
					}
				}
				return files;
			});
