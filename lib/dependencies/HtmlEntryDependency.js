	...
	apply(dependency, source, templateContext) {
		...
		if (inject !== false) {
			...
			if (isStylesheet) {
				...
				// Stylesheets always land in <head> when the page has one —
				// html-webpack-plugin and Vite put styles in head regardless of
				// script placement — but never behind the first script tag, so a
				// later entry's CSS still loads ahead of every script.
				const tagCssAt = dep.cssAnchor === -1 ? dep.tagStart : dep.cssAnchor;
				const cssAt =
					dep.headAnchor >= 0 && dep.headAnchor < tagCssAt
						? dep.headAnchor
						: tagCssAt;
