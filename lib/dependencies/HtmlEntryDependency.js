 ...
 * Exported for unit tests: the tie paths need index collisions a real
 * compilation can't produce.
	...
	apply(dependency, source, templateContext) {
		...
		if (inject !== false) {
			...
			if (isStylesheet) {
				...
				// later entry's stylesheet still loads ahead of every script — under
				// inject:"head" too, where the head anchor may sit after a script.
				const tagCssAt = dep.cssAnchor === -1 ? dep.tagStart : dep.cssAnchor;
				const cssAt = headAt >= 0 && headAt < tagCssAt ? headAt : tagCssAt;
		...
		if (
			...
			if (hints.length > 0) {
				...
				// hints are the document's first fetches; fall back to the
				// pre-script anchor when the page has no written `<head>` (e.g. an
				// HTML fragment).
				const hintAt =
					dep.headOpenEnd >= 0
						? dep.headOpenEnd
...
module.exports.compareCssChunkOrder = compareCssChunkOrder;
