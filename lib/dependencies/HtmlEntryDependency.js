			...
			if (resourceHints !== undefined && resourceHints !== false) {
				...
				const rel = prefetchMode
					...
					: isModuleOutput
						? "modulepreload"
						: "preload";
				// `output.resourceHints.fetchPriority` — only emitted here, on the hints
				// written into the HTML: the preload scanner sees those, so the attribute
				// is honored (browsers ignore it on runtime-injected `modulepreload`).
				const autoFetchPriorityAttr =
					rhOptions && rhOptions.fetchPriority
						? ` fetchpriority="${escapeAttr(rhOptions.fetchPriority)}"`
						: "";
				...
				if (
					...
					for (const chunk of orderedChunks) {
						...
						// fetching twice.
						const attrs = `${dep.copyableAttrsText}${crossOrigin}${integrityAttr}${autoFetchPriorityAttr}`;
