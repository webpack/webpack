 ...
 * @property {{ rel: "modulepreload" | "preload" | "prefetch", chunk: string }[]} defaultHints the auto initial-graph hints
 */
	...
	apply(dependency, source, templateContext) {
		...
		if (
			...
			if (hints.length > 0) {
				...
				// `output.modulePreloadPolyfill: false` opts out (strict CSP).
				if (
					!runtimeTemplate.supportsModulePreload() &&
					compilation.outputOptions.modulePreloadPolyfill !== false &&
