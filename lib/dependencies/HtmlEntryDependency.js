...
HtmlEntryDependency.Template = class HtmlEntryDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		if (inject !== false) {
			...
			let cssAt = -1;
			if (isStylesheet) {
				// A JS-only shared or runtime chunk can sit in a stylesheet entrypoint,
				// so clone the `<link>` only for siblings that really emit a `.css`
				// asset; otherwise it points at a file that does not exist.
