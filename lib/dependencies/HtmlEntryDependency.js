...
/** @typedef {import("../serialization/ObjectMiddleware").ObjectDeserializerContext<[string, string, HtmlEntryElementKind, number, number, boolean, string, number, boolean, number, (Range | null), (Range | null), boolean]>} ObjectDeserializerContext */
/** @typedef {import("../serialization/ObjectMiddleware").ObjectSerializerContext<[string, string, HtmlEntryElementKind, number, number, boolean, string, number, boolean, number, (Range | null), (Range | null), boolean]>} ObjectSerializerContext */
 ...
 * @property {Chunk[]} chunks the entrypoint's initial chunks in load order
 * @property {import("../Compilation")} compilation the compilation
 * @property {{ rel: "modulepreload" | "preload", chunk: string }[]} defaultHints the auto initial-graph hints
 */
 ...
 * @param {{ href: string, as?: string, type?: string, crossorigin?: string, integrity?: string, media?: string }} attrs attributes (already href-resolved; `crossorigin`/`integrity` are full ` name="…"` spans)
...
HtmlEntryDependency.Template = class HtmlEntryDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		const siblings = [];
		const headClose =
			inject === "head" ? originalContent.search(/<\/head>/i) : -1;
		...
		const headAt =
			headClose < 0
				? -1
				: dep.tagStart >= 0 && dep.tagStart < headClose
					? dep.tagStart
					: headClose;
		...
		// Resource hints are opt-in (unlike Vite/Next, webpack already loads the
		// initial chunks via parallel `<script>` tags, so there is no waterfall to
		// prevent by default). `output.html.resourceHints` requests them: `true`
		// preloads the entry's initial dependency chunks (runtime, vendor, split);
		// an array or function adds custom hints (a function may spread the auto
		// `defaultHints`). Only for extracted pages — a JS-string HTML export would
		// embed the sentinels in the JS chunk.
		const resourceHints =
			typeof htmlOption === "object" ? htmlOption.resourceHints : undefined;
		if (
			...
			// The auto initial-graph preload as descriptors carrying their exact tag,
			// built only when needed (`true` emits them, a function may spread them).
			// The entry chunk is skipped — it's already the `<script src>`; ESM uses
			// `modulepreload` (fetch + parse), classic output `preload as="script"`.
			// Dynamic `import()`s keep webpack's on-demand runtime, which already
			// loads a chunk and its siblings in parallel.
			// `_tag` (the exact prebuilt tag) is reused verbatim when a function
			// returns this default hint, keeping its exact attributes.
			/** @type {{ rel: "modulepreload" | "preload", chunk: string, _tag: string }[]} */
			const defaultHints = [];
			if (resourceHints === true || typeof resourceHints === "function") {
				for (const chunk of orderedChunks) {
					if (
						chunk === entryChunk ||
						isChunkInlined(chunk) ||
						!chunkHasJs(chunk, chunkGraph)
					) {
						continue;
					}
					const href = HtmlGenerator.makeChunkUrlSentinel(chunk, "javascript");
					const integrityAttr = integrityOn
						? ` integrity="${HtmlGenerator.makeChunkIntegritySentinel(
								chunk,
								"javascript"
						...
						: "";
					// Mirror the entry tag's `nonce`/`crossorigin`/`referrerpolicy` (and
					// `output.crossOriginLoading`) so the request matches the sibling
					// `<script>`'s and the browser reuses it instead of fetching twice.
					const attrs = `${dep.copyableAttrsText}${crossOrigin}${integrityAttr}`;
					const tag = isModuleOutput
						? `<link rel="modulepreload" href="${href}"${attrs}>`
						: `<link rel="preload" as="script" href="${href}"${attrs}>`;
					defaultHints.push({
						rel: isModuleOutput ? "modulepreload" : "preload",
						chunk: chunk.name || String(chunk.id),
						_tag: tag
					});
				}
			}
				...
				if (Array.isArray(descriptors) && descriptors.length > 0) {
					...
					for (const desc of descriptors) {
						...
						if (prebuilt) {
							...
							for (const tag of resolveResourceHint(desc, ctx)) hints.push(tag);
						}
					}
				}
			}
			...
			if (hints.length > 0) {
				...
				if (
					!runtimeTemplate.supportsModulePreload() &&
				...
				// Insert right after the `<head>` open tag so the hints are the
				// document's first fetches; fall back to the pre-script anchor when
				// the page has no `<head>` (e.g. an HTML fragment).
				const headOpen = /<head(?:\s[^>]*)?>/i.exec(originalContent);
				const hintAt = headOpen
