...
HtmlEntryDependency.Template = class HtmlEntryDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// default. `output.resourceHints.chunks` requests them: `true` preloads
		// the entry's initial dependency chunks (runtime, vendor, split); an
		// array or function adds custom hints (a function may spread the auto
		// `defaultHints`). Only for extracted pages — a JS-string HTML export
		// would embed the sentinels in the JS chunk. `output.resourceHints.assets`
		// adds `<link>` tags for URL-referenced assets (fonts, images) reachable
		// from this entry.
		const resourceHintResolver =
			ResourceHintPlugin.getCompilationResolver(compilation);
		const resourceHints = resourceHintResolver.chunks;
		if (
			isExtractedHtml &&
			...
			if (resourceHints !== undefined && resourceHints !== false) {
				...
				const preloadMode =
					resourceHints === true || resourceHints === "preload";
				...
				const rel = prefetchMode
					...
					: isModuleOutput
						? "modulepreload"
						: "preload";
				/** @type {{ rel: "modulepreload" | "preload" | "prefetch", chunk: string, _tag: string }[]} */
				const defaultHints = [];
				if (
					preloadMode ||
					prefetchMode ||
