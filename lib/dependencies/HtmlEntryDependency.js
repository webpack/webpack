...
/** @typedef {import("../serialization/ObjectMiddleware").ObjectDeserializerContext<[string, string, HtmlEntryElementKind, number, number, boolean, string, number, boolean, number, boolean]>} ObjectDeserializerContext */
/** @typedef {import("../serialization/ObjectMiddleware").ObjectSerializerContext<[string, string, HtmlEntryElementKind, number, number, boolean, string, number, boolean, number, boolean]>} ObjectSerializerContext */
...
const cloneTagWithUrl = (
	...
	// Drop the author's `integrity` — it's content-specific and would be wrong
	// for a different chunk's file. A correct per-chunk one is re-added below
	// from `output.html.integrity`. The match handles all three quoting styles
	// (`"…"`, `'…'`, unquoted) and the bare-attribute form; the `includes` gate
	// skips the regex for the common no-SRI tag.
	...
	// Insert `crossorigin` (from `output.crossOriginLoading`) and the per-chunk
	// `integrity` sentinel right after the tag name (its parse-time offset). The
	// type/integrity edits above only touch text at or after the name boundary,
	// so the offset stays valid.
	const inserted = crossOrigin + integrity;
	if (inserted) {
		body =
			body.slice(0, tagNameEndInTag) + inserted + body.slice(tagNameEndInTag);
	}
...
HtmlEntryDependency.Template = class HtmlEntryDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// inserted right after the tag name so they sit alongside `type="module"`.
		if (!entryInlined && tagIsNative && dep.tagNameEnd >= 0) {
