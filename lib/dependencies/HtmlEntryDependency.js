...
/** @typedef {import("../serialization/ObjectMiddleware.js").ObjectDeserializerContext<[string, string, HtmlEntryElementKind, number, number, boolean, string, number, boolean, number, (Range | null), (Range | null), boolean]>} ObjectDeserializerContext */
/** @typedef {import("../serialization/ObjectMiddleware.js").ObjectSerializerContext<[string, string, HtmlEntryElementKind, number, number, boolean, string, number, boolean, number, (Range | null), (Range | null), boolean]>} ObjectSerializerContext */
...
const getEntrypointChunksInLoadOrder = (entrypoint) => {
	...
	/** @type {Set<import("../ChunkGroup.js").default>} */
	const visitedGroups = new Set();
	const walk = (/** @type {import("../ChunkGroup.js").default} */ group) => {
	...
	apply(dependency, source, templateContext) {
		...
		const isExtractedHtml = chunkGraph
			.getModuleSourceTypes(
				/** @type {import("../Module.js").default} */ (templateContext.module)
			)
			.has("html");
		const entryInlined =
			isExtractedHtml &&
		...
		if (isStylesheet) {
			...
			// Comparison instead of subtraction: both indices may be `Infinity`
			// (documented fallback), and `Infinity - Infinity` is `NaN`. Stable
			// `Array#sort` keeps insertion order for those ties.
...
export default HtmlEntryDependency;

export { HtmlEntryDependency as "module.exports" };
