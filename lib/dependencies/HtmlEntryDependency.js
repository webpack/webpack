...
HtmlEntryDependency.Template = class HtmlEntryDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// Resource hints are opt-in: the initial chunks already load through
		// parallel `<script>` tags, so there is no default waterfall to prevent.
		// Extracted pages only — a JS-string export would embed the sentinels.
		...
		if (
			...
			if (resourceHints !== undefined && resourceHints !== false) {
				// Descriptors for the auto initial-graph preload, carrying their exact
				// tag. The entry chunk is skipped, being the `<script src>` already, and
				// `prefetch` takes no `as` or `integrity`, which the spec omits.
