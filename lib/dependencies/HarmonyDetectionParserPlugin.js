...
const T_IMPORT_DECLARATION = SoaAst.TYPE.ImportDeclaration;
const T_EXPORT_NAMED_DECLARATION = SoaAst.TYPE.ExportNamedDeclaration;
const T_EXPORT_DEFAULT_DECLARATION = SoaAst.TYPE.ExportDefaultDeclaration;
const T_EXPORT_ALL_DECLARATION = SoaAst.TYPE.ExportAllDeclaration;

/**
 * Scans the top level for module syntax on the columns when the Program is
 * store-backed, so the detection doesn't materialize `body`.
 * @param {Program} ast the program
 * @returns {boolean} true when an import/export statement is present
 */
const hasModuleSyntax = (ast) => {
	...
	if (
		...
		const len = store.listLens[rootId];
		const start = store.listStarts[rootId];
		const { flat, types } = store;
		for (let i = 0; i < len; i++) {
			const t = types[flat[start + i]];
			if (
				t === T_IMPORT_DECLARATION ||
				t === T_EXPORT_NAMED_DECLARATION ||
				t === T_EXPORT_DEFAULT_DECLARATION ||
				t === T_EXPORT_ALL_DECLARATION
			) {
				return true;
			}
		}
...
module.exports = class HarmonyDetectionParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.program.tap(PLUGIN_NAME, (ast) => {
			...
			const isHarmony = isStrictHarmony || hasModuleSyntax(ast);
			if (isHarmony) {
