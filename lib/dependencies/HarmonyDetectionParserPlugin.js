...
const HarmonyAwaitDependency = require("./HarmonyAwaitDependency");
		...
		parser.hooks.topLevelAwait.tap(PLUGIN_NAME, (node) => {
			...
			if (node && node.type === "AwaitExpression") {
				module.addPresentationalDependency(
					new HarmonyAwaitDependency(node.range, node.argument.range)
				);
			} else {
				// If it's something else (like `for await...of`), fallback to Webpack's default crash warning.
