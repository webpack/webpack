...
module.exports = class HarmonyDetectionParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.topLevelAwait.tap("HarmonyDetectionParserPlugin", () => {
			...
			if (!parser.state.harmonyModule) {
				this.markAsHarmony(parser, true);
			}
	...
	markAsHarmony(parser, asyncHarmony = false) {
		const { isAsync, isHarmony, isStrictHarmony } = this.moduleInfoCache.get(
			parser.state.module
		);

		if (isHarmony || asyncHarmony) {
			const module = parser.state.module;
			...
			module.buildMeta.async = isAsync || asyncHarmony;
