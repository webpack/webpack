...
URLDependency.Template = class URLDependencyTemplate extends (
	...
	apply(dependency, source, templateContext) {
		...
		// WHY: ESM module output emits the analyzable
		// `new URL("./asset", import.meta.url)` form — a literal specifier, no runtime
		// helpers — so other bundlers and webpack itself can statically follow the
		// asset, while `url: "relative"` keeps the runtime form. A prefetch or preload
		// hint does not force it: the `<link>` is emitted separately at chunk startup
		// (`StartupAssetHintRuntimeModule`), independent of the call site.
		if (
			!dep.relative &&
...
makeSerializable(URLDependency, "webpack/lib/dependencies/url/URLDependency");
registerLegacyRequest(URLDependency, "webpack/lib/dependencies/URLDependency");
