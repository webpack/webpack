...
/** @typedef {import("../optimize/InnerGraphPlugin").GraphNode} GraphNode */
...
module.exports = class HarmonyImportDependencyParserPlugin {
	...
	apply(parser) {
		 ...
		 * @returns {void}
		 */
		const addLinkToGraphNode = (graphNode, dep) => {
			if (!graphNode) return;
			const currentTopLevelSymbol = InnerGraph.getTopLevelNode(parser.state);
			if (!currentTopLevelSymbol) {
				graphNode.addLink(true);
			} else {
				graphNode.addLink(currentTopLevelSymbol);
			}

			InnerGraph.m = InnerGraph.m || new Set();
			InnerGraph.hooks.usedByExports
				.for(graphNode)
				.tap("HarmonyImportSpecifierDependency", value => {
					InnerGraph.m.add(dep);
		...
		parser.hooks.expression
			...
			.tap("HarmonyImportDependencyParserPlugin", expr => {
				...
				const graphNode = InnerGraph.getNode(parser.state, expr, settings.name);
				addLinkToGraphNode(graphNode, dep);
				return true;
			});
