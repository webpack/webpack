...
class HarmonyImportDependencyParserPlugin {
	...
	apply(parser) {
		...
		// default member-chain path emits `<importVar>.a.foo`, whose `.a` getter
		// evaluates the deferred module. So only the bare `ns` is walked and
		// replaced by the proxy, leaving `.foo = value` as plain code.
