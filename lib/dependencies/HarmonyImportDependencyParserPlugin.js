		...
		parser.hooks.expressionMemberChain
			...
			.tap(
				...
				(expression, members, membersOptionals, memberRangeStarts) => {
					...
					const startsWithNamespace = settings.ids.length === 0;
					const dep = new HarmonyImportSpecifierDependency(
						settings.source,
						settings.sourceOrder,
						ids,
						...
						exportPresenceMode,
						settings.assertions,
						memberRangeStarts.slice(
							startsWithNamespace ? 1 : 0, // skip one if starting with an explicit namespace
							nonOptionalMembers.length
						)
					);
					dep.referencedPropertiesInDestructuring =
						parser.destructuringAssignmentPropertiesFor(expr);
