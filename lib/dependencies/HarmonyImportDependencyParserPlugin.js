...
const getHarmonyImportSpecifierGuardParserPlugin = memoize(() =>
	require("./HarmonyImportSpecifierGuardParserPlugin")
);
...
module.exports = class HarmonyImportDependencyParserPlugin {
	 ...
	 */
	apply(parser) {
		const HarmonyImportSpecifierGuardParserPlugin =
			getHarmonyImportSpecifierGuardParserPlugin();
		const guardParserPlugin = new HarmonyImportSpecifierGuardParserPlugin(
			this.options
		);
		guardParserPlugin.apply(parser);
		...
		parser.hooks.expression
			...
			.tap(PLUGIN_NAME, (expr) => {
				...
				const dep = new HarmonyImportSpecifierDependency(
					...
					/** @type {Range} */
					(expr.range),
					guardParserPlugin.getExportPresenceMode(
						parser,
						settings.ids.slice(1)
					),
					settings.phase,
					settings.attributes,
					[]
			...
			.tap(
				...
				(expression, members, membersOptionals, memberRanges) => {
					...
					const dep = new HarmonyImportSpecifierDependency(
						...
						guardParserPlugin.getExportPresenceMode(parser, ids.slice(1)),
						settings.phase,
						settings.attributes,
						ranges
					);
