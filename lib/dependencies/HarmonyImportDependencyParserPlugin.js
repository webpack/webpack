...
class HarmonyImportDependencyParserPlugin {
	...
	apply(parser) {
		 ...
		 * @param {AssignmentExpression | UpdateExpression} expr the write
		...
		parser.hooks.update.for(harmonySpecifierTag).tap(PLUGIN_NAME, (expr) => {
			markImportedBindingWrite(expr, /** @type {Identifier} */ (expr.argument));
		});
		...
		parser.hooks.updateMemberChain
			.for(harmonySpecifierTag)
			.tap(PLUGIN_NAME, (expr, members) => {
				const settings = /** @type {HarmonySettings} */ (parser.currentTagData);
				if (isNamespaceMemberWrite(settings, members)) {
					assignmentTarget = /** @type {MemberExpression} */ (expr.argument);
				}
			});
