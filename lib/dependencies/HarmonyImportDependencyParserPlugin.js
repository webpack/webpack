...
class HarmonyImportDependencyParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.classExtendsExpression.tap(
			PLUGIN_NAME,
			(_expression, classy) => {
				if (classy.id) classHeritageNames.add(classy.id.name);
			}
		);
		parser.hooks.finish.tap(PLUGIN_NAME, () => {
			...
			if (classHeritageNames.size > 0) {
				/** @type {JavascriptModuleBuildInfo} */
				(parser.state.module.buildInfo).classHeritageNames = new Set(
					classHeritageNames
				);
			}
			declaredSpecifiers.length = 0;
