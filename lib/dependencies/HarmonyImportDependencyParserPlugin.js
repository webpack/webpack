 ...
 * @param {any} node node
 * @param {boolean} deferImportEnabled defer import enabled
 * @returns {{sync: boolean, defer: boolean}} import attributes
 */
function getImportMode(parser, node, deferImportEnabled) {
	const result = { sync: false, defer: false };
	let syncUsedInAttributes = false;
	const assertions = getAssertions(node);
	if (assertions) {
		if ("webpackSync" in assertions) {
			syncUsedInAttributes = true;
			if (assertions.webpackSync === "true") {
				result.sync = true;
			} else if (assertions.webpackSync === "false") {
				result.sync = false;
			} else {
				parser.state.module.addWarning(
					new UnsupportedFeatureWarning(
						`\`webpackSync\` expected a string that is "true" or "false", but received: ${assertions.webpackSync}.`,
						node.loc
					)
				);
			}
	...
	if ("webpackSync" in options) {
		if (syncUsedInAttributes && options.webpackSync !== result.sync) {
			parser.state.module.addWarning(
				new UnsupportedFeatureWarning(
					"webpackSync has different values in the magic comment and import attributes.",
					node.loc
				)
			);
		}
		if (typeof options.webpackSync === "boolean") {
			result.sync = options.webpackSync;
		} else {
			parser.state.module.addWarning(
				new UnsupportedFeatureWarning(
					"webpackSync magic comment expected a boolean value.",
					node.loc
				)
			);
		}
	...
	if (!deferImportEnabled) result.defer = false;
	return result;
}
	...
	apply(parser) {
		...
		parser.hooks.import.tap(
			...
			(statement, source) => {
				...
				const { defer, sync } = getImportMode(
					parser,
					statement,
					this.deferImport
				);
...
module.exports.getImportAttributes = getImportMode;
