...
module.exports = class HarmonyImportDependencyParserPlugin {
	...
	apply(parser) {
		...
		/**
		 * @param {Expression} expression guard test expression
		 * @returns {{ consequent: GuardFrame, alternate: GuardFrame | undefined } | void} guard frames when collected
		 */
		const collectGuardsHandler = (expression) => {
		...
		parser.hooks.collectGuards.tap(PLUGIN_NAME, collectGuardsHandler);
		// without a harmony import no guard test can reference a specifier
		parser.registerColumnGate(
			parser.hooks.collectGuards,
			collectGuardsHandler,
			() =>
				!parser.scope.isAsmJs && Boolean(parser.state.lastHarmonyImportOrder)
		);
	}
};
