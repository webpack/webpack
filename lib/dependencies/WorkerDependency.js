...
/**
 * Resolves the static literal specifier (already quoted) for the worker's
 * `new URL(<here>, import.meta.url)`, or `null` when the worker chunk filename or the
 * publicPath isn't statically known (a content hash or a dynamic/relative publicPath).
 * @param {WorkerDependency} dep the dependency
 * @param {Chunk} chunk the worker entrypoint chunk
 * @param {DependencyTemplateContext} templateContext the template context
 * @returns {string | null} a JS string literal, or `null` to fall back to the runtime form
 */
const getAnalyzableWorkerSpecifier = (dep, chunk, templateContext) => {
	...
	// `workerPublicPath` wins over `output.publicPath` (mirrors the runtime branch).
	const workerPublicPath = dep.options.publicPath;
	if (workerPublicPath) {
		return workerPublicPath.includes("[")
			? null
			: JSON.stringify(workerPublicPath + filename);
	}
	...
	apply(dependency, source, templateContext) {
		...
		const analyzableSpecifier =
			!dep.options.resourceHint && runtimeTemplate.supportsAnalyzableEsm()
				? getAnalyzableWorkerSpecifier(dep, chunk, templateContext)
				: null;
