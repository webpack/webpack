	...
	apply(dependency, source, templateContext) {
		...
		if (dep.crossOriginLoading) {
			source.replace(
				dep.range[0],
				dep.range[1] - 1,
				`/* worker import */ "data:application/javascript," + ` +
					`encodeURIComponent("` +
					`__webpack_public_path__ = " + JSON.stringify(${RuntimeGlobals.publicPath}) + ";` +
					`__webpack_base_uri__ = " + JSON.stringify(${RuntimeGlobals.baseURI}) + ";` +
					`importScripts(" + JSON.stringify(new URL(${
