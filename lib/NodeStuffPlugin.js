	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				 ...
				 * @returns {void}
				 */
				const handler = (parser, parserOptions, { dirname, filename }) => {
					 ...
					 * @returns {void}
					 */
					const setUrlModuleConstant = (expressionName, fn, identifier) => {
						parser.hooks.expression
							.for(expressionName)
					...
					if (localOptions.__filename) {
						switch (localOptions.__filename) {
							case "mock":
								setConstant(filename, "/index.js", "__filename");
								break;
							case "warn-mock":
								setConstant(
									filename,
