...
class JavascriptGenerator {
	...
	sourceBlock(
		...
		for (let i = 0; i < block.dependencies.length; i++) {
			...
			if (
				dependency instanceof RequireHeaderDependency &&
				previousDependency instanceof CommonJsRequireDependency &&
				previousDependency.module &&
				previousDependency.module.externalType === "amd"
			) {
				dependency = block.dependencies[i] = new ConstDependency(
					"",
					dependency.range
				);
			}
