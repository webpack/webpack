...
/**
 * Shape of the per-method cache backend exposed (privately) by
 * enhanced-resolve's `CachedInputFileSystem`. Only `purge` is consumed
 * here, to invalidate a single `readdir` entry without disturbing other
 * backends (see `purgeChanges` below).
 * @typedef {object} CacheBackend
 * @property {(what?: string | string[] | Set<string>) => void} purge
 */

/**
 * `InputFileSystem` augmented with the optional `_readdirBackend` slot
 * that `CachedInputFileSystem` instances carry.
 * @typedef {InputFileSystem & { _readdirBackend?: CacheBackend }} InputFileSystemWithBackends
 */
...
class NodeWatchFileSystem {
	...
	watch(
		...
		// file events. `fs.purge(dir)` uses `key.startsWith(dir)` internally,
		// which would wipe the stat cache of every file inside the directory
		// even though only file-level events actually invalidate file stats.
		// For directories we explicitly watch, invalidate only the directory's
		// own `readdir` entry; file-level events in `changes` already handle
		// their stat invalidation (and parent `readdir` via `purgeParent`).
		/**
		...
		const purgeChanges = (changes, removals) => {
			const fs =
				/** @type {InputFileSystemWithBackends | null} */
				(this.inputFileSystem);
			if (!fs || !fs.purge) return;
			const readdirBackend = fs._readdirBackend;
			const canNarrowPurge =
				readdirBackend && typeof readdirBackend.purge === "function";
			if (changes) {
				for (const item of changes) {
					if (canNarrowPurge && directoriesSet.has(item)) {
						/** @type {CacheBackend} */ (readdirBackend).purge(item);
					} else {
						fs.purge(item);
					}
				}
