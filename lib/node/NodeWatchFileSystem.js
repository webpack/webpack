...
class NodeWatchFileSystem {
	...
	watch(files, dirs, missing, startTime, options, callback, callbackUndelayed) {
		...
		let changes = [];
		if(this.files)
			changes.concat(files.concat(this.files).filter(f => !files.includes(f) || !this.files.includes(f)));
		if(this.dirs)
			changes.concat(dirs.concat(this.dirs).filter(f => !dirs.includes(f) || !this.dirs.includes(f)));
		if(changes)
