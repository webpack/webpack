...
NodeMainTemplatePlugin.prototype.apply = function(mainTemplate) {
	...
	mainTemplate.plugin("require-ensure", function(_, chunk, hash) {
		...
		if(self.asyncChunkLoading) {
			...
			let request = this.applyPluginsWaterfall("asset-path", JSON.stringify("./" + chunkFilename), {
				hash: "\" + " + this.renderCurrentHashCode(hash) + " + \"",
				hashWithLength: function(length) {
					return "\" + " + this.renderCurrentHashCode(hash, length) + " + \"";
				}.bind(this),
	...
	mainTemplate.plugin("hot-bootstrap", function(source, chunk, hash) {
		let hotUpdateChunkFilename = this.outputOptions.hotUpdateChunkFilename;
		let hotUpdateMainFilename = this.outputOptions.hotUpdateMainFilename;
		let chunkMaps = chunk.getChunkMaps();
		let currentHotUpdateChunkFilename = this.applyPluginsWaterfall("asset-path", JSON.stringify(hotUpdateChunkFilename), {
			hash: "\" + " + this.renderCurrentHashCode(hash) + " + \"",
			hashWithLength: function(length) {
				return "\" + " + this.renderCurrentHashCode(hash, length) + " + \"";
			}.bind(this),
		...
		let currentHotUpdateMainFilename = this.applyPluginsWaterfall("asset-path", JSON.stringify(hotUpdateMainFilename), {
			hash: "\" + " + this.renderCurrentHashCode(hash) + " + \"",
			hashWithLength: function(length) {
				return "\" + " + this.renderCurrentHashCode(hash, length) + " + \"";
			}.bind(this)
