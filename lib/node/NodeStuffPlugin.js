...
const CachedConstDependency = require("../dependencies/core/CachedConstDependency");
...
const ExternalModuleDependency = require("./ExternalModuleDependency");
const ExternalModuleInitFragmentDependency = require("./ExternalModuleInitFragmentDependency");

const getNodeStuffInWebError = memoize(() =>
	require("../errors/NodeStuffInWebError")
);
...
const { IMPORT_META_DIRNAME, IMPORT_META_FILENAME, isImportMetaFieldEnabled } =
	ImportMetaPlugin;
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
						...
						hooks.propertyInDestructuring.tap(PLUGIN_NAME, (usingProperty) => {
							if (property === usingProperty.id) {
								if (warning) {
									parser.state.module.addWarning(
										new (getNodeStuffInWebError())(
											parser.getLocation(usingProperty),
											expressionName,
											warning
										)
									);
						...
						hooks.propertyInDestructuring.tap(PLUGIN_NAME, (usingProperty) => {
							if (property === usingProperty.id) {
								...
								if (
									...
									dep.loc = parser.getLocation(usingProperty);
									parser.state.module.addPresentationalDependency(dep);
				 ...
				 * @param {JavascriptParser} parser the parser
				 * @param {NodeOptions} nodeOptions options
				 * @param {{ dirname: string, filename: string, dirnameEnabled?: boolean, filenameEnabled?: boolean }} identifiers options
				 * @returns {void}
				 */
