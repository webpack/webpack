	...
	generate() {
		...
		try {
			if (chunk.hasEntryModule()) {
				const outputName = this.compilation.hooks.assetPath.call(
					this.compilation.outputOptions.filename + "",
					{
						chunk,
						hash: chunk.hash
					},
				...
				const chunkInnerDirsCount = outputName.split(path.sep).length - 1;
				rootOutputDir =
					chunkInnerDirsCount > 0
						? `__dirname + ${JSON.stringify(path.sep)} + ${JSON.stringify(
								Array(chunkInnerDirsCount)
									.fill("..")
									.join(path.sep)
						  )}`
						: "'.'";
