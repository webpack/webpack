...
class ReadFileChunkLoadingRuntimeModule extends RuntimeModule {
	...
	getInstalledChunkHandlers() {
		...
		if (runtimeRequirements.has(RuntimeGlobals.ensureChunkHandlers)) {
			handlers.push([RuntimeGlobals.ensureChunkHandlers, "readFileVm"]);
		}
		...
		if (runtimeRequirements.has(RuntimeGlobals.hmrDownloadUpdateHandlers)) {
			handlers.push([RuntimeGlobals.ensureChunkHandlers, "readFileVmHmr"]);
		}
		if (runtimeRequirements.has(RuntimeGlobals.onChunksLoaded)) {
			handlers.push([RuntimeGlobals.onChunksLoaded, "readFileVm"]);
		}
		return handlers;
	}
	...
	generate() {
		...
		const rootOutputDir = runtimeTemplate.chunkRootOutputDir(chunk, false);
