...
class ReadFileChunkLoadingRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		conditionMap[chunk.id] = true;
		const hasJsMatcherIncludeSelf = compileBooleanMatcher(conditionMap);
		if (hasJsMatcherIncludeSelf === false)
		...
		return Template.asString([
			...
			withOnChunkLoad
				? `${
						RuntimeGlobals.onChunksLoaded
				  }.readFileVm = ${runtimeTemplate.basicFunction("chunkId", [
						`return installedChunks[chunkId] === 0${
							hasJsMatcherIncludeSelf === true
								? ""
								: ` || !(${hasJsMatcherIncludeSelf("chunkId")})`
						};`
				  ])};`
				: "// no on chunks loaded",
			"",
