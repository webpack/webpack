...
class ReadFileCompileAsyncWasmPlugin {
	...
	apply(compiler) {
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			// The call site already passes a complete `URL`; `readFile` takes one as-is, so
			// neither the `import.meta.url` base nor the `__dirname` join is needed.
			...
			const generateLoadBinaryCode = this._import
				? (path) =>
						Template.asString([
							...
							Template.indent([
								`readFile(${baked ? path : compilation.runtimeTemplate.importMetaUrl(path)}, (err, buffer) => {`,
								Template.indent([
						...
						Template.asString([
							...
							Template.indent([
								...
								Template.indent([
									...
									"",
									`readFile(${baked ? path : `join(__dirname, ${path})`}, function(err, buffer){`,
									Template.indent([
										"if (err) return reject(err);",
										"",
										"// Fake fetch response",
