...
class RuleSetCompiler {
	 ...
	 * value, and a relative pattern matches at any depth.
	 * @param {string} path current path
	...
	compileGlobCondition(path, glob) {
		if (Array.isArray(glob)) {
			return this.combineConditionsOr(
				glob.map((item, i) => this.compileGlobCondition(`${path}[${i}]`, item))
			);
		}
		if (typeof glob !== "string") {
			throw this.error(
				path,
				glob,
				`Unexpected ${typeof glob} when glob pattern was expected`
			);
		}
		const match = createPathGlobMatcher(glob);
		if (match === null) {
			throw this.error(path, glob, "Invalid glob pattern");
		}
		return {
...
const globReferencesExtension = (glob, extension) => {
	if (typeof glob === "string") return glob.includes(extension);
	return (
		Array.isArray(glob) &&
