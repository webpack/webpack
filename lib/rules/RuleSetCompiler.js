 ...
 * @property {Set<string>} conditionProperties the effect data properties any rule matches on
...
class RuleSetCompiler {
	...
	compile(ruleSet) {
		...
		collectConditionProperties(rules, conditionProperties);
	...
	compileCondition(path, condition) {
		...
		if (typeof condition === "string") {
			const prefix = fileUrlToPath(condition);
			return {
				matchWhenEmpty: prefix.length === 0,
				fn: (str) => typeof str === "string" && str.startsWith(prefix)
			};
		}
...
/**
 * Collects the effect data each rule matches on, so a consumer can tell what a match
 * depends on. A nested property contributes the data property it reads from.
 * @param {CompiledRule[]} rules compiled rules
 * @param {Set<string>} result where property names are collected
 * @returns {void}
 */
const collectConditionProperties = (rules, result) => {
	for (const rule of rules) {
		for (const { property } of rule.conditions) {
			result.add(Array.isArray(property) ? property[0] : property);
		}
		if (rule.rules) collectConditionProperties(rule.rules, result);
		if (rule.oneOf) collectConditionProperties(rule.oneOf, result);
	}
};
