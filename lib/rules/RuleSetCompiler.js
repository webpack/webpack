 ...
 * @property {(effectData: EffectData, matchedProperties?: Set<string>) => Effect[]} exec execute the rule set
...
class RuleSetCompiler {
	...
	compile(ruleSet) {
		 ...
		 * @param {Set<string> | undefined} matchedProperties collects the properties conditions of matching rules read
		 * @returns {boolean} true, if the rule has matched
		 */
		const execRule = (data, rule, effects, matchedProperties) => {
			...
			rule.used = true;
			if (matchedProperties !== undefined) {
				for (const condition of rule.conditions) {
					const p = condition.property;
					matchedProperties.add(Array.isArray(p) ? p[0] : p);
				}
			}
			...
			if (rule.rules) {
				for (const childRule of rule.rules) {
					execRule(data, childRule, effects, matchedProperties);
				}
			}
			if (rule.oneOf) {
				for (const childRule of rule.oneOf) {
					if (execRule(data, childRule, effects, matchedProperties)) {
						break;
					}
				}
			}
		...
		return {
			references: refs,
			exec: (data, matchedProperties) => {
				/** @type {Effect[]} */
				const effects = [];
				for (const rule of rules) {
					execRule(data, rule, effects, matchedProperties);
				}
				return effects;
			},
			unusedRules: () => {
