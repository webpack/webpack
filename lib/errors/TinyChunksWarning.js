...
class TinyChunksWarning extends WebpackError {
	 ...
	 * @param {number} minSize the floor they fall under
	 */
	constructor(chunks, total, totalSize, minSize) {
		const list = chunks
			.map((chunk) => `\n  ${chunk.name} (${formatSize(chunk.size)})`)
			.join("");

		super(
			`tiny chunks: ${total} chunks are loaded on demand but carry less than ${formatSize(
				minSize
			)}, ${formatSize(totalSize)} between them:${list}\nEach costs a request of its own, and that floor is 'optimization.splitChunks.minSize' — the size below which webpack itself declines to make a chunk. Giving several 'import()' calls one 'webpackChunkName' groups them into a single chunk.\nFor more info visit https://webpack.js.org/api/module-methods/#magic-comments`
		);
