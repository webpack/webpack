...
/**
 * Error raised when an entry asks for `mergeExports` but webpack cannot build
...
class UnmergeableEntryExportsError extends WebpackError {
	...
	constructor(chunk, reason) {
		super(
			`Entry "${chunk.name || chunk.id}" can't merge the exports of its modules because ${reason}. Remove 'mergeExports' from the entry, or re-export the modules from a single entry module instead.`
		);
