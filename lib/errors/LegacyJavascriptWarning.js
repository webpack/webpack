	...
	constructor(packages, totalSize, features) {
		...
		super(
			`legacy javascript: ${formatSize(
				totalSize
			)} of the build emulates features the target already has natively:${list}\n'output.environment' says this target supports ${features.join(
				", "
			)}, so whatever selects these polyfills — usually '@babel/preset-env' — is compiling for older browsers than webpack is. Lining its targets up with the browserslist webpack reads drops them.\nFor more info visit https://webpack.js.org/configuration/target/#browserslist`
		);
