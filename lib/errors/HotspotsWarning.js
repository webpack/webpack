...
class HotspotsWarning extends WebpackError {
	...
	constructor(hotspots, total, hooks) {
		...
		super(
			`hotspots: ${total} ${total === 1 ? "thing holds" : "things hold"} the main thread long enough to be worth looking at:${list}${byHook}\nThis is the time each one spent running its own code, with anything it called out to counted against that instead — so it is what the loader or plugin itself costs, not what it waited for. 'ProfilingPlugin' records the same work as a trace when the ordering matters.\nFor more info visit https://webpack.js.org/plugins/profiling-plugin/`
		);
