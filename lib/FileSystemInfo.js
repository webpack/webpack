...
const getEsmModuleMap = memoize(() => {
	try {
		// @ts-expect-error
		// eslint-disable-next-line node/no-missing-require
		return require("internal/process/esm_loader").ESMLoader.moduleMap;
	} catch (err) {
		return null;
	}
});
			...
			({ type, context, path, expected }, callback) => {
				...
				switch (type) {
					...
					case RBDT_FILE_DEPENDENCIES: {
						...
						// Check esm cache for the module
						if (supportsEsm) {
							const moduleMap = getEsmModuleMap();
							if (moduleMap) {
								if (!this._warnAboutExperimentalEsmTracking) {
									this.logger.info(
										"To efficiently track dependencies of EcmaScript Modules in the Node.js dependency graph access to Node.js internals is required.\n" +
											"Run node with `node --expose-internals` to grant access to the ESM dependency graph.\n" +
											"E. g. run `node --expose-internals node_modules/webpack/bin/webpack.js ...`.\n" +
											"Without access to internals, webpack has to parse the files to guess dependencies and this can lead to expensive and incorrect tracking."
									);
									this._warnAboutExperimentalEsmTracking = true;
								}
								const context = dirname(this.fs, path);
								const moduleJob = moduleMap.get(pathToFileURL(path).href);
								if (moduleJob) {
									moduleJob.linked
										.then(dependencies => {
											if (dependencies && dependencies.length > 0) {
												const deps = moduleJob.module.getStaticDependencySpecifiers();
												for (let i = 0; i < dependencies.length; i++) {
													const childPath = fileURLToPath(
														dependencies[i].module.url
													);
													queue.push({
														type: RBDT_FILE,
														path: childPath
								...
								if (!this._warnAboutExperimentalEsmTracking) {
									this.logger.info(
										"Node.js doesn't offer a (nice) way to introspect the ESM dependency graph yet.\n" +
											...
											"An alternative experimental more efficient way to track dependencies of EcmaScript Modules in the Node.js dependency graph is available, but needs access to Node.js internals.\n" +
											"Run node with `node --expose-internals` to grant access to the ESM dependency graph.\n" +
											"E. g. run `node --expose-internals node_modules/webpack/bin/webpack.js ...`."
									);
									this._warnAboutExperimentalEsmTracking = true;
