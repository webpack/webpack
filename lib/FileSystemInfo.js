...
class ResolveResults extends Map {
	serialize({ write, writePath }) {
		write(this.size);

		for (const key of this.keys()) {
			const [prefix, context, path] = key.split("\n");
	...
	static deserialize({ read, readPath }) {
		const size = read();
		const map = new Map();
		const keys = [];
		...
		for (let i = 0; i < size; i++) {
			const prefix = read();
			const context = readPath();
			const path = readPath();

			keys.push(`${prefix}\n${context}\n${path}`);
		}
		for (let i = 0; i < size; i++) {
			map.set(keys[i], readPath());
		}

		return Object.setPrototypeOf(map, ResolveResults.prototype);
	}
}
...
class FileSystemInfo {
	...
	resolveBuildDependencies(context, deps, callback) {
		...
		/** @type {Map<string, string>} */
		const resolveResults = new ResolveResults();
