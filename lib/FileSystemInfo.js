...
// only the mjs build-dependency walk needs it
const getEsModuleLexer = memoize(() => require("es-module-lexer"));

/** @import { ResolveRequest, ResolveFunctionAsync } from "enhanced-resolve" */
/** @import { HashFunction } from "../declarations/WebpackOptions" */
/** @import { Logger } from "./logging/Logger" */
/** @import WebpackError from "./errors/WebpackError" */
/** @import { JsonObject, IStats, InputFileSystem } from "./util/fs" */
/**
 * @import {
...
/**
 * The managed directory a `snapshot.managedPaths` RegExp matched. The schema
 * documents it as matching that directory, so a capture-less pattern is it.
 * @param {RegExpExecArray} match a match of a managed path RegExp
 * @returns {string} the managed directory
 */
const getManagedPathFromMatch = (match) =>
	match[1] === undefined ? match[0] : match[1];
							...
							if (IS_BUN && module.children.length === 0) {
								this.fs.readFile(path, (err, content) => {
									...
									try {
										...
										for (const dependency of collectCjsRequireSpecifiers(
											source
										)) {
									...
									try {
										...
										for (const imp of imports) {
											try {
												...
												const dependency =
													imp.specifier ||
													parseString(source.slice(imp.start, imp.end).trim());
												...
												push({
													...
													context,
													path: dependency,
													expected: imp.type === "dynamic" ? false : undefined,
													issuer: job
												});
...
FileSystemInfo.Snapshot = Snapshot;
