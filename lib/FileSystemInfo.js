...
const parseString = (str) => {
	...
	while (i < inner.length) {
		...
		// Escape sequence — advance past the backslash
		i++;
		...
		if (esc === "x") {
			...
			if (inner[i + 1] === "{") {
				// Unicode code point escape: \u{N...}
				const closeIdx = inner.indexOf("}", i + 2);
				if (closeIdx !== -1) {
					const codeStr = inner.slice(i + 2, closeIdx);
					if (/^[0-9a-fA-F]+$/.test(codeStr)) {
						result += String.fromCodePoint(parseInt(codeStr, 16));
						i = closeIdx + 1;
					} else {
						result += esc;
						i++;
			...
			// Any other escape (\', \", \\, \`, …): use the character itself
			result += esc;
			i++;
		}
	}
