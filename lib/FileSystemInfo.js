		...
		capturedFiles: for (const path of capturedFiles) {
			...
			if (optimizationEntry.shared > 0) {
				...
				if (nonSharedFiles.size === 0) {
					...
					for (const [path, ts] of snapshot[key]) {
						if (nonSharedFiles.has(path)) continue;
						commonMap.set(path, ts);
						snapshot[key].delete(path);
					}
				...
				for (const path of capturedFiles) {
					const ts = snapshot[key].get(path);
				...
				for (const path of commonMap.keys()) snapshot[key].delete(path);
...
class FileSystemInfo {
	...
	constructor(fs, { managedPaths = [], immutablePaths = [], logger } = {}) {
		...
		this._fileHashesOptimization = new SnapshotOptimization("fileHashes");
		this._contextTimestampsOptimization = new SnapshotOptimization(
			"contextTimestamps"
		);
		this._contextHashesOptimization = new SnapshotOptimization("contextHashes");
	...
	logStatistics() {
		...
		const cached =
			this._statTestedSnapshotsCached + this._statTestedChildrenCached;
		this.logger.log(
			`${
				cached && Math.round((cached * 100) / total)
			}% of tested snapshots cached (${this._statTestedSnapshotsCached} + ${
				this._statTestedChildrenCached
			} / ${
			...
			}), ${this._statTestedEntries} entries tested`
		);
		this.logger.log(
			`File info in cache: ${this._fileTimestamps.size} timestamps ${this._fileHashes.size} hashes`
		);
		this.logger.log(
			`File timestamp snapshot optimization: ${this._fileTimestampsOptimization.getStatisticMessage()}`
		);
		this.logger.log(
			`File hash snapshot optimization: ${this._fileHashesOptimization.getStatisticMessage()}`
		);
		this.logger.log(
			`Directory info in cache: ${this._contextTimestamps.size} timestamps ${this._contextHashes.size} hashes`
		);
		this.logger.log(
			`Directory timestamp snapshot optimization: ${this._contextTimestampsOptimization.getStatisticMessage()}`
		);
		this.logger.log(
			`Directory hash snapshot optimization: ${this._contextHashesOptimization.getStatisticMessage()}`
		);
		this.logger.log(
			`Missing items snapshot optimization: ${this._missingExistenceOptimization.getStatisticMessage()}`
		);
		this.logger.log(
			`Managed items info in cache: ${this._managedItems.size} items`
		);
		this.logger.log(
			`Managed items snapshot optimization: ${this._managedItemInfoOptimization.getStatisticMessage()}`
		);
	}
	...
	createSnapshot(startTime, files, directories, missing, options, callback) {
		...
		const jobDone = () => {
			if (--jobs === 0) {
				...
				if (fileTimestamps.size !== 0) {
					snapshot.fileTimestamps = fileTimestamps;
				...
				if (fileHashes.size !== 0) {
					snapshot.fileHashes = fileHashes;
				...
				if (contextHashes.size !== 0) {
					snapshot.contextHashes = contextHashes;
		...
		const captureNonManaged = items => {
			const capturedItems = new Set();
			for (const path of items) {
				if (!checkManaged(path)) capturedItems.add(path);
			}
			return capturedItems;
		};
		if (files) {
			if (options && options.hash) {
				const capturedFiles = captureNonManaged(files);
		...
		if (directories) {
			if (options && options.hash) {
				const capturedDirectories = captureNonManaged(directories);
		...
		if (missing) {
			const capturedMissing = captureNonManaged(missing);
