				...
				(file, callback) => {
					...
					this.fs.lstat(child, (err, stat) => {
						...
						if (stat.isSymbolicLink()) {
							this.fs.realpath(child, (err, realpath) => {
								if (err) return callback(err);
								if (
									// Use getFileTimestamp to get timestamp for symbolic link
									// which target path is it's parent,
									// or _readContextTimestamp for path will never end.
									child.startsWith(realpath) ||
									// Use getFileTimestamp to get timestamp for symbolic link
									// which is already in contextTimestampQueue
									// or _readContextTimestamp for path will never end.
									this.contextTimestampQueue.isProcessing(realpath) ||
									this.contextTimestampQueue.isQueued(realpath)
								) {
									return this.getFileTimestamp(realpath, callback);
								}
								this.contextTimestampQueue.increaseParallelism();
								this.getContextTimestamp(realpath, (err, tsEntry) => {
					...
					this.fs.lstat(child, (err, stat) => {
						...
						if (stat.isSymbolicLink()) {
							this.fs.realpath(child, (err, realpath) => {
								if (err) return callback(err);
								if (
									// Skip symbolic link which target path is it's parent,
									// or _readContextHash for path will never end.
									child.startsWith(realpath) ||
									// Skip symbolic link which target path is already in contextHashQueue,
									// or _readContextHash for path will never end.
									this.contextHashQueue.isProcessing(realpath) ||
									this.contextHashQueue.isQueued(realpath)
								) {
									return callback(null, "");
								}
								this.contextHashQueue.increaseParallelism();
								this.getContextHash(realpath, (err, hash) => {
