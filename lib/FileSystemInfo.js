...
class SnapshotOptimization {
	 ...
	 * @param {function(Snapshot): Map<string, T>} get get value
	 * @param {function(Snapshot, Map<string, T>): void} set set value
	 */
		...
		capturedFiles: for (const path of capturedFiles) {
			...
			if (optimizationEntry.shared > 0) {
				...
				if (nonSharedFiles.size === 0) {
					...
					for (const [path, ts] of snapshotEntries) {
				...
				for (const path of capturedFiles) {
					const ts = snapshotEntries.get(path);
