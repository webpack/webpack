...
class Snapshot {
	...
	/**
	 * @param {ManagedFiles | undefined} value managed files
	 */
...
class SnapshotOptimization {
	 ...
	 */
	constructor(get, set, useStartTime = true, isSet = /** @type {U} */ (false)) {
		this._get = get;
		this._set = set;
	...
	getContextTimestamp(path, callback) {
		...
		if (cache !== undefined) {
			...
			return this._resolveContextTimestamp(
				/** @type {ResolvedContextFileSystemInfoEntry} */ (cache),
				callback
			);
		}
		...
		if (cachedHash !== undefined) {
			if (cachedTimestamp !== undefined) {
				...
				this.contextTimestampQueue.add(path, (err, entry) => {
					if (err) return callback(err);
					finalize(
						/** @type {ContextFileSystemInfoEntry} */ (entry),
						cachedHash
					);
				});
			}
