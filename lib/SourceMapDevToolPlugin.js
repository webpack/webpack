...
/**
 * Shape of the `clearCache` method added in webpack-sources 3.5. Inlined as a
 * structural type so this plugin still type-checks against the older `Source`
 * declarations shipped with the currently published webpack-sources.
 * @typedef {object} SourceClearCacheOptions
 * @property {boolean=} maps
 * @property {boolean=} source
 * @property {boolean=} parsedMap
 * @property {boolean=} hash
 * @property {boolean=} size
 * @property {boolean=} recursive
 */
/**
 * @typedef {Source & { clearCache?: (options?: SourceClearCacheOptions) => void }} SourceMaybeWithClearCache
 */
					...
					asyncLib.each(
						...
						(file, callback) => {
							...
							cacheItem.get((err, cacheEntry) => {
								...
								//
								// We deliberately do NOT pass a shared `visited`
								// WeakSet across calls here. A shared set would skip
								// any subtree already visited by an earlier asset's
								// walk, but the next asset's `sourceAndMap()` may
								// repopulate a shared module's caches before its own
								// `clearCache` runs — and skipping it would leave
								// those freshly populated caches behind. Letting
								// `webpack-sources` create a fresh per-call WeakSet
								// keeps the deduplication *within* one asset's
								// recursive walk while still releasing every cache
								// populated by the asset's `sourceAndMap`.
								// Feature-detected so this is a no-op on
								// webpack-sources versions before 3.5.
								const sourceWithClearCache =
									/** @type {SourceMaybeWithClearCache} */
									(asset.source);
								if (
									task &&
									typeof sourceWithClearCache.clearCache === "function"
								) {
									sourceWithClearCache.clearCache({
										maps: true,
										source: false,
										parsedMap: true
									});
