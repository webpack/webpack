...
const getTaskForFile = (
	...
	const { root } = compilation.compiler;
...
class SourceMapDevToolPlugin {
	...
	apply(compiler) {
		...
		const { sourceMapFilename } = this;
		const { sourceMappingURLComment } = this;
		const { moduleFilenameTemplate } = this;
		const { namespace } = this;
		const { fallbackModuleFilenameTemplate } = this;
