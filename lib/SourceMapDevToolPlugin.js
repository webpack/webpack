		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.processAssets.tapAsync(
				...
				(assets, callback) => {
					...
					// instances, so a per-call WeakSet would re-walk the same
					// subtree once per chunk. One set across phase 1 keeps the
					// work proportional to the count of unique sources rather
					// than chunks × modules. Only used when the installed
					// webpack-sources exposes `clearCache` (3.5+); on older
					// versions the check below no-ops and this set is unused.
					...
					asyncLib.each(
						...
						(file, callback) => {
							...
							cacheItem.get((err, cacheEntry) => {
								...
								// `clearCacheVisited` is shared across every call so
								// shared module sources are walked at most once across
								// all chunks. Feature-detected so this is a no-op on
