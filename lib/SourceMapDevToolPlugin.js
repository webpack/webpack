				...
				(assets, callback) => {
					...
					asyncLib.each(
						...
						(file, callback) => {
							...
							// The identifier carries the per-instance salt, or two instances
							// targeting one `file` would write different content to the same
							// key and invalidate every pack on each build.
							...
							cacheItem.get((err, cacheEntry) => {
								...
								// `task.mapSource`, not `asset.source`: where the extraction
								// fell back to the pinned original, it is the original's
								// caches that were populated.
