	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap("SourceMapDevToolPlugin", compilation => {
			...
			compilation.hooks.afterOptimizeChunkAssets.tap(
				//TODO need to fix type of Tap, currently { name: string; type: TapType; fn: Function; stage: number; context: boolean; }
				//@ts-ignore
				...
				(context, chunks) => {
					...
					tasks.forEach((task, index) => {
						...
						/**
						 * In css comment with `//` commenting next block as well, so workaround is to replace comment `//` with `/**\/`
						 */
						if (
