...
/**
 * Consume a list of component values for a declaration value, per
 * https://www.w3.org/TR/css-syntax-3/#consume-list-of-components-values
 * with `<semicolon-token>` as the stop token (and the implicit `<{-token>`
 * close for nested rules). The terminator (`;` or `}`) is **not** consumed —
 * the returned `end` is the position of the terminator (or EOF) so the outer
 * parser can fire its own callbacks for that token.
 * @param {string} input source
 * @param {number} pos start position (post-`:`, post-whitespace)
 * @param {LocConverter} locConverter shared loc converter
 ...
 */
const consumeComponentValuesForDeclaration = (
	input,
	pos,
	locConverter,
	comment
	...
	let terminate = false;
	const top = () => stack[stack.length - 1];
	const push = (/** @type {Node} */ node) => {
		top().values.push(node);
	};

	const base = _makeValueCallbacks(input, locConverter, stack, () => {
		// Declaration parsing keeps a sentinel root frame; `onRootClose` only
		// fires if someone pops past it (e.g. a stray `)` after the root is
		// gone), which can't happen here.
	});
	...
	const end = walkCssTokens(input, pos, {
		...
		rightCurlyBracket: (_input, s, e) => {
			if (top().kind === "{") {
				// Nested `{}` simple block — close normally.
				const frame = /** @type {Frame} */ (stack.pop());
...
/**
 * Consume an at-rule prelude — the list of component values that sit between
 * the at-keyword and the at-rule's terminator. Stops at the first top-level
 * `<semicolon-token>`, `<{-token>`, or `<}-token>`; nested simple blocks and
 * functions are consumed normally. The terminator itself is **not** consumed
 * (its position is returned in `end` so the outer parser can fire its own
 * callback for it).
 * @param {string} input source
 * @param {number} pos start position (just past the at-keyword)
 * @param {LocConverter} locConverter shared loc converter
 * @param {((input: string, start: number, end: number) => number)=} comment optional comment-token callback
 * @returns {{ values: Node[], end: number, terminator: AtRuleTerminator }} prelude component values, terminator position, terminator kind
 */
const consumeComponentValuesForAtRulePrelude = (
	input,
	pos,
	locConverter,
	comment
	...
	let terminate = false;
	/** @type {AtRuleTerminator} */
	let terminator = null;
	const top = () => stack[stack.length - 1];

	const base = _makeValueCallbacks(input, locConverter, stack, () => {
		// Root frame is a sentinel that never closes via the value callbacks.
	});
	...
	const end = walkCssTokens(input, pos, {
		...
		leftCurlyBracket: (_input, s, e) => {
			if (stack.length === 1) {
				// Top-level `{` — the at-rule's block begins here. Leave it
				// for the outer parser so it can fire its own
				// `leftCurlyBracket:` callback (which manages scope, pure-
				// mode tracking, etc.).
				terminate = true;
				terminator = "{";
				return s;
			}
			// Nested `{` simple block — push a frame.
			stack.push({
				values: [],
				kind: "{",
				name: "",
		...
		rightCurlyBracket: (_input, s, e) => {
			...
			// Top-level `}` with no matching open `{`: spec calls this a
			// parse error and treats `}` as a delim, but for our streaming
			// integration we terminate so the outer parser can close the
			// surrounding block.
			terminate = true;
			terminator = "}";
			return s;
...
const parseAtRule = (input, pos, locConverter, comment) => {
	...
	const { values, end, terminator } = consumeComponentValuesForAtRulePrelude(
		input,
		nameEnd,
		locConverter,
		comment
