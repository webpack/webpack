...
const parseAComponentValue = (input, pos, locConverter, options = {}) => {
	...
	const end = walkCssTokens(input, pos, {
		...
		// Bail as soon as the root frame has captured one complete
		// non-whitespace value. Nested frames keep `stack.length > 1`,
...
const _consumeIntoSeedFrame = (
	...
	const base = _makeValueCallbacks(input, locConverter, stack, (frame, end) => {
		// The seed frame closed — record the node and stop the walker.
		// (Reached via `)`/`]` rightParenthesis/rightSquareBracket in
	...
	const end = walkCssTokens(input, startPos, {
		...base,
		comment,
		needTerminate: () => terminate,
		rightCurlyBracket: (_input, _s, e) => {
