 ...
 * @returns {Record<string, (t: CssToken) => void>} handler map keyed by token type
 */
const _makeValueCallbacks = (input, locConverter, stack, onRootClose) => {
	...
	return {
		...
		colon: (t) => push(new Token(T_COLON, ":", t.start, t.end, locConverter)),
		comma: (t) => push(new Token(T_COMMA, ",", t.start, t.end, locConverter)),
		function: (t) => {
		...
		rightSquareBracket: (t) => {
			if (stack.length > 0 && top().kind === "[") closeTop(t.end);
		}
	};
};
...
const parseAListOfComponentValues = (
	...
	for (const t of walkCssTokens(input, pos)) {
		...
		if (t.type === "semicolon") {
			...
			top().values.push(new Token(T_SEMICOLON, ";", t.start, t.end, locConverter));
			continue;
		}
		...
		const handler = /** @type {Record<string, ((t: CssToken) => void) | undefined>} */ (
			/** @type {unknown} */ (base)
		)[t.type];
...
const parseAComponentValue = (input, pos, locConverter, options = {}) => {
	...
	const base = _makeValueCallbacks(input, locConverter, stack, () => {
		// Root sentinel — `_makeValueCallbacks` pops past the bottom on a
		// stray `)`/`]`, which is correct for `parseAFunction`'s seed frame
		// but would orphan our sentinel root and crash every subsequent
		// callback (they all push into `top()`). We override
		// `rightParenthesis` / `rightSquareBracket` below to preserve the
		// root, so this hook is never reached in practice.
	});
...
const _consumeIntoSeedFrame = (
	...
	const base = _makeValueCallbacks(input, locConverter, stack, (frame, end) => {
		// The seed frame closed — record the node. (Reached via `)`/`]`
		// rightParenthesis/rightSquareBracket handlers in
		// `_makeValueCallbacks` when they pop the seed frame.)
	...
	for (const t of walkCssTokens(input, startPos)) {
		...
		if (t.type === "rightCurlyBracket") {
			...
			// what `_makeValueCallbacks`'s `rightParenthesis` does for
			// stray `)`.
