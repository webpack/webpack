...
const _customPropertyToken = (path, child, writer) => {
	...
	if (
		...
		return closes ? _closeAtEof(source) : source;
	}
	const start = path.start(child);
	...
	return `${_S._input.slice(start, opened)}${inner}${
...
const _closeAtEof = (text) => {
	...
	let out = text;
	// Comments are consumed between tokens, so one left open is what follows the last.
	let at = lastEnd;
	for (;;) {
		while (at < out.length && isWhitespace(out.charCodeAt(at))) at++;
