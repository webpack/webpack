...
const _makeRaw = (start, end) => {
	let from = start;
	let to = end;
	while (from < to && _isWhiteSpace(_S._input.charCodeAt(to - 1))) to--;
	while (from < to && _isWhiteSpace(_S._input.charCodeAt(from))) from++;
...
const _customPropertyToken = (path, child, writer) => {
	...
	if (path.end(child) === _S._input.length) {
		...
		if (
			type === T_URL &&
			source !== _S._input.slice(path.start(child), path.end(child))
		) {
			return `${source})`;
		}
	...
	const closed = _S._input[end - 1] === closer;
	...
	return `${_S._input.slice(start, opened)}${inner}${closed ? closer : ""}`;
};
...
const A = {
	...
	source(n = _S._currentNode) {
		...
		if (end !== _S._input.length || !_endsInLoneEscape(text)) return text;
	...
	name(n = _S._currentNode) {
		...
		return _types[i] === T_AT_RULE
			? _S._input.slice(_starts[i] + 1, _aux0[i])
			: _S._input.slice(_starts[i], _aux0[i]);
	},
	/**
	 * @param {Node=} n node
...
module.exports._isWhiteSpace = _isWhiteSpace;
