	...
	if (hoisted) {
		...
		if (rest !== null && copies !== undefined) {
			for (const copy of copies) {
				_pendingNested.push({ prelude: copy, rest, owner });
			}
		}
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			// A rewritten rule is several rules with preludes of their own, which is
			// no one block to join onto — but every copy carries this rule's block,
			// so the hoist takes them out as the rules they are.
			const copies =
				out === text || _prefixCopyPreludes.length === 0
					? undefined
					: _prefixCopyPreludes;
