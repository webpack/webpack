...
const _inIntegerProperty = () =>
	...
	INTEGER_PROPERTIES.has(toLowerCaseIfNeeded(A.name(_S._valueDeclaration)));
...
const _inCalcRejectingProperty = () =>
	...
	CALC_REJECTING_PROPERTIES.has(
		toLowerCaseIfNeeded(A.name(_S._valueDeclaration))
	);
...
const _foldLosesClamp = (term) => {
	...
	const property = toLowerCaseIfNeeded(A.name(_S._valueDeclaration));
...
const _inTapHighlightColor = () =>
	_S._valueDeclaration !== null &&
	A.name(_S._valueDeclaration).toLowerCase() === "-webkit-tap-highlight-color";
...
const _inFontFamily = () =>
	_S._valueDeclaration !== null &&
	FAMILY_LIST_PROPERTIES.has(asciiLowerCaseName(A.name(_S._valueDeclaration)));
...
const _inColorOnlyProperty = () =>
	_S._valueDeclaration !== null &&
	COLOR_ONLY_PROPERTIES.has(toLowerCaseIfNeeded(A.name(_S._valueDeclaration)));
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_DECLARATION: {
			...
			const custom =
				name.startsWith("--") ||
				(_S._inPropertyRule && equalsLowerCase(name, "initial-value")) ||
				...
				(_S._inFunctionRule && equalsLowerCase(name, "result")) ||
