...
// How many rules the run wrote, and the prelude length of the first. Read out
// of band, so a caller can say what its text is: a run that wrote one rule
// leaves a text that is one rule, which the rules around it may join onto.
let _nestedRuleCount = 0;
let _nestedFirstPrelude = 0;
...
const _flushRun = (run) => {
	...
	if (_nestedRuleCount === 0) _nestedFirstPrelude = prelude.length;
	_nestedRuleCount++;
	return prelude + run.rest;
};

/**
 * Record what a hoisted run wrote, where it wrote one rule: a rule the writer
 * can read a prelude off is one the rules either side of it may be joined onto,
 * and a text that is several rules has no one prelude to read.
 * @param {string} text what the run wrote
 * @returns {void}
 */
const _recordHoistedRule = (text) => {
	if (_nestedRuleCount !== 1) return;
	_setRuleEntry(/** @type {Node} */ (_S._currentNode), {
		text,
		prelude: _nestedFirstPrelude,
		atRule: false,
		plain: true,
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_STRING: {
			...
			if (!minify || _S._inCustomProperty) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (
				...
				// The rules written in it are not empty for its being so.
				{
					...
					if (minify && written.length !== 0) _recordHoistedRule(written);
					return written;
				}
			}
			...
			if (nested.length !== 0) {
				if (_nestedTookParent) {
					if (minify) _recordHoistedRule(nested);
					return nested;
				}
				return text + nested;
			}
		...
		case T_RAW:
			...
			if (minify) _blockSpans.push(_NO_BLOCK_ENTRY);
			{
				...
				const open =
					...
					_S._input.length - raw.length <= _S._openCommentStart &&
					_S._input.endsWith(raw);
				return `${raw}${open ? "*/" : ""};`;
			}
		case T_HASH: {
			const raw = path.source();
