...
const _writeNested = (path, node, prelude, rest, hoistable) => {
	...
	for (let i = 0; i < taken.length; i++) {
		...
		if (held !== null) out += _settledPrelude(held) + held.rest;
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (nested.length !== 0) {
				// The run wrote this rule's selectors into itself, and only where
				// nothing but them and its block stood here.
				return _nestedTookParent && text === prelude + rest
					? nested
					: text + nested;
			}
