	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// When a CSS module is referenced via new URL(), promote it to
				// an entry chunk so its @import/url() graph is bundled into a
				// standalone CSS file whose URL the JS side can consume.
				const URLEntryPlugin = require("../url/URLEntryPlugin");

				const urlEntryHooks = URLEntryPlugin.getCompilationHooks(compilation);

				urlEntryHooks.entryOptions.tap(PLUGIN_NAME, (module, dep) => {
					if (
						...
						return /** @type {import("../Entrypoint").EntryOptions} */ ({
							...dep.entryOptions,
							chunkLoading: false,
							wasmLoading: false,
							runtime: false
				...
				urlEntryHooks.getChunkFilename.tap(PLUGIN_NAME, (_chunk, dep) => {
					...
					if (
						...
						return RuntimeGlobals.getChunkCssFilename;
					}
				});
