...
class CssModulesPlugin {
	 ...
	 * @param {CachedSource} options.moduleSourceContent source
	...
	renderModule({
		...
		if (
			...
			/** @type {Source} */
			let moduleSource = new ReplaceSource(moduleSourceContent.original());
			let match;
	...
	renderChunk({
		...
		/** @type {Map<CssModule, CachedSource>} */
		const moduleSources = new Map();
		/** @type {Map<Module, CssIcssImportData>} */
		const moduleIcssImports = new Map();
		for (const module of modules) {
			const [moduleSource, moduleExport, moduleIcssImport] = this.getIcssData({
				chunk,
				module,
				codeGenerationResults,
				runtimeTemplate
			...
			if (moduleSource) {
				moduleSources.set(module, moduleSource);
			}
			if (moduleExport) {
		...
		for (const module of modules) {
			try {
				const moduleSource = this.renderModule({
					...
					codeGenerationResults,
					module,
					moduleSourceContent: moduleSources.get(module),
					runtimeTemplate,
					hooks,
					moduleIcssImports,
					moduleExports
	 ...
	 * @param {CssModule} options.module css module
	 * @param {RuntimeTemplate} options.runtimeTemplate runtime template
	 * @returns {[CachedSource, CssExportsData | undefined, CssIcssImportData]} css module source
	 */
	getIcssData({ chunk, module, runtimeTemplate, codeGenerationResults }) {
		...
		return [moduleSourceContent, cssExportsData, icssImports];
	}
