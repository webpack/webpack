...
/** @typedef {import("../DependencyTemplate").CssIcssImportData} CssIcssImportData */
/** @typedef {import("../Module")} Module */
/** @typedef {import("../Template").RuntimeTemplate} RuntimeTemplate */
/** @typedef {import("../TemplatedPathPlugin").TemplatePath} TemplatePath */
/** @typedef {import("../util/Hash")} Hash */
/** @typedef {import("../util/memoize")} Memoize */
/** @typedef {import("./CssGenerator").SourceWithReplace} SourceWithReplace */
...
class CssModulesPlugin {
	 ...
	 * @param {Map<Module, CssIcssImportData>} options.moduleIcssImports icss imports
	 * @param {Map<Module, CssExportsData>} options.moduleExports module exports
	...
	renderModule({
		...
		if (
			...
			let originalReplaceSource;
			if (moduleSourceContent instanceof CachedSource)
				originalReplaceSource = /** @type {SourceWithReplace} */ (
					moduleSourceContent.original()
				).originalReplaceSource;
			const publicPathAutoRegex = new RegExp(
				CssUrlDependency.PUBLIC_PATH_AUTO,
				"g"
			);
			const icssImports = moduleIcssImports.get(module);
			if (icssImports && originalReplaceSource) {
				for (const importName of Object.keys(icssImports)) {
					const resolvedModule = icssImports[importName].module;
					let moduleId = chunkGraph.getModuleId(resolvedModule);
					if (typeof moduleId === "string") {
						moduleId = moduleId.replace(/\\/g, "/");
					}
					// get resolvedModule's exports
					const resolvedExports = moduleExports.get(resolvedModule);
					const importValue = icssImports[importName].importValue;
					if (resolvedExports) {
						for (const range of icssImports[importName].replaceRanges) {
							originalReplaceSource.replace(
								range[0],
								range[1],
								resolvedExports.exports.get(importValue)
							);
						}
					}
				}
			...
			// icss re-export
			const exportsData = moduleExports.get(module);
			if (exportsData && icssImports) {
				for (const [exportName, exportValue] of exportsData.exports.entries()) {
					for (const token of Object.keys(icssImports)) {
						if (token === exportValue) {
							const reExportValue = icssImports[token].importValue;
							const resolvedModule = icssImports[token].module;
							// get resolvedModule's exports
							const resolvedExports = moduleExports.get(resolvedModule);
							if (resolvedExports) {
								exportsData.exports.set(
									exportName,
									resolvedExports.exports.get(reExportValue)
								);
							}
						}
					}
			...
			/** @type {Source} */
			let moduleSource =
				moduleSourceContent instanceof CachedSource
					? new ReplaceSource(moduleSourceContent.original())
					: new ReplaceSource(moduleSourceContent);
			let match;
			...
			const esModule = exportsData && exportsData.esModule;
			const exports = exportsData && exportsData.exports;
	...
	renderChunk({
		...
		/** @type {string[]} */
		const metaData = [];
		/** @type {Map<Module, CssExportsData>} */
		const moduleExports = new Map();
		/** @type {Map<Module, CssIcssImportData>} */
		const moduleIcssImports = new Map();
		for (const module of modules) {
			const [moduleExport, moduleIcssImport] = this.getIcssData({
				chunk,
				module,
				codeGenerationResults,
				runtimeTemplate
			...
			if (moduleExport) {
				moduleExports.set(module, moduleExport);
			}
			if (moduleIcssImport) {
				moduleIcssImports.set(module, moduleIcssImport);
			}
		}
	 ...
	 * @param {CssModule} options.module css module
	 * @param {RuntimeTemplate} options.runtimeTemplate runtime template
	 * @returns {[CssExportsData | undefined, CssIcssImportData]} css module source
	 */
	getIcssData({ chunk, module, runtimeTemplate, codeGenerationResults }) {
		const codeGenResult = codeGenerationResults.get(module, chunk.runtime);
		/** @type {CssExportsData | undefined} */
		const cssExportsData =
			codeGenResult.data && codeGenResult.data.get("css-exports");
		/** @type {CssIcssImportData | undefined} */
		const icssImports =
			codeGenResult.data && codeGenResult.data.get("css-icss-imports");
		return [cssExportsData, icssImports];
	}
