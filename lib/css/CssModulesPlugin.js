...
class CssModulesPlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
						...
						.tap(PLUGIN_NAME, (createData, resolveData) => {
							if (resolveData.dependencies.length > 0) {
								...
								if (
									...
									dependency.assertions.type === "css"
								) {
									return new CssModule({
										...createData,
										assertions: dependency.assertions
									});
								}
							}
				...
				javascriptHooks.renderModuleContent.tap(
					PLUGIN_NAME,
					(source, module, renderContext) => {
						if (
							...
							return this.replacePublicPathAuto(source, undoPath);
						}
						return source;
					}
				);
	 ...
	 * @param {string} undoPath undoPath
	 * @returns {ReplaceSource} source with public path
	 */
	replacePublicPathAuto(source, undoPath) {
		const moduleSourceCode = /** @type {string} */ (source.source());
		const publicPathAutoRegex = new RegExp(
			CssUrlDependency.PUBLIC_PATH_AUTO,
			"g"
		);
		const moduleSource = new ReplaceSource(source);
		let match;
	...
	renderModule({
		...
		if (
			...
			/** @type {Source} */
			let moduleSource = this.replacePublicPathAuto(
				moduleSourceContent,
				undoPath
			);
