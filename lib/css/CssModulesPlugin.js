	...
	getModulesInOrder(chunk, modules, compilation) {
		...
		for (;;) {
			...
			if (hasFailed) {
				...
				const conflictingGroups = modulesByChunkGroup
					.filter(
						({ set }) => set.has(fallbackModule) && set.has(selectedModule)
					)
					.map(({ chunkGroup }) => chunkGroup.name || chunkGroup.id)
					.join(", ");
				...
				compilation.warnings.push(
					new WebpackError(
						`chunk ${chunk.name || chunk.id}\nConflicting order between ${fallbackModule.readableIdentifier(
							...
							compilation.requestShortener
						)}\ncaused by chunk groups ${conflictingGroups}`
					)
				);
