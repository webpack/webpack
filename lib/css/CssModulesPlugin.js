...
class CssModulesPlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			{
				name: PLUGIN_NAME,
				stage: 100 // ensure we set compilation.dependencyTemplates after JavascriptModulesPlugin
			},
...
class ProxyHarmonyImportSideEffectDependencyTemplate extends HarmonyImportSideEffectDependency.Template {
	/**
	...
	apply(dependency, source, templateContext) {
		const module = /** @type {Module} */ (
			templateContext.moduleGraph.getModule(dependency)
		);
		...
		if (
			...
			// only render css type, no need to execute js
			return;
		}
