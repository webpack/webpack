		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				JavascriptModulesPlugin.getCompilationHooks(
					...
					/**
					 * @param {CssModule} cssModule the CSS module to generate HMR code for
					...
					// Handle ConcatenatedModule containing CSS style/link modules
					if (module instanceof ConcatenatedModule) {
						for (const innerModule of module.modules) {
							if (
								innerModule instanceof CssModule &&
								innerModule.hot &&
								innerModule.exportType &&
								["link", "style"].includes(innerModule.exportType)
							) {
								return appendCssHmrCode(/** @type {CssModule} */ (innerModule));
							}
						}
					}
