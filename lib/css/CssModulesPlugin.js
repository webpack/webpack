...
const CssInjectStyleRuntimeModule = require("./CssInjectStyleRuntimeModule");
			...
			(compilation, { normalModuleFactory }) => {
					...
					if (module instanceof CssModule && module.hot) {
						...
						const hmrCode = Template.asString([
							...
							Template.indent([
								...
								"if (data.__webpack_style_api__) {",
								Template.indent("data.__webpack_style_api__.dispose();"),
								"}"
							]),
							"});"
						]);
				...
				compilation.hooks.runtimeRequirementInTree
					...
					.tap(PLUGIN_NAME, (chunk) => {
						compilation.addRuntimeModule(
							chunk,
							new CssInjectStyleRuntimeModule()
						);
					});
			}
		);
