...
class CssModulesPlugin {
					...
					normalModuleFactory.hooks.createModuleClass
						...
						.tap(PLUGIN_NAME, (createData, resolveData) => {
							...
							if (resolveData.dependencies.length > 0) {
								...
								if (dependency instanceof CssUrlEntryDependency) {
									exportType = "link";
								}
							}
	...
	static chunkHasCss(chunk, chunkGraph) {
		return (
			...
			chunkHasAsyncEntrypoints(chunk, chunkGraph)
		);
	}
}
 ...
 * @returns {boolean} true, when the chunk has css
 */
function chunkHasAsyncEntrypoints(chunk, chunkGraph) {
	return [...chunkGraph.getChunkModules(chunk)].some(
		(m) => /** @type {CssModule} */ (m).exportType === "url"
	);
}
