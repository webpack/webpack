...
const computePublicPathPlaceholderPlan = (content) => {
	...
	const autoLen = autoPlaceholder.length;
	for (
		let idx = content.indexOf(autoPlaceholder);
		idx !== -1;
		idx = content.indexOf(autoPlaceholder, idx + autoLen)
	) {
		autos.push(idx);
	}
	/** @type {{ start: number, end: number, length: number }[]} */
	const hashes = [];
	CssUrlDependency.walkFullHashPlaceholders(content, (start, end, length) => {
	...
	static renderModule(module, renderContext, hooks) {
		...
		if (
			...
			if (plan.autos.length > 0 || (hash && plan.hashes.length > 0)) {
				...
				const autoLen = CssUrlDependency.PUBLIC_PATH_AUTO.length;
