	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// Promote new URL() targets with exportType "url" for this compilation.
				const urlEntryHooks = URLEntryPlugin.getCompilationHooks(compilation);

				urlEntryHooks.promote.tap(PLUGIN_NAME, (module, dep) => {
					if (!(module instanceof CssModule) || module.exportType !== "url") {
						return;
					}
					return {
						entryOptions: {
						...
						chunkFilenameGlobal: RuntimeGlobals.getChunkCssFilename
					};
				});
