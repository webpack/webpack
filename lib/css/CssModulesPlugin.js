...
class CssModulesPlugin {
	...
	renderChunk({
		...
		for (const [module, moduleSource] of moduleSources.entries()) {
			...
			if (icssImports) {
				for (const name of Object.keys(icssImports)) {
					const resolvedModule = icssImports[name].module;
					let moduleId = chunkGraph.getModuleId(resolvedModule);
					if (typeof moduleId === "string") {
						moduleId = moduleId.replace(/\\/g, "/");
					}
					const exports = moduleExports.get(resolvedModule);
					const importValue = icssImports[name].value;
					if (exports) {
						replaceSource = replaceSource.replace(
							CssIcssImportDependency.generateReplacementStr(
								String(moduleId),
								importValue
							),
							exports.exports.get(importValue)
						);
					}
				}
			}
			...
			if (exportsData && icssImports) {
				for (const [key, value] of exportsData.exports.entries()) {
					for (const token of Object.keys(icssImports)) {
						if (token === value) {
							const reExportValue = icssImports[token].value;
							const resolvedModule = icssImports[token].module;
							const exports = moduleExports.get(resolvedModule);
							if (exports) {
								exportsData.exports.set(
									key,
									exports.exports.get(reExportValue)
								);
							}
						}
					}
