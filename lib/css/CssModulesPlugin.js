...
class CssModulesPlugin {
	...
	getOrderedChunkCssModules(chunk, chunkGraph, compilation) {
		...
		return /** @type {CssModule[]} */ ([
			...
			...orderModules(
				...
				const moduleNamespaces = /** @type {CssModuleBuildInfo} */ (
					module.buildInfo
				).namespaces;
				if (moduleNamespaces) {
					for (const { prefix, rule } of moduleNamespaces) {
						const existing = namespaces.get(prefix);
						// Per spec the last declaration of a prefix wins; redeclaring it
						// with a different URI is nonconforming, so warn but keep the last.
						if (existing !== undefined && existing !== rule) {
							const err = new WebpackError(
								`Duplicate @namespace at-rules detected: the module ${module.readableIdentifier(
									compilation.requestShortener
								)} (in chunk ${chunk.name || chunk.id}) redeclares ${
									prefix ? `prefix "${prefix}"` : "the default namespace"
								} as "${rule}", overriding the earlier "${existing}"`
							);
	...
	renderChunk(
		...
		for (const module of modules) {
			...
			if (moduleNamespaces) {
				// Per spec the last declaration of a prefix wins.
				for (const { prefix, rule } of moduleNamespaces) {
					namespaces.set(prefix, rule);
				}
			}
		...
		// Hoist `@charset` then `@namespace` rules to the chunk top, in spec order.
		let prefix = charset ? `@charset "${charset}";\n` : "";
		for (const rule of namespaces.values()) prefix += `${rule};\n`;
