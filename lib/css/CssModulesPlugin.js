...
const CssUrlEntryDependency = require("../dependencies/CssUrlEntryDependency");
...
/**
 * Returns a (cached) map from stringified chunk id to chunk.
 * @param {Compilation} compilation the compilation
 * @returns {Map<string, Chunk>} chunk-by-id map
 */
const getChunkById = (compilation) => {
	let map = chunkByIdCache.get(compilation);
	if (map === undefined) {
		map = new Map();
		for (const chunk of compilation.chunks) {
			if (chunk.id !== null) map.set(String(chunk.id), chunk);
		}
		chunkByIdCache.set(compilation, map);
	}
	return map;
};
...
class CssModulesPlugin {
	...
	static renderModule(module, renderContext, hooks) {
		...
		if (
			...
			// Resolve `url(./x.css)` references to an `exportType: "url"` chunk:
			// the placeholder carries the hex-encoded chunk id; substitute the real
			// filename now that the target chunk's contenthash exists.
			const chunkPrefix = CssUrlDependency.CHUNK_FILENAME;
			const chunkPrefixLen = chunkPrefix.length;
			const chunkSourceLen = moduleSourceCode.length;
			for (let idx = moduleSourceCode.indexOf(chunkPrefix); idx !== -1; ) {
				let hexEnd = idx + chunkPrefixLen;
				while (hexEnd < chunkSourceLen) {
					const cc = moduleSourceCode.charCodeAt(hexEnd);
					// hex digits: 0-9 (48-57) or a-f (97-102)
					if (!((cc >= 48 && cc <= 57) || (cc >= 97 && cc <= 102))) break;
					hexEnd++;
				}
				let nextSearch;
				if (
					hexEnd > idx + chunkPrefixLen &&
					hexEnd + 1 < chunkSourceLen &&
					moduleSourceCode.charCodeAt(hexEnd) === 95 &&
					moduleSourceCode.charCodeAt(hexEnd + 1) === 95
				) {
					const id = Buffer.from(
						moduleSourceCode.slice(idx + chunkPrefixLen, hexEnd),
						"hex"
					).toString();
					const compilation = renderContext.runtimeTemplate.compilation;
					const targetChunk = getChunkById(compilation).get(id);
					const filename = targetChunk
						? compilation.getPath(
								CssModulesPlugin.getChunkFilenameTemplate(
									targetChunk,
									compilation.outputOptions
								...
								{
									hash,
									runtime: targetChunk.runtime,
									chunk: targetChunk,
									contentHashType: "css"
								}
							)
						: "data:,";
					replaceSource.replace(idx, hexEnd + 1, filename);
					nextSearch = hexEnd + 2;
				} else {
					nextSearch = idx + chunkPrefixLen;
				}
				idx = moduleSourceCode.indexOf(chunkPrefix, nextSearch);
			}
