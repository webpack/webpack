 ...
 * @property {("import" | "name")=} order algorithm to order CSS modules within a chunk
 */
...
class CssModulesPlugin {
	...
	/**
	 * @param {CssModulesPluginOptions=} options plugin options
	 */
	constructor(options = {}) {
		/** @type {WeakMap<Source, ModuleFactoryCacheEntry>} */
		this._moduleFactoryCache = new WeakMap();
		this._order = options.order || "import";
	}
	...
	getOrderedChunkCssModules(chunk, chunkGraph, compilation) {
		...
		const byName = this._order === "name";
		const orderModules = byName
			? /** @type {(c: Chunk, modules: Iterable<Module> | undefined) => Module[]} */ (
					(_chunk, modules) => (modules ? [...modules] : [])
				)
			: /** @type {(c: Chunk, modules: Iterable<Module> | undefined) => Module[]} */ (
					(c, modules) => this.getModulesInOrder(c, modules, compilation)
				);
