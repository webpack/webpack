		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				JavascriptModulesPlugin.getCompilationHooks(
					...
					/**
					 * @param {Module} cssModule the CSS module to generate HMR code for
					 * @returns {Source} source with HMR code appended
					 */
					const appendCssHmrCode = (cssModule) => {
						const cssData = /** @type {BuildInfo} */ (cssModule.buildInfo)
							.cssData;
						if (!cssData) return source;
					...
					/**
					 * Determines if a module needs CSS HMR code
					 * @param {Module} module the module to check
					 * @returns {boolean} true, when CSS HMR code needs to be appended
					 */
					const needCssHmrCode = (module) => {
						if (module instanceof CssModule && module.hot) {
							const exportType = module.exportType || "link";
					...
					if (needCssHmrCode(module)) {
						return appendCssHmrCode(module);
					}
					if (
						module instanceof ConcatenatedModule &&
						needCssHmrCode(module.rootModule)
					) {
						return appendCssHmrCode(module);
					}
