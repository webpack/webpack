...
class CssModulesPlugin {
	 ...
	 * @returns {[Source, Map<string, string>, Map<string, [number, number][]>]} css module source
	 */
	renderModule({
		metaData,
		undoPath,
		...
		const imports = codeGenResult.data && codeGenResult.data.get("icss-imports");
	...
	renderChunk({
		...
		/** @type {Map<Module, Map<string, [number, number][]>>} */
		const moduleIcssImports = new Map();
		for (const module of modules) {
			try {
				const [moduleSource, moduleExport, moduleIcssImport] = this.renderModule({
					metaData,
					undoPath,
					chunk,
					chunkGraph,
		...
		for (const [module, moduleSource] of moduleSources.entries()) {
			...
			const icssImportDep = module.dependencies.filter(
				dep => dep instanceof CssIcssImportDependency
			);
			if (icssImportDep.length) {
				icssImportDep.forEach(dep => {
					const resolvedModule = chunkGraph.moduleGraph.getResolvedModule(dep);
					const exports = moduleExports.get(resolvedModule);
					if (exports && moduleIcssImports.get(module)) {
						for (const item of moduleIcssImports.get(module)) {
							Object.keys(item).forEach(name => {
								for (const range of item[name].replaceRanges) {
									replaceSource.replace(
										range[0],
										range[1],
										exports.get(item[name].value)
									);
								}
							});
						}
