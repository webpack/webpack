 ...
 * } from "./CssModule"
 */
/** @import Module, { RuntimeRequirements } from "../module/Module" */
...
/**
 * Whether the CSS runtimes emit their no-DOM branch, which collects styles for a
 * server render: a node platform always, the neutral one behind a runtime check.
 * Other document-less platforms (`target: "webworker"`) keep the DOM branch.
 * @param {Compilation} compilation the compilation
 * @returns {boolean} true when styles may be collected instead of applied
 */
const collectsServerStyles = (compilation) =>
	!compilation.outputOptions.environment.document &&
	(compilation.runtimeTemplate.isNeutralPlatform() ||
		compilation.compiler.platform.node === true);

/**
 * Smallest post-order index among a chunk's css modules, taken from the group's
 * view of the dependency graph — where its rules belong in the cascade.
 * `group.chunks` alone does not give that: `splitChunks` inserts each new chunk
 * before the one it split from, so split css siblings come out reversed.
 * @param {Chunk} chunk the chunk
 * @param {ChunkGroup} group the group whose module order decides the cascade
 * @param {ChunkGraph} chunkGraph chunk graph
 * @returns {number} the index, `Infinity` when the group's walk reaches none
 */
const firstCssModulePostOrderIndex = (chunk, group, chunkGraph) => {
	...
	for (const sourceType of [CSS_TYPE, CSS_IMPORT_TYPE]) {
		...
		for (const module of modules) {
			const index = group.getModulePostOrderIndex(module);
...
// The cascade order, with the key breaking ties: `Array#sort` is not stable on
// the legacy node versions this repo supports, so the order must be total.
const compareCssChunkOrder = concatComparators(
	compareSelect(
		/** @type {(entry: { index: number, key: string }) => number} */
		((entry) => entry.index),
		compareNumbers
	),
	compareSelect(
		/** @type {(entry: { index: number, key: string }) => string} */
		((entry) => entry.key),
		compareStrings
	)
);
...
class CssModulesPlugin {
	...
	apply(compiler) {
		...
		// TODO in the next major release: css defaults to `true`, drop this implicit-only fallback.
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const registerDependencies = () => {
					...
					const CssImportDependency = require("../dependencies/css/CssImportDependency");
					const CssUrlDependency = require("../dependencies/css/CssUrlDependency");
					const CssIcssImportDependency = require("../dependencies/css/CssIcssImportDependency");
					const CssIcssExportDependency = require("../dependencies/css/CssIcssExportDependency");
					const CssIcssSymbolDependency = require("../dependencies/css/CssIcssSymbolDependency");
				...
				const addNoDomStyleRegistry = (set) => {
					if (
						collectsServerStyles(compilation) &&
	 ...
	 * @param {Iterable<Chunk>} chunks candidate chunks, a group's own or a narrower load order
	 * @param {ChunkGroup} group the group whose module order decides the cascade
	 * @param {ChunkGraph} chunkGraph chunk graph
	 * @returns {Chunk[]} the css chunks, in cascade order
	 */
	static getCssChunksInCascadeOrder(chunks, group, chunkGraph) {
		/** @type {{ chunk: Chunk, index: number, key: string }[]} */
		const ordered = [];
		for (const chunk of chunks) {
			if (!CssModulesPlugin.chunkHasCss(chunk, chunkGraph)) continue;
			ordered.push({
				chunk,
				index: firstCssModulePostOrderIndex(chunk, group, chunkGraph),
				key: `${chunk.name || ""} ${chunk.id === null || chunk.id === undefined ? "" : chunk.id}`
			});
		}
		ordered.sort(compareCssChunkOrder);
		return ordered.map(({ chunk }) => chunk);
	}
}
