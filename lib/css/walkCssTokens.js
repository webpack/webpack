...
// Public surface kept deliberately small. The CSS-syntax-level helpers
// (`eat*`, `parseA*`, `isWhiteSpace`, …) are reachable through
// `SourceProcessor`'s static members — see the assignments above — so
// `CssParser.js` imports only `SourceProcessor` and never the raw
// helpers. The AST node classes below are exported because they're
// referenced as `@typedef {import("./walkCssTokens").X}` types by
// `CssParser.js`. `escapeIdentifier` / `unescapeIdentifier` are
// exported for the `cssIdentifier` unit test, which asserts they're
// the same references `CssParser` re-exports.
