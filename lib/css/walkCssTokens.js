...
class HashToken extends Token {
	 ...
	 * @param {LocConverter} locConverter shared loc converter
	 */
	constructor(value, id, range, locConverter) {
		super(T_HASH, value, range, locConverter);
		this.id = id;
	}
}
...
class UrlToken extends Token {
	 ...
	 * @param {[number, number]} range `[start, end)` byte range (including `url(` … `)`)
	 * @param {LocConverter} locConverter shared loc converter
	 */
	constructor(value, contentStart, contentEnd, range, locConverter) {
		super(T_URL, value, range, locConverter);
...
class FunctionNode extends Node {
	 ...
	 * @param {[number, number]} range `[start, end)` byte range (`name` start to after `)` / EOF)
	 * @param {LocConverter} locConverter shared loc converter
	 */
	constructor(name, nameRange, value, range, locConverter) {
		super(T_FUNCTION, range, locConverter);
		this.name = name;
...
class SimpleBlock extends Node {
	 ...
	 * @param {[number, number]} range `[start, end)` byte range (opening char to after closing char / EOF)
	 * @param {LocConverter} locConverter shared loc converter
	 */
	constructor(token, value, range, locConverter) {
		super(T_SIMPLE_BLOCK, range, locConverter);
		this.token = token;
		this.value = value;
	}
}
...
class Declaration extends Node {
	 ...
	 * @param {[number, number]} range `[start, end)` byte range (first non-whitespace byte to the terminating `;`/`}` / EOF)
	 * @param {LocConverter} locConverter shared loc converter
	 */
	constructor(name, nameRange, value, important, range, locConverter) {
		super(T_DECLARATION, range, locConverter);
		this.name = name;
		this.nameRange = nameRange;
		this.value = value;
 ...
 * `@name … { … }`, `.block` is the `{ … }` and `range[1]` points past `}`; for
 * a `;`-terminated at-rule, `.block` is `null` and `range[1]` is the `;`
 * position. To tell a `;`-terminated at-rule from a `}` / EOF one, a caller
 * checks the byte at `range[1]`.
 */
class AtRule extends Node {
	 ...
	 * @param {[number, number]} range `[start, end)` byte range (the `@` to past `}` for a block, or the `;` / `}` / EOF position otherwise)
	 * @param {LocConverter} locConverter shared loc converter
	 */
	constructor(name, nameRange, prelude, range, locConverter) {
		super(T_AT_RULE, range, locConverter);
		this.name = name;
		this.nameRange = nameRange;
...
class QualifiedRule extends Node {
	 ...
	 * @param {[number, number]} range `[start, end)` byte range (first byte of the prelude to past `}` / EOF)
	 * @param {LocConverter} locConverter shared loc converter
	 */
	constructor(prelude, block, range, locConverter) {
		super("QualifiedRule", range, locConverter);
		this.prelude = prelude;
		this.block = block;
	}
}
...
/**
 * Materialize a single non-block, non-function `CssToken` as its leaf AST
 * node — the spec's "consume a token" result (CSS Syntax §5.4.8 "anything
 * else"). Returns `undefined` for the structural close / CDO / CDC tokens
 * that reach this point only on malformed input; the callers drop them
 * (parse-css recovery). `(` / `[` / `{` / function-token never arrive here:
 * `consumeAComponentValue` routes them to `consumeASimpleBlock` /
 * `consumeAFunction` first.
 * @param {CssToken} t token from the lexer
 * @param {string} input source
 * @param {LocConverter} locConverter shared loc converter
 * @returns {Node | undefined} the leaf node, or `undefined` to drop the token
 */
const tokenToNode = (t, input, locConverter) => {
	...
	switch (t.type) {
		...
		default:
			// Stray `)` / `]` / `}` / CDO / CDC — only reachable on malformed
			// input; drop them (parse-css recovery). Comments never arrive
			// (the stream filters them out).
			return undefined;
	}
};
 ...
 * lexer and the stream in one class: the `tokenize` method is the token
 * generator (the CSS tokenizer), primed into `_gen` by the constructor, and
 * `peek` / `next` / `mark` / `reset` read from it. `parse*` entry points wrap
 * a source string in one of these and every `consume*` algorithm reads tokens
 * from it instead of re-scanning the source with byte math, so a string is
 * tokenized exactly once per parse.
 *
...
class TokenStream {
	 ...
	 * whitespace tokens included — and returning the post-EOF position. The
	 * constructor primes `_gen` with it; `peek` / `next` then buffer and
	 * comment-filter the stream, while iterating it directly
	 * (`new TokenStream(src).tokenize()`) yields the raw token list. Override
	 * in a subclass to drive a different language through the same
	 * `SourceProcessor` machinery.
	 * @param {number=} pos starting byte offset (default `0`)
	 * @returns {Generator<CssToken, number, void>} generator yielding every CSS token in `this.input`; final return value is the post-EOF position
	 */
	*tokenize(pos = 0) {
	 ...
	 * consuming it, or the EOF sentinel once the source is exhausted. This is
	 * the token the consume algorithms dispatch on (the spec's "process").
	 * @returns {CssToken | EofToken} the next token
	 */
	peek() {
		this._fill(this._index + 1);
		const t = this._buf[this._index];
		if (t) return t;
	...
	/**
	 * Discard a token (CSS Syntax §3 "discard a token") — advance past the next
	 * token without returning it.
	 * @returns {void}
	 */
	discard() {
		if (this.peek().type !== "EOF") this._index++;
	}

	/**
	 * Mark (CSS Syntax §3 "mark") — push the current position.
	 * @returns {void}
	 */
	mark() {
		this._marks.push(this._index);
	}

	/**
	 * Restore a mark (CSS Syntax §3 "restore a mark") — pop the last mark and
	 * rewind to it. Buffered tokens are reused, so no re-tokenization happens
	 * and already-fired comments are not re-fired.
	 * @returns {void}
	 */
	restoreMark() {
		this._index = /** @type {number} */ (this._marks.pop());
	}
...
const parseARule = (input, pos, comment) => {
	...
	while (ts.peek().type === "whitespace") ts.discard();
	const head = ts.peek();
...
const parseADeclaration = (input, pos, comment) => {
	const ts = normalizeIntoTokenStream(input, pos, comment);
	// §5.3.7: discard leading whitespace, then only consume at an ident-token.
	while (ts.peek().type === "whitespace") ts.discard();
	if (ts.peek().type !== "identifier") return undefined;
...
const parseAComponentValue = (input, pos, options = {}) => {
	const ts = normalizeIntoTokenStream(input, pos, options.comment);
	// §5.3.9: discard leading whitespace before consuming the value.
	while (ts.peek().type === "whitespace") ts.discard();
 ...
 * @returns {{ rules: (AtRule | QualifiedRule)[], end: number }} top-level rules and the final position (EOF)
 */
const consumeAStylesheetsContents = (ts) => {
	...
	for (;;) {
		...
		if (t.type === "whitespace" || t.type === "cdo" || t.type === "cdc") {
			...
			return { rules, end: t.start };
		} else if (t.type === "atKeyword") {
			const at = consumeAnAtRule(ts);
			if (at) rules.push(at);
		} else {
			// Anything else → qualified rule (non-nested, no stop token); it
			// consumes a stray top-level `}` itself, so this loop always advances.
...
/**
 * Consume an at-rule, CSS Syntax Level 3 [§5.4.2](https://drafts.csswg.org/css-syntax/#consume-at-rule) — consumes the at-keyword, then the prelude up to `;` / `{` / `}` / EOF (each component value via `consumeAComponentValue`); `{` becomes the rule's block, a `;` is discarded, and a top-level `}` (when not `nested`) is consumed as a parse error. Returns `undefined` only when the stream is not at an at-keyword. NB: the spec's `{` case consumes the block's contents eagerly; webpack keeps the raw `{ … }` simple block on `.block` and re-parses it lazily in the grammar.
 * @param {TokenStream} ts token stream
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	// The tokenizer only emits an at-keyword for `@` *immediately* followed by
	// an ident-sequence, so `@ foo` is a delim and never reaches here.
	if (ts.peek().type !== "atKeyword") return undefined;
	// Consume a token → the at-keyword.
	...
	for (;;) {
		...
		if (t.type === "semicolon" || t.type === "EOF") {
			// `end` points AT the terminator so callers can detect a
			// `;`-terminated rule; the `;` itself is discarded.
			if (t.type === "semicolon") ts.discard();
			return new AtRule(
				name,
				[start, nameEnd],
				prelude,
		...
		if (t.type === "rightCurlyBracket") {
			if (nested) {
				// Enclosing block close — leave it for the caller.
				return new AtRule(
					name,
					[start, nameEnd],
					prelude,
			...
			// Parse error: consume the stray top-level `}` (dropped) and continue.
			ts.discard();
		} else if (t.type === "leftCurlyBracket") {
			// `{` → the at-rule's block.
			const block = /** @type {SimpleBlock} */ (consumeASimpleBlock(ts));
			const at = new AtRule(
				name,
				[start, nameEnd],
				prelude,
			...
			const cv = consumeAComponentValue(ts);
			if (cv) prelude.push(cv.value);
		}
	}
};

/**
 * Consume a qualified rule, CSS Syntax Level 3 [§5.4.3](https://drafts.csswg.org/css-syntax/#consume-qualified-rule) — consumes the prelude (each component value via `consumeAComponentValue`) up to its `{` block; a top-level `}` either ends it (`nested`) or is consumed as a parse error. Two deliberate webpack deviations from the spec: the `{` case keeps the raw `{ … }` simple block on `.block` and re-parses it lazily in the grammar (instead of eagerly consuming the block's contents), and a block-less rule (EOF / `stopToken` / nested `}`) is still returned with `block: null` when it has a prelude — so orphaned component values (e.g. `url()`s after a stray `;`) are still visited — rather than dropped.
 * @param {TokenStream} ts token stream
 * @param {string=} stopToken token type that ends the prelude (parse error)
 * @param {boolean=} nested true inside a `{}` block — a top-level `}` ends the rule (left for the caller)
 * @returns {QualifiedRule | undefined} parsed qualified rule, or `undefined` when there is neither a block nor a prelude
 */
const consumeAQualifiedRule = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		...
		if (t.type === "rightCurlyBracket") {
			if (nested) {
				// Parse error — the enclosing `}` is left for the caller.
				end = t.start;
				break;
			}
			// Parse error: consume the stray top-level `}` (dropped) and continue.
			...
			const cv = consumeAComponentValue(ts);
			if (cv) {
				prelude.push(cv.value);
				end = cv.end;
			}
		}
	}
	// The spec returns nothing for a block-less qualified rule; webpack instead
	// keeps it (with `block: null`) when it has a prelude, so orphaned component
	// values — e.g. `url()`s after a stray `;` — are still visited for
	// dependency collection. An empty prelude yields nothing.
	return prelude.length > 0
		? new QualifiedRule(prelude, null, [start, end], locConverter)
		: undefined;
};
...
const consumeABlocksContents = (ts) => {
	...
	for (;;) {
		...
		if (t.type === "whitespace" || t.type === "semicolon") {
			...
			// Enclosing `}` / EOF — left in the stream for the caller.
			return { values, end: t.start };
		} else if (t.type === "atKeyword") {
			const atRule = consumeAnAtRule(ts, true);
			if (atRule) values.push(atRule);
		} else {
			// Mark, try a declaration; on failure restore and try a qualified
			// rule (CSS Nesting / nested style rule). Both are nested.
			ts.mark();
			...
			if (decl) {
				...
				ts.restoreMark();
				const rule = consumeAQualifiedRule(ts, undefined, true);
				if (rule) values.push(rule);
			}
		}
	}
};
...
const consumeADeclaration = (ts, nested = false) => {
	const { input, locConverter } = ts;
	// Leading whitespace is the caller's responsibility (§5.3.7 / §5.4.5).
	const head = ts.peek();
	...
	// Step 5: consume the value as a list of component values up to the
	// top-level `;` (the stop token, left in the stream); a top-level `}` ends
	// it when `nested`. A top-level `{}` block is consumed into the value.
	const { values, end } = consumeAListOfComponentValues(
		ts,
		"semicolon",
		nested
	);

	// CSS Nesting disambiguation: a non-custom declaration whose value contains
	// a top-level `{}` block is actually a qualified rule (e.g.
	// `div:not(:local(.x)) { … }`). Returning `undefined` lets the
	// `consumeABlocksContents` caller `reset` and retry it as a qualified rule,
	// matching §5.4.6's "fail when the value contains a top-level `{}` block,
	...
	if (values.length >= 2) {
		...
		if (
			...
			prev.type === T_DELIM &&
			input.charCodeAt(prev.range[0]) === CC_EXCLAMATION
		) {
			important = true;
...
/**
 * Consume a list of component values, CSS Syntax Level 3 [§5.4.7](https://drafts.csswg.org/css-syntax/#consume-list-of-components) — repeatedly consumes a component value (recursing into blocks/functions via `consumeAComponentValue`) until EOF, the optional `stopToken`, or — when `nested` — a top-level `}`; the stop token is left in the stream for the caller.
 * @param {TokenStream} ts token stream
 * @param {string=} stopToken token type that terminates the list (left unconsumed)
 * @param {boolean=} nested true inside a `{}` block — a top-level `}` ends the list (left unconsumed); false treats a stray `}` as a parse error and drops it
 * @returns {{ values: Node[], end: number }} consumed component values and the
 * final position (the stop token's / EOF position)
 */
const consumeAListOfComponentValues = (ts, stopToken, nested = false) => {
	/** @type {Node[]} */
	const values = [];
	let end = ts.peek().start;
	for (;;) {
		const t = ts.peek();
		if (t.type === "EOF" || t.type === stopToken) {
			end = t.start;
		...
		if (t.type === "rightCurlyBracket") {
			...
			// §5.4.7: a stray top-level `}` is a parse error — consume a token
			// and append it; `tokenToNode` drops stray close brackets, so
			// nothing is added (matching parse-css recovery).
			ts.consume();
			end = t.end;
			continue;
		}
...
/**
 * Consume a component value, CSS Syntax Level 3 [§5.4.8](https://drafts.csswg.org/css-syntax/#consume-component-value) — consumes exactly the next value (simple block, function, or single token) at the current position, with no leading-whitespace handling (the `parse*` caller does that).
 * @param {TokenStream} ts token stream
 * @returns {{ value: Node, end: number } | undefined} the consumed component value and the position after it, or `undefined` at EOF
 */
const consumeAComponentValue = (ts) => {
	const { input, locConverter } = ts;
	const t = ts.peek();
	if (t.type === "EOF") return undefined;
	// `(`/`[`/`{` → simple block (§5.4.9); function-token → function (§5.4.10).
	if (
		...
		const block = /** @type {SimpleBlock} */ (consumeASimpleBlock(ts));
		return { value: block, end: block.range[1] };
	}
	if (t.type === "function") {
		const fn = consumeAFunction(ts);
		return { value: fn, end: fn.range[1] };
	}
	// Anything else → consume a token and return it (a dropped close bracket /
	// CDO / CDC yields `undefined`).
	ts.consume();
	const node = tokenToNode(t, input, locConverter);
	return node ? { value: node, end: t.end } : undefined;
};

/**
 * Consume a simple block, CSS Syntax Level 3 [§5.4.9](https://drafts.csswg.org/css-syntax/#consume-simple-block) — the next token must be `(`, `[`, or `{` (else returns `undefined`); consumes component values via `consumeAComponentValue` until the mirror closing token (`)`, `]`, `}`) or EOF, returning the partial block on EOF (parse error). Pass a source string (+ `pos`) to wrap `[pos, EOF)` in a fresh `TokenStream`, or an existing `TokenStream` to consume from its current position (sharing its `LocConverter`).
 * @param {string | TokenStream} input source string or an existing token stream
 * @param {number=} pos start position (string input only; the `(`, `[`, or `{` byte)
 * @param {((input: string, start: number, end: number) => number)=} comment optional comment-token callback (string input only)
 * @returns {SimpleBlock | undefined} parsed simple block, or undefined when the next token isn't a `(` / `[` / `{`
 */
const consumeASimpleBlock = (input, pos, comment) => {
	...
	let ending;
	if (open.type === "leftParenthesis") {
		kind = "(";
		ending = "rightParenthesis";
	} else if (open.type === "leftSquareBracket") {
		kind = "[";
		ending = "rightSquareBracket";
	} else if (open.type === "leftCurlyBracket") {
		kind = "{";
		ending = "rightCurlyBracket";
	} else {
		return undefined;
	}
	// Discard a token → the opening `(` / `[` / `{`.
	ts.discard();
	/** @type {Node[]} */
	const values = [];
	let end = open.end;
	for (;;) {
		const t = ts.peek();
		// EOF before the mirror closer → parse error; return the partial block.
		if (t.type === "EOF") {
			end = t.start;
			break;
		}
	...
	return new SimpleBlock(kind, values, [open.start, end], locConverter);
};

/**
 * Consume a function, CSS Syntax Level 3 [§5.4.10](https://drafts.csswg.org/css-syntax/#consume-function) — the next token must be a `<function-token>` (the caller has peeked it); consumes component values via `consumeAComponentValue` until the matching `)` or EOF, returning the partial function on EOF (parse error).
 * @param {TokenStream} ts token stream
 * @returns {FunctionNode} the consumed function node
 */
const consumeAFunction = (ts) => {
	const { input, locConverter } = ts;
	// Consume a token → the function-token; `end` points just past the `(`, so
	// the name occupies [start, end - 1).
	const fnTok = ts.consume();
	const name = input.slice(fnTok.start, fnTok.end - 1);
	/** @type {[number, number]} */
	const nameRange = [fnTok.start, fnTok.end - 1];
	/** @type {Node[]} */
	const values = [];
	let end = fnTok.end;
	for (;;) {
			...
			switch (node.type) {
				...
				case "QualifiedRule": {
					...
					if (r.block && recurseBlocks) {
						// Body is [block.range[0]+1, matching `}`] — same bytes
						...
						const body = parseABlocksContents(
							new TS(input, r.block.range[0] + 1, locConverter, comment)
						);
