 ...
 * @property {function(string, number): boolean=} isConsumeColon
...
const consumePotentialPseudo = (input, pos, callbacks) => {
	...
	pos++;
	if (
		callbacks.isConsumeColon &&
		callbacks.isConsumeColon(input, pos) &&
		callbacks.colon
	)
		return callbacks.colon(input, start, pos);
	if (
		(callbacks.isSelector && !callbacks.isSelector(input, pos)) ||
		!_startsIdentifier(input, pos)
	)
