...
/**
 * Spec-style runtime assertion. The CSS Syntax algorithms phrase their entry
 * preconditions as "Assert: the next token is …" — every consume function
 * exposes such a precondition to its callers (always the algorithms themselves
 * in the spec). A failure here means the parser called the wrong consume.
 * @param {unknown} cond condition that must hold
 * @param {string} msg what was expected
 */
const assertSpec = (cond, msg) => {
	if (!cond) throw new Error(`Spec assertion failed: ${msg}`);
};
...
const parseAStylesheet = (input, comment) => {
	...
	// 3-5. Create a new stylesheet, consume its contents into rules, return it.
	return consumeAStylesheet(ts);
};
...
/**
 * Consume a stylesheet (CSS Syntax §5.3.4 step 3–5): create a new stylesheet, consume its contents into `rules`, return it.
 * @param {TokenStream} ts token stream
 * @returns {Stylesheet} the parsed stylesheet
 */
const consumeAStylesheet = (ts) => {
...
const consumeABlock = (ts) => {
	...
	const open = ts.next();
	assertSpec(open.type === "leftCurlyBracket", "next token must be <{-token>");
 ...
 * @param {TokenStream} ts token stream
 * @returns {SimpleBlock} parsed simple block
 */
const consumeASimpleBlock = (ts) => {
	// Assert: the next token of input is <{-token>, <[-token>, or <(-token>.
	const open = ts.next();
	/** @type {{ ending: string, token: SimpleBlockToken } | undefined} */
	const pair =
		open.type === "leftParenthesis"
			? { ending: "rightParenthesis", token: "(" }
			: open.type === "leftSquareBracket"
				? { ending: "rightSquareBracket", token: "[" }
				: open.type === "leftCurlyBracket"
					? { ending: "rightCurlyBracket", token: "{" }
					: undefined;
	assertSpec(pair, "next token must be <{-token>, <[-token>, or <(-token>");
	const { ending, token } =
		/** @type {{ ending: string, token: SimpleBlockToken }} */ (pair);
...
const consumeAFunction = (ts) => {
	...
	assertSpec(
		ts.next().type === "function",
		"next token must be a <function-token>"
	);
