...
// CSS-syntax-level helpers re-exposed as static members of
// `SourceProcessor` so `CssParser.js` reaches them through the class
// rather than via raw module exports. `eat*` are byte-skipping
// primitives (whitespace, whitespace + comments, single-newline,
// `eatUntil(char)` factory); `parseAComponentValue` / `parseASimpleBlock`
// are spec parse helpers used by feature handlers for sub-range parsing;
// `isWhiteSpace` is a char-code predicate. The pure string utilities
// `escapeIdentifier` / `unescapeIdentifier` are *not* here — they're
// plain module exports consumed directly.
