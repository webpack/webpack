 ...
 * @param {string} input input
 * @param {number} pos position
 * @returns {[number, number] | undefined} position after whitespace
 */
 ...
 * @param {string} input input
 * @param {number} pos position
 * @param {(start: number, end: number) => boolean} fn function
 * @returns {number} position the last token
 */
module.exports.eatNextTokenUntil = (input, pos, fn) => {
	let before;
	...
	do {
		...
		// Consume the next input code point.
		pos++;
		pos = consumeAToken(input, pos, {});
	} while (fn(before, pos) && pos < input.length);
