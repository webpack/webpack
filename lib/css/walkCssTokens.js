...
/** @type {CharHandler} */
const consumeString = (input, pos, callbacks) => {
...
const _isIdentCodePoint = cc =>
	_isIdentifierStartCode(cc) || _isDigit(cc) || cc === CC_HYPHEN_MINUS;
/**
 * @param {number} cc char code
...
const _ifThreeCodePointsWouldStartAnIdentSequence = (input, pos, f, s, t) => {
	...
	// ident-start code point
	else if (_isIdentifierStartCode(first)) {
		return true;
	}
	// U+005C REVERSE SOLIDUS (\)
...
const consumeAToken = (input, pos, callbacks) => {
	...
	switch (cc) {
		...
		case CC_QUOTATION_MARK:
			return consumeString(input, pos, callbacks);
		// U+0023 NUMBER SIGN (#)
		case CC_NUMBER_SIGN:
