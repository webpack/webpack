 ...
 * by `tokenToNode` for the matching type. `tokenize` allocates a fresh one per
 * yielded token (consumers collect the list).
 ...
 * the hot path allocates no per-token iterators. The token is written into the
 * caller-supplied `out` (reused across calls on the `next` path), so no per-token
 * object is allocated either.
 * @param {string} input input
 * @param {number} pos byte offset to read from
 * @param {MutableToken} out token to populate
 * @returns {MutableToken | undefined} the next token, or undefined at EOF
 */
function readTokenInto(input, pos, out) {
	if (pos >= input.length) return undefined;
	const cc = input.charCodeAt(pos);
 ...
 * at-rule. `blockRange` is `[start of {, end of }]` (webpack extension, not in
 * spec; the spec doesn't track brace positions). `range[1]` points past `}` for
 * a block, or at the `;` / `}` / EOF position otherwise (callers check the byte
 * at `range[1]` to tell them apart).
 * @typedef {Node & { name: string, nameRange: [number, number], prelude: ComponentValue[], declarations: Declaration[] | null, childRules: Rule[] | null, blockRange: [number, number] | null }} AtRule
 */
 ...
 * reference impl), or both `null` when EOF was hit before `{`. `blockRange` is
 * `[start of {, end of }]` (webpack extension).
 * @typedef {Node & { prelude: ComponentValue[], declarations: Declaration[] | null, childRules: Rule[] | null, blockRange: [number, number] | null }} QualifiedRule
 */
...
const tokenToNode = (t, input, locConverter) => {
	switch (t.type) {
		...
		case TT_DIMENSION: {
			...
			dim.typeFlag =
				numPart.includes(".") || numPart.includes("e") || numPart.includes("E")
					? "number"
					: "integer";
			dim.sign =
				numPart[0] === "+" || numPart[0] === "-"
...
class TokenStream {
	 ...
	 * wrapper over the shared `readTokenInto` core (which `next` also uses); a
	 * fresh token object is allocated per yield since consumers collect the list.
	 * @returns {Generator<MutableToken | EofToken, void, void>} generator yielding every CSS token in `this.input`, then the `<eof-token>`
	 */
	*tokenize() {
		const input = this.input;
		let pos = 0;
		for (;;) {
			const t = readTokenInto(input, pos, createToken());
			if (t === undefined) {
	...
	next() {
		if (this._next === undefined) {
			...
			let pos = this._pos;
			for (;;) {
				const t = readTokenInto(input, pos, tok);
				if (t === undefined) {
