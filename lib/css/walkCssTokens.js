...
class TokenStream {
	 ...
	 * Defaults to the CSS tokenizer (`TokenStream.tokenize`); override in a
	 * subclass to drive a different language through the same `SourceProcessor`
	 * machinery.
	 * @param {string} input source
	 * @param {number} pos start byte offset
	 ...
	 */
	lex(input, pos) {
		return TokenStream.tokenize(input, pos);
	}

	/**
	 * The CSS tokenizer (CSS Syntax Level 3 §4). Yields every `CssToken` in
	 * `input` from `pos` in source order — comment and whitespace tokens
	 * included — and returns the post-EOF position. The consumer drives by
	 * reading from the iterator; there is no position-modification handshake.
	 * `peek` / `next` buffer and comment-filter this stream; iterate it
	 * directly (`TokenStream.tokenize(src)`) for the raw token list.
