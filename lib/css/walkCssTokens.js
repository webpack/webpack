 ...
 * `token` is the opening character (`[`, `(`, `{`). `value` is the list of
 * component values inside — except for an at-rule / qualified-rule body block
 * (produced by `consumeABlock`, §5.4.4), whose `value` is the parsed
 * `Declaration` / `AtRule` / `QualifiedRule` contents of the `{ … }`.
 */
class SimpleBlock extends Node {
	/**
	 * @param {SimpleBlockToken} token opening character
	 * @param {Node[]} value component values inside the block (or parsed contents for a rule body block)
...
/**
 * Lazy, buffered view over the lexer — webpack's stand-in for the spec's
 ...
 * tokenizer), primed into `_gen` by the constructor, and the spec token-stream
 * operations `next` / `consume` / `discard` / `mark` / `restoreMark` /
 * `discardMark` read from it. `parse*` entry points wrap a source string in one
 * of these and every `consume*` algorithm reads tokens from it instead of
 * re-scanning the source with byte math, so a string is tokenized exactly once
 * per parse.
 *
 ...
 * Buffering makes `next` and `mark` / `restoreMark` cheap — the
 * declaration-vs-qualified-rule backtracking in `consumeABlocksContents`
 * rewinds via `restoreMark`. Tokens are pulled lazily, so a `parse*` that only
...
class TokenStream {
	 ...
	 * §4: the token stream's final token). The constructor primes `_gen` with
	 * it; `next` then buffers and comment-filters the stream, while iterating it
	 * directly (`new TokenStream(src).tokenize()`) yields the raw token list.
	 * Override in a subclass to drive a different language through the same
	...
	*tokenize(pos = 0) {
	 ...
	 * non-comment tokens (the `<eof-token>` included), firing `onComment` for
	 * every comment skipped.
	 ...
	 * token the consume algorithms dispatch on (the spec's "process").
	 * @returns {CssToken | EofToken} the next token
	 */
	next() {
		this._fill(this._index + 1);
		const t = this._buf[this._index];
		// Past the buffered `<eof-token>` (the last entry) → keep returning it.
		return t === undefined ? this._buf[this._buf.length - 1] : t;
	}
	 ...
	 */
	discard() {
		if (this.next().type !== "EOF") this._index++;
	}
...
const parseADeclaration = (input, pos, comment) => {
	...
	while (ts.next().type === "whitespace") ts.discard();
	if (ts.next().type !== "identifier") return undefined;
...
/**
 * Consume an at-rule, CSS Syntax Level 3 [§5.4.2](https://drafts.csswg.org/css-syntax/#consume-at-rule) — consumes the at-keyword, then the prelude up to `;` / `{` / `}` / EOF (each component value via `consumeAComponentValue`); `{` consumes the block (§5.4.4) onto `.block`, a `;` is discarded, and a top-level `}` (when not `nested`) is consumed as a parse error. Returns `undefined` only when the stream is not at an at-keyword.
 * @param {TokenStream} ts token stream
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	if (ts.next().type !== "atKeyword") return undefined;
	...
	for (;;) {
		...
		if (t.type === "rightCurlyBracket") {
			...
			// `{` → consume the at-rule's block (§5.4.4: its parsed contents).
...
const consumeAQualifiedRule = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		const t = ts.next();
		// EOF or the stop token → parse error, return nothing.
		if (t.type === "EOF" || t.type === stopToken) return undefined;
		if (t.type === "rightCurlyBracket") {
			// Parse error.
			if (nested) return undefined;
			// Consume the stray top-level `}` and continue.
			ts.consume();
		} else if (t.type === "leftCurlyBracket") {
			// `{` → consume the block (§5.4.4: its parsed contents).
			const block = consumeABlock(ts);
 ...
 * @param {TokenStream} ts token stream
 * @returns {SimpleBlock} the block (its `value` is the parsed contents)
 */
const consumeABlock = (ts) => {
	const { locConverter } = ts;
	// Assert: the next token is `{`. Discard it.
	const start = ts.next().start;
	ts.discard();
	const { values, end: contentsEnd } = consumeABlocksContents(ts);
	// Discard the closing `}` (EOF if unterminated — a parse error).
	const close = ts.next();
	let end = contentsEnd;
	if (close.type === "rightCurlyBracket") {
		end = close.end;
		ts.discard();
	}
	return new SimpleBlock("{", values, [start, end], locConverter);
};
...
const consumeADeclaration = (ts, nested = false) => {
	...
	if (ts.next().type !== "colon") return undefined;
	ts.discard();
...
/**
 * Consume a function, CSS Syntax Level 3 [§5.4.10](https://drafts.csswg.org/css-syntax/#consume-function) — the next token must be a `<function-token>` (the caller has checked it); consumes component values via `consumeAComponentValue` until the matching `)` or EOF, returning the partial function on EOF (parse error).
 * @param {TokenStream} ts token stream
 ...
 * lexer + stream class). `recurseBlocks: false` skips walking block bodies'
 * (eagerly parsed) nested rules (caller drives nested traversal itself).
