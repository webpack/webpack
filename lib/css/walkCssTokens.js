...
const parseAStylesheet = (input, comment) => {
	...
	return parseAStylesheetsContents(ts, comment);
};
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	for (;;) {
		...
		else if (t.type === "rightCurlyBracket" && nested) {
			return /** @type {AtRule} */ (
				makeNode(T_AT_RULE, [start, t.start], locConverter, {
					name,
					nameRange: [start, nameEnd],
 ...
 * @param {TokenStream} ts source string or an existing token stream
...
const consumeASimpleBlock = (ts) => {
	...
	for (;;) {
		...
		if (t.type === "EOF" || t.type === ending) {
			...
			ts.discard();
			return /** @type {SimpleBlock} */ (
				makeNode(T_SIMPLE_BLOCK, [open.start, t.end], ts.locConverter, {
					token: kind,
					value: values
				})
			);
	...
	for (;;) {
		...
		if (t.type === "EOF" || t.type === "rightParenthesis") {
			...
			return /** @type {FunctionNode} */ (
				makeNode(T_FUNCTION, [tFn.start, t.end], locConverter, {
