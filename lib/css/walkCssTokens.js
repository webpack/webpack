...
/**
 * Hash token (`#foo`). `value` is the name without the leading `#`; `id` is the
 * spec type-flag — true when the name forms a valid `<id>` selector.
 * @typedef {Token & { id: boolean }} HashToken
 */
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	for (;;) {
		...
		if (t.type === "semicolon" || t.type === "EOF") {
			...
			return /** @type {AtRule} */ (
				makeNode(T_AT_RULE, [start, t.start], locConverter, {
					name,
		...
		if (t.type === "rightCurlyBracket") {
			...
			return /** @type {AtRule} */ (
				makeNode(T_AT_RULE, [start, block.range[1]], locConverter, {
					name,
					nameRange: [start, nameEnd],
					prelude,
					block
...
const consumeAQualifiedRule = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		...
		if (t.type === "rightCurlyBracket") {
			...
			const block = consumeABlock(ts);
			return /** @type {QualifiedRule} */ (
				makeNode("QualifiedRule", [start, block.range[1]], locConverter, {
					prelude,
					block
				})
			);
...
const consumeABlock = (ts) => {
	...
	return /** @type {SimpleBlock} */ (
		makeNode(T_SIMPLE_BLOCK, [start, end], locConverter, {
			token: "{",
			value: values
		})
	);
...
const consumeADeclaration = (ts, nested = false) => {
	...
	return /** @type {Declaration} */ (
		makeNode(T_DECLARATION, [declStart, end], locConverter, {
...
const consumeASimpleBlock = (input, pos, comment) => {
	...
	return /** @type {SimpleBlock} */ (
		makeNode(T_SIMPLE_BLOCK, [open.start, end], locConverter, {
			token: kind,
			value: values
		})
	);
...
const consumeAFunction = (ts) => {
	...
	return /** @type {FunctionNode} */ (
		makeNode(T_FUNCTION, [fnTok.start, end], locConverter, {
			name,
			nameRange,
			value: values
		})
...
/**
 * The `SourceProcessor` grammar: parse via `parseAStylesheet`, walk the tree in
 * source order firing `enter` / `exit`. `recurseBlocks: false` skips walking
