 ...
 * @property {("stylesheet" | "block-contents")=} as which CSS Syntax §5.3 production to parse the source as: `"stylesheet"` (default, streamed) or `"block-contents"` (a block's contents, e.g. an HTML `style` attribute)
 */
...
const grammar = (input, ctx) => {
	...
	if (ctx.as === "block-contents") {
		// `as: "block-contents"` — parse a block's contents (§5.4.4 algorithm,
		// e.g. an HTML `style` attribute) and walk its declarations then nested
		// rules. The whole block is small, so it's materialized.
...
class SourceProcessor {
	 ...
	 * AST retained (in the default `"stylesheet"` mode).
	 * @param {string} input source text
