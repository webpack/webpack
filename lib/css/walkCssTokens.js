...
class Node {
	...
	/**
	 * Serialize this node back to source. The default re-slices the original
	 * input that this node's `LocConverter` was constructed with — zero
	 * allocation for untouched nodes. Once mutation visitors land (replace /
	 * remove / insert), composite subclasses will override this to recurse
	 * into their children when an internal dirty flag is set. The contract
	 * `Node#toString()` returns the CSS-text equivalent of this node holds
	 * either way, so consumers can rely on it without knowing whether the
	 * subtree was touched.
...
const parseAListOfComponentValues = (
	...
	let endPos = pos;
	for (const t of tokenize(input, pos)) {
		endPos = t.end;
		if (t.type === "comment") {
 ...
 * optional comment-token callback forwarded to `tokenize` while
 ...
 * @param {LocConverter} locConverter shared loc converter
 * @param {((input: string, start: number, end: number) => number)=} comment
 * optional comment-token callback forwarded to `tokenize`
...
/**
 * Internal: drive `tokenize` with a seed frame already pushed on the
...
const _consumeIntoSeedFrame = (
	...
	let endPos = startPos;
	for (const t of tokenize(input, startPos)) {
		endPos = t.end;
		if (t.type === "comment") {
			if (comment) comment(input, t.start, t.end);
			continue;
...
// Cache `escape`/`unescapeIdentifier` results per
// `state.compilation.compiler.root` — CssParser binds the cache once
// per parse via `unescapeIdentifier.bindCache(state.compilation.compiler.root)`
// and re-uses it for every identifier in the source. Matches main's
// `53c363b` perf optimization.
const escapeIdentifier = makeCacheable(_escapeIdentifier);
const unescapeIdentifier = makeCacheable(_unescapeIdentifier);

/**
 * A Babel-style visitor map keyed by `Node#type`. A bucket is either a
 * shorthand function (enter-only) or `{ enter?, exit? }`. Multiple
 * `SourceProcessor#use()` calls accumulate buckets per type — each enter /
 * exit list runs in registration order.
 * @typedef {(node: Node, parent: Node | null) => void} VisitorFn
...
/**
 * Default grammar for `SourceProcessor` — parses CSS via `parseAStylesheet`
 * and walks the resulting tree in source order, firing the visitor map.
 * Kept as a plain function (not bound to the class) so callers can swap
 * it for an HTML / Markdown grammar later by passing `{ grammar }` to the
 * `SourceProcessor` constructor.
 *
 * Two walkers below handle the two flavors of CSS subtree:
 *
 * `walkRule` — `AtRule` / `QualifiedRule` / `Declaration` and their
 * bodies. When `ctx.recurseBlocks` is true (default) and a block at-rule
 * or qualified rule carries a `SimpleBlock` body, its raw token list is
 * **re-parsed** via `parseABlocksContents` so nested declarations and
 * CSS-Nesting rules surface to the visitor as `Declaration` / `AtRule`
 * / `QualifiedRule` — not as a flat soup of tokens. The re-parse is
 * O(block-bytes) and runs at most once per block, so the end-to-end
 * traversal stays a single pass over the source. Pass
 * `recurseBlocks: false` to suppress the re-parse — useful when the
 * consumer drives nested traversal itself (e.g. while migrating from
 * a hand-rolled walker) and only needs visitor events at the top
 * level.
 *
 * `walkValue` — component-value subtrees (`FunctionNode`, `SimpleBlock`
 * inside an expression, atomic tokens). These are already fully formed
 * by `parseAComponentValue` so the walker just recurses into structural
 * children.
 *
 * Visitors fire `enter` on the way down and `exit` on the way up. Atomic
 * nodes (tokens, declarations with no value composites, empty functions)
 * still fire both — `exit` is called immediately after `enter` so
 * consumers can rely on the pair without branching on arity.
...
const cssGrammar = (input, ctx) => {
	...
	// Skip-children flag set by `ctx.skipChildren()` inside an `enter`
	// visitor. Babel calls this `path.skip()` — for CSS we only need the
	// children-skip variant (replacing / removing nodes is a future
	// mutation-API concern). Cleared after each `enter` dispatch so it
	// applies to that node only.
	let skipFlag = false;
	const visitorCtx = {
		skipChildren() {
			skipFlag = true;
	...
	const fireExit = (node, parent) => {
		const b = visitors.get(node.type);
		if (b) for (const fn of b.exit) fn(node, parent);
	};

	/**
	 * Walk a component-value subtree (FunctionNode / SimpleBlock inside an
	 * expression / atomic Token). Children are already fully materialized
	 * by `parseAComponentValue`; no re-parsing is needed here.
	...
	/**
	 * Walk a structural subtree (AtRule / QualifiedRule / Declaration).
	 * Block bodies are re-parsed into nested rules via
	 * `parseABlocksContents` so the visitor sees a fully structured
	 * tree even though `parseAStylesheet` keeps the SimpleBlock
	 * body as raw tokens.
			...
			switch (node.type) {
				...
				case "QualifiedRule": {
					...
					if (r.block && recurseBlocks) {
						// `block.start` is on the opening `{`; the body starts one
						// byte past it. `parseABlocksContents` stops at the
						// matching `}` (the position of which already lives in
						// `block.end - 1`), so the re-parse covers exactly the
						// same bytes the tokenizer already accepted into
						// `block.value` — no risk of seeing more or less than the
						// original parse.
...
/**
 * `SourceProcessor` is the language-agnostic visitor coordinator. It owns
 * the `_visitors` registry; everything else (lexer, AST node shapes,
 * grammar / state machine) is injected so the same class can drive CSS,
 * HTML, Markdown, … parsers in the future. The CSS bindings live in this
 * file for now because they share the underlying `tokenize` /
 * `parseA*` helpers; a follow-up will split `SourceProcessor` and the
 * generic `Node` base into a language-neutral module under
 * `lib/parsing/`.
 *
 * The visitor surface is Babel-style:
 *
 * ```
 * processor.use({
 *   Declaration: { enter(d) { ... }, exit(d) { ... } },
 *   AtRule:      (at) => { ... },        // shorthand = enter only
 *   Function:    { enter(fn) { ... } }
 * });
 * processor.parse(source);
 * ```
 * Future extension points (deliberately stubbed today; no caller change
 * needed when they land):
 *
 * **Tapable hooks** — `_visitors` becomes `Map<type, SyncHook>` and `use()`
 * taps the relevant hook. The public `use(map)` shape doesn't change.
 *
 * **Retained AST + transforms** — `parse(input, { retain: true })` will
 * return the tree; visitors will gain `ctx.replace(node)` / `ctx.remove()`
 * / `ctx.skip()` once the mutation API lands.
 *
 * **Source-map-carrying serialization** — `Node#toAstSource(loc)` will
 * sit alongside `toString()` and use the byte ranges already stored.
 */
class SourceProcessor {
	 ...
	 * `lexer` — token source (default: `tokenize`). Swap for an
	 * HTML / Markdown tokenizer when wiring a non-CSS processor.
	 *
	 * `nodes` — language-specific node constructors. Currently unused by
	 * `cssGrammar` (the `parseA*` helpers allocate `Declaration` /
	 * `AtRule` / … directly), but the slot is wired so a future grammar
	 * that builds nodes generically — or a richer CSS dialect that needs
	 * typed sub-nodes (e.g. `Selector`, `MediaQuery`) — can plug in
	 * without touching `SourceProcessor`. Name is intentionally **`nodes`**
	 * rather than `astBuilder`: the future contract is broader than
	 * "build" — each node also carries serialization (`toString()` today,
	 * `toAstSource()` later) and hook-firing behavior, so the bag is
	 * "the node types this processor uses," not "a factory."
	 *
	 * `grammar` — language-specific state machine (default: `cssGrammar`).
	 * Receives the input plus a `ctx` carrying the compiled visitor map
	 * and a shared `LocConverter`; walks the parsed tree and fires events.
	 */
	...
	/**
	 * Register a Babel-style visitor map. Multiple calls accumulate per
	 * node type — useful for plugin composition. A bucket value may be
	 * either a function (shorthand for `{ enter: fn }`) or
	 * `{ enter?, exit? }`.
	...
	/**
	 * Drive the configured grammar over `input`, firing accumulated
	 * visitors as the parser walks the source. No AST is retained.
	 * @param {string} input source text
	 * @param {{ locConverter?: LocConverter, comment?: (input: string, start: number, end: number) => number, recurseBlocks?: boolean }=} ctx
	 * `locConverter` — reuse an existing converter (default: fresh).
	 * `comment` — comment-token callback forwarded to the underlying tokenizer.
	 * `recurseBlocks` — when `false`, `cssGrammar` stops at top-level rules
	 * and does not re-parse `{ ... }` bodies into nested rules. Useful
	 * while migrating a consumer that already drives nested traversal
	 * itself; default `true`.
	 */
	parse(input, ctx = {}) {
...
// CSS-syntax-level utilities re-exposed as static members of
// `SourceProcessor`. CssParser.js can now import only `SourceProcessor`
// and reach the helpers via destructuring — no direct dependency on
// `walkCssTokens` exports beyond the class itself.
//
// `eat*` are byte-skipping primitives (whitespace, whitespace + comments,
// single-newline, `eatUntil(char)` factory). `parseAComponentValue` /
// `parseASimpleBlock` are spec parse helpers used by feature handlers
// for sub-range parsing. `escapeIdentifier` / `unescapeIdentifier` are
// pure string utilities (escape per CSS Syntax 3 §4.3.7).
// `isWhiteSpace` is a char-code predicate.
SourceProcessor.eatUntil = eatUntil;
SourceProcessor.eatWhiteLine = eatWhiteLine;
SourceProcessor.eatWhitespace = eatWhitespace;
SourceProcessor.eatWhitespaceAndComments = eatWhitespaceAndComments;
SourceProcessor.escapeIdentifier = escapeIdentifier;
SourceProcessor.isWhiteSpace = _isWhiteSpace;
SourceProcessor.parseAComponentValue = parseAComponentValue;
SourceProcessor.parseASimpleBlock = parseASimpleBlock;
SourceProcessor.unescapeIdentifier = unescapeIdentifier;

// Public surface. The AST node classes are exported because they're
// referenced as `@typedef {import("./walkCssTokens").X}` types by
// `CssParser.js`. The `parseA*` family is the CSS Syntax Level 3 parser
// entry-point surface (parse a stylesheet / a list of component values /
// a comma-separated list / a component value / a declaration, and the
// consume-algorithm helpers for at-rules, qualified rules, blocks-contents,
// simple blocks and functions) — kept complete so the module is a usable
// spec parser, independent of which subset `CssParser.js` currently calls.
// `escapeIdentifier` / `unescapeIdentifier` are exported for the
// `cssIdentifier` unit test, which asserts they're the same references
// `CssParser` re-exports. The byte-level `eat*` / `isWhiteSpace` helpers
// are intentionally **not** exported — they're reachable through
// `SourceProcessor`'s static members for `CssParser.js`.
