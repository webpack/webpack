 ...
 *
 * Note: `consumeABlock` (§5.4.4) returns the parsed block *contents* directly
 * (a `(Declaration[] | Rule)[]` per §5.4.5), not a SimpleBlock wrapper — see
 * `AtRule.childRules` / `QualifiedRule.childRules`.
 ...
 * `childRules` holds the parsed block contents (a `(Declaration[] | Rule)[]`),
 * or `null` for a `;`-terminated at-rule. `blockRange` is `[start of {, end of }]`
 * (webpack extension, not in spec; the spec doesn't track brace positions).
 * `range[1]` points past `}` for a block, or at the `;` / `}` / EOF position
 * otherwise (callers check the byte at `range[1]` to tell them apart).
 * @typedef {Node & { name: string, nameRange: [number, number], prelude: ComponentValue[], childRules: (Declaration[] | Rule)[] | null, blockRange: [number, number] | null }} AtRule
 */

/**
 * Qualified rule: `<prelude> { <block> }`. `prelude` is the component values
 * before the `{` (selectors, keyframe parameters, …); `childRules` is the
 * parsed `{ ... }` body, or `null` when EOF was hit before `{`. `blockRange` is
 * `[start of {, end of }]` (webpack extension).
 * @typedef {Node & { prelude: ComponentValue[], childRules: (Declaration[] | Rule)[] | null, blockRange: [number, number] | null }} QualifiedRule
 */
 ...
 * @param {((input: string, start: number, end: number) => number)=} comment optional comment-token callback (string input only)
 * @returns {(Declaration[] | Rule)[]} block contents (consecutive declarations are grouped into sub-lists per §5.4.5)
 */
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	for (;;) {
		...
		// Consume a block from input, and assign the result to rule's child rules.
			...
			if (block) {
				rule.childRules = block.contents;
...
/**
 * Consume a block, CSS Syntax Level 3 [§5.4.4](https://drafts.csswg.org/css-syntax/#consume-block) — the next token must be `<{-token>`; discards it, consumes the block's contents (§5.4.5), discards the closing `}`, and returns the result. The spec returns just the contents; we also return the `[start of {, end of }]` range so callers can record the block's source position.
 * @param {TokenStream} ts token stream
 * @returns {{ contents: (Declaration[] | Rule)[], range: [number, number] } | undefined} block contents and `[start of {, end of }]` range, or `undefined` if the precondition assert failed
 */
const consumeABlock = (ts) => {
	...
	ts.discard();
	const contents = consumeABlocksContents(ts);
	const close = ts.next();
	const end = close.type === "rightCurlyBracket" ? close.end : close.start;
	ts.discard();
	return { contents, range: [open.start, end] };
};

/**
 * Consume a block's contents, CSS Syntax Level 3 [§5.4.5](https://drafts.csswg.org/css-syntax/#consume-block-contents). The returned list contains either a `Rule` (at-rule / qualified-rule), or a sub-list of consecutive `Declaration`s (the spec's "list of declarations" grouping) — `consumeABlocksContents` flushes the running decls list to the parent on each at-rule / qualified-rule boundary.
 * @param {TokenStream} ts token stream
 * @returns {(Declaration[] | Rule)[]} consumed contents (stops at the enclosing `}` / EOF, left in the stream)
 */
const consumeABlocksContents = (ts) => {
	...
	for (;;) {
		...
		// If decls is not empty, append it to rules. Return rules.
			...
			if (decls.length > 0) rules.push(decls);
			return rules;
		}
		// <at-keyword-token>
		...
		// Otherwise, restore a mark from input, then consume a qualified rule from input, with nested set to true, and <semicolon-token> as the stop token. If a rule was returned, if decls is not empty, append decls to rules, and set decls to a fresh empty list of declarations. Append the rule to rules.
		else {
			ts.mark();
...
const consumeADeclaration = (ts, nested = false, consumeBadRemnants = true) => {
	...
	const isCustomProperty = input.startsWith("--", decl.nameRange[0]);
	if (
		!isCustomProperty &&
		decl.value.some(
			(v) =>
		...
		if (!skip) {
			switch (node.type) {
				...
				case T_QUALIFIED_RULE: {
					...
					if (r.childRules && recurseBlocks) {
						// Child rules (§5.4.5): each item is either a single Rule or a Declaration[] sub-list; walk both as flat siblings.
						for (const item of r.childRules) {
							if (Array.isArray(item)) {
								for (const d of item) walkRule(d, node);
							} else {
								walkRule(item, node);
							}
						}
