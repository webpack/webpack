...
/**
 * Number token (`123`, `-1.5`, `+2e3`). `value` is the raw source slice (the
 * spec's "value"). `numericValue` / `typeFlag` / `sign` are derived from it
 * lazily — the parse + classification is skipped for the (common) numeric tokens
 * whose numeric value is never read.
 */
class NumberToken extends Token {
	...
	/** @returns {"integer" | "number"} the spec type flag */
	get typeFlag() {
		return _typeFlagOf(this.value);
	}

	/** @returns {"+" | "-" | ""} the spec sign */
	get sign() {
...
/**
 * Percentage token (`50%`). `value` is the raw slice including `%`; `numericValue`
 * (without `%`) and `sign` are derived lazily.
 */
class PercentageToken extends Token {
	/** @returns {number} the parsed value (without `%`) */
	get numericValue() {
		return Number(this.value.slice(0, -1));
	}
...
/**
 * Dimension token (`100px`, `1.5em`). `value` is the raw slice (number + unit),
 * split at `unitStart`. `numericValue` / `typeFlag` / `sign` (of the numeric
 * part) and `unit` (lower-cased) are derived lazily.
 */
class DimensionToken extends Token {
	/**
	 * @param {number} start byte offset of the first numeric code point
	 * @param {number} end byte offset just past the unit
	 * @param {LocConverter} locConverter shared loc converter
	 * @param {number} unitStart byte offset of the first unit code point
	 */
	constructor(start, end, locConverter, unitStart) {
		super(T_DIMENSION, start, end, locConverter);
		this.unitStart = unitStart;
	}

	/** @returns {string} the numeric part (no unit) */
	get _numPart() {
		return this._locConverter._input.slice(this.start, this.unitStart);
	}

	/** @returns {number} the parsed numeric value */
	get numericValue() {
		return Number(this._numPart);
	}

	/** @returns {"integer" | "number"} the spec type flag */
	get typeFlag() {
		return _typeFlagOf(this._numPart);
	}

	/** @returns {"+" | "-" | ""} the spec sign */
	get sign() {
		return _signOf(this._numPart);
	}

	/** @returns {string} the unit, lower-cased per spec */
	get unit() {
		return this._locConverter._input
			.slice(this.unitStart, this.end)
			.toLowerCase();
	}
}
...
const tokenToNode = (t, input, locConverter) => {
	switch (t.type) {
		...
		// `unit` are lazy getters, so the `Number()` parse + slices + `toLowerCase`
		// are skipped for the (common) tokens whose numeric value is never read.
		case TT_NUMBER:
			return new NumberToken(T_NUMBER, t.start, t.end, locConverter);
		case TT_PERCENTAGE:
			return new PercentageToken(T_PERCENTAGE, t.start, t.end, locConverter);
		case TT_DIMENSION:
			return new DimensionToken(t.start, t.end, locConverter, t.unitStart);
		case TT_HASH: {
