		...
		while (pos < input.length) {
			if (
				...
				if (callbacks.comment) {
					pos = callbacks.comment(input, start, pos + 2);
				} else {
					pos += 2;
				}
				break;
...
/** @type {CharHandler} */
const consumeCommentsRev = (input, pos, callbacks) => {
	if (
		input.charCodeAt(pos) === CC_SOLIDUS &&
		input.charCodeAt(pos - 1) === CC_ASTERISK
	) {
 ...
 */
module.exports.eatWhitespaceAndComments = (input, pos, callbacks) => {
	for (;;) {
		const originalPos = pos;
		pos = consumeComments(input, pos, callbacks);
		while (_isWhiteSpace(input.charCodeAt(pos))) {
 ...
 * @returns {number} position after whitespace and comments
 */
module.exports.eatWhitespaceAndCommentsRev = (input, pos) => {
	for (;;) {
		const originalPos = pos;
		pos = consumeCommentsRev(input, pos, {});
		while (_isWhiteSpace(input.charCodeAt(pos))) {
			pos--;
		}
		if (originalPos === pos) {
