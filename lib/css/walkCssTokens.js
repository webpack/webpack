...
const consumeAStylesheetsContents = (ts) => {
	...
	for (;;) {
		const t = ts.peek();
		if (t.type === "EOF") return { rules, end: t.start };
		// §5.4.1: discard whitespace and top-level CDO / CDC tokens.
		...
		// Anything else → qualified rule (the token is left in the stream for
		// `consumeAQualifiedRule` to reconsume).
		const mark = ts.mark();
		const rule = consumeAQualifiedRule(ts);
		// Defensive: a stray `}` makes no progress — drop a token to advance.
		if (ts.mark() === mark) {
			ts.next();
			continue;
		}
...
const consumeAnAtRule = (ts) => {
	...
	// Prelude: consume component values until the at-rule's `;`, its block
	// `{`, or an enclosing `}` / EOF (§5.4.2 step 2).
	/** @type {Node[]} */
	const prelude = [];
	let end = nameEnd;
	for (;;) {
		...
		if (t.type === "semicolon") {
			// `;` ends the at-rule; record its position as `end` (so callers can
			// detect the terminator), then consume it.
			end = t.start;
			ts.next();
			break;
		}
		...
		if (t.type === "leftCurlyBracket") {
			// `{` → consume the block as the at-rule's body. The grammar
			// re-parses its raw values into nested rules later.
...
/**
 * Consume a qualified rule, CSS Syntax Level 3 [§5.4.3](https://drafts.csswg.org/css-syntax/#consume-qualified-rule) — consumes the prelude (via `consumeAComponentValue`) up to `{` then the `{ … }` block; on EOF/`}` before `{` it returns a `block: null` rule (the spec drops it) so callers keep the prelude span.
 * @param {TokenStream} ts token stream
...
const consumeAQualifiedRule = (ts) => {
	...
	for (;;) {
		const t = ts.peek();
		if (t.type === "EOF" || t.type === "rightCurlyBracket") {
			// EOF or stray `}` before the block opened — parse error; keep the
			// prelude span and leave the token for the caller.
			end = t.start;
...
const consumeABlocksContents = (ts) => {
	...
	for (;;) {
		...
		// Enclosing `}` — let the caller consume it.
...
const consumeADeclaration = (ts) => {
	...
	// Step 5: consume the value as a list of component values, stopping at the
	// top-level `;` (the stop token, left in the stream) or — since a
	// declaration is always inside a `{}` block — a top-level `}`. A top-level
	// `{}` block is consumed into the value as a simple block.
	const { values, end } = consumeAListOfComponentValues(ts, "semicolon", true);
...
const consumeAListOfComponentValues = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		...
		if (t.type === "rightCurlyBracket") {
			...
			// §5.4.7: a stray top-level `}` is a parse error — consume it as a
			// token (`tokenToNode` drops it, matching parse-css recovery).
			ts.next();
			end = t.end;
			continue;
		}
...
const consumeAComponentValue = (ts) => {
	...
	// Otherwise consume a single token (a dropped token yields `undefined`).
	ts.next();
