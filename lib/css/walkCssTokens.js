 ...
 * bodies. When a block at-rule or qualified rule carries a `SimpleBlock`
 * body, its raw token list is **re-parsed** via `parseABlocksContents`
 * so nested declarations and CSS-Nesting rules surface to the visitor
 * as `Declaration` / `AtRule` / `QualifiedRule` — not as a flat soup of
 * tokens. The re-parse is O(block-bytes) and runs at most once per
 * block, so the end-to-end traversal stays a single pass over the
 * source.
 *
 ...
 * @param {{ visitors: CompiledVisitorMap, locConverter: LocConverter, comment?: (input: string, start: number, end: number) => number }} ctx grammar context
 */
 ...
 *   "at-rule":   (at) => { ... },        // shorthand = enter only
 *   function:    { enter(fn) { ... } }
 * });
...
class SourceProcessor {
	 ...
	 * @param {string} input source text
	 * @param {{ locConverter?: LocConverter, comment?: (input: string, start: number, end: number) => number }=} ctx
