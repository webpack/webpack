...
/**
 * Consume a component value, CSS Syntax Level 3
 * [§5.4.8](https://drafts.csswg.org/css-syntax/#consume-component-value).
 * Discards leading whitespace, then consumes exactly **one** component value
 * from the stream — a token, a simple block, or a function call — leaving the
 * rest of the stream untouched. Returns `undefined` if the stream is at EOF
 * after the whitespace skip.
 *
 ...
 * Strict entry point: consumes one component value and then requires the rest
 * of the input to be whitespace only, returning `undefined` (syntax error) if
 * anything else trails. Callers that want "one value, ignore the rest" should
 * use `consumeAComponentValue` on a stream instead.
 * @param {string | TokenStream} input source string or an existing token stream
 * @param {number=} pos start position (string input only)
 ...
 * `undefined` if the stream is not at an at-keyword. Per the spec, the
 * at-rule's own block is consumed here: when the prelude is followed by a
 * `{`, the `{ … }` block becomes `AtRule#block`; a trailing `;` is
 * discarded; a `}` / EOF ends the at-rule and is left for the enclosing
 * block's contents.
 *
 * Difference from the spec: `consume the remnants of a bad declaration`-style
 * recovery is not run, and `}` at the prelude's top level ends the prelude
 ...
 * before the declaration attempt and `reset`ting on failure. At-rules and
 * qualified rules consume their own blocks (`consumeAnAtRule` /
 * `consumeAQualifiedRule`), so this just collects whatever they return.
