 ...
 * list of component values between the `@-keyword` and the stop token
 * (whitespace preserved — callers strip as needed). The block (`{...}`) is
 * **not** consumed by `consumeAnAtRule` — it leaves the stream positioned at
 * the stop token (`;` / `{` / `}` / EOF), parse-css style, so the caller
 * peeks to decide. When `consumeABlocksContents` walks a block, it fills in
 * `.block` and updates `.end` to point past `}`. To tell a `;`-terminated
 * at-rule from a `}`/EOF one, a caller checks the byte at `.end`.
 */
class AtRule extends Node {
	 ...
	 * @param {Node[]} prelude list of component values up to the stop token
	 * @param {number} start start offset (the `@`)
	 * @param {number} end end offset (the stop token's position, or EOF)
	 * @param {LocConverter} locConverter shared loc converter
	 */
 ...
 * the stream so the caller can inspect it with `ts.peek()` (e.g.
 * `consumeABlocksContents` consumes a declaration's trailing `;`,
 * `consumeAQualifiedRule` reads the `{`). This mirrors tabatkins/parse-css,
 * which leaves the stream positioned at the stop token rather than returning
 * a separate terminator value.
...
const consumeADeclaration = (ts) => {
	...
	// `;`, `}`, or `{`. The stop token itself is **not** consumed — it
	...
	// CSS Nesting disambiguation: if we stopped at a `{` (now the next token
	// in the stream), this isn't a declaration — it's the body of a qualified
	// rule (e.g. `div:not(:local(.x)) { … }`). Returning `undefined` lets the
	...
	if (ts.peek().type === "leftCurlyBracket") return undefined;
 ...
 * is **not** consumed — the stream is left positioned at the stop token
 * (`{` to enter a block, `;` / `}` / EOF for an ended at-rule), so the
 * caller peeks to decide (parse-css style).
 *
 * Difference from the spec: `consume the remnants of a bad declaration`-style
 * recovery is not run, and `}` at the prelude's top level stops the prelude
 * rather than being consumed as a delim. Both deviations mirror the
 * conservatism of `consumeADeclaration` — the caller handles structural
 * recovery one level up.
 * @param {TokenStream} ts token stream
...
const consumeAnAtRule = (ts) => {
	...
	return new AtRule(name, [start, nameEnd], values, start, end, locConverter);
};
 ...
 * of the enclosing block (`end` points **at** the `}`; not consumed, left
 * in the stream for the caller) — or at EOF (`end === input.length`).
 *
...
const consumeABlocksContents = (ts) => {
	...
	for (;;) {
		...
		if (head.type === "atKeyword") {
			...
			// `consumeAnAtRule` left the stream at the stop token.
			if (ts.peek().type === "leftCurlyBracket") {
				// `@media (…) { … }` — consume the block.
				const block = /** @type {SimpleBlock} */ (
					_consumeSimpleBlockFromStream(ts)
				);
				atRule.block = block;
				atRule.end = block.end;
			} else if (ts.peek().type === "semicolon") {
...
const parseARule = (input, pos, comment) => {
	...
	if (head.type === "atKeyword") {
		...
		if (at && ts.peek().type === "leftCurlyBracket") {
			const block = /** @type {SimpleBlock} */ (
				_consumeSimpleBlockFromStream(ts)
			);
			at.block = block;
				...
				case "QualifiedRule": {
					...
					if (r.block && recurseBlocks) {
						...
						const body = parseABlocksContents(
							new TokenStream(input, r.block.start + 1, locConverter, comment)
						);
...
class SourceProcessor {
	 ...
	 * currently unused), `grammar` (default `grammar`).
	 */
	constructor({ lexer, nodes, grammar: grammarOption } = {}) {
