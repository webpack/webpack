...
class TokenStream {
	...
	/**
	 * The CSS tokenizer (CSS Syntax Level 3 §4): returns every `CssToken` in
	 * `this.input` from `pos` in source order — comment and whitespace tokens
	 * included — ending with an `<eof-token>` (CSS Syntax §4: the token stream's
	 * final token). `next` instead pulls one token at a time via the shared
	 * `readTokenInto` core (filtering comments); this collects the whole list.
	 * Not a generator: every consumer materializes the full list anyway, so a
	 * plain array avoids per-token iterator-result objects and the suspend/resume
	 * cost (~40-60% faster on large inputs). Production parsing never calls this —
	 * it drives the streaming `next` cursor.
	 * @returns {(MutableToken | EofToken)[]} every CSS token in `this.input`, then the `<eof-token>`
	 */
	tokenize() {
		const input = this.input;
		/** @type {(MutableToken | EofToken)[]} */
		const out = [];
		let pos = 0;
		for (;;) {
			const t = readTokenInto(input, pos, createToken());
			if (t === undefined) {
				out.push({ type: TT_EOF, start: input.length, end: input.length });
				return out;
			}
			out.push(t);
			pos = t.end;
