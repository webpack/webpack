...
const parseAComponentValue = (input, pos, options = {}) => {
	const ts = normalizeIntoTokenStream(input, pos, options.comment);
	// §5.3.9: discard leading whitespace; EOF here is a syntax error.
	while (ts.next().type === "whitespace") ts.discard();
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	for (;;) {
		const t = ts.next();
		// `;` / EOF, or a nested enclosing `}`, end the at-rule. `range[1]` points
		// AT the terminator (left in the stream for `}`) so callers can tell a
		// `;`-terminated rule from a `}` / EOF one; the `;` is discarded.
		...
		if (t.type === "rightCurlyBracket" && nested) {
			return /** @type {AtRule} */ (
				makeNode(T_AT_RULE, [start, t.start], locConverter, {
					name,
					nameRange: [start, nameEnd],
		...
		// Anything else (incl. a non-nested `}`, a parse error) → append.
		prelude.push(consumeAComponentValue(ts));
	}
};
...
const consumeAQualifiedRule = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		...
		// A nested top-level `}` → parse error, return nothing.
		if (t.type === "rightCurlyBracket" && nested) return undefined;
		if (t.type === "leftCurlyBracket") {
...
const consumeABlock = (ts) => {
	...
	// Discard the closing `}` (or the `<eof-token>` if unterminated — a parse error).
	const close = ts.next();
...
const consumeAListOfComponentValues = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		...
		// A nested `}` ends the list (left in the stream); a non-nested one is a parse error appended via `consumeAComponentValue`.
		if (t.type === "rightCurlyBracket" && nested) break;
 ...
 * @param {TokenStream} ts token stream
 * @returns {ComponentValue} the consumed component value
 */
const consumeAComponentValue = (ts) => {
	...
	// Anything else → consume a token and return it. (Assert: not EOF.)
	ts.consume();
	return tokenToNode(/** @type {CssToken} */ (t), input, locConverter);
};
...
const consumeASimpleBlock = (input, pos, comment) => {
	...
	if (open.type === "leftParenthesis") {
		...
		// Not a `(` / `[` / `{` — the public byte entry's stand-in for the spec assert.
		return undefined;
	}
	...
	for (;;) {
		...
		if (t.type === "EOF" || t.type === ending) {
			// Discard the closer (or EOF — a parse error) and return the block.
			ts.discard();
			return /** @type {SimpleBlock} */ (
				makeNode(T_SIMPLE_BLOCK, [open.start, t.end], locConverter, {
					token: kind,
					value: values
				})
			);
...
const consumeAFunction = (ts) => {
	const { input, locConverter } = ts;
	// Assert: the next token is a <function-token>. Consume it; `end` points just
	// past the `(`, so the name occupies [start, end - 1).
	...
	/** @type {ComponentValue[]} */
	const values = [];
	// Process input (§5.4.10).
	for (;;) {
		const t = ts.next();
		if (t.type === "EOF" || t.type === "rightParenthesis") {
			// Discard the `)` (or EOF — a parse error) and return the function.
			ts.discard();
			return /** @type {FunctionNode} */ (
				makeNode(T_FUNCTION, [fnTok.start, t.end], locConverter, {
					name,
					nameRange,
					value: values
				})
...
const grammar = (input, ctx) => {
	...
	// Stream the top-level rules (§5.4.1): consume one and walk it immediately, so no whole-stylesheet array is built before firing visitors.
	...
	for (;;) {
		...
		if (t.type === "whitespace" || t.type === "cdo" || t.type === "cdc") {
			...
			const at = consumeAnAtRule(ts);
			if (at) walkRule(at, null);
		} else {
			const rule = consumeAQualifiedRule(ts);
			if (rule) walkRule(rule, null);
		}
	}
};
