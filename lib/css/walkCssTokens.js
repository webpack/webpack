 ...
 * `consumeAComponentValue`) ignore it because they intercept `)` / `]` in
...
/**
 * Consume a single component value, CSS Syntax Level 3
 * [§5.4.8](https://drafts.csswg.org/css-syntax/#consume-a-component-value).
 ...
 */
const consumeAComponentValue = (input, pos, locConverter, options = {}) => {
	pos = eatWhitespaceAndComments(input, pos)[0];
 ...
 */
const consumeADeclaration = (input, pos, locConverter, comment) => {
	...
	// `consumeAListOfComponentValues` rewinds to its start so the outer
 ...
 * an EOF-implicitly-closed version). This is how a function (seed kind
 * `"function"`, closed by `)`) and `consumeASimpleBlock` (seed kind
 * `"(" | "[" | "{"`, closed by the matching `]` / `}` / `)`) are consumed
 * — the CSS-Syntax §5.4.10 "consume a function" algorithm is inlined here
 ...
 */
const consumeAnAtRule = (input, pos, locConverter, comment) => {
 ...
 */
const consumeAQualifiedRule = (input, pos, locConverter, comment) => {
	const start = pos;
	...
	const block = /** @type {SimpleBlock} */ (
		consumeASimpleBlock(input, end, locConverter, comment)
	);
 ...
 * `consumeADeclaration` and `consumeAQualifiedRule`. When an at-rule's
 * prelude is `{`-terminated we additionally consume the block via
 * `consumeASimpleBlock`, set `atRule.block`, and update `atRule.end` to
 ...
 */
const consumeABlocksContents = (input, pos, locConverter, comment) => {
	...
	while (pos < input.length) {
		...
		if (input.charCodeAt(pos) === CC_AT_SIGN) {
			const atRule = consumeAnAtRule(input, pos, locConverter, comment);
			if (!atRule) {
			...
			if (atRule.terminator === "{") {
				...
				const block = /** @type {SimpleBlock} */ (
					consumeASimpleBlock(input, atRule.end, locConverter, comment)
				);
				atRule.block = block;
		...
		// `!important`. `consumeADeclaration` returns undefined if the head
		// isn't a valid declaration; fall through to qualified-rule.
		const decl = consumeADeclaration(input, pos, locConverter, comment);
		if (decl) {
			values.push(decl);
			pos = decl.end;
		...
		const rule = consumeAQualifiedRule(input, pos, locConverter, comment);
...
const parseAStylesheetsContents = (input, locConverter, comment) => {
	const result = consumeABlocksContents(input, 0, locConverter, comment);
 ...
 * @param {string} input source
 * @param {number} pos start position (just past the opening `{`, or 0)
 * @param {LocConverter} locConverter shared loc converter
 * @param {((input: string, start: number, end: number) => number)=} comment optional comment-token callback forwarded to the tokenizer
...
const parseABlocksContents = (input, pos, locConverter, comment) =>
	consumeABlocksContents(input, pos, locConverter, comment);
...
const parseARule = (input, pos, locConverter, comment) => {
	...
	if (pos >= input.length) return undefined;
	if (input.charCodeAt(pos) === CC_AT_SIGN) {
		const at = consumeAnAtRule(input, pos, locConverter, comment);
		if (at && at.terminator === "{") {
			const block = /** @type {SimpleBlock} */ (
				consumeASimpleBlock(input, at.end, locConverter, comment)
			);
			at.block = block;
			at.end = block.end;
		}
	...
	return consumeAQualifiedRule(input, pos, locConverter, comment);
};
...
const parseADeclaration = (input, pos, locConverter, comment) =>
	consumeADeclaration(input, pos, locConverter, comment);
 ...
 * @param {number} pos start position
 * @param {LocConverter} locConverter shared loc converter
 * @param {{ comment?: (input: string, start: number, end: number) => number }=} options optional comment-token callback
 * @returns {{ value: Node, end: number } | undefined} the parsed component value, or undefined
 */
const parseAComponentValue = (input, pos, locConverter, options = {}) =>
	consumeAComponentValue(input, pos, locConverter, options);
 ...
 */
const parseAListOfComponentValues = (input, pos, locConverter, options = {}) =>
	consumeAListOfComponentValues(input, pos, locConverter, options);
			...
			switch (node.type) {
				...
				case "QualifiedRule": {
					...
					if (r.block && recurseBlocks) {
						...
						const body = consumeABlocksContents(
							input,
							r.block.start + 1,
							locConverter,
							comment
...
// Byte-level / sub-range helpers exposed on `SourceProcessor` so
// `CssParser.js` reaches them through the class.
...
// `parseA*` entry-point surface, and the `escape` / `unescapeIdentifier`
// string utils. The §5.4 `consumeA*` algorithms and the byte-level
// `eat*` / `isWhiteSpace` helpers stay internal.
