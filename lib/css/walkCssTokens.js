 ...
 * @property {("stylesheet" | "declaration-list")=} as parse the source as a full stylesheet (default) or a bare list of declarations (e.g. an HTML `style` attribute, CSS Syntax §5.4.5)
 */
...
const grammar = (input, ctx) => {
	...
	if (ctx.as === "declaration-list") {
		// `style` attributes (and other declaration-only contexts) are a block's
		// contents, not a stylesheet — top-level declarations are valid here.
		const { decls, rules } = consumeABlocksContents(ts);
...
class SourceProcessor {
	 ...
	 * AST retained.
	 * @param {string} input source text
	 * @param {{ locConverter?: LocConverter, comment?: (input: string, start: number, end: number) => number, recurseBlocks?: boolean, as?: ("stylesheet" | "declaration-list") }=} ctx reuse a `locConverter`, forward a `comment` callback, set `recurseBlocks: false` to stop at top-level rules, or set `as: "declaration-list"` to parse a bare declaration list
	 */
	process(input, ctx = {}) {
