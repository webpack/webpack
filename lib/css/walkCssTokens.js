...
/**
 * A `TopLevelConsumer` for a bare declaration list (a block's contents,
 * §5.4.5) — e.g. an HTML `style` attribute, where top-level declarations are
 * valid. Emits each declaration, then each nested rule, to `onNode`.
 * @param {TokenStream} ts token stream
...
/**
 * Drives the top-level consumption for a walk: reads from `ts` and hands each
 * top-level node (a rule, or — for a declaration list — a declaration) to
 * `onNode`. `consumeAStylesheetsContents` and `consumeADeclarationList` both
 * satisfy this shape.
 ...
 * @property {TopLevelConsumer=} consume top-level consumer driving the walk; defaults to `consumeAStylesheetsContents` (a full stylesheet). Pass `consumeADeclarationList` to parse a bare declaration list (e.g. an HTML `style` attribute)
 */
...
const grammar = (input, ctx) => {
	...
	const consume = ctx.consume || consumeAStylesheetsContents;
...
class SourceProcessor {
	 ...
	 * AST retained.
	 * @param {string} input source text
	 * @param {{ locConverter?: LocConverter, comment?: (input: string, start: number, end: number) => number, recurseBlocks?: boolean, consume?: TopLevelConsumer }=} ctx reuse a `locConverter`, forward a `comment` callback, set `recurseBlocks: false` to stop at top-level rules, or pass a `consume` driver (e.g. `consumeADeclarationList`) to parse something other than a full stylesheet
	 */
	process(input, ctx = {}) {
...
module.exports.consumeADeclarationList = consumeADeclarationList;
