 ...
 * `onRule` is the same streaming extension `consumeAStylesheetsContents`
 * exposes: when given, each top-level rule is handed to it instead of being
 * collected (the returned list is then empty), so the whole stylesheet is
 ...
 */
const parseAStylesheetsContents = (input, comment, onRule) => {
	// 1. Normalize input, and set input to the result.
	const ts = normalizeIntoTokenStream(input, 0, comment);
	// 2. Consume a stylesheet’s contents from input, and return the result.
	return consumeAStylesheetsContents(ts, onRule);
};
...
/**
 * A `TopLevelParser` for a bare declaration list — "parse a block's contents"
 * (§5.3.6), e.g. an HTML `style` attribute, where top-level declarations are
 * valid. Consumes from the existing `ts` (no re-tokenization) and hands each
 * declaration, then each nested rule, to `onNode`.
 * @param {TokenStream} ts token stream
 * @param {(node: Rule | Declaration) => void} onNode per-node sink
 * @returns {void}
 */
const parseADeclarationList = (ts, onNode) => {
	const { decls, rules } = parseABlocksContents(ts);
	for (let i = 0; i < decls.length; i++) onNode(decls[i]);
	for (let i = 0; i < rules.length; i++) onNode(rules[i]);
};
...
/**
 * Drives the top-level parse for a walk: reads from an existing `ts` (no
 * re-tokenization) and hands each top-level node (a rule, or — for a
 * declaration list — a declaration) to `onNode`. `parseADeclarationList`
 * satisfies this shape; the default is "parse a stylesheet's contents".
 * @typedef {(ts: TokenStream, onNode: (node: Rule | Declaration) => void) => void} TopLevelParser
 */
 ...
 * @property {TopLevelParser=} parse top-level parser driving the walk; defaults to `parseAStylesheetsContents` (a full stylesheet). Pass `parseADeclarationList` to parse a bare declaration list (e.g. an HTML `style` attribute)
 */
...
const grammar = (input, ctx) => {
	...
	const onNode = (/** @type {Node} */ node) => walkRule(node, null);
	// Default to "parse a stylesheet's contents"; a caller can inject another
	// top-level parser (e.g. `parseADeclarationList`). Both reuse `ts`, so no
	// re-tokenization, and the default still streams rule-by-rule.
	if (ctx.parse) ctx.parse(ts, onNode);
	else parseAStylesheetsContents(ts, undefined, onNode);
};
...
class SourceProcessor {
	 ...
	 * AST retained.
	 * @param {string} input source text
	 * @param {{ locConverter?: LocConverter, comment?: (input: string, start: number, end: number) => number, recurseBlocks?: boolean, parse?: TopLevelParser }=} ctx reuse a `locConverter`, forward a `comment` callback, set `recurseBlocks: false` to stop at top-level rules, or pass a `parse` driver (e.g. `parseADeclarationList`) to parse something other than a full stylesheet
	 */
	process(input, ctx = {}) {
...
module.exports.parseADeclarationList = parseADeclarationList;
