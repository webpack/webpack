...
const consumeAComponentValue = (ts) => {
	...
	// Otherwise consume a single token.
	ts.next();
	if (t.type === "semicolon") {
		return {
			value: new Token(T_SEMICOLON, ";", t.start, t.end, locConverter),
			end: t.end
		};
	}
	/** @type {Frame} */
	...
	pushTokenAsNode(t, input, locConverter, [root], () => {});
	const node = root.values[0];
