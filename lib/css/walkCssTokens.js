 ...
 * @property {function(string, number, number, number, number): number=} supports
 * @property {function(string, number, number, number, number): number=} layer
...
const CC_LOWER_S = "s".charCodeAt(0);
...
/** @type {CharHandler} */
const consumePotentialLayer = (input, pos, callbacks) => {
	const start = pos;
	pos = _consumeIdentifier(input, pos);
	if (pos === start + 5 && input.slice(start, pos + 1) === "layer(") {
		pos++;
		let cc = input.charCodeAt(pos);
		while (_isWhiteSpace(cc)) {
			pos++;
		...
		for (;;) {
			...
			if (cc === CC_RIGHT_PARENTHESIS) {
				...
				pos++;
				if (callbacks.layer !== undefined) {
					return callbacks.layer(input, start, pos, contentStart, contentEnd);
				}
				return pos;
			} else if (cc === CC_LEFT_PARENTHESIS) {
				return pos;
...
/** @type {CharHandler} */
const consumePotentialSupports = (input, pos, callbacks) => {
	const start = pos;
	let level = 0;
	pos = _consumeIdentifier(input, pos);
	if (pos === start + 8 && input.slice(start, pos + 1) === "supports(") {
		pos++;
		let cc = input.charCodeAt(pos);
		while (_isWhiteSpace(cc)) {
			pos++;
...
const CHAR_MAP = Array.from({ length: 0x80 }, (_, cc) => {
	...
	switch (cc) {
		...
		case CC_AT_SIGN:
			return consumeAt;
		case CC_LOWER_L:
			return consumePotentialLayer;
		case CC_LOWER_S:
			return consumePotentialSupports;
		case CC_LOWER_U:
		case CC_UPPER_U:
