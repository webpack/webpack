 ...
 * the opening character. `value` is the component values inside.
 *
 * `consumeABlock` (§5.4.4) produces a specialized `{`-block whose `value` holds
 * the parsed block contents (§5.4.5): a list whose items are either a `Rule`
 * (at-rule / qualified-rule) or a sub-list of consecutive `Declaration`s (the
 * spec's "list of declarations" grouping). That specialized shape is the
 * `BlockBody` typedef below; use it via cast when iterating an at-rule /
 * qualified-rule's `.block`.
 * @typedef {Node & { token: SimpleBlockToken, value: ComponentValue[] }} SimpleBlock
 */

/**
 * The `{`-`SimpleBlock` that holds a rule body, produced by `consumeABlock`.
 * `value` holds the parsed block contents (§5.4.5) — see `SimpleBlock`.
 * @typedef {Node & { token: "{", value: (Declaration[] | Rule)[] }} BlockBody
 */
 ...
 * @typedef {Node & { name: string, nameRange: [number, number], prelude: ComponentValue[], block: BlockBody | null }} AtRule
 */
 ...
 * @typedef {Node & { prelude: ComponentValue[], block: BlockBody | null }} QualifiedRule
 */
...
/**
 * Consume a block, CSS Syntax Level 3 [§5.4.4](https://drafts.csswg.org/css-syntax/#consume-block) — the next token must be `{`; discards it, consumes the block's contents (§5.4.5), discards the closing `}`, and returns a `BlockBody` (a `{`-`SimpleBlock` whose `value` holds the parsed block contents). Used for at-rule / qualified-rule bodies.
 * @param {TokenStream} ts token stream
 * @returns {BlockBody | undefined} the block (its `value` is the parsed contents), or `undefined` if the precondition assert failed
 */
			...
			switch (node.type) {
				...
				case T_QUALIFIED_RULE: {
					...
					if (r.block && recurseBlocks) {
						...
						// Block contents (§5.4.5) are either a single Rule node or a Declaration[] sub-list; walk both as flat siblings.
						for (const item of r.block.value) {
							if (Array.isArray(item)) {
								for (const d of item) walkRule(d, r.block);
							} else {
								walkRule(item, r.block);
							}
						}
