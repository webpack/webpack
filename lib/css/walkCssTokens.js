 ...
 * @param {number} pos consume start position
 * @param {CssTokenCallbacks} callbacks callbacks
 * @param {number} direction consume direction -1: reverse, 1: forward
 * @returns {number} position after consume
 */
const consumeComments = (input, pos, callbacks, direction = 1) => {
	...
	if (
		...
		input.charCodeAt(pos + direction) === CC_ASTERISK
	) {
		const start = pos;
		pos += direction;
		while (direction > 0 ? pos < input.length : pos > -1) {
			if (
				input.charCodeAt(pos) === CC_ASTERISK &&
				input.charCodeAt(pos + direction) === CC_SOLIDUS
			) {
				if (callbacks.comment) {
					pos = callbacks.comment(input, start, pos + 2 * direction);
				} else {
					pos += 2 * direction;
				}
				break;
...
/**
 * Do not pass commentCallbacks when the range has already been walked.
 * This will cause comments to be collected multiple times.
 * @param {string} input input
 * @param {number} pos position
 * @param {CssTokenCallbacks} callbacks callbacks
 * @returns {number} position after comments
 */
module.exports.eatComments = (input, pos, callbacks) => {
	for (;;) {
 ...
 */
module.exports.eatWhitespaceAndComments = (
	input,
	pos,
	callbacks,
	direction = 1
	...
	for (;;) {
		const originalPos = pos;
		pos = consumeComments(input, pos, callbacks, direction);
		while (_isWhiteSpace(input.charCodeAt(pos))) {
			pos += direction;
		}
		if (originalPos === pos) {
