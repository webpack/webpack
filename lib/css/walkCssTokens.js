...
class FunctionNode extends Node {
	 ...
	 * @param {ComponentValue[]} value component values inside `(...)`
...
class SimpleBlock extends Node {
	 ...
	 * @param {(ComponentValue | Declaration | AtRule | QualifiedRule)[]} value component values inside the block (or parsed contents for a rule body block)
...
class Declaration extends Node {
	 ...
	 * @param {[number, number]} nameRange name [start, end)
	 * @param {ComponentValue[]} value list of component values
...
class AtRule extends Node {
	 ...
	 * @param {ComponentValue[]} prelude list of component values up to the at-rule's `;`, block, or enclosing `}`
...
class QualifiedRule extends Node {
	/**
	 * @param {ComponentValue[]} prelude list of component values before the `{`
 ...
 * @param {LocConverter} locConverter shared loc converter
 * @returns {Token | undefined} the leaf token node, or `undefined` to drop the token
 */
...
const parseACommaSeparatedListOfComponentValues = (
	...
	const values = consumeAListOfComponentValues(ts);
	/** @type {ComponentValue[][]} */
	const groups = [];
	/** @type {ComponentValue[]} */
	let current = [];
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	for (;;) {
		...
		if (t.type === "semicolon" || t.type === "EOF") {
			// `range[1]` points AT the terminator so callers can detect a
...
const consumeABlock = (ts) => {
	...
	const start = ts.next().start;
	ts.discard();
	const values = consumeABlocksContents(ts);
	// Discard the closing `}` (the `<eof-token>` if unterminated — a parse error).
	const close = ts.next();
	let end;
...
const consumeADeclaration = (ts, nested = false) => {
	...
	// it when `nested`. A top-level `{}` block is consumed into the value. The
	// declaration ends at that terminator — the next token after the list.
	const values = consumeAListOfComponentValues(ts, "semicolon", nested);
...
const consumeAListOfComponentValues = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		const t = ts.next();
		if (t.type === "EOF" || t.type === stopToken) break;
		if (t.type === "rightCurlyBracket") {
			if (nested) break;
...
/**
 * Consume a component value, CSS Syntax Level 3 [§5.4.8](https://drafts.csswg.org/css-syntax/#consume-component-value) — consumes exactly the next value (simple block, function, or single token) at the current position, with no leading-whitespace handling (the `parse*` caller does that). Its byte end is the node's `range[1]`.
 * @param {TokenStream} ts token stream
 * @returns {ComponentValue | undefined} the consumed component value, or `undefined` at EOF / on a dropped stray token
 */
