...
/**
 * A CSS Syntax §5.3 top-level parse entry point used to drive a walk:
 * `parseAStylesheet` (a full stylesheet) or `parseABlocksContents` (a block's
 * contents, e.g. an HTML `style` attribute). Receives the already-built `ts`
 * (so no re-tokenization) and returns the nodes to walk.
 * @typedef {(ts: TokenStream) => (Stylesheet | { decls: Declaration[], rules: Rule[] })} TopLevelParser
 */
 ...
 * @property {TopLevelParser} parse top-level parse entry point driving the walk: `parseAStylesheet` for a full stylesheet, or `parseABlocksContents` for a block's contents (e.g. an HTML `style` attribute)
 */
...
const grammar = (input, ctx) => {
	...
	// The caller picks the top-level parse entry point (`parseAStylesheet` /
	// `parseABlocksContents`); it reuses this `ts` (no re-tokenization) and
	// returns the nodes to walk — a stylesheet's `rules`, or a block's
	// `{ decls, rules }`.
	const ts = new TokenStream(input, 0, locConverter, comment);
	const parsed = ctx.parse(ts);
	if ("decls" in parsed) {
		const { decls, rules } = parsed;
		for (let i = 0; i < decls.length; i++) walkRule(decls[i], null);
		for (let i = 0; i < rules.length; i++) walkRule(rules[i], null);
	} else {
		const { rules } = parsed;
		for (let i = 0; i < rules.length; i++) walkRule(rules[i], null);
	}
...
class SourceProcessor {
	...
	/**
	 * Run the grammar over `input`, firing visitors in source order.
	 * @param {string} input source text
	 * @param {{ parse: TopLevelParser, locConverter?: LocConverter, comment?: (input: string, start: number, end: number) => number, recurseBlocks?: boolean }} ctx the top-level `parse` entry point (`parseAStylesheet` / `parseABlocksContents`), plus an optional shared `locConverter`, a `comment` callback, and `recurseBlocks: false` to stop at top-level rules
	 */
	process(input, ctx) {
