...
/**
 * Whether every target browser is at or past the versions in one such table.
 * @param {Map<string, number> | undefined} since when each browser first read it
 * @returns {boolean} true when the selection reads it
 */
const _readsAll = (since) => {
	...
	for (let i = 0; i < prelude.length; i++) {
		...
		const since =
			...
			(spelling.charCodeAt(1) === CC_COLON
				? undefined
				: SELECTOR_SUPPORTED_FROM.get(`:${spelling}`));
		// A pseudo the table does not name is one no engine is known to parse — a
		// vendor spelling, a CSS-modules one, or simply newer than the data — and
		// no selection makes it safe, so this precedes the target check.
		if (since === undefined || !_readsAll(since)) return false;
		i = end - 1;
	}
	return true;
};
