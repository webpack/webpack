...
/** @typedef {{ taken: TakenPiece, span: RuleSpan, entry: RuleEntry, prelude: number, grown: boolean }} PriorTopLevel the top-level rule last written out, the piece it stands in, how far into it its block starts, and whether a later rule wrote its selectors into it */
...
const _priorOf = (taken, entry) => {
	...
	return { taken, span, entry, prelude: entry.prelude, grown: false };
};
...
const _joinsPrior = (writer, prior, held) => {
	...
	if (
		...
		_rangeEquals(text, span.at, span.at + prior.prelude, entry.text, 0, entry.prelude)
	) {
		return true;
	}
	if (!_lendSelectorsTo(writer, prior.taken, span, prior.prelude, entry)) {
		return false;
	}
