 ...
 * @property {boolean=} minimize print the safely-minified serialization (collapsed whitespace, dropped redundant separators) as `process` walks, and return it (default false = walk only, return `""`)
 */
 ...
 */
const _exitNode = (node, parent, b, writer) => {
 ...
 * @param {PrintContext | undefined} writer print context when printing, else undefined
 */
const _walkValue = (node, parent, writer) => {
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		for (let i = vs; i < ve; i++) {
			_walkValue(_nodeRef(_soaFlat[i]), node, writer);
		}
	}
	_exitNode(node, parent, b, writer);
};
 ...
 * @param {PrintContext | undefined} writer print context when printing, else undefined
 */
const _walkRule = (node, parent, writer) => {
	const i0 = _nodeIndex(node);
			...
			if (_recurseBlocks) {
				...
				if (decls) {
					for (let i = 0; i < decls.length; i++) {
						_walkRule(decls[i], node, writer);
					}
				}
				const ch = _soaChildRuleLists[i0];
				if (ch) {
					for (let i = 0; i < ch.length; i++) _walkRule(ch[i], node, writer);
				}
			}
...
const _walkTopLevel = (node, writer) => {
	_walkRule(node, null, writer);
	// The walk fired each node's printer into the context; take this finished
	// top-level node's text (top-level nodes concatenate — no separator between).
	if (writer !== undefined) {
		// Source map: anchor the node's output start to its source position (the
		// loc converter is 1-based line, 0-based column; source maps are 0-based).
		if (writer.tracksSourceMap()) {
			const start = A.loc(node).start;
			writer.mark(start.line - 1, start.column);
		}
		writer.take(node);
	}
