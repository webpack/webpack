 ...
 * @property {(boolean | "all")=} comments drop a comment no tool reads; `"all"` drops the banners and annotations too
 ...
 * @property {(string | RegExp)[]=} preserveComments patterns naming comments to keep, on top of the ones minifying always keeps (a `/*!` banner, `@license` / `@preserve`, and the source-map pragmas)
 */
...
const _isKeptComment = (src, start, end) => {
	const level = _transforms.comments;
	// Off, every comment is kept: the option says which of them minifying may
	// drop, and none is the answer here.
	if (level === false) return true;
	const marker = src.charCodeAt(start + 2);
	// A `/*#` pragma is a link to a source map rather than a comment, so it is
	// not one of the ones `"all"` drops either.
	if (marker === CC_NUMBER_SIGN) return true;
	if (
		level !== "all" &&
	...
	// An author-supplied pattern outranks the rule above: a marker a build step
	// of the author's own reads is inert to an engine and load-bearing anyway.
	if (_preserveComments !== undefined) {
		const text = src.slice(start + 2, end - 2);
		for (let i = 0; i < _preserveComments.length; i++) {
			if (_preserveComments[i].test(text)) return true;
		}
	}
	return false;
};
 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties" | "renderEmbeddedSource" | "transforms" | "preserveComments">} CssPrintOptions
 */
...
const _minifyUrlFunction = (fn, inner) => {
	...
	const decoded =
		_transforms.escapes || _renderEmbeddedSource !== undefined
			? _decodePercentEscapes(written, false, mark)
			: written;
	// A `data:` payload the renderer rewrote: serialized afresh, since what it
	// handed back is no longer what the quotes were written around.
	const rendered = _renderDataUrl(decoded);
	if (rendered !== null) return `${fn}(${_serializeUrl(rendered, mark)})`;
	// Two rewrites in one place, each its own switch: writing a percent-escape
	// as the byte it names, and taking the quotes off what is a url-token
	// without them.
	const body = _transforms.escapes ? decoded : written;
	if (!_transforms.quotes) {
	...
	switch (path.type()) {
		...
		case T_URL: {
			...
			if (_renderEmbeddedSource !== undefined) {
				...
				if (renderedUrl !== null) {
					return `${head}${_serializeUrl(renderedUrl, '"')})`;
				}
			}
...
// Patterns naming comments to keep whatever the banner rule says (see
// `CssProcessOptions`), or undefined when the author named none.
