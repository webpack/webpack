 ...
 * @property {((token: MutableToken) => void)=} token invoked once per token with the shared, reused token object
 */

/**
 * Push tokenizer over `input` from `pos`: reads every token via the `readToken`
 * pull primitive (one reused token, zero per-token allocation) and invokes
 * `callbacks.token` for each. Mirrors the HTML `tokenize` interface; CSS keeps
 * `readToken` as its pull primitive — the parser's backtracking `TokenStream`
 * drives that directly for lookahead/rewind, which a push walk can't express.
 * @param {string} input input
 * @param {number=} pos start byte offset (default `0`)
 * @param {CssTokenCallbacks=} callbacks callbacks
 * @returns {number} final byte position
 */
const tokenize = (input, pos = 0, callbacks = {}) => {
	const out = createToken();
	const onToken = callbacks.token;
	let p = pos;
	for (;;) {
		if (readToken(input, p, out) === undefined) return p;
		p = out.end;
		if (onToken !== undefined) onToken(out);
	}
};
