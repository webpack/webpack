...
/**
 * Drop every rule an identical later one makes dead. The later copy restates
 * each declaration at the same specificity under the same conditions, so
 * nothing standing between them survives it and the earlier copy is read for
 * nothing. `_supersededItems` already does this among one block's own children;
 * this reaches the copies two blocks apart, which the printer cannot, having
 * emitted the earlier one before the later one was even read.
 *
 * Run over a printed sheet rather than during the walk, so a rule is weighed
 * against every other however far away it stands.
 * @param {string} css a minified stylesheet
 * @returns {string} it, without the rules a later copy makes dead
 */
const _dropDeadRulesOnce = (css) => {
	if (css.length === 0) return css;
	/** @type {{ start: number, end: number, signature: string }[]} */
	const rules = [];
	/** @type {string[]} */
	const chain = [];
	// A rule's conditions are its ancestors' preludes: the same selector and
	// block under a different `@media` is a different rule.
	const prelude = (/** @type {CssPath} */ path) => {
		const start = path.start();
		const open = css.indexOf("{", start);
		const end = path.end();
		return open === -1 || open > end
			? css.slice(start, end)
			: css.slice(start, open);
	};
	/** @type {{ start: number, end: number, open: number }[]} */
	const droppable = [];
	/** @type {EXPECTED_ANY} */
	const visitors = {
		[NodeType.AtRule]: {
			enter: (/** @type {CssPath} */ path) => {
				// Which empty at-rules may go is the printer's own answer, read from
				// the table it reads: `@keyframes`, `@page` and `@scope` each mean
				// something while empty and stay.
				const start = path.start();
				const end = path.end();
				if (
					DROPPABLE_WHEN_EMPTY_AT_RULES.has(path.name().toLowerCase()) &&
					css.charCodeAt(end - 1) === CC_RIGHT_CURLY
				) {
					const open = css.indexOf("{", start);
					if (open !== -1 && open < end) droppable.push({ start, end, open });
				}
				chain.push(prelude(path));
			},
			exit: () => chain.pop()
		},
		[NodeType.QualifiedRule]: {
			enter: (/** @type {CssPath} */ path) => {
				...
				rules.push({
					...
					signature: `${chain.join(" ")}${css.slice(start, end)}`
				});
				chain.push(prelude(path));
			},
			exit: () => chain.pop()
	...
	try {
		...
		// Unreadable input is carried through whole, as everything else here is.
		return css;
	}
	/** @type {Map<string, number>} */
	const last = new Map();
	for (let i = 0; i < rules.length; i++) last.set(rules[i].signature, i);
	/** @type {{ start: number, end: number }[]} */
	const dead = [];
	for (let i = 0; i < rules.length; i++) {
		if (last.get(rules[i].signature) !== i) dead.push(rules[i]);
	}
	if (dead.length === 0) return css;
	// A block whose whole body the cuts take is dead with it, and so is the block
	// around that. Innermost first, so an outer one sees its children gone.
	dead.sort((one, other) => one.start - other.start);
	droppable.sort((one, other) => other.open - one.open);
	for (const block of droppable) {
		let at = block.open + 1;
		for (const cut of dead) {
			if (cut.start < at) continue;
			if (cut.start > at) break;
			at = cut.end;
		}
		if (at === block.end - 1) {
			dead.push({ start: block.start, end: block.end });
			dead.sort((one, other) => one.start - other.start);
		}
	}
	// A dead rule nested in another is already gone with it, so only the
	// outermost ranges are cut — they arrive in source order, nesting included.
	let out = "";
	let at = 0;
	let reach = -1;
 ...
 * @returns {string} it, without the rules a later copy makes dead
 */
const dropDeadRules = (css) =>
	// A cut can empty the block around it, which the pass that made the cut had
	// already walked past — so it is run until it finds nothing more. Each round
	// strictly shortens the sheet, so this ends.
	_dropDeadRulesOnce(css);
...
module.exports.dropDeadRules = dropDeadRules;
