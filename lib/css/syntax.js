...
const _walkValue = (node, parent, index, writer) => {
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		if (ty === T_FUNCTION) {
			...
			if (!_inSteppedFunction && STEPPED_FUNCTIONS.has(name)) {
				enteredStepped = true;
				_inSteppedFunction = true;
			}
		}
		for (let i = vs; i < ve; i++) {
	...
	if (enteredStepped) _inSteppedFunction = false;
};
...
/**
 * Whether emitting `next` directly after `last` would re-tokenize as one token.
 * Two printed siblings normally concatenate to exactly their source, but not when
 * a dropped comment was all that separated them, or when one was rewritten
 * (`1.0.5` is two numbers, printed as `1` and `.5`) — then the junction can fuse
 * and change the declaration. The test is deliberately exact rather than
 * conservative: a space
 * inserted where none is needed would turn a compound selector (`.a.b`) into a
 * descendant one (`.a .b`), so over-separating is as unsafe as under-separating.
 * @param {number} last last emitted code point
 * @param {number} next first code point of the fragment about to be emitted
 * @returns {boolean} true when a separator has to be inserted between them
 */
const _wouldFuseTokens = (last, next) => {
	...
	if (_isIdentLike(last)) {
		...
		// `1` + `.5` reads back as the single number `1.5`.
		if (_isDigit(last) && next === CC_FULL_STOP) return true;
...
const _join = (parts, beautify, trim = _TRIM_NOTHING) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (pending) {
			...
			pending = false;
		} else if (out.length !== 0 && _wouldFuseTokens(last, first)) {
...
const _normalizeNumericToken = (text) => {
	...
	if (_inSupportsPrelude || _inSteppedFunction) return num + unit;
...
/**
 * Add two doubles, or decline. The result is only used when it carries no
 * rounding of its own: everything here has to stay the value an engine would
 * have computed, and a folded expression is no longer there to be recomputed.
 * @param {number} a one term
 * @param {number} b the other
 * @returns {number | null} their exact sum, or `null` when it is not exact
 */
const _exactAdd = (a, b) => {
	const sum = a + b;
...
const _evaluateCalcSum = (tokens, cursor) => {
	...
	for (;;) {
		...
		for (const [key, coefficient] of right) {
			const scaled = _exactMultiply(coefficient, sign);
			...
			const added = _exactAdd(previous, scaled);
	...
	for (;;) {
		...
		if (token.type === "/") {
			...
			for (const [key, coefficient] of sum) {
				const quotient = _exactDivide(coefficient, divisor);
		...
		for (const [key, coefficient] of other) {
			const product = _exactMultiply(coefficient, factor);
...
const _evaluateCalcValue = (tokens, cursor) => {
	...
	const value = _exactMultiply(token.value, scale);
 ...
 */
const _printCalcTerm = (key, coefficient) => {
	if (key === "" || key === "%") {
	...
	for (const [candidate, to] of ABSOLUTE_UNIT_SCALE) {
		...
		const value = _exactDivide(coefficient, to[1]);
 ...
 */
const _exactFloorDivide = (value, step) => {
	let n = Math.floor(value / step);
	if (!Number.isFinite(n)) return null;
	for (let attempt = 0; attempt < 3; attempt++) {
		const at = _exactMultiply(n, step);
		const next = _exactMultiply(n + 1, step);
 ...
 * @param {number} step the step, greater than zero
 * @param {string} strategy one of the grammar's rounding strategies
 * @returns {number | null} the rounded multiple, or `null`
 */
const _roundToStep = (value, step, strategy) => {
	const below = _exactFloorDivide(value, step);
	if (below === null) return null;
	const at = /** @type {number} */ (_exactMultiply(below, step));
	...
	if (strategy === "down") {
		...
		const remainder = _exactAdd(value, -at);
		if (remainder === null) return null;
		const doubled = _exactMultiply(remainder, 2);
		if (doubled === null) return null;
		multiple = doubled >= step ? below + 1 : below;
	}
	return _exactMultiply(multiple, step);
};

/**
 * The coefficients of a list of sums that all reduced to the same one unit, or
 * `null`. A percentage is refused: its basis can be negative (a
 * `background-position` against an image wider than its box), and comparing two
 * of them depends on that sign in a way `calc()`'s arithmetic does not — scaling
 * a percentage is linear, picking the smaller of two is not.
 * @param {Map<string, number>[]} sums the evaluated arguments
 * @returns {[string, number[]] | null} the shared key and the coefficients
 */
const _comparableCoefficients = (sums) => {
	/** @type {string | null} */
	let shared = null;
	/** @type {number[]} */
	const values = [];
 ...
 */
const _exactSquareRoot = (value) => {
	if (!(value >= 0)) return null;
	const root = Math.sqrt(value);
	const back = _exactMultiply(root, root);
 ...
 */
const _exactIntegerPower = (base, exponent) => {
	if (!Number.isInteger(exponent) || Math.abs(exponent) > _POWER_LIMIT) {
		return null;
	}
	let power = 1;
	for (let n = Math.abs(exponent); n > 0; n--) {
		const next = _exactMultiply(power, base);
		if (next === null) return null;
		power = next;
	}
	return exponent < 0 ? _exactDivide(1, power) : power;
};

/**
 * The coefficients of a list of sums that all reduced to a plain `<number>`, the
 * only argument the exponential functions take.
 * @param {Map<string, number>[]} sums the evaluated arguments
 * @returns {number[] | null} the numbers, or `null`
 */
const _numberCoefficients = (sums) => {
	const comparable = _comparableCoefficients(sums);
	return comparable === null || comparable[0] !== "" ? null : comparable[1];
};

/** @typedef {(sums: Map<string, number>[], keyword: string) => Map<string, number> | null} MathReducer */

// Sine, cosine and tangent an eighth turn apart, `null` where the value is not
// one a stylesheet can hold: sine and cosine are irrational on the odd eighths,
// and tangent has its asymptotes on the odd quarters.
const _EIGHTH_SINE = [0, null, 1, null, 0, null, -1, null];
const _EIGHTH_COSINE = [1, null, 0, null, -1, null, 0, null];
const _EIGHTH_TANGENT = [0, 1, null, -1, 0, 1, null, -1];

/**
 * Which eighth turn an angle is, or `null` when it is not a whole one. A plain
 * number is an angle in radians, where only zero lands on one.
 * @param {Map<string, number>[]} sums the evaluated arguments
 * @returns {number | null} the eighth 0..7, or `null`
 */
const _angleEighth = (sums) => {
	const comparable = _comparableCoefficients(sums);
	if (comparable === null) return null;
	const [unit, [angle]] = comparable;
	if (unit === "") return angle === 0 ? 0 : null;
	const quarter = QUARTER_TURN_ANGLE.get(unit);
	if (quarter === undefined) return null;
	// Halving a quarter turn is exact in each unit that spells one: 45, 50 and
	// an eighth, which is a power of two.
	const eighths = _exactDivide(angle, quarter / 2);
	if (eighths === null || !Number.isInteger(eighths)) return null;
	return ((eighths % 8) + 8) % 8;
};

/**
 * Fold one trig function over an angle a whole number of eighth turns from zero.
 * @param {(number | null)[]} table its value at each eighth
 * @returns {MathReducer} the reducer
 */
const _trigReducer = (table) => (sums) => {
	const eighth = _angleEighth(sums);
	if (eighth === null) return null;
	const value = table[eighth];
	return value === null ? null : new Map([["", value]]);
};

// The angle each inverse trig function answers with, as `argument -> degrees`.
// Only the three arguments where the answer is a whole number of degrees; the
// rest are transcendental and stay written out.
const _ARC_SINE = new Map([
	[-1, -90],
	[0, 0],
	[1, 90]
]);
const _ARC_COSINE = new Map([
	[-1, 180],
	[0, 90],
	[1, 0]
]);
const _ARC_TANGENT = new Map([
	[-1, -45],
	[0, 0],
	[1, 45]
]);
...
/**
 * Fold one inverse trig function over a plain number.
 * @param {Map<number, number>} table the arguments it answers for, in degrees
 * @returns {MathReducer} the reducer
 */
const _arcReducer = (table) => (sums) => {
	const values = _numberCoefficients(sums);
	if (values === null) return null;
	const degrees = table.get(values[0]);
	return degrees === undefined ? null : new Map([["deg", degrees]]);
};

/**
 * What each foldable math function does to its evaluated arguments. The set of
 * functions and how many arguments each takes are read off the spec's grammars
 * (`MATH_FUNCTIONS`, `MATH_FUNCTION_ARITY`); what the function *means* is not
 * something any dataset states, so it is written here. A function with no entry
 * is left alone, which is what makes adding one a matter of adding a line.
 * @type {Map<string, MathReducer>}
 */
const _MATH_REDUCERS = new Map([
	[
		"min",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			return new Map([[comparable[0], Math.min(...comparable[1])]]);
		}
	],
	[
		"max",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			return new Map([[comparable[0], Math.max(...comparable[1])]]);
		}
	],
	[
		"clamp",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			const [key, [lower, value, upper]] = comparable;
			// CSS Values 4 §10.4: the lower bound wins a contradictory pair.
			return new Map([[key, Math.max(lower, Math.min(value, upper))]]);
		}
	],
	[
		"abs",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			return new Map([[comparable[0], Math.abs(comparable[1][0])]]);
		}
	],
	[
		"sign",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			// The one reducer that changes the unit: a sign is a `<number>`. Every
			// unit reaching here scales by a positive factor, so the sign of the
			// coefficient is the sign of the value.
			return new Map([["", Math.sign(comparable[1][0])]]);
		}
	],
	[
		"hypot",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			const [key, values] = comparable;
			let total = 0;
			for (const value of values) {
				const square = _exactMultiply(value, value);
				if (square === null) return null;
				const sum = _exactAdd(total, square);
				if (sum === null) return null;
				total = sum;
			}
			const root = _exactSquareRoot(total);
			return root === null ? null : new Map([[key, root]]);
		}
	],
	[
		"round",
		(sums, strategy) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			const [key, [value, step]] = comparable;
			// A step of zero is NaN per the spec, and engines do not agree on what
			// that renders as; a negative one is left alone rather than reasoned
			// about. Both keep what was written.
			if (!(step > 0)) return null;
			const rounded = _roundToStep(value, step, strategy);
			return rounded === null ? null : new Map([[key, rounded]]);
		}
	],
	[
		"mod",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			const [key, [value, divisor]] = comparable;
			if (divisor === 0) return null;
			const remainder = value % divisor;
			// A zero remainder is the boundary these two share with `round()`, and
			// engines do not agree on it: headless Chromium reads `mod(10px,-2px)`
			// and `mod(-9px,3px)` as the divisor where both are zero.
			if (remainder === 0) return null;
			// `mod()` takes the divisor's sign, so a remainder on the other side of
			// zero is brought back across it.
			if (remainder < 0 === divisor < 0) return new Map([[key, remainder]]);
			const shifted = _exactAdd(remainder, divisor);
			return shifted === null ? null : new Map([[key, shifted]]);
		}
	],
	[
		"rem",
		(sums) => {
			...
			if (divisor === 0) return null;
			const remainder = value % divisor;
			// The same boundary as `mod()` above.
			if (remainder === 0) return null;
			// `rem()` takes the dividend's sign, which is what `%` already does.
			return new Map([[key, remainder]]);
		}
	],
	[
		"sqrt",
		(sums) => {
			const values = _numberCoefficients(sums);
			if (values === null) return null;
			const root = _exactSquareRoot(values[0]);
			return root === null ? null : new Map([["", root]]);
		}
	],
	[
		"pow",
		(sums) => {
			const values = _numberCoefficients(sums);
			if (values === null) return null;
			const power = _exactIntegerPower(values[0], values[1]);
			return power === null ? null : new Map([["", power]]);
		}
	],
	[
		"log",
		(sums) => {
			const values = _numberCoefficients(sums);
			if (values === null) return null;
			// A logarithm is transcendental except where it lands on a whole power of
			// its base, so the candidate is raised back and only an exact match is
			// taken. The natural logarithm's base is not a double at all, which
			// leaves only `log(1)`.
			if (values.length === 1) {
				return values[0] === 1 ? new Map([["", 0]]) : null;
			}
			const [value, base] = values;
			const exponent = Math.round(Math.log(value) / Math.log(base));
			const back = _exactIntegerPower(base, exponent);
			return back === null || back !== value ? null : new Map([["", exponent]]);
		}
	],
	[
		"exp",
		(sums) => {
			const values = _numberCoefficients(sums);
			if (values === null) return null;
			// `e` is not a double, so every other power of it is a number this
			// cannot write down and an engine's math library rounds its own way.
			return values[0] === 0 ? new Map([["", 1]]) : null;
		}
	],
	["sin", _trigReducer(_EIGHTH_SINE)],
	["cos", _trigReducer(_EIGHTH_COSINE)],
	["tan", _trigReducer(_EIGHTH_TANGENT)],
	["asin", _arcReducer(_ARC_SINE)],
	["acos", _arcReducer(_ARC_COSINE)],
	["atan", _arcReducer(_ARC_TANGENT)],
	[
		"atan2",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			if (comparable === null) return null;
			const [y, x] = comparable[1];
			// The eight directions the answer is a whole number of degrees in. Both
			// zero is refused: the spec leaves it to the engine.
			if (y === 0 && x === 0) return null;
			let degrees;
			if (y === 0) degrees = x > 0 ? 0 : 180;
			else if (x === 0) degrees = y > 0 ? 90 : -90;
			else if (Math.abs(y) !== Math.abs(x)) return null;
			else if (x > 0) degrees = y > 0 ? 45 : -45;
			else degrees = y > 0 ? 135 : -135;
			return new Map([["deg", degrees]]);
		}
	]
]);
...
const _foldMathFunction = (fn, inner) => {
	...
	if (fn === "calc") {
		...
		const reduce = _MATH_REDUCERS.get(fn);
		if (reduce === undefined) return null;
		result = reduce(sums, keyword);
		if (result === null) return null;
	}
	return _printCalcSum(result);
};
...
const _minifyUrlFunction = (name, fn, inner) => {
	...
	return _UNQUOTABLE_URL_RE.test(body) ? null : `${name}(${body})`;
};
...
const _collapseShorthand = (path, property, children, writer) => {
	...
	return property === "flex"
		? _collapseFlexShorthand(path, children, writer)
		: null;
};
 ...
 */
const _mergeBoxLonghands = (path, decls, writer) => {
	/** @type {Map<string, number>} */
	const at = new Map();
	/** @type {Set<string>} */
	...
	let out = null;
	for (const [shorthand, longhands] of BOX_LONGHANDS) {
		...
		const prefix = /** @type {string} */ (BOX_FAMILY_PREFIX.get(shorthand));
		let blocked = false;
		for (let i = ordered[0] + 1; i < ordered[3] && !blocked; i++) {
		...
		out.set(
			decls[ordered[0]],
			`${shorthand}:${box.join(" ")}${important ? "!important" : ""};`
		);
		for (let i = 1; i < 4; i++) out.set(decls[ordered[i]], "");
	}
	return out;
};
...
// needs its unit, and `0%` is a percentage, a different type. No dataset states
// the list, so it is the spec's, spelled out.
// cspell:ignore rlh cqmin cqmax vmin vmax dvmin dvmax lvmin lvmax svmin svmax whib
const _ZERO_LENGTH_RE =
	/^0(?:px|em|rem|ex|ch|cap|ic|lh|rlh|v[wh]|vmin|vmax|v[ib]|[sld]v[wh]|[sld]vmin|[sld]vmax|[sld]v[ib]|cq[whib]|cqmin|cqmax|cm|mm|in|pt|pc|q)$/i;
 ...
 * expression would stop parsing.
 *
 * TODO reach inside a function argument too (`translateX(0px)` -> `translateX(0)`),
 * which needs a per-function grammar table: `scale()` takes a `<number>`, so
 * `scale(0px)` is a declaration the engine drops and `scale(0)` is not — the
 * rewrite would revive it.
 ...
 * a negative is clamped inside `calc()` and a parse error outside it
 * (`width:calc(-5px)` renders at `0`, `width:-5px` at `auto`), and a fraction is
 * rounded where the grammar wants an `<integer>` (`z-index:calc(1.5)` computes
 * to `2`, `z-index:1.5` is dropped). A unit or a percentage settles the second
 * on its own, since no `<integer>` carries either. A unitless zero is a third:
 * it is the one number a length accepts, so `width:calc(0)` is dropped and
 * `width:0` is not.
 * TODO take the parentheses off a negative too where the property accepts one
 * (`margin-left:calc(-5px)` is `-5px`). The grammars annotate their ranges
 * (`<length [0,∞]>`), which `tooling/generate-css-data.js` already parses into
 * `TypeNode.min`, so the set is derivable the way `INTEGER_PROPERTIES` is.
...
const _unwrapCalc = (fragment, property) => {
	...
	if (number.charCodeAt(0) === 0x2d) return fragment;
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const easing = path.inValue() ? _minifyEasingFunction(fn, inner) : null;
			...
			const url = _minifyUrlFunction(name, fn, inner);
			return url !== null ? url : `${name}(${inner})`;
		}
		case T_URL: {
			const raw = path.source();
		...
		case T_STRING: {
			const raw = path.source();
			return minify ? _minifyString(raw) : raw;
		}
		case T_IDENT: {
			const raw = path.source();
		...
		case T_DECLARATION: {
			...
			if (children.length !== 0) {
				...
				// TODO offer the custom-property rewrites behind an option: the
				// per-token ones are exact CSS (`--r:0.5rem` → `.5rem`, worth ~0.3% on
				// utility-heavy sheets), but a custom property is the one declaration
				// whose authored text `getComputedStyle().getPropertyValue()` hands
				// back, and no other minifier rewrites it by default either.
				if (custom || property === "unicode-range") {
		...
		case T_QUALIFIED_RULE: {
			...
			// A box family's four longhands are their shorthand, so they print as it.
			// Only with a child rule out of the way: one between them would be
			// stepped over by the merge, which the adjacency check cannot see.
			const merged =
				minify && decls.length >= 4 && (rules === null || rules.length === 0)
					? _mergeBoxLonghands(path, decls, writer)
					: null;
			...
			for (const item of items) {
				const replacement = merged === null ? undefined : merged.get(item);
