	...
	if (!skip) {
		if (ty === T_AT_RULE || ty === T_QUALIFIED_RULE) {
			...
			if (ty === T_AT_RULE) {
				...
				if (equalsLowerCase(atName, "property")) _inPropertyRule = true;
				else if (equalsLowerCase(atName, "function")) _inFunctionRule = true;
...
// The `@property` / `@function` state this frame was entered under. A streamed
// block outlives the call that set it, so it is restored when the frame closes.
...
// The tokenizer met a token the source never closed, or one it had to give up on
// — a bad string or a bad url. Each is a shape the printer has to spell back with
// a terminator of its own, so the output is checked against the tree rather than
// trusted. Nothing else in CSS reaches for the check.
let _errorToken = false;
...
/** @type {{ node: Node, offset: number, line: number | undefined, column: number | undefined, entry: RuleEntry, owned: boolean, own: BlockSpans | undefined } | null} */
 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties">} CssPrintOptions
 */
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_BAD_URL: {
			...
			// §4.3.6 runs the remnants to `)` or EOF, so one that stopped at EOF
			// swallows whatever the printer writes after it — the `;` closing the
			// declaration and the `}` closing the rule. Write the `)` back, as a url
			// token closed at EOF does, and it stops where it stopped for the
			// tokenizer.
			return _isUnterminatedUrl(raw) ? _terminate(raw, ")", "\uFFFD") : raw;
		}
		case T_NUMBER:
		case T_PERCENTAGE:
		case T_DIMENSION: {
...
/**
 * One node type's contribution to {@link _cssomDigest}. A factory rather than a
 * literal in the loop that fills the map: each closure reads the collector, and
 * one built in a loop would carry the loop's own bindings.
 * @param {number} type the `NodeType`
 * @param {boolean} named whether it has a name of its own
 * @param {boolean} leaf whether its value is its own rather than its children's
 * @param {boolean} numeric whether it carries a numeric type flag
 * @returns {EXPECTED_OBJECT} the visitor pair
 */
const _digestVisitor = (type, named, leaf, numeric) => ({
	/** @param {CssPath} path the accessor */
	enter(path) {
		let record = `${_digestDepth}\u0001${type}`;
		if (named) record += `\u0001${path.unescapedName()}`;
		if (leaf) record += `\u0001${path.unescaped()}`;
		if (numeric) record += `\u0001${path.typeFlag()}`;
		if (type === NodeType.Declaration) record += `\u0001${path.important()}`;
		_digestOut.push(record);
		_digestDepth++;
	},
	exit() {
		_digestDepth--;
...
/** @type {VisitorMap | null} built once, on the first check */
let _digestVisitors = null;
/** @type {string[]} the digest being collected */
let _digestOut = [];
let _digestDepth = 0;

/**
 * The stylesheet as a string, for the one check the printer cannot make from the
 * tree alone: whether what it wrote parses back to what it wrote from.
 *
 * Whitespace and comments are left out — they are not in the CSSOM — and so is
 * the text of a bad string or a bad url, which belongs to a declaration the
 * CSSOM drops. A container's own `value` is its raw source span, which moves
 * with the whitespace inside it, so only leaves report one.
 * @param {string} source css
 * @returns {string} a rendering equal for stylesheets that hold the same thing
 */
const _cssomDigest = (source) => {
	if (_digestVisitors === null) {
		/** @type {VisitorMap} */
		const map = {};
		for (const type of Object.values(NodeType)) {
			...
			const leaf =
				...
				type !== NodeType.BadString &&
				type !== NodeType.BadUrl;
			map[type] = _digestVisitor(type, named, leaf, numeric);
		}
		_digestVisitors = map;
	}
	/** @type {string[]} */
	const out = [];
	const outer = _digestOut;
	const outerDepth = _digestDepth;
	_digestOut = out;
	_digestDepth = 0;
	try {
		new SourceProcessor()
			.use(/** @type {VisitorMap} */ (_digestVisitors))
			.process(source);
	} finally {
		_digestOut = outer;
		_digestDepth = outerDepth;
	}
	return out.join("\n");
};
...
class SourceProcessor extends GenericSourceProcessor {
	...
	/**
	 * {@link GenericSourceProcessor#process}, plus the check a stylesheet the
	 * tokenizer had to give up somewhere in earns: a bad string, a bad url or a
	 * string the source never closed is a shape the printer spells back with a
	 * terminator of its own, and one of those swallowing what follows it is not
	 * something the tree can be read for. Print it, read the output back, and hand
	 * back the source when the two do not hold the same thing — the source built
	 * that tree, so it always agrees.
	 *
	 * Only the modes that preserve: minifying rewrites the CSSOM on purpose —
	 * dropping a declaration nothing can use, folding a value — so there is
	 * nothing here to compare it against.
	 * @param {string} input source
	 * @param {EXPECTED_OBJECT=} options process options
	 * @returns {EXPECTED_ANY} the processed source
	 */
	process(input, options) {
		const result = super.process(input, options);
		if (
			result === undefined ||
			!_errorToken ||
			(options !== undefined &&
		...
		const printed = /** @type {{ code: string }} */ (result).code;
		if (printed === input || _cssomDigest(input) === _cssomDigest(printed)) {
			return result;
		}
		return { .../** @type {EXPECTED_OBJECT} */ (result), code: input };
	}
}
...
const A = {
	...
	unescaped(n = _currentNode) {
		...
		// §4.3.5 returns the string token at EOF as well, and that one has no closing
		// quote to drop — nor does a final quote the source escaped, which an odd
