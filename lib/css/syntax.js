...
/**
 * The engine a vendor spelling belongs to — its prefix, or the whole spelling
 * where it is somehow not prefixed at all, which groups it on its own.
 * @param {string} spelling a vendor spelling
 * @returns {string} the engine's prefix
 */
const _enginePrefix = (spelling) => {
	const found = VENDOR_PREFIX.exec(spelling);
	return found === null ? spelling : found[1];
};
...
const _prefixAtRule = (path, ruleText, prelude, scope, top) => {
	...
	if (base === null || !PREFIXED_AT_RULES.has(base)) return ruleText;
...
const _printedValue = (text, colon) => {
	...
	return toLowerCaseIfNeeded(text.slice(colon + 1, end));
};
...
const _prefixQualifiedRule = (ruleText, parts, soft, body, scope, top) => {
	...
	for (const selector of selectors) {
		...
		// One engine for the whole list: a list mixing a vendor-spelled pseudo with
		// a plainly spelled one, or with a second engine's, is neither this rule's
		// twin nor a copy of it.
		if (found !== undefined) {
			const engine = _enginePrefix(found[1]);
			if (listPrefix !== undefined && listPrefix !== engine) return ruleText;
			listPrefix = engine;
		}
	...
	if (listPrefix !== undefined) {
		// Every pseudo of the list carries the engine's spelling, and each is dead
		// weight on its own.
	...
	if (top) _retractPrefixTwin(scope, signature);
	/** @type {(Set<string> | null)[]} */
	// One copy per engine rather than per spelling: two selectors of a list may
	// want different names of the same engine (`:-webkit-any`,
	// `::-webkit-full-screen`), and that engine reads them in one list.
	...
	for (let i = 0; i < selectors.length; i++) {
		...
		for (const spelling of spellings) {
			byEngine.set(_enginePrefix(spelling), spelling);
		}
		needed.push(byEngine);
		if (engines === null) engines = new Set();
		for (const engine of byEngine.keys()) engines.add(engine);
	}
	if (engines === null) return ruleText;
	let out = "";
	for (const engine of engines) {
		// Skip only an engine the source itself already carries (marked above when
		...
		if (seen.has(`${signature}\0${engine}`)) continue;
		const list = preludeOf((i) => {
			const byEngine = needed[i];
			if (byEngine === null) return null;
			const spelling = byEngine.get(engine);
			return spelling === undefined ? null : spelling + args[i];
		});
		out += `${list}${soft}{${body}}`;
	}
	return out + ruleText;
				...
				for (let i = 0; i < items.length; i++) {
					...
					values.push(
						valueTable === undefined
							? ""
							: _printedValue(texts[i], property.length)
					);
					if (
						valueTable !== undefined ||
				...
				if (prefixable) {
					...
					for (let i = 0; i < properties.length; i++) {
							...
							if (keyword !== undefined) {
								...
								if (spellings !== null) {
									...
									for (const spelling of spellings) {
										...
										add += `${nl + texts[i].slice(0, colon + 1) + spelling + texts[i].slice(colon + 1 + value.length)}`;
									}
								}
							}
						}
						...
						if (prefix !== null) {
							...
							if (needed !== null) {
								for (const spelling of needed) {
									if (!present.has(spelling)) {
										add += nl + spelling + texts[i].slice(property.length);
									}
								}
							}
						}
