...
/**
 * FNV-1a over the head of a range of a string, so a window indexes a rule's
 * prelude and its block without cutting either out.
...
const _flushTopLevel = (writer) => {
	if (_topLevelWindow.held.length === 0) return;
	if (writer === undefined) {
