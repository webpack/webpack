 ...
 * keywords. The origin is recognized by the `from` its arguments open with, so
 * a function webpack knows nothing about guards its origin too.
 * @param {number} i0 the function's node index
...
const _readsColorsAsNumbers = (i0, vs, ve) => {
	...
	const last = _input.charCodeAt(_aux0[i0] - 1) | 0x20;
	if (last === CC_LOWER_T) {
		if (GRADIENT_FUNCTION_RE.test(_gradientName(i0))) return true;
	} else if (
		last === CC_LOWER_X &&
		rangeEqualsLowerCase(_input, _starts[i0], _aux0[i0], "color-mix")
	) {
		return true;
	}
	for (let i = vs; i < ve; i++) {
		...
		return (
			childType === T_IDENT &&
			rangeEqualsLowerCase(_input, _starts[child], _ends[child], "from")
		);
	}
	return false;
};
...
const _minifyColorFunction = (fn, inner, hexAlpha, operand) => {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (i === 3) {
			...
			// The engine keeps the channel as written, so a hex is that color only
			// where it is already a byte — `rgb(50% 50% 50%)` is 127.5, not `gray`.
			// One out of range is not: the engine clamps it to the same byte.
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			// relative color's origin — is left as written wherever converting it is
			// not exact: a space other than sRGB is echoed there rather than
			// computed, so a hex moves the ramp, the mix or the channels the
			// keywords read. The legacy sRGB forms all compute to `rgb(…)` whichever
			// way they are spelled, so those still fold where their channels are
			// already bytes.
...
// gradient stop and a `color-mix()` argument are interpolated, and a relative
// color's origin has its channels read off. Converting one through 8-bit sRGB
// moves the color the engine computes there, not only how it is spelled.
