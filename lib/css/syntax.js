...
const _foldColorMix = (inner, hexAlpha, forFallback) => {
	...
	for (let i = 1; i <= 2; i++) {
		...
		colors.push(color);
		// Achromatic, so a polar space gives it no hue: the engine leaves the hue
		// missing and interpolates the other color's own (CSS Color 4 §12.2), and a
		// hue written in that space is lost on the way through sRGB regardless.
		if (
			space.hue !== -1 &&
			color.linear[0] === color.linear[1] &&
			color.linear[1] === color.linear[2]
		) {
			return null;
		}
	}
