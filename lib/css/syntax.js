 ...
 * @property {boolean=} cssGradientDoublePosition two positions on one gradient color stop (`red 0% 50%`) are available
 ...
 * @property {boolean=} cssPlaceShorthand the `place-content` / `place-items` / `place-self` shorthand properties are available
 * @property {string[]=} browsers the browserslist selection (`["chrome 100", "safari 15"]`), so vendor prefixes are added / dropped for exactly these browsers; absent leaves prefixes untouched
 */
...
// The browserslist selection this parse prefixes for, as `browser -> selected
// versions`; null unless minifying for one, which is what turns prefixing on.
...
// A streamed block's children are written out as they finish, so the merge that
// runs over a collected block's children cannot: this is the same window the top
// level holds, one per streamed depth, so the rules in a block too big to
// collect still join. Built only for a depth that streams (see `JoinWindow`).
/** @type {(JoinWindow | null)[]} */
const _frameWindow = Array.from({ length: _STREAM_MAX_DEPTH }, () => null);
...
/**
 * Write out whatever a streamed block still holds, in order.
 * @param {PrintContext} writer the print context
 * @param {number} d the block's depth
 * @returns {void}
 */
const _flushStreamWindow = (writer, d) => {
	const window = _frameWindow[d];
	if (window !== null && window.held.length !== 0) _flushWindow(writer, window);
};
...
const _streamOpen = (d, rule) => {
	...
	// A streamed rule writes straight through, so the rules held back for a join
	// where it stands land before its opener — after the opener is read, since
	// taking one clears the store the prelude's own children were printed into.
	if (d === 0) _flushTopLevel(writer);
	else _flushStreamWindow(writer, d - 1);
	_frameWindow[d] = null;
...
const _streamEmitChild = (d, child) => {
	...
	if (!minify || _nodeTypeOf(child) !== T_DECLARATION) {
		...
		if (minify && _prefixDropCandidate === null) {
			const open = _frameWindow[d];
			if (open !== null && !_windowPaysOff(open)) {
				_flushStreamWindow(writer, d);
				writer._emit(body);
				return;
			}
			// The same join a collected block's children are offered, over the ones
			// this block has not written out yet.
			const entry = _ruleEntryOf(child, text);
			let window = _frameWindow[d];
			if (window === null) {
				window = _newJoinWindow();
				_frameWindow[d] = window;
			}
			if (!_offerToWindow(window, entry)) {
				_holdInWindow(
					writer,
					window,
					_newHeldRule(entry, -1, undefined, undefined)
				);
			}
			return;
		}
	...
	// its own pass. A declaration is read at its own place, so whatever the block
	// still holds goes out above it.
	_flushStreamWindow(writer, d);
...
// The rules held back so a later one printing the same block can join its
// selectors, or one printing the same prelude take on its declarations (see
// `_mergeSiblingRules` — the same merge, over rules the writer takes one at a
// time). A window and not the whole run: what is held is what has not been
// written out yet, and a duplicate is written within a few rules of its twin.
/**
 * @typedef {object} HeldRule
 * @property {number} offset the node's source offset, `-1` for a streamed
 * block's child, which is written with no anchor of its own
 * @property {number | undefined} line 0-based source line, when mapping
 * @property {number | undefined} column 0-based source column, when mapping
 * @property {RuleEntry} entry what it printed
 * @property {boolean} owned whether `entry` is one this run built
 * @property {Set<string> | null} writes what it is read as writing, read only
 * where a join asks whether it stands between two rules
 * @property {number} blockHash a hash of the block it printed, for the window's
 * index and for telling in one comparison that a join is not on
 * @property {number} preludeHash the same, for its prelude
 */
/**
 * @typedef {object} JoinWindow
 * @property {(HeldRule | null)[]} held the rules held back, in order from
 * `head`: written-out ones are nulled rather than shifted off, so a run of them
 * costs a pointer where splicing the array costs a copy per rule
 * @property {Map<number, number>} byKey where the last rule printing each block
 * and each prelude stands, counted from the start of the run so a position
 * outlives the shifts writing the window out does. Keyed by a hash of the text
 * rather than by the text: a rule's block is most of it, and hashing a piece the
 * window would have to cut out first costs more per rule than the joins are
 * worth. A collision only offers a join `_joinRuleEntries` then declines.
 * @property {number} head where the first rule still held sits in `held`
 * @property {number} base how many rules the window has already written out
 * @property {number} bytes how much text it holds
 * @property {number} offered how many rules it has been offered
 * @property {number} joined how many of those joined
 */

// The budget past which the window's front is written out: the run must not be
// assembled in memory before any of it is emitted.
const HELD_BUDGET = 16384;

// How much of a range the window's index reads. A block is most of a rule, and
// hashing all of it costs a pass over the whole output per rule — while the
// length plus the head tells one block from another well enough that the rest
// only ever saves a `_joinRuleEntries` that declines.
const _HASH_SPAN = 48;

/**
 * A length-seeded 31-multiplier hash over the head of a range of a string, so a
 * window indexes a rule's prelude and its block without cutting either out.
 * @param {string} text the string to read
 * @param {number} from where the range starts
 * @param {number} to where it ends
 * @returns {number} the hash
 */
const _hashRange = (text, from, to) => {
	const length = to - from;
	let hash = length;
	const end = length > _HASH_SPAN ? from + _HASH_SPAN : to;
...
/** @returns {JoinWindow} a window holding nothing */
const _newJoinWindow = () => ({
	held: [],
	byKey: new Map(),
	head: 0,
	base: 0,
...
// A block whose rules do not repeat pays the join's comparison on every one of
// them for nothing, and a utility sheet is tens of thousands of rules of exactly
// that. So a window that has been offered this many rules and joined fewer than
// one in `_JOIN_RATE` of them stops being offered any: what it would still find
// is a rounding error against what looking costs.
const _JOIN_SAMPLE = 2048;
const _JOIN_RATE = 64;

/**
 * Whether a window is still finding enough joins to be worth offering to.
 * @param {JoinWindow} window the window
 * @returns {boolean} true while it is worth offering to
 */
const _windowPaysOff = (window) =>
	window.offered < _JOIN_SAMPLE || window.joined * _JOIN_RATE >= window.offered;

// How many written-out slots a window keeps before the array is compacted.
const _HELD_SLACK = 1024;

/**
 * @param {PrintContext} writer the print context
 * @param {HeldRule} held one held rule
 * @returns {void}
 */
const _writeHeld = (writer, held) => {
	if (held.offset === -1) writer._emit(held.entry.text);
	else writer.takeText(held.entry.text, held.offset, held.line, held.column);
};

/**
 * Write out a whole window, in order.
 * @param {PrintContext} writer the print context
 * @param {JoinWindow} window the window to empty
 * @returns {void}
 */
const _flushWindow = (writer, window) => {
	const held = window.held;
	for (let i = window.head; i < held.length; i++) {
		_writeHeld(writer, /** @type {HeldRule} */ (held[i]));
	}
	held.length = 0;
	window.byKey.clear();
	window.head = 0;
...
/**
 * A window record for a rule, with the hashes the index and the join both read.
 * @param {RuleEntry} entry the rule as printed
 * @param {number} offset the node's source offset, `-1` for a streamed child
 * @param {number | undefined} line 0-based source line, when mapping
 * @param {number | undefined} column 0-based source column, when mapping
 * @returns {HeldRule} the record
 */
const _newHeldRule = (entry, offset, line, column) => ({
	offset,
	line,
	column,
	entry,
	...
	blockHash:
		entry.prelude === -1
			? 0
			: _hashRange(entry.text, entry.prelude, entry.text.length),
	preludeHash:
		entry.prelude === -1 ? 0 : _hashRange(entry.text, 0, entry.prelude)
});

/**
 * Offer a rule to a window: it joins the one at the back, or one further back
 * that nothing since writes what its own rules do.
 * @param {JoinWindow} window the window to join into
 * @param {RuleEntry} entry the rule as printed
 * @returns {boolean} whether it joined, so it needs no place of its own
 */
const _offerToWindow = (window, entry) => {
	window.offered++;
	const held = window.held;
	const text = entry.text;
	const joinable = entry.prelude !== -1;
	// Two rules join only where a prelude or a block is the same one twice, so
	// the hashes the window indexes by answer that before any text is compared —
	// which is what keeps a sheet whose rules never repeat from paying for the
	// comparison on every one of them.
	const blockHash = joinable ? _hashRange(text, entry.prelude, text.length) : 0;
	const preludeHash = joinable ? _hashRange(text, 0, entry.prelude) : 0;
	const last = /** @type {HeldRule | undefined} */ (held[held.length - 1]);
	if (
		last !== undefined &&
		(!joinable ||
			last.blockHash === blockHash ||
			last.preludeHash === preludeHash)
	) {
		const merged = _joinRuleEntries(last.entry, entry, last.owned);
		if (merged !== null) {
			window.bytes += merged.text.length - last.entry.text.length;
			_reindexHeld(
				window,
				last,
				merged,
				window.base + (held.length - 1 - window.head)
			);
			window.joined++;
			return true;
		}
	}
	if (!joinable || (!entry.atRule && !entry.plain)) return false;
	...
	if (entry.atRule) {
		...
		const mode = _atRuleJoinMode(prelude, text);
		if (mode === AT_JOIN_NONE) return false;
		// A named layer is read only against another block of that layer; anything
		// else against whatever writes what its own rules do.
		if (mode === AT_JOIN_LAYER) layer = prelude;
		else own = _writeKeysOf(text);
	}
	// A qualified rule offers two keys, the block first: it moves a selector
	// rather than a whole block of them. An at-rule offers only its prelude.
	for (let which = entry.atRule ? 1 : 0; which < 2; which++) {
		const key = which === 0 ? blockHash : preludeHash;
		const at = window.byKey.get(key);
		if (at === undefined) continue;
		// A join leaves the key it grew out of behind, so a key can name a rule
		// already written out — and the one at the back is the adjacent join above.
		const from = window.head + (at - window.base);
		if (from < window.head || from >= held.length - 1) continue;
		if (own === null && layer === null) {
			own = _blockWriteKeys(
				_blockProperties(text.slice(entry.prelude + 1, -1))
			);
		}
		let blocked = false;
		for (let i = from + 1; i < held.length; i++) {
			const between = /** @type {HeldRule} */ (held[i]);
			if (layer !== null) {
				blocked = _mayHoldLayer(between.entry.text, layer);
				if (blocked) break;
				continue;
			}
			if (between.writes === null) {
				between.writes = _writeKeysOf(between.entry.text);
			}
			if (_writesSame(between.writes, /** @type {Set<string>} */ (own))) {
				blocked = true;
				break;
			}
		}
		if (blocked) continue;
		const anchor = /** @type {HeldRule} */ (held[from]);
		const merged = _joinRuleEntries(anchor.entry, entry, anchor.owned);
		if (merged === null) continue;
		window.bytes += merged.text.length - anchor.entry.text.length;
		_reindexHeld(window, anchor, merged, at);
		window.joined++;
		return true;
	}
	return false;
...
/**
 * Record what a held rule has become after a join, and index it under the keys
 * it now answers to — one of the two grew, so it would otherwise stay findable
 * only by the one it kept.
 * @param {JoinWindow} window the window it is held in
 * @param {HeldRule} held the record to update
 * @param {RuleEntry} entry what it printed after the join
 * @param {number} at where it stands
 * @returns {void}
 */
const _reindexHeld = (window, held, entry, at) => {
	held.entry = entry;
	held.owned = true;
	held.writes = null;
	if (entry.prelude === -1 || (!entry.atRule && !entry.plain)) return;
	const text = entry.text;
	held.blockHash = _hashRange(text, entry.prelude, text.length);
	held.preludeHash = _hashRange(text, 0, entry.prelude);
	if (!entry.atRule) window.byKey.set(held.blockHash, at);
	window.byKey.set(held.preludeHash, at);
};

/**
 * Add a rule to a window, writing its front out if that puts it over budget.
 * @param {PrintContext} writer the print context
 * @param {JoinWindow} window the window to hold it in
 * @param {HeldRule} record the rule to hold
 * @returns {void}
 */
const _holdInWindow = (writer, window, record) => {
	const held = window.held;
	const entry = record.entry;
	if (entry.prelude !== -1 && (entry.atRule || entry.plain)) {
		const at = window.base + (held.length - window.head);
		if (!entry.atRule) window.byKey.set(record.blockHash, at);
		window.byKey.set(record.preludeHash, at);
	}
	held.push(record);
	window.bytes += record.entry.text.length;
	if (window.bytes <= HELD_BUDGET) return;
	while (window.bytes > HELD_BUDGET && window.head < held.length - 1) {
		const front = /** @type {HeldRule} */ (held[window.head]);
		_writeHeld(writer, front);
		window.bytes -= front.entry.text.length;
		// The map is the window's own index, so a rule leaving it takes its keys —
		// unless a later rule has already claimed them.
		if (window.byKey.get(front.blockHash) === window.base) {
			window.byKey.delete(front.blockHash);
		}
		if (window.byKey.get(front.preludeHash) === window.base) {
			window.byKey.delete(front.preludeHash);
		}
		held[window.head] = null;
		window.head++;
		window.base++;
	}
	if (window.head >= _HELD_SLACK) {
		held.splice(0, window.head);
		window.head = 0;
	}
};

// The top-level rules held back. One window, since a top-level node is handed to
// the writer as it finishes rather than collected with its siblings.
const _topLevelWindow = _newJoinWindow();

/**
 * Hand a finished top-level node to the writer, holding a qualified rule back so
 * a later one printing the same block can join its selectors onto it.
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	// The node's own text first — holding it drops the store it sits in.
	...
	if (candidate !== null && candidate.node === node) {
		_flushWindow(writer, _topLevelWindow);
	...
	// Its kept comments stay queued: they land where it stood, which is after the
	// held rules, and until then they block a join across that gap.
	if (own.length === 0) return;
	// A kept comment between two rules was written above the second, so the ones
	// before it go out first and the window starts again after it.
	if (writer.hasInsertBefore(offset)) {
		_flushWindow(writer, _topLevelWindow);
		writer.flushInsertsBefore(offset);
	}
	if (_offerToWindow(_topLevelWindow, entry)) return;
	_holdInWindow(
		writer,
		_topLevelWindow,
		_newHeldRule(entry, offset, line, column)
	);
};

/**
 * Emit the held top-level rules, if any. Called once the stylesheet is consumed,
 * and before a streamed rule writes straight through them.
 * @param {PrintContext=} writer the print context
 * @returns {void}
 */
const _flushTopLevel = (writer) => {
	if (writer !== undefined) {
		if (_topLevelWindow.held.length !== 0) {
			_flushWindow(writer, _topLevelWindow);
		}
		return;
	}
	// Unconditional: a parse that threw mid-eviction leaves `head` past 0, which
	// the next parse's flush would start reading from — dropping that many rules.
	_topLevelWindow.held.length = 0;
	_topLevelWindow.byKey.clear();
	_topLevelWindow.head = 0;
	_topLevelWindow.base = 0;
	_topLevelWindow.bytes = 0;
	_topLevelWindow.offered = 0;
	_topLevelWindow.joined = 0;
};
...
const grammar = (input, visitors, writer, options) => {
	...
	_hexAlphaAllowed =
		environment === undefined || environment.cssColorHexAlpha !== false;
	_doublePositionAllowed =
		environment === undefined ||
		environment.cssGradientDoublePosition !== false;
	_insetShorthandAllowed =
		environment === undefined || environment.cssInsetShorthand !== false;
	_rangeSpellingAllowed =
		environment === undefined || environment.cssMediaQueryRange !== false;
	_placeShorthandAllowed =
		environment === undefined || environment.cssPlaceShorthand !== false;
	...
	if (browsers === undefined) {
		...
		_seenPrefixRules = _prefixBrowsers === null ? null : new Map();
	}
	_visitors = visitors;
 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits">} CssPrintOptions
 */
...
/** @type {Map<Map<string, [string, [string, number, number][]][]>, Map<string, Set<string> | null>>} */
let _neededPrefixMemo = new Map();

/**
 * Parse a browserslist selection into `browser -> selected versions`, memoized
 * on the selection itself, and make it the one this parse prefixes for. Null
 * where it resolves to no browser at all, which is not a target to prefix for.
...
const _useBrowsers = (browsers) => {
	...
	if (key !== _browsersKey) {
		...
		for (const entry of browsers) {
			...
			if (version === null || !PREFIX_BROWSERS.has(name)) continue;
			...
			const versions = parsed.get(name);
			if (versions === undefined) parsed.set(name, [version]);
			else versions.push(version);
		}
		_parsedBrowsersMemo = parsed.size === 0 ? null : parsed;
	}
	_prefixBrowsers = _parsedBrowsersMemo;
};
...
const _encodeBrowserVersion = (version) => {
	// Newer than any real version, but finite — a still-prefixed feature carries
	// `to === Infinity`, and `TP < Infinity` must hold for TP to need the prefix.
	if (version === "TP") return Number.MAX_SAFE_INTEGER;
 ...
 * @param {Map<string, [string, [string, number, number][]][]>} table its axis' prefix table
...
const _neededPrefixes = (table, name) => {
	...
	if (prefixes !== undefined) {
		...
		for (const [prefix, entries] of prefixes) {
			for (const [browser, from, to] of entries) {
				const versions = parsed.get(browser);
...
// What makes a value one an engine may fail to read: a call, a vendor spelling,
// or a custom property — the three a fallback pair is written in.
const _FALLBACK_SHAPED_RE = /[("']|--|(?:^|[\s,/])-[a-z]/;

// A dimension's unit, read off the number so a hex color's digits are not one.
const _DIMENSION_UNIT_RE = /(?:^|[\s,/])[+-]?(?:\d+\.?\d*|\.\d+)([a-z%]*)/g;

// A value spelled the way one engine alone reads it, which is a fallback for the
// unprefixed one beside it however plainly that one is written: an engine that
// reads `display:-ms-grid` is exactly one that does not read `display:grid`.
const _VENDOR_SPELLED_RE = /(?:^|[\s,/(])-[a-z]/;

/**
 * Whether a value is one every engine reading the property also reads, which is
 * what makes an earlier declaration of that property dead rather than its
 * fallback. Anything a target might not parse — a call, a vendor spelling, a
 * unit newer than the properties it can sit on — answers false.
 * @param {string} value the lowercased printed value
 * @returns {boolean} true when a later declaration of it supersedes an earlier
 */
const _supersedingValue = (value) => {
	if (value.length === 0 || _FALLBACK_SHAPED_RE.test(value)) return false;
	_DIMENSION_UNIT_RE.lastIndex = 0;
	for (;;) {
		const match = _DIMENSION_UNIT_RE.exec(value);
		if (match === null) return true;
		if (match[1].length !== 0 && !OVERRIDABLE_UNITS.has(match[1])) return false;
	}
};
...
/** @type {Map<Map<string, [string, [string, number, number][]][]>, Map<string, string>>} */
const _prefixedValueKeywords = new Map();
/**
 * @param {Map<string, [string, [string, number, number][]][]>} table one property's value table
...
const _prefixQualifiedRule = (ruleText, parts, soft, body, scope, top) => {
	...
	const signature = `s${preludeOf((i) => (selectors[i].at === -1 ? "" : bases[i] + args[i]))}`;
 ...
 * @param {string} name the function name as written
 * @param {string} inner the already-joined argument text
...
const _minifyAnPlusB = (name, inner) => {
	...
	const first = NTH_NAMED_EQUIVALENTS.get(name.toLowerCase());
...
const _minifyUrlFunction = (name, fn, inner) => {
	...
	const body = _decodePercentEscapes(written, false, mark);
	...
	if (escaped !== null) return `${name}(${escaped})`;
	// It keeps its quotes, and with them whatever the escapes gave back.
	return body === written ? null : `${name}(${mark}${body}${mark})`;
};
...
const _hasSubstitutionInSpan = (from, to) => {
	for (let i = from; i < to; i++) {
		if (_input.charCodeAt(i) === CC_LEFT_PARENTHESIS) {
			return _SUBSTITUTION_RE.test(_input.slice(from, to));
		}
	}
	return false;
};
...
const _customPropertyGap = (writer, start, end, out, next) => {
	...
	if (kept !== "") return kept;
	return next !== "" &&
		_wouldFuseTokens(out.charCodeAt(out.length - 1), next, out)
		? " "
		: "";
};
...
const _dropShadowZeroLengths = (property, components) => {
	...
	for (let i = 0; i < layers.length; i++) {
		...
		let last = end;
		while (last - start > minimum && layer[last - 1] === "0") {
			last--;
			changed = true;
		}
...
const _dropDefaultGradientDirection = (fn, inner) => {
	if (!LINEAR_GRADIENTS.has(fn)) return null;
	const comma = inner.indexOf(",");
...
const _valueFragment = (fragment, property, minify) => {
	let out = fragment;
	if (minify && !_inSupportsPrelude) out = _unwrapCalc(out, property);
...
// `[ first | last ]? baseline`, the alignment value a shorthand does not copy.
const BASELINE_POSITION_RE = /(^|\s)baseline$/i;
 ...
 */
const _mergeBoxLonghands = (path, decls, writer, table, mode, at, repeated) => {
	...
	for (const [shorthand, longhands] of table) {
		...
		if (mode === MERGE_FAMILY) {
			...
			// `place-content` is the one pair that does not copy an omitted second
			// value: after a `<baseline-position>` it defaults to `start`.
			if (
				box.length === 1 &&
				shorthand === "place-content" &&
				BASELINE_POSITION_RE.test(box[0])
			) {
				continue;
			}
			value = box.join(" ");
...
const _COMPOUND = `(?:\\*|${_IDENT}|[.#]${_IDENT}|${_ATTRIBUTE})(?:[.#]${_IDENT}|${_ATTRIBUTE})*`;
...
const _isJoinablePrelude = (prelude, keyframe, nested) => {
	...
	// A joinable compound is `*`, an identifier, `.x` / `#x` or `[…]`, and only an
	// identifier's escape or an attribute's quoted value can carry a `:`. So an
	// unescaped one with no `[` in sight is a pseudo-class, which no shape here
	// accepts — answered without splitting the list or running the shape.
	if (!prelude.includes("[")) {
		for (let i = 0; i < prelude.length; i++) {
			const code = prelude.charCodeAt(i);
			if (code === CC_REVERSE_SOLIDUS) i++;
			else if (code === CC_COLON) return false;
		}
	}
	// One selector is the common case and needs no list built to walk it.
	if (!prelude.includes(",")) return shape.test(prelude);
...
const _joinRuleEntries = (before, entry, owned) => {
	...
	const list = `${before.text.slice(0, before.prelude)},${entry.text.slice(0, entry.prelude)}`;
	return {
...
// The keys a declaration is read as writing: the property, the shorthands its
// own name says it is part of (`margin-top` is under `margin`), and the
// longhands it resets that its name cannot say (`inset` sets `top`, `gap` sets
// `row-gap`). Two declarations write the same property exactly when their key
// sets meet, so one function answers both the writing and the asking side.
/**
 * @param {string} property one lowercased property name
 * @param {Set<string>} into the keys collected so far
 * @returns {void}
 */
const _addWriteKeys = (property, into) => {
	into.add(property);
	// A leading `-` is a custom property or a vendor spelling, and neither is a
	// longhand of the name its first segment reads as.
	if (property.charCodeAt(0) !== CC_HYPHEN_MINUS) {
		for (
			let at = property.indexOf("-");
			at !== -1;
			at = property.indexOf("-", at + 1)
		) {
			into.add(property.slice(0, at));
		}
	}
	const reset = RESET_LONGHANDS.get(property);
	if (reset !== undefined) for (const longhand of reset) into.add(longhand);
};

/**
 * What a printed sibling is read as writing, however deep — a rule's own block,
 * and the blocks of the rules an at-rule wraps. A prelude's `:` is not one: the
 * segment it sits in ends at a `{` rather than at a `;` or a `}`.
 * @param {string} text one printed sibling
 * @returns {Set<string>} its write keys
 */
const _writeKeysOf = (text) => {
	/** @type {Set<string>} */
	const out = new Set();
	let depth = 0;
	let quote = 0;
	...
	for (let i = 0; i < text.length; i++) {
		...
		if (cc === CC_QUOTATION_MARK || cc === CC_APOSTROPHE) {
			quote = cc;
		} else if (cc === CC_LEFT_PARENTHESIS || cc === CC_LEFT_SQUARE) {
			depth++;
		} else if (cc === CC_RIGHT_PARENTHESIS || cc === CC_RIGHT_SQUARE) {
			depth--;
		} else if (depth === 0) {
			if (cc === CC_COLON) {
				if (colon === -1) colon = i;
			} else if (cc === CC_LEFT_CURLY) {
				start = i + 1;
				colon = -1;
			} else if (cc === CC_SEMICOLON || cc === CC_RIGHT_CURLY) {
				if (colon !== -1) {
					_addWriteKeys(toLowerCaseIfNeeded(text.slice(start, colon)), out);
				}
				start = i + 1;
				colon = -1;
			}
...
/**
 * The write keys of a block a join would move, asked once per candidate.
 * @param {Set<string>} properties the block's own property names
 * @returns {Set<string>} its write keys
 */
const _blockWriteKeys = (properties) => {
	/** @type {Set<string>} */
	const out = new Set();
	for (const property of properties) _addWriteKeys(property, out);
	return out;
};

// How a mergeable at-rule's join reads what stands between it and the block it
// would join: a named layer is read only against another block of that layer,
// since a declaration in any other layer wins or loses on the layer rather than
// on where it stands, while everything else is read against whatever writes
// what it does. `AT_JOIN_NONE` is a rule that may not move at all.
const AT_JOIN_NONE = -1;
const AT_JOIN_LAYER = 0;
const AT_JOIN_CASCADE = 1;

// Only the canonical spelling is told apart from another layer's, so a sheet
// shouting `@LAYER` joins nothing rather than joining the wrong thing.
const _LAYER_OPENER_RE = /@layer/gi;

/**
 * How far back a mergeable at-rule may be moved, if at all.
 * @param {string} prelude the at-rule's printed prelude
 * @param {string} text the whole rule as printed
 * @returns {number} `AT_JOIN_NONE`, `AT_JOIN_LAYER` or `AT_JOIN_CASCADE`
 */
const _atRuleJoinMode = (prelude, text) => {
	const layer = prelude.startsWith("@layer ");
	_LAYER_OPENER_RE.lastIndex = layer ? prelude.length : 0;
	// A block opening a layer of its own may not move: a layer is ordered by
	// where it is first opened, which moving the block would change.
	if (_LAYER_OPENER_RE.test(text)) return AT_JOIN_NONE;
	return layer ? AT_JOIN_LAYER : AT_JOIN_CASCADE;
};

/**
 * Whether a sibling standing between two blocks of one layer could hold rules in
 * that same layer, which is the only thing their order is read against.
 * @param {string} text the sibling as printed
 * @param {string} prelude the layer's printed prelude
 * @returns {boolean} true when the join would change which declaration wins
 */
const _mayHoldLayer = (text, prelude) => {
	_LAYER_OPENER_RE.lastIndex = 0;
	for (;;) {
		const match = _LAYER_OPENER_RE.exec(text);
		if (match === null) return false;
		// Anything but its own opener is a layer this cannot tell from the one
		// being joined; its own names a layer of its own unless it spells this one.
		if (match.index !== 0 || match[0] !== "@layer") return true;
		if (
			text.startsWith(prelude) &&
			text.charCodeAt(prelude.length) === CC_LEFT_CURLY
		) {
			return true;
		}
	}
...
/**
 * Whether a sibling standing between two rules writes what their shared block
 * does, so joining them would change which declaration wins.
 * @param {Set<string>} between what the sibling writes
 * @param {Set<string>} own the block's write keys
 * @returns {boolean} true when the join would change the cascade
 */
const _writesSame = (between, own) => {
	// `all` resets every property, whichever side declares it.
	if (between.has("all")) return true;
	if (own.has("all")) return between.size !== 0;
	// The smaller set is the one worth walking.
	if (between.size < own.size) {
		for (const key of between) if (own.has(key)) return true;
		return false;
	}
	for (const key of own) if (between.has(key)) return true;
	return false;
};

/**
 * Join sibling rules that print the same block. Adjacent ones join outright —
 * nothing stands between them, so the cascade is unchanged. A later one joins a
 * rule further back only where nothing between the two declares a property the
 * shared block writes: the join reads those declarations at the earlier place,
 * and a rule in between writing the same property would then win where it lost.
 * The later rule's text becomes empty and its selectors move onto the earlier.
 ...
 * @returns {void}
 */
const _mergeSiblingRules = (items, texts, pending) => {
	let previous = -1;
	/** @type {RuleEntry | null} */
	let held = null;
	// Whether `held` is a rule this run built, rather than one the printer stored.
	let owned = false;
	// The last rule to print each block, so a later one printing it again joins
	// where it stands rather than only next door. Two siblings is the common
	// case and can only ever be adjacent, so neither is built for it.
	/** @type {Map<string, number> | null} */
	const anchors = items.length > 2 ? new Map() : null;
	// What each sibling writes, read only where a join asks and kept at its own
	// index: a join moves text backwards only, so a set read here still stands
	// for everything between two indexes however the two have since moved.
	/** @type {(Set<string> | null)[] | null} */
	const writes = anchors === null ? null : items.map(() => null);
	// The at-rules this run has already extended, whose children array it may go
	// on extending rather than copying a growing one.
	/** @type {Set<number> | null} */
	let ownedAnchors = null;
	for (let i = 0; i < items.length; i++) {
		...
		// Only a block of declarations moves back over another rule: an at-rule
		// carries a whole condition, and a nested rule's `&` lends no selector.
		if (writes !== null && !entry.atRule && entry.plain) {
			const block = entry.text.slice(entry.prelude);
			const prelude = entry.text.slice(0, entry.prelude);
			// Either way round: a rule printing this block lends it these selectors,
			// and a rule printing this prelude takes on these declarations. The
			// block is tried first — it moves a selector rather than a whole block.
			/** @type {Set<string> | null} */
			let own = null;
			/** @type {RuleEntry | null} */
			let merged = null;
			let at = -1;
			for (const key of [block, prelude]) {
				const anchorAt = /** @type {Map<string, number>} */ (anchors).get(key);
				if (anchorAt === undefined || anchorAt === previous) continue;
				if (own === null) {
					own = _blockWriteKeys(_blockProperties(block.slice(1, -1)));
				}
				let blocked = false;
				for (let j = anchorAt + 1; j < i && !blocked; j++) {
					let between = writes[j];
					if (between === null) {
						between = _writeKeysOf(texts[j]);
						writes[j] = between;
					}
					blocked = _writesSame(between, own);
				}
				if (blocked) continue;
				merged = _joinRuleEntries(
					_ruleEntryOf(items[anchorAt], texts[anchorAt]),
					entry,
					false
				);
				if (merged !== null) {
			...
			if (merged !== null) {
				// The text is read where the rule it joined stands, and this is the
				// last turn its own text is to hand.
				writes[i] = _writeKeysOf(texts[i]);
				_ruleEntry.set(items[at], merged);
				texts[at] = merged.text;
				texts[i] = "";
				// The join grew one of the two keys, so the rule is registered under
				// both again rather than left findable only by the one it kept.
				/** @type {Map<string, number>} */ (anchors).set(
					merged.text.slice(merged.prelude),
					at
				);
				/** @type {Map<string, number>} */ (anchors).set(
					merged.text.slice(0, merged.prelude),
					at
				);
				// The rule after this one now stands next to `held`, which is where
				// the run was: it is offered the adjacent join on the next turn.
				continue;
			}
			/** @type {Map<string, number>} */ (anchors).set(prelude, i);
			/** @type {Map<string, number>} */ (anchors).set(block, i);
		} else if (writes !== null && entry.atRule) {
			// An at-rule moves back over what it opens no condition against: another
			// block of the same layer, or a sibling writing what its own rules do.
			const prelude = entry.text.slice(0, entry.prelude);
			const mode = _atRuleJoinMode(prelude, entry.text);
			const anchorAt =
				mode === AT_JOIN_NONE
					? undefined
					: /** @type {Map<string, number>} */ (anchors).get(prelude);
			/** @type {RuleEntry | null} */
			let merged = null;
			if (anchorAt !== undefined && anchorAt !== previous) {
				const own = mode === AT_JOIN_LAYER ? null : _writeKeysOf(entry.text);
				let blocked = false;
				for (let j = anchorAt + 1; j < i && !blocked; j++) {
					if (texts[j].length === 0) continue;
					if (own === null) {
						blocked = _mayHoldLayer(texts[j], prelude);
						continue;
					}
					let between = writes[j];
					if (between === null) {
				...
				if (!blocked) {
					merged = _joinRuleEntries(
						_ruleEntryOf(items[anchorAt], texts[anchorAt]),
						entry,
						ownedAnchors !== null && ownedAnchors.has(anchorAt)
					);
				}
			}
			if (merged !== null) {
				const at = /** @type {number} */ (anchorAt);
				if (ownedAnchors === null) ownedAnchors = new Set();
				ownedAnchors.add(at);
				writes[i] = _writeKeysOf(texts[i]);
				_ruleEntry.set(items[at], merged);
				texts[at] = merged.text;
...
const _rulePrelude = (path, writer, minify, outParts) => {
	...
	if (minify && _rangeSpellingAllowed && path.type() === T_AT_RULE) {
		// One read of the name for both tests: each is a slice of the source.
		const name = path.name();
		if (equalsLowerCase(name, "media") || equalsLowerCase(name, "container")) {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			if (!path.inValue() && NTH_PSEUDO_FUNCTIONS.has(name.toLowerCase())) {
				return _minifyAnPlusB(name, inner);
			}
			...
			const oneAxis = X_AXIS_TRANSFORMS.get(fn);
			if (
				oneAxis !== undefined &&
				path.inValue() &&
				!_inSubstitutedValue &&
			...
			if (path.inValue() && !_inSubstitutedValue) {
				...
				const omitted = FILTER_FUNCTION_OMITTED.get(fn);
				...
				if (stops !== null) return `${name}(${stops})`;
		...
		case T_DECLARATION: {
			...
			if (minify && count === 0 && !custom) return "";
			let value = "";
			if (count !== 0) {
				...
				if (custom) {
					...
					// document — which `syntaxEquivalence` forbids, whatever a
		...
		case T_QUALIFIED_RULE: {
			...
			const preludeParts =
				_prefixBrowsers !== null ? /** @type {string[]} */ ([]) : undefined;
			...
			const childScope =
				_seenPrefixRules === null
					? undefined
					: _seenPrefixRules.get(_currentNode);
			if (minify) {
				_mergeSiblingRules(
					items,
					texts,
			...
			// the earlier ones carry nothing — whatever stands between them, since
			// they all resolve to that same last one. Identical text always does;
			// a differently spelled one only where the later value is one no engine
			// reading the property can fail to read, since otherwise the earlier is
			// its fallback — dropping those is how esbuild and lightningcss lose a
			// `color(display-p3 …)` pair.
			/** @type {Set<number> | null} */
			let superseded = null;
			// One item can supersede nothing, and most rules hold one or none.
			if (minify && items.length > 1) {
				superseded = new Set();
				/** @type {Map<string, number>} */
				const seen = new Map();
				/** @type {Map<string, number[]>} */
				...
				for (let i = 0; i < items.length; i++) {
					...
					if (earlier === undefined) {
						...
						// A value no engine reading the property can fail to read
						// supersedes whatever stands before it. One that may fail still
						// supersedes an earlier declaration whose every item it writes
						// again, where the items are names rather than values.
						const plain = _supersedingValue(value);
						const listed = !plain && CUSTOM_IDENT_LIST_PROPERTIES.has(property);
						if (plain || listed) {
							...
							for (let j = earlier.length - 1; j >= 0; j--) {
								...
								const beforeValue = _printedValue(
									beforeText,
									beforeText.indexOf(":")
								);
								if (plain) {
									if (_VENDOR_SPELLED_RE.test(beforeValue)) continue;
								} else if (!_coveredByLater(value, beforeValue)) {
									continue;
								}
			...
			if (_prefixBrowsers !== null) {
				...
				for (let i = 0; i < items.length; i++) {
					...
					values.push(
						valueTable === undefined ? "" : _printedValue(text, colon)
					);
					if (
			...
			if (_seenPrefixRules !== null) _seenPrefixRules.delete(_currentNode);
