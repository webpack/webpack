 ...
 * tokens (minifying) or unconditionally (beautifying). In a selector prelude
 * (`isSelector`) whitespace adjacent to a top-level combinator is also dropped.
 ...
 * @param {boolean=} isSelector whether this is a qualified rule's selector prelude
 * @returns {string} the joined text
 */
const _join = (parts, beautify, isSelector) => {
	let out = "";
	let last = -1;
	let pending = false;
		...
		if (pending) {
			const keep =
				...
				(_keepSeparator(last, first) &&
					!(isSelector && (_isCombinator(last) || _isCombinator(first))));
...
const _minifyString = (raw) => {
	...
	for (let i = 1; i < n - 1; i++) {
		const c = raw.charCodeAt(i);
		if (c !== CC_REVERSE_SOLIDUS || i + 1 > n - 2) {
			out += raw[i];
			continue;
		}
		const next = raw.charCodeAt(i + 1);
 ...
 * `url()` quote normalization) and drops rules whose block ends up empty — each
 * value-identical, so meaning never changes.
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_SIMPLE_BLOCK: {
			...
			// `[…]` outside a value is an attribute selector, whose quoted value may be
			// a bare identifier; inside one it is a grid line-name list, which has none.
			const parts =
				minify && open === "[" && !path.inValue()
					? _printAttributeSelector(path, children, writer)
					: children.map((c) => writer.get(c));
			const inner = _join(parts, !minify);
		...
		case T_DECLARATION: {
			...
			if (children.length !== 0) {
				if (custom || name.toLowerCase() === "unicode-range") {
		...
		case T_QUALIFIED_RULE: {
			...
			const prelude = _join(parts, !minify, path.type() === T_QUALIFIED_RULE);
