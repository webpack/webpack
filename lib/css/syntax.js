...
const _trimCssWhiteSpace = (text) => {
	let start = 0;
	let end = text.length;
	while (start < end && _isWhiteSpace(text.charCodeAt(start))) start++;
	while (end > start && _isWhiteSpace(text.charCodeAt(end - 1))) end--;
...
const _forgetLayersWritten = (layers, text) => {
	...
	for (let i = 0; i < text.length; i++) {
		...
		if (end >= text.length || !_isWhiteSpace(text.charCodeAt(end))) continue;
		while (
			end < text.length &&
