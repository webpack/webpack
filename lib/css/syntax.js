 ...
 */
/**
 * Whether a code point is one a string may hold as itself. The two quotes and
 * the reverse solidus are left out: each would only need escaping again, which
 * is what the escape already does.
 * @param {number} code a code point
 * @returns {boolean} true when the character needs no escape
 */
const _isPlainStringCharacter = (code) =>
	code >= 0x20 &&
	code <= 0x7e &&
	code !== CC_QUOTATION_MARK &&
	code !== CC_APOSTROPHE &&
	code !== CC_REVERSE_SOLIDUS;

/**
 * Write out the code-point escapes a string can spell as the character itself.
 * §4.3.7 lets one whitespace close an escape, so `\30 ` is the single character
 * `0` written in five bytes. Only ASCII is unescaped: a character past it would
 * turn a stylesheet that was ASCII into one whose bytes depend on the encoding
 * it is served with.
 * @param {string} body the string's text between its quotes
 * @returns {string} the body with those escapes written out
 */
const _unescapeStringBody = (body) => {
	const n = body.length;
	let out = "";
	// Whether the last escape emitted is still open: under six digits and closed
	// by neither a whitespace nor a non-digit, so one more would join it.
	let openEscape = false;
	for (let i = 0; i < n; i++) {
		if (body.charCodeAt(i) !== CC_REVERSE_SOLIDUS) {
			out += body[i];
			openEscape = false;
			continue;
		}
		let end = i + 1;
		while (end < n && end - i <= 6 && _isHexDigit(body.charCodeAt(end))) end++;
		if (end === i + 1) {
			// Not a code point: the escape stands for the one character after it.
			out += body.slice(i, i + 2);
			openEscape = false;
			i++;
			continue;
		}
		// One whitespace after the digits closes the escape rather than following it.
		const closed = end < n && _isWhiteSpace(body.charCodeAt(end));
		const stop = closed ? end + 1 : end;
		const code = Number.parseInt(body.slice(i + 1, end), 16);
		// A digit or whitespace written out next to an open escape would be read
		// as part of it — a sixth digit, or the whitespace that closes it.
		const joins =
			openEscape && (_isHexDigit(code) || _isWhiteSpace(code) || code === 0x20);
		if (_isPlainStringCharacter(code) && !joins) {
			out += String.fromCharCode(code);
			openEscape = false;
		} else {
			out += body.slice(i, stop);
			openEscape = !closed && end - i <= 6;
		}
		i = stop - 1;
	}
	return out;
...
// A body whose last escape has no character to escape — the string ran to EOF
// and swallowed its own closing quote.
const _TRAILING_ESCAPE_RE = /(?:^|[^\\])(?:\\\\)*\\$/;

/**
 * Shorten a string token: write out the escapes the characters themselves say,
 * then quote it whichever way costs least.
 * @param {string} raw the string token's text, quotes included
 * @returns {string} the shortest equal spelling
 */
const _minifyString = (raw) => {
	...
	if (n < 2 || raw.charCodeAt(n - 1) !== raw.charCodeAt(0)) return raw;
	if (!raw.includes("\\")) return _normalizeStringQuotes(raw);
	const body = raw.slice(1, -1);
	if (_TRAILING_ESCAPE_RE.test(body)) return raw;
	return _normalizeStringQuotes(
		`${raw[0]}${_unescapeStringBody(body)}${raw[0]}`
	);
};

/**
 * Wrap a string token in whichever quote costs least: `"` unless the text
 * carries more of those than `'`.
 * @param {string} raw the string token's text, quotes included
 * @returns {string} the same string, quoted the cheaper way
 */
const _normalizeStringQuotes = (raw) => {
	const quote = raw.charCodeAt(0);
	const n = raw.length;
 ...
 */
// Each `rotate3d()` axis in order, and the function naming a rotation about it.
// The z one is `rotate()` rather than `rotateZ()`: the two are the same rotation
// and the short spelling is a byte less.
const _ROTATE_AXIS_FUNCTIONS = ["rotateX", "rotateY", "rotate"];

// An angle that is a number and a unit, which is the only shape whose sign the
// text alone can flip.
const _SIGNED_ANGLE_RE = /^([+-]?)((?:\d+\.?\d*|\.\d+)[a-z]+)$/i;

/**
 * Rewrite a `rotate3d()` about one axis as the rotation it is. CSS Transforms 2
 * §11 normalizes the vector, so only each component's sign carries: a negative
 * axis turns the same amount the other way.
 * @param {string[]} args the call's four arguments
 * @returns {string | null} the shorter call, or `null` to keep this one
 */
const _reduceRotate3d = (args) => {
	let axis = -1;
	for (let i = 0; i < 3; i++) {
		if (_ZERO_COMPONENT_RE.test(args[i])) continue;
		// A second axis makes it a rotation no single-axis call names.
		if (axis !== -1) return null;
		axis = i;
	}
	if (axis === -1) return null;
	let angle = args[3];
	if (args[axis].charCodeAt(0) === CC_HYPHEN_MINUS) {
		const signed = _SIGNED_ANGLE_RE.exec(angle);
		if (signed === null) return null;
		angle = signed[1] === "-" ? signed[2] : `-${signed[2]}`;
	}
	const out = `${_ROTATE_AXIS_FUNCTIONS[axis]}(${angle})`;
	return out.length < `rotate3d(${args.join(",")})`.length ? out : null;
};

/**
 * Rewrite a transform function as the shorter one naming the same matrix.
 * @param {string} fn the lowercased function name
 * @param {string} inner the already-joined argument text
 * @returns {string | null} the shorter call, or `null` to keep this one
 */
const _reduceTransformFunction = (fn, inner) => {
	...
	// A `scale3d()` that leaves z alone scales in the plane, which `scale()` says.
	if (fn === "scale3d" && args.length === 3 && args[2] === "1") {
		return args[0] === args[1]
			? `scale(${args[0]})`
			: `scale(${args[0]},${args[1]})`;
	}
	if (fn === "rotate3d" && args.length === 4) return _reduceRotate3d(args);
	return null;
};
...
const _collapseShorthand = (path, property, children, writer) => {
	...
	if (
		...
		return _orderTransitionSlots(components) || dropped;
	}
