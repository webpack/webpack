...
const _minifyAnPlusB = (name, inner) => {
	...
	const first = _NTH_FIRST_LAST.get(name.toLowerCase());
...
// The four pseudo-classes whose first / last element has its own name, and what
// `An+B` of exactly `1` means for each.
const _NTH_FIRST_LAST = new Map([
...
// Zero of any percentage: a percentage of any size is still nothing.
const _ZERO_PERCENTAGE_RE = /^[+-]?0(?:\.0*)?%$/;
...
const _reduceTransformFunction = (fn, inner) => {
	...
	const zeroOffset = (index) =>
		zero(index) ||
		_ZERO_PERCENTAGE_RE.test(args[index]) ||
	...
	if (fn === "scale" && args.length === 2) {
		...
		// own function.
		if (args[1] === "1") return `scaleX(${args[0]})`;
		if (args[0] === "1") return `scaleY(${args[1]})`;
		return null;
	}
	if (fn === "scale3d" && args.length === 3) {
		// A z factor of 1 scales nothing along it, leaving the 2D scale.
		if (args[2] === "1") return `scale(${args[0]},${args[1]})`;
		// ...and a 2D pair of 1 leaves the z scale alone.
		if (args[0] === "1" && args[1] === "1") return `scaleZ(${args[2]})`;
	}
...
const _joinRuleEntries = (before, entry, owned) => {
	...
	// The seam can repeat a selector the other side already names, which matches
	// nothing twice. Order is left alone: each half is already in its own, and
	// re-sorting across the seam costs more compressed than the bytes it saves.
	const list = _dedupeSelectorList(`${beforePrelude},${prelude}`);
	return {
 ...
 * elements at the same specificity, so one canonical order lets equal lists
 * compress alike, and a repeat inside it is dropped.
 * @param {string} prelude the printed selector list
 * @returns {string} it, sorted and deduplicated
 */
...
/**
 * The same, keeping the order: a repeat is dropped but nothing moves.
 * @param {string} prelude the printed selector list
 * @returns {string} it, without repeats
 */
const _dedupeSelectorList = (prelude) => {
	const list = _splitSelectorList(prelude);
	if (list.length < 2) return prelude;
	const seen = new Set(list);
	return seen.size === list.length ? prelude : [...seen].join(",");
};
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			if (path.childCount() === 1) {
				...
				// straight back — and a trim only ever parts two of them.
			...
			if (children.length !== 0) {
				...
				if (custom) {
					...
					const shorthand = minify
						...
						: null;
					if (shorthand === null && children.length === 1) {
						...
						value = _valueFragment(writer.get(children[0]), property, minify);
					} else {
						const fragments =
							shorthand === null
								? children.map((c) => writer.get(c))
