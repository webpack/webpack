...
const _walkValue = (node, parent, index, writer) => {
	...
	// included, so the flag rides the recursion the way `_inValue` does — and is
	// still set when this node's own printer runs, which is what joins the
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		if (ty === T_FUNCTION) {
			...
			if (!_inMathFunction && MATH_FUNCTIONS.has(name)) {
				enteredMath = true;
				_inMathFunction = true;
			}
	...
	if (enteredMath) _inMathFunction = false;
...
const _printCalcTerm = (key, coefficient) => {
	...
	// Counted in the group's base unit: back to a real unit first, then through
	// the same search a plain dimension takes.
	const value = _exactDivide(coefficient, base[1]);
	if (value === null) return null;
	const text = _normalizeNumber(String(value));
	if (Number(text) !== value) return null;
	return _convertUnit(text, base[0]);
};
 ...
 * @type {Map<string, (sums: Map<string, number>[], keyword: string) => Map<string, number> | null>}
 */
const _MATH_REDUCERS = new Map([
	...
	[
		...
		(sums) => {
			...
			// Irrational for most inputs, so only a root that squares back exactly
			// is one this can write down.
			const root = Math.sqrt(total);
			const back = _exactMultiply(root, root);
			if (back === null || back !== total) return null;
			return new Map([[key, root]]);
		}
	],
	[
		"round",
...
const _foldMathFunction = (fn, inner) => {
	...
	const [[key, coefficient]] = result;
	return _printCalcTerm(key, coefficient);
};
...
// unit (so never an `<integer>` context) or already an integer. A unitless
// fraction is left out — there the property decides, which only the declaration
// printer knows — as is anything negative.
const _BARE_TERM_RE = /^(?:\d*\.\d+|\d+)(?:%|[a-z]+)$|^\d+$/i;
 ...
 * on its own, since no `<integer>` carries either.
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const inner = _join(
				...
				!minify,
				minify && _inMathFunction ? _TRIM_MATH : _TRIM_NOTHING
			);
			...
			if (!_inSupportsPrelude) {
				...
				if (term !== null) {
					...
					const folded = _BARE_TERM_RE.test(term) ? term : `calc(${term})`;
		...
		case T_SIMPLE_BLOCK: {
			...
			let trim = minify && _inMathFunction ? _TRIM_MATH : _TRIM_NOTHING;
			if (structural && open === "(") {
...
// Set by the walk below a math function, where `*` and `/` are operators that
// need no whitespace rather than value separators.
let _inMathFunction = false;
