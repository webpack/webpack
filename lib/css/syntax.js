...
const _cutEmptiedSpans = (writer, taken) => {
	...
	// Every span is a qualified rule (`_collectRuleSpans`), which is dropped empty
	// wherever it stands — an at-rule, whose block can be its point, is never one.
	for (const other of taken.spans) {
...
const _rewriteSpan = (writer, taken, span, text) => {
	...
	for (const other of taken.spans) {
		...
		if (other.at >= at && other.at + other.len <= end) {
			...
			other.len += delta;
		} else if (other.at >= end) other.at += delta;
	}
};
