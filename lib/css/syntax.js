...
const _heldForLayerGather = (rule, blockStart) => {
	if (_streamWriter === undefined || !_transforms.mergeRules) return false;
	if (!_isNamedLayerAtRule(rule)) return false;
	const name = _trimCssWhiteSpace(
		_input.slice(_aux0[_nodeIndex(rule)], blockStart)
	);
...
const _joinRuleEntries = (before, entry, owned) => {
	...
	if (before.atRule) {
		...
		if (
			...
			from = seam === null ? 0 : 1;
			// The join concatenates, and nothing further is appended to this seam:
			// ordering it here is what a second pass is otherwise left to do. Once
			// per join rather than once per child, so a run stays linear.
			if (seam !== null) joined[at] = _canonicalizeJoined(seam);
		}
