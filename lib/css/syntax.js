...
const _walkRule = (node, parent, index, writer) => {
	...
	if (!skip) {
		if (ty === T_AT_RULE || ty === T_QUALIFIED_RULE) {
			...
			// its tokens print squeezed but unrewritten (see `_inCustomProperty`).
...
/**
 * Drop the selectors a list repeats. A repeat matches what its first spelling
 * already matches, at the same specificity, so only the first is read. Written
 * order is kept: the compressor reads a stylesheet in author order, where like
 * rules already sit together.
 * @param {string} prelude the printed selector list
 * @returns {string} the list without repeats, or the prelude when it is not one
 */
const _dedupeSelectorList = (prelude) => {
	if (!prelude.includes(",")) return prelude;
	const selectors = _splitSelectorList(prelude);
	if (selectors.length < 2) return prelude;
	/** @type {string[]} */
	const unique = [];
	const seen = new Set();
	for (const selector of selectors) {
		// A list a comment emptied is not one this can rewrite.
		if (selector.length === 0) return prelude;
		if (seen.has(selector)) continue;
		seen.add(selector);
		unique.push(selector);
	}
	return unique.length === selectors.length ? prelude : unique.join(",");
};
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			// CSS Values 4 §10.1: a `calc()` standing inside a math expression is
			// what a plain parenthesis already says, so the keyword carries nothing.
			if (fn === "calc" && _mathFunctionDepth > 1) return `(${inner})`;
			return `${name}(${inner})`;
		}
		case T_URL: {
			const raw = path.source();
		...
		case T_DECLARATION: {
			...
			if (children.length !== 0) {
				...
				if (property === "unicode-range") {
					// `U+…` tokenizes as numbers, which numeric normalization would
					// corrupt, so this one keeps its raw source.
					value = children.map((c) => path.source(c)).join("");
				} else if (custom) {
					// A custom property's value is a token stream, so squeezing the
					// whitespace between its tokens and normalizing their spelling leaves
					// every `var()` substitution identical. Only the spelling: the walk
					// holds back the rewrites that restate a value in another notation
					// (see `_inCustomProperty`), since this is the one declaration whose
					// text `getComputedStyle().getPropertyValue()` hands back.
					value = _join(
						children.map((c) => writer.get(c)),
						!minify
					);
		...
		case T_QUALIFIED_RULE: {
			...
			if (minify && path.type() === T_QUALIFIED_RULE) {
				prelude = _dedupeSelectorList(prelude);
