...
// so they share one handler instead of one `case` each).
const HC_DELIM = 0;
...
// unmatched stays HC_DELIM (0). Keep this in sync with the table in the block above.
for (let i = 0; i < 128; i++) {
	if (_singleTT[i] !== 0) _charClass[i] = HC_SINGLE;
	else if (_isWhiteSpace(i)) _charClass[i] = HC_WHITESPACE;
	else if (i === CC_QUOTATION_MARK || i === CC_APOSTROPHE)
		_charClass[i] = HC_STRING;
	else if (i === CC_NUMBER_SIGN) _charClass[i] = HC_NUMBER_SIGN;
	else if (i === CC_PLUS_SIGN) _charClass[i] = HC_PLUS_SIGN;
	else if (i === CC_HYPHEN_MINUS) _charClass[i] = HC_HYPHEN_MINUS;
	else if (i === CC_FULL_STOP) _charClass[i] = HC_FULL_STOP;
	else if (i === CC_LESS_THAN_SIGN) _charClass[i] = HC_LESS_THAN;
	else if (i === CC_AT_SIGN) _charClass[i] = HC_AT_SIGN;
	else if (i === CC_REVERSE_SOLIDUS) _charClass[i] = HC_REVERSE_SOLIDUS;
	else if (_isDigit(i)) _charClass[i] = HC_DIGIT;
	else if (_isIdentStartCodePointCC(i)) _charClass[i] = HC_IDENT;
	// else stays HC_DELIM (0)
}
 ...
 */
const tokenToNode = (t, locConverter) => {
	const tt = t.type;
	// URL is the only leaf with own state (its content range); all others are a
	// plain \`Token\` whose node type comes from the map.
	if (tt === TT_URL) {
	...
	return new Token(_ttToNodeType[tt], t.start, t.end, locConverter);
};
...
const consumeAnAtRule = (ts, nested = false) => {
	...
	const rule = /** @type {AtRule} */ (
		new Container(T_AT_RULE, head.start, head.end, ts.locConverter)
	);
	...
	// `Container` defaults (no block: the `;` / EOF / nested-`}` at-rule forms).
	...
	for (;;) {
		...
		rule.prelude.push(consumeAComponentValue(ts, t));
	}
};
...
const consumeATokenAsNode = (ts) => {
	const t = ts.consume();
	return tokenToNode(t, ts.locConverter);
};
...
const consumeAQualifiedRule = (ts, stopToken, nested = false) => {
	...
	const rule = /** @type {QualifiedRule} */ (
		new Container(T_QUALIFIED_RULE, start, start, ts.locConverter)
	);
	rule.prelude = /** @type {ComponentValue[]} */ ([]);
	// declarations / childRules / blockStart (-1) / blockEnd (-1) keep their
	// `Container` defaults (no block until a `{` is reached).
...
const consumeADeclaration = (ts, nested = false) => {
	...
	const decl = /** @type {Declaration} */ (
		new Container(T_DECLARATION, start, start, ts.locConverter)
	);
...
const consumeASimpleBlock = (ts) => {
	...
	const block = /** @type {SimpleBlock} */ (
		new Container(T_SIMPLE_BLOCK, open.start, open.end, ts.locConverter)
	);
	block.token = token;
	...
	for (;;) {
		...
		block.value.push(consumeAComponentValue(ts, t));
	}
};
...
const consumeAFunction = (ts) => {
	...
	const fn = /** @type {FunctionNode} */ (
		new Container(T_FUNCTION, tFn.start, tFn.end, locConverter)
	);
	...
	for (;;) {
		...
		fn.value.push(consumeAComponentValue(ts, t));
	}
};
...
const A = {
	...
	source: (n) => n.toString(),
	/** @type {(n: Node) => string} */
	value: (n) => /** @type {Token} */ (n).value,
	/** @type {(n: Node) => string} */
	unescaped: (n) => /** @type {Token} */ (n).unescaped,
	/** @type {(n: Node) => string} */
	typeFlag: (n) => /** @type {HashToken} */ (n).typeFlag,
	/** @type {(n: Node) => number} */
	contentStart: (n) => /** @type {UrlToken} */ (n).contentStart,
	/** @type {(n: Node) => number} */
	contentEnd: (n) => /** @type {UrlToken} */ (n).contentEnd,
	/** @type {(n: Node) => string} */
	name: (n) => /** @type {Container} */ (n).name,
	/** @type {(n: Node) => number} */
	nameStart: (n) => /** @type {Container} */ (n).nameStart,
	/** @type {(n: Node) => number} */
	nameEnd: (n) => /** @type {Container} */ (n).nameEnd,
	/** @type {(n: Node) => string} */
	unescapedName: (n) => n.unescapedName,
	/** @type {(n: Node) => ComponentValue[]} */
	children: (n) => /** @type {FunctionNode} */ (n).value,
	/** @type {(n: Node) => ComponentValue[]} */
	prelude: (n) => /** @type {AtRule} */ (n).prelude,
	/** @type {(n: Node) => Declaration[] | null} */
	declarations: (n) => /** @type {AtRule} */ (n).declarations,
	/** @type {(n: Node) => Rule[] | null} */
	childRules: (n) => /** @type {AtRule} */ (n).childRules,
	/** @type {(n: Node) => number} */
	blockStart: (n) => /** @type {AtRule} */ (n).blockStart,
	/** @type {(n: Node) => number} */
	blockEnd: (n) => /** @type {AtRule} */ (n).blockEnd,
	/** @type {(n: Node) => boolean} */
	important: (n) => /** @type {Declaration} */ (n).important,
	/** @type {(n: Node) => SimpleBlockToken} */
	blockToken: (n) => /** @type {SimpleBlock} */ (n).token,
