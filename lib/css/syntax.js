...
const _minifyPolarColorFunction = (fn, inner, hexAlpha) => {
	...
	if (alpha < 1) {
		// The alpha is the author's own number, not a conversion's output.
		const alphaByte = _channelByte(alpha * 255, _ROUNDING_MARGIN);
...
const _mergeBoxLonghands = (path, decls, writer) => {
	...
	for (const [shorthand, longhands] of BOX_LONGHANDS) {
		...
		for (let i = ordered[0] + 1; i < ordered[3] && !blocked; i++) {
			...
			const between = toLowerCaseIfNeeded(path.name(decls[i]));
			blocked =
				between === "all" ||
				between === prefix ||
