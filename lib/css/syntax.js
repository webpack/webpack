...
const _foldBelowStepCount = (term, path) => {
	...
	// An omitted position is `end`, whose minimum is the one every position but
	// `jump-none` takes; a spelling the table has not met reads as the largest,
	// so the fold declines rather than guessing at its grammar.
	let minimum = 1;
	const count = path.childCount(parent);
	for (let at = 0; at < count; at++) {
		const child = path.childAt(parent, at);
		if (path.type(child) !== T_IDENT) continue;
		const named = STEP_POSITION_MINIMUM_COUNTS.get(
...
const _valueReadByTarget = (property, value) => {
	// No selection names no browser to answer for, and one naming a browser the
	// tables do not cover answers for less than the whole of it; either way
	// nothing is known to read the later value and the earlier one stands.
	if (_prefixBrowsers === null || _someBrowserUnanswered) return false;
	const open = value.indexOf("(");
	if (open === -1) {
		return _readsAll(
...
const _composeBlockBody = (
	...
	// pair.
	// TODO answer the same question with no browserslist selection, as csso and
	// cssnano do: theirs is unconditional, and naming the engines is what makes
	// this one sound — with no selection there are none to name.
