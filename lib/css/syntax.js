 ...
 * @property {boolean=} lowercaseNames print every name CSS matches ASCII case-insensitively in one case
 ...
 * @property {boolean=} shortenEscapes write an identifier's escapes and a `url()`'s percent-escapes in their shortest equal form
...
const _minifyUrlFunction = (fn, inner) => {
	...
	const decoded =
		_transforms.shortenEscapes || _renderEmbeddedSource !== undefined
	...
	// without them.
	const body = _transforms.shortenEscapes ? decoded : written;
...
const _minifyIdentEscapes = (text) => {
	if (!_transforms.shortenEscapes) return text;
...
const _lowercasePseudoNames = (parts) => {
	if (!_transforms.lowercaseNames) return;
	for (let i = 1; i < parts.length; i++) {
		const part = parts[i];
...
const _rulePrelude = (path, writer, minify, outParts) => {
	...
	if (path.type() === T_AT_RULE) {
		...
		const lower =
			minify && _transforms.lowercaseNames && !equalsLowerCase(name, "charset");
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const out = !_transforms.lowercaseNames
				? name
				: printed === undefined
					? fn
					: printed;
		...
		case T_URL: {
			...
			const head = _transforms.lowercaseNames
			...
			return `${head}${
				percent && _transforms.shortenEscapes && !value.includes("\\")
		...
		case T_IDENT: {
			...
			if (
				...
				const shorter = _transforms.shortenColors
		...
		case T_DECLARATION: {
			...
			const printedName =
				minify && _transforms.lowercaseNames && !custom
