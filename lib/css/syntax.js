...
const _walkValue = (node, parent, index, writer) => {
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		if (ty === T_FUNCTION && GRADIENT_FUNCTION_RE.test(_gradientName(i0))) {
			_inGradient = true;
		}
		for (let i = vs; i < ve; i++) {
...
// An at-keyword that could name `@namespace`: the spelling itself, or any
// at-keyword carrying an escape, which only decoding would tell apart. Over-wide
// on purpose — a false positive keeps an empty rule, which is the safe side.
