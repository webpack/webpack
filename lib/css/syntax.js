 ...
 * Returning it unchanged declines it, and the URL is emitted as written.
...
const asciiLowerCaseName = (s) => {
	...
	for (let i = 0; i < s.length; i++) {
		const c = s.charCodeAt(i);
		if (c === CC_REVERSE_SOLIDUS) return s;
 ...
 * @property {boolean=} colors write each color in the shortest spelling of the same value
 * @property {boolean=} escapes write an identifier's escapes and a `url()`'s percent-escapes in their shortest equal form
 * @property {boolean=} functions fold a call into the shorter call computing the same value (math, transform, gradient, easing, filter)
 * @property {boolean=} lowercase print every name CSS matches ASCII case-insensitively in one case
 * @property {boolean=} mediaQueries write a media feature in its range spelling and collapse an `and` of two into the interval
 * @property {boolean=} numbers write each number in its shortest equal spelling
 * @property {boolean=} quotes normalize a string's, `url()`'s, font family's and attribute value's quoting
 * @property {boolean=} rules drop and merge whole rules and declarations
 * @property {boolean=} selectors rewrite a selector into an equal one
 * @property {boolean=} shorthands write a value as the shorthand that already implies it, and merge a family of longhands into one
 */
...
const _streamEmitChild = (d, child) => {
	...
	writer.flushPending();
	const minify = writer.options.mode === "minify";
	// Dropping a dead rule or declaration and folding two named layer blocks
	// into one is the one option, so each gather below runs only where it is on.
	const dropping = minify && _transforms.rules;
	if (!minify || _nodeTypeOf(child) !== T_DECLARATION) {
		...
		const positional = dropping && POSITIONAL_AT_RULE_RE.test(text);
		if (positional) {
...
const _emitTopLevel = (writer, node, offset, line, column, text, own) => {
	// Taking back a rule an identical later one makes dead, and folding two named
	// layer blocks into one, are both `transforms.rules`.
	const minify = writer.options.mode === "minify" && _transforms.rules;
 ...
 * @returns {Required<CssTransformOptions>} every entry resolved
 */
 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties" | "renderEmbeddedSource" | "transforms">} CssPrintOptions
 */
...
const _normalizeNumericToken = (text) => {
	...
	if (_transforms.lowercase) {
		const lowered = asciiLowerCaseName(unit);
		const canonical = CANONICAL_NAMES.get(lowered);
		unit = canonical === undefined ? lowered : canonical;
	}
	if (!_transforms.numbers) return text.slice(0, end) + unit;
...
const _reduceMathArguments = (fn, inner) => {
	if (!_transforms.functions) return null;
...
const _minifyHash = (text) => {
	if (!_transforms.colors) return null;
...
const _minifyColorFunction = (fn, inner, hexAlpha) => {
	if ((fn !== "rgb" && fn !== "rgba") || !_transforms.colors) return null;
...
// Where each browser's version sits in a `SUPPORT_PROFILES` row. Built from the
// order the rows are stated in, so the two cannot drift apart.
/** @type {Map<string, number>} */
...
const _readsAll = (profile) => {
	...
	const since = SUPPORT_PROFILES[profile];
	for (const [browser, versions] of parsed) {
		const at = SUPPORT_BROWSER_SLOT.get(browser);
...
const _neededPrefixes = (table, name) => {
	...
	if (prefixes !== undefined) {
		...
		for (const [prefix, windows] of prefixes) {
			for (const [browser, from, to] of PREFIX_WINDOWS[windows]) {
...
const _minifyAnPlusB = (name, inner) => {
	if (!_transforms.selectors) return `${name}(${inner})`;
...
const _unquoteFontFamily = (raw) => {
	if (!_transforms.quotes) return null;
...
const _inKeywordOnlyValue = () => {
	...
	return (
		KEYWORD_ONLY_PROPERTIES.has(property) || COLOR_ONLY_PROPERTIES.has(property)
	);
};
...
const _minifyUrlFunction = (fn, inner) => {
	if (fn !== "url" || !_transforms.quotes) return null;
	...
	// A `data:` payload the renderer rewrote: quote it as written, since the
	// unquoted form below cannot carry what a document payload contains.
	const rendered = _renderDataUrl(body);
...
const _minifyIdentEscapes = (text) => {
	if (!_transforms.escapes) return text;
...
const _minifyString = (raw) => {
	if (!_transforms.quotes) return raw;
	const quote = raw.charCodeAt(0);
	const n = raw.length;
 ...
 * @returns {void}
 */
const _lowercasePseudoNames = (parts) => {
	if (!_transforms.lowercase) return;
	for (let i = 1; i < parts.length; i++) {
		const part = parts[i];
		if (part.length === 0 || parts[i - 1] !== ":" || part.includes("(")) {
			continue;
		}
		parts[i] = asciiLowerCaseName(part);
	}
};
...
const _dropLegacyPseudoElementColon = (parts) => {
	if (!_transforms.selectors) return;
...
const _minifyUnicodeRange = (range) => {
	if (!_transforms.numbers) return range;
...
const _useRangeSpelling = (parts) => {
	if (!_transforms.mediaQueries) return;
...
const _numberAlphaValue = (property, value) => {
	if (!_transforms.numbers) return value;
...
const _dropDefaultSecondValue = (property, value) => {
	if (!_transforms.shorthands) return value;
...
const _dropDefaultGradientDirection = (fn, inner) => {
	if (!LINEAR_GRADIENTS.has(fn) || !_transforms.functions) return null;
	const comma = inner.indexOf(",");
...
const _collapseShorthand = (path, property, node, writer) => {
	if (!_transforms.shorthands) return null;
...
const _joinRuleEntries = (before, entry, owned) => {
	if (!_transforms.rules) return null;
...
const _dropZeroLengthUnit = (fragment) =>
	_transforms.numbers && _ZERO_LENGTH_RE.test(fragment) ? "0" : fragment;
...
const _dropZeroLengthUnitInCall = (fragment) => {
	if (!_transforms.numbers) return fragment;
...
const _canonicalSelectorList = (prelude) => {
	if (!_transforms.selectors) return prelude;
...
const _rulePrelude = (path, writer, minify, outParts) => {
	...
	if (path.type() === T_AT_RULE) {
		...
		const lower =
			minify && _transforms.lowercase && !equalsLowerCase(name, "charset");
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const printed = CANONICAL_NAMES.get(fn);
			const out = !_transforms.lowercase
				? name
				: printed === undefined
					? fn
					: printed;
			...
			const oneAxis = _transforms.functions
			...
			if (path.inValue() && !_inSubstitutedValue) {
				...
				const omitted = _transforms.functions
		...
		case T_URL: {
			...
			const written = raw.slice(0, open + 1);
			const head = _transforms.lowercase
				? asciiLowerCaseName(written)
				: written;
			...
			if (_renderEmbeddedSource !== undefined) {
				...
				if (renderedUrl !== null) {
					return `${raw.slice(0, open + 1)}${_serializeUrl(renderedUrl, '"')})`;
				}
			}
			...
			if (
				...
				return head === written ? raw : head + raw.slice(open + 1);
			}
			...
			return `${head}${
				percent && _transforms.escapes && !value.includes("\\")
		...
		case T_IDENT: {
			...
			if (
				...
				const shorter = _transforms.colors
					? COLOR_NAME_TO_SHORTEST.get(toLowerCaseIfNeeded(raw))
					: undefined;
				if (shorter !== undefined && _inColorOnlyProperty()) return shorter;
			}
			// A property whose value is keywords alone — or a color, which is
			// keywords and numbers — names nothing of the author's, so an identifier
			// standing directly in one of its values is a keyword and matches ASCII
			// case-insensitively. Directly only: a call's arguments are read against
			// the function's own grammar, where a name may be the author's. Asked
			// last, after the ident is known to carry a capital, which is what keeps
			// the property lookup off every value.
			if (
				path.inValue() &&
				_transforms.lowercase &&
				!_inCustomProperty &&
				!_inSupportsPrelude &&
				!_inSubstitutedValue &&
				path.parent === _valueDeclaration
			) {
				const lowered = asciiLowerCaseName(raw);
				if (lowered !== raw && _inKeywordOnlyValue()) return lowered;
			}
		...
		case T_SIMPLE_BLOCK: {
			...
			if (structural && open === "(") {
				...
				if (trim === _TRIM_COMBINATORS) _lowercasePseudoNames(parts);
			}
		...
		case T_DECLARATION: {
			...
			if (minify && _transforms.rules && count === 0 && !custom) return "";
			let value = "";
			if (count !== 0) {
			...
			// Property names match ASCII case-insensitively; a custom property's
			// does not, which is what `custom` already stands for.
			const printedName =
				minify && _transforms.lowercase && !custom
		...
		case T_QUALIFIED_RULE: {
			...
			if (minify && _transforms.rules && items.length > 1) {
				superseded = new Set();
				/** @type {Map<string, number>} */
				const seen = new Map();
				/** @type {Map<string, number[]>} */
...
const _renderDataUrl = (url) => {
	...
	let rendered;
	try {
		rendered = _renderEmbeddedSource(payload, { type, hostType: CSS_TYPE });
	} catch (_err) {
		return null;
	}
	// Anything but text is a renderer that did not answer, not a payload to write.
	if (typeof rendered !== "string" || rendered === payload) return null;
	return buildDataURI(parsed, rendered);
};
