...
const {
	ABSOLUTE_UNIT_SCALE,
	ANGLE_UNITS,
	BACKGROUND_POSITION_PROPERTIES,
	BOX_FAMILY_PREFIX,
	BOX_LONGHANDS,
	BOX_SHORTHANDS,
...
// …of those, the ones a backslash alone escapes into a url-token.
const _URL_ESCAPE_RE = /[ "'()]/g;

// …and the ones it does not: a control code point takes a hex escape that is
// never shorter, and a backslash already there is an escape this printer did
// not write, so the string keeps its quotes.
// eslint-disable-next-line no-control-regex -- a control code point is exactly what a backslash cannot escape into a url-token
const _UNESCAPABLE_URL_RE = /[\\\u0000-\u001F\u007F]/;
...
const _minifyUrlFunction = (name, fn, inner) => {
	...
	// costs a byte where the two quotes cost two — so exactly one of them is
	// shorter escaped. A control code point needs a hex escape that never is,
	// and a body already carrying an escape is left as the string it is.
	if (_UNESCAPABLE_URL_RE.test(body)) return null;
	const escaped = body.replace(_URL_ESCAPE_RE, "\\$&");
	return escaped.length < body.length + 2 ? `${name}(${escaped})` : null;
};
 ...
 * `<repeat-style>` keywords are the one-value form, and a `<bg-position>` of
...
const _collapseRepeatedPair = (property, components) => {
	...
	if (
		components.length !== 0 &&
		components.length <= 2 &&
		BACKGROUND_POSITION_PROPERTIES.has(property) &&
		components.every((one) => equalsLowerCase(one, "center"))
	) {
		return ["50%"];
	}
	return null;
