...
const { BOX_SHORTHANDS } = require("./shorthands");
...
// Named CSS colors that can be the shortest opaque form (name → [r, g, b]). A
// verified subset — any color without an entry just stays hex, which is safe.
/** @type {{ [name: string]: [number, number, number] }} */
const _NAME_TO_RGB = {
	red: [255, 0, 0],
	tan: [210, 180, 140],
	navy: [0, 0, 128],
...
const _RGB_TO_NAME = new Map();
for (const name of Object.keys(_NAME_TO_RGB)) {
	const [r, g, b] = _NAME_TO_RGB[name];
	const key = (r << 16) | (g << 8) | b;
	const prev = _RGB_TO_NAME.get(key);
	if (prev === undefined || name.length < prev.length) {
		_RGB_TO_NAME.set(key, name);
	}
}

/**
 * The shortest opaque color text for (r, g, b): a collapsed 3-hex, a 6-hex, or a
 * named color when strictly shorter.
...
const _shortestColor = (r, g, b) => {
	...
	const hex =
		rr[0] === rr[1] && gg[0] === gg[1] && bb[0] === bb[1]
			? `#${rr[0]}${gg[0]}${bb[0]}`
			: `#${rr}${gg}${bb}`;
	const name = _RGB_TO_NAME.get((r << 16) | (g << 8) | b);
	return name !== undefined && name.length < hex.length ? name : hex;
};
...
const _minifyIdentEscapes = (text) => {
	...
	while (i < n) {
		...
		// consumed as its terminator rather than being part of the name.
		let end = digitEnd;
		if (end < n && _isWhiteSpace(text.charCodeAt(end))) end++;
...
const _SUBSTITUTION_RE = /(?:^|[^\w-])(?:var|env)\(|--[\w-]*\(/i;
 ...
 */
const _collapseBoxShorthand = (path, children, writer) => {
...
const _collapseShorthand = (path, property, children, writer) => {
	if (BOX_SHORTHANDS.has(property)) {
		return _collapseBoxShorthand(path, children, writer);
	}
	...
	switch (path.type()) {
		...
		case T_SIMPLE_BLOCK: {
			...
			const inner = _join(
				parts,
				!minify,
				structural && open === "(" ? _TRIM_CONDITIONS : _TRIM_NOTHING
			);
		...
		case T_QUALIFIED_RULE: {
			...
			const nl = minify ? "" : "\n";
			let body = "";
			for (const item of items) body += nl + writer.get(item);
