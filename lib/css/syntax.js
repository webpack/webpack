...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_RAW:
			...
			// again before a `}`).
			// Its parent counts it among its children with a body, so it leaves the
			// entry that says it carries no rule — without one, the children after it
			// read the entry belonging to the next.
			if (minify) _blockSpans.push(_NO_BLOCK_ENTRY);
			return `${path.source()};`;
		case T_HASH: {
			const raw = path.source();
