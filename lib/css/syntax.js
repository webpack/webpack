...
// The top-level rules held back, so a later one printing the same block can join
// its selectors (see `_mergeSiblingRules` — the same merge, across nodes the
// writer takes one at a time). A window and not the whole sheet: what is held is
// what has not been written out yet.
/**
 * @typedef {object} HeldRule
 * @property {number} offset the node's source offset
...
/** @type {HeldRule[]} */
const _heldTopLevel = [];
// Where the last rule printing each block stands, counted from the start of the
// stylesheet so a position outlives the shifts writing the window out does.
/** @type {Map<string, number>} */
const _heldByBlock = new Map();
// How many rules the window has already written out.
let _heldBase = 0;
// How much text it holds, and the budget past which its front is written out:
// a duplicate block is usually written within a few rules, and the sheet must
// not be assembled in memory before any of it is emitted.
let _heldBytes = 0;
const HELD_BUDGET = 16384;

/**
 * Write out the whole window, in order.
 * @param {PrintContext} writer the print context
 * @returns {void}
 */
const _flushHeld = (writer) => {
	for (let i = 0; i < _heldTopLevel.length; i++) {
		const held = _heldTopLevel[i];
		writer.takeText(held.entry.text, held.offset, held.line, held.column);
	}
	_heldTopLevel.length = 0;
	_heldByBlock.clear();
	_heldBase = 0;
	_heldBytes = 0;
};
...
/**
 * Write out the front of the window until it is back inside its budget.
 * @param {PrintContext} writer the print context
 * @returns {void}
 */
const _trimHeld = (writer) => {
	let out = 0;
	while (_heldBytes > HELD_BUDGET && out < _heldTopLevel.length - 1) {
		const held = _heldTopLevel[out];
		writer.takeText(held.entry.text, held.offset, held.line, held.column);
		_heldBytes -= held.entry.text.length;
		// The map is the window's own index, so a rule leaving it takes its key —
		// unless a later rule printing the same block has already claimed it.
		const entry = held.entry;
		if (entry.plain && !entry.atRule) {
			const block = entry.text.slice(entry.prelude);
			if (_heldByBlock.get(block) === _heldBase + out) {
				_heldByBlock.delete(block);
			}
		}
		out++;
	}
	if (out === 0) return;
	_heldTopLevel.splice(0, out);
	_heldBase += out;
};

/**
 * Add a rule to the window, writing its front out if that puts it over budget.
 * @param {PrintContext} writer the print context
 * @param {HeldRule} held the rule to hold
 * @returns {void}
 */
const _holdTopLevel = (writer, held) => {
	const last = _heldTopLevel[_heldTopLevel.length - 1];
	// Only the rule at the back is ever offered an at-rule join, so the children
	// of the one it displaces are dead weight from here on.
	if (last !== undefined && last.entry.children !== null) {
		last.entry.children = null;
	}
	_heldTopLevel.push(held);
	_heldBytes += held.entry.text.length;
	const entry = held.entry;
	if (entry.plain && !entry.atRule) {
		_heldByBlock.set(
			entry.text.slice(entry.prelude),
			_heldBase + _heldTopLevel.length - 1
		);
	}
	if (_heldBytes > HELD_BUDGET) _trimHeld(writer);
};
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	...
	if (last !== undefined) {
		...
		if (merged !== null) {
			_heldBytes += merged.text.length - last.entry.text.length;
			last.entry = merged;
			last.owned = true;
			last.writes = null;
			return;
	...
	// A block of declarations also joins a rule further back, where nothing
	// between the two writes a property it does — the same rule the merge inside
	// a block reads (see `_mergeSiblingRules`).
	if (entry.plain && !entry.atRule && entry.prelude !== -1) {
		const block = entry.text.slice(entry.prelude);
		const at = _heldByBlock.get(block);
		if (at !== undefined && at - _heldBase < _heldTopLevel.length - 1) {
			const own = _blockWriteKeys(_blockProperties(block.slice(1, -1)));
			let blocked = false;
			for (let i = at - _heldBase + 1; i < _heldTopLevel.length; i++) {
				const held = _heldTopLevel[i];
				if (held.writes === null) held.writes = _writeKeysOf(held.entry.text);
				if (_writesSame(held.writes, own)) {
					blocked = true;
					break;
				}
			}
			const anchor = _heldTopLevel[at - _heldBase];
			const merged = blocked
				? null
				: _joinRuleEntries(anchor.entry, entry, anchor.owned);
			if (merged !== null) {
				_heldBytes += merged.text.length - anchor.entry.text.length;
				anchor.entry = merged;
				anchor.owned = true;
				anchor.writes = null;
				return;
...
const _flushTopLevel = (writer) => {
	if (_heldTopLevel.length === 0) return;
	if (writer === undefined) {
		_heldTopLevel.length = 0;
		_heldByBlock.clear();
		_heldBase = 0;
...
const _mergeSiblingRules = (items, texts, pending) => {
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (writes !== null && !entry.atRule && entry.plain) {
			const block = entry.text.slice(entry.prelude);
			const anchorAt = /** @type {Map<string, number>} */ (anchors).get(block);
			/** @type {RuleEntry | null} */
			let merged = null;
