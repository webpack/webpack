...
const _joinRuleEntries = (before, entry, owned) => {
	...
	const list = _canonicalSelectorList(`${beforePrelude},${prelude}`, false);
	return {
 ...
 * canonical order lets equal lists compress alike. `order` is passed only where
 * the whole list is the author's — across a join seam each half already has its
 * own, and re-sorting the two costs more compressed than the bytes it saves.
 * @param {string} prelude the printed selector list
 * @param {boolean} order whether to sort it as well as deduplicate it
 * @returns {string} it, or `prelude` when nothing there is shorter
 */
const _canonicalSelectorList = (prelude, order) => {
	...
	const repeated = seen.size !== list.length;
	if (!repeated && (!order || _isSorted(list))) return prelude;
	const out = [...seen];
	if (order) out.sort();
	return out.join(",");
};
...
const _rulePrelude = (path, writer, minify) => {
	...
	if (minify && path.type() === T_QUALIFIED_RULE) {
		...
		return _canonicalSelectorList(prelude, true);
	}
	return prelude;
};
