...
// sparse over the ASCII range, so a `switch (cc)` compiles to a jump table
// spanning U+0009..U+007D with the common idents falling through to `default`.
// `_charClass` collapses that to a dense 0..12 handler id per ASCII code point;
// `consumeAToken` does one table load + a compact jump table, and idents (the
// most frequent lead) dispatch directly instead of after the digit/whitespace
// misses. `_singleTT` holds the token type for the one-code-point tokens so they
// share a single handler. cc >= 128 is always an ident-start code point.
const HC_DELIM = 0;
...
const tokenToNode = (t, locConverter) => {
	switch (t.type) {
		...
		// hash / at-keyword carry no own fields: `value` strips the `#` / `@` and
		// `typeFlag` re-derives id-ness from the source (see `Token`).
		case TT_HASH:
			return new Token(T_HASH, t.start, t.end, locConverter);
		case TT_AT_KEYWORD:
			return new Token(T_AT_KEYWORD, t.start, t.end, locConverter);
		case TT_URL: {
			const ut = /** @type {CssUrlToken} */ (t);
			const url = /** @type {UrlToken} */ (
				new Token(T_URL, t.start, t.end, locConverter)
			);
			// Content range is the token's only own state; `value` slices it.
