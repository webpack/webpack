...
const _setupParse = (input, lc) => {
	...
	_distantRules = new Map();
	_distantDeclaredAt = new Map();
	_distantSeq = 0;
...
/**
 * Call `each` with the property and every name a shorthand of it would carry —
 * `margin-top` is reached by `margin` as well, so one map answers both ways.
 ...
 */
const _eachPropertyPrefix = (property, each) => {
	let from = 0;
	for (;;) {
...
const _noteDistantRule = (taken, entry) => {
	...
	for (const property of _blockProperties(block.slice(1, -1))) {
		_eachPropertyPrefix(property, note);
	}
...
const _mergeIntoDistantRule = (writer, entry) => {
	...
	for (const property of _blockProperties(block.slice(1, -1))) {
		if (_eachPropertyPrefix(property, since)) return false;
	}
	...
	// text: a `,` inside one makes this decline a merge it could have made, which
	// costs a join rather than correctness.
	if (`,${list},`.includes(`,${selector},`)) return false;
	_growSpanList(writer, found.taken, found.span, preludeNow, `,${selector}`);
	// A rule is keyed by the text it went out as, so the grown list is keyed
	// again by what it now says: left under the old text, a later copy of that
	// text would take the whole rule back, and the selectors with it.
	...
	span.key = found.taken.text.slice(span.at, span.at + span.len);
	...
	if (!rules.has(span.key)) rules.set(span.key, { taken: found.taken, span });
	return true;
};
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	...
	if (held !== null) {
		const taken = _emitHeld(writer, held);
...
/**
 * Whether two property sets can name the same declaration — a shorthand holds
 * every longhand its name prefixes, and `all` holds the lot.
...
const _mergeDistantRules = (items, texts) => {
	...
	for (let i = 0; i < items.length; i++) {
		...
		// A block repeated at a distance is already cheap to compress, so this
		// earns only where the copy it drops outweighs the selector it writes
		// instead — measured over the framework sheets, the merges below this
		// line cost more bytes than they save.
	...
	if (minify && _transforms.removeDeadRules && items.length > 1) {
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (earlier === undefined) {
				...
				const stands = !keyframe || !important;
				for (let j = stands ? earlier.length - 1 : -1; j >= 0; j--) {
...
// Whether a rule may take the selectors of one printing the same block further
// down, past what stands between them. Off by default: it reorders the cascade,
// so it holds only where nothing between declares what the block does.
...
/** @type {Map<string, number>} */
let _distantDeclaredAt = new Map();
let _distantSeq = 0;
