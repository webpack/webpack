...
class Token extends Node {
	 ...
	 * form to match keywords / export as a CSS-Modules name against. Computed on
	 * read; `unescapeIdentifier` fast-returns the value unchanged when it has no
	 * escapes (the common case), so nothing is stored per token.
	 * @returns {string} the unescaped value
	 */
	get unescaped() {
		return unescapeIdentifier(this.value);
	}
