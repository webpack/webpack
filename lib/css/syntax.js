...
const _normalizeNumericToken = (text) => {
	...
	if (_transforms.lowercaseNames) {
		...
		// up, and `asciiLowerCaseName` says an already-lowercase one folds to
		// itself by handing its argument straight back — so the table is read for
		// the shouted units alone and not for every number in the sheet.
		if (lowered !== unit) {
...
const _lowercasePseudoNames = (parts) => {
	...
	for (let i = 1; i < parts.length; i++) {
		const part = parts[i];
		if (part.length === 0 || parts[i - 1] !== ":") continue;
		// Folded first: a part that folds to itself needs no answer from the guard
		// below, and that is every part of a selector written in one case.
		const lowered = asciiLowerCaseName(part);
		if (lowered !== part && !part.includes("(")) parts[i] = lowered;
	}
};
...
const _lowercaseConditionParts = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		const part = parts[i];
		// Folded first, for the reason `_lowercasePseudoNames` folds first.
		const lowered = asciiLowerCaseName(part);
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_URL: {
			...
			// url, and every url but a shouted one folds to itself.
			let head = null;
			if (_transforms.lowercaseNames) {
		...
		case T_DECLARATION: {
			...
			const printedName =
				minify && _transforms.lowercaseNames && !custom && lowered !== name
