...
function consumeCommercialAt(input, pos, out) {
	...
	if (
		...
		pos = _consumeAnIdentSequence(input, pos);
		// Noted here so the layer merge can skip its whole pass over a sheet
		// naming none: a block that opens one is tokenized before the parent it
		// is a sibling in composes, so a false flag means there is none to find.
		if (rangeEqualsLowerCase(input, start + 1, pos, "layer")) {
			_sawLayerAtKeyword = true;
		}
...
const _mergeNamedLayerBlocks = (texts) => {
	if (!_transforms.mergeRules || !_sawLayerAtKeyword) return;
	/** @type {Map<string, SeenLayer> | null} */
	let first = null;
	for (let i = 0; i < texts.length; i++) {
		const text = texts[i];
		// Only a sibling starting with `@` can open one, and on a sheet naming no
		// layer that is every sibling — so the call itself is the loop's cost.
		const prelude =
			text.charCodeAt(0) === CC_AT_SIGN ? _namedLayerOpener(text) : null;
		if (prelude === null) {
...
const _dropZeroLengthUnitInCall = (fragment) => {
	...
	// `_LONE_CALL_RE` needs the whole fragment to be one call, so anything not
	// ending in `)` answers with a character read rather than a match attempt.
...
const _unwrapCalc = (fragment, property) => {
	// `_LONE_CALC_RE` is anchored on `calc(`, so a fragment starting with any
	// other letter answers with a character read rather than a match attempt.
...
const _composeBlockBody = (path, decls, rules, writer, minify, nl, owner) => {
	...
	let superseded = null;
	// One item can supersede nothing, and most rules hold one or none. The two
	// switches share the set but not each other: folding a longhand into the
	// shorthand before it is `mergeLonghands`' rewrite wherever dead
	// declarations are being dropped or not.
	...
	if (dropDead || foldLonghands) {
		...
		if (foldLonghands) {
			_foldFollowingLonghands(path, items, texts, dead);
		}
	}
...
// Set while tokenizing when the source names `@layer` at all, so the merge that
// gathers named layer blocks can leave a sheet naming none alone.
let _sawLayerAtKeyword = false;
