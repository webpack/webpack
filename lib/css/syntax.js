...
const _walkValue = (node, parent, index, writer) => {
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		if (
			...
			!_inMathFunction &&
			MATH_FUNCTIONS.has(
				toLowerCaseIfNeeded(_input.slice(_starts[i0], _aux0[i0]))
			)
		) {
			enteredMath = true;
...
const _normalizeNumericToken = (text) => {
	...
	if (_inSupportsPrelude) return num + unit;
 ...
 * @type {Map<string, (sums: Map<string, number>[]) => Map<string, number> | null>}
 */
...
const _foldMathFunction = (fn, inner) => {
	...
	const tokens = _tokenizeCalc(inner);
