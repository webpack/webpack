...
/** @type {{ node: Node, offset: number, line: number, column: number, text: string, prelude: number, atRule: boolean } | null} */
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	...
	const prelude = _rulePreludeLength.get(node);
	if (prelude === undefined) {
		// Not a rule this can join: flush what is held, then take it as it stands.
		// Its own text first — taking the held one clears the store it sits in.
		...
		if (held !== null) {
			_heldTopLevel = null;
			writer.take(held.node, held.offset, held.line, held.column, held.text);
		}
		writer.take(node, offset, line, column, own);
		return;
	}
	const text = writer.get(node);
	// A kept comment between the two was written above the second rule.
	if (held !== null && !writer.hasInsertBefore(offset)) {
		const merged =
			held.atRule === (A.type(node) === T_AT_RULE)
				? _joinRuleTexts(
						held.atRule,
						held.text.slice(0, held.prelude),
						held.text.slice(held.prelude),
						text.slice(0, prelude),
						text.slice(prelude)
					)
				: null;
		if (merged !== null) {
			held.text = merged.text;
			held.prelude = merged.prelude;
			return;
		}
	}
	if (held !== null) {
...
const _walkTopLevel = (node, writer) => {
	...
	// The held rule keeps its own prelude length, so the map is done with.
	_rulePreludeLength.clear();
...
// Each printed qualified rule -> the length of its prelude, so a parent can
// part its text without re-scanning for the `{`. Cleared per top-level node.
/** @type {Map<Node, number>} */
const _rulePreludeLength = new Map();
...
const _ATTRIBUTE = String.raw`\[(?:[^[\]"']|"(?:[^"\\]|\\[\s\S])*"|'(?:[^'\\]|\\[\s\S])*')*\]`;
 ...
 * block — the block of a condition the sheet already opened once.
 * @param {boolean} atRule whether the two are at-rules
 * @param {string} beforePrelude the earlier rule's prelude
 * @param {string} beforeBlock the earlier rule's block, braces included
 * @param {string} prelude the later rule's prelude
 * @param {string} block the later rule's block, braces included
 * @returns {{ text: string, prelude: number } | null} the one rule, or `null`
 */
const _joinRuleTexts = (atRule, beforePrelude, beforeBlock, prelude, block) => {
	if (atRule) {
		...
		if (beforeBlock.length === 2 || block.length === 2) return null;
		// TODO the two blocks' rules meet at the seam already printed, so a pair
		// there that would join is left alone; joining them needs the children's
		// texts, which the parent no longer holds.
		return {
			text: `${beforePrelude}{${beforeBlock.slice(1, -1)}${block.slice(1, -1)}}`,
			prelude: beforePrelude.length
		};
	}
	// The same selector twice matches nothing more and prints longer.
	if (beforeBlock !== block || beforePrelude === prelude) return null;
	const joined = `${beforePrelude},${prelude}`;
	return { text: joined + beforeBlock, prelude: joined.length };
};
...
const _mergeAdjacentRules = (path, items, texts) => {
	...
	for (let i = 0; i < items.length; i++) {
		...
		const length = _rulePreludeLength.get(items[i]);
		if (length === undefined) {
			previous = -1;
			continue;
		}
		if (previous !== -1 && path.type(items[previous]) === path.type(items[i])) {
			const before = texts[previous];
			const beforeLength = /** @type {number} */ (
				_rulePreludeLength.get(items[previous])
			);
			const beforePrelude = before.slice(0, beforeLength);
			const prelude = texts[i].slice(0, length);
			const merged = _joinRuleTexts(
				path.type(items[i]) === T_AT_RULE,
				beforePrelude,
				before.slice(beforeLength),
				prelude,
				texts[i].slice(length)
			);
			if (merged !== null) {
				texts[previous] = merged.text;
				_rulePreludeLength.set(items[previous], merged.prelude);
				texts[i] = "";
				continue;
			}
		}
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (minify) _mergeAdjacentRules(path, items, texts);
			...
			// Where the block is what a sibling is compared against, remember where
			// the prelude ends rather than have the parent look for the `{`. Only a
			// block of declarations: a nested rule's `&` stands for the whole list,
			// and `:is(…)` takes the specificity of its most specific selector, so
			// joining the preludes would move what the nested rules beat.
			if (
				minify &&
				path.type() === T_AT_RULE &&
				MERGEABLE_AT_RULES.has(path.name().toLowerCase())
			) {
				_rulePreludeLength.set(_currentNode, prelude.length);
			}
			if (
				minify &&
				path.type() === T_QUALIFIED_RULE &&
				...
				_isJoinablePrelude(
					prelude,
					inKeyframes,
					path.parent !== null && path.type(path.parent) === T_QUALIFIED_RULE
				)
			) {
				_rulePreludeLength.set(_currentNode, prelude.length);
			}
