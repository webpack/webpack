...
/** @type {{ node: Node, offset: number, line: number, column: number, text: string, prelude: number } | null} */
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	...
	if (
		held !== null &&
		held.text.slice(held.prelude) === text.slice(prelude) &&
		// The same selector twice matches nothing more and prints longer.
		held.text.slice(0, held.prelude) !== text.slice(0, prelude) &&
		// A kept comment between the two was written above the second rule.
		!writer.hasInsertBefore(offset)
	) {
		const joined = `${held.text.slice(0, held.prelude)},${text.slice(0, prelude)}`;
		held.text = joined + held.text.slice(held.prelude);
		held.prelude = joined.length;
		return;
	}
	if (held !== null) {
		writer.take(held.node, held.offset, held.line, held.column, held.text);
	...
	_heldTopLevel = { node, offset, line, column, text, prelude };
};
 ...
 */
const _isJoinablePrelude = (prelude, keyframe) => {
	const shape = keyframe ? _JOINABLE_KEYFRAME_RE : _JOINABLE_SELECTOR_RE;
...
const _mergeAdjacentRules = (path, items, texts) => {
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (path.type(items[i]) !== T_QUALIFIED_RULE) {
			previous = -1;
			continue;
		}
		...
		if (previous !== -1) {
			...
			if (
				before.slice(beforeLength) === texts[i].slice(length) &&
				// The same selector twice matches nothing more and prints longer.
				beforePrelude !== prelude
			) {
				const joined = `${beforePrelude},${prelude}`;
				texts[previous] = joined + before.slice(beforeLength);
				_rulePreludeLength.set(items[previous], joined.length);
				texts[i] = "";
				continue;
			}
		}
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (
				...
				_isJoinablePrelude(prelude, inKeyframes)
			) {
