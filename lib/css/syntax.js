...
const _prefixAtRule = (path, ruleText, prelude, writer) => {
	...
	// The unprefixed spelling of this rule's prelude, its cross-prefix identity.
	const signature =
		found === null ? prelude : `@${base}${prelude.slice(1 + name.length)}`;
	if (found !== null) {
		if (
			seen.has(signature) &&
...
const _prefixQualifiedRule = (path, ruleText, parts, soft, body, writer) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (
			i !== 0 &&
			parts[i - 1] === ":" &&
			(PREFIXED_SELECTORS.has(parts[i].toLowerCase()) ||
				_prefixedSelectorName(parts[i].toLowerCase()) !== undefined)
		) {
	...
	if (nameIndex === -1) return ruleText;
	const found = _prefixedSelectorName(parts[nameIndex].toLowerCase());
	const base = found === undefined ? parts[nameIndex].toLowerCase() : found[0];
	const signature = `s${_join([...parts.slice(0, nameIndex), base, ...parts.slice(nameIndex + 1)], false, _TRIM_COMBINATORS)}`;
	/**
 ...
 * @param {string[]=} outParts filled with the prelude's tokens, for the prefixer
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			// The prefixer reads the prelude's tokens, not its joined text.
			const preludeParts =
