...
const grammar = (input, visitors, writer, options) => {
	...
	if (_prefixBrowsers !== null) {
		...
		// The function, not the name: a class called `.scheme-light-dark` holds the
		// same text, and reading it as a call lowers a sheet that has none.
		_sourceNamesLightDark =
			_loweringLightDark && _input.includes("light-dark(");
		_lightDarkUsed = false;
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (
				...
				// A block already carrying the pair is one a lowering pass wrote, so
				// adding it again would say the same thing twice and grow with each.
				!body.includes(`${LIGHT_PROPERTY}:`)
			) {
