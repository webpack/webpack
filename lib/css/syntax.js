...
const grammar = (input, visitors, writer, options) => {
	...
	_seenPrefixRules =
		...
		writer.options.environment &&
		writer.options.environment.browsers
			? new Set()
			: null;
	_visitors = visitors;
...
/**
 * Point the memos at `browsers`, clearing them when the selection changed.
 * @param {string[]} browsers the browserslist selection
 * @returns {void}
 */
const _useBrowsers = (browsers) => {
	const key = browsers.join(",");
	if (key === _browsersKey) return;
	_browsersKey = key;
	_parsedBrowsersMemo = null;
...
/**
 * The `["name", version]` pairs a browserslist selection parses to, cached.
 * @param {string[]} browsers the browserslist selection
 * @returns {[string, number][]} the parsed `[name, encodedVersion]` pairs
 */
const _parsedBrowsers = (browsers) => {
	_useBrowsers(browsers);
	if (_parsedBrowsersMemo === null) {
		const parsed = /** @type {[string, number][]} */ ([]);
		for (const entry of browsers) {
			const space = entry.indexOf(" ");
			if (space === -1) continue;
			const version = _encodeBrowserVersion(entry.slice(space + 1));
			if (version !== null) parsed.push([entry.slice(0, space), version]);
		}
		_parsedBrowsersMemo = parsed;
	}
	return _parsedBrowsersMemo;
};
 ...
 * `from <= V < to`. Null when the target list is unknown (leave prefixes alone),
 ...
 */
const _neededPrefixes = (table, name, tag, writer) => {
	const environment = /** @type {CssEnvironment | undefined} */ (
		writer.options.environment
	);
	const browsers = environment ? environment.browsers : undefined;
	...
	if (prefixes !== undefined) {
		const parsed = _parsedBrowsers(browsers);
		const needed = new Set();
		for (const [prefix, entries] of prefixes) {
			for (const [browser, from, to] of entries) {
				let hit = false;
				for (const [targetName, targetVersion] of parsed) {
					if (
						targetName === browser &&
						targetVersion >= from &&
						targetVersion < to
 ...
 */
const _prefixAtRule = (path, ruleText, prelude, writer) => {
	...
	if (found !== null) {
		if (
			seen.has(signature) &&
			_prefixRemovable(PREFIXED_AT_RULES, base, found[1], "a", writer)
		) {
			return "";
		}
	...
	seen.add(signature);
	const needed = _neededPrefixes(PREFIXED_AT_RULES, base, "a", writer);
 ...
 * @param {CssPath} path the accessor on the qualified rule
 * @param {string} ruleText the rule's own serialized text
 ...
 * @returns {string} the rule text, with prefixed copies added or itself dropped
 */
const _prefixQualifiedRule = (path, ruleText, parts, soft, body, writer) => {
	const seen = /** @type {Set<string>} */ (_seenPrefixRules);
	...
	if (found !== undefined) {
		if (
			seen.has(signature) &&
			_prefixRemovable(PREFIXED_SELECTORS, base, found[1], "s", writer)
		) {
			return "";
		}
		seen.add(`${signature}\0${found[1]}`);
	...
	seen.add(signature);
	const needed = _neededPrefixes(PREFIXED_SELECTORS, base, "s", writer);
	if (needed === null) return ruleText;
	let out = "";
	for (const prefix of needed) {
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (
				...
				for (let i = 0; i < items.length; i++) {
					if (path.type(items[i]) === T_DECLARATION && texts[i].length !== 0) {
						present.add(_printedProperty(texts[i]));
					}
				}
				for (let i = 0; i < items.length; i++) {
					if (path.type(items[i]) !== T_DECLARATION || texts[i].length === 0) {
			...
			const out =
				minify && _seenPrefixRules !== null && path.parent === null
					? path.type() === T_AT_RULE
						? _prefixAtRule(path, text, prelude, writer)
			...
			if (
				...
				for (let i = 0; i < texts.length; i++) {
					if (texts[i].length === 0) continue;
					if (superseded !== null && superseded.has(i)) continue;
					children.push(_ruleEntryOf(items[i], texts[i]));
				}
				let head = "";
