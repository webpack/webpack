...
// A parse owns the module-scope walk state, so a second one started from inside
// a visitor would clobber the first.
let _parsing = false;
...
const grammar = (input, visitors, writer, options) => {
	if (_parsing) {
		throw new Error(
			"css syntax: process() is already running; a visitor cannot start another CSS parse"
		);
	}
	_parsing = true;
