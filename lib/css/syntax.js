...
/**
 * Sort a selector list and drop the selectors it repeats. Order inside a list
 * changes neither what matches nor the cascade — every selector carries its own
 * specificity — so this only groups like shapes together for the compressor.
 * @param {string} prelude the printed selector list
 * @returns {string} the sorted list, or the prelude when it is not one
 */
const _sortSelectorList = (prelude) => {
	...
	for (const selector of selectors) {
		// A list a comment emptied is not one this can reorder.
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (minify && path.type() === T_QUALIFIED_RULE) {
				prelude = _sortSelectorList(prelude);
			}
