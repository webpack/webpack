...
// The prefixed top-level rule the writer is holding, and the signature of the
// unprefixed twin that would make it dead weight. Null when the held rule is
// not one.
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	...
	// A prefixed rule whose unprefixed twin would make it dead weight is held even
	// where nothing can join it (`@-webkit-keyframes` is not a mergeable at-rule),
	// so the twin the next rule may be can still drop it.
	const holdForPrefix =
		_prefixDropCandidate !== null && _prefixDropCandidate.node === node;
	if (entry.prelude === -1 && !holdForPrefix) {
	...
	if (held !== null && !holdForPrefix && !writer.hasInsertBefore(offset)) {
...
const grammar = (input, visitors, writer, options) => {
	...
	if (browsers === undefined) {
		...
		_seenPrefixRules = _prefixBrowsers === null ? null : new Set();
	}
	_visitors = visitors;
 ...
 */
const _prefixRemovable = (table, base, prefix) => {
...
/**
 * Drop the rule the writer is holding when it is a prefixed twin of `signature`
 * no target needs. The writer holds one top-level rule back so the next can join
 * onto it, and that is exactly the lookahead the usual authoring order — the
 * prefixed spelling first, its unprefixed twin right after — asks for.
 * @param {{ node: Node, signature: string } | null} candidate the held rule's prefix candidacy
 * @param {string} signature the unprefixed rule's sibling signature
 * @returns {void}
 */
const _dropHeldPrefixTwin = (candidate, signature) => {
	if (candidate === null || candidate.signature !== signature) return;
	const held = _heldTopLevel;
	// Never a rule that has already gathered another into it: its text is then
	// several rules, and only one of them is the twin.
	if (held === null || held.node !== candidate.node || held.owned) return;
	_heldTopLevel = null;
};
 ...
 */
const _prefixAtRule = (path, ruleText, prelude, candidate) => {
	...
	if (found !== null) {
		const removable = _prefixRemovable(PREFIXED_AT_RULES, base, found[1]);
		if (seen.has(signature) && removable) return "";
		seen.add(`${signature}\0${found[1]}`);
		// Its twin may still be the next rule, which is where it is dropped.
		if (removable) _prefixDropCandidate = { node: _currentNode, signature };
		return ruleText;
	}
	seen.add(signature);
	_dropHeldPrefixTwin(candidate, signature);
 ...
 * @returns {string} the rule text, with prefixed copies added or itself dropped
 */
const _prefixQualifiedRule = (ruleText, parts, soft, body, candidate) => {
	const seen = /** @type {Set<string>} */ (_seenPrefixRules);
	...
	if (found !== undefined) {
		const removable = _prefixRemovable(PREFIXED_SELECTORS, base, found[1]);
		if (seen.has(signature) && removable) return "";
		seen.add(`${signature}\0${found[1]}`);
		// Its twin may still be the next rule, which is where it is dropped.
		if (removable) _prefixDropCandidate = { node: _currentNode, signature };
	...
	seen.add(signature);
	_dropHeldPrefixTwin(candidate, signature);
	const needed = _neededPrefixes(PREFIXED_SELECTORS, base);
	if (needed === null) return ruleText;
	let out = "";
	for (const prefix of needed) {
...
const _joinRuleEntries = (before, entry, owned) => {
	...
	if (
		beforeBlock !== block ||
		!_entryListable(before) ||
		!_entryListable(entry)
	) {
		return null;
	}
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
				...
				if (prefixable) {
					...
					for (let i = 0; i < properties.length; i++) {
						...
						if (prefix !== null) {
							...
							if (
								present.has(base) &&
								_prefixRemovable(PREFIXED_PROPERTIES, base, prefix[1])
							) {
			...
			let out = text;
			if (_seenPrefixRules !== null && path.parent === null) {
				// Read and clear here, so the lookahead is the one rule the writer
				// holds and never a rule further back.
				const candidate = _prefixDropCandidate;
				_prefixDropCandidate = null;
				out =
					path.type() === T_AT_RULE
						? _prefixAtRule(path, text, prelude, candidate)
