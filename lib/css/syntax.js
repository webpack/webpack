...
const _walkValue = (node, parent, index, writer) => {
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		const previousInterpolated = _inInterpolatedGradient;
		if (ty === T_FUNCTION && _namesInterpolationSpace(i0)) {
			_inInterpolatedGradient = true;
		}
		for (let i = vs; i < ve; i++) {
			_walkValue(_nodeRef(_flat[i]), node, i - vs, writer);
		}
		_inInterpolatedGradient = previousInterpolated;
	}
...
// A gradient, whatever it is prefixed with, and the `in <space>` its stops
// interpolate under — which stands before the first comma.
const GRADIENT_FUNCTION_RE = /(?:^|-)(?:linear|radial|conic)-gradient$/i;
const INTERPOLATION_SPACE_RE = /(?:^|[\s)])in\s+[\w-]/i;

/**
 * Whether a function is a gradient naming the color space its stops
 * interpolate in.
 * @param {number} i0 the function's node index
 * @returns {boolean} true while its colors have to stay as written
 */
const _namesInterpolationSpace = (i0) => {
	const name = _input.slice(_starts[i0], _aux0[i0]);
	if (!GRADIENT_FUNCTION_RE.test(name)) return false;
	const open = _aux0[i0] + 1;
	const comma = _input.indexOf(",", open);
	const end = comma === -1 || comma > _ends[i0] ? _ends[i0] : comma;
	return INTERPOLATION_SPACE_RE.test(_input.slice(open, end));
};
...
const _walkRule = (node, parent, index, writer) => {
	...
	if (!skip) {
		if (ty === T_AT_RULE || ty === T_QUALIFIED_RULE) {
			...
			if (
				_input.startsWith("--", _starts[i0]) ||
				(_inPropertyRule &&
					rangeEqualsLowerCase(_input, _starts[i0], _aux0[i0], "initial-value"))
			) {
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const color =
				_inSupportsPrelude || _inInterpolatedGradient
					? null
...
// Whether the value being printed sits inside a gradient that names the color
// space its stops interpolate in. A color rewritten there is converted through
// sRGB, and the engine maps the result back into that space as a different
// point — so the ramp between the stops is not the one the source paints.
let _inInterpolatedGradient = false;
...
const NAMESPACE_AT_RULE_RE = /@namespace/i;
