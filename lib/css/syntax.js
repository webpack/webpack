 ...
 * @property {boolean=} mergeDistantRules give a rule the selectors of a later one printing the same block, and an at-rule the block of a later one stating the same condition, past the rules between them; off by default, since it reorders the cascade and is sound only where nothing between declares what the block being moved does
...
const _gatherDistantAtRule = (
	...
	const declared = _atRuleBodyProperties(entry);
...
/**
 * The properties a mergeable at-rule's block declares, or `null` where they
 * cannot all be read — a child at-rule, or one whose own block holds a rule.
 * @param {RuleEntry} entry a mergeable at-rule
 * @returns {Set<string> | null} the property names, or null when unanswerable
 */
const _atRuleBodyProperties = (entry) => {
	const children = entry.children;
	...
	for (const child of children) {
		...
		const body = child.text.slice(child.prelude + 1, -1);
		for (const property of _blockProperties(body)) declared.add(property);
	}
	return declared;
};
