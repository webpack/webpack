...
let _printing = false;
// Top-level selectors / at-rules met so far in this walk, so a prefixed rule can
// be added or dropped against the sibling it needs without buffering the whole
// stylesheet (streaming holds ~one node). Null unless minifying for a target.
// Markers: `signature` (the unprefixed rule is present) and `signature\0prefix`.
...
// Whether every entry of that selection is a browser the tables know. One they
// do not hides what it needs, so only adding a prefix stays sound.
let _prefixRemovalSound = false;
...
const grammar = (input, visitors, writer, options) => {
	...
	if (browsers === undefined) {
		...
		_prefixRemovalSound = false;
	} else {
		_seenPrefixRules = new Set();
...
let _knownBrowsersMemo = false;
/** @type {Map<string, Set<string> | null>} */
let _neededPrefixMemo = new Map();

/**
 * Point the memos at `browsers`, parsing the selection when it changed, and
 * make it the one this parse prefixes for.
...
const _useBrowsers = (browsers) => {
	...
	if (key !== _browsersKey) {
		...
		for (const entry of browsers) {
			...
			if (version === null || !PREFIX_BROWSERS.has(name)) {
				// A browser the tables cannot resolve — a browserslist name BCD has no
				// entry for (`op_mini`, `and_uc`, `ie_mob`, …), or a version that did
				// not parse. Nothing states what it needs, so removal stops being sound
				// for the whole selection; adding is unaffected.
				known = false;
				continue;
			}
		...
		_parsedBrowsersMemo = parsed;
		_knownBrowsersMemo = known;
	}
	_prefixBrowsers = _parsedBrowsersMemo;
	_prefixRemovalSound = _knownBrowsersMemo;
};
...
/**
 * The vendor prefixes at least one target browser still needs for a construct —
 * a target at version V needs prefix P when some `[browser, from, to]` of P has
 * `from <= V < to`. Null when not minifying for a target (leave prefixes alone),
 * the construct is never prefixed, or no target needs any of its prefixes.
 ...
 * @param {string} tag a per-axis cache-key prefix, so two axes' names never collide
 * @returns {Set<string> | null} the needed prefixes, or null
 */
const _neededPrefixes = (table, name, tag) => {
	const parsed = _prefixBrowsers;
	if (parsed === null) return null;
	const cache = _neededPrefixMemo;
	const key = tag + name;
...
/**
 * Whether a present prefixed construct is dead weight: its prefix is one the
 * table knows for the base construct and no target browser needs it, so an
 * unprefixed sibling already covers every target. Never, once the selection
 * named a browser the tables cannot resolve — a prefix it needs would read as
 * one nothing needs.
 * @param {Map<string, [string, [string, number, number][]][]>} table its axis' prefix table
 * @param {string} base the unprefixed construct name
 * @param {string} prefix the vendor prefix found on it
 * @param {string} tag the axis cache-key prefix
 * @returns {boolean} true when the prefixed construct can be dropped
 */
const _prefixRemovable = (table, base, prefix, tag) => {
	if (!_prefixRemovalSound) return false;
	const prefixes = table.get(base);
	if (prefixes === undefined) return false;
	if (!prefixes.some(([known]) => known === prefix)) return false;
	const needed = _neededPrefixes(table, base, tag);
	return needed === null || !needed.has(prefix);
};
 ...
 * `@keyframes x` pair up. `_seenPrefixRules` is the running sibling set.
 ...
 */
const _prefixAtRule = (path, ruleText, prelude) => {
	const seen = /** @type {Set<string>} */ (_seenPrefixRules);
	const name = path.name().toLowerCase();
	const found = VENDOR_PREFIX.exec(name);
	const base = found === null ? name : name.slice(found[1].length);
	...
	if (found !== null) {
		if (
			seen.has(signature) &&
			_prefixRemovable(PREFIXED_AT_RULES, base, found[1], "a")
		) {
			return "";
		}
		seen.add(`${signature}\0${found[1]}`);
		return ruleText;
	}
	seen.add(signature);
	const needed = _neededPrefixes(PREFIXED_AT_RULES, base, "a");
	if (needed === null) return ruleText;
	let out = "";
	for (const prefix of needed) {
		// Skip only a prefix the source itself already carries (marked when the
		...
		if (seen.has(`${signature}\0${prefix}`)) continue;
		out += `@${prefix}${ruleText.slice(1)}`;
	}
	return out + ruleText;
};
...
const _prefixedSelectorName = (name) => {
	if (_prefixedSelectorNames === null) {
		...
		for (const [base, prefixes] of PREFIXED_SELECTORS) {
			for (const [prefix] of prefixes) {
				_prefixedSelectorNames.set(prefix + base, [base, prefix]);
			}
		}
	}
 ...
 */
const _printedProperty = (text) => {
	const colon = text.indexOf(":");
	return toLowerCaseIfNeeded(colon === -1 ? text : text.slice(0, colon));
};

/**
 * A qualified rule prefixed against the target when its selector is a single
 * prefixable pseudo (`::placeholder`): a copy with the pseudo's engine spelling
 * is prepended for each prefix a target needs, and a prefixed-only rule no
 * target needs is dropped once its unprefixed twin has been seen. A selector
 * list is left alone — a pseudo one engine rejects would drop the whole list, so
 * it must be split, which this does not do. The author's colons carry over.
 * @param {string} ruleText the rule's own serialized text
 ...
 * @returns {string} the rule text, with prefixed copies added or itself dropped
 */
const _prefixQualifiedRule = (ruleText, parts, soft, body) => {
	const seen = /** @type {Set<string>} */ (_seenPrefixRules);
	// A comma is a selector list; the name index is the sole pseudo's ident, which
	// must sit right after a `:` so a class of the same spelling is not mistaken
	// for it.
	// A pseudo name matches ASCII case-insensitively, as property and at-rule
	// names do, so it is lowercased before every table lookup.
	let nameIndex = -1;
	for (let i = 0; i < parts.length; i++) {
		if (parts[i] === ",") return ruleText;
		if (
			i !== 0 &&
			parts[i - 1] === ":" &&
			(PREFIXED_SELECTORS.has(_selectorPartName(parts[i]).toLowerCase()) ||
				_prefixedSelectorName(_selectorPartName(parts[i]).toLowerCase()) !==
					undefined)
		) {
			if (nameIndex !== -1) return ruleText;
			nameIndex = i;
		}
	}
	if (nameIndex === -1) return ruleText;
	...
	const raw = parts[nameIndex];
	const nameOnly = _selectorPartName(raw).toLowerCase();
	const args = raw.slice(_selectorPartName(raw).length);
	const found = _prefixedSelectorName(nameOnly);
	const base = found === undefined ? nameOnly : found[0];
	const signature = `s${_join([...parts.slice(0, nameIndex), base + args, ...parts.slice(nameIndex + 1)], false, _TRIM_COMBINATORS)}`;
	/**
	 * @param {string} name the pseudo's spelling to print with
	 * @returns {string} the rule with that pseudo
	 */
	const withName = (name) => {
		const swapped = [...parts];
		swapped[nameIndex] = name;
		return `${_join(swapped, false, _TRIM_COMBINATORS)}${soft}{${body}}`;
	};
	if (found !== undefined) {
		if (
			seen.has(signature) &&
			_prefixRemovable(PREFIXED_SELECTORS, base, found[1], "s")
		) {
			return "";
		}
		seen.add(`${signature}\0${found[1]}`);
	...
	seen.add(signature);
	const needed = _neededPrefixes(PREFIXED_SELECTORS, base, "s");
	if (needed === null) return ruleText;
	let out = "";
	for (const prefix of needed) {
		// Skip only a prefix the source itself already carries (marked below when
		// the prefixed rule is met); a copy is still added for every unprefixed rule
		// of this signature, so a later one that overrides an earlier keeps its
		// prefixed twin winning too.
		if (seen.has(`${signature}\0${prefix}`)) continue;
		out += withName(prefix + base + args);
	}
	return out + ruleText;
};
 ...
 * @property {boolean} listable whether its prelude may also join another's
 * selector list, which a plain block under a plain selector may
...
const _joinRuleEntries = (before, entry, owned) => {
	...
	if (before.atRule) {
		if (beforePrelude !== prelude) return null;
	...
	const beforeBlock = before.text.slice(before.prelude);
	...
	if (beforePrelude === prelude) {
		...
		if (beforeBlock === block) return before;
		...
		const body = beforeBlock.slice(1, -1);
		const rest = block.slice(1, -1);
		...
		return {
			...
			atRule: false,
			plain: entry.plain,
			listable: before.listable && entry.listable,
			children: null,
			head: ""
		};
	}
	if (beforeBlock !== block || !before.listable || !entry.listable) return null;
	// Concatenated, not merged: merging gains no gzip byte and costs 13-16% of the
	// minify. Re-minifying may reorder a list, so comparers canonicalize.
	const list = `${beforePrelude},${prelude}`;
	return {
		text: list + beforeBlock,
		prelude: list.length,
		atRule: false,
		plain: true,
		listable: true,
 ...
 * @returns {void}
 */
const _mergeAdjacentRules = (items, texts) => {
	let previous = -1;
	/** @type {RuleEntry | null} */
	let held = null;
...
/**
 * @param {string[]} list any list
 * @returns {boolean} whether it is already in ascending order
 */
const _isSorted = (list) => {
...
const _canonicalSelectorList = (prelude) => {
	...
	if (seen.size === list.length && _isSorted(list)) return prelude;
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			const preludeParts =
				_seenPrefixRules !== null ? /** @type {string[]} */ ([]) : undefined;
			...
			if (minify) _mergeAdjacentRules(items, texts);
			...
			if (_prefixBrowsers !== null) {
				...
				for (let i = 0; i < items.length; i++) {
					...
					const property = _printedProperty(texts[i]);
				...
				if (prefixable) {
					...
					for (let i = 0; i < properties.length; i++) {
						if (properties[i].length !== 0) present.add(properties[i]);
					}
					for (let i = 0; i < properties.length; i++) {
						const property = properties[i];
						if (property.length === 0) continue;
						const prefix = VENDOR_PREFIX.exec(property);
						if (prefix !== null) {
							const base = property.slice(prefix[1].length);
							if (
								present.has(base) &&
								_prefixRemovable(PREFIXED_PROPERTIES, base, prefix[1], "p")
							) {
						...
						const needed = _neededPrefixes(PREFIXED_PROPERTIES, property, "p");
						if (needed === null) continue;
						let add = "";
						for (const missing of needed) {
							if (!present.has(missing + property)) {
								add += nl + missing + texts[i];
							}
						}
						if (add.length !== 0) {
			...
			// Top-level only: `_seenPrefixRules` has no nesting key, so a nested
			// `@keyframes` / pseudo must not add or drop against a same-named
			// top-level twin in another scope. Declarations are prefixed above.
			const out =
				_seenPrefixRules !== null && path.parent === null
					? path.type() === T_AT_RULE
						? _prefixAtRule(path, text, prelude)
			...
			if (minify && path.type() === T_QUALIFIED_RULE) {
				...
				const parent = path.parent;
				// A keyframe selector is a list of its own shape, which `_rulePrelude`
				// has already rewritten `from` in.
				const inKeyframes =
					parent !== null &&
					path.type(parent) === T_AT_RULE &&
					KEYFRAMES_AT_RULE_RE.test(path.name(parent));
				const listable =
					...
					_isJoinablePrelude(
						prelude,
						inKeyframes,
						parent !== null && path.type(parent) === T_QUALIFIED_RULE
					);
				_ruleEntry.set(_currentNode, {
					text,
					prelude: prelude.length,
