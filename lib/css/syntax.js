...
/**
 * How a function's arguments are read, by the name `MATH_FUNCTION_FOLD` gives:
 * every reader answers `[unit, values]`, so an operation below never has to know
 * which one ran. `eighth-turn` answers a lookup key rather than a magnitude —
 * `lookup` is the operation that takes it.
 * @type {Map<string, (sums: Map<string, number>[]) => [string, number[]] | null>}
 */
const _ARGUMENT_READERS = new Map([
	["same-unit", _comparableCoefficients],
	[
		"number",
		(sums) => {
			const comparable = _comparableCoefficients(sums);
			return comparable === null || comparable[0] !== "" ? null : comparable;
		}
	],
	[
		"eighth-turn",
		(sums) => {
			...
			return eighth === null ? null : ["", [eighth]];
		}
	]
]);

/**
 * The arithmetic each `apply` name stands for, one entry per name however many
 * functions select it — the six trig ones all select `lookup`. Every one is a
 * number or `null`, and `null` leaves the call written out: the result has to be
 * a value a stylesheet can hold, which most of these are not for most inputs.
 * @type {Map<string, (values: number[], keyword: string, table: Map<number, number> | null) => number | null>}
 */
const _MATH_OPERATIONS = new Map([
	["minimum", (values) => Math.min(...values)],
	["maximum", (values) => Math.max(...values)],
	// CSS Values 4 §10.4: the lower bound wins a contradictory pair.
	["clamp", ([lower, value, upper]) => Math.max(lower, Math.min(value, upper))],
	["absolute", ([value]) => Math.abs(value)],
	// The one that changes the unit: a sign is a `<number>`. Every unit reaching
	// here scales by a positive factor, so the coefficient's sign is the value's.
	...
	[
		...
		([value, step], strategy) =>
			...
			step > 0 ? _roundToStep(value, step, strategy) : null
	],
	[
		"modulus",
		([value, divisor]) => {
	...
	[
		...
		([value, divisor]) => {
			if (divisor === 0) return null;
			// `rem()` takes the dividend's sign, which is what `%` already does, and
			// declines the same zero boundary as `mod()`.
			const remainder = value % divisor;
			return remainder === 0 ? null : remainder;
		}
	],
	["squareRoot", ([value]) => _exactSquareRoot(value)],
	["power", ([base, exponent]) => _exactIntegerPower(base, exponent)],
	[
		"logarithm",
		([value, base]) => {
	...
	// `e` is not a double, so every other power of it is a number this cannot
	// write down and an engine's math library rounds its own way.
	["exponential", ([value]) => (value === 0 ? 1 : null)],
	[
		"lookup",
		([key], _keyword, table) => {
	...
	[
		"arcTangent2",
		([y, x]) => {
			// The eight directions the answer is a whole number of degrees in, an
			// eighth turn apart. Both zero is refused: the spec leaves it to the
			// engine.
...
const _foldMathFunction = (fn, inner) => {
	...
	if (fn === "calc") return _printCalcSum(sums[0]);
	const fold = MATH_FUNCTION_FOLD.get(fn);
	if (fold === undefined) return null;
	const read = _ARGUMENT_READERS.get(fold.read);
	const apply = _MATH_OPERATIONS.get(fold.apply);
	if (read === undefined || apply === undefined) return null;
	const argument = read(sums);
	if (argument === null) return null;
	const value = apply(argument[1], keyword, fold.table);
