 ...
 * Returning the input unchanged declines it, and the URL is then emitted
...
const _minifyUrlFunction = (name, fn, inner) => {
	...
	if (rendered !== null) return `${name}(${mark}${rendered}${mark})`;
	...
	switch (path.type()) {
		...
		case T_URL: {
			...
			if (_renderEmbeddedSource !== undefined) {
				...
				if (renderedUrl !== null) {
					return `${raw.slice(0, open + 1)}${renderedUrl})`;
				}
			}
