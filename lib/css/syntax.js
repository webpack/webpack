...
// under, so a rule elsewhere under the same conditions keys the same. Null where
// an anonymous `@layer` encloses it: that is a new layer each time it is
// written, so nothing inside two of them is the same rule said twice.
/** @type {(string | null)[]} */
const _frameChainKey = Array.from({ length: _STREAM_MAX_DEPTH }, () => null);
/**
 * @typedef {{ key: string, at: number, len: number }} RuleSpan
 * @typedef {{ bodyAt: number, prelude: string, keyPrelude: string, spans: RuleSpan[] }} BlockSpans
 */
// Where each finished block's rules landed in it, in the order the blocks
// finished. A node id is a slot the next top-level node reuses, so a block is
// found by the order it printed in rather than by which id it holds: its parent
// takes one off for each child that has a body, youngest last.
/** @type {BlockSpans[]} */
const _blockSpans = [];
 ...
 */
// Every rule taken so far, by its key and conditions, to where it sits. Sheet-
// wide: a rule is as dead from a copy in another block as from one beside it.
...
const _streamOpen = (d, rule) => {
	...
	_frameChainKey[d] = !minify || above === null ? null : `${above}${opener}`;
...
const _drainTopLevelSpans = (node) => {
	...
	return _nodeTypeOf(node) === T_QUALIFIED_RULE ? _WHOLE_TEXT_SPANS : own;
};
...
const _cutSpan = (writer, taken, span) => {
	...
	taken.text = `${taken.text.slice(0, span.at)}${taken.text.slice(span.at + span.len)}`;
	...
	for (const other of taken.spans) {
		if (other.at > span.at) other.at -= span.len;
	}
	span.len = 0;
};
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	...
	const spans = _drainTopLevelSpans(node);
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			/** @type {RuleSpan[] | null} */
			let spans = null;
			// One per child with a body, in the order they finished.
			let bodies = 0;
			for (const item of items) {
				if (path.type(item) !== T_DECLARATION) bodies++;
			}
			const childSpans =
				bodies === 0 ? _NO_BLOCK_SPANS : _blockSpans.splice(-bodies, bodies);
			let atChild = 0;
			for (let i = 0; i < texts.length; i++) {
				const isBlock = path.type(items[i]) !== T_DECLARATION;
			...
			_blockSpans.push({
				...
				prelude,
				keyPrelude: `${prelude}{`,
				spans: spans === null ? _NO_RULE_SPANS : spans
			});
