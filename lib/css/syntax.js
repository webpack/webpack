...
const consumeAQualifiedRule = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		...
		if (skip) {
			...
			// The url visitor keeps url and function tokens (a function may hold one,
			// as `:unknown(url(x))` does), and a `(` / `[` block has to stay
			// balanced; every other selector leaf has no non-modules consumer.
			if (
				tt === TT_FUNCTION ||
				tt === TT_URL ||
