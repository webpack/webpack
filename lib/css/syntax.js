 ...
 * @property {boolean=} dropOverriddenDeclarations drop a declaration a later one in the same block overrides even when spelled differently, which loses a fallback pair (default false)
 */
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			// `dropOverriddenDeclarations` widens this to any earlier declaration of
			// the same property, which is what csso and cssnano expose.
			...
			if (minify && items.length > 1) {
				...
				for (let i = 0; i < items.length; i++) {
					...
					const key = _dropOverriddenDeclarations
						? toLowerCaseIfNeeded(path.name(items[i]))
						: texts[i];
					const at = seen.get(key);
					// A later plain declaration does not override an earlier
					// `!important` one, so that one is still read and stays.
					if (
						at !== undefined &&
						(!_dropOverriddenDeclarations ||
							!path.important(items[at]) ||
							path.important(items[i]))
					) {
						superseded.add(at);
					}
					seen.set(key, i);
...
// Whether a differently-spelled declaration a later one overrides may be
// dropped — off by default: such a pair is usually a fallback.
