...
const _dropDeadRulesOnce = (css) => {
	...
	const visitors = {
		[NodeType.AtRule]: {
			enter: (/** @type {CssPath} */ path) => chain.push(prelude(path)),
			exit: () => chain.pop()
		},
...
const dropDeadRules = (css) => {
	...
	const cut = _dropDeadRulesOnce(css);
	if (cut === css) return css;
	// A cut can leave the block around it empty. Which empty at-rules may go is
	// the printer's own answer — `@keyframes`, `@page` and `@scope` are each kept
	// there for a reason — so it is asked rather than repeated here. Printing an
	// already printed sheet changes nothing else.
	return new SourceProcessor().process(cut, { mode: "minify" }).code;
};
