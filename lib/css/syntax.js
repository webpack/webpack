 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "convertApproximateColors" | "mergeDistantRules" | "rewriteCustomProperties" | "renderEmbeddedSource" | "deferEmbeddedSource" | "transforms" | "unusedSymbols" | "pseudoClasses">} CssPrintOptions
 */
...
const _mergeDistantRules = (items, texts) => {
	...
	for (let i = 0; i < items.length; i++) {
		...
		// TODO join two `@media` a rule stands between as well, which is this
		// same question asked of every property the body declares — csso loses
		// ~12,900 Tailwind classes to doing it without asking it.
		if (entry.prelude === -1 || entry.atRule || !_entryListable(entry)) {
			continue;
		}
		const block = entry.text.slice(entry.prelude);
