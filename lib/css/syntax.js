					...
					if (sink) {
						...
						if (_nodeCount > limit) {
							if (_streamWriter !== undefined && rules === null) {
								// The printer may still merge a longhand family in here, which
								// needs every declaration at once. A child rule is where that
								// is given up rather than hold an unbounded block: streaming
								// one is the better trade, and the merge is the rarer case.
 ...
 *
 * A hex escape is left as the author wrote it. Writing the character instead is
 * value-identical and csso and lightningcss both do it, but it trades a
 * repetitive ASCII alphabet for 3-byte UTF-8 runs: measured over 19 framework
 * stylesheets it saves 6420 raw bytes and costs 593 gzip ones, which is the
 * number a user downloads. It would also make an all-ASCII asset depend on
 ...
 * written, and only what stands *between* two of them is rewritten: a run of
 * whitespace tokens and dropped comments is one boundary, printed as the single
 * space the tokens need, or as nothing where a comma or a block edge is already
 * one. Whitespace between tokens carries no meaning a substitution can read, so
 * this leaves every `var()` reading the same token stream.
 * @param {CssPath} path the accessor positioned on the declaration
...
const _customPropertyValue = (path, children, writer, from, to) => {
	...
	for (const child of children) {
		...
		// Nothing fuses with a comma, and nothing with the `(` this value opens
		// after — so those boundaries print as themselves.
		if (!comma && out !== "") {
			const last = out.charCodeAt(out.length - 1);
			if (spaced || (dropped && _wouldFuseTokens(last, text, out))) out += " ";
		}
		out += text;
		spaced = false;
		dropped = false;
 ...
 * at any depth, so it is written back whole.
 * @param {CssPath} path the accessor positioned on the declaration
...
const _dropShadowZeroLengths = (property, components) => {
	...
	for (let i = 0; i < layers.length; i++) {
		...
		// trailing zero is `0px` as often as `0`, and testing for the bare spelling
		// would read the collapse off whichever the author happened to write.
	...
	switch (path.type()) {
		...
		case T_DECLARATION: {
			...
			if (count !== 0) {
				...
				if (custom) {
					...
					// back, so rewriting a token in it builds a different CSSOM from the
					// same document — which `syntaxEquivalence` forbids, whatever a
					// substitution would read. Tokens as written, then; only what stands
					// between two of them minifies, down to the boundary it stands for.
		...
		case T_QUALIFIED_RULE: {
			...
			if (mergeable && decls.length >= 2) {
				...
				if (mergeableNames >= 2) {
					// How many child rules stand before each declaration. A rule
					// *between* two longhands would be stepped over by the merge and can
					// write either of them — a nested `@supports` re-declaring one is
					// what Tailwind emits — so the two have to count the same rules
					// ahead of them; one outside their span is nothing the merge moves.
					// Built here, where a family is actually in reach, not for every
					// nested block.
