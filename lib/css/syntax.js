...
const NodeType = {
	...
	Comment: 27,
	// WHY: not a spec node, but a workaround. §5.4 discards input a block's
	// contents reject, and keeping the source is what stops minifying from losing
	// what the unminified asset shows. Built only while printing, so a walk-only
	// parse never sees one.
	Raw: 28
};
const {
	Ident: T_IDENT,
...
// split mirrors tabatkins/parse-css, where `parse*` are the documented entry
// points and `consume*` the algorithms driving the tokenizer.
...
const consumeAQualifiedRule = (ts, stopToken, nested = false) => {
	...
	for (;;) {
		...
		// returns nothing — the remnants of a bad declaration when nested, a block
		// otherwise — which is what tells `--foo: { … }` from a nested rule.
	...
	for (;;) {
		...
		// a nested qualified rule stopping at <semicolon-token>, into rules.
			...
			if (!childRule && _printing) {
				// WHY: deliberately off-spec. §5.4 drops input both productions reject,
				// which would silently delete IE hacks and template placeholders from
				// minified output, so the source is kept verbatim. Printing only, so the
				// spec-exact tree is what every consumer sees.
...
const consumeADeclaration = (ts, nested = false) => {
	...
	// 5. Consume a list of component values, nested, stopping at <semicolon-token>,
	//    as decl's value. A nested non-custom declaration bails on a top-level `{`:
	//    step 8 rejects it and the caller restores its mark, so parsing that block
	//    only to re-parse it as a qualified rule would be waste.
	...
	// 8. A `--` custom property takes any value, a top-level `{}` block included.
	//    Otherwise such a block is the whole value or nothing, and anything beside
	//    it returns nothing, which is what has `consumeABlocksContents` retry the
	//    input as a qualified rule. The spec's further "no non-whitespace token at
	//    the top level" check is dropped: callers rely on `@value name:;`.
	if (!isCustomProperty) {
		let block = false;
		let beside = false;
...
const _streamOpen = (d, rule) => {
	...
	_frameRuleRun[d] = minify && _transforms.mergeRules ? [] : null;
...
const _streamClose = (d, rule) => {
	...
	if (minify && layerOpener !== null && _transforms.mergeRules) {
...
// Every top-level rule written out, in order, a null wherever something else
// was. A cut lands late, so what ends up beside what is known only at the end.
/** @type {(PriorTopLevel | null)[] | null} */
...
const _cutEmptiedSpans = (writer, taken) => {
	if (!_transforms.removeDeadRules) return;
 ...
 * @param {PrintContext} writer the print context
 * @param {(PriorTopLevel | null)[]} list where each rule of the block stands
 * @returns {void}
 */
const _reconcileRun = (writer, list) => {
	...
	for (const rule of list) {
		if (rule !== null && rule.grown) _orderRuleList(writer, rule);
	}
};
...
const _dropPrefixTwin = (scope, signature) => {
	...
	if (covering !== null) {
		for (let i = covering.length - 1; i >= 0; i--) {
			...
			covering.splice(i, 1);
			if (one.piece !== -1) {
				/** @type {PrintContext} */ (_streamWriter).retract(one.piece);
...
const _holdPrefixTwin = (scope, signature, top, missing) => {
	_prefixDropCandidate = { node: _currentNode, signature, missing };
	if (top) return;
	if (missing !== null) {
		if (scope.covering === null) scope.covering = [];
		scope.covering.push({ missing, piece: -1, node: _currentNode });
		return;
	}
...
const _hasSubstitutionInSpan = (from, to) => {
	...
	for (
		let at = _input.indexOf("(", from);
		at !== -1 && at < to;
		at = _input.indexOf("(", at + 1)
	) {
		if (_substitutionCallAt(_input, from, at)) {
			has = true;
			break;
		}
	}
...
const _mergeBoxLonghands = (
	...
	for (let e = 0; e < table.length; e++) {
		...
		for (
			...
			if (_prefixingOn && name.charCodeAt(0) === CC_HYPHEN_MINUS) {
...
const _dropRepeatedChildren = (joined, added) => {
	...
	while (i < joined.length) {
		...
		if (POSITIONAL_AT_RULE_RE.test(text)) {
			i++;
			continue;
		}
		let j = i - 1;
...
const _joinRuleEntries = (before, entry, owned) => {
	...
	if (before.atRule) {
		...
		if (from < children.length && _transforms.removeDeadRules) {
			const dropped = _dropRepeatedChildren(joined, children.length - from);
			if (dropped) head = _joinedHead(joined);
		}
...
const _composeBlockBody = (
	...
	// Each switch governs its own rewrite: folding a longhand into the shorthand
	// before it is `mergeLonghands`', dead declarations dropped here or not.
	const dropDead = minify && _transforms.removeDeadRules && items.length > 1;
	const foldLonghands =
		minify && _transforms.mergeLonghands && items.length > 1;
