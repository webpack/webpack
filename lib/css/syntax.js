...
// alongside `walkTokens` (the per-token lexer primitive) for the unit test.
const TT_COMMENT = 1;
 ...
 * by `tokenToNode` for the matching type. Pass a fresh one per `walkTokens` call
 ...
 */
function walkTokens(input, pos, out) {
 ...
 * stream in one class: the `walkTokens` primitive lexes one token (the CSS
 ...
 * swapping the tokenizer — the per-token `walkTokens` primitive — for its own.
 */
class TokenStream {
	/**
	 * @param {string} input source
	...
	next() {
		if (!this._hasNext) {
			...
			let pos = this._pos;
			for (;;) {
				const t = walkTokens(input, pos, tok);
				if (t === undefined) {
