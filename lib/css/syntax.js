...
const _useBrowsers = (browsers) => {
	...
	if (key !== _browsersKey) {
		...
		for (const entry of browsers) {
			...
			// A browserslist name no compat dataset covers, or a version that did not
			// parse, is skipped — the same browsers lightningcss's target mapping
			// drops, so both minifiers prefix for the same selection.
			const slot =
...
// or a shape the parser passed through without validating.
// TODO widen this to the pseudos `mdn-data`'s `selectors.json` names, derived in
// the generator like every other table, once it says which engine reads which
// prefix.
...
const _composeBlockBody = (
	...
	if (shaped >= 2) {
		...
		// property, which was written twice, and how many are a longhand a shorthand
		// could gather. A shorthand needs two, so a block holding fewer — almost
		// every block — writes none whatever the tables say.
		/** @type {Map<string, number>} */
		const at = new Map();
		/** @type {Set<string>} */
	...
	// always resolves that way; a differently spelled value only where the later
	// writes every name the earlier does, since otherwise the earlier is its
	// fallback — dropping those is how esbuild and lightningcss lose a
	// `color(display-p3 …)` pair.
	// TODO offer the wider "drop whatever a later declaration overrides" behind an
	// option, as csso and cssnano expose theirs. It needs support data per
	// keyword: a bare ident may be invalid, or newer than the value it stands
	// after, and neither supersedes anything.
	...
	// nothing, and most rules hold one or none. Each switch governs its own
	// rewrite, `mergeLonghands` owning the fold into a shorthand.
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			// a `calc()` whatever it was: the parentheses stay, since dropping them
			// turns a clamped negative into a parse error, and only the declaration
			// printer, which knows the property, takes them off.
