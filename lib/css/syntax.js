 ...
 * @typedef {import("../util/SourceProcessor").VisitorFn<Node, CssAst>} VisitorFn
 * @typedef {import("../util/SourceProcessor").VisitorBucket<Node, CssAst>} VisitorBucket
 * @typedef {import("../util/SourceProcessor").VisitorMap<Node, CssAst>} VisitorMap
 * @typedef {import("../util/SourceProcessor").CompiledVisitorMap<Node, CssAst>} CompiledVisitorMap
 */
		...
		commentBucket === undefined
			...
			: /** @type {(input: string, start: number, end: number) => number} */ (
					(_input, start, end) => {
						...
						for (let i = 0; i < e.length; i++) e[i](A, node, null, visitorCtx);
						const x = commentBucket.exit;
						for (let i = 0; i < x.length; i++) x[i](A, node, null, visitorCtx);
						return end;
					}
				);
 ...
 *
 * ```
 * new SourceProcessor({ skip }).use({ [NodeType.AtRule]: (api, at) => {} }).process(source);
 * ```
 * @extends {GenericSourceProcessor<Node, CssAst, CssProcessOptions>}
 */
...
/**
 * The CSS AST accessor — what every visitor receives as its first argument.
 * @typedef {typeof A} CssAst
 */
