 ...
 * @property {boolean=} mergeDistantRules give a rule the selectors of a later one printing the same block, past the rules between them; off by default, since it reorders the cascade and is sound only where nothing between declares what the block does
...
const _walkValue = (node, parent, index, writer) => {
	...
	if (!skip && (ty === T_FUNCTION || ty === T_SIMPLE_BLOCK)) {
		...
		const previousColorOperand = _inColorOperand;
		if (
			ty === T_FUNCTION &&
			!_inColorOperand &&
			_readsColorsAsNumbers(i0, vs, ve)
		) {
			_inColorOperand = true;
		}
		for (let i = vs; i < ve; i++) {
			_walkValue(_nodeRef(_flat[i]), node, i - vs, writer);
		}
		_inColorOperand = previousColorOperand;
	}
...
const _rangeHasEscape = (start, end) => {
	for (let i = start; i < end; i++) {
		if (_input.charCodeAt(i) === CC_REVERSE_SOLIDUS) return true;
	}
	return false;
};
...
const _identNamesFrom = (start, end) => {
	const length = end - start;
	if (length === 4) return rangeEqualsLowerCase(_input, start, end, "from");
	if (length < 4 || length > MAX_ESCAPED_FROM_LENGTH) return false;
	return (
		_rangeHasEscape(start, end) &&
		equalsLowerCase(unescapeIdentifier(_input.slice(start, end)), "from")
	);
};
...
const _readsColorsAsNumbers = (i0, vs, ve) => {
	...
	if (_rangeHasEscape(start, end)) {
		...
		const name = unescapeIdentifier(_input.slice(start, end));
		...
		const last = _input.charCodeAt(end - 1) | 0x20;
		if (last === CC_LOWER_T) {
			if (GRADIENT_FUNCTION_RE.test(_gradientName(i0))) return true;
		} else if (
			last === CC_LOWER_X &&
			rangeEqualsLowerCase(_input, start, end, "color-mix")
		) {
			return true;
		}
	}
...
const _streamClose = (d, rule) => {
	...
	if (d === 0) {
		if (_seenTopLevelLayers !== null) _seenTopLevelLayers.clear();
	} else if (_frameSeenLayers[d - 1] !== null) {
		/** @type {Map<string, SeenLayer>} */ (_frameSeenLayers[d - 1]).clear();
	}
...
/** @typedef {{ taken: TakenPiece, span: RuleSpan, entry: RuleEntry, grown: boolean }} PriorTopLevel the top-level rule last written out, the piece it stands in, and whether a later rule wrote its selectors into it */

// The rule written out immediately before the held one, kept so a join its own
// block only became eligible for can still be made. Null when what went out
// last was not one whole qualified rule.
...
const _emitHeld = (writer, held) => {
	...
	_priorTopLevel = null;
	// Its block is settled only here. A join grew it into one the rule beside it
	// already prints, and that rule was offered this one before it had grown, so
	// the offer is made once more against the block it ends up with.
	...
	if (_mergingDistantRules) _noteDistantRule(taken, held.entry);
	_settlePrior(writer);
	_priorTopLevel = _priorOf(taken, held.entry);
...
const _priorOf = (taken, entry) => {
	...
	if (!entry.plain || !_entryListable(entry)) return null;
	const span = taken.spans[0];
	if (
		taken.spans.length !== 1 ||
		span.at !== 0 ||
		span.len !== taken.text.length
	) {
		return null;
	}
	return { taken, span, entry, grown: false };
};
...
const _settlePrior = (writer) => {
	const prior = _priorTopLevel;
	_priorTopLevel = null;
	if (prior === null || !prior.grown) return;
	const span = prior.span;
	if (span.len === 0) return;
	const taken = prior.taken;
	const block = prior.entry.text.slice(prior.entry.prelude);
	const prelude = span.len - block.length;
	const list = taken.text.slice(span.at, span.at + prelude);
	...
	writer.rewriteRetractable(taken.piece, taken.text);
	// Keyed by the text it went out as, so the ordered list is keyed again.
	const rules = span.scope.rules;
...
const _joinsPrior = (writer, prior, held) => {
	if (!_transforms.mergeRules) return false;
	const entry = held.entry;
	if (entry.prelude === -1 || entry.atRule || !_entryListable(entry)) {
		return false;
	}
	// A kept comment between the two was written above the held rule, which is
	// read where it still stands rather than here, the flush having cleared it.
	if (held.parted) return false;
	// Where the block starts in the rule as it now stands — a list this already
	// grew is longer than the one that was taken.
	const preludeNow = prior.span.len - (prior.entry.text.length - prior.entry.prelude);
	if (
		!_rangeEquals(
			prior.taken.text,
			prior.span.at + preludeNow,
			prior.span.at + prior.span.len,
			entry.text,
			entry.prelude,
			entry.text.length
		)
	...
	if (!_lendSelectorsTo(writer, prior.taken, prior.span, preludeNow, entry)) {
		return false;
	}
	prior.grown = true;
	return true;
...
const _mergeIntoDistantRule = (writer, entry) => {
	...
	const list = found.taken.text.slice(
		found.span.at,
		found.span.at + preludeNow
	);
	return _lendSelectorsTo(writer, found.taken, found.span, preludeNow, entry);
};
 ...
 */
const _lendSelectorsTo = (writer, taken, span, preludeNow, entry) => {
	const list = taken.text.slice(span.at, span.at + preludeNow);
	const selector = entry.text.slice(0, entry.prelude);
	...
	// A rule is keyed by the text it went out as, so the grown list is keyed
	// again: left under the old text, a later copy would take the rule back.
	const rules = span.scope.rules;
	const registered = rules.get(span.key);
	if (registered !== undefined && registered.span === span) {
		rules.delete(span.key);
	...
	span.key = taken.text.slice(span.at, span.at + span.len);
	// One already standing under it is the later of the two, and taking a rule
	// back is its answer to give — this one only stops being one it can take.
	if (!rules.has(span.key)) rules.set(span.key, { taken, span });
	return true;
};
...
const _orderDistantLists = (writer) => {
	for (const [block, found] of _distantRules) {
		...
		taken.text = `${taken.text.slice(0, at)}${canonical}${taken.text.slice(at + prelude)}`;
		writer.rewriteRetractable(taken.piece, taken.text);
...
const _takeTopLevel = (node, writer, offset, line, column) => {
	...
	// A kept comment between the two was written above the second rule. Read
	// once: the flush below clears it, and the hold carries the answer on for the
	// look-back join the rule beside this one is offered when it is released.
 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "mergeDistantRules" | "rewriteCustomProperties" | "renderEmbeddedSource" | "deferEmbeddedSource" | "transforms" | "unusedSymbols" | "pseudoClasses">} CssPrintOptions
 */
...
const _normalizeNumericToken = (text) => {
	...
	if (lowered !== unit && !_inSubstitutedValue && _transforms.foldCase) {
...
const _holdPrefixTwin = (scope, signature, top) => {
	...
	_holdBySignature(scope.pending, signature, _currentNode);
};
...
// magnitude: up to 9.53e-4 of it, across every space and the Lab family.
//
...
// only the engines it can be run against have been measured. Each is the widest
// a run has reported rather than the last one's, so re-running it for a newer
// Chromium or an engine it has not seen widens them and never narrows them.
...
const _predefinedColorChannels = (inner) => {
	// `color(<space> c1 c2 c3[ / <alpha>])` — the alpha stands after the one slash
	// the grammar has and nowhere else, and a comma is no separator here at all.
	// Reading either as whitespace turns a declaration the engine drops into one
	// it paints.
...
const _colorFromChannels = (raw, alpha, conversion, hexAlpha, forFallback) => {
	...
	// exactly as `rgba()`'s does — and one that quantizes to `255` is the opaque
	// color, which is written without an alpha at all.
...
const _hueEndpoints = (from, to) => {
	...
	// half turn — which is what every method is stated as a correction to. Two
	// opposite hues have no shorter arc, and CSS Color 4 §12.4 breaks the tie by
	// which one is greater: the arc is kept as it stands, so a half turn the
	// conversion left a hair over is not read as the arc the other way round.
...
const _resolveNestedPrelude = (prelude, parent) => {
	...
	for (let i = 0; i < written.length; i++) {
		...
		written[i] = _namesParent(one)
			? _substituteAmpersand(one, reference)
			: `${reference} ${one}`;
	}
	return written.join(",");
};
...
const _declarationsPrecedeRules = (path, rule) => {
	...
	let firstRule = Number.POSITIVE_INFINITY;
	for (let i = 0; i < rules.length; i++) {
		const at = path.start(rules[i]);
		if (at < firstRule) firstRule = at;
	}
	for (let i = 0; i < decls.length; i++) {
		if (path.start(decls[i]) > firstRule) return false;
	}
	return true;
};
 ...
 */
const _writeNested = (path, node, prelude, rest, hoistable) => {
	...
	/** @type {{ prelude: string, rest: string, owner: Node | null }[]} */
	const taken = [];
	...
	if (hoisted) {
		if (rest !== null) _pendingNested.push({ prelude, rest, owner });
	...
	let out = "";
	for (let i = 0; i < taken.length; i++) {
		out += taken[i].prelude + taken[i].rest;
	}
	return out;
};
...
const _foldPseudoNames = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (second !== -1 && parts[second] === ":") {
			const lowered = asciiLowerCaseName(part);
			if (_transforms.foldCase && lowered !== part && !part.includes("(")) {
				part = lowered;
				parts[i] = lowered;
			}
...
const _reduceTransformFunctionDeep = (fn, inner) => {
	...
	for (;;) {
		...
		if (next === null) {
			...
			const args = _transforms.shortenNumbers
...
const _writeIsDead = (path, writer, earlier, later, keyframe) => {
	...
	return _laterWriteWins(
		path,
		later,
		_printedValue(laterText, laterColon),
		earlier,
		_printedValue(earlierText, earlierColon),
		CUSTOM_IDENT_LIST_PROPERTIES.has(_printedProperty(laterText, laterColon)),
		keyframe
	);
};
...
/**
 * Whether every rule between two siblings leaves the shared block's properties
 * alone, so moving a selector past them cannot change what any element reads.
 * Anything that is not a plain qualified rule — a declaration, an at-rule whose
 * own nested rules this cannot see into — is taken as standing in the way.
 * @param {Node[]} items the parent's children in source order
 * @param {string[]} texts their printed texts
 * @param {number} from the index the selector would move to
 * @param {number} to the index it moves from
 * @param {Set<string>} declared the shared block's properties
...
const _nothingBetweenShadows = (items, texts, from, to, declared) => {
	for (let i = from + 1; i < to; i++) {
		...
		if (between.prelude === -1 || between.atRule || !between.plain) {
			return false;
		}
		const body = between.text.slice(between.prelude + 1, -1);
		if (_propertiesOverlap(declared, _blockProperties(body))) return false;
	}
	return true;
};
...
const _mergeAdjacentRules = (items, texts, pending, dead) => {
	 ...
	 * what ends a run: the joins themselves only concatenate.
	 * @returns {void}
	 */
	const finish = () => {
		if (!owned || held === null) return;
		const canonical = _canonicalizeJoined(held);
		if (canonical === held) return;
		_setRuleEntry(items[previous], canonical);
		texts[previous] = canonical.text;
	};
	for (let i = 0; i < items.length; i++) {
...
const _rulePrelude = (path, writer, minify, outParts) => {
	...
	if (path.type() === T_AT_RULE) {
		...
		const lower =
			minify && _transforms.foldCase && !equalsLowerCase(name, "charset");
	...
	if (minify && path.type() === T_AT_RULE) {
		...
		if (media || container) {
			...
			if (media) {
				...
				if (_transforms.resolveCustomAtRules && _customMedia.size !== 0) {
					_writeCustomMedia(parts);
				}
				if (_transforms.foldCase) _lowercaseConditionParts(parts);
	...
	if (minify && path.type() === T_QUALIFIED_RULE) {
		...
		if (_transforms.resolveCustomAtRules && _customSelectors.size !== 0) {
		...
		if (!_dirSelectorAllowed && _transforms.rewriteDirSelector) {
...
const _withColorFallback = (one) => {
	if (!_colorFallbacksNeeded || !_transforms.colorFallbacks) return one;
...
const _composeBlockBody = (
	...
	if (minify && _colorFallbacksNeeded && _transforms.colorFallbacks) {
		...
		for (let i = 0; i < items.length; i++) {
			...
			// A shorthand the target cannot read is lowered below into longhands that
			// carry their own fallbacks, in a spelling this one would not be written in.
			if (
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const out =
				_inSubstitutedValue || !_transforms.foldCase
					? name
					: fn === name
						? fn
			...
			// A color read as numbers — a gradient stop, a `color-mix()` argument, a
			// relative color's origin — is left as written unless converting it is
			// exact, which the legacy sRGB forms are where their channels are bytes.
			let color = _inSupportsPrelude
				? null
				: _minifyColorFunction(fn, inner, _hexAlphaAllowed, _inColorOperand) ||
					(_inColorOperand
						? null
		...
		case T_URL: {
			...
			let head = null;
			if (!_inSubstitutedValue && _transforms.foldCase) {
		...
		case T_IDENT: {
			...
			return _transforms.rewriteEscapes ? _minifyIdentEscapes(raw) : raw;
		}
		case T_SIMPLE_BLOCK: {
		...
		case T_QUALIFIED_RULE: {
			...
			if (decls === null) {
				...
				if (
					minify &&
					_transforms.resolveCustomAtRules &&
					equalsLowerCase(path.name(), "custom-media") &&
				...
				if (
					...
					_transforms.resolveCustomAtRules &&
					_isSelectorAllowed &&
					equalsLowerCase(path.name(), "custom-selector") &&
			...
			if (
				...
				// The rules written in it are not empty for its being so.
				return (
					_writeNested(
						path,
						/** @type {Node} */ (_currentNode),
			...
			// A rewritten rule is several rules with preludes of their own, so it
			// stays put — the rules written in it still come out against it.
			...
			if (nested.length !== 0) return text + nested;
		...
		case T_HASH: {
			...
			if (!path.inValue()) {
				return _transforms.rewriteEscapes
