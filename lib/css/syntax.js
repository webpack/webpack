 ...
 * output pieces of the prefixed ones an unprefixed twin would make dead
 * (`retractable`, only for the stylesheet's own rules — a nested rule's text is
 * gone into its parent's by the time a twin could ask).
...
const _prefixScope = (parent) => {
	...
	if (scope === undefined) {
		scope = { seen: new Set(), retractable: null };
...
/**
 * Take back the prefixed rule this one is the unprefixed twin of, where it was
 * emitted as a piece of its own for exactly that. The pair is usually adjacent —
 * every stylesheet writes the prefixed spelling first — but nothing in between
 * matters: the piece stays retractable until the stylesheet ends.
 * @param {PrefixScope} scope the block's running sibling state
 * @param {string} signature the unprefixed rule's sibling signature
 * @returns {void}
 */
const _retractPrefixTwin = (scope, signature) => {
	const retractable = scope.retractable;
	if (retractable === null) return;
...
const _prefixAtRule = (path, ruleText, prelude, scope, top) => {
	...
	seen.add(signature);
	if (top) _retractPrefixTwin(scope, signature);
...
const _prefixQualifiedRule = (ruleText, parts, soft, body, scope, top) => {
	...
	for (const selector of selectors) {
		...
		// One spelling for the whole list: a list mixing a prefixed pseudo with an
		// unprefixed one, or with a second engine's, is neither this rule's twin nor
		// a copy of it.
		if (found !== undefined) {
	...
	if (listPrefix !== undefined) {
		// Every pseudo of the list carries it, and each is dead weight on its own.
		const removable =
			carried.every(
				(prefix, i) => selectors[i].at === -1 || prefix === listPrefix
			) &&
			bases.every(
				(base, i) =>
					selectors[i].at === -1 ||
					_prefixRemovable(PREFIXED_SELECTORS, base, listPrefix)
			);
		if (seen.has(signature) && removable) return "";
	...
	for (let i = 0; i < selectors.length; i++) {
		...
		if (union === null) union = new Set(one);
		else for (const prefix of one) union.add(prefix);
	}
	if (union === null) return ruleText;
	let out = "";
	for (const prefix of union) {
		// Skip only a prefix the source itself already carries (marked above when
		...
		const list = preludeOf((i) => {
			const one = needed[i];
			return one === null || !one.has(prefix)
				? null
				: prefix + bases[i] + args[i];
		});
		out += `${list}${soft}{${body}}`;
	}
	return out + ruleText;
