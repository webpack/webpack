...
const _customPropertyToken = (path, child, writer) => {
	...
	const type = path.type(child);
	// A url token the input ran out of mid-escape holds a value the text no
	// longer spells, and the engine writes that value back closed — so without
	// the `)` the `}` after it here would be read as part of the url.
	if (
		type === T_URL &&
		path.end(child) === _input.length &&
