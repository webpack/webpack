...
// Every layer a node's text writes into, as the names its `@layer` rules carry.
const WRITTEN_LAYER_RE = /@layer\s+([^{;]+)[{;]/g;
...
const _forgetLayersWritten = (layers, text) => {
	...
	WRITTEN_LAYER_RE.lastIndex = 0;
	for (
		let found = WRITTEN_LAYER_RE.exec(text);
		found !== null;
		found = WRITTEN_LAYER_RE.exec(text)
	) {
		for (const name of found[1].split(",")) written.add(name.trim());
	}
	for (const opener of layers.keys()) {
		if (written.has(opener.slice(NAMED_LAYER_OPENER_HEAD, -1).trim())) {
			layers.delete(opener);
		}
	}
};
