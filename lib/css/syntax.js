...
const NodeType = {
	...
	Comment: 27,
	// Input a block's contents dropped as invalid (`consumeABlocksContents`).
	// Only materialized while printing, so minification can carry it through
	// verbatim instead of losing it; a walk-only parse never sees one.
	Raw: 28
};
const {
	Ident: T_IDENT,
...
// Dropped input, trimmed of the whitespace the block's own separators already
// cover; an all-whitespace span carries nothing and yields no node.
	...
	for (;;) {
		...
		// Otherwise, restore a mark from input, then consume a qualified rule from input, with nested set to true, and <semicolon-token> as the stop token. If a rule was returned, append it to rules.
			...
			if (rule) {
				...
				// Both productions rejected the input, so it is gone from the tree —
				// keep the source verbatim so minifying never drops what the
				// unminified asset keeps (IE hacks, template placeholders).
 ...
 * @property {boolean=} minimize print the safely-minified serialization (collapsed whitespace, dropped redundant separators) as `process` walks and return `{ code, map }` (default false = walk only, return `undefined`)
 ...
 */
const _minifyColorFunction = (name, inner) => {
 ...
 * each value-identical, so meaning never changes.
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const color = minify ? _minifyColorFunction(name, inner) : null;
			return color !== null ? color : `${name}(${inner})`;
		}
		case T_SIMPLE_BLOCK: {
		...
		case T_RAW:
			// Dropped input sits in a declaration list, so it carries the `;` that
			// separates it from the next item (stripped again before a `}`).
			return `${path.source()};`;
		case T_HASH: {
			const raw = path.source();
