...
// CSS Cascade 5 §6.4.1: every `@layer {` opens a layer of its own, and for an
// `!important` declaration the earliest layer wins — so a copy in a later one
// is weaker, not stronger, and cannot make an earlier copy dead.
let _anonymousLayers = 0;

/**
 * What an opener contributes to the key the rules under it are read by.
...
const _openerKey = (opener) =>
	ANONYMOUS_LAYER_RE.test(opener.slice(0, -1))
