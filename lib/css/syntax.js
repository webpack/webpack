 ...
 * has to enumerate it.
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties" | "renderEmbeddedSource">} CssPrintOptions
 */
...
const _normalizeNumericToken = (text) => {
	...
	const lowered = asciiLowerCaseName(text.slice(end));
	const canonical = CANONICAL_NAMES.get(lowered);
	const unit = canonical === undefined ? lowered : canonical;
...
/**
 * Whether the declaration being printed is one whose value grammar is keywords
 * alone, so an identifier standing directly in it is a keyword.
 * @returns {boolean} true inside such a declaration
 */
const _inKeywordOnlyProperty = () =>
	_valueDeclaration !== null &&
	KEYWORD_ONLY_PROPERTIES.has(
		_standardSpelling(toLowerCaseIfNeeded(A.name(_valueDeclaration)))
	);
...
const _rulePrelude = (path, writer, minify, outParts) => {
	...
	if (path.type() === T_AT_RULE) {
		...
		parts.push(
			`@${minify && !equalsLowerCase(name, "charset") ? asciiLowerCaseName(name) : name}`
		);
	}
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const out = printed === undefined ? fn : printed;
		...
		case T_URL: {
			...
			const head = asciiLowerCaseName(written);
		...
		case T_IDENT: {
			...
			// A property whose value is keywords alone names nothing of the author's,
			// so an identifier standing directly in one of its values is a keyword
			// and matches ASCII case-insensitively. Directly only: a call's arguments
			// are read against the function's own grammar, where a name may be the
			// author's. Asked last, after the ident is known to carry a capital,
			// which is what keeps the property lookup off every value.
			if (
				path.inValue() &&
				!_inCustomProperty &&
				!_inSupportsPrelude &&
				...
				if (lowered !== raw && _inKeywordOnlyProperty()) return lowered;
			}
		...
		case T_DECLARATION: {
			...
			const printedName = minify && !custom ? asciiLowerCaseName(name) : name;
