...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			if (path.inValue() && ZERO_ANGLE_FUNCTIONS.has(fn)) {
		...
		case T_QUALIFIED_RULE: {
			...
			const ruleText = `${prelude}${soft}{${body}${nl}}`;
			// Rule-level prefixing is top-level only (`path.parent === null`):
			// `_seenPrefixRules` has no nesting key, so a nested `@keyframes` / pseudo
			// must not add or drop against a same-named top-level twin in another
			// scope. A nested rule's declarations are still prefixed in the block above.
			if (minify && _seenPrefixRules !== null && path.parent === null) {
				if (path.type() === T_AT_RULE) {
					return _prefixAtRule(path, ruleText, prelude, writer);
				}
				return _prefixQualifiedRule(path, ruleText, parts, soft, body, writer);
			}
			return ruleText;
		}
		case T_RAW:
