...
/**
 * Whether the target reads a two-position color stop. Unset means it does: only
 * the browserslist target reports otherwise (see `output.environment`).
 * @param {PrintContext} writer the print context
 * @returns {boolean} true when `red 0% 50%` may be emitted
 */
const _doublePositionAllowed = (writer) => {
	const environment = writer.options.environment;
	return (
		environment === undefined || environment.cssGradientDoublePosition !== false
	);
};
...
/** @type {WeakMap<string[], [string, number][]>} */
const _parsedBrowsersCache = new WeakMap();
/** @type {WeakMap<string[], Map<string, Set<string> | null>>} */
const _neededPrefixCache = new WeakMap();
...
const _parsedBrowsers = (browsers) => {
	let parsed = _parsedBrowsersCache.get(browsers);
	if (parsed === undefined) {
		parsed = [];
		...
		_parsedBrowsersCache.set(browsers, parsed);
	}
	return parsed;
};
...
const _neededPrefixes = (table, name, tag, writer) => {
	...
	let cache = _neededPrefixCache.get(browsers);
	if (cache === undefined) {
		cache = new Map();
		_neededPrefixCache.set(browsers, cache);
	}
	const key = tag + name;
...
const _dropRatioDenominator = (property, value) => {
	...
	if (!RATIO_PROPERTIES.has(property) || _SUBSTITUTION_RE.test(value)) {
		return value;
	}
	return value.replace(_RATIO_OVER_ONE_RE, "$1$2");
};

// A size whose omitted second value is `auto` (CSS Backgrounds 3 §3.9, CSS
// Masking 1 §4.5), not the first repeated as a box longhand's is.
const _AUTO_SECOND_VALUE_PROPERTIES = new Set(["background-size", "mask-size"]);
...
// 0 the one-axis call already means (CSS Transforms 1 §11).
const _X_AXIS_TRANSFORMS = new Map([
...
const _reduceTransformFunction = (fn, inner) => {
	...
	if (fn === "scale3d" && args.length === 3 && args[2] === "1") {
	...
	if (fn === "rotate3d" && args.length === 4) {
		...
		const axis =
			zero(1) && zero(2) && args[0] === "1"
				? "rotateX"
				: zero(0) && zero(2) && args[1] === "1"
					? "rotateY"
					: zero(0) && zero(1) && args[2] === "1"
						? "rotate"
						: null;
...
const _collapseShorthand = (path, property, children, writer) => {
	...
	const written = _valueComponents(path, children, writer);
	...
	const dropped = _dropInitialKeywords(property, written);
	const components = dropped === null ? written : dropped;
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const oneAxis = _X_AXIS_TRANSFORMS.get(fn);
			if (
				oneAxis !== undefined &&
				path.inValue() &&
				!_inSubstitutedValue &&
		...
		case T_QUALIFIED_RULE: {
			...
			if (
				...
				for (let i = 0; i < items.length; i++) {
					if (path.type(items[i]) === T_DECLARATION) {
						present.add(toLowerCaseIfNeeded(path.name(items[i])));
					}
				}
				for (let i = 0; i < items.length; i++) {
					...
					const property = toLowerCaseIfNeeded(path.name(items[i]));
