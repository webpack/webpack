...
const _joinRuleEntries = (before, entry, owned) => {
	...
	// Concatenated, never canonicalized. Measured: merging the two sorted lists
	// instead gains no gzip byte on Bootstrap / Pico / Water, loses 122 brotli
	// bytes on Bootstrap, and costs 13-16% of the minify — 17x on a run of 1800
	// joins, since concatenation is a rope and re-reading the list is not. The
	// price is that minifying an already-minified sheet can reorder one list, so
	// a caller comparing two spellings has to canonicalize to a fixed point.
