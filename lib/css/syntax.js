...
const _SVG_DATA_URL_RE =
	/^(data:image\/svg\+xml(?:;[^,]*)?,)([^#]*)(#[\s\S]*)?$/i;
const _BASE64_RE =
	/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/;

/** @type {typeof import("../html/syntax") | undefined} */
let _htmlSyntax;

/**
 * Minify the SVG a `data:` URL carries, returning the URL with its payload
 * decoded. The payload is decoded whole rather than read through the escapes
 * `_decodePercentEscapes` leaves in place: the minifier may write a `#` where
 * the source had a color name, and an unescaped one would start the fragment.
 * @param {string} written the string's content, quotes excluded
 * @returns {{ url: string, fragment: string } | null} the minified URL and the fragment to append, or null to keep what is there
 */
const _minifySvgDataUrl = (written) => {
	const parts = _SVG_DATA_URL_RE.exec(written);
	if (parts === null) return null;
	const head = parts[1];
	// A `#` starts the URL's fragment, so it and everything after it are not
	// payload and are carried over untouched.
	const fragment = parts[3] || "";
	const base64 = /;base64,$/i.test(head);
	let svg;
	try {
		// `Buffer.from` drops what it cannot read rather than throwing, so a
		// payload it would silently rewrite is rejected before it is decoded.
		if (base64 && !_BASE64_RE.test(parts[2])) return null;
		svg = base64
	...
	let minified;
	try {
		const htmlSyntax = _htmlSyntax || (_htmlSyntax = require("../html/syntax"));
		minified = new htmlSyntax.SourceProcessor().process(svg, {
			mode: "minify",
			xml: true,
			// The payload is read back through the URL, so the serialization is
			// its own: an empty element keeps its end tag rather than self-closing.
			keepEndTags: true,
	...
	return {
		url:
			head +
			(base64 ? Buffer.from(minified, "utf8").toString("base64") : minified),
		fragment
	};
};
...
const _minifyUrlFunction = (name, fn, inner) => {
	...
	let mark = String.fromCharCode(quote);
	const svg = _minifySvgDataUrl(written);
	let body;
	if (svg === null) {
		body = _decodePercentEscapes(written, false, mark);
	} else {
		// A `#` starts the fragment, a `%` the next escape, the quote ends the
		// string, and non-ASCII would make the stylesheet's encoding load-bearing.
		body = "";
		for (const quoted of ['"', "'"]) {
			const candidate = svg.url.replace(/["']|[^ -~]|[#%\\]/gu, (one) =>
				(one === '"' || one === "'") && one !== quoted
					? one
					: Array.from(
			...
			if (body === "" || candidate.length < body.length) {
				mark = quoted;
				body = candidate;
			}
		}
		body += svg.fragment;
		if (body.length >= written.length) return null;
	}
