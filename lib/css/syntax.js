 ...
 */
const _mergeBoxLonghands = (path, decls, writer, table) => {
	/** @type {Map<string, number>} */
	const at = new Map();
	/** @type {Set<string>} */
	...
	for (const [shorthand, longhands] of table) {
		...
		if (box.length !== 1 && ONE_VALUE_PAIR_SHORTHANDS.has(shorthand)) continue;
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			// sides, or the two a pair shorthand sets. Only with a child rule out of
			// the way: one between them would be stepped over by the merge, which
			// the adjacency check cannot see.
			const mergeable = minify && (rules === null || rules.length === 0);
			const merged =
				mergeable && decls.length >= 4
					? _mergeBoxLonghands(path, decls, writer, BOX_LONGHANDS)
					: null;
			const mergedPairs =
				mergeable && decls.length >= 2
					? _mergeBoxLonghands(path, decls, writer, PAIR_LONGHANDS)
					: null;
			if (mergedPairs !== null) {
				for (const [node, text] of mergedPairs) {
					// A box merge already claimed it, and four sides beat two.
					if (merged === null || !merged.has(node)) mergedPairs.set(node, text);
				}
			}
			...
			for (const item of items) {
				let replacement = merged === null ? undefined : merged.get(item);
				if (replacement === undefined && mergedPairs !== null) {
					replacement = mergedPairs.get(item);
				}
