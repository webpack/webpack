...
// An at-keyword as it may be written: an escape is part of the name, and a hex
// one ends at a whitespace the name does not carry — which is why matching one
// cannot be a single character class.
const AT_RULE_NAME_SOURCE = String.raw`(?:\\[\da-f]{1,6}[ \t\n\r\f]?|\\[^\n]|[^\s({;\\])+`;
 ...
 * @property {boolean=} mergeDistantRules give a rule the selectors of a later one printing the same block, past the rules between them; off by default, since it reorders the cascade and is sound only where nothing between declares what the block does — a condition between counting for what its own rules declare
...
const _enterAtRulePrelude = (i0) => {
	const atName = _atKeyword(_input.slice(_starts[i0] + 1, _aux0[i0]));
	if (atName === "property") {
...
const BARE_AT_RULE_RE = new RegExp(`^@(${AT_RULE_NAME_SOURCE})\\s*\\{$`, "i");
		...
		if (
			...
			(_takeUnusedRule(rule) ||
				...
				DROPPABLE_WHEN_EMPTY_AT_RULES.has(
					_atKeyword(_input.slice(_starts[i0] + 1, _aux0[i0]))
				))
		) {
			writer.dropPending();
			return;
...
const _isAnonymousLayerPrelude = (prelude) => {
	...
	return (
		...
		prelude.slice(match[0].length).trim().length === 0
	);
};
...
const _mergeDistantRules = (items, texts) => {
	...
	for (let i = 0; i < items.length; i++) {
		...
		// TODO gather two blocks of one condition as well. Measured over daisyUI 5 it
		// loses: the body it moves up is then in a rule merge's way, +25 gzip bytes.
		if (entry.prelude === -1 || entry.atRule || !_entryListable(entry)) {
			continue;
		}
		const block = entry.text.slice(entry.prelude);
