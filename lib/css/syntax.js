 ...
 * @property {boolean=} customMedia write the query a `@custom-media` names wherever a condition asks for it, and drop the rule that named it
...
const _declarationsPrecedeRules = (path, rule) => {
	const rules = path.childRules(rule);
 ...
 */
const _writeNested = (path, node, prelude, rest) => {
	...
	const hoisted = _hoistedOutOf(path, path.parent);
 ...
 * written out, and the condition is left as the author had it.
 * @param {string[]} parts the prelude's printed parts
 * @returns {void}
 */
const _writeCustomMedia = (parts) => {
	for (let i = 0; i < parts.length; i++) {
		...
		if (query !== undefined) parts[i] = query;
	}
};
	...
	if (minify && path.type() === T_AT_RULE) {
		...
		if (media || container) {
			...
			if (media) {
				...
				if (_transforms.customMedia && _customMedia.size !== 0) {
...
const _withColorFallback = (one) => {
	...
	if (colon <= 0) return one;
	const fallback = _colorFallbackDeclaration(one, colon);
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_QUALIFIED_RULE: {
			...
			// stays where it was written rather than being taken out.
