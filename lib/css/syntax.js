...
/**
 * Fold a printed `calc()` body to the one value it is equal to. Only a fully
 * collapsed expression is returned: a sum that still holds two units (a
...
const _foldCalc = (inner) => {
	...
	const folded = _printCalcTerm(key, coefficient);
	// A division can land on a value that needs every digit of a double to write
	// down, which is longer than the expression that produced it.
	return folded !== null && folded.length < inner.length ? folded : null;
};
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			// A `calc()` whose operands are all constants of one unit is that one
			// value. The parentheses stay here: dropping them turns a clamped
			// negative into a parse error and a rounded fraction into one, so only
			// the declaration printer — which knows the property — takes them off.
			if (fn === "calc" && !_inSupportsPrelude) {
				const folded = _foldCalc(inner);
				if (folded !== null) return `calc(${folded})`;
			}
