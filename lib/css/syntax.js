...
const _prefixAtRule = (path, ruleText, prelude, writer) => {
	...
	for (const prefix of needed) {
		const marker = `${signature}\0${prefix}`;
...
const _prefixQualifiedRule = (path, ruleText, parts, soft, body, writer) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (
			i !== 0 &&
			parts[i - 1] === ":" &&
			(PREFIXED_SELECTORS.has(parts[i]) ||
				_prefixedSelectorName(parts[i]) !== undefined)
		) {
	...
	if (nameIndex === -1) return ruleText;
	const found = _prefixedSelectorName(parts[nameIndex]);
	const base = found === undefined ? parts[nameIndex] : found[0];
