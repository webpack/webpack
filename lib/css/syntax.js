 ...
 * @property {CssTransformOptions=} transforms the rewrites to make beyond the safe default; only read while printing, and every entry defaults to off
 */

/**
 * Rewrites that keep what the stylesheet computes but change something a build
 * may still be reading, so each is named rather than assumed.
 * @typedef {object} CssTransformOptions
 * @property {boolean=} customPropertyValues rewrite a custom property's value the way an ordinary declaration's is rewritten (`--r:0.5rem` becomes `--r:.5rem`). Exact CSS, but a custom property is the one declaration whose authored text `getComputedStyle().getPropertyValue()` hands back, so a script comparing that string sees the change
 */
...
const _walkRule = (node, parent, index, writer) => {
	...
	if (!skip) {
		if (ty === T_AT_RULE || ty === T_QUALIFIED_RULE) {
			...
			if (
				ty === T_AT_RULE &&
				equalsLowerCase(_input.slice(_starts[i0] + 1, _aux0[i0]), "supports")
			) {
 ...
 */
const _minifyColorFunction = (fn, inner) => {
	...
	if (alpha < 1) return null; // keep alpha functions (numbers already normalized)
	return _shortestColor(channels[0], channels[1], channels[2]);
};

/**
 * Whether the author asked for a custom property's value to be rewritten.
 * @param {PrintContext} writer the print context
 * @returns {boolean} true when the rewrites may run there
 */
const _customPropertyValuesWanted = (writer) => {
	const transforms = writer.options.transforms;
	return transforms !== undefined && transforms.customPropertyValues === true;
};
...
const _HSL_NUMBER_RE = /^([+-]?(?:\d+\.?\d*|\.\d+))(%?)$/;
// The hue's default unit; the other angle units would need their own conversion
// before the same exactness test could be applied.
const _HSL_HUE_RE = /^([+-]?(?:\d+\.?\d*|\.\d+))(?:deg)?$/i;

/**
 * Minify an `hsl()` / `hsla()` color to the shortest hex — but only where the
 * conversion lands on a byte with nothing to round. Rounding is exactly where
 * engines part company: measured against headless Chromium, spec-exact rounding
 * still misses 12 of 1560 samples by one, and esbuild and lightningcss disagree
 * with each other on `hwb()`. A conversion that rounds nothing cannot disagree
 * with anyone, so that subset converts and the rest keeps its function.
 * @param {string} fn the lowercased function name
 * @param {string} inner the already-joined argument text
 * @returns {string | null} the shortest color, or null to keep the function
 */
const _minifyHslFunction = (fn, inner) => {
	if (fn !== "hsl" && fn !== "hsla") return null;
	const args = inner
		.replace(/\//g, " ")
		.split(/[\s,]+/)
		.filter((s) => s.length !== 0);
	if (args.length !== 3 && args.length !== 4) return null;
	// A partly transparent color would need the hex-alpha spelling, which is the
	// target's question, not this one's.
	if (args.length === 4 && args[3] !== "1" && args[3] !== "100%") return null;
	const hue = _HSL_HUE_RE.exec(args[0]);
	const saturation = _HSL_NUMBER_RE.exec(args[1]);
	const lightness = _HSL_NUMBER_RE.exec(args[2]);
	if (hue === null || saturation === null || lightness === null) return null;
	if (saturation[2] !== "%" || lightness[2] !== "%") return null;
	...
	/** @type {number[]} */
	const channels = [];
	for (const n of [0, 8, 4]) {
		const k = (n + h / 30) % 12;
		const raw = (l - a * Math.max(-1, Math.min(k - 3, 9 - k, 1))) * 255;
		const byte = Math.round(raw);
		// Anything that had to be rounded is where implementations differ.
		if (Math.abs(raw - byte) > 1e-9) return null;
		channels.push(byte);
	}
	return _shortestColor(channels[0], channels[1], channels[2]);
};
...
const _inTapHighlightColor = () => {
	...
	const name = A.name(_valueDeclaration);
	// A custom property is substituted somewhere this cannot see, so whether the
	// bug applies is unknowable — keep the function form there too.
	if (name.startsWith("--")) return true;
	return name.toLowerCase() === "-webkit-tap-highlight-color";
};

// `<easing-function>` argument lists that are exactly a shorter keyword, keyed
// by the arguments as `Number` prints them.
const _CUBIC_BEZIER_KEYWORDS = new Map([
...
const _minifyEasingFunction = (fn, inner) => {
	...
	if (fn === "cubic-bezier") {
		...
		const keyword = _CUBIC_BEZIER_KEYWORDS.get(key);
...
// pseudo-elements CSS 1 and 2 introduced, so their second colon carries nothing.
// Only these four — `::selection` and the rest have no legacy spelling.
const _LEGACY_PSEUDO_ELEMENTS = new Set([
	"before",
	"after",
	"first-line",
	"first-letter"
...
const _dropLegacyPseudoElementColon = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (
			...
			parts[first] === ":" &&
			parts[second] === ":" &&
			_LEGACY_PSEUDO_ELEMENTS.has(parts[i].toLowerCase())
		) {
			parts[first] = "";
		}
		first = second;
...
// in the same compound. A `_SEP` between them would be a descendant combinator
// instead, and `|` makes the `*` a namespace's, not a redundant one.
const _COMPOUND_CONTINUATIONS = new Set([":", ".", "#", "["]);
...
const _dropImpliedUniversalSelector = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if (
			previous !== -1 &&
			parts[previous] === "*" &&
			_COMPOUND_CONTINUATIONS.has(part.charAt(0)) &&
...
// No dataset states the list; it is CSS Values 4's, spelled out.
const _CSS_WIDE_KEYWORDS = new Set([
	"inherit",
	"initial",
	"revert",
	"revert-layer",
...
const _collapseBox = (values) => {
	...
	for (const value of values) {
		...
		if (n > 1 && _CSS_WIDE_KEYWORDS.has(toLowerCaseIfNeeded(value))) {
			return null;
		}
	}
	let end = n;
...
// The two `flex` values CSS Flexbox 7.1.1 gives a keyword spelling. `1 1 0` is
// deliberately absent: the one-value `flex:1` expands to `1 1 0%`, and a length
// `0` is not a percentage `0%` when the container's main size is indefinite.
const _FLEX_KEYWORDS = new Map([
	["0 0 auto", "none"],
	["1 1 auto", "auto"]
]);
...
const _collapseFlexShorthand = (path, children, writer) => {
	...
	const keyword = _FLEX_KEYWORDS.get(components.join(" ").toLowerCase());
	return keyword === undefined ? null : [keyword];
};

// CSS Fonts 4 §2.2 defines these two keywords as those weights, and the
// computed value is the number either way — `getComputedStyle().fontWeight`
// already reports `400` for `normal`. `bolder` / `lighter` are relative to the
// parent's weight, so they have no fixed number.
const _FONT_WEIGHT_NUMBERS = new Map([
	["normal", "400"],
	["bold", "700"]
]);
...
const _collapseFontWeight = (children, writer) => {
	...
	const number = _FONT_WEIGHT_NUMBERS.get(parts[0].toLowerCase());
...
const _mergeBoxLonghands = (path, decls, writer) => {
	...
	for (const [shorthand, longhands] of BOX_LONGHANDS) {
		...
		if (ordered[3] - ordered[0] !== 3) continue;
...
// `flex-basis` is the one place a zero's unit is still load-bearing: IE 11 drops
// a `flex` shorthand whose basis has none, and the shorthand carries one too.
const _ZERO_UNIT_KEEPING_PROPERTIES = new Set(["flex", "flex-basis"]);
...
// At-rules whose empty block is inert, so dropping it changes nothing. Not
// `@keyframes` (an empty one still runs the animation, firing its events) and
// not `@layer` (an empty block declares the layer's cascade order).
const _DROPPABLE_WHEN_EMPTY_AT_RULES = new Set([
	"media",
	"supports",
	"container"
]);
	...
	switch (path.type()) {
		...
		case T_FUNCTION: {
			...
			const color = _inSupportsPrelude
				? null
				: _minifyColorFunction(fn, inner) || _minifyHslFunction(fn, inner);
			if (
				color !== null &&
		...
		case T_DECLARATION: {
			...
			if (children.length !== 0) {
				...
				// A custom property's value is a token stream `var()` substitutes
				// elsewhere, so the per-token rewrites hold — each rewrites a token to
				// one that reads back the same. It is still off unless asked for: a
				// custom property is the one declaration whose authored text
				// `getComputedStyle().getPropertyValue()` hands back, and no other
				// minifier rewrites it by default either. Nothing property-aware runs,
				// since the property it lands in is not known here.
				const rewriteCustom =
					custom && minify && _customPropertyValuesWanted(writer);
				if ((custom && !rewriteCustom) || property === "unicode-range") {
					...
					if (minify && !_ZERO_UNIT_KEEPING_PROPERTIES.has(property)) {
		...
		case T_QUALIFIED_RULE: {
			...
			if (minify) {
				...
				// `_DROPPABLE_WHEN_EMPTY_AT_RULES`).
				if (
					body.length === 0 &&
					(path.type() === T_QUALIFIED_RULE ||
						_DROPPABLE_WHEN_EMPTY_AT_RULES.has(path.name().toLowerCase()))
				) {
					return "";
				}
			}
