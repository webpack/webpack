...
class CssInjectStyleRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		// a node platform has no DOM, so it always collects; the neutral platform has
		// to decide at runtime because the same bundle also runs in a browser
		const withoutDocument =
			!outputOptions.environment.document &&
