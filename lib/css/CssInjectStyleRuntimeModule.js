...
class CssInjectStyleRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		// Only emit the nonce check when the nonce global is part of the runtime
		...
		const withScriptNonce =
			_runtimeRequirements && _runtimeRequirements.has(RuntimeGlobals.nonce);
		...
		const createStyleElementCode = Template.asString([
			...
			...(withScriptNonce
				? [
						...
						Template.indent(
							`style.setAttribute("nonce", ${RuntimeGlobals.nonce});`
						),
						"}"
					]
				: []),
...
export default CssInjectStyleRuntimeModule;

export { CssInjectStyleRuntimeModule as "module.exports" };
