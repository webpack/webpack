...
/**
 * Compilation hooks that let plugins customize how runtime-generated `<style>`
 * elements are created before css-injection code is emitted.
...
/**
 * Runtime module that injects compiled CSS into `<style>` tags and supports
 * removing or replacing those tags during hot updates.
 */
class CssInjectStyleRuntimeModule extends RuntimeModule {
	/**
	 * Returns the per-compilation hook set used to customize emitted style-tag
	 * creation code.
	...
	/**
	 * Stores the runtime requirements so the emitted helper can enable optional
	 * features such as CSS HMR support.
	...
	/**
	 * Generates helpers that locate, insert, update, and remove runtime-managed
	 * `<style>` elements keyed by module identifiers.
