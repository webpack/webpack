...
class CssInjectStyleRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		// a platform known to have no DOM (`target: "node"`) always collects; the
		// neutral platform has to decide at runtime because the same bundle also
		// runs in a browser
		const withoutDocument =
			!outputOptions.environment.document && !isNeutralPlatform;
		const collectStyle = [
		...
		/**
		 * @param {string[]} body the no-DOM branch
		 * @returns {string[]} the branch, guarded when the platform is only maybe DOM-less
		 */
		const withoutDocumentGuard = (body) => {
			if (withoutDocument) return body;
			if (!isNeutralPlatform) return [];
			return [
		...
		const injectGuard = withoutDocumentGuard(collectStyle);
		const removeGuard = withoutDocumentGuard(dropStyles);
