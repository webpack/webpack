...
// `SourceProcessor` drives the parse and hands already-built AST nodes to the visitors; byte-level source-cursor scans live in this module (see `skipWhitespace` & friends below).
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * One inherited-context entry per open block. `ancestorHadLocal` lets
		 * nested rules inherit purity from a local-bearing ancestor;
		 * `skipChildren` propagates a check-suppressing ancestor (e.g. the
		 * keyframe selectors inside `@keyframes`). The reporting decision is
		 * made at rule entry from the already-parsed body, so no per-rule
		 * accounting is carried on the frame.
		...
		/**
		 * Pure-mode at-rules whose prelude is itself selector-checked, so their
		 * body is opaque to the enclosing rule's declaration accounting.
		...
		/**
		 * Whether a rule body holds a declaration counted against the enclosing
		 * rule: a direct declaration, or one inside a transparent conditional-
		 * group at-rule (`@media` / `@supports` / `@layer` / …) whose
		 * declarations belong to the nearest real rule. Qualified rules and
		 * selector-checked at-rules are opaque.
		...
		/**
		 * Whether a rule body holds a nested block — a qualified rule or any
		 * block-bearing at-rule.
		 * @param {SimpleBlock} block rule body (`{ … }`)
		...
		/**
		 * Pure-mode rule entry. The prelude purity is known and the body is
		 * already parsed, so an impure leaf-ish rule is reported immediately;
		 * then the inherited-context frame is pushed and the per-rule selector
		 * flags are reset.
		...
		/**
		 * The child component-value list of an AST node — a function / block's
		 * `value` or a rule's `prelude` — for sibling lookahead.
		 ...
		 * @returns {void}
		 */
		// AST walker over `parseAStylesheet`'s tree; emits deps while the streaming walker (run after) owns everything not yet migrated.
		// `allowAstImport` mirrors `allowImportAtRule`: true until the first top-level block-bearing rule.
		let allowAstImport = true;
		// Persistent CSS-Modules mode for a top-level rule (mirrors `modeData`): set by bare `:local` / `:global`, leaks into sibling rules, reset at each top-level `}`.
		/** @type {"local" | "global" | undefined} */
		let astModeData;
		// Suppress localizing the next qualified rule's selectors after a non-AST `;`-terminated at-rule (mirrors `isNextRulePrelude = false`).
		let astSuppressNextRulePrelude = false;
		// Whether the current rule's prelude declared a local-mode anchor selector (used to avoid overlapping the streaming walker's `composes` strip-dep).
		let astCurrentRuleHasLocalAnchor = false;
		// The current rule's local class / id names in source order (composes reads `[0]` as the anchor); saved / restored per `walkAstQualifiedRule`.
		/** @type {string[]} */
		...
		const walkAstSelectorList = (values, localMode, topLevel = true) => {
			// At a rule's top level, inherit persistent `astModeData` (or the one-shot `astSuppressNextRulePrelude` → "global"); recursive calls use the passed `localMode`.
				...
				if (v.type === "Colon") {
					...
					if (next.type === "Ident") {
						...
						if (id === "local" || id === "global") {
							// Bare `:local` / `:global`: switch the segment (and top-level persistent) mode and strip the marker + trailing whitespace (source-level so a comment stops the strip).
		...
		/**
		 * Whether url() deps are emitted in the current value context, decided
		 * from the enclosing structural node (`astCurrentStructural`): false
		 * outside `options.url`, and inside an `@import` prelude (the url there
		 * is the import target, unless malformed-`@import` url recovery is on).
		 * @returns {boolean} true when url() deps should be emitted
		 */
		const urlActive = () => {
			if (!this.options.url || !astCurrentStructural) return false;
			if (astCurrentStructural.type === "AtRule") {
				return (
					`@${/** @type {AtRule} */ (astCurrentStructural).name.toLowerCase()}` !==
		...
		/**
		 * At-rules whose CSS-Modules prelude / body handling is owned by a
		 * dedicated path, so the generic `local()` / `global()` value rewrite
		 * and dashed-ident scoping are off for them.
		 * @param {string} name at-rule name including the leading `@`, lower-cased
		...
		/**
		 * Whether a `composes:` declaration with a local anchor selector owns the
		 * whole declaration (its strip-dep covers the value), suppressing the
		 * generic value rewrites.
		...
		/**
		 * Whether `local()` / `global()` value functions are rewritten to ICSS in
		 * the current value context, decided from `astCurrentStructural`.
		 * @returns {boolean} true when the rewrite is active
		 */
		const localGlobalActive = () => {
			if (!isModules || !astCurrentStructural) return false;
			if (astCurrentStructural.type === "AtRule") {
				return !isLocalHandledAtRule(
					`@${/** @type {AtRule} */ (astCurrentStructural).name.toLowerCase()}`
				);
			}
			if (astCurrentStructural.type === "Declaration") {
				return !composesAnchorSkip(
					/** @type {Declaration} */ (astCurrentStructural)
				);
			}
			return false;
		};
		...
		/**
		 * Whether `@value`-defined idents / function names are ICSS-rewritten in
		 * the current value context, decided from `astCurrentStructural`.
		 * @returns {boolean} true when the rewrite is active
		 */
		const icssActive = () => {
			if (!isModules || !astCurrentStructural) return false;
			if (astCurrentStructural.type === "AtRule") {
				const name = `@${/** @type {AtRule} */ (astCurrentStructural).name.toLowerCase()}`;
				return name !== "@value" && name !== "@import";
			}
			if (astCurrentStructural.type === "Declaration") {
				const decl = /** @type {Declaration} */ (astCurrentStructural);
				return (
		...
		const visitors = {
			AtRule: {
				...
				// At-rule exit: pure-frame finalization, `astSuppressNextRulePrelude`, scope restore, top-level reset.
				exit: (node, parent) => {
					...
					if (parent === null) {
						if (/** @type {AtRule} */ (node).block) allowAstImport = false;
			...
			Declaration: (decl) => {
				...
				composesHandling: if (isComposesWithAnchor) {
					...
					// Strip the whole `composes: …;` (property name included) plus trailing same-line whitespace.
					const resumeAt =
			...
			Url: (node) => {
				...
				if (
					astCurrentStructural &&
					astCurrentStructural.type === "Declaration"
				) {
					const prop = /** @type {Declaration} */ (astCurrentStructural).name
						.replace(/^(-\w+-)/, "")
						.toLowerCase();
					const localMode = astModeData
						? astModeData === "local"
						: mode === "local";
