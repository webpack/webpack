...
/** @typedef {import("./walkCssTokens").TopLevelParser} TopLevelParser */
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// `as` selects the top-level parser. A `style` attribute is a bare
		// declaration list — "parse a block's contents" (§5.3.6) — so walk its
		// declarations then nested rules; everything else is a full stylesheet
		// (the default).
		/** @type {TopLevelParser | undefined} */
		const parse =
			this.options.as === "declaration-list"
				? (ts, onNode) => {
		...
		new SourceProcessor()
			...
			.process(source, { locConverter, comment, parse });
