...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		let balanced = [];
		/** @type {undefined | { start: number, url?: string, urlStart?: number, urlEnd?: number, layer?: string, layerStart?: number, layerEnd?: number, supports?: string, supportsStart?: number, supportsEnd?: number, media?: string  }} */
				...
				switch (scope) {
					...
					case CSS_MODE_AT_IMPORT_EXPECT_LAYER_OR_SUPPORTS_OR_MEDIA: {
						...
						if (
							...
							this._emitWarning(
								state,
								`An URL in @import at ${importData.urlStart} should be before @layer or @supports`,
								locConverter,
								importData.start,
								end
							);
						...
						if (importData.layerStart > importData.supportsStart) {
							this._emitWarning(
								state,
								`The layer(...) in @import at ${importData.urlStart} should be before @supports`,
								locConverter,
								importData.start,
								end
							);
						...
						if (pos !== semicolonPos - 1) {
							importData.media = input.slice(pos, semicolonPos - 1).trim();
						}
