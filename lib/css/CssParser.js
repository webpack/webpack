					...
					for (const cv of cvs) {
						if (cv.type === "function") {
							...
							if (fname === "local" || fname === "global") {
								...
								// E3c: `url()` / `src()` / `image-set()` URL deps
								// are emitted by `walkAstBlockContents` instead;
								// just recurse through other functions for the
								// remaining local/global/var side-effects.
		...
		const walkAstBlockContents = (items, topLevel) => {
			for (const item of items) {
				if (item.type === "at-rule") {
					...
					item.type === "declaration" && // URL deps in declaration values (e.g. `background:
					// url(...)`, `src: url(...) format(...)`, `image-set(...)`).
					// Other declaration processing (CSS-Modules name
					// rewriting, `composes:` etc.) still lives in the
					// streaming walker until E4. In modules + local mode
					// the streaming walker's `case "url"` for unquoted url
					// tokens was skipped past by `processLocalDeclaration`
					// — but only when that helper actually consumed the
					// whole declaration, which happens iff
					// `isLocalMode() && knownProperties.has(propertyName)`.
					// Otherwise it bails right after the `:` and the
					// streaming walker resumed firing url-token deps for
					// the value. Mirror the same gate.
					this.options.url
				) {
					...
					const propertyName = decl.name.replace(/^(-\w+-)/, "").toLowerCase();
					const skipUrlTokens =
						isModules &&
						effectiveLocalMode &&
						knownProperties.has(propertyName);
