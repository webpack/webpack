...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {undefined | { start:number, keyStart?: number, keyEnd?:number, isNextAlias?: boolean, aliasStart?: number, aliasEnd?: number, isNextPath?:boolean, pathStart?: number, pathEnd?: number, valueStart?: number, valueEnd?:number}} */
		let atValueData;
		...
		walkCssTokens(source, {
			...
			isConsumeColon: () => Boolean(atValueData),
			...
			string: (input, start, end) => {
				switch (scope) {
					...
					case CSS_MODE_TOP_LEVEL: {
						if (atValueData && atValueData.isNextPath) {
							atValueData.pathStart = start;
							atValueData.pathEnd = end;
							atValueData.isNextPath = false;
						}
					}
				}
				return end;
			...
			semicolon: (input, start, end) => {
				switch (scope) {
					...
					case CSS_MODE_TOP_LEVEL: {
						if (atValueData && atValueData.keyStart) {
							if (atValueData.valueStart) {
								const valueStart = walkCssTokens.eatWhitespaceAndComments(
									input,
									atValueData.valueStart
								);
								const key = input.slice(
									atValueData.keyStart,
									atValueData.keyEnd
								);
								const value = input.slice(valueStart, start);
								if (!value) {
									this._emitWarning(
										state,
										`Invalid value definition: ${key}`,
										locConverter,
										atValueData.start,
										end
									);
								}
								const exportDep = new CssExportDependency(key, value);
								...
								modulesValues[key] = value;
								const pos = walkCssTokens.eatWhitespaceAndComments(input, end);
								const constDep = new ConstDependency("", [
									atValueData.start,
									pos
								]);
								module.addPresentationalDependency(constDep);
								atValueData = undefined;
								return pos;
							} else if (atValueData.pathStart) {
								// :import
							} else {
								this._emitWarning(
									state,
									`Invalid value definition: ${input.slice(atValueData.keyStart, atValueData.keyEnd)}`,
									locConverter,
									atValueData.start,
									end
								);
			...
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						const value = modulesValues[input.slice(start, end)];
						if (value !== undefined) {
							const dep = new ConstDependency(value, [start, end]);
							module.addPresentationalDependency(dep);
						}
						if (isLocalMode()) {
					...
					case CSS_MODE_TOP_LEVEL: {
						if (atValueData && atValueData.start !== undefined) {
							if (input.slice(start, end) === "from") {
								atValueData.isNextPath = true;
							} else if (input.slice(start, end) === "as") {
								atValueData.isNextAlias = true;
							} else if (atValueData.isNextAlias) {
								atValueData.aliasStart = start;
								atValueData.aliasEnd = end;
								atValueData.isNextAlias = false;
							} else if (!atValueData.keyStart) {
								atValueData.keyStart = start;
								atValueData.keyEnd = end;
							}
							return end;
						}
					}
			...
			class: (input, start, end) => {
				const value = modulesValues[input.slice(start + 1, end)];
				if (value !== undefined) {
					const dep = new ConstDependency(value, [start + 1, end]);
					module.addPresentationalDependency(dep);
				}
				if (isLocalMode()) {
			...
			colon: (input, start, end) => {
				if (atValueData && atValueData.keyStart && !atValueData.valueStart) {
					atValueData.valueStart = end;
				}
				return end;
			}
		});
