...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// `as` selects the top-level parse entry point: a `style` attribute is a
		// block's contents (§5.3.6), everything else a full stylesheet.
		const parse =
			this.options.as === "block" ? parseABlocksContents : parseAStylesheet;
		new SourceProcessor()
