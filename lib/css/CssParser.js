...
class CssParser extends Parser {
	...
	/**
	 * @template T
	 * @param {Array<T>} stack stack
	 * @returns {T | undefined} last element of stack
	 */
	peekStack(stack) {
	...
	parse(source, state) {
		...
		// keep a stack of class definitions
		/** @type {CssModuleDefineDependency[]} */
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (propertyName.startsWith("--")) {
				...
				let parentClassDefinition = this.peekStack(classDefinitions);
		...
		walkCssTokens(source, {
			...
			rightCurlyBracket: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						...
						let lastClassDefinition = this.peekStack(classDefinitions);

						if (
							lastClassDefinition &&
							lastClassDefinition.getBlockNestingLevel() === blockNestingLevel
						) {
							lastClassDefinition = classDefinitions.pop();

							lastClassDefinition.setRange([
								lastClassDefinition.getRange()[0],
								end
							]);
							...
							const { line: el, column: ec } = locConverter.get(
								lastClassDefinition.getRange()[1]
							);
							lastClassDefinition.setLoc(sl, sc, el, ec);
						}
			...
			class: (input, start, end) => {
				...
				// check if another class definition is already in progress at same level
				// else create a new class definition
				const lastClassDefinition = this.peekStack(classDefinitions);
				if (
					lastClassDefinition &&
					lastClassDefinition.getBlockNestingLevel() === blockNestingLevel
				) {
					classDefinition = lastClassDefinition;
				} else {
					let parentClassDefinition = lastClassDefinition;

					classDefinition = new CssModuleDefineDependency(
						[],
						[0, 0],
						blockNestingLevel,
						parentClassDefinition
					);
					module.addDependency(classDefinition);
					classDefinitions.push(classDefinition);
				}

				classDefinition.appendIdentifier({
					name,
					range: [start + 1, end],
					isGlobal: !isLocalMode()
				});

				// update the range of the class if this is the first identifier
				if (classDefinition.getIdentifiers().length === 1)
					classDefinition.setRange([start, 0]);
