...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const processAttributeSelector = (input, start, end) => {
			// Pre-skip leading whitespace; failure paths return this position
			// so the streaming walker can re-tokenize the (non-class)
			// attribute name as a normal identifier.
			...
			if (!nameCV || nameCV.value.type !== "ident") return end;
			const name = unescapeIdentifier(
				input.slice(nameCV.value.start, nameCV.value.end)
			);
			if (name.toLowerCase() !== "class") return end;
		...
		// not localised. Set in `walkAstAtRule` and cleared at the
		...
		const walkAstSelectorList = (values, localMode, topLevel = true) => {
			...
			// (so class selectors aren't localised), matching the
