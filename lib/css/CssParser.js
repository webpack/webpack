...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const processDashedIdent = (input, start, end) => {
			...
			const source = walkCssTokens.eatIdentSequenceOrString(
				input,
				walkCssTokens.eatWhitespaceAndComments(input, fromWord[1])[0]
			);
			if (!source) return end;
			const isString = source[2] === false;
			const text = input.slice(source[0], source[1]);
			if (!isString && text === "global") {
				emitDashedIdentFromGlobal(identEnd, source[1]);
			} else if (isString) {
