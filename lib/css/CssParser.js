...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// SourceProcessor fires while walking that node's value / prelude.
		...
		/**
		 * Url-token visitor — bare `url(unquoted)`.
		 * @param {AstNode} node url token
		 */
		const onValueUrl = (node) => {
			if (vUrl && !vUrlSkip) {
				processOldURLFunction(/** @type {UrlToken} */ (node));
			}
		};

		/**
		 * Comma visitor — advances the magic-comment fence.
		 * @param {AstNode} node comma token
		 */
		const onValueComma = (node) => {
			if (vUrl) lastTokenEndForComments = node.start;
		};

		/**
		 * Function visitor — url / src / image-set (deps), local / global
		 * (ICSS), and `@value`-defined function names (ICSS rewrite).
		 * @param {AstNode} node function node
		 */
		const onValueFunction = (node) => {
			const fn = /** @type {FunctionNode} */ (node);
			const fnameRaw = fn.name.replace(/\\/g, "");
			const fname = fnameRaw.toLowerCase();
			if (vUrl) {
				if (fname === "url" || fname === "src") processURLFunction(fn, fname);
				else if (IMAGE_SET_FUNCTION.test(fname)) processImageSetFunction(fn);
			}
		...
		/**
		 * Ident visitor — `@value`-defined ident rewrite.
		 * @param {AstNode} node ident token
		 */
		const onValueIdent = (node) => {
			if (!vIcss) return;
		...
		const processAstDeclaration = (decl, topLevel) => {
			...
			// Skip bare url-tokens for known properties in local mode (the
			// streaming walker's `processLocalDeclaration` consumed the whole
			// value then).
			if (this.options.url) {
				vUrl = true;
				vUrlSkip =
			...
			if (isComposesWithAnchor) processComposesAst(decl);
			...
			// Dashed-ident (custom-property) ICSS export. The property name is
			// itself an export; the value walk (kept inline — it has `from
			// <source>` lookahead that doesn't map to a node visitor) handles
			// nested `var()` / `style()` / `--foo()` and top-level dashed
			// idents in unknown-property values.
			if (
				this.options.dashedIdents &&
				isModules &&
				effectiveLocalMode &&
		...
		const walkAstAtRuleEnter = (at, topLevel) => {
			...
			// `@import` url() is handled by `processAtImport` (it calls
			// `walkFunctionsForUrl` in its recovery path), so skip url here.
			const effectiveLocalMode = astModeData
				? astModeData === "local"
				: mode === "local";
			...
			vUrl = vUrlSkip = vLocalGlobal = vIcss = vIcssDashedHandled = false;
			if (this.options.url && name !== "@import") {
				vUrl = true;
		...
		const runAstWalker = () => {
			...
			processor.use({
				...
				// The enclosing structural enter has set `vUrl` / `vIcss` / …
				// to gate them per container.
				Url: onValueUrl,
				Comma: onValueComma,
				Function: onValueFunction,
				Ident: onValueIdent
			});
