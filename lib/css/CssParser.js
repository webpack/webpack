...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @typedef {{ kind: string, line: number, column: number, kinds: Set<string> }} DeclaredExport */
		/** @type {Map<string, DeclaredExport>} */
		const declaredExports = new Map();

		/**
		 * Registers an export dependency and emits a warning when a
		 * declaration's export name was already declared elsewhere with a
		 * different kind (for example a class and a custom property both
		 * named `foo`). Each conflicting kind is reported at most once per
		 * name.
		 *
		 * Property-value references (e.g. `animation: foo` referencing a
		 * `@keyframes foo`) are not declarations and pass `isDeclaration =
		 * false`; they are added to the dependency list but skipped for
		 * conflict tracking, so referencing a class name from `animation-name`
		 * does not produce a spurious warning.
		 *
		 * `EXPORT_MODE.NONE` (e.g. attribute selectors) does not produce a JS
		 * export and is not tracked. `EXPORT_MODE.APPEND` (e.g. `composes`)
		 * and `EXPORT_MODE.SELF_REFERENCE` are intentional contributions to an
		 * existing export and are also skipped.
		 * @param {CssIcssExportDependency} dep export dependency
		 * @param {string} kind human-readable export kind
		 * @param {number} start start offset (within the original source) of the conflicting declaration
		 * @param {number} end end offset of the conflicting declaration
		 * @param {boolean=} isDeclaration true when this is a declaration site (default), false for property-value references that should not participate in conflict tracking
		 */
		const addExportDependency = (dep, kind, start, end, isDeclaration) => {
			if (
				isDeclaration !== false &&
				...
				const existing = declaredExports.get(dep.name);
				if (existing) {
					if (!existing.kinds.has(kind)) {
						this._emitWarning(
							state,
							`Conflicting CSS module export "${dep.name}": already declared as ${existing.kind} at line ${existing.line}:${existing.column}, redeclared as ${kind}`,
							locConverter,
							start,
							end
						);
					...
					const { line, column } = locConverter.get(start);
					declaredExports.set(dep.name, {
						kind,
						line,
						column,
						kinds: new Set([kind])
		...
		const processImportOrExport = (type, input, pos) => {
			...
			const createDep = (name, value, start, end) => {
				if (type === 0) {
					...
					const { line: el, column: ec } = locConverter.get(end);
					dep.setLoc(sl, sc, el, ec);
					addExportDependency(dep, ":export", start, end);
				}
			};
			...
			if (
				...
				for (const { importName, localName } of items) {
...

					{
						...
						const { line: el, column: ec } = locConverter.get(end);
						dep.setLoc(sl, sc, el, ec);
						addExportDependency(dep, "@value", start, end);
					}
				}
			} else {
		 ...
		 * @param {number} start start position
		 * @param {number} end end position
		 * @param {string=} kindHint human-readable export kind for the surrounding context (used when the inner identifier is not a custom property)
		 * @param {boolean=} isDeclaration whether this `:local(...)` is a declaration (selector context) or a reference (property-value context)
		 * @returns {number} position after handling
		 */
		const processLocalOrGlobalFunction = (
			input,
			type,
			start,
			end,
				...
				start,
				{
					identifier(input, start, end) {
						if (type === 1) {
							...
							addExportDependency(
								dep,
								isDashedIdent ? "custom property" : kindHint || "class",
								start,
								end,
								isDeclaration
							);
		 ...
		 * @param {string} kind human-readable export kind (e.g. `@keyframes`)
		 * @returns {number} position after handling
		 */
		const processLocalAtRule = (input, end, options, kind) => {
			let found = false;
				...
				end,
				{
					string(_input, start, end) {
						if (!found && options.string) {
							...
							dep.setLoc(sl, sc, el, ec);
							addExportDependency(dep, kind, start, end);
							found = true;
				...
				},
				{
					function: (input, start, end) => {
						...
						if (!found && type) {
							...
							return processLocalOrGlobalFunction(
								input,
								type,
								start,
								end,
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (isLocalMode() && knownProperties.has(propertyName)) {
				...
				const isGridTemplate = isGridProperty
					...
					: false;
				const valueKind = isGridProperty
					? "grid identifier"
					: propertyName === "animation" || propertyName === "animation-name"
						? "@keyframes"
						: propertyName === "container" || propertyName === "container-name"
							? "@container"
							: "@counter-style";
				...
				if (values.length > 0) {
					for (const value of values) {
						...
						dep.setLoc(sl, sc, el, ec);
						addExportDependency(dep, valueKind, start, end, false);
					}
				}
		...
		const processIdSelector = (input, start, end) => {
			...
			const { line: el, column: ec } = locConverter.get(end);
			dep.setLoc(sl, sc, el, ec);
			addExportDependency(dep, "id", valueStart, end);
			if (pureMode) currentSelectorHasLocal = true;
			return end;
		};
		...
		const processClassSelector = (input, start, end) => {
			...
			dep.setLoc(sl, sc, el, ec);
			addExportDependency(dep, "class", ident[0], ident[1]);
			if (pureMode) currentSelectorHasLocal = true;
			return ident[1];
		};
