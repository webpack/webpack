...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// const getWebpackIgnoreCommentValue = (input, comment, pos) => {
		// 	const isNear = (input, prevPos, nextPos) => {
		// 		prevPos++;
		// 		while (input.charCodeAt(prevPos) === CC_LINE_FEED) {
		// 			prevPos++;
		// 		}
		// 		return prevPos === nextPos;
		// 	};
		// 	const matched = comment.content.match(WEBPACK_IGNORE_COMMENT_REGEX);
		// 	return (
		// 		isNear(input, lastComment.end, pos) && matched && matched[2] === "true"
		// 	);
		// };
		...
		walkCssTokens(source, {
			...
			semicolon: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_AT_IMPORT: {
						...
						const { options } = this.parseCommentOptions([start, end]);
			...
			comment: (input, start, end) => {
				this.comments.push({ value: input.slice(start, end), start, end });
				return end;
			}
		});
