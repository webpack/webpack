	...
	parse(source, state) {
		...
		const emitUrlFunction = (fn, fname) => {
			if (fname === "url" || fname === "src") {
				...
				if (webpackIgnored(commentRange, string.loc)) {
					return;
				}
				...
				const { options } = this.parseCommentOptions(commentRange);
				...
				for (const cv of fn.value) {
					...
					const commentRange = /** @type {Range} */ ([start, string.end]);
					if (webpackIgnored(commentRange, string.loc)) continue;
					const dep = new CssUrlDependency(
						value,
						[string.start, string.end],
						"url"
		...
		const visitors = {
			...
			[NodeType.Url]: (node) => {
				...
				const urlCommentRange =
					/** @type {Range} */
					([lastTokenEndForComments, node.end]);
				if (webpackIgnored(urlCommentRange, rangeLoc(node.start, node.end))) {
					return;
				}
				const { options: urlComments } =
					this.parseCommentOptions(urlCommentRange);
				...
				applyResourceHintDefaults(
					...
					urlComments,
					{
						start: locConverter.get(node.start),
						end: locConverter.get(node.end)
					}
				);
				module.addDependency(dep);
				module.addCodeGenerationDependency(dep);
