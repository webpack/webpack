...
// Functions whose arguments are *guaranteed* not to contain a `url()` /
// `src()` / `image-set()` — calc / var / attr / env, the math fns, the
// color fns, the gradients, and the other shape / counter / format
// helpers. Used by `walkFunctionsForUrl` to skip recursion into bodies
// where there'd be nothing to find. The list is conservative — only
// functions whose grammar can be reasoned about from the CSS specs.
const URL_FREE_FUNCTION_NAMES = new Set([
	"calc",
	"var",
	"attr",
	"env",
...
const eatUntilLeftCurly = eatUntil("{");
	...
	parse(source, state) {
		...
		const processAtImport = (atRule) => {
			...
			if (urlNode.type === "Ident") {
				...
				if (!def) {
					...
					// Consume the whole at-rule so the unresolved identifier isn't re-tokenized into a malformed `@import`.
			...
			const newline = eatWhiteLine(input, semi);
			...
			const mediaStart = eatWhitespaceAndComments(input, lastPrefixPart.end)[0];
			/** @type {undefined | string} */
			let media;
		...
		const walkFunctionsForUrl = (cvs, skipUrlTokens = false) => {
			for (const cv of cvs) {
				if (cv.type === "Comma") {
					...
					} else if (!URL_FREE_FUNCTION_NAMES.has(fname)) {
						// Skip recursion into functions whose grammar
						// can't carry a `url()` / `src()` /
						// `image-set()` — `calc(...)`, gradients, color
						// functions, etc. (see `URL_FREE_FUNCTION_NAMES`).
						walkFunctionsForUrl(fn.value, skipUrlTokens);
					}
		...
		const processImportOrExport = (type, input, pos) => {
			...
			if (type === 0) {
				...
				if (
					!headerCV ||
					headerCV.value.type !== "SimpleBlock" ||
		...
		const processComposesAst = (decl) => {
			...
			const resumeAt =
				source.charCodeAt(decl.end) === CC_SEMICOLON
					? eatWhitespace(source, decl.end + 1)
					: decl.end;
			module.addPresentationalDependency(
		...
		const processAttributeSelector = (input, start, end) => {
			end = eatWhitespaceAndComments(input, end)[0];
			...
			end = eatWhitespaceAndComments(input, nameCV.value.end)[0];
		 ...
		 */
		/**
		 * Process a `Declaration` AST node — URL deps, `local(…)` /
		 * `global(…)` strip + ICSS exports, composes processing,
		 * dashed-ident exports, ICSS-symbol rewrites. Read `decl.name`
		 * / `decl.value` / `decl.nameRange`.
		 *
		 * Mirrors the streaming walker's `case "identifier"` /
		 * `case "function"` / `case "url"` dispatch in declaration
		 * value position. `topLevel` is forwarded only to drive the
		 * end-of-item `astModeData = undefined` reset that the
		 * streaming walker did at every top-level `;` / `}` —
		 * SourceProcessor's nested visitor calls pass `false`.
		 * @param {Declaration} decl declaration node
		 * @param {boolean} topLevel whether this declaration sits at the stylesheet's top level
		 * @returns {void}
		 */
		const processAstDeclaration = (decl, topLevel) => {
			if (pureMode) {
				// Pure-mode: a direct declaration in the body marks the
				// enclosing rule frame as having declarations. The streaming
				// walker set this in `case "identifier"` / `case "semicolon"`;
				// mirroring it here is structurally cleaner because the AST
				// already separates declarations from nested rules.
				const top = pureTop();
				if (top) top.hasDirectDecl = true;
			}
			const declPropertyName = decl.name.replace(/^(-\w+-)/, "").toLowerCase();
			// `walkFunctionsForUrl` reads `lastTokenEndForComments` to attach
			...
			if (this.options.url) {
				// In modules + local mode the streaming walker's `case "url"`
				// for unquoted url tokens was skipped past by
				// `processLocalDeclaration` — but only when that helper
				// actually consumed the whole declaration, which happens
				// iff `isLocalMode() && knownProperties.has(propertyName)`.
				// Otherwise it bailed right after the `:` and the streaming
				// walker resumed firing url-token deps for the value. Mirror
				// the same gate.
			...
			// `local(…)` / `global(…)` declaration-value functions: emit
			// strip-dep for the wrapper + an ICSS export per inner ident
			// (the latter for `local(…)` only). `processLocalOrGlobalFunction`
			// does both. Only relevant in modules mode — non-modules don't
			// have ICSS exports.
			//
			// Skip when this is a `composes` / `compose-with` declaration AND
			// the rule has a local class / id / attribute anchor — in that
			// case the streaming walker's composes branch already emits a
			// strip-dep covering the whole `composes: …;` and an inner
			// ICSS-export rewrite from this walker would overlap with that
			// strip and leak the localized name into the body. The "bailed
			// composes" case (no anchor, e.g. global-mode rules) still needs
			// this walk — the streaming walker's composes branch returns just
			// past `:` and the inner `local(…)` / `global(…)` functions need
			// their strip + ICSS emission from somewhere.
			const isComposesWithAnchor =
				COMPOSES_PROPERTY.test(declPropertyName) &&
				astCurrentRuleHasLocalAnchor;
			// E4a-3: `composes:` / `compose-with:` declarations with a
			// local class anchor — process the CSS-Modules composition
			// grammar and emit the composes deps + strip-dep here. The
			// "bailed composes" case (no local anchor, e.g. in global-mode
			// rules) falls through to the regular `walkFunctionsForLocalGlobal`
			// / `walkDashedIdentsInValue` walks below so any nested
			// `local(…)` / `global(…)` / `var(--foo)` inside the composes
			// value still get their normal source rewriting.
			if (isComposesWithAnchor) {
				processComposesAst(decl);
				// The composes strip-dep covers the whole `composes: …;` so
				// the per-walk deps below would overlap with the strip and
				// leak the localized names into the output. The legacy
				// `skipForComposes` gate stays here for the dashed-ident /
				// function-LG walks below.
			}
			const skipForComposes = isComposesWithAnchor;
			if (isModules && !skipForComposes) {
				walkFunctionsForLocalGlobal(decl.value);
			}
			// E4a-2: Property-name local emission for known properties
			// (`animation-name: foo` → emits ICSS export for `foo`). Gated
			// on local mode + knownProperty.
			const effectiveLocalMode = astModeData
				? astModeData === "local"
				: mode === "local";
			if (
			...
			// E4d: dashed-ident (custom-property) ICSS export emission. The
			// dashed property name itself is an export (`--my-color: …`
			// registers `--my-color`). The value is walked for nested
			// `var()` / `style()` / `--foo(args)` calls, plus any top-level
			// dashed idents in *unknown* property values (matching the
			// streaming walker's `case "identifier"` for dashed-idents in
			// non-knownProperty declarations where `processLocalDeclaration`
			// bailed past the `:`).
			//
			// Skip when this is a `composes` / `compose-with` declaration
			// AND the rule has a local class / id / attribute anchor — same
			// gate as `walkFunctionsForLocalGlobal` (E4a-1). In that case
			// the streaming walker's composes branch set `skipUntil =
			// decl.end` past the whole declaration and inner
			// `case "identifier"` for dashed-idents never fired. Emitting
			// from the AST walker would overlap with the composes strip-dep
			// and leak the localized name into the output.
			if (
				this.options.dashedIdents &&
				isModules &&
				effectiveLocalMode &&
			...
			// E4e: ICSS-symbol rewrite in declaration values (`color: foo`
			// → CSS-Modules rewrite when `foo` is `@value`-defined). The
			// streaming walker's `case "identifier"` icss branch covered
			// this; mirror it here. Skipped when the property is "known"
			// (the streaming walker's `processLocalDeclaration`
			// short-circuited past the entire declaration so no inner
			// `case "identifier"` fired), when the rule has the composes
			// anchor (the composes strip-dep already covers the whole
			// `composes: …;`), and when the property name itself is dashed
			// (the dashed-ident walk owns its rewrite).
			if (
				isModules &&
				!skipForComposes &&
				!knownProperties.has(declPropertyName)
			) {
				// "dashed wins" priority matches the streaming walker's
				// `case "identifier"`: dashed idents are skipped here only
				// when `walkDashedIdentsInValue` will run over the same
				// value (local mode + dashed-idents option).
				...
				if (
					...
					// Property name is itself an `@value`-defined ident (e.g.
					// `myProp: …` where `myProp` is `@value myProp: actualProp;`).
					// Streaming walker's `case "identifier"` would rewrite
		...
		const walkAstBlockContents = (startPos, topLevel) => {
			...
			while (pos < source.length) {
				...
				pos = eatWhitespaceAndComments(source, pos, comment)[0];
				...
				if (source.charCodeAt(pos) === CC_AT_SIGN) {
					// The single in-flight pass: parse helpers receive
					// the `comment` callback so each comment they see is
					// pushed to `this.comments` exactly once in source
					// order. (The legacy comment-only pre-pass over
					// `walkCssTokens` is gone — see the
					// `walkAstBlockContents`-driven `runAstWalker`
					// below.)
					const at = parseAtRule(source, pos, locConverter, comment);
					if (!at) {
						pos++;
						continue;
					}
				...
				// Same `comment` callback as `parseAtRule` above.
				const decl = parseADeclaration(source, pos, locConverter, comment);
				...
				if (decl) {
					processAstDeclaration(decl, topLevel);
					pos = decl.end;
					if (pos < source.length && source.charCodeAt(pos) === CC_SEMICOLON) {
						pos++;
					}
		 ...
		 */
		/**
		 * State stack for per-at-rule scope tracking. `enter` pushes,
		 * `exit` pops + restores. Each frame carries the saved scope plus
		 * a flag for whether the at-rule has a block body — the exit half
		 * uses it to decide between the block-cleanup branch and the
		 * `astSuppressNextRulePrelude` branch.
		 * @type {{ savedAnchor: boolean, savedLocalIdentifiers: { localName: string, identifierStart: number, identifierEnd: number, isGlobal: boolean, isProcessed: boolean }[], name: string, hasBlock: boolean, terminator: "{" | "}" | ";" | null }[]}
		 */
		const atRuleStateStack = [];

		/**
		 * Enter half of the (former) `walkAstAtRule`. Runs the scope
		 * save, the named-at-rule switch dispatch, the prelude walks
		 * (URL, local/global, dashed-idents, ICSS-symbols), the pure-mode
		 * `atSkipChildren` / `atTreatAsLeaf` calculation, and the
		...
		const walkAstAtRuleEnter = (at, topLevel) => {
			...
			// e.g. `@supports (background-image: url(...))`. `@import` is
			...
			// AST-handled at-rules are skipped to avoid double-emission.
			...
			// E4d: dashed-ident emission in at-rule preludes.
			if (
				this.options.dashedIdents &&
				isModules &&
			...
			atRuleStateStack.push({
				...
				terminator: at.terminator
			});
		};

		/**
		 * Exit half of the (former) `walkAstAtRule`. Pops the
		 * `pureBlockStack` frame with finalization (for block at-rules),
		 * sets `astSuppressNextRulePrelude` for non-recognized
		 * `;`-terminated at-rules, and restores the saved scope.
		 * @returns {void}
		 */
		...
		/**
		 * Walk an at-rule. Wrapper that calls Enter, walks the body via
		 * `walkAstBlockContents` (the pre-SourceProcessor-recursion path),
		 * patches `at.end` / `at.block.end`, and calls Exit. Kept as a
		 * single entry point during the transition.
		 * @param {AtRule} at at-rule node
		 * @param {boolean} topLevel whether this at-rule sits at the stylesheet's top level
		 * @returns {void}
		 */
				...
				if (v.type === "Colon") {
					...
					if (next.type === "Ident") {
						...
						if (id === "local" || id === "global") {
							...
							const stripEnd = eatWhitespace(source, next.end);
							if (isModules) {
								module.addPresentationalDependency(
							...
							// `foundWhitespace`.
							const probe = eatWhitespaceAndComments(source, next.end);
							if (!probe[1]) {
								this._emitWarning(
									state,
						...
						if (fname === "local" || fname === "global") {
							...
							if (isModules) {
								...
								const stripLeadEnd = eatWhitespaceAndComments(
									source,
									afterParen
								)[0];
								module.addPresentationalDependency(
		...
		/**
		 * State stack for per-qualified-rule scope tracking. `enter` pushes,
		 * `exit` pops + restores. Each frame is either `{ bailed: true }`
		 * (for `:import` / `:export` pseudo-rules that handle the body
		 * themselves) or a full snapshot of the surrounding rule state.
		 * @type {{ bailed: true } | { bailed: false, savedAnchor: boolean, savedLocalIdentifiers: { localName: string, identifierStart: number, identifierEnd: number, isGlobal: boolean, isProcessed: boolean }[], savedPrevComposesFile: string | undefined, savedComposesFiles: Set<string> }[]}
		 */
		const qualifiedRuleStateStack = [];

		/**
		 * Enter half of the (former) `walkAstQualifiedRule`. Runs the
		 * scope setup, prelude walks (selectors, URLs, dashed-idents), and
		 * the `pureBlockStack` push. The matching `walkAstQualifiedRuleExit`
		 * does the corresponding teardown — once SourceProcessor drives
		 * the nested traversal it walks the body **between** these two
		 * halves and fires `Declaration` / nested `AtRule` /
		 * `QualifiedRule` visitors in source order.
		 * @param {QualifiedRule} rule qualified-rule node
		 * @param {boolean} topLevel true when this rule sits directly at the stylesheet's top level (used to gate ICSS `:import` / `:export` pseudo-rule processing the way the streaming walker did via `CSS_MODE_TOP_LEVEL`)
		 * @param {{ skipChildren(): void }=} ctx visitor context — when provided, `:import` / `:export` pseudo-rules call `skipChildren()` so SourceProcessor doesn't walk their body as a regular block
		 * @returns {void}
		 */
		const walkAstQualifiedRuleEnter = (rule, topLevel, ctx) => {
				...
				if (
					...
					if (name === "import" || name === "export") {
						if (topLevel) {
							...
							// CSS-Modules construct — leave the body alone
							// (walking it would mis-handle its declarations as
							// ICSS exports), but still advance `rule.end` past
							// the closing `}`.
							const blk = parseASimpleBlock(
								source,
								rule.block.start,
								locConverter,
								comment
			...
			// Qualified-rule preludes (selectors) don't normally carry url() /
			// src() / image-set(), but a malformed declaration followed by
			// orphan `url(...)` tokens (e.g.
			// `background: x; url("./a.png"), url("./b.png");`) gets parsed
			// as a (degenerate) nested qualified-rule whose prelude is the
			// url() chain. The streaming walker fired `case "function"` on
			// those urls and emitted URL deps — mirror that by walking the
			// prelude.
		...
		/**
		 * Exit half of the (former) `walkAstQualifiedRule`. Pops the
		 * `pureBlockStack` frame with finalization and restores the saved
		 * scope. No-op for `:import` / `:export` pseudo-rules whose enter
		 * bailed early.
		 * @returns {void}
		 */
		...
		/**
		 * Walk a qualified rule (a style rule). Wrapper that calls the
		 * Enter half, walks the body via `walkAstBlockContents` (the
		 * pre-SourceProcessor-recursion path), and then the Exit half.
		 * Kept as a single entry point for `walkAstBlockContents` callers
		 * during the transition — once `walkAstBlockContents` is gone the
		 * Enter / Exit pair is wired directly to the SourceProcessor
		 * visitor and this wrapper goes away.
		 * @param {QualifiedRule} rule qualified-rule node
		 * @param {boolean} topLevel true when this rule sits directly at stylesheet top level
		 * @returns {void}
		 */
		const walkAstQualifiedRule = (rule, topLevel) => {
			walkAstQualifiedRuleEnter(rule, topLevel);
			const stackTop =
				qualifiedRuleStateStack[qualifiedRuleStateStack.length - 1];
			if (!stackTop || stackTop.bailed) {
				walkAstQualifiedRuleExit();
				return;
			}
			if (rule.block) {
				// Body walk — preserved for `walkAstBlockContents`-driven
				// callers (nested recursion inside `walkAstAtRule`). When
				// SourceProcessor drives the body walk directly, this code
				// path is replaced by the visitor recursion firing
				// `Declaration` / nested `AtRule` / `QualifiedRule`
				// visitors in source order.
		 ...
		 * @returns {void}
		 */
		// Top-level dispatch via `SourceProcessor` (the Babel-style
		// visitor coordinator on `walkCssTokens`). The processor parses
		// the stylesheet once and fires `enter` visitors for each
		// top-level `AtRule` / `QualifiedRule` in source order; the
		// callbacks then call the existing per-rule handlers, which
		// already drive nested traversal themselves via
		// `walkAstBlockContents(start, /* topLevel */ false)`.
		//
		// `recurseBlocks: false` tells `cssGrammar` to not re-parse
		// `{ ... }` bodies into nested rules — we'd otherwise pay the
		// re-parse cost twice (the visitor handlers' own
		// `walkAstBlockContents` already walks bodies). One day, when
		// the nested recursion has been migrated onto visitors too,
		// this flag goes away and the SourceProcessor becomes the sole
		// driver.
		//
		// The `comment` callback is forwarded through `parse()` and
		// reaches every `eatWhitespaceAndComments` / `parseAtRule` /
		// `parseADeclaration` / `parseAListOfComponentValues` call the
		// grammar makes, so each comment in the source is pushed to
		// `this.comments` exactly once, in source order — no separate
		// up-front tokenization needed.
		const runAstWalker = () => {
			const processor = new SourceProcessor();
			processor.use({
				AtRule: (at, parent) => {
					if (parent !== null) return;
					if (pureMode) astAdvanceCommentCursor(at.start);
					// `parseAStylesheet` already ran `parseASimpleBlock` on
					// the body for `{`-terminator at-rules, so `at.block`
					// is already set with `block.start = '{'` /
					// `block.end = past '}'`. `walkAstAtRule` re-walks the
					// body itself (via `walkAstBlockContents`) so we don't
					// need anything else here.
					walkAstAtRule(/** @type {AtRule} */ (at), true);
					if (at.terminator === "{") allowAstImport = false;
					if (pureMode) seenTopLevelRule = true;
					astModeData = undefined;
				},
				QualifiedRule: (rule, parent) => {
					if (parent !== null) return;
					if (pureMode) astAdvanceCommentCursor(rule.start);
					walkAstQualifiedRule(/** @type {QualifiedRule} */ (rule), true);
					allowAstImport = false;
					if (pureMode) seenTopLevelRule = true;
					astModeData = undefined;
				}
				// No `Declaration` visitor: top-level declarations are spec
				// parse errors (`parseAStylesheet` already filters them out
				// of `rules`) so we never see them here.
			});
