...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		let importData;
		/** @type {{ key: string, value: string, start: number }[]} */
		let valueData = [];
		/** @type {boolean} */
		...
		const parseValue = (input, pos) => {
			...
			pos++;
			valueData.push({ key, value, start });
			return pos;
		};
		/**
		 * @param {string} input input
		...
		walkCssTokens(source, {
			...
			atKeyword: (input, start, end) => {
				...
				if (name === "@namespace") {
					...
					const pos = parseValue(input, end);
