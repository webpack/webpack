...
class CssParser extends Parser {
	...
	parse(source, state) {
		 ...
		 */
		const getReexport = (value, localName) => {
			const reexport = icssDefinitions.get(value);

			if (reexport) {
				if (reexport.importName) {
					return [reexport.localName || value, reexport.importName];
				}
		...
		const processAtValue = (input, start, end) => {
			...
			if (request) {
				...
				for (const alias of aliases) {
					const [name, localName = name] = alias.split(/\s+as\s+/);

					{
						...
						icssDefinitions.set(localName, { importName: name });
					}
