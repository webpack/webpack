...
const OPTIONALLY_VENDOR_PREFIXED_ANIMATION_PROPERTY =
	/^(-\w+-)?animation(-name)?$/i;
const LIST_STYLE_PROPERTY = /^(list-style(-type)?)$/i;
const COUNTER_STYLE_PROPERTIES = /^(system|fallback|speak-as)$/i;
const CONTAINER_PROPERTY = /^(container(-name)?)$/i;
...
class CssParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * @param {boolean} needExport true when we need to export value, otherwise false
		 * @param {number} start start position
		 * @param {number} end end position
		 * @returns {number} position after handling
		 */
				...
				start,
				{
					identifier(input, start, end) {
						if (type === 1) {
							...
							const dep = needExport
								? new CssIcssLocalIdentifierDependency(
										isDashedIdent ? identifier.slice(2) : identifier,
										[start, end],
										isDashedIdent && identifier.length >= 3 ? "--" : ""
									)
								: new CssIcssSelfLocalIdentifierDependency(
										...
										[start, end],
										isDashedIdent ? "--" : "",
										isDashedIdent ? declaredCssVariables : undefined
									);
		 ...
		 * @param {string} input input
		 * @param {number} end name end position
		 * @param {{ string?: boolean, identifier?: boolean | RegExp, dashed?: boolean }} options types which allowed to handle
		 * @returns {number} position after handling
		 */
			...
			return walkCssTokens.consumeUntil(
				...
				end,
				{
					...
					identifier(input, start, end) {
						if (!found) {
							...
							if (
								options.identifier ||
								(options.dashed && isDashedIdentifier(identifier))
							) {
								const name = unescapeIdentifier(identifier);
								...
								const { line: el, column: ec } = locConverter.get(end);
								const dep = new CssIcssLocalIdentifierDependency(
									options.dashed ? name.slice(2) : name,
									[start, end],
									options.dashed ? "--" : ""
								);
								dep.setLoc(sl, sc, el, ec);
								...
								if (options.dashed) {
									declaredCssVariables.add(name.slice(2));
								}
							}
						}
						return end;
				...
				},
				{
					function(input, start, end) {
						...
						if (isLocalMode() && (name === "var" || name === "style")) {
		 ...
		 * @param {number} start start position
		 * @param {number} end end position
		 * @param {boolean=} declaration true when in declaration, otherwise false
		 * @returns {number} position after handling
		 */
		const processDashedIdent = (input, start, end, declaration) => {
			...
			if (
				...
				if (declaration) {
					declaredCssVariables.add(identifier);
				}
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (OPTIONALLY_VENDOR_PREFIXED_ANIMATION_PROPERTY.test(propertyName)) {
				...
				return end;
			} else if (LIST_STYLE_PROPERTY.test(propertyName)) {
				/** @type {[number, number][]} */
				const counterStyleNames = [];
				/** @type {Record<string, number>} */
				const parsedKeywords = Object.create(null);
				...
				const end = walkCssTokens.consumeUntil(
					...
					pos,
					{
						identifier(input, start, end) {
							...
							if (
								LIST_STYLE_KEYWORDS[keyword] &&
								parsedKeywords[keyword] < LIST_STYLE_KEYWORDS[keyword]
							) {
								return end;
							}

							counterStyleNames.push([start, end]);
							return end;
						}
					},
					{
				...
				if (counterStyleNames.length > 0) {
					for (const counterStyleName of counterStyleNames) {
						const { line: sl, column: sc } = locConverter.get(
							counterStyleName[0]
						);
						...
						const [start, end] = counterStyleName;
						const type = unescapeIdentifier(input.slice(start, end));
						const dep = new CssIcssSelfLocalIdentifierDependency(
							type,
							undefined,
							[start, end]
				...
				return end;
			} else if (COUNTER_STYLE_PROPERTIES.test(propertyName)) {
				/** @type {[number, number][]} */
				const counterStyleNames = [];
				/** @type {Record<string, number>} */
				const parsedKeywords = Object.create(null);
				...
				const end = walkCssTokens.consumeUntil(
					...
					pos,
					{
						identifier(input, start, end) {
							...
							const keywords =
								propertyName.toLowerCase() === "system"
									? SYSTEM_KEYWORDS
									: propertyName.toLowerCase() === "fallback"
										? FALLBACK_KEYWORDS
										: SPEAK_AS_KEYWORDS;
				...
				if (counterStyleNames.length > 0) {
					for (const counterName of counterStyleNames) {
						const { line: sl, column: sc } = locConverter.get(counterName[0]);
						const { line: el, column: ec } = locConverter.get(counterName[1]);
						const [start, end] = counterName;
						const type = unescapeIdentifier(input.slice(start, end));
						const dep = new CssIcssSelfLocalIdentifierDependency(
							type,
							undefined,
				...
				return end;
			} else if (CONTAINER_PROPERTY.test(propertyName)) {
				/** @type {[number, number][]} */
				const counterNames = [];
				/** @type {Record<string, number>} */
				const parsedContainerKeywords = Object.create(null);
				...
				const end = walkCssTokens.consumeUntil(
					...
					pos,
					{
						identifier(input, start, end) {
							...
							if (isLocalMode() && isDashedIdentifier(identifier)) {
								return processDashedIdent(input, start, end);
							}

							const keyword = identifier.toLowerCase();

							parsedContainerKeywords[keyword] =
								typeof parsedContainerKeywords[keyword] !== "undefined"
									? parsedContainerKeywords[keyword] + 1
									: 0;

							if (
								CONTAINER_KEYWORDS[keyword] &&
								parsedContainerKeywords[keyword] < CONTAINER_KEYWORDS[keyword]
							) {
								return end;
							}

							counterNames.push([start, end]);
							return end;
						}
					},
					{
				...
				if (isModules) {
					...
					if (isLocalMode() && isDashedIdentifier(identifier)) {
						const isProperty =
							scope === CSS_MODE_IN_BLOCK &&
							input.charCodeAt(
								walkCssTokens.eatWhitespaceAndComments(input, end)
							) === CC_COLON;

						return processDashedIdent(input, start, end, isProperty);
					}
		...
		if (!buildMeta.exportType) {
			...
			if (parent instanceof CssModule) {
				buildMeta.exportType =
					/** @type {BuildMeta} */ (parent.buildMeta).exportType || "link";
			}
		}
