...
/** @typedef {import("./walkCssTokens").BlockBody} BlockBody */
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const pure = {
			 ...
			 * @param {{ isRulePrelude: boolean, treatAsLeaf: boolean, ownSkip: boolean, block: BlockBody | null, preludeStart: number, preludeEnd: number }} opts entry options
			 */
			enterBlock(opts) {
				if (!this.enabled) return;
				const {
		...
		/**
		 * Flatten a `{`-block's contents into siblings (declarations + rules). `consumeABlocksContents` (§5.4.5) clusters consecutive declarations into a `Declaration[]` sub-list; downstream sites treat them as siblings.
		 * @param {BlockBody} block rule body (`{ … }`)
		 * @returns {(Declaration | Rule)[]} flattened children
		 */
		const blockChildren = (block) => block.value.flat();
		...
		const scanRuleBody = (block) => {
			...
			for (const child of blockChildren(block)) {
		...
		const processImportOrExport = (type, second, rule) => {
			...
			for (const item of blockChildren(block)) {
		...
		const finishTopLevelRule = (node, blockBearing) => {
			if (!blockBearing || /** @type {AtRule} */ (node).block) {
				allowImport = false;
			}
