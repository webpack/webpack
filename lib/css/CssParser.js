...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {{name: string, range: Range, identifierRange: Range}} */
		...
		walkCssTokens(source, {
						...
						if (--blockNestingLevel === 0) {
							...
							if (isLocalMode()) {
								...
								const dep = new CSSModuleDefineDependency(
									lastClassDefinition.name,
									lastClassDefinition.range,
									lastClassDefinition.identifierRange
								);
			...
			class: (input, start, end) => {
				if (isLocalMode()) {
					...
					switch (scope) {
						case CSS_MODE_TOP_LEVEL: {
							lastClassDefinition.name = name;
							lastClassDefinition.range = [start, 0];
							lastClassDefinition.identifierRange = [start + 1, end];
							break;
						}
					}
				}
