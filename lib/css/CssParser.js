...
/**
 * Skip zero or more `/*…*\/` comments, dispatching each via `onComment`.
 * @param {string} input source
 * @param {number} pos position
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * Comment callback: push every comment-token (in source order) onto `this.comments`, read back by `astAdvanceCommentCursor` (pure-mode flags) and `parseCommentOptions` (magic comments).
		...
		// Index into `this.comments` consumed by `astAdvanceCommentCursor`, advancing in source order so pure-mode comment flags toggle without re-tokenizing.
		let astCommentCursor = 0;

		/**
		 * Advance `astCommentCursor` past every comment closing at/before `until`, applying its pure-mode side effect: `pureIgnorePending` (next rule) or the file-level `pureNoCheck` (only before the first top-level rule).
		...
		/**
		 * Per-at-rule scope frames; `exit` reads `hasBlock` to pick the block-cleanup vs `astSuppressNextRulePrelude` branch.
		...
		/**
		 * Whether url() deps are emitted here (from `astCurrentStructural`): false outside `options.url` and inside an `@import` prelude (the import target, unless url recovery is on).
		...
		/**
		 * Whether `local()` / `global()` value functions are rewritten to ICSS here (from `astCurrentStructural`).
		...
		/**
		 * Whether `@value`-defined idents / function names are ICSS-rewritten here (from `astCurrentStructural`).
		 * @returns {boolean} true when the rewrite is active
		 */
		const icssActive = () => {
			if (!isModules || !astCurrentStructural) return false;
				...
				enter: (node, parent) => {
					...
					switch (name) {
						...
						case "@import": {
							...
							// Media query = whatever sits between the last url/layer/supports part and the closing `;`, trimmed.
