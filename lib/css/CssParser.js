...
class CssParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * @returns {number} position after handling
		 */
		const processAtKeyword = (input, end) => {
		 ...
		 * @returns {number} position after handling
		 */
		const processAtProperty = (input, end) => {
		 ...
		 * @returns {number} position after handling
		 */
		const processLocalVarFunction = (input, end) => {
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (propertyName.startsWith("--") && propertyName.length >= 3) {
					...
					},
					{
						function(input, start, end) {
							...
							if (isLocalMode() && name === "var") {
								return processLocalVarFunction(input, end);
							}
				...
				while (true) {
					...
					const className = walkCssTokens.eatIdentSequence(input, pos);
					...
					if (isComma || isSemicolon || isRightCurly) {
						...
						for (const className of classNames) {
							...
							const dep = new CssIcssComposesSelfDependency(
								identifier,
								lastLocalIdentifier,
								[start, end]
							);
						...
						if (from) {
							...
							for (const className of classNames) {
								const [start, end] = className;
								const dep = new CssIcssComposesLocalDependency(
									request,
									/** @type {"local" | "global"} */
									(mode),
							...
							if (from && input.slice(from[0], from[1]) === "global") {
								for (const className of classNames) {
									const [start, end] = className;
									const dep = new CssIcssComposesGlobalDependency(
										unescapeIdentifier(input.slice(start, end)),
										/** @type {string} */
										(lastLocalIdentifier),
										[start, end]
					...
					default: {
						if (isModules) {
							if (name === "@value") {
								...
								return processAtKeyword(input, end);
							} else if (name === "@property" && isLocalMode()) {
								return processAtProperty(input, end);
							} else if (name === "@scope") {
								isNextRulePrelude = true;
								return end;
							}
		...
		buildMeta.exportType =
			this.exportType || /** @type {CssModule} */ (module).exportType;
