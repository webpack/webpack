...
/** @type {WeakMap<CssModule, number>} */
const cssUrlIndexMap = new WeakMap();
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		if (/** @type {CssModule} */ (module).exportType !== "url") {
			...
			const { moduleGraph, outputOptions, compiler } = compilation;
			...
			for (const connection of incomingConnections) {
				...
				if (!(dependency instanceof URLDependency)) continue;
				if (connection.module !== module) continue;

				const originModule = connection.originModule;
				if (!originModule) continue;

				/** @type {import("../Entrypoint").EntryOptions} */
				const entryOptions = {};

				if (entryOptions.runtime === undefined) {
					const i =
						cssUrlIndexMap.get(/** @type {CssModule} */ (originModule)) || 0;
					cssUrlIndexMap.set(/** @type {CssModule} */ (originModule), i + 1);
					const name = `${cachedContextify(originModule.identifier())}|${i}`;
					const hash = createHash(outputOptions.hashFunction);
					hash.update(name);
					const digest = hash.digest(outputOptions.hashDigest);
				...
				const block = new AsyncDependenciesBlock({
					...
					circular: false,
					entryOptions: {
						chunkLoading: outputOptions.chunkLoading,
						wasmLoading: outputOptions.wasmLoading,
						...entryOptions
					}
				});
				block.loc = dependency.loc;
