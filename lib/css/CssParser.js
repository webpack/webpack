...
/**
 * UA-maintained counters: `list-item` drives list numbering, `page`/`pages` are the paged-media counters. Renaming them would detach the counter from the engine that increments it.
 * @type {Record<string, number>}
 */
const PREDEFINED_COUNTER_NAMES = {
	"list-item": Infinity,
	page: Infinity,
	pages: Infinity
};
...
/** @type {Record<string, number>} */
const RESERVED_COUNTER_NAMES = {
	none: Infinity,
	...PREDEFINED_COUNTER_NAMES,
	...GLOBAL_VALUES
};

/** @type {Record<string, number>} */
const COUNTER_NAMES = {
	// `counter-reset: reversed(name)` — the name lives inside the function, which `walkExports` recurses into.
	reversed: Infinity,
	...RESERVED_COUNTER_NAMES
};

/** @type {Record<string, number>} */
const RESERVED_COUNTER_STYLE_NAMES = {
	none: Infinity,
...
const buildKnownProperties = (options = {}) => {
	...
	if (options.customIdents) {
		...
		knownProperties.set("counter-reset", COUNTER_NAMES);
		knownProperties.set("counter-increment", COUNTER_NAMES);
		knownProperties.set("counter-set", COUNTER_NAMES);
...
class CssParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * @param {Record<string, number>} reserved names that must not be localized
		 * @returns {void}
		 */
		const emitCounterIdent = (node, reserved) => {
			const name = A.unescaped(node);
			if (isDashedIdentifier(name)) return;
			if (reserved[toLowerCaseIfNeeded(name)]) return;
			const start = A.start(node);
			const end = A.end(node);
			const { line: sl, column: sc } = locConverter.get(start);
			const { line: el, column: ec } = locConverter.get(end);
		...
		const walkCounterFunction = (cvs, nameIndex) => {
			...
			for (const cv of cvs) {
				...
				if (group === nameIndex) {
					if (loneIdent) emitCounterIdent(cv, RESERVED_COUNTER_NAMES);
					lastIdent = undefined;
				} else {
			...
			if (lastIdent !== undefined && group > nameIndex) {
				emitCounterIdent(lastIdent, RESERVED_COUNTER_STYLE_NAMES);
			}
		};
		...
		const visitors = {
			...
			[NodeType.Declaration]: (path) => {
				...
				if (source.charCodeAt(nameStart) === CC_HYPHEN_MINUS) {
					if (
						source.charCodeAt(nameStart + 1) === CC_HYPHEN_MINUS &&
						nameEnd - nameStart >= 3
					) {
						// Custom property: never vendor-prefixed / known / composes
						// (escaped dashes aren't raw `--` bytes, same as the string form).
						declIsDashed = true;
					} else {
						// Vendor-prefixed (or degenerate short) — rare, string path.
