...
// `consumeAComponentValue` / `consumeASimpleBlock` are the spec §5.4 helpers
	...
	parse(source, state) {
		...
		const processImportOrExport = (type, input, pos) => {
			...
			if (type === 0) {
				...
				const headerCV = consumeAComponentValue(
			...
			const bodyCV = consumeAComponentValue(
		...
		const walkAstAtRuleEnter = (at, topLevel) => {
			...
			switch (name) {
				...
				case "@charset": {
					if (/** @type {CssModule} */ (module).exportType !== "style") {
						const atRuleEnd =
							source.charCodeAt(at.end) === CC_SEMICOLON ? at.end + 1 : at.end;
		...
		const walkAstSelectorList = (values, localMode, topLevel = true) => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				if (
					...
					const attrNameCV = consumeAComponentValue(
					...
					const attrValueCV = consumeAComponentValue(
						new TokenStream(source, attrEnd, locConverter)
					);
