...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const processImportOrExport = (type, input, pos) => {
			...
			// Body: `{ name1: value1; name2: value2; … }`. `parseASimpleBlock`
			// consumes the whole `{ … }` in one call and returns its
			// component values pre-tokenized; we then walk the array
			// extracting `ident COLON … SEMICOLON?` triples, which is
			// equivalent to what the previous callbacks-based walker did
			// but without dragging the streaming tokenizer through the
			// body a second time.
			const body = parseASimpleBlock(input, pos, locConverter, comment);
			if (!body || body.token !== "{") {
				return pos;
			}
