...
/** @typedef {{url?: string, start: number, end?: number, imports: {[key: string]: { value: string, replaceRanges: [number, number][]}}}} IcssImportData */
		...
		walkCssTokens(source, {
				...
				switch (scope) {
					...
					case CSS_MODE_IN_ICSS_IMPORT: {
						if (currIcssImportDeclaration && currIcssImportDeclaration.value) {
							currIcssImport.imports[currIcssImportDeclaration.name] = {
								value: currIcssImportDeclaration.value,
								replaceRanges: []
							};
			...
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						if (icssImportsData) {
							const dataIndex = icssImportsData.findIndex(
								data => data.imports[input.slice(start, end)]
							);
							if (dataIndex > -1) {
								icssImportsData[dataIndex].imports[
