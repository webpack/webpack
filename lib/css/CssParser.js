			...
			string: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_AT_IMPORT: {
						const insideURLFunction =
							...
							(balanced[balanced.length - 1][0] === "url" ||
								balanced[balanced.length - 1][0] === "src");
					...
					case CSS_MODE_IN_BLOCK: {
						...
						if (last) {
							const name = last[0].replace(/\\/g, "").toLowerCase();

							if (
								name === "url" ||
								name === "src" ||
								IMAGE_SET_FUNCTION.test(name)
							) {
								const value = normalizeUrl(
									input.slice(start + 1, end - 1),
									true
									...
									if (ident[1] === end) {
										this._emitWarning(
											...
											`Missing whitespace after ':local' in '${input.slice(
												start,
												walkCssTokens.eatNextTokenUntil(
													input,
													end,
													(start, end) =>
														input.charCodeAt(start) !== CC_LEFT_CURLY
												)
											)}'`,
											locConverter,
											start,
