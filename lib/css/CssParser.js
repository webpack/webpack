...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// `CssExportsDependency` per module (emitted at parse end) instead of one
		// `Dependency` per export — far fewer retained objects on CSS-heavy builds.
		/** @type {import("../dependencies/CssExportsDependency").CssExportEntry[]} */
		 ...
		 * @param {import("../dependencies/CssExportsDependency").Value} value value or [localName, importName, request?]
		 * @param {Range=} range source range to replace, when present
		 * @param {boolean=} interpolate whether the value needs interpolation
		 * @param {import("../dependencies/CssExportsDependency").ExportMode=} exportMode export mode
		 * @param {import("../dependencies/CssExportsDependency").ExportType=} exportType export type
		 * @returns {void}
		 */
		const addCssExport = (
			sl,
			...
			range,
			interpolate = false,
			exportMode = CssExportsDependency.EXPORT_MODE.REPLACE,
			exportType = CssExportsDependency.EXPORT_TYPE.NORMAL
		) => {
			...
			if (type === 1) {
				for (const cv of fn.value) {
					...
					addCssExport(
						...
						[cv.start, cv.end],
						true,
						CssExportsDependency.EXPORT_MODE.ONCE,
						isDashedIdent
							? CssExportsDependency.EXPORT_TYPE.CUSTOM_VARIABLE
							: CssExportsDependency.EXPORT_TYPE.NORMAL
					);
				}
			}
			...
			for (let i = 0; i < values.length; i++) {
				...
				if (
					...
					addCssExport(
						...
						true,
						CssExportsDependency.EXPORT_MODE.NONE
					);
					continue;
				}
									...
									for (const { importName, localName } of items) {
...

										{
											...
											addCssExport(
												...
												undefined,
												false,
												CssExportsDependency.EXPORT_MODE.REPLACE
											);
										}
									}
								} else {
							...
							for (const { start, end } of classNames) {
								...
								if (selfReference) {
									...
									addCssExport(
										...
										[start, end],
										true,
										CssExportsDependency.EXPORT_MODE.SELF_REFERENCE,
										CssExportsDependency.EXPORT_TYPE.COMPOSES
									);
									continue;
								}
								...
								addCssExport(
									...
									[start, end],
									true,
									CssExportsDependency.EXPORT_MODE.APPEND,
									CssExportsDependency.EXPORT_TYPE.COMPOSES
								);
							}
				...
				if (
					...
					for (const value of values) {
						...
						addCssExport(
							...
							CssExportsDependency.EXPORT_MODE.ONCE,
							isGridProperty
								? CssExportsDependency.EXPORT_TYPE.GRID_CUSTOM_IDENTIFIER
								: CssExportsDependency.EXPORT_TYPE.NORMAL
						);
					}
				}
		...
		if (cssExportEntries.length > 0) {
			module.addDependency(new CssExportsDependency(cssExportEntries));
		}
