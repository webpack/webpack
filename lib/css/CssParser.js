...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {Visit<UrlToken>} Url-token visitor — bare `url(unquoted)`. */
		const onValueUrl = (node) => {
			if (vUrl && !vUrlSkip) processOldURLFunction(node);
		};

		/** @type {Visit<Token>} Comma visitor — advances the magic-comment fence. */
		 ...
		 * @type {Visit<FunctionNode>}
		 */
		const onValueFunction = (fn) => {
		...
		/** @type {Visit<Token>} Ident visitor — `@value`-defined ident rewrite. */
		 ...
		 * @type {{ savedAnchor: boolean, savedLocalIdentifiers: string[], name: string, hasBlock: boolean, terminator: "{" | "}" | ";" | null }[]}
		 */
		...
		const runAstWalker = () => {
			...
			const visitors = {
				...
				Declaration: (decl) => processAstDeclaration(decl, false),
