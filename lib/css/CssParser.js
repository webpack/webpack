...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * Pure-mode frame per open block: `ancestorHadLocal` / `skipChildren` are the inherited context for descendants; the rest defer the impure-rule report to exit (when the body is fully walked — required for a single-pass walk, where the block isn't consumed at enter).
		 * @type {{ ancestorHadLocal: boolean, skipChildren: boolean, report: boolean, treatAsLeaf: boolean, block: (SimpleBlock | null), preludeStart: number, preludeEnd: number }[]}
		 */
		...
		const enterPureBlock = ({
			...
			pureBlockStack.push({
				...
				// Deferred to exit: report only an impure rule prelude that isn't ignored / skipped.
				report:
					!pureNoCheck &&
					!pureIgnorePending &&
					!skipOwn &&
		...
		/**
		 * Pure-mode rule exit: drop the frame and, with the body now fully walked, report an impure leaf-ish rule.
		 * @returns {void}
		 */
		const exitPureBlock = () => {
			...
			if (
				...
				(frame.treatAsLeaf ||
					!frame.block ||
					blockHasOwnDeclaration(frame.block) ||
					!blockHasNestedBlock(frame.block))
			) {
