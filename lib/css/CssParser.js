...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const walkAstAtRule = (at, topLevel) => {
			...
			switch (name) {
				...
				default: {
					...
					// at this level. Their selector preludes (for `@scope`)
					// and bodies are handled by E3/E4 / the streaming
					// walker.
				}
			}
		...
		/**
		 * Walk a qualified rule (a style rule). Selectors (E3) and
		 * declaration values (E4) aren't migrated yet; this commit
		 * only recurses into the body so nested rules / at-rules get
		 * visited by E2's at-rule handler.
