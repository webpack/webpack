...
// `unescapeIdentifier` is a pure string utility
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// Set by `processAtImport` for a malformed `@import` whose prelude
		...
		const walkAstAtRuleEnter = (at, topLevel) => {
			...
			// `@import` url() is the import target (handled by
			// `processAtImport`), so url isn't walked here unless the import
			// was malformed (`importNeedsUrlRecovery`).
			const effectiveLocalMode = astModeData
				? astModeData === "local"
				: mode === "local";
			...
			vUrl = vUrlSkip = vLocalGlobal = vIcss = vIcssDashedHandled = false;
			// `@import` url() is the import target (handled by
			// `processAtImport`) — only walk its prelude for url deps when
			// the import was malformed and `processAtImport` asked for it.
