...
class CssParser extends Parser {
	...
	parse(source, state) {
			...
			for (let i = 0; i < cvs.length; i++) {
				...
				if (cv.type === "Ident") {
					...
					if (emitTopLevelIdents && isDashedIdentifier(identValue)) {
						// Lookahead for the `--foo from "./x.css"` / `--foo from global` suffix, dispatching to the right `emitDashedIdent…` helper.
						let j = i + 1;
					...
					if (fname === "local" || fname === "global") {
						// `:local(--foo)` / `:global(--foo)` already emit via `walkFunctionsForLocalGlobal`; don't recurse or we'd double-emit.
						continue;
					}
					if (fname === "var" || fname === "style") {
						// `var(--foo, …)` / `style(--foo, …)`: emit the first ident, then recurse the fallback with top-level idents off.
						processDashedIdentInVarFunction(fn);
					...
					// Recurse into the args, propagating the caller's `emitTopLevelIdents`.
		...
		const walkAstQualifiedRuleEnter = (rule, topLevel, ctx) => {
			...
			// Dashed-ident emission for the deprecated `--foo: { … }` custom-property-set syntax (parsed as a nested rule), only when the prelude starts with a dashed-ident.
		...
		const runAstWalker = () => {
			...
			const visitors = {
				...
				Declaration: (decl) => {
					...
					// Dashed-ident (custom-property) ICSS export: the property name exports, and the inline value walk handles `var()` / `style()` / `--foo()` and value dashed idents.
					if (
						this.options.dashedIdents &&
						isModules &&
						effectiveLocalMode &&
