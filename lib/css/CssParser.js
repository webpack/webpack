...
class CssParser extends Parser {
	...
	/**
	 * A grid or custom (dashed) identifier renames differently from a plain
	 * identifier of the same name, but both share one JS export key. Demote the
	 * grid/dashed entries to CSS-only rewrites so the plain identifier (usually
	 * a class) deterministically keeps the export, and warn for demoted custom
	 * identifiers since their renamed value becomes unreachable from JS.
	 * @param {ParserState} state parser state
	 * @param {import("../dependencies/CssIcssExportDependency").CssExportEntry[]} entries collected export entries
	 */
	_resolveAmbiguousExports(state, entries) {
		const NONE = CssIcssExportDependency.EXPORT_MODE.NONE;
		const CUSTOM_VARIABLE = CssIcssExportDependency.EXPORT_TYPE.CUSTOM_VARIABLE;
		const GRID = CssIcssExportDependency.EXPORT_TYPE.GRID_CUSTOM_IDENTIFIER;
		// One string-hash per entry: names of grid/dashed entries (with their entry
		// indices), then probe the plain entries against them. Modules without
		// grid/dashed exports pay only the integer scan.
		...
		for (let i = 0; i < entries.length; i++) {
			...
			if (
				(entry.exportType === CUSTOM_VARIABLE || entry.exportType === GRID) &&
				entry.exportMode !== NONE
			) {
				if (candidateNames === undefined) {
					candidateNames = new Set();
					candidateIndices = [];
				}
				candidateNames.add(entry.name);
				/** @type {number[]} */ (candidateIndices).push(i);
			}
		}
		...
		for (const entry of entries) {
			if (
				entry.exportType !== CUSTOM_VARIABLE &&
				entry.exportType !== GRID &&
				entry.exportMode !== NONE &&
				candidateNames.has(entry.name)
			) {
				if (shadowed === undefined) shadowed = new Set();
		...
		/** @type {Set<string> | undefined} */
		let warned;
		for (const i of /** @type {number[]} */ (candidateIndices)) {
			const entry = entries[i];
			if (!shadowed.has(entry.name)) continue;
			entry.exportMode = NONE;
			if (entry.exportType !== CUSTOM_VARIABLE) continue;
			if (warned === undefined) warned = new Set();
			if (warned.has(entry.name)) continue;
			warned.add(entry.name);
			state.current.addWarning(
				new ModuleDependencyWarning(
					state.module,
					new WebpackError(
						`Export name '${entry.name}' is ambiguous: it resolves to the identifier '${entry.name}', so the renamed value of the custom identifier '--${entry.name}' is not accessible from JavaScript — rename one of them if both are needed`
					),
					{
	...
	parse(source, state) {
		...
		if (cssExportEntries.length > 0) {
			if (isModules) this._resolveAmbiguousExports(state, cssExportEntries);
