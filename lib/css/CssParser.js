...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {undefined | { propertyNameStart?: number, propertyNameEnd?: number } } */
		...
		const parseExports = (input, pos) => {
			...
			for (;;) {
				...
				const start = pos;
				let name;
				[pos, name] = eatText(input, pos, eatExportName, commentCallbacks);
				if (pos === input.length) return pos;
				...
				if (pos === input.length) return pos;
				let value;
				[pos, value] = eatText(input, pos, eatExportValue, commentCallbacks);
				if (pos === input.length) return pos;
				const cc = input.charCodeAt(pos);
				if (cc === CC_SEMICOLON) {
					pos++;
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			currentDeclaration = { propertyNameStart, propertyNameEnd };
		...
		walkCssTokens(source, {
			...
			url: (input, start, end, contentStart, contentEnd) => {
				...
				switch (scope) {
					...
					case CSS_MODE_IN_BLOCK: {
						...
						const preLineNeedIgnore = this.parseCommentOptions([
							walkCssTokens.eatWhitespaceAndCommentsRev(
								input,
								currentDeclaration.propertyNameStart - 1
							),
							currentDeclaration.propertyNameStart
						]).options.needIgnore;

						const preNeedIgnore = this.parseCommentOptions([
							walkCssTokens.eatWhitespaceAndCommentsRev(input, start - 1),
							start
						]).options.needIgnore;
						if (
							(preNeedIgnore === undefined && preLineNeedIgnore) ||
							preNeedIgnore
						)
							break;
				...
				switch (scope) {
					...
					case CSS_MODE_IN_BLOCK: {
						...
						if (
							...
							const preFunctionNeedIgnore = this.parseCommentOptions([
								walkCssTokens.eatWhitespaceAndCommentsRev(input, last[1] - 1),
								last[1]
							]).options.needIgnore;

							const preStringNeedIgnore = this.parseCommentOptions([
								walkCssTokens.eatWhitespaceAndCommentsRev(input, start - 1),
								start
							]).options.needIgnore;
							if (
								preStringNeedIgnore !== undefined
									? preStringNeedIgnore
									: preFunctionNeedIgnore !== undefined
										? preFunctionNeedIgnore
										: preLineNeedIgnore
							)
								break;
			...
			semicolon: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_AT_IMPORT: {
						...
						const { options } = this.parseCommentOptions([
							walkCssTokens.eatWhitespaceAndComments(input, start, {}),
							end
						]);
						if (options.needIgnore) {
			...
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						...
						processDeclaration(input, start, end);
						break;
					}
					case CSS_MODE_IN_AT_IMPORT: {
			...
			comment: commentCallbacks.comment
		});
		...
		for (const comment of comments) {
			...
			if (value) {
				...
				try {
					...
					if (matched) {
						options.needIgnore = matched[2] === "true";
					}
				} catch (err) {
