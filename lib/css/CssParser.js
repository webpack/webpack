...
// CSS-wide keywords — never scoped as a `font-family` name.
const CSS_WIDE_KEYWORDS = /^(?:inherit|initial|unset|revert|revert-layer)$/i;
// A `font-family` name is only scoped when it is already a valid identifier, so
// the generated local ident (which embeds it) stays valid — names needing
// quotes (spaces, etc.) are left global.
const VALID_FONT_FAMILY_IDENT =
	/^-?[A-Za-z_\u00A0-\uFFFF][A-Za-z0-9_\u00A0-\uFFFF-]*$/;
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {Map<string, { text: string, supported: boolean }> | undefined} */
		let customMediaDefs;
		/** @type {{ name: string, start: number, end: number, invalid: boolean }[] | undefined} */
		let customMediaUses;
		...
		// `@font-face` family scoping (opt-in): a `font-family` name is renamed only when a local `@font-face` defines it (families otherwise share a global namespace with system/web fonts). Definitions can follow their uses, so both are collected during the walk and reconciled after it. Modules-only.
		const mayHaveFontFace =
			this.options.fontFace && isModules && input.includes("@font-face");
		 ...
		 * @type {{ savedAnchor: boolean, savedLocalIdentifiers: string[], savedInsideFontFace: boolean, name: string, hasBlock: boolean, endsWithSemicolon: boolean }[]}
		 */
		...
		/**
		 * Collect one `@custom-media --name <value>` definition (last-wins) and drop the rule. Values reduce to a media-in-parens string; comma lists become `(a or b)`, `not (…)` is wrapped, and media-type / `true` / `false` / nested custom-media values are marked unsupported (warned on use).
		...
		const collectCustomMedia = (at) => {
			...
			const flush = () => {
				...
				const first = seg[a];
				const text = input.slice(A.start(first), A.end(seg[b - 1]));
				if (
					b - a === 1 &&
					A.type(first) === NodeType.SimpleBlock &&
					loneDashedIdentOfBlock(/** @type {SimpleBlock} */ (first), extra) ===
						undefined
				) {
					segments.push(text);
				} else if (
					A.type(first) === NodeType.Ident &&
					rangeEqualsLowerCase(input, A.start(first), A.end(first), "not")
				) {
					segments.push(`(${text})`);
				} else {
					supported = false;
			...
			for (; i < prelude.length; i++) {
				if (A.type(prelude[i]) === NodeType.Comma) {
					flush();
					seg = [];
				} else {
					seg.push(prelude[i]);
		...
		/**
		 * Record every `(--name)` custom-media reference in a media prelude (recursing into nested groups/functions) for post-walk resolution; `invalid` flags a non-boolean-context use like `(--name: 1px)`.
		 * @param {readonly AstNode[]} tokens prelude (or nested block) tokens
		 */
		const scanCustomMediaUses = (tokens) => {
			const extra = { hasExtra: false };
			for (const t of tokens) {
				...
				if (tt === NodeType.SimpleBlock) {
					...
					if (name !== undefined) {
						...
						scanCustomMediaUses(A.children(t));
					}
			...
			if (customMediaUses) {
				for (const use of customMediaUses) {
					...
					if (!def.supported) {
						this._emitWarning(
							state,
							`Custom media query '${use.name}' uses an unsupported value (media type, 'true'/'false', or a nested custom media) and was left unresolved`,
							locConverter,
							use.start,
							use.end
						);
		...
		/**
		 * Collect the single-token family names of a `font-family` declaration (each comma segment that is one ident or one string; multi-word unquoted names are skipped). A `@font-face` descriptor also registers the name as locally defined.
		 * @param {Declaration} decl the `font-family` declaration
		 * @param {boolean} isDefinition whether `decl` is a `@font-face` descriptor
		 */
		const collectFontFamily = (decl, isDefinition) => {
			/** @type {AstNode[]} */
			let seg = [];
			const flush = () => {
				let a = 0;
				...
				const tok = seg[a];
				const tt = A.type(tok);
				const isString = tt === NodeType.String;
				if (tt !== NodeType.Ident && !isString) return;
				const start = A.start(tok);
				const end = A.end(tok);
				const name = isString
					? unescapeRange(start + 1, end - 1)
					: unescapeRange(start, end);
				// Skip CSS-wide keywords and names that aren't already valid idents (they'd produce an invalid scoped ident).
				if (
					CSS_WIDE_KEYWORDS.test(name) ||
					!VALID_FONT_FAMILY_IDENT.test(name)
				) {
					return;
				}
				if (isDefinition) {
					(localFontFamilies || (localFontFamilies = new Set())).add(name);
				}
				(fontFamilyTokens || (fontFamilyTokens = [])).push({
					name,
					start,
					end
				});
		...
		/**
		 * Rename every collected `font-family` token whose name a local `@font-face` defines, exporting it as a CSS-Modules local; non-local (system/web) families are left untouched.
		 */
		const resolveFontFamilies = () => {
			if (!fontFamilyTokens || !localFontFamilies) return;
			for (const tok of fontFamilyTokens) {
				if (!localFontFamilies.has(tok.name)) continue;
				const { line: sl, column: sc } = locConverter.get(tok.start);
				const { line: el, column: ec } = locConverter.get(tok.end);
				addCssExport(
					sl,
					sc,
					el,
		...
		const visitors = {
			...
			[NodeType.AtRule]: {
				...
				enter: (path) => {
					...
					switch (name) {
						...
						case "@media": {
							if (mayHaveCustomMedia) scanCustomMediaUses(A.prelude(at));
							break;
						}
						default: {
							if (!isModules) break;
					...
					const savedInsideFontFace = insideFontFace;
					if (mayHaveFontFace && name === "@font-face") insideFontFace = true;
				...
				exit: (path) => {
					...
					insideFontFace = state.savedInsideFontFace;
			...
			[NodeType.Declaration]: (path) => {
				...
				// `@font-face` family scoping: collect `font-family` names (a `@font-face` descriptor also defines them); renamed after the walk only when locally defined.
				if (
					mayHaveFontFace &&
					effectiveLocalMode &&
					rangeEqualsLowerCase(source, nameStart, nameEnd, "font-family")
				) {
					collectFontFamily(decl, insideFontFace);
				}
		...
		if (fontFamilyTokens) resolveFontFamilies();
