...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * Push a pure-mode block frame for a qualified rule or block-bearing
		 * at-rule, capturing the current selector impurity / ancestor purity,
		 * then reset the per-rule selector flags.
		 * @param {{ isRulePrelude: boolean, treatAsLeaf: boolean, ownSkip: boolean, preludeStart: number, preludeEnd: number }} opts frame options
		 * @returns {void}
		 */
		const pushPureBlock = ({
			isRulePrelude,
			...
			pureBlockStack.push({
				...
				skipChildren: ownSkip || inheritedSkip,
				treatAsLeaf,
				ancestorHadLocal:
					parentEffectivePure() ||
		...
		/**
		 * Pop a pure-mode block frame: report an impure-selector error for a
		 * leaf-ish rule, and propagate a non-rule block's (e.g. `@media`) direct
		 * declarations up to its parent.
		 * @returns {void}
		 */
		const popPureBlock = () => {
			const frame = pureBlockStack.pop();
			if (!frame) return;
			// `impure` is only set for rule preludes, so it implies `isRulePrelude`.
			if (
				!pureNoCheck &&
				!frame.ignored &&
				!frame.skipOwn &&
		...
		const walkAstQualifiedRuleEnter = (rule, topLevel, ctx) => {
			...
			// Pure-mode: push a qualified-rule frame (capturing `impure` / `ancestorHadLocal`) before walking the body.
			if (pureMode) {
				pushPureBlock({
					isRulePrelude: true,
					treatAsLeaf: false,
		...
		const runAstWalker = () => {
			...
			const visitors = {
				...
				Declaration: (decl) => {
					...
					vDashedEmit = false;
					if (pureMode) {
						// Pure-mode: a direct declaration marks the enclosing rule frame.
						const top = pureTop();
