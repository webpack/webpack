...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// `url` modules don't emit CSS themselves: they re-reference the same
		// resource as a regular `link` CSS module placed in its own chunk, and
		// export that chunk's URL. Skip the CSS token walk entirely.
		if (/** @type {CssModule} */ (module).exportType === "url") {
			const block = new AsyncDependenciesBlock({ name: undefined });
			const dep = new CssUrlEntryDependency(module.resource);
			...
			buildMeta.exportsType = this.options.namedExports
				? "namespace"
				: "default";
