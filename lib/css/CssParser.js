...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * Streaming-walker comment callback. Only handles pure-mode
		 * side effects (`pureIgnorePending` / `pureNoCheck`) that the
		 * `leftCurlyBracket:` case reads synchronously, in source order,
		 * before the AST walker runs. Comment *collection* itself
		 * (`this.comments.push`) is owned by `astCommentCb` below, which
		 * `runAstWalker`'s `parseAStylesheet` call hands every comment
		 * in the source — so `processAtImport` / `processAtValue` see a
		 * fully-populated `this.comments` for magic-comment lookups.
		 * @param {string} input input
		...
		/**
		 * AST-side comment callback handed to `parseAStylesheet` in
		 * `runAstWalker`. Pushes the comment metadata to
		 * `this.comments` so `parseCommentOptions` (used by
		 * `processAtImport` / `processAtValue` for `webpackIgnore`,
		 * `webpackChunkName`, …) can look it up at AST emit time.
		 * @param {string} input input
		 * @param {number} start start
		 * @param {number} end end
		 * @returns {number} end
		 */
		const astCommentCb = (input, start, end) => {
		...
		const processClassSelector = (input, start, end) => {
			...
			if (!ident) return end;
			// E3b: dep emission moved to `walkAstSelectorList`.
			// `lastLocalIdentifiers` accumulation moved to
			// `walkAstSelectorList` (`astCurrentRuleLocalIdentifiers`) once
			// `composes:` resolution migrated to the AST (E4a-3). All that
			// remains here is the pure-mode side effect that
			// `leftCurlyBracket:`'s `finalizeSelector` reads synchronously,
		 ...
		 */
		const runAstWalker = () => {
			const stylesheet = parseAStylesheet(source, locConverter, astCommentCb);
