...
// `SourceProcessor`'s static members — the spec parse helpers
// `CssParser.js` calls for sub-range parsing. The byte-level source-cursor
// scans (whitespace / comment / newline skipping) are local to this module
// (see `skipWhitespace` & friends below).
const { parseAComponentValue, consumeASimpleBlock } = SourceProcessor;
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const processAtImport = (atRule) => {
			...
			if (urlNode.type === "Ident") {
				...
				if (!def) {
					this._emitWarning(
						state,
						`Unknown '@value' identifier '${name}' in '${input.slice(
							start,
							semi
						)}'`,
						locConverter,
		...
		const processImportOrExport = (type, input, pos) => {
			pos = skipWhitespaceAndComments(input, pos)[0];
			/** @type {string | undefined} */
			let request;
			if (type === 0) {
			...
			// helper instead of through a separate `skipWhitespaceAndComments`
		...
		const processComposesAst = (decl) => {
			...
			const resumeAt =
				source.charCodeAt(decl.end) === CC_SEMICOLON
					? skipWhitespace(source, decl.end + 1)
					: decl.end;
			module.addPresentationalDependency(
		...
		const processAttributeSelector = (input, start, end) => {
			end = skipWhitespaceAndComments(input, end)[0];
			...
			end = skipWhitespaceAndComments(input, nameCV.value.end)[0];
		...
		const walkAstSelectorList = (values, localMode, topLevel = true) => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				if (v.type === "Colon") {
					...
					if (next.type === "Ident") {
						...
						if (id === "local" || id === "global") {
							...
							const stripEnd = skipWhitespace(source, next.end);
							if (isModules) {
								module.addPresentationalDependency(
							...
							// `foundWhitespace`.
							const probe = skipWhitespaceAndComments(source, next.end);
							if (!probe[1]) {
								this._emitWarning(
									state,
				...
				if (
					...
					const { line: idSl, column: idSc } = locConverter.get(v.start);
					const { line: idEl, column: idEc } = locConverter.get(v.end);
					...
					urlHandling: if (vUrl) {
						if (fname === "url" || fname === "src") {
							...
							for (const cv of fn.value) {
								...
								const { options, errors: commentErrors } =
									this.parseCommentOptions([rangeStart, string.end]);
								if (commentErrors) {
									for (const e of commentErrors) {
										const { comment } = e;
										state.module.addWarning(
