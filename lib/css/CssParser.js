...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {undefined | { start:number, keyStart?: number, keyEnd?:number}} */
		let atValueData;
		...
		const parseAtValue = (input, pos) => {
			const start = pos;
			const key = input.slice(atValueData.keyStart, atValueData.keyEnd);
			pos = walkCssTokens.eatWhitespaceAndComments(input, pos);
			if (pos === input.length) return pos;
			if (input.charCodeAt(pos) === CC_COLON) {
				// @value key: value;
			...
			const constDep = new ConstDependency("", [atValueData.start, pos]);
