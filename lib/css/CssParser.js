...
const { SourceProcessor } = require("./walkCssTokens");

// All CSS-syntax-level helpers reach `CssParser.js` through
// `SourceProcessor` — the `walkCssTokens` module is the implementation
// home (to keep this commit's diff tight), but the import surface
// here is just the single `SourceProcessor` class. Future renames /
// relocations change this `require()` line, nothing else.
const {
	eatUntil,
	eatWhiteLine,
	eatWhitespace,
...
class CssParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * @returns {void}
		 */
		// Drive **the entire** AST walk through `SourceProcessor`. The
		// processor parses the stylesheet once via `parseAStylesheet`
		// and walks the result in source order, firing `enter` / `exit`
		// for every `AtRule` / `QualifiedRule` and `enter` for every
		// `Declaration` — including nested ones inside `@media` /
		// `@supports` / `@layer` / nested style rules. Each visitor half
		// is the corresponding `walkAst…Enter` / `walkAst…Exit` helper,
		// with `topLevel` derived from `parent === null`.
		//
		// `recurseBlocks` defaults to true so `cssGrammar` re-parses
		// `{ ... }` bodies into nested rules — the visitor recursion
		// then fires for every nested declaration / at-rule / qualified
		// rule. Together with the `walkAst{AtRule,QualifiedRule}{Enter,Exit}`
		// split and the `Declaration: processAstDeclaration` registration,
		// this replaces the hand-rolled `walkAstBlockContents` recursion
		// entirely.
		//
			...
			processor.use({
				AtRule: {
					enter(at, parent) {
						...
						walkAstAtRuleEnter(/** @type {AtRule} */ (at), topLevel);
					},
					exit(at, parent) {
						walkAstAtRuleExit();
						if (parent === null) {
							if (/** @type {AtRule} */ (at).terminator === "{") {
				...
				Declaration(decl) {
					// Top-level declarations are spec parse errors — filtered
					// out by `parseAStylesheet` before we ever see them, so
					// `parent` is always a `SimpleBlock` here. Forward to
					// `processAstDeclaration` with `topLevel: false`.
					processAstDeclaration(/** @type {Declaration} */ (decl), false);
				}
			});
			processor.parse(source, { locConverter, comment });
		};
		runAstWalker();
