...
/** @typedef {{url?: string, start: number, end?: number, importMap: ImportMap}} IcssImportData */
		...
		walkCssTokens(source, {
				...
				switch (scope) {
					...
					case CSS_MODE_IN_ICSS_IMPORT: {
						if (currIcssImportDeclaration && currIcssImportDeclaration.value) {
							currIcssImport.importMap.set(currIcssImportDeclaration.name, {
			...
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						if (icssImportsData) {
							const dataIndex = icssImportsData.findIndex(data =>
								data.importMap.get(input.slice(start, end))
							);
							if (dataIndex > -1) {
								icssImportsData[dataIndex].importMap
									.get(input.slice(start, end))
									.replaceRanges.push([start, end - 1]);
							}
						}
						if (isLocalMode()) {
			...
			pseudoFunction: (input, start, end) => {
				...
				if (this.allowModeSwitch) {
					...
					if (name === ":global") {
						...
						currIcssImport = { start, importMap: new Map() };
					}
				}
