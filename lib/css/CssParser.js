...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// exportGlobals: also export `:global(...)` class / id names, each mapped to its unscoped self.
		const exportGlobals = this.options.exportGlobals === true;
		...
		/**
		 * exportGlobals: export a global class / id name mapped to its unscoped self — no rename, no source replacement (`range` omitted, `interpolate` false).
		 * @param {number} start start offset of the name
		 * @param {number} end end offset of the name
		 * @param {string} name unescaped name
		 */
		const emitGlobalExport = (start, end, name) => {
			const { line: sl, column: sc } = locConverter.get(start);
			const { line: el, column: ec } = locConverter.get(end);
			addCssExport(
				sl,
		...
		const walkSelectorList = (values, localMode, topLevel = true) => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				switch (A.type(v)) {
					...
					case NodeType.Hash: {
						if (A.typeFlag(v) !== "id") break;
						if (segmentMode !== "local") {
							// exportGlobals: a global id name is exported as-is (no rename).
							if (exportGlobals) {
								emitGlobalExport(A.start(v), A.end(v), A.unescaped(v));
							}
							break;
						}
					...
					case NodeType.Delim: {
						...
						if (segmentMode === "local") {
							...
							// exportGlobals: a global class name is exported as-is (no rename).
							if (exportGlobals) {
								emitGlobalExport(A.start(next), A.end(next), A.unescaped(next));
							}
							// `.<ident>` in global mode: not localized, but the ident may be `@value`-defined and need ICSS rewrite.
							if (icssDefinitions.size !== 0) {
