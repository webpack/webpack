	...
	parse(source, state) {
		...
		const walkAstBlockContents = (items, topLevel) => {
			for (const item of items) {
				if (pureMode) astAdvanceCommentCursor(item.start);
		...
		const runAstWalker = () => {
			...
			for (const rule of stylesheet.rules) {
				...
				// Pure-mode: seal the `pureNoCheck`-eligibility window
				// after the first top-level rule / at-rule. Matches the
				// streaming walker's `seenTopLevelRule = true` set on
				// the first `case "leftCurlyBracket"` / `case
				// "atKeyword"` at top level.
