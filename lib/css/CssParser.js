...
// `parseAComponentValue` / `consumeASimpleBlock` are the spec parse helpers
// `CssParser.js` calls for sub-range parsing; `unescapeIdentifier` is a pure
// string utility (CSS Syntax 3 §4.3.7). All are imported directly from the
// module. The byte-level source-cursor scans (whitespace / comment / newline
// skipping) are local to this module (see `skipWhitespace` & friends below).
	...
	parse(source, state) {
		...
		const processImportOrExport = (type, input, pos) => {
			...
			if (type === 0) {
				...
				const headerCV = parseAComponentValue(
					new TokenStream(input, pos, locConverter)
				);
				if (
					!headerCV ||
			...
			const bodyCV = parseAComponentValue(
				new TokenStream(input, pos, locConverter, comment)
			);
			if (
				!bodyCV ||
			...
			switch (name) {
				...
				default: {
					...
					if (name === "@value") {
						...
						const semi = at.end;
						const atRuleEnd = at.terminator === ";" ? semi + 1 : semi;
		...
		const walkAstSelectorList = (values, localMode, topLevel = true) => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				if (
					...
					// other attribute shape.
					let attrEnd = skipWhitespaceAndComments(source, v.start + 1)[0];
					const attrNameCV = parseAComponentValue(
						new TokenStream(source, attrEnd, locConverter)
					);
					if (!attrNameCV || attrNameCV.value.type !== "Ident") continue;
					const attrName = unescapeIdentifier(
						source.slice(attrNameCV.value.start, attrNameCV.value.end)
					);
					if (attrName.toLowerCase() !== "class") continue;
					attrEnd = skipWhitespaceAndComments(source, attrNameCV.value.end)[0];
					const attrIsTilde = source.charCodeAt(attrEnd) === CC_TILDE;
					if (
						source.charCodeAt(attrEnd) !== CC_EQUAL &&
						source.charCodeAt(attrEnd) !== CC_TILDE
					) {
						continue;
					}
					attrEnd += 1;
					if (attrIsTilde) {
						if (source.charCodeAt(attrEnd) !== CC_EQUAL) continue;
						attrEnd += 1;
					}
					const attrValueCV = parseAComponentValue(
						new TokenStream(source, attrEnd, locConverter)
					);
					if (!attrValueCV) continue;
					/** @type {number} */
					let classNameStart;
					/** @type {number} */
					let classNameEnd;
					if (attrValueCV.value.type === "String") {
						classNameStart = attrValueCV.value.start + 1;
						classNameEnd = attrValueCV.value.end - 1;
					} else if (attrValueCV.value.type === "Ident") {
						classNameStart = attrValueCV.value.start;
						classNameEnd = attrValueCV.value.end;
					} else {
						continue;
					}
					...
					if (name === "import" || name === "export") {
						if (topLevel) {
							...
							const blk = consumeASimpleBlock(
								new TokenStream(source, rule.block.start, locConverter, comment)
							);
							if (blk) {
