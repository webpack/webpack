...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// Tracks the kind of every declared CSS Modules export (class, id,
		// custom property, @keyframes, @counter-style, @container, @value,
		// :export). A cross-kind redeclaration silently drops or overrides
		// one side depending on `EXPORT_MODE`, which is almost always a bug
		// — warn once per (name, new-kind) pair. Same-kind redeclarations
		// (two `.foo` selectors, two `--foo: …` rules, …) stay silent.
		...
		/**
		 * Record a declaration site and, if a same-name declaration of a
		 * different kind was already seen, emit a warning pointing at both
		 * locations. Property-value references (`animation: foo` that just
		 * shares a name with a `@keyframes foo`) go through
		 * `addCssExport` directly and skip this check.
		 * @param {string} name export name
		 * @param {string} kind human-readable declaration kind
		 * @param {number} line start line (1-based) of the new declaration
		 * @param {number} column start column (0-based) of the new declaration
		 * @param {number} startOffset start offset of the new declaration (for the warning range)
		 * @param {number} endOffset end offset of the new declaration
		 * @returns {void}
		 */
		const checkExportConflict = (
			name,
			kind,
			line,
			column,
			...
			const existing = declaredExports.get(name);
			if (existing) {
				if (!existing.kinds.has(kind)) {
					this._emitWarning(
						state,
						`Conflicting CSS module export "${name}": already declared as ${existing.kind} at line ${existing.line}:${existing.column}, redeclared as ${kind}`,
						locConverter,
						startOffset,
						endOffset
					);
		...
		const processImportOrExport = (type, second, rule) => {
			...
			const createDep = (name, value, start, end) => {
				if (type === 0) {
					...
					const { line: sl, column: sc } = locConverter.get(start);
					const { line: el, column: ec } = locConverter.get(end);
					checkExportConflict(name, ":export", sl, sc, start, end);
			...
			for (const cv of A.prelude(atRule)) {
				...
				if (cvType === NodeType.String) {
					if (!found && options.string) {
						...
						const { line: sl, column: sc } = locConverter.get(A.start(cv));
						const { line: el, column: ec } = locConverter.get(A.end(cv));
						checkExportConflict(value, kind, sl, sc, A.start(cv), A.end(cv));
						addCssExport(
							sl,
							sc,
							el,
			...
			if (
				...
				for (const { importName, localName } of items) {
...

					{
						...
						checkExportConflict(localName, "@value", sl, sc, start, nameEnd);
						addCssExport(
							sl,
							sc,
							el,
