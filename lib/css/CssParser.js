...
const { parseADeclaration } = require("./CssAst");
	...
	parse(source, state) {
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (isLocalMode() && knownProperties.has(propertyName)) {
				const decl = parseADeclaration(input, propertyNameStart, locConverter);
				if (!decl) return pos;
				 ...
				 * inside `repeat(...)`).
				 * @param {import("./CssAst").Node[]} cvs component values
				 */
				const walkExports = (cvs) => {
					for (const cv of cvs) {
						switch (cv.type) {
							...
							case "delim":
								afterExclamation =
									/** @type {import("./CssAst").Token} */ (cv).value === "!";
								break;
							case "ident": {
								...
								const identifier = /** @type {import("./CssAst").Token} */ (cv)
									.value;
							...
							case "simple-block": {
								const block = /** @type {import("./CssAst").SimpleBlock} */ (
									cv
								);
								if (block.token === "[") {
							...
							case "function":
								if (isGridTemplate) {
									walkExports(
										/** @type {import("./CssAst").FunctionNode} */ (cv).value
									);
								}
								break;
				...
				const walkFunctions = (cvs) => {
					for (const cv of cvs) {
						if (cv.type === "function") {
							const fn = /** @type {import("./CssAst").FunctionNode} */ (cv);
							const fname = fn.name.replace(/\\/g, "").toLowerCase();
							const afterOpenParen = fn.nameRange[1] + 1;
							if (fname === "local" || fname === "global") {
								...
								processDashedIdent(input, afterOpenParen, afterOpenParen);
							} else if (
								this.options.url &&
								(fname === "src" || fname === "url")
							) {
								processURLFunction(input, afterOpenParen, fname);
							} else if (this.options.url && IMAGE_SET_FUNCTION.test(fname)) {
								processImageSetFunction(input, fn.start, afterOpenParen);
							...
							walkFunctions(
								/** @type {import("./CssAst").SimpleBlock} */ (cv).value
							);
						}
					}
				};
					...
					if (isComma || isSemicolon || isRightCurly) {
						...
						if (from) {
							...
							for (const entry of classNames) {
								...
								if (selfReference) {
									// `composes: foo from "./self.module.css"` inside that same file collapses to a plain self-reference (a no-op when the composed name equals the local class).
