...
// Skip options for a non-CSS-Modules parse: the selector prelude is never walked
// without modules, and the value leaves nothing reads. `url`, functions, strings
// and at-rule preludes stay. A CSS-Modules parse skips nothing.
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const visitors = {
			...
			[NodeType.Ident](path) {
				const node = path.node;
				// Exit before slicing the ident value: outside dashed-ident scoping and
				// ICSS a bare ident carries no work, and idents are the commonest node.
				// The dashed probe reads two char codes rather than slicing.
