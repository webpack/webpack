		...
		walkCssTokens(source, {
			...
			delim: (input, start, end) => {
				if (isNextRulePrelude && isLocalMode()) {
					const identEnd = walkCssTokens.eatIdentSequence(input, end);

					if (end !== identEnd) {
						const name = input.slice(end, identEnd);
						const dep = new CssLocalIdentifierDependency(name, [end, identEnd]);
						const { line: sl, column: sc } = locConverter.get(start);
			...
			colon: (input, start, end) => {
				if (isModules) {
					const identEnd = walkCssTokens.eatIdentSequence(input, end);
					const name = input.slice(start, identEnd).toLowerCase();

					if (input.charCodeAt(identEnd) === CC_LEFT_PARENTHESIS) {
						return end;
					}
					...
					switch (scope) {
						case CSS_MODE_TOP_LEVEL: {
							if (name === ":export") {
								const pos = parseExports(input, identEnd);
						...
						// falls through
						default: {
							if (isNextRulePrelude && end !== identEnd) {
								if (name === ":global") {
									modeData = "global";
									// Eat extra whitespace and comments
									end = walkCssTokens.eatWhitespace(input, identEnd);
									const dep = new ConstDependency("", [start, end]);
									module.addPresentationalDependency(dep);
									return end;
								} else if (name === ":local") {
			...
			function: (input, start, end) => {
				...
				if (scope === CSS_MODE_IN_AT_IMPORT && name === "supports") {
				...
				if (
					isNextRulePrelude &&
					isModules &&
					input.charCodeAt(start - 1) === CC_COLON
				) {
					if (name === "global") {
						modeData = "global";
						const dep = new ConstDependency("", [start - 1, end]);
						module.addPresentationalDependency(dep);
