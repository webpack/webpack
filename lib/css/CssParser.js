...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// === AST-driven parallel walker (step E1 of the streaming-walker
		// retirement) ===
		//
		// `parseAStylesheet` consumes the whole source up front and gives
		// us a structural tree of qualified rules + at-rules; subsequent
		// commits will progressively migrate the streaming walker's
		// per-token dep-emission logic into per-node handlers driven from
		// this tree. This first commit only adds the scaffold: we parse
		// the AST, recursively visit every node, and emit nothing.
		//
		// Running the AST walk before the streaming walker means any
		// parse-time exception surfaces here first, which makes the
		// inevitable bug reports in E2…E5 easier to bisect — the
		// streaming walker's state hasn't been touched yet, so a failure
		// here proves the issue is in the AST module rather than in the
		// migration. The cost is one extra tokenization pass (~the same
		// work the streaming walker does), which we will reclaim in E5
		// when the streaming walker itself is deleted.
		//
		// `_astSink` is a sentinel that side-effects nothing — present so
		// the recursion can be measured (and so a stray `return` from a
		// handler still has somewhere to land). Once E2…E5 wire real
		// handlers in, they take over from this no-op.
		/**
		 * @param {AstNode} _node node being walked
		 * @returns {void}
		 */
		const _astSink = (_node) => {};
		/**
		 * Walk a list of items returned by `parseABlocksContents`. Each
		 * item is a `Declaration`, an `AtRule`, or a `QualifiedRule`.
		 * @param {(Declaration | AtRule | QualifiedRule)[]} items items to walk
		 * @returns {void}
		 */
		const walkAstBlockContents = (items) => {
			for (const item of items) {
				if (item.type === "declaration") {
					_astSink(/** @type {Declaration} */ (item));
				} else if (item.type === "at-rule") {
					walkAstAtRule(/** @type {AtRule} */ (item));
		...
		/**
		 * Walk an at-rule. The prelude is handed to `_astSink` (E3/E4
		 * will swap in selector/value emitters); the body — if the
		 * at-rule has one — is re-parsed via `parseABlocksContents` and
		 * walked recursively, matching what the streaming walker does
		 * via its `leftCurlyBracket`/`rightCurlyBracket` callbacks.
		 * @param {AtRule} at at-rule node
		 * @returns {void}
		 */
		const walkAstAtRule = (at) => {
			_astSink(at);
			if (at.block) {
				// `comment` is intentionally omitted from the recursive
				// `parseABlocksContents` calls — the streaming walker is
				// still authoritative for comment side-effects and we
				// don't want to double-record them. Removed in E5 when
				// the streaming walker goes away and we route
				// `comment` through here instead.
				const inner = parseABlocksContents(
					source,
					at.block.start + 1,
					locConverter
				);
				walkAstBlockContents(inner.values);
			}
		};
		/**
		 * Walk a qualified rule (a style rule). Both the prelude
		 * (selectors) and the body are visited; the body is the same
		 * `parseABlocksContents` re-parse pattern as for at-rules with
		 * blocks (CSS Nesting puts declarations and child rules inside
		 * a qualified rule's `{ … }`).
		...
		};
		{
			...
			for (const rule of stylesheet.rules) {
				if (rule.type === "at-rule") {
					walkAstAtRule(/** @type {AtRule} */ (rule));
		...
		walkCssTokens(source, 0, {
			...
			atKeyword: (input, start, end) => {
				...
				// For `;`-terminated at-rules, advance the prelude pointer so a later impure rule's reported selector doesn't include this at-rule's text (body-bearing ones return at `{`).
