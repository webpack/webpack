...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// True inside an at-rule prelude (keyword up to `{`/`;`) so prelude identifiers aren't counted as declarations.
		 ...
		 * resolves to an `@value`-defined ICSS symbol. The streaming
		 * walker still owns the call site (`case "identifier"`)
		 * because the symbol look-up depends on source-order
		 * `@value` resolution that the streaming pass builds up in
		 * `icssDefinitions` as it walks — moving the look-up to the
		 * AST walker (which runs after streaming, with the final
		 * `icssDefinitions` snapshot) would mis-resolve every
		 * reference to a name that gets redefined later in the file.
		 * @param {string} name ICSS symbol name
		 * @param {number} start start position
		 * @param {number} end end position
		 * @returns {number} `end` (used as `skipUntil` by the
		 * streaming walker so its `case "identifier"`
		 * fall-through doesn't run on the ident)
		 */
		...
		const processLocalDeclaration = (input, pos, end) => {
			// `pos`/`end` already bound the identifier the streaming
			// walker just emitted, so re-running `eatWhitespaceAndComments`
			// + `eatIdentSequence` is dead work — just verify a `:`
			// follows.
			const propertyNameStart = pos;
			const identEnd = end;
			pos = walkCssTokens.eatWhitespaceAndComments(input, identEnd)[0];
			...
			const propertyName = input
				.slice(propertyNameStart, identEnd)
				.replace(/^(-\w+-)/, "")
				.toLowerCase();
			...
			if (isLocalMode() && knownProperties.has(propertyName)) {
				...
				// E4a-2: property-name local emission moved to
				// `processLocalDeclarationPropertyNames` (called from
				// `walkAstBlockContents`).
				// E3c / E4a-1: URL deps and `local()` / `global()`
				// strip + ICSS emission also moved to the AST walker.
				// E4d: `var(--dashed)` / `style(--dashed)` ICSS export
				// emission moved to `walkDashedIdentsInValue` (also
				// called from `walkAstBlockContents`).
				//
				// What stays here is the `skipUntil = decl.end` return
				// value — the streaming walker uses it to skip past the
				// whole known-property declaration so its `case
				// "identifier"` doesn't fire for inner idents (the AST
				// walker is the sole emitter now, but the
				// `skipUntil` still prevents re-tokenization).

				return decl.end;
			} else if (COMPOSES_PROPERTY.test(propertyName)) {
				// E4a-3: `composes:` / `compose-with:` declaration
				// processing moved to `processComposesAst`
				// (called from `walkAstBlockContents` when the rule
				// has a local anchor). The streaming walker still
				// consumes the whole declaration via the
				// `skipUntil = decl.end` below so its
				// `case "identifier"` doesn't fire on inner
				// composes operands (they'd otherwise be parsed as
				// property-name heads and trigger a recursive
				// `processLocalDeclaration` for each one).
				if (lastLocalIdentifiers.length === 0) {
					// No local anchor — let the main walker continue
					// and re-tokenize the value so any nested
					// `local(…)` / `global(…)` inside still get their
					// normal source rewriting from the AST walker's
		...
		const processClassSelector = (input, start, end) => {
			...
			if (!ident) return end;
			// E3b: dep emission moved to `walkAstSelectorList`. What stays
			// here is the state both `composes:` resolution and
			// `leftCurlyBracket:`'s pure-mode dispatch read synchronously,
			// before the AST pass runs:
			//   * `lastLocalIdentifiers` — list of class/id names on the
			//     current rule's selectors; consumed by
			//     `processLocalDeclaration` when it sees `composes: foo`.
			//   * `currentSelectorHasLocal` — pure-mode flag used by
			//     `leftCurlyBracket:` to decide whether the new block
			//     opened a local-rule scope.
		...
		/**
		 * Walk a list of items returned by `parseABlocksContents`.
		 ...
		 * @param {(Declaration | AtRule | QualifiedRule)[]} items items to walk
		 * @param {boolean} topLevel whether this list is the stylesheet's top level
		 * @returns {void}
		 */
		const walkAstBlockContents = (items, topLevel) => {
			for (const item of items) {
				if (item.type === "at-rule") {
					walkAstAtRule(/** @type {AtRule} */ (item), topLevel);
					if (topLevel && /** @type {AtRule} */ (item).terminator === "{") {
						allowAstImport = false;
					}
				} else if (item.type === "qualified-rule") {
					walkAstQualifiedRule(/** @type {QualifiedRule} */ (item));
					if (topLevel) allowAstImport = false;
				} else if (item.type === "declaration") {
					// Declaration-value side effects: URL deps, `local(…)`
					// / `global(…)` strip + ICSS exports. Other
					// declaration processing (CSS-Modules name rewriting
					// for `animation-name`, `composes:` etc.) still
					// lives in the streaming walker's
					// `processLocalDeclaration` until the rest of E4.
				...
				// Match the streaming walker's modeData lifecycle: a top-
				// level `}` resets it to `undefined`. Inside a top-level
				// rule's body, sibling nested rules inherit each other's
				// trailing modeData, so this reset only fires between
				// **top-level** items.
		...
		const walkAstAtRule = (at, topLevel) => {
			...
			switch (name) {
				...
				default: {
					if (!isModules) break;
					if (name === "@value") {
						// `@value` is deliberately *not* migrated to the AST
						// walker. Source-order matters: `@value`
						// definitions and the declaration-value references
						// to them have to be processed interleaved (the
						// references resolve against `icssDefinitions` at
						// the moment they're seen). The streaming walker
						// still owns this until E4 migrates declaration
						// values; trying to move it here would require
						// teaching the AST walker to recurse into
						// declarations too, which is exactly E4.
						break;
					} else if (
			...
			if (at.block) {
				const inner = parseABlocksContents(
					source,
					at.block.start + 1,
					locConverter
				);
				walkAstBlockContents(inner.values, false);
			} else if (
				isModules &&
				at.terminator === ";" &&
				name !== "@import" &&
		...
		/**
		 * Walk a qualified rule (a style rule). Selectors (E3a — ID and
		 * attribute only so far) are emitted here; class selectors and
		 * declaration values are left to the streaming walker / E3b.
		 * @param {QualifiedRule} rule qualified-rule node
		 * @returns {void}
		 */
		const walkAstQualifiedRule = (rule) => {
			// `:import("path") { … }` / `:export { … }` ICSS pseudo-rules
			// are processed by the streaming walker's `colon:` case
			// (`processImportOrExport`) — both the import bindings and
			// the export name/value list are emitted there. Skipping
			// the AST walk for these prevents the body's declarations
			// from being mistakenly handled as regular declarations
			// (which would, for example, auto-export their dashed
			// property names as ICSS exports).
			if (isModules) {
				let firstIdx = 0;
				while (
		 ...
		 */
		const runAstWalker = () => {
			const stylesheet = parseAStylesheet(source, locConverter);
			for (const rule of stylesheet.rules) {
				if (rule.type === "at-rule") {
					walkAstAtRule(/** @type {AtRule} */ (rule), true);
					if (/** @type {AtRule} */ (rule).terminator === "{") {
						allowAstImport = false;
					}
				} else if (rule.type === "qualified-rule") {
					walkAstQualifiedRule(/** @type {QualifiedRule} */ (rule));
					allowAstImport = false;
				}
				// Top-level `}` resets the persistent CSS-Modules mode —
				// matches the streaming walker's `rightCurlyBracket:`
				// `modeData = undefined` on the outermost block close.
				astModeData = undefined;
			}
		};

		// Consume the token stream from `walkCssTokens` directly —
		// switch on `t.type` for each token. Callbacks that used to
		// return a "skip past" position now set `skipUntil`; the
		// next iteration ignores any token whose `start` falls before
		// it. Callbacks that returned `end` (no skip) just `break`.
		let skipUntil = -1;
		for (const t of walkCssTokens(source, 0)) {
			...
			switch (t.type) {
				...
				case "rightCurlyBracket": {
					if (scope !== CSS_MODE_IN_BLOCK) break;
				...
				case "url": {
					// E3c: url-dep emission moved to `walkAstBlockContents`
					// (declaration values walked through
					// `walkFunctionsForUrl`).
					break;
				}
				case "atKeyword": {
					...
					// E2: dep emission for at-rules has moved to `walkAstAtRule`
					// (see the AST-driven walker above). What remains here is
					// position-advancement and the per-at-rule state flags the
					// other streaming-walker callbacks still rely on
					// (`isNextRulePrelude`, `nextBlockChildrenSkip`,
					// `nextBlockTreatAsLeaf`, `currentRulePreludeStart`).
					//
					// `@scope` keeps its old "don't consume the prelude — let
					// the main walker tokenize it as a selector list" path so
					// selector emission (`delim:`/`hash:`/`leftSquareBracket:`)
					// still fires until E3 migrates selector dispatch.
					if (isModules && name === "@scope") {
						isNextRulePrelude = true;
						break;
					}
					// `@media`, `@supports`, `@layer (without block)`, `@font-face`,
					// `@page` and the rest of the non-selector-prelude at-rules:
					// the prelude carries no CSS-Modules-relevant content, so
					// returning `end` lets the streaming walker tokenize it as
					// generic component values. Same fallback as the pre-E2
					// "default fall-through" branch.
					//
					// `@keyframes` / `@counter-style` / `@container` are only
					// "AST handled" when their respective option is enabled —
					// otherwise the AST walker emits nothing and the streaming
					// walker still needs to tokenize the prelude so its
					// `function:` callback can pick up explicit
					// `local()` / `global()` markers (the pre-E2 behaviour).
					//
					// `@value` is **not** in the AST-handled list — its
					// `icssDefinitions` updates are source-order-sensitive
					// (declaration-value references resolve against the live
					// definition list), so the streaming walker keeps owning
					// it until E4 migrates declaration values. The `@value`
					// case below restores the pre-E2 emission.
					if (isModules && name === "@value") {
						const at = parseAtRule(input, start, locConverter, comment);
						const pos = at ? processAtValue(at) : eatUntilSemi(input, end) + 1;
					...
					const isAstHandled =
						...
						(isModules &&
							((this.options.animation &&
								...
								(this.options.container && name === "@container")));
					if (!isAstHandled) {
						if (isModules) isNextRulePrelude = false;
						break;
					}

					// AST-handled at-rule: advance past it. The AST walker has
					// already emitted any deps; we just need to skip the
					// at-rule's source span so the streaming walker resumes
					// after it (without firing token callbacks for prelude
					// content that's already been processed).
					const at = parseAtRule(input, start, locConverter, comment);
					let pos = at ? at.end : eatUntilSemi(input, start);

					// `@keyframes` / `@counter-style` under pure-mode + local-mode
					// need their bodies marked "skip" / "treat as leaf" so
					// pure-mode bookkeeping doesn't double-warn on inner
					// declarations (matches PCSL).
					//
					// Pure-mode also needs `currentSelectorHasLocal` set
					// before `leftCurlyBracket:` fires — `processLocalAtRule`
					// (now in the AST walker, which runs **after** the
					// streaming walker) used to be responsible for that.
					// Scan the parsed prelude here for a *local* marker:
					// `local(<name>)` / `:local(<name>)` and bare idents /
					// quoted strings count; `global(<name>)` /
					// `:global(<name>)` explicitly opt out, matching
					// `processLocalAtRule`'s first-non-whitespace dispatch.
					if (
						pureMode &&
						isModules &&
						isLocalMode() &&
						...
						for (const cv of at.prelude) {
							...
							if (cv.type === "ident") {
								...
								const text = input.slice(cv.start, cv.end);
					...
					// `;`-terminated at-rules: advance past the `;` and a single
					// trailing newline (the pre-AST `processAtImport`/
					// `processAtValue` did this internally). Keeping the
					// behaviour preserves source positions for any warning
					// emitted by a *later* at-rule.
					if (at && at.terminator === ";") {
						pos = walkCssTokens.eatWhiteLine(input, at.end + 1);
					}

					// Pure-mode prelude-start tracking for `;`-terminated
					// top-level at-rules. Body-bearing at-rules return at `{`
					// (their `.terminator === "{"`) — `leftCurlyBracket:`
					// handles those.
					if (pureMode && wasTopLevel && pos > end) {
						let probe = pos - 1;
						while (
				...
				case "identifier": {
					if (isModules) {
						...
						// E4d: dashed-ident emission for top-level idents
						// (property names + value-level idents in
						// non-knownProperty declarations) moved to
						// `walkDashedIdentsInValue` / the declaration
						// branch in `walkAstBlockContents`. Keep a
						// dashed-ident short-circuit here so the
						// `icssDefinitions.has` check below doesn't run
						// for dashed-idents that happen to also be
						// `@value`-defined — the streaming walker's
						// original priority was "dashed wins, no
						// rewrite", and removing the dashed branch
						// inverted that.
						if (
							this.options.dashedIdents &&
							isLocalMode() &&
						...
						if (icssDefinitions.has(identifier)) {
							skipUntil = emitICSSSymbol(identifier, start, end);
							break;
						}
						...
						switch (scope) {
							case CSS_MODE_IN_BLOCK: {
								if (isModules && !isNextRulePrelude) {
									...
									skipUntil = processLocalDeclaration(input, start, end);
									break;
								}
								break;
							}
				...
				case "delim": {
					...
					if (ch === CC_FULL_STOP && isNextRulePrelude && isLocalMode()) {
						skipUntil = processClassSelector(input, start, end);
						break;
					}
					if (
						ch === CC_AMPERSAND &&
				...
				case "hash": {
					...
					if (
						isNextRulePrelude &&
						isLocalMode() &&
						isID && // E3a: ID-selector dep emission moved to
						// `walkAstSelectorList`. The pure-mode side effect
						// (`currentSelectorHasLocal = true`) still has to
						// happen synchronously so `leftCurlyBracket:`'s
						// `finalizeSelector` sees the right state.
						pureMode
					) {
						currentSelectorHasLocal = true;
					}
						...
						switch (scope) {
							...
							default: {
								if (
									...
									// E3d: `:local` / `:global` strip-dep emission
									// moved to `walkAstSelectorList`. What stays
									// here is the synchronous bookkeeping the rest
									// of the streaming walker still relies on:
									//   * `modeData` — read by `isLocalMode()` for
									//     in-block declaration processing
									//     (`processLocalDeclaration` /
									//     `processClassSelector`) that runs
									//     before the AST pass.
									//   * `balanced.push([`:name`, …, true])` —
									//     the matching `rightParenthesis:` pops
									//     it to restore `modeData`.
									//   * `skipUntil` — keeps the streaming
									//     walker from re-tokenizing the marker
									//     and (mis-)emitting class / hash deps
									//     for the `local`/`global` ident itself.
									//   * the "missing whitespace after :…"
									//     warning for the bare form.
									const mode = name === "local" ? LOCAL_MODE : GLOBAL_MODE;
									const isFn =
										input.charCodeAt(ident[1]) === CC_LEFT_PARENTHESIS;
									modeData = mode;
									if (isFn) {
										const stripEnd = walkCssTokens.eatWhitespaceAndComments(
											input,
											ident[1] + 1
										)[0];
										balanced.push([`:${name}`, start, stripEnd, true]);
										skipUntil = stripEnd;
										break;
									}
									const after = walkCssTokens.eatWhitespaceAndComments(
										input,
										ident[1]
									);
									if (!after[1]) {
										this._emitWarning(
											state,
											`Missing whitespace after ':${name}' in '${input.slice(
												start,
									...
									skipUntil = walkCssTokens.eatWhitespace(input, ident[1]);
									break;
								}
							}
						}
				...
				case "function": {
					...
					// E3c: url-dep emission for `url()` / `src()` /
					// `image-set()` moved to `walkAstBlockContents` (each
					// declaration value walked through
					// `walkFunctionsForUrl`).
					// E4a-1: `local(…)` / `global(…)` strip-dep + ICSS
					// export emission moved to `walkAstBlockContents` /
					// `walkAstAtRule` (walked through
					// `walkFunctionsForLocalGlobal`). The streaming
					// walker still needs to skip past these functions
					// so its `case "identifier"` doesn't re-process the
					// inner dashed-ident as a property reference (which
					// would double-emit the ICSS export via
					// `processDashedIdent`).
					if (
						isModules &&
						!isNextRulePrelude &&
					...
					// E4d: `--foo(args)` custom-function call ICSS
					// export emission moved to
					// `walkDashedIdentsInValue` / the declaration
					// branch in `walkAstBlockContents`.

					break;
				}
				case "leftSquareBracket": {
					// E3a: attribute-selector dep emission moved to
					// `walkAstSelectorList`. `processAttributeSelector` had no
					// per-token side effects (no `currentSelectorHasLocal`
					// update, no `lastLocalIdentifiers` push), so the
					// streaming walker has nothing left to do here.
					break;
				}
				...
				case "rightParenthesis": {
					...
					if (isModules && popped) {
						...
						if (isLocal || isGlobal) {
							// E3d: strip-dep emission for the closing `)`
							// (and trailing whitespace for `:local`) moved
							// to `walkAstSelectorList`. The `modeData`
							// restore stays — declaration-value
							// processing still reads `isLocalMode()`
