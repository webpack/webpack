...
class CssParser extends Parser {
	...
	_resolveAmbiguousExports(state, declaredExports, cssExportEntries) {
		...
		for (const [name, decls] of declaredExports) {
			...
			// Prefer `class` as the winner (matches css-loader / lightning css / postcss-modules).
			let winner;
				...
				if (
					...
					state.current.addWarning(
						new ModuleDependencyWarning(
							state.module,
							new WebpackError(
								`Conflicting CSS module export "${name}": already declared as ${first.kind} at line ${first.line}:${first.column}, redeclared as ${loser.kind}`
							),
							{
								start: {
									line: anchor.locStartLine,
