...
/** @typedef {{value: string, start: number, end: number}} Comment */
...
const WEBPACK_IGNORE_COMMENT_REGEX = /webpackIgnore:(\s+)?(true|false)/;
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const balanced = [];
		/** @type {undefined | { start: number, url?: string, urlStart?: number, urlEnd?: number, layer?: string, layerStart?: number, layerEnd?: number, supports?: string, supportsStart?: number, supportsEnd?: number, inSupports?:boolean, media?: string, comment?: Comment  }} */
		let importData;
		/** @type {boolean} */
		let inAnimationProperty = false;
		/** @type {boolean} */
		let isNextRulePrelude = true;
		/** @type {undefined | { propertyNameStart?: number, propertyNameEnd?: number, comment?: Comment } } */
		...
		/**
		 * Do not pass callbacks when the range has already been walked.
		 ...
		 */
		const eatText = (input, pos, eater, callbacks) => {
			let text = "";
			for (;;) {
				if (input.charCodeAt(pos) === CC_SLASH) {
					const newPos = walkCssTokens.eatComments(input, pos, callbacks);
					if (pos !== newPos) {
						pos = newPos;
		...
		const parseExports = (input, pos) => {
			pos = walkCssTokens.eatWhitespaceAndComments(input, pos, callbacks);
			const cc = input.charCodeAt(pos);
			if (cc !== CC_LEFT_CURLY) {
				this._emitWarning(
					state,
			...
			for (;;) {
				...
				const start = pos;
				let name;
				[pos, name] = eatText(input, pos, eatExportName, callbacks);
				if (pos === input.length) return pos;
				...
				if (pos === input.length) return pos;
				let value;
				[pos, value] = eatText(input, pos, eatExportValue, callbacks);
				if (pos === input.length) return pos;
				const cc = input.charCodeAt(pos);
				if (cc === CC_SEMICOLON) {
					pos++;
		...
		const processDeclaration = (input, pos, end) => {
			const propertyNameStart = pos;
			const [propertyNameEnd] = eatText(input, pos, eatPropertyName, callbacks);
			if (input.charCodeAt(propertyNameEnd) !== CC_COLON) return end;
			currentDeclaration = {
				propertyNameStart,
				propertyNameEnd,
		...
		const callbacks = {
			...
			url: (input, start, end, contentStart, contentEnd) => {
				...
				switch (scope) {
					...
					case CSS_MODE_IN_BLOCK: {
						...
						const declarationIgnore = currentDeclaration.comment
							? this._parseComment(currentDeclaration.comment).option.needIgnore
							: undefined;
						const needIgnore = this.parseCommentOptions([
							lastCommaPos || currentDeclaration.propertyNameEnd,
							start
						]).options.needIgnore;
						if (needIgnore !== undefined ? needIgnore : declarationIgnore) {
							break;
						}
				...
				switch (scope) {
					...
					case CSS_MODE_IN_BLOCK: {
						...
						if (
							...
							const declarationIgnore = currentDeclaration.comment
								? this._parseComment(currentDeclaration.comment).option
										.needIgnore
								: undefined;
							const needIgnore = this.parseCommentOptions([
								lastCommaPos || currentDeclaration.propertyNameEnd,
							...
							let stringIgnore;
							if (IMAGE_SET_FUNCTION.test(last[0].replace(/\\/g, ""))) {
								stringIgnore = this.parseCommentOptions([
									lastCommaPos || last[2],
									start
								]).options.needIgnore;
							}
			...
			atKeyword: (input, start, end) => {
				...
				if (name === "@namespace") {
					...
					importData = { start, comment: lastComment };
				} else if (
					...
					pos = walkCssTokens.eatWhitespaceAndComments(input, pos, callbacks);
					if (pos === input.length) return pos;
					const [newPos, name] = eatText(input, pos, eatKeyframes, callbacks);
			...
			semicolon: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_AT_IMPORT: {
						...
						const { start, comment } = importData;
						const outsideIgnore = comment
							? this._parseComment(comment).option.needIgnore
							: undefined;
						const needIgnore = this.parseCommentOptions([start, end]).options
							.needIgnore;
						if (needIgnore !== undefined ? needIgnore : outsideIgnore) {
			...
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						...
						if (!currentDeclaration) processDeclaration(input, start, end);
						break;
					}
					case CSS_MODE_IN_AT_IMPORT: {
			...
			function: (input, start, end) => {
				...
				if (isLocalMode()) {
					...
					if (name === "var") {
						...
						if (pos === input.length) return pos;
						const [newPos, name] = eatText(input, pos, eatNameInVar, callbacks);
			...
			comment: (input, start, end) => {
				const comment = {
					value: input.slice(start + 2, end - 2).trim(),
					start,
					end
				};
		...
		walkCssTokens(source, callbacks);
		if (oldDefaultMode) {
	 ...
	 * @returns {Comment[]} comments in range
	 */
	getComments(range) {
	...
	/**
	 * @param {[number,number]} range range
	 * @returns {{options: {needIgnore: boolean}, errors: unknown[]}} options and errors
	 */
	parseCommentOptions(range) {
		...
		for (const comment of comments) {
			const { option, error } = this._parseComment(comment);
			options.needIgnore = option.needIgnore;
			if (error) {
				errors.push(error);
			}
		}
	...
	/**
	 * @param {Comment} comment comment
	 * @returns {{option: {needIgnore?: boolean}, error?: Error}} comment parse result
	 */
	_parseComment(comment) {
		...
		if (value) {
			...
			try {
				const matched = value.match(WEBPACK_IGNORE_COMMENT_REGEX);
				if (matched) {
					option.needIgnore = matched[2] === "true";
				}
			} catch (err) {
				error = new Error(String(err.message));
				error.stack = String(err.stack);
				Object.assign(error, { comment });
			}
		}
		return { option, error };
	}
