				...
				end,
				{
					...
					identifier(input, start, end) {
						if (!found) {
							...
							const isDashed = options.dashed && isDashedIdentifier(value);

							if (options.identifier || isDashed) {
								let identifier = unescapeIdentifier(value);
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (OPTIONALLY_VENDOR_PREFIXED_ANIMATION_PROPERTY.test(propertyName)) {
				...
				if (counterStyleNames.length > 0) {
					for (const styleName of counterStyleNames) {
						const [start, end] = styleName;
						const { line: sl, column: sc } = locConverter.get(start);
						const { line: el, column: ec } = locConverter.get(end);
