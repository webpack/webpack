...
const knownPropertyForRange = (index, input, start, end) => {
	...
	for (let i = start; i < end; i++) {
		...
		// An escaped property name hashes as its raw bytes, which the table never
		// holds — the loop is already reading them, so this costs a comparison
		// rather than the separate scan a miss would otherwise pay.
		if (c === CC_REVERSE_SOLIDUS) {
			return knownPropertyForName(
				index,
...
/**
 * `knownPropertyForRange` for a name already unescaped — only an escaped property pays it.
 * @param {Map<number, string | string[]>} index hash index from `getKnownPropertyIndex`
 * @param {string} name unescaped property name
 * @returns {string | undefined} canonical lowercase name, or undefined when unknown
 */
const knownPropertyForName = (index, name) => {
	let h = name.length;
	for (let i = 0; i < name.length; i++) {
	...
	if (hit === undefined) return undefined;
	if (typeof hit === "string") {
		return equalsLowerCase(name, hit) ? hit : undefined;
	}
	for (let i = 0; i < hit.length; i++) {
		if (equalsLowerCase(name, hit[i])) return hit[i];
	}
	return undefined;
};
...
/**
 * Whether a raw ident range names a custom property. An escaped dash spells one
 * too (`\2d\2d x` is `--x`), so only a name that could carry one — it has to
 * open with a dash or a backslash — pays the unescape.
 * @param {string} input source
 ...
 * Tokens are whitespace-separated (a whitespace closing a hex escape does not
 * separate), and a run of only `.` is a null cell that names nothing.
 * @param {string} input source
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * The custom property's name, without its `--`. Two raw bytes in the plain
		 * spelling; an escaped dash makes the prefix longer, so it comes off the
