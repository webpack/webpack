			...
			for (const cv of cvs) {
				if (cv.type === "Comma") {
					...
					if (fname === "src" || fname === "url") {
						...
						// Recurse into every other function — a `url()` /
						// `src()` can be nested anywhere the grammar allows
						// a component value. The walk over the body is the
						// natural thing the SourceProcessor `walkValue`
						// recursion would do anyway.
						walkFunctionsForUrl(fn.value, skipUrlTokens);
					}
