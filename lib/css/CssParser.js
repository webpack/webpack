	...
	parse(source, state) {
		...
		const urlActive = () => {
			...
			if (currentStructural.type === NodeType.AtRule) {
				...
				// `@namespace`'s url() is a namespace URI, not a resource.
				if (equalsLowerCase(at.name, "namespace")) return false;
						...
						case "@namespace": {
							const atRuleEnd =
								source.charCodeAt(at.end) === CC_SEMICOLON
									? at.end + 1
									: at.end;
							// Strip here; the rule is hoisted to the top of the output
							// (extract: CssModulesPlugin; inline: CssGenerator) — an
							// `@namespace` is only valid before any style rule.
							module.addPresentationalDependency(
								new ConstDependency("", [at.start, atRuleEnd])
							);
							// First ident in the prelude is the prefix (absent = default namespace).
							const prefixNode = at.prelude.find(
								(v) => v.type !== NodeType.Whitespace
							);
							const prefix =
								prefixNode && prefixNode.type === NodeType.Ident
									? source.slice(prefixNode.start, prefixNode.end)
									: "";
							const buildInfo = /** @type {CssModuleBuildInfo} */ (
								module.buildInfo
							);
							(buildInfo.namespaces = buildInfo.namespaces || []).push({
								prefix,
								rule: source.slice(at.start, at.end).trim()
							});
							break;
						}
						case "@charset": {
