...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (KNOWN_PROPERTIES[propertyName] && isLocalMode()) {
						...
						identifier(input, start, end) {
							...
							parsedKeywords[keyword] =
								typeof parsedKeywords[keyword] !== "undefined"
									...
									: 0;
							const keywords = KNOWN_PROPERTIES[propertyName];
				...
				switch (name) {
					...
					default: {
						if (isModules) {
							...
							} else if (name === "@counter-style" && isLocalMode()) {
								return processLocalAtRule(input, end, {
									identifier: true
								});
							} else if (name === "@container" && isLocalMode()) {
								return processLocalAtRule(input, end, {
