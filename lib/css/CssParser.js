						...
						switch (scope) {
							...
							default: {
								if (
									...
									modeData = mode;
									if (isFn) {
										// `:local(…)` / `:global(…)` — strip the marker and
										// any whitespace inside the open paren so output sees
										// just the body's selectors. The matching `)` is
										// popped from `balanced` in `rightParenthesis:`.
										const stripEnd = walkCssTokens.eatWhitespaceAndComments(
											input,
											ident[1] + 1
										)[0];
										module.addPresentationalDependency(
											new ConstDependency("", [start, stripEnd])
										);
										balanced.push([`:${name}`, start, stripEnd, true]);
										skipUntil = stripEnd;
										break;
									}
									// Bare `:local` / `:global` — must be followed by
									// whitespace to be a valid pseudo-class prefix.
				...
				case "rightParenthesis": {
					...
					if (isModules && popped) {
						...
						if (isLocal || isGlobal) {
							...
							if (popped[3] && isLocal) {
								while (
									walkCssTokens.isWhiteSpace(input.charCodeAt(start - 1))
								) {
									start -= 1;
								}
							}
