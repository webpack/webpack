...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// Nearest enclosing declaration / at-rule / qualified-rule, set by each structural enter; the Url / Function / Comma visitors read it to decide url() handling from the node hierarchy instead of carrying url flags.
		/** @type {AstNode | undefined} */
		...
		const visitors = {
			AtRule: {
				...
				enter: (node, parent) => {
					...
					vLocalGlobal = vIcss = vDashed = false;
			...
			// Value-level visitors gated by `urlActive()` / the `vIcss` / … flags the enclosing structural enter set.
			Url: (node) => {
				if (!urlActive()) return;
