...
const parseValueAtRuleParams = (str) => {
	if (VALUE_IMPORT_FORM.test(str)) {
		...
		// `from` is a keyword: split on the first whitespace-delimited `from`, so a
		// path or import name containing the substring (e.g. `./from-colors.css`)
		// isn't mistaken for the separator.
		const fromMatch = /^([\s\S]+?)\s+from\s+([\s\S]+)$/i.exec(str);
