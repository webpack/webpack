		...
		const walkAstBlockContents = (items, topLevel) => {
			for (const item of items) {
				if (item.type === "at-rule") {
					...
					walkAstQualifiedRule(/** @type {QualifiedRule} */ (item), topLevel);
		...
		for (const t of walkCssTokens(source, 0)) {
			...
			switch (t.type) {
				...
				case "atKeyword": {
					...
					//
					// `@value` (E4c) is processed by the AST walker now; the
					// streaming walker just needs to advance past it (via the
					// generic AST-handled at-rule fall-through below) so its
					// `;`/`{` body doesn't get re-tokenized into bogus
					// `case "identifier"` / `case "colon"` fires.
					const isAstHandled =
						name === "@import" ||
						name === "@charset" ||
						name === "@namespace" ||
						(isModules && name === "@value") ||
						(isModules &&
							((this.options.animation &&
								OPTIONALLY_VENDOR_PREFIXED_KEYFRAMES_AT_RULE.test(name)) ||
								(this.options.customIdents && name === "@counter-style") ||
				...
				case "identifier": {
					if (isModules) {
						...
						// streaming walker's `processLocalDeclaration`
						// fall-through below doesn't try to treat a
						// dashed ident as a property head.
						if (
							this.options.dashedIdents &&
							isLocalMode() &&
						...
						// E4e: ICSS-symbol identifier rewrite
						// (`color: foo` → CSS-Modules rewrite when `foo`
						// is `@value`-defined) moved to
						// `walkIcssSymbolsInValue` and the declaration
						// branch of `walkAstBlockContents`. Source-order
						// semantics are preserved because the AST walker
						// iterates rules in source order, populating
						// `icssDefinitions` from `@value` /
						// `:import` AST visits before any later
						// declaration value walk reads it.
					...
					if (isModules) {
						...
						switch (scope) {
							case CSS_MODE_TOP_LEVEL: {
								if (name === "import" || name === "export") {
									// E4b: dep emission for `:import("path") { … }` /
									// `:export { … }` moved to the AST walker
									// (`walkAstQualifiedRule`). Here we just
									// `skipUntil` past the rule body so the
									// streaming walker doesn't re-tokenize its
									// declarations into bogus
									// `case "identifier"` / `case "colon"`
									// emissions. The strip-dep over the whole
									// rule is added by the AST walker.
									const rule = parseAQualifiedRule(
										input,
										start,
										locConverter,
										comment
									...
									const pos = rule
										? walkCssTokens.eatWhiteLine(input, rule.end)
										: end;
									skipUntil = pos;
									break;
								}
