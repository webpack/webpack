...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * Emit the CSS-Modules dep for a top-level dashed-ident in a value,
		 * resolving the `--foo from "./x.css"` / `--foo from global` import
		 * suffix via sibling lookahead.
		 * @param {Token} node the dashed `<ident-token>`
		 * @param {AstNode | null} parent enclosing node (for sibling lookahead)
		 * @returns {void}
		 */
		const emitDashedFromIdent = (node, parent) => {
			const siblings = childrenOf(parent);
			const i = siblings ? siblings.indexOf(node) : -1;
			if (siblings && i !== -1) {
				let j = i + 1;
				while (j < siblings.length && siblings[j].type === "Whitespace") j++;
				if (
					j < siblings.length &&
		...
		// Value-visitor context: set by a Declaration / at-rule / qualified-rule enter, read by the Url / Function / Ident / Comma visitors.
		...
		let vDashed = false; // dashed-ident (custom-property) scoping is active
		 ...
		 * `pureBlockStack` push.
		 * @param {AstNode} node at-rule node
		 * @param {AstNode | null} parent enclosing node (null at the stylesheet top level)
		 * @returns {void}
		 */
		const walkAstAtRuleEnter = (node, parent) => {
			...
			vUrl = vUrlSkip = vLocalGlobal = vIcss = vDashed = false;
			...
			if (isModules && name !== "@value" && name !== "@import") vIcss = true;
		 ...
		 * scope restore, and the top-level `@import` / mode-data reset.
		 * @param {AstNode} node at-rule node
		 * @param {AstNode | null} parent enclosing node (null at the stylesheet top level)
		 * @returns {void}
		 */
		const walkAstAtRuleExit = (node, parent) => {
		 ...
		 * @param {AstNode} node qualified-rule node
		 * @param {AstNode | null} parent enclosing node (null at the stylesheet top level — gates ICSS `:import` / `:export`)
		 * @param {VisitorContext} ctx visitor context
		 * @returns {void}
		 */
		const walkAstQualifiedRuleEnter = (node, parent, ctx) => {
		...
		/**
		 * Qualified-rule exit: pure-frame finalization + scope restore and the
		 * top-level `@import` / mode-data reset; no-op for bailed
		 * `:import` / `:export`.
		 * @param {AstNode} node qualified-rule node
		 * @param {AstNode | null} parent enclosing node (null at the stylesheet top level)
		 * @returns {void}
		 */
		const walkAstQualifiedRuleExit = (node, parent) => {
		...
		/** @type {CssVisitors} */
		const visitors = {
			AtRule: { enter: walkAstAtRuleEnter, exit: walkAstAtRuleExit },
			QualifiedRule: {
				enter: walkAstQualifiedRuleEnter,
				exit: walkAstQualifiedRuleExit
			},
			...
			// Value-level visitors gated by the `vUrl` / `vIcss` / … flags the enclosing structural enter set.
			Url: (node) => {
			...
			Ident(node, parent) {
				...
				if (vDashed && isDashedIdentifier(identValue)) {
					...
					if (vDashedEmit) emitDashedFromIdent(node, parent);
					return;
				}
				if (!vIcss) return;
