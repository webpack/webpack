...
/** @typedef {{identifierNames: string[], range: Range, identifierRanges: Range[], blockNestingLevel: number}} ClassDefinition */
					...
					case CSS_MODE_IN_BLOCK: {
						if (isLocalMode()) {
							...
							if (classDefinitions.length > 0) {
								const lastClassDefinition = classDefinitions.pop();
								lastClassDefinition.range[1] = end;

								const dep = new CSSModuleDefineDependency(
									lastClassDefinition.identifierNames,
									lastClassDefinition.range,
									lastClassDefinition.identifierRanges
								);
								const { line: sl, column: sc } = locConverter.get(
									lastClassDefinition.range[0]
								);
								const { line: el, column: ec } = locConverter.get(
									lastClassDefinition.range[1]
								);
								dep.setLoc(sl, sc, el, ec);
								module.addDependency(dep);
							}
			...
			class: (input, start, end) => {
				if (isLocalMode()) {
					...
					if (
						...
						classDefinitions[classDefinitions.length - 1].blockNestingLevel ===
							blockNestingLevel
					) {
						classDefinition = classDefinitions[classDefinitions.length - 1];
					} else {
						classDefinition = {
							identifierNames: [],
							range: [0, 0],
					...
					// add class name to the collector
					classDefinition.identifierNames.push(name);

					// update the range of the class if this is the first identifier
					if (classDefinition.identifierNames.length === 1)
						classDefinition.range = [start, 0];

					classDefinition.identifierRanges.push([start + 1, end]);
				}
