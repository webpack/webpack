...
const CSS_MODE_AT_IMPORT_EXPECT_LAYER = 4;
const CSS_MODE_AT_IMPORT_EXPECT_SUPPORTS = 5;
const CSS_MODE_AT_IMPORT_EXPECT_MEDIA = 6;
const CSS_MODE_AT_OTHER = 7;
...
const explainMode = mode => {
	switch (mode) {
		...
		case CSS_MODE_AT_IMPORT_EXPECT_LAYER:
			return "parsing @import (expecting optionally layer)";
		case CSS_MODE_AT_IMPORT_EXPECT_SUPPORTS:
			return "parsing @import (expecting optionally supports)";
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		walkCssTokens(source, {
			...
			url: (input, start, end, contentStart, contentEnd) => {
				...
				switch (mode) {
					case CSS_MODE_AT_IMPORT_EXPECT_URL: {
						modeData.url = value;
						modePos = end;
						mode = CSS_MODE_AT_IMPORT_EXPECT_LAYER;
						break;
					}
					case CSS_MODE_AT_IMPORT_EXPECT_LAYER:
					case CSS_MODE_AT_IMPORT_EXPECT_MEDIA:
			...
			layer: (input, start, end, contentStart, contentEnd) => {
				const value = cssUnescape(input.slice(contentStart, contentEnd));
				switch (mode) {
					case CSS_MODE_AT_IMPORT_EXPECT_LAYER: {
						modeData.layer = value.trim();
						modePos = end;
			...
			supports: (input, start, end, contentStart, contentEnd) => {
				const value = cssUnescape(input.slice(contentStart, contentEnd));
				switch (mode) {
					case CSS_MODE_AT_IMPORT_EXPECT_LAYER:
					case CSS_MODE_AT_IMPORT_EXPECT_SUPPORTS: {
						modeData.supports = value.trim();
						modePos = end;
						mode = CSS_MODE_AT_IMPORT_EXPECT_MEDIA;
						break;
					}
					case CSS_MODE_AT_IMPORT_EXPECT_MEDIA:
					case CSS_MODE_AT_IMPORT_EXPECT_URL:
			...
			atKeyword: (input, start, end) => {
				...
				if (name === "@layer") {
					...
					if (pos === input.length) return pos;
					if (
						input.charCodeAt(pos) !== CC_LEFT_CURLY &&
						input.charCodeAt(pos) !== CC_SEMICOLON
					) {
						throw new Error(
							`Unexpected ${input[pos]} at ${pos} during parsing of @layer (expected '{' or ';')`
						);
					}
					return pos + 1;
				}
