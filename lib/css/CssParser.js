...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const pure = {
			 ...
			 * @param {{ isRulePrelude: boolean, treatAsLeaf: boolean, ownSkip: boolean, childRules: (Declaration[] | Rule)[] | null, preludeStart: number, preludeEnd: number }} opts entry options
			 */
			enterBlock(opts) {
				if (!this.enabled) return;
				const {
				...
				const reportable =
					...
					this.ruleImpure;
				if (reportable) {
					let leaf = treatAsLeaf || !childRules;
					if (!leaf && childRules) {
						const { hasDirectDecl, hasNestedBlock } = scanRuleBody(childRules);
		...
		/**
		 * Flatten an at-rule / qualified-rule's `childRules` into siblings (declarations + rules). Per §5.4.5 consecutive declarations are grouped into a `Declaration[]` sub-list; downstream sites treat them as siblings.
		 * @param {(Declaration[] | Rule)[]} childRules child rules from the rule's body
		 * @returns {(Declaration | Rule)[]} flattened children
		 */
		const flatChildRules = (childRules) => childRules.flat();
		 ...
		 * @param {(Declaration[] | Rule)[]} childRules rule body
		 * @returns {{ hasDirectDecl: boolean, hasNestedBlock: boolean }} scan result
		 */
		const scanRuleBody = (childRules) => {
			let hasDirectDecl = false;
			let hasNestedBlock = false;
			for (const child of flatChildRules(childRules)) {
				if (child.type === "Declaration") {
					...
					if (!at.childRules) continue;
					hasNestedBlock = true;
					if (
						!hasDirectDecl &&
						!isPureBodyAtRule(`@${at.name.toLowerCase()}`) &&
						scanRuleBody(at.childRules).hasDirectDecl
					) {
						hasDirectDecl = true;
					}
				}
		...
		const processImportOrExport = (type, second, rule) => {
			...
			if (!rule.childRules || !rule.blockRange) return second.range[1];
			for (const item of flatChildRules(rule.childRules)) {
		...
		const finishTopLevelRule = (node, blockBearing) => {
			if (!blockBearing || /** @type {AtRule} */ (node).childRules) {
				allowImport = false;
			}
		...
		const visitors = {
			AtRule: {
				...
				enter: (node, parent) => {
					...
					if (at.childRules && at.blockRange) {
					...
					atRuleStateStack.push({
						...
						name,
						hasBlock: Boolean(at.childRules),
