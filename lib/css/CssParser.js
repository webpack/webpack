...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {[{url: string, range: [number, number], imports: {[key: string]: { value: string, replaceRanges: [number, number][]}}}]} */
		const icssImportData = [];
		...
		const parseImports = (input, start, pos) => {
			let url = "";
			pos = walkCssTokens.eatWhitespaceAndComments(input, pos);
			[pos, url] = eatText(input, pos, eatUntil(")"));
			pos++;
			const data = { url, range: [0, 0], imports: {} };
			pos = walkCssTokens.eatWhitespaceAndComments(input, pos);
			const cc = input.charCodeAt(pos);
			if (cc !== CC_LEFT_CURLY) {
				this._emitWarning(
			...
			for (;;) {
				...
				if (input.charCodeAt(pos) !== CC_COLON) {
					this._emitWarning(
						state,
						`Unexpected '${input[pos]}' at ${pos} during parsing of export name in ':import' (expected ':')`,
						locConverter,
						start,
						pos
					);
				...
				if (cc === CC_SEMICOLON) {
					...
					this._emitWarning(
						state,
						`Unexpected '${input[pos]}' at ${pos} during parsing of export value in ':import' (expected ';' or '}')`,
						locConverter,
						start,
						pos
					);
				...
				data.imports[name] = { value, replaceRanges: [] };
			}
			pos++;
			data.range = [start, pos];
			icssImportData.push(data);
			return pos;
		};
		...
		walkCssTokens(source, {
			...
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						if (icssImportData) {
							const dataIndex = icssImportData.findIndex(
								data => data.imports[input.slice(start, end)]
							);
							if (dataIndex > -1) {
								icssImportData[dataIndex].imports[
			...
			pseudoFunction: (input, start, end) => {
				...
				if (this.allowModeSwitch) {
					...
					if (name === ":global") {
						...
						return parseImports(input, start, end);
					}
				}
		...
		if (icssImportData.length) {
			for (const data of icssImportData) {
				const dep = new CssIcssImportDependency(
					data.url.slice(1, -1),
					data.range,
					data.imports
				);
				const { line: sl, column: sc } = locConverter.get(data.range[0]);
				const { line: el, column: ec } = locConverter.get(data.range[1]);
				dep.setLoc(sl, sc, el, ec);
				module.addDependency(dep);
			}
		}
