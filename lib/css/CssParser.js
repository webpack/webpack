...
/** @typedef {Map<string, { importValue: string, replaceRanges: [number, number][]}>} ImportMap */
/** @typedef {{url?: string, start: number, end?: number, keys: string[]}} IcssImportData */
...
const CSS_MODE_IN_ICSS_IMPORT = 5;
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {IcssImportData[]} */
		const icssImportsData = [];
		/** @type {ImportMap} */
		const icssImportMap = new Map();
		/** @type {undefined | IcssImportData} */
		let currIcssImport;
		/** @type {{name: string, value?: string}} */
		let currIcssImportDeclaration;
		...
		walkCssTokens(source, {
			...
			string: (input, start, end) => {
				switch (scope) {
					...
					case CSS_MODE_TOP_LEVEL: {
						if (currIcssImport) {
							const url = normalizeUrl(input.slice(start + 1, end - 1), true);
							currIcssImport.url = url;
						}
					}
				}
				return end;
			...
			semicolon: (input, start, end) => {
				switch (scope) {
					...
					case CSS_MODE_IN_ICSS_IMPORT: {
						if (currIcssImportDeclaration && currIcssImportDeclaration.value) {
							icssImportMap.set(currIcssImportDeclaration.name, {
								importValue: currIcssImportDeclaration.value,
								replaceRanges: []
							});
							currIcssImport.keys.push(currIcssImportDeclaration.name);
							currIcssImportDeclaration = undefined;
						} else if (
							currIcssImportDeclaration &&
							!currIcssImportDeclaration.value
						) {
							this._emitWarning(
								state,
								`Invalid :import ${currIcssImportDeclaration} declaration`,
								locConverter,
								currIcssImport.start,
								end
							);
						} else {
							this._emitWarning(
			...
			rightCurlyBracket: (input, start, end) => {
				switch (scope) {
					...
					case CSS_MODE_IN_ICSS_IMPORT: {
						scope = CSS_MODE_TOP_LEVEL;
						const dep = new ConstDependency("", [currIcssImport.start, end]);
						module.addPresentationalDependency(dep);
						icssImportsData.push(currIcssImport);
						currIcssImport = undefined;
					}
				}
				return end;
			},
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						const importData = icssImportMap.get(input.slice(start, end));
						if (importData) importData.replaceRanges.push([start, end - 1]);
						if (isLocalMode()) {
					...
					case CSS_MODE_IN_ICSS_IMPORT: {
						if (!currIcssImportDeclaration) {
							currIcssImportDeclaration = {
								name: input.slice(start, end)
							};
						} else {
							currIcssImportDeclaration.value = input.slice(start, end);
						}
					}
				}
				return end;
		...
		for (const data of icssImportsData) {
			const importMap = new Map();
			for (const key of data.keys) {
				importMap.set(key, icssImportMap.get(key));
			}
			console.log(importMap.entries());
			const dep = new CssIcssImportDependency(
				data.url,
				[data.start, data.end],
				importMap
			);
			const { line: sl, column: sc } = locConverter.get(data.start);
			const { line: el, column: ec } = locConverter.get(data.end);
			dep.setLoc(sl, sc, el, ec);
			module.addDependency(dep);
		}
