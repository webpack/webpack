...
const { CSS_MODULE_TYPE_AUTO } = require("../module/ModuleTypeConstants");
...
// WHY: skip options for a non-CSS-Modules parse. The selector prelude is never
// walked without modules, and the value and function-arg leaves reach nothing —
// the `Ident` visitor no-ops, the `Declaration` visitor returns early, and there
// is no ICSS. `url`, functions, strings, blocks and commas are kept, carrying
// the `url()` rewrites and image-set fences, and at-rule preludes too, since
// `@media` and `@import` are read. A CSS-Modules parse skips nothing: selectors
// are walked, and ICSS `:export { k: v }` captures each value's byte range from
...
// A `@value` whose whole value is one `.class` / `#id`, the only shape whose
// localizable ident is knowable without re-parsing the substitution.
const SIMPLE_CLASS_OR_ID_VALUE =
	/^([.#])(-?(?:[A-Za-z_]|[\u0080-\uFFFF])(?:[\w-]|[\u0080-\uFFFF])*)$/;
...
const parseValueAtRuleParams = (str) => {
	if (VALUE_IMPORT_FORM.test(str)) {
		...
		// Split on the first `from` keyword delimited by CSS whitespace (only
		// ` \t\n\f\r`, not JS `\s`, which also matches U+00A0 valid in a name),
		// so a path or name containing the substring (`./from-colors.css`) isn't it.
		const fromMatch = /^([\s\S]+?)[ \t\n\f\r]+from[ \t\n\f\r]+([\s\S]+)$/i.exec(
			str
		);
		const path = (fromMatch ? fromMatch[2] : "").trim().replace(/['"]/g, "");
		let content = (fromMatch ? fromMatch[1] : str).trim();
...
class CssParser extends Parser {
	 ...
	 * @param {import("../dependencies/css/CssIcssExportDependency").CssExportEntry[]} cssExportEntries all export entries — losers are demoted in place
	 * @returns {void}
	 */
	...
	parse(source, state) {
		 ...
		 * @param {import("../graph/Dependency").DependencyLocation} loc location for warnings
		 * @returns {void}
		 */
		...
		// Start offsets of the idents an at-rule's own prelude handler already
		// replaced: two deps over one range concatenate rather than the later winning.
		/** @type {Set<number>} */
		const handledPreludeIdents = new Set();
		// Only a prelude is walked by both a handler and the value visitors, so
		// nothing outside one pays for the bookkeeping.
		let trackingPreludeIdents = false;

		// Start offsets of `@value`-named dashed idents a `var()` handler already
		// scoped; only those are walked twice, so plain dashed idents cost nothing.
		/** @type {Set<number>} */
		const handledDashedValueIdents = new Set();
		...
		/** @type {import("../dependencies/css/CssIcssExportDependency").CssExportEntry[]} */
		 ...
		 * @param {string} name export name
		 * @param {import("../dependencies/css/CssIcssExportDependency").Value} value value or [localName, importName, request?]
		 * @param {Range=} range source range to replace, when present
		 * @param {boolean=} interpolate whether the value needs interpolation
		 * @param {import("../dependencies/css/CssIcssExportDependency").ExportMode=} exportMode export mode
		 * @param {import("../dependencies/css/CssIcssExportDependency").ExportType=} exportType export type
		 * @returns {void}
		 */
		const addCssExport = (
			sl,
			...
			if (trackingPreludeIdents && range) handledPreludeIdents.add(range[0]);
		 ...
		 * @param {import("./syntax-parser").CssPath} path walk path at the comment node
		 */
		...
		/**
		 * Emit a `CssIcssSymbolDependency` rewrite for an ident resolving to an `@value`-defined ICSS symbol (source-order semantics hold since the walker handles each `@value` before later references). A no-op for an ident an at-rule's own prelude handler already replaced.
		...
		const emitICSSSymbol = (name, start, end) => {
			if (handledPreludeIdents.has(start)) return end;
			if (trackingPreludeIdents) handledPreludeIdents.add(start);
			const def =
				/** @type {IcssDefinition} */
		...
		/**
		 * The `.class` / `#id` a locally defined `@value` stands for, when its whole value is exactly one of those. Undefined for an imported value or any richer selector, which stay a plain textual substitution.
		 * @param {string} name `@value` name used where a selector is expected
		 * @returns {["." | "#", string] | undefined} the selector's prefix and the ident to localize
		 */
		const localizableSelectorValue = (name) => {
			const def = icssDefinitions.get(name);
			if (!def || def.importName || typeof def.value !== "string") {
				return undefined;
			}
			const match = SIMPLE_CLASS_OR_ID_VALUE.exec(def.value.trim());
			return match
		...
		/**
		 * The dashed ident a locally defined `@value` stands for, without its `--` prefix, when its whole value is exactly one. Undefined for an imported value or any richer value, which stay a plain textual substitution.
		 * @param {string} name `@value` name used where a dashed ident is expected
		 * @returns {string | undefined} the name to localize, sans `--`
		 */
		const localizableDashedValue = (name) => {
			if (icssDefinitions.size === 0) return undefined;
			const def = icssDefinitions.get(name);
			if (!def || def.importName || typeof def.value !== "string") {
				return undefined;
			}
			const value = def.value.trim();
			return isDashedIdentifier(value) ? value.slice(2) : undefined;
		};
		...
		/**
		 * Emit the ICSS export declaring this module exports the given custom property, naming it explicitly rather than reading it back from the replaced range. A `@value` standing in for a dashed ident is scoped under the name it resolves to, not the name written at the use site.
		 * @param {string} identifier custom property name, sans `--`
		 * @param {number} identStart start of the replaced ident
		 * @param {number} identEnd end of the ident (exclusive)
		 */
		const emitNamedDashedIdentExport = (identifier, identStart, identEnd) => {
		...
		const emitDashedIdentExport = (identStart, identEnd) => {
			emitNamedDashedIdentExport(
				dashedIdentName(identStart, identEnd),
				identStart,
				identEnd
			);
		};
		...
		const processDashedIdentInVarFunction = (fn) => {
			...
			if (!rangeIsDashedIdentifier(input, identStart, identEnd)) {
				// `var(v)` where `@value v: --foo` names the same property `var(--foo)`
				// does, so it is scoped here rather than substituted verbatim.
				const dashedName = localizableDashedValue(A.value(identNode));
				if (dashedName !== undefined) {
					handledDashedValueIdents.add(identStart);
					emitNamedDashedIdentExport(dashedName, identStart, identEnd);
				}
				return;
			}
		...
		const walkSelectorList = (values, localMode, topLevel = true) => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				switch (A.type(v)) {
					...
					case NodeType.Ident: {
						// A bare `@value`-defined ident stands in for a selector; only worth the slice when definitions exist.
						if (icssDefinitions.size === 0) break;
						const ident = A.value(v);
						if (isDashedIdentifier(ident) || !icssDefinitions.has(ident)) {
							break;
						}
						const identStart = A.start(v);
						const identEnd = A.end(v);
						// A value naming one class / id is localized like the selector the
						// author would have written; anything else is substituted verbatim.
						const localizable =
							segmentMode === "local"
								? localizableSelectorValue(ident)
								: undefined;
						if (localizable === undefined) {
							emitICSSSymbol(ident, identStart, identEnd);
							break;
						}
						const [prefix, localName] = localizable;
						const { line: sl, column: sc } = locConverter.get(identStart);
						const { line: el, column: ec } = locConverter.get(identEnd);
						module.addPresentationalDependency(
							new ConstDependency(prefix, [identStart, identStart])
						);
						recordDeclaration(
							localName,
							prefix === "." ? "class" : "id",
						...
						if (prefix === ".") {
							currentRule.hasLocalAnchor = true;
							currentRule.localIdentifiers.push(localName);
						}
						pure.markLocal();
						break;
					}
		...
		const visitors = {
			...
			[NodeType.AtRule]: {
				...
				enter: (path) => {
					...
					// The dispatch below is the only place a prelude is rewritten ahead
					// of the value visitors, so the marks live exactly as long as it.
					if (handledPreludeIdents.size !== 0) handledPreludeIdents.clear();
					trackingPreludeIdents = true;
					switch (name) {
						case "@namespace": {
							this._emitWarning(
								state,
			...
			[NodeType.Ident](path) {
				const node = path.node;
				// WHY: a fast exit before slicing the ident value. Outside dashed-ident
				// scoping and ICSS context — any non-CSS-Modules stylesheet — a bare
				// ident carries no work, and idents are the most common node, so skipping
				// the per-ident `value` slice matters. With no `@value` or `:import`
				// definitions the ICSS probe can never hit, so it does not warrant the
				// slice either. The dashed probe reads two char codes instead of slicing,
				// and an escaped dash does not match, as in the string form.
				...
				if (!icssDefinitions.has(identValue)) return;
				// Already scoped by the enclosing `var()` handler.
				if (
					handledDashedValueIdents.size !== 0 &&
					handledDashedValueIdents.has(identStart)
				) {
					return;
				}
				// A `@value` resolving to a dashed ident sits where `--foo` would be
				// scoped, so it is localized rather than substituted verbatim.
				if (dashedActive && dashed.emit) {
					const dashedName = localizableDashedValue(identValue);
					if (dashedName !== undefined) {
						emitNamedDashedIdentExport(dashedName, identStart, identEnd);
						return;
					}
		...
		// `as` selects the top-level production of §5.3: a `style` attribute is a
		// block's contents, anything else a full stylesheet. `as` and `skip` are
		// per-parse, so only the per-source `locConverter` reaches `process`.
		new SourceProcessor()
