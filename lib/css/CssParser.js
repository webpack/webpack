...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		// Single-pass: `walkAstBlockContents` threads the `comment`
		// callback through every `eatWhitespaceAndComments` /
		// `parseAtRule` / `parseADeclaration` /
		// `parseAListOfComponentValues` call it makes, so each comment
		// in the source is pushed to `this.comments` exactly once, in
		// source order — no separate up-front tokenization needed.
		runAstWalker();
