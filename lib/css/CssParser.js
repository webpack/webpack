...
/** @typedef {import("./CssAst").Declaration} Declaration */
/** @typedef {import("./CssAst").FunctionNode} FunctionNode */
/** @typedef {import("./CssAst").Node} AstNode */
/** @typedef {import("./CssAst").QualifiedRule} QualifiedRule */
	...
	parse(source, state) {
		...
		const walkAstBlockContents = (items, topLevel) => {
			for (const item of items) {
				...
				// Declarations are left to the streaming walker until E4.
		...
		for (const t of walkCssTokens(source, 0)) {
			...
			switch (t.type) {
				...
				case "url": {
					const contentStart = t.contentStart;
					const contentEnd = t.contentEnd;
					if (!this.options.url) {
						break;
					}
