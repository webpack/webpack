...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const processDashedIdent = (input, start, end) => {
			// Use the tokenizer's ident probe directly here — the streaming
			// walker invokes us from both the `identifier:` and `function:`
			// callbacks, so at `start` the input may be either `--foo` or
			// `--foo(`. We want only the ident bounds; `parseAComponentValue`
			// would recursively consume the whole function body in the
			// second case.
			...
			// Fast-path byte-peek for `from`: the vast majority of dashed-
			// ident declarations have no `from` suffix, and even probing
			// the next component value costs a tokenizer step. If the next
			// non-whitespace byte isn't `f`/`F`, we're done.
			...
			const fromCV = parseAComponentValue(input, afterIdent, locConverter);
			if (
				!fromCV ||
				fromCV.value.type !== "ident" ||
				input.slice(fromCV.value.start, fromCV.value.end).toLowerCase() !==
					"from"
			) {
				return end;
			}
			// `from <source>` accepts either an ident (`global`) or a
			// quoted string (`"./file.css"`). `parseAComponentValue` picks
			// the right token kind in one call — the previous code had a
			// string probe + ident fallback.
			const sourceCV = parseAComponentValue(
				input,
				fromCV.value.end,
				locConverter
			);
			if (!sourceCV) return end;
			if (sourceCV.value.type === "string") {
				const text = input.slice(sourceCV.value.start, sourceCV.value.end);
			...
			if (
				sourceCV.value.type === "ident" &&
				input.slice(sourceCV.value.start, sourceCV.value.end) === "global"
			) {
				emitDashedIdentFromGlobal(identEnd, sourceCV.value.end);
			}
			return end;
		};
		...
		const processLocalDeclaration = (input, pos, end) => {
			...
			if (isLocalMode() && knownProperties.has(propertyName)) {
				...
				//
				// What stays here is the `var(--dashed)` side-effect
				// — `processDashedIdentInVarFunction` is the only
				// emitter for that and the streaming walker still
				// owns it pending E4d. The `walkFunctions` recursion
				// also still drives the `skipUntil = decl.end`
				// behaviour above (returning past the whole
				// declaration suppresses the streaming walker's
				// `case "identifier"` from emitting
				// `processDashedIdent` on the inner ident again).
				/**
				 * @param {AstNode[]} cvs component values
				 */
		...
		const walkAstBlockContents = (items, topLevel) => {
			for (const item of items) {
				if (item.type === "at-rule") {
					...
					// knownProperty.
					if (isModules && knownProperties.has(declPropertyName)) {
						const effectiveLocalMode = astModeData
							? astModeData === "local"
							: mode === "local";
				...
				case "identifier": {
					if (isModules) {
						...
						if (
							...
							skipUntil = processDashedIdent(input, start, end);
							break;
						}

						if (icssDefinitions.has(identifier)) {
							skipUntil = processICSSSymbol(identifier, start, end);
							break;
						}
