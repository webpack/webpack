...
class CssParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * @returns {void}
		 */
		const walkAstSelectorList = (values, localMode) => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				if (v.type === "function") {
					const fname = /** @type {FunctionNode} */ (v).name
						.replace(/\\/g, "")
						.toLowerCase();
					if (fname === "local" || fname === "global") {
						// Bare `local(…)` / `global(…)` — scope to args only.
						walkAstSelectorList(
		...
		walkCssTokens(source, 0, {
			...
			leftCurlyBracket: (input, start, end) => {
				...
				if (pureMode) {
					...
					pureBlockStack.push({
						...
						// This rule fully pure (no impure comma-segment) OR any ancestor pure; matches PCSL's `[isPureSelectorSymbol]`.
						ancestorHadLocal:
							parentEffectivePure() ||
			...
			rightCurlyBracket: (input, start, end) => {
				...
				if (pureMode) {
					...
					if (frame) {
						// PCSL throws on impure rules whose body has non-rule content; rules with only nested rules are skipped (children carry the check).
						if (
							!pureNoCheck &&
							!frame.ignored &&
							!frame.skipOwn &&
						...
						// Propagate "has direct declaration" through at-rule frames so a parent of only e.g. `@media { decl }` still counts as having declarations.
			...
			atKeyword: (input, start, end) => {
				...
				if (pureMode) {
					...
					// Match PCSL's `isPureCheckDisabled`: any non-comment top-level node seals the leading-comments window.
					if (wasTopLevel) seenTopLevelRule = true;
				}
			...
			semicolon: (input, start, end) => {
				...
				if (pureMode) {
					if (scope === CSS_MODE_IN_BLOCK) {
						...
						// Top-level `;` ends a statement; advance the prelude pointer so a later impure rule's selector excludes the preceding text.
						currentRulePreludeStart = end;
					}
					inAtRulePrelude = false;
				}
			...
			leftSquareBracket: (_input, _start, end) =>
			...
			comma: (input, start, end) => {
				if (isModules && balanced.length === 0) {
					// Reset mode tracking at each comma so segments like `:global .class, :local .class-other` are handled independently.
					modeData = undefined;
