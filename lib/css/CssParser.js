...
import { CSS_MODULE_TYPE_AUTO } from "../ModuleTypeConstants.js";
...
/** @typedef {import("./syntax.js").AtRule} AtRule */
/** @typedef {import("./syntax.js").Declaration} Declaration */
/** @typedef {import("./syntax.js").FunctionNode} FunctionNode */
/** @typedef {import("./syntax.js").Node} AstNode */
/** @typedef {import("./syntax.js").QualifiedRule} QualifiedRule */
/** @typedef {import("./syntax.js").Rule} Rule */
/** @typedef {import("./syntax.js").SimpleBlock} SimpleBlock */
/** @typedef {import("./syntax.js").Token} Token */
/** @typedef {import("./syntax.js").UrlToken} UrlToken */
/** @typedef {import("./syntax.js").HashToken} HashToken */
/** @typedef {import("./syntax.js").VisitorMap} VisitorMap */
/** @typedef {import("../../declarations/WebpackOptions.js").CssAutoOrModuleParserOptions} CssAutoOrModuleParserOptions */
...
const normalizeUrl = (str, isString) => {
	...
	if (isString && str.includes("\\")) {
		str = str.replaceAll(STRING_MULTILINE, "");
	}
	...
	if (
		...
		str = str.replaceAll(TRIM_WHITE_SPACES, "");
	}

	// Unescape
	if (str.includes("\\")) {
		str = str.replaceAll(UNESCAPE, (match) => {
			if (match.length > 2) {
...
const parseValueAtRuleParams = (str) => {
	if (VALUE_IMPORT_FORM.test(str)) {
		str = str.replaceAll(CSS_COMMENT, " ").trim().replace(/;$/, "");
	...
	if (idx !== -1) {
		localName = str.slice(0, idx).replaceAll(CSS_COMMENT, "").trim();
		value = str.slice(idx + 1);
	} else {
		const mask = str.replaceAll(CSS_COMMENT, (m) => " ".repeat(m.length));
	...
	if (
		value.length > 0 &&
		!ONLY_WHITESPACE.test(value.replaceAll(CSS_COMMENT, ""))
	) {
		value = value.trim();
	}
...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {import("../dependencies/CssIcssExportDependency.js").CssExportEntry[]} */
		 ...
		 * @param {import("../dependencies/CssIcssExportDependency.js").Value} value value or [localName, importName, request?]
		 * @param {Range=} range source range to replace, when present
		 * @param {boolean=} interpolate whether the value needs interpolation
		 * @param {import("../dependencies/CssIcssExportDependency.js").ExportMode=} exportMode export mode
		 * @param {import("../dependencies/CssIcssExportDependency.js").ExportType=} exportType export type
		 * @returns {void}
		 */
		const addCssExport = (
			sl,
		 ...
		 * @param {import("./syntax.js").CssPath} path walk path at the comment node
		 */
...
export { CssParser as "module.exports" };
