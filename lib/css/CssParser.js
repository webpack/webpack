...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const pure = {
			...
			/** Whether the current comma-separated selector has carried a local class / id. */
			selectorHasLocal: false,
			/** Whether any comma-separated selector in the current rule's prelude was impure. */
			ruleHasImpureSelector: false,
			...
			/**
			 * Mark the current comma-separated selector as having a local class / id (no-op when pure-mode is off).
			 */
			markLocalSelector() {
				if (this.enabled) this.selectorHasLocal = true;
			},
			/**
			 * Mark the just-finished comma selector (or whole prelude at `{`) impure if it lacks a local and no ancestor compensates (no-op when pure-mode is off).
			 */
			finalizeSelector() {
				if (!this.enabled) return;
				if (!this.selectorHasLocal && !this.ancestorHadLocal()) {
					this.ruleHasImpureSelector = true;
				}
				this.selectorHasLocal = false;
			},
			/**
			...
			enterBlock(opts) {
				...
				const reportable =
					...
					!skipOwn &&
					isRulePrelude &&
					this.ruleHasImpureSelector;
				if (reportable) {
					let leaf = treatAsLeaf || !block;
					if (!leaf && block) {
						const { hasDirectDecl, hasNestedBlock } = scanRuleBody(block);
				...
				this.stack.push({
					ancestorHadLocal:
						this.ancestorHadLocal() ||
						(isRulePrelude && !this.ruleHasImpureSelector),
					skipChildren: ownSkip || skipOwn
				});
				this.ignorePending = false;
				this.selectorHasLocal = false;
				this.ruleHasImpureSelector = false;
			},
			/**
			 ...
			 * @param {string} value comment body (without the surrounding `/*` and `*` `/`)
			 */
			applyComment(value) {
		 ...
		 */
		const scanRuleBody = (block) => {
			let hasDirectDecl = false;
			let hasNestedBlock = false;
			for (const child of block.value) {
				if (child.type === "Declaration") {
					hasDirectDecl = true;
				} else if (child.type === "QualifiedRule") {
					hasNestedBlock = true;
					...
					if (
						...
						scanRuleBody(at.block).hasDirectDecl
					) {
						hasDirectDecl = true;
					}
				}
		...
		/**
		 * Advance `commentCursor` past every comment closing at/before `until`, applying its pure-mode side effect: `pure.ignorePending` (next rule) or the file-level `pure.noCheck` (only before the first top-level rule).
		...
		const processLocalAtRule = (atRule, options) => {
			...
			for (const cv of atRule.prelude) {
				...
				if (cv.type === "Function") {
					...
					if (!found && type) {
						found = true;
						if (type === 1) pure.markLocalSelector();
		...
		const visitors = {
			AtRule: {
				...
				enter: (node, parent) => {
					...
					if (pure.enabled) advanceCommentCursor(at.range[0]);
					...
					if (
						...
						for (const cv of at.prelude) {
							if (cv.type === "Whitespace") continue;
							if (cv.type === "String") {
								if (acceptString) pure.selectorHasLocal = true;
								break;
							}
							if (cv.type === "Ident") {
							...
							if (cv.type === "Function") {
								const fname = /** @type {FunctionNode} */ (cv).name
									.replace(/\\/g, "")
									.toLowerCase();
								if (fname === "local") pure.selectorHasLocal = true;
								break;
							}
						}
					}
			...
			QualifiedRule: {
				...
				enter: (node, parent, ctx) => {
					...
					const topLevel = parent === null;
					if (pure.enabled) advanceCommentCursor(rule.range[0]);
