...
// Strip CSS escape backslashes from an ident / function name. Escapes are rare, so fast-path the common no-backslash case (avoids the regex engine per value-token visit).
const ESCAPE_BACKSLASH = /\\/g;
/**
 * @param {string} name ident / function name
 * @returns {string} name with CSS escape backslashes removed
 */
const removeCssEscapes = (name) =>
	name.includes("\\") ? name.replace(ESCAPE_BACKSLASH, "") : name;
	...
	for (const cv of prelude) {
		...
		if (!urlNode) {
			...
			if (
				...
				equalsLowerCase(
					removeCssEscapes(/** @type {FunctionNode} */ (cv).name),
					"url"
				)
			) {
				urlNode = cv;
		...
		if (!layerNode && !supportsNode) {
			if (cv.type === NodeType.Ident) {
				if (
					equalsLowerCase(
						removeCssEscapes(/** @type {Token} */ (cv).value),
						"layer"
					)
				) {
					layerNode = cv;
		...
		const processLocalAtRule = (atRule, options) => {
			...
			for (const cv of atRule.prelude) {
				...
				if (cv.type === NodeType.Function) {
					...
					const fname = removeCssEscapes(fn.name).toLowerCase();
					const type =
				...
				switch (v.type) {
					...
					case NodeType.Colon: {
						...
						if (next.type === NodeType.Ident) {
							...
							const rawName = removeCssEscapes(fn.name);
					...
					case NodeType.Delim: {
						...
						if (segmentMode === "local") {
							...
							const name = unescapeIdentifier(
								source.slice(next.start, next.end)
							);
							const {
								start: { line: sl, column: sc },
								end: { line: el, column: ec }
		...
		const visitors = {
			...
			[NodeType.Function]: {
				enter: (node) => {
					...
					const fnameRaw = removeCssEscapes(fn.name);
