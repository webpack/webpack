...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		/** @type {{identifierNames: string[], range: Range, identifierRanges: Range[]}} */
		let lastClassDefinition = {
			identifierNames: [],
			range: [0, 0],
			identifierRanges: []
		};
		...
		walkCssTokens(source, {
			...
			class: (input, start, end) => {
				if (isLocalMode()) {
					...
					// handle only top level classnames and not nested css
					switch (scope) {
						case CSS_MODE_TOP_LEVEL: {
							lastClassDefinition.identifierNames.push(name);
							lastClassDefinition.range = [start, 0];
							lastClassDefinition.identifierRanges.push([start + 1, end]);
							break;
						}
					}
				}
