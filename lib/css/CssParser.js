...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const parseModulesValues = (input, pos) => {
			const start = pos;
			pos = walkCssTokens.eatWhitespaceAndComments(input, pos);
			if (pos === input.length) return pos;
			let key;
			[pos, key] = eatText(input, pos, eatUntil(":{};/"));
			if (pos === input.length) return pos;
			if (input.charCodeAt(pos) !== CC_COLON) {
				this._emitWarning(
					state,
					`Unexpected '${input[pos]}' at ${pos} during parsing of key in '@value' (expected ':')`,
					locConverter,
					start,
					pos
				);
			...
			if (pos === input.length) return pos;
			if (input.charCodeAt(pos) !== CC_SEMICOLON) {
				this._emitWarning(
					state,
					`Unexpected '${input[pos]}' at ${pos} during parsing of value in '@value' (expected ';')`,
					locConverter,
					start,
					pos
				);
			...
			modulesValues.push({ key, value });
			const dep = new CssExportDependency(key, value);
			const { line: sl, column: sc } = locConverter.get(start);
			const { line: el, column: ec } = locConverter.get(pos);
			dep.setLoc(sl, sc, el, ec);
			module.addDependency(dep);
		...
		walkCssTokens(source, {
			...
			identifier: (input, start, end) => {
				switch (scope) {
					case CSS_MODE_IN_BLOCK: {
						...
						if (item) {
							const dep = new ConstDependency(item.value, [start, end]);
							module.addPresentationalDependency(dep);
						}
						if (isLocalMode()) {
			...
			class: (input, start, end) => {
				const item = modulesValues.find(
					item => input.slice(start + 1, end) === item.key
				);
				if (item) {
					const dep = new ConstDependency(item.value, [start + 1, end]);
					module.addPresentationalDependency(dep);
				}
				if (isLocalMode()) {
