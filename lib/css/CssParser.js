...
class CssParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * @param {import("./syntax").CssAst} _api AST accessor (unused — the module-level `A` is the same object)
		 * @param {AstNode} node comment node
		 */
		const commentVisitor = (_api, node) => {
		...
		const visitors = {
			...
			[NodeType.Declaration]: (_api, node) => {
			...
			[NodeType.Ident](_api, node, parent) {
