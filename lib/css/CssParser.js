...
class CssParser extends Parser {
	...
	parse(source, state) {
		...
		const module = /** @type {CssModule} */ (state.module);
		...
		walkCssTokens(source, {
			...
			atKeyword: (input, start, end) => {
				...
				if (name === "@namespace") {
					...
					if (module.assertions && module.assertions.type === "css") {
						...
						this._emitWarning(
							state,
							"Css module scripts doesn't support @import now",
							locConverter,
							start,
							end
						);
			...
			semicolon: (input, start, end) => {
				switch (scope) {
					...
					case CSS_MODE_AT_IMPORT_INVALID: {
						if (importData) {
							const dep = new ConstDependency("", [importData.start, end]);
							module.addPresentationalDependency(dep);
						}
						scope = CSS_MODE_TOP_LEVEL;
						break;
		...
		module.buildMeta.exportsType =
			module.assertions || !this.namedExports ? "default" : "namespace";
