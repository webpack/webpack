...
// The `@font-face` src extensions worth a preload `type`; the emission guesses
// `as="font"`. Which formats to hint is webpack's call, but what each one *is*
// comes from `mime-db` rather than a second copy of the mapping.
...
const fontMimeType = (request) => {
	...
	// `lookup` reads the extension off a path, and the request's may carry a
	// query or hash — so ask about a bare name carrying the same one.
	return getMimeTypes().lookup(`font.${extension}`);
};
