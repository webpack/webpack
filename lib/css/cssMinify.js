...
const cssMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	// - a shorthand gathering a whole family (`border`, `font`, `outline`,
	//   `list-style`, `text-decoration`, `transition`, `background`, `flex`,
	//   `columns`) resets longhands `computed` does not name — `border` clears
	//   `border-image`, `font` clears `font-size-adjust` — so the merge would
	//   drop a declaration nothing in the family wrote;
	// - a shorthand materially newer than its longhands (two-value `overflow`,
	//   `place-items`/`-content`/`-self`) would lose both declarations, not one,
	//   on a target reading only the longhands. `output.environment` states this
	//   for `inset` alone;
	// - a pair two shorthands both claim, which cannot be right for both:
	//   `mdn-data` gives `corner-block-start-shape` and `corner-inline-start-shape`
	//   the same two longhands and Chromium disagrees with one of them.
