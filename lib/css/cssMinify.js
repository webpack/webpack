...
const cssMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	// - converting a color function whose result rounds to a different byte than
	//   the engine's. Spec-exact rounding differs from headless Chromium on 141 of
	//   14520 `hsl()` samples and 420 of 14520 `hwb()` ones — floating-point
	//   association, not a spec disagreement, but enough that esbuild and
	//   lightningcss disagree with each other. `lab()` / `lch()` are further out:
	//   the computed value stays in that space, so sRGB is a gamut change.
