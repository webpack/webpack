...
const cssMinify = async (input, sourceMap, minimizerOptions = {}) => {
	...
	// - Two color conversions are declined rather than guessed. Every polar and Lab
	//   function converts to hex, except a channel landing near a `.5` boundary,
	//   where implementations round opposite ways (which is why esbuild and
	//   lightningcss emit different bytes for `hwb(194 0% 0%)`), and a Lab-family
	//   color outside the sRGB gamut, which hex would clip rather than respell.
