...
const cssMinify = async (input, sourceMap, minimizerOptions = {}) => {
	...
	// make, not what it declines.

	// WHY: each transform webpack declines is on in other minifiers, and each costs
	// something measured. Dropping a declaration a later one overrides loses the
	// fallback pair an engine that cannot read the newer spelling depends on.
	// Merging rules a third stands between reorders the cascade — csso loses some
	// 12,900 Tailwind classes to it — so only adjacent rules sharing a block join.
	// Two color conversions are declined rather than guessed: a channel near a `.5`
	// boundary, where esbuild and lightningcss disagree on `hwb(194 0% 0%)`, and a
	// Lab-family color outside sRGB, which hex would clip to another color.

	// WHY: `tooling/generate-css-data.js` decides which longhand families merge, and
	// excludes three kinds for good. A shorthand gathering a whole family resets
	// longhands `computed` never names — `border` clears `border-image` — so the
	// merge would drop a declaration nothing in the family wrote; the ones checked
	// against a browser do merge. A shorthand materially newer than its longhands,
	// such as `place-items`, would lose both declarations on a target reading only
	// the longhands. A pair two shorthands both claim cannot be right for both.
