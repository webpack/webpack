...
class CssLoadingRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		// a platform known to have no DOM (`target: "node"`): the stylesheet is read
		// from disk, and unlike the neutral platform the link code is statically dead
		const withoutDocument = !environment.document && !isNeutralPlatform;
		const withPrefetch =
		...
		const readCssFromDisk = !environment.document
