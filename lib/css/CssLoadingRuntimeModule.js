...
/** @import Chunk, { ChunkId } from "../graph/Chunk" */
/** @import ChunkGraph from "../graph/ChunkGraph" */
/** @import { ReadOnlyRuntimeRequirements } from "../module/Module" */
...
class CssLoadingRuntimeModule extends RuntimeModule {
	...
	/**
	 * The initial chunks carrying a stylesheet, and the matcher for the ones the
	 * handler can still be asked to load.
	 * @private
	 * @returns {{ initialChunkIds: Set<ChunkId>, hasCssMatcher: ReturnType<typeof compileBooleanMatcher> }} loadable chunks
	 */
	_getLoadableChunks() {
		...
		return { initialChunkIds, hasCssMatcher };
	}
	...
	getInstalledChunkHandlers() {
		...
		const chunk = /** @type {Chunk} */ (this.chunk);
		const chunkGraph = /** @type {ChunkGraph} */ (this.chunkGraph);
		const runtimeRequirements = this._runtimeRequirements;
		const withLoading =
			runtimeRequirements.has(RuntimeGlobals.ensureChunkHandlers) &&
			this._getLoadableChunks().hasCssMatcher !== false;
		// Nothing at all is emitted without either, so nothing is installed either.
		if (
			!withLoading &&
		...
		const withHint =
			compilation.outputOptions.environment.document ||
			compilation.runtimeTemplate.isNeutralPlatform();
		/** @type {[string, string][]} */
		const handlers = [];
		if (withLoading) handlers.push([RuntimeGlobals.ensureChunkHandlers, "css"]);
		if (
			withHint &&
			runtimeRequirements.has(RuntimeGlobals.prefetchChunkHandlers) &&
			chunk.hasChildByOrder(chunkGraph, "prefetch", true, chunkHasCss)
		) {
			handlers.push([RuntimeGlobals.prefetchChunkHandlers, "s"]);
		}
		if (
			withHint &&
			runtimeRequirements.has(RuntimeGlobals.preloadChunkHandlers) &&
			(chunk.hasChildByOrder(chunkGraph, "preload", true, chunkHasCss) ||
				chunk.hasChildByOrder(chunkGraph, "cssPreload", true, chunkHasCss))
		) {
			handlers.push([RuntimeGlobals.preloadChunkHandlers, "s"]);
		}
		return handlers;
	}
	...
	generate() {
		...
		const { initialChunkIds, hasCssMatcher } = this._getLoadableChunks();
		...
		// only a node platform can read the stylesheet from disk; other document-less
		// platforms (`target: "webworker"`) have no `fs`, so they keep the link code
		const withoutDocument =
			!environment.document && compilation.compiler.platform.node === true;
		// Only a build that renders on a server meets a stylesheet it did not
		// create, so every other build keeps the shorter adoption check.
		const withServerRenderedLinks = this._runtimeRequirements.has(
			RuntimeGlobals.hasServerRenderedStylesheets
		);
		const withPrefetch =
		...
		// Whether an existing link still has rules on the way, and what its timeout
		// then means: only a webpack-created link is an error, never an adopted one.
		const adoptedLinkIsPending = withServerRenderedLinks
			? 'fetching || (!link.sheet && link.href.indexOf(location.origin + "/") == 0)'
			: "link.getAttribute(loadingAttribute)";
		const timeoutEventType = withServerRenderedLinks
			? "fetching ? 'timeout' : 'load'"
			: "'timeout'";
		// no DOM: read the emitted CSS via dynamic import('fs') (works on every node),
		// collect it; never reject on a missing file
		const readCssFromDisk =
			withoutDocument || isNeutralPlatform
		...
		return Template.asString([
			...
			(withLoading && !withoutDocument) || withHmr
				? Template.asString([
						...
						`${cst} loadStylesheet = ${runtimeTemplate.basicFunction(
							...
							[
								...
								...(withServerRenderedLinks
									? [`${cst} fetching = link.getAttribute(loadingAttribute);`]
									: []),
								// An adopted stylesheet without a `sheet` has not applied yet, so a streamed
								// server render no longer resolves before its rules are in.
								`if(${adoptedLinkIsPending}) {`,
								Template.indent([
									// An adopted link that already failed replays no event, so its timeout
									// resolves as a load: webpack did not create it and must not remove it.
									`timeout = setTimeout(onLinkComplete.bind(null, undefined, { type: ${timeoutEventType}, target: link }), ${loadTimeout});`,
									"link.onerror = onLinkComplete.bind(null, link.onerror);",
									"link.onload = onLinkComplete.bind(null, link.onload);"
								]),
								"} else onLinkComplete(undefined, { type: 'load', target: link });", // a cross-origin sheet never exposes `sheet`, so it keeps the render-blocking assumption
											...
											withoutDocument
												...
												: Template.asString([
														...
														isNeutralPlatform
															? Template.asString([
																	"} else {",
																	Template.indent([readCssFromDisk]),
																	"}"
																])
															: ""
													])
						...
						Template.indent([
							...
							dedupePrefetch
								? Template.asString([
										...
										Template.indent([
											`${cst} l = links[i];`,
											'if(l.href === link.href && (l.rel === "prefetch" || l.rel === "preload")) return;'
										]),
										"}"
									])
								: "",
			...
			withHmr
				? Template.asString([
						...
						`${
							...
							[
								isNeutralPlatform || withoutDocument
									? Template.asString([
											"if (typeof document === 'undefined') {",
											Template.indent([
