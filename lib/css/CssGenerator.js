...
class CssGenerator extends Generator {
	...
	/**
	 * Build the URL expression for an `exportType: "url"` module: the public
	 * path joined with the filename of the chunk that holds the bundled CSS.
	 * @param {NormalModule} module the module
	 * @param {GenerateContext} generateContext the generate context
	 * @returns {Source} the URL expression source
	 */
	_generateUrlExport(module, generateContext) {
		const { chunkGraph, runtimeRequirements } = generateContext;
		...
		runtimeRequirements.add(RuntimeGlobals.getChunkCssFilename);

		return new RawSource(
			`${RuntimeGlobals.publicPath} + ${RuntimeGlobals.getChunkCssFilename}(${JSON.stringify(chunk.id)})`
		);
	}
			...
			case JAVASCRIPT_TYPE: {
				...
				const generateExportCode = () => {
					...
					const generateJSDefaultExport = () => {
						switch (exportType) {
							case "url": {
								return this._generateUrlExport(module, generateContext);
							}
							case "text": {
					...
					if (exportType === "url" || (!isCssModule && !needNsObj)) {
						return new ConcatSource(
	...
	getTypes(module) {
		const exportType = /** @type {CssModule} */ (module).exportType || "link";
		if (exportType === "style" || exportType === "url") {
		...
		for (const connection of connections) {
			...
			// The bundled CSS reached via `new URL()` is referenced only to be
			// emitted as a standalone file — its origin (the `url` module) is
			// JS, but that must not pull a JS output channel into this module.
			if (connection.dependency instanceof CssUrlEntryDependency) {
				continue;
			}
