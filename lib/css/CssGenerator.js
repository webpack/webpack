...
const ESMImportSideEffectDependency = require("../dependencies/esm/ESMImportSideEffectDependency");
const ModuleFilenameHelpers = require("../devtool/ModuleFilenameHelpers");
const { UsageState } = require("../graph/ExportsInfo");
const LoaderImportDependency = require("../loaders/LoaderImportDependency");
...
class CssGenerator extends Generator {
	...
	generateError(error, module, generateContext) {
		switch (generateContext.type) {
			...
			case CSS_TYPE: {
				const message = Generator.buildErrorMessage(
					error,
					generateContext.runtimeTemplate.requestShortener
				);
	...
	getTypes(module) {
		...
		for (const connection of connections) {
			...
			if (connection.dependency instanceof ESMImportSideEffectDependency) {
				continue;
			}
	...
	_cssToJsLiteral(cssSource, devtool, generateContext) {
		...
		// A `renderEmbeddedSource` tap may return a mapped source whatever `devtool`
		// says, so wanting no map is checked here too: this data URI would ship the
		// sources inside the bundle. `hidden` answers the same.
	...
	_generateCssText(module, generateContext, asJsLiteral) {
		...
		// Only text bound for a JS literal. Whether a `CSS_TEXT_TYPE` inline
		// `<style>` is CSS at all turns on the element's `type`, a call the HTML
		// layer makes. After the `@charset` prefix, so a tap sees a whole sheet.
