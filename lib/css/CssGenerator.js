...
class CssGenerator extends Generator {
	...
	_cssSourceToJsStringLiteral(cssSource, module, generateContext) {
		...
		if (map) {
			...
			// Honour `nosources-*` devtools the same way SourceMapDevToolPlugin
			// does for external maps — strip `sourcesContent` from the inline
			// data URI so it doesn't leak the original source bodies.
			const inlineMap =
