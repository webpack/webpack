...
/** @typedef {import("./CssModule").CssBuildInfo} CssBuildInfo */
/** @typedef {import("./CssModule").CssBuildMeta} CssBuildMeta */
...
class CssGenerator extends Generator {
	...
	_getEffectiveCharset(module, moduleGraph, visited = new WeakSet()) {
		...
		const own =
			module.buildInfo &&
			/** @type {CssBuildInfo} */ (module.buildInfo).charset;
	...
	generate(module, generateContext) {
		...
		switch (generateContext.type) {
			case JAVASCRIPT_TYPE: {
				...
				const isCssModule = /** @type {CssBuildMeta} */ (module.buildMeta)
					.isCssModule;
	...
	getSize(module, type) {
		switch (type) {
			case JAVASCRIPT_TYPE: {
				const cssData = /** @type {CssBuildInfo} */ (module.buildInfo).cssData;
				if (!cssData) {
					return 42;
				}
				if (cssData.exports.size === 0) {
					if (/** @type {CssBuildMeta} */ (module.buildMeta).isCssModule) {
						return 42;
					}
					return 0;
				}
