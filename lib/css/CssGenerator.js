...
class CssGenerator extends Generator {
	...
	_cssToJsLiteral(cssSource, devtool, generateContext) {
		...
		if (!content.includes(hashPrefix)) {
		...
		let last = 0;
		let spliced = false;
		let idx = content.indexOf(hashPrefix);
		while (idx !== -1) {
			let digitEnd = idx + hashPrefixLen;
			while (digitEnd < contentLen) {
			...
			if (
				...
				digitEnd + 1 < contentLen &&
				content.charCodeAt(digitEnd) === 95 &&
				content.charCodeAt(digitEnd + 1) === 95
			) {
				const length = Number.parseInt(
					content.slice(idx + hashPrefixLen, digitEnd),
					10
				);
				result.add(JSON.stringify(content.slice(last, idx)));
				result.add(
					length === 0
				...
				spliced = true;
				last = digitEnd + 2;
				idx = content.indexOf(hashPrefix, last);
			} else {
				idx = content.indexOf(hashPrefix, idx + hashPrefixLen);
			}
		}
