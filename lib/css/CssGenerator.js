			...
			case JAVASCRIPT_TYPE: {
				...
				const generateExportCode = () => {
					...
					const generateCssText = () => {
						...
						// Expose the merged CSS text as a side-channel on the
						// code generation data for consumers that want the raw
						// processed CSS (e.g. HtmlModulesPlugin's inline
						// `<style>` injection) rather than the JS string
						// literal wrapper. We only emit it when an HTML
						// inline-style consumer is actually attached to this
						// module — checked by dependency category to avoid a
						// cross-import from the html package.
						if (exportType === "text" && generateContext.getData) {
							let hasHtmlInlineStyleConsumer = false;
							for (const conn of generateContext.moduleGraph.getIncomingConnections(
								module
							)) {
								const dep = conn.dependency;
								if (dep && dep.category === "html-style") {
									hasHtmlInlineStyleConsumer = true;
									break;
								}
							}
							if (hasHtmlInlineStyleConsumer) {
								const charsetPrefix =
									effectiveCharset !== undefined
										? `@charset "${effectiveCharset}";\n`
										: "";
								/** @type {Source} */
								const textSource = cssSource
									? charsetPrefix
										? new ConcatSource(charsetPrefix, cssSource)
										: cssSource
									: new RawSource(charsetPrefix);
								generateContext.getData().set("css-text", textSource);
							}
						}
