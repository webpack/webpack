...
class CssGenerator extends Generator {
				...
				const generateContentCode = () => {
					switch (exportType) {
						case "style": {
							...
							if (!rendered) return "";
							const cssSource = this._renderCssInJavascript(
								rendered,
								module,
								generateContext
							);
	...
	updateHash(hash, { module, runtimeTemplate }) {
		...
		if (runtimeTemplate && exportType !== "link") {
			// A `renderCssInJavascript` tap runs only here, and what it varies on has to
			// reach the codegen cache key, or a changed option replays the output
			// from before it.
			CssModulesPlugin.getCompilationHooks(
				runtimeTemplate.compilation
			).cssInJavascriptHash.call(module, hash);
	...
	/**
	 * Offer CSS bound for a JS string literal to `renderCssInJavascript`. No `.css`
	 ...
	 */
	_renderCssInJavascript(cssSource, module, generateContext) {
		const { compilation } = generateContext.runtimeTemplate;
		return CssModulesPlugin.getCompilationHooks(
			compilation
		).renderCssInJavascript.call(cssSource, module);
	}
	...
	_generateCssText(module, generateContext, asJsLiteral) {
		...
		if (result && asJsLiteral) {
			result = this._renderCssInJavascript(result, module, generateContext);
		}
