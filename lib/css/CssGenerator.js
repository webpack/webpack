...
class CssGenerator extends Generator {
	...
	getConcatenationBailoutReason(module, context) {
		...
		// TODO: support style concatenation
		// Concatenated inner modules are removed from Compilation.modules, so they
		// may not have a moduleId assigned. The "style" exportType relies on moduleId
		// for HMR — hmrDownloadUpdateHandlers receives removedModules as moduleIds
		// to clean up <style> tags, but inner modules without moduleId cannot be tracked.
		// Also, when multiple style modules are concatenated into one ConcatenatedModule,
		// their CSS should ideally be merged into a single <style> tag to reduce DOM nodes,
		// but this coupling between independent modules needs careful design.
		if (exportType === "style") {
			return "CSS module with exportType style cannot be concatenated";
		}
