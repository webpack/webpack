...
class CssGenerator extends Generator {
				...
				const generateContentCode = () => {
					switch (exportType) {
						case "style": {
							...
							if (!rendered) return "";
							const cssSource = this._renderEmbeddedCss(
								rendered,
								module,
								generateContext
							);
	...
	updateHash(hash, { module, runtimeTemplate }) {
		...
		if (runtimeTemplate && exportType !== "link") {
			// A `renderEmbeddedCss` tap runs only here, and what it varies on has to
			...
			).embeddedCssHash.call(module, hash);
	...
	/**
	 * Offer CSS bound for a JS string literal to `renderEmbeddedCss`. No `.css`
	...
	_generateCssText(module, generateContext, asJsLiteral) {
		...
		if (result && asJsLiteral) {
			result = this._renderEmbeddedCss(result, module, generateContext);
		}
