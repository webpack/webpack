...
/** @typedef {import("../DependenciesBlock").AsyncDependenciesBlock} AsyncDependenciesBlock */
/** @typedef {import("../dependencies/CssUrlEntryDependency")} CssUrlEntryDependency */
	...
	generate(module, generateContext) {
		...
		const generateJSDefaultExport = () => {
			switch (exportType) {
				...
				case "url": {
					const { moduleGraph, chunkGraph } = generateContext;
					const outgoingConnections = [
						...moduleGraph.getOutgoingConnections(module)
					];
					if (outgoingConnections.length !== 1) {
						throw new WebpackError(
							`Expected 1 outgoing connection for CSS module ${module.identifier()}, but got ${outgoingConnections.length}`
						);
					}
					const { dependency } = outgoingConnections[0];
					const block =
						/** @type {import("../DependenciesBlock").AsyncDependenciesBlock} */ (
							moduleGraph.getParentBlock(
								/** @type {CssUrlEntryDependency} */ (dependency)
							)
						);
					...
					generateContext.runtimeRequirements.add(
						RuntimeGlobals.hasCssModulesInUrl
					);
					generateContext.runtimeRequirements.add(RuntimeGlobals.publicPath);
					generateContext.runtimeRequirements.add(
	...
	getTypes(module) {
		...
		for (const connection of connections) {
			...
			if (connection.originModule.type.split("/")[0] !== CSS_TYPE) {
				...
				const originExportType = /** @type {CssModule} */ (
