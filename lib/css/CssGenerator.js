...
class CssGenerator extends Generator {
			...
			case JAVASCRIPT_TYPE: {
				const generateModuleCode = () => {
					switch (exportType) {
						case "style": {
							/**
							 * Generate style list in style-loader format: [[id, cssText, media, sourceMap, supports, layer]]
							 * @returns {string} style list code
							 */
							const generateStyleList = () => {
								const moduleCode = this._generateModuleCode(
									module,
									generateContext
								);
								...
								styleItems.push(
									`[${JSON.stringify(moduleId || "")}, ${JSON.stringify(moduleCode)}]`
								);
							...
							const styleList = generateStyleList();
							const constOrVar = generateContext.runtimeTemplate.renderConst();
							return `(${generateContext.runtimeTemplate.basicFunction("", [
								`${this._generateImportCode(module, generateContext, false)
									.map((part) => part.expr)
									.join(", ")}`,
								`${constOrVar} list = ${styleList};`,
								`${constOrVar} api = ${RuntimeGlobals.cssInjectStyle}(list, {});`,
								"if (module.hot && module.hot.data) {",
								Template.indent([
									"module.hot.data.__webpack_style_api__ = api;"
								]),
								"}",
								"api.update(list);"
							])})()`;
				...
				const source = new ConcatSource();
				for (const part of [generateModuleCode(), generateExportCode()]) {
					if (part) {
						source.add(part);
						source.add("\n");
					}
	...
	getTypes(module) {
		...
		if (
			this._exportsOnly ||
			/** @type {boolean} */ (
				exportType && !["link", "style"].includes(exportType)
			)
		) {
