...
class CssGenerator extends Generator {
	...
	generate(module, generateContext) {
		...
		const generateJSDefaultExport = () => {
			switch (exportType) {
				...
				case "url": {
					const chunkGraph = generateContext.chunkGraph;
					...
					throw new WebpackError("No chunk found for CSS URL");
				}
				default:
					return null;
			}
		...
		switch (generateContext.type) {
			...
			case CSS_TYPE: {
				if (
					!this._exportsOnly &&
					(!exportType || ["link", "url"].includes(exportType))
				) {
	...
	getTypes(module) {
		...
		const onlyJs =
			this._exportsOnly ||
			(exportType && ["text", "css-style-sheet"].includes(exportType));
