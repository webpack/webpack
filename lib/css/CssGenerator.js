...
class CssGenerator extends Generator {
	...
	/**
	 * Collect the `@namespace` rules that apply to a module's inlined output
	 * (own plus build-time inlined imports), with the last declaration of a
	 * prefix winning per the CSS Namespaces spec. Returns rule text in
	 * declaration order so the caller can hoist it above the content.
	 * @param {NormalModule} module the module
	 * @param {ModuleGraph} moduleGraph the module graph
	 * @param {Set<NormalModule>=} visited cycle guard (lazily allocated)
	 * @param {Map<string, string>=} acc accumulator (prefix -> rule text)
	 * @returns {Map<string, string> | undefined} prefix -> rule text, or undefined when none
	 */
	_getEffectiveNamespaces(module, moduleGraph, visited, acc) {
		if (!module) return acc;
		// The cycle guard and accumulator are allocated lazily, so a module
		// subtree with no `@namespace` (the common case) does no extra work.
		if (visited === undefined) {
			// first call, no cycle possible yet
		} else if (visited.has(module)) {
			return acc;
		} else {
			visited.add(module);
		}
		// Inlined imports precede own content, so collect them first; a later
		// (re)declaration of the same prefix overrides the earlier one.
		for (const dep of module.dependencies) {
			if (dep instanceof CssImportDependency) {
				if (visited === undefined) {
					visited = new Set();
				...
				acc = this._getEffectiveNamespaces(
					/** @type {NormalModule} */ (moduleGraph.getModule(dep)),
					moduleGraph,
					visited,
					acc
				);
		...
		const own =
			module.buildInfo &&
			/** @type {CssModuleBuildInfo} */ (module.buildInfo).namespaces;
		if (own) {
			if (acc === undefined) acc = new Map();
			for (const { prefix, rule } of own) {
				acc.delete(prefix);
				acc.set(prefix, rule);
			}
		}
	...
	/**
	 * Render collected `@namespace` rules as CSS text to hoist above content.
	 * @param {Map<string, string> | undefined} namespaces prefix -> rule text
	 * @returns {string} CSS text (possibly empty)
	 */
	_renderNamespacePrefix(namespaces) {
		if (namespaces === undefined) return "";
		let text = "";
		for (const rule of namespaces.values()) text += `${rule};\n`;
		return text;
	}
		...
		switch (generateContext.type) {
			case JAVASCRIPT_TYPE: {
				...
				const generateContentCode = () => {
					switch (exportType) {
						case "style": {
							let cssSource = this._generateContentSource(
								module,
								generateContext
							);
							if (!cssSource) return "";

							// Each style module injects its own `<style>`, so hoist only
							// this module's own `@namespace` above its content.
							const ownNamespaces = /** @type {CssModuleBuildInfo} */ (
								module.buildInfo
							).namespaces;
							if (ownNamespaces && ownNamespaces.length > 0) {
								const nsMap = new Map();
								for (const { prefix, rule } of ownNamespaces) {
									nsMap.set(prefix, rule);
								}
								cssSource = new ConcatSource(
									new RawSource(this._renderNamespacePrefix(nsMap)),
									cssSource
								);
							}
				...
				const generateExportCode = () => {
					...
					const generateCssText = () => {
						...
						// Hoist `@namespace` above the merged content (inside the literal,
						// so quoting is handled; `@charset` is added after, ending up first).
						const namespacePrefix = this._renderNamespacePrefix(
							this._getEffectiveNamespaces(module, generateContext.moduleGraph)
						);
						const contentSource =
							namespacePrefix && cssSource
								? new ConcatSource(new RawSource(namespacePrefix), cssSource)
								: cssSource;

						let jsLiteral = contentSource
							? this._cssToJsLiteral(contentSource, devtool, generateContext)
							: new RawSource('""');
			...
			case CSS_TEXT_TYPE: {
				...
				const charsetPrefix =
					effectiveCharset !== undefined
						? `@charset "${effectiveCharset}";\n`
						: "";
				// `@charset` then `@namespace`, hoisted above the merged text.
				const prefix =
					charsetPrefix +
					this._renderNamespacePrefix(
						this._getEffectiveNamespaces(module, generateContext.moduleGraph)
					);

				if (!cssSource) {
					return prefix ? new RawSource(prefix) : new RawSource("");
				}
				return prefix ? new ConcatSource(prefix, cssSource) : cssSource;
			}
			default:
				return null;
		}
