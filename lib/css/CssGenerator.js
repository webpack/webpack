...
class CssGenerator extends Generator {
	 ...
	 */
	generateImportCode(module, generateContext) {
		...
		for (const dep of module.dependencies) {
			if (
				dep instanceof CssImportDependency ||
				dep instanceof CssIcssImportDependency
			) {
	 ...
	 */
	generateModuleCode(module, generateContext) {
	...
	generate(module, generateContext) {
		...
		const generateJSDefaultExport = () => {
			switch (exportType) {
				case "text": {
					const importCode = this.generateImportCode(module, generateContext);
					const moduleCode = this.generateModuleCode(module, generateContext);

					return [...importCode, JSON.stringify(moduleCode)]
						.filter(Boolean)
						.join(" + ");
				}
				// case "style-sheet"
		...
		switch (generateContext.type) {
			case "javascript": {
				...
				const needDefaultExport =
					defaultExport !== null && !cssData.exports.has("default");
				/**
				...
				if (!isCSSModule && !generateContext.concatenationScope && !needNsObj) {
					return new RawSource(
						`${RuntimeGlobals.module}.exports = ${defaultExport}`
					);
				}
