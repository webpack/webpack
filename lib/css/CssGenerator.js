...
class CssGenerator extends Generator {
	...
	constructor(options, moduleGraph) {
		...
		/** @type {WeakMap<RuntimeRequirements, Source[]>} */
		this._sourceModuleCache = new WeakMap();
	}
	 ...
	 * @returns {string[]} JavaScript code that concatenates all imported CSS
	 */
	_generateImportCode(module, generateContext) {
		...
		for (const dep of module.dependencies) {
			if (dep instanceof CssImportDependency) {
				...
				parts.push(
					`(${RuntimeGlobals.compatGetDefaultExport}(${importVar})() || "")`
				);
			}
		}
	 ...
	 * @returns {ReplaceSource} the source for the module
	 */
	sourceModule(module, initFragments, generateContext) {
		const exportType = /** @type {BuildMeta} */ (module.buildMeta).exportType;
		const isEmpty =
			generateContext.type === "javascript" && exportType === "link";

		if (!this._sourceModuleCache.has(generateContext.runtimeRequirements)) {
			this._sourceModuleCache.set(
				generateContext.runtimeRequirements,
				/** @type {Source[]} */ ([])
			);
		}

		const cachedIndex = isEmpty ? 0 : 1;
		const cachedSource = /** @type {Source[]} */ (
			this._sourceModuleCache.get(generateContext.runtimeRequirements)
		)[cachedIndex];
		if (cachedSource) {
			return /** @type {ReplaceSource} */ (cachedSource);
		}
		const source = isEmpty
			? new ReplaceSource(new RawSource(""))
			: new ReplaceSource(/** @type {Source} */ (module.originalSource()));
		this._sourceModule(module, initFragments, source, generateContext);
		/** @type {Source[]} */ (
			this._sourceModuleCache.get(generateContext.runtimeRequirements)
		)[cachedIndex] = source;
		return source;
	 ...
	 * @returns {void}
	 */
	_sourceModule(module, initFragments, source, generateContext) {
	...
	generate(module, generateContext) {
		...
		const source = this.sourceModule(module, initFragments, {
			...generateContext,
			cssData
		});
<<<<<<< HEAD
=======
		...
		const generateCssText = () => {
			...
			if (importCode.length > 0) {
				...
				return `${RuntimeGlobals.cssMergeStyleSheets}([${[...importCode, JSON.stringify(moduleCode)].join(", ")}], "${exportType}")`;
			}
			return JSON.stringify(moduleCode);
		};

>>>>>>> fe75ab7d5 (feat(css): added css-style-sheet to exportType for CSSStyleSheet return)
		/**
