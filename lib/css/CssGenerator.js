					...
					const generateJSDefaultExport = () => {
						switch (exportType) {
							...
							case "css-style-sheet": {
								...
								// Universal targets run in node too, where `CSSStyleSheet`
								// is absent; fall back to the raw CSS text there.
								if (generateContext.runtimeTemplate.isNeutralPlatform()) {
									return new ConcatSource(
										`(${fnPrefix}${constOrVar} cssText = `,
										generateCssText(),
										`;\nif (typeof CSSStyleSheet === "undefined") return cssText;\n${constOrVar} sheet = new CSSStyleSheet();\nsheet.replaceSync(cssText);\nreturn sheet;\n})()`
									);
								}
								return new ConcatSource(
