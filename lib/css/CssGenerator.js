...
class CssGenerator extends Generator {
	 ...
	 *
	 * The CSS is embedded as a single JS string, so we cannot keep
	 * webpack-sources' fine-grained mappings (everything collapses onto one
	 * line after JSON.stringify and the line/column information no longer
	 * matches). To still let DevTools resolve the original sources when the
	 * string is applied at runtime — `<style>` injection, `CSSStyleSheet`,
	 * or any consumer of the exported text — we append an inline
	 * `sourceMappingURL` data URI to the CSS content itself. The wrapping
	 * `OriginalSource` keeps the emitted JS literal visible in the bundle's
	 * JS source map under the CSS module's identifier.
	...
	_cssSourceToJsStringLiteral(cssSource, module) {
		...
		if (map) {
			const base64Map = Buffer.from(JSON.stringify(map), "utf8").toString(
				"base64"
			);
			const trailingNewline = content.endsWith("\n") ? "" : "\n";
			content += `${trailingNewline}/*# sourceMappingURL=data:application/json;charset=utf-8;base64,${base64Map}*/`;
		}
		return new OriginalSource(JSON.stringify(content), module.identifier());
	}
