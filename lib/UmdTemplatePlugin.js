...
const accessorAccess = (base, accessor, joinWith = ", ") => {
	...
	return accessors
		.map((_, idx) => {
			const a = base
				? base + propertyAccess(accessors.slice(0, idx + 1))
				: accessors[0] + propertyAccess(accessors.slice(1, idx + 1));
						...
						return replaceKeys(
							modules
								.map(m => {
									...
									return `root${propertyAccess(request.split("/"))}`;
								})
								.join(", ")
						);
					};
								...
								.map(m => {
									...
									if (Array.isArray(request)) {
										expr = `require(${JSON.stringify(
											request[0]
										)})${propertyAccess(request.slice(1))}`;
									} else {
