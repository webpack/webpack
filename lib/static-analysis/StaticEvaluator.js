...
const StaticNodeTypeEvaluator = {
	...
	"UnaryExpression": (expression, parser) => {
		if(expression.operator === "typeof") {
			...
			if(expression.argument.name === "undefined") {
				return new BasicEvaluatedExpression().setString("undefined").setRange(expression.range);
			}

			if(expression.argument.type === "Identifier") {
				var name = parser.nameInCurrentScope(expression.argument.name);
				if(parser.scope.definitions.indexOf(name) === -1) {
					return parser.applyPluginsBailResult1("evaluate typeof " + name, expression);
				}
			}
			if(expression.argument.type === "MemberExpression") {
				...
				if(expressionArgument.type === "Identifier") {
					...
					if(parser.scope.definitions.indexOf(name) === -1) {
						...
						return parser.applyPluginsBailResult1("evaluate typeof " + exprName, expression);
					}
				}
			}
