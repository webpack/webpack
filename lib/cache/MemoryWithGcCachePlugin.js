	...
	apply(compiler) {
		...
		compiler.hooks.afterDone.tap(PLUGIN_NAME, () => {
			...
			for (const [identifier, entry] of cache) {
				...
				if (entry !== undefined) {
					// Set to undefined rather than deleted, reserving the slot and sparing
					// a rehash as entries come and go. It goes for good once `oldCache`
					// drops it.
		...
		compiler.cache.hooks.get.tap(
			...
			(identifier, etag, gotHandlers) => {
				...
				// WHY: etags are compared by identity, and a lazy one is interned per
				// source object, so equal content reached through a second object is a
				...
				// `null` here would bail the hook and lose that, and `Cache.get` maps it
				// to `undefined` regardless, so it buys the caller nothing.
