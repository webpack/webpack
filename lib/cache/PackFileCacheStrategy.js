...
const { absoluteToRequest, requestToAbsolute } = require("../util/identifier");
const { createFileSerializer } = require("../util/serialization");

const ABSOLUTE_PATH_IDENTIFIER_REGEX = /\/(\/.*)$/;
const RESOLVE_IDENTIFIER_REGEX = /path=(.*)\/request=.*$/;

/** @typedef {import("../Cache").Etag} Etag */
/** @typedef {import("../logging/Logger").Logger} Logger */
/** @typedef {import("../util/fs").IntermediateFileSystem} IntermediateFileSystem */
/** @typedef {import("./types").CacheStrategyOptions} CacheStrategyOptions */
/** @typedef {import("./types").CacheWriteReadAbsolutePath} CacheWriteReadAbsolutePath */
/** @template T @typedef {import("../util/LazySet")<T>} LazySet<T> */

/**
 * @param {string} cacheLocation cache location
 * @param {boolean} contextify contextify flag
 * @returns {CacheWriteReadAbsolutePath} context
 */
function createAbsolutePathsWriteReadMethods(cacheLocation, contextify) {
	if (contextify === false) {
	...
	const writeAbsolutePath = path => {
		...
		return absoluteToRequest(cacheLocation, path);
	};
	const readAbsolutePath = path => {
		if (!path) return path;

		return requestToAbsolute(cacheLocation, path);
	};
...
class PackFileCacheStrategy {
	/**
	 * @param {CacheStrategyOptions} options options
	 */
	constructor({
		fs,
		context,
		...
		const contextifyMethods = createAbsolutePathsWriteReadMethods(
			cacheLocation,
			contextify
		);
		this.fileSerializer = createFileSerializer(fs, contextifyMethods);
		this.fileSystemInfo = new FileSystemInfo(fs, {
			managedPaths,
			immutablePaths,
			logger: logger.getChildLogger("webpack.FileSystemInfo")
		});
		this.contextifyAbsolutePath = contextifyMethods.writeAbsolutePath;
		this.context = context;
		this.contextify = contextify;
		this.identCache = new Map();
		this.cacheLocation = cacheLocation;
		this.version = version;
		this.logger = logger;
		/** @type {Set<string>} */
		this.buildDependencies = new PathSet();
		/** @type {LazySet<string>} */
		this.newBuildDependencies = new PathLazySet();
	...
	contextifyIdentifier(ident) {
		if (!this.contextify) return ident;

		let contextified = this.identCache.get(ident);

		if (contextified) return contextified;

		let matched = false;
		contextified = ident.replace(
			ABSOLUTE_PATH_IDENTIFIER_REGEX,
			(match, path) => {
				matched = true;

				return "/" + this.contextifyAbsolutePath(path);
			}
		);

		if (matched) {
			this.identCache.set(ident, contextified);
			return contextified;
		}

		contextified = ident.replace(RESOLVE_IDENTIFIER_REGEX, (match, path, i) => {
			return (
				"path=" +
				this.contextifyAbsolutePath(path) +
				ident.slice(i + path.length + 5 /* "path=".length */)
			);
		});
	...
	store(identifier, etag, data) {
		return this.packPromise.then(pack => {
			...
			pack.set(
				this.contextifyIdentifier(identifier),
