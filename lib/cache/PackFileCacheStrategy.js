 ...
 * @property {boolean} portable write/read paths as is or contextify relative to cache folder
...
class Pack {
	...
	serialize({ write, writeSeparate, writePathArray }) {
		this._persistFreshContent();
		this._optimizeSmallContent();
		this._optimizeUnusedContent();

		writePathArray(Array.from(this.itemInfo.keys()));
	...
	deserialize({ read, readPathArray, logger }) {
		this.logger = logger;
		{
			const items = readPathArray();
...
class PackContent {
	...
	getMap() {
		...
		if (!this.outdated && this.content) {
			const map = new PathAsKeyMap(this.content);
