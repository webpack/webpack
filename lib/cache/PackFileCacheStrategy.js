...
function createAbsolutePathsWriteReadMethods(cacheLocation, contextify) {
	if (contextify === false) {
		...
		return {
			...
			writeAbsolutePaths: s => s,
			readAbsolutePaths: s => s
		};
	}
	...
	return {
		...
		writeAbsolutePaths: set => {
			...
			for (const path of set) {
				result.add(writeAbsolutePath(path));
			}

			return result;
		},
		readAbsolutePaths: set => {
			if (!set) return set;
			...
			for (const path of set) {
				result.add(readAbsolutePath(path));
			}
...
class PackFileCacheStrategy {
	...
	constructor({
		...
		this.fileSerializer = createFileSerializer(
			fs,
			createAbsolutePathsWriteReadMethods(cacheLocation, contextify)
		);
