...
const { resolveBuildDependencies } = require("./sharedBuildDependencies");

/** @import { SnapshotOptions } from "../../declarations/WebpackOptions" */
...
class PackContainer {
	 ...
	 * @param {InstanceType<Snapshot>} buildSnapshot snapshot of all build dependencies
	 ...
	 * @param {InstanceType<Snapshot>} resolveBuildDependenciesSnapshot snapshot of the dependencies of the build dependencies resolving
	 */
	constructor(
		data,
		version,
...
class PackContent {
	// `used` is a subset of `items`, itself a subset of the content's keys, and the
	// two coincide while `outdated` is false. `lazy` and `content` hold the same
	// data whenever both are set, and `get` takes only an item of `items`.
...
class PackFileCacheStrategy {
	...
	constructor({
		...
		/** @type {FileSystemDependencies} */
		this.newOptionalBuildDependencies = new LazySet();
		/** @type {InstanceType<Snapshot> | undefined} */
	 ...
	 * @param {FileSystemDependencies | Iterable<string>} optionalDependencies optional dependencies to store
	 * @returns {void}
	 */
	storeBuildDependencies(dependencies, optionalDependencies) {
		if (this.readonly) return;
		this.newBuildDependencies.addAll(dependencies);
		this.newOptionalBuildDependencies.addAll(optionalDependencies);
	}
	...
	afterAllStored() {
		...
		return (this.storePromise = packPromise
			.then((pack) => {
				...
				/** @type {Set<string>} */
				const optionalBuildDependencies = new Set();
				for (const dep of this.newBuildDependencies) {
					if (!this.buildDependencies.has(dep)) {
						newBuildDependencies.add(dep);
						if (this.newOptionalBuildDependencies.has(dep)) {
							optionalBuildDependencies.add(dep);
						}
					}
				}
				...
				return promise.then(() => {
					...
					return this.fileSerializer
						...
						.then(() => {
							...
							this.newOptionalBuildDependencies.clear();
			...
			while (queue.length > 0) {
				...
				const mtimeMs = await new Promise((resolve, reject) => {
					fs.stat(file, (err, stats) => {
						...
						resolve(
							/** @type {number} */ (
								/** @type {import("../fs/fs").IStats} */ (stats).mtimeMs
							)
						);
					});
				});
			...
			for (const file of files) {
				...
				const stats = await new Promise((resolve) => {
					fs.stat(path, (err, stats) => {
						resolve(
							err ? undefined : /** @type {import("../fs/fs").IStats} */ (stats)
						);
					});
				});
