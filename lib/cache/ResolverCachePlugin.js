...
const PathLazySet = require("../util/PathLazySet");
...
class CacheEntry {
	...
	serialize({ write, writePath }) {
		if (!this.result) {
			...
			write(rest);
			writePath(descriptionFilePath);
			writePath(descriptionFileRoot);
			writePath(path);
		}
	...
	deserialize({ read, readPath }) {
		let result = read();

		if (result) {
			result.descriptionFilePath = readPath();
			result.descriptionFileRoot = readPath();
			result.path = readPath();
		}
	...
	apply(compiler) {
		...
		const doRealResolve = (
			...
			const newResolveContext = {
				...resolveContext,
				stack: new Set(),
				missingDependencies: new PathLazySet(),
				fileDependencies: new PathLazySet(),
				contextDependencies: new PathLazySet()
			};
			const propagate = key => {
					...
					(resolver, options, userOptions) => {
						...
						resolver.hooks.resolve.tapAsync(
							...
							(request, resolveContext, callback) => {
								...
								if (r.request) {
									r.request = absolutify(r.path, r.request, this);
								}
