...
const { sizeof } = require("../util/size");
...
class IdleFileCachePlugin {
	 ...
	 * @param {object} cacheFilter filter options for what to cache
	 */
	constructor(
		strategy,
		idleTimeout,
		...
		/** @type {object} */ cacheFilter = { maxAssetSize: 2048 } // Default: only cache assets < 2KB
	) {
		...
		this.cacheFilter = cacheFilter; // Store the filter config
	}
	...
	apply(compiler) {
		...
		compiler.cache.hooks.store.tap(
			...
			(identifier, etag, data) => {
				// Apply cache filter for asset-related data
				if (
					identifier.includes("|asset/") ||
					identifier.startsWith("Compilation/assets|")
				) {
					const size = sizeof(data); // Estimate size of the data
					const { maxAssetSize } = this.cacheFilter;

					// Skip caching if the data exceeds the size limit
					if (typeof maxAssetSize === "number" && size > maxAssetSize) {
						return; // Don't cache large assets
					}
					// Future extension: Add more filter conditions if needed
				}
