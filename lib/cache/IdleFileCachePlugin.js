...
class IdleFileCachePlugin {
	...
	apply(compiler) {
		...
		const MAX_ASSET_CACHE_SIZE = 2 * 1024; // 2KB limit

compiler.cache.hooks.store.tap(
  { name: "IdleFileCachePlugin", stage: Cache.STAGE_DISK },
  (identifier, data) => {
    console.log("Cache Hook Triggered:", identifier);

    if (typeof identifier === "string" && identifier.includes("|asset/")) {
      if (Buffer.isBuffer(data) && data.length > MAX_ASSET_CACHE_SIZE) {
        return; // Skip caching large binary assets
      }
    }

    return data; // Cache everything else normally
  }
);
