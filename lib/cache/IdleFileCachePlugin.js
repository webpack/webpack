...
/** @import PackFileCacheStrategy from "./PackFileCacheStrategy" */
...
class IdleFileCachePlugin {
	...
	apply(compiler) {
		...
		// Safety flush to drain all remaining pending tasks and persist the pack
		const onBeforeExit = () => {
			if (!pendingIdleTasks.size) return;
			currentIdlePromise = Promise.all([
				...[...pendingIdleTasks.values()].map(fn => Promise.resolve().then(fn)),
				currentIdlePromise
			])
				.then(() => strategy.afterAllStored())
				.then(() => process.once("beforeExit", onBeforeExit));
