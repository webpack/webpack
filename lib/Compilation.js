...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// context each entry dep was factorized with; an entry has no importer to
		// recover it from, and it is not always the compiler's (see HtmlModulesPlugin)
		/** @type {Map<Dependency, string>} */
		this._entryContexts = new Map();
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		if (this._incrementalRebuild && entryData !== undefined) {
			...
			if (existing !== undefined) {
				...
				if (existingModule) {
					// the graph keeps `existing`, so the context has to travel with it:
					// a plugin may build a fresh dep every make (HtmlModulesPlugin does)
					this._entryContexts.set(existing, context);
					/** @type {Set<Dependency>} */
		...
		this.handleModuleCreation(
			{
				...
				dependencies: [dependency],
				originModule,
				// an entry has no importer whose context could be reused, and its
				// own directory is not it either when it sits below the context
				context:
