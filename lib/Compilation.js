	...
	_refactorizeModule(module, normalModuleFactory, changedSet, callback) {
		...
		for (const c of moduleGraph.getIncomingConnections(module)) {
			...
			if (
				...
				// mid-flight and the re-factorization would reattach a stale
				// connection; such modules are reached by the importer's walk instead
				!(c.originModule && changedSet.has(c.originModule)) &&
