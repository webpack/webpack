...
class Compilation {
	...
	processRuntimeRequirements(entrypoints) {
		...
		const { runtimeRequirementInModule } = this.hooks;
		for (const module of this.modules) {
			if (chunkGraph.getNumberOfModuleChunks(module) > 0) {
				let set;
	...
	summarizeDependencies() {
		...
		for (const module of this.modules) {
			const { fileDependencies } = module.buildInfo;
			const { contextDependencies } = module.buildInfo;
			const { missingDependencies } = module.buildInfo;
			if (fileDependencies) {
				this.fileDependencies.addAll(fileDependencies);
			}
	...
	createHash() {
		...
		const { hashFunction } = outputOptions;
		const { hashDigest } = outputOptions;
		const { hashDigestLength } = outputOptions;
