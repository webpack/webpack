...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// experiments.incremental: reuse the previous compilation's module graph
		// and modules, so a rebuild only re-processes changed modules (_incrementalMake)
		/** @type {Compilation | undefined} */
		const incrementalPrev =
			options.experiments.incremental && compiler._lastCompilation !== undefined
		...
		// previous compilation's sealed chunk graph + structures, swapped in by the
		// seal fast path to skip rebuilding them (see seal())
	...
	/**
	 * Incremental make (experiments.incremental): against the persisted module
	...
	_incrementalMake(normalModuleFactory, callback) {
		...
		this.moduleGraph.restoreFromMutationJournal();
		const context = {
			compilation: this,
			fileSystemInfo: this.fileSystemInfo,
			valueCacheVersions: this.valueCacheVersions
		...
		asyncLib.each(
			...
			(err) => {
				...
				if (changed.length === 0) {
					// nothing rebuilt — chunk structure is unchanged
					this._incrementalNoStructuralChange = true;
					return callback();
				}
				...
				const pushDeps = (deps, parts) => {
					for (const dep of deps) {
						...
						parts.push(
							`${dep.constructor.name}::${d.request || ""}::${d.name || ""}::${ids}`
						);
					}
				};
				...
				asyncLib.each(
					changed,
					(module, cb) => {
						// refresh factory-derived parser/generator so the rebuild uses
						// this compilation's plugin hooks (e.g. DefinePlugin values)
						module.restoreParserAndGenerator(normalModuleFactory);
						this._rebuildModule(module, cb);
					},
					(err) => {
						if (err) return callback(err);
						let structural = this.modules.size !== modulesBefore;
						if (!structural) {
							for (const module of changed) {
	 ...
	 */
	seal(callback) {
		if (this.options.experiments.incremental) {
			...
			this.moduleGraph.startMutationJournal();
		}
		/**
		...
		// experiments.incremental seal fast path: when the rebuild changed no chunk
		// structure, reuse the previous compilation's chunk graph + ids and only
		// re-hash/re-generate the changed modules — skip buildChunkGraph + ids.
		// Gated off concatenation for now (it adds modules during optimizeChunkModules)
		const fastSeal =
		...
		if (fastSeal) {
			...
			for (const chunk of this.chunks) {
				// drop the previous build's runtime modules so processRuntimeRequirements
				// re-adds fresh instances (else the old ones have no code gen entry)
				/** @type {RuntimeModule[]} */
				...
				for (const rm of runtimeModules) {
					...
					// also drop it from the persisted module graph + module set so they
					// don't accumulate across rebuilds
					this.moduleGraph.removeModule(rm);
					this.modules.delete(rm);
					this._modules.delete(rm.identifier());
				}
				// the chunk's full-/dependent-hash module sets still hold the old
				// runtime modules (each keeps its Compilation alive) — clear them; the
				// fresh runtime modules re-attach in processRuntimeRequirements
				this.chunkGraph.clearRuntimeModuleHashes(chunk);
