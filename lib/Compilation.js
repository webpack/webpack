...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		/**
		 * What `renderEmbeddedSource` returned, by the text it was given. Read on
		 * the pass that follows, so identical text is only rendered once.
		 ...
		 * @type {Map<string, { source: Source, info: EmbeddedSourceInfo, jobModule: Module }>}
		 * @private
		 */
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
	...
	_resolveEmbeddedSource(source, info) {
		...
		const resolved = this._embeddedSources.get(text);
		if (resolved !== undefined) return resolved;
		this._pendingEmbeddedSources.set(text, {
			source,
			info,
			jobModule: this._generatingModule || info.module
		});
		return source;
	}
	...
	_resolveEmbeddedSources(jobs, callback) {
		...
		const pending = [...this._pendingEmbeddedSources];
			...
			([text, { source, info }], cb) => {
				this.hooks.renderEmbeddedSource.callAsync(
					...
					(err, result) => {
						...
						this._embeddedSources.set(text, result || source);
						cb();
					}
				);
			},
