...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		// Avoid breaking changes caused by plugins resetting compilation.modules = new Set(modules)
		if (typeof this.modules.sortWith === "function") {
			// Ensure modules are sorted before rendering, similar to JavascriptModulesPlugin.renderChunk
			// https://github.com/webpack/webpack/pull/19861#issuecomment-3243628469
			this.modules.sortWith(compareModulesByIdOrIdentifier(this.chunkGraph));
		}
