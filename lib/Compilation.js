...
class Compilation extends Tapable {
	constructor(compiler) {
		...
		this.fileTimestamps = null;
		this.contextTimestamps = null;
		this.compilationDependencies = null;
		this.name = null;
	...
	addModuleDependencies(
		...
		let __this = this;
		const start = __this.profile && Date.now();
		const currentProfile = __this.profile && {};
		...
		asyncLib.forEach(
			...
			(item, callback) => {
				...
				const warningAndCallback = err => {
					err.origin = module;
					__this.warnings.push(err);
					callback();
				};

				const semaphore = __this.semaphore;
				semaphore.acquire(() => {
					if (__this === null) return semaphore.release();
					...
					factory.create(
						...
						(err, dependentModule) => {
							...
							const addModuleResult = __this.addModule(
								dependentModule,
								cacheGroup
							);
							...
							const afterBuild = () => {
								...
								if (recursive && addModuleResult.dependencies) {
									__this.processModuleDependencies(dependentModule, callback);
								} else {
									return callback();
								}
							};
							...
							if (addModuleResult.build) {
								...
								semaphore.release();
								__this.waitForBuildingFinished(dependentModule, afterBuild);
							}
						}
					);
				});
	...
	assignIndex(module) {
		...
		const assignIndexToModule = module => {
			...
			if (typeof module.index !== "number") {
				module.index = __this.nextFreeModuleIndex++;

				// leave module
				queue.push(() => (module.index2 = __this.nextFreeModuleIndex2++));
