...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		/**
		 * What `renderEmbeddedSource` returned, by module and by the text it was
		 * given. Read on the pass that follows. Never shared between modules: a
		 * tap may return a source carrying a map, which names the module it came
		 * from — two modules with identical text still get their own.
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
	...
	_resolveEmbeddedSources(jobs, callback) {
		...
		/** @type {{ text: string, source: Source, info: EmbeddedSourceInfo, jobModule: Module }[]} */
		const pending = [];
		for (const byText of this._pendingEmbeddedSources.values()) {
			for (const [text, entry] of byText) {
				pending.push({ text, ...entry });
			}
		}
