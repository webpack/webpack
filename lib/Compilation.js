...
/**
 * @typedef {Object} DependenciesWithSameRequest
...
class Compilation extends Tapable {
	...
	processModuleDependencies(module, callback) {
		...
		for (const [factory, resources] of dependencies) {
			for (const [, dependencies] of resources) {
	 ...
	 * @param {DependenciesWithSameRequest[]} sortedDependencies set of dependencies grouped by request to iterate through
					...
					factory.create(
						...
						(err, dependentModule) => {
							...
							dependencies.forEach(dependency => {
								dependency.module = dependentModule;
								dependentModule.addReason(module, dependency);
							});
	...
	seal(callback) {
		...
		for (const { module, name, request } of this._preparedEntrypoints) {
			const chunk = this.addChunk(name);
			const entrypoint = new Entrypoint(name);
			entrypoint.setRuntimeChunk(chunk);
			entrypoint.addOrigin(null, name, request);
