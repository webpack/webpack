...
const { getRuntimeKey, mergeRuntime } = require("./util/runtime");
		...
		for (const module of this.modules) {
			...
			if (runtimes.size === 1) {
				...
				for (const runtime of runtimes) {
					...
					if (list === undefined) {
						map.set(hash, [runtime]);
					} else {
						list.push(runtime);
					}
				}
				for (const [hash, runtimes] of map) {
					jobs.push({ module, hash, runtime: runtimes[0], runtimes });
				}
			}
		}
