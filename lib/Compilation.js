	...
	_buildModule(module, callback) {
		...
		module.needBuild(
			...
			(err, needBuild) => {
				...
				/**
				 * @param {WebpackError | null | undefined} err build error
				 */
				...
				const workerBuilder = this.compiler._workerModuleBuilder;
				const NormalModuleClass = require("./NormalModule");
				if (
					workerBuilder &&
					module instanceof NormalModuleClass &&
					module.resource &&
					...
					// Worker build path: worker reads file + parses →
					// returns plain dependency descriptors (no serialization)
					workerBuilder
						.build(module.resource, module.request)
						.then((result) => {
							const {
								OriginalSource
							} = require("webpack-sources");
							const createDep = require("./build/createDependencyFromDescriptor");

							// Set source
							module._source = new OriginalSource(
								result.source,
								module.resource
							);

							// Set build metadata
							module.buildMeta = result.buildMeta || {};
							module.buildInfo = {
								cacheable: true,
								parsed: true,
								fileDependencies: new Set([module.resource]),
							...
							// Rebuild dependencies from plain descriptors
							module.clearDependenciesAndBlocks();
							for (const desc of result.deps) {
								const dep = createDep(desc);
								if (dep) module.addDependency(dep);
							}
							for (const desc of result.presentationalDeps) {
								const dep = createDep(desc);
								if (dep) module.addPresentationalDependency(dep);
							}
