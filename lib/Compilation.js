...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_processModuleDependencies(module, callback) {
		...
		/** @type {boolean} */
		const hasDelayedDependencies = module.dependencies.some(
			(d) => /** @type {ModuleDependency} */ (d).sourceOrder === Infinity
		);
		...
		const processDependencyForResolving = (dep) => {
			...
			if (resourceIdent !== undefined && resourceIdent !== null) {
				...
				if (list === undefined) {
					...
					if (hasDelayedDependencies) {
						let insertIndex = sortedDependencies.length;
						while (insertIndex > 0) {
							const item = sortedDependencies[insertIndex - 1];
							const isAllDelayedDependencies = item.dependencies.every(
								(d) =>
									/** @type {ModuleDependency} */ (d).sourceOrder === Infinity
							);
							if (isAllDelayedDependencies) {
								insertIndex--;
							} else {
								break;
							}
