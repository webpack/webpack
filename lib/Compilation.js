...
// lazy — Compilation and NormalModule require each other
/** @type {typeof import("./NormalModule") | undefined} */
let LazyNormalModule;
const getNormalModule = () =>
	LazyNormalModule || (LazyNormalModule = require("./NormalModule"));
...
/** @typedef {{ id: ModuleId, modules?: Map<Module, string>, blocks?: (ChunkId | null)[], sourceTypes?: SourceTypes }} References */
...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// reuse the previous graph so a rebuild only re-processes changed modules
		/** @type {Compilation | undefined} */
		const incrementalPrev =
			compiler.watchMode &&
			// build-time execution poisons the durable graph (Compiler#_executeModuleSeen)
			!compiler._executeModuleSeen &&
			compiler._lastCompilation !== undefined &&
			// only a compilation whose seal prepared for reuse (base snapshot +
			// mutation journal) can be reused — a run() compilation cannot
			compiler._lastCompilation._incrementalReady
		...
		// (no module add/remove, no async block change)
		/** @type {boolean} */
		this._incrementalNoStructuralChange = false;
		// previous compilation's sealed chunk graph, swapped in by the seal fast
		// path to skip rebuilding it (see seal())
		/** @type {{ chunkGraph: ChunkGraph, chunks: Chunks, chunkGroups: ChunkGroup[], entrypoints: Map<string, Entrypoint>, asyncEntrypoints: Entrypoint[], namedChunks: Map<string, Chunk>, namedChunkGroups: Map<string, ChunkGroup>, entryOptions: Map<string, EntryOptions> } | undefined} */
		this._incrementalPrevSeal = incrementalPrev
			? {
					chunkGraph: incrementalPrev.chunkGraph,
					chunks: incrementalPrev.chunks,
					chunkGroups: incrementalPrev.chunkGroups,
					entrypoints: incrementalPrev.entrypoints,
					asyncEntrypoints: incrementalPrev.asyncEntrypoints,
					namedChunks: incrementalPrev.namedChunks,
					namedChunkGroups: incrementalPrev.namedChunkGroups,
					// settled options per entry — the next make resets them, and a
					// changed option (filename, layer, runtime) reshapes the chunks
					entryOptions: Compilation._captureEntryOptions(incrementalPrev)
				}
			: undefined;
		this.outputOptions =
		...
		if (this._backCompat) {
			...
			// the reused base set is a fresh snapshot from the previous seal and
		...
		// module count before make seeding; a make-phase change forces a full seal
		/** @type {number | undefined} */
		this._incrementalPreMakeModuleCount = undefined;
		// seeding-pass window / whether it built anything (forces a full seal)
		...
		// per-module factory resolve deps, folded into the build snapshot so
		...
		// per-origin missing-resolution probes, snapshotted after make so an
		// unedited module re-resolves when an inner node_modules appears
	...
	_addModule(module, callback) {
		...
		if (alreadyAddedModule) {
			// persisted modules never reach the cache path below where updateCacheModule
			// runs; without it dependency-carried state goes stale (mini-css content)
	...
	_buildModule(module, callback) {
		...
		if (
			...
			this._incrementalNormalModuleFactory !== undefined &&
			/** @type {{ parser?: import("./Parser") }} */ (module).parser ===
				undefined
		) {
			module._restoreParserAndGenerator(this._incrementalNormalModuleFactory);
		}
		const currentProfile = this.profile
			? this.moduleGraph.getProfile(module)
			: undefined;
			...
			(err, needBuild) => {
				...
				module.build(
					...
					(err) => {
						...
						if (this._incrementalDeferredStaleRefs.size > 0) {
							const staleRefs = this._incrementalDeferredStaleRefs.get(module);
							if (staleRefs !== undefined) {
								// deferred re-factorize: purge the pre-rebuild connections the
								// direct path removes via its own cleanup
								this._incrementalDeferredStaleRefs.delete(module);
								this._removePreRebuildConnections(module, staleRefs);
							}
						}
						if (
							this._incrementalRebuild &&
							...
							for (const dependency of module.dependencies) {
								if ("setLazy" in dependency) {
									/** @type {EXPECTED_ANY} */ (dependency).setLazy(false);
								}
							}
						}
						if (currentProfile !== undefined) {
	...
	_addOriginResolveDeps(originModule, fr) {
		// only node_modules MISSING probes matter (a nested package shadowing a
		// hoisted one); relative extension probes are huge and never materialize
	...
	_createIncrementalResolveSnapshots(callback) {
		if (
			this.compiler._executeModuleSeen ||
			this._incrementalOriginResolveDeps.size === 0
		) {
			return callback();
		}
	 ...
	 * @property {boolean=} reusedDependencies dependencies are persisted from the previous compilation (incremental re-factorize), not freshly minted seeds
	 */
			...
			(err, factoryResult) => {
				...
				this.addModule(newModule, (err, _module) => {
					...
					if (
						...
						// a fresh seeding dependency is minted each make (prefetch chains) —
						// drop the previous makes' connections or they accumulate unbounded;
						// a re-factorize reuses a persisted dependency, where this would
						// drop sibling entries pointing at the same module
					...
					if (this.compiler.watchMode && !this.compiler._executeModuleSeen) {
						const fr = /** @type {ModuleFactoryResult} */ (factoryResult);
						// a loader's package.json can change which loader file is used — fold
						// its file/context deps into the module's snapshot (missing: see below)
	...
	_handleModuleBuildAndDependencies(
		...
		this.buildModule(module, (err) => {
			...
			// an unchanged module's deps + connections are already in the persisted
			// graph; re-walking would duplicate connections across its subtree
			if (this._incrementalRebuild && !this.builtModules.has(module)) {
				// side-effect-free modules fall through so a lazy barrel can still
				// un-lazy targets newly requested by the changed importer
			...
			if (unlazyItems !== undefined) {
				...
				const onJobDone = (err) => {
					...
					if (--inProgress === 0) {
						// a barrel un-lazied from the importer's walk has export deps whose
						// parent differs from their origin — flush by origin so they resolve
						if (this._incrementalRebuild) {
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		// the entry dep — return the existing module instead
		if (this._incrementalRebuild && entryData !== undefined) {
			const touchedEntryData =
				/** @type {Set<EntryData>} */
				(this._incrementalTouchedEntryData);
			...
			const existing = entryData[target].find(
				(d) =>
					...
					/** @type {EXPECTED_ANY} */ (d).request === e.request
			);
			if (existing !== undefined) {
				const existingModule = this.moduleGraph.getModule(existing);
				if (existingModule) {
					/** @type {Set<Dependency>} */
					...
					this.hooks.addEntry.call(existing, options);
					this.hooks.succeedEntry.call(existing, options, existingModule);
					return callback(null, existingModule);
				}
				// never resolved (a failed entry) — drop the stale dep and factorize
		...
		if (this._incrementalTouchedEntries !== undefined) {
			this._incrementalTouchedEntries.add(entry);
		}
	...
	/**
	 * Snapshots each entry's settled options as shallow copies — the next make
	 * resets the live objects in place.
	 * @param {Compilation} compilation compilation to snapshot
	 * @returns {Map<string, EntryOptions>} options by entry name ("" = global)
	 */
	static _captureEntryOptions(compilation) {
		/** @type {Map<string, EntryOptions>} */
		const snapshot = new Map();
		snapshot.set("", { ...compilation.globalEntry.options });
		for (const [name, entryData] of compilation.entries) {
			snapshot.set(name, { ...entryData.options });
		}
		return snapshot;
	}

	/**
	 * Compares the current entries' settled options against the previous seal's
	 * snapshot — a changed option (filename, layer, runtime, dependOn) reshapes
	 * the chunks even when the module graph did not change.
	 * @param {Map<string, EntryOptions>} snapshot previous options by entry name
	 * @returns {boolean} true when every entry kept the same options
	 */
	_entryOptionsUnchanged(snapshot) {
		const current = Compilation._captureEntryOptions(this);
		if (current.size !== snapshot.size) return false;
		for (const [name, options] of current) {
			const previous = snapshot.get(name);
			if (previous === undefined) return false;
			const keys = new Set([...Object.keys(previous), ...Object.keys(options)]);
			for (const key of keys) {
				const a = /** @type {EXPECTED_ANY} */ (previous)[key];
				const b = /** @type {EXPECTED_ANY} */ (options)[key];
				if (a === b) continue;
				if (
					Array.isArray(a) &&
					Array.isArray(b) &&
	...
	_incrementalMake(normalModuleFactory, callback) {
		...
		// drop persisted entry deps this make's entry plugins did not re-add
		// (a dynamic entry disappeared) — else their modules stay rooted forever
		let entriesDropped = false;
		const touchedEntries =
			/** @type {Set<Dependency>} */
			(this._incrementalTouchedEntries);
		const configEntryDependencies = this.compiler._configEntryDependencies;
		/** @type {(entryData: EntryData) => void} */
		const reconcileEntryData = (entryData) => {
		...
		// a build during make seeding can reshape the graph — forces a full seal
			...
			(err) => {
				...
				if (this._incrementalErrorModules.size > 0) {
					...
					for (const m of retry) {
						if (this.modules.has(m) && !changedSet.has(m)) changed.push(m);
					}
				}
				this.logger.log(
				...
				if (changed.length === 0) {
					// make seeding may have changed the graph even with nothing to rebuild
					const preMake0 =
						this._incrementalPreMakeModuleCount !== undefined
				...
				// snapshot chunk-affecting state before rebuild
				...
				const pushBlocks = (blocks, parts) => {
					for (const block of blocks) {
						// group options (webpackChunkName, prefetch/preload, priority)
						// shape the chunk graph without touching any dependency
						parts.push(
							`##${block.dependencies.length}${JSON.stringify(block.groupOptions)}`
						);
				...
				const shapeKey = (module) => {
					...
					// side-effect state steers connection activity (SideEffectsFlagPlugin),
					// hence chunk membership of the modules it imports
					const buildMeta = module.buildMeta;
					const factoryMeta = module.factoryMeta;
					parts.push(
						`~${buildMeta && buildMeta.sideEffectFree}~${
							factoryMeta && factoryMeta.sideEffectFree
						}`
					);
					// outgoing activity: a rebuild can change a connection's condition
					// (inner-graph usedByExports) without changing any dependency
					let states = "";
					for (const c of this.moduleGraph.getOutgoingConnections(module)) {
						if (!c.dependency || !c.module) continue;
						const state = c.getActiveState(undefined);
						states += state === true ? "T" : state === false ? "F" : "O";
					}
					parts.push(states);
					return parts.join("|");
				};
				...
				asyncLib.each(
					...
					(err) => {
						...
						// pre-make count so modules added during make seeding count too
						const preMake =
							this._incrementalPreMakeModuleCount !== undefined
								? this._incrementalPreMakeModuleCount
								: modulesBefore;
						// runtime modules are seal artifacts — they must not tip the decision
	...
	_refactorizeModule(module, normalModuleFactory, changedSet, callback) {
		// only loader/scheme modules can re-resolve elsewhere; anything else gets a
		// plain rebuild (ContextModules re-scan in build())
		const m = /** @type {EXPECTED_ANY} */ (module);
		const canReresolve =
			module instanceof getNormalModule() &&
		...
		for (const c of moduleGraph.getIncomingConnections(module)) {
			...
			if (d === undefined || d === null || !d.constructor) continue;
			...
			if (c.originModule && changedSet.has(c.originModule)) {
		...
		if (dependency === undefined) {
			if (hasRebuildingImporter) {
				// defer — the rebuilding importer's dependency walk re-factorizes
				// this module with the full pipeline (scheme version hooks included);
				// force the build so a still-valid build snapshot (resolve-snapshot
				// invalidations) doesn't short-circuit the re-walk of its own deps
				module.invalidateBuild();
				// that walk replaces the deps without this path's cleanup — record
				// the old ones so the build callback can purge their connections
				this._incrementalDeferredStaleRefs.set(module, {
		...
		this.moduleGraph.unfreeze();
		// factorization re-resolves this dependency and adds a fresh connection;
		// detach the persisted one or the graph gains one connection per rebuild
		this.moduleGraph._removeConnectionLoose(dependency);
		this.handleModuleCreation(
			{
				factory,
				dependencies: [dependency],
			...
			(err, newModule) => {
				...
				// drop the pre-rebuild deps of the old module
				this._removePreRebuildConnections(module, {
					dependencies: oldDependencies,
					blocks: oldBlocks
				});
				if (built !== module) {
					this._incrementalRefactorReplaced = true;
					// move the other importers too, else they render against the
	 ...
	 * @param {{ dependencies: Dependency[], blocks: AsyncDependenciesBlock[] }} staleRefs pre-rebuild deps and blocks
	 * @returns {void}
	 */
	_removePreRebuildConnections(module, staleRefs) {
		this.removeReasonsOfDependencyBlock(module, staleRefs);
		if (!this._incrementalRebuild) return;
		/** @type {Set<Dependency>} */
		const staleDeps = new Set(staleRefs.dependencies);
		/** @type {(blocks: AsyncDependenciesBlock[]) => void} */
		...
		collectBlockDeps(staleRefs.blocks);
	...
	_computeAffectedModulesWithChunkGraph() {
		...
		let statNew = 0;
		/**
		 * Encode a connected module's reference: its id plus the active state of
		 * every connection — an optimization (e.g. side effects) can flip a
		 * connection without any id changing, which must invalidate cached
		 * hashes/codegen of the origin
		 * @param {Module} m connected module
		 * @param {readonly ModuleGraphConnection[]} connections connections to it
		 * @returns {string} reference key
		 */
		const referenceKey = (m, connections) => {
			let key = `${chunkGraph.getModuleId(m)}`;
			for (const c of connections) {
				const state = c.getActiveState(undefined);
				key += state === true ? "T" : state === false ? "F" : "O";
			}
			return key;
		};
		/**
		 ...
		 * @param {Map<Module, string>=} references.modules modules
		...
		const compareReferences = (
			...
			if (modules !== undefined) {
				const outgoing = moduleGraph.getOutgoingConnectionsByModule(module);
				for (const [m, key] of modules) {
					const connections = outgoing && outgoing.get(m);
					if (
						connections === undefined ||
						referenceKey(m, connections) !== key
					) {
						return false;
					}
				}
	...
	seal(callback) {
		...
		// seal fast path: the rebuild changed no chunk structure, so reuse the
		// previous chunk graph + ids and re-hash/generate only what changed.
		// Gated off concatenation (it adds modules during optimizeChunkModules).
		const fastSeal =
			this._incrementalNoStructuralChange &&
			this._incrementalPrevSeal !== undefined &&
			!this.options.optimization.concatenateModules &&
			this._entryOptionsUnchanged(this._incrementalPrevSeal.entryOptions);
		if (fastSeal) {
			const prev =
				/** @type {NonNullable<Compilation["_incrementalPrevSeal"]>} */
				(this._incrementalPrevSeal);
			this.chunkGraph = prev.chunkGraph;
			this.chunks = prev.chunks;
			this.chunkGroups = prev.chunkGroups;
			this.entrypoints = prev.entrypoints;
			this.asyncEntrypoints = prev.asyncEntrypoints;
			this.namedChunks = prev.namedChunks;
			this.namedChunkGroups = prev.namedChunkGroups;
			for (const chunk of this.chunks) {
				// drop the previous build's runtime modules so
				// processRuntimeRequirements re-adds fresh instances
				/** @type {RuntimeModule[]} */
				const runtimeModules = [];
				for (const rm of this.chunkGraph.getChunkRuntimeModulesIterable(
					chunk
				)) {
					runtimeModules.push(rm);
				}
				for (const rm of runtimeModules) {
					this.chunkGraph.disconnectChunkAndRuntimeModule(chunk, rm);
					this.chunkGraph.disconnectChunkAndModule(chunk, rm);
					this.moduleGraph._removeModule(rm);
					this.modules.delete(rm);
					this._modules.delete(rm.identifier());
				}
				// the chunk's hash module sets still hold the old runtime modules
				// (each keeps its compilation alive) — fresh ones re-attach later
				this.chunkGraph._clearRuntimeModuleHashes(chunk);
				this.chunkGraph._clearChunkSealState(chunk);
				// reset per-seal asset state so createChunkAssets/emit re-derive it
				chunk.files.clear();
				chunk.auxiliaryFiles.clear();
				chunk.rendered = false;
			}
			for (const module of this.modules) {
				this.chunkGraph._clearModuleSealState(module);
			}
		}
		...
		if (fastSeal) {
			...
			// chunk structure is reused, but chunk-independent hooks still apply:
			// afterChunks re-reports chunk-placement diagnostics (wasm in initial
			// chunk) and optimizeModules re-flags rebuilt modules (isCircular)
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, (err) => {
			...
			this.hooks.optimizeChunkModules.callAsync(
				...
				(err) => {
					...
					if (!fastSeal) this.logger.timeEnd("optimize");
	...
	executeModule(module, options, callback) {
		...
		processAsyncTree(
			...
			(module, push, callback) => {
				...
				// an unchanged persisted module was never queued (waitFor would fail);
				// its connections are already complete — aggregate straight from them
				if (this._incrementalRebuild && !this.builtModules.has(module)) {
					return aggregate();
				}
