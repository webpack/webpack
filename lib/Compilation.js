...
const getBuildCycleError = lazyModule.sync(() =>
	require("./errors/BuildCycleError")
);
const getChunkRenderError = lazyModule.sync(() =>
	require("./errors/ChunkRenderError")
);
const getCodeGenerationError = lazyModule.sync(() =>
	require("./errors/CodeGenerationError")
);
const getModuleDependencyError = lazyModule.sync(() =>
	require("./errors/ModuleDependencyError")
);
const getModuleDependencyWarning = lazyModule.sync(() =>
	require("./errors/ModuleDependencyWarning")
);
const getModuleHashingError = lazyModule.sync(() =>
	require("./errors/ModuleHashingError")
);
const getModuleNotFoundError = lazyModule.sync(() =>
	require("./errors/ModuleNotFoundError")
);
const getModuleRestoreError = lazyModule.sync(() =>
	require("./errors/ModuleRestoreError")
);
const getModuleStoreError = lazyModule.sync(() =>
	require("./errors/ModuleStoreError")
);
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
	...
	_attachPendingRuntimeModules(callback) {
		...
		this._pendingRuntimeModules = undefined;
		Promise.all(pending.map(({ load }) => load())).then((loaded) => {
