...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
	...
	reportDependencyErrorsAndWarnings(module, blocks) {
		/** @type {import("./Dependency").DependencyCheckMode} */
		const checkMode = {
			useSyncAssertion: !!this.options.experiments.syncImportAssertion,
			deferModuleMode: this.options.experiments.deferImport
		};
		...
		for (let indexBlock = 0; indexBlock < blocks.length; indexBlock++) {
			...
			for (let indexDep = 0; indexDep < dependencies.length; indexDep++) {
				...
				const warnings = d.getWarnings(this.moduleGraph, checkMode);
				if (warnings) {
				...
				const errors = d.getErrors(this.moduleGraph, checkMode);
				if (errors) {
