...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// incremental rebuilds: reuse the previous compilation's module graph
		// and modules, so a rebuild only re-processes changed modules (_incrementalMake)
		/** @type {Compilation | undefined} */
		const incrementalPrev =
			compiler.watchMode &&
			// see Compiler#_executeModuleSeen — build-time execution and the
			// durable graph cannot share state across rebuilds
			!compiler._executeModuleSeen &&
			compiler._lastCompilation !== undefined
				? compiler._lastCompilation
				: undefined;
		/** @type {boolean} */
		this._incrementalRebuild = incrementalPrev !== undefined;
		// set by _incrementalMake: true when the rebuild changed no chunk structure
		// (no module add/remove, no async block change) — enables the seal fast path
		/** @type {boolean} */
		...
		/** @type {ModuleGraph} */
		this.moduleGraph = incrementalPrev
			? incrementalPrev.moduleGraph
			: new ModuleGraph();
		/** @type {ChunkGraph} */
		...
		this.entries = incrementalPrev ? incrementalPrev.entries : new Map();
		/** @type {EntryData} */
		this.globalEntry = incrementalPrev
			? incrementalPrev.globalEntry
			: {
					dependencies: [],
					includeDependencies: [],
					options: {
		...
		// reuse the end-of-make module set (before seal added runtime/concatenated
		// modules and removed concatenated inner modules), not the post-seal set
		/** @type {Set<Module>} */
		this.modules = incrementalPrev
			? incrementalPrev._incrementalBaseModules
			: new Set();
		if (this._backCompat) {
		 ...
		 */
		this._modules = incrementalPrev
			? incrementalPrev._incrementalBaseModulesMap
			: new Map();
		// modules whose dependency factorization failed (e.g. missing module):
		// their needBuild snapshot can't see the missing path, so retry them every
		// incremental rebuild until the dependency resolves (self-clearing)
		/** @type {Set<Module>} */
		this._incrementalErrorModules = incrementalPrev
			? incrementalPrev._incrementalErrorModules
			: new Set();
		// snapshot of the module set at end of make (captured at seal start), used
		// as the base for the next incremental rebuild; defaults to the live sets
		/** @type {Set<Module>} */
		this._incrementalBaseModules = this.modules;
		/** @type {Map<string, Module>} */
		this._incrementalBaseModulesMap = this._modules;
		// incremental: modules re-seeded this compilation via a null-origin chain
		// (AutomaticPrefetchPlugin/PrefetchPlugin re-run through hooks.make). They are
		// graph roots (issuer null) even though persisted with an importer's issuer.
		/** @type {Set<Module>} */
		this._incrementalSeedRoots = new Set();
		// previous compilation's post-seal module set; seal-only modules (runtime
		// and concatenated modules) are swept from the durable graph in
		// _incrementalMake so their graph entries don't accumulate across rebuilds
		/** @type {Set<Module> | undefined} */
		this._incrementalPrevSealModules = incrementalPrev
			? incrementalPrev.modules
			: undefined;
		// incremental: factory used to lazily restore a released parser when a
		// persisted module is (re)built during the hooks.make seeding pass
		/** @type {import("./NormalModuleFactory") | undefined} */
		this._incrementalNormalModuleFactory = undefined;
		// incremental: module count before the make seeding pass, so a make-phase
		// graph change forces a full seal instead of the fast path
		/** @type {number | undefined} */
		this._incrementalPreMakeModuleCount = undefined;
		// incremental: true while the make seeding pass runs; set when it builds any
		// module, so _incrementalMake can force a full seal for make-phase changes
		this._incrementalMakePhase = false;
		this._incrementalMakeBuilt = false;
		// incremental: set when re-factorizing a changed module resolved to a new
		// module (changed loader/scheme/resolve), so the orphaned old one is pruned
		this._incrementalRefactorReplaced = false;
		// incremental: per-module factory resolve dependencies (loader/package.json
		// probes), folded into the module's build snapshot so needBuild catches a
		// changed loader resolution. WeakMap — released with the module.
		/** @type {WeakMap<Module, { fileDependencies?: LazySet<string> | Set<string>, contextDependencies?: LazySet<string> | Set<string> }>} */
		this._incrementalModuleResolveDeps = new WeakMap();
		// incremental: per-ORIGIN-module accumulation of where its dependencies
		// resolved (missing/context probes), snapshotted after make and checked in the
		// needBuild scan so a module re-resolves when an inner node_modules appears or
		// a managed package version changes without any edit to the module itself.
		/** @type {Map<Module, LazySet<string>>} */
		this._incrementalOriginResolveDeps = new Map();
		/** @type {Records | null} */
		this.records = null;
		/** @type {string[]} */
	...
	_addModule(module, callback) {
		...
		if (alreadyAddedModule) {
			// incremental: the persisted graph carries this module across compilations,
			// so a freshly factorized duplicate never reaches the persistent-cache path
			// below where updateCacheModule runs. Some modules carry state on the
			// dependency, applied only through updateCacheModule (e.g. mini-css's
			// CssModule content) — refresh it here or the stale content persists.
			if (this._incrementalRebuild && alreadyAddedModule !== module) {
				alreadyAddedModule.updateCacheModule(module);
			}
	...
	_buildModule(module, callback) {
		// incremental: a module built via hooks.make seeding (a prefetch chain or a
		// newly discovered dependency) may be a persisted module whose parser was
		// released after the previous seal. Restore it before the build needs it.
		if (
			this._incrementalRebuild &&
			this._incrementalNormalModuleFactory !== undefined &&
			/** @type {EXPECTED_ANY} */ (module).parser === undefined &&
			typeof (
				/** @type {EXPECTED_ANY} */ (module)._restoreParserAndGenerator
			) === "function"
		) {
			/** @type {EXPECTED_ANY} */ (module)._restoreParserAndGenerator(
				this._incrementalNormalModuleFactory
			);
		}
		const currentProfile = this.profile
			? this.moduleGraph.getProfile(module)
		...
		module.needBuild(
			...
			(err, needBuild) => {
				...
				// a real build during the make seeding pass forces a full seal
				if (this._incrementalMakePhase) this._incrementalMakeBuilt = true;
				module.build(
					this.options,
					this,
	 ...
	 * @param {ModuleFactoryResult} fr the factory result carrying the resolve deps
	 * @returns {void}
	 */
	_addOriginResolveDeps(originModule, fr) {
		// only MISSING probes matter for re-resolution: a path absent at resolve time
		// later appearing changes where the dependency resolves. Narrow to probes
		// inside a package directory (node_modules) — a nested package shadowing a
		// hoisted one is the case that re-resolves an unedited importer; relative
		// extension probes (./x.ts for a ./x.jsx that resolved) are a huge per-module
		// set that virtually never materializes, so tracking them only taxes cold.
		if (!fr.missingDependencies) return;
		/** @type {LazySet<string> | undefined} */
		let missing;
		for (const dep of fr.missingDependencies) {
			if (!dep.includes("node_modules")) continue;
			if (missing === undefined) {
				missing = this._incrementalOriginResolveDeps.get(originModule);
				if (missing === undefined) {
					missing = new LazySet();
					this._incrementalOriginResolveDeps.set(originModule, missing);
				}
			}
			missing.add(dep);
		}
	...
	/**
	 * Incremental: snapshot each origin module's accumulated resolve dependencies
	 * (from `_addOriginResolveDeps`) so the next rebuild's needBuild scan can detect
	 * a changed resolution (inner node_modules, managed version) and rebuild it.
	 * @private
	 * @param {(err?: WebpackError | null) => void} callback signals completion
	 * @returns {void}
	 */
	_createIncrementalResolveSnapshots(callback) {
		if (this._incrementalOriginResolveDeps.size === 0) return callback();
		const snapshotOptions = this.options.snapshot.resolve;
		const startTime = this.compiler.fsStartTime || this.startTime;
		const empty = new LazySet();
		asyncLib.each(
			[...this._incrementalOriginResolveDeps],
			([module, missing], cb) => {
			...
			(err) => {
				// snapshots now live on the durable modules; drop the per-make deps map
				this._incrementalOriginResolveDeps.clear();
		...
		this.factorizeModule(
			...
			(err, factoryResult) => {
				...
				if (err) {
					...
					if (this.compiler.watchMode && originModule) {
						this._incrementalErrorModules.add(originModule);
					}
				...
				this.addModule(newModule, (err, _module) => {
					...
					if (
						...
						// re-seeded root (entry/prefetch): a persisted module carries an
						// importer's issuer, but a root has none. Force null to match a full
						// make, and record it so the prune keeps it a root (not reassigned).
						moduleGraph.setIssuer(module, null);
						this._incrementalSeedRoots.add(module);
					} else {
						moduleGraph.setIssuerIfUnset(
							module,
							originModule !== undefined ? originModule : null
					...
					// incremental: track where this dependency resolved so needBuild can
					// react to a changed resolution without an edit to the module itself.
					if (this.compiler.watchMode) {
						const fr = /** @type {ModuleFactoryResult} */ (factoryResult);
						// the created module's own loader resolution: a loader's package.json
						// (a fileDependency) can change which loader file is used, so fold
						// file/context deps into its build snapshot. The missing probes are
						// the resolved RESOURCE's extension candidates — a huge per-module set
						// whose re-resolution is the origin's concern (below), so skip them.
						if (fr.fileDependencies || fr.contextDependencies) {
							this._incrementalModuleResolveDeps.set(module, {
								fileDependencies: fr.fileDependencies,
								contextDependencies: fr.contextDependencies
							});
						}
						// the ORIGIN module's resolution of this dependency: where it resolved
						// can change (an inner node_modules appears, a higher-priority
						// extension is added) with no edit to the origin's own inputs, and its
						// build snapshot (taken before this dep factorized) can't see it.
						const resource = /** @type {EXPECTED_ANY} */ (module).resource;
						if (
							originModule !== null &&
							originModule !== undefined &&
							fr.missingDependencies &&
							typeof resource === "string" &&
							resource.includes("node_modules")
						) {
							this._addOriginResolveDeps(originModule, fr);
						}
					}
	...
	_handleModuleBuildAndDependencies(
		...
		this.buildModule(module, (err) => {
			...
			// incremental rebuild: an unchanged module reached while re-walking a
			// changed module already has its dependencies + connections in the
			// persisted graph. Don't re-walk them — that re-processes (and duplicates
			// connections across) the whole unchanged subtree.
			if (this._incrementalRebuild && !this.builtModules.has(module)) {
				// unchanged module reached while re-walking a changed one: skip the
				// re-walk of its subtree. But a lazy barrel must still un-lazy targets
				// newly requested by the changed importer, which needs the normal
				// un-lazy path below (build + connect the export dep). Let side-effect-
				// free modules fall through; its unchanged children hit this guard again
				// and short-circuit, so only the barrel's own deps are re-walked.
				const fm = module.factoryMeta;
				if (fm === undefined || !fm.sideEffectFree) {
			...
			if (unlazyItems !== undefined) {
				...
				const onJobDone = (err) => {
					...
					if (--inProgress === 0) {
						// incremental: an already-built barrel un-lazied here wires its new
						// re-export target from the importer's walk, so the export dep's
						// parent differs from its origin and getConnection's parent-keyed
						// flush never assigns it. Flush by origin so its exports resolve.
						if (this._incrementalRebuild) {
							this.moduleGraph._flushUnassignedConnections(module);
						}
						callback(null, module);
					}
				};
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		// incremental: entries are seeded in the persisted graph. hooks.make re-runs
		// the entry plugins, but re-pushing would duplicate the entry dep (and its
		// chunk). Skip when an equivalent dep is already present; return its module.
		if (this._incrementalRebuild && entryData !== undefined) {
			const e = /** @type {EXPECTED_ANY} */ (entry);
			const existing = entryData[target].find((d) => {
				const dd = /** @type {EXPECTED_ANY} */ (d);
				return (
					d.constructor === entry.constructor &&
					dd.request === e.request &&
					dd.name === e.name
				);
			});
			if (existing !== undefined) {
				return callback(
					null,
					this.moduleGraph.getModule(existing) || undefined
				);
			}
		}
	 ...
	 * @returns {void}
	 */
	_incrementalMake(normalModuleFactory, callback) {
		// undo the previous seal's graph mutations (concatenation) so make runs
		// against the clean durable module graph
		this.moduleGraph._restoreFromMutationJournal();
		// with connections restored, drop the previous seal's synthetic modules
		// (runtime/concatenated) from the durable graph — see the constructor note
		if (this._incrementalPrevSealModules !== undefined) {
			for (const module of this._incrementalPrevSealModules) {
				if (!this.modules.has(module)) {
					this.moduleGraph._removeModule(module);
				}
			}
			this._incrementalPrevSealModules = undefined;
		}
		const context = {
			compilation: this,
			fileSystemInfo: this.fileSystemInfo,
		...
		// whether the make seeding pass (re)built any module (a prefetch chain
		// rebuilding a context or a non-cacheable module); such a build can add or
		// reshape modules the fast seal path would miss, forcing a full seal below
		const builtDuringMake = this._incrementalMakeBuilt;
		this._incrementalMakePhase = false;
		/** @type {Module[]} */
		const changed = [];
		asyncLib.each(
			...
			(module, cb) => {
				module.needBuild(context, (err, need) => {
					...
					// a module whose own inputs are unchanged can still need a rebuild if
					// WHERE one of its dependencies resolved changed (an inner node_modules
					// appeared, a managed package version bumped) — its resolve snapshot,
					// unlike its build snapshot, covers those probes.
					...
					// fill lazy-barrel state that make normally fills when a module's
					// deps are processed; unchanged barrels are never processed here, so
					// a newly-imported deferred re-export could not otherwise un-lazy
					this._lazyBarrelController.classify(module);
					cb();
				});
			},
			(err) => {
				if (err) return callback(err);
				// retry modules whose deps failed to factorize last time (a missing
				// module may now exist); clear first — still-failing ones re-register
				if (this._incrementalErrorModules.size > 0) {
					const retry = [...this._incrementalErrorModules];
					this._incrementalErrorModules.clear();
					for (const m of retry) {
						if (this.modules.has(m) && !changed.includes(m)) changed.push(m);
					}
				}
				this.logger.log(
					`incremental make: ${changed.length} of ${this.modules.size} module(s) changed`
				);
				// build-time execution reads provided exports DURING make, before they are
				// re-determined at finishModules; reset changed modules' provided exports so
				// an `export *` target re-exports the fresh set, not the persisted stale one
				for (const m of changed) {
					this.moduleGraph.getExportsInfo(m)._resetProvidedExports();
					// the persisted graph's meta keeps the module's importVarMap, so
					// dropped imports leave gaps and later imports get higher indices than
					// a full build assigns; clear it so import vars are numbered fresh
					const meta = this.moduleGraph.getMeta(m);
					meta.importVarMap = undefined;
					meta.deferredImportVarMap = undefined;
				}
				if (changed.length === 0) {
					// nothing needed rebuild, but the make seeding pass may still have
					// built/added modules (a dynamic entry, a re-scanned context) with an
					// empty needBuild set — apply the same structural signals the rebuild
					// path uses so such a change still forces a full seal
					const preMake0 =
						this._incrementalPreMakeModuleCount !== undefined
							? this._incrementalPreMakeModuleCount
							: this.modules.size;
					let nonRuntimeCount0 = 0;
					for (const m of this.modules) {
						if (m.type !== "runtime") nonRuntimeCount0++;
					}
					this._incrementalNoStructuralChange =
						!builtDuringMake && nonRuntimeCount0 === preMake0;
					return callback();
				}
				this.moduleGraph.unfreeze();
				// snapshot chunk-affecting state before rebuild: module count, and for
				// each changed module its async block count + its set of outgoing target
				// modules. Any of these changing means the chunk graph must be rebuilt.
				const modulesBefore = this.modules.size;
				// a rebuild is chunk-structure-safe only if the module's dependency and
				// block shape is unchanged. Capture a shape key over all dependencies
				// (type + name + request + ids) and blocks — this catches added, removed
				// or changed imports AND export name changes (which affect dependents).
				/** @type {(deps: import("./Dependency")[], parts: string[]) => void} */
				const pushDeps = (deps, parts) => {
					for (const dep of deps) {
						const d = /** @type {EXPECTED_ANY} */ (dep);
						const ids = Array.isArray(d.ids) ? d.ids.join(".") : d.id || "";
						parts.push(
							`${dep.constructor.name}::${d.request || ""}::${
								d.name || ""
							}::${ids}`
						);
					}
				};
				/** @type {(blocks: import("./AsyncDependenciesBlock")[], parts: string[]) => void} */
				const pushBlocks = (blocks, parts) => {
					for (const block of blocks) {
						parts.push(`##${block.dependencies.length}`);
						pushDeps(block.dependencies, parts);
				...
				const shapeKey = (module) => {
					/** @type {string[]} */
					const parts = [];
					pushDeps(module.dependencies, parts);
				...
				asyncLib.each(
					changed,
					(module, cb) => {
						// re-factorize (not just rebuild) so a changed loader/scheme/resolve
						// result is picked up, not the persisted one; the parser is restored
						// lazily in _buildModule when the module builds
						this._refactorizeModule(module, normalModuleFactory, cb);
					},
					(err) => {
						if (err) return callback(err);
						...
						// re-factorization that resolved to a new module leaves the old one
						// orphaned; prune it like a dropped import
						if (this._incrementalRefactorReplaced) shapeChanged = true;
						// a changed dependency shape can leave modules unreachable (a
						// dropped import, a context re-scan losing deleted entries). A full
						// make only ever contains reachable modules, so prune them here or
						// they linger, get rebuilt and error on the now-missing file.
						if (shapeChanged) this._pruneUnreachableModules();
						// compare against the pre-make count so modules added during make
						// (not just during rebuild) count; builtDuringMake also catches a
						// make-phase reshape (e.g. context re-scan) with no net count change
						const preMake =
							this._incrementalPreMakeModuleCount !== undefined
								? this._incrementalPreMakeModuleCount
								: modulesBefore;
						// count only source modules — build-time execution (importModule)
						// leaves runtime modules behind that a full seal re-derives anyway,
						// so they must not tip the fast-seal decision (see preMake above)
						let nonRuntimeCount = 0;
						for (const m of this.modules) {
							if (m.type !== "runtime") nonRuntimeCount++;
						}
						const structural =
							shapeChanged || nonRuntimeCount !== preMake || builtDuringMake;
						this._incrementalNoStructuralChange = !structural;
						callback();
					}
				);
			}
	...
	/**
	 * Removes modules no longer reachable from the entries after an incremental
	 * rebuild changed the dependency graph. Mirrors a full make, whose module set
	 * is exactly the reachable set. Runs once on the settled graph (after all
	 * rebuilds and their dependency processing) so unsafe-cache re-references are
	 * already in place; a still-referenced module stays reachable and is kept. The
	 * module objects survive (only detached), so unsafe-cache can reuse them.
	 * @private
	 * @returns {void}
	 */
	_pruneUnreachableModules() {
		...
		/** @param {Dependency[]} deps root dependencies */
		const addRoots = (deps) => {
			for (const dep of deps) {
				const m = moduleGraph.getModule(dep);
				if (m && !reachable.has(m)) {
					reachable.add(m);
					// entry-reached modules have no issuer (matches a full make)
					moduleGraph.setIssuer(m, null);
				}
			}
		};
		addRoots(this.globalEntry.dependencies);
		addRoots(this.globalEntry.includeDependencies);
		for (const { dependencies, includeDependencies } of this.entries.values()) {
		...
		for (const module of this._incrementalSeedRoots) {
			if (this.modules.has(module) && !reachable.has(module)) {
				reachable.add(module);
				moduleGraph.setIssuer(module, null);
			}
		}
		// breadth-first over outgoing connections (Set iteration visits newly added
		// entries), following every connection like assignDepths does. Reassign the
		// issuer to the first importer reached in this walk — persisted issuers can
		// otherwise form a cycle once the graph changes, which the (fresh each make)
		// full build never produces.
		for (const module of reachable) {
			...
			if (connections !== undefined) {
				for (const refModule of connections.keys()) {
					if (refModule && !reachable.has(refModule)) {
						reachable.add(refModule);
						moduleGraph.setIssuer(refModule, module);
					}
				}
			}
		}
		...
		for (const module of this.modules) {
			...
			this._modules.delete(module.identifier());
			moduleGraph._removeModule(module);
			const resource = /** @type {EXPECTED_ANY} */ (module).resource;
			if (module.getNumberOfErrors() > 0 && typeof resource === "string") {
				prunedResources.push(resource);
			}
		}
		// A full make never builds an unreachable module. This one was built before
		// it could be pruned, and a build that failed on a deleted file cached that
		// ENOENT in the input filesystem — where nothing purges it, because the path
		// is no longer a dependency. Re-creating the file would then keep failing for
		// the rest of the watch session, so drop those entries now.
		if (prunedResources.length > 0) {
			const inputFileSystem = this.compiler.inputFileSystem;
			if (inputFileSystem && inputFileSystem.purge) {
				inputFileSystem.purge(prunedResources);
			}
		}
	}

	/**
	 * Incremental rebuild that also re-runs the module's factory (loader/scheme/
	 * resolve pipeline), not just its build. `_rebuildModule` reuses the persisted
	 * `module.loaders`/resource, so a changed loader resolution (a package.json
	 * `main` change, a `virtual:` scheme value, an inner node_modules appearing)
	 * would otherwise be missed. Re-factorizes through one incoming dependency; the
	 * factory dedupes to the same module for a content edit, or resolves to a new
	 * module (different identifier) that replaces the old one (pruned as unreachable).
	 * @param {Module} module module to re-factorize and rebuild
	 * @param {import("./NormalModuleFactory")} normalModuleFactory the normal module factory
	 * @param {ModuleCallback} callback callback when finished; passes the built module
	 * @returns {void}
	 */
	_refactorizeModule(module, normalModuleFactory, callback) {
		// re-factorize only modules whose factory resolution can actually change: a
		// NormalModule with loaders (their resolution follows a package.json `main`)
		// or a scheme resource (virtual: value deps). A plain module resolves to the
		// same place, and ContextModules re-scan in build() — a plain rebuild is both
		// correct and cheaper, and avoids churning modules the factory can't move.
		const m = /** @type {EXPECTED_ANY} */ (module);
		const canReresolve =
			module instanceof require("./NormalModule") &&
			((m.loaders && m.loaders.length > 0) ||
				(typeof m.resource === "string" &&
					getScheme(m.resource) !== undefined));
		if (!canReresolve) {
			module._restoreParserAndGenerator(normalModuleFactory);
			return this._rebuildModule(module, callback);
		}
		const moduleGraph = this.moduleGraph;
		/** @type {Dependency | undefined} */
		let dependency;
		/** @type {Module | null} */
		let originModule = null;
		for (const c of moduleGraph.getIncomingConnections(module)) {
			...
			if (
				...
				d !== null &&
				d.constructor &&
				this.dependencyFactories.get(
					/** @type {DependencyConstructor} */ (d.constructor)
				) !== undefined
			) {
				dependency = d;
				originModule = c.originModule;
				break;
			}
		}
		...
		this.handleModuleCreation(
			{
				...
				originModule,
				context:
					(originModule ? originModule.context : module.context) || undefined,
				connectOrigin: originModule !== null
			},
			(err, newModule) => {
				const built = newModule || module;
				if (err) {
					return this.hooks.finishRebuildingModule.callAsync(module, () =>
						callback(err)
					);
				}
				// drop the pre-rebuild deps of the old module (its incoming was
				// re-targeted to `built` if the identifier changed)
				this.removeReasonsOfDependencyBlock(module, {
					dependencies: oldDependencies,
					blocks: oldBlocks
				});
				/** @type {Set<Dependency>} */
				const staleDeps = new Set(oldDependencies);
				/** @type {(blocks: import("./AsyncDependenciesBlock")[]) => void} */
				const collectBlockDeps = (blocks) => {
					for (const b of blocks) {
						for (const d of b.dependencies) staleDeps.add(d);
						if (b.blocks) collectBlockDeps(b.blocks);
					}
				};
				collectBlockDeps(oldBlocks);
				this.moduleGraph._removeStaleOutgoingConnections(module, staleDeps);
				if (built !== module) {
					this._incrementalRefactorReplaced = true;
					// only the dependency re-factorized above re-targets to the new module;
					// move the old module's other importers too, else they render against the
					// now-disconnected old module
					this.moduleGraph.moveModuleConnections(
						module,
						built,
						(c) => c.module === module
					);
				}
				// pair with the hooks.rebuildModule(module) call above — the state cache
				// is keyed by the original module (a new re-resolved module is orphaned)
	...
	_rebuildModule(module, callback) {
		...
		this.buildModule(module, (err) => {
			...
			this.processModuleDependencies(module, (err) => {
				...
				// incremental: purge stale outgoing connections of the replaced deps
				// (incl. inactive ones left by SideEffectsFlagPlugin re-targeting) that the
				// dependency-keyed removeReasonsOfDependencyBlock misses, else they linger in
				// the old target's incoming set and crash seal-time plugins
				if (this._incrementalRebuild) {
					/** @type {Set<Dependency>} */
					const staleDeps = new Set(oldDependencies);
					/** @type {(blocks: import("./AsyncDependenciesBlock")[]) => void} */
	...
	finish(callback) {
		...
		this.hooks.finishModules.callAsync(modules, (err) => {
			...
			// incremental: snapshot per-module resolve dependencies so the next
			// rebuild detects a changed resolution (inner node_modules, managed version)
			this._createIncrementalResolveSnapshots((err2) => {
				if (err2) return callback(/** @type {WebpackError} */ (err2));
				callback();
			});
		});
	}
	...
	seal(callback) {
		if (this.compiler.watchMode) {
			// snapshot the module set now, before seal adds runtime/concatenated
			// modules and removes concatenated inner modules; the next incremental
			// rebuild reuses this clean base
			this._incrementalBaseModules = new Set(this.modules);
			// the next compilation adopts this fresh Set as its `modules`, so it needs
			// the Array-compat shims installed here — its constructor can't add them
			// later (`length` is defined non-configurable and would throw)
			if (this._backCompat) {
				arrayToSetDeprecation(
					this._incrementalBaseModules,
					"Compilation.modules"
				);
			}
			this._incrementalBaseModulesMap = new Map(this._modules);
			// record seal-time graph mutations (concatenation) so the next rebuild
			// can restore the durable module graph
			this.moduleGraph._startMutationJournal();
		}
		/**
...
This prevents using hashes of each other and should be avoided.`);
	...
	executeModule(module, options, callback) {
		// sticky per watch session — see Compiler#_executeModuleSeen
		this.compiler._executeModuleSeen = true;
		...
		processAsyncTree(
			...
			(module, push, callback) => {
				...
				// incremental: an unchanged persisted module was never queued this
				// compilation, so waitFor would fail — but its connections are already
				// complete in the durable graph, so aggregate straight from them.
				// Re-driving its build here would duplicate connections and perturb
				// this build's hashes. Modules that were (re)built this compilation
				// went through the normal queues and fall through to the waits below.
				if (this._incrementalRebuild && !this.builtModules.has(module)) {
					return aggregate();
				}
