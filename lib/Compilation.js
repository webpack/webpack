...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_processModuleDependencies(module, callback) {
		...
		/** @type {boolean} */
		const hasAcceptDependencies = module.dependencies.some(
			(d) =>
				d instanceof ModuleHotAcceptDependency ||
				d instanceof HarmonyAcceptImportDependency
		);
		...
		const processDependencyForResolving = (dep) => {
			...
			if (resourceIdent !== undefined && resourceIdent !== null) {
				...
				if (list === undefined) {
					...
					if (hasAcceptDependencies) {
						// ModuleHotAcceptDependency and HarmonyAcceptImportDependency don't carry import attributes
						let insertIndex = sortedDependencies.length;
						while (insertIndex > 0) {
							const item = sortedDependencies[insertIndex - 1];
							const isAllHMRAccept = item.dependencies.every(
								(d) =>
									d instanceof ModuleHotAcceptDependency ||
									d instanceof HarmonyAcceptImportDependency
							);
