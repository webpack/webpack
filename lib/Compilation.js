...
function iterationBlockVariable(variables, fn) {
	for(var indexVariable = 0, lenVariables = variables.length; indexVariable < lenVariables; indexVariable++) {
		var varDep = variables[indexVariable].dependencies;
		for(var indexVDep = 0, lenVDep = varDep.length; indexVDep < lenVDep; indexVDep++) {
			fn(varDep[indexVDep]);
		}
	}
}
...
class Compilation extends Tapable {
	...
	buildModule(module, optional, origin, dependencies, thisCallback) {
		...
		module.build(this.options, this, this.resolvers.normal, this.inputFileSystem, (error) => {
			...
			for(var indexError = 0, lenErrors = errors.length; indexError < lenErrors; indexError++) {
			...
			for(var indexWarning = 0, lenWarnings = warnings.length; indexWarning < lenWarnings; indexWarning++) {
		...
		async.forEach(factories, function iteratorFactory(item, callback) {
			...
			factory.create({
				...
				function iterationDependencies(depend) {
					for(var index = 0, len = depend.length; index < len; index++) {
	...
	reportDependencyErrorsAndWarnings(module, blocks) {
		for(var indexBlock = 0, lengthBlocks = blocks.length; indexBlock < lengthBlocks; indexBlock++) {
			var block = blocks[indexBlock];
			var dependencies = block.dependencies;

			for(var indexDep = 0, lengthDep = dependencies.length; indexDep < lengthDep; indexDep++) {
				var d = dependencies[indexDep];

				const warnings = d.getWarnings();
				if(warnings) {
					for(var indexWar = 0, lengthWar = warnings.length; indexWar < lengthWar; indexWar++) {
				...
				if(errors) {
					for(var indexErr = 0, lengthErr = errors.length; indexErr < lengthErr; indexErr++) {
	...
	removeChunkFromDependencies(block, chunk) {
		...
		for(var indexBlock = 0, lenBlocks = blocks.length; indexBlock < lenBlocks; indexBlock++) {
			var chunks = blocks[indexBlock].chunks;
			for(var indexChunk = 0, lenChunks = chunks.length; indexChunk < lenChunks; indexChunk++) {
	...
	applyModuleIds() {
		...
		for(var indexModule1 = 0, lenModule1 = modules1.length; indexModule1 < lenModule1; indexModule1++) {
		...
		for(var indexModule2 = 0, lenModule2 = modules2.length; indexModule2 < lenModule2; indexModule2++) {
	...
	applyChunkIds() {
		...
		function getNextFreeChunkId(usedChunkIds) {
			...
			for(var index = 0, len = keyChunks.length; index < len; index++) {
		...
		for(var indexChunk = 0, lenChunk = chunks.length; indexChunk < lenChunk; indexChunk++) {
	...
	sortItemsWithModuleIds() {
		...
		var modules = this.modules;
		for(var indexModule = 0, lenModules = modules.length; indexModule < lenModules; indexModule++) {
	...
	summarizeDependencies() {
		...
		for(var indexChildren = 0, lenChildrens = children.length; indexChildren < lenChildrens; indexChildren++) {
		...
		for(var indexModule = 0, lenModules = modules.length; indexModule < lenModules; indexModule++) {
			...
			if(module.fileDependencies) {
				...
				for(var indexFileDep = 0, lenFileDeps = fileDependencies.length; indexFileDep < lenFileDeps; indexFileDep++) {
					this.fileDependencies.push(fileDependencies[indexFileDep]);
				}
			}
			if(module.contextDependencies) {
				var contextDependencies = module.contextDependencies;
				for(var indexContextDep = 0, lenContextDeps = contextDependencies.length; indexContextDep < lenContextDeps; indexContextDep++) {
