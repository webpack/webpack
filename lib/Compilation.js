...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		/** @type {Map<DependencyConstructor, { entryOptions: (dependency: Dependency, module: Module) => (import("./Entrypoint").EntryOptions | undefined), getChunkFilename: (dependency: Dependency, chunk: import("./Chunk")) => (string | undefined) }>} */
		...
		this.factorizeModule(
			...
			(err, factoryResult) => {
				...
				this.addModule(newModule, (err, _module) => {
					...
					// Promote dependencies to entry blocks when a
					// dependencyEntries factory returns entryOptions.
					if (originModule) {
						for (const dependency of dependencies) {
							const entryDef = this.dependencyEntries.get(
								/** @type {DependencyConstructor} */
								(dependency.constructor)
							);
							if (!entryDef) continue;
							const entryOptions = entryDef.entryOptions(dependency, module);
							if (!entryOptions) continue;
							const idx = originModule.dependencies.indexOf(dependency);
							if (idx < 0) continue;
							originModule.dependencies.splice(idx, 1);
							...
							block.loc = /** @type {DependencyLocation} */ (dependency.loc);
							block.addDependency(dependency);
							originModule.addBlock(block);
							moduleGraph.setParents(dependency, block, originModule);
						}
					}
