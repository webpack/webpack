...
class Compilation extends Tapable {
	...
	processDependenciesBlocksForChunkGroups(inputChunkGroups) {
		...
		/** @type {Map<ChunkGroup, boolean>} */
		const chunkAlreadyQueued = new Map();
		...
		while (queue2.length) {
			...
			for (const nextChunkGroup of nextChunkGroups) {
				// Only need to queue each chunk group once.
				if (chunkAlreadyQueued.get(nextChunkGroup)) {
					queue2.enqueue({
						chunkGroup: nextChunkGroup,
						availableModules: newAvailableModules,
						needCopy: nextChunkGroup.size !== 1
					});
					chunkAlreadyQueued.set(nextChunkGroup, true);
				}
			}
		}
