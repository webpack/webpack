...
Compilation.prototype.sortItems = function sortItems() {
	...
	function byIdentifier(a, b) {
		return byString(a.identifier(), b.identifier());
	}
	...
	this.chunks.sort(byName);
	this.modules.sort(byIdentifier);
	this.modules.forEach(function(module) {
		module.chunks.sort(byName);
		module.reasons.sort(function(a, b) {
			return byIdentifier(a.module, b.module)
		});
	});
	this.chunks.forEach(function(chunk) {
		chunk.modules.sort(byIdentifier);
	});
};
