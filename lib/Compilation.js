...
const LazyBarrelDependencies = require("./LazyBarrelDependencies");
...
/** @typedef {import("./LazyBarrelDependencies").LazyBarrelGroup} LazyBarrelGroup */
/**
 * Defines the lazy barrel state type used by this module.
 * @typedef {object} LazyBarrelState
 * @property {Set<string> | true} requestedIds export names requested so far, true for all
 * @property {LazyBarrelDependencies | undefined} dependencies deferred dependencies, undefined until classified
 */
/**
...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		/**
		 * Per side-effect-free module lazy barrel state (experiments.lazyBarrel).
		 * @private
		 * @type {Map<Module, LazyBarrelState> | undefined}
		 */
		this._lazyBarrelModules = options.experiments.lazyBarrel
			? new Map()
			: undefined;
	}
	...
	_processModuleDependencies(module, callback) {
		...
		const lazyBarrelDeferred =
			this._lazyBarrelModules !== undefined &&
			this._classifyLazyBarrelDependencies(module);
				...
				try {
					...
					if (unsafeCachedModule !== undefined) {
						if (
							...
							if (this._lazyBarrelModules !== undefined) {
	...
	/**
	 * Classifies deferrable dependencies of a side-effect-free module (lazy barrel).
	 * @private
	...
	_classifyLazyBarrelDependencies(module) {
		const lazyBarrelModules =
			/** @type {Map<Module, LazyBarrelState>} */
			(this._lazyBarrelModules);
		const factoryMeta = module.factoryMeta;
		if (factoryMeta === undefined || !factoryMeta.sideEffectFree) {
			return false;
		}
		/** @type {LazyBarrelDependencies | undefined} */
		let info;
		for (const dep of module.dependencies) {
			const until = dep.getLazyUntil();
			if (until === null) continue;
			if (info === undefined) info = new LazyBarrelDependencies();
			if (typeof until === "object" && "local" in until) {
				info.addTerminal(until.local);
				continue;
			}
			const resourceIdent = dep.getResourceIdentifier();
			if (resourceIdent === undefined || resourceIdent === null) continue;
			const factory = this.dependencyFactories.get(
				/** @type {DependencyConstructor} */ (dep.constructor)
			);
			if (factory === undefined) continue;
			const category = dep.category;
			// must match the request grouping key of processDependencyForResolving
			const requestKey =
				category === esmDependencyCategory
					? resourceIdent
					: `${category}${resourceIdent}`;
			info.addLazy(requestKey, dep, factory, dep.getContext());
			if (typeof until === "object") info.addForwardId(until.id, requestKey);
			else if (until === "fallback") info.addFallback(requestKey);
		}
		...
		if (state !== undefined) {
			// replay every name requested before classification
			state.dependencies = info;
			info.takeRequested(state.requestedIds);
		} else {
	...
	/**
	 * Takes the deferred dependency groups of a lazy barrel that are needed to
	 * provide the requested export names. Accumulates the request when the
	 * module's dependencies were not classified yet.
	 * @private
	 ...
	 */
	_takeLazyBarrelItems(module, forwardedIds) {
		...
		if (state === undefined) {
			...
			if (factoryMeta !== undefined && factoryMeta.sideEffectFree) {
				lazyBarrelModules.set(module, {
					requestedIds: forwardedIds,
					dependencies: undefined
				});
			}
			return;
		}
		if (state.requestedIds !== true) {
			if (forwardedIds === true) state.requestedIds = true;
			else for (const id of forwardedIds) state.requestedIds.add(id);
		}
		const info = state.dependencies;
		// pending requests are replayed during classification
		if (info === undefined) return;
		const groups = info.takeRequested(forwardedIds);
	...
	_unlazyBarrelForDependency(module, dependency, sortedDependencies) {
		...
		if (id === null) return;
		const items = this._takeLazyBarrelItems(
			module,
			id === true ? true : new Set([id])
		);
	...
	_handleNewModuleFromUnsafeCache(originModule, dependency, module, callback) {
		...
		this._handleModuleBuildAndDependencies(
			...
			false,
			this._lazyBarrelModules !== undefined
				? LazyBarrelDependencies.getForwardedIds([dependency])
				: undefined,
			callback
		);
	}
	...
	handleModuleCreation(
		...
		const forwardedIds =
			this._lazyBarrelModules !== undefined
				? LazyBarrelDependencies.getForwardedIds(dependencies)
				: undefined;
	 ...
	 * @param {Set<string> | true | undefined} forwardedIds export names the creating dependencies request (lazy barrel)
	 * @param {ModuleCallback} callback callback
	 * @returns {void}
	 */
	_handleModuleBuildAndDependencies(
		...
		this.buildModule(module, (err) => {
			...
			// lazy barrel: take deferred groups requested by the creating dependencies
			const lazyItems =
				forwardedIds !== undefined
					? this._takeLazyBarrelItems(module, forwardedIds)
					: undefined;
			if (lazyItems !== undefined) {
				// process the taken groups in parallel to the module's own dependencies
				let inProgress = lazyItems.length + 1;
				let errored = false;
				/**
