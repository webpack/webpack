...
/** @typedef {{ chunk: Chunk, load: () => EXPECTED_ANY, create: (loaded: EXPECTED_ANY) => RuntimeModule, chunkGraph: ChunkGraph }} PendingRuntimeModule */
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
			...
			this.hooks.optimizeChunkModules.callAsync(
				...
				(err) => {
					...
					this.codeGeneration((err) => {
						...
						this._attachPendingRuntimeModules(() => {
	 ...
	 * @param {() => EXPECTED_ANY} load loads the runtime module implementation
	...
	/**
	 * Builds and attaches everything `addLazyRuntimeModule` queued. The only
	 * place a deferred runtime module implementation is loaded, and the seam an
	 * `import()`-based loader would await.
	 * @param {() => void} callback signals when the call finishes
	 * @returns {void}
	 */
	_attachPendingRuntimeModules(callback) {
		...
		if (pending !== undefined) {
			this._pendingRuntimeModules = undefined;
			for (const { chunk, load, create, chunkGraph } of pending) {
				this.addRuntimeModule(chunk, create(load()), chunkGraph);
			}
		}
		callback();
	}
