...
/**
 * Context for the `renderChunkAsset` hook: a rendered chunk asset whose hash is now
 * known but which has not yet been cached or emitted. Taps may return a transformed
 * source to resolve post-hash references (e.g. analyzable ESM chunk filenames) for
 * any output format (JavaScript, CSS, HTML).
 * @typedef {object} RenderChunkAssetContext
 * @property {Chunk} chunk the chunk the asset belongs to
 * @property {string} filename the final (post-hash) asset filename
 * @property {AssetInfo} assetInfo the asset info
 */
...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		this.hooks = Object.freeze({
			...
			/** @type {SyncWaterfallHook<[Source, RenderChunkAssetContext]>} */
			renderChunkAsset: new SyncWaterfallHook([
				"source",
				"renderChunkAssetContext"
			]),
						...
						assetCacheItem.get((err, sourceFromCache) => {
							...
							try {
								...
								if (alreadyWritten !== undefined) {
									...
									// The hash is now known: let plugins resolve post-hash
									// references (e.g. analyzable ESM chunk filenames) before the
									// source is cached and emitted. Cached under the chunk hash, so
									// a change to a referenced hash re-renders through here.
									source = this.hooks.renderChunkAsset.call(source, {
										chunk,
										filename: file,
										assetInfo
									});
