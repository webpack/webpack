...
/** @typedef {[string, import("webpack-sources").RawSourceMap | undefined]} ProcessContentEntry */
...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		this.hooks = Object.freeze({
			...
			/** @type {HookMap<AsyncSeriesWaterfallHook<[ProcessContentEntry, string]>>} */
			processContent: new HookMap(
				() => new AsyncSeriesWaterfallHook(["content", "name"])
			),
	...
	_codeGenerationModule(
		...
		cache.get((err, cachedResult) => {
			...
			const handleError = (err) => {
				...
				finish({ sources: new Map(), runtimeRequirements: null });
			};
