...
const { MultiItemCache } = require("./cache/CacheFacade");
const ErrorHelpers = require("./errors/ErrorHelpers");
const {
	makeWebpackError,
...
const ChunkGraph = require("./graph/ChunkGraph");
const ChunkGroup = require("./graph/ChunkGroup");
const Dependency = require("./graph/Dependency");
const Entrypoint = require("./graph/Entrypoint");
const ModuleGraph = require("./graph/ModuleGraph");
const buildChunkGraph = require("./graph/buildChunkGraph");
const { LogType, Logger } = require("./logging/Logger");
const CodeGenerationResults = require("./module/CodeGenerationResults");
const Module = require("./module/Module");
const ModuleProfile = require("./module/ModuleProfile");
const { WEBPACK_MODULE_TYPE_RUNTIME } = require("./module/ModuleTypeConstants");
const LazyBarrelController = require("./optimize/LazyBarrel");
...
const ChunkTemplate = require("./template/ChunkTemplate");
const DependencyTemplates = require("./template/DependencyTemplates");
const MainTemplate = require("./template/MainTemplate");
const ModuleTemplate = require("./template/ModuleTemplate");
const RuntimeTemplate = require("./template/RuntimeTemplate");
...
const getModuleStoreError = memoize(() => require("./errors/ModuleStoreError"));

const getModuleRestoreError = memoize(() =>
	require("./errors/ModuleRestoreError")
);
...
const getModuleHashingError = memoize(() =>
	require("./errors/ModuleHashingError")
);
...
const getCodeGenerationError = memoize(() =>
	require("./errors/CodeGenerationError")
);

const getChunkRenderError = memoize(() => require("./errors/ChunkRenderError"));

const getBuildCycleError = memoize(() => require("./errors/BuildCycleError"));
...
/**
 * @import {
 * 	OutputNormalized as OutputOptions,
 * 	HashFunction,
 * 	HashDigest,
 * 	HashDigestLength,
 * 	StatsOptions,
 ...
 * 	OutputNormalizedWithDefaults as OutputOptionsWithDefaults
 * } from "./config/defaults"
 */
/** @import AsyncDependenciesBlock from "./graph/AsyncDependenciesBlock" */
/** @import Cache from "./cache/Cache" */
/** @import CacheFacade from "./cache/CacheFacade" */
/** @import { ChunkName, ChunkId } from "./graph/Chunk" */
/** @import { ChunkGroupOptions } from "./graph/ChunkGroup" */
/**
 * @import Compiler, {
 * 	CompilationParams,
 * 	MemCache,
 ...
 * } from "./Compiler"
 */
/** @import DependenciesBlock from "./graph/DependenciesBlock" */
/** @import { DependencyLocation, ReferencedExports } from "./graph/Dependency" */
/** @typedef {import("./graph/Entrypoint").EntryOptions} EntryOptions */
/**
 * @import {
 * 	NameForCondition,
 * 	BuildInfo,
...
/** @import RequestShortener from "./util/RequestShortener" */
/** @import RuntimeModule from "./runtime/RuntimeModule" */
/** @import { RenderManifestEntry, RenderManifestOptions } from "./template/Template" */

/**
 * One language's source embedded in another's output.
 * @typedef {object} EmbeddedSourceInfo
 * @property {SourceType} type the embedded source's type, e.g. `"css"` / `"html"`
 * @property {SourceType} hostType the type of the source it is embedded in, e.g. `"javascript"`
 * @property {Module} module the module being generated
 * @property {string=} as which of that language's productions this is, where more than one can be embedded — a `style=""` holds `"block-contents"`, an inline `<style>` a whole `"stylesheet"`. Absent where the language has only one
 */
/**
 * @import {
 * 	StatsAsset,
 * 	StatsError,
 * 	StatsModule
 * } from "./stats/DefaultStatsFactoryPlugin"
 */
...
/** @typedef {((id: string) => ExecuteModuleExports) & { i?: ((options: ExecuteOptions) => void)[], c?: Record<string, ExecuteModuleObject>, p?: string }} WebpackRequire */
...
const deprecatedNormalModuleLoaderHook = util.deprecate(
	 ...
	 */
	(compilation) =>
		require("./module/NormalModule").getCompilationHooks(compilation).loader,
...
/** @typedef {{ chunk: Chunk, load: () => Promise<EXPECTED_ANY>, create: (loaded: EXPECTED_ANY) => RuntimeModule, chunkGraph: ChunkGraph }} PendingRuntimeModule */
...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		this.hooks = Object.freeze({
			...
			// TODO in the next major release: deprecate
			/** @deprecated */
			additionalAssets:
			 ...
			 * @type {AsyncSeriesWaterfallHook<[Source, EmbeddedSourceInfo]>}
			 * @since 5.110.0
			 */
			renderEmbeddedSource: new AsyncSeriesWaterfallHook(["source", "info"]),
			/**
			 ...
			 * @type {SyncHook<[Module, Hash]>}
			 * @since 5.110.0
			 */
			embeddedSourceHash: new SyncHook(["module", "hash"]),
		...
		/**
		 * What `renderEmbeddedSource` returned, by module and by language pair and
		 * text. Never shared between modules: a returned source may carry a map,
		 * which names the module it came from.
		 * @type {Map<Module, Map<string, Source>>}
		 * @private
		 */
		this._embeddedSources = new Map();
		/**
		 * Embedded source seen this pass that the hook has not been run over yet.
		 * @type {Map<Module, Map<string, { source: Source, info: EmbeddedSourceInfo, jobModule: Module }>>}
		 * @private
		 */
		this._pendingEmbeddedSources = new Map();
		/**
		 * The module whose code generation is currently running — the outer one
		 * when a generator runs nested, as `ConcatenatedModule` does.
		...
		/** @type {Error[]} */
		this.warnings = [];
		// Hints reported with `performance.hints: "stats"` — surfaced in stats
		// only, so they neither fail the build nor count as warnings.
		/** @type {Error[]} */
		this.hints = [];
		/** @type {Compilation[]} */
		this.children = [];
		...
		/** @type {PendingRuntimeModule[] | undefined} */
		this._pendingRuntimeModules = undefined;
	...
	_addModule(module, callback) {
		...
		this._modulesCache.get(identifier, null, (err, cacheModule) => {
			if (err) return callback(new (getModuleRestoreError())(module, err));
						...
						this._modulesCache.store(
							...
							(err) => {
								...
								if (err) {
									...
									return callback(new (getModuleStoreError())(module, err));
								}
	...
	_processModuleDependencies(module, callback) {
		...
		const processDependencyForResolving = (dep) => {
			...
			if (resourceIdent !== undefined && resourceIdent !== null) {
				...
				const cacheKey =
					category === Dependency.ESM_CATEGORY
						? resourceIdent
				...
				for (const item of set) {
					...
					if (blockReasons !== undefined) {
						for (const m of blockReasons) {
							if (m === module) {
								return callback(new (getBuildCycleError())(module));
							}
							set.add(m);
						}
					}
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
			...
			for (const d of dependencies) {
				...
				if (warnings) {
					for (const w of warnings) {
						...
						const warning = new (getModuleDependencyWarning())(
							module,
							w,
				...
				if (errors) {
					for (const e of errors) {
						const error = new (getModuleDependencyError())(
							module,
							e,
	...
	/**
	 * Offer source one language embeds in another to `renderEmbeddedSource`. The
	 * hook is async and generation is not, so the first pass returns the source
	 * untouched and records it; `_resolveEmbeddedSources` awaits the hook and
	 * generates those modules again, when the resolved source is waiting here.
	 * @param {Source} source the embedded source
	 * @param {EmbeddedSourceInfo} info what it is and where it is going
	 * @returns {Source} the resolved source, or `source` until it is resolved
	 */
	_resolveEmbeddedSource(source, info) {
		// Untapped, nothing can change: skip reading the text and the second pass.
		if (this.hooks.renderEmbeddedSource.taps.length === 0) return source;
		...
		getOrInsert(this._pendingEmbeddedSources, info.module, () => new Map()).set(
			key,
			{ source, info, jobModule: this._generatingModule || info.module }
		);
		return source;
	}

	/**
	 * Run `renderEmbeddedSource` over everything the last pass recorded, then
	 * generate those modules again so the resolved source is the one embedded.
	 * @private
	 * @param {CodeGenerationJobs} jobs the jobs the last pass ran
	 * @param {Callback} callback callback
	 * @returns {void}
	 */
	_resolveEmbeddedSources(jobs, callback) {
		if (this._pendingEmbeddedSources.size === 0) return callback();
		/** @type {{ key: string, source: Source, info: EmbeddedSourceInfo, jobModule: Module }[]} */
		const pending = [];
		for (const byKey of this._pendingEmbeddedSources.values()) {
			for (const [key, entry] of byKey) {
				pending.push({ key, ...entry });
			}
		}
		this._pendingEmbeddedSources.clear();
		asyncLib.each(
			pending,
			({ key, source, info }, cb) => {
				this.hooks.renderEmbeddedSource.callAsync(
					source,
					info,
					/**
					 * @param {(Error | null)=} err error
					 * @param {Source=} result the resolved source
					 * @returns {void}
					 */
					(err, result) => {
						if (err) return cb(err);
						getOrInsert(
							this._embeddedSources,
							info.module,
							() => new Map()
			...
			(err) => {
				if (err) return callback(/** @type {WebpackError} */ (err));
				const affected = new Set(pending.map(({ jobModule }) => jobModule));
				// A module referencing one of these may have copied what it generated
				// into its own code (an analyzable `new URL`), so it goes again after.
				/** @type {Set<Module>} */
				const referencing = new Set();
				for (const { info } of pending) {
					for (const connection of this.moduleGraph.getIncomingConnections(
						info.module
					)) {
						const { originModule } = connection;
						if (originModule !== null && !affected.has(originModule)) {
							referencing.add(originModule);
						}
					}
				}
				...
				const redo = [
					...jobs.filter(({ module }) => affected.has(module)),
					// A referencing module concatenated into another is generated as
					// part of it, so the job to redo is the concatenated module's.
					...jobs.filter(
						({ module }) =>
							!affected.has(module) &&
							((module instanceof ConcatenatedModule &&
								module.modules.some((inner) => referencing.has(inner))) ||
								referencing.has(module))
					)
				];
				// Generated directly rather than through the codegen cache: the module
				// hash is unchanged, so the cache would hand back the first pass again.
				const results =
					/** @type {CodeGenerationResults} */
					(this.codeGenerationResults);
				for (const { module, runtime, runtimes } of redo) {
					...
					for (const r of runtimes) results.add(module, r, result);
				}
				// A tap that changed the source again is resolved on the next round.
				this._resolveEmbeddedSources(redo, callback);
			}
		);
	}
	...
	_runCodeGenerationJobs(jobs, callback) {
		...
		// `jobs` is reassigned per iteration; embedded source is resolved over all.
		// Untapped nothing is recorded, so the array is not held past its iteration.
		const allJobs =
			this.hooks.renderEmbeddedSource.taps.length === 0 ? undefined : jobs;
		...
		const runIteration = () => {
			...
			asyncLib.eachLimit(
				...
				(err) => {
					...
					this._resolveEmbeddedSources(allJobs || jobs, (err) => {
						this._embeddedSources.clear();
						this._pendingEmbeddedSources.clear();
						callback(err);
					});
				}
	 ...
	 * @param {() => Promise<EXPECTED_ANY>} load resolves the runtime module implementation
	 * @param {(loaded: EXPECTED_ANY) => RuntimeModule} create builds the runtime module from it
	 * @param {ChunkGraph} chunkGraph the chunk graph
	 * @returns {void}
	 */
	addLazyRuntimeModule(chunk, load, create, chunkGraph = this.chunkGraph) {
		if (this._pendingRuntimeModules === undefined) {
			this._pendingRuntimeModules = [];
		}
		this._pendingRuntimeModules.push({ chunk, load, create, chunkGraph });
	}

	/**
	 * Awaits and attaches everything `addLazyRuntimeModule` queued. The only
	 * place a deferred runtime module implementation is loaded.
	 * @param {Callback} callback signals when the call finishes
	 * @returns {void}
	 */
	_attachPendingRuntimeModules(callback) {
		const pending = this._pendingRuntimeModules;
		if (pending === undefined) return callback(null);
		this._pendingRuntimeModules = undefined;
		Promise.all(pending.map(({ load }) => load()))
			.then((loaded) => {
				for (let i = 0; i < pending.length; i++) {
					const { chunk, create, chunkGraph } = pending[i];
					this.addRuntimeModule(chunk, create(loaded[i]), chunkGraph);
				}
			})
			// a `runtimeModule` tap may have queued more
			.then(() => this._attachPendingRuntimeModules(callback), callback);
	}

	// TODO in the next major release: make chunkGraph argument non-optional
	/**
	 * Adds runtime module.
	 * @param {Chunk} chunk target chunk
	...
	sortItemsWithChunkIds() {
		...
		// `hints` is sorted where stats read it — producers tap `afterSeal` and
		// `afterEmit`, so they append after this point.
	...
	_createModuleHash(
		...
		try {
			...
			// A diagnostic that already names its module says more than this wrapper
			// would, e.g. which runtime module's `generate()` threw.
			errors.push(
				err instanceof WebpackError && err.module
					? err
...
This prevents using hashes of each other and should be avoided.`);
				...
				asyncLib.each(
					...
					(fileManifest, callback) => {
						...
						assetCacheItem.get((err, sourceFromCache) => {
							/** @type {string | import("./template/TemplatedPathPlugin").TemplatePathFn<EXPECTED_ANY>} */
	 ...
	 * @param {string | import("./template/TemplatedPathPlugin").TemplatePathFn<T>} filename used to get asset path with hash
	...
	executeModule(module, options, callback) {
		...
		processAsyncTree(
			...
			(err) => {
				...
				chunkGraph.buildTimeExecution = true;
				...
				asyncLib.eachLimit(modules, 10, codeGen, (err) => {
					...
					// TODO in the next major release: drop it
