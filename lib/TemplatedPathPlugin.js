 ...
 * @property {HookMap<SyncWaterfallHook<[string], string>>} assetInfoHash
 */
...
const hashLength = (hooks, replacer, handler, assetInfo, hashName) => {
	...
	const fn = (match, arg, input) => {
		...
		if (assetInfo) {
			...
			result = hooks.assetInfoHash.for(hashName).call(result);
...
class TemplatedPathPlugin {
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				assetInfoHash: new HookMap(() => new SyncWaterfallHook(["result"]))
			};
