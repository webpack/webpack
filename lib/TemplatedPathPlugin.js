...
const { HookMap, SyncWaterfallHook } = require("tapable");
 ...
 * @property {HookMap<SyncWaterfallHook<[number | undefined], number | undefined>>} hashLength
 */
 ...
 */
const hashLength = (hooks, replacer, handler, assetInfo, hashName) => {
	/** @type {Replacer} */
	const fn = (match, arg, input) => {
		/** @type {string} */
		let result;
		let length = arg ? Number.parseInt(arg, 10) : undefined;
		length = hooks.hashLength.for(hashName).call(length);
 ...
 */
const replacePathVariables = (hooks, path, data, assetInfo) => {
...
class TemplatedPathPlugin {
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				hashLength: new HookMap(() => new SyncWaterfallHook(["result"]))
			};
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(plugin, (compilation) => {
			const hooks = TemplatedPathPlugin.getCompilationHooks(compilation);
			compilation.hooks.assetPath.tap(plugin, (path, options, assetInfo) =>
				replacePathVariables(hooks, path, options, assetInfo)
			);
		});
	}
}
