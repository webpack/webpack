...
const hashLength = (hooks, replacer, handler, assetInfo, hashName) => {
	...
	const fn = (match, arg, input) => {
		...
		if (assetInfo) {
			...
			result = hooks.assetInfoHash.call(result, hashName);
 ...
 * @property {SyncWaterfallHook<[string, string]>} assetInfoHash
 */
...
class TemplatedPathPlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(plugin, compilation => {
			...
			compilation.hooks.assetPath.tap(plugin, (path, data, assetInfo) =>
				replacePathVariables(hooks, path, data, assetInfo)
			);
		});
	}
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				assetInfoHash: new SyncWaterfallHook(["result", "hashName"])
			};
