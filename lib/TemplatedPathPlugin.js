...
import {
	decode as decodeBase,
	encode as encodeBase
} from "./util/hash/hash-digest.js";
...
/** @typedef {import("./Compilation.js").HashWithDigestFunction} HashWithDigestFunction */
...
const getPresentKinds = (path) => {
	...
	/** @type {Set<string>} */
	const kinds = new Set();
	for (const m of path.matchAll(REGEXP)) {
...
const prepareId = (id) => {
	...
	return id.replaceAll(/(^[.-]|[^a-z0-9_-])+/gi, "_");
};
	...
	const fn = (match, arg, input, digest) => {
		...
		if (assetInfo) {
			...
			if (digest && recordDigest) {
				...
				(assetInfo.contenthashDigest ||= {})[result] =
					digest === "base64safe" ? "base64url" : digest;
			}
...
const interpolate = (path, data, assetInfo) => {
	...
	path = path.replaceAll(REGEXP, (match, content) => {
...
export default TemplatedPathPlugin;
export { getPresentKinds };
export { interpolate };
export { reEncodeDigest };
...
export { TemplatedPathPlugin as "module.exports" };
