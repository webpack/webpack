...
// In `[fullhash:<arg>]` a non-numeric argument names a digest, a numeric one just
// a length. A digest re-encodes the hash rather than reading it, so no stand-in
// can stand for it and the settled value must be inlined.
...
/**
 * Maps a `[path]`/`[file]` value into the output directory: an absolute root or
 * a leading `..` becomes `_` (as `file-loader` did), so a module resolved
 * outside of `context` can't emit its asset outside of `output.path`.
 * @param {string} path relative path of the source file
 * @returns {string} path that stays inside the output directory
 */
const toContainedPath = (path) => {
	if (!path) return path;
	// A resource on another Windows drive stays absolute and win32-separated
	// when made relative to the context.
	const normalized = posix.normalize(
		WINDOWS_ABS_PATH_REGEXP.test(path)
			? path.replace(WINDOWS_PATH_SEPARATOR_REGEXP, "/")
			: path
	);
	return normalized
		.replace(ABSOLUTE_PATH_REGEXP, "_/")
		.replace(PARENT_SEGMENTS_REGEXP, (match) => "_/".repeat(match.length / 3));
};

/**
 * Source file name a filename template sees. `[path]`/`[file]` of a module
 * resolved outside of `context` start with `..` and would emit the asset
 * outside of `output.path`, so they are contained like
 * `[containedpath]`/`[containedfile]` with `experiments.futureDefaults`.
 * @param {string} sourceFilename context-relative path of the source file
 * @param {Compilation} compilation compilation
 * @returns {string} source file name for the filename template
 */
const toTemplateSourceFileName = (sourceFilename, compilation) =>
	// TODO webpack 6: remove the check, always contain
	compilation.options.experiments.futureDefaults
		? toContainedPath(sourceFilename)
		: sourceFilename;
...
const interpolate = (path, data, assetInfo) => {
	...
	// Build replacers only for the placeholders a template uses, since most use a
	// handful. Construction has no side effects — those fire on invocation — so
	...
	// Filename context. Of `/some/path/file.js?query#fragment`, `[file]` is the
	// path without query or fragment, `[base]` its basename, `[path]` its
	// directory, `[name]`/`[ext]` its halves; `contained*` stay under `output.path`.
	if (
		...
		(presentKinds.has("file") ||
			...
			presentKinds.has("containedfile") ||
			presentKinds.has("containedpath"))
	) {
		...
		if (match) {
			...
			if (presentKinds.has("containedfile")) {
				replacements.set("containedfile", emptyReplacer);
			}
			if (presentKinds.has("containedpath")) {
				replacements.set("containedpath", emptyReplacer);
			}
			...
			if (presentKinds.has("containedfile")) {
				replacements.set("containedfile", replacer(toContainedPath(file)));
			}
			if (presentKinds.has("containedpath")) {
				replacements.set(
					"containedpath",
					replacer(toContainedPath(path), true)
				);
			}
	...
	// Compilation context: `[fullhash]`, with `[hash]` as its legacy spelling.
	...
	// Chunk context: `[id]`, `[name]`, `[chunkhash]`, and `[contenthash]` for the
	// chunk's content hash of the requested type.
	if (data.chunk) {
	...
	// Module context: `[id]` and `[hash]`, with `[moduleid]` and `[modulehash]` as
	// their legacy spellings.
	if (data.module) {
	...
	// Everything else: `[url]`, and `[uniqueName]` from `output.uniqueName`, which
	// `[uniquename]` aliases.
...
TemplatedPathPlugin.getPresentKinds = getPresentKinds;
TemplatedPathPlugin.interpolate = interpolate;
TemplatedPathPlugin.reEncodeDigest = reEncodeDigest;
TemplatedPathPlugin.toContainedPath = toContainedPath;
TemplatedPathPlugin.toTemplateSourceFileName = toTemplateSourceFileName;
TemplatedPathPlugin.usesFullHashDigest = usesFullHashDigest;
