...
const replacePathVariables = (path, data, assetInfo) => {
	...
	if (data.filename) {
		...
		if (typeof data.filename === "string") {
			...
			let obj = {};
			Object.keys(data.filename).forEach(assetId => {
				let filename = data.filename[assetId];
				let rep = getFilenameReplacementObj(filename);
				Object.keys(rep).forEach(fileKey => {
					if (!obj[fileKey]) {
						obj[fileKey] = {};
					}
					obj[fileKey][assetId] = rep[fileKey];
				});
			});

			Object.keys(obj).forEach(fileKey => {
				obj[fileKey] = `" + ${JSON.stringify(obj[fileKey])}[wasmModuleId] + "`;
			});
			replacementObj = obj;
		}
