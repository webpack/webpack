...
const { WINDOWS_ABS_PATH_REGEXP, parseResource } = require("./util/identifier");
...
// `-` is part of a placeholder kind (`[contained-path]`), not a separator.
const REGEXP = /\[\\*([\w:-]+)\\*\]/g;
...
const ABSOLUTE_ROOT_REGEXP = /^(?:[a-z]:\/|\/)/i;
...
const toContainedPath = (path) => {
	...
	const normalized = posix.normalize(
		WINDOWS_ABS_PATH_REGEXP.test(path) ? path.replace(/\\/g, "/") : path
	);
	return normalized
		.replace(ABSOLUTE_ROOT_REGEXP, "_/")
...
const interpolate = (path, data, assetInfo) => {
	...
	// [ext] - .js
	//
	// [contained-file] and [contained-path] are [file] and [path] kept inside
	// `output.path`
	if (
		...
		(presentKinds.has("file") ||
			...
			presentKinds.has("contained-file") ||
			presentKinds.has("contained-path"))
	) {
		...
		if (match) {
			...
			if (presentKinds.has("contained-file")) {
				replacements.set("contained-file", emptyReplacer);
			}
			if (presentKinds.has("contained-path")) {
				replacements.set("contained-path", emptyReplacer);
			}
			...
			if (presentKinds.has("contained-file")) {
				replacements.set("contained-file", replacer(toContainedPath(file)));
			}
			if (presentKinds.has("contained-path")) {
				replacements.set(
					"contained-path",
