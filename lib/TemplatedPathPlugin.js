...
const replacePathVariables = (path, data) => {
	...
	if (data.filename) {
		file.name = basename(data.filename).split(".")[0];
		file.basename = basename(data.filename);
		file.extname = extname(data.filename).slice(1);
	}
	...
	return (
		path
			...
			.replace(REGEXP_FILEBASE, getReplacer(file.basename))
			.replace(REGEXP_EXT, getReplacer(file.extname))
