	...
	if (
		...
		throw new Error(
			`Cannot use [chunkhash] or [contenthash] for chunk in '${path}' (use [hash] instead)${
				typeof data.noChunkHash === "string" ? ` (${data.noChunkHash})` : ""
			}`
		);
	}
		...
		compiler.hooks.compilation.tap("TemplatedPathPlugin", compilation => {
			...
			mainTemplate.hooks.globalHash.tap(
				...
				(chunk, paths) => {
					...
					if (REGEXP_HASH_FOR_TEST.test(publicPath)) {
						return "[hash] in publicPath";
					}
					if (REGEXP_CHUNKHASH_FOR_TEST.test(publicPath)) {
						return "[chunkhash] in publicPath";
					}
					if (REGEXP_CONTENTHASH_FOR_TEST.test(publicPath)) {
						return "[contenthash] in publicPath";
					}
					if (REGEXP_NAME_FOR_TEST.test(publicPath)) {
						return "[name] in publicPath";
					}
					if (REGEXP_HASH_FOR_TEST.test(chunkFilename)) {
						return "[hash] in output.chunkFilename";
					}
					if (REGEXP_HASH_FOR_TEST.test(paths.join("|"))) {
						return "[hash] somewhere";
					}
				}
			);
