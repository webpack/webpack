...
import webpackSchema from "../schemas/WebpackOptions.json" with { type: "json" };
 ...
 */
const getArguments = (
	schema = /** @type {Schema} */ (/** @type {unknown} */ (webpackSchema))
) => {
	/** @type {Flags} */
	const flags = {};
	...
	const pathToArgumentName = (input) =>
		input
			...
			.replaceAll(/-?[^\p{Uppercase_Letter}\p{Lowercase_Letter}\d]+/gu, "-")
			.toLowerCase();
