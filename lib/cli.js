 ...
 * @property {string} originPath the JSON pointer to the schema part
 */
...
const getArguments = (schema = webpackSchema) => {
	...
	const addFlag = (path, multiple) => {
		...
		if (
			...
			if (multiple) {
				throw new Error(
					`Conflicting schema for ${path[0].path} (${path[0].originPath}) with ${argConfig.type} type (array type must be before single item type)`
				);
			}
			return 0;
		}
	 ...
	 * @param {string} originPath the JSON pointer to the current schema
			...
			if (schemaPart.properties) {
				for (const property of Object.keys(schemaPart.properties)) {
					addedArguments += traverse(
						...
						fullPath,
						inArray,
						`${originPath}/properties/${property}`
					);
				}
			}
