...
const DependenciesBlock = require("../graph/DependenciesBlock");
...
/** @import { AllTypes } from "./ModuleSourceTypeConstants" */
/** @import FileSystemInfo from "../fs/FileSystemInfo" */
/** @import { ConnectionState } from "../graph/ModuleGraphConnection" */
/** @import { ModuleTypes } from "./ModuleTypeConstants" */
/** @import { OptimizationBailouts } from "../graph/ModuleGraph" */
...
// TODO in the next major release: compilation will be required in CodeGenerationContext
/**
 ...
 * @property {string} externalReexport native external reexport source and attributes
 * @property {string} exportsSource export definitions left out of a javascript module's source on demand (see `KnownMeta.onDemandExports`), to be placed before its body where it is wrapped
 * @property {string} exportsBindingSource export definitions left out on demand that read the module's own declarations, to be placed after its body where it is wrapped
 * @property {Record<string, string>} exportsFinalName final name of each used export of a concatenated module whose definitions were left out on demand
 ...
 * @typedef {object} BuildDiagnostics
 * @property {string[]=} optimizationBailouts what parser plugins reported as optimization bailouts, replayed into the module graph of every compilation that reuses the module
 * @property {string=} sideEffectStatement the statement whose side effect keeps the module in the bundle, as `type at location`
 * @property {number=} ineffectivePureAnnotations how many `#__PURE__` annotations sit where the parser does not read them
 * @property {number=} topLevelThis how many times the module reads `this` at its top level
 * @property {boolean=} usesEval true when the module calls `eval` directly
 ...
 * @property {boolean=} specNamespaceObject the module hands out a spec Module Namespace Exotic Object
 ...
 * @property {boolean=} isCircular true when the module is in a synchronous circular dependency chain
 * @property {number=} circularKind CircularKind bit flags from CircularModulesPlugin; unset when the plugin did not run
 ...
 * @property {BuildDiagnostics=} diagnostics what the build recorded for stats and the performance hints to report
 */
...
class Module extends DependenciesBlock {
	...
	constructor(type, context = null, layer = null) {
		...
		// TODO in the next major release: use an enum
		/** @type {boolean} */
	 ...
	 * @returns {boolean} true when the module has a module source
	 */
	hasModuleSource() {
		return false;
	}
	 ...
	 * rebuild so this compilation's plugin hooks apply.
	 * @param {NormalModuleFactory} normalModuleFactory the normal module factory
	 * @returns {void}
	 */
	_restoreParserAndGenerator(normalModuleFactory) {}

	/**
	 * Releases the cached parser between incremental rebuilds so it doesn't keep
	 * the previous compilation alive.
	 * @returns {void}
	 */
	_cleanupParserForCache() {}
...
makeSerializable(Module, "webpack/lib/module/Module");
registerLegacyRequest(Module, "webpack/lib/Module");
