 ...
 * @property {boolean=} moduleSource the module was built as the source phase representation of its resource
 */
...
class NormalModule extends Module {
	...
	constructor({
		...
		/** @type {NormalModuleCreateData['moduleSource']} */
	 ...
	 */
	identifier() {
		// A source phase build is a build of its own, so it must not be merged with
		// the evaluation build of the same request.
		if (this.moduleSource) {
			return `${this.type}|${this.request}|${this.layer}|source`;
		}
		if (this.layer === null) {
	 ...
	 */
	hasModuleSource() {
		return Boolean(this.moduleSource);
	}
	...
	updateCacheModule(module) {
		...
		this.moduleSource = m.moduleSource;
	}
