...
const getGenerator = memoize(() => require("./Generator"));

const getExtractSourceMap = memoize(() => require("./extractSourceMap"));

const getInvalidDependenciesModuleWarning = memoize(() =>
	require("../errors/InvalidDependenciesModuleWarning")
);

const getUnhandledSchemeError = memoize(() =>
...
const getModuleError = memoize(() => require("../errors/ModuleError"));

const getModuleBuildError = memoize(() =>
	require("../errors/ModuleBuildError")
);

// Drops the redundant string/buffer copy of a source, keeping its map: a
// parsed map is far more expensive to rebuild than a `toString`.
const CLEAR_SOURCE_COPIES = { maps: false, source: true };
...
/** @import Generator, { GenerateErrorFn } from "./Generator" */
 ...
 * @property {ImportAttributes=} importAttributes The import attributes.
...
/**
 * @template T
 * @typedef {NormalModuleLoaderContext<T> & import("../loaders/LoaderRunner").LoaderRunnerLoaderContext<T> & import("../loaders/LoaderPlugin").LoaderPluginLoaderContext & import("../hmr/HotModuleReplacementPlugin").HotModuleReplacementPluginLoaderContext} LoaderContext
 */
...
/**
 * Walks the `ESMImportSideEffectDependency` graph to decide how `rootMod`
 ...
 * @param {ModuleGraph} moduleGraph the module graph
 * @param {EXPECTED_ANY} SideEffectDep `ESMImportSideEffectDependency` constructor, resolved once at the entry
...
const createCompilationHooks = () => {
	...
	return {
		 ...
		 * @type {AsyncSeriesHook<[NormalModule]>}
		 * @since 5.59.0
		 */
		beforeParse: new AsyncSeriesHook(["module"]),
		/**
 ...
 * @property {ImportAttributes=} attributes import attributes
...
class NormalModule extends Module {
	...
	constructor({
		...
		/** @type {NormalModuleCreateData['attributes']} */
	...
	cleanupForCache() {
		...
		// TODO in the next major release: reconsider this
		if (this.buildInfo) {
	...
	_createLoaderContext(resolver, options, compilation, fs, hooks) {
		...
		const loaderContext = {
			...
			importAttributes: this.attributes,
			_module: this,
					...
					try {
						...
						if (
							...
							try {
								...
								this.addWarning(
									new (getModuleWarning())(/** @type {Error} */ (err))
								);
	...
	_initBuildHash(compilation) {
		...
		if (this._source) {
			...
			// WHY: hashing reads the bytes, so the source now holds its content
			// as a string and as a buffer. Dropping one is only a win where the
			// buffer is the copy: a map-carrying source keeps the buffer and
			// drops the string, which the generator then rebuilds.
			// Feature-detected, since `clearCache` landed in webpack-sources 3.5.
			if (
				!this.useSourceMap &&
				!this.useSimpleSourceMap &&
				typeof this._source.clearCache === "function"
			) {
				this._source.clearCache(CLEAR_SOURCE_COPIES);
			}
		}
		hash.update("meta");
	...
	build(options, compilation, resolver, fs, callback) {
		...
		return this._doBuild(options, compilation, resolver, fs, hooks, (err) => {
			...
			const handleParseError = (e) => {
				...
				const error = new (getModuleParseError())(
					source,
					e,
					loaders,
					this.type
			...
			hooks.beforeParse.callAsync(this, (err) => {
				if (err) {
					this.markModuleAsErrored(err);
					this._initBuildHash(compilation);
					return callback();
	...
	getSideEffectsConnectionState(moduleGraph) {
		...
		return walkSideEffects(this, moduleGraph, ESMImportSideEffectDependency);
	}
	...
	codeGeneration({
		...
		for (const type of sourceTypes || chunkGraph.getModuleSourceTypes(this)) {
			// TODO in the next major release: make generateError required
			const generator =
...
makeSerializable(NormalModule, "webpack/lib/module/NormalModule");
registerLegacyRequest(NormalModule, "webpack/lib/NormalModule");
