...
const { dirname, join, relative } = require("../fs/fs");
...
/** @import ResolverFactory, { ResolverWithOptions } from "../resolve/ResolverFactory" */
...
/** @import AsyncWebAssemblyParser from "../wasm/AsyncWebAssemblyParser" */
						...
						this.hooks.createModule.callAsync(
							...
							(err, createdModule) => {
								if (!createdModule) {
									...
									// TODO in the next major release: make it required and move javascript/wasm/asset properties to own module
			...
			(data, callback) => {
				...
				const continueCallback = needCalls(2, (err) => {
					...
					if (
						...
						// Discriminating on the source phase is the user declaring how the
						// source representation is built — a JavaScript module has no other.
						if (phase === "source") {
							matchedProperties = new Set();
						}
						...
						const applyCreateData = () => {
							try {
								Object.assign(data.createData, {
									...
									moduleSource:
										matchedProperties !== undefined &&
										matchedProperties.has("phase")
								});
							} catch (createDataErr) {
