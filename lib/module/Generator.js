...
const { contextifyStackFrames } = require("../errors/ErrorHelpers");
...
const getModuleParseError = memoize(() =>
	require("../errors/ModuleParseError")
);
...
class Generator {
	 ...
	 * @param {RequestShortener=} requestShortener request shortener, absolute paths are kept without one
	 * @returns {string} message to emit
	 */
	static buildErrorMessage(error, requestShortener) {
		return requestShortener
			? contextifyStackFrames(error.message, requestShortener)
			: error.message;
	}
	 ...
	 * @param {RequestShortener=} requestShortener request shortener for the stack the message carries
	 * @returns {string} javascript statement
	 */
	static throwBuildErrorCode(
		error,
		parseErrorConstructor = "SyntaxError",
		...
		const message = Generator.buildErrorMessage(error, requestShortener);

		return `throw new ${name}(${JSON.stringify(message)});`;
	}
