		...
		this.applyPluginsAsyncWaterfall("before-resolve", {
			...
			asyncLib.parallel([
				...
				this.applyPluginsAsyncWaterfall("after-resolve", {
					...
					return callback(null, new ContextModule(result.resolveDependencies, result.resource, result.recursive, result.regExp, result.loaders, result.async, dependency.chunkName, dependency));
				});
			});
		});
	}

	resolveDependencies(fs, resource, recursive, regExp, contextDependency, callback) {
		const cmf = this;
		if(!regExp || !resource)
		...
		(function addDirectory(directory, callback) {
			fs.readdir(directory, (err, files) => {
				if(err) return callback(err);
				files = cmf.applyPluginsWaterfall("context-module-files", files, contextDependency);
