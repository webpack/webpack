...
/** @import { RuleSet } from "./rules/RuleSetCompiler" */
...
class ContextModuleFactory extends ModuleFactory {
	 ...
	 * @param {RuleSet=} ruleSet the rule set modules created in a context are matched against
	 */
	constructor(resolverFactory, ruleSet) {
		super();
		...
		// splitting contexts by issuer costs deduplication, so it only happens when a rule asks for it
		/** @type {boolean} */
		this._keepIssuer = Boolean(
			ruleSet && ruleSet.conditionProperties.has("issuer")
		);
	}
		...
		this.hooks.beforeResolve.callAsync(
			{
				...
				issuer:
					this._keepIssuer && data.contextInfo.issuer
