...
const IssuerContextModule = require("./IssuerContextModule");
...
/** @import { ModuleOptionsNormalized as ModuleOptions, RuleSetRules } from "../declarations/WebpackOptions" */
...
/**
 * Checks whether a rule tree contains an issuer condition.
 * @param {RuleSetRules | undefined} rules rules to inspect
 * @returns {boolean} whether an issuer condition is present
 */
const hasIssuerCondition = (rules) => {
	if (!rules) return false;
	for (const rule of rules) {
		if (!rule || typeof rule !== "object") continue;
		if (rule.issuer !== undefined) return true;
		if (hasIssuerCondition(rule.rules) || hasIssuerCondition(rule.oneOf)) {
			return true;
		}
	}
	return false;
...
class ContextModuleFactory extends ModuleFactory {
	 ...
	 * @param {ModuleOptions=} moduleOptions module options
	 */
	constructor(resolverFactory, moduleOptions) {
		super();
		...
		/** @type {boolean} */
		this.issuerSensitive =
			moduleOptions !== undefined &&
			(hasIssuerCondition(moduleOptions.defaultRules) ||
				hasIssuerCondition(moduleOptions.rules));
	}
					...
					(err, result) => {
						...
						this.hooks.afterResolve.callAsync(
							...
							(err, result) => {
								...
								const ContextModuleClass = this.issuerSensitive
									? IssuerContextModule
									: ContextModule;
								const contextModuleOptions = this.issuerSensitive
									? { ...result, issuer: data.contextInfo.issuer }
									: result;
								return callback(null, {
									module: new ContextModuleClass(
