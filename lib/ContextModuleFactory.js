...
ContextModuleFactory.prototype.create = function(data, callback) {
	...
	this.applyPluginsAsyncWaterfall("before-resolve", {
		...
		let recursive = result.recursive;
		let regExp = result.regExp;
		let asyncContext = result.async;
		let dependencies = result.dependencies;

		let loaders, resource, loadersPrefix = "";
		let idx = request.lastIndexOf("!");
		if(idx >= 0) {
			loaders = request.substr(0, idx + 1);
			for(let i = 0; i < loaders.length && loaders[i] === "!"; i++) {
				loadersPrefix += "!";
			}
		...
		let resolvers = module.resolvers;
	...
	(function addDirectory(directory, callback) {
		fs.readdir(directory, function(err, files) {
			...
			asyncLib.map(files.filter(function(p) {
				...
				let subResource = path.join(directory, seqment);
					...
					if(stat.isDirectory()) {
						...
						this.applyPluginsAsyncWaterfall("alternatives", [obj], function(err, alternatives) {
							...
							alternatives = alternatives.filter(function(obj) {
								...
								let dep = new ContextElementDependency(obj.request);
								dep.optional = true;
								return dep;
							});
