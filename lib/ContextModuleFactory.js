...
class ContextModuleFactory extends ModuleFactory {
	...
	resolveDependencies(fs, options, callback) {
		...
		const isImportMetaGlob = Boolean(globPatterns && globContext);
			...
			fs.readdir(directory, (err, files) => {
				...
				asyncLib.map(
								...
								if (stat.isDirectory()) {
									...
									if (
										...
										if (
											exposedUserRequest &&
											!globDeps.some(
												(d) => d.userRequest === exposedUserRequest
											)
										) {
					...
					(err, result) => {
						...
						if (isImportMetaGlob) {
							for (const dep of globDeps) {
								if (
									!flattenedResult.some(
										(d) => d.userRequest === dep.userRequest
									)
								) {
		...
		const visitResource = (resource, callback) => {
			...
			const resolvedGlobPatterns =
				isImportMetaGlob && globPatterns && globContext
					? globPatterns.map((pattern) =>
							resolveContextModuleGlobPattern(pattern, globContext, resource)
						)
					: undefined;
