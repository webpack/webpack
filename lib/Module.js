...
/**
 * What a build recorded for the module graph to report as optimization bailouts, kept on the build info so a module restored from the cache still has it.
 * @typedef {object} OptimizationBailoutRecord
 * @property {string[]} reasons the bailouts as reported by the module graph
 * @property {string=} sideEffect the statement whose side effect keeps the module in the bundle, as `type at location`
 */
 ...
 * @property {OptimizationBailoutRecord=} optimizationBailout what the build recorded for the module graph to report as optimization bailouts, replayed into every compilation that reuses the module
 */
