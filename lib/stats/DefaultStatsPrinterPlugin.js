...
const AVAILABLE_FORMATS = {
	...
	formatError(message, colors) {
		// If the message is already colored, return it as is
		if (typeof message === "string" && message.includes("\u001B["))
			return message;

		const { green, yellow, red } = colors;

		// Handle object format (from our modified EXTRACT_ERROR)
		if (typeof message === "object" && message !== null) {
			let formattedMessage = message.message || "";

			// Include location if available
			if (message.loc) {
				formattedMessage = `${message.loc}: ${formattedMessage}`;
			}

			// Include module info if available
			if (message.moduleName) {
				formattedMessage = `Module ${message.moduleName}\n${formattedMessage}`;
			}

			// Format nested errors from AggregateError
			if (message.aggregatedErrors && message.aggregatedErrors.length > 0) {
				formattedMessage += "\nAggregated Errors:";
				for (let i = 0; i < message.aggregatedErrors.length; i++) {
					const err = message.aggregatedErrors[i];
					const nestedMessage = this.formatError(err, colors);
					formattedMessage += `\n  Error ${i + 1}: ${nestedMessage.replace(/\n/g, "\n  ")}`;
				}
			}

			// Format cause chain
			if (message.cause) {
				const causeMessage = this.formatError(message.cause, colors);
				formattedMessage += `\nCaused by: ${causeMessage.replace(/\n/g, "\n  ")}`;
			}

			// Apply color formatting to the generated message
			message = formattedMessage;
		}

		// Continue with the existing color formatting logic for strings
		const highlights = [
