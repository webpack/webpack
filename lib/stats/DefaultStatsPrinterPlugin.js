...
/** @import { ChunkId, ChunkName } from "../Chunk" */
/** @import Compiler from "../Compiler" */
/** @import { LogTypeEnum } from "../logging/Logger" */
/**
 * @import {
 * 	KnownStatsAsset,
 * 	KnownStatsChunk,
...
const SIMPLE_ITEMS_JOINER = {
	...
	"compilation.hints": itemsJoinMoreSpacing,
...
const SIMPLE_ELEMENT_JOINERS = {
	...
	"hints[].error": (items, { cyan }) =>
		`${cyan("HINT")} in ${joinExplicitNewLine(items, "")}`,
 ...
 * @typedef {T extends (firstArg: any, ...rest: infer R) => any ? R : never} TailParameters
 */
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			compilation.hooks.statsPrinter.tap(PLUGIN_NAME, (stats, options) => {
				...
				for (const simplePrinters of /** @type {Record<string, any>[]} */ ([
