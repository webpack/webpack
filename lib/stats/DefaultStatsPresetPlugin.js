...
/** @typedef {keyof (StatsOptions | NormalizedStatsOptions)} NormalizerKeys */
/** @typedef {{ [Key in NormalizerKeys]: (value: StatsOptions[Key]) => NormalizedStatsOptions[Key] }} Normalizers */
...
const NORMALIZER = {
	excludeModules: value => {
		if (!Array.isArray(value)) {
			value = value
				? /** @type {NormalizedStatsOptions["excludeModules"]} */ ([value])
				: [];
		}
	...
	logging: value => {
		...
		return /** @type {NormalizedStatsOptions["logging"]} */ (value);
	},
	loggingDebug: value => {
		if (!Array.isArray(value)) {
			value = value
				? /** @type {NormalizedStatsOptions["loggingDebug"]} */ ([value])
				: [];
		}
		return value.map(normalizeFilter);
	}
			...
			compilation.hooks.statsNormalize.tap(
				...
				(options, context) => {
					...
					for (const key of Object.keys(NORMALIZER)) {
						options[key] =
							/** @type {Normalizers[NormalizerKeys]} */
