...
/** @import StatsFactory, { StatsFactoryContext } from "./StatsFactory" */
/** @import Chunk, { ChunkId, ChunkName } from "../graph/Chunk" */
/** @import { ModuleId } from "../graph/ChunkGraph" */
/** @import ChunkGroup, { OriginRecord } from "../graph/ChunkGroup" */
/**
 * @import Compilation, {
 * 	Asset,
 * 	AssetInfo,
 ...
 */
/** @import Module, { NameForCondition, BuildInfo } from "../module/Module" */
 ...
 * @import {
 * 	AggregateError
 * } from "../serialization/AggregateErrorSerializer"
 */
/** @import { ErrorWithCause } from "../serialization/ErrorObjectSerializer" */
 ...
 * @template I, G
 * @typedef {import("./smartGrouping").GroupConfig<I, G>} GroupConfig
 */
...
const EXTRACT_ERROR = {
	_: (object, error, context, { requestShortener }) => {
		// TODO in the next major release: disallow strings in the errors/warnings list
...
const SIMPLE_EXTRACTORS = {
	compilation: {
		...
		hints: (object, compilation, context, options, factory) => {
			const { type } = context;
			// Sorted here rather than in `sortItemsWithChunkIds`, which runs before
			// the `afterSeal` and `afterEmit` producers have appended their hints.
			object.hints = factory.create(
				`${type}.hints`,
				[...compilation.hints].sort(compareErrors),
				context
			);
		},
		hintsCount: (object, compilation) => {
			object.hintsCount = countWithChildren(compilation, (c) => c.hints);
		},
	...
	chunkGroup: {
		...
		chunkGroupResourceHints: (
			...
			// An SSR framework reads this to inject the `<link>` tags into its own
			// HTML without walking the chunk graph. Only entrypoints carry hints, so
			// split-chunk and async groups are skipped.
	...
	module$visible: {
		_: (object, module, context, { requestShortener }, factory) => {
			...
			const { cacheable, diagnostics } = /** @type {BuildInfo} */ (
				module.buildInfo
			);
			const notCacheableReasons =
				diagnostics === undefined ? undefined : diagnostics.notCacheableReasons;
