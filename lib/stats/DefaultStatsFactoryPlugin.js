...
const EXTRACT_ERROR = {
	_: (object, error, context, { requestShortener }, factory) => {
		...
		if (typeof error === "string") {
			...
			if (error.errors && Array.isArray(error.errors)) {
				object.aggregatedErrors = error.errors.map(err => {
					const nestedError = {
						message: ""
					};

					// Create a properly structured statsOptions object
					const statsOptions =
						/** @type {import("../Compilation").NormalizedStatsOptions} */ ({
							requestShortener
						});
			...
			// Handle Error with cause
			if (error.cause) {
				// Initialize with required message property
				const causeError = { message: "" };

				// Create a properly structured statsOptions object for the cause error
				const causeStatsOptions =
					/** @type {import("../Compilation").NormalizedStatsOptions} */ ({
						requestShortener
					});
				...
				EXTRACT_ERROR._(
					causeError,
					/** @type {WebpackError} */ (error.cause),
					context,
					causeStatsOptions,
					factory
				);
				object.cause = causeError;
			}
		}
	},
