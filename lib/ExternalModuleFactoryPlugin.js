		...
		normalModuleFactory.hooks.factorize.tapAsync(
			...
			(data, callback) => {
				...
				const handleExternal = (value, type, callback) => {
					...
					// TODO make it pluggable/add hooks to `ExternalModule` to allow output own code?
					const depConstructor =
						/** @type {DepConstructor} */
						(dependency.constructor);
					/** @type {DependencyMeta | undefined} */
					let dependencyMeta;

					switch (depConstructor) {
						case CssImportDependency: {
							const { layer, supports, media } =
								/** @type {CssImportDependency} */ (dependency);
							dependencyMeta = { layer, supports, media };
						}
					}
