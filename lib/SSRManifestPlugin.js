 ...
 */
const getRequiredChunks = (chunk, chunkGraph, moduleGraph) => {
	/** @type {Set<Chunk>} */
	const chunks = new Set([chunk]);

	for (const current of chunks) {
		for (const module of chunkGraph.getChunkModulesIterable(current)) {
			for (const connection of moduleGraph.getOutgoingConnections(module)) {
				...
				for (const target of chunkGraph.getModuleChunksIterable(dependency)) {
					...
					if (!areLoadedTogether(target, current)) continue;
					chunks.add(target);
				}
			}
		}
			...
			compilation.hooks.processAssets.tap(
				...
				() => {
					...
					for (const chunk of compilation.chunks) {
						...
						/** @type {string[]} */
						const files = [];
						for (const required of getRequiredChunks(
							chunk,
							chunkGraph,
							moduleGraph
						)) {
