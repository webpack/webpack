...
class ProfilingPlugin {
	...
	apply(compiler) {
		...
		// Compiler Hooks
		for (const [hookName, hook] of getHooks(compiler.hooks)) {
			hook.intercept(makeInterceptorFor("Compiler", tracer)(hookName));
		}

		for (const [hookName, hook] of getHooks(compiler.resolverFactory.hooks)) {
...
const getHooks = instanceHooks => {
	const hooks = new Map();
	if (instanceHooks) {
		for (const hookName of Object.keys(instanceHooks)) {
			const { value } = Object.getOwnPropertyDescriptor(
				instanceHooks,
				hookName
			);
			if (value !== undefined) {
				hooks.set(hookName, value);
			}
		}
	}
...
const interceptTemplateInstancesFrom = (compilation, tracer) => {
	...
	[
		...
		for (const [hookName, hook] of getHooks(templateObject.instance.hooks)) {
			hook.intercept(makeInterceptorFor(templateObject.name, tracer)(hookName));
		}
	});
};

const interceptAllHooksFor = (instance, tracer, logLabel) => {
	for (const [hookName, hook] of getHooks(instance.hooks)) {
...
// 			if (
// 				Object.getOwnPropertyDescriptors(instance.hooks, hookName).value !==
// 				undefined
// 			) {
