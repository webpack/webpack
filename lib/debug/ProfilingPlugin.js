...
const { dirname, mkdirpSync } = require("../fs/fs");
const {
	CSS_MODULES,
	JAVASCRIPT_MODULES,
	JSON_MODULE_TYPE,
...
/**
 * @schema plugins/debug/ProfilingPlugin
 * @typedef {object} ProfilingPluginOptions
 * @property {AbsolutePath=} outputPath Path to the output file e.g. `path.resolve(__dirname, 'profiling/events.json')`. Defaults to `events.json`.
 */
	...
	stopProfiling() {
		return this.sendCommand("Profiler.stop").then(({ profile }) => {
			...
			if (profile.startTime < this._startTime || profile.endTime > endTime) {
				// `process.hrtime` and the inspector timestamps are each relative to an
				// unknown point in time, with no guarantee it is the same point, so the
				// two are rescaled onto one another here.
...
class ProfilingPlugin {
	...
	constructor(options = {}) {
		if (typeof options.outputPath === "string") {
			options.outputPath = fileUrlToPath(options.outputPath);
		}
...
ProfilingPlugin.Profiler = Profiler;
