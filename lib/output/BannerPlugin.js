...
/** @import { PathDataChunk } from "../Compilation" */
/** @import Compiler from "../Compiler" */
/** @import Chunk from "../graph/Chunk" */
/** @import { MatcherFn } from "../devtool/ModuleFilenameHelpers" */

/**
 * The banner as function, it will be wrapped in a comment.
 * @tsType import('./BannerPlugin').BannerFunction
 ...
 * @typedef {RegExp | NonEmptyString | import("../devtool/ModuleFilenameHelpers").MatcherFn} Rule
 */

/**
 * A rule condition.
 * @inline
 * @typedef {Rule} RuleCondition
 */

/**
 * Filtering rules.
 * @typedef {RuleCondition[] | Rule} Rules
 */

/**
 * @inline
 * @title BannerPluginOptions
 * @typedef {object} BannerPluginOptions
 * @property {string | BannerFunction} banner Specifies the banner.
 * @property {boolean=} entryOnly If true, the banner will only be added to the entry chunks.
 * @property {Rules=} exclude Exclude all modules matching any of these conditions.
 * @property {boolean=} footer If true, banner will be placed at the end of the output.
 * @property {Rules=} include Include all modules matching any of these conditions.
 * @property {boolean=} raw If true, banner will not be wrapped in a comment.
 * @property {number=} stage Specifies the stage when add a banner.
 * @property {Rules=} test Include all modules that pass test assertion.
 */

/**
 * The banner as string, it will be wrapped in a comment.
 * @inline
 * @typedef {NonEmptyString} BannerString
 */

/**
 * @schema plugins/BannerPlugin
 * @typedef {BannerString | BannerPluginOptions | BannerFunction} BannerPluginArgument
 */
	...
	apply(compiler) {
		compiler.hooks.validate.tap(PLUGIN_NAME, () => {
			compiler.validate(
				...
				(options) =>
					require("../../schemas/plugins/BannerPlugin.check")(options)
			);
		});
				...
				for (const chunk of compilation.chunks) {
					...
					for (const file of chunk.files) {
						...
						const comment = compilation.getPath(
							/** @type {string | import("../template/TemplatedPathPlugin").TemplatePathFn<PathDataChunk>} */
							(banner),
							data
						);
