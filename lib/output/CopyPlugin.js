...
const { PERMISSIONS_MASK, join } = require("../fs/fs");
...
const createCompilationHooks = () => ({
	 ...
	 * @since 5.112.0
	 */
	ignore: new SyncBailHook(["path"])
});

/**
 * @typedef {ReturnType<typeof createCompilationHooks>} CopyPluginCompilationHooks
 */
...
/** @import { Dirent, InputFileSystem, IStats } from "../fs/fs" */
...
/**
 * The one answer for a path. A followed symlink is asked about under its own
 * name and passed its real path too, because that is where it reads from.
 * @typedef {object} IgnoreMatcher
 * @property {(normalizedPath: string, normalizedRealPath?: string) => boolean} file whether a file is not copied
 * @property {(normalizedPath: string, normalizedRealPath?: string) => boolean} directory whether a directory is not walked at all
 */
...
/**
 * Answers whether the path is a symbolic link itself rather than what one points at.
 * @param {InputFileSystem} fs input file system
 * @param {string} filePath path
 * @returns {Promise<boolean>} true when it is a symbolic link
 */
const isSymbolicLink = (fs, filePath) =>
	new Promise((resolve, reject) => {
		// a file system without it reports what the link points at, which is the
		// behavior this answer then leaves in place
		if (!fs.lstat) {
			resolve(false);
			return;
		}
		fs.lstat(filePath, (err, stats) => {
			...
			resolve(/** @type {IStats} */ (stats).isSymbolicLink());
		});
	});
	...
	const results = await mapLimit(
		...
		async (entry) => {
			...
			if (isSymlink && !followSymlinks) {
				if (ignore.file(normalizedPath)) return [];
				links.add(filePath);
				return [filePath];
			}
			...
			// a followed link is read as what it points at, so that is asked about
			// alongside the name the walk reached it under
			const resolvedPath = isSymlink ? await realpath(fs, filePath) : undefined;
			const realPath =
				resolvedPath === undefined
					? undefined
					: normalizePathSeparatorsForPath(resolvedPath);
			if (stats.isFile()) {
				return ignore.file(normalizedPath, realPath) ? [] : [filePath];
			}
			...
			if (ignore.directory(normalizedPath, realPath)) return [];
...
const walkBase = async (options, base, maxDepth) => {
	...
	if (options.followSymlinks) {
		visited.add(await realpath(options.fs, base));
	}
	return walkDirectory(options, base, visited, maxDepth);
};
...
/**
 * Whether a path counts as the compilation's own output.
 * @callback InOutputPathMatcher
 * @param {string} normalizedPath path with `/` separators
 * @returns {boolean} true when the path is inside the output directory
 */

/**
 * @param {string} outputPath `output.path` with `/` separators
 * @returns {InOutputPathMatcher} what sits inside the output directory
 */
const createOutputPathMatcher = (outputPath) => {
	const inside = `${outputPath}/`;
	return (normalizedPath) =>
		normalizedPath === outputPath || normalizedPath.startsWith(inside);
};

/**
 * Joins everything which keeps a path from being copied into one answer, with the
 * `ignore` hook ahead of them all so a plugin can overrule any of them.
 * @param {CopyPluginCompilationHooks} hooks the compilation's copy hooks
 * @param {InOutputPathMatcher} isInOutputPath what sits inside the output directory
 * @param {IgnoreMatcher} ignore what `globOptions.ignore` does not copy
 * @returns {IgnoreMatcher} the one answer for a path
 */
const createIgnoreMatcher = (hooks, isInOutputPath, ignore) => {
	/**
	 * @param {(normalizedPath: string) => boolean} byGlobOptions what `globOptions.ignore` says
	 * @returns {(normalizedPath: string, normalizedRealPath?: string) => boolean} what the copy does
	 */
	const answer = (byGlobOptions) => (normalizedPath, normalizedRealPath) => {
		const decision = hooks.ignore.call(normalizedPath);
		if (decision !== undefined) return decision;
		return (
			byGlobOptions(normalizedPath) ||
			isInOutputPath(normalizedPath) ||
			// a copy reading its own output copies it one directory deeper on every
			// build, whichever name it reached the output directory under
			(normalizedRealPath !== undefined && isInOutputPath(normalizedRealPath))
		);
	};
	return { file: answer(ignore.file), directory: answer(ignore.directory) };
};
...
class CopyPlugin {
	...
	async _copyPattern(compiler, compilation, logger, cache, item, index) {
		...
		// a copy reading its own output copies it one directory deeper on every
		// build, which invalidates the watch and grows the output without bound
		const ignore = createIgnoreMatcher(
			CopyPlugin.getCompilationHooks(compilation),
			createOutputPathMatcher(
				normalizePathSeparatorsForPath(compiler.outputPath)
			),
			this._createIgnoreMatcher(context, globOptions.ignore, matchOptions)
		);
		/** @type {Set<string>} */
		const links = new Set();
	...
	async _resolveFrom(
		...
		// `stat` follows the link, so a `from` naming one is classified as what it
		// points at — a directory to walk into, or a file whose bytes to read —
		// where a `followSymlinks: false` copies the link itself instead
		const namesLink =
			!followSymlinks && (await isSymbolicLink(fs, absoluteFrom));
		const stats = await stat(fs, absoluteFrom);
		/** @type {TypeOfFrom} */
		const typeOfFrom = namesLink
			? "file"
			: !stats
				? "glob"
				: stats.isDirectory()
		...
		// `stat` followed the link to call this a file, and a base can be a link of
		// its own, so the name the pattern used says nothing about what it reaches
		const realTarget = followSymlinks
			? normalizePathSeparatorsForPath(
					await realpath(fs, typeOfFrom === "file" ? absoluteFrom : base)
				)
			: undefined;
		// asked once for the whole pattern, so the walk and the watch agree on it
		const ignored =
			typeOfFrom === "file"
				? ignore.file(normalizePathSeparatorsForPath(absoluteFrom), realTarget)
				: ignore.directory(normalizedBase, realTarget);
		const entries = ignored
			...
			: typeOfFrom === "file"
				...
				: (
						...
						globMatchNormalizedWithExplicitDot(
							glob,
							normalizePathSeparatorsForPath(filePath),
							normalizedBase,
							matchOptions
						)
					);

		// what the link says is what is copied, the same as one the walk reaches
		if (namesLink && entries.length > 0) links.add(absoluteFrom);

		// a base which copies nothing is not watched either, or the compilation
		// would rebuild on what it wrote there itself
		if (!ignored) {
...
CopyPlugin.getCompilationHooks = createHooksRegistry(createCompilationHooks);
