...
/**
 * Whether a path is `output.path` or sits below it.
 * @callback InOutputPathMatcher
 * @param {string} normalizedPath path with `/` separators
 * @returns {boolean} true when the path is the output directory or inside it
 */

/**
 * @param {string} outputPath `output.path` with `/` separators
 * @returns {InOutputPathMatcher} what is inside the output directory
 */
const createInOutputPathMatcher = (outputPath) => {
