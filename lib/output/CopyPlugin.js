...
class CopyPlugin {
	...
	async _resolveFrom(
		...
		const entries =
			typeOfFrom === "file"
				? ignore.file(normalizePathSeparatorsForPath(absoluteFrom)) ||
					// `stat` followed the link to call this a file, so the name the
					// pattern used says nothing about what it reaches
					(followSymlinks &&
						(await resolvesIntoOutputPath(fs, absoluteFrom, isInOutputPath)))
					? []
					: [absoluteFrom]
				: (
						await walkBase(
		...
		// only rebuild on what this compilation itself wrote there
		if (!isInOutputPath(normalizedBase)) {
