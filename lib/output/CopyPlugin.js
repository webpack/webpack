...
const walkDirectory = async (options, directory, visited, maxDepth) => {
	...
	const results = await mapLimit(
		...
		async (entry) => {
			...
			if (stats.isFile()) {
				return ignore.file(normalizedPath) ? [] : [filePath];
			}
			...
			if (ignore.directory(normalizedPath)) return [];
			if (isSymlink) {
...
/**
 * Puts the `ignore` hook ahead of `globOptions.ignore`, so a plugin can overrule it either way.
 ...
 */
const createIgnoreMatcher = (hooks, ignore) => {
	/**
	 * @param {(normalizedPath: string) => boolean} byGlobOptions what `globOptions.ignore` says
	 * @returns {(normalizedPath: string) => boolean} what the copy does
	 */
	const answer = (byGlobOptions) => (normalizedPath) => {
		const decision = hooks.ignore.call(normalizedPath);
		if (decision !== undefined) return decision;
		return byGlobOptions(normalizedPath);
	};
	...
	async _resolveFrom(
		...
		const ignored =
			typeOfFrom === "file"
				...
				: ignore.directory(normalizedBase);
