 ...
 * @property {InOutputPathMatcher} isInOutputPath what a followed symlink must not resolve into
...
/**
 * A followed symlink is read as what it points at, so where its own name sits
 * says nothing about where its bytes come from.
 * @param {InputFileSystem} fs input file system
 * @param {string} filePath absolute path of a file reached through a symlink
 * @param {InOutputPathMatcher} isInOutputPath what is treated as output
 * @returns {Promise<boolean>} true when the file resolves into the output directory
 */
const resolvesIntoOutputPath = async (fs, filePath, isInOutputPath) => {
	const realPath = await realpath(fs, filePath);
	return isInOutputPath(normalizePathSeparatorsForPath(realPath));
};
...
const walkDirectory = async (options, directory, visited, maxDepth) => {
	const { fs, followSymlinks, ignore, isInOutputPath, links } = options;
	...
	const results = await mapLimit(
		...
		async (entry) => {
			...
			if (stats.isFile()) {
				return isSymlink &&
					(await resolvesIntoOutputPath(fs, filePath, isInOutputPath))
					? []
					: [filePath];
			}
			...
			if (isSymlink) {
				const realPath = await realpath(fs, filePath);
				// a link resolving into the output directory reaches it under a name
				// the lexical check never sees
				if (isInOutputPath(normalizePathSeparatorsForPath(realPath))) return [];
...
const walkBase = async (options, base, maxDepth) => {
	...
	if (options.followSymlinks) {
		const realPath = await realpath(options.fs, base);
		if (options.isInOutputPath(normalizePathSeparatorsForPath(realPath))) {
			return [];
		}
 ...
 * @returns {boolean} true when the path is treated as output and not copied
 */

/**
 * The `ignore` hook answers for a path before `output.path` is consulted, so a
 * plugin can copy out of the output directory deliberately.
 * @param {CopyPluginCompilationHooks} hooks the compilation's copy hooks
 * @param {string} outputPath `output.path` with `/` separators
 * @returns {InOutputPathMatcher} what is treated as output
 */
const createInOutputPathMatcher = (hooks, outputPath) => {
	const inside = `${outputPath}/`;
	return (normalizedPath) => {
		const decision = hooks.ignore.call(normalizedPath);
		if (decision !== undefined) return decision;
		return normalizedPath === outputPath || normalizedPath.startsWith(inside);
	};
};

/**
 * @param {InOutputPathMatcher} isInOutputPath what is inside the output directory
 * @param {IgnoreMatcher} ignore what the pattern itself does not copy
 * @returns {IgnoreMatcher} the same, with the output directory added
 */
const ignoringOutputPath = (isInOutputPath, ignore) => ({
	file: (normalizedPath) =>
		isInOutputPath(normalizedPath) || ignore.file(normalizedPath),
	directory: (normalizedPath) =>
		isInOutputPath(normalizedPath) || ignore.directory(normalizedPath)
});
...
class CopyPlugin {
	 ...
	 * @param {InOutputPathMatcher} options.isInOutputPath what is inside the output directory
	...
	async _resolveFrom(
		...
		compilation,
		logger,
		{ from, context, globOptions, matchOptions, ignore, isInOutputPath, links }
	) {
		...
		// `stat` followed the link to call this a file, and `walkBase` resolves a
		// base of its own, so the name the pattern used says nothing about what it
		// reaches
		const readsOutputPath =
			followSymlinks &&
			(await resolvesIntoOutputPath(
				fs,
				typeOfFrom === "file" ? absoluteFrom : base,
				isInOutputPath
			));
		const entries =
			typeOfFrom === "file"
				...
				: (
						await walkBase(
							{ fs, followSymlinks, ignore, isInOutputPath, links },
							base,
							maxDepth
						)
					).filter((filePath) => {
		...
		// a base inside the output directory copies nothing, so watching it would
		// only rebuild on what this compilation itself wrote there — under the
		// alias's own name, where the lexical check never looks
		if (!isInOutputPath(normalizedBase) && !readsOutputPath) {
