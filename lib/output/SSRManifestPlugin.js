 ...
 * @since 5.111.0
 * @inline
 * @typedef {AbsolutePath} SSRManifestContext
 */
 ...
 * @since 5.111.0
 * @inline
 * @typedef {NonEmptyRelativePath} SSRManifestFilename
 */

/**
 * @schema plugins/SSRManifestPlugin
 * @typedef {object} SSRManifestPluginOptions
 * @property {SSRManifestContext=} context
 * @property {SSRManifestFilename=} filename
 */
...
class SSRManifestPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			// The manifest is what a server renders its `<link>` tags from, so this
			// is the build whose css runtime meets stylesheets it did not create.
			compilation.hooks.additionalTreeRuntimeRequirements.tap(
				PLUGIN_NAME,
				(chunk, set) => {
					set.add(RuntimeGlobals.hasServerRenderedStylesheets);
				}
			);
