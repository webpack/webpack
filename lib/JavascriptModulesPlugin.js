...
const plugin = "JavascriptModulesPlugin";
...
class JavascriptModulesPlugin {
	...
	apply(compiler) {
		...
		compilation.tap(plugin, (compilation, { normalModuleFactory }) => {
			...
			hooks.shouldRender.tap(plugin, module => {
			...
			createParser.for("javascript/auto").tap(plugin, options => {
				return new JavascriptParser(options, "auto");
			});
			createParser.for("javascript/dynamic").tap(plugin, options => {
				return new JavascriptParser(options, "script");
			});
			createParser.for("javascript/esm").tap(plugin, options => {
				return new JavascriptParser(options, "module");
			});

			createGenerator.for("javascript/auto").tap(plugin, () => {
				return new JavascriptGenerator();
			});
			createGenerator.for("javascript/dynamic").tap(plugin, () => {
				return new JavascriptGenerator();
			});
			createGenerator.for("javascript/esm").tap(plugin, () => {
				return new JavascriptGenerator();
			});
			...
			chunkTemplate.hooks.renderManifest.tap(plugin, (result, options) => {
				...
				const hasModules = Array.from(chunkGraph.getChunkModules(chunk)).filter(
					module => {
