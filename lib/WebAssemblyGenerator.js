 ...
 * @returns {ArrayBuffer} bin'
 */
const rewriteImportedGlobals = ({ importedGlobals }) => bin => {
	debug("rewriteImportedGlobals");

	const globalImportCount = importedGlobals.length;

	// Work arround the bug discribed above
	for (let i = 0, nbr = globalImportCount; i < nbr; i++) {
		let removedThisOne = false;
		let newGlobal;

		bin = edit(bin, {
			ModuleImport(path) {
				if (removedThisOne === true) {
					return;
				}
...
class WebAssemblyGenerator {
	generate(module) {
		...
		const transform = compose(
			...
			rewriteImportedGlobals({ importedGlobals }),
