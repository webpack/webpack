...
class MainTemplate extends Template {
	constructor(outputOptions) {
		...
		this.plugin("render", function(bootstrapSource, chunk, hash, moduleTemplate, dependencyTemplates) {
			...
			source.add(new webpackSources.PrefixSource("/******/", bootstrapSource));
			source.add("/******/ })\n");
			source.add("/************************************************************************/\n");
			source.add("/******/ (");
		...
		this.plugin("module-obj", function() {
	...
	render(hash, chunk, moduleTemplate, dependencyTemplates) {
		...
		let source = this.applyPluginsWaterfall("render", new webpackSources.OriginalSource(`${this.prefix(buf, " \t")}\n`, `webpack/bootstrap ${hash}`), chunk, hash, moduleTemplate, dependencyTemplates);
		...
		chunk.rendered = true;
		return new webpackSources.ConcatSource(source, ";");
	}
	...
	entryPointInChildren(chunk) {
		function checkChildren(chunk, alreadyCheckedChunks) {
			return chunk.chunks.some(child => {
				if(alreadyCheckedChunks.indexOf(child) >= 0) {
					return;
				}
