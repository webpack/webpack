...
function MainTemplate(outputOptions) {
	...
	this.plugin("require", function(source, chunk, hash) {
		return this.asString([
			...
			"};",
			"",
			"// See if the module was previously loaded",
			"if (modules[moduleId].loaded !== true) {",
			this.indent("modules[moduleId].loaded = true;"),
			this.indent("modules[moduleId].moduleIndex = moduleId;"),
			this.indent("// Execute the module function"),
			this.indent("modules[moduleId].call(module.exports, module, module.exports, " + this.renderRequireFunctionForModule(hash, chunk, "moduleId") + ");"),
			"} else {",
			this.indent("// Grab the previously loaded module's id and exports"),
			this.indent("module.id = installedModules[modules[moduleId].moduleIndex].id;"),
			this.indent("module.exports = installedModules[modules[moduleId].moduleIndex].exports;"),
			"}",
			"",
