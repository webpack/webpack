...
// One resolution per `[input, context]` — several callers ask for the same
// target, and each miss re-reads the config and re-runs the queries. The list is
// handed out as it stands, so no caller may write to it.
...
const resolve = (browsers) => {
	...
	return {
		...
		// CSS Box Alignment 3's `place-*` shorthands, newer than the `align-*` /
		// `justify-*` longhands they merge. One flag covers all three, so each
		// version is the newest of theirs — Safari shipped `place-content` in 9.
		cssPlaceShorthand: cssChecker({
			chrome: 59,
			and_chr: 59,
			edge: 79,
			firefox: 45,
