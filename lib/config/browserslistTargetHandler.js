...
const resolve = browsers => {
	 ...
	 * @param {number} requiredMinor required minor version
	 * @param {string} parsedMajor parser major version
	 * @param {string} parserMinor parser minor version
	 * @returns {boolean} true if supports
	 */
	...
	const nodeChecker = (major, minor = 0) => {
		return browsers.every(v => {
			...
			const [, v1, v2] = match;
			return nodeVersionComparator(major, minor, v1, v2 || "0");
		});
	};
	/**
