...
/** @typedef {import("./target").CssTargetProperties} CssTargetProperties */
...
// One resolution per `[input, context]`: the same target is asked for several
// times over — the default-target probe, the target's own properties, and the
// CSS minifier's browser list — and each of those reads the config file and runs
// the queries again. Held exactly as long as browserslist holds its own config
// and query caches, and not held at all where it is told to hold neither. The
// list is handed out as it stands, so no caller may write to it.
/** @type {Map<string, string[] | undefined> | null} */
const cache = process.env.BROWSERSLIST_DISABLE_CACHE ? null : new Map();
...
const load = (input, context) => {
	if (cache === null) return loadBrowsers(input, context);
	// A newline separates the two: an input carries a config path, an env or a
	// query, and a directory carries none.
	const key = `${input || ""}\n${context}`;
	let browsers = cache.get(key);
	if (browsers === undefined && !cache.has(key)) {
		browsers = loadBrowsers(input, context);
		cache.set(key, browsers);
	}
	return browsers;
};

/**
 * Read the selection out of the browserslist config or query.
 * @param {string | null | undefined} input input string
 * @param {string} context the context directory
 * @returns {string[] | undefined} selected browsers
 */
const loadBrowsers = (input, context) => {
 ...
 * @returns {EcmaTargetProperties & PlatformTargetProperties & ApiTargetProperties & CssTargetProperties} target properties
 */
const resolve = (browsers) => {
	 ...
	 */
	const rawChecker = (versions) => checkAll(browsers, versions);
	/**
	 * Checks a subset of the list against a version number
	 * @param {string[]} list the browsers to check
	 * @param {Record<string, number | [number, number]>} versions first supported version
	 * @returns {boolean} true if all support
	 */
	const checkAll = (list, versions) =>
		list.every((v) => {
	...
	/**
	 * Checks the rendering engines against a version number. A CSS feature is a
	 * property of those alone, so node entries say nothing about it — and a list
	 * with no engine at all renders no stylesheet, which leaves the modern
	 * spelling available.
	 * @param {Record<string, number | [number, number]>} versions first supported version
	 * @returns {boolean} true if all rendering engines support
	 */
	const cssChecker = (versions) => {
		const engines = browsers.filter((b) => !b.startsWith("node "));
		return engines.length === 0 || checkAll(engines, versions);
	};
	...
	return {
		...
		// CSS Color 4's `#rgba` / `#rrggbbaa`.
		cssColorHexAlpha: cssChecker({
			chrome: 62,
			and_chr: 62,
			edge: 79,
			firefox: 49,
		...
		// Media Queries 4's range syntax (`(width >= 600px)`).
		cssMediaQueryRange: cssChecker({
			chrome: 104,
			and_chr: 104,
			edge: 104,
			firefox: 102,
			...
			samsung: 20,
			android: 104
			// baidu / and_qq / and_uc / kaios: Not tracked
		}),
		// CSS Images 4's two-position color stop (`red 0% 50%`), which stands for
		// the two stops it would otherwise take.
		cssGradientDoublePosition: cssChecker({
			chrome: 72,
			and_chr: 72,
			edge: 79,
			firefox: 83,
		...
		// CSS Position 3's `inset` shorthand. Newer than the `top` / `right` /
		// `bottom` / `left` longhands it merges, which are CSS 1.
		cssInsetShorthand: cssChecker({
			chrome: 87,
			and_chr: 87,
			edge: 87,
			firefox: 66,
