...
/**
 * @import {
 * 	CacheOptionsNormalized as CacheOptions,
...
/**
 * @typedef {"embeddedSourceMaps" | "entrypointOverlap" | "unusedAliases" | "unusedDefines" | "unusedExternals" | "unusedReexports" | "unusedRules"} DeprecatedPerformanceCheck
 */

/**
 * @typedef {"duplicateModules" | "sourceMaps" | "unusedConfig" | "unusedModules"} RenamedPerformanceCheck
 */

/**
 * The performance checks renamed when they were grouped, as the deprecated
 * name to the one that replaces it and the deprecation code it warns under.
 * @type {[DeprecatedPerformanceCheck, RenamedPerformanceCheck, string][]}
 */
const RENAMED_PERFORMANCE_CHECKS = [
	["embeddedSourceMaps", "sourceMaps", "EMBEDDED_SOURCE_MAPS"],
	["entrypointOverlap", "duplicateModules", "ENTRYPOINT_OVERLAP"],
	["unusedAliases", "unusedConfig", "UNUSED_ALIASES"],
	["unusedDefines", "unusedConfig", "UNUSED_DEFINES"],
	["unusedExternals", "unusedConfig", "UNUSED_EXTERNALS"],
	["unusedReexports", "unusedModules", "UNUSED_REEXPORTS"],
	["unusedRules", "unusedConfig", "UNUSED_RULES"]
];

/** @type {Map<DeprecatedPerformanceCheck, (value: boolean) => boolean>} */
const deprecatedPerformanceChecks = new Map(
	RENAMED_PERFORMANCE_CHECKS.map(([from, to, code]) => [
		from,
		util.deprecate(
			/**
			 * @param {boolean} value what the deprecated name was set to
			 * @returns {boolean} the same value, for the option replacing it
			 */
			(value) => value,
			`performance.${from} is deprecated in favor of performance.${to}`,
			`DEP_WEBPACK_CONFIGURATION_PERFORMANCE_${code}`
		)
	])
);
 ...
 * @param {import("../../declarations/WebpackOptions").ResourceHints=} resourceHints raw value
 * @returns {import("../../declarations/WebpackOptions").ResourceHintsOptions | undefined} normalized object form
 */
const normalizeResourceHints = (resourceHints) => {
	if (resourceHints === undefined) return undefined;
	/**
	 * @param {import("../../declarations/WebpackOptions").ResourceHintsInitial=} value initial value
	 * @returns {import("../../declarations/WebpackOptions").ResourceHintsInitial | undefined} `"preload"` aliased to `true`
	 */
	const alias = (value) => (value === "preload" ? true : value);
	if (
		typeof resourceHints === "object" &&
		!Array.isArray(resourceHints) &&
		typeof resourceHints !== "function"
	) {
		return { ...resourceHints, initial: alias(resourceHints.initial) };
	}
	return { initial: alias(resourceHints) };
};
...
/**
 * @param {import("../../declarations/WebpackOptions").BuildDependencyItem} item build dependency
...
/**
 * @param {import("../../declarations/WebpackOptions").FileCacheOptions["buildDependencies"]=} buildDependencies build dependencies
 * @returns {Record<string, NormalizedBuildDependencyItem[]>} normalized build dependencies
 */
const normalizeBuildDependencies = (buildDependencies) => {
	/** @type {Record<string, NormalizedBuildDependencyItem[]>} */
	const result = Object.create(null);
	if (buildDependencies === undefined) return result;
	for (const key of Object.keys(buildDependencies)) {
		result[key] = buildDependencies[key].map(normalizeBuildDependencyItem);
	}
	return result;
};
...
const getNormalizedWebpackOptions = (config) => ({
	...
	optimization: nestedConfig(config.optimization, (optimization) => ({
		...
		// TODO webpack 6: normalize `minimize` to the per-asset-type object and
		// drop `minimizeOptions` — a breaking change for what plugins read back.
		minimize:
			typeof optimization.minimize === "object" ? true : optimization.minimize,
		minimizeOptions: getNormalizedOptimizationMinimizeOptions(
	...
	output: nestedConfig(config.output, (output) => {
		...
		const result = {
			...
			library: libraryBase && {
				...
				umdAmdContainer: libraryBase.umdAmdContainer,
				umdNamedDefine:
			...
			resourceHints: normalizeResourceHints(output.resourceHints),
	...
	performance: optionalNestedConfig(config.performance, (performance) => {
		...
		for (const [from, to] of RENAMED_PERFORMANCE_CHECKS) {
			...
			// Warned for whenever it is written, even where it changes nothing.
			const mapped =
				/** @type {(value: boolean) => boolean} */
				(deprecatedPerformanceChecks.get(from))(value);

			// The name written in this config wins; among deprecated names a
			// `true` does, since several of them now ask for one grouped check.
			if (performance[to] !== undefined) continue;
			if (result[to] === undefined || mapped) result[to] = mapped;
		}
...
/**
 * Gets the per-asset-type minimize options. `minimize` itself stays a boolean,
 * and its object form is a shorthand an explicit `minimizeOptions` overrides.
 * @param {Optimization["minimize"]} minimize minimize option
 * @param {Optimization["minimizeOptions"]} minimizeOptions minimizeOptions option
 * @returns {OptimizationNormalized["minimizeOptions"]} normalized minimize options
 */
const getNormalizedOptimizationMinimizeOptions = (
	minimize,
	minimizeOptions
) => {
	if (minimizeOptions !== undefined) return cloneObject(minimizeOptions);
	if (typeof minimize !== "object") return;
	return cloneObject(minimize);
};
