...
const getNormalizedWebpackOptions = (config) => ({
	...
	output: nestedConfig(config.output, (output) => {
		...
		const result = {
			...
			autoPreconnect: output.autoPreconnect,
			...
			modulePreloadPolyfill: output.modulePreloadPolyfill,
			path: output.path,
			pathinfo: output.pathinfo,
			publicPath: output.publicPath,
			resourceHints:
				output.resourceHints === "preload" ? true : output.resourceHints,
			resourceHintsManifest: output.resourceHintsManifest,
