...
/**
 * @template {keyof WebpackOptionsNormalized} F
 * @param {WebpackOptionsNormalized} options normalized option
 * @param {F} field normalized option field to intercept
 * @param {(value: WebpackOptionsNormalized[F]) => WebpackOptionsNormalized[F]} getter the getter
 * @param {(value: WebpackOptionsNormalized[F]) => void=} onChange used for setter
 */
function interceptOptions(options, field, getter, onChange) {
	let fieldValue = options[field];

	/**
	 * @param {WebpackOptionsNormalized[F]} value value
	 */
	const setter = (value) => {
		fieldValue = value;
...
const applyWebpackOptionsInterception = (options) => {
	...
	interceptOptions(
		...
		"devtool",
		(devtool) => {
			if (Array.isArray(devtool)) {
				// prefer `all`, then `javascript`, then `css`
				const match = ["all", "javascript", "css"]
					.map((type) => devtool.find((item) => item.type === type))
					.find(Boolean);

				return match ? match.use : devtool;
			}
			return devtool;
		},
		(devtool) => {
			options.devtools = Array.isArray(devtool)
				? devtool
