...
const getNormalizedWebpackOptions = (config) => ({
	...
	optimization: nestedConfig(config.optimization, (optimization) => ({
		...
		minimize: getNormalizedOptimizationMinimize(optimization.minimize),
		minimizer:
...
/**
 * Gets normalized optimization minimize: the `true` shorthand becomes an empty
 * per-asset-type object, so downstream only sees `false` or the object form.
 * @param {Optimization["minimize"]} minimize minimize option
 * @returns {OptimizationNormalized["minimize"]} normalized minimize option
 */
const getNormalizedOptimizationMinimize = (minimize) => {
	if (minimize === undefined) return;
	if (minimize === false) return false;
	if (minimize === true) return {};
