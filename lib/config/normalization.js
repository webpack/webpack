...
/**
 * Converts a file URL to the path it names. Anything that is not a string is
 * returned unchanged, so `false` and `undefined` pass through.
 * @template {string | false | undefined} T
 * @param {T} pathOrUrl an absolute path, a file URL, `false` or `undefined`
 * @returns {T} an absolute path, `false` or `undefined`
 */
const optionalFileUrlToPath = (pathOrUrl) =>
	typeof pathOrUrl === "string"
		? /** @type {T} */ (fileUrlToPath(pathOrUrl))
		: pathOrUrl;

/**
 * Converts every file URL of a list to the path it names.
 * @param {(string | RegExp)[]} pathsOrUrls absolute paths, file URLs or regular expressions
 * @returns {(string | RegExp)[]} absolute paths and regular expressions
 */
const fileUrlsToPaths = (pathsOrUrls) =>
	pathsOrUrls.map((pathOrUrl) =>
		typeof pathOrUrl === "string" ? fileUrlToPath(pathOrUrl) : pathOrUrl
	);
...
/**
 * @param {import("../../declarations/WebpackOptions").BuildDependencies=} buildDependencies build dependencies
...
const getNormalizedWebpackOptions = (config) => ({
	amd: config.amd,
	bail: config.bail,
	buildDependencies: normalizeBuildDependencies(config.buildDependencies),
	cache:
				...
				switch (cache.type) {
					case "filesystem":
						return {
							...
							cacheDirectory: optionalFileUrlToPath(cache.cacheDirectory),
							cacheLocation: optionalFileUrlToPath(cache.cacheLocation),
	...
	context: optionalFileUrlToPath(config.context),
			...
			nestedConfig(config.module, (module) => ({
				...
				parser: keyedNestedConfig(module.parser, cloneObject, {
					javascript: (parserOptions) => ({
						// TODO in the next major release: remove from `ModuleOptions`, keep only `*ByModuleType`
	...
	optimization: nestedConfig(config.optimization, (optimization) => ({
		...
		// TODO in the next major release: normalize `minimize` to the per-asset-type object and
	...
	output: nestedConfig(config.output, (output) => {
		...
		const result = {
			...
			copy: optionalNestedConfig(output.copy, (copy) => ({
				patterns: typeof copy === "string" ? [copy] : [...copy]
			})),
			...
			// TODO in the next major release: remove `libraryTarget`/`auxiliaryComment`/`amdContainer`/etc in favor of the `library` option
			library: libraryBase && {
				type:
				...
				export:
					output.libraryExport !== undefined
						...
						: libraryBase.export,
				entryExports: libraryBase.entryExports,
			...
			path: optionalFileUrlToPath(output.path),
	...
	recordsInputPath: optionalFileUrlToPath(
		config.recordsInputPath !== undefined
			? config.recordsInputPath
			: config.recordsPath
	),
	recordsOutputPath: optionalFileUrlToPath(
	...
	snapshot: nestedConfig(config.snapshot, (snapshot) => ({
		...
		managedPaths: optionalNestedArray(snapshot.managedPaths, fileUrlsToPaths),
...
const applyWebpackOptionsInterception = (options) => {
	...
	// TODO in the next major release: remove compatibility logic and move `devtools` fully into `devtool` with multi-type support
