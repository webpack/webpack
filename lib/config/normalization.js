...
const { maybeFileURLToPath } = require("../util/fs");
...
const getNormalizedWebpackOptions = (config) => ({
	...
	// resolved once here since context is read in many places
	context: maybeFileURLToPath(config.context),
	...
	output: nestedConfig(config.output, (output) => {
		...
		const result = {
			...
			// resolved once here since output.path is read in many places
			path: maybeFileURLToPath(output.path),
	...
	stats: nestedConfig(config.stats, (stats) => {
		...
		const result = { ...stats };
		// resolved once here since stats.context is read in multiple stats plugins
		if (result.context !== undefined) {
			result.context = maybeFileURLToPath(result.context);
		}
		return result;
	}),
