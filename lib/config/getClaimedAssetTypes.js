 ...
 * @returns {{ css: boolean, html: boolean, xml: boolean }} types another minimizer handles
 */
module.exports = (compiler) => {
	...
	for (const candidate of [...minimizer, ...plugins]) {
		...
		if (!xml) {
			xml = claims(candidate, "file.svg") || claims(candidate, "file.xml");
		}
		if (css && html && xml) break;
	}
	return { css, html, xml };
};
