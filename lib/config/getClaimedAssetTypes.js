 ...
 * @returns {{ css: boolean, html: boolean, svg: boolean }} types another minimizer handles
 */
module.exports = (compiler) => {
	...
	for (const candidate of [...minimizer, ...plugins]) {
		...
		// An SVG minimizer (svgo, `image-minimizer-webpack-plugin`) claims a type
		// the HTML minifier now also prints, so it is probed on its own.
		if (!svg) svg = claims(candidate, "file.svg");
		if (css && html && svg) break;
	}
	return { css, html, svg };
};
