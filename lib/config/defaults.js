...
const applyModuleDefaults = (
	...
	A(module, "defaultRules", () => {
		...
		if (css) {
			...
			// The `issuer` guard keeps the same-file re-reference (a `.css` issuer)
			// on the normal CSS path instead of recursing back into `url`.
			cssUrlRules.push(
				{
					test: /\.css$/i,
					issuer: { not: /\.css$/i },
					type: CSS_MODULE_TYPE_AUTO,
					parser: { exportType: "url", namedExports: false },
					generator: { esModule: false },
					resolve
				},
				{
					scheme: /^data$/,
