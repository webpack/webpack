 ...
 * 	Node as WebpackNode,
 * 	OptimizationMinimizeOptions,
 * 	OptimizationNormalized as Optimization,
 * 	OptimizationSplitChunksOptions,
 * 	OutputNormalized as Output,
 ...
 * @import {
 * 	ChunkLoadingTypes
 * } from "../javascript/EnableChunkLoadingPlugin"
 */
/** @import { WasmLoadingTypes } from "../wasm/EnableWasmLoadingPlugin" */
/** @import { PlatformTargetProperties, TargetProperties } from "./target" */
 ...
 * minimizeOptions: NonNullable<Optimization["minimizeOptions"]>,
...
const DEFAULTS = {
	// TODO in the next major release: use xxhash64
	HASH_FUNCTION: "md4"
};
...
/**
 * Whether the library type emits an ECMAScript module, which only `output.module`
 * output can carry — every other type is written into a script.
 * @param {LibraryType | undefined} type library type
...
const applyWebpackOptionsDefaults = (options, compilerIndex) => {
	...
	// Progress lives on infrastructureLogging; the next major release shows the interactive
	...
	).some((type) => futureDefaults || isModuleLibraryType(type));
	...
	// `output.resourceHints.urlHints` fans the project-wide rules out as the base
	// `urlHints` of every URL-emitting parser. They go first, so a parser-scoped
	// rule matches later and wins.
...
const applyExperimentsDefaults = (
	...
	// TODO in the next major release: do we need sync web assembly?
	...
	// Resolve `"auto"`: enable the built-in type unless a loader is already
	// registered for these files, so the default breaks no existing setup. Both
	// extensions are sampled, since `/\.css$/i` also covers `.module.css`.

	// Enabled implicitly the value stays `"auto"`, and the plugins then step
	// aside per module for requests loaders were applied to.
	// TODO in the next major release: css/html default to `true`, drop the `"auto"` marker.
...
const applyJavascriptParserOptionsDefaults = (
	...
	D(parserOptions, "specNamespaceObject", false);
...
const applyModuleDefaults = (
	...
	if (html) {
		// `extract` is deliberately left undefined, which HtmlGenerator reads as
		// "extract only a module that is a compilation entry". `true` forces it for
		// imported modules too, `false` disables it everywhere.
		F(module.generator, HTML_MODULE_TYPE, () => ({}));
		// `sources` defaults to `true`, so HtmlParser extracts URL-like attributes
		// as dependencies from its built-in list. `false` disables extraction, and
		// an array customizes it, with `"..."` keeping the defaults.
	...
	A(module, "defaultRules", () => {
		...
		if (css) {
			...
			// For `composes: className from "./style.css"`. A file already detected as
			// a CSS module by extension is skipped: it gets modules mode from the auto
			// rule anyway, and a second type stamp would duplicate the module.
		...
		if (html) {
			...
			// `<iframe srcdoc>` content goes back through the HTML pipeline as a
			// `data:text/html` module. `extract: "inline"` exposes it on the `html`
			// channel without emitting a file: it is never a page on its own.
			rules.push({
			...
			if (css) {
				// Inline `<style>` content goes into the CSS pipeline as a
				// `data:text/css` module. `exportType: "text"` is forced, so the
				// processed text reaches the `css-text` channel the template reads.
				rules.push({
...
const applyOutputDefaults = (
	...
	/**
	 * Every library type the configuration states, wherever it was declared.
	 * @returns {LibraryType[]} the declared library types
	 */
	const declaredLibraryTypes = () => {
		/** @type {LibraryType[]} */
		const types = [];
		if (output.library && output.library.type) types.push(output.library.type);
		// A plugin (`ModuleFederationPlugin`) pushed its type here before the defaults;
		// `"..."` is the extend placeholder `A` resolves, never a type of its own.
		...
		forEachEntry((desc) => {
			if (desc.library && desc.library.type) types.push(desc.library.type);
		});
		return types;
	};

	const libraryTypes = declaredLibraryTypes();

	// A library type that is an ECMAScript module is another spelling of
	// `output.module`, so it turns it on rather than disagreeing with it.
	if (libraryTypes.some(isModuleLibraryType)) {
		output.module = true;
	}

	// Every other library type is read out of a script, which is a statement that
	// the output is not an ECMAScript module — so it holds the default below back.
	const hasScriptLibrary = libraryTypes.some(
		(type) => !isModuleLibraryType(type)
	);
	...
	F(output, "module", () =>
		Boolean(
			...
			((tp.node === null && tp.web === null && tp.module !== false) ||
				tp.deno === true ||
				tp.bun === true ||
				// the next major release emits ESM wherever the target reads one — a browserslist
				// query (or `esX`/`nodeX`) without ESM support keeps script output
				(futureDefaults && tp.module !== false && !hasScriptLibrary))
		)
	);
	...
	// Only the browserslist target reports these; unset elsewhere, which
	// `optimistic` reads as the modern spelling being available.

	// `let` was added after `const`, so a target predating it reports only
	// `const` and falls back to it. One reporting `let` keeps its own value.
	F(environment, "let", () =>
	...
	F(
		...
		"topLevelAwait",
		() =>
			tp && optimistic(/** @type {boolean | undefined} */ (tp.topLevelAwait))
	);
	F(
		environment,
		"forOf",
	...
	// Stage 3 syntax, so these are the one pair read pessimistically: a preset that
	// says nothing means unavailable, and the phase keeps webpack's own form.
	F(environment, "deferImport", () =>
		Boolean(tp && /** @type {boolean | undefined} */ (tp.deferImport))
	);
	F(environment, "sourceImport", () =>
		Boolean(tp && /** @type {boolean | undefined} */ (tp.sourceImport))
	);
	F(
		environment,
		"destructuring",
	...
	// For ESM output, preload the entry's initial dependency graph, which native
	// `import()` would otherwise fetch in a waterfall. Classic output loads its
	// initial chunks in parallel already, so it stays opt-in.
	if (output.module && output.resourceHints === undefined) {
		output.resourceHints = {};
	}
	if (output.resourceHints) {
		D(output.resourceHints, "dedupe", false);
	...
	// Derive the html filenames from `output.filename`, as the CSS pipeline does.
	// A derived template with no per-module placeholder falls back to
	// `[name].html`, or two extracted modules would want the same file.
	...
	F(output, "globalObject", () => {
		if (tp) {
			...
			// A universal target has no identifier defined on both sides, so this is
			// `globalThis` where nothing argues against it and a guarded pair where
			// something does: naming one alone leaves the other undeclared.
			if (tp.web === null && tp.node === null) {
				return tp.globalThis === undefined
					? "globalThis"
			...
			if (output.module) {
				...
				throw new Error(
					"For the selected environment is no default script chunk format available:\n" +
						`${
							tp.module
								? "Module ('module') can be chosen when ES modules are available (please set 'output.module' to `true`)"
								: ""
						}\n` +
	...
	D(
		...
		`[runtime].[fullhash].hot-update.${
			output.chunkLoading === "import" ? "json.mjs" : "json"
		}`
	);
...
const applyExternalsPresetsDefaults = (
	...
	// Opt-in only: never auto-externalize installed packages, it would change existing builds.
	D(externalsPresets, "nodeModules", false);
	D(
		externalsPresets,
		"deno",
		/** @type {boolean | undefined} */
...
const applyNodeDefaults = (
	...
	const handlerForNames = () => {
		// TODO in the next major release: remove `node-module` in favor of `eval-only`
		if (targetProperties) {
		...
		// TODO in the next major release: should we use `warn-even-only`?
...
const applyPerformanceDefaults = (performance, { production }) => {
	...
	// Stands in for every check left unset, so an explicit one still wins.
	const all = performance.all !== undefined ? performance.all : false;

	D(performance, "analyzableBailouts", all);
	D(performance, "asyncChunkWaterfalls", all);
	D(performance, "broadContexts", all);
	D(performance, "cacheEffectiveness", all);
	D(performance, "circularDependencies", all);
	D(performance, "conflictingResourceHints", all);
	D(performance, "duplicateModules", all);
	D(performance, "duplicatePackages", all);
	D(performance, "dynamicExports", all);
	D(performance, "evalUsage", all);
	D(performance, "hotspots", all);
	D(performance, "legacyJavascript", all);
	D(performance, "largeModules", all);
	D(performance, "inlinedAssets", all);
	D(performance, "mixedExports", all);
	D(performance, "missingSideEffects", all);
	D(performance, "osDependentRules", all);
	D(performance, "pureAnnotations", all);
	D(performance, "redundantDynamicImports", all);
	D(performance, "scopeHoistingBailouts", all);
	D(performance, "splitChunksCapped", all);
	D(performance, "sourceMaps", all);
	D(performance, "tinyChunks", all);
	D(performance, "topLevelThis", all);
	D(performance, "unsplitVendors", all);
	D(performance, "unusedAssets", all);
	D(performance, "unusedConfig", all);
	D(performance, "unusedModules", all);
 ...
 * @param {boolean} options.css is css enabled
 * @param {boolean} options.esmLibrary whether the output is an ECMAScript module consumed as a library
 * @param {boolean} options.records using records
 * @param {boolean} options.futureDefaults use defaults of the next major version
 * @returns {void}
 */
const applyOptimizationDefaults = (
	optimization,
	{ production, development, css, esmLibrary, records, futureDefaults }
) => {
	...
	// A plugin assigning the object shorthand in `apply()` runs after
	// normalization: it replaces `minimizeOptions`, keeping `minimize` boolean.
	const minimizeAssignedInApply = /** @type {unknown} */ (
		optimization.minimize
	);
	if (typeof minimizeAssignedInApply === "object") {
		optimization.minimizeOptions = /** @type {OptimizationMinimizeOptions} */ (
			minimizeAssignedInApply
		);
		optimization.minimize = true;
	}
	D(optimization, "minimize", production);
	F(optimization, "minimizeOptions", () => ({}));
	const minimizeOptions =
		/** @type {NonNullable<Optimization["minimizeOptions"]>} */
		(optimization.minimizeOptions);
	if (optimization.minimize) {
		// Only ESM output carries these annotations, and only a consumer that
		// re-bundles it reads them; terser drops them everywhere else, which is
		// what a script-loaded output — a federation container — wants.
		F(minimizeOptions, "javascript", () =>
			esmLibrary
				? {
						compress: { passes: 2 },
		...
		// webpack's printer replaces terser's phases one at a time, each held to
		// what terser writes; until proven on real builds it is a future default.
		// A copy, so the user's object is not written to; their own value wins.
		if (futureDefaults && minimizeOptions.javascript) {
			minimizeOptions.javascript = {
				printer: true,
				...minimizeOptions.javascript
			};
		}
		F(minimizeOptions, "css", () => ({}));
		F(minimizeOptions, "html", () => ({}));
		D(minimizeOptions, "json", futureDefaults);
	}
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// WHY: one minimizer instance, so one worker-thread pool, handles JS and
				// — where native CSS and HTML are enabled — those assets too, as an array
				// of minify functions each dispatched by its own `filter`: `jsMinify`
				// (terser) to JS, `cssMinify` to CSS, `htmlMinify` to HTML. The CSS and
				// HTML serializers stay worker-safe by resolving their parser with
				// `require("webpack/…")` in the worker, so all three share the pool, and
				// `test` is widened to let those assets reach the dispatcher.

				// Step aside for a minimizer the user configured for these types:
				// whichever ran first would mark the asset `minimized` and silently
				// disable the other. Matchers are read directly, coarse but stable.
				const claimedByOtherMinimizer = require("./getClaimedAssetTypes")(
					compiler
				);

				// Per asset type: `false` disables that type, an object is its options.
				const minimizeByType = /** @type {OptimizationMinimizeOptions} */ (
					compiler.options.optimization.minimizeOptions
				);
				const terserOptions = minimizeByType.javascript || {
					compress: { passes: 2 }
				};

				const { css, html } = compiler.options.experiments;
				const minifyJs = minimizeByType.javascript !== false;
				const minifyCss =
					Boolean(css) &&
					!claimedByOtherMinimizer.css &&
					minimizeByType.css !== false;
				const minifyHtml =
					Boolean(html) &&
					!claimedByOtherMinimizer.html &&
					minimizeByType.html !== false;
				// `=== true` rather than `!== false`: outside `futureDefaults`, JSON is opt-in.
				const minifyJson =
					minimizeByType.json === true && !claimedByOtherMinimizer.json;
				if (!minifyJs && !minifyCss && !minifyHtml && !minifyJson) return;
				// An asset name keeps the query and the fragment its filename template
				// wrote (`[query][fragment]`, `main.js#[contenthash]`), so both may follow.
				/**
				 * @param {string[]} extensions extension patterns
				 * @returns {RegExp} matcher for asset names with those extensions
				 */
				const assetNameTest = (extensions) =>
					new RegExp(`^[^?#]*\\.(?:${extensions.join("|")})(?:[?#].*)?$`, "i");
				if (minifyJs && !minifyCss && !minifyHtml && !minifyJson) {
					new TerserPlugin({
						test: assetNameTest(["[cm]?js"]),
						minify: require("../javascript/jsMinify"),
						minimizerOptions: terserOptions
					}).apply(/** @type {EXPECTED_ANY} */ (compiler));
					return;
				}
				/** @type {(typeof import("../javascript/jsMinify") | typeof import("../css/cssMinify") | typeof import("../html/htmlMinify") | typeof import("../json/jsonMinify"))[]} */
				const minify = [];
				/** @type {EXPECTED_OBJECT[]} */
				const minimizerOptions = [];
				/** @type {string[]} */
				const extensions = [];
				// `minimize.css` follows the CSS minifier wherever it runs: the HTML
				// entry gets the same options, like the shared environment below.
				const cssOptions =
					typeof minimizeByType.css === "object"
						? minimizeByType.css
						: undefined;
				// The browserslist selection, when the target is one. It decides both the
				// prefixes and every spelling a target can read, so `vendorPrefixes`
				// turns off the first alone — resolving it is what the second needs.
				const { target } = compiler.options;
				const browsers = target
					? getTargetsBrowsers(
							Array.isArray(target) ? target : [target],
							/** @type {Context} */ (compiler.options.context)
						)
					: undefined;
				const cssEnvironment = {
					browsers,
					vendorPrefixes:
						cssOptions === undefined || cssOptions.vendorPrefixes !== false
				};
				const htmlOptions =
					typeof minimizeByType.html === "object"
						? minimizeByType.html
						: undefined;
				if (minifyJs) {
					minify.push(require("../javascript/jsMinify"));
					minimizerOptions.push(terserOptions);
					extensions.push("[cm]?js");
				}
				if (minifyCss) {
					minify.push(require("../css/cssMinify"));
					minimizerOptions.push({
				...
				if (minifyHtml) {
					minify.push(require("../html/htmlMinify"));
					// `minimize.css` rides under its own key rather than spread beside the
					// HTML options: the two name some of the same switches, so one flat
					// object would hand each language the other's answer.
					minimizerOptions.push({
						environment: cssEnvironment,
						css: cssOptions,
						...htmlOptions
				...
				if (minifyJson) {
					minify.push(require("../json/jsonMinify"));
					minimizerOptions.push({});
					// A web app manifest is JSON under its own extension.
					extensions.push("json", "webmanifest");
				}
				new TerserPlugin({
					test: assetNameTest(extensions),
					minify,
					minimizerOptions
				}).apply(/** @type {EXPECTED_ANY} */ (compiler));
			}
	...
	if (splitChunks) {
		...
		D(splitChunks, "dedupDepth", futureDefaults ? 1 : 0);
