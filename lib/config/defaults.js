...
const applyOutputDefaults = (
	...
	/**
	 * Whether a library type is asked for that can only be emitted as an
	 * ECMAScript module, wherever it was declared.
	 * @returns {boolean} true, when an ESM-only library type is configured
	 */
	const hasModuleLibrary = () => {
		...
		// before the defaults and has pushed its type here already.
		if (
		...
		let found = false;
		forEachEntry((desc) => {
			if (desc.library && isModuleLibraryType(desc.library.type)) found = true;
		});
		return found;
	};
