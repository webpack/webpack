...
const applyWebpackOptionsDefaults = (options, compilerIndex) => {
	...
	// `output.urlHints` shorthand: fan the project-wide rules out as the base
	// `urlHints` of every URL-emitting parser (JS `new URL`, CSS `url()`, HTML
	// `<img src>`). They go first so parser-scoped `parser.<type>.urlHints`
	// rules — matched later, last-match-wins — still override them.
	const outputUrlHints = options.output.urlHints;
