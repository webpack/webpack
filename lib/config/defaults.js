...
const applyExperimentsDefaults = (
	...
	// unless the target cannot read one, which no output format can repair, so
	// leave `output.chunkFormat` to say so rather than emitting a module the
	// target has already reported it will not load
	const universal =
		Boolean(targetProperties) &&
		/** @type {TargetProperties} */ (targetProperties).node === null &&
		/** @type {TargetProperties} */ (targetProperties).web === null &&
		/** @type {TargetProperties} */ (targetProperties).module !== false;
...
const applyOutputDefaults = (
	...
	// No `topLevelAwait` here: the option is held back from the patch release,
	// so nothing derives it from `tp` yet. Reapply by reverting #21881.
	F(
		environment,
		"forOf",
...
const applyPerformanceDefaults = (performance, { production }) => {
	...
	D(performance, "entrypointOverlap", all);
	...
	D(performance, "unusedAliases", all);
	D(performance, "unusedDefines", all);
	D(performance, "unusedExternals", all);
	D(performance, "unusedReexports", all);
	D(performance, "unusedRules", all);
...
const applyOptimizationDefaults = (
	...
	// A plugin can assign the object shorthand in `apply()`, which runs after
	// normalization: fold it in here too, so `minimize` stays a boolean. It
	// replaces the configured options rather than defaulting them, as
	// overwriting the resolved option always has. The normalized type is
	// already a boolean, hence the cast.
	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// Normalized + defaulted `optimization.minimizeOptions` is the
				// per-asset-type object: `false` disables a type, an object is its
				// options.
