...
const applyOutputDefaults = (
	...
	F(
		...
		"cssColorHexAlpha",
		() =>
			tp && optimistic(/** @type {boolean | undefined} */ (tp.cssColorHexAlpha))
	);
	F(
		environment,
		"cssGradientDoublePosition",
		() =>
			tp &&
			optimistic(
				/** @type {boolean | undefined} */ (tp.cssGradientDoublePosition)
			)
	);
	F(
		environment,
		...
		() =>
			tp &&
			optimistic(/** @type {boolean | undefined} */ (tp.cssInsetShorthand))
	);
	F(
		environment,
		"cssMediaQueryRange",
		() =>
			tp &&
			optimistic(/** @type {boolean | undefined} */ (tp.cssMediaQueryRange))
	);
	F(
		environment,
		"cssPlaceShorthand",
		() =>
			tp &&
			optimistic(/** @type {boolean | undefined} */ (tp.cssPlaceShorthand))
	);
...
const applyPerformanceDefaults = (performance, { production }) => {
	...
	D(performance, "constReassignment", all);
	...
	D(performance, "loadersOnNodeModules", all);
	D(performance, "missingSideEffects", all);
	D(performance, "namespaceCall", all);
	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				/** @type {string[]} */
				const extensions = [];
				// The spellings the CSS minifier may reach for are the ones the target
				// supports, the way `output.environment` already gates the JS output's.
				// HTML gets the same object: an inline `<style>` or `style=""` runs
				// through that minifier too, and the two must not disagree.
				const outputEnvironment = compiler.options.output.environment || {};
				...
				const cssOptions =
					typeof minimizeByType.css === "object"
						? minimizeByType.css
						: undefined;
				// The browserslist selection, when the target is one, so the minifier can
				// add and drop prefixes for exactly these browsers — its only reader, so
				// `vendorPrefixes: false` skips resolving it at all.
				...
				const cssEnvironment = {
					cssColorHexAlpha: outputEnvironment.cssColorHexAlpha !== false,
					cssGradientDoublePosition:
						outputEnvironment.cssGradientDoublePosition !== false,
					cssInsetShorthand: outputEnvironment.cssInsetShorthand !== false,
					cssMediaQueryRange: outputEnvironment.cssMediaQueryRange !== false,
					cssPlaceShorthand: outputEnvironment.cssPlaceShorthand !== false,
					browsers
				};
				const htmlOptions =
