	...
	switch (cache.type) {
		case "filesystem":
			...
			F(cache, "cacheDirectory", () => {
				...
				// @ts-ignore: escalade typings use es module "export default" which is treated as "{ default: escalade }" by typescript; can be fixed with migration to es modules
				const pkgPath = escalade(cwd, (dir, names) => {
