	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// `optimization.minimize` as an object configures the built-in
				// minimizer per asset type: `false` excludes a type, an object carries
				// what that type's minifier may do beyond the always-applied transforms.
				const { minimize } = compiler.options.optimization;
				/** @type {OptimizationMinimizeOptions} */
				const minimizeByType = typeof minimize === "object" ? minimize : {};
