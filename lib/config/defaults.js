...
const applyOptimizationDefaults = (
	...
	if (minimize) {
		// Fill each asset type, so the object form is canonical and the JS
		// minimizer's defaults are visible in the resolved config rather than
		// hidden in the plugin wiring below.
	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// Normalization and the defaults above canonicalized
				// `optimization.minimize` into the per-asset-type object whenever this
				// plugin is applied at all: `false` disables a type, an object is that
				// type's minifier options.
				...
				// `minimize.css` reaches the CSS minifier wherever it runs — the HTML
				// entry gets the same options as the `.css` one, for the same reason
