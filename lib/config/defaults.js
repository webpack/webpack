...
const applyModuleDefaults = (
	...
	A(module, "defaultRules", () => {
		...
		if (css) {
			...
			// A `.css` reached through the `url` dependency category — JS
			// `new URL("./x.css", import.meta.url)`, CSS `url("./x.css")`, or an
			// HTML URL source — is bundled (resolving `@import`/`url()`) into its
			// own chunk and referenced by URL. The same-file re-reference uses the
			// `css-import` category, so it doesn't re-enter this rule.
