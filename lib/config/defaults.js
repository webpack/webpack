 ...
 * & { devtoolByTypes: NonNullable<WebpackOptionsNormalized["devtoolByTypes"]> }
...
const applyWebpackOptionsDefaults = (options, compilerIndex) => {
	...
	F(options, "devtoolByTypes", () => {
		const devtool = options.devtool;

		if (typeof devtool === "string") {
			return /** @type {WebpackOptionsNormalized["devtoolByTypes"]} */ (
				development && options.experiments.css
					? [
							{
								type: "css",
								use: "source-map"
