...
const applyModuleDefaults = (
	...
	A(module, "defaultRules", () => {
		...
		if (css) {
			...
			// `new URL("./style.css", import.meta.url)` → own CSS chunk + its URL.
			// Only JS `new URL()` should get this; CSS `url()` and HTML sources
			// share the `url` dependency category, so exclude their module types
			// by issuer (CSS/HTML keep their existing pipelines; this also stops
			// the same-file re-reference from recursing). `nameForCondition`
			// drops any `?query`, so these patterns match queried issuers too.
			const cssUrlParser = { exportType: "url", namedExports: false };
			const cssUrlGenerator = { esModule: false };
			const cssUrlIssuer = { not: { or: [/\.css$/i, /\.html$/i] } };
			cssUrlRules.push(
				{
					test: /\.css$/i,
					issuer: cssUrlIssuer,
