 ...
 * @param {boolean} options.css is css enabled
 * @param {boolean} options.library whether the output is consumed as a library
 * @param {boolean} options.records using records
 * @returns {void}
 */
const applyOptimizationDefaults = (
	optimization,
	{ production, development, css, library, records }
) => {
	...
	if (optimization.minimize) {
		// Canonicalize the per-asset-type options: the JS minimizer's defaults
		// become visible in the resolved config rather than hidden in the wiring.
		// A library is read by another build, so the annotations carrying what the
		// modules do to it have to survive minification; terser drops them otherwise.
	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// of minify functions each dispatched by its own `filter`: terser to JS,
				// `cssMinify` to CSS, `htmlMinify` to HTML. The CSS and HTML serializers
				// stay worker-safe by resolving their parser with `require("webpack/…")`
				// in the worker, so all three share the pool, and `test` is widened to let
				// those assets reach the dispatcher.
				...
				const htmlOptions =
					typeof minimizeByType.html === "object"
						...
						: undefined;
				if (minifyJs) {
					minify.push(TerserPlugin.terserMinify);
