...
const applyOutputDefaults = (
	...
	// Only the browserslist target reports these; everywhere else they stay
	// unset, which `optimistic` reads as the modern spelling being available —
	// the same policy the JS features above follow.
	...
	F(output, "globalObject", () => {
		if (tp) {
			...
			// Universal target (web + node): no one identifier is defined on both
			// sides, so `globalThis` where the target reports nothing against it,
			// and a guarded pair where it does — `self` covers window and worker
			// scopes, `global` covers Node.js. Naming either alone leaves the other
			// platform reading an undeclared binding.
	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// One minimizer instance (one worker-thread pool) handles JS and, when
				// native CSS / HTML is enabled, those assets too — an array of minify
				// functions each dispatched by its own `filter` (terser -> JS,
				// cssMinify -> CSS, htmlMinify -> HTML). The CSS / HTML serializers stay
				...
				if (minifyHtml) {
					...
					// `minimize.css` rides along under its own key rather than spread
					// beside the HTML options: the two name some of the same switches
					// (`quotes`, `comments`), and one flat object would hand
					// each language the other's answer.
					minimizerOptions.push({
						environment: cssEnvironment,
						css: cssOptions,
						...htmlOptions
