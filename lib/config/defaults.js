...
const applyOptimizationDefaults = (
	...
	if (optimization.minimize) {
		...
		if (futureDefaults) F(minimizeOptions, "xml", () => ({}));
	}
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// Off unless asked for: only an object, which futureDefaults supplies.
				const minifyXml =
					!claimedByOtherMinimizer.xml &&
					typeof minimizeByType.xml === "object";
				if (!minifyJs && !minifyCss && !minifyHtml && !minifyXml) return;
				if (minifyJs && !minifyCss && !minifyHtml && !minifyXml) {
					new TerserPlugin({
				...
				/** @type {(typeof import("../javascript/jsMinify") | typeof import("../css/cssMinify") | typeof import("../html/htmlMinify") | typeof import("../html/xmlMinify"))[]} */
				const minify = [];
				...
				if (minifyXml) {
					minify.push(require("../html/xmlMinify"));
					minimizerOptions.push({
						environment: cssEnvironment,
						css: cssOptions,
						...minimizeByType.xml
					});
					extensions.push("svg", "xml");
				}
				new TerserPlugin({
