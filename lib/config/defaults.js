	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				const { futureDefaults } = compiler.options.experiments;
				// webpack's printer replaces terser's phases one at a time, each held
				// to what terser writes; until it is proven on real builds it runs
				// under `futureDefaults` only, and terser as published everywhere else.
				const terserOptions = {
					...(minimizeByType.javascript || { compress: { passes: 2 } }),
					...(futureDefaults ? { printer: true } : undefined)
				};
