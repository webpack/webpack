	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				if (minifyHtml) {
					...
					// An inline `<style>` and a `style=""` are CSS, so `minimize.css`
					// turning the CSS minifier off has to reach them too.
					const minifyStyles =
						minimizeByType.css !== false &&
						(htmlOptions === undefined || htmlOptions.minifyStyles !== false);
					minimizerOptions.push({
						environment: cssEnvironment,
						css: cssOptions,
						...htmlOptions,
