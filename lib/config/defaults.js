	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				const cssOptions =
					typeof minimizeByType.css === "object" ? minimizeByType.css : undefined;
				// The browserslist selection, when the target is one, so the minifier can
				// add / drop vendor prefixes for exactly these browsers. Absent for any
				// other target — with no browser list there is nothing to prefix for — and
				// absent where `minimize.css.vendorPrefixes` turned prefixing off, which is
				// all the selection is read for.
