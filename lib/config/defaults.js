...
// Reuse the ruleset condition logic (string/regexp/function/array/and/or/not)
// instead of re-implementing it; no plugins are needed for `compileCondition`.
const ruleConditionCompiler = new RuleSetCompiler([]);

/**
 * Matches a rule condition against a sample resource path via the ruleset compiler.
 * @param {RuleSetConditionAbsolute} condition condition
 * @param {string} resource sample resource path
 * @returns {boolean} whether the condition matches
 */
const matchRuleCondition = (condition, resource) => {
	try {
		return ruleConditionCompiler
...
/**
 * Best-effort detection of a user-registered loader (or explicit module type) for a
 * resource. Used to resolve the `experiments.css`/`experiments.html`/`experiments.
 * asyncWebAssembly` "auto" defaults: when the user already handles these files we
 * keep the built-in module type off so enabling it by default is non-breaking.
 *
 * `include`/`exclude` are treated leniently (a matching `test`/`resource`/`include`
 * with a loader counts even if another narrowing condition would scope it down): a
 * loader scoped to e.g. `include: /src/` already handles those files today, so the
 * built-in type must stay off to avoid double-processing them.
 * @param {RuleSetRules | undefined} rules user `module.rules`
 ...
 */
const hasRuleForResource = (rules, resource, inherited = false) => {
	if (!rules) return false;
	for (const rule of rules) {
		if (!rule || typeof rule !== "object") continue;
		const matched =
			inherited ||
			(rule.test !== undefined && matchRuleCondition(rule.test, resource)) ||
			(rule.resource !== undefined &&
				matchRuleCondition(rule.resource, resource)) ||
			(rule.include !== undefined &&
				matchRuleCondition(rule.include, resource));
		if (matched && (rule.use || rule.loader || rule.type)) return true;
		if (rule.oneOf && hasRuleForResource(rule.oneOf, resource, matched)) {
			return true;
		}
		if (rule.rules && hasRuleForResource(rule.rules, resource, matched)) {
			return true;
		}
	}
	return false;
...
const applyExperimentsDefaults = (
	...
	if (experiments.css === "auto") {
		experiments.css = !(
			hasRuleForResource(rules, "/file.css") ||
			hasRuleForResource(rules, "/file.module.css")
		);
	}
	if (experiments.html === "auto") {
		experiments.html = !hasRuleForResource(rules, "/file.html");
	}
	// `syncWebAssembly` already claims `.wasm`, so don't let "auto" override it.
	if (experiments.asyncWebAssembly === "auto") {
		experiments.asyncWebAssembly =
			!experiments.syncWebAssembly && !hasRuleForResource(rules, "/file.wasm");
	}
