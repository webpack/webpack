	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				if (css) minify.push(require("../css/cssMinify"));
				if (html) minify.push(require("../html/htmlMinify"));
				for (let i = 1; i < minify.length; i++) minimizerOptions.push({});

				// The built-in CSS / HTML minifiers are a fallback: a minimizer the user
				// configured for those types taps the default stage, so it runs first and
				// this pass skips whatever it marked `minimized`. Without the later stage
				// a `["...", new CssMinimizerPlugin()]` config would let this one win and
				// silently disable the user's minimizer.
				// TODO drop the guard once the dependency requires >= 5.7.0 — older
				// installs don't know `stage` and their schema rejects it.
				const [pluginMajor, pluginMinor] =
					require("minimizer-webpack-plugin/package.json")
						.version.split(".")
						.map(Number);

				const supportsStage =
					pluginMajor > 5 || (pluginMajor === 5 && pluginMinor >= 7);
				// An unknown key still trips the plugin's `additionalProperties: false`
				// even when its value is `undefined`, so `stage` is only ever set.
				/** @type {EXPECTED_ANY} */
				const pluginOptions = {
					test: new RegExp(
						`\\.(?:[cm]?js${css ? "|css" : ""}${html ? "|html" : ""})(\\?.*)?$`,
						"i"
					),
					minify,
					minimizerOptions
				...
				new TerserPlugin(pluginOptions).apply(
					/** @type {EXPECTED_ANY} */ (compiler)
				);
			}
		}
