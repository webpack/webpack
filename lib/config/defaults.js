	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// The browserslist selection, when the target is one, so the minifier
				// can add / drop vendor prefixes for exactly these browsers. Absent for
				// any other target — with no browser list there is nothing to prefix for.
				...
				// entry gets the same options, like the shared environment above.
				const cssOptions =
