...
const applyModuleDefaults = (
	...
	A(module, "defaultRules", () => {
		...
		if (css) {
			...
			// `new URL("./style.css", import.meta.url)` bundles the CSS (resolving
			// `@import`/`url()`) into its own chunk and resolves to its URL. Only
			// the `url` dependency category (JS `new URL()` and generic HTML URL
			// refs) opts in; CSS `url()` is the `css-url` category and stays a raw
			// asset, and the same-file re-reference uses `css-import`, so neither
			// re-enters this rule.
			const cssUrlParser = { exportType: "url", namedExports: false };
			const cssUrlGenerator = { esModule: false };
			cssUrlRules.push(
				{
					test: /\.css$/i,
					type: CSS_MODULE_TYPE_AUTO,
					parser: cssUrlParser,
					generator: cssUrlGenerator,
					resolve
				},
				{
					scheme: /^data$/,
		...
		rules.push(
			{
				dependency: "url",
				oneOf: [...cssUrlRules, ...urlAssetRules]
			},
			// CSS `url()` (the `css-url` category, only present with
			// `experiments.css`) always resolves to a raw asset — never the
			// `exportType: "url"` CSS handling that JS `new URL()` gets.
			...(css
				? [
						{
							dependency: "css-url",
