...
const applyModuleDefaults = (
	...
	if (html) {
		...
		// `module.parser.html.scriptModule` defaults to `false`: a `<script>`
		// without an explicit `type` is a classic script (the browser default).
		// ES-module output still emits module scripts regardless (see HtmlParser).
		D(
			/** @type {NonNullable<ParserOptionsByModuleTypeKnown[HTML_MODULE_TYPE]>} */
			(module.parser[HTML_MODULE_TYPE]),
			"scriptModule",
