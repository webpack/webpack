...
const applyOutputDefaults = (
	...
	F(output, "globalObject", () => {
		if (tp) {
			...
			// Universal target (web + node): `globalThis` is the only accessor available
			// everywhere, so prefer it even for non-ESM (e.g. UMD), unless a merged
			// version explicitly lacks it (tp.globalThis === false).
			if (tp.web === null && tp.node === null && tp.globalThis !== false) {
				return "globalThis";
			}
		}
		// ESM output only runs where `globalThis` (ES2020) exists, so prefer it to `self`.
		if (output.module) return "globalThis";
		return "self";
	});
