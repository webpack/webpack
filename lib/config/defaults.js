...
// `schema-utils`' documented cross-version switch, read by every copy of it in
// the process.
const SKIP_VALIDATION_ENV = "SKIP_VALIDATION";

/**
 * Constructs a plugin with its `schema-utils` option check suppressed, restoring
 * the switch exactly. Only for options webpack builds itself — the first check
 * in a process spends ~100 ms bootstrapping ajv, which webpack otherwise avoids
 * by precompiling its own schemas.
 * @template T
 * @template {EXPECTED_ANY} O
 * @param {new (options: O) => T} Plugin the plugin class
 * @param {O} options the options webpack constructed
 * @returns {T} the plugin instance
 */
const createWithoutOptionCheck = (Plugin, options) => {
	const previous = process.env[SKIP_VALIDATION_ENV];
	process.env[SKIP_VALIDATION_ENV] = "y";
	try {
		return new Plugin(options);
	} finally {
		if (previous === undefined) {
			delete process.env[SKIP_VALIDATION_ENV];
		} else {
			process.env[SKIP_VALIDATION_ENV] = previous;
		}
	}
};
...
const applyOptimizationDefaults = (
	...
	if (minimize) {
		...
		// visible in the resolved config rather than hidden in the wiring.
	...
	A(optimization, "minimizer", () => [
		{
			apply: (compiler) => {
				...
				// Normalized + defaulted `optimization.minimize` is the per-asset-type
				// object here: `false` disables a type, an object is its options.
				...
				if (minifyJs && !minifyCss && !minifyHtml) {
					createWithoutOptionCheck(TerserPlugin, { terserOptions }).apply(
				...
				createWithoutOptionCheck(TerserPlugin, {
