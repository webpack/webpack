...
const applyResolveDefaults = (
	...
	F(resolve, "conditionNames", () => {
		/** @type {string[]} */
		const conditions = ["webpack"];

		conditions.push(mode === "development" ? "development" : "production");
		if (webTarget) conditions.push("browser");
	...
	F(resolve, "exportsFields", () => ["exports"]);
	F(resolve, "mainFields", () =>
		webTarget ? ["browser", "module", "main"] : ["module", "main"]
	);
	F(resolve.byDependency, "commonjs", () => ({}));
	F(resolve.byDependency.commonjs, "conditionNames", () => ["require", "..."]);
	F(resolve.byDependency, "amd", () => ({}));
	F(resolve.byDependency.amd, "conditionNames", () => ["require", "..."]);
	F(resolve.byDependency, "esm", () => ({}));
	F(resolve.byDependency.esm, "conditionNames", () => [
		"import",
		"module",
		"..."
	]);
	F(resolve.byDependency, "wasm", () => ({}));
	F(resolve.byDependency.wasm, "conditionNames", () => ["import", "..."]);
};
...
const applyResolveLoaderDefaults = (resolve, { cache }) => {
	D(resolve, "cache", cache);
	F(resolve, "conditionNames", () => ["loader", "require", "node"]);
	F(resolve, "exportsFields", () => ["exports"]);
