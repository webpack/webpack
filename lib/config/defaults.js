...
/**
 * Best-effort match of a rule condition against a sample resource path. Only the
 * forms commonly used to target a file extension are handled — enough to resolve
 * `experiments.css`/`experiments.html` "auto", not a substitute for the ruleset.
 ...
 */
const matchResourceCondition = (condition, resource) => {
	if (typeof condition === "string") return resource.startsWith(condition);
	if (typeof condition === "function") {
		try {
			return Boolean(condition(resource));
		} catch (_err) {
			return false;
		}
	}
	...
	if (Array.isArray(condition)) {
		return condition.some((c) => matchResourceCondition(c, resource));
	}
	if (condition && typeof condition === "object") {
		if (
			condition.and &&
			!condition.and.every((c) => matchResourceCondition(c, resource))
		) {
			return false;
		}
		if (
			condition.or &&
			!condition.or.some((c) => matchResourceCondition(c, resource))
		) {
			return false;
		}
		if (condition.not && matchResourceCondition(condition.not, resource)) {
			return false;
		}
		return Boolean(condition.and || condition.or || condition.not);
	}
	return false;
};

/**
 * Best-effort detection of a user-registered loader (or explicit module type) for
 * a resource. Used to resolve `experiments.css`/`experiments.html` "auto": when the
 * user already handles these files we keep the built-in module type off.
...
const hasRuleForResource = (rules, resource, inherited = false) => {
	...
	for (const rule of rules) {
		...
		const test = rule.test !== undefined ? rule.test : rule.resource;
		if (test !== undefined) {
			if (!matchResourceCondition(test, resource)) continue;
			matched = true;
		}
		if (rule.include !== undefined) {
			if (!matchResourceCondition(rule.include, resource)) continue;
			matched = true;
		}
		if (
			rule.exclude !== undefined &&
			matchResourceCondition(rule.exclude, resource)
		) {
			continue;
		}
 ...
 * @param {RuleSetRules} options.rules user `module.rules`, used to resolve css/html "auto"
 * @returns {void}
 */
