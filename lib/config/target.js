...
You can also more options via the 'target' option: 'browserslist' / 'browserslist:env' / 'browserslist:query' / 'browserslist:path-to-config' / 'browserslist:path-to-config:env'`);
	...
	[
		"universal",
		"Universal target running in browser, web worker and Node.js. Output is always ECMAScript modules.",
		/^universal$/,
		(context) => ({
			...mergeTargetProperties([
				getTargetProperties("web", context),
				getTargetProperties("webworker", context),
				getTargetProperties("node", context)
			]),
			// universal output is always ESM, so these are always available
			module: true,
			dynamicImport: true,
