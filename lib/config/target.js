...
// browserslist pulls in caniuse-lite; only a browserslist target needs it
 ...
 * @property {(boolean | null)=} deferImport deferred evaluation (`import defer`, `import.defer()`) is available (optional; absent means "assume unavailable")
 * @property {(boolean | null)=} sourceImport source phase imports (`import source`, `import.source()`) are available (optional; absent means "assume unavailable")
 */

/**
 * Defines the css target properties type used by this module. Only the
 * browserslist target states these — everywhere else they stay unset, which
 * `defaults` reads as the modern spelling being available.
 * @typedef {object} CssTargetProperties
 * @property {boolean | null} cssColorHexAlpha 4- and 8-digit hex colors (`#rgba`, `#rrggbbaa`) are available
 * @property {boolean | null} cssGradientDoublePosition two positions on one gradient color stop (`red 0% 50%`) are available
 * @property {boolean | null} cssInsetShorthand the `inset` shorthand property is available
 * @property {boolean | null} cssMediaQueryRange media query range syntax (`(width >= 600px)`) is available
 * @property {boolean | null} cssPlaceShorthand the `place-content` / `place-items` / `place-self` shorthand properties are available
 */
...
/** @typedef {Mix<Mix<PlatformTargetProperties, ElectronContextTargetProperties>, Mix<ApiTargetProperties, Mix<EcmaTargetProperties, CssTargetProperties>>>} TargetProperties */
...
// A `browserslist` target, with an optional `:env` / `:query` / config path
// after the keyword that `browserslistTargetHandler.load` reads.
const BROWSERSLIST_TARGET = /^browserslist(?::(.+))?$/;
	...
	[
		...
		(asyncFlag, _, major, minor) => {
			...
			return {
				...
				module: v(12, 17),
				// v24.5.0, for WebAssembly only; `import defer` is not implemented
				sourceImport: v(24, 5)
			};
		}
	],
...
// Absence normally means "no opinion", but for these it means "cannot read it", so a
// target omitting one votes against rather than letting another's `true` stand for the
// whole set — `["web", "deno2.8"]` reads neither phase.
const UNSUPPORTED_WHEN_ABSENT = new Set(["deferImport", "sourceImport"]);
		...
		for (const tp of targetProperties) {
			...
			switch (value) {
				...
				case false:
					...
					break;
				default:
					if (value === undefined && UNSUPPORTED_WHEN_ABSENT.has(key)) {
						hasFalse = true;
					}
			}
		}
...
/**
 * The raw browserslist selection behind the targets (`["chrome 100", …]`), merged
 * across every `browserslist` target and deduplicated — undefined when none is
 * one, and prefixing stays off with no browser list to prefix for.
 * @param {string[]} targets the targets
 * @param {string} context the context directory
 * @returns {string[] | undefined} the selected browsers, or undefined
 */
const getTargetsBrowsers = (targets, context) => {
	const { load } = getBrowserslistTargetHandler();
	/** @type {Set<string>} */
	const browsers = new Set();
	for (const target of targets) {
		const match = BROWSERSLIST_TARGET.exec(target);
		if (!match) continue;
		const selected = load(match[1] ? match[1].trim() : null, context);
		if (selected) for (const browser of selected) browsers.add(browser);
	}
	return browsers.size === 0 ? undefined : [...browsers];
};

module.exports.getDefaultTarget = getDefaultTarget;
module.exports.getTargetProperties = getTargetProperties;
module.exports.getTargetsBrowsers = getTargetsBrowsers;
