...
// The half of browserslist that finds a config, the same module instance the
// queries use, so its caches stay shared. A packaging change could stop
// resolving it, hence the fallback to loading browserslist itself.
const getBrowserslistConfigLoader = memoize(() => {
	try {
		return require("browserslist/node");
	} catch (_err) {
		return undefined;
	}
});
...
const getDefaultTarget = (context) => {
	// WHY: the default only turns on whether a browserslist config exists, and
	// the config half answers that without the caniuse data the queries read —
	// 1.3 MB that a build with no such config never uses.
	const configLoader = getBrowserslistConfigLoader();
	if (configLoader && !configLoader.loadConfig({ path: context })) return "web";
 ...
 * @property {boolean | null} topLevelAwait top-level await is available
...
You can also more options via the 'target' option: 'browserslist' / 'browserslist:env' / 'browserslist:query' / 'browserslist:path-to-config' / 'browserslist:path-to-config:env'`);
	...
	[
		...
		/^bun((\d+)(?:\.(\d+))?)?$/,
		() => ({
			// Every released Bun supports the full modern feature set, so a version is
			// accepted but gates nothing. Output is always ESM, leaving no synchronous
			// CommonJS `require`.
			node: true,
			bun: true,
			web: false,
