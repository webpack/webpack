...
const TARGETS = [
	// todo find out how we can do this thru a plugin
	[
		"browserslist[:[///path-to-config][>/path-to-browserslist][?query][:browserslist-env]]",
		"Resolve features from browserslist. Will resolve browserslist config automatically. `browserslist:modern` to use `modern` environment from browserslist config",
		/^browserslist(?::(.+))?$/,
		rest => {
	...
	[
		...
		"Web browser. Will automatically use browserslist config",
		/^web$/,
		() => {
			return {
				web: true,
