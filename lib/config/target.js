...
// queries use, so its caches stay shared and a request for one costs nothing
// twice. browserslist's own entry requires it, so it resolves wherever that does
const getBrowserslistConfigLoader = memoize(() => require("browserslist/node"));
...
const getDefaultTarget = (context) => {
	...
	const config = getBrowserslistConfigLoader().loadConfig({ path: context });
	if (!config) return "web";
