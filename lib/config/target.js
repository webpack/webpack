...
// cspell:word browserslist,caniuse
...
/**
 * Is support ES feature
 * @param {string[]} browsers supported browsers list
 * @param {string} feature an ES feature to test
 * @returns {boolean} true if supports
 */
const isESFeatureSupported = (browsers, feature) => {
	const supportsArrowFunction = browserslist(`supports ${feature}`);
	return browsers.every(v => supportsArrowFunction.includes(v));
};

/**
 * Resolve ES feature from browserslist
 * @param {string} env browserslist env name
 * @returns {EcmaTargetProperties} map of supported ES features
 */
const resolveESFeaturesFromBrowserslist = env => {
	const config = browserslist.loadConfig({ path: process.cwd(), env });
	const browsers = browserslist(config);

	return {
		const: isESFeatureSupported(browsers, "es6"),
		arrowFunction: isESFeatureSupported(browsers, "es6"),
		forOf: isESFeatureSupported(browsers, "es6"),
		destructuring: isESFeatureSupported(browsers, "es6"),
		bigIntLiteral: isESFeatureSupported(browsers, "bigint"),
		module: isESFeatureSupported(browsers, "es6-module"),
		dynamicImport: isESFeatureSupported(browsers, "es6-module-dynamic-import"),
		dynamicImportInWorker: isESFeatureSupported(
			browsers,
			"es6-module-dynamic-import"
		),
		// caniuse browserslist does not have info about globalThis
...
const TARGETS = [
	[
		"web[:browserslist-env]",
		"Web browser. Will automatically use browserslist config. `web:modern` will use `modern` environment from browserslist config",
		/^web(?::(.+))?$/,
		browserslistEnv => {
			return {
				web: true,
				...
				...resolveESFeaturesFromBrowserslist(browserslistEnv)
			};
		}
	],
	[
		"webworker[:browserslist-env]",
		"Web Worker, SharedWorker or Service Worker. Will automatically use browserslist config. `webworker:modern` will use `modern` environment from browserslist config",
		/^webworker(?::(.+))?$/,
		browserslistEnv => {
			return {
				web: true,
