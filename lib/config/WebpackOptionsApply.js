...
/** @import { BuildDependencyItem } from "../../declarations/WebpackOptions" */
...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
...

		{
			...
			for (const key of Object.keys(options.buildDependencies)) {
				for (const item of options.buildDependencies[key]) {
					buildDependencies.push(item);
				}
			}
			if (buildDependencies.length > 0) {
				const addPlugin = new AddBuildDependenciesPlugin(buildDependencies);
				addPlugin.apply(compiler);
				if (options.watchOptions.buildDependencies !== false) {
					const WatchBuildDependenciesPlugin = require("../watch/WatchBuildDependenciesPlugin");

					new WatchBuildDependenciesPlugin(addPlugin.buildDependencies).apply(
						compiler
					);
				}
			}
