...
const CommonJsPlugin = require("../dependencies/commonjs/CommonJsPlugin");
const RequireEnsurePlugin = require("../dependencies/commonjs/RequireEnsurePlugin");
const RequireIncludePlugin = require("../dependencies/commonjs/RequireIncludePlugin");
const ImportMetaContextPlugin = require("../dependencies/context/ImportMetaContextPlugin");
const RequireContextPlugin = require("../dependencies/context/RequireContextPlugin");
const ESMModulesPlugin = require("../dependencies/esm/ESMModulesPlugin");
const ImportPlugin = require("../dependencies/import/ImportPlugin");
const ImportMetaPlugin = require("../dependencies/import-meta/ImportMetaPlugin");

const SystemPlugin = require("../dependencies/system/SystemPlugin");
const URLPlugin = require("../dependencies/url/URLPlugin");

const WorkerAndWorkletPlugin = require("../dependencies/worker/WorkerAndWorkletPlugin");
const WarnCaseSensitiveModulesPlugin = require("../diagnostics/WarnCaseSensitiveModulesPlugin");
const EntryOptionPlugin = require("../entry/EntryOptionPlugin");
const RecordIdsPlugin = require("../ids/RecordIdsPlugin");
const APIPlugin = require("../javascript/APIPlugin");
const CompatibilityPlugin = require("../javascript/CompatibilityPlugin");
const ConstPlugin = require("../javascript/ConstPlugin");
const ExportsInfoApiPlugin = require("../javascript/ExportsInfoApiPlugin");
const JavascriptMetaInfoPlugin = require("../javascript/JavascriptMetaInfoPlugin");

const JavascriptModulesPlugin = require("../javascript/JavascriptModulesPlugin");
const UseStrictPlugin = require("../javascript/UseStrictPlugin");
const WebpackIsIncludedPlugin = require("../javascript/WebpackIsIncludedPlugin");

const JsonModulesPlugin = require("../json/JsonModulesPlugin");
const LoaderPlugin = require("../loaders/LoaderPlugin");
const NodeStuffPlugin = require("../node/NodeStuffPlugin");
const FlagDependencyExportsPlugin = require("../optimize/FlagDependencyExportsPlugin");
...
const RuntimePlugin = require("../runtime/RuntimePlugin");
...
const TemplatedPathPlugin = require("../template/TemplatedPathPlugin");
 ...
 * } from "./defaults"
 */
/** @import { WebpackOptionsInterception } from "./normalization" */
/** @import Compiler from "../Compiler" */
/** @import { InputFileSystem, IntermediateFileSystem } from "../fs/fs" */
...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler, interception) {
		...
		if (options.output.clean) {
			const CleanPlugin = require("../output/CleanPlugin");
		...
		if (options.output.copy) {
			const CopyPlugin = require("../output/CopyPlugin");

			new CopyPlugin(options.output.copy).apply(compiler);
		}

		if (options.dotenv) {
			const DotenvPlugin = require("../define/DotenvPlugin");
		...
		for (const item of devtool) {
			...
			if (use) {
				if (use.includes("source-map")) {
					...
					const scopes = use.includes("scopes");
					const Plugin = evalWrapped
						? require("../devtool/EvalSourceMapDevToolPlugin")
						: require("../devtool/SourceMapDevToolPlugin");
					const assetExt =
						type === "javascript"
							? /\.((c|m)?js)($|\?)/i
							: type === "css"
					...
					const EvalDevToolModulePlugin = require("../devtool/EvalDevToolModulePlugin");
		...
		if (options.experiments.asyncWebAssembly) {
			const AsyncWebAssemblyModulesPlugin = require("../wasm/AsyncWebAssemblyModulesPlugin");
		...
		if (options.experiments.lazyCompilation) {
			...
			if (isUniversalTarget) {
				...
				const NormalModuleReplacementPlugin = require("../resolve/NormalModuleReplacementPlugin");

				// Override emitter that using `EventEmitter` to `EventTarget`
				// TODO in the next major release: migrate to `EventTarget` by default
		...
		new ESMModulesPlugin({
			deferImport: options.experiments.deferImport
		}).apply(compiler);
		if (options.amd !== false) {
			const AMDPlugin = require("../dependencies/amd/AMDPlugin");
			const RequireJsStuffPlugin = require("../dependencies/amd/RequireJsStuffPlugin");
		...
		if (typeof options.mode !== "string") {
			const WarnNoModeSetPlugin = require("../diagnostics/WarnNoModeSetPlugin");
		...
		if (options.optimization.usedExports) {
			const FlagDependencyUsagePlugin = require("../optimize/FlagDependencyUsagePlugin");
		...
		const circularHints =
			options.performance && options.performance.circularDependencies
				...
				: false;
		// ConstExportsPlugin Binding checks need CircularModulesPlugin when this is on.
		const inlineExports = options.optimization.inlineExports;

		if (options.mode === "production" || circularHints || inlineExports) {
		...
		if (!options.optimization.emitOnErrors) {
			const NoEmitOnErrorsPlugin = require("../diagnostics/NoEmitOnErrorsPlugin");

			new NoEmitOnErrorsPlugin().apply(compiler);
		}
		// ESM output may bake a name into a chunk whose hashed name would then go stale;
		// the repair runs for those alone where it was not asked for outright.
		if (options.optimization.realContentHash || options.output.module) {
			...
			new RealContentHashPlugin({
				...
				onDemand: !options.optimization.realContentHash
			}).apply(compiler);
		}
		...
		if (moduleIds) {
			switch (moduleIds) {
				...
				case "hashed": {
					const WarnDeprecatedOptionPlugin = require("../diagnostics/WarnDeprecatedOptionPlugin");
		...
		if (options.optimization.nodeEnv) {
			const DefinePlugin = require("../define/DefinePlugin");
			...
			// Marked internal: these keys are webpack's, so `performance.unusedConfig`
		...
		if (options.performance) {
			...
			if (options.performance.analyzableBailouts) {
				const AnalyzableBailoutsPlugin = require("../performance/AnalyzableBailoutsPlugin");

				new AnalyzableBailoutsPlugin(options.performance).apply(compiler);
			}
			...
			// One switch for source maps: a devtool that ships the map to everyone,
			// and a loader that loses it, are both about the map being worth its cost.
			if (options.performance.sourceMaps) {
				const EmbeddedSourceMapsPlugin = require("../performance/EmbeddedSourceMapsPlugin");
				const MissingSourceMapsPlugin = require("../performance/MissingSourceMapsPlugin");

				new EmbeddedSourceMapsPlugin(options.performance).apply(compiler);
				new MissingSourceMapsPlugin(options.performance).apply(compiler);
			}

			// One switch for configuration that did nothing: the four read different
			// parts of the config, but the finding and the fix are the same.
			if (options.performance.unusedConfig) {
			...
			// One switch for a module nothing uses: the two differ only in what kept
			// it there, and each names the cause it knows about.
			if (options.performance.unusedModules) {
				const UnusedModulesPlugin = require("../performance/UnusedModulesPlugin");
				const UnusedReexportsPlugin = require("../performance/UnusedReexportsPlugin");

				new UnusedModulesPlugin(options.performance).apply(compiler);
		...
		if (options.ignoreWarnings && options.ignoreWarnings.length > 0) {
			const IgnoreWarningsPlugin = require("../diagnostics/IgnoreWarningsPlugin");
		...
		/**
		 * Supplies the compiler's input filesystem as the default for a resolver.
		 * A filesystem set in the configuration is kept, so that `resolve.fileSystem`
		 * reaches enhanced-resolve the way `resolve.resolver` already does.
		 * @param {ResolveOptionsWithDependencyType} resolveOptions merged resolve options
		 * @returns {ResolveOptionsWithDependencyType} the same options
		 */
		const withFileSystem = (resolveOptions) => {
			if (!resolveOptions.fileSystem) {
		...
		compiler.resolverFactory.hooks.resolveOptions
			...
			.tap(CLASS_NAME, (resolveOptions) =>
				withFileSystem(cleverMerge(options.resolve, resolveOptions))
			);
		compiler.resolverFactory.hooks.resolveOptions
			.for("context")
			.tap(CLASS_NAME, (resolveOptions) => {
				resolveOptions = withFileSystem(
					cleverMerge(options.resolve, resolveOptions)
				);
		...
		compiler.resolverFactory.hooks.resolveOptions
			.for("loader")
			.tap(CLASS_NAME, (resolveOptions) =>
				withFileSystem(cleverMerge(options.resolveLoader, resolveOptions))
			);
