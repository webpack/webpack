...
const { ConcatSource, RawSource, util } = require("webpack-sources");
...
const generateDebugId = require("./generateDebugId");

/** @import { RawSourceMap, Source } from "webpack-sources" */
/**
 * @import {
 * 	DevtoolNamespace,
...
class EvalSourceMapDevToolPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			if (options.scopes) {
				compilation.dependencyTemplates.enableImportBindingScopes();
			}
			...
			hooks.renderModuleContent.tap(
				...
				(source, m, { chunk, runtimeTemplate, chunkGraph }) => {
					...
					if (options.scopes && codeGenerationResults) {
						// The clone above shares `names` with the source's own map, and
						// encoding appends to it.
						sourceMap.names = sourceMap.names ? [...sourceMap.names] : [];
						util.scopes.addScopesToSourceMap(sourceMap, (sourceIndex) => {
							...
							return codeGenerationResults.getData(
								module,
								chunk.runtime,
								"importBindings"
							);
