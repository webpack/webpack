...
const { dirname, relative } = require("../fs/fs");
const ProgressPlugin = require("../logging/ProgressPlugin");
 ...
 */
/** @import { OutputFileSystem } from "../fs/fs" */
 ...
 * A rule condition.
 * @inline
 * @typedef {Rule} RulesBranch1Item
 */

/**
 * Append no SourceMap comment to the bundle, but still generate SourceMaps.
 * @inline
 * @typedef {(false | null)} SourceMapDevToolPluginOptionsAppendBranch1
 */

/**
 * Custom function generating the identifier.
 * @inline
 * @typedef {import("./ModuleFilenameHelpers").ModuleFilenameTemplateFunction} SourceMapDevToolPluginOptionsFallbackModuleFilenameTemplateBranch2
 */

/**
 * Disable separate SourceMap file and inline SourceMap as DataUrl.
 * @inline
 * @typedef {(false | null)} SourceMapDevToolPluginOptionsFilenameBranch1
 */

/**
 * Custom function generating the identifier.
 * @inline
 * @typedef {import("./ModuleFilenameHelpers").ModuleFilenameTemplateFunction} SourceMapDevToolPluginOptionsModuleFilenameTemplateBranch2
 */

/**
 * Condition used to match resource (string, RegExp or Function).
 * @typedef {RegExp | NonEmptyString | import("./ModuleFilenameHelpers").MatcherFn} Rule
 */

/**
 * One or multiple conditions used to match resource.
 * @typedef {RulesBranch1Item[] | Rule} Rules
 */

/**
 * @schema plugins/SourceMapDevToolPlugin
 * @typedef {object} SourceMapDevToolPluginOptions
 * @property {SourceMapDevToolPluginOptionsAppendBranch1 | NonEmptyString | import("../template/TemplatedPathPlugin").TemplatePathFn=} append Appends the given value to the original asset. Usually the #sourceMappingURL comment. [url] is replaced with a URL to the source map file. false disables the appending.
 * @property {boolean=} columns Indicates whether column mappings should be used (defaults to true).
 * @property {boolean=} debugIds Emit debug IDs into source and SourceMap.
 * @property {Rules=} exclude Exclude modules that match the given value from source map generation.
 * @property {NonEmptyString | SourceMapDevToolPluginOptionsFallbackModuleFilenameTemplateBranch2=} fallbackModuleFilenameTemplate Generator string or function to create identifiers of modules for the 'sources' array in the SourceMap used only if 'moduleFilenameTemplate' would result in a conflict.
 * @property {string=} fileContext Path prefix to which the [file] placeholder is relative to.
 * @property {SourceMapDevToolPluginOptionsFilenameBranch1 | NonEmptyRelativePath=} filename Defines the output filename of the SourceMap (will be inlined if no value is provided).
 * @property {Rules=} ignoreList Decide whether to ignore source files that match the specified value in the SourceMap.
 * @property {Rules=} include Include source maps for module paths that match the given value.
 * @property {boolean=} module Indicates whether SourceMaps from loaders should be used (defaults to true).
 * @property {NonEmptyString | SourceMapDevToolPluginOptionsModuleFilenameTemplateBranch2=} moduleFilenameTemplate Generator string or function to create identifiers of modules for the 'sources' array in the SourceMap.
 * @property {string=} namespace Namespace prefix to allow multiple webpack roots in the devtools.
 * @property {boolean=} noSources Omit the 'sourceContents' array from the SourceMap.
 * @property {string=} publicPath Provide a custom public path for the SourceMapping comment.
 * @property {string=} sourceRoot Provide a custom value for the 'sourceRoot' property in the SourceMap.
 * @property {Rules=} test Include source maps for modules based on their extension (defaults to .js and .css).
 */
