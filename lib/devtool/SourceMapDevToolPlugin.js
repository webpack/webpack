							...
							asyncLib.each(
								...
								(task, callback) => {
									...
									if (options.scopes && codeGenerationResults) {
										// The spread above shares `names` with a map a
										// `SourceMapSource` may hold, and encoding appends to it.
										outputSourceMap.names = sourceMap.names
											? [...sourceMap.names]
											: [];
										const runtime = chunk && chunk.runtime;
										addScopesToSourceMap(outputSourceMap, (sourceIndex) => {
											const module = modules[sourceIndex];
											if (
												typeof module === "string" ||
												!codeGenerationResults.has(module, runtime)
											) {
												return;
											}
