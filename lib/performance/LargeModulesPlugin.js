...
// What `performance.maxAssetSize` defaults to, for a plugin built by hand.
const DEFAULT_MAX_ASSET_SIZE = 250000;
...
class LargeModulesPlugin {
	...
	apply(compiler) {
		...
		// Falls back to what `maxAssetSize` defaults to: reading it as `undefined`
		// would make the floor `NaN`, which no size is ever below.
		const maxAssetSize =
			typeof this.maxAssetSize === "number"
				? this.maxAssetSize
				: DEFAULT_MAX_ASSET_SIZE;
		const minSize = maxAssetSize * MIN_SHARE_OF_ASSET_LIMIT;
			...
			compilation.hooks.afterSeal.tap(PLUGIN_NAME, () => {
				...
				for (const chunk of compilation.chunks) {
					...
					for (const [module, size] of sizes) {
						...
						// One module can weigh down several chunks; the heaviest is the
						// one worth naming, and it stands for the rest.
						if (previous && previous.size >= size) continue;
