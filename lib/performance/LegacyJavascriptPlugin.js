...
// Packages whose whole purpose is to emulate language syntax. A module of one of
// these in a build whose target has the syntax is weight for nobody.
//
// `core-js` is deliberately absent: it polyfills APIs rather than syntax, and
// `output.environment` says nothing about whether the target has `Array#flat`,
// so a target with every feature below can still need it. Naming it here would
// take a package's presence as proof of something the option cannot answer.
...
const emulatingPackage = (resource) => {
	for (const name of EMULATING_PACKAGES) {
		// Matched against the path rather than the request, so a module reached
		// through a re-export or an alias is still recognized.
		if (resource.includes(`node_modules/${name}/`)) return name;
		if (resource.includes(`node_modules\\${name.replace("/", "\\")}\\`)) {
			return name;
		}
	}
