		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				compilation.hooks.afterSeal.tap(PLUGIN_NAME, () => {
					...
					/**
					 * Drops every alias the request matches.
					...
					// An absolute name is matched against the path the resolver saw,
					// which for a relative request is the issuer's context joined with
					// it — a request that never appears as a `rawRequest`.
						...
						for (const module of compilation.modules) {
							...
							while (blocks.length > 0) {
								...
								for (const dependency of block.dependencies) {
									...
									consume(join(fs, context, request));
								}
