...
EmittedAssetSizeLimitPlugin.prototype.apply = function(compiler) {
	...
	compiler.plugin("after-emit", function(compilation, callback) {
		...
		var assets = Object.keys(compilation.assets).map(function(asset) {
			...
			if(formatSize(obj.size).number > sizeLimit) {
		...
		if(assetsOverSizeLimit.length) {
			warnings.push(new AssetsOverSizeLimitWarning(assetsOverSizeLimit, compilation, formatSize));
			warnings.push(new EntrypointsOverSizeLimitWarning(entrypointsOverSizeLimit, compilation, formatSize));
		} else {
