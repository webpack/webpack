...
const everyImportBinds = (module, moduleGraph) => {
	for (const connection of moduleGraph.getIncomingConnections(module)) {
		if (connection.dependency instanceof ESMImportBareSideEffectDependency) {
			return false;
		}
	}
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.optimizeModules.tap(PLUGIN_NAME, (modules) => {
				...
				for (const module of modules) {
					...
					// Recorded while parsing, so a module restored from the cache still has it.
					// Absent means the module is kept for a reason this hint cannot name.
					const diagnostics =
						/** @type {BuildInfo} */
						(module.buildInfo).diagnostics;
					const statement =
						diagnostics === undefined
							? undefined
							: diagnostics.sideEffectStatement;
