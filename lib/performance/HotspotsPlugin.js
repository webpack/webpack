...
/**
 * @param {Map<string, number>} map where the time is kept
...
class HotspotsPlugin {
	...
	apply(compiler) {
		 ...
		 * @template T
		 * @param {Map<string, number>} map where its time is kept
		...
		const measure = (map, name, hookName, run) => {
			...
			try {
				...
				if (hookName !== undefined) charge(hookTime, hookName, own);
				...
				for (const [kind, map] of /** @type {const} */ ([
					...
					for (const [name, ms] of map) {
						...
						hotspots.push({
							...
							name,
							ms,
							runs: /** @type {number} */ (runs.get(name))
						});
					}
				}
