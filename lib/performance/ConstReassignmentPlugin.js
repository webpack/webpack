...
const ConstReassignmentWarning = require("../errors/ConstReassignmentWarning");
...
/** @import { ConstReassignmentDetails } from "../errors/ConstReassignmentWarning" */

const PLUGIN_NAME = "ConstReassignmentPlugin";

// Enough to name the offenders without listing every module.
const MAX_REPORTED_MODULES = 5;

class ConstReassignmentPlugin {
	/**
	 * Creates an instance of ConstReassignmentPlugin.
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser) => {
					parser.hooks.program.tap(PLUGIN_NAME, (ast) => {
						...
						const found = [...bindings.assigned]
							.filter(
								(name) =>
									isSoleBinding(bindings, name) &&
									(bindings.declared.get(name) || []).includes("const")
							)
							.sort(compareStrings);
						...
						buildInfo.constReassignments = found;
					});
				};
				...
				compilation.hooks.afterSeal.tap(PLUGIN_NAME, () => {
					...
					for (const parent of compilation.modules) {
						...
						for (const module of getSourceModules(parent)) {
							...
							const found = buildInfo.constReassignments;
					...
					const warning = new ConstReassignmentWarning(
...
module.exports = ConstReassignmentPlugin;
