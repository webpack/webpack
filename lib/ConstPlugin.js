...
class ConstPlugin {
				...
				const handler = parser => {
					...
					parser.hooks.statementIf.tap(PLUGIN_NAME, statement => {
						...
						if (typeof bool === "boolean") {
							...
							if (branchToRemove) {
								this.eliminateDeadStatement(parser, branchToRemove);
							}
							return bool;
						}
					});
					parser.hooks.deadStatement.tap(PLUGIN_NAME, statement => {
						if (
							...
							return;
						this.eliminateDeadStatement(parser, statement);
						return true;
					});
	...
	/**
	 * Eliminate a dead statement.
	 * @param {JavascriptParser} parser the parser
	 * @param {Statement} statement the statement to remove
	 * @returns {void}
	 */
	eliminateDeadStatement(parser, statement) {
