	...
	while (stack.length > 0) {
		...
		switch (node.type) {
			...
			case "FunctionDeclaration":
				if (includeFunctionDeclarations) collectDeclaration(declarations, node.id);
				break;
			case "VariableDeclaration":
				if (node.kind === "var") for (const decl of node.declarations) collectDeclaration(declarations, decl.id);
				break;
		}
	}
