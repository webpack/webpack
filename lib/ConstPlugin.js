		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = parser => {
					...
					const definedSymbolIdentifierHandler = (name, expr) => {
						return evaluateToIdentifier(name, name, () => [], true)(expr);
					};
					parser.hooks.evaluateIdentifier
						.for("Symbol.iterator")
						.tap(PLUGIN_NAME, expr =>
							definedSymbolIdentifierHandler("Symbol.iterator", expr)
						);
					parser.hooks.evaluateIdentifier
						.for("Symbol.asyncIterator")
						.tap(PLUGIN_NAME, expr =>
							definedSymbolIdentifierHandler("Symbol.asyncIterator", expr)
						);
					parser.hooks.evaluateIdentifier
						.for("__resourceQuery")
						.tap(PLUGIN_NAME, expr => {
