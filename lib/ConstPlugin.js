...
/**
 * True when evaluating the operand row can only produce an unknown value,
 * so the const-folding taps provably bail without observing their node.
 * @param {JavascriptParser} parser the parser
 * @param {import("./javascript/syntax").SoaAst} store column store
 * @param {number} operandId operand row (0 for a foreign operand)
 * @returns {boolean} true when folding is provably impossible
 */
const evalProvablyUnknown = (parser, store, operandId) =>
	parser._soaEvalUnknowable(store, operandId);
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser) => {
					...
					/**
					 * @param {import("estree").IfStatement} statement if statement
					 * @returns {boolean | void} test verdict when folded
					 */
					const statementIfHandler = (statement) => {
					...
					parser.hooks.statementIf.tap(PLUGIN_NAME, statementIfHandler);
					// an unknowable test provably cannot fold the statement
					parser.registerColumnGate(
						parser.hooks.statementIf,
						statementIfHandler,
						(_parser, store, id) =>
							!parser.scope.isAsmJs &&
							!evalProvablyUnknown(parser, store, store.kid0[id])
					);
					...
					/**
					 * @param {import("estree").ConditionalExpression} expression conditional expression
					 * @returns {boolean | void} test verdict when folded
					 */
					const conditionalOperatorHandler = (expression) => {
						if (parser.scope.isAsmJs) return;
					...
					parser.hooks.expressionConditionalOperator.tap(
						PLUGIN_NAME,
						conditionalOperatorHandler
					);
					// an unknowable test provably cannot fold either branch
					parser.registerColumnGate(
					...
					/**
					 * @param {import("estree").LogicalExpression} expression logical expression
					 * @returns {boolean | void} keep-right verdict when folded
					 */
					const logicalOperatorHandler = (expression) => {
						if (parser.scope.isAsmJs) return;
					...
					// an unknowable left operand provably cannot short-circuit
					parser.registerColumnGate(
