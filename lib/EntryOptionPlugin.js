	...
	static applyEntryOption(compiler, context, entry) {
		if (typeof entry === "function") {
			...
			for (const name of Object.keys(entry)) {
				...
				// `output.html` (or entry `html`) wraps a non-HTML entry in a
				// synthetic HTML module, so the existing HTML pipeline injects the
				// entry's JS/CSS chunks and applies the `template` option.
				if (html && !descImport.every((r) => /\.html(\?|$)/i.test(r))) {
					const tags = descImport
						.map((r) =>
							/\.css(\?|$)/i.test(r)
						...
						.join("");
					const request = `data:text/html,<!doctype html><html><head></head><body>${tags}</body></html>`;
