...
Plugins which provide custom library types must call EnableLibraryPlugin.setEnabled(compiler, type) to disable this error.`);
			...
			const pluginInstance =
				/** @type {{ apply?: (compiler: Compiler) => void } | null} */
				(type);
			if (pluginInstance && typeof pluginInstance.apply === "function") {
				pluginInstance.apply(compiler);
			} else {
				throw new Error(
					`Unsupported library type ${typeof type === "object" ? JSON.stringify(type) : String(type)}.`
				);
			}
		}
	}
