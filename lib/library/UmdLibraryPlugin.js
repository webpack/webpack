...
const {
	getChunkDependencyPaths,
	toAmdModuleId,
	updateHashForChunkDependencies
} = require("./chunkDependencies");
...
/** @import { RequestRecord } from "../externals/ExternalModule" */
 ...
 * @property {boolean | undefined} declareChunkDependencies
 */
...
class UmdLibraryPlugin extends AbstractLibraryPlugin {
	...
	parseOptions(library) {
		...
		return {
			...
			declareChunkDependencies: library.declareChunkDependencies
		};
	}
	 ...
	 * @returns {void}
	 */
	chunkHash(chunk, hash, chunkHashContext, { options, chunkGraph }) {
		if (!options.declareChunkDependencies) return;
		hash.update("UmdLibraryPlugin declareChunkDependencies");
		updateHashForChunkDependencies(hash, chunk, chunkGraph);
	}
	...
	render(
		...
		// Only the AMD branch can state these: a loader is the one host that can fetch
		// them. The other branches leave it to whoever loads the bundle.
		const chunkDependencies = options.declareChunkDependencies
			? getChunkDependencyPaths(chunk, chunkGraph, compilation).map(
					toAmdModuleId
				)
			: [];

		/**
		 * Externals deps array, with the chunks a loader must fetch first appended.
		...
		const externalsDepsArray = (modules) =>
			`[${[
				...
				...chunkDependencies.map((id) => JSON.stringify(id))
			]
				.filter(Boolean)
				.join(", ")}]`;
		...
		const defineCall = (defineFunction) => {
			const deps =
				requiredExternals.length > 0 || chunkDependencies.length > 0
