...
class UmdLibraryPlugin extends AbstractLibraryPlugin {
	...
	render(
		...
		const getRoot = () => {
			// In Web Workers `this` is `undefined` so we fallback on `self`
			return `typeof ${runtimeTemplate.outputOptions.globalObject} !== "undefined" ? ${runtimeTemplate.outputOptions.globalObject} : root || self`;
		};
