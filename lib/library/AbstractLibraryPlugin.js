...
class AbstractLibraryPlugin {
		...
		compiler.hooks.thisCompilation.tap(_pluginName, (compilation) => {
			...
			if (this._finishAllEntryModules) {
				hooks.inlineInRuntimeBailout.tap(_pluginName, (module, context) => {
					...
					if (
						this._getMergeableEntryModules(compilation.chunkGraph, chunk)
							.length > 0
					) {
	 ...
	 * @param {Module} startupModule the entry module whose exports are already assigned to the exports object by the startup code
	 * @param {StartupRenderContext} renderContext render context
	 * @returns {string} the merge code or an empty string
	 */
	_renderEntryExportsMerge(startupModule, renderContext) {
		...
		// the startup module's exports are already in the exports object, all
		// other entry modules are merged via the (cached) require function
		for (const module of entries.filter((m) => m !== startupModule).reverse()) {
