				...
				() => {
					for (const [
						...
						if (options !== false) {
							...
							// which is the last one unless the exports are merged.
							const exportingDeps = mergeExports ? deps : deps.slice(-1);
							for (const dep of exportingDeps) {
