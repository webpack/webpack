			...
			compilation.hooks.finishModules.tap(
				...
				() => {
					for (const [
						...
						const options = this._parseOptionsCached(resolvedLibrary);
						if (options !== false) {
							...
							// Only the exported entry modules must survive tree shaking,
							// which is the last one unless their exports are merged.
							if (
								resolvedLibrary !== undefined &&
								resolvedLibrary.entryExports === "all"
							) {
								for (const dep of deps) {
									const module = compilation.moduleGraph.getModule(dep);
									if (module) {
										this.finishEntryModule(module, name, libraryContext);
									}
								}
							} else {
								const dep = deps[deps.length - 1];
								const module = dep && compilation.moduleGraph.getModule(dep);
								if (module) {
									this.finishEntryModule(module, name, libraryContext);
								}
							}
