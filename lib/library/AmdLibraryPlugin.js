...
class AmdLibraryPlugin extends AbstractLibraryPlugin {
	...
	parseOptions(library) {
		...
		return {
			...
			globalObject: /** @type {string=} */ (globalObject)
		};
	}
	...
	render(
		...
		if (options.globalObject) {
			globalObjectPrefix = `${options.globalObject}.`;
		}

		if (this.requireAsWrapper) {
			return new ConcatSource(
				`${globalObjectPrefix}require(${externalsDepsArray}, ${fnStart}`,
				source,
				`${fnEnd});`
			);
			...
			return new ConcatSource(
				`${globalObjectPrefix}define(${JSON.stringify(
					name
			...
			return new ConcatSource(
				`${globalObjectPrefix}define(${fnStart}`,
				source,
				`${fnEnd});`
			);
		}
