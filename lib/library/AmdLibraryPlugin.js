...
class AmdLibraryPlugin extends AbstractLibraryPlugin {
	...
	render(
		...
		if (this.requireAsWrapper) {
			...
			return new ConcatSource(
				`${options.globalObjectPrefix}define(${JSON.stringify(
					name
			...
			return new ConcatSource(
				`${options.globalObjectPrefix}define(${fnStart}`,
				source,
				`${fnEnd});`
			);
		}
	...
	chunkHash(chunk, hash, chunkHashContext, { options, compilation }) {
		...
		if (this.requireAsWrapper) {
			...
			hash.update(name);
		} else if (options.globalObjectPrefix) {
			hash.update("globalObjected");
			hash.update(options.globalObjectPrefix);
		}
	}
}
