 ...
 * @property {boolean=} declareChunkDependencies
 */
...
class AmdLibraryPlugin extends AbstractLibraryPlugin {
	...
	parseOptions(library) {
		const { name, amdContainer, declareChunkDependencies } = library;
		...
		return {
			name: _name,
			amdContainer: _amdContainer,
			declareChunkDependencies
		};
	}
	...
	render(
		...
		// The loader has no other way to learn that a chunk without a runtime of its
		// own can't be evaluated until the chunks carrying it have been.
		const dependencies = [
			...externals.map((m) =>
			...
			...(options.declareChunkDependencies
				? getChunkDependencyPaths(chunk, chunkGraph, compilation).map(
						toAmdModuleId
					)
				: [])
		];
		const externalsDepsArray = JSON.stringify(dependencies);
		...
		} else if (externalsArguments || dependencies.length > 0) {
			return new ConcatSource(
	...
	chunkHash(
		...
		hash,
		chunkHashContext,
		{ options, compilation, chunkGraph }
	) {
		hash.update("AmdLibraryPlugin");
		if (options.declareChunkDependencies) {
			hash.update("declareChunkDependencies");
