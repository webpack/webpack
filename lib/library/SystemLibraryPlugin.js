...
class SystemLibraryPlugin extends AbstractLibraryPlugin {
	...
	parseOptions(library) {
		const { name, declareChunkDependencies } = library;
	...
	render(source, { chunkGraph, moduleGraph, chunk }, { options, compilation }) {
		// System resolves a relative id against this module, so the chunks carrying the
		// runtime become plain dependencies with nothing to set.
		const chunkDependencies = options.declareChunkDependencies
			? getChunkDependencyPaths(chunk, chunkGraph, compilation)
			: [];
		...
		// The system.register format requires one setter per dependency.
		const externalSetters = externals.map((module, i) => {
		...
		const setters =
			externalSetters.length === 0 && chunkDependencies.length === 0
				? ""
				: Template.asString([
						"setters: [",
						Template.indent(
							[
								...externalSetters,
								...chunkDependencies.map(() => "function() {}")
							].join(",\n")
						),
						"],"
					]);
