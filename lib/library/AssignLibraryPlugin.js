...
class AssignLibraryPlugin extends AbstractLibraryPlugin {
	...
	renderStartup(
		...
		if (staticExports) {
			...
			// A merged entry exposes what its modules expose together, so the names
			// written out are theirs rather than the last module's alone.
			const mergedModules = mergedEntryModules(chunk, chunkGraph, compilation);
			const names = mergedModules
				? getMergedExports(
						mergedModules,
						moduleGraph,
						chunk.runtime
					).exports.map(({ name }) => name)
				: [...exportsInfo.orderedExports]
			...
			for (const name of names) {
				const nameAccess = propertyAccess([name]);
				result.add(
