...
/** @typedef {import("../Module").GenerationMeta} GenerationMeta */
/** @typedef {import("../javascript/JavascriptModulesPlugin").StartupRenderContext} StartupRenderContext */
/** @typedef {import("../javascript/JavascriptModulesPlugin").ModuleContentRenderContext} ModuleContentRenderContext */
...
class ModuleLibraryPlugin extends AbstractLibraryPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			const { lazyExportGeneration } =
				ConcatenatedModule.getCompilationHooks(compilation);
			lazyExportGeneration.tap(PLUGIN_NAME, (module) => {
				for (const chunk of compilation.chunkGraph.getModuleChunksIterable(
					module
				)) {
					if (compilation.chunkGraph.getNumberOfEntryModules(chunk) > 1) {
						return false;
					}
				}
				return true;
	...
	renderStartup(
		source,
		module,
		{ moduleGraph, chunk, codeGenerationResults, inlined },
		{ options, compilation }
	) {
		...
		const definitions = inlined
			? (module.generationMeta &&
					/** @type {GenerationMeta} */ module.generationMeta
						.exportsFinalName) ||
				{}
			: {};
		/** @type {string[]} */
	 ...
	 * @param {Source} source source
	 * @param {Module} module module
	 * @param {ModuleContentRenderContext} renderContext render context
	...
	renderModuleContent(
		source,
		module,
		{ moduleGraph, chunk, codeGenerationResults, factory },
		{ options, compilation }
	) {
		const result = new ConcatSource(source);
		if (
			factory &&
			module.generationMeta &&
			module.generationMeta.exportsBindingStmt
		) {
			result.add(module.generationMeta.exportsBindingStmt);
		}
		return result;
	}
}
