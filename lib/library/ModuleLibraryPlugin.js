...
/** @typedef {import("../javascript/JavascriptModule").JavascriptBuildMeta} JavascriptBuildMeta */
...
class ModuleLibraryPlugin extends AbstractLibraryPlugin {
	...
	finishEntryModule(
		...
		if (options.export) {
			...
			if (
				...
				(module.buildMeta &&
					/** @type {JavascriptBuildMeta} */ (module.buildMeta)
						.treatAsCommonJs) ||
	...
	renderStartup(source, module, renderContext, { options, compilation }) {
		...
		const treatAsCommonJs =
			module.buildMeta &&
			/** @type {JavascriptBuildMeta} */ (module.buildMeta).treatAsCommonJs;
