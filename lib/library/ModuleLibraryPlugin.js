...
class ModuleLibraryPlugin extends AbstractLibraryPlugin {
			...
			compilation.hooks.beforeCodeGeneration.tap(PLUGIN_NAME, () => {
				...
				for (const { dependencies } of compilation.entries.values()) {
					for (const dep of dependencies) {
						...
						if (
							root === module &&
							!definesOnlyNativeExports(module, moduleGraph)
						) {
							continue;
						}
	...
	renderStartup(source, module, renderContext, { options, compilation }) {
		...
		const bindingsTakenOver =
			data !== undefined && data.has("exportsBindingSource");
		...
		for (const exportInfo of exportInfos) {
			...
			// A re-export is marked provided even when its target lacks the export
			if (
				target &&
				target.export &&
				target.export[0] !== "default" &&
				moduleGraph
					.getExportsInfo(target.module)
					.isExportProvided(target.export[0]) === false
			) {
				continue;
			}
