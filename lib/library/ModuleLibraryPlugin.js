				...
				if (
					...
					if (
						...
						if (importedModule instanceof ExternalModule) {
							let attributes = "";
							if (dep.attributes) {
								const { _isLegacyAssert, ...values } = dep.attributes;
								attributes = ` ${_isLegacyAssert ? "assert" : "with"} ${JSON.stringify(values)}`;
							}
							externalReexports.add(
								`export * from "${importedModule.userRequest}"${attributes};\n`
							);
						} else {
