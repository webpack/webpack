...
class ModuleLibraryPlugin extends AbstractLibraryPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			// The same for a plain (non-concatenated) entry, whose export definitions
			// come from a dependency template rather than from concatenation. Placed
			// after the module body, which its value bindings read.
	...
	renderModuleContent(
		...
		// or as an inlined startup module wrapped in an IIFE. Value bindings read the
		// declarations above them, so those go after the body rather than before it.
