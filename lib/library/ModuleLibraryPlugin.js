	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, compilation => {
			...
			exportsDefinitions.tap(PLUGIN_NAME, (definitions, module) => {
				// Skip defining properties for tree-shakable module output, except in the following cases:
				// 1. When HMR is enabled
				if (module.hot) {
					return false;
				}

				// 2. When we have connections so not all modules were concatenated
				const connections =
				...
				// 3. When dealing with runtime and splitting chunks (requires defining wrapper for now)
