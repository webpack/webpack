...
const { mergedEntryModules } = require("../javascript/StartupHelpers");
...
/**
 * The exports of several entry modules as one ordered list, later modules
 * winning a name an earlier one also provides.
 * @param {Module[]} modules the entry modules
 * @param {ModuleGraph} moduleGraph the module graph
 * @returns {Iterable<InstanceType<ExportInfo>>} the exports to render
 */
const unionOrderedExports = (modules, moduleGraph) => {
	if (modules.length === 1) {
		return moduleGraph.getExportsInfo(modules[0]).orderedExports;
	}
	/** @type {Map<string, InstanceType<ExportInfo>>} */
	const byName = new Map();
	for (const module of modules) {
		for (const exportInfo of moduleGraph.getExportsInfo(module)
			.orderedExports) {
			byName.set(exportInfo.name, exportInfo);
		}
	}
...
/**
 * Renders the import attributes of a reexport in the syntax the author wrote,
 * since a legacy `assert` clause and a `with` clause are not interchangeable.
 * @param {ImportAttributes | undefined} attributes the import attributes
 * @returns {string} the attributes clause, empty when there are none
 */
const toAttributesClause = (attributes) => {
	if (!attributes) return "";
	const { _isLegacyAssert: isLegacyAssert, ...values } = attributes;
	return ` ${isLegacyAssert ? "assert" : "with"} ${JSON.stringify(values)}`;
};
...
class ModuleLibraryPlugin extends AbstractLibraryPlugin {
			...
			for (const connection of moduleGraph.getOutgoingConnections(module)) {
				...
				if (
					...
					if (
						...
						if (importedModule instanceof ExternalModule) {
							const request = importedModule.getResolvedRequest();
							// A property path is reported by `_checkStarReexportedExternals`,
							// and an absent one by the external's own build
							const specifier =
								typeof request === "string"
									? request
									: Array.isArray(request) && request.length === 1
										? request[0]
										: undefined;
							if (specifier !== undefined) {
								externalReexports.add(
									`export * from ${toJsStringLiteral(specifier)}${toAttributesClause(dep.attributes)};\n`
								);
							}
						} else if (!visitedReexportModules.has(importedModule)) {
							visitedReexportModules.add(importedModule);
	...
	renderStartup(source, module, renderContext, { options, compilation }) {
		...
		// A merged entry exposes the union of its modules' exports, so the ESM
		// export list is theirs together rather than the last module's alone.
		...
		const exportInfos = options.export
			...
			: unionOrderedExports(exportedModules, moduleGraph);
		...
		const isAsync = exportedModules.some((m) => moduleGraph.isAsync(m));

		const treatAsCommonJs =
			module.buildMeta &&
			/** @type {JavascriptModuleBuildMeta} */ (module.buildMeta)
				.treatAsCommonJs;
		// One ESM module among the merged ones still provides a real `default`.
		const skipRenderDefaultExport = exportedModules.every((m) =>
			Boolean(
				m.buildMeta &&
		...
		outer: for (const exportInfo of exportInfos) {
			...
			const externalTarget = exportInfo.getTarget(moduleGraph);
			if (
				externalTarget &&
				externalTarget.module instanceof ExternalModule &&
				externalTarget.export &&
				externalTarget.export.length === 1 &&
				codeGenerationResults.has(externalTarget.module, chunk.runtime)
			) {
				const reexport = codeGenerationResults.getData(
					externalTarget.module,
					chunk.runtime,
					"externalReexport"
				);
				if (reexport) {
					...
					result.add(
						`export { ${toExportName(externalTarget.export[0])} as ${toExportName(originalName)} } ${reexport};\n`
					);
