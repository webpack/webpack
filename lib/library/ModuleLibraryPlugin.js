...
const { getEntryRuntime, intersectRuntime } = require("../util/runtime");
...
class ModuleLibraryPlugin extends AbstractLibraryPlugin {
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.additionalChunkRuntimeRequirements.tap(
				...
				(chunk, set, { chunkGraph, codeGenerationResults }) => {
					...
					// An entry another module imports stays in the registry and is rendered
					// as a factory, which re-emits the stashed definitions. `dependOn` puts
					// that importer in a chunk of its own.
					const { moduleGraph } = compilation;
					if (
						someInIterable(
	...
	renderStartup(source, module, renderContext, { options, compilation }) {
		...
		// A single inlined entry renders at the top level, so its declarations are
		// bindings that can be exported directly, keeping ESM live bindings rather
		// than snapshotting them into a `const`.
