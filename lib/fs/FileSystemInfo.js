...
const { dirname, join, lstatReadlinkAbsolute, relative } = require("./fs");
...
/** @import { JsonObject, IStats, InputFileSystem } from "./fs" */
/**
 * @import {
...
/**
 * Blanks out TypeScript syntax, keeping offsets, so the ESM lexer can read a natively loaded `.ts` config.
 * @param {string} path file path
 * @param {string} source file content
 * @returns {string} source the ESM lexer can parse
 */
const stripTypes = (path, source) => {
	if (!/\.[mc]?ts$/.test(path) || !("stripTypeScriptTypes" in nodeModule)) {
		// without Node.js type stripping, lexing the raw source is the best effort
		return source;
	}
...
makeSerializable(Snapshot, "webpack/lib/fs/FileSystemInfo", "Snapshot");
registerLegacyRequest(Snapshot, "webpack/lib/FileSystemInfo", "Snapshot");
...
class FileSystemInfo {
	...
	constructor(
		...
		// memoized _classifyPath results: 0 = not managed, 1 = immutable, string = managed item
	...
	/**
	 * Classifies a path by the snapshot path options; results are memoized.
	 * @param {string} path path
	 * @returns {0 | 1 | string} 0 when unmanaged, 1 when immutable, otherwise the managed item containing the path
	 */
	_classifyPath(path) {
		const cached = this._pathClassificationCache.get(path);
		if (cached !== undefined) return cached;
		/** @type {0 | 1 | string} */
		let classification = 0;
		search: {
			for (const unmanagedPath of this.unmanagedPathsRegExps) {
				if (unmanagedPath.test(path)) break search;
			}
			for (const unmanagedPath of this.unmanagedPathsWithSlash) {
				if (path.startsWith(unmanagedPath)) break search;
			}
		...
		this._pathClassificationCache.set(path, classification);
	 ...
	 * @param {string} path path
	 * @returns {string | true | undefined} the managed item, `true` when the path is immutable, `undefined` when unmanaged
	 */
	getManagedItemOf(path) {
		const classification = this._classifyPath(path);
		if (classification === 0) return undefined;
	...
	resolveBuildDependencies(context, deps, optionalDeps, callback) {
		...
		/**
		 * Queues what an ES module imports, found by parsing it.
		 * @param {string} path path of the module
		 * @param {string} rawSource its source
		 * @param {Job} job the job reading it
		 * @param {(job: Job) => void} push queues a job
		 * @returns {void}
		 */
		const pushEsmImports = (path, rawSource, job, push) => {
			...
			try {
				...
				const source = stripTypes(path, rawSource);
				...
				for (const imp of imports) {
					try {
						...
						/** @type {string | null} */
						const dependency =
							imp.n || parseString(source.slice(imp.s, imp.e).trim());
						...
						if (
							...
							if (IS_BUN && module.children.length === 0) {
								this.fs.readFile(path, (err, content) => {
									...
									// Bun lists ES modules in `require.cache` too, also without children
									if (!supportsEsm || !/\.(?:m?js|[mc]?ts)$/.test(path)) {
										return process.nextTick(callback);
									}
									getEsModuleLexer().init.then(() => {
										pushEsmImports(path, source, job, push);
										process.nextTick(callback);
									}, callback);
								});
								break;
							}
						} else if (supportsEsm && /\.(?:m?js|[mc]?ts)$/.test(path)) {
							getEsModuleLexer().init.then(() => {
								this.fs.readFile(path, (err, content) => {
									if (err) return callback(err);
									pushEsmImports(
	...
	createSnapshot(startTime, files, directories, missing, options, callback) {
		...
		const checkManaged = (path, managedSet) => {
			...
			if (classification !== 1) managedItems.add(classification);
			managedSet.add(path);
			return true;
		};
		/**
