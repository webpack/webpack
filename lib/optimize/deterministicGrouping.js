	...
	if (initialNodes.length > 0) {
		...
		if (initialGroup.nodes.length > 0) {
			...
			while (queue.length) {
				...
				// WHY: both sides already clear minSize here.
				//      left v   v right
				// [ O O O ] O O O [ O O O ]
				// ^^^^^^^^^ leftSize
				//       rightSize ^^^^^^^^^
				...
				if (left - 1 > right) {
					...
					// No split holds minSize, which is preferred over maxSize, so these
					// nodes are returned oversized. Keeping maxSize > minSize * 3
					// avoids it.
					result.push(group);
					continue;
				}
				if (left <= right) {
					// In the area between left and right, split at the minimum
					// similarity, where the key space is separated the most — without
					// creating groups that are too small.
					let best = -1;
					let bestSimilarity = Infinity;
					let pos = left;
					const rightSize = sumSize(group.nodes, pos);

					// [ O O O ] O O O [ O O O ]   pos, then right
					// ^^^^^^^^^ leftSize
					// rightSize ^^^^^^^^^^^^^^^
