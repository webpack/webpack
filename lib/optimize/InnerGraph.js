...
/** @typedef {(value: boolean | Set<string> | Map<string, string[]> | undefined, module: Module) => void} UsageCallback */
 ...
 * @property {string=} propertyContext
 */

/** @typedef {false | StateObject} State */
/** @typedef {string | TopLevelSymbol | true} Usage */
/** @typedef {Set<string> | Map<string, string[]> | boolean} UsedByExports */
 ...
 * @property {(parserState: ParserState, propertyName: string) => void} setPropertyContext
 * @property {(parserState: ParserState) => void} clearPropertyContext
...
module.exports.getInnerGraphUtils = (compilation) => {
	 ...
	 */
	function setPropertyContext(state, propertyName) {
		const innerGraphState = getState(state);
		if (innerGraphState) {
			innerGraphState.propertyContext = propertyName;
		}
	}
	 ...
	 * @param {ParserState} state parser state
	 */
	function clearPropertyContext(state) {
		const innerGraphState = getState(state);
		if (innerGraphState) {
			innerGraphState.propertyContext = undefined;
		}
	}
	...
	function onUsage(state, onUsageCallback) {
		...
		if (innerGraphState) {
			const { usageCallbackMap, currentTopLevelSymbol, propertyContext } =
				innerGraphState;

			/** @type {UsageCallback} */
			const effectiveCallback = propertyContext
				? (usedByExports, module) => {
						if (
							...
							onUsageCallback(usedByExports, module);
						} else {
							/** @type {Map<string, string[]>} */
							const map = new Map();
							for (const name of /** @type {Set<string>} */ (usedByExports)) {
								map.set(name, /** @type {string[]} */ ([propertyContext]));
							}
							onUsageCallback(map, module);
						}
					}
				: onUsageCallback;
			...
			if (currentTopLevelSymbol) {
				...
				callbacks.add(effectiveCallback);
			} else {
				effectiveCallback(true, state.module);
			}
		} else {
