...
/** @typedef {false|{innerGraph: InnerGraph, allExportDependentDependencies: Set<Dependency>, currentTopLevelSymbol: TopLevelSymbol|void, usageCallbackMap: WeakMap<TopLevelSymbol | Dependency, Set<UsageCallback>>}} State */
/** @typedef {function(TopLevelSymbol | Dependency, true | Set<string>): void} UsageCallback */
...
/**
 * @param {State} state inner graph state
 * @param {TopLevelSymbol | Dependency} symbol a symbol
 * @param {UsageCallback} callback a function that will be executed when the symbol will be used
 */
function onUsage(state, symbol, callback) {
	if (!state) {
		return;
	}

	let callbacks = state.usageCallbackMap.get(symbol);

	if (!callbacks) {
		callbacks = new Set();
		state.usageCallbackMap.set(symbol, callbacks);
	}
...
/**
 * @param {State} state inner graph state
 * @param {TopLevelSymbol | Dependency} dep dependency
 * @param {Set<string | TopLevelSymbol> | true} usage usage
 */
function flushOnUsage(state, dep, usage) {
	if (!state) {
		return;
	}

	const list = state.usageCallbackMap.get(dep) || [];

	for (const callback of list) {
		callback(dep, usage === undefined ? false : usage);
	}
}
...
exports.inferDependencyUsage = state => {
	...
	for (const dep of allExportDependentDependencies) {
		...
		flushOnUsage(innerGraphState, dep, value);
	}
	return result;
};
 ...
 */
exports.addDependency = (state, dep, onUsageCallback) => {
	const innerGraphState = getState(state);

	if (innerGraphState) {
		innerGraphState.allExportDependentDependencies.add(dep);
		if (onUsage) onUsage(innerGraphState, dep, onUsageCallback);
	}
};

/**
 * @param {Dependency & { usedByExports: boolean | Set<string> }} dependency dependency
 * @param {true | Set<string>} usage usage
 */
exports.defaultUsageCallback = (dependency, usage) => {
	dependency.usedByExports = usage;
};
