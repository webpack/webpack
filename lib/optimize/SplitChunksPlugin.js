		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				(chunks) => {
					...
					// Standalone entries (no runtime + no chunk loading) produce
					// a single self-contained file. Splitting is harmful because
					// split pieces have no loading mechanism and are silently lost.
					for (const chunk of chunks) {
						for (const group of chunk.groupsIterable) {
							if (
								typeof (
									/** @type {Entrypoint} */ (group).getEntrypointChunk
								) === "function"
							) {
								const opts = /** @type {EntryOptions} */ (group.options);
								if (opts.runtime === false && opts.chunkLoading === false) {
