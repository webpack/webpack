...
const { discoverIntersections } = require("./chunkSetIntersections");
...
const normalizeChunksFilter = (chunks) => {
	/** @type {ChunkFilterFn | undefined} */
	let base;
	if (chunks === "initial") {
		base = INITIAL_CHUNK_FILTER;
	} else if (chunks === "async") {
		base = ASYNC_CHUNK_FILTER;
	} else if (chunks === "all") {
		base = ALL_CHUNK_FILTER;
	} else if (chunks instanceof RegExp) {
		base = (chunk) => (chunk.name ? chunks.test(chunk.name) : false);
	} else if (typeof chunks === "function") {
		base = chunks;
	}
	if (!base) return base;
	return (chunk) => !chunk.preventSplitting && base(chunk);
};
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				(chunks) => {
					...
					// Rounds of discovery: what the option says, or what the defaults
					// of the next major imply, as a plugin constructed by hand reads
					// no defaults of its own
					const dedupDepth =
						this.options.dedupDepth === undefined
							? compiler.options.experiments.futureDefaults
								? 1
								: 0
					...
					/** @typedef {Map<bigint, ChunkSet>} ChunkSetsInGraph */
						...
						for (const module of compilation.modules) {
							...
							for (const chunks of groupedChunks) {
								...
								if (chunks.length === 1) {
									...
									// More than one chunk, so the key is the bigint of their ids
									const setKey = /** @type {bigint} */ (chunksKey);
									if (!chunkSetsInGraph.has(setKey)) {
										chunkSetsInGraph.set(setKey, new Set(chunks));
									}
								}
								if (sizeByKey !== undefined) {
									sizeByKey.set(
					...
					/**
					 * The chunk sets a combination can be taken from, in the order they are
					 * combined in, indexed by the chunk fewest of them hold.
					 * @typedef {object} ChunkSetIndex
					 * @property {ChunkSet[]} sets the chunk sets, smaller counts first
					 * @property {Map<Chunk, number[]>} setsByRarestChunk what to try for a chunk
					 * @property {Map<Chunk, number>} singleOrder order of the single chunk sets
					 */

					/**
					 * Indexes chunk sets for the subsets of a set to be found without walking all
					 * of them: a subset holds the chunk fewest sets hold of it, so only the sets
					 * indexed under a chunk of the superset can be one.
					 ...
					 * @returns {ChunkSetIndex} the sets, and where to look for a subset
					 */
					const indexChunkSets = (chunkSets, singleChunkSets) => {
						// Grouped by count, as only a smaller set can be a proper subset, and
						// combined in that order
						/** @type {Map<CountOfChunk, ChunkSet[]>} */
						const setsByCount = new Map();
						/** @type {Map<Chunk, number>} */
						const holders = new Map();
						for (const chunksSet of chunkSets) {
							const group = setsByCount.get(chunksSet.size);
							if (group === undefined) {
								setsByCount.set(chunksSet.size, [chunksSet]);
							} else {
								group.push(chunksSet);
							}
							for (const chunk of chunksSet) {
								holders.set(chunk, (holders.get(chunk) || 0) + 1);
							}
						}
						/** @type {ChunkSet[]} */
						const sets = [];
						for (const group of setsByCount.values()) {
							for (const chunksSet of group) sets.push(chunksSet);
						}
						/** @type {Map<Chunk, number[]>} */
						const setsByRarestChunk = new Map();
						for (let i = 0; i < sets.length; i++) {
							/** @type {Chunk | undefined} */
							let rarest;
							let fewest = 0;
							for (const chunk of sets[i]) {
								const count = /** @type {number} */ (holders.get(chunk));
								if (rarest === undefined || count < fewest) {
									rarest = chunk;
									fewest = count;
								}
							}
							if (rarest === undefined) continue;
							const indices = setsByRarestChunk.get(rarest);
							if (indices === undefined) setsByRarestChunk.set(rarest, [i]);
							else indices.push(i);
						}
						/** @type {Map<Chunk, number>} */
						const singleOrder = new Map();
						for (const chunk of singleChunkSets) {
							singleOrder.set(chunk, singleOrder.size);
						}
						return { sets, setsByRarestChunk, singleOrder };
					};
					const getChunkSetIndex = memoize(() => {
						const { chunkSetsInGraph, singleChunkSets } = getChunkSetsInGraph();
						return indexChunkSets(chunkSetsInGraph.values(), singleChunkSets);
					});
					const getExportsChunkSetIndex = memoize(() => {
					...
					const discoverChunkSetIntersections = (
						...
						// A chunk of its own is keyed by the chunk, so what holds both is wider
						// than what the graph's chunk sets are keyed by
						let memberCount = 0;
					...
					/**
					 * Where a chunk set's own combinations end, for a cache group that
					 * takes no intersection.
					 * @param {Combinations} combinations the combinations
					 * @param {boolean} withIntersections whether the group takes the intersections too
					 * @param {Map<Combinations, number>} originalCounts how many of them are chunk sets themselves
					 * @returns {number} where to stop
					 */
					const originalCombinationCount = (
						combinations,
						withIntersections,
						originalCounts
					) => {
						if (withIntersections || originalCounts.size === 0) {
							return combinations.length;
						}
						const originals = originalCounts.get(combinations);
						return originals === undefined ? combinations.length : originals;
					};
					 ...
					 * @param {ChunkSetIndex} index the chunk sets a combination is taken from
					...
					const createGetCombinations = (
						...
						const { sets, setsByRarestChunk, singleOrder } = index;
						...
						return (key, withIntersections) => {
							...
							// A subset holds the chunk fewest sets hold of it, so only the sets
							// indexed under a chunk of this one can be one
							/** @type {number[]} */
							const subsets = [];
							for (const chunk of chunksSet) {
								const indices = setsByRarestChunk.get(chunk);
								if (indices === undefined) continue;
								for (const candidate of indices) {
									const set = sets[candidate];
									// "equal" is not needed because they would have been merge in the first step
									if (set.size < chunksSet.size && isSubset(chunksSet, set)) {
										subsets.push(candidate);
									}
								}
							}
							// Combined in the order they were indexed in, smaller counts first
							if (subsets.length > 1) subsets.sort((a, b) => a - b);
							for (const candidate of subsets) array.push(sets[candidate]);
							/** @type {Chunk[]} */
							const singles = [];
							for (const chunk of chunksSet) {
								if (singleOrder.has(chunk)) singles.push(chunk);
							}
							if (singles.length > 1) {
								singles.sort(
									(a, b) =>
										/** @type {number} */ (singleOrder.get(a)) -
										/** @type {number} */ (singleOrder.get(b))
								);
							}
							for (const chunk of singles) array.push(chunk);
					...
					const getCombinationsFactory = memoize(() => {
						const { chunkSetsInGraph } = getChunkSetsInGraph();
					...
					const getExportsCombinationsFactory = memoize(() => {
						const { chunkSetsInGraph } = getExportsChunkSetsInGraph();
						return createGetCombinations(
							chunkSetsInGraph,
							getExportsChunkSetIndex(),
							exportsIntersectionSettings === undefined
								? undefined
					...
					/**
					 * Returns combinations of the module's chunks grouped by used exports. A
					 * module whose chunks all use the same exports takes the combinations of that
					 * one group as they are, without a copy to merge into.
					 * @param {Module} module the module
					 * @param {boolean} withIntersections whether to take the intersections of the chunk sets too
					 * @returns {Combinations} chunk combinations
					 */
					const computeCombinationsByUsedExports = (
						...
						if (groupedByUsedExports.length === 1) {
							return getExportsCombinations(
								getKey(groupedByUsedExports[0]),
								withIntersections
							);
						}
						/** @type {Combinations} */
						...
						for (const chunks of groupedByUsedExports) {
							...
							const end = originalCombinationCount(
								combinations,
								withIntersections,
								originalExportsCombinationCounts
							);
					...
					for (const module of compilation.modules) {
						...
						for (const cacheGroupSource of cacheGroups) {
							...
							if (cacheGroup.usedExports) {
								...
								combs = cached;
								// The intersections trail the chunk sets, so a cache group that takes
								// none of them stops where they begin
								end = originalCombinationCount(
									combs,
									withIntersections,
									originalExportsCombinationCounts
								);
