			...
			compilation.hooks.optimizeChunks.tap(
				...
				chunks => {
					...
					const addModuleToChunksInfoMap = (
						...
						/**
						 * @param {string} name name of current cache group
						 * @param {Chunk[]} selectedChunks chunks selected for this module
						 * @param {bigint | Chunk} selectedChunksKey a key of selectedChunks
						 * @returns {void}
						 */
						const addModuleToMappedSelectedChunks = (
							name,
							selectedChunks,
							selectedChunksKey
						) => {
						...
						if (typeof name === "string") {
							addModuleToMappedSelectedChunks(
								name,
								selectedChunks,
								selectedChunksKey
							);
						} else if (name instanceof Map) {
							for (const [cacheGroupName, chunks] of name) {
								addModuleToMappedSelectedChunks(
									cacheGroupName,
									chunks,
									getKey(chunks)
							...
							throw new Error(
								'Value of "name" should be type of "string" or "Map"'
							);
						}
					};
