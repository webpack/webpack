...
/**
 * @callback OptimizationSplitChunksGetCacheGroups
 * @param {Module} module
 * @returns {OptimizationSplitChunksCacheGroup | OptimizationSplitChunksCacheGroup[] | void}
 */
...
/**
 * @param {GetCacheGroups | Record<string, false|string|RegExp|Function|OptimizationSplitChunksCacheGroup>} cacheGroups the cache group options
 * @param {false | string | Function} name the cache group name
 * @returns {GetCacheGroups} a function to get the cache groups
 */
const normalizeCacheGroups = (cacheGroups, name) => {
		...
		const fn = (module, context) => {
			...
			for (const key of Object.keys(cacheGroups)) {
				...
				if (typeof option === "string" || option instanceof RegExp) {
					...
					const getCacheGroups = /** @type {OptimizationSplitChunksGetCacheGroups} */ (option);
					const result = getCacheGroups(module);
					if (result) {
...
const createCacheGroupSource = options => {
	return {
		...
		getName: normalizeName(options.name) || defaultGetName,
...
module.exports = class SplitChunksPlugin {
	...
	constructor(options = {}) {
		...
		this.options = {
			...
			getCacheGroups: normalizeCacheGroups(options.cacheGroups, options.name),
					...
					for (const chunk of Array.from(compilation.chunks)) {
						...
						for (let i = 0; i < results.length; i++) {
							...
							if (i !== results.length - 1) {
								let newPart = compilation.addChunk(name);
								chunk.split(newPart);
