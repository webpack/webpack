 ...
 */
/** @import { ChunkName, ChunkFilenameTemplate } from "../Chunk" */
/** @import ChunkGraph from "../ChunkGraph" */
/** @import ChunkGroup from "../ChunkGroup" */
/** @import Compilation from "../Compilation" */
/** @import Compiler from "../Compiler" */
/** @import Module, { SourceType } from "../Module" */
/** @import ModuleGraph from "../ModuleGraph" */
/** @typedef {import("../util/deterministicGrouping").GroupedItems<Module>} DeterministicGroupingGroupedItemsForModule */
/** @typedef {import("../util/deterministicGrouping").Options<Module>} DeterministicGroupingOptionsForModule */
/** @import { Sizes } from "../util/deterministicGrouping" */
...
const MIN_INTERSECTION_CHUNKS = 1;
const MAX_INTERSECTION_CANDIDATE_FACTOR = 2;
const MAX_INTERSECTION_CANDIDATES = 4096;
const MAX_INTERSECTION_PAIRS = 500000;
const MAX_INTERSECTION_COMPARISONS = 16000000;
const MAX_INTERSECTION_PROPAGATION_CHECKS = 500000;
const MAX_INTERSECTION_PROPAGATION_COMPARISONS = 16000000;
const MAX_INTERSECTION_PROPAGATION_EXPANSIONS = 500000;
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				(chunks) => {
					...
					const CHUNK_SET_MASK = ~(START - ONE);
					let index = START;
					let denseIndex = 0;
					...
					const getChunkSetsInGraph = memoize(() => {
						...
						const chunkSetModuleCounts = new Map();
						/** @type {ChunkSet} */
						...
						for (const module of compilation.modules) {
							...
							if (typeof chunksKey === "bigint") {
								chunkSetModuleCounts.set(
									chunksKey,
									(chunkSetModuleCounts.get(chunksKey) || 0) + 1
								);
						...
						return { chunkSetsInGraph, singleChunkSets, chunkSetModuleCounts };
					});
					 ...
					 * @param {Iterable<ChunkSet>} chunkSets set of sets of chunks
					...
					/**
					 * Discovers intersections and associates them with containing original sets.
					 * @param {ChunkSetsInGraph} chunkSets original chunk sets
					 * @param {Map<bigint, number>} chunkSetModuleCounts module count by original set key
					 * @returns {Map<bigint, ChunkSet[]>} intersections grouped by original set key
					 */
					const getIntersectionsByChunkSet = (
						chunkSets,
						chunkSetModuleCounts
					) => {
						/** @type {Map<bigint, ChunkSet>} */
						const chunkSetsByMembership = new Map();
						for (const [key, chunks] of chunkSets) {
							chunkSetsByMembership.set(key & CHUNK_SET_MASK, chunks);
						}
						const result = findIntersections(chunkSetsByMembership, {
							minimumSize: MIN_INTERSECTION_CHUNKS,
							maximumCandidates: Math.min(
								MAX_INTERSECTION_CANDIDATES,
								chunkSetsByMembership.size * MAX_INTERSECTION_CANDIDATE_FACTOR
							),
							maximumPairs: MAX_INTERSECTION_PAIRS,
							maximumComparisons: MAX_INTERSECTION_COMPARISONS
						});
						const propagation = associateIntersections(
							chunkSets,
							result.entries,
							chunkSetModuleCounts,
							{
								maximumChecks: MAX_INTERSECTION_PROPAGATION_CHECKS,
								maximumComparisons: MAX_INTERSECTION_PROPAGATION_COMPARISONS,
								maximumExpansions: MAX_INTERSECTION_PROPAGATION_EXPANSIONS
							}
						);
						logger.debug(
							"Prepared %d additional split chunk intersections from %d pairs and %d comparisons%s; propagated with %d checks, %d comparisons, and %d downstream visits%s",
							result.sets.length,
							result.pairs,
							result.comparisons,
							...
							propagation.expansions,
							propagation.limited ? " (limited)" : ""
						);
						return propagation.intersectionsByOriginalKey;
					};
					const getIntersectionsByChunkSetKey = memoize(() => {
						const { chunkSetsInGraph, chunkSetModuleCounts } =
							getChunkSetsInGraph();
						return getIntersectionsByChunkSet(
							chunkSetsInGraph,
							chunkSetModuleCounts
						);
					});
					 ...
					 * @param {Map<bigint, ChunkSet[]> | undefined} intersectionsByChunkSetKey intersections grouped by original set key
					...
					const createGetCombinations = (
						...
						return (key) => {
							...
							const intersections =
								intersectionsByChunkSetKey === undefined
									? undefined
									: intersectionsByChunkSetKey.get(key);
							if (intersections !== undefined) array.push(...intersections);
					...
					const getCombinationsWithIntersectionsFactory = memoize(() => {
						const { chunkSetsInGraph, singleChunkSets } = getChunkSetsInGraph();
						return createGetCombinations(
							chunkSetsInGraph,
							singleChunkSets,
					 ...
					 * @param {boolean} includeIntersections whether to include additional intersections
					 * @returns {Combinations} combinations by key
					 */
					const getCombinations = (key, includeIntersections) =>
						(includeIntersections
							? getCombinationsWithIntersectionsFactory()
							: getCombinationsFactory())(key);
					...
					const getExportsCombinationsWithIntersectionsFactory = memoize(() => {
						const { chunkSetsInGraph, singleChunkSets } =
							getExportsChunkSetsInGraph();
						return createGetCombinations(
							chunkSetsInGraph,
					 ...
					 */
					const getExportsCombinations = (key, includeIntersections) =>
						(includeIntersections
							? getExportsCombinationsWithIntersectionsFactory()
					 ...
					 */
					const computeCombinations = (module, includeIntersections) =>
						getCombinations(
					...
					const computeCombinationsByUsedExports = (
						...
						for (const chunks of groupedByUsedExports) {
							for (const comb of getExportsCombinations(
								getKey(chunks),
								includeIntersections
							)) {
								set.add(comb);
					...
					for (const module of compilation.modules) {
						...
						// Each mode is computed at most once per module.
						/** @type {(Combinations | undefined)[]} */
						const combinationsCache = [];
						/** @type {(Set<ChunkSet | Chunk> | undefined)[]} */
						const combinationsByUsedExportsCache = [];

						let cacheGroupIndex = 0;
						for (const cacheGroupSource of cacheGroups) {
							const cacheGroup = this._getCacheGroup(cacheGroupSource);
							const includeIntersections =
								cacheGroup.getName === defaultGetName &&
								(cacheGroup._validateSize || cacheGroup._validateSizeReduction);
							const combinationsCacheIndex = includeIntersections ? 1 : 0;

							let combs;
							if (cacheGroup.usedExports) {
								combs = combinationsByUsedExportsCache[combinationsCacheIndex];
								if (combs === undefined) {
									combs = computeCombinationsByUsedExports(
										module,
										includeIntersections
									);
									combinationsByUsedExportsCache[combinationsCacheIndex] =
										combs;
								}
							} else {
								combs = combinationsCache[combinationsCacheIndex];
								if (combs === undefined) {
									combs = computeCombinations(module, includeIntersections);
									combinationsCache[combinationsCacheIndex] = combs;
								}
							}
