		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				(chunks) => {
					...
					/** @type {Map<Module, CacheGroupSource[]>} */
					const moduleCacheGroups = new Map();
					/** @type {Map<Module, Set<Chunk>>} */
					const remainingModuleChunks = new Map();
					/** @type {Set<number>} */
					const priorities = new Set();
					if (dedupDepth > 0) {
						for (const module of compilation.modules) {
							const groups = this.options.getCacheGroups(module, context);
							if (Array.isArray(groups)) {
								moduleCacheGroups.set(module, groups);
								for (const group of groups) {
									priorities.add(this._getCacheGroup(group).priority);
								}
							}
							remainingModuleChunks.set(
								module,
					...
					/**
					 * Returns original module edges left by earlier priorities, excluding split destinations.
					 * @param {Module} module module
					 * @returns {Iterable<Chunk>} remaining original chunks
					 */
					const getModuleChunks = (module) =>
						remainingModuleChunks.get(module) ||
						chunkGraph.getModuleChunksIterable(module);
					const priorityBatches =
						dedupDepth > 0
							? [...priorities].sort((a, b) => b - a)
							: [undefined];
					for (const priority of priorityBatches) {
						logger.time("prepare");
						...
						const getChunkSetsInGraph = memoize(() => {
							...
							for (const module of compilation.modules) {
								const chunks = getModuleChunks(module);
						...
						const groupChunksByExports = (module) => {
							...
							for (const chunk of getModuleChunks(module)) {
						...
						// Only smaller sets can be proper subsets.
						/**
						...
						/**
						 * Discovers intersections and all their original supports for this priority.
						 * @param {ChunkSetsInGraph} chunkSets original chunk sets
						 * @returns {Map<bigint, ChunkSet[]>} intersections by original key
						 */
						const getIntersectionsByChunkSet = (chunkSets) => {
							/** @type {Map<bigint, ChunkSet>} */
							const memberships = new Map();
							for (const [key, chunks] of chunkSets) {
								memberships.set(key & CHUNK_SET_MASK, chunks);
							}
							const intersections = findIntersections(memberships, {
								minimumSize: 1,
								dedupDepth
							});
							return associateIntersections(chunkSets, intersections);
						};
						const getIntersectionsByChunkSetKey = memoize(() =>
							getIntersectionsByChunkSet(getChunkSetsInGraph().chunkSetsInGraph)
						);
						const getExportsIntersectionsByChunkSetKey = memoize(() =>
							getIntersectionsByChunkSet(
								getExportsChunkSetsInGraph().chunkSetsInGraph
							)
						);
						...
						const addModuleToChunksInfoMap = (
							...
							if (info.modules.size !== oldSize) {
								for (const type of module.getSourceTypes()) {
									info.sizes[type] =
										(info.sizes[type] || 0) + module.size(type);
								}
							}
						...
						for (const module of compilation.modules) {
							...
							const cacheGroups =
								dedupDepth > 0
									? moduleCacheGroups.get(module)
									: this.options.getCacheGroups(module, context);
							...
							for (const cacheGroupSource of cacheGroups) {
								...
								const currentCacheGroupIndex = cacheGroupIndex++;
								if (
									priority !== undefined &&
									cacheGroup.priority !== priority
								) {
									continue;
								}
								const includeIntersections =
									...
									(cacheGroup._validateSize ||
										cacheGroup._validateSizeReduction);
								...
								for (const chunkCombination of combs) {
									// Break if minimum number of chunks is not reached
									const count =
										chunkCombination instanceof Chunk
											? 1
							...
							if (!isReusedWithAllModules) {
								...
								for (const module of item.modules) {
									...
									for (const chunk of usedChunks) {
										...
										const remaining = remainingModuleChunks.get(module);
										if (remaining !== undefined) remaining.delete(chunk);
									}
								}
							} else {
							...
							if (
								...
								maxSizeQueueMap.set(newChunk, {
									...
									maxInitialSize: oldMaxSizeSettings
										...
										: item.cacheGroup.maxInitialSize,
									automaticNameDelimiter:
										item.cacheGroup.automaticNameDelimiter,
