...
const { findIntersections, isSubset } = require("../util/SetHelpers");
		...
		compiler.hooks.thisCompilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				(chunks) => {
					...
					/**
					 * Discovers intersections and groups them with the original chunk sets.
					 * @param {ChunkSetsInGraph} chunkSets original chunk sets
					 * @returns {ChunkSetsByCount} original and additional sets grouped by size
					 */
					const groupChunkSetsWithIntersections = (chunkSets) => {
						/** @type {Map<bigint, ChunkSet>} */
						...
						logger.debug(
							"Prepared %d additional split chunk intersections from %d pairs and %d comparisons%s",
							result.sets.length,
							result.pairs,
							result.comparisons,
							result.limited ? " (limited)" : ""
						);
						return groupChunkSetsByCount([
							...chunkSets.values(),
							...result.sets
						]);
					};
					const getChunkSetsWithIntersectionsByCount = memoize(() =>
						groupChunkSetsWithIntersections(
							getChunkSetsInGraph().chunkSetsInGraph
						)
					);
					const getExportsChunkSetsWithIntersectionsByCount = memoize(() =>
						groupChunkSetsWithIntersections(
					...
					const getCombinationsWithIntersectionsFactory = memoize(() => {
						...
						return createGetCombinations(
							chunkSetsInGraph,
							singleChunkSets,
							getChunkSetsWithIntersectionsByCount()
						);
					});
					...
					const getExportsCombinationsWithIntersectionsFactory = memoize(() => {
						...
						return createGetCombinations(
							chunkSetsInGraph,
							singleChunkSets,
							getExportsChunkSetsWithIntersectionsByCount()
						);
					});
					/**
						...
						for (const cacheGroupSource of cacheGroups) {
							...
							const includeIntersections =
								...
								(cacheGroup._validateSize ||
									hasNonZeroSizes(cacheGroup.minSizeReduction));
