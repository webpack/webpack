...
class LazyBarrelInfo {
	constructor() {
		...
		/** @type {Set<string> | undefined} ordinary import request keys */
		this._ordinaryRequests = undefined;
	}
	...
	/**
	 * Records an ordinary import that is safe to defer while inspecting exports.
	 * @param {string} requestKey request key
	 */
	addOrdinary(requestKey) {
		if (this._ordinaryRequests === undefined) {
			this._ordinaryRequests = new Set();
		}
		this._ordinaryRequests.add(requestKey);
	}
	...
	request(forwardedIds) {
		...
		let requestOrdinary = false;
		if (this._terminalIds !== undefined) {
			for (const id of forwardedIds) {
				if (this._terminalIds.has(id)) {
					requestOrdinary = true;
					break;
				}
			}
		}
		...
		if (requestOrdinary && this._ordinaryRequests !== undefined) {
			for (const requestKey of this._ordinaryRequests) take(requestKey);
		}
...
class LazyBarrelController {
	...
	classify(module) {
		...
		const forwardedIds = state && state.forwardedIds;
		let inspectOnly =
		...
		for (const dep of dependencies) {
			if (
				...
				if (typeof name === "string") terminalIds.add(name);
			}
		}
		if (inspectOnly && forwardedIds !== undefined && forwardedIds !== true) {
			for (const id of forwardedIds) {
		...
		// Ordinary ESM imports are only deferred when this module is being built to
		// inspect a named star export and that name is not locally provided.
		if (inspectOnly) {
			for (const dep of dependencies) {
				if (
					"getLazyUntil" in dep &&
					(dep.getLazyUntil() === null ||
						dep.getLazyUntil() === Dependency.LAZY_UNTIL_REQUEST) &&
					dep.category === Dependency.ESM_CATEGORY &&
					"setLazy" in dep &&
					dep.getResourceIdentifier() !== null
				) {
					const factory = dependencyFactories.get(
						/** @type {DependencyConstructor} */ (dep.constructor)
					);
					if (factory !== undefined) {
						const resourceIdent = dep.getResourceIdentifier();
						const requestKey = /** @type {string} */ (resourceIdent);
						if (info === undefined) info = new LazyBarrelInfo();
						dep.setLazy(true);
						info.addLazy(requestKey, dep, factory, dep.getContext());
						info.addOrdinary(requestKey);
						hasOrdinary = true;
					}
				}
			}
	...
	request(module, dependencies) {
		...
		const getRequestedIds = () => {
			const directIds = getForwardedIds(dependencies);
			if (directIds !== true && directIds.size !== 0) return directIds;
			for (const dependency of dependencies) {
				if (
					"getLazyUntil" in dependency &&
					dependency.getLazyUntil() === Dependency.LAZY_UNTIL_FALLBACK
				) {
					const parent =
						this._compilation.moduleGraph.getParentModule(dependency);
					const parentState =
						parent === undefined ? undefined : modules.get(parent);
					if (parentState !== undefined) {
						return parentState.forwardedIds;
					}
				}
			}
			if (directIds === true || directIds.size !== 0) return directIds;
		...
		if (state === undefined) {
			const forwardedIds = getRequestedIds();
