...
const evaluateWithFalseValue = (sideEffects, file) =>
	(sideEffects === "false") ?
	// Assume we mean (Boolean) false, except for files/modules named false.
	(/false.*$/).test(file) :
	sideEffects !== false;

// Handle variations in `sideEffects` value.
const hasSideEffects = (resolveData) => {
	const sideEffectsValue = resolveData.descriptionFileData.sideEffects;
	const relativePath = resolveData.relativePath;
	return Array.isArray(sideEffectsValue) ?
		// It's an array of globs.
		!!sideEffectsValue.find((glob) => globToRegExp(glob).test(relativePath)) :
		// It's false or "false".
		evaluateWithFalseValue(sideEffectsValue, relativePath);
};
		...
		compiler.hooks.normalModuleFactory.tap("SideEffectsFlagPlugin", (nmf) => {
			nmf.hooks.module.tap("SideEffectsFlagPlugin", (module, data) => {
				...
				if(resolveData && resolveData.descriptionFileData && resolveData.relativePath) {
					module.factoryMeta.sideEffectFree = !hasSideEffects(resolveData);
				}
