		...
		compiler.hooks.normalModuleFactory.tap("SideEffectsFlagPlugin", (nmf) => {
			nmf.hooks.module.tap("SideEffectsFlagPlugin", (module, data) => {
				...
				if(resolveData && resolveData.descriptionFileData && resolveData.relativePath) {
					...
					module.factoryMeta.sideEffectFree = !SideEffectsFlagPlugin.moduleHasSideEffects(relativePath, sideEffectsValue);
				}
			...
			case "string":
				{
					if(flagValue === "false" || flagValue === "true") {
						return flagValue === "false" ? (/false.*$/).test(modulePath) : true;
					}

					if(flagValue === modulePath) {
						return true;
					}
					...
					return matchesRegex || globToRegExp(flagValue).test(modulePath);
				}
			case "object":
