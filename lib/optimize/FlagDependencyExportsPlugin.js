 ...
 */
/** @import ExportsInfo, { RestoreProvidedData } from "../graph/ExportsInfo" */
				...
				(modules, callback) => {
					...
					asyncLib.each(
						...
						(module, callback) => {
							...
							// detection cannot invalidate the mem cache while the un-lazied
							// connections are unsafe-cached, being skipped in the comparison
						...
						(err) => {
							...
							// WHY: enqueueing at the branches above ordered the work by how
							// fast each cache lookup answered, letting the persistent cache
							// decide it. A module analyzed before the one it re-exports from
							// over-estimates its exports, and this loop can add exports info
							// but never retract it, so that over-estimate would survive. Sorted
							// roots and post-order visit each module after what it points at.
							/** @type {Set<Module>} */
