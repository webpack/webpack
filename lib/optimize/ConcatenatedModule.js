 ...
 * @property {boolean} deferredNamespaceObjectUsed
 * @property {string | undefined} deferredNamespaceObjectName
 * @property {boolean} interopNamespaceObjectUsed
 * @property {string | undefined} interopNamespaceObjectName
 * @property {boolean} interopNamespaceObject2Used
 * @property {string | undefined} interopNamespaceObject2Name
...
const getFinalBinding = (
	...
	switch (info.type) {
		...
		case "external": {
			...
			return {
				info,
				rawName: info.name + (deferred ? ".a" : "") + comment,
				ids: used,
				exportName
			};
		}
...
class ConcatenatedModule extends Module {
	...
	_createConcatenationList(rootModule, modulesSet, runtime, moduleGraph) {
		...
		const getConcatenatedImports = module => {
			...
			// do not combine. defer is not the same as source or range which has a comparable number, defer is only moving them.
			...
			switch (info.type) {
				...
				case "external": {
					...
					if (info.deferred) {
						...
						result.add(`var ${info.name} = ${loader};`);
					} else {
