 ...
 * @property {boolean} wrapped the module renders inside the lazy CJS wrapper instead of being scope-hoisted
 * @property {Module} module
 * @property {RuntimeSpec | boolean} runtimeCondition
 * @property {NonDeferAccess} nonDeferAccess
 */
...
const entryCanBeWrapped = (entry) =>
	(entry.module instanceof getNormalModule() &&
		!entry.module.type.startsWith("css/")) ||
	(entry.module instanceof getExternalModule() &&
...
class ConcatenatedModule extends Module {
		...
		const getConcatenatedImports = (module) => {
			...
			const references = connections
				...
				.map((connection) => {
					...
					return {
						connection,
						// Coerce a missing sourceOrder (require() edges) to NaN:
						// compareNumbers sorts NaN consistently (last), while undefined
						// would make the comparator inconsistent and the order depend on
						// async connection insertion order.
						sourceOrder:
							typeof dep.sourceOrder === "number"
								? dep.sourceOrder
								: Number.NaN,
	 ...
	 *  |     plain    -> ReplaceSource + ast + eslint-scope scopes
		...
		for (const info of moduleToInfoMap.values()) {
			...
			if (!info.wrapped) {
				...
				for (const reference of ConcatenationScope.findModuleReferences(
					...
					const match = ConcatenationScope.matchModuleReference(reference.name);
					if (!match) continue;
					const referencedInfo = modulesWithInfo[match.index];
					if (referencedInfo.type === "reference") {
						throw new Error("Module reference can't point to a reference");
