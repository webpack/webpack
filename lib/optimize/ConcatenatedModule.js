...
const getExternalImport = (
	...
	return asCall && callContext === false ? `Object(${reference})` : reference;
};
	...
	source({ dependencyTemplates, runtimeTemplate, moduleGraph, chunkGraph }) {
			...
			if (info.type === "concatenated") {
				...
				if (info.globalScope) {
					for (const reference of info.globalScope.through) {
						...
						if (
							/^__WEBPACK_MODULE_REFERENCE__\d+_([\da-f]+|ns)(_call)?(_directImport)?(_strict)?__$/.test(
								name
							)
						) {
		...
		for (const info of modulesWithInfo) {
			if (info.type === "concatenated") {
				for (const reference of info.globalScope.through) {
					const name = reference.identifier.name;
					const match = /^__WEBPACK_MODULE_REFERENCE__(\d+)_([\da-f]+|ns)(_call)?(_directImport)?(_strict)?__$/.exec(
						name
					);
					if (match) {
						...
						const directImport = !!match[4];
						const strictHarmonyModule = !!match[5];
						const finalName = getFinalName(
							moduleGraph,
							referencedModule,
							exportName,
...
class HarmonyImportSpecifierDependencyConcatenatedTemplate extends DependencyTemplate {
	...
	apply(dependency, source, templateContext) {
		...
		const directImportFlag = dep.directImport ? "_directImport" : "";
		...
		if (ids.length === 0) {
			...
			content = `__WEBPACK_MODULE_REFERENCE__${info.index}_${exportData}${callFlag}${directImportFlag}${strictFlag}__`;
		}
		if (dep.shorthand) {
