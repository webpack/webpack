 ...
 * @property {SyncBailHook<[ConcatenatedModule], boolean | void>} lazyExportGeneration
 */
...
class ConcatenatedModule extends Module {
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				lazyExportGeneration: new SyncBailHook(["module"])
			};
	...
	build(options, compilation, resolver, fs, callback) {
		...
		this.generationMeta = rootModule.generationMeta;
	...
	codeGeneration({
		...
		if (exportsMap.size > 0) {
			...
			const { lazyExportGeneration } = ConcatenatedModule.getCompilationHooks(
				/** @type {Compilation} */
				(this.compilation)
			);
			...
			if (lazyExportGeneration.call(this)) {
				if (/** @type {GenerationMeta} */ this.generationMeta) {
					this.generationMeta.exportsBindingStmt = `${RuntimeGlobals.definePropertyGetters}(${
						this.exportsArgument
					}, {${definitions.join(",")}\n});\n`;
					this.generationMeta.exportsFinalName = exportsFinalName;
				}
			} else {
				result.add(
