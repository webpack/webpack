...
class ConcatenatedModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		// Track whether any inner module is async so we can mark this
		// concatenated module async (`buildMeta.async = true` plus
		// `moduleGraph.setAsync`) and emit the outer `__webpack_require__.a`
		// wrapper during code generation.
		...
		for (const m of this._modules) {
			...
			if (/** @type {BuildMeta} */ (m.buildMeta).async) {
				containsAsync = true;
			}
		...
		if (containsAsync && !(/** @type {BuildMeta} */ (this.buildMeta).async)) {
			// Only spread (and so allocate) when we actually need to differ
			// from the root module's buildMeta — keeps cache equivalence for
			// the common (non-async) case.
			this.buildMeta = { ...this.buildMeta, async: true };
		}
		callback();
	}
	...
	codeGeneration({
		...
		if (exportsMap.size > 0) {
			...
			// When this module is async, the body is wrapped in
			// `__webpack_require__.a(module, async (...) => { ... })` below.
			// Inner module variables (`var foo = ...`) live in the async
			// function's scope, so the export getters that reference them
			// must also be inside that scope. The library on-demand-exports
			// stash would render the `.d(...)` outside the wrapper, breaking
			// scoping — fall back to the in-body emission in that case.
			const isAsyncForExports =
				/** @type {BuildMeta} */ (this.buildMeta).async === true;
			if (
				isAsyncForExports ||
...
${defineGetters}`
		...
		for (const rawInfo of modulesWithInfo) {
			...
			switch (info.type) {
				case "concatenated": {
					...
					// Concatenated externals whose source resolves to a Promise
					// (the `import` / `script` / `promise` external types) need
					// to be awaited through the outer
					// `__webpack_handle_async_dependencies__` so that the
					// namespace variable holds the unwrapped value before any
					// later inner module body references it.
					// ExternalModule emits `var` (not `const`) for async
					// modules in concat scope so the reassignment below is
					// legal. Async wasm and TLA ESM modules do their own
					// `await` inside their rendered source, so they don't
					// need an external await wrapper here.
					if (
						/** @type {BuildMeta} */ (this.buildMeta).async &&
						info.module instanceof ExternalModule &&
						moduleGraph.isAsync(info.module) &&
						info.namespaceObjectName
					) {
						result.add(
							`\nvar __webpack_async_dependencies__ = __webpack_handle_async_dependencies__([${info.namespaceObjectName}]);\n[${info.namespaceObjectName}] = __webpack_async_dependencies__.then ? (await __webpack_async_dependencies__)() : __webpack_async_dependencies__;`
						);
					}
				...
				case "external": {
					...
					if (!info.deferred) {
						...
						// When this concatenated module is async (see
						// `containsAsync` in `build`), an async external must be
						// awaited through the outer
						// `__webpack_handle_async_dependencies__` so consumers see
						// the unwrapped value, not a Promise.
						if (
							/** @type {BuildMeta} */ (this.buildMeta).async &&
							moduleGraph.isAsync(info.module)
						) {
							result.add(
								`\nvar __webpack_async_dependencies__ = __webpack_handle_async_dependencies__([${info.name}]);\n[${info.name}] = __webpack_async_dependencies__.then ? (await __webpack_async_dependencies__)() : __webpack_async_dependencies__;`
							);
						}
						name = info.name;
					}
		...
		// When this concatenated module contains any async inner module
		// (e.g. an async wasm module or a top-level-await ESM), the rendered
		// body has top-level `await`s and must be wrapped in
		// `__webpack_require__.a(module, async (handle, result) => { ... })`.
		// `__webpack_handle_async_dependencies__` is then in scope for inner
		// generators and for the external/concatenated await emission above.
		const isAsync = /** @type {BuildMeta} */ (this.buildMeta).async === true;
		const finalSource = isAsync
			? (() => {
					runtimeRequirements.add(RuntimeGlobals.asyncModule);
					runtimeRequirements.add(RuntimeGlobals.module);
					const wrapped = new ConcatSource();
					wrapped.add(
						`${RuntimeGlobals.asyncModule}(${this.moduleArgument}, async (__webpack_handle_async_dependencies__, __webpack_async_result__) => { try {\n`
					);
					wrapped.add(result);
					wrapped.add(
		...
		const resultEntry = {
			sources: new Map([[JAVASCRIPT_TYPE, new CachedSource(finalSource)]]),
			data,
			runtimeRequirements
		};
