 ...
 * @property {SyncBailHook<[Module], boolean | void>} skipMakeNamespaceObject
 */
...
class ConcatenatedModule extends Module {
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				...
				/** @type {SyncBailHook<[Module], boolean | void>} */
				skipMakeNamespaceObject: new SyncBailHook(["module"])
			};
	...
	codeGeneration({
		...
		const {
			concatenatedModuleInfo,
			skipMakeNamespaceObject: skipMakeNamespaceObjectHook
		...
		for (const info of neededNamespaceObjects) {
			...
			const shouldSkipMakeNamespaceObject = skipMakeNamespaceObjectHook.call(
				info.module
			);
			const makeNs = shouldSkipMakeNamespaceObject
				? ""
...
var ${name} = {};
${makeNs}${defineGetters}`
			);
			if (!shouldSkipMakeNamespaceObject) {
