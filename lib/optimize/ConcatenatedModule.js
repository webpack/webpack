...
const getInlineExports = memoize(() => require("./InlineExports"));
		...
		case "concatenated": {
			...
			if (reexport === false) {
				...
				if (target && target.export) {
					const inlined = moduleGraph
						.getExportsInfo(target.module)
						.getUsedName([...target.export, ...exportName.slice(1)], runtime);
					const { InlinedUsedName } = getInlineExports();
					if (inlined instanceof InlinedUsedName) {
						return {
							info,
							rawName: inlined.render(
								Template.toNormalComment(
									`inlined export ${propertyAccess(exportName)}`
								)
							),
...
class ConcatenatedModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		this.buildInfo = {
			...
			assets: undefined,
			inlineExports: /** @type {BuildInfo} */ (rootModule.buildInfo)
				.inlineExports
		};
	...
	codeGeneration({
		...
		for (const info of neededNamespaceObjects) {
			...
			for (const exportInfo of exportsInfo.orderedExports) {
				...
				// Skip inlined exports — namespace import already disabled inlining
				if (usedName instanceof InlinedUsedName) continue;
				if (usedName) {
					const finalName = getFinalName(
						moduleGraph,
						info,
