...
/**
 * Whether a hoisted member can move into the lazy wrapper instead. A nested
 * concatenation takes part only as an "external" entry, so wrapping it defers
 * its `require()` call and nothing else; stopping propagation there would
 * evaluate it ahead of the lazily required member it was reached through.
...
const entryCanBeWrapped = (entry) =>
	...
	entry.module instanceof ConcatenatedModule ||
