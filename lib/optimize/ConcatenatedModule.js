...
const CommonJsExportRequireDependency = require("../dependencies/commonjs/CommonJsExportRequireDependency");
const CommonJsFullRequireDependency = require("../dependencies/commonjs/CommonJsFullRequireDependency");
const CommonJsRequireDependency = require("../dependencies/commonjs/CommonJsRequireDependency");
const { ImportPhaseUtils } = require("../dependencies/core/ImportPhase");
...
const { JAVASCRIPT_MODULE_TYPE_ESM } = require("../module/ModuleTypeConstants");
...
/** @import { UpdateHashContext } from "../graph/Dependency" */
/** @import ESMImportDependency from "../dependencies/esm/ESMImportDependency" */
 ...
 * 	ConnectionState
 * } from "../graph/ModuleGraphConnection"
 */
/** @import RequestShortener from "../util/RequestShortener" */
/** @import { ResolverWithOptions } from "../resolve/ResolverFactory" */
 ...
 * @property {Set<number> | undefined} shorthandIdentifierStarts where each shorthand property's identifier starts
 ...
 * @property {boolean} specNamespaceObjectUsed an import reads the spec namespace object the module hands out
 * @property {boolean} interopNamespaceObjectUsed "default-with-named" namespace
 * @property {string | undefined} interopNamespaceObjectName "default-with-named" namespace
 * @property {boolean} interopNamespaceObject2Used "default-only" namespace
 * @property {string | undefined} interopNamespaceObject2Name "default-only" namespace
...
const entryCanBeWrapped = (entry) => {
	// TODO in the next major release: call it directly. A module built against an older webpack
	// extends a base class without it, so fall back to the check this replaced.
	if (typeof entry.module.canBeWrappedInConcatenation !== "function") {
		return !entry.module.type.startsWith("css/");
	}
	return entry.module.canBeWrappedInConcatenation();
};
	...
	if (moduleExportsAccess) {
		...
		switch (exportsType) {
			...
			case "default-with-named":
				switch (exportName[0]) {
					case "default":
						...
						if (deferred) {
							// WHY: `ns.default` for a deferred default-with-named external
							// module must read through the optimized `.a` getter, which
							// evaluates the
							// module lazily and returns its exports, rather than through the
							// proxy namespace — otherwise `typeof ns.default` and
							// `ns.default instanceof X` observe the proxy, not the actual
							// default.
							return {
								info,
			...
			case "default-only": {
				...
				if (deferred) {
					// As with default-with-named above, a deferred default-only
					// external reads through the `.a` getter, so `typeof` and
					// `instanceof` see the value rather than the proxy.
					return {
						info,
						rawName: `${info.deferredName}.a`,
						ids: exportName,
	...
	if (exportName.length === 0) {
		switch (info.type) {
			...
			case "external": {
				...
				// `require()` reads the exports object rather than a namespace, so
				// only an import gets the spec object and interop keeps working.
				const spec =
					!moduleExportsAccess &&
					exportsType === "namespace" &&
					Boolean(
						/** @type {BuildInfo} */
						(info.module.buildInfo).specNamespaceObject
					);
				const rawName = getExternalModuleExpr(info);

				if (spec) info.specNamespaceObjectUsed = true;

				return {
					info,
					rawName: spec
						? `${RuntimeGlobals.specNamespaceObject}(${rawName})`
						: rawName,
					ids: exportName,
					exportName,
					accessorCall: spec || info.wrapped
				};
			}
		}
	}
		...
		case "concatenated": {
			...
			if (directExport) {
				...
				if (usedName instanceof InlinedUsedName) {
					return {
						info,
						// Render the inlined literal only, not its property suffix: that
						// comes back in `ids` and getFinalName appends it once, so emitting
						// it here too would duplicate the access.
...
class ConcatenatedModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		for (const m of this._modules) {
			// populate cacheable
			const { cacheable, diagnostics } = /** @type {BuildInfo} */ (m.buildInfo);
			if (!cacheable) {
				...
				if (notCacheableReasons) {
					const own = getBuildDiagnostics(this.buildInfo);
					const reasons =
						own.notCacheableReasons || (own.notCacheableReasons = []);
			...
			// populate codeGenerationDependencies: the inner templates are applied
			// during this module's own codeGeneration, so what they reference must be
			// generated by then. References back into the concat set are skipped.
	...
	_createConcatenationList(rootModule, modulesSet, runtime, moduleGraph) {
		...
		const getConcatenatedImports = (module) => {
			...
			const references = connections
				...
				.map((connection) => {
					const dep =
						/** @type {ESMImportDependency} */
						(connection.dependency);
					return {
						connection,
						// A require() edge carries no sourceOrder, and Infinity sorts it
						// last. Not NaN: `compareNumbers` returns 0 against every number,
						// which makes the comparator non-transitive.
						sourceOrder:
	...
	// WHY: how the inner modules become one flat source. Stages [1]-[5] only mutate
	// the per-module info records (names, ReplaceSource patches); stages [6]-[9]
	// append to the single output ConcatSource, in exactly the order shown.
	//
	...
	//  |     plain    -> ReplaceSource + scope analysis
	...
	//  |     binding; through the scopes for analysed bodies, textually for wrapped
	...
	codeGeneration({
		...
		// #region [1] ANALYSE: run each inner module's own codeGeneration and
		// scope-analyse the body; a non-wrapped one keeps its scopes for [3]
		...
		const getEscapeNamespaceObjectName = (info) => {
			...
			const exportsInfo = moduleGraph.getExportsInfo(info.module);
			// Its names are not mangled, and the wrapped binding is already the
			// namespace, so a decoupled object would only shadow it.
			if (
				/** @type {BuildInfo} */
				(info.module.buildInfo).specNamespaceObject
			) {
		...
		for (const info of modulesWithInfo) {
			if (info.type === "concatenated") {
				if (info.wrapped) {
					...
					// only its free globals reach freeNames; the reference tokens among
					// them are resolved textually below and patched the same way in [4]
					...
					// A wrapped body's references are found textually, as [4] patches
					// them. They still have to be resolved here: binding one is what
		...
		for (const info of moduleToInfoMap.values()) {
			...
			switch (info.type) {
				case "concatenated": {
					if (info.wrapped) {
						// A wrapped module declares one shared-scope name, the lazy accessor
						// holding its body. Stage 2 claimed that body's identifiers, so
						// findNewName avoids the ones that would shadow it from inside.
						...
						for (const variable of variables) {
							...
							if (newName) {
								...
								const shorthandStarts =
									/** @type {NonNullable<ConcatenatedModuleInfo["shorthandIdentifierStarts"]>} */
									(info.shorthandIdentifierStarts);
								for (const identifier of allIdentifiers) {
									const r = /** @type {Range} */ (identifier.range);
									if (shorthandStarts.has(r[0])) {
		...
		for (const info of moduleToInfoMap.values()) {
			...
			if (!info.wrapped) {
				...
				// A wrapped body's reference tokens are found textually rather than
				// in its AST. [2] scanned the same source, so this reuses its result.
		...
		// unused and inlined. Used ones stay thunks, since resolving one can still
		// register a namespace object that stage 7 must see.
		...
		if (exportsMap.size > 0) {
			...
			if (meta !== undefined && meta.onDemandExports === true) {
				onDemandExportsSource = `${harmonyFlagSource}${exportsSource}`;
			} else {
		...
		// and emitted by stage 8; an external's goes next to its import variable or
		// accessor instead, defined later.

		// Decoupled namespace objects for escaping mangled namespaces carry the same
		// getters keyed by the original export names, so untracked access by name
		// keeps working. Built first, so any nested namespace they pull in exists.
...
${defineGetters}`
		...
		for (const info of wrappedInfos) {
			if (info.type === "concatenated") {
				...
				result.add(
					...
					}(${runtimeTemplate.moduleFactoryFnHeader({
						module: info.module,
						// the trailing `module.exports` assignment below reads it
						needModule:
							Boolean(info.namespaceExportSymbol) ||
							bodyRuntimeRequirements.has(RuntimeGlobals.module),
						needExports: bodyRuntimeRequirements.has(RuntimeGlobals.exports),
						needRequire: false,
						needThisAsExports: bodyRuntimeRequirements.has(
							RuntimeGlobals.thisAsExports
						)
					})}\n`
				);
				result.add(/** @type {ReplaceSource} */ (info.source));
				// A generated body binds its value to a local rather than writing
				// exports, so hand that local to the accessor. The name is verbatim: a
				// wrapped body is not scope-analysed and keeps the reserved symbol.
				result.add(
					info.namespaceExportSymbol
				...
				const required = `${RuntimeGlobals.require}(${moduleId})`;

				// The wrapping happens per reference, so only the helper has to be
				// declared here.
				if (info.specNamespaceObjectUsed) {
			...
			if (info.type === "concatenated") {
				...
				result.add(
					`\n// DEFERRED EXTERNAL MODULE: ${info.module.readableIdentifier(
						requestShortener
					)}\nvar ${info.deferredName} = ${runtimeTemplate.sideEffectsAnnotation()}${loader};`
				);
				if (info.deferredNamespaceObjectUsed) {
					runtimeRequirements.add(RuntimeGlobals.makeDeferredNamespaceObject);
					// Every call site for a module must agree on the shape, since the
					// runtime caches one namespace object per module and mode.
					const deferredExportsType = getExportsType(
						moduleGraph,
						info,
						strictHarmonyModule
					);
		...
		for (const rawInfo of modulesWithInfo) {
			...
			switch (info.type) {
				...
				case "external": {
					...
					if (!info.deferred && !info.wrapped) {
						...
						const required = `${runtimeTemplate.moduleSideEffectsAnnotation(
							info.module,
							moduleGraph
						)}__webpack_require__(${moduleId})`;

						// The wrapping happens per reference, so only the helper has
						// to be declared here.
						if (info.specNamespaceObjectUsed) {
							runtimeRequirements.add(RuntimeGlobals.specNamespaceObject);
						}

						result.add(`var ${info.name} = ${required};`);
			...
			// at the top level and break require() cycles, so it is deferred behind a
			// hoisted function, memoized on the function object itself.
			/**
		...
		if (onDemandExportsSource !== undefined) {
			data.set("exportsSource", onDemandExportsSource);
			data.set("exportsFinalName", exportsFinalName);
		}
	...
	_analyseModule(
		...
		if (info.type !== "concatenated") return;
		const m = info.module;
		try {
			...
			try {
				...
				({ ast } = JavascriptParser._parse(
					code,
					{
						// a wrapped CommonJS body may `return` at its top level
						sourceType: info.wrapped ? "auto" : "module",
						ranges: true,
			...
			// the wrapper isolates the body's declarations, so [3] renames nothing in
			// it and its scopes are not kept; [2] claims its identifiers textually
			if (info.wrapped) return;
			info.shorthandIdentifierStarts = analysis.shorthandIdentifierStarts;
				...
				switch (info.type) {
					case "concatenated":
						item = {
							...
							shorthandIdentifierStarts: undefined,
