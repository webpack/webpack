...
${defineGetters}`
		...
		const deferredModulesMap = new Map();
		if (deferEnabled) {
			for (const moduleInfo of modulesWithInfo) {
				if (
					moduleInfo.type === "external" &&
					moduleInfo.module &&
					moduleGraph.isDeferred(moduleInfo.module)
				) {
					deferredModulesMap.set(moduleInfo.module, moduleInfo);
				}
			}
		}
					...
					if (deferEnabled) {
						for (const dep of info.module.dependencies) {
							if (
								...
								const deferredModuleInfo =
									deferredModulesMap.get(referredModule);
