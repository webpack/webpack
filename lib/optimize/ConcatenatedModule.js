 ...
 * @property {SyncBailHook<[ConcatenatedModule, RuntimeSpec[], string, Record<string, string>, RuntimeRequirements], boolean>} onDemandExportsGeneration
...
class ConcatenatedModule extends Module {
	...
	codeGeneration({
		...
		// Tracks whether the `exports`/`definePropertyGetters` requirements we
		// add below have a producer in the result other than the (potentially
		// stashed) on-demand exports source. Used by the
		// `onDemandExportsGeneration` consumer to decide whether the
		// requirements are still needed when the source is dropped.
		let onDemandIsOnlyDefinePropertyGettersReason = false;
		let onDemandIsOnlyExportsReason = false;
		...
		if (exportsMap.size > 0) {
			...
			if (
				...
				// On-demand source was stashed by the hook. Track that, before
				// generating namespace objects and deferred external modules,
				// the on-demand source is the sole producer for these globals
				// in the result. Subsequent additions below will clear the
				// flags as appropriate.
				onDemandIsOnlyDefinePropertyGettersReason = true;
				onDemandIsOnlyExportsReason = !shouldAddHarmonyFlag;
			}
		}
		...
		for (const info of neededNamespaceObjects) {
			...
			if (nsObj.length > 0) {
				runtimeRequirements.add(RuntimeGlobals.definePropertyGetters);
				onDemandIsOnlyDefinePropertyGettersReason = false;
			}
...
${defineGetters}`
		...
		if (onDemandIsOnlyDefinePropertyGettersReason) {
			data.set("onDemandIsOnlyDefinePropertyGettersReason", true);
		}
		if (onDemandIsOnlyExportsReason) {
			data.set("onDemandIsOnlyExportsReason", true);
		}
