...
class ConcatenatedModule extends Module {
	...
	source(dependencyTemplates, outputOptions, requestShortener) {
		...
		modulesWithInfo.forEach(info => {
			info.globalScope.through.map(reference => reference.identifier.name).forEach(name => {
		...
		modulesWithInfo.forEach(info => {
			const namespaceObjectName = this.findNewName("namespaceObject", allUsedNames, info.module.readableIdentifier(requestShortener));
	...
	findNewName(oldName, usedNamed, extraInfo) {
		let name = oldName;
		...
		while(splittedInfo.length) {
			...
			if(!usedNamed.has(Template.toIdentifier(name))) return Template.toIdentifier(name);
		}

		while(!usedNamed.has(name = name + "_"));
		return name;
	}
		...
		if(dep.originModule === this.rootModule) {
			if(this.modulesMap.get(dep.importDependency.module)) {
				...
				exportDefs.forEach(def => {
					...
					const content = `/* concated harmony reexport */__webpack_require__.d(${exportsName}, ${JSON.stringify(def.name)}, function() { return ${finalName}; });\n`;
