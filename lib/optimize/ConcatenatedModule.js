 ...
 * @property {InitFragment<ChunkRenderContext>[] | undefined} chunkInitFragments
 * @property {ReadOnlyRuntimeRequirements | undefined} runtimeRequirements
 * @property {Map<string, string | string[] | { name: string[] }> | undefined} exportMap
 * @property {Map<string, string | string[]> | undefined} rawExportMap
 * @property {string | undefined} namespaceExportSymbol
 * @property {boolean} interopNamespaceObjectUsed "default-with-named" namespace
 * @property {string | undefined} interopNamespaceObjectName "default-with-named" namespace
 * @property {boolean} interopNamespaceObject2Used "default-only" namespace
 * @property {string | undefined} interopNamespaceObject2Name "default-only" namespace
...
class ConcatenatedModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		const { moduleArgument, exportsArgument, module } =
			/** @type {BuildInfo} */
...
${defineGetters}`
		...
		for (const rawInfo of modulesWithInfo) {
			...
			switch (info.type) {
				...
				case "external": {
					...
					// Check if this is an ESM external module
					const externalModule = /** @type {ExternalModule} */ (info.module);
					const isESMExternal =
						externalModule.externalType === "module" &&
						/** @type {BuildInfo} */ (this.buildInfo).module;

					if (isESMExternal) {
						// Generate ES module imports for external modules when output.module is true

						// Create a concatenation scope for tracking names and avoiding collisions
						// Cast to ConcatenatedModuleInfo for compatibility
						const externalConcatenationScope = new ConcatenationScope(
							moduleToInfoMap,
							/** @type {ConcatenatedModuleInfo} */ (
								/** @type {unknown} */ (info)
							),
						...
						const codeGenResult = externalModule.codeGeneration({
							dependencyTemplates,
							runtimeTemplate,
							moduleGraph,
							chunkGraph,
							runtime,
							concatenationScope: externalConcatenationScope,
							codeGenerationResults,
							sourceTypes: JS_TYPES
						});

						// Collect InitFragments (import statements) from the external module
						const data = codeGenResult.data;
						if (data) {
							const externalChunkInitFragments = data.get("chunkInitFragments");
							if (externalChunkInitFragments) {
								for (const f of externalChunkInitFragments) {
									chunkInitFragments.push(f);
								}
							}
						}

						// Handle deferred ESM external modules
						if (info.deferred) {
						...
						// Legacy require-style handling for non-ESM externals
	...
	_analyseModule(
		...
		if (info.type === "concatenated") {
			...
			// Analyze external module exports for tree-shaking optimization
			const externalModule = /** @type {ExternalModule} */ (info.module);
			const buildInfo = /** @type {BuildInfo} */ (this.buildInfo);

			// Only analyze ESM external modules when outputting ES modules
			if (externalModule.externalType === "module" && buildInfo.module) {
				const exportsInfo = moduleGraph.getExportsInfo(externalModule);
				const usedExports = exportsInfo.getUsedExports(runtime);

				// Track used exports for optimal tree-shaking
				if (
					usedExports !== false &&
					usedExports !== true &&
					usedExports !== null
					...
					for (const exportName of usedExports) {
						if (Array.isArray(exportName)) {
							...
							if (info.exportMap) info.exportMap.set(key, key);
							if (info.rawExportMap) info.rawExportMap.set(key, exportName);
						} else {
							// Handle simple exports
							if (info.exportMap) info.exportMap.set(exportName, exportName);
							if (info.rawExportMap) {
								info.rawExportMap.set(exportName, [exportName]);
							}
						}
					}
				}
