...
${defineGetters}`
		...
		for (const info of wrappedInfos) {
			if (info.type === "concatenated") {
				...
				// Wrapping the accessor covers every reference at once, and the
				// runtime caches per exports object so identity still holds.
				let required = `${RuntimeGlobals.require}(${moduleId})`;
				...
				if (
					...
					required = `${RuntimeGlobals.specNamespaceObject}(${required})`;
				}
			...
			switch (info.type) {
				...
				case "external": {
					...
					if (!info.deferred && !info.wrapped) {
						...
						let required = `__webpack_require__(${moduleId})`;

						// The binding covers every reference at once, and the runtime
						// caches per exports object so identity still holds.
						if (
							/** @type {BuildInfo} */
							(info.module.buildInfo).specNamespaceObject &&
							getExportsType(
