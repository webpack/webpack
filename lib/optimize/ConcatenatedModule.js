...
${defineGetters}`
			...
			if (info.type === "concatenated") {
				...
				result.add(
					`\n// DEFERRED EXTERNAL MODULE: ${info.module.readableIdentifier(
						requestShortener
					)}\nvar ${info.deferredName} = /*#__PURE__*/${loader};`
				);
					...
					if (info.wrapped && info.eager) {
						...
						result.add(
							`\n;// ${info.module.readableIdentifier(
								requestShortener
							)}\n${runtimeTemplate.moduleSideEffectsAnnotation(
								info.module,
								moduleGraph
							)}${info.namespaceFnName}();\n`
						);
	...
	_analyseModule(
		...
		if (info.type === "concatenated") {
			const m = info.module;
			if (info.wrapped) {
				// the wrapper isolates the body's top-level declarations, so only a
				// concatenation scope is needed, rewriting in-scope `require()` targets.
				// Stage 2 claims its identifiers textually, in place of that analysis.
