			...
			case "default-with-named":
				switch (exportName[0]) {
					case "default":
						...
						if (deferred) {
							// `ns.default` here must read through the `.a` getter, which
							// evaluates the module lazily, not the proxy namespace: else
							// `typeof` and `instanceof` observe the proxy.
							return {
								info,
...
class ConcatenatedModule extends Module {
	...
	// Stages 1-5 only mutate the per-module info records — names and ReplaceSource
	// patches — while 6-10 append to the one output ConcatSource: exports, then the
	// definitions every body needs before it runs, then the bodies themselves.
	/**
