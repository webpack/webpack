...
class ConcatenatedModule extends Module {
	...
	_createOrderedConcatenationList(rootModule, modulesSet) {
		...
		function getConcatenatedImports(module) {
			...
			const lastConcatenatedItem = findLastIndex(allDeps, dep => modulesSet.has(dep.module));

			if(lastConcatenatedItem === -1) return [];

			return allDeps.slice(0, lastConcatenatedItem + 1).map(dep => dep.module);
		}
	...
	source(dependencyTemplates, outputOptions, requestShortener) {
		...
		const modules = this._orderedConcatenationList.filter(item => item.type === "concatenated").map(item => item.module);

		// Metainfo for each module
		const modulesWithInfo = modules.map((m, idx) => {
		...
		this._orderedConcatenationList.forEach(entry => {
			switch(entry.type) {
				case "concatenated":
					{
						result.add(`\n// CONCATENATED MODULE: ${entry.module.readableIdentifier(requestShortener)}\n`);
						const info = moduleToInfoMap.get(entry.module);
				...
				case "external":
					...
					break;
				default:
					throw new Error(`Unsupported concatenation entry type ${entry.type}`);
			}
		});
