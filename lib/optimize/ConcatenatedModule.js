 ...
 * @property {string} deferredNamespaceObjectName
 * @property {boolean} interopNamespaceObjectUsed
		...
		switch (info.type) {
			...
			case "external":
				if (deferred) {
					...
					return {
						info,
						rawName: info.deferredNamespaceObjectName,
						ids: exportName,
						exportName
					};
				}
...
class ConcatenatedModule extends Module {
	...
	_createConcatenationList(rootModule, modulesSet, runtime, moduleGraph) {
		...
		const getConcatenatedImports = module => {
			 ...
			 */
			references.sort(
				concatComparators(byDeferOrder, bySourceOrder, byRangeStart)
			);
