 ...
 * @property {boolean | "partial-deferred"} deferred the module is deferred at least once
 ...
 * @property {RuntimeSpec | boolean} runtimeCondition
 * @property {boolean | "partial-deferred"} deferred
...
class ConcatenatedModule extends Module {
	...
	_createConcatenationList(rootModule, modulesSet, runtime, moduleGraph) {
		...
		const list = [];
		/** @type {Map<Module, { runtimeCondition: RuntimeSpec | true, deferred: boolean | "partial-deferred" }>} */
		const existingEntries = new Map();

		/**
		 * @param {Module} module a module
		 * @returns {Iterable<{ connection: ModuleGraphConnection, runtimeCondition: RuntimeSpec | true, deferred: boolean | "partial-deferred" }>} imported modules in order
		 */
		const getConcatenatedImports = (module) => {
			...
			/** @type {Map<Module, { connection: ModuleGraphConnection, runtimeCondition: RuntimeSpec | true, deferred: boolean | "partial-deferred" }>} */
			...
			for (const { connection, defer } of references) {
				...
				entry.deferred =
					entry.deferred === defer ? entry.deferred : "partial-deferred";
			}
		 ...
		 * @param {boolean | "partial-deferred"} deferred deferred condition
		 * @returns {void}
		 */
		const enterModule = (connection, runtimeCondition, deferred) => {
			const module = connection.module;
			if (!module) return;
			...
			if (
				...
				existingEntry.deferred === "partial-deferred"
			) {
				return;
			}
			if (modulesSet.has(module)) {
				...
				for (const { connection, runtimeCondition, deferred } of imports) {
					enterModule(connection, runtimeCondition, deferred);
				}
				list.push({
					type: "concatenated",
				...
				if (existingEntry !== undefined) {
					const deferredChanged = existingEntry.deferred !== deferred;
					const reducedRuntimeCondition = subtractRuntimeCondition(
						runtimeCondition,
						existingEntry.runtimeCondition,
						runtime
					);
					if (reducedRuntimeCondition === false && !deferredChanged) return;
...
${defineGetters}`
		...
		for (const rawInfo of modulesWithInfo) {
			...
			switch (info.type) {
				...
				case "external": {
					const deferred =
						rawInfo.type === "reference" ? rawInfo.deferred : info.deferred;
					...
					if (!deferred) {
						...
						name = info.name;
					} else if (deferred === "partial-deferred") {
						result.add(
