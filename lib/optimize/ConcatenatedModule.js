	...
	source(dependencyTemplates, outputOptions, requestShortener) {
		...
		modulesWithInfo.forEach(info => {
			if(info.globalScope) {
				info.globalScope.through.forEach(reference => {
					...
					if(/^__WEBPACK_MODULE_REFERENCE__\d+_(\d+|ns)(_call)?__$/.test(name)) {
		...
		modulesWithInfo.forEach(info => {
			if(info.type === "concatenated") {
				info.globalScope.through.forEach(reference => {
					const name = reference.identifier.name;
					const match = /^__WEBPACK_MODULE_REFERENCE__(\d+)_(\d+|ns)(_call)?__$/.exec(name);
					if(match) {
...
class HarmonyImportSpecifierDependencyConcatenatedTemplate {
	...
	apply(dep, source, outputOptions, requestShortener, dependencyTemplates) {
		...
		if(dep.id === null) {
			...
			content = exportIdx === -1 ? "undefined" : `__WEBPACK_MODULE_REFERENCE__${info.index}_${exportIdx}${dep.call ? "_call" : ""}__`;
		}
		if(dep.shorthand) {
