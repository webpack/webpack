...
import { STAGE_DEFAULT } from "../OptimizationStages.js";
import { compareModulesByIdentifier } from "../util/comparators.js";
import {
	filterRuntime,
	intersectRuntime,
	mergeRuntime,
...
class ModuleConcatenationPlugin {
			...
			compilation.hooks.optimizeChunkModules.tapAsync(
				...
				(allChunks, modules, callback) => {
					...
					for (const module of modules) {
						...
						const basicTypes = module.getSourceBasicTypes();
	...
	_getImports(compilation, module, runtime) {
		...
		for (const dep of module.dependencies) {
			...
			if (!dep.canConcatenate()) continue;
	...
	_tryToAdd(
		...
		/** @type {Map<Module, ReadonlyArray<import("../ModuleGraphConnection.js").default>>} */
		...
		for (const [originModule, connections] of incomingConnectionsFromModules) {
			const selected = connections.filter(
				(connection) =>
					!connection.dependency || !connection.dependency.canConcatenate()
			);
...
export default ModuleConcatenationPlugin;

export { ModuleConcatenationPlugin as "module.exports" };
