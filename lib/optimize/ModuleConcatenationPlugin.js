...
class ModuleConcatenationPlugin {
			...
			compilation.hooks.optimizeChunkModules.tapAsync(
				...
				(allChunks, modules, callback) => {
					...
					for (const module of modules) {
						...
						// ConcatenatedModule only outputs javascript, so modules
						// with non-javascript source types cannot be a root
						// because their non-js output would be lost after concatenation
									...
									if (m !== rootModule) {
										...
										moduleGraph.copyOutgoingModuleConnections(
											...
											(c) =>
												...
												!(
													c.dependency &&
													Dependency.supportsConcatenation(c.dependency) &&
	...
	_getImports(compilation, module, runtime) {
		...
		for (const dep of module.dependencies) {
			...
			if (!Dependency.supportsConcatenation(dep)) continue;
		...
		for (const [originModule, connections] of incomingConnectionsFromModules) {
			const selected = connections.filter(
				(connection) =>
					...
					!Dependency.supportsConcatenation(connection.dependency)
			);
