			...
			compilation.hooks.optimizeChunkModules.tapAsync(
				...
				(allChunks, modules, callback) => {
					...
					for (const module of modules) {
						...
						// Async modules require the ConcatenatedModule to emit the
						// outer `__webpack_require__.a` wrapper. The generator
						// signals readiness by returning `undefined` from
						// `getConcatenationBailoutReason` (already passed above).
					...
					asyncLib.each(
						...
						(concatConfiguration, callback) => {
							...
							const integrate = () => {
								...
								// If the concatenated module wraps any async
								// inner module its body emits the
								// `__webpack_require__.a` wrapper, so the new
								// module is async too. `InferAsyncModulesPlugin`
								// already ran during `finishModules` and won't
								// see this module, so mark it here.
								if (/** @type {BuildMeta} */ (newModule.buildMeta).async) {
									moduleGraph.setAsync(newModule);
								}
