...
const { isCommonJsConcatenationEnabled } = require("../util/concatenate");
const {
	filterRuntime,
	intersectRuntime,
	mergeRuntime,
...
/** @import { BuildInfo, BuildMeta } from "../module/Module" */
...
// Read back by `performance.scopeHoistingBailouts` to tell these reasons apart
// from the other kinds `getOptimizationBailout` collects.
const BAILOUT_PREFIX = "ModuleConcatenation bailout: ";

// A root's reason for rejecting another module, rather than the module's own.
const REJECTED_PREFIX = "Cannot concat with ";
 ...
 */
const formatBailoutReason = (msg) => `${BAILOUT_PREFIX}${msg}`;
...
class ModuleConcatenationPlugin {
			...
			const formatBailoutWarning = (module, problem) => (requestShortener) => {
				if (typeof problem === "function") {
					return formatBailoutReason(
						`${REJECTED_PREFIX}${module.readableIdentifier(
							requestShortener
			...
			compilation.hooks.optimizeChunkModules.tapAsync(
				...
				(allChunks, modules, callback) => {
					...
					for (const module of modules) {
						...
						// Concatenation materializes the namespace inline, which would
						// silently drop the spec exotic object the module asked for.
						if (
							/** @type {BuildInfo} */
							(module.buildInfo).specNamespaceObject
						) {
							setBailoutReason(
								module,
								"Module hands out a spec namespace object"
							);
							continue;
						}
						...
						// The root stands in for the whole concatenation, so an
						// expression only an inner module can answer statically rules it out
						if (
							/** @type {JavascriptModuleBuildInfo} */
							(module.buildInfo).moduleConcatenationRootBailout
						) {
							canBeRoot = false;
						}
					...
					// Largest configurations first, so a root that also landed inside a
					// bigger one loses to it. None is reused across roots: a module shared by
					// several chunks is concatenated into each consumer.
	...
	_defersModuleIn(moduleGraph, module, config) {
		for (const dep of module.dependencies) {
			if (
				dep instanceof ESMImportDependency &&
	...
	_getImports(compilation, module, runtime) {
		...
		for (const dep of module.dependencies) {
			...
			if (!Dependency.canConcatenate(dep, concatenateCommonJsModules)) continue;
	...
	_tryToAdd(
		...
		/** @type {Map<Module, ReadonlyArray<InstanceType<ModuleGraph.ModuleGraphConnection>>>} */
		...
		/** @type {Map<Module, ReadonlyArray<InstanceType<ModuleGraph.ModuleGraphConnection>>>} */
		const nonTargetConnections = new Map();
		for (const [originModule, connections] of incomingConnectionsFromModules) {
			...
			if (selected.length > 0) {
				nonTargetConnections.set(originModule, selected);
			}
		}
		if (nonTargetConnections.size > 0) {
			/**
			 * Returns problem description.
			 * @param {RequestShortener} requestShortener request shortener
			 * @returns {string} problem description
			 */
			const problem = (requestShortener) => {
				const names = [...nonTargetConnections]
					.map(
...
ModuleConcatenationPlugin.BAILOUT_PREFIX = BAILOUT_PREFIX;
ModuleConcatenationPlugin.REJECTED_PREFIX = REJECTED_PREFIX;
