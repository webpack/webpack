...
class MinChunkSizePlugin {
	constructor(options) {
		const pluginValidationErrors = validateSchema(minChunkSizePluginSchema, options);

		if(pluginValidationErrors.length) {
			throw new WebpackPluginValidationError(pluginValidationErrors, minChunkSizePluginSchema);
		}
