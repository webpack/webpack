...
const { CONST_BINDING_TAG } = require("./ConstExportsPlugin");
...
class InnerGraphPlugin {
	/**
	 * @param {{ usedExportProperties?: boolean }} options options
	 */
	constructor(options = {}) {
		this._usedExportProperties = options.usedExportProperties || false;
	}
	 ...
	 */
	apply(compiler) {
		const usedExportProperties = this._usedExportProperties;
				...
				const handler = (parser, parserOptions) => {
					...
					parser.hooks.statement.tap(PLUGIN_NAME, (statement) => {
						...
						if (parser.scope.topLevelScope === true) {
							...
							// Compound component: TopLevelSymbol.prop = value
							if (
								usedExportProperties &&
								...
								const objName = statement.expression.left.object.name;
								const propName = statement.expression.left.property.name;
								const rhs = statement.expression.right;
								const objSymbol = /** @type {TopLevelSymbol | undefined} */ (
									parser.getTagData(objName, topLevelSymbolTag)
								);
								if (
									objSymbol &&
									parser.getTagData(objName, CONST_BINDING_TAG) !== undefined
								) {
									const buildInfo = parser.state.module.buildInfo;
									if (!buildInfo) return;
									if (!buildInfo.objectExportProperties) {
										buildInfo.objectExportProperties = new Map();
									}
									const existing =
										buildInfo.objectExportProperties.get(objName);
									if (existing) {
										existing.push(propName);
									} else {
										buildInfo.objectExportProperties.set(objName, [propName]);
									}
									// Pure RHS (e.g. Form.Item = Item): gate per nested property.
									// Impure RHS (e.g. x.primary = x[5], or calls): gate only when
									// the whole export is unused — keeps side effects when the
									// object is still live, and avoids `null.prop = …`.
									const rhsPure = parser.isPure(
										rhs,
										/** @type {Range} */ (statement.expression.left.range)[1]
									);
									innerGraph.setTopLevelSymbol(parser.state, objSymbol);
									if (rhsPure) {
										innerGraph.setPropertyContext(parser.state, propName);
									}
									const loc = parser.getLocation(statement);
									innerGraph.onUsage(parser.state, (usedByExports, module) => {
										switch (usedByExports) {
											...
											default: {
												const dep = new PureExpressionDependency(
													/** @type {Range} */ (statement.expression.range)
												);
												dep.loc = loc;
												dep.usedByExports = usedByExports;
												addPureDependency(module, dep);
									...
									parser.walkExpression(rhs);
									if (rhsPure) {
										innerGraph.clearPropertyContext(parser.state);
									}
									innerGraph.setTopLevelSymbol(parser.state, undefined);
									return true;
								}
