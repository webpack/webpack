		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser, parserOptions) => {
					...
					const pureClassSupers = new WeakSet();
						...
						if (parser.scope.topLevelScope === true) {
							if (statement.type === "ClassDeclaration") {
								...
								if (
									...
									isPure(statement, parser, statement.superClass.range[0])
								) {
									pureClassSupers.add(statement.superClass);
								}
						...
						if (parser.scope.topLevelScope === true) {
							...
							if (fn) {
								...
								if (
									...
									statement.superClass &&
									pureClassSupers.has(statement.superClass)
								) {
									InnerGraph.onUsage(parser.state, usedByExports => {
										switch (usedByExports) {
											case undefined:
											case true:
												return;
											default: {
												const sup = statement.superClass;
