...
/** @typedef {import("estree").ExpressionStatement} ExpressionStatementNode */
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser, parserOptions) => {
					...
					/** @type {WeakSet<StatementNode>} */
					const pureExpressionStatements = new WeakSet();
						...
						if (parser.scope.topLevelScope === true) {
							...
							if (
								...
								parser.isPure(
									...
									parser.prevStatement ? parser.prevStatement.end : 0
								)
							) {
								pureExpressionStatements.add(statement);
							}
						}
					});
					...
					parser.hooks.statement.tap("InnerGraphPlugin", statement => {
						...
						if (parser.scope.topLevelScope === true) {
							...
							if (
								pureExpressionStatements.has(
									/** @type {StatementNode} */ (statement)
								)
							) {
								const dep = new PureExpressionDependency(
									/** @type {ExpressionStatementNode} */ (statement).expression.range
								);
