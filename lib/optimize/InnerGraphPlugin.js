		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser, parserOptions) => {
					...
					/**
					 * @param {VariableDeclarator} decl declarator
					 * @param {VariableDeclaration} statement declaration statement
					 */
					const preDeclaratorHandler = (decl, statement) => {
						if (!innerGraph.isEnabled(parser.state)) return;
					...
					parser.registerColumnGate(
						...
						() =>
							parser.scope.topLevelScope === true &&
							innerGraph.isEnabled(parser.state)
					);
					...
					parser.hooks.statement.tap(PLUGIN_NAME, statementHandler);
					// mirrors the handler's bail conditions on the columns
					parser.registerColumnGate(
					...
					/**
					 * @param {VariableDeclarator} decl declarator
					 * @param {Statement} _statement declaration statement
					 * @returns {boolean | void} true to stop the declarator walk
					 */
					const declaratorHandler = (decl, _statement) => {
						if (!innerGraph.isEnabled(parser.state)) return;
					...
					// only declarators observed by the preDeclarator pass can carry a
					// symbol, and those are exactly the already-materialized facades
					parser.registerColumnGate(
						parser.hooks.declarator,
						declaratorHandler,
						(_parser, store, did) => {
							if (!innerGraph.isEnabled(parser.state)) return false;
							const facade = /** @type {VariableDeclarator | undefined} */ (
								store.facades[did]
							);
							if (facade === undefined) return false;
							if (declWithTopLevelSymbol.has(facade)) return true;
							const init = facade.init;
							return Boolean(
								init &&
								init.type === "ClassExpression" &&
								classWithTopLevelSymbol.has(init)
							);
						}
					);
