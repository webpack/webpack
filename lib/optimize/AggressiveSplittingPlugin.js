...
const makeRelative = require("../pathUtil").makeRelative;
		...
		compiler.plugin("compilation", (compilation) => {
			compilation.plugin("optimize-chunks-advanced", (chunks) => {
				...
				for(let j = 0; j < usedSplits.length; j++) {
					...
					for(i = 0; i < chunks.length; i++) {
						chunk = chunks[i];
						const chunkModuleNames = chunk.modules
							.map(module => makeRelative(compiler.context, module.identifier()));
					...
					if(size > maxSize && chunk.modules.length > 1) {
						...
						if(newChunk.modules.length > 0) {
							...
							compilation._aggressiveSplittingSplits = (compilation._aggressiveSplittingSplits || []).concat({
								modules: newChunk.modules.map(module => makeRelative(compiler.context, module.identifier()))
							});
							return true;
						} else {
			...
			compilation.plugin("record-hash", (records) => {
				...
				compilation.chunks.forEach((chunk) => {
					...
					const modules = chunk.modules.map(module => makeRelative(compiler.context, module.identifier()));
