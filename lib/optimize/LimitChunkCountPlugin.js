...
const LazyBucketSortedSet = require("./LazyBucketSortedSet");
...
/**
 * @schema plugins/optimize/LimitChunkCountPlugin
 * @typedef {object} LimitChunkCountPluginOptions
 ...
 * @property {PositiveNumber} maxChunks Limit the maximum number of chunks using a value greater than or equal to 1.
 */
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			compilation.hooks.optimizeChunks.tap(
				...
				(chunks) => {
					...
					// All `chunks * (chunks - 1) / 2` combinations, held in a multi-layer
					// bucket sort that sorts only the buckets actually reached.
							...
							for (const group of queue) {
								for (const mChunk of modifiedChunks) {
									if (mChunk !== a && mChunk !== b && mChunk.isInGroup(group)) {
										// A pair needing recalculation, which cannot happen now yet may
										// merge before the ones that follow, so room is left for it and
										// the chunks count as modified for the worst case.
