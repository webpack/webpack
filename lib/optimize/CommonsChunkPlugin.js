...
Take a look at the "name"/"names" or async/children option.`);
	...
	getAffectedUnnamedChunks(targetChunk, asyncOption) {
		// nothing to do here
		if(!targetChunk.chunks) {
			return [];
		}

		const affectedChunks = targetChunk.chunks.filter((chunk) => {
			// we can only move modules from this chunk if the "commonChunk" is the only parent
			return asyncOption || chunk.parents.length === 1;
		});
		if(!this.deepChildren) {
			return affectedChunks;
		}
		return affectedChunks.reduce((previous, chunk) => previous.concat(this.getAffectedUnnamedChunks(chunk, asyncOption)), affectedChunks);
	}
	...
	getAffectedChunks(compilation, allChunks, targetChunk, targetChunks, currentIndex, selectedChunks, asyncOption, children) {
		...
		if(asyncOrNoSelectedChunk) {
			return this.getAffectedUnnamedChunks(targetChunk, asyncOption);
		}
