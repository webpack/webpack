...
The available options are:
	...
	minSize: number`);
		...
		this.deepChildren = options.deepChildren;
	}
	apply(compiler) {
		...
		const deepChildren = this.deepChildren;
		compiler.plugin("this-compilation", (compilation) => {
			compilation.plugin(["optimize-chunks", "optimize-extracted-chunks"], (chunks) => {
				function pushUsedChunks(usedChunks, commonChunk) {
					(commonChunk.chunks || []).forEach((chunk) => {
						if(asyncOption || chunk.parents.every((parentChunk) => parentChunk === commonChunk || usedChunks.indexOf(parentChunk) >= 0)) {
							if(usedChunks.findIndex(usedChunk => usedChunk === chunk) < 0) {
								usedChunks.push(chunk);
							}

							if(deepChildren === true) {
								pushUsedChunks(usedChunks, chunk);
							}
						}
					});
				}
				...
				commonChunks.forEach(function processCommonChunk(commonChunk, idx) {
					...
					if(Array.isArray(selectedChunks)) {
						...
						usedChunks = [];
						pushUsedChunks(usedChunks, commonChunk, { deepChildren, asyncOption });
					} else {
