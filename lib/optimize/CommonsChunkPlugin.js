...
Take a look at the "name"/"names" or async/children option.`);
	...
	getAffectedUnnamedChunks(targetChunk) {
		// nothing to do here
		...
		return affectedChunks.reduce((previous, chunk) => previous.concat(this.getAffectedUnnamedChunks(chunk)), affectedChunks);
	}
	...
	getAffectedChunks(compilation, allChunks, targetChunk, targetChunks, currentIndex, selectedChunks, asyncOption, children) {
		...
		if(asyncOrNoSelectedChunk) {
			return this.getAffectedChunks(targetChunk);
		}
