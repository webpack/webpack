	...
	apply(compiler) {
		...
		compiler.plugin("this-compilation", (compilation) => {
			compilation.plugin(["optimize-chunks", "optimize-extracted-chunks"], (chunks) => {
				function pushUsedChunks(usedChunks, commonChunk, deepChildren) {
					(commonChunk.chunks || []).forEach((chunk) => {
						if(chunk.parents.every((parentChunk) => parentChunk === commonChunk || usedChunks.indexOf(parentChunk) >= 0)) {
							usedChunks.push(chunk);
						}
						if(deepChildren === true) {
							pushUsedChunks(usedChunks, chunk, deepChildren);
						}
					});
				}
				...
				commonChunks.forEach(function processCommonChunk(commonChunk, idx) {
					...
					if(Array.isArray(selectedChunks)) {
						...
						usedChunks = [];
						pushUsedChunks(usedChunks, commonChunk, deepChildren);
					} else {
