	...
	apply(compiler) {
		...
		compiler.plugin("this-compilation", (compilation) => {
			compilation.plugin(["optimize-chunks", "optimize-extracted-chunks"], (chunks) => {
				function pushUsedChunks(usedChunks, commonChunk, flags) {
					(commonChunk.chunks || []).forEach((chunk) => {
						if(flags.asyncOption || chunk.parents.every((parentChunk) => parentChunk === commonChunk || usedChunks.indexOf(parentChunk) >= 0)) {
							usedChunks.push(chunk);
						}
						if(flags.deepChildren === true) {
							pushUsedChunks(usedChunks, chunk, flags);
						}
					});
				}
