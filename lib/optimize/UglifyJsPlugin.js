...
function UglifyJsPlugin(options) {
	...
	if(typeof options.compress === 'undefined' && typeof options.compressor !== "undefined") {
			...
			files.forEach(function(file) {
				...
				try {
					...
					if(options.compress !== false) {
						ast.figure_out_scope(extend(options.compress));
						var compress = uglify.Compressor(extend(options.compress)); // eslint-disable-line new-cap
						ast = ast.transform(compress);
					}
					if(options.mangle !== false) {
						ast.figure_out_scope(extend(options.mangle));
						ast.compute_char_frequency(extend(options.mangle));
						ast.mangle_names(extend(options.mangle));
						if(options.mangle && options.mangle.props) {
							uglify.mangle_properties(ast, extend(options.mangle.props));
						}
					}
					var output = {};
