...
/**
 * Hex encodings of the export paths a concatenation references, keyed by the
 * module map every scope of that concatenation shares, so they die with it.
 * @type {WeakMap<Map<Module, ModuleInfo>, { single: Map<string, string>, multi: Map<string, string> }>}
 */
const exportDataCache = new WeakMap();

/**
 * Hex-encodes one export path. A path is named by many references over one
 * concatenation, so the encoding is cached rather than rebuilt per reference.
 * @param {Map<Module, ModuleInfo>} modulesMap the scope's module map
 * @param {string[]} ids the export path
 * @returns {string} the hex encoding of the path
 */
const encodeExportData = (modulesMap, ids) => {
	let caches = exportDataCache.get(modulesMap);
	if (caches === undefined) {
		caches = { single: new Map(), multi: new Map() };
		exportDataCache.set(modulesMap, caches);
	}
	// a one-element path is keyed by the element itself, so the common case
	// costs no key building; anything longer is keyed by its JSON
	const single = ids.length === 1;
	const cache = single ? caches.single : caches.multi;
	const key = single ? ids[0] : JSON.stringify(ids);
	const cached = cache.get(key);
	if (cached !== undefined) return cached;
	const encoded = Buffer.from(
		single ? JSON.stringify(ids) : key,
		"utf8"
	).toString("hex");
	...
	createModuleReference(
		...
		const asiSafeFlag = asiSafe
			...
			: asiSafe === false
				? "_asiSafe0"
				: "";
		const exportData = ids ? encodeExportData(this._modulesMap, ids) : "ns";
