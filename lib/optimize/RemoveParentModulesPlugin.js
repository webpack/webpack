	...
	apply(compiler) {
		compiler.hooks.compilation.tap("RemoveParentModulesPlugin", compilation => {
			...
			const handler = (chunks, chunkGroups) => {
				 ...
				 */
				function* getModulesFromMask(mask) {
					let offset = 31;
				...
				for (const chunkGroup of chunkGroups) {
					...
					for (const chunk of chunkGroup.chunks) {
						const chunkInfo = chunkMasks.get(chunk);
				...
				for (const chunkGroup of queue) {
					...
					for (const parent of chunkGroup.parentsIterable) {
						...
						if (availableModulesInParent !== undefined) {
							const parentMask =
								availableModulesInParent.mask |
								chunkGroupMasks.get(parent).mask;
				...
				for (const chunk of chunks) {
					const chunkInfo = chunkMasks.get(chunk);
					if (chunkInfo === undefined) continue; // No info about this chunk
					...
					const toRemoveMask = chunkInfo.mask & availableModulesMask;
					if (toRemoveMask !== ZERO_BIGINT) {
						for (const module of getModulesFromMask(toRemoveMask)) {
