...
function hasModule(chunk, module, checkedChunks) {
	...
	return allHaveModule(chunk.parents, module, checkedChunks);
}

function allHaveModule(someChunks, module, auditChunks) {
	var checkedChunks = auditChunks || Object.create(null);
	var chunks = [];
	for(var i = 0; i < someChunks.length; i++) {
		var chunk = someChunks[i];

		if(auditChunks && checkedChunks[chunk.debugId]) {
			continue;
		}

		checkedChunks[chunk.debugId] = chunk;
		var subChunks = hasModule(chunk, module, checkedChunks);
