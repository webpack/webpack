...
const plugin = "RealContentHashPlugin";
...
class RealContentHashPlugin {
	 ...
	 * @param {string=} options.hashPrefix the hash prefix to use
	 */
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(plugin, compilation => {
			TemplatedPathPlugin.getCompilationHooks(compilation).assetInfoHash.tap(
				plugin,
				(result, hashName) => {
					if (hashName === "contenthash") {
						return this._hashPrefix + result;
					}
				}
			);

			const cacheAnalyse = compilation.getCache(`${plugin}|analyse`);
			const cacheGenerate = compilation.getCache(`${plugin}|generate`);
	...
	.join("\n")}`);
					...
					for (const oldHash of hashesInOrder) {
						...
						if (!newHash) {
							...
							newHash = /** @type {string} */ (
								digest.slice(0, oldHashWithoutPrefix.length)
							);
						}
