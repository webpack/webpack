		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, compilation => {
			...
			compilation.hooks.processAssets.tapPromise(
				...
				async () => {
					 ...
					 * @param {(hash: string, isString: boolean, start: number, index: number) => void} inContent callback
					 */
					const walkHashes = (content, inContent) => {
						try {
							const handler = (
								/** @type {string} */ str,
								/** @type {(hash: string, index: number) => void} */ cb
							) => {
								let result;
							...
							walk.simple(ast, {
								Literal(/** @type {import('acorn').Literal} */ node) {
									const { raw, value, start } = node;
									handler(
										raw || "",
										(/** @type {string} */ hash, /** @type {number} */ index) =>
											inContent(hash, typeof value === "string", start, index)
									);
								},
								Property(/** @type {import('acorn').Node} */ node) {
									const { name, raw, value, type, start } =
										/** @type {import('acorn').Literal & import('acorn').Identifier} */ (
											node
										);
									handler(
										raw || name,
					 ...
					 * @param {OwnHashes=} ownHashes owned hashes of the asset
					 * @returns {string} the replaced content
					 */
					const replaceHashes = (content, ownHashes) => {
						/** @type {{start: number, end: number, hash: string}[]} */
						const positions = [];
						walkHashes(content, (hash, quoted, start, index) => {
							const wrap = (/** @type {string} */ str) =>
								quoted ? str : `'${str}'`;
							positions.push({
								start: start + index,
