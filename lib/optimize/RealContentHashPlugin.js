		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, compilation => {
			...
			compilation.hooks.processAssets.tapPromise(
				...
				async () => {
					...
					/** @param {OwnHashes} [ownHashes] */
					const hashReplacement = ownHashes => (/** @type string */ ignore, /** @type string */ caller, /** @type string */ hash) => {
						if (ownHashes && ownHashes.has(hash)) {
							return "";
						}
						const newHash = /** @type {string} */ (hashToNewHash.get(hash));
						return /[^\d]/.test(newHash) && caller
							? `('${newHash}')`
							: caller
								? `(${newHash})`
								: newHash;
