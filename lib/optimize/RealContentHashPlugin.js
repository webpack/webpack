 ...
 * @property {string=} hashPrefix the hash prefix to use
 */
...
class RealContentHashPlugin {
	 ...
	 */
	constructor({ hashFunction, hashDigest, hashPrefix = "" }) {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			compilation.hooks.contentHash.tap(
				...
				(chunk) => {
					for (const type of Object.keys(chunk.contentHash)) {
						chunk.contentHash[type] =
							this._hashPrefix + chunk.contentHash[type];
					}
				}
			);
			TemplatedPathPlugin.getCompilationHooks(compilation)
				.hashLength.for("contenthash")
				.tap(
					PLUGIN_NAME,
					(length) => length && length + this._hashPrefix.length
				);

			const cacheAnalyse = compilation.getCache(`${PLUGIN_NAME}|analyse`);
			const cacheGenerate = compilation.getCache(`${PLUGIN_NAME}|generate`);
...
${referencingAssets
	...
	.join("\n")}`);
					...
					for (const oldHash of hashesInOrder) {
						...
						if (!oldHash.startsWith(this._hashPrefix)) {
							throw new Error(
								`Unexpected hash ${oldHash} (expected to start with ${this._hashPrefix})`
							);
						}
						const oldHashWithoutPrefix = oldHash.slice(this._hashPrefix.length);
						let newHash = hooks.updateHash.call(
							assetsContent,
							oldHashWithoutPrefix
						);
						if (!newHash) {
							...
							newHash = digest.slice(0, oldHashWithoutPrefix.length);
						}
