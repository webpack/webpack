...
/** @import ExportsInfo, { ExportInfo } from "../ExportsInfo" */
/** @import { UsedNames } from "../util/concatenate" */
...
// Sort by name
/** @type {Comparator<InstanceType<ExportInfo>>} */
...
const mangleExportsInfo = (deterministic, exportsInfo, isNamespace) => {
	...
	// A non-provided export is left unmangled outside a namespace export, where the
	// prototype chain may still carry it, and under deterministic mode beside other
	// provided exports, where it would break the determinism.
		...
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			compilation.hooks.optimizeCodeGeneration.tap(PLUGIN_NAME, (modules) => {
				...
				for (const module of modules) {
					// A spec namespace reports the names the module exports, so a
					// module handing one out cannot have them renamed.
					if (
						/** @type {BuildInfo} */
