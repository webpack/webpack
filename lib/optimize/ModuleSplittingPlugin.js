...
/** @typedef {{ specifiers: Map<string, Dependency[]>, namespaceConsumers: Dependency[], reexported: boolean, opaque: boolean }} IncomingInfo */
...
class ModuleSplittingPlugin {
	 ...
	 */
	isOriginExclusive(incoming, origin, syncReachable) {
		const originInfo = incoming.get(origin.module);
		if (originInfo === undefined) return true;
		if (originInfo.opaque || originInfo.reexported) return false;
			...
			if (
				...
				(local ||
					(specifierDeps.length === 0 &&
						this.isOriginExclusive(incoming, origin, syncReachable)))
			) {
