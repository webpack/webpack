...
/** @typedef {{ candidates: Map<string, Candidate>, hint: boolean | undefined }} AnalysisResult */
...
class ModuleSplittingPlugin {
	 ...
	 * Whether `module`'s exports may be split, by precedence: in-source
	 * `webpackSplit` hint > `module.rules` flag > experiments include/exclude >
	 * automatic default. The automatic default follows Turbopack: a module that
	 * declares side effects (`package.json "sideEffects"` / a `sideEffects` rule)
	 * is left alone unless explicitly opted in above.
	...
	isEligible(module, cache) {
		...
		if (this.include && !this.matchesFilter(this.include, resource)) {
			return false;
		}
		if (this.exclude && this.matchesFilter(this.exclude, resource)) {
			return false;
		}
		// Turbopack-style: don't touch modules declared to have side effects.
		if (factoryMeta && factoryMeta.sideEffectFree === false) return false;
		return true;
	}
	...
	analyze(module, cache) {
		...
		result = { candidates: new Map(), hint: undefined };
		if (this.isStrictEsm(module)) {
			...
			if (original) {
				...
				const list = this.findSplittableExports(source);
				if (list) {
