...
class ModuleSplittingPlugin {
	...
	splitModules(compilation, modules) {
		...
		/** @type {Map<Module, Map<string, Candidate>>} */
		...
		for (const host of modules) {
			...
			if (info.namespaceConsumers.length === 0) {
				const candidates = this.getCandidates(host, candidateCache);
	...
	/**
	 * Returns the splittable export candidates of a module, keyed by export name
	 * (cached for the current compilation).
	 * @param {Module} module module
	 * @param {Map<Module, Map<string, Candidate>>} cache per-compilation cache
	 * @returns {Map<string, Candidate>} candidates by name
	 */
	getCandidates(module, cache) {
		let map = cache.get(module);
		if (map !== undefined) return map;
		map = new Map();
		if (this.isStrictEsm(module)) {
			const original = module.originalSource && module.originalSource();
			if (original) {
				const list = this.findSplittableExports(String(original.source()));
				if (list) {
					for (const candidate of list) map.set(candidate.name, candidate);
				}
			}
		}
		cache.set(module, map);
		return map;
	}
	 ...
	 * @param {IncomingInfo} info incoming info
	 * @param {Set<Module>} syncReachable sync-reachable modules
	 * @param {Map<Module, Map<string, Candidate>>} cache candidate cache
	 * @returns {void}
	 */
	facadeSplit(compilation, host, info, syncReachable, cache) {
		...
		for (const exportInfo of exportsInfo.orderedExports) {
			...
			const candidate = this.getCandidates(origin.module, cache).get(
				origin.name
			);
