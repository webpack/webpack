...
class ModuleSplittingPlugin {
	...
	splitModules(compilation, modules) {
		...
		for (const host of modules) {
			...
			// `export *` re-export layers need recursive splitting (not yet handled).
			...
			if (info.namespaceConsumers.length === 0) {
				...
				this.facadeSplit(compilation, host, info, byName, syncReachable);
			}
		}
	}
	 ...
	 * @param {Candidate[]} candidates splittable export candidates
	...
	/**
	 * Phase 2: when the host's whole namespace is consumed (dynamic `import()` /
	 * `import *`), build a facade that re-exposes every export — splittable
	 * async-only ones from their parts, the rest from the host — and redirect the
	 * namespace consumers to it, so heavy split exports follow the async chunk.
	 * @param {Compilation} compilation compilation
	 * @param {Module} host host module
	 * @param {IncomingInfo} info incoming info
	 * @param {Map<string, Candidate>} byName candidates by export name
	 * @param {Set<Module>} syncReachable sync-reachable modules
	 * @returns {void}
	 */
	facadeSplit(compilation, host, info, byName, syncReachable) {
		const moduleGraph = compilation.moduleGraph;
		...
		for (const exportInfo of exportsInfo.orderedExports) {
			if (exportInfo.provided === true && exportInfo.name) {
		...
		for (const name of provided) {
			...
			if (candidate !== undefined && allAsync) {
				const part = this.createPart(compilation, host, candidate);
				for (const dep of specifierDeps) moduleGraph.updateModule(dep, part);
				entries.push({
					exportName: name,
