...
class ModuleSplittingPlugin {
	...
	/**
	 * Analyzes a module once (cached): its splittable export candidates keyed by
	 * name and any in-source `webpackSplit` hint.
	 * @param {Module} module module
		...
		if (this.isStrictEsm(module)) {
			...
			if (original) {
				...
				if (ast) {
					...
					result.sideEffectFree = this.isSideEffectFreeBody(body);
					for (const candidate of this.findSplittableExports(source, body)) {
	...
	/**
	 * Source-level side-effect analysis: a module is side-effect-free when every
	 * top-level statement is an import/export/declaration with a pure initializer
	 * (no top-level expression statements, calls, etc.).
	 * @param {EXPECTED_ANY[]} body program body
	 * @returns {boolean} true if side-effect-free
	 */
	isSideEffectFreeBody(body) {
		for (const node of body) {
			switch (node.type) {
				...
				case "VariableDeclaration":
					if (
						!node.declarations.every((/** @type {EXPECTED_ANY} */ d) =>
							this.isPureDeclarator(d)
						)
					) {
						return false;
					}
				...
				case "ExportNamedDeclaration":
					if (
						...
						!node.declaration.declarations.every(
							(/** @type {EXPECTED_ANY} */ d) => this.isPureDeclarator(d)
						)
					) {
						return false;
					}
				...
				case "ExportDefaultDeclaration":
					if (
						...
						!isPureInitializer(node.declaration)
					) {
						return false;
					}
					break;
	...
	/**
	 * @param {EXPECTED_ANY} declarator variable declarator
	 * @returns {boolean} true if it has no initializer or a pure one
	 */
	isPureDeclarator(declarator) {
		return declarator.init === null || isPureInitializer(declarator.init);
	}
	...
	/**
	 * Returns the named/default `const` exports that are pure and self-contained.
	 * @param {string} source module source
	 * @param {EXPECTED_ANY[]} body parsed program body
	 * @returns {Candidate[]} candidates
	 */
	findSplittableExports(source, body) {
		...
		this.collectHoistedVars(body, moduleScopeNames);
