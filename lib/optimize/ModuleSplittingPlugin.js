...
/** @typedef {{ specifiers: Map<string, Dependency[]>, namespaceConsumers: Dependency[], reexportedBy: Set<Module>, opaque: boolean }} IncomingInfo */
...
class ModuleSplittingPlugin {
	...
	rawCandidate(name, localName, prefix, range, init) {
		// Every referenced name counts, shadowed or not: subtracting nested
		// bindings would hide a module binding a same-named inner one shadows.
	...
	splitModules(compilation, modules) {
		...
		const cache = new Map();
		/** @type {Map<string, SplitExportModule>} */
		const parts = new Map();
		for (const host of modules) {
			...
			if (info.reexportedBy.size > 0) continue;
			// Consumed in a way this analysis cannot follow — leave it alone.
	...
	analyze(module, cache) {
		...
		if (this.isStrictEsm(module) && info) {
			...
			if (original) {
				...
				const rewritten = this.collectRewrittenRanges(module);
				for (const candidate of info.candidates) {
					if (candidate.referenced.some((ref) => scope.has(ref))) continue;
					// The declaration is copied verbatim, so anything a dependency
					// template would rewrite (DefinePlugin, `import.meta`, …) must not
					// overlap it.
					if (
						rewritten.some(
							([start, end]) => start < candidate.end && end > candidate.start
						)
					) {
						continue;
					}
	...
	/**
	 * Source ranges a dependency template rewrites during code generation.
	 * @param {Module} module module
	 * @returns {Range[]} ranges
	 */
	collectRewrittenRanges(module) {
		/** @type {Range[]} */
		const ranges = [];
		/**
		 ...
		 */
		const add = (dep) => {
			// Only ever wraps its range (and does nothing while the export is used),
			// so the declaration text itself survives verbatim.
			if (dep instanceof PureExpressionDependency) return;
			const { range, rangeStatement, declarationId } =
				/** @type {{ range?: Range, rangeStatement?: Range, declarationId?: { range?: Range } }} */ (
					dep
				);
			// An export header rewrites only the `export …` prefix ahead of the
			// declaration it carries the range of, never the declaration itself.
			if (Array.isArray(rangeStatement) && Array.isArray(range)) {
				ranges.push([rangeStatement[0], range[0]]);
				if (declarationId && Array.isArray(declarationId.range)) {
					ranges.push(declarationId.range);
				}
				return;
			}
			if (Array.isArray(range)) ranges.push(range);
		};
		this.forEachDependency(module, add);
		if (module.presentationalDependencies) {
			for (const dep of module.presentationalDependencies) add(dep);
		}
		return ranges;
	}
	...
	/**
	 * Whether a re-exported origin's binding is reached only through the barrel:
	 * nothing imports it by specifier and no consumer holds it synchronously.
	 * @param {Map<Module, IncomingInfo>} incoming incoming index
	 * @param {{ module: Module, name: string }} origin origin module and name
	 * @param {Module} host the barrel re-exporting the origin
	 * @returns {boolean} true when the part may own the binding exclusively
	 */
	isOriginExclusive(incoming, origin, host) {
		const originInfo = incoming.get(origin.module);
		if (originInfo === undefined) return true;
		if (originInfo.opaque) return false;
		// Only this barrel may re-export it; a second one would get its own copy.
		for (const other of originInfo.reexportedBy) {
			if (other !== host) return false;
		}
		const specifierDeps = originInfo.specifiers.get(origin.name);
		if (specifierDeps !== undefined && specifierDeps.length > 0) return false;
		return originInfo.namespaceConsumers.length === 0;
	}
	 ...
	 * @param {Map<string, SplitExportModule>} parts part modules by identifier
	 * @returns {void}
	 */
	splitSpecifierExports(
		compilation,
		host,
		info,
		candidates,
		...
		for (const candidate of candidates.values()) {
			...
			const part = this.createPart(compilation, host, candidate, parts);
	 ...
	 * @returns {void}
	 */
	facadeSplit(compilation, host, info, syncReachable, cache, incoming, parts) {
		const moduleGraph = compilation.moduleGraph;
		...
		for (const exportInfo of exportsInfo.orderedExports) {
			...
			// specifier (avoids rename/identity hazards) and no other module reaches
			// the origin's binding on its own — otherwise the part would recompute a
			// binding someone else already holds. A local export can also redirect
			// its async specifier importers to the part.
			...
			if (
				...
				(local ||
					(specifierDeps.length === 0 &&
						this.isOriginExclusive(incoming, origin, host)))
			) {
	...
	buildIncomingIndex(compilation) {
		...
		for (const module of compilation.modules) {
			this.forEachDependency(module, (dep) => {
				...
				} else if (dep instanceof ImportWeakDependency) {
					// A weak import creates no chunk, so it must keep resolving to the
					// host rather than to a facade no chunk contains.
					entryOf(target).opaque = true;
				} else if (dep instanceof ImportDependency) {
					entryOf(target).namespaceConsumers.push(dep);
				} else if (dep instanceof HarmonyExportImportedSpecifierDependency) {
					const parent = moduleGraph.getParentModule(dep);
					if (parent) entryOf(target).reexportedBy.add(parent);
				} else if (!(dep instanceof HarmonyImportSideEffectDependency)) {
					// Anything else (CommonJS `require`, custom dependencies, …) may
					// consume exports in ways this analysis cannot follow.
					entryOf(target).opaque = true;
				}
			});
		}
	 ...
	 */
	createPart(compilation, host, candidate, parts) {
		const moduleGraph = compilation.moduleGraph;
		...
		// One module per identifier: a second instance would duplicate the binding
		// and defeat the compilation's identifier-based deduplication.
		const existing = parts.get(part.identifier());
		if (existing !== undefined) return existing;
		parts.set(part.identifier(), part);
		part.build(
			compilation.options,
			compilation,
			/** @type {ResolverWithOptions} */ (/** @type {unknown} */ (null)),
