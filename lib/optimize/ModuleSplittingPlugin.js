...
class ModuleSplittingPlugin {
	...
	splitModules(compilation, modules) {
		...
		for (const host of modules) {
			...
			for (const candidate of candidates) {
				const specifierDeps = this.findImporterSpecifiers(
					compilation,
					host,
					candidate.name
				);
				...
				if (specifierDeps.length === 0) continue;
	...
	findSplittableExports(source) {
		...
		for (const node of body) {
			...
			collectReferenced(declarator.init, referenced);
			const inner = new Set();
			collectInnerBindings(declarator.init, inner);
			let selfContained = true;
			for (const ref of referenced) {
				if (!inner.has(ref) && moduleScopeNames.has(ref)) {
					selfContained = false;
					break;
				}
			}
	...
	/**
	 * Finds `import { name }` specifier dependencies across all modules that
	 * resolve to `host` and reference `name`.
	 * @param {Compilation} compilation compilation
	 * @param {Module} host host module
	 * @param {string} name export name
	 * @returns {import("../Dependency")[]} matching specifier dependencies
	 */
	findImporterSpecifiers(compilation, host, name) {
		const moduleGraph = compilation.moduleGraph;
		const result = [];
		for (const module of compilation.modules) {
			for (const dep of module.dependencies) {
				if (dep.constructor.name !== "HarmonyImportSpecifierDependency") {
					continue;
				}
				const ids = /** @type {{ ids?: string[] }} */ (dep).ids;
				if (!Array.isArray(ids) || ids.length === 0 || ids[0] !== name) {
					continue;
				}
				const connection = moduleGraph.getConnection(dep);
				if (connection && connection.module === host) result.push(dep);
			}
		}
		return result;
	}
