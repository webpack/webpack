...
class ModuleSplittingPlugin {
	...
	tapParser(parser) {
		...
		parser.hooks.program.tap(PLUGIN_NAME, (ast) => {
			...
			const body = /** @type {EXPECTED_ANY[]} */ (
				/** @type {EXPECTED_ANY} */ (ast).body
			);
			...
			for (const node of body) {
				if (
					...
					if (
						...
						d.init &&
						parser.isPure(d.init, d.init.range[0])
					) {
						info.candidates.push(
							this.rawCandidate(d.id.name, d.id.name, "const ", d.range, d.init)
						);
					}
			...
			/** @type {EXPECTED_ANY} */ (module.buildInfo).moduleSplitting = info;
		});
	}
	 ...
	 * @param {[number, number]} range declaration value range
	 * @param {EXPECTED_ANY} init initializer node (for self-containment)
	...
	collectModuleScope(body) {
		...
		this.collectHoistedVars(body, names);
		return names;
	}
		...
		for (const node of body) {
			switch (node.type) {
				...
				case "ExportAllDeclaration":
					break;
				case "VariableDeclaration":
					if (!parser.isPure(node, node.range[0])) return false;
					break;
				case "ExportNamedDeclaration":
					if (
						node.declaration &&
						!parser.isPure(node.declaration, node.declaration.range[0])
					) {
						return false;
					}
					break;
	...
	analyze(module, cache) {
		...
		const info =
			module.buildInfo &&
			/** @type {{ moduleSplitting?: ModuleSplittingInfo }} */ (
				module.buildInfo
			).moduleSplitting;
