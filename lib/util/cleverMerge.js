 ...
 * @typedef {Map<string, unknown>} ByValues
 */
...
/** @typedef {{ [p: string]: { [p: string]: unknown } } | DynamicFunction} ByObject */
	...
	switch (getValueType(secondEntry.base)) {
		...
		case VALUE_TYPE_UNDEFINED:
			if (!firstEntry.byProperty) {
				...
				const newByValues =
					/** @type {Map<string, T & O>} */
					(/** @type {unknown} */ (new Map(firstEntry.byValues)));
...
const getFromByValues = (byValues, key) => {
	if (key !== "default" && byValues.has(key)) {
		return /** @type {V | undefined} */ (byValues.get(key));
	}
	return /** @type {V | undefined} */ (byValues.get("default"));
};
