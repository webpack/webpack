...
const getValueType = value => {
	if (value === undefined) {
		...
		if (value.some(item => item === "...")) return VALUE_TYPE_ARRAY_EXTEND;
...
const cleverMerge = (first, second) => {
	...
	const firstInfo = parseObject(first);
	const secondInfo = parseObject(second);
	...
	for (const [key, firstEntry] of firstInfo) {
		...
		const entry =
			secondEntry !== undefined
				? mergeEntries(firstEntry, secondEntry)
				: firstEntry;
 ...
 */
const mergeEntries = (firstEntry, secondEntry) => {
	switch (getValueType(secondEntry.base)) {
		...
		case VALUE_TYPE_UNDEFINED:
			if (!firstEntry.byProperty) {
				...
				for (const [key, value] of secondEntry.byValues) {
					...
					newByValues.set(key, mergeSingleValue(firstValue, value));
				}
				return {
					base: firstEntry.base,
		...
		default: {
			if (!firstEntry.byProperty) {
				...
				return {
					base: mergeSingleValue(firstEntry.base, secondEntry.base),
					byProperty: secondEntry.byProperty,
					byValues: secondEntry.byValues
				};
			}
			...
			for (const [key, value] of intermediateByValues) {
				intermediateByValues.set(
					key,
					mergeSingleValue(value, secondEntry.base)
				);
			}
			if (
				...
				newBase = mergeSingleValue(firstEntry.base, secondEntry.base);
			} else {
...
const mergeSingleValue = (a, b) => {
	...
	switch (bType) {
		...
		case VALUE_TYPE_OBJECT: {
			return aType === VALUE_TYPE_OBJECT ? cleverMerge(a, b) : b;
		}
		case VALUE_TYPE_UNDEFINED:
			return a;
