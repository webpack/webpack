...
const getOnlyHandlerKey = (runtimeModule, handlers) => {
	...
	for (const module of chunkGraph.getChunkRuntimeModulesIterable(chunk)) {
		// TODO in the next major release: call it directly, every runtime module answers by then. A
		// module built against an older webpack extends a base class without it.
		const installed =
			typeof module.getInstalledChunkHandlers === "function"
				? module.getInstalledChunkHandlers()
				: null;
