 ...
 * print: each name is what the options set it to, and on where they set
 * nothing — so no read of the result asks whether the option was given. A
 * switch with more than the two states keeps whatever it was given, which is
 * still falsy only when it is `false`. Where nothing is set, `all` itself comes
 * back — one frozen object shared by every print that names none.
 * @template {object} T
