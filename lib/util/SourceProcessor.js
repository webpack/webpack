 ...
 */
const deferredWrite = (id) => `${DEFERRED_MARKER}${id}${DEFERRED_MARKER}`;

const DEFERRED_ID_RE = /^\d+$/;

/**
 * Read the deferred write standing at `start`, if one does: `NUL id NUL` whose
 * id the caller still answers for. A NUL the source itself carried — an RCDATA
 * element (`<textarea>`, `<title>`) and an attribute value both keep the one
 * they were written with rather than turning it into U+FFFD — pairs with the
 * next NUL and spells no id, so nothing is read there.
 ...
 * @returns {{ end: number, replacement: string } | null} where the write ends and what stands in for it, or null where none stands there
 */
const readDeferredWrite = (text, start, resolve) => {
	...
	if (!DEFERRED_ID_RE.test(body)) return null;

	const replacement = resolve(Number(body));

	return replacement === undefined ? null : { end, replacement };
};

/**
 * Stand the answers in for every deferred write in one replacement. A body
 * offered whole holds the markers of the bodies inside it, and its answer
 * carries them through, so what comes back is resolved again until nothing is
 * left standing in.
 * @param {string} text one answer
 * @param {(id: number) => string | undefined} resolve the final text for a deferred write
 * @returns {string} the answer with every write inside it resolved
 */
const expandDeferredWrites = (text, resolve) => {
	...
	while (at !== -1) {
		const write = readDeferredWrite(text, at, resolve);
		...
		out +=
			text.slice(from, at) + expandDeferredWrites(write.replacement, resolve);
		from = write.end + 1;
...
class PrintContext {
	...
	substitute(resolve) {
		...
		for (let i = 0; i <= chunks.length; i++) {
			...
			const shifts = [];
			while (start !== -1) {
				const write = readDeferredWrite(text, start, resolve);
				...
				const { end } = write;
				const replacement = expandDeferredWrites(write.replacement, resolve);
