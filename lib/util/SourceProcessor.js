...
// How many times `processAsync` prints again: an answer can make a print offer
// something new (a `<style>` run joined after a sheet the input left open, once
// that sheet is answered), not forever.
...
class SourceProcessor {
	...
	async processAsync(input, options) {
		...
		// again with every answer in hand. Nothing is asked twice: what a print
		// offers that no earlier one did is asked for, and the print runs again.
		...
		for (let round = 1; ; round++) {
			/** @type {Map<string, Omit<DeferredWrite, "build">>} */
			const unasked = new Map();
			/**
			 * @param {Omit<DeferredWrite, "build">} offer an offer, `build` and all or without it
			 * @returns {string | undefined} what an earlier print was answered for it
			 */
			const embeddedAnswers = (offer) => {
				const key = deferredKey(offer);
				if (!known.has(key) && !unasked.has(key)) unasked.set(key, offer);
			...
			for (const hole of again) embeddedAnswers(hole);
			// Bounded: an offer left unasked in the last round prints as written.
			if (unasked.size === 0 || round === MAX_REPRINTS) {
				return reprint.finish((id) => {
					const hole = again[id];

					return hole === undefined
						? undefined
						: hole.build(known.get(deferredKey(hole)));
				});
			}
			const offers = [...unasked.values()];
			const more = await Promise.all(
				offers.map((offer) => render(offer.source, offer))
			);
			for (let id = 0; id < offers.length; id++) {
				known.set(deferredKey(offers[id]), more[id]);
			}
		}
	}
