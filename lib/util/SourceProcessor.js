...
class PrintContext {
	 ...
	 * its own account.
	 * @param {(id: number) => string} resolve the final text for a deferred write
	 * @returns {void}
	 */
	substitute(resolve) {
		const chunks = this._chunks;
		...
		for (let i = 0; i <= chunks.length; i++) {
			...
			while (start !== -1) {
				...
				// A body offered whole holds the markers of the bodies inside it, and
				// its answer carries them through, so what comes back is resolved
				// again until nothing is left standing in.
				let replacement = resolve(Number(text.slice(start + 1, end)));

				while (replacement.includes(DEFERRED_MARKER)) {
					let expanded = "";
					let from = 0;
					let at = replacement.indexOf(DEFERRED_MARKER);

					while (at !== -1) {
						const to = replacement.indexOf(DEFERRED_MARKER, at + 1);

						expanded +=
							replacement.slice(from, at) +
							resolve(Number(replacement.slice(at + 1, to)));
						from = to + 1;
						at = replacement.indexOf(DEFERRED_MARKER, from);
					}

					replacement = expanded + replacement.slice(from);
				}
...
class SourceProcessor {
	...
	async processAsync(input, options) {
		...
		return finish((id) => deferred[id].build(answers[id]));
	}
	 ...
	 * @returns {{ finish: (resolve: (id: number) => string) => { code: string, map: SourceMap | undefined } }} the deferred print
	 */
