 ...
 * @typedef {{ mode: "minify" | "beautify", environment?: Readonly<Record<string, boolean>>, convertLengthUnits?: boolean, collapseWhitespace?: boolean, renderEmbeddedSource?: EmbeddedSourceRenderer }} PrintOptions
 */
/**
 * Renders source a serializer found embedded in its own output — an inline
 * `<style>` or `<script>`, and whatever the next language nests. `info` is the
 * print-time counterpart of `Compilation`'s `EmbeddedSourceInfo`, minus the
 * `module` a serializer has no notion of; it is spelled inline so that name
 * stays the compilation hook's. Returning the input unchanged declines it.
 * Synchronous: this is called from inside the print walk, so a caller with an
...
class SourceProcessor {
	...
	process(input, options) {
		...
		const printOpts =
			/** @type {{ mode?: PrintOptions["mode"], environment?: Readonly<Record<string, boolean>>, convertLengthUnits?: boolean, collapseWhitespace?: boolean, renderEmbeddedSource?: EmbeddedSourceRenderer }} */ (
				opts
			);
		...
		const ctx = new PrintContext(
			{
				...
				renderEmbeddedSource: printOpts.renderEmbeddedSource
			},
