...
/**
 * The switches a minifying print is told to make or skip, resolved once per
 * print: each name is what the options set it to, and what `all` gives it where
 * they set nothing — so no read of the result asks whether the option was
 * given. A switch with more than the two states keeps whatever it was given,
 * and `all` carries the state it stands at unasked.
 *
 * Always a copy, and always made the one way: spreading `all` and then writing
 * over a name it already has leaves the object's shape alone, so every read of
 * the result — and there is one per token — sees a single hidden class. Handing
 * `all` itself back where nothing is set would save this object and cost every
 * one of those reads a second shape, `Object.freeze` giving a frozen object a
 * map of its own.
 * @template {object} T
 * @param {T} all every switch on, frozen and shared
 * @param {Partial<T> | undefined} options what the caller set
 * @returns {T} every switch resolved
 */
const resolveSwitches = (all, options) => {
	/** @type {EXPECTED_ANY} */
	const out = { ...all };
...
/**
 * The switches out of a wider options object — the flat shape a config names
 * them in, beside options that are not switches at all. `undefined` where none
 * of them is set, which is what lets `resolveSwitches` hand back its shared
 * object.
 * @template {object} T
 * @param {T} all every switch, which is also the set of names to pick
 * @param {object} options an options object that may carry them
 * @returns {Partial<T> | undefined} the ones set, or undefined when none is
 */
const pickSwitches = (all, options) => {
	/** @type {EXPECTED_ANY} */
	let out;
	for (const name of Object.keys(all)) {
		const value = /** @type {EXPECTED_ANY} */ (options)[name];
...
/**
 * A `comments` option resolved to what it means per comment: `true` every one,
 * `false` none, `"some"` the ones the language says carry something, or a
 * predicate over the comment's own text — which is what a pattern compiles to
 * here, so no print re-reads one per comment. A pattern is matched from the
 * start each time, since a `g` flag would otherwise carry an index between
 * comments.
 * @param {boolean | "all" | "some" | string | RegExp | ((comment: string) => boolean)} comments the option
...
class SourceProcessor {
	 ...
	 * @overload
	 * @param {string} input
	 * @param {TProcessOptions & { mode: PrintOptions["mode"] } & { source: string, content?: string }} options
	 * @returns {{ code: string, map: SourceMap }}
	 */
	/**
	 * @overload
	 * @param {string} input
	 * @param {TProcessOptions & { mode: PrintOptions["mode"] }} options
	 * @returns {{ code: string, map: undefined }}
	 */
	/**
	 * @overload
	 * @param {string} input
...
SourceProcessor.resolveSwitches = resolveSwitches;
SourceProcessor.pickSwitches = pickSwitches;
SourceProcessor.keptComments = keptComments;
