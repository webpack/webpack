 ...
 * and `all` carries the state it stands at unasked. Where nothing is set, `all`
 * itself comes back — one frozen object shared by every print that names none.
 * @template {object} T
...
const resolveSwitches = (all, options) => {
	...
	for (const name of Object.keys(all)) {
		...
		out[name] =
			value === undefined ? /** @type {EXPECTED_ANY} */ (all)[name] : value;
	}
	return out;
};
