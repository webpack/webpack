 ...
 * @typedef {{ mode: "minify" | "beautify", environment?: Readonly<Record<string, boolean>>, convertLengthUnits?: boolean, collapseWhitespace?: boolean | "conservative" | "smart" | "all", removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: "smart" | "all", sortAttributes?: boolean, sortClassNames?: boolean }} PrintOptions
 */
/**
...
class SourceProcessor {
	 ...
	 * @param {NodePrinter<TPath, TNode>=} printer the node printer, fired per node once its visitors and children are done; the same `path` a visitor gets plus the print context as its writer. Required to print (e.g. `mode`); a future API can let a developer supply their own
	 */
	...
	process(input, options) {
		...
		const printOpts =
			/** @type {{ mode?: PrintOptions["mode"], environment?: Readonly<Record<string, boolean>>, convertLengthUnits?: boolean, collapseWhitespace?: boolean | "conservative" | "smart" | "all", removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: "smart" | "all", sortAttributes?: boolean, sortClassNames?: boolean }} */ (
				opts
			);
		const asked = printOpts.mode;
		...
		const ctx = new PrintContext(
			{
				...
				removeEmptyAttributes: printOpts.removeEmptyAttributes,
				removeEmptyElements: printOpts.removeEmptyElements,
				preserveComments: printOpts.preserveComments,
				removeRedundantAttributes: printOpts.removeRedundantAttributes,
				sortAttributes: printOpts.sortAttributes,
				sortClassNames: printOpts.sortClassNames
			},
