...
import {
	...
	ReplaceSource,
	SourceMapSource
} from "./webpack-sources.js";

// acorn declares these as types only, but they are runtime classes
const { Position, SourceLocation } = /** @type {EXPECTED_ANY} */ (acorn);

const { register } = serialization;
/** @typedef {import("acorn").Position} Position */
/** @typedef {import("../Dependency.js").RealDependencyLocation} RealDependencyLocation */
/** @typedef {import("../Dependency.js").SourcePosition} SourcePosition */
...
register(
	...
	new (class SourceLocationSerializer {
		 ...
		 * @param {import("../serialization/ObjectMiddleware.js").ObjectSerializerContext<number[]>} context context
		 * @returns {void}
		 */
		serialize(loc, { write }) {
		...
		/**
		 * Restores this instance from the provided deserializer context.
		 * @param {import("../serialization/ObjectMiddleware.js").ObjectDeserializerContext<number[]>} context context
	...
	new (class SourceMapSourceSerializer {
		...
		deserialize({ read }) {
			return new SourceMapSource(
				.../** @type {ConstructorParameters<typeof SourceMapSource>} */ (read())
			);
		}
	})()
);
