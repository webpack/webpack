...
/** @typedef {import("./lazyModule").LazyModuleSyncAccessor<EXPECTED_ANY>} AnyLazyModuleSyncAccessor */
 ...
 * @param {Iterable<[string, AnyLazyModuleSyncAccessor[]]>} table module type to the accessors its taps read
 * @returns {void}
 */
	...
	for (const [type, accessors] of table) {
		normalModuleFactory.hooks.prepareModuleType
			...
			.tapPromise(pluginName, () =>
				Promise.all(accessors.map((accessor) => accessor.preload())).then(
					() => undefined
				)
			);
	}
