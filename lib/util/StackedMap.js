...
class StackedMap {
	...
	/**
	 * Looks up a key through the stack, caching the result in the current layer.
	 * @param {K} item the key to look up
	 * @returns {InternalCell<V> | undefined} the internal value or undefined if not found
	 */
	_lookup(item) {
		const map = this.map;
		const topValue = map.get(item);
		if (topValue !== undefined) return topValue;
		const stack = this.stack;
		for (let i = stack.length - 2; i >= 0; i--) {
			const value = stack[i].get(item);
			if (value !== undefined) {
				map.set(item, value);
				return value;
			}
		}
		if (stack.length > 1) map.set(item, TOMBSTONE);
		return undefined;
	}
	 ...
	 */
	has(item) {
		const value = this._lookup(item);
		return value !== undefined && value !== TOMBSTONE;
	}
