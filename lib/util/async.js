 ...
 * Every item is started from one flat loop and they share a single completion
 * callback, so a synchronous iterator neither grows the stack nor allocates
 * per item.
 * @template T
 * @template E
...
const each = (collection, iterator, callback) => {
	...
	if (Array.isArray(collection)) {
		...
		for (let i = 0; i < length; i++) iterator(array[i], done);
		return;
	}
...
const map = (collection, iterator, callback) => {
	...
	/** @type {R[]} */
	const results = [];
	// Sized up front rather than grown, so a result never lands past the end
	// while earlier items are still running.
	if (array !== undefined) results.length = array.length;
	let remaining = 1;
	let finished = false;
	...
	const createCallback = (index) => {
		...
		return (err, result) => {
			const at = position;
			// Guards against an iterator that calls back twice, which would
			// otherwise settle the whole run early.
			if (at === -1) return;
			position = -1;
