...
/**
 * Runs `iterator` over every item, at most `limit` of them in flight, and
 * collects the results in the order of the collection, not of completion.
 * @template T
 * @template R
 * @template E
 * @param {Iterable<T>} collection items to iterate
 ...
 * @returns {void}
 */
const mapLimit = (collection, limit, iterator, callback) => {
	/** @type {R[]} */
	const results = [];
	let index = 0;
	eachLimit(
		...
		limit,
		(item, done) => {
			// the dispatch loop hands out one item at a time, so this counts the
			// position of the item rather than of whichever finishes first
			const position = index++;
			iterator(item, (err, result) => {
				if (err) return done(err);
				results[position] = /** @type {R} */ (result);
				done(null);
			});
		},
		(err) => {
