...
const compareChunkGroupsByIndex = (a, b) => {
	const ai = /** @type {number} */ (a.index);
	const bi = /** @type {number} */ (b.index);
	if (ai < bi) return -1;
	if (ai > bi) return 1;
	return 0;
};
...
const keepOriginalOrder = (iterable) => {
	...
	return (a, b) => {
		const ai = /** @type {number} */ (map.get(a));
		const bi = /** @type {number} */ (map.get(b));
		if (ai < bi) return -1;
		if (ai > bi) return 1;
		return 0;
	};
...
const sortWithSourceOrder = (
	...
	if (withSourceOrder.length <= 1) return;
	...
	for (let i = 0; i < positions.length; i++) {
		...
		const dep = withSourceOrder[i].dep;
		dependencies[depIndex] = dep;
		if (onDependencyReSort) onDependencyReSort(dep, depIndex);
	}
};
