...
/** @import ESMImportSideEffectDependency from "../dependencies/esm/ESMImportSideEffectDependency" */
/** @import ESMImportSpecifierDependency from "../dependencies/esm/ESMImportSpecifierDependency" */
...
// `compilation.errors`/`warnings`/`hints` are typed as plain `Error`s, so the
// webpack-specific fields are read through a cast.
const compareErrors = concatComparators(
	compareSelect(
		/**
		 * @param {Error} err error
		 * @returns {string} identifier of the module the error belongs to
		 */
		(err) => {
			const { module } = /** @type {WebpackErrorType} */ (err);
			return (module && module.identifier()) || "";
		},
	...
	compareSelect(
		/**
		 * @param {Error} err error
		 * @returns {DependencyLocation | undefined} location
		 */
		(err) => /** @type {WebpackErrorType} */ (err).loc,
		compareLocations
	),
	compareSelect(
		/**
...
/**
 * For ESMImportSideEffectDependency and ESMImportSpecifierDependency, we should prioritize import order to match the behavior of running modules directly in a JS engine without a bundler.
