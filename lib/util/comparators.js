...
import { getFullModuleName } from "../ids/IdHelpers.js";
import { compareRuntime } from "./runtime.js";
...
// Kept for plugin API compatibility — stable `Array#sort` makes it unnecessary internally
/**
 * Keep original order.
 * @template T
...
const compareChunksNatural = (chunkGraph) => {
	const cmpFn = cachedCompareModulesById(chunkGraph);
...
export { compareChunkGroupsByIndex };

/** @type {ParameterizedComparator<ChunkGraph, Chunk>} */
const cachedCompareChunks = createCachedParameterizedComparator(compareChunks);

export { cachedCompareChunks as compareChunks };
/**
 * Returns compare result.
 * @param {Chunk} a chunk
 * @param {Chunk} b chunk
 * @returns {-1 | 0 | 1} compare result
 */
export const compareChunksById = (a, b) =>
	compareIds(/** @type {ChunkId} */ (a.id), /** @type {ChunkId} */ (b.id));
export { compareChunksNatural };

export { compareIds };

export { compareIterables };

export { compareLocations };

/** @type {ParameterizedComparator<Compiler, Module>} */
const cachedCompareModulesByFullName = createCachedParameterizedComparator(
	compareModulesByFullName
);

export { cachedCompareModulesByFullName as compareModulesByFullName };

/** @type {ParameterizedComparator<ChunkGraph, Module>} */
const cachedCompareModulesById =
	createCachedParameterizedComparator(compareModulesById);

export { cachedCompareModulesById as compareModulesById };

/** @type {ParameterizedComparator<ChunkGraph, Module>} */
const cachedCompareModulesByIdOrIdentifier =
	createCachedParameterizedComparator(compareModulesByIdOrIdentifier);

export { cachedCompareModulesByIdOrIdentifier as compareModulesByIdOrIdentifier };
/**
 * Returns compare result.
 * @param {Module} a module
 * @param {Module} b module
 * @returns {-1 | 0 | 1} compare result
 */
export const compareModulesByIdentifier = (a, b) =>
	compareIds(a.identifier(), b.identifier());

/** @type {ParameterizedComparator<ModuleGraph, Module>} */
const cachedCompareModulesByPostOrderIndexOrIdentifier =
	createCachedParameterizedComparator(
		compareModulesByPostOrderIndexOrIdentifier
	);

export { cachedCompareModulesByPostOrderIndexOrIdentifier as compareModulesByPostOrderIndexOrIdentifier };

/** @type {ParameterizedComparator<ModuleGraph, Module>} */
const cachedCompareModulesByPreOrderIndexOrIdentifier =
	createCachedParameterizedComparator(
		compareModulesByPreOrderIndexOrIdentifier
	);

export { cachedCompareModulesByPreOrderIndexOrIdentifier as compareModulesByPreOrderIndexOrIdentifier };

export { compareNumbers };
export { compareSelect };
export { compareStrings };
export { compareStringsNumeric };

export { concatComparators };

export { keepOriginalOrder };
export { sortWithSourceOrder };
