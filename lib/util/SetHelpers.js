 ...
 * @property {number} expansions the weighted number of downstream item visits
	...
	search: for (const { set: intersection } of orderedIntersections) {
		...
		for (const key of possibleOriginalKeys) {
			...
			if (isSubset(original, intersection)) {
				// Each module occurrence can visit every selected chunk, including
				// calls through a configured chunks filter.
