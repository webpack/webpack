 ...
 */
const associateIntersections = (originalsByKey, intersections, options) => {
	const { maximumChecks, maximumComparisons } = options;
	/** @type {Map<T, bigint[]>} */
	...
	search: for (const { set: intersection } of orderedIntersections) {
		...
		for (const key of possibleOriginalKeys) {
			...
			if (isSubset(original, intersection)) matches.push(key);
		}
	...
	return { intersectionsByOriginalKey, checks, comparisons, limited };
};
