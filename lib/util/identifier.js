...
/**
 * Find the index of the `#` fragment separator in a resource string.
 *
 * When the string looks like an absolute filesystem path, a `#` followed by
 * a path separator (`/` or `\`) is treated as part of the path rather than a
 * fragment marker. This allows paths with `#` in directory names (e.g.
 * `/home/user/f#/webpack/file.js`) to be parsed correctly.
 * See https://github.com/webpack/webpack/issues/16819
 * @param {string} str input string
 * @param {number} queryStart index of the first `?`, or -1 if none
 * @returns {number} fragment start index, or -1 if there is no fragment
 */
const findFragmentStart = (str, queryStart) => {
	...
	// Only apply the special path handling within the path portion
	// (before the query, if any). A `#` after `?` is always a fragment.
	const pathEnd = queryStart < 0 ? str.length : queryStart;
	let searchFrom = 0;
	while (searchFrom < pathEnd) {
		const idx = str.indexOf("#", searchFrom);
		if (idx === -1 || idx >= pathEnd) break;
		const next = str.charCodeAt(idx + 1);
		// `#` followed by a path separator is part of the path
		if (next === 47 /* "/" */ || next === 92 /* "\\" */) {
			searchFrom = idx + 1;
			continue;
		}
	...
	return queryStart < 0 ? -1 : str.indexOf("#", queryStart);
};
...
const _parseResource = (str) => {
	...
	const fragmentStart = findFragmentStart(str, queryStart);
