	...
	if (WINDOWS_ABS_PATH_REGEXP.test(maybeAbsolutePath)) {
		...
		if (!WINDOWS_ABS_PATH_REGEXP.test(resource)) {
			resource = relativePathToRequest(
				resource.replaceAll(WINDOWS_PATH_SEPARATOR_REGEXP, "/")
			);
		}
		return querySplitPos === -1
			? resource
...
const _parseResource = (str) => {
	...
	if (firstEscape !== -1) {
		...
		return {
			resource: str,
			path: match[1].replaceAll(ZERO_ESCAPE_REGEXP, "$1"),
			query: match[2] ? match[2].replaceAll(ZERO_ESCAPE_REGEXP, "$1") : "",
			fragment: match[3] || ""
		};
	}
...
const escapeHashInPathRequest = (request) => {
	...
	return pathPart.replaceAll(HASH_REGEXP, "\0#") + request.slice(lastSep);
};

export { ABSOLUTE_PATH_REGEXP };
export { WINDOWS_ABS_PATH_REGEXP };
export { absolutify };
export { contextify };
export { escapeHashInPathRequest };
export { getUndoPath };
