...
/**
 *
 * @param {OutputOptions} outputOptions Checked to see if public path is set
 * @param {string} filename potentially passed to getUndoPath
 * @param {string} outputPath potentially  passed to getUndoPath
 * @param {boolean} enforceRelative potentially passed to getUndoPath
 * @returns {string} If set, the publicPath is returned. Otherwise, repeated ../ to leave the directory of the provided filename to be back on output dir
 * @see getUndoPath
 */
exports.getPublicOrUndoPath = (outputOptions, filename, outputPath, enforceRelative) => {
	const publicPath = outputOptions.publicPath;
	const publicPathType = typeof publicPath;
	if (publicPathType === 'string') {
		return /** @type {string} */ (publicPath);
	}
	if (publicPathType === 'function') {
		throw new Error('Dynamic publicPath not implemented in this context yet.')
	}
	return exports.getUndoPath(filename, outputPath, enforceRelative);
};
