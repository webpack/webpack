...
exports.contextify = (context, request) => {
	return request
		...
		.map(r => {
			if (
				!request.startsWith("./node_modules") &&
				!request.startsWith("node_modules") &&
				!request.startsWith("/node_modules") &&
				request.indexOf("node_modules") >= 0 &&
				fs.lstatSync(path.join(context, "node_modules")).isSymbolicLink()
			) {
				const allNodeModules = request.split("node_modules");
				allNodeModules.shift(); // remove whatever comes before the first node_modules.
				return path.join(
					context,
					"node_modules",
					allNodeModules.join("node_modules") // everything after node_modules
				);
			}
