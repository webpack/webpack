...
class TupleQueue {
	 ...
	 * @param {Iterable<[T, V, ...any]>=} items The initial elements.
	 */
	constructor(items) {
		/**
		 * @private
	 ...
	 * @param {[T, V, ...any]} item The element to add.
	 * @returns {void}
	 */
	enqueue(...item) {
		this._set.add(...item);
	 ...
	 * @returns {[T, V, ...any] | undefined} The head of the queue of `undefined` if this queue is empty.
	 */
	dequeue() {
		...
		this._set.delete(.../** @type {[T, V, ...any]} */ (result.value));
		return result.value;
	}
}
