...
function trimIdsToThoseImported(ids, moduleGraph, dependency) {
	...
	for (let i = 0; i < ids.length; i++) {
		...
		if (exportInfo.provided === false) {
			...
			// Never trim to nothing (i === 0 for invalid imports), keep the full chain.
			return i === 0 ? ids : ids.slice(0, i);
		}
		const nestedInfo = exportInfo.getNestedExportsInfo();
		if (!nestedInfo) {
			// once all nested exports are traversed, the next item is the actual import so stop there
			return i + 1 === ids.length ? ids : ids.slice(0, i + 1);
		}
		currentExportsInfo = nestedInfo;
	}
	// All ids are imports (e.g. namespace chains) — nothing to trim.
	return ids;
}
