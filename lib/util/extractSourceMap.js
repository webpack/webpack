/*
    ...
    Migrated from https://github.com/webpack-contrib/source-map-loader
*/
...
function getSourceMappingURL(code) {
	...
	const lastLine = code.slice(Math.max(0, lastLineIndex + 1));
...
function fetchFromDataURL(fs, sourceURL) {
	const dataURL = dataURLRegex.exec(sourceURL);

	if (dataURL) {
		const encodingName = dataURL[1] ? "base64" : "ascii";
		return Buffer.from(dataURL[2], encodingName).toString("utf8");
	}
...
/**
 * @param {ExtractSourceMapOptions} options the options for the source map extractor
 * @returns {SourceMapExtractorFunction} the source map extractor function
 */
function extractSourceMap(options) {
	const filterSourceMappingUrl =
		typeof options === "object" ? options.filterSourceMappingUrl : undefined;

	/** @type {SourceMapExtractorFunction} */
	return async (stringOrBuffer, resourcePath, fs) => {
		const input =
		...
		return {
			source: input.replace(replacementString, ""),
			sourceMap: /** @type {EXPECTED_ANY} */ (newMap),
