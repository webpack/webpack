...
/** @typedef {import("../NormalModule").ReadResource} ReadResource */
/**
 ...
 * @param {ReadResource} readResourceFromURL The read resource function
...
async function extractSourceMap(
	...
	/** @type {ReadResource} */
	const readResource = (resourcePath) =>
		readResourceFromURL(resourcePath).catch((err) => {
			throw new Error(
	...
	return {
		...
		fileDependencies: output.fileDependencies.filter(
			(dep) => !/^(\/\/|https?:\/\/|#)/.test(dep)
		)
	};
}
