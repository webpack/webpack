...
/**
 * Converts a file `URL` instance to an absolute path (matching how
 * enhanced-resolve handles `URL` options). Any other value (plain path
 * string, `false`, `RegExp`, ...) is returned unchanged.
 * @template T
 * @param {T} value a path or a file `URL` instance
 * @returns {Exclude<T, URL> | string} an absolute path for file URLs, otherwise `value`
 */
const maybeFileURLToPath = (value) =>
	value instanceof URL
		? fileURLToPath(value)
		: /** @type {Exclude<T, URL> | string} */ (value);
