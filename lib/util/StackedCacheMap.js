 ...
 * @template V
 *
 * StackedCacheMap is a data structure that can be used in place of a Map
 * when multiple items have to be added and the largest map is
 * used most of the time.
 *
 * It is optimized for adding multiple items at once.
 * Multiple items can be added at once using `addAll` method.
 *
 * It has a fallback Map that is used when the map to be added is mutable.
 *
 * Note: `delete` and `has` are not supported.
 *
 * @example
 * ```js
