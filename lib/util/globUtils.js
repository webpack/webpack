...
const {
	...
	REPEATED_PATH_SEPARATOR_REGEXP,
...
const normalizePathSeparatorsForPath = (s) =>
	s.replace(WINDOWS_PATH_SEPARATOR_REGEXP, "/");
...
const braceFreeGlobToPathRegExpSource = (glob) => {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		if (segment === "" || segment === ".") {
			// leading separator (absolute) and trailing one (directory); `//` and
			// `/./` collapse
			if (segment === "" && (i === 0 || isLast)) source += "/";
			continue;
		}
		if (segment === "**") {
			source += isLast
		...
		source += "(?=[^/])";
		if (!segment.startsWith(".")) source += NON_DOT_PREFIX;
		// `**` within a segment is a plain `*`
		source += braceFreeGlobToRegExpSource(
 ...
 * follows `path.matchesGlob`, extended globs (`+(a|b)`) aside.
 * @param {string} pattern glob pattern
 * @param {GlobMatchOptions=} options options
	...
	return (str) => {
		...
		return regexp.test(
			normalizedPath.includes("//")
				? normalizedPath.replace(REPEATED_PATH_SEPARATOR_REGEXP, "/")
				: normalizedPath
		);
	};
};
