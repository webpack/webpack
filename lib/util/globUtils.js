...
const globToRegExp = (pattern, caseSensitive) => {
	...
	let regexp = globRegExpCache.get(cacheKey);
	...
	try {
		regexp = new RegExp(`^${source}$`, caseSensitive ? "" : "i");
	} catch (_err) {
		regexp = null;
	}
	// patterns come from source code globs; guard against unbounded growth
	if (globRegExpCache.size > 10000) globRegExpCache.clear();
