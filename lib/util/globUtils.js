...
/**
 * Drops the `.` and empty segments and resolves `..` against the segment
 * before it — the normalization `path.matchesGlob` runs over a pattern and a
 * path alike. The first and last segment stay, so a leading `./`, a leading
...
const optimizeGlobSegments = (parts) => {
	...
	do {
		...
		let parentIndex = 0;
		while ((parentIndex = parts.indexOf("..", parentIndex + 1)) !== -1) {
			const previous = parts[parentIndex - 1];
			// `**/..` is not resolvable, and neither is `../..`
			if (
				previous &&
				previous !== "." &&
				previous !== ".." &&
 ...
 * literal `*`), and a relative pattern matches at any depth. Matching itself
 * follows `path.matchesGlob`.
 * @param {string} pattern glob pattern
 * @param {GlobMatchOptions=} options options
