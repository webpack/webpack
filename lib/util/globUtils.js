...
/**
 * @param {string} patternBytes pattern
 * @param {string} pathBytes path
 * @returns {boolean} matches
 */
const globMatchBytes = (patternBytes, pathBytes) => {
	const p = Buffer.from(patternBytes);
	const s = Buffer.from(pathBytes);
	...
	const match = (pi, si) => {
		...
		if (pi === patternLen) {
			...
			if (pi + 1 < patternLen && p[pi + 1] === 42 /* * */) {
				// **
				const rest = pi + 2;
				if (rest < patternLen && p[rest] === 47 /* / */) {
			...
			result = si < pathLen && s[si] !== 47 /* / */ && match(pi + 1, si + 1);
		} else if (p[pi] === 123 /* { */) {
			let end = pi + 1;
			let depth = 1;
			while (end < patternLen && depth > 0) {
				if (p[end] === 123) depth++;
				else if (p[end] === 125) depth--;
				end++;
			}
			const inner = p.slice(pi + 1, end - 1).toString();
			for (const alt of inner.split(",")) {
				if (
					globMatchBytes(
						`${p.slice(0, pi).toString()}${alt}${p.slice(end).toString()}`,
						pathBytes
					)
				) {
					result = true;
			...
			let end = pi + 1;
			let negated = false;
			if (end < patternLen && p[end] === 33 /* ! */) {
				negated = true;
				end++;
			} else if (end < patternLen && p[end] === 94 /* ^ */) {
				negated = true;
				end++;
			}
			const classStart = end;
			while (end < patternLen && p[end] !== 93 /* ] */) end++;
			const classPattern = p.slice(classStart, end).toString();
			const char = si < pathLen ? String.fromCharCode(s[si]) : "";
			let inClass = false;
...
const globPatternsAreRecursive = (patterns, commonBaseDir) =>
	patterns
		.filter((pattern) => !pattern.negative)
		.some((pattern) => {
			if (pattern.absolutePattern.includes("**")) return true;
			const suffix = pattern.absolutePattern.startsWith(commonBaseDir)
				? pattern.absolutePattern.slice(commonBaseDir.length)
				: pattern.absolutePattern;
