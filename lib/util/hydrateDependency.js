	...
	hydrateDependency: function(serializedDep, module) {
		try {
			serializedDep.options.forEach((opt, index) => {
				...
				} else if(opt && opt.serializedRegExp !== undefined) {
					if(opt.serializedRegExp === null) {
						serializedDep.options[index] = null;
					} else {
						const fragments = opt.serializedRegExp.match(/\/(.*?)\/([gimy])?$/);
						if(!fragments) {
							// debugging, remove me
							console.log("about to fail to hydrate a regex", opt);
						}
