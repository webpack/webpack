...
// A charset other than UTF-8 names bytes this cannot read back as text.
const FOREIGN_CHARSET_REGEXP = /;charset=(?!(?:utf-?8|us-ascii)(?:;|$))/i;

/**
 * Decode a payload's percent-escapes the way the URL parser does: a `%` and two
 * hex digits is the byte they name, and any other `%` is itself.
 * @param {string} payload the payload as written
 * @returns {Buffer} the bytes it names
 */
const percentDecode = (payload) => {
	const input = Buffer.from(payload, "utf8");
	const output = Buffer.allocUnsafe(input.length);
	let length = 0;
	for (let i = 0; i < input.length; i++) {
		const byte = input[i];
		if (byte === 0x25 && i + 2 < input.length) {
			const digits = String.fromCharCode(input[i + 1], input[i + 2]);
			if (/^[0-9a-f]{2}$/i.test(digits)) {
				output[length++] = Number.parseInt(digits, 16);
				i += 2;
				continue;
			}
		}
...
/**
 * The payload as text, or `null` when reading it would not round-trip. A raw
 * `#` is read as content, as webpack's own `data:` modules read it, so a
 * rebuilt URI escapes it rather than cutting the payload short.
...
const decodeDataURIPayload = (parsed) => {
	if (!parsed.base64) {
		...
		if (FOREIGN_CHARSET_REGEXP.test(parsed.parameters)) return null;
		const bytes = percentDecode(payload);
		const text = bytes.toString("utf8");
		// Bytes that are no UTF-8 text are not written back as the same bytes.
 ...
 * `data:` URI, a media type naming no language, a payload that would not
 * round-trip, or a raw `#`, where a browser reads the rest as a fragment.
 * @param {string} uri the URI, unquoted and unescaped
