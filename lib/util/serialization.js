...
/** @typedef {import("../cache/types").CacheWriteReadAbsolutePath} CacheWriteReadAbsolutePath */
 ...
 * @param {CacheWriteReadAbsolutePath} options options
 * @returns {Serializer} serializer instance
 */
exports.createFileSerializer = (
	fs,
	{ writeAbsolutePath, readAbsolutePath, writeAbsolutePaths, readAbsolutePaths }
) => {
	...
	function extendContext(context) {
		if (context.write) {
			...
			context.writeAbsolutePath = value => {
				context.write(writeAbsolutePath(value));
			};
			context.writeAbsolutePaths = value => {
				context.write(writeAbsolutePaths(value));
			};
		} else if (context.read) {
			context.readAbsolutePath = () => {
				return readAbsolutePath(context.read());
			};
			context.readAbsolutePaths = () => {
				return readAbsolutePaths(context.read());
			};
		}
	}
