...
class HotModuleReplacementPlugin {
	...
	apply(compiler) {
		...
		/**
		 * @param {JavascriptParser} parser the parser
		 * @param {typeof ImportMetaHotAcceptDependency} ParamDependency dependency
		 * @returns {(expr: CallExpression) => boolean | undefined} callback
		 */
		const createImportMetaAcceptHandler = (parser, ParamDependency) => {
			const { hotAcceptCallback, hotAcceptWithoutCallback } =
				HotModuleReplacementPlugin.getParserHooks(parser);
		...
		/**
		 * @param {JavascriptParser} parser the parser
		 * @param {typeof ImportMetaHotDeclineDependency} ParamDependency dependency
		 * @returns {(expr: CallExpression) => boolean | undefined} callback
		 */
		const createImportMetaDeclineHandler =
			(parser, ParamDependency) => expr => {
				const module = parser.state.module;
				const dep = new ConstDependency(
		 ...
		 * @returns {(expr: Expression) => boolean | undefined} callback
		 */
		const createImportMetaHMRExpressionHandler = parser => expr => {
			const module = parser.state.module;
			const dep = new ConstDependency(
		...
		const applyImportMetaHot = parser => {
			...
			parser.hooks.call
				...
				.tap(
					PLUGIN_NAME,
					createImportMetaAcceptHandler(parser, ImportMetaHotAcceptDependency)
				);
			parser.hooks.call
				.for("import.meta.webpackHot.decline")
				.tap(
					PLUGIN_NAME,
					createImportMetaDeclineHandler(parser, ImportMetaHotDeclineDependency)
				);
			parser.hooks.expression
				.for("import.meta.webpackHot")
				.tap(PLUGIN_NAME, createImportMetaHMRExpressionHandler(parser));
		};
