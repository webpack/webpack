		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
					...
					source => {
						...
						buf.push(
							mainTemplate.requireFn +
								".h = function() { return state.hotCurrentHash; };"
						);
					...
					(source, chunk, hash) => {
						...
						return Template.asString([
							...
							hotInitCode
								...
								.replace(/\$currentChunk\$/g, JSON.stringify(chunk.id))
								.replace(
				...
				mainTemplate.hooks.currentHash.tap(
					...
					(_, length) => {
						if (isFinite(length)) {
							return `state.hotCurrentHash.substr(0, ${length})`;
						} else {
							return "state.hotCurrentHash";
						}
					}
				);
				...
				mainTemplate.hooks.moduleObj.tap(
					...
					(source, chunk, hash, varModuleId) => {
						return Template.asString([
							...
							"parents: (state.hotCurrentParentsTemp = state.hotCurrentParents, state.hotCurrentParents = [], state.hotCurrentParentsTemp),",
							"children: []"
						]);
					}
				);
