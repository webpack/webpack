			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.hooks.record.tap(PLUGIN_NAME, (compilation, records) => {
					...
					for (const chunk of compilation.chunks) {
						...
						// Also record inner modules of ConcatenatedModules so that
						// HMR can detect their removal (e.g. CSS style modules that
						// inject <style> tags need to be cleaned up)
						for (const m of chunkGraph.getChunkModulesIterable(chunk)) {
							if (m instanceof ConcatenatedModule && m.modules) {
								for (const innerModule of /** @type {ConcatenatedModule} */ (m)
									.modules) {
									const innerId = chunkGraph.getModuleId(innerModule);
									if (
										innerId !== null &&
										!records.chunkModuleIds[chunkId].includes(innerId)
									) {
										records.chunkModuleIds[chunkId].push(innerId);
									}
								}
							}
						}
