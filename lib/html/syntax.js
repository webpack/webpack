 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { cssTransforms?: CssTransformOptions, transforms?: HtmlTransformOptions, collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", xml?: boolean, renderEmbeddedSource?: EmbeddedSourceRenderer, deferEmbeddedSource?: DeferredEmbeddedSource[], deferSrcdoc?: boolean }} HtmlPrintOptions
 */
...
const _minifySvgPresentation = (name, raw) => {
	...
	const minified = _minifyStyleAttribute(key);
	const value = minified.startsWith(`${name}:`)
		? minified.slice(name.length + 1)
		: raw;
	...
	while (stack.length !== 0) {
		...
		if (type === NodeType.Element) {
			...
			if (
				...
				(_absorbedStyles !== null && _absorbedStyles.has(node))
			) {
				continue;
			}
			flushText();
			...
			for (const a of A.attributes(node)) {
				...
				if (REWRITABLE_ATTRIBUTES.has(a.name) || a.name === "srcdoc") {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					if (
						...
						// Read now, not when a deferred answer lands: the print is over by
						// then and the switch has been reset.
