 ...
 * between its attributes to one space. Each attribute is copied byte-for-byte —
 * name, `=` and value with the quotes it was written with — so the tag stays
 * matchable by the quote-assuming regexes asset post-processors run on emitted
 * HTML (webpack's own inline-script plugin is one, and it runs *after*
 * minification). Dropping those quotes would be DOM-safe and is why
 * `removeAttributeQuotes` exists in html-minifier, but it is off by default
...
const _minifyOpenTag = (path, open) => {
	...
	for (let i = 0; i < count; i++) {
		...
		const valueEnd = path.attributeValueEnd(attribute) + (quoted ? 1 : 0);
