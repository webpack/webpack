...
const _resetAstColumns = () => {
	...
	const usedNodes = Math.min(_nodeCount, _nodeStrings.length - 1);
...
const dispatch = () => {
	...
	// Gate inlined here so the overwhelmingly common "nothing to do" token costs
	// three loads and a compare rather than a call into `_streamSync`.
	if (_streaming) {
...
const parseHtml = (input, pos = 0, options = {}) => {
	...
	_resetAstColumns();
	// Consumed once per parse: a direct `parseHtml` call never streams.
	_streaming = _streamRequested;
	...
	// keep `len/48`.
	// A streamed parse recycles each completed subtree, so its high-water mark
	// is the open subtree, not the document — pre-sizing from the input length
	// would allocate (and keep) orders of magnitude more than it ever uses.
	let nodeEstimate = (input.length / (skipText ? 24 : 12)) | 0;
	let attrEstimate = (input.length / 48) | 0;
	if (_streaming) {
		// A streamed parse recycles each flushed subtree, so it never needs the
		// whole document — but a document that keeps everything open (deep nesting)
		// still grows, and reaching the size by doubling overshoots where one exact
		// allocation would not. Cap the estimate at what a batch can hold.
		const cap = _FLUSH_BATCH * 2;
		if (nodeEstimate > cap) nodeEstimate = cap;
		if (attrEstimate > cap) attrEstimate = cap;
	}
 ...
 * the emitted text, so an intervening comment or whitespace node keeps the tag
 * even when minification later drops it — conservative, never wrong.
 * @param {HtmlPath} path the accessor positioned on the element
 ...
 * CSS assets, so an inline sheet is held to the same rules. Anything it rejects
 * is handed back untouched rather than failing the build.
 ...
 * The token set is what the DOM reads, and it is unchanged.
 * @param {string} raw raw (undecoded) value
 * @returns {string} the collapsed list
 */
const _normalizeTokenList = (raw) =>
	_asciiTrim(raw).replace(/[\t\n\f\r ]+/g, " ");
...
const _rewriteAttributeValue = (element, name, raw, viewport) => {
	...
	if (
		...
		return _normalizeTokenList(raw);
	}
...
const _minifyOpenTag = (path, open) => {
	...
	for (let i = 0; i < count; i++) {
		...
		// value itself.
		out += ` ${attributeName}=${open.slice((quoted ? valueStart - 1 : valueStart) - tagStart, valueEnd - tagStart)}`;
		unquotedTail = !quoted;
		consumedEnd = valueEnd;
	}
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		case NodeType.Element: {
			...
			const tag = minify && open !== "" ? _minifyOpenTag(path, open) : open;
			...
			const close =
				...
				(minify && _canOmitEndTag(path, name, path.node))
					? ""
					: minify && path.namespace() === NS_HTML
						? `</${_asciiLowerCase(path.closeTag().slice(2, -1))}>`
						: path.closeTag();
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "style") {
					return _isCssStyleElement(path, parent)
						? _minifyInlineCss(data)
						: data;
				}
		...
		case NodeType.Doctype: {
			...
			return head === null
				? source
				: `<!doctype ${_asciiLowerCase(head[1])}${source.slice(head[0].length)}`;
		}
...
const _exitNode = (node) => {
	...
	if (_writer !== undefined) _writer.printNode(node, A);
};
...
/**
 * Depth-first walk of `root`'s subtree, `root` included.
 * @param {HtmlNodeRef} root subtree root
 */
const _walkSubtree = (root) => {
	let node = root;
	descend: for (;;) {
		if (_enterNode(node)) {
...
// === Streaming walk ===
// Visitors fire while the tree is still being built and each completed subtree
// is recycled right after, so peak node storage is O(open subtree) instead of
// O(document) — the HTML analog of the CSS grammar's per-top-level-rule
// recycling. Only walk-only passes stream: printing needs every node's text
// alive until its parent prints.
/** @type {boolean} whether this parse streams its walk */
let _streaming = false;
/** @type {boolean} set by `grammar` for the next `parseHtml` call */
let _streamRequested = false;
...
/**
 * Move a childless node's columns to a lower free id and make it its parent's
 * only child. Used for a trailing text node held back from a flush.
 * @param {HtmlNodeRef} from current id
 * @param {HtmlNodeRef} to free id to move to
 * @param {HtmlNodeRef} container parent container
 */
const _relocateOnlyChild = (from, to, container) => {
	_nodeTypes[to] = _nodeTypes[from];
	_nodeFlags[to] = _nodeFlags[from];
	_nodeStarts[to] = _nodeStarts[from];
	_nodeEnds[to] = _nodeEnds[from];
	_nodeTagEnds[to] = _nodeTagEnds[from];
	_nodeNameEnds[to] = _nodeNameEnds[from];
	_nodeContentEnds[to] = _nodeContentEnds[from];
	_nodeAttrStarts[to] = _nodeAttrStarts[from];
	_nodeAttrCounts[to] = _nodeAttrCounts[from];
	_nodeStrings[to] = _nodeStrings[from];
	_nodeStrings[from] = "";
	_nodeParents[to] = container;
	_nodeFirstChildren[to] = 0;
	_nodeLastChildren[to] = 0;
	_nodeNextSiblings[to] = 0;
	_nodeFirstChildren[container] = to;
	_nodeLastChildren[container] = to;
};
 ...
 * the deepest open element). A trailing text node is held back and relocated:
...
const _streamRewind = (to) => {
	...
	if (to >= top && to < _nodeCount) _nodeCount = to;
};
...
const _streamFlush = (parent, openChild, visit, hold) => {
	...
	} else if (child === _nodeCount && child !== parent + 1) {
		// Safe only when the held node is the sole survivor above `parent`: its
		// target is free and the rewind leaves no gap for a later id to reuse.
		_relocateOnlyChild(child, parent + 1, container);
		_streamRewind(parent + 1);
	}
};
 ...
 * `_streamSafe()` holds, so an unsafe stretch (incorrectly nested formatting elements,
 * tables, templates) simply buffers and the next catch-up sees the final tree.
 */
const _streamSync = () => {
	// Enter lazily: as long as nothing already entered has closed and too little
	// has piled up under the deepest entered element, do nothing at all. An
	// element that opens and closes inside that window is never entered — it is
	// walked as one completed subtree later, which is both cheaper and gives its
	// visitors a final `end`. Shallow, sibling-heavy markup lives in this path,
	// where keying the gate on the deepest *open* element would defeat it: the
	// open stack changes on every start and end tag.
	...
	if (!_streamSafe()) return;
	// The two stacks diverge in a suffix: only pushes and pops reach here, since
	// a mid-stack `open.splice` sets `_openSpliced` and forces the full scan.
	let i;
	if (_openSpliced) {
		i = 0;
		...
		i = _entered.length < depth ? _entered.length : depth;
...
const grammar = (input, visitors, writer, options) => {
	...
	_streamRequested = writer === undefined;
	const root = parseHtml(input, 0, options);
	if (writer !== undefined) {
		_walkSubtree(root);
		// A single root, printed last: take its text as the output (the printer read
		// every node's source into the store during the walk, so this needs no source).
		writer.take(root);
	}
