 ...
 * (`count` 0 = none).
 * @typedef {{ start: number, count: number }} AttrRun
 */

// Shared frozen empty run for attributeless elements and synthesized tags.
const EMPTY_ATTRS = /** @type {AttrRun} */ (
...
/** @typedef {{ type: typeof TOKEN_START_TAG, name: string, attrs: AttrRun, selfClosing: boolean, pos: TagPos, swallowNewline?: boolean }} StartTagToken */
 ...
 * @typedef {{ type: number, name: string, data: string, attrs: AttrRun, selfClosing: boolean, start: number, end: number, publicId: (string | null), systemId: (string | null), swallowNewline: boolean, pos: TagPos }} MutableToken
 */
...
const hasFontBreakoutAttr = (/** @type {AttrRun} */ run) => {
	for (let i = run.start; i < run.start + run.count; i++) {
...
const buildHtmlAst = (source, fragmentContext, skip = EMPTY_SKIP) => {
	...
	const adjustMathmlAttrs = (/** @type {AttrRun} */ run) => {
		for (let i = run.start; i < run.start + run.count; i++) {
...
const A = {
	...
	/** @type {(n: HtmlElement) => number} */
	attrCount: (n) => _hACount[n],
	// the i-th attribute of an element, as an id for the `attr*` reads below
	/** @type {(n: HtmlElement, i: number) => HtmlAttrRef} */
	attrAt: (n, i) => _hAStart[n] + i,
	// linear lookup by (lowercased) name; 0 = not present
	/** @type {(n: HtmlElement, name: string) => HtmlAttrRef} */
	findAttr: (n, name) => _aFind(_hAStart[n], _hACount[n], name),
	/** @type {(a: HtmlAttrRef) => string} */
	attrName: (a) => _aName[a],
	// raw (undecoded) attribute value ("" when valueless)
	/** @type {(a: HtmlAttrRef) => string} */
	attrValue: (a) => _aValueOf(a),
	/** @type {(a: HtmlAttrRef) => number} */
	attrNameStart: (a) => _aNameStart[a],
	/** @type {(a: HtmlAttrRef) => number} */
	attrNameEnd: (a) => _aNameEnd[a],
	// -1 when valueless or on adoption-agency clones
	/** @type {(a: HtmlAttrRef) => number} */
	attrValueStart: (a) => _aValStart[a],
	/** @type {(a: HtmlAttrRef) => number} */
	attrValueEnd: (a) => _aValEnd[a],
