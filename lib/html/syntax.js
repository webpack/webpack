...
// Set once the fostered runs are settled, on the element whose content prints
// from source and on everything still open around it — the walk asks this of
// every node, so it is a flag rather than a lookup.
const FLAG_FOSTER_REGION = 128;
// The source wrote this element's end tag, rather than the parser popping it for
// an implied close.
const FLAG_SOURCE_END_TAG = 64;
...
const _growAttrColumns = (need, exact) => {
	...
	if (exact) {
		...
		// Doubling overshoots when the need lands just past the pre-size, and a
		// capacity over the shrink threshold is released after the parse, so the
		// next one re-allocates from nothing and grows again. The smaller capacity
		// that still holds it lets the columns survive instead.
...
// Whether the token in hand is an end tag: the adoption agency and "any other end
// tag" both also run for a start tag, and close nothing the source spelled.
let _sourceEndTag = false;
let eofInTag = false;
...
const reconstructAfe = () => {
	...
	for (; i < afe.length; i++) {
		const e = afe[i];
		// Writing this one back is only unreachable when its own start tag would do
		// something: §13.2.6.4.7 gives `a` and `nobr` a rule of their own, and every
		// other formatting element re-nests as written.
...
const closePElement = () => {
	...
	while (open.length) {
		...
		if (_namespaceOf(el) === NS_HTML && _tagNameOf(el) === "p") {
			if (_sourceEndTag) _nodeFlags[el] |= FLAG_SOURCE_END_TAG;
			break;
		}
	}
};
...
const adoptionAgency = (
	...
	while (outer < 8) {
		...
		if (_sourceEndTag) _nodeFlags[fmt] |= FLAG_SOURCE_END_TAG;
...
const anyOtherEndTag = (/** @type {string} */ name) => {
	for (let i = open.length - 1; i >= 0; i--) {
		...
		if (_namespaceOf(node) === NS_HTML && _tagNameOf(node) === name) {
			if (_sourceEndTag) _nodeFlags[node] |= FLAG_SOURCE_END_TAG;
...
const startTagInBody = (/** @type {StartTagToken} */ t) => {
	...
	if (name === "form") {
		...
		closeIfPInButtonScope();
		// A nested form reaches the tree only because an end tag cleared the pointer
		// on the way, and writing that end tag back here generates implied end tags
		// where the source's did not — so anything open between the two moves.
	...
	if (name === "li") {
		...
		for (let i = open.length - 1; i >= 0; i--) {
			...
			if (
				...
				if (_itemOpenBelow(i, false)) _printFromSource = true;
				break;
			}
		}
		closeIfPInButtonScope();
	...
	if (name === "dd" || name === "dt") {
		...
		for (let i = open.length - 1; i >= 0; i--) {
			...
			if (
				...
				if (_itemOpenBelow(i, true)) _printFromSource = true;
				break;
			}
		}
		closeIfPInButtonScope();
...
modes.text = (t) => {
	...
	if (t.type === TOKEN_END_TAG) {
		...
		const closed = /** @type {HtmlElement} */ (popOpen());
		_nodeEnds[closed] = tokenEnd;
		if (_sourceEndTag) _nodeFlags[closed] |= FLAG_SOURCE_END_TAG;
		mode = originalMode;
	}
};
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all" }} HtmlPrintOptions
 */
...
// The tree holds something no arrangement of tags reaches: an element §13.2.4.3
// or the adoption agency built that no source tag wrote, or a list item left open
// only because a special element stopped the loop that would have closed it.
		...
		if (quoted) {
			...
			const minimal =
				...
				(_asciiTrim(decoded) !== "" || _asciiTrim(rawValue) === "")
					...
					: null;
			const unquotable = _canUnquoteAttributeValue(value);
			if (
				minimal !== null &&
...
/**
 * Whether a list item is still open below the element the dd/dt/li loop stopped
 * at. §13.2.6.4.7 stops it at a special element, so that item stays open — a
 * shape reproduced only while the element that stopped it stays open too, which
 * no arrangement of tags the printer writes can guarantee.
 * @param {number} from index the loop stopped at
 * @param {boolean} dd true for the dd/dt loop, false for li
 * @returns {boolean} true when one is still open below it
 */
const _itemOpenBelow = (from, dd) => {
	for (let i = from - 1; i >= 0; i--) {
		const node = open[i];
		if (_namespaceOf(node) !== NS_HTML) continue;
		const name = _tagNameOf(node);
		if (dd ? name === "dd" || name === "dt" : name === "li") return true;
	}
	return false;
};
...
const _startTagEnds = (openName, name) => {
	...
	// §4.13 says which end tags may be left out, which is not the same list as the
	// start tags §13.2.6.4.7 closes a `p` for — `center` is in one and not in the
	// other, and it is the parser's list that decides what the tree comes out as.
...
const _fosterRunMoves = (path, table) => {
	...
	// The run moves or stays as a whole, so one node needing to move and another
	// unable to replay is the verbatim case whichever order they come in — worth a
	// second look only once something is known to block it.
	if (blocked) {
		closes = false;
	...
	if (blocked) {
		...
		// The holder, not the table: an earlier run in it prints end tags the source
		// never wrote, and those leave the list of active formatting elements
		// somewhere else before this table is even reached.
...
const _digest = (root) => {
	...
	const walk = (node, depth) => {
		...
		const first = content !== 0 ? A.firstChild(content) : A.firstChild(node);
		for (let c = first; c !== 0; c = A.nextSibling(c)) walk(c, depth + 1);
	};
...
const grammar = (input, visitors, writer, options) => {
	...
	try {
		...
		// The parse met something whose shape no arrangement of tags is known to
		// reach. Rather than give up on printing it, keep the tree to check the
		// output against — a fraction of a percent of documents reach this.
		const digest =
			writer !== undefined && _printFromSource ? _digest(root) : "";
		if (writer !== undefined) {
			...
			// Only for those: print, then read the output back and see whether it
			// parses to the tree it came from. The source always does, so it is what
			// stands in when the printed form does not.
			if (digest !== "") {
...
const A = {
	...
	/**
	 * @param {HtmlElement=} n element
	 * @returns {boolean} whether the source wrote this element's end tag
	 */
	sourceClosed(n = _currentNode) {
		return (_nodeFlags[n] & FLAG_SOURCE_END_TAG) !== 0;
	},
