 ...
 * @property {(boolean | "conservative" | "smart" | "all")=} collapseWhitespace collapse each run of whitespace in text to a single space, except where an ancestor renders it verbatim; `"smart"` also drops what sits against a block edge and `"all"` drops every edge; an inline `<script>` holding JavaScript or JSON is trimmed at its edges, a data block's body never (default false)
...
/**
 * The text of an embedded body: a document in a language of its own, so the
 * host's whitespace around it is not part of it. Kept whole where nothing
 * collapses whitespace, and where it is only whitespace — emptying the element
 * hands it to `removeEmptyElements`, and the DOM would lose what the source
 * wrote.
 * @param {string} source the nested body
 * @returns {string} the body to embed
 */
const _embeddedBody = (source) => {
	if (_collapseWhitespace === "none") return source;
	const trimmed = _asciiTrim(source);
	return trimmed === "" ? source : trimmed;
};

/**
 * `_renderEmbeddedOrNull` for a body webpack has no minifier of its own for, so
 * a renderer that declines leaves it as embedding found it.
 * @param {string} source the nested body
 * @param {string} type its language, e.g. `"css"` / `"javascript"` / `"svg"`
 * @returns {string} the rendered body, or the original
 */
const _renderEmbedded = (source, type) => {
	const body = _embeddedBody(source);
	// A body that is only whitespace is no source to render, and what a
	// renderer made of it would empty the element.
	if (_asciiTrim(body) === "") return body;
	if (_deferEmbeddedSource !== undefined) {
		return _deferEmbedded(body, type, (rendered) =>
			typeof rendered === "string" ? rendered : body
		);
	}
	if (_renderEmbeddedSource === undefined) return body;
	const rendered = _renderEmbeddedOrNull(body, type);
	return rendered === null ? body : rendered;
};
...
const _isRedundantAttribute = (markers, defaults, tagName, name, value) => {
	...
	if (tagName === "script" && name === "type") {
		const type = value.includes("&")
			? _asciiLowerCase(_asciiTrim(decodeEntities(value, true)))
			: lowered;
		return (
...
const _scriptType = (path, element) => {
	for (const attribute of path.attributes(element)) {
		if (attribute.name === "type") {
			...
			// Guarded on the reference, so the decode stays off the hot path.
			return _asciiLowerCase(
...
const _minifyInlineJson = (json) => {
	const body = _embeddedBody(json);
	if (_asciiTrim(body) === "") return body;
	if (_deferEmbeddedSource !== undefined) {
		return _deferEmbedded(body, JSON_TYPE, (rendered) =>
			typeof rendered === "string" ? rendered : _builtinInlineJson(body)
		);
	}
	if (_renderEmbeddedSource !== undefined) {
		const rendered = _renderEmbeddedOrNull(body, JSON_TYPE);
		if (rendered !== null) return rendered;
	}
	return _builtinInlineJson(body);
};
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					// A data block — a template, a payload in a language nothing here
					// names. Its whitespace can be its own syntax, so it is left alone.
					if (!JAVASCRIPT_SCRIPT_TYPES.has(type)) return data;
					// Program text, embedded like any other: webpack ships no JS
					// minifier, so what comes back is a caller's renderer or the edges.
