 ...
 * @property {boolean=} xml print XML rather than HTML, for a standalone `.svg`: quotes stay on, no tag is left out, every `&` is escaped and a `<script>` / `<style>` body is not raw text (default false)
 */
 ...
 * @typedef {{ transforms?: HtmlTransformOptions, collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, mergeScripts?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", xml?: boolean, renderEmbeddedSource?: EmbeddedSourceRenderer, deferEmbeddedSource?: DeferredEmbeddedSource[], deferSrcdoc?: boolean }} HtmlPrintOptions
 */
...
/**
 * A `<script>` / `<style>` body as XML: raw text in HTML, ordinary content in
 * XML, so anything holding a `<` or `&` goes back inside a CDATA section.
 * @param {string} body the element's text
 * @returns {string} the body, wrapped when XML needs it
 */
const _cdataText = (body) =>
	body.includes("<") || body.includes("&") || body.includes("]]>")
		? `<![CDATA[${body.split("]]>").join("]]]]><![CDATA[>")}]]>`
		: body;

/**
 * `_cdataText` where the print is XML. Reads the switch now, so a caller whose
 * answer lands after the print holds `_cdataText` itself instead.
 * @param {string} body the element's text
 * @returns {string} the body, wrapped when XML needs it
 */
const _wrapCdata = (body) => (_xml ? _cdataText(body) : body);
...
const _canOmitEndTag = (path, name, node) => {
	if (_xml || path.namespace(node) !== NS_HTML) return false;
...
/**
 * One attribute, written with whichever delimiter its value needs fewer escapes
 * inside — a minified document is mostly `"`, so one holding a `"` is shorter
 * quoted with `'`. Where neither costs anything the source's own is kept.
...
const _writeQuotedAttribute = (name, value, source) => {
	...
	return ` ${name}=${delimiter}${escapeAttribute(value, quote, true)}${delimiter}`;
};
 ...
 * @param {string} type its language, e.g. `"css"` / `"javascript"` / `"svg"`
 * @param {string=} as which production of that language it is, for one with more than one
 * @param {((body: string) => string)=} wrap applied to the answer, deferred or not — a body a renderer returns still has to be spelled for the syntax around it
 * @returns {string} the rendered body, or the original
 */
const _renderEmbedded = (source, type, as, wrap) => {
	const spell =
		wrap === undefined ? (/** @type {string} */ body) => body : wrap;
	if (_deferEmbeddedSource !== undefined) {
		return _deferEmbedded(
			source,
			type,
			(rendered) => spell(typeof rendered === "string" ? rendered : source),
			as
		);
	}
	const rendered = _renderEmbeddedOrNull(source, type, as);
	return spell(rendered === null ? source : rendered);
};
...
// Print XML rather than HTML, which a standalone `.svg` is.
let _xml = false;
...
const _writeAttribute = (attributeName, value, rawValue, decoded) => {
	...
	const unquotable =
		...
		!_xml &&
		_canUnquoteAttributeValue(value);
...
// SVG 2 §9.3: parameters per path command, and which are one-character flags.
...
/**
 * Rewrite one path-data number in fewer bytes. Textual only — a `+`, a leading
 * zero, a trailing run of fraction zeros and an upper-case `E` are all the
 * value re-spelled, so nothing here reformats the number itself.
 * @param {string} text the number as written
 * @returns {string} the same value in no more bytes
 */
const _shortenPathNumber = (text) => {
	// Most numbers in a path are already as short as they go, so the shape is
	// read off the characters before anything is sliced or rebuilt.
	let at = 0;
	if (text.charCodeAt(0) === 45) at = 1;
	let shortest = true;
	let digits = 0;
	let dotAt = -1;
	let exponentAt = -1;
	for (let i = at; i < text.length; i++) {
		...
		if (c === 46) {
			...
			// A leading zero says nothing unless it is the whole integer part.
			if (digits === 0 && c === 48 && i + 1 < text.length) shortest = false;
			digits++;
		}
	}
	if (shortest && exponentAt === -1) {
		const last = text.charCodeAt(text.length - 1);
		// A trailing fraction zero and a bare `.` both go.
		if (dotAt === -1 || (last !== 48 && last !== 46)) return text;
	}
	let rest = text;
	let sign = "";
	if (rest[0] === "+" || rest[0] === "-") {
	...
	let exponent = "";
	const e = rest.search(/[eE]/);
	if (e !== -1) {
		exponent = rest.slice(e + 1);
		rest = rest.slice(0, e);
	}
	const dot = rest.indexOf(".");
	const whole = (dot === -1 ? rest : rest.slice(0, dot)).replace(/^0+/, "");
	const fraction = (dot === -1 ? "" : rest.slice(dot + 1)).replace(/0+$/, "");
	if (whole === "" && fraction === "") return "0";
	let out = fraction === "" ? whole : `${whole}.${fraction}`;
	if (exponent !== "") {
		let expSign = "";
		...
		if (exponent !== "") out += `e${expSign}${exponent}`;
	}
	return sign + out;
};

/**
 * Print an SVG `<path>`'s `d` in fewer bytes: the same commands, with every
 * separator and repeated command letter the grammar can imply left out.
 * @param {string} raw the attribute value
 * @returns {string} the same path, or `raw` when it is not path data
 */
const _minifyPathData = (raw) => {
	const n = raw.length;
	let i = 0;
	let out = "";
	let previous = "";
	let lastNumber = "";
	const skip = () => {
		while (i < n && (isSpace(raw.charCodeAt(i)) || raw.charCodeAt(i) === 44)) {
			i++;
		}
	};
	const startsNumber = () => {
		...
		return isAsciiDigit(c) || c === 45 || c === 43 || c === 46;
	};
	const readNumber = () => {
		const start = i;
		...
		let digits = 0;
		while (i < n && isAsciiDigit(raw.charCodeAt(i))) {
			i++;
			digits++;
		}
		if (i < n && raw.charCodeAt(i) === 46) {
			i++;
			while (i < n && isAsciiDigit(raw.charCodeAt(i))) {
				i++;
				digits++;
		...
		if (i < n && (raw.charCodeAt(i) === 101 || raw.charCodeAt(i) === 69)) {
			const mark = i;
			i++;
			if (i < n && (raw.charCodeAt(i) === 43 || raw.charCodeAt(i) === 45)) i++;
			let exponentDigits = 0;
			while (i < n && isAsciiDigit(raw.charCodeAt(i))) {
				i++;
				exponentDigits++;
			}
			if (exponentDigits === 0) i = mark;
		}
		return raw.slice(start, i);
	};

	skip();
	if (i < n && raw[i] !== "M" && raw[i] !== "m") return raw;
	while (i < n) {
		const spec = PATH_COMMANDS.get(_asciiLowerCase(raw[i]));
		...
		let repetition = 0;
		do {
			// A repeated parameter set repeats the command — except a moveto's, whose
			// repeat is a lineto, which is why a moveto's letter is never left out.
			const letter =
				repetition === 0 || (written !== "M" && written !== "m")
					? written
					: written === "M"
						? "L"
						: "l";
			...
			for (let p = 0; p < arity; p++) {
				...
				if (flags & (1 << p)) {
					const c = raw.charCodeAt(i);
					if (c !== 48 && c !== 49) return raw;
					text = raw[i];
					i++;
				} else {
					const number = readNumber();
					if (number === null) return raw;
					text = _shortenPathNumber(number);
				}
				// A `-` and a second `.` each terminate the number before them.
				if (
					lastNumber !== "" &&
					!(
						text.charCodeAt(0) === 45 ||
						(text.charCodeAt(0) === 46 && /[.eE]/.test(lastNumber))
					)
				) {
					out += " ";
				}
				out += text;
				lastNumber = flags & (1 << p) ? "" : text;
			}
			repetition++;
			skip();
		} while (startsNumber());
...
// A document spells the same handful of presentation values over and over, so
// each pair is minified once. Cleared per parse: a value is this source's.
/** @type {Map<string, string>} */
const _presentationValues = new Map();

/**
 * Minify an SVG presentation attribute, whose value is the CSS value of the
 * property it is named for.
 * @param {string} name lowercased attribute name
 * @param {string} raw raw (undecoded) attribute value
 * @returns {string} the minified value
 */
const _minifySvgPresentation = (name, raw) => {
	// A presentation attribute is one value, not a declaration list, so a `;`
	// would make the CSS parser read a second declaration.
	if (raw.trim() === "" || raw.includes(";")) return raw;
	const key = `${name}:${raw}`;
	// An answer that no longer states this property — a value dropped whole, a
	// comment — is no answer at all, so the attribute keeps what it had.
	const readBack = (/** @type {string | undefined} */ rendered) =>
		rendered !== undefined && rendered.startsWith(`${name}:`)
			? rendered.slice(name.length + 1)
			: raw;
	// Each marker stands for its own hole, so this path is the one the memo
	// below cannot serve.
	if (_deferEmbeddedSource !== undefined) {
		return _deferEmbedded(key, CSS_TYPE, readBack, BLOCK_CONTENTS);
	}
	const seen = _presentationValues.get(key);
	if (seen !== undefined) return seen;
	const value = readBack(_minifyStyleAttribute(key));
	_presentationValues.set(key, value);
	return value;
};
...
const _minimalAttributeValue = (value) => {
	...
	for (const bare of _xml ? [] : [underDouble, underSingle]) {
...
const _isRedundantAttribute = (markers, defaults, tagName, name, value) => {
	...
	if (tagName === "script" && name === "type") {
		// Read the way `_scriptType` reads it, or the two disagree about which
		// `<script>` this is and the body's handler and this one part ways.
		const type = _asciiLowerCase(_asciiTrim(decodeEntities(value, true)));
		return (
			type !== "" && type !== "module" && JAVASCRIPT_SCRIPT_TYPES.has(type)
		);
	}
...
/**
 * A foreign element's own `xml:space`, or undefined when it states none.
 * @param {HtmlPath} path the accessor
 * @param {HtmlNodeRef} element the element
 * @returns {string | undefined} the value, lowercased
 */
const _xmlSpace = (path, element) => {
	for (const attribute of path.attributes(element)) {
		if (_asciiLowerCase(attribute.name) === "xml:space") {
...
const _mergeableScriptAfter = (path, element) => {
	if (path.attributeCount(element) !== 0) return 0;
	for (
		let next = _nodeNextSiblings[element];
		next !== 0;
		next = _nodeNextSiblings[next]
...
/**
 * XML has no void elements and no optional end tags, so an element with no
 * children is spelled `<name/>` — the same element per XML 1.0 §3.1. A
 * `<template>` keeps its children in a content fragment, not the child chain.
 * @param {HtmlPath} path the accessor positioned on the element
 * @returns {boolean} whether it prints as an empty-element tag
 */
const _xmlSelfCloses = (path) =>
	_xml && path.firstChild() === 0 && path.templateContent() === 0;
...
const _minifyOpenTag = (path, open) => {
	...
	// for byte; decided before the name is sliced, so it allocates nothing. One
	// that self-closes falls through: its terminator is not the source's.
	if (count === 0 && open.length === nameEnd + 1 && !_xmlSelfCloses(path)) {
		let folds = false;
		if (html) {
	...
	// Only where the document is XML: inline in HTML these values are readable
	// from script, and respelling one is a difference an engine can tell apart.
	const isSvg = _xml && namespace === NS_SVG;
	const isSvgPath = isSvg && elementName === "path";
	...
	for (let k = 0; k < count; k++) {
		...
		if (
			...
			if (_deferEmbeddedSource !== undefined && _deferSrcdoc) {
				...
				const delimiter = quoted ? open[valueStart - tagStart - 1] : "";
		...
		if (quoted) {
			...
			if (!hasReference) {
				...
				} else if (isSvgPath && attributeName === "d") {
					value = _minifyPathData(rawValue);
				} else if (isSvg && SVG_PRESENTATION.has(attributeName)) {
					value = _minifySvgPresentation(attributeName, rawValue);
				}
			}
	...
	return `${out}${_xmlSelfCloses(path) ? "/>" : ">"}`;
};
...
const _elementCloseTag = (path, name, open, innerEmpty, minify) => {
	...
	if (
		...
		(minify && (_xmlSelfCloses(path) || _canOmitEndTag(path, name, path.node)))
	) {
		return "";
	}
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "style") {
					...
					if (!_mergeStyles) {
						...
						return _wrapCdata(minified === null ? data : minified);
					}
				...
				if (literalName === "script") {
					...
					if (JAVASCRIPT_SCRIPT_TYPES.has(type) && body.trim() !== "") {
						return _renderEmbedded(
							...
							// Read now: a deferred answer lands after the print, by when
							// `_xml` belongs to whatever parse ran next.
							_xml ? _cdataText : undefined
						);
					}
					return _wrapCdata(body);
				}
				return _wrapCdata(data);
			}
			...
			if (minify && _collapseWhitespace !== "none" && !data.includes("<")) {
				...
				for (
					...
					if (path.namespace(ancestor) !== NS_HTML) {
						const space = _xmlSpace(path, ancestor);
						if (space !== undefined) {
							preformatted = space === "preserve";
							break;
						}
						continue;
					}
					if (LEADING_NEWLINE_ELEMENTS.has(path.tagName(ancestor))) {
						preformatted = true;
						break;
					}
				}
			...
			// A bare `&` carried over is fatal in XML, not literal text.
			if (
				!_xml &&
		...
		case NodeType.Doctype: {
			...
			// XML spells the keyword in upper case and reads the name case-sensitively,
			// so neither is folded there.
			if (_xml) return `<!DOCTYPE ${source.slice("<!doctype ".length)}`;
			return `<!doctype ${_asciiLowerCase(head[1])}${source.slice(
				head[0].length
			)}`;
		}
...
const grammar = (input, visitors, writer, options) => {
	...
	_xml = writer !== undefined && writer.options.xml === true;
