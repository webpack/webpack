...
// A `<template shadowrootmode>` the parser turned into its parent's shadow
// root, and the parent it hangs off. The engine takes such a template out of
// the tree entirely; webpack keeps it where it was written, so the two agree
// once it is written back — but it is no longer a child the tree construction
// may move away from its host.
const FLAG_SHADOW_ROOT = 16;
const FLAG_SHADOW_HOST = 32;

// The elements `attachShadow()` accepts by name (DOM §4.9); anything else is a
// shadow host only when it is a valid custom element name.
...
// A hyphenated name the spec keeps for itself, so it is not a custom one.
...
// The three the parser implies around every document; their end tags are
// decided by `_canOmitShellEnd`, not by what follows them.
...
const _escapesStyleWrapper = (value) => {
	...
	for (let i = 0; i < value.length; i++) {
		...
		if (c === 0x5c) {
			i++;
			// The escape the value ran out of leaves nothing open.
			if (i >= value.length) return depth !== 0;
