 ...
 * @property {(boolean | "keep-head-and-body")=} tagOmission drop the `<html>` / `<head>` / `<body>` tags §13.1.2.4 allows; `"keep-head-and-body"` drops only `<html>` (default false)
...
// `optimization.minimize.html.tagOmission`: false keeps the document shell as
// authored, "keep-head-and-body" drops only `<html>`, true drops all six tags.
/** @type {boolean | "keep-head-and-body"} */
let _tagOmission = false;

/**
 * Whether a node reaches the output. A comment minification drops does not, so
 * it cannot separate an implied tag from what follows it.
 * @param {HtmlPath} path the accessor
 * @param {HtmlNodeRef} node the node, or 0
 * @returns {boolean} true when the node survives into the output
 */
const _survivesMinify = (path, node) =>
	node !== 0 &&
	(path.type(node) !== NodeType.Comment ||
		_keepComment(path.data(node), path.source(node)));

/**
 * Whether the `<html>` / `<head>` / `<body>` start tag may be left out (§13.1.2.4).
 * @param {HtmlPath} path the accessor
 * @param {string} name the element's lowercased tag name
 * @param {HtmlNodeRef} node the element
...
const _canOmitShellStart = (path, name, node) => {
	...
	while (first !== 0 && !_survivesMinify(path, first)) {
		first = path.nextSibling(first);
	}
	if (first === 0) return name === "html" || _tagOmission === true;
	const type = path.type(first);
	// A surviving comment would move above any of the three implied tags.
	if (type === NodeType.Comment) return false;
	if (name === "html") return true;
	if (_tagOmission !== true) return false;
	if (name === "head") return type === NodeType.Element;
	// Whitespace would move above the implied `<body>`, and so would an element
	// the parser still reads as head content.
	if (type === NodeType.Text) return !isAllWs(path.data(first));
	return !BODY_START_KEPT_BEFORE.has(path.tagName(first));
};

/**
 * Whether the `<html>` / `<head>` / `<body>` end tag may be left out (§13.1.2.4).
 * @param {HtmlPath} path the accessor
 * @param {string} name the element's lowercased tag name
 * @param {HtmlNodeRef} node the element
...
const _canOmitShellEnd = (path, name, node) => {
	...
	while (next !== 0 && !_survivesMinify(path, next)) {
		next = path.nextSibling(next);
	}
	if (next === 0) return true;
	const type = path.type(next);
	// A surviving comment would move inside the element the tag closed.
	if (type === NodeType.Comment) return false;
	// `</head>` alone also keeps the tag for whitespace: it would move into `<body>`.
	return name !== "head" || type !== NodeType.Text || !isAllWs(path.data(next));
};
...
const _elementCloseTag = (path, name, open, innerEmpty, minify) => {
	...
	// there is no source end tag to echo.
	const closeName = _asciiLowerCase(sourceClose.slice(2, -1)).trim() || name;
...
const grammar = (input, visitors, writer, options) => {
	...
	_visitors = visitors;
	_writer = writer;
	// `_streamTags` reads this when an element opens, before the printer's first
	// call, so it cannot be set from there like the other print options.
