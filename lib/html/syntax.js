		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					// Program text: its edges sit between tokens, and the parser is
					// where the body ends. Trimmed before the offer, so a renderer that
					// declines still leaves them behind. A body that is only whitespace
					// keeps it: emptying the element hands it to `removeEmptyElements`,
					// and the DOM would lose a `<script>` the source wrote.
					const trimmed =
						_collapseWhitespace === "none" ? data : _asciiTrim(data);
					const body = trimmed === "" ? data : trimmed;
