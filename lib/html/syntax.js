...
// The parse goal a `<script type=module>` body is read with: top-level `await`
// is a syntax error as a classic script, so it is said rather than guessed.
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					// No built-in: webpack's own printers cover no JS, so an inline
					// script is touched only by a renderer — `htmlMinify` brings one.
					if (
						(_renderEmbeddedSource !== undefined ||
							_deferEmbeddedSource !== undefined) &&
						...
						return _renderEmbedded(
							data,
							JAVASCRIPT_TYPE,
							type === MODULE_SCRIPT ? MODULE_SCRIPT : undefined
						);
					}
				}
				return data;
