	...
	try {
		...
		if (writer !== undefined) {
			...
			if (verify) {
				...
				// A deferred body — an `<svg>` subtree, an inline `<style>`, a
				// `srcdoc` — prints as a marker, which reads back as text where its
				// node stood. Put back first, or every such document fails the
				// comparison and is handed back whole.
				const printed =
					_deferEmbeddedSource === undefined ||
