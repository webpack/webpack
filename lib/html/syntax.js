 ...
 * @property {boolean=} mergeScripts print a run of adjacent bare `<script>` elements as one, which removes elements (default false)
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { cssTransforms?: CssTransformOptions, transforms?: HtmlTransformOptions, collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, mergeScripts?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", renderEmbeddedSource?: EmbeddedSourceRenderer, deferEmbeddedSource?: DeferredEmbeddedSource[], deferSrcdoc?: boolean }} HtmlPrintOptions
 */
...
// A run of adjacent `<style>` or `<script>` elements prints as one, so the
// elements after the first print nothing at all. Filled by the first one's text as it prints, which
 ...
 * a trailing line comment is fine because the join writes a newline first. A
 * regex holding a quote reads here as an open string, which only declines a
 * merge.
...
/**
 * Whether a script body opens with something only a script's own start can
 * carry: a hashbang, or a directive prologue — a leading string literal, which
 * `"use strict"` is. Appended after another body both stop meaning that, so a
 * body opening with either is never folded into the one before it.
 * @param {string} js the script's text
...
const _opensAScript = (js) => {
	...
	if (i === 0 && js.startsWith("#!")) return true;
	return js[i] === '"' || js[i] === "'";
};
...
const _mergedScriptRun = (path, element, js) => {
	...
	let out = js;
	for (
		let next = _mergeableScriptAfter(path, element);
		next !== 0;
		next = _mergeableScriptAfter(path, next)
	) {
		const body =
			_nodeStrings[/** @type {HtmlNodeRef} */ (_onlyTextChild(next))];
		if (!_scriptEndsClean(body) || _opensAScript(body)) break;
		if (_absorbed === null) _absorbed = new Set();
		_absorbed.add(next);
		out += `\n;${body}`;
	}
