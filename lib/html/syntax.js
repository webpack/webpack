 ...
 * @property {(boolean | "keep-head-and-body")=} tagOmission leave out the `<html>` / `<head>` / `<body>` tags §13.1.2.4 lets the parser imply; `false` keeps all three and `true` drops all three (default `"keep-head-and-body"`, which drops only `<html>`)
 */
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits"> & { collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortClassNames?: boolean, tagOmission?: boolean | "keep-head-and-body" }} HtmlPrintOptions
 */
...
// Which of the `<html>` / `<head>` / `<body>` tags §13.1.2.4 lets the parser
// imply may be left out (see `HtmlProcessOptions`). Every *other* optional tag
// goes unconditionally — nothing can observe that — but these three are the
// ones a consumer reading the page with a regexp rather than a parser looks
// for, so `<head>` and `<body>` stay unless asked. `<html>` goes: it is only
// omittable when it carries no attribute at all, so a `<html lang=en>` a
// robot reads keeps its tag anyway.
/** @type {boolean | "keep-head-and-body"} */
let _tagOmission = "keep-head-and-body";
...
const _canOmitShellStart = (path, name, node) => {
	...
	while (
		first !== 0 &&
		path.type(first) === NodeType.Comment &&
		!_keepComment(path.data(first), path.source(first))
	) {
		first = path.nextSibling(first);
	}
	if (first === 0) return name === "html" || _tagOmission === true;
	const type = path.type(first);
	if (type === NodeType.Comment) return false;
	if (name === "html") return true;
	if (_tagOmission !== true) return false;
...
/**
 * The same, for the end tag: a comment behind it would move inside the element
 * the tag closed, and whitespace behind `</head>` into `<body>`.
 * @param {HtmlPath} path the accessor
 * @param {string} name the element's lowercased tag name
 * @param {HtmlNodeRef} node the element
...
const _canOmitShellEnd = (path, name, node) => {
	if (name !== "html" && _tagOmission !== true) return false;
	let next = path.nextSibling(node);
	while (
		next !== 0 &&
		path.type(next) === NodeType.Comment &&
		!_keepComment(path.data(next), path.source(next))
	) {
...
const _rewriteAttributeValue = (element, name, raw, viewport, enumeratedOn) => {
	...
	if (
		...
		// Only `class` sorts: the other lists the DOM folds are sets too, but
		// `class` is where the repetition a compressor pays for actually is.
		return _normalizeTokenList(
			raw,
			DOM_TOKEN_LIST_ATTRIBUTES.has(name),
			_sortClassNames && name === "class"
		);
	}
...
const _streamTags = (path, minify) => {
	...
	if (
		...
		(name === "tbody"
			...
			: name === "colgroup"
				? _canOmitColgroupStart(path, path.node)
				: _tagOmission !== false && _canOmitShellStart(path, name, path.node))
	) {
		return {
			open: "",
			...
			if (
				...
				(name === "tbody"
					...
					: name === "colgroup"
						? _canOmitColgroupStart(path, path.node)
						: _tagOmission !== false &&
							_canOmitShellStart(path, name, path.node))
			) {
...
const grammar = (input, visitors, writer, options) => {
	...
	const tagOmission = writer === undefined ? false : writer.options.tagOmission;
	_tagOmission =
		tagOmission === undefined
			? "keep-head-and-body"
	...
	try {
		...
		_parsing = false;
		_tagOmission = "keep-head-and-body";
		_streamEligible = false;
		_streaming = false;
		_docSkipped = false;
