 ...
 * @property {boolean=} minimize print the safely-minified serialization (nodes rebuilt from source, inert comments dropped, opening tags rewritten) as `process` walks, and return it (default false = walk only, return `""`)
 */
...
// What forces an attribute value to keep its quotes: whitespace and `>` end an
// unquoted value, the rest are parse errors inside one (§13.2.5.34).
const _MUST_QUOTE_ATTRIBUTE_RE = /[\t\n\f\r "'`=<>]/;

/**
 * Serialize one attribute's raw (undecoded) value with the cheapest quoting that
 * re-tokenizes to it. A trailing `/` must stay quoted: unquoted it would fuse
 * with a self-closing tag's `/>`.
 * @param {string} value the value's source text, quotes excluded
 * @returns {string} the value as written after `=`
 */
const _quoteAttributeValue = (value) => {
	if (
		value.length !== 0 &&
		!_MUST_QUOTE_ATTRIBUTE_RE.test(value) &&
 ...
 * @param {boolean} minify whether to write each value with its cheapest quoting
 * @returns {string} serialized opening tag
 */
const _synthesizeOpenTag = (path, minify) => {
	...
	for (const attribute of path.attributes()) {
		...
		if (attribute.value === "") {
			...
			out += ` ${name}=${
				minify
					? _quoteAttributeValue(attribute.value)
...
/**
 * Rewrite an opening tag from its own source spans: one space between
 * attributes, the cheapest quoting per value, and `x=""` written as the
 * equivalent valueless `x`. Every attribute keeps its source name and raw value,
 * so the tag re-parses to the same element. Falls back to the source tag when an
 * attribute lies outside it — `<body>` merges a later duplicate tag's attributes
 * onto the element, and those cannot be sliced from here.
 * @param {HtmlPath} path the accessor positioned on the element
...
const _minifyOpenTag = (path, open) => {
	...
	let out = `<${open.slice(1, path.nameEnd() - tagStart)}`;
	...
	for (let i = 0; i < count; i++) {
		...
		out += ` ${open.slice(nameStart - tagStart, nameEnd - tagStart)}`;
		unquotedTail = false;
		consumedEnd = nameEnd;
		const valueStart = path.attributeValueStart(attribute);
		if (valueStart === -1) continue;
		const valueEnd = path.attributeValueEnd(attribute);
		if (valueStart < tagStart || valueEnd > tagEnd) return open;
		consumedEnd = valueEnd;
		// `x=""` and a valueless `x` are the same attribute.
		if (valueEnd === valueStart) continue;
		const written = _quoteAttributeValue(
			open.slice(valueStart - tagStart, valueEnd - tagStart)
		);
		out += `=${written}`;
		unquotedTail = written.charCodeAt(0) !== 34 && written.charCodeAt(0) !== 39;
	}
	// A source `/>` must survive — it self-closes a foreign element — but only
	// when the `/` really is the self-closing flag: in `<a href=x/>` it is the
	// last character of an unquoted value instead.
	if (open.charCodeAt(open.length - 2) === 47 && tagEnd - 2 >= consumedEnd) {
 ...
 * minify transforms are dropping inert comments and rewriting opening tags (one
 * space between attributes, the cheapest quoting per value); DOM-absent
 * whitespace (between the doctype and `<html>`, etc.) naturally falls away.
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		case NodeType.Element: {
			...
			if (path.selfClosing()) {
				return open !== "" ? tag : _synthesizeOpenTag(path, minify);
			}
			...
			if (open === "") {
				...
				return `${_synthesizeOpenTag(path, minify) + inner}</${name}>`;
			}
