 ...
 * @property {((code: string, isModule: boolean) => string | undefined)=} minifyJs minifies an inline `<script>` body — returning undefined keeps it as written. Printing is synchronous and every JS minifier is async, so the caller resolves the bodies first (see `htmlMinify`) and this looks them up
 */
...
// The caller's inline-`<script>` minifier (see `HtmlProcessOptions.minifyJs`).
// Set per parse: the print context carries the options every language shares,
// and a JS hook is the HTML printer's alone.
/** @type {((code: string, isModule: boolean) => string | undefined) | undefined} */
let _minifyJs;
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					// A classic script's type is absent, empty or a JavaScript MIME
					// essence, and `module` is the one other type holding script text;
					// anything else is a data block the browser never executes.
					if (
						_minifyJs !== undefined &&
						data.trim() !== "" &&
						(type === "" ||
						...
						const minified = _minifyJs(data, type === "module");
