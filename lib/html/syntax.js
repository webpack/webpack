...
const _streamFlush = (parent, openChild, visit, hold) => {
	...
	let child = _nodeFirstChildren[container];
	if (child === 0 || child === openChild) return;
	let visited = 0;
	while (child !== 0 && child !== openChild) {
		const next = _nodeNextSiblings[child];
		if (hold && next === openChild && _nodeTypes[child] === NodeType.Text) {
			break;
		}
		if (visit) _walkSubtree(child);
		visited = child;
		child = next;
	}
	if (visited === 0) return;
...
const _streamDetach = (el) => {
	...
	_nodeNextSiblings[el] = 0;
	_nodeParents[el] = 0;
	if (next === 0 && _nodeCount === el) _nodeCount = el - 1;
};
 ...
 */
const _streamSync = () => {
	// Runs per token, so bail before `_streamSafe` on the common shape: the open
	// stack is untouched and the current element holds nothing a flush would
	// take (empty, or the one trailing text node a flush holds back anyway).
	const depth = open.length;
	if (
		depth !== 0 &&
		depth === _entered.length &&
		_entered[depth - 1] === open[depth - 1]
	) {
		const first = _nodeFirstChildren[open[depth - 1]];
		if (first === 0) return;
		if (
			first === _nodeLastChildren[open[depth - 1]] &&
