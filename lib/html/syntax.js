...
import GenericSourceProcessor from "../util/SourceProcessor.js";
...
const buildHtmlAst = (source, fragmentContext, skip = EMPTY_SKIP) => {
	...
	const foreignContent = (/** @type {Token} */ t) => {
		if (t.type === TOKEN_CHAR) {
			const data = t.data.replaceAll("\0", "�");
	...
	modes.inBody = (t) => {
		if (t.type === TOKEN_CHAR) {
			if (t.data.includes("\0")) {
				t = { ...t, data: t.data.replaceAll("\0", "") };
			}
			if (t.data === "") return;
			reconstructAfe();
	...
	modes.inTableText = (t) => {
		if (t.type === TOKEN_CHAR) {
			const data = t.data.includes("\0") ? t.data.replaceAll("\0", "") : t.data;
			if (data === "") return;
	...
	walkHtmlTokens(source, 0, {
		...
		comment: (input, start, end) => {
			...
			if (e < s) e = s;
			tok.type = TOKEN_COMMENT;
			tok.data = input.slice(s, e).replaceAll("\0", "�");
			tok.start = start;
			tok.end = end;
			dispatch();
			return end;
		...
		text: (input, start, end) => {
			...
			const s = !raw.includes("\r") ? raw : raw.replaceAll(/\r\n?/g, "\n");
			...
			if (rawMode && data.includes("\0")) data = data.replaceAll("\0", "�");
...
/**
 * @typedef {import("../util/SourceProcessor.js").VisitorFn<HtmlPath>} VisitorFn
 * @typedef {import("../util/SourceProcessor.js").VisitorBucket<HtmlPath>} VisitorBucket
 * @typedef {import("../util/SourceProcessor.js").VisitorMap<HtmlPath>} VisitorMap
 * @typedef {import("../util/SourceProcessor.js").CompiledVisitorMap<HtmlPath>} CompiledVisitorMap
 */
...
export { QUOTE_NONE };
export { QUOTE_SINGLE };
export { SVG_TAG_ADJUST };
export { SourceProcessor };
...
export { decodeHtmlEntitiesWithMap };
export { parseSrcset };
export { walkHtmlTokens };
