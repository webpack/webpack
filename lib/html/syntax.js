 ...
 * @property {boolean=} dropOverriddenDeclarations forwarded to that CSS minifier with `environment`, so an inline sheet is held to the same rules as a `.css` asset (default false)
...
let _cssDropOverriddenDeclarations = false;
...
const _minifyInlineCss = (css) => {
	...
	try {
		...
		return new cssSyntax.SourceProcessor().process(css, {
			...
			dropOverriddenDeclarations: _cssDropOverriddenDeclarations
		}).code;
	} catch (_err) {
		return css;
	}
...
const printer = (path, writer) => {
	...
	_cssDropOverriddenDeclarations =
		writer.options.dropOverriddenDeclarations === true;
