...
const _fosterRunMoves = (path, table) => {
	...
	// Interleaving the run with the table's own children reads both in source
	// order, which only holds while they do not overlap: a run node written inside
	// a sibling's span has no place among them, only inside one.
	if (!blocked) {
...
const grammar = (input, visitors, writer, options) => {
	...
	try {
		...
		if (writer !== undefined) {
			...
			// Two things the printer cannot reason its way back from: a shape no
			// arrangement of tags is known to reach, and foster parenting — the one
			// insertion that puts a node where its source position does not describe,
			// so the repair that puts it back is checked rather than trusted. Keep the
			// tree to check the output against; no real document reaches either, none
			// of 967 in test/wpt.
