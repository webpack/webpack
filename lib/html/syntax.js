 ...
 * @property {(boolean | "all")=} collapseBooleanAttributes write a boolean attribute as the bare name its presence already means; `true` only the spellings the spec canonicalizes, `"all"` any value (default true)
 * @property {boolean=} normalizeAttributeValues rewrite a value into the shortest spelling its own grammar reads the same way — a `srcset`'s or `class`'s whitespace, an enumerated keyword's case, a viewport `content` list, a URL's surrounding whitespace (default true)
 * @property {boolean=} minifyStyleAttribute minify a `style` attribute's declaration list, the way an inline `<style>` is (default true)
 * @property {boolean=} removeAttributeQuotes drop the quotes around a value the tokenizer reads the same way unquoted (default true)
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", collapseBooleanAttributes?: boolean | "all", normalizeAttributeValues?: boolean, minifyStyleAttribute?: boolean, removeAttributeQuotes?: boolean }} HtmlPrintOptions
 */
...
// Which spellings of a boolean attribute collapse to the bare name (see
// `HtmlProcessOptions`), and whether the value rewrites below run at all.
/** @type {"none" | "canonical" | "all"} */
let _collapseBooleanAttributes = "canonical";
let _normalizeAttributeValues = true;
let _minifyStyleAttributes = true;
let _removeAttributeQuotes = true;
...
const _minimalAttributeValue = (value) => {
	...
	for (const bare of _removeAttributeQuotes ? [underDouble, underSingle] : []) {
...
const _rewriteAttributeValue = (element, name, raw, viewport, enumeratedOn) => {
	...
	// `sortTokenLists` is asked for on its own, so it outlives the normalization
	// it would otherwise be part of.
	if (
		!_normalizeAttributeValues &&
		name !== "style" &&
	...
	if (name === "style") {
		return _minifyStyleAttributes ? _minifyStyleAttribute(raw) : raw;
	}
	...
	for (let k = 0; k < count; k++) {
		...
		if (quoted) {
			...
			if (
				rawValue === "" ||
				(html &&
					_collapseBooleanAttributes !== "none" &&
					(_collapseBooleanAttributes === "all" ||
			...
			const minimal =
				...
				(_asciiTrim(decoded) !== "" || _asciiTrim(rawValue) === "")
					...
					: null;
			const unquotable =
				_removeAttributeQuotes && _canUnquoteAttributeValue(value);
			if (
				minimal !== null &&
...
const grammar = (input, visitors, writer, options) => {
	...
	const collapseBooleans =
		writer === undefined ? true : writer.options.collapseBooleanAttributes;
	_collapseBooleanAttributes =
		collapseBooleans === false
			? "none"
			: collapseBooleans === "all"
				? "all"
				: "canonical";
	_normalizeAttributeValues =
		writer === undefined || writer.options.normalizeAttributeValues !== false;
	_minifyStyleAttributes =
		writer === undefined || writer.options.minifyStyleAttribute !== false;
	_removeAttributeQuotes =
		writer === undefined || writer.options.removeAttributeQuotes !== false;
	...
	try {
		...
		_minifying = false;
		_collapseBooleanAttributes = "canonical";
		_normalizeAttributeValues = true;
		_minifyStyleAttributes = true;
		_removeAttributeQuotes = true;
