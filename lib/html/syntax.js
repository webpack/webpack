 ...
 * host's whitespace around it is not part of it and goes with every other
 * thing minifying drops. A body that is only whitespace keeps it — emptying
 * the element hands it to `removeEmptyElements`, and the DOM would lose what
 * the source wrote.
 * @param {string} source the nested body
 * @returns {string} the body to embed
 */
const _embeddedBody = (source) => {
	if (!_minifying) return source;
