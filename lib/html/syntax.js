	...
	modes.inHead = (t) => {
		...
		if (t.type === TOKEN_START_TAG) {
			...
			if (t.name === "template") {
				...
				const el = cur();
				_hTc[el] = _hAlloc(NodeType.DocumentFragment, 0, 0);
				return;
			}
 ...
 * @typedef {import("../util/SourceProcessor").VisitorFn<HtmlVisitableNode, HtmlAst>} VisitorFn
 * @typedef {import("../util/SourceProcessor").VisitorBucket<HtmlVisitableNode, HtmlAst>} VisitorBucket
 * @typedef {import("../util/SourceProcessor").VisitorMap<HtmlVisitableNode, HtmlAst>} VisitorMap
 * @typedef {import("../util/SourceProcessor").CompiledVisitorMap<HtmlVisitableNode, HtmlAst>} CompiledVisitorMap
 */
...
const grammar = (input, visitors, options) => {
	...
	/**
	 * Fetches the node's visitor bucket once (reused for enter + exit) and
	 * follows the sibling links directly — the SoA tree has no children arrays.
	 * @param {HtmlNodeRef} node subtree root
	 * @param {HtmlNodeRef | null} parent enclosing node
	 */
	const walk = (node, parent) => {
		...
		if (b !== undefined && b.enter.length !== 0) {
			skipFlag = false;
			const e = b.enter;
			for (let i = 0; i < e.length; i++) e[i](A, node, parent, visitorCtx);
			skip = skipFlag;
			skipFlag = false;
		}
		if (!skip) {
			if (ty === NodeType.Element) {
				...
				// the child list — walk it first so template content is not skipped.
				const tc = _hTc[node];
				if (tc !== 0) walk(tc, node);
				for (let c = _hFirst[node]; c !== 0; c = _hNext[c]) walk(c, node);
			} else if (ty === NodeType.Document || ty === NodeType.DocumentFragment) {
				for (let c = _hFirst[node]; c !== 0; c = _hNext[c]) walk(c, node);
			}
		...
		if (b !== undefined) {
			const x = b.exit;
			for (let i = 0; i < x.length; i++) x[i](A, node, parent, visitorCtx);
		}
	};
 ...
 *
 * ```
 * processor.use({ [NodeType.Element]: (api, el) => {}, [NodeType.Comment]: { enter, exit } });
 * processor.process(source);
 * ```
 * @extends {GenericSourceProcessor<HtmlVisitableNode, HtmlAst, HtmlProcessOptions>}
 */
...
/**
 * The HTML AST accessor — what every visitor receives as its first argument.
 * @typedef {typeof A} HtmlAst
 */
...
const A = {
	/** @type {(n: HtmlNodeRef) => number} */
	type: (n) => _hTy[n],
	/** @type {(n: HtmlNodeRef) => number} */
	start: (n) => _hSt[n],
	/** @type {(n: HtmlNodeRef) => number} */
	end: (n) => _hEn[n],
	// lowercased (foreign-content: adjusted) tag name
	/** @type {(n: HtmlElement) => string} */
	tagName: (n) => _hStr[n],
	/** @type {(n: HtmlElement) => number} */
	namespace: (n) => _hFl[n] & NS_MASK,
	/** @type {(n: HtmlElement) => boolean} */
	selfClosing: (n) => (_hFl[n] & FLAG_SELF_CLOSING) !== 0,
	...
	/** @type {(n: HtmlElement) => HtmlAttribute[]} */
	attributes: (n) => {
		const out = [];
	...
	/** @type {(n: HtmlElement) => number} */
	attributeCount: (n) => _hACount[n],
	// the i-th attribute of an element, as an id for the `attribute*` reads below
	/** @type {(n: HtmlElement, i: number) => HtmlAttributeRef} */
	attributeAt: (n, i) => _hAStart[n] + i,
	// linear lookup by (lowercased) name; 0 = not present
	/** @type {(n: HtmlElement, name: string) => HtmlAttributeRef} */
	findAttribute: (n, name) => _aFind(_hAStart[n], _hACount[n], name),
	/** @type {(a: HtmlAttributeRef) => string} */
	attributeName: (a) => _aName[a],
	// raw (undecoded) attribute value ("" when valueless)
	/** @type {(a: HtmlAttributeRef) => string} */
	attributeValue: (a) => _aValueOf(a),
	/** @type {(a: HtmlAttributeRef) => number} */
	attributeNameStart: (a) => _aNameStart[a],
	/** @type {(a: HtmlAttributeRef) => number} */
	attributeNameEnd: (a) => _aNameEnd[a],
	// -1 when valueless or on adoption-agency clones
	/** @type {(a: HtmlAttributeRef) => number} */
	attributeValueStart: (a) => _aValStart[a],
	/** @type {(a: HtmlAttributeRef) => number} */
	attributeValueEnd: (a) => _aValEnd[a],
	// end offset of the opening tag (after `>`)
	/** @type {(n: HtmlElement) => number} */
	tagEnd: (n) => _hTagEnd[n],
	// end offset of the tag name
	/** @type {(n: HtmlElement) => number} */
	nameEnd: (n) => _hNameEnd[n],
	// under `skip.text`, end offset of a raw-text element's body (`tagEnd` when empty)
	/** @type {(n: HtmlElement) => number} */
	contentEnd: (n) => _hCEnd[n],
	// `<template>` content fragment (0 = none)
	/** @type {(n: HtmlElement) => HtmlDocumentFragment} */
	templateContent: (n) => _hTc[n],
	// decoded text / comment data
	/** @type {(n: HtmlText | HtmlComment) => string} */
	data: (n) => _hStr[n],
	/** @type {(n: HtmlDoctype) => string} */
	doctypeName: (n) => _hStr[n],
	/** @type {(n: HtmlDoctype) => string | null} */
	doctypePublicId: (n) => _hDocPub,
	/** @type {(n: HtmlDoctype) => string | null} */
	doctypeSystemId: (n) => _hDocSys,
	// tree links (0 = none)
	/** @type {(n: HtmlNodeRef) => HtmlNodeRef} */
	firstChild: (n) => _hFirst[n],
	/** @type {(n: HtmlNodeRef) => HtmlNodeRef} */
	nextSibling: (n) => _hNext[n],
	/** @type {(n: HtmlNodeRef) => HtmlNodeRef} */
	parent: (n) => _hParent[n],
	// materialized child list — test/tooling convenience, allocates
	/** @type {(n: HtmlNodeRef) => HtmlNodeRef[]} */
	children: (n) => {
		const out = [];
