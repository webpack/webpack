...
const _minifySvgPresentation = (name, raw) => {
	...
	// would make the CSS parser read a second declaration and drop what the
	// attribute's own grammar rejects outright.
	if (raw.trim() === "" || raw.includes(";")) return raw;
	// Whatever it cannot minify it hands back as written, so the answer is always
	// this property's own declaration.
	return _minifyStyleAttribute(`${name}:${raw}`).slice(name.length + 1);
};
