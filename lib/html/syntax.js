...
const _escapeTextContent = (s, minify) => {
	...
	// nothing but bytes. `<` and `&` do have to go: they start a tag and a
	// character reference.
	if (minify) {
...
const _normalizeCommaList = (raw, internal) => {
	const items = raw.split(",").map((item) => _asciiTrim(item));
	return items
...
const _rewriteAttributeValue = (element, name, raw, viewport) => {
	...
	if (urlOn !== undefined && urlOn.has(element)) return _asciiTrim(raw);
