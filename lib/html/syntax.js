...
// nextSibling); the only heap references are the string payload and the
// attribute lists in the two side arrays. Columns are module-level and reused
...
const _hStr = [];
/** @type {HtmlAttribute[][]} attribute list per node (`EMPTY_ATTRS` when none) */
const _hAttrs = [];
...
/** @type {(el: HtmlNodeRef) => HtmlAttribute[]} */
const _attrsOf = (el) => _hAttrs[el];
...
const _hReset = () => {
	...
	_hStr.length = 0;
	_hAttrs.length = 0;
	// Keep id 0 ("no node") occupied so indexed writes stay packed.
	_hStr.push("");
	_hAttrs.push(EMPTY_ATTRS);
	_hDocPub = null;
	_hDocSys = null;
};

// Release the side arrays' heap references (strings, attribute lists) once a
// walk has consumed the tree, so the retained columns don't pin the parsed
// source until the next parse.
const _hRelease = () => {
	_hN = 0;
	_hStr.length = 0;
	_hAttrs.length = 0;
...
const buildHtmlAst = (source, fragmentContext, skip = EMPTY_SKIP) => {
	...
	const mkEl = (
		...
		_hStr[el] = tagName;
		_hAttrs[el] = attributes || EMPTY_ATTRS;
		if (pos) {
	...
	const reconstructAfe = () => {
		...
		for (; i < afe.length; i++) {
			const e = afe[i];
			const el = mkEl(_tag(e), _ns(e), cloneAttrs(_attrsOf(e)), null);
			const place = appropriatePlace();
			insertAtPlace(place, el);
			open.push(el);
			afe[i] = el;
	...
	const htmlIntegrationPoint = (/** @type {HtmlElement} */ el) => {
		if (_ns(el) === NS_MATHML && _tag(el) === "annotation-xml") {
			const enc = findAttr(_attrsOf(el), "encoding");
			if (
				enc &&
	...
	const startTagInBody = (/** @type {StartTagToken} */ t) => {
		...
		if (name === "html") {
			...
			for (const a of t.attrs) {
				if (!_attrsOf(htmlEl).some((x) => x.name === a.name)) {
					_attrsOf(htmlEl).push(a);
				}
			}
			return;
		}
		...
		if (name === "body") {
			...
			framesetOk = false;
			for (const a of t.attrs) {
				if (!_attrsOf(second).some((x) => x.name === a.name)) {
					_attrsOf(second).push(a);
				}
			}
			return;
		}
...
const cloneSubtree = (node) => {
	...
	_hAttrs[clone] = cloneAttrs(_hAttrs[node]);
...
const selectedOption = (select) => {
	...
	for (let i = options.length - 1; i >= 0; i--) {
		if (_hAttrs[options[i]].some((a) => a.name === "selected")) {
			return options[i];
		}
	}
	return options[0];
