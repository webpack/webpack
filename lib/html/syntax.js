...
// Elements whose text children serialize literally, no escaping (WHATWG raw-text
// serialization) — `textarea` / `title` are escapable, so they are not here.
const _LITERAL_TEXT_PARENTS = new Set([
	"script",
	"style",
	"xmp",
	"iframe",
...
// still begins with a newline, one is re-added so it round-trips to the same DOM.
 ...
 * URIs), this keeps newlines and U+00A0 literal — byte-lean and the same DOM.
 * @param {string} s decoded text
 * @returns {string} text-content-safe string
 */
const _escapeTextContent = (s) => {
...
// §13.1.2.4 optional end tags: the element names whose end tag may go when the
// next sibling is one of these. `<html>`, `<head>` and `<body>` are deliberately
// excluded — their end tags are omissible too, but non-browser consumers
// routinely mis-parse a document that is missing them.
const _P_FOLLOWED_BY = new Set([
	"address",
	"article",
	"aside",
	"blockquote",
...
const _OPTIONAL_END_TAG_FOLLOWERS = new Map([
	["li", new Set(["li"])],
	["dt", new Set(["dt", "dd"])],
	["dd", new Set(["dd", "dt"])],
	["rt", new Set(["rt", "rp"])],
...
// The subset that may also drop the end tag with nothing left in the parent.
// `<dt>` and `<thead>` are absent: the spec gives them no such clause.
const _OPTIONAL_END_TAG_AT_END = new Set([
	"li",
	"dd",
	"rt",
	"rp",
...
// A trailing `</p>` stays inside these: the spec's carve-out for parents whose
// content model would otherwise absorb what follows.
const _P_KEEPS_END_TAG_IN = new Set([
	"a",
	"audio",
	"del",
	"ins",
...
const _canOmitEndTag = (path, name, node) => {
	const followers = _OPTIONAL_END_TAG_FOLLOWERS.get(name);
	if (followers === undefined || path.namespace(node) !== NS_HTML) return false;
	const next = path.nextSibling(node);
	if (next === 0) {
		if (!_OPTIONAL_END_TAG_AT_END.has(name)) return false;
		...
		return !_P_KEEPS_END_TAG_IN.has(parentName) && !parentName.includes("-");
	}
	if (
	...
	return (
		...
		!_TRANSPARENT_IMPLIED_ELEMENTS.has(path.tagName(next))
	);
};
...
const _minifyInlineCss = (css) => {
	...
	try {
		...
		return new cssSyntax.SourceProcessor().process(css, { minimize: true })
			.code;
	} catch (_err) {
		return css;
	}
...
// The elements each boolean attribute is boolean *on* (§2.4.2). Elsewhere the
// same name is an ordinary attribute whose value a script may well read.
const _BOOLEAN_ATTRIBUTES = new Map([
...
const _isBooleanAttribute = (tagName, name) => {
	const elements = _BOOLEAN_ATTRIBUTES.get(name);
	if (elements === undefined) return _BOOLEAN_ATTRIBUTES.has(name);
 ...
 */
const _normalizeTokenList = (raw) => raw.trim().replace(/[\t\n\f\r ]+/g, " ");
 ...
 */
const _rewriteAttributeValue = (name, raw, viewport) => {
	if (_SRCSET_ATTRIBUTES.has(name)) return _normalizeSrcset(raw);
	if (name === "style") return _minifyStyleAttribute(raw);
	if (name === "class") return _normalizeTokenList(raw);
...
const _isViewportMeta = (path) => {
	for (const attribute of path.attributes()) {
		if (attribute.name === "name") {
			return attribute.value.trim().toLowerCase() === "viewport";
		}
	}
	return false;
};
...
const _SRCSET_ATTRIBUTES = new Set(["srcset", "imagesrcset"]);
...
/**
 * ASCII-lowercase a tag or attribute name, which is all HTML parsing folds — a
 * Unicode `toLowerCase` would rewrite names the parser leaves alone. Returns the
 * input untouched when there is nothing to fold (the common case).
 * @param {string} name source name
 * @returns {string} the name as the parser would hold it
 */
const _asciiLowerName = (name) => {
	for (let i = 0; i < name.length; i++) {
...
const _minifyOpenTag = (path, open) => {
	...
	const elementName = html ? _asciiLowerName(sourceName) : sourceName;
	let out = `<${elementName}`;
	...
	for (let i = 0; i < count; i++) {
		...
		const attributeName = html
			? _asciiLowerName(sourceAttributeName)
			: sourceAttributeName;
		...
		if (quoted) {
			...
			const value =
				html && !rawValue.includes("&")
					? _rewriteAttributeValue(attributeName, rawValue, isViewportMeta)
					: rawValue;
			...
			if (
				html &&
				(rawValue === "" || _asciiLowerName(rawValue) === attributeName) &&
	...
	switch (path.type()) {
		case NodeType.Element: {
			...
			if (tc !== 0) {
				...
				if (
					...
					_LEADING_NEWLINE_ELEMENTS.has(path.tagName())
				) {
					inner = `\n${inner}`;
				}
			}
			const name = path.tagName();
			// A `<tbody>` the parser would re-imply needs no start tag; its end tag
			// is handled by the optional-end-tag rules like any other.
			if (minify && name === "tbody" && _canOmitTbodyStart(path, path.node)) {
			...
			const close =
				...
				(minify && _canOmitEndTag(path, name, path.node))
					...
					: minify && path.namespace() === NS_HTML
						? `</${_asciiLowerName(path.closeTag().slice(2, -1))}>`
						: path.closeTag();
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && _LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				return minify &&
					path.tagName(parent) === "style" &&
					_isCssStyleElement(path, parent)
		...
		case NodeType.Doctype: {
			...
			return head === null
				? source
				: `<!doctype ${_asciiLowerName(head[1])}${source.slice(head[0].length)}`;
		}
...
const grammar = (input, visitors, writer, options) => {
	 ...
	 * post-order point: its children are already printed). `writer` undefined =
	 * walk only.
	 * @param {HtmlNodeRef} node node
	 */
	const exit = (node) => {
	...
	if (writer !== undefined) writer.take(root);
