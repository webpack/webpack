 ...
 * the adoption agency only reparents elements drawn from `open` / `afe` and the
 ...
 * `_streamSafe()` holds, so an unsafe stretch (misnested formatting elements,
 * tables, templates) simply buffers and the next catch-up sees the final tree.
 */
const _streamSync = () => {
	if (!_streamSafe()) return;
	let i = 0;
	while (i < _entered.length && i < open.length && _entered[i] === open[i]) i++;
