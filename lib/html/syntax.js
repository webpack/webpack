 ...
 * JavaScript, and an `<svg>` subtree. Returning the input unchanged declines
 ...
 * @property {EmbeddedSourceRenderer=} renderEmbeddedSource renders each nested body this document embeds — an inline `<style>`, every `style=""` (handed over as a whole stylesheet, SVG's and MathML's included), a `<script>` holding JSON or JavaScript, and an `<svg>` subtree. Replaces the built-in CSS and JSON minifiers, and is the only way inline JavaScript and SVG are reached at all
 */
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", renderEmbeddedSource?: EmbeddedSourceRenderer }} HtmlPrintOptions
 */
...
const _renderEmbedded = (source, type) => {
	try {
		return /** @type {EmbeddedSourceRenderer} */ (_renderEmbeddedSource)(
			source,
			{ type, hostType: HTML_TYPE }
		);
	} catch (_err) {
		return source;
	}
