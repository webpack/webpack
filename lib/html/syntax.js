 ...
 * @property {boolean=} collapseWhitespace collapse each run of whitespace in text to a single space, except where an ancestor renders it verbatim (default false)
...
// Whether text whitespace may be collapsed (see `HtmlProcessOptions`) — set
// per print, like `_cssEnvironment`.
let _collapseWhitespace = false;
...
const printer = (path, writer) => {
	...
	_collapseWhitespace = writer.options.collapseWhitespace === true;
	switch (path.type()) {
		...
		case NodeType.Text: {
			...
			if (minify && _collapseWhitespace && !data.includes("<")) {
				...
				if (!preformatted) {
					const collapsed = collapseWhitespaceRuns(data);
