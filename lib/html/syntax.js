		...
		switch (state) {
			...
			case STATE_CDATA_SECTION:
				...
				if (cc === CC_RIGHT_SQUARE_BRACKET) {
					...
					// Fast-forward to the next `]` or NUL, the code points with their
					// own arcs.
					pos++;
					const nextBracket = input.indexOf("]", pos);
					if (nextNul < pos) {
						nextNul = input.indexOf("\0", pos);
						if (nextNul === -1) nextNul = len;
					}
					pos = Math.min(nextBracket === -1 ? len : nextBracket, nextNul);
				}
				break;
...
// The typed-array columns grow to the largest document parsed, and are shrunk
// on release above this capacity so one pathological file cannot pin tens of MB
