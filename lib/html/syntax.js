...
// The options each `style` body is minified with, built once per print and
// per production, since `as` is the only thing that differs between the two.
/** @type {(import("../css/syntax").CssProcessOptions & { mode: "minify" }) | null} */
let _cssSheetOptions = null;
/** @type {(import("../css/syntax").CssProcessOptions & { mode: "minify" }) | null} */
let _cssBlockOptions = null;
...
const _builtinStyleBody = (css, as) => {
	try {
		...
		let options = as === BLOCK_CONTENTS ? _cssBlockOptions : _cssSheetOptions;
		if (options === null) {
			options = {
				mode: "minify",
			...
			if (as === BLOCK_CONTENTS) _cssBlockOptions = options;
			else _cssSheetOptions = options;
		}

		return new cssSyntax.SourceProcessor().process(css, options).code;
	} catch (_err) {
		return null;
	}
};
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					if (
						...
						return _renderEmbedded(body, JAVASCRIPT_TYPE);
					}
					return body;
				}
				return data;
...
const grammar = (input, visitors, writer, options) => {
	...
	try {
		...
		_cssSheetOptions = null;
		_cssBlockOptions = null;
		_emptyElements.clear();
		_streamEligible = false;
		_streaming = false;
		_docSkipped = false;
