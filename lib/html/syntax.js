...
/**
 * Collapse every run of HTML whitespace to one space. A run is never removed
 * outright: `a <b>c</b>` and `a<b>c</b>` render differently, and the printer
 * cannot see a `white-space` declaration that would make the rest significant.
...
// Whether text whitespace may be collapsed (see `HtmlProcessOptions`). Set per
// parse, like `_cssEnvironment`.
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		case NodeType.Element: {
			// A repeated `<html>` / `<body>` start tag adds its unseen attributes to
			// the open element and is itself dropped, so that element's source tag no
			// longer spells its attributes — rebuild it rather than echo a stale one.
			const open = _mergedAttrNodes.has(path.node)
			...
			// An element whose open tag was rebuilt (merged attributes) can still be
			// parser-implied and so have no source close tag to echo.
			const sourceClose = path.closeTag();
			// `</name>` spelled by the source, falling back to the element's own name
			// when there is no source end tag to echo — an element whose open tag was
			// rebuilt (merged attributes) can still be parser-implied.
			const closeName =
				_asciiLowerCase(sourceClose.slice(2, -1)).trim() || name;
			const close =
				name === "plaintext" ||
				(inner === "" && open.endsWith("/>")) ||
		...
		case NodeType.Text: {
			...
			if (minify && _collapseWhitespace && !data.includes("<")) {
				// `white-space: pre` is invisible from here, so only the elements that
				// render whitespace verbatim without any CSS are excluded — and by
				// ancestor, since the property inherits.
				let preformatted = false;
				for (let a = parent; a !== 0; a = path.parentOf(a)) {
					if (
						path.namespace(a) === NS_HTML &&
						LEADING_NEWLINE_ELEMENTS.has(path.tagName(a))
					) {
						preformatted = true;
						break;
					}
				...
				if (!preformatted) {
					const collapsed = collapseWs(data);
...
const grammar = (input, visitors, writer, options) => {
	_collapseWhitespace = options.collapseWhitespace === true;
