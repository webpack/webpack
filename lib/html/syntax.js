 ...
 * @typedef {(source: string, info: { type: string, hostType: string }) => string} EmbeddedSourceRenderer
 */
 ...
 * @property {boolean=} booleanAttributes write a boolean attribute spelled with its own name (`disabled="disabled"`) as the bare name
 * @property {boolean=} comments drop a comment no parser or server reads
 * @property {boolean=} enumeratedAttributes fold an enumerated value to the keyword it names
 * @property {boolean=} listAttributes normalize a space-, comma- or descriptor-separated list value (`class`, `rel`, `srcset`, `sizes`, the viewport `content`)
 ...
 * @property {boolean=} numericAttributes write an integer attribute the one way its rules read it
 * @property {boolean=} optionalTags leave out an optional tag other than the `<html>` / `<head>` / `<body>` shell, which is `removeImpliedTags`
 * @property {boolean=} quotes drop or re-pick an attribute value's quotes
 * @property {boolean=} urlAttributes trim the whitespace around a URL attribute
 */
 ...
 * @property {(string | RegExp)[]=} cssPreserveComments CSS's comment patterns too, handed over with `environment` — an inline `<style>` holds a stylesheet, so HTML's own `preserveComments` is not what applies inside one
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { cssTransforms?: CssTransformOptions, cssPreserveComments?: (string | RegExp)[], transforms?: HtmlTransformOptions, collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", renderEmbeddedSource?: EmbeddedSourceRenderer }} HtmlPrintOptions
 */
...
const _canOmitEndTag = (path, name, node) => {
	...
	if (!_transforms.optionalTags) return false;
...
// And which comments it keeps. CSS's own patterns, not HTML's: an inline
// `<style>` holds a stylesheet, so what is worth keeping in it is what is worth
// keeping in a `.css` asset.
/** @type {(string | RegExp)[] | undefined} */
let _cssPreserveComments;
...
/** @type {Required<HtmlTransformOptions>} */
let _transforms = _ALL_TRANSFORMS;
...
const pickTransforms = (options) =>
	GenericSourceProcessor.pickSwitches(_ALL_TRANSFORMS, options);
 ...
 * Anything it throws leaves the body untouched: a document must not fail to
 * serialize because something inside it did not parse.
...
const _renderEmbedded = (source, type) => {
	try {
		...
		)(source, { type, hostType: HTML_TYPE });
...
const _canOmitStartTag = (path, name, node, open) => {
	if (name === "tbody") {
		return _transforms.optionalTags && _canOmitTbodyStart(path, node);
	}
	if (name === "colgroup") {
		return _transforms.optionalTags && _canOmitColgroupStart(path, node);
	}
	return (
		open !== "" &&
...
const _minifyStyleBody = (css) => {
	...
	try {
		if (_renderEmbeddedSource !== undefined) {
			return _renderEmbedded(css, CSS_TYPE);
		}
		...
		return new cssSyntax.SourceProcessor().process(css, {
			...
			preserveComments: _cssPreserveComments
		}).code;
	} catch (_err) {
		return null;
	}
...
const _rewriteAttributeValue = (element, name, raw, viewport, enumeratedOn) => {
	...
	if (
		_transforms.enumeratedAttributes &&
	...
	if (SRCSET_ATTRIBUTES.has(name)) {
		return _transforms.listAttributes ? _normalizeSrcset(raw) : raw;
	}
	if (name === "style") return _minifyStyleAttribute(raw);
	const tokenListOn = _transforms.listAttributes
...
const _minifyInlineJson = (json) => {
	...
	if (_renderEmbeddedSource !== undefined) {
		return _renderEmbedded(json, JSON_TYPE);
	}
	try {
		JSON.parse(json);
	} catch (_err) {
	...
	for (let k = 0; k < count; k++) {
		...
		if (quoted) {
			...
			if (
				...
				const rendered = _renderEmbedded(document, HTML_TYPE);
			...
			const minimal =
				...
				(_asciiTrim(decoded) !== "" || _asciiTrim(rawValue) === "")
					...
					: null;
			const unquotable = _transforms.quotes && _canUnquoteAttributeValue(value);
			if (
				minimal !== null &&
...
const _keepComment = (data, source) => {
	if (!_transforms.comments) return true;
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		case NodeType.Element: {
			...
			const svgRoot =
				minify && _renderEmbeddedSource !== undefined && _isSvgRoot(path);
...
const grammar = (input, visitors, writer, options) => {
	...
	_cssPreserveComments =
		writer === undefined ? undefined : writer.options.cssPreserveComments;
	_transforms = GenericSourceProcessor.resolveSwitches(
		_ALL_TRANSFORMS,
