...
/**
 * A value the source escaped its quotes into, respelled under the other
 * delimiter so they can stand literal: `alt="say &quot;hi&quot;"` carries the
 * same value as `alt='say "hi"'` five bytes shorter per escape. Null when the
 * value holds an apostrophe too, where the swap escapes no less.
 * @param {string} value the decoded attribute value
 * @returns {string | null} the respelled value, quotes included, or null
 */
const _respellAttributeValue = (value) => {
 ...
 * way, so `removeEmptyAttributes` may drop it. Scoped to the elements the spec
 * defines it on: anywhere else the same spelling is an author attribute whose
 * meaning is a script's, so `<x-foo rel="">` keeps it.
...
const _minifyOpenTag = (path, open) => {
	...
	for (let k = 0; k < count; k++) {
		...
		if (quoted) {
			...
			// Guarded on the escape, so the decode stays off the hot path — and
			// tried before unquoting, which keeps every `&quot;` it leaves bare.
			const respelled =
				quote === CC_QUOTATION_MARK &&
				value === rawValue &&
				rawValue.includes("&quot;")
					? _respellAttributeValue(decodeEntities(rawValue, true))
					: null;
			const unquotable = _canUnquoteAttributeValue(value);
			if (
				respelled !== null &&
				respelled.length < (unquotable ? value.length : rawValue.length + 2)
			) {
				out += ` ${attributeName}=${respelled}`;
				unquotedTail = false;
				consumedEnd = valueEnd;
				continue;
			}
