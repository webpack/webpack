		...
		if (quoted) {
			...
			const value =
				html && !rawValue.includes("&")
					...
					: rawValue;
			// Dropped before any rewrite: the value only restates the default, or
...
const _streamTags = (path, minify) => {
	// Folded into the `<style>` before it, which already printed its text.
	if (minify && _absorbedStyles !== null && _absorbedStyles.has(path.node)) {
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		case NodeType.Element: {
			// Folded into the `<style>` before it, which printed the whole run.
			if (
				minify &&
				_absorbedStyles !== null &&
				_absorbedStyles.has(path.node)
			) {
				return "";
			}
