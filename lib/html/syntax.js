...
const _minifyOpenTag = (path, open) => {
	...
	for (let k = 0; k < count; k++) {
		...
		if (verbatim) {
			...
			// A space of its own where the source wrote none: the quotes this print
			// dropped are what kept the value off the template's next byte.
			if (
				unquotedTail &&
				(separator === "" || !isSpace(separator.charCodeAt(0)))
			) {
				separator = ` ${separator}`;
			}
			out += separator;
	...
	if (verbatim) {
		...
		// The same fusing rule as between two attributes: a `/` written right after
		// an unquoted value is read as the last character of it.
		return unquotedTail && tail !== "" && !isSpace(tail.charCodeAt(0))
			? `${out} ${tail}>`
			: `${out}${tail}>`;
	}
