 ...
 * @property {boolean=} trimUrlAttributes trim the whitespace around a URL attribute
 */
...
const _ALL_TRANSFORMS = Object.freeze({
	...
	trimUrlAttributes: true
});

// The same, as a copy: `_transforms` is read once per token, and `Object.freeze`
// gives the table above a hidden class of its own — so handing that one out
// where nothing is set, and a copy where something is, would make every one of
// those reads see two shapes.
const _DEFAULT_TRANSFORMS = GenericSourceProcessor.resolveSwitches(
	_ALL_TRANSFORMS,
	undefined
);
...
const _rewriteAttributeValue = (element, name, raw, viewport, enumeratedOn) => {
	...
	const urlOn = _transforms.trimUrlAttributes
