...
/** @import { CssEnvironment, CssProcessOptions } from "../css/syntax" */
 ...
 * @property {boolean=} keepEndTags print an empty element's end tag rather than the empty-element tag XML lets it take, for a payload whose serialization is read back — a `data:` URL's (default false)
 */
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", xml?: boolean, keepEndTags?: boolean }} HtmlPrintOptions
 */
...
const _canOmitEndTag = (path, name, node) => {
	...
	if (
		...
		return _canOmitShellEnd(path, name, node);
	}
...
let _xml = false;
// Where the serialization itself is read back — a `data:` URL's payload —
// an empty element keeps its end tag rather than gaining a `/>`.
let _keepEndTags = false;
...
const _canOmitStartTag = (path, name, node, open) => {
	...
	if (name === "tbody") return _canOmitTbodyStart(path, node);
	if (name === "colgroup") return _canOmitColgroupStart(path, node);
	return (
		open !== "" &&
		_removeImpliedTags !== "none" &&
...
const _STYLE_WRAPPER_OPEN = "a{";
const _STYLE_WRAPPER_CLOSE = "}";

/**
 * Whether a `style` attribute's value would not survive being wrapped in a rule
 * — either it holds a `}` outside every string, comment and bracket, which
 * closes the wrapper early and drops the declarations after it, or it ends
 * inside one of those, which swallows the wrapper's own `}`. The attribute is a
 * declaration list: such a `}` is part of a bad declaration and what follows it
 * still counts, and what the value leaves open the attribute's end closes.
 * @param {string} value the raw attribute value
 * @returns {boolean} true when the value cannot be wrapped in a rule
 */
const _escapesStyleWrapper = (value) => {
	let depth = 0;
	for (let i = 0; i < value.length; i++) {
		const c = value.charCodeAt(i);
		if (c === 0x5c) {
			i++;
			// The escape the value ran out of takes the wrapper's own `}` next.
			if (i >= value.length) return true;
		} else if (c === 0x22 || c === 0x27) {
			let closed = false;
			for (i++; i < value.length; i++) {
				const inside = value.charCodeAt(i);
				if (inside === 0x5c) {
					i++;
				} else if (inside === c) {
					closed = true;
 ...
 * inside a throwaway rule, which is what gives the list the duplicate-drop and
 * shorthand merges a rule's block gets.
...
const _minifyStyleAttribute = (raw) => {
	...
	try {
		const cssSyntax = _cssSyntax || (_cssSyntax = require("../css/syntax"));
		out = new cssSyntax.SourceProcessor().process(
			`${_STYLE_WRAPPER_OPEN}${raw}${_STYLE_WRAPPER_CLOSE}`,
			{
				mode: "minify",
				environment: _cssEnvironment,
				convertLengthUnits: _cssConvertLengthUnits,
	...
	// Only the block is wanted, and only when the whole print is that one rule:
	// a block that minified to nothing takes the rule with it, and a kept comment
	// is hoisted out behind it, neither of which leaves a body to read.
	if (
		!out.startsWith(_STYLE_WRAPPER_OPEN) ||
		!out.endsWith(_STYLE_WRAPPER_CLOSE)
	) {
		return raw;
	}
	return out.slice(
		_STYLE_WRAPPER_OPEN.length,
		out.length - _STYLE_WRAPPER_CLOSE.length
	);
};
...
const _minifySvgPresentation = (name, raw) => {
	...
	const value = _minifyStyleAttribute(key).slice(name.length + 1);
...
const _rewriteAttributeValue = (element, name, raw, viewport, enumeratedOn) => {
	...
	if (ENUMERATED_ATTRIBUTE_NAMES.has(name)) {
...
const grammar = (input, visitors, writer, options) => {
	...
	_keepEndTags = writer !== undefined && writer.options.keepEndTags === true;
