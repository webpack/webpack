...
const adoptionAgency = (
	...
	while (outer < 8) {
		...
		if (furthestIdx === -1) {
			while (open.length > openIdx) popOpen();
			afeRemoveAt(fmtIdx);
			return true;
		}
 ...
 * @property {CssEnvironment=} environment what the target can read, forwarded to the CSS minifier this runs over an inline `<style>` and every `style=""`
 * @property {boolean=} convertLengthUnits forwarded to that CSS minifier with `environment`: a length may be rewritten into a shorter unit it exactly equals (default false)
 ...
 * @property {boolean=} removeEmptyAttributes drop `class` / `id` / `style` / `dir` when empty, which the spec reads as their absence (default false)
 ...
 * @property {(string | RegExp)[]=} preserveComments patterns naming comments to keep, on top of the ones minifying always keeps
 * @property {boolean=} sortAttributes print an element's attributes in name order, which nothing in HTML reads (default false)
 * @property {boolean=} sortClassNames print a `class` list in token order, which nothing in CSS reads (default false)
 * @property {("smart" | "all")=} removeRedundantAttributes drop an attribute whose value is the element's own default; `"all"` also drops the spec defaults a selector can match (default `"smart"`)
 */

/** @typedef {import("../util/SourceProcessor").PrintContext<HtmlPath, HtmlNodeRef>} PrintContext */
 ...
 * the §13.3 serialization is followed to the letter. Unlike the exported
 * `escapeText` (which also numerically encodes newlines and U+00A0 for
 * single-line `data:` URIs), this keeps newlines and U+00A0 literal — byte-lean
 * and the same DOM.
 * @param {string} s decoded text
...
/**
 * Whether minifying drops this node outright, so a rule about what sits beside
 * it has to look past it. Only whitespace directly under `<head>` / `<html>`: no
 * formatting context renders it, which is why the text printer drops it. Body
 * whitespace does render, so it still blocks what it sits beside.
 * @param {HtmlPath} path the accessor
 * @param {HtmlNodeRef} node node (0 = none)
 * @returns {boolean} true when nothing of it reaches the output
 */
const _isDroppedWhitespace = (path, node) => {
	if (node === 0 || path.type(node) !== NodeType.Text) return false;
	...
	return (
		(parentName === "head" || parentName === "html") && isAllWs(path.data(node))
	);
};

/**
 * The next sibling that will still be there once minifying is done.
 * @param {HtmlPath} path the accessor
 * @param {HtmlNodeRef} node node
 * @returns {HtmlNodeRef} the sibling, or 0
 */
const _nextPrintedSibling = (path, node) => {
	let next = path.nextSibling(node);
	while (_isDroppedWhitespace(path, next)) next = path.nextSibling(next);
	return next;
};

/**
 * Whether an attribute-less `<html>` / `<head>` / `<body>` may print with no tags
 * at all, because the parser re-implies it in exactly that place. §13.1.2.4
 * gives each start tag its own condition; the end tags ride on
 * `_canOmitEndTag`. Both halves have to hold — omitting one and printing the
 * other is legal but not worth the extra reasoning for the bytes it saves.
 * @param {HtmlPath} path the accessor
 * @param {string} name lowercased element name
 * @param {HtmlElement} node the element
 * @returns {boolean} true when both its tags may go
 */
const _canOmitStructuralTags = (path, name, node) => {
	if (path.attributeCount(node) !== 0 || path.namespace(node) !== NS_HTML) {
		return false;
	}
	let first = path.firstChild(node);
	while (_isDroppedWhitespace(path, first)) first = path.nextSibling(first);
	const firstType = first === 0 ? -1 : path.type(first);
	if (name === "html") {
		// "unless the first thing inside is a comment"
		if (firstType === NodeType.Comment) return false;
	} else if (name === "head") {
		// "if the element is empty, or if the first thing inside is an element"
		if (first !== 0 && firstType !== NodeType.Element) return false;
	} else if (name === "body") {
		// "if the element is empty, or if the first thing inside is not ASCII
		// whitespace or a comment, except in front of `BODY_START_TAG_BLOCKERS`"
		if (firstType === NodeType.Comment) return false;
		if (firstType === NodeType.Text && isAllWs(path.data(first))) return false;
		if (
			firstType === NodeType.Element &&
			path.namespace(first) === NS_HTML &&
			BODY_START_TAG_BLOCKERS.has(path.tagName(first))
		) {
			return false;
		}
	} else {
	...
	return _canOmitEndTag(path, name, node);
};

/**
 * Whether an element's end tag may be omitted (§13.1.2.4). Reads the AST, not
 * the emitted text, so an intervening comment keeps the tag even when
 * minification later drops it — conservative, never wrong. Whitespace that
 * minifying is certain to drop is the one thing it does read past.
 * @param {HtmlPath} path the accessor positioned on the element
...
const _canOmitEndTag = (path, name, node) => {
	...
	if (OPTIONAL_END_TAG_UNLESS_COMMENT.has(name)) {
		const trailing = path.nextSibling(node);
		// Whitespace may follow these two: the parser puts it back inside.
		return trailing === 0 || path.type(trailing) !== NodeType.Comment;
	}
	if (OPTIONAL_END_TAG_UNLESS_TRAILING_NODE.has(name)) {
		// Whitespace or a comment behind it would move inside once the tag goes;
		// an element closes it through the insertion mode instead. Whitespace that
		// is itself dropped cannot move anywhere, so `</head>` reads past it.
...
/**
 * The node before `node` in its parent's child chain, or 0 — the accessor only
 * walks forward.
 * @param {HtmlPath} path the accessor
 * @param {HtmlNodeRef} node the element
 * @returns {HtmlNodeRef} the preceding sibling, or 0
 */
const _previousSibling = (path, node) => {
	let previous = 0;
	for (
...
const _canOmitTbodyStart = (path, node) => {
	...
	const previous = _previousSibling(path, node);
...
// Which attribute defaults may be dropped (see `HtmlProcessOptions`). `"all"`
// is opt-in: it drops defaults a selector can match.
/** @type {"smart" | "all"} */
let _removeRedundantAttributes = "smart";
...
// Patterns naming comments to keep whatever the inert-comment rule says (see
// `HtmlProcessOptions`), or undefined when the author named none.
/** @type {RegExp[] | undefined} */
let _preserveComments;
...
// A `style` attribute holds a declaration list, so it is minified inside a
// throwaway rule; anything that did not stay one rule escaped the wrapper.
const _STYLE_ATTRIBUTE_WRAPPER = /^a\{([^{}]*)\}$/;

/**
 * Minify a `style` attribute's declaration list with webpack's CSS minifier.
...
const _minifyStyleAttribute = (raw) => {
	...
	try {
		const cssSyntax = _cssSyntax || (_cssSyntax = require("../css/syntax"));
		out = new cssSyntax.SourceProcessor().process(`a{${raw}}`, {
			mode: "minify",
			environment: _cssEnvironment,
			convertLengthUnits: _cssConvertLengthUnits
		}).code;
	...
	const wrapped = _STYLE_ATTRIBUTE_WRAPPER.exec(out);
	return wrapped === null ? raw : wrapped[1];
};
 ...
 * which matches the content attribute, with nothing to match.
 ...
 */
const _isRedundantAttribute = (tagName, name, value) => {
	const markers = REDUNDANT_TYPE_ATTRIBUTES[tagName];
	const defaults =
		_removeRedundantAttributes === "all"
			? REDUNDANT_DEFAULT_ATTRIBUTES[tagName]
			: undefined;
	// Runs per printed attribute, and almost none is on an element either table
	// names — leave that path without a scan or an allocation.
	if (markers === undefined && defaults === undefined) return false;
...
/**
 * Whether an attribute carries nothing and the spec reads its absence the same
 * way, so `removeEmptyAttributes` may drop it.
 * @param {string} name lowercased attribute name
 * @param {string} value the attribute's raw value
 * @returns {boolean} true when the attribute may be dropped
 */
const _isRemovableEmptyAttribute = (name, value) =>
...
const _isRemovableEmptyElement = (path, element) =>
	...
	_nodeFirstChildren[element] === 0 &&
	path.attributeCount(element) === 0 &&
	path.templateContent(element) === 0 &&
	path.namespace(element) === NS_HTML &&
	// Void is otherwise disqualifying — a `<br>` is childless and still renders.
	// A void element that belongs in the head is the exception: with no
	// attributes it states nothing.
 ...
 * and drop a token already in it. The token set is what the DOM reads — the
 * ordered set parser builds a set, so a repeat was never a second token — and it
 * is unchanged. Order is not: the first spelling of each token stays put.
 * @param {string} raw raw (undecoded) value
 * @param {boolean} sort whether to print the tokens in order (`sortClassNames`)
 * @returns {string} the collapsed list
 */
const _normalizeTokenList = (raw, sort) => {
	const collapsed = _asciiTrim(raw).replace(/[\t\n\f\r ]+/g, " ");
	if (!collapsed.includes(" ")) return collapsed;
	const tokens = collapsed.split(" ");
	let duplicated = false;
	for (let i = 1; i < tokens.length && !duplicated; i++) {
		duplicated = tokens.indexOf(tokens[i]) !== i;
	}
	if (!duplicated && !sort) return collapsed;
	const unique = duplicated ? [...new Set(tokens)] : tokens;
	return (sort ? unique.sort() : unique).join(" ");
};
...
/**
 * The keywords an attribute enumerates on this element, or `undefined` when it
 * enumerates none. An element's own entry wins over the global one, so a name
 * that means something else on one element is not folded by the other's rules.
 * @param {string} element lowercased element name
 * @param {string} name lowercased attribute name
 * @returns {Set<string> | undefined} the keywords, or undefined
 */
const _enumeratedKeywords = (element, name) => {
	const own = ENUMERATED_KEYWORDS[element];
	const scoped = own === undefined ? undefined : own[name];
	return scoped === undefined ? ENUMERATED_KEYWORDS["*"][name] : scoped;
};
 ...
 */
const _rewriteAttributeValue = (element, name, raw, viewport) => {
	// An enumerated value matches ASCII-case-insensitively, so folding one the
	// spec names changes nothing the DOM reads. Checked first because the lookup
	// is what tells us to scan the value at all.
	const enumerated = _enumeratedKeywords(element, name);
	if (enumerated !== undefined) {
		const lowered = _asciiLowerCase(_asciiTrim(raw));
		// A value nobody enumerated is left exactly as written: `target` names a
		// browsing context, and `type` on a custom element means what it says.
		if (enumerated.has(lowered)) return lowered;
	}
	if (SRCSET_ATTRIBUTES.has(name)) return _normalizeSrcset(raw);
	if (name === "style") return _minifyStyleAttribute(raw);
	const tokenListOn = TOKEN_LIST_ATTRIBUTES.get(name);
	if (
		tokenListOn !== undefined &&
		(tokenListOn === null || tokenListOn.has(element))
	) {
		// Only `class` sorts: the other lists are sets too, but `ping` is the
		// order the requests go out in, and `class` is where the repetition a
		// compressor pays for actually is.
		return _normalizeTokenList(raw, _sortClassNames && name === "class");
	}
...
const _mergedStyleRun = (path, element, css) => {
	...
	for (
		...
		if (body === null || _SHEET_LEADING_AT_RULE_REGEXP.test(body)) break;
...
/**
 * The element's attribute indices in name order, for `sortAttributes`. Nothing
 * in HTML reads attribute order, so a stable spelling across pages is worth the
 * repeats it hands a compressor.
 * @param {HtmlPath} path the accessor positioned on the element
 * @param {number} count its attribute count
 * @returns {number[]} the indices, sorted by attribute name
 */
const _sortedAttributes = (path, count) => {
	const order = [];
	for (let i = 0; i < count; i++) order.push(i);
	return order.sort((one, other) => {
		const a = path.attributeName(path.attributeAt(one));
		const b = path.attributeName(path.attributeAt(other));
		if (a === b) return 0;
		return a < b ? -1 : 1;
	});
};
...
const _minifyOpenTag = (path, open) => {
	...
	for (let k = 0; k < count; k++) {
		...
		if (valueStart === -1) {
			...
			if (html && _isRemovableEmptyAttribute(attributeName, "")) {
				consumedEnd = nameEnd;
				continue;
			}
			out += ` ${attributeName}`;
		...
		if (
			html &&
			(_isRedundantAttribute(elementName, attributeName, rawValue) ||
				_isRemovableEmptyAttribute(attributeName, rawValue))
		) {
		...
		if (quoted) {
			...
			const value =
				html && !rawValue.includes("&")
					...
					: rawValue;
			// A boolean attribute's presence is its value, so the canonical spellings
			// (`disabled`, `disabled=""`) carry nothing. Any other value is left as
			// written -- it is not the form the spec canonicalizes.
			if (
				html &&
				(rawValue === "" || _asciiLowerCase(rawValue) === attributeName) &&
			...
			if (_canUnquoteAttributeValue(value)) {
...
const _keepComment = (data, source) => {
	// An author-supplied pattern outranks the inert-comment rule: a license
	// banner or a server-side marker is inert to a parser and load-bearing
	// anyway.
	if (_preserveComments !== undefined) {
		for (let i = 0; i < _preserveComments.length; i++) {
			if (_preserveComments[i].test(data)) return true;
		}
	}
...
const _streamTags = (path, minify) => {
	...
	// A `<tbody>` / `<colgroup>` the parser would re-imply prints without its
	// start tag, whatever the source spelled.
	if (
		minify &&
		(name === "tbody"
			? _canOmitTbodyStart(path, path.node)
	...
	// A source-spelled `<html>` / `<head>` / `<body>` the parser would re-imply
	// takes the same path as one that was never spelled, safety net included.
	if (
		source !== "" &&
		minify &&
		_canOmitStructuralTags(path, name, path.node)
	) {
		return {
			open: "",
			close: "",
...
const printer = (path, writer) => {
	...
	const collapse = writer.options.collapseWhitespace;
	_collapseWhitespace =
		collapse === true
			? "conservative"
	...
	switch (path.type()) {
		case NodeType.Element: {
			...
			if (path.selfClosing()) {
				return open !== "" ? tag : _synthesizeOpenTag(path);
			}
			...
			const name = path.tagName();
			// A `<tbody>` / `<colgroup>` the parser would re-imply needs no start tag;
			// its end tag is handled by the optional-end-tag rules like any other.
			if (
				minify &&
				(name === "tbody"
					? _canOmitTbodyStart(path, path.node)
			...
			// A source-spelled `<html>` / `<head>` / `<body>` the parser would
			// re-imply is transparent too, on the same terms as an implied one.
			if (
				open === "" ||
				(minify && _canOmitStructuralTags(path, name, path.node))
			) {
				...
				if (
					...
					const leading = inner.charCodeAt(0);
					if (
						(name === "body" || name === "html") &&
						(leading === 0x09 ||
							leading === 0x0a ||
...
const grammar = (input, visitors, writer, options) => {
	...
	_removeRedundantAttributes =
		(writer !== undefined && writer.options.removeRedundantAttributes) ||
		"smart";
	_removeEmptyAttributes =
	...
	_preserveComments =
		preserve === undefined || preserve.length === 0
			? undefined
			: preserve.map((pattern) =>
					typeof pattern === "string" ? new RegExp(pattern) : pattern
				);
	_sortAttributes =
		writer !== undefined && writer.options.sortAttributes === true;
	_sortClassNames =
		writer !== undefined && writer.options.sortClassNames === true;
...
const A = {
	 ...
	 * @param {HtmlElement=} n element
	 * @returns {string} closing-tag text
	 */
	closeTag(n = _currentNode) {
		return `</${_htmlSource.slice(_nodeStarts[n] + 1, _nodeNameEnds[n])}>`;
	},
	/**
	 * @param {HtmlElement=} n element
