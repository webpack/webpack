...
const printer = (path, writer) => {
	switch (path.type()) {
		...
		case NodeType.Text: {
			...
			// Text that decoded to itself re-tokenizes to itself, so emit the source
			// bytes rather than escaping `<`/`>` — that keeps template placeholders
			// (`<%= x %>`, `{{ x }}`) intact in HTML webpack only passes through.
			// A length mismatch means the node was merged or foster-parented and its
			// range no longer describes `data`; a trailing `<` could fuse with a
			// following sibling once a comment between them is dropped.
			if (
