	...
	while (stack.length !== 0) {
		...
		if (type === NodeType.Element) {
			...
			for (const a of A.attributes(node)) {
				...
				// different quote and a different escape and still be the same
				// attribute — with a boolean spelled by its own name as the bare
				// presence the printer writes it back as.
				attributes.push(
					`${name}=${
						html &&
