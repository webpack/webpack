...
const _minifyOpenTag = (path, open) => {
	...
	for (let i = 0; i < count; i++) {
		...
		if (quoted) {
			...
			// A character reference would decode to something the srcset grammar
			// reads differently, so only a reference-free value is normalized.
			const value =
				html && _SRCSET_ATTRIBUTES.has(attributeName) && !rawValue.includes("&")
					? _normalizeSrcset(rawValue)
					: rawValue;
