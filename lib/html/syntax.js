 ...
 * @property {(boolean | "smart" | "all")=} tagOmission how much of the `<html>` / `<head>` / `<body>` shell §13.1.2.4 lets the parser imply may be left out: `"smart"` leaves out only the `<html>` start tag, `true` (or `"all"`) all six, `false` none (default `"smart"`)
 */
 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits"> & { collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortClassNames?: boolean, tagOmission?: boolean | "smart" | "all" }} HtmlPrintOptions
 */
...
const _canOmitShellStart = (path, name, node) => {
	...
	if (first === 0) return name === "html" || _tagOmission === "all";
	const type = path.type(first);
	if (type === NodeType.Comment) return false;
	if (name === "html") return true;
	if (_tagOmission !== "all") return false;
...
const _streamTags = (path, minify) => {
	...
	// start tag, whatever the source spelled. A shell element the source never
	// spelled falls through instead: it has no end tag to keep either, and the
	// transparent-implied path below already prints nothing for it.
	if (
		minify &&
		(name === "tbody"
...
const grammar = (input, visitors, writer, options) => {
	...
	_tagOmission =
		tagOmission === undefined || tagOmission === "smart"
			? "smart"
			: tagOmission === false
				? "none"
				: "all";
