...
const _minifyOpenTag = (path, open) => {
	...
	for (let k = 0; k < count; k++) {
		...
		if (quoted || html || attributeName === "style" || rawValue.includes("&")) {
			...
			// A rewrite changed what the value says, so the source's own spelling
			// cannot stand — and with it out of the running, the shortest wins.
			if (rewritten !== null && rewritten !== spellable) {
				const shortest = _transforms.normalizeAttributeQuotes
					? _minimalAttributeValue(rewritten)
					: null;
				if (shortest === null) {
					out += _writeQuotedAttribute(
						attributeName,
					...
					const opened = shortest.charCodeAt(0);
					out += ` ${attributeName}=${shortest}`;
					unquotedTail =
						opened !== CC_QUOTATION_MARK && opened !== CC_APOSTROPHE;
				}
				consumedEnd = valueEnd;
				continue;
			}
