...
// Set per parse rather than read off the print context: that context carries the
// options every language shares, and a JS hook is the HTML printer's alone.
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					// essence; `module` is the one other type that holds script text.
					// Anything else is a data block the browser never executes, so its
					// body is not JavaScript.
					if (
						_minifyJs !== undefined &&
						data.trim() !== "" &&
						(type === "" ||
