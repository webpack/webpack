...
// WHY: the longest WHATWG named entity is 32 chars with its `;`
// (`CounterClockwiseContourIntegral;`), 31 without. Capping the tokenizer's
// reference run and the decoder's longest-prefix backtrack at that keeps `&`
// followed by thousands of alphanumerics linear-time.
...
const tokenize = (input, pos = 0, callbacks = {}) => {
		...
		switch (state) {
			...
			case STATE_END_TAG_OPEN:
				...
				if (cc === CC_GREATER_THAN) {
					// WHY: `>` is a missing-end-tag-name parse error, switch to data. No
					// token is emitted, so `</>` is dropped rather than kept in the text
					// run: a browser renders nothing for it, and keeping it would
	...
	if (
		...
		// WHY: a deliberate deviation from the spec's per-character emission
...
let source = "";
// WHY: whole-document properties, so a substring cannot hold what the input
// does not, and decoding introduces neither — a numeric reference to 0 yields
// U+FFFD, and CR normalization runs on the raw slice first. Defaulting to
// `true` leaves an unset flag costing no more than the old scan.
let inputHasCr = true;
