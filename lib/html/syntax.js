 ...
 */
const _canOmitEndTag = (path, name) => {
	const followers = _OPTIONAL_END_TAG_FOLLOWERS.get(name);
	if (followers === undefined || path.namespace() !== NS_HTML) return false;
...
const _minifyOpenTag = (path, open) => {
	...
	for (let i = 0; i < count; i++) {
		...
		if (quoted) {
			...
			if (_canUnquoteAttributeValue(rawValue)) {
				// The slice ends at the opening quote, so it already carries `name=`.
				out += ` ${open.slice(nameStart - tagStart, valueStart - 1 - tagStart)}${rawValue}`;
				unquotedTail = true;
				consumedEnd = valueEnd;
				continue;
			}
		}
		out += ` ${open.slice(nameStart - tagStart, valueEnd - tagStart)}`;
		unquotedTail = !quoted;
		consumedEnd = valueEnd;
	}
	...
	// last character of an unquoted value instead. After one of those the `/`
	// needs a space, or it would fuse into the value.
	...
	switch (path.type()) {
		case NodeType.Element: {
			...
			const close =
				name === "plaintext" ||
				(inner === "" && open.endsWith("/>")) ||
				(minify && _canOmitEndTag(path, name))
					? ""
					: path.closeTag();
