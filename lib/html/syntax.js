...
/** @typedef {import("../util/SourceProcessor").EmbeddedSourceRenderer} EmbeddedSourceRenderer */

// A `<script>` body that is JSON is not a module source type, so it has no
// constant in `ModuleSourceTypeConstants` — it names itself here.
...
// replaces the built-in CSS/JSON minifiers below and is the only way inline
// JavaScript is reached — webpack ships no JS minifier of its own.
/** @type {EmbeddedSourceRenderer | undefined} */
let _renderEmbeddedSource;

/**
 * Hand one nested body to the caller's renderer, or to `builtin` when there is
 * none. Anything the renderer throws leaves the body untouched: a document must
 * not fail to serialize because something inside it did not parse.
 * @param {string} source the nested body
 * @param {string} type its language, e.g. `"css"` / `"javascript"` / `"json"`
 * @param {((source: string) => string)=} builtin webpack's own minifier for it
 * @returns {string} the rendered body, or the original
 */
const _renderEmbedded = (source, type, builtin) => {
	if (source.trim() === "") return source;
	try {
		if (_renderEmbeddedSource !== undefined) {
			return _renderEmbeddedSource(source, { type, hostType: HTML_TYPE });
		}
		return builtin === undefined ? source : builtin(source);
	} catch (_err) {
		return source;
	}
};
...
/**
 * Minify a `<style>` body — by default with webpack's own CSS minifier, the one
 * that runs on CSS assets, so an inline sheet is held to the same rules.
 * @param {string} css the element's text
 * @returns {string} the minified text
 */
const _minifyInlineCss = (css) =>
	_renderEmbedded(css, CSS_TYPE, _builtinMinifyCss);

/**
 * webpack's own CSS minifier, the default a caller replaces by passing
 * `renderEmbeddedSource`.
 * @param {string} css a stylesheet
 * @returns {string} the minified stylesheet
 */
const _builtinMinifyCss = (css) => {
...
/**
 * Minify a `style` attribute's declaration list.
...
const _minifyStyleAttribute = (raw) => {
	...
	const out = _renderEmbedded(`a{${raw}}`, CSS_TYPE, _builtinMinifyCss);
...
const _minifyInlineJson = (json) =>
	_renderEmbedded(json, JSON_TYPE, _builtinMinifyJson);

/**
 * @param {string} json a `<script>` body
 * @returns {string} the stripped body
 */
const _builtinMinifyJson = (json) => {
	try {
		JSON.parse(json);
	} catch (_err) {
...
const printer = (path, writer) => {
	...
	_renderEmbeddedSource = writer.options.renderEmbeddedSource;
	switch (path.type()) {
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					if (JAVASCRIPT_SCRIPT_TYPES.has(type)) {
...
const grammar = (input, visitors, writer, options) => {
	...
	_visitors = visitors;
	_writer = writer;
	// Bound here, not only in `printer`: an element's open tag — and so every
	// `style=""` in it — is serialized by `_streamTags` during the walk, before
	// that element's printer runs.
	if (writer !== undefined) {
		_cssEnvironment = writer.options.environment;
		_cssConvertLengthUnits = writer.options.convertLengthUnits === true;
		_collapseWhitespace = writer.options.collapseWhitespace === true;
