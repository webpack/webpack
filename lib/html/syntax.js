...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case NodeType.Text: {
			...
			// TODO `renderInlineJavascript` / `renderInlineCss` options, to reach an
			// inline `<script>` and to let a caller replace the hardcoded CSS one.
			// Options, not compilation hooks: this runs in the minimizer's worker.
