 ...
 * @typedef {Pick<CssProcessOptions, "environment" | "convertLengthUnits" | "rewriteCustomProperties"> & { cssTransforms?: CssTransformOptions, transforms?: HtmlTransformOptions, collapseWhitespace?: boolean | "conservative" | "smart" | "all", mergeStyles?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: boolean | "smart" | "all", sortAttributes?: boolean, sortTokenLists?: boolean, removeImpliedTags?: boolean | "smart" | "all", renderEmbeddedSource?: EmbeddedSourceRenderer }} HtmlPrintOptions
 */
...
/**
 * The rewrites a print makes, each on unless the option says otherwise. Built
 * once per print, so no read of it asks whether the option was given.
 * @param {HtmlTransformOptions | undefined} options the `transforms` option
 * @returns {Required<HtmlTransformOptions>} every entry resolved
 */
const _transformsFrom = (options) => {
	if (options === undefined) return _ALL_TRANSFORMS;
	/** @type {EXPECTED_ANY} */
	const out = {};
	for (const name of Object.keys(_ALL_TRANSFORMS)) {
		out[name] = /** @type {EXPECTED_ANY} */ (options)[name] !== false;
	}
	return out;
};
