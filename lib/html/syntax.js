 ...
 */
const _writeAttribute = (attributeName, value, rawValue, decoded, quoted) => {
	// `normalizeAttributeQuotes` holds back the delimiters, not the spelling
	// inside them, so with it off the decoded value is written under a quote.
	const minimal =
		decoded === null
			? null
			: _transforms.normalizeAttributeQuotes
				? _minimalAttributeValue(decoded)
				: `"${escapeAttribute(decoded, CC_QUOTATION_MARK, true)}"`;
	const unquotable =
	...
	for (let k = 0; k < count; k++) {
		...
		if (quoted || html || attributeName === "style" || rawValue.includes("&")) {
			...
			const writeStyle = (minified) =>
				declarations !== null
					? _writeAttribute(attributeName, rawValue, rawValue, minified, quoted)
					: _writeAttribute(
							attributeName,
							minified,
							rawValue,
			...
			const written = isStyle
				...
				: _writeAttribute(attributeName, value, rawValue, rewritten, quoted);
			...
			if (rewritten !== null && rewritten !== spellable) {
				out += ` ${attributeName}="${escapeAttribute(rewritten, CC_QUOTATION_MARK, true)}"`;
				unquotedTail = false;
				consumedEnd = valueEnd;
				continue;
			}
