 ...
 */
/**
 * Remove an element from the middle of the open element stack. Only a removal
 * that takes an *entered* element out from under still-open descendants breaks
 * the streamed walk: the element leaves the stack without its subtree ending,
 * which the entered/open comparison cannot express. Removing the top is just a
 * pop, and an element the walk never entered costs it nothing — neither needs
 * to stop streaming.
...
const removeOpenAt = (index) => {
	...
	if (_streaming && index < open.length && _entered.includes(el)) {
		_streamHalted = true;
	}
};
...
const dispatch = () => {
	...
	if (_streaming && (_openStackChanged || _openSpliced)) {
...
const parseHtml = (input, pos = 0, options = {}) => {
	...
	if (_streaming) _streamFinish(doc);
...
// parent prints.
/** @type {boolean} whether this parse streams its walk (owned by `grammar`) */
