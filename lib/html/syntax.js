...
/**
 * Drop the whitespace a comma-separated list's grammar ignores. `internal`
 * keeps the whitespace inside an item, which `sizes` needs — its items carry a
 * media condition.
 * @param {string} raw raw (undecoded) value
 * @param {boolean} internal whether whitespace inside an item is significant
 * @returns {string} the normalized list
 */
const _normalizeCommaList = (raw, internal) => {
	const items = raw.split(",").map((item) => item.trim());
	return items
		.map((item) => (internal ? item : item.replace(/[\t\n\f\r ]+/g, "")))
		.join(",");
};

/**
 * Normalize an integer attribute: the parse rules skip leading whitespace and
 * stop at the first non-digit, so the ends and any leading zeros carry nothing.
 * A value those rules would not read as one integer is left alone.
 * @param {string} raw raw (undecoded) value
 * @returns {string} the normalized integer
 */
const _normalizeInteger = (raw) => {
...
const _rewriteAttributeValue = (element, name, raw, viewport) => {
	...
	if (TOKEN_LIST_ATTRIBUTES.has(name)) return _normalizeTokenList(raw);
	if (viewport && name === "content") return _normalizeViewport(raw);
	const commaList = COMMA_LIST_ATTRIBUTES.get(name);
	if (commaList !== undefined) return _normalizeCommaList(raw, commaList);
	const urlOn = URL_ATTRIBUTES.get(name);
	if (urlOn !== undefined && urlOn.has(element)) return raw.trim();
	const integerOn = INTEGER_ATTRIBUTES.get(name);
	if (integerOn !== undefined && integerOn.has(element)) {
		return _normalizeInteger(raw);
	}
	return raw;
};
...
const _scriptType = (path, element) => {
	for (const attribute of path.attributes(element)) {
		if (attribute.name === "type") return attribute.value.trim().toLowerCase();
	}
	return "";
};
...
// `<script>` bodies that are JSON: the two dedicated types plus any `+json`
// subtype (`application/ld+json`).
const _JSON_SCRIPT_TYPES = new Set([
	"application/json",
	"importmap",
	"speculationrules"
]);
		...
		case NodeType.Text: {
			...
			if (parent !== 0 && _LITERAL_TEXT_PARENTS.has(path.tagName(parent))) {
				...
				if (literalName === "script") {
					...
					if (_JSON_SCRIPT_TYPES.has(type) || _JSON_SUBTYPE_REGEXP.test(type)) {
