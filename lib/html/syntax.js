...
const _minifyOpenTag = (path, open) => {
	...
	// Rebuilding a tag another language wrote would print the spec's duplicate
	// drop into the output, which a re-parse of the source makes again anyway.
	if (
		count !== 0 &&
	...
	for (let k = 0; k < count; k++) {
		...
		if (quoted || html || attributeName === "style" || rawValue.includes("&")) {
			...
			if (isStyle && _deferEmbeddedSource !== undefined) {
