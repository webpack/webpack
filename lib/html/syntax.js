...
/**
 * Whether a `style` attribute's value holds a `}` that would close the rule it
 * is minified inside — one outside every string, comment and bracket. The
 * attribute is a declaration list, where such a `}` is part of a bad
 * declaration and the declarations after it still count, so a value holding one
 * is left as written rather than cut short at it.
 * @param {string} value the raw attribute value
 * @returns {boolean} true when the value cannot be wrapped in a rule
 */
const _closesStyleWrapper = (value) => {
	let depth = 0;
...
const _minifyStyleAttribute = (raw) => {
	if (raw.trim() === "" || _closesStyleWrapper(raw)) return raw;
	let out;
	try {
		const cssSyntax = _cssSyntax || (_cssSyntax = require("../css/syntax"));
