...
const _canOmitStructuralTags = (path, name, node) => {
	// Name first: this is asked of every element printed, and only three can
	// ever answer yes.
	const isHtml = name === "html";
	const isHead = !isHtml && name === "head";
	if (
		(!isHtml && !isHead && name !== "body") ||
		path.attributeCount(node) !== 0 ||
		path.namespace(node) !== NS_HTML
	) {
		return false;
	}
...
const _normalizeTokenList = (raw, sort) => {
	...
	// either way.
	if (!sort && !_repeatsAToken(collapsed)) return collapsed;
	const unique = [...new Set(collapsed.split(" "))];
...
const _minifyOpenTag = (path, open) => {
	...
	const markers = html ? REDUNDANT_TYPE_ATTRIBUTES[elementName] : undefined;
	const defaults =
