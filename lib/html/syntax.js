 ...
 * @property {boolean=} collapseBooleanAttributes collapse a canonically-spelled boolean attribute to its bare name (default true)
 * @property {boolean=} rewriteAttributeValues rewrite an attribute value into a shorter spelling of what it already parses to (default true)
 */
...
// The two attribute rewrites, on unless turned off: each keeps what the DOM
// parses but changes the bytes `getAttribute()` returns.
let _collapseBooleanAttributes = true;
let _rewriteAttributeValues = true;
...
const _rewriteAttributeValue = (element, name, raw, viewport) => {
	if (!_rewriteAttributeValues) return raw;
...
const printer = (path, writer) => {
	...
	_collapseBooleanAttributes =
		writer.options.collapseBooleanAttributes !== false;
	_rewriteAttributeValues = writer.options.rewriteAttributeValues !== false;
	switch (path.type()) {
