 ...
 * @param {HtmlPrintOptions & { css?: { convertLengthUnits?: boolean, rewriteCustomProperties?: boolean, preserveComments?: (string | RegExp)[] }, minifyConditionalComments?: boolean, minifySrcdoc?: boolean }=} minimizerOptions minimizer options — `environment` and `css` (`optimization.minimize.css`, whole) reach the CSS minifier this runs over inline CSS; the rest is `optimization.minimize.html`. The two stand apart rather than merged: both languages name a `quotes` and a `preserveComments`, and one flat object would hand each the other's answer
...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	const printOptions = {
		...
		cssPreserveComments: css.preserveComments,
