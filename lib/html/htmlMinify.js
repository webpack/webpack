 ...
 * @param {object=} minimizerOptions minimizer options — `environment` carries the target's CSS abilities and `convertLengthUnits` the CSS minimizer's option of that name, both for the inline CSS this minifies
...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	// the same target abilities and options a `.css` asset gets — without them the
	// two would disagree on a hex alpha, `inset`, a media range or a length's unit.
