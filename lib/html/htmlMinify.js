...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	const { code: out } = new SourceProcessor().process(code, { minimize: true });
	return { code: out };
};
...
// plugin / worker pool. It claims only what `HtmlModulesPlugin` rendered (the
// `html` asset-info marker): a `.html` asset another plugin emitted was never
// parsed by webpack, so re-serializing it could change bytes it is not allowed
// to change.
/**
 * @param {string} name asset filename
 * @param {AssetInfo=} info the asset's info, when the caller tracks it
 * @returns {boolean} true for HTML assets webpack rendered
 */
htmlMinify.filter = (name, info) =>
	info !== undefined ? info.html === true : /\.html(\?.*)?$/i.test(name);
