...
const htmlMinify = async (input, sourceMap, minimizerOptions = {}) => {
	...
	const renderer = async (
		...
		if (renderEmbeddedSource !== undefined) {
			// A JavaScript minifier is handed whole scripts, so the function the
			// body belongs to goes out — a classic one — and its body comes back.
			const handler =
				hole.type === "javascript" && hole.as === EVENT_HANDLER
					? { ...hole, source: asFunction(source), as: CLASSIC_SCRIPT }
					: null;
