 ...
 * @param {{ environment?: CssEnvironment, convertLengthUnits?: boolean, collapseWhitespace?: boolean, renderEmbeddedSource?: EmbeddedSourceRenderer }=} minimizerOptions minimizer options — `environment` and `convertLengthUnits` reach the CSS minifier this runs over inline CSS; `renderEmbeddedSource` replaces it and reaches an inline `<script>` too, and only travels when the pool runs in-process (a function cannot be cloned into a worker); the rest is `optimization.minimize.html`
