...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	// Both nested languages a document can hold are minified as the documents they
	// are, so this is what each of them is put back through — an `<iframe srcdoc>`
	// carrying a conditional comment gets both, in that order.
	//
	...
	const minifyDocument = (text) => {
		...
		// Only a document naming the attribute at all pays the extra parse.
		let source = text;
		...
		if (minifyConditionalComments) {
			processor.use({
				[NodeType.Comment]: (path) => {
					...
					conditionals.set(comment, comment.slice(opened + 2, closes));
				}
			});
		}
