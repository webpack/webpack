...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	const printOptions = {
		...
		convertLengthUnits,
		rewriteCustomProperties,
		// The CSS minifier's per-transform switches stand beside those, so
		// `pickTransforms` is what tells them from the HTML options around them.
		transforms: webpack.css.syntax.pickTransforms(minimizerOptions),
		collapseWhitespace,
		mergeStyles,
		removeEmptyAttributes,
		removeEmptyElements,
