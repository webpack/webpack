...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	// parse runs at a time. So the bodies are collected in a walk of their own and
	// spliced into the finished output by exact source text — never by a pattern
	// over the output, which would find a `<!--[if` written inside a script.
	/** @type {Map<string, string>} */
	const conditionals = new Map();
	if (minifyConditionalComments) {
		const collector = new SourceProcessor();
		collector.use({
	...
	for (const [source, body] of conditionals) {
		...
		const { code: minified } = new SourceProcessor().process(body, {
			mode: "minify",
			environment,
			convertLengthUnits,
			collapseWhitespace,
