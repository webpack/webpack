 ...
 * @param {{ environment?: CssEnvironment, convertLengthUnits?: boolean, collapseWhitespace?: boolean | "conservative" | "smart" | "all", minifyConditionalComments?: boolean, removeEmptyAttributes?: boolean, removeEmptyElements?: boolean, preserveComments?: (string | RegExp)[], removeRedundantAttributes?: "smart" | "all", sortAttributes?: boolean, sortClassNames?: boolean }=} minimizerOptions minimizer options — `environment` and `convertLengthUnits` reach the CSS minifier this runs over inline CSS; the rest is `optimization.minimize.html`
...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	// A downlevel-hidden conditional comment holds markup, and the printer cannot
	// minify it where it sits: its own grammar is mid-parse, and only one HTML
	// parse runs at a time. So the bodies are collected as the document is
	// printed — a visitor rides the walk that prints, so this costs no second
	// parse — and spliced into the finished output by exact source text, never by
	// a pattern over it, which would find a `<!--[if` written inside a script.
	...
	// A fresh one for the bodies: the collector above is registered on
	// `processor`, and a nested conditional comment would write into the map
	// being read here.
	const inner = new SourceProcessor();
	let spliced = out;
	for (const [source, body] of conditionals) {
		if (!spliced.includes(source)) continue;
		const { code: minified } = inner.process(body, printOptions);
