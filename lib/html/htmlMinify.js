...
const htmlMinify = async (input, sourceMap, minimizerOptions = {}) => {
	...
	// Printing is synchronous and terser is async, so the first pass collects the
	// inline scripts (leaving each as written) and the second prints with the
	// resolved bodies. An asset with no inline script needs only the first.
	/** @type {[string, boolean][]} */
	const scripts = [];
	...
	// Resolved in the worker like `webpack` above; terser is what
	// `minimizer-webpack-plugin` already minifies this build's JS assets with.
	// eslint-disable-next-line n/no-unpublished-require
	const { minify } = require("terser");

	// A body terser could not turn into code maps to undefined, which is what the
	// hook reads as "keep it as written".
	/** @type {Map<string, string | undefined>} */
	const minified = new Map();
	// `format` is terser's current spelling of `output`; both reach it as one
	// object, so the escape below lands whichever the JS assets were given.
	...
	for (const [js, isModule] of scripts) {
		...
		try {
			...
			// which lets terser mangle names a classic script must leave global.
			// `inline_script` escapes a `</script>` the output would otherwise carry
			// — inside an element that ends at the first one, it is not optional.
