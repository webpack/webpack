 ...
 * @param {{ environment?: CssEnvironment, convertLengthUnits?: boolean, collapseWhitespace?: boolean }=} minimizerOptions minimizer options — `environment` and `convertLengthUnits` reach the CSS minifier this runs over inline CSS; the rest is `optimization.minimize.html`
...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	// No `renderEmbeddedSource` here: these options are cloned into the worker
	// pool, and a function is not cloneable. It reaches the serializer from a
	// caller running in-process, off the compilation hook of the same name.
	const { environment, convertLengthUnits, collapseWhitespace } =
		minimizerOptions;
