...
const htmlMinify = async (input, sourceMap, minimizerOptions = {}) => {
	...
	const [[name, given]] = Object.entries(input);
	...
	// A standalone `.svg` is XML, where every HTML shorthand below is an error.
	const xml = /\.svg(\?.*)?$/i.test(name);
 ...
 * @returns {boolean} true for HTML and SVG assets
 */
htmlMinify.filter = (name) => /\.(?:html|svg)(\?.*)?$/i.test(name);
