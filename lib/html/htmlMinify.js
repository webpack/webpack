...
const htmlMinify = async (input, sourceMap, minimizerOptions = {}) => {
	...
	/**
	 * The function an event handler's body belongs to. Named past any run of `_`
	 * the body holds, so nothing in it resolves to the function instead of what it
	 * meant; the newline ends a line comment the body may close with.
	 * @param {string} body the handler's text
	 * @returns {string} the script it is the body of
	 */
	const asFunction = (body) => {
		let longest = 0;
		let run = 0;

		for (let i = 0; i < body.length; i++) {
			run = body.charCodeAt(i) === 95 ? run + 1 : 0;
	...
	/**
	 * The handler body inside the function a renderer answered with. `undefined`
	 ...
	 * @param {string} answered what the renderer answered
	 * @returns {string | undefined} the body, or undefined
	 */
	const handlerBody = (answered) => {
		const opened = answered.indexOf("{");

		return opened !== -1 &&
			answered.endsWith("}") &&
			/^function\s+[^\s(]+\s*\(\s*\)\s*$/.test(answered.slice(0, opened))
			? answered.slice(opened + 1, -1).trim()
			: undefined;
	};
	...
	const renderer = async (
		...
		if (renderEmbeddedSource !== undefined) {
			// A JavaScript minifier is handed whole scripts and takes no `as`, so
			// the function the body belongs to goes out and its body comes back.
			const handler =
				hole.type === "javascript" && hole.as === EVENT_HANDLER
					? { ...hole, source: asFunction(source), as: undefined }
					: null;
			const answered = embeddedText(
				await askEmbeddedRenderer(
					renderEmbeddedSource,
					handler === null ? hole : handler,
					reported
				)
			);

			if (typeof answered === "string") {
				return handler === null ? answered : handlerBody(answered);
			}
		}
	...
	const printDocument = async (text) => {
		// With no caller's renderer the only body the built-ins cannot answer is
		// a nested document, so a text spelling no `srcdoc` prints synchronously.
		if (renderEmbeddedSource === undefined && !/srcdoc/i.test(text)) {
			return new SourceProcessor().process(text, printOptions).code;
		}
		return (
 ...
 * inline `<style>`, a `<script>`, every event handler attribute (as the whole
 * script its body belongs to), an `<svg>` subtree and the document an
