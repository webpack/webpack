 ...
 * @param {HtmlPrintOptions & { minifyConditionalComments?: boolean, minifySrcdoc?: boolean }=} minimizerOptions minimizer options — `environment`, `convertLengthUnits` and `rewriteCustomProperties` reach the CSS minifier this runs over inline CSS; the rest is `optimization.minimize.html`
...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	const { SourceProcessor, NodeType, decodeEntities, escapeAttribute } =
		webpack.html.syntax;
	...
	// An `<iframe srcdoc>` holds a whole document, and like the conditional
	// comments below it cannot be minified where it is found. It is put back into
	// the source rather than into the output, because the printer respells an
	// attribute value — the recorded text is not what the output ends up holding.
	// Only a document that names the attribute at all pays the extra parse.
	//
	...
	/**
	 * @param {string} text markup that may hold `<iframe srcdoc>` documents
	 * @returns {string} the same markup, each of those minified in place
	 */
	const minifySrcdocIn = (text) => {
		if (!/srcdoc/i.test(text)) return text;
		...
		for (let i = found.length - 1; i >= 0; i--) {
			...
			const body = minifySrcdocIn(decodeEntities(value, true));
			const { code: inner } = new SourceProcessor().process(body, printOptions);
			out = `${out.slice(0, start)}${escapeAttribute(inner, text.charCodeAt(start - 1), true)}${out.slice(end)}`;
		}
		return out;
	};
	const source = minifySrcdoc ? minifySrcdocIn(code) : code;
