 ...
 * @param {HtmlPrintOptions & { minifyConditionalComments?: boolean, minifySrcdoc?: boolean }=} minimizerOptions minimizer options — `environment`, `convertLengthUnits`, `rewriteCustomProperties` and `transforms` reach the CSS minifier this runs over inline CSS; the rest is `optimization.minimize.html`
...
const htmlMinify = (input, sourceMap, minimizerOptions = {}) => {
	...
	const printOptions = {
		...
		convertLengthUnits,
		rewriteCustomProperties,
		// Each language's per-transform switches stand beside the rest rather than
		// nested in them, so its own `pickTransforms` is what tells them apart.
		cssTransforms: webpack.css.syntax.pickTransforms(minimizerOptions),
	...
	const minifyDocument = (text) => {
		...
		// Same shape as the srcdoc pass above, and for the same reason: the body is
		// markup this parse cannot start a second one over.
		if (minifyConditionalComments && /<!--\[if\s/i.test(source)) {
			/** @type {[number, number][]} */
			const found = [];
			new SourceProcessor()
				.use({
					[NodeType.Comment]: (path) => {
						...
						found.push([path.start() + opened + 2, path.start() + closes]);
					}
				})
				.process(source, {});
			// Back to front, so an earlier splice does not move a later range.
			for (let i = found.length - 1; i >= 0; i--) {
				...
				source = `${source.slice(0, start)}${minifyDocument(body)}${source.slice(end)}`;
			}
		}
		...
		return new SourceProcessor().process(source, printOptions).code;
	};
