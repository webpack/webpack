	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// HtmlGenerator#generate); the filename template comes from the
				// per-module generator option `filename`, falling back to
				// `[name].html`. Path data follows the asset-module pattern —
				...
				// than the importing chunk's name (e.g. `main`).
				const { makePathsRelative } = require("../util/identifier");

				compilation.hooks.renderManifest.tap(
					PLUGIN_NAME,
					(result, { chunk, codeGenerationResults }) => {
						...
						for (const module of modules) {
							...
							const htmlSource = codeGenResult.sources.get(HTML_TYPE);
							if (!htmlSource) continue;

							const generator = /** @type {HtmlGenerator} */ (
								normalModule.generator
							);
							const generatorOptions =
								/** @type {import("../../declarations/WebpackOptions").HtmlGeneratorOptions} */
								(normalModule.generatorOptions || {});
