				...
				NormalModule.getCompilationHooks(compilation).processResult.tap(
					...
					(result, module) => {
						if (module.type === HTML_MODULE_TYPE) {
							...
							let content = removeBOM(source);
							// Apply `module.parser.html.template` here, not in
							// HtmlParser: the transformed source must become the
							// module's stored source so the parser's dependency
							// offsets and the generator's render base (which both
							// read this source) line up. Running it in the parser
							// would only change what is parsed, leaving the
							// generator to rewrite the untransformed original.
							const { template } = /** @type {HtmlParser} */ (module.parser);
							if (template) {
								const text =
								...
								const transformed = template(text, {
									...
									// Register e.g. template partials so editing them
									// triggers a rebuild and invalidates the cache.
									addDependency: (dependency) => {
										const { fileDependencies } = /** @type {BuildInfo} */ (
											module.buildInfo
										);
										if (fileDependencies) fileDependencies.add(dependency);
									},
									emitWarning: (warning) =>
										module.addWarning(
											warning instanceof Error
												? warning
												: new WebpackError(warning)
										),
									emitError: (error) =>
										module.addError(
