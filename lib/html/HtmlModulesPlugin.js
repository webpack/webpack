	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// so final bytes are in place. Only real HTML pages carry these
				// sentinels — JS chunks embedding HTML strings must stay untouched,
				// so the htmlAssetNames guard skips non-HTML assets.
				if (integrityOn) {
				...
				compilation.hooks.processAssets.tap(
					...
					(assets) => {
						...
						for (const name of Object.keys(assets)) {
							// Only real HTML pages carry sentinels; skip JS chunks.
