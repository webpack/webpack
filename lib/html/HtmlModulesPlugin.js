...
/**
 * Builds the head tag string for a synthetic HTML page from `output.html` options.
 * Order: charset meta → base → remaining meta tags → title (links follow separately).
 * @param {import("../../declarations/WebpackOptions").OutputHtmlOptions} opts html options
 * @returns {string} concatenated head tags
 */
	...
	apply(compiler) {
		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				// Per-entry `html: true` means "use output.html defaults"; an object overrides.
				const htmlObj =
					typeof html === "object"
						? html
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// Inject title/meta/base into authored HTML pages.
				// Synthetic wrappers already have them baked in at generation time.
						...
						compilation.hooks.processAssets.tap(
							...
							(assets) => {
								...
								for (const name of Object.keys(assets)) {
									...
									if (typeof content !== "string") continue;
									// charset — insert at start of <head>; skip if already present
									if (
										meta &&
										meta.charset &&
									...
									// base — insert right after <head> opening; skip if already present
									if (base && !/<base[\s>]/i.test(content)) {
										...
										content = content.replace(
											/(<head[^>]*>)/i,
											`$1<base href="${escAttr(href)}"${targetAttr}>`
										);
									}
									// remaining meta tags — append before </head>
									if (meta) {
										let metaTags = "";
										for (const [mname, content_] of Object.entries(meta)) {
											if (mname === "charset") continue;
											const attr =
												mname.startsWith("og:") || mname.startsWith("twitter:")
													? `property="${escAttr(mname)}"`
													: `name="${escAttr(mname)}"`;
											metaTags += `<meta ${attr} content="${escAttr(content_)}">`;
										}
										if (metaTags) {
											content = content.replace(/(<\/head>)/i, `${metaTags}$1`);
										}
									}
									// title — insert before </head>; skip if already present
