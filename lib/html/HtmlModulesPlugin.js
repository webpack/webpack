...
// Bundled webpack-logo favicon, injected by default. Absolute (forward-slashed)
// so it resolves from any entry context, then bundled like any `<link>` href.
const DEFAULT_FAVICON = path
	.resolve(__dirname, "favicon.svg")
	.replace(/\\/g, "/");
	...
	apply(compiler) {
		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				// `false` disables; a string is the user's own icon. The href is
				// bundled into a hashed asset by HtmlParser like any other `<link>`.
				const favicon =
				...
				let faviconTag = "";
				if (favicon === true) {
					faviconTag = `<link rel="icon" type="image/svg+xml" href="${DEFAULT_FAVICON}">`;
				} else if (typeof favicon === "string") {
					faviconTag = `<link rel="icon" href="${favicon}">`;
				}
				return `data:text/html,<!doctype html><html><head>${faviconTag}${links.join(
					""
