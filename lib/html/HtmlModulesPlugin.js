...
class HtmlModulesPlugin {
	...
	apply(compiler) {
		...
		// HtmlEntryDependency.
		/** @type {Set<string>} */
		const templateFilePaths = new Set();
				...
				const htmlObj =
					typeof html === "object"
						...
						: typeof htmlOption === "object"
							...
							: {};
				const template =
					typeof html === "object" && html.template !== undefined
						? html.template
						: typeof htmlOption === "object"
							? htmlOption.template
							: undefined;
				if (template !== undefined) {
					const inject =
						typeof html === "object" && html.inject !== undefined
							? html.inject
							: typeof htmlOption === "object" &&
								  htmlOption.inject !== undefined
								? htmlOption.inject
								: output.module
									? "head"
									: "body";
					const tmplPath = path.resolve(context, template);
					templateFilePaths.add(tmplPath);
					const rawTmpl = compiler.inputFileSystem.readFileSync(tmplPath);
					let tmplContent = removeBOM(rawTmpl).toString("utf8");
					if (inject !== false) {
						const lower = tmplContent.toLowerCase();
						// Blank out comment content so <!-- </head> --> or
						// <!-- </body> --> don't produce false injection points.
						const searchable = lower.replace(/<!--[\s\S]*?-->/g, (m) =>
							" ".repeat(m.length)
						);
						if (inject === "head") {
							const headClose = searchable.lastIndexOf("</head>");
							const headInsert =
								headClose !== -1 ? headClose : tmplContent.length;
							tmplContent =
								tmplContent.slice(0, headInsert) +
								links.join("") +
								scripts.join("") +
								tmplContent.slice(headInsert);
						} else {
							const bodyClose = searchable.lastIndexOf("</body>");
							const headClose = searchable.lastIndexOf("</head>");
							if (bodyClose !== -1) {
								if (headClose !== -1 && links.length > 0) {
									// Apply larger offset first to keep the smaller one valid.
									if (bodyClose > headClose) {
										tmplContent =
											tmplContent.slice(0, bodyClose) +
											scripts.join("") +
											tmplContent.slice(bodyClose);
										tmplContent =
											tmplContent.slice(0, headClose) +
											links.join("") +
											tmplContent.slice(headClose);
									} else {
										tmplContent =
											tmplContent.slice(0, headClose) +
											links.join("") +
									...
									// No </head> or no CSS: put everything before </body>.
									tmplContent =
										tmplContent.slice(0, bodyClose) +
										links.join("") +
										scripts.join("") +
					...
					return `data:text/html;base64,${Buffer.from(tmplContent, "utf8").toString("base64")}`;
				}
					...
					typeof html === "object" && html.inject !== undefined
						...
						: typeof htmlOption === "object" && htmlOption.inject !== undefined
							...
							: output.module
								...
								: "body";
				if (inject === false) {
					return `data:text/html,<!doctype html><html><head>${headTags}</head><body></body></html>`;
				}
				if (inject === "head") {
					return `data:text/html,<!doctype html><html><head>${headTags}${links.join("")}${scripts.join("")}</head><body></body></html>`;
				}
				return `data:text/html,<!doctype html><html><head>${headTags}${links.join(
					""
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				for (const p of templateFilePaths) {
