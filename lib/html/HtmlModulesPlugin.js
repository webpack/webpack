...
class HtmlModulesPlugin {
	/**
	 * Compute a `[contenthash]`-style hash for HTML output: hash the bytes
	 * through `output.hashFunction` (with `output.hashSalt` mixed in if set),
	 * truncate the digest to `output.hashDigestLength`, and pass it through
	 * `nonNumericOnlyHash` so the result is always a path-safe slug.
	 * Reused by `renderManifest` for both the
	 * `[contenthash]`-in-`htmlFilename` slot (hashed against the rewritten
	 * placeholder content) and the final asset cache key (hashed against
	 * the post-`undoPath` content), so neither call drifts from the other
	 * recipe.
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const { resolveHtmlChunkUrlSentinels } = require("./htmlChunkUrl");
				...
				compilation.hooks.renderManifest.tap(
					...
					(result, { chunk, codeGenerationResults, hash: compilationHash }) => {
						...
						for (const module of modules) {
							...
							// Resolve chunk-URL sentinels emitted by
							// `HtmlInlineScriptDependency` /
							// `HtmlScriptSrcDependency` *before* hashing — by now
							// `createHash()` has populated every
							// `chunk.contentHash[type]` / `chunk.hash` /
							// `compilation.hash`, so `[contenthash]`,
							// `[chunkhash]`, and `[fullhash]` in the chunk
							// filename templates can all be substituted. Hashing
							// the resolved content (rather than the raw
							// placeholderContent) is what makes the HTML's
							// own `[contenthash]` invalidate when the referenced
							// chunks' filenames change.
							const resolvedContent = resolveHtmlChunkUrlSentinels(
								placeholderContent,
								compilation
							);
