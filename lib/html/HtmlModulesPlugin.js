	...
	apply(compiler) {
		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				// `output.crossOriginLoading` and SRI are applied to the injected
				// tags centrally in HtmlScriptSrcDependency (covers these synthetic
				// tags and real `.html` template tags alike).
				const htmlOption = compiler.options.output.html;
				const scriptLoading =
					(typeof htmlOption === "object" && htmlOption.scriptLoading) ||
					"auto";
				let scriptAttr;
				if (compiler.options.output.module) {
					// ESM output becomes `type="module"` downstream (always deferred),
					// so `scriptLoading` has no effect — see the warning below.
					scriptAttr = "";
				} else if (scriptLoading === "blocking") {
					scriptAttr = "";
				} else {
					// `auto` and `defer` both defer a classic script.
					scriptAttr = " defer";
				}
				const links = [];
				const scripts = [];
		...
		if (
			...
			(scriptLoading === "defer" || scriptLoading === "blocking")
		) {
