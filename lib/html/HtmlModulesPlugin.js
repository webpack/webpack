		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				if (template !== undefined) {
					...
					let tmplContent = fs.readFileSync(tmplPath, "utf8");
					if (inject !== false) {
						const lower = tmplContent.toLowerCase();
						if (inject === "head") {
							const at = lower.lastIndexOf("</head>");
							const pos = at !== -1 ? at : tmplContent.length;
							tmplContent =
								tmplContent.slice(0, pos) +
								links.join("") +
								scripts.join("") +
								tmplContent.slice(pos);
						} else {
							const bodyAt = lower.lastIndexOf("</body>");
							if (bodyAt !== -1) {
								tmplContent =
									tmplContent.slice(0, bodyAt) +
									scripts.join("") +
									tmplContent.slice(bodyAt);
								const lower2 = tmplContent.toLowerCase();
								const headAt = lower2.lastIndexOf("</head>");
								if (headAt !== -1 && links.length > 0) {
									tmplContent =
										tmplContent.slice(0, headAt) +
										links.join("") +
										tmplContent.slice(headAt);
								}
							} else {
								const headAt = lower.lastIndexOf("</head>");
								const pos = headAt !== -1 ? headAt : tmplContent.length;
								tmplContent =
									tmplContent.slice(0, pos) +
									links.join("") +
									scripts.join("") +
