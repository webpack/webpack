 ...
 * page, exposed for in-place mutation by `transformTags`. Mutate `attrs` or set
				...
				compilation.hooks.processAssets.tapPromise(
					...
					async (assets) => {
						...
						for (const name of Object.keys(assets)) {
							...
							if (hooks.transformTags.taps.length > 0) {
								const collected = HtmlGenerator.collectTags(resolved);
								await hooks.transformTags.promise(collected.tags, {
									outputName: name,
									html: resolved
								});
							...
							// the browser sees. Runs before `transformHtml` so a plugin can still
