...
const faviconLinkTag = (rel, icon) => {
	...
	const type = icon.type || mimeTypes.lookup(icon.href) || "";
	const attrs = [
...
const manifestLinkTag = (manifest, name) => {
	...
	return `<link rel="manifest" href="${htmlSyntax.escapeAttribute(href)}">`;
};
	...
	apply(compiler) {
		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				const headTags = getHtmlParser.loaded().buildHeadTags(htmlObj);
