...
// Use a file:// URL so HtmlParser resolves the absolute FS path correctly on all platforms.
const DEFAULT_FAVICON = `file://${DEFAULT_FAVICON_PATH.replace(/\\/g, "/")}`;
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const globalFaviconOpt =
					typeof htmlOption === "object" &&
					htmlOption !== null &&
					htmlOption.favicon !== undefined
						? htmlOption.favicon
						: true;

				if (globalFaviconOpt !== false) {
					const faviconAssetName =
						globalFaviconOpt === true
							? "favicon.svg"
							: path.basename(/** @type {string} */ (globalFaviconOpt));
					const faviconSrc =
						globalFaviconOpt === true
							? DEFAULT_FAVICON_PATH
							: path.resolve(
									compiler.context,
									/** @type {string} */ (globalFaviconOpt)
								);
							...
							for (const name of Object.keys(assets)) {
								...
								if (!faviconEmitted) {
									if (!compilation.getAsset(faviconAssetName)) {
										let faviconContent;
										try {
											faviconContent = fs.readFileSync(faviconSrc, "utf8");
										} catch (err) {
