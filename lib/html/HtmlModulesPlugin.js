...
class HtmlModulesPlugin {
	...
	apply(compiler) {
		...
		// the entry's own (see `collectHtmlEntryRequests`).
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			PLUGIN_NAME,
			(context, name, desc) => {
				const html =
					desc.html !== undefined ? desc.html : compiler.options.output.html;
				...
				// Mirror `output.crossOriginLoading` onto the injected tags, matching
				// how the runtime loads chunks. `crossorigin` is a copyable sibling
				// attribute, so putting it on these entry tags propagates to every
				// cloned/synthesized chunk tag (see HtmlScriptSrcDependency).
				// TODO: also emit `integrity` (SRI) here once a core output option
				// for it exists; for now SRI is left to external plugins.
				const { crossOriginLoading } = compiler.options.output;
				const crossOrigin = crossOriginLoading
				...
				for (const r of requests) {
					if (CSS_REQUEST_RE.test(r)) {
						links.push(`<link rel="stylesheet" href="${r}"${crossOrigin}>`);
					} else {
						scripts.push(`<script src="${r}"${crossOrigin}></script>`);
					}
				}
