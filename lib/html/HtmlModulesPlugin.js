...
class HtmlModulesPlugin {
	 ...
	 */
	apply(compiler) {
		// Reuse the same `RawSource` across builds when bytes are unchanged —
		// `getLazyHashedEtag`'s WeakMap is identity-keyed, so a fresh source
		// would invalidate `RealContentHashPlugin|analyse` on warm rebuilds.
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// Resolve sentinels left by `_renderHtml`'s JS-export path.
				// Stage runs after `createHash()` and before `realContentHash`.
				compilation.hooks.processAssets.tap(
					...
					(assets) => {
						for (const name of Object.keys(assets)) {
							...
							// Skip binary assets without decoding — the sentinel
							// is ASCII, so `Buffer#indexOf` catches it cheaply
							// and avoids a full UTF-8 decode of large blobs.
							let content;
							if (typeof raw === "string") {
								if (!raw.includes("__WEBPACK_HTML_CHUNK_URL__")) continue;
								content = raw;
							} else {
								if (!raw.includes("__WEBPACK_HTML_CHUNK_URL__")) continue;
							...
							const resolved = HtmlGenerator.resolveChunkUrlSentinels(
								...
								.join("");
							if (resolved !== content) {
								const prior = sentinelResolvedSourceCache.get(name);
								let newSource;
								if (prior !== undefined && prior.content === resolved) {
									newSource = prior.source;
								} else {
									newSource = new RawSource(resolved);
									sentinelResolvedSourceCache.set(name, {
										content: resolved,
										source: newSource
									});
								}
