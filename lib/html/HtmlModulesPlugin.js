	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				// Resolve the `integrity` sentinels left in the emitted HTML into
				// real SRI hashes. Runs after `RealContentHashPlugin`
						...
						(assets) => {
							for (const name of Object.keys(assets)) {
								...
								if (
									...
									!content.includes("__WEBPACK_HTML_INTEGRITY__")
								) {
									continue;
								}
								const resolved = HtmlGenerator.resolveChunkIntegritySentinels(
									content,
									compilation,
