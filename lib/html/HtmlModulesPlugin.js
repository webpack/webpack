				...
				compilation.hooks.renderManifest.tap(
					...
					(result, { chunk, codeGenerationResults, hash: compilationHash }) => {
						...
						for (const module of modules) {
							...
							// emitted (which would also collide with the host on the
							// `data:text/html` → chunk-name path below). An O(1) flag check,
							// not a per-`(chunk, module)` incoming-connection scan.
							if (
								normalModule.generatorOptions &&
								normalModule.generatorOptions.extract === "inline"
							) {
								continue;
							}
