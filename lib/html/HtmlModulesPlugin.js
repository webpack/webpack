						...
						compilation.hooks.processAssets.tap(
							...
							(assets) => {
								...
								for (const name of Object.keys(assets)) {
									...
									// charset must be first in <head>; skip if already present
									if (
										meta &&
										meta.charset &&
									...
									// base goes after any charset meta, before other links; skip if present
									if (base && !/<base[\s>]/i.test(content)) {
										...
										// insert after charset meta when present, otherwise after <head>
