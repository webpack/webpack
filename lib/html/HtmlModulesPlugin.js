	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// The `data:text/css` URI we use to feed inline `<style>` content
				// into the CSS pipeline carries no real filesystem context, so
				// without help, the synthetic module's `context` ends up as
				// `getContext("data:text/...")` and nested `url(...)` / `@import`
				// references would resolve against that nonsense path instead of
				// the HTML file's directory. Propagate the issuer's context onto
				// the resolved resource data so child CSS dependencies are
				// resolved relative to the HTML module. Scoped to dependencies
				// whose category is `"html-style"` so other `data:` URI uses
