	...
	apply(compiler) {
		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				// `inject: false` only suppresses sibling-chunk injection (in
				// HtmlEntryDependency), so the shell keeps its entry tags at the
				// default placement. ESM defaults to <head> — module scripts are
				// implicitly deferred.
				const inject = htmlObj.inject;
				const scriptsInHead =
					inject === "head" || (inject !== "body" && output.module);
				// `defer`/module scripts precede the stylesheets in <head> (Vite's
				// order — they execute after pending CSS anyway and start fetching
				// earlier); blocking scripts stay after the CSS.
				const scriptsFirst = output.module || scriptAttr === " defer";
				const headScripts = scriptsInHead ? scripts.join("") : "";
				return `data:text/html,<!doctype html><html><head>${headTags}${
					scriptsFirst
