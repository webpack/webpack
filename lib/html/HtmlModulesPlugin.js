...
/** @typedef {{ outputName: string }} HtmlAlterHtmlContext */
 ...
 * @property {AsyncSeriesWaterfallHook<[string, HtmlAlterHtmlContext]>} alterHtml called with each emitted page's final HTML (all sentinels resolved) just before it is written; return the (possibly transformed) HTML — e.g. to minify, inject a CSP meta, or rewrite tags
				...
				compilation.hooks.processAssets.tapPromise(
					...
					async (assets) => {
						...
						for (const name of Object.keys(assets)) {
							...
							// the browser sees. Runs before `alterHtml` so a plugin can still
							...
							resolved = await hooks.alterHtml.promise(resolved, {
								outputName: name
							});
							if (resolved !== content) {
...
HtmlModulesPlugin.getCompilationHooks = createHooksRegistry(
	() =>
		/** @type {HtmlCompilationHooks} */ ({
			...
			alterHtml: new AsyncSeriesWaterfallHook(["html", "context"]),
