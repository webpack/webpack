...
/** @typedef {{ context: string, request: string, name: string, index: number, dependOn: string[] | undefined }} HtmlEntrySpec */
		...
		compiler.hooks.finishMake.tapAsync(PLUGIN_NAME, (compilation, callback) => {
			...
			const recordNames = (module, seen) => {
				...
				for (const entry of ordered) {
					// A name the page or `output.html.chunkName` asked for is what the
					// file is called too; otherwise the tag's url lends its basename.
					const base =
				...
				for (const link of links) {
					...
					if (link.chunkName) pageNames.set(link.entryName, link.chunkName);
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const computePageEmit = (module, chunk) => {
					...
					// A page is named after its own path, unless it asked for a name;
					// a synthetic `output.html` entry is a `data:` module with no path,
					// so it falls back to the entry that named it.
