...
/** @typedef {{ request: string, entryName: string, type: "script" | "script-module" | "modulepreload" | "stylesheet" | "html" }} HtmlEntryInfo */
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const computePageEmit = (
					...
					return { resolvedContent: cached.resolvedContent, filename, info };
				};
				...
				compilation.hooks.renderManifest.tap(
					...
					(result, { chunk }) => {
						...
						for (const module of modules) {
							...
							if (!emit) continue;
							const { resolvedContent, filename, info } = emit;

							// Resolve any remaining `[webpack/auto]` placeholders to an undo path
							// computed from the emitted HTML's location, so asset/chunk URLs stay
							// relative to `output.path` even when the page emits into a subdirectory.
							const undoPath = getUndoPath(
								filename,
								/** @type {string} */ (outputOptions.path),
								false
							);
							// Resolve linked-page (`type: "html"`) sentinels to each page's emitted
							// filename before the undo path, so the href is relative to this page.
