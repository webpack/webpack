	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// "srcdoc-only" is a moduleGraph property, not a chunk one, yet
				// `renderManifest` runs per `(chunk, module)` and reruns across
				// hashing/render passes. Memoize so the incoming-connection scan
				// happens once per module instead of once per `(chunk, module)`.
				// "srcdoc-only" is a moduleGraph property, not a chunk one, yet
				// `renderManifest` runs per `(chunk, module)` and reruns across
				// hashing/render passes. Memoize so the incoming-connection scan
				// happens once per module instead of once per `(chunk, module)`.
				/** @type {WeakMap<Module, boolean>} */
				const inlineSrcdocOnlyCache = new WeakMap();
				...
				compilation.hooks.renderManifest.tap(
					...
					(result, { chunk, codeGenerationResults, hash: compilationHash }) => {
						...
						for (const module of modules) {
							...
							// `<iframe srcdoc>` modules produce `html` only so
							// HtmlInlineHtmlDependency can write the processed markup back
							// into the host attribute — they are never standalone pages, so
							// skip emitting a `.html` file (which would also collide with the
							// host on the `data:text/html` → chunk-name path below).
							let inlineSrcdocOnly = inlineSrcdocOnlyCache.get(module);
							if (inlineSrcdocOnly === undefined) {
								inlineSrcdocOnly = false;
								for (const connection of compilation.moduleGraph.getIncomingConnections(
									module
								)) {
									if (
										connection.dependency instanceof HtmlInlineHtmlDependency
									) {
										inlineSrcdocOnly = true;
									} else {
										inlineSrcdocOnly = false;
										break;
									}
								}
								inlineSrcdocOnlyCache.set(module, inlineSrcdocOnly);
							}
