	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// Emit extracted `.html` files, naming them from `output.htmlFilename`
				// or `output.htmlChunkFilename`. `chunk` is left out of the path data,
				// so `[name]` resolves to the HTML source's basename, not the chunk's.
				...
				compilation.hooks.renderManifest.tap(
					...
					(result, { chunk }) => {
						...
						for (const module of modules) {
							...
							// An `<iframe srcdoc>` module is never a standalone page, and
							// `extract: "inline"` marks it so. Without that, every srcdoc module
							// in a chunk would emit the same `<chunk>.html`.
