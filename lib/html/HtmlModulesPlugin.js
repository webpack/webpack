...
/** @typedef {{ outputName: string }} HtmlBeforeEmitContext */
/** @typedef {{ outputName: string }} HtmlAfterEmitContext */
/**
 ...
 * page, exposed for in-place mutation by `alterTags`. Mutate `attrs` or set
 * `remove`; positions are fixed, so don't reorder the array.
 ...
 */
/** @typedef {{ outputName: string, html: string }} HtmlAlterTagsContext */
/**
 ...
 * @property {AsyncSeriesHook<[HtmlMutableTag[], HtmlAlterTagsContext]>} alterTags called with the page's `<script>`/`<link>`/`<style>`/`<meta>` tags (webpack's own and any injected) as mutable descriptors; mutate `attrs` (add a `nonce`/`data-*`, switch `defer`↔`async`, …) or set `remove: true` and webpack rewrites each changed tag in place. Add new tags with `injectTags` instead
 * @property {AsyncSeriesWaterfallHook<[string, HtmlBeforeEmitContext]>} beforeEmit called with each emitted page's final HTML (all sentinels resolved) just before it is written; return the (possibly transformed) HTML — e.g. to minify, inject a CSP meta, or rewrite tags
 * @property {AsyncSeriesHook<[HtmlAfterEmitContext]>} afterEmit called once each page's HTML asset has been finalized — a post-emit notification (nothing to return)
 */
				...
				compilation.hooks.processAssets.tapPromise(
					...
					async (assets) => {
						...
						for (const name of Object.keys(assets)) {
							...
							// Structured tag injection: plugins return descriptors, webpack
							// places them by `injectTo`. Before CSP so an injected inline
							// `<script>`/`<style>` is hashed.
							const tags = await hooks.injectTags.promise([], {
								outputName: name,
								html: resolved
							});
							if (tags.length > 0) {
								resolved = HtmlGenerator.injectTags(resolved, tags);
							}
							// Let plugins mutate the tags already in the page (webpack's own
							// and any just injected) — add a nonce/attribute, drop a tag —
							// rewritten in place. Only parse when someone taps the hook.
							if (hooks.alterTags.taps.length > 0) {
								const collected = HtmlGenerator.collectTags(resolved);
								await hooks.alterTags.promise(collected.tags, {
									outputName: name,
									html: resolved
								});
								resolved = HtmlGenerator.applyTagMutations(resolved, collected);
							}
							// Inline content is final here, so CSP hashes cover the exact bytes
							// the browser sees. Runs before `beforeEmit` so a plugin can still
							// rewrite the injected meta.
							if (csp) resolved = HtmlGenerator.injectCsp(resolved, csp);
							// Final, fully-resolved HTML — let plugins transform it (minify,
							// inject a CSP meta, rewrite tags) before it is written.
							resolved = await hooks.beforeEmit.promise(resolved, {
								outputName: name
							});
							if (resolved !== content) {
								compilation.updateAsset(name, new RawSource(resolved));
							}
							await hooks.afterEmit.promise({ outputName: name });
						}
...
HtmlModulesPlugin.getCompilationHooks = createHooksRegistry(
	() =>
		/** @type {HtmlCompilationHooks} */ ({
			...
			alterTags: new AsyncSeriesHook(["tags", "context"]),
			beforeEmit: new AsyncSeriesWaterfallHook(["html", "context"]),
			afterEmit: new AsyncSeriesHook(["context"])
		})
);
