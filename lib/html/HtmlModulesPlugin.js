...
class HtmlModulesPlugin {
	...
	apply(compiler) {
		...
		// grouped into classic (CommonJS-resolved) and esm (ESM-resolved). The
		// first entry in each group owns the runtime; every subsequent entry
		// in the same group sets `dependOn: [leader]` so they share that
		// runtime without forcing one another to execute — important for
		// `<link rel="modulepreload">`, which must preload without running.
			...
			for (const module of compilation.modules) {
				...
				for (const group of Object.values(htmlEntryScripts)) {
					...
					for (const entry of group) {
						const dependOn =
							leaderName !== undefined ? [leaderName] : undefined;
						if (leaderName === undefined) leaderName = entry.entryName;
						promises.push(
