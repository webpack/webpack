	...
	apply(compiler) {
		...
		compiler.hooks.finishMake.tapAsync(PLUGIN_NAME, (compilation, callback) => {
			...
			const recordNames = (module, seen) => {
				...
				// An inline `<script>` body has no url of its own, so the page that
				// wrote it lends the name — and a `data:` page (an `<iframe srcdoc>`)
				// has no path either, so the page holding it is walked out to.
				/** @type {string | undefined} */
				let pageBase;
