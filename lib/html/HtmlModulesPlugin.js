...
// Absolute FS path to the bundled webpack-logo SVG.
const DEFAULT_FAVICON_PATH = path.resolve(__dirname, "favicon.svg");
// Forward-slashed absolute path used as a webpack module request inside the
// synthetic data:text/html wrapper (resolved like any other <link> href).
const DEFAULT_FAVICON = DEFAULT_FAVICON_PATH.replace(/\\/g, "/");
	...
	apply(compiler) {
		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				if (favicon === true) {
					...
					faviconTag = `<link rel="icon" href="${escAttr(favicon)}">`;
				}
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.hooks.renderManifest.tap(
					...
					(result, { chunk }) => {
						...
						for (const module of modules) {
							...
							// Track synthetic wrappers so the favicon pass can skip them.
				...
				// Inject the favicon tag into authored HTML assets that don't already
				// have <link rel="icon">. Synthetic wrappers already got it during
				// wrapper generation; this covers HTML files used directly as entries.
				const faviconOpt =
				...
				if (faviconOpt !== false) {
					...
					compilation.hooks.processAssets.tap(
						...
						(assets) => {
							let faviconAssetName = "";
							for (const name of Object.keys(assets)) {
								if (!name.endsWith(".html")) continue;
								if (syntheticHtmlAssets.has(name)) continue;
								const content = assets[name].source();
								if (typeof content !== "string") continue;
								if (content.includes('<link rel="icon"')) continue;
								// Emit the favicon file once, using the asset name derived
								// from the option (default: "favicon.svg" from the bundled logo).
								if (!faviconAssetName) {
									if (faviconOpt === true) {
										faviconAssetName = "favicon.svg";
										if (!compilation.getAsset(faviconAssetName)) {
											compilation.emitAsset(
												faviconAssetName,
												new RawSource(
													fs.readFileSync(DEFAULT_FAVICON_PATH, "utf8")
												)
											);
										}
									} else {
										faviconAssetName = path.basename(
										...
										if (!compilation.getAsset(faviconAssetName)) {
											...
											compilation.emitAsset(
												faviconAssetName,
												new RawSource(fs.readFileSync(src, "utf8"))
											);
										}
									}
								}
								const tag =
									faviconOpt === true
										? `<link rel="icon" type="image/svg+xml" href="${escAttr(faviconAssetName)}">`
										: `<link rel="icon" href="${escAttr(faviconAssetName)}">`;
								compilation.updateAsset(
									name,
									new RawSource(content.replace(/<head>/i, `<head>${tag}`))
								);
							}
						}
					);
