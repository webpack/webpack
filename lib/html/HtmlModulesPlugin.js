...
/**
 * Resolves `output.html.favicon` for one page to `[rel, request]` pairs.
 * `false`/absent → none; `true` → the webpack logo; a string → one `icon`
 * link; an object maps each `rel` to an icon path; a function receives the
 * page name and returns any of those.
 * @param {FaviconOption} favicon favicon option
 * @param {string} name page/entry name
 * @returns {[string, string][]} rel/request pairs, in order
 */
const resolveFaviconLinks = (favicon, name) => {
	const value = typeof favicon === "function" ? favicon(name) : favicon;
	if (value === true) return [["icon", DEFAULT_FAVICON]];
	if (typeof value === "string") return [["icon", value]];
 ...
 * @param {string} request icon request/path
 * @returns {string} a `<link rel=… [type=…] href=…>` tag; the `type` follows the file format
 */
const faviconLinkTag = (rel, request) => {
	const mime = getMimeTypes().lookup(request);
	const type = mime ? ` type="${mime}"` : "";
	return `<link rel="${escapeAttribute(rel)}"${type} href="${escapeAttribute(
		request
	)}">`;
};
		...
		EntryOptionPlugin.getHooks(compiler).entry.tap(
			...
			(context, name, desc) => {
				...
				const faviconTag = resolveFaviconLinks(favicon, name)
					.map(([rel, request]) => faviconLinkTag(rel, request))
					.join("");
			...
			(compilation, { normalModuleFactory }) => {
				...
				const computePageEmit = (
					...
					if (cached === undefined) {
						...
						const resolvedContent = HtmlGenerator.embedInlineChunkHashes(
