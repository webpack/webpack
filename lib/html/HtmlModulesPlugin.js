...
class HtmlModulesPlugin {
	...
	apply(compiler) {
		...
		/** @type {Map<string, { content: string, source: import("webpack-sources").RawSource }>} */
		const faviconInjectedSourceCache = new Map();
					...
					compilation.hooks.processAssets.tap(
						...
						(assets) => {
							...
							for (const name of Object.keys(assets)) {
								...
								const cached = faviconInjectedSourceCache.get(name);
								let source;
								if (cached !== undefined && cached.content === newContent) {
									source = cached.source;
								} else {
									source = new RawSource(newContent);
									faviconInjectedSourceCache.set(name, {
										content: newContent,
										source
									});
								}
