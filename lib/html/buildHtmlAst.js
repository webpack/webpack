...
const buildHtmlAst = (source, fragmentContext) => {
	...
	const inSelectScope = (/** @type {string} */ tagName) =>
		hasElementInScopeMatching(
			(el) => el.namespace === NS_HTML && el.tagName === tagName,
			(el) =>
				!(
					el.namespace === NS_HTML &&
					(el.tagName === "optgroup" || el.tagName === "option")
				)
		);
	...
	const resetInsertionMode = () => {
		...
		for (let i = open.length - 1; i >= 0; i--) {
			...
			if (node.namespace === NS_HTML) {
				if (tn === "select") {
					// simplified select-in-table detection
					if (!last) {
						for (let j = i - 1; j >= 0; j--) {
							const anc = open[j];
							if (anc.tagName === "template" && anc.namespace === NS_HTML) {
								break;
							}
							if (anc.tagName === "table" && anc.namespace === NS_HTML) {
	...
	modes.inHeadNoscript = (t) => {
		...
		if (t.type === "endTag" && t.name === "br") {
			/* fallthrough */
		} else if (t.type === "startTag" && ["head", "noscript"].includes(t.name)) {
			return;
		}
		open.pop();
		mode = "inHead";
	...
	modes.text = (t) => {
		...
		if (t.type === "endTag") {
			// The tokenizer emits a partial end tag at EOF even when it doesn't
			// match the open rawtext/RCDATA element; per spec that is text.
			if (cur() && t.name !== cur().tagName) {
				insertCharacters(
	...
	modes.inSelect = (t) => {
		if (t.type === "char") {
			if (t.data.includes("\0")) t = { ...t, data: t.data.replace(/\0/g, "") };
			if (t.data) insertCharacters(t.data, t.start, t.end);
			return;
		}
		if (t.type === "comment") {
			insertComment(t.data, t.start, t.end);
		...
		if (t.type === "startTag") {
			if (t.name === "html") return modes.inBody(t);
			if (t.name === "option") {
				if (cur().tagName === "option") open.pop();
				insertHtmlElement("option", t.attrs, t.pos);
				return;
			}
			if (t.name === "optgroup") {
				if (cur().tagName === "option") open.pop();
				if (cur().tagName === "optgroup") open.pop();
				insertHtmlElement("optgroup", t.attrs, t.pos);
				return;
			}
			if (t.name === "select") {
				if (!inSelectScope("select")) return;
				popUntil("select");
				resetInsertionMode();
				return;
			}
			...
			if (["input", "keygen", "textarea"].includes(t.name)) {
				if (!inSelectScope("select")) return;
				popUntil("select");
				resetInsertionMode();
				return process(t);
			}
			if (["script", "template"].includes(t.name)) return modes.inHead(t);
		}
		if (t.type === "endTag") {
			if (t.name === "optgroup") {
				if (
					cur().tagName === "option" &&
					open[open.length - 2] &&
					open[open.length - 2].tagName === "optgroup"
				) {
					open.pop();
				}
				if (cur().tagName === "optgroup") open.pop();
