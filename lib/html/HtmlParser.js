...
class HtmlParser extends Parser {
	...
	constructor(options) {
		...
		for (const entry of sources) {
			if (entry === "...") continue;
			// User entries carry no `filter` (the schema has none); only
			// the built-in polymorphic defaults do.
			/** @type {SourceItem} */
			const item = { type: entry.type, filter: undefined };
	 ...
	 * @returns {string} the (possibly transformed) source
	 */
	applyTemplate(source, module, compilation) {
		if (!this.template) return source;
		// `parse` runs after `_doBuild` has initialized these dependency sets,
		...
		const transformed = this.template(source, {
			module,
	...
	parse(source, state) {
		...
		const module = /** @type {NormalModule} */ (state.module);
		const compilation = state.compilation;

		source = this.applyTemplate(source, module, compilation);
