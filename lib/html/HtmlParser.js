...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		// The entry names this page's tags are extracted under start with its own
		// path, so two pages of one compilation never claim the same name — read
		// from `resource`, since `identifier()` is `html|<path>` to contextify.
		/** @type {string} */
		const resource =
							...
							switch (type) {
								...
								case "script-module":
									if (content) {
										...
										// Module output upgrades a classic body's tag, so every
										// extracted tag defers and the kinds schedule alike; without
										// it the authored kind decides, and the two kinds part.
										const scheduling = outputModule ? "deferred" : scriptType;
										// `async` evaluates whenever it loads and a `<template>` body
										// never does, so neither joins a run — and a deferred tag runs
										// wherever it sits, so only module output spans markup.
										const joinsRun =
											templateDepth === 0 &&
										...
										let removeRange;
										let entryName = `${entryPrefix}-${index}`;
										if (joinsRun) {
											if (
												inlineRunEntryName === undefined ||
								...
								case "prefetch": {
									...
									for (const [url, us, ue] of parsed) {
										...
										if (type === "html") {
											const index = nextEntryIndex++;
											const entryName = `${entryPrefix}-${index}`;
