...
/**
 * User-supplied `HtmlParserOptions.sources` entry. `"srcset"` is the
 * user-facing alias for "parse the attribute value as a `srcset`-style
 * candidate list"; internally it resolves to `{ parse: parseSrcset,
 * type: "src" }`.
 * @typedef {{ tag?: string, attribute: string, type: SourceType | "srcset", filter?: (attributes: Map<string, string>) => boolean }} SourceEntry
 */
...
// Any-tag entries (user `sources` items with no `tag`) live under this
// sentinel key in the per-tag table. `\0` can't appear in an HTML tag
// name, so it can't collide with a real tag — and using one table
// (instead of two fields on the parser) keeps construction and lookup
// to a single shape.
const ANY_TAG = "\0";

// Shared `SourceItem` singletons used by the built-in defaults — keeping
// one instance per (parser, kind) pair stabilizes V8's hidden classes
// across lookups in the walk.
/** @type {SourceItem} */
const PLAIN_SRC = { parse: parseSrc, type: "src", filter: undefined };
/** @type {SourceItem} */
const PLAIN_SRCSET = { parse: parseSrcset, type: "src", filter: undefined };
...
const DEFAULT_SOURCES_BY_TAG = {
	audio: { src: PLAIN_SRC },
	embed: { src: PLAIN_SRC },
	img: { src: PLAIN_SRC, srcset: PLAIN_SRCSET },
	input: { src: PLAIN_SRC },
	link: {
		href: {
			parse: parseSrc,
		...
		imagesrcset: { parse: parseSrcset, type: "src", filter: filterLinkHref }
	},
	meta: {
		content: { parse: parseSrc, type: "src", filter: filterMetaContent }
	},
	object: { data: PLAIN_SRC },
	script: {
		src: {
			parse: parseSrc,
			// Non-JS `<script>` types (e.g. `application/ld+json`,
			// `importmap`) stay plain asset URLs so the browser keeps
			// seeing them as data blocks. `type="module"` opts into the
			// ESM entry chunk; everything else in `JS_SCRIPT_TYPES` is a
			// classic script.
			type: (attrs) =>
	...
	source: { src: PLAIN_SRC, srcset: PLAIN_SRCSET },
	track: { src: PLAIN_SRC },
	video: { poster: PLAIN_SRC, src: PLAIN_SRC },
	// SVG
	image: { "xlink:href": PLAIN_SRC, href: PLAIN_SRC },
	use: { "xlink:href": PLAIN_SRC, href: PLAIN_SRC }
};

/** @type {Record<string, Record<string, SourceItem>>} */
const EMPTY_SOURCES_BY_TAG = {};

/**
 * Builds a sources-by-tag lookup table from a user-supplied array.
 * Each tag bucket is rebuilt via spread, so we never carry around a
 * mutable `bucket` local: `byTag[tag] = { ...byTag[tag], [attr]: item }`
 * works whether or not the tag has been seen before (spreading
 * `undefined` is a no-op). `"..."` expands `DEFAULT_SOURCES_BY_TAG`
 * in place at its array position; later entries override earlier ones
 * per `(tag, attribute)`.
 * @param {readonly (SourceEntry | "...")[]} userSources user entries
 * @returns {Record<string, Record<string, SourceItem>>} tag → attribute → item (any-tag entries under `ANY_TAG`)
 */
const buildSourcesByTag = (userSources) => {
	/** @type {Record<string, Record<string, SourceItem>>} */
	const byTag = {};
	for (const entry of userSources) {
		if (entry === "...") {
			for (const tag of Object.keys(DEFAULT_SOURCES_BY_TAG)) {
				byTag[tag] = { ...byTag[tag], ...DEFAULT_SOURCES_BY_TAG[tag] };
			}
			continue;
		}
		const isSrcset = entry.type === "srcset";
		const tag = entry.tag === undefined ? ANY_TAG : entry.tag.toLowerCase();
		byTag[tag] = {
			...byTag[tag],
			[entry.attribute.toLowerCase()]: {
				...
				type: isSrcset
					? "src"
					: /** @type {SourceTypeOrResolver} */ (entry.type),
				filter: entry.filter
			}
		};
	}
...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		// default table — zero allocation, zero work per parser. `false`
		// reuses the shared empty table. Only a user array pays for the
		// build, which expands `"..."` inline against the same default
		// table — no intermediate array allocation.
		const userSources = options && options.sources;
		this.sourcesByTag =
			userSources === false
				? EMPTY_SOURCES_BY_TAG
				: userSources === undefined || userSources === true
					? DEFAULT_SOURCES_BY_TAG
					: buildSourcesByTag(userSources);
		// Cached any-tag bucket (or `undefined` when the user supplied no
		// tag-less entries; always `undefined` for the defaults). The
		// walk uses its truthiness to short-circuit the any-tag fallback
		// without paying for a `sourcesByTag[ANY_TAG]` lookup per element.
		this.anyTagSources = this.sourcesByTag[ANY_TAG];
	}
	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				// Per-tag source dict (or undefined) and the cached any-tag
				// fallback. Two property reads — the cheapest possible
				// test. Most elements (`<div>`, `<p>`, …) miss both and
				// short-circuit out without ever touching
				// `pendingAttributes`. `<script>` always falls through
				// so the inline-script handler below can pick it up.
				const tagSources = this.sourcesByTag[elementName];
				const anyTagSources = this.anyTagSources;
				if (!tagSources && !anyTagSources && elementName !== "script") {
					pendingAttributes.length = 0;
					return end;
				}
				...
				for (const attr of pendingAttributes) {
					const attributeName = input
						.slice(attr.nameStart, attr.nameEnd)
						.toLowerCase();
					// Per-tag wins; fall back to any-tag. Each lookup is a
					// single property read — no `Map.get`, no string concat.
					const sourceItem =
						(tagSources && tagSources[attributeName]) ||
						(anyTagSources && anyTagSources[attributeName]);
