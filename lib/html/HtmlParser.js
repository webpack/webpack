...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		// One chunk per run of inline `<script type="module">`: they are deferred
		// and evaluate in tag order, so only what executes between them matters.
		/** @type {string | undefined} */
		let inlineModuleEntryName;
		// Element offset of the run's last member — any other `<script>` ends it.
		let inlineModuleEntryTag = -1;
				...
				[NodeType.Element]: {
					...
					enter: (path) => {
							...
							switch (type) {
								...
								case "script-module":
									if (content) {
										...
										if (joinsRun) {
											if (inlineModuleEntryName === undefined) {
												inlineModuleEntryName = entryName;
											} else {
												entryName = inlineModuleEntryName;
											...
											inlineModuleEntryTag = elementStart;
										}
						...
						if (
							...
							inlineModuleEntryName = undefined;
						}
					},
					exit: (path) => {
