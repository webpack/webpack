...
class HtmlParser extends Parser {
	 ...
	 * @param {Module} module the html module
			...
			typeof source === "string" ? source : source.toString("utf8"),
			{
				...
				addDependency: (dependency) => {
					if (buildInfo.fileDependencies) {
						buildInfo.fileDependencies.add(dependency);
					}
				},
				addContextDependency: (dependency) => {
					if (buildInfo.contextDependencies) {
						buildInfo.contextDependencies.add(dependency);
					}
				},
				addMissingDependency: (dependency) => {
					if (buildInfo.missingDependencies) {
						buildInfo.missingDependencies.add(dependency);
					}
				},
				emitWarning: (warning) =>
					module.addWarning(
