...
/** @typedef {"src" | "srcset" | "script" | "script-module" | "stylesheet" | "stylesheet-inline"} UserSourceType */
/** @typedef {{ parse: (input: string) => ParsedSource[] | undefined, filter?: (attributes: Map<string, string>) => boolean, entry?: boolean | ((attributes: Map<string, string>) => boolean), entryCategory?: string, userType?: UserSourceType }} SourceItem */
 ...
 *
 * Built-in defaults carry `entry`/`filter`/`entryCategory` metadata and
 * are identified by reference equality with `DEFAULT_SOURCES` (the
 * `"..."` expansion preserves those references). User-supplied sources
 * carry a `userType` instead, which selects the dependency / entry kind
 * directly without going through the default-only conditional logic.
...
const resolveSources = (sources) => {
	...
	for (const entry of sources) {
		...
		/** @type {SourceItem} */
		const item = {
			// `stylesheet-inline` takes the whole attribute value as CSS
			// text, but the walk still needs a `parse` result to drive
			// the per-source loop — `parseSrc` returns a single range
			// covering the trimmed value, which is exactly what the
			// inline-CSS dependency uses for its replace-range.
			parse: entry.type === "srcset" ? parseSrcset : parseSrc,
			userType: entry.type
		};
	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				const defaultsForTag = DEFAULT_SOURCES.get(elementName);
				for (const attr of hasMatchingSources ? pendingAttributes : []) {
					...
					// Built-in defaults are identified by reference equality
					// with their `DEFAULT_SOURCES` slot (the `"..."`
					// expansion preserves references). Anything else is a
					// user-supplied source and dispatches off `userType`.
					const isDefaultSource = Boolean(
						defaultsForTag && defaultsForTag.get(attributeName) === sourceItem
					);
					...
					if (sourceItem.userType === "stylesheet-inline") {
					...
					// link. Only built-in defaults reach this conditional —
					// user-typed sources select their kind via `userType`
					// below.
					...
					const userTypeKind =
						sourceItem.userType === "script"
							? "classic"
							: sourceItem.userType === "script-module"
								? "esm-script"
								: sourceItem.userType === "stylesheet"
									? "stylesheet"
									: undefined;
					const isEntry =
						userTypeKind !== undefined ||
						(isDefaultSource &&
							(isStylesheetEntry ||
					...
					// `output.module` / `experiments.outputModule`. User-typed
					// `script-module` sources opt in explicitly regardless of
					// the tag's `type` attribute.
					const useEsmEntry =
						userTypeKind === "esm-script" ||
						(userTypeKind === undefined &&
							((elementName === "script" &&
								isModuleScript(getAttributesMap())) ||
