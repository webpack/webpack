...
/**
 * Creates a fresh null-prototype dictionary populated from `props`.
 * Used to build lookup tables without `Object.prototype` ancestry, so
 * keys like `__proto__` or `toString` can't collide with inherited
 * properties at lookup time.
 * @template {object} T
 * @param {T} props initial properties
 * @returns {T} same own properties, null prototype
 */
const dict = (props) => Object.assign(Object.create(null), props);
...
/** @type {Record<string, Record<string, SourceItem>>} */
const EMPTY_SOURCES_BY_TAG = Object.create(null);

/**
 * Builds a sources-by-tag lookup table from a user-supplied array.
 * `"..."` literals expand inline by copying tag buckets from the
 * precomputed `DEFAULT_SOURCES_BY_TAG` — no intermediate array
 * allocation. Later entries override earlier ones per `(tag, attribute)`.
	...
	for (const entry of userSources) {
		if (entry === "...") {
			for (const tag of Object.keys(DEFAULT_SOURCES_BY_TAG)) {
				let bucket = byTag[tag];
				if (!bucket) byTag[tag] = bucket = Object.create(null);
				Object.assign(bucket, DEFAULT_SOURCES_BY_TAG[tag]);
			}
			continue;
		}
		...
		let bucket = byTag[tag];
		if (!bucket) byTag[tag] = bucket = Object.create(null);
		bucket[entry.attribute.toLowerCase()] = {
	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				// Per-tag and any-tag source dicts (the latter at the
				// sentinel `ANY_TAG` key). Two property reads on a
				// null-prototype object — the cheapest possible test.
				// Most elements (`<div>`, `<p>`, …) miss both and
				// short-circuit out without ever touching
				// `pendingAttributes`. `<script>` always falls through so
				// the inline-script handler below can pick it up.
				const sourcesByTag = this.sourcesByTag;
				const tagSources = sourcesByTag[elementName];
				const anyTagSources = sourcesByTag[ANY_TAG];
				...
				for (const attr of pendingAttributes) {
					...
					// single property read on a null-prototype object — no
					// `Map.get`, no string concat, no prototype walk.
					const sourceItem =
