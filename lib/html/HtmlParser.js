...
const attrSourceSpan = (source, attr) => {
	const valueStart = A.attributeValueStart(attr);
	const valueEnd = A.attributeValueEnd(attr);
	const end =
		valueStart === -1
			? A.attributeNameEnd(attr)
			: source.charCodeAt(valueStart - 1) === CC_QUOTATION ||
				  source.charCodeAt(valueStart - 1) === CC_APOSTROPHE
				? valueEnd + 1
				: valueEnd;
	return source.slice(A.attributeNameStart(attr) - 1, end);
};
...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		const reconcileScriptTypeAttr = (typeAttr, nameEnd, type, input) => {
			const valueStart = typeAttr !== 0 ? A.attributeValueStart(typeAttr) : -1;
			const valueEnd = typeAttr !== 0 ? A.attributeValueEnd(typeAttr) : -1;
			if (outputModule && type === "script") {
				...
				let attrEnd;
				if (valueStart === -1) {
					attrEnd = A.attributeNameEnd(typeAttr);
				} else if (input[valueEnd] === '"' || input[valueEnd] === "'") {
					attrEnd = valueEnd + 1;
				} else {
					attrEnd = valueEnd;
				}
				let attrStart = A.attributeNameStart(typeAttr);
				if (
					attrStart > 0 &&
		...
		new SourceProcessor()
			.use({
				[NodeType.Comment]: (_api, n) => {
					const start = A.start(n);
					const end = A.end(n);
				...
				[NodeType.Element]: {
					...
					enter: (_api, n) => {
						...
						const elementName = A.tagName(n);
						const attributeCount = A.attributeCount(n);
						const elementStart = A.start(n);
						const tagEnd = A.tagEnd(n);
						const nameEnd = A.nameEnd(n);
						...
						for (let i = 0; i < attributeCount + 1; i++) {
							...
							if (content) {
								...
								attr = A.attributeAt(n, i);
								const item = sources[A.attributeName(attr)];
								if (!item) continue;
								if (
									item.namespace !== undefined &&
									A.namespace(n) !== item.namespace
								) {
									continue;
								}
								// Adoption-agency clones carry no offsets; skip blank values.
								if (A.attributeValueStart(attr) === -1) {
									continue;
								}
								value = A.attributeValue(attr);
								...
								start = A.attributeValueStart(attr);
								end = A.attributeValueEnd(attr);
								type =
									...
									try {
										...
										module.addError(
											new ModuleDependencyError(
												module,
												new WebpackError(
													`Bad value for attribute "${A.attributeName(
														attr
										...
										if (
											...
											if (elementStart >= 0) {
												for (const copyableName of COPYABLE_SIBLING_ATTRS) {
													const copyableAttr = A.findAttribute(n, copyableName);
													if (copyableAttr !== 0) {
											...
											if (elementStart >= 0 && elementName === nativeTag) {
												const integrityAttr = A.findAttribute(n, "integrity");
												if (
													integrityAttr !== 0 &&
													A.attributeNameStart(integrityAttr) >= 0
												) {
													let rs = A.attributeNameStart(integrityAttr);
													while (
														rs > 0 &&
													...
													let re;
													const ivs = A.attributeValueStart(integrityAttr);
													if (ivs === -1) {
														re = A.attributeNameEnd(integrityAttr);
													} else {
														const q = source.charCodeAt(ivs - 1);
														const ive = A.attributeValueEnd(integrityAttr);
														re =
												...
												if (elementKind === "script-module") {
													const typeAttr = A.findAttribute(n, "type");
													if (
														typeAttr !== 0 &&
														A.attributeValueStart(typeAttr) >= 0
													) {
														typeValueRange = [
															A.attributeValueStart(typeAttr) - elementStart,
															A.attributeValueEnd(typeAttr) - elementStart
														];
													}
												}
											}
