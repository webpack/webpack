		...
		const walkNode = (node) => {
			...
			if (elementName === "script") {
				...
				if (!hasSrc && isExecutableJsScript(scriptAttrs)) {
					/** @type {import("./buildHtmlAst").HtmlText | undefined} */
					let firstTextChild;
					/** @type {import("./buildHtmlAst").HtmlText | undefined} */
					let lastTextChild;
					for (const child of node.children) {
						if (child.type === "text") {
							if (firstTextChild === undefined) firstTextChild = child;
							lastTextChild = child;
						}
					}
					const contentStart = firstTextChild
						? firstTextChild.start
						: node.tagEnd;
					const contentEnd = lastTextChild ? lastTextChild.end : node.tagEnd;
