...
// FuncIRIs and apply to every SVG-namespace element, hence `namespace`-gated
// rather than per-tag. `parseCssUrls` leaves an internal `url(#id)` untouched.
/** @type {SourceItem} */
const SVG_CSS_URL = {
	type: "css-url",
...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		// Hand the collected entries to HtmlModulesPlugin, which creates the real
		// ones during finishMake. `script` and `script-module` chain through a
		// leader-only dependOn; everything else is independent.
		if (
