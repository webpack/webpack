...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		// Per-module prefix keeping generated entry names from colliding across the
		// HTML modules of one compilation. It hashes `module.resource` rather than
		// `identifier()`, whose `html|` prefix `contextify` would leave absolute.
		/** @type {string} */
		const resource =
							...
							switch (type) {
								...
								case "script-module":
									if (content) {
										...
										let removeRange;
										let entryName = `__html_${moduleHash}_${index}`;
										if (joinsRun) {
