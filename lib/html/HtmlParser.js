...
/** @typedef {"src" | "srcset" | "script" | "script-module" | "modulepreload" | "stylesheet" | "stylesheet-style" | "stylesheet-style-attribute" | "srcdoc"} SourceType */
				...
				[NodeType.Element]: {
					...
					enter: (n) => {
						...
						for (const attr of tagSources ? attrs : []) {
							...
							// this file; the processed HTML is written back into the attribute.
							if (type === "srcdoc") {
								const htmlText = decodeHtmlEntities(attributeValue, true);
								// Only spin up a nested module when there is markup to rewrite.
								if (!htmlText.includes("<")) continue;
