 ...
 * @param {import("./syntax").HtmlAttrRef} attr attribute ref
 * @returns {string} the attribute's source slice
 */
const attrSourceSpan = (source, attr) => {
	const valueStart = A.attrValueStart(attr);
	const valueEnd = A.attrValueEnd(attr);
	const end =
		valueStart === -1
			? A.attrNameEnd(attr)
			: source.charCodeAt(valueStart - 1) === CC_QUOTATION ||
				...
				? valueEnd + 1
				: valueEnd;
	return source.slice(A.attrNameStart(attr) - 1, end);
};
...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * @param {import("./syntax").HtmlAttrRef} typeAttr type attribute ref (0 = none)
		...
		const reconcileScriptTypeAttr = (typeAttr, nameEnd, type, input) => {
			const valueStart = typeAttr !== 0 ? A.attrValueStart(typeAttr) : -1;
			const valueEnd = typeAttr !== 0 ? A.attrValueEnd(typeAttr) : -1;
			if (outputModule && type === "script") {
				...
				let attrEnd;
				if (valueStart === -1) {
					attrEnd = A.attrNameEnd(typeAttr);
				} else if (input[valueEnd] === '"' || input[valueEnd] === "'") {
					attrEnd = valueEnd + 1;
				} else {
					attrEnd = valueEnd;
				}
				let attrStart = A.attrNameStart(typeAttr);
				if (
					attrStart > 0 &&
			...
			.use({
				...
				[NodeType.Element]: {
					...
					enter: (n) => {
						...
						// body (`i === attrCount`); the attr ref is only read on the
						// attribute steps, so it stays in bounds.
						for (let i = 0; i < attrCount + 1; i++) {
							const content = i === attrCount;
							/** @type {SourceType} */
							let type;
							/** @type {string} */
							let value;
							...
							/** @type {number} */
							let end;
							/** @type {import("./syntax").HtmlAttrRef} */
							let attr = 0;
							if (content) {
								...
								attr = A.attrAt(n, i);
								const item = sources[A.attrName(attr)];
								if (!item) continue;
								if (
								...
								if (A.attrValueStart(attr) === -1) {
									continue;
								}
								value = A.attrValue(attr);
								...
								start = A.attrValueStart(attr);
								end = A.attrValueEnd(attr);
								type =
									...
									try {
										...
										module.addError(
											new ModuleDependencyError(
												module,
												new WebpackError(
													`Bad value for attribute "${A.attrName(
														attr
										...
										if (
											...
											if (elementStart >= 0) {
												for (const copyableName of COPYABLE_SIBLING_ATTRS) {
													const copyableAttr = A.findAttr(n, copyableName);
													if (copyableAttr !== 0) {
											...
											if (elementStart >= 0 && elementName === nativeTag) {
												const integrityAttr = A.findAttr(n, "integrity");
												if (
													integrityAttr !== 0 &&
													A.attrNameStart(integrityAttr) >= 0
												) {
													let rs = A.attrNameStart(integrityAttr);
													while (
														rs > 0 &&
													...
													let re;
													const ivs = A.attrValueStart(integrityAttr);
													if (ivs === -1) {
														re = A.attrNameEnd(integrityAttr);
													} else {
														const q = source.charCodeAt(ivs - 1);
														const ive = A.attrValueEnd(integrityAttr);
														re =
												...
												if (elementKind === "script-module") {
													const typeAttr = A.findAttr(n, "type");
													if (
														typeAttr !== 0 &&
														A.attrValueStart(typeAttr) >= 0
													) {
														typeValueRange = [
															A.attrValueStart(typeAttr) - elementStart,
															A.attrValueEnd(typeAttr) - elementStart
														];
													}
												}
											}
