...
const SVG_NAMESPACE = buildHtmlAst.NS_SVG;

/**
 * @param {number} code char code
 * @returns {boolean} true when a CSS identifier character
 */
const isCssIdentChar = (code) =>
	(code >= 97 && code <= 122) ||
	(code >= 65 && code <= 90) ||
	(code >= 48 && code <= 57) ||
	code === 45 /* - */ ||
...
/**
 * Extracts `url(...)` references (CSS syntax, quoted or unquoted) from a
 * presentation attribute value like `fill="url(./paints.svg#g) red"`.
 * Malformed references are skipped — never an error, unlike `parseSrc`.
 * @param {string} input input
 * @returns {ParsedSource[]} parsed url() references
 */
const parseCssUrls = (input) => {
	...
	const len = input.length;
	let pos = 0;
	while ((pos = lower.indexOf("url(", pos)) !== -1) {
		// Not preceded by an identifier char (e.g. half of `image-url(`)
		if (pos > 0 && isCssIdentChar(input.charCodeAt(pos - 1))) {
			pos += 4;
			continue;
		}
		let i = pos + 4;
		while (i < len && isASCIIWhitespace(input.charCodeAt(i))) i++;
		const quote = input.charCodeAt(i);
		let start;
		let end;
		if (quote === 34 /* " */ || quote === 39 /* ' */) {
			start = i + 1;
			end = input.indexOf(input[i], start);
			if (end === -1) return refs;
			i = end + 1;
		} else {
			start = i;
			while (
				...
				!isASCIIWhitespace(input.charCodeAt(i))
			) {
				i++;
			}
			end = i;
		...
		while (i < len && isASCIIWhitespace(input.charCodeAt(i))) i++;
		if (input.charCodeAt(i) !== 41 /* ) */) {
			pos = i;
			continue;
		}
		if (end > start) refs.push([input.slice(start, end), start, end]);
		pos = i + 1;
	}
	return refs;
};
...
// SVG presentation attributes whose value can reference an external file
// through CSS `url()` syntax (`fill="url(./paints.svg#g)"`). Checked only on
// SVG-namespace elements, and only when the default sources are active.
const SVG_URL_FUNCTION_ATTRS = new Set([
	"clip-path",
	"cursor",
	"fill",
	"filter",
...
class HtmlParser extends Parser {
	...
	constructor(options) {
		...
		// Default-only behaviors (SVG presentation `url()` attributes); off
		// when the user replaces the sources list without `"..."`.
		this.defaultUrlSources = true;

		const sources = options.sources;
		if (sources === undefined || sources === true) return;
		if (sources === false) {
			this.sourcesByTag = EMPTY_SOURCES_BY_TAG;
			this.defaultUrlSources = false;
			return;
		}
		...
		for (const entry of sources) {
			...
			if (entry.tag === undefined) {
				...
				const adjusted = buildHtmlAst.SVG_TAG_ADJUST[tag];
		...
		this.defaultUrlSources = sources.includes("...");
	}
	...
	parse(source, state) {
		 ...
		 * @param {import("./buildHtmlAst").HtmlElement} node element node
		 * @returns {void}
		 */
		 ...
		 * @returns {number} the same position
		 */
		const identityTranslate = (pos) => pos;

		/**
		 * Finds a `srcdoc` attribute, parses its entity-decoded HTML document
		 * and walks it with offsets composed back to raw source positions.
		 * @param {import("./buildHtmlAst").HtmlAttribute[]} attrs attributes
		 * @param {(pos: number) => number} translate maps offsets in the surrounding document text to raw source offsets
		 * @returns {void}
		 */
		const walkSrcdocAttr = (attrs, translate) => {
			for (const attr of attrs) {
				if (attr.name !== "srcdoc") continue;
				if (attr.valueStart === undefined || attr.valueStart === -1) break;
				const value = attr.value;
				if (!value || !/\S/.test(value)) break;
				const { text, map } = decodeHtmlEntitiesWithMap(value, true);
				const base = attr.valueStart;
				/** @type {(pos: number) => number} */
				const inner = (pos) =>
					translate(base + (map === undefined ? pos : map[pos]));
				for (const child of buildHtmlAst(text).children) {
					walkSrcdocNode(child, inner);
				}
				break;
			}
		};
		...
		/**
		 * Restricted walk for `<iframe srcdoc>` content. The iframe is a
		 * separate browsing context, so every reference — scripts and
		 * stylesheets included — stays a plain asset URL instead of joining
		 * the outer page's entry graph. Inline `<style>`/`<script>` content
		 * and `style` attributes are left untouched.
		 * @param {import("./buildHtmlAst").HtmlNode} node node of the srcdoc document
		 * @param {(pos: number) => number} translate maps offsets in the srcdoc document text to raw source offsets
		 * @returns {void}
		 */
		const walkSrcdocNode = (node, translate) => {
			if (node.type !== "element") return;
			const attrs = node.attributes;
			const tagSources = this.sourcesByTag[node.tagName] || this.anyTagSources;
			const svgUrlAttrs =
				this.defaultUrlSources && node.namespace === SVG_NAMESPACE;
			...
			for (const attr of tagSources || svgUrlAttrs ? attrs : []) {
				const sourceItem = tagSources ? tagSources[attr.name] : undefined;
				const cssUrl =
					sourceItem === undefined &&
					svgUrlAttrs &&
					SVG_URL_FUNCTION_ATTRS.has(attr.name);
				if (!sourceItem && !cssUrl) continue;
				...
				const type = cssUrl
					? "src"
					: typeof (/** @type {SourceItem} */ (sourceItem).type) === "function"
						? /** @type {(attrs: Map<string, string>, css: boolean) => SourceType} */ (
								/** @type {SourceItem} */ (sourceItem).type
							)(getAttributesMap(), css)
						: /** @type {SourceType} */ (
								/** @type {SourceItem} */ (sourceItem).type
							);
				// CSS-typed custom attributes can't round-trip through the
				// extra entity-escaping layer — skip them here.
				if (
				...
				try {
					...
					const { line: sl, column: sc } = locConverter.get(
						translate(attr.valueStart)
					);
					const { line: el, column: ec } = locConverter.get(
						translate(attr.valueEnd)
					);
					module.addError(
						new ModuleDependencyError(
							module,
							new WebpackError(
								`Bad value for attribute "${attr.name}" on element "${node.tagName}" in srcdoc: ${
				...
				for (const parsedSource of parsedAttributeValue) {
					...
					const sourceStart = translate(attr.valueStart + rawStart);
					const sourceEnd = translate(attr.valueStart + rawEnd);
			...
			if (node.tagName === "iframe") {
				walkSrcdocAttr(attrs, translate);
			}

			if (node.templateContent !== undefined) {
				for (const child of node.templateContent.children) {
					walkSrcdocNode(child, translate);
				}
			}
		...
		const walkNode = (node) => {
			...
			// `<iframe srcdoc>` carries a whole entity-escaped HTML document —
			// parse the decoded text and rewrite its asset URLs in place.
			if (elementName === "iframe") {
				walkSrcdocAttr(attrs, identityTranslate);
			}

			// SVG presentation attributes can reference external files via
			// CSS `url()` syntax (`fill="url(./paints.svg#g)"`).
			const svgUrlAttrs =
				this.defaultUrlSources && node.namespace === SVG_NAMESPACE;
			const tagSources = this.sourcesByTag[elementName] || this.anyTagSources;
			if (!tagSources && !svgUrlAttrs && elementName !== "script") {
				walkElementChildren(node);
				return;
			}
