...
/**
 * Builds the per-tag attribute lookup table consumed by the walk:
 * `byTag[tag][attribute] = item`. Null-prototype dicts beat `Map`
 * here — `obj[name]` is faster than `map.get(name)` and a null
 * prototype keeps names like `__proto__` from colliding with
 * `Object.prototype`. Later entries override earlier ones per
 * `(tag, attribute)`.
 * @param {readonly SourceEntry[]} entries entries to merge
 * @returns {Record<string, Record<string, SourceItem>>} tag → attribute → item (any-tag entries live under `ANY_TAG`)
 */
const buildSourcesByTag = (entries) => {
...
// Built once at module load and shared by every parser created with
// the default `sources` config — the common case pays zero per-parser
// work and V8's inline caches stay hot across instances since the
// object shape is identical.
const DEFAULT_SOURCES_BY_TAG = buildSourcesByTag(DEFAULT_SOURCES);
...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		// build, which expands `"..."` in place via `flatMap`.
		...
		this.sourcesByTag =
			userSources === false
				...
				: userSources === undefined || userSources === true
					...
					: buildSourcesByTag(
							userSources.flatMap((e) => (e === "..." ? DEFAULT_SOURCES : [e]))
						);
	}
