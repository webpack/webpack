...
// Cheap pre-filter for an SVG presentation attribute (`fill`, `clip-path`, …):
// only tokenize values that can hold a `url(...)` FuncIRI. Most are plain
// colors/keywords (`#fff`, `red`, `none`).
const FUNC_IRI_URL_REGEXP = /url\(/i;

// SVG presentation attributes whose value is a CSS FuncIRI / paint that may
// carry an external `url(file.svg#id)` reference (internal `url(#id)` is left
// untouched). Names are the foreign-content lowercased forms.
const SVG_URL_PRESENTATION_ATTRS = new Set([
	"fill",
	"stroke",
	"clip-path",
	"mask",
			...
			.use({
				...
				[NodeType.Element]: {
					...
					enter: (n) => {
						...
						// SVG presentation attributes (`fill`, `clip-path`, `filter`,
						// `mask`, `marker-*`, `cursor`) take CSS `url(...)` FuncIRI
						// values. External `url(file.svg#id)` references are assets;
						// internal `url(#id)` is left untouched (skipped below). The CSS
						// lexer locates the url spans. Disabled with `sources: false`.
						if (
							node.namespace === NS_SVG &&
							this.sourcesByTag !== EMPTY_SOURCES_BY_TAG
						) {
							for (const attr of attrs) {
								if (!SVG_URL_PRESENTATION_ATTRS.has(attr.name)) continue;
								if (attr.valueStart === undefined || attr.valueStart === -1) {
									continue;
								}
								...
								if (
									!attributeValue ||
									!FUNC_IRI_URL_REGEXP.test(attributeValue)
								) {
									continue;
								}
								// Browsers decode entities before CSS-tokenizing the value;
								// map spans back to raw offsets (as the source loop does).
								...
								for (const [value, innerStart, innerEnd] of parseCssUrls(
									parseValue
								)) {
						...
						// `<iframe srcdoc>` is an entity-encoded HTML document; feed the
						// decoded markup back through the HTML pipeline as a virtual
						// `data:text/html` module so its asset URLs resolve relative to
						// this file, then write the processed HTML back into the attribute.
						if (elementName === "iframe") {
							for (const attr of attrs) {
								...
								const value = attr.value;
								// Only spin up a nested module when there is markup to rewrite.
								if (!value || !value.includes("<")) break;
								const htmlText = decodeHtmlEntities(value, true);
								const request = `data:text/html;base64,${Buffer.from(
									htmlText,
									"utf8"
								).toString("base64")}`;
								const { line: sl, column: sc } = locConverter.get(
								...
								const dep = new HtmlInlineHtmlDependency(request, [
									attr.valueStart,
									attr.valueEnd
								]);
								dep.setLoc(sl, sc, el, ec);
						...
						for (const attr of tagSources ? attrs : []) {
							...
							for (const parsedSource of parsedAttributeValue) {
								...
								// TODO external/absolute URLs (`http://`, `https://`,
								// protocol-relative `//`) are still turned into dependencies
								// and resolve to an empty `data:,` when no scheme handler
								// applies, they should be left untouched instead.
								const rawStart =
								...
								// `hasOwnCrossOrigin` lets the template skip
								// `output.crossOriginLoading` when the author set `crossorigin`.
