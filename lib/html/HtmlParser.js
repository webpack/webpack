...
/** @typedef {"src" | "script" | "script-module" | "modulepreload" | "stylesheet" | "stylesheet-inline"} SourceKind */
/** @typedef {{ parse: (input: string) => ParsedSource[] | undefined, kind: SourceKind | ((attrs: Map<string, string>, css: boolean) => SourceKind), filter?: (attributes: Map<string, string>) => boolean }} SourceItem */

/** @type {SourceItem} */
const PLAIN_SRC = { parse: parseSrc, kind: "src" };
/** @type {SourceItem} */
const PLAIN_SRCSET = { parse: parseSrcset, kind: "src" };

/** @type {Map<string, Map<string, SourceItem>>} */
const DEFAULT_SOURCES = new Map([
	["audio", new Map([["src", PLAIN_SRC]])],
	["embed", new Map([["src", PLAIN_SRC]])],
	[
		"img",
		new Map([
			["src", PLAIN_SRC],
		...
		new Map([
			[
				...
				/** @type {SourceItem} */ ({
					...
					// seeing them as data blocks. `type="module"` opts into
					// the ESM entry chunk; everything else in
					// `JS_SCRIPT_TYPES` is a classic script.
					kind: (attrs) =>
...
/**
 * Resolves an `HtmlParserOptions.sources` value into the per-tag and
 * any-tag maps consumed by the parser walk. Disabled extraction returns
 * empty maps so the walk has a single code path.
 *
 * User-supplied sources translate one-to-one onto the same `SourceItem`
 * shape as the built-in defaults: the user's `type` becomes the item's
 * `kind` (with `"srcset"` mapped to `parse: parseSrcset, kind: "src"`),
 * so the walk dispatches off `kind` without needing to distinguish
 * defaults from user sources.
 * @param {HtmlParserOptions["sources"]} sources sources option
 * @returns {{ byTag: Map<string, Map<string, SourceItem>>, anyTag: Map<string, SourceItem> }} resolved maps
 */
const resolveSources = (sources) => {
	if (sources === undefined || sources === true) {
		return { byTag: DEFAULT_SOURCES, anyTag: new Map() };
	}
	if (sources === false) {
		return { byTag: new Map(), anyTag: new Map() };
	}

	/** @type {Map<string, Map<string, SourceItem>>} */
	const byTag = new Map();
	/** @type {Map<string, SourceItem>} */
	const anyTag = new Map();

	for (const entry of sources) {
		if (entry === "...") {
			for (const [tag, attrs] of DEFAULT_SOURCES) {
				const existing = byTag.get(tag);
				if (existing) {
					for (const [attribute, item] of attrs) existing.set(attribute, item);
				} else {
					byTag.set(tag, new Map(attrs));
				}
			}
			continue;
		}
		...
		const item =
			entry.type === "srcset"
				...
				: entry.type === "src"
					? PLAIN_SRC
					: { parse: parseSrc, kind: entry.type };
		const attribute = entry.attribute.toLowerCase();
		if (entry.tag === undefined) {
			anyTag.set(attribute, item);
		} else {
			const tag = entry.tag.toLowerCase();
			let target = byTag.get(tag);
			if (!target) byTag.set(tag, (target = new Map()));
			target.set(attribute, item);
		}
	}
...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		this.css = css;
		this.resolvedSources = resolveSources(options && options.sources);
	}
	...
	parse(source, state) {
		 ...
		 * @param {"script" | "script-module"} kind chunk kind decided by the parser
		 * @param {string} input full source string
		 * @returns {void}
		 */
		const reconcileScriptTypeAttr = (typeAttr, nameEnd, kind, input) => {
			if (this.outputModule && kind === "script") {
			...
			} else if (!this.outputModule && kind === "script-module" && typeAttr) {
		...
		// Script src / modulepreload references are collected per-kind
		 ...
		 * @property {"script" | "script-module" | "modulepreload" | "stylesheet"} kind
		 */
		...
		walkHtmlTokens(source, 0, {
			...
			closeTag: (input, start, end, nameStart, nameEnd) => {
				...
				if (pendingInlineScript) {
					...
					if (elName === "script") {
						...
						const kind = useEsmEntry ? "script-module" : "script";
						...
						const collection =
							kind === "script" ? scriptEntries : scriptModuleEntries;
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				// Per-tag and any-tag source maps from the resolved
				// `sources` config (defaults, custom list, or disabled).
				// When sources are disabled both maps are empty — the same
				// no-match code path then leaves URL attributes untouched
				// and only the inline-`<script>` handler runs.
				const { byTag, anyTag } = this.resolvedSources;
				const tagSources = byTag.get(elementName);
				const hasMatchingSources = Boolean(tagSources) || anyTag.size > 0;

				if (!hasMatchingSources && elementName !== "script") {
					pendingAttributes.length = 0;
					return end;
				}
				...
				for (const attr of hasMatchingSources ? pendingAttributes : []) {
					const attributeName = input
						.slice(attr.nameStart, attr.nameEnd)
						.toLowerCase();
					// Per-tag sources win over any-tag so a user can shadow a
					// tag-less entry with a tag-specific one.
					const sourceItem =
						(tagSources && tagSources.get(attributeName)) ||
						anyTag.get(attributeName);
					...
					// pick a kind based on the element's other attributes
					// (`rel`, `type`); user-supplied sources have a static
					// `kind` string set in `resolveSources`.
					const kind =
						typeof sourceItem.kind === "function"
							? sourceItem.kind(getAttributesMap(), Boolean(this.css))
							: sourceItem.kind;
					...
					for (const parsedSource of parsedAttributeValue) {
						...
						// Entry kinds: script, script-module, modulepreload, stylesheet.
						const entryName = `__html_${moduleHash}_${nextEntryIndex++}`;
						...
						const entryCategory =
							kind === "stylesheet"
								? "css-import"
								: kind === "script-module" || kind === "modulepreload"
									? "esm"
									: undefined;
						...
						if (
							elementName === "script" &&
							(kind === "script" || kind === "script-module")
						) {
