	...
	parse(source, state) {
		...
		const entryNameFor = (requested, request, index, type) => {
			...
			if (typeof name !== "string" || name === "") {
				const minted = `${entryPrefix}-${index}`;
				claimedEntryNames.add(minted);
				return minted;
			}
			// Two tags of one page may not claim one name: webpack appends to the
			// entry that has it, merging code the document keeps apart.
			const claimed = claimedEntryNames.has(name) ? `${name}-${index}` : name;
