...
/** @typedef {{ parse: (input: string) => ParsedSource[] | undefined, filter?: (attributes: Map<string, string>) => boolean, entry?: boolean | ((attributes: Map<string, string>) => boolean), entryCategory?: string }} SourceItem */

/**
 * Resolves an `HtmlParserOptions.sources` value into the per-tag and
 * wildcard maps consumed by the parser walk. Returns `null` when source
 * extraction is disabled entirely.
 * @param {HtmlParserOptions["sources"]} sources sources option
 * @returns {null | { byTag: Map<string, Map<string, SourceItem>>, wildcard: Map<string, SourceItem> }} resolved maps, or null when disabled
 */
const resolveSources = (sources) => {
	if (sources === false) return null;
	if (sources === undefined || sources === true) {
		return { byTag: DEFAULT_SOURCES, wildcard: new Map() };
	}
	 ...
	 */
	const setByTag = (tag, attribute, item) => {
		let attrs = byTag.get(tag);
		if (!attrs) {
			attrs = new Map();
			byTag.set(tag, attrs);
		}
		attrs.set(attribute, item);
	};

	for (const entry of sources.list) {
		if (entry === "...") {
			// Inline defaults — keep the original SourceItem references so
			// the entry / filter / category metadata attached to defaults
			// (e.g. `<script src>` becoming a compilation entry) survives.
			for (const [tag, attrs] of DEFAULT_SOURCES) {
				for (const [attribute, item] of attrs) {
					setByTag(tag, attribute, item);
				}
			}
			continue;
		}
		const parse = entry.type === "srcset" ? parseSrcset : parseSrc;
		// User-supplied sources are always pure URL rewrites — no entry,
		// no filter, no entry category. Users wanting `<script src>` to
		// keep becoming a compilation entry should include `"..."` in
		// their list to inline the defaults.
		/** @type {SourceItem} */
		const item = { parse };
		const tag = (entry.tag || "*").toLowerCase();
		const attribute = entry.attribute.toLowerCase();
		if (tag === "*") {
			wildcard.set(attribute, item);
		} else {
			setByTag(tag, attribute, item);
		}
	}

	return { byTag, wildcard };
};
	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				// `module.parser.html.sources: false` disables URL-attribute
				// extraction entirely: `<img src>`, `<link href>`,
				// `<script src>` and friends are left untouched and never
				// turned into webpack dependencies or compilation entries.
				// Inline `<script>` bodies are still processed further down —
				// those aren't URL extraction. Per-tag and wildcard maps come
				// from the resolved `sources` config (defaults, custom list,
				// or disabled).
				const resolvedSources = this.resolvedSources;
				const tagSources =
					resolvedSources && resolvedSources.byTag.get(elementName);
				const wildcardSources =
					resolvedSources && resolvedSources.wildcard.size > 0
						? resolvedSources.wildcard
						: undefined;
				const hasMatchingSources = Boolean(tagSources || wildcardSources);
				...
				const attributesToProcess = hasMatchingSources ? pendingAttributes : [];
				for (const attr of attributesToProcess) {
					const attributeName = input
						.slice(attr.nameStart, attr.nameEnd)
						.toLowerCase();
					// Per-tag sources win over wildcard so a user can shadow a
					// `tag: '*'` entry with a tag-specific one.
					const sourceItem =
						(tagSources && tagSources.get(attributeName)) ||
						(wildcardSources && wildcardSources.get(attributeName));

					if (!sourceItem) continue;

					// User-supplied sources never trigger entry creation /
					// stylesheet handling — only the built-in defaults do.
					// Identify a default source by strict-equality with its
					// `DEFAULT_SOURCES` slot (`"..."` expansion preserves
					// references, so user lists that opt into defaults
					// keep entry behaviour).
					...
					// link. Only built-in defaults trigger entry creation —
					// custom user sources from `sources.list` are always plain
					// URL rewrites.
