...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		// Default and `false` paths just reference a precomputed table —
		// no allocation, no loop, no helper call.
		if (userSources === false) {
			this.sourcesByTag = EMPTY_SOURCES_BY_TAG;
			this.anyTagSources = undefined;
			return;
		}
		if (userSources === undefined || userSources === true) {
			this.sourcesByTag = DEFAULT_SOURCES_BY_TAG;
			this.anyTagSources = undefined;
			return;
		}

		// User array — build per-tag and any-tag tables. `"..."` expands
		// `DEFAULT_SOURCES_BY_TAG` in place at its array position; later
		// entries override earlier ones per `(tag, attribute)`.
		/** @type {Record<string, Record<string, SourceItem>>} */
		const byTag = {};
		...
		// Fold any-tag entries into every per-tag bucket as a base so the
		// walk's per-attribute lookup is one property read. Tag-specific
		// entries still win via spread order.
		if (anyTag) {
