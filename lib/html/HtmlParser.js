...
/** @typedef {{ parse: (input: string) => ParsedSource[] | undefined, type: SourceType | ((attrs: Map<string, string>, css: boolean) => SourceType), filter?: (attributes: Map<string, string>) => boolean }} SourceItem */

/** @type {SourceItem} */
const PLAIN_SRC = { parse: parseSrc, type: "src" };
/** @type {SourceItem} */
const PLAIN_SRCSET = { parse: parseSrcset, type: "src" };
...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		// `sources: true` / `undefined` → defaults; `false` → nothing;
		// an array merges user entries on top of optional defaults
		// (a `"..."` literal in the array expands the defaults in place).
		// User entries with `tag` omitted go into `anyTagSources`; the
		// rest into `sourcesByTag`.
		const sources = options && options.sources;
		/** @type {Map<string, Map<string, SourceItem>>} */
		const byTag =
			sources === undefined || sources === true ? DEFAULT_SOURCES : new Map();
			...
			for (const entry of sources) {
				if (entry === "...") {
					for (const [tag, attrs] of DEFAULT_SOURCES) {
						...
						if (existing) {
							for (const [a, i] of attrs) existing.set(a, i);
						} else {
				...
				const item =
					entry.type === "srcset"
						...
						: entry.type === "src"
							? PLAIN_SRC
							: { parse: parseSrc, type: entry.type };
	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				const tagSources = this.sourcesByTag.get(elementName);
				const anyTag = this.anyTagSources;
