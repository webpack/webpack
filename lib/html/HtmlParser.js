							...
							switch (type) {
								...
								case "script-module":
									if (content) {
										...
										// `async` evaluates whenever it loads and a `<template>`
										// body never does, so neither joins a run. A deferred tag
										// runs wherever it sits, so only module output spans markup.
										const joinsRun =
											templateDepth === 0 &&
										...
										if (joinsRun) {
											if (inlineRunEntryName === undefined) {
