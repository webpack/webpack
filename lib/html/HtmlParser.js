...
class HtmlParser extends Parser {
	 ...
	 */
	constructor(hashFunction, context) {
		super();
	...
	parse(source, state) {
		...
		// HTML modules in the same compilation. The module identifier
		// (absolute resource path) is unique per module; contextifying it
		// against the compilation root keeps the hash stable across machines
		// for the same project layout.
		const moduleHash = createHash(this.hashFunction || "md4")
			.update(
				this.context
					? contextify(this.context, module.identifier())
					: module.identifier()
			)
			.digest("hex")
			.slice(0, 8);
