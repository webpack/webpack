	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				// sentinel `ANY_TAG` key). Two property reads — the
				// cheapest possible test. Most elements (`<div>`, `<p>`,
				// …) miss both and short-circuit out without ever
				// touching `pendingAttributes`. `<script>` always falls
				// through so the inline-script handler below can pick
				// it up.
