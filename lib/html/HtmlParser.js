...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		// User array — build per-tag and any-tag tables. `"..."` merges
		// `DEFAULT_SOURCES_BY_TAG` in such that user entries always win,
		// independent of where `"..."` sits in the array.
		/** @type {Record<string, Record<string, SourceItem>>} */
		const byTag = {};
		...
		for (const entry of sources) {
			if (entry === "...") {
				for (const tag of Object.keys(DEFAULT_SOURCES_BY_TAG)) {
					byTag[tag] = { ...DEFAULT_SOURCES_BY_TAG[tag], ...byTag[tag] };
				}
				continue;
			}
		...
		// Tag-specific entries still win via spread order.
		if (anyTag) {
