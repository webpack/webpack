...
/** @typedef {"src" | "script" | "script-module" | "modulepreload" | "stylesheet" | "stylesheet-inline"} SourceType */
/** @typedef {SourceType | ((attrs: Map<string, string>, css: boolean) => SourceType)} SourceTypeOrResolver */
/**
 * The flat entry shape shared by built-in defaults (`DEFAULT_SOURCES`)
 * and user-supplied `HtmlParserOptions.sources` entries. `"srcset"` is
 * the user-facing alias for "parse with `parseSrcset`, dependency kind
 * `src`" — defaults can also use a function for polymorphic dispatch.
 * @typedef {{ tag?: string, attribute: string, type: SourceTypeOrResolver | "srcset", filter?: (attributes: Map<string, string>) => boolean }} SourceEntry
 */
/** @typedef {{ parse: (input: string) => ParsedSource[] | undefined, type: SourceTypeOrResolver, filter?: (attributes: Map<string, string>) => boolean }} SourceItem */
...
const DEFAULT_SOURCES = [
	...
	{ tag: "object", attribute: "data", type: "src" },
	{
		tag: "script",
		attribute: "src",
		// Non-JS `<script>` types (e.g. `application/ld+json`,
		// `importmap`) stay plain asset URLs so the browser keeps seeing
		// them as data blocks. `type="module"` opts into the ESM entry
		// chunk; everything else in `JS_SCRIPT_TYPES` is a classic script.
		type: (attrs) =>
	...
	{ tag: "image", attribute: "href", type: "src" },
	{ tag: "use", attribute: "xlink:href", type: "src" },
	{ tag: "use", attribute: "href", type: "src" }
];

/**
 * @typedef {object} ResolvedSources
 * @property {Record<string, Record<string, SourceItem>>} sourcesByTag tag → attribute → resolved source item
 * @property {Record<string, SourceItem>} anyTagSources attribute → resolved source item (entries with no `tag`)
 * @property {boolean} hasAnyTagSource whether any tag-less entry exists; used to short-circuit the fallback lookup
 */

/**
 * Builds the per-tag and any-tag attribute lookup tables consumed by
 * the walk. Plain dictionary objects (`Object.create(null)`) are used
 * instead of `Map` for two reasons: (1) `obj[name]` is faster than
 * `map.get(name)` once the shape stabilizes, and (2) a null prototype
 * means tag/attribute names like `__proto__` or `toString` can't
 * collide with `Object.prototype`. Later entries override earlier ones
 * per `(tag, attribute)`.
 * @param {readonly SourceEntry[]} entries entries to merge
 * @returns {ResolvedSources} per-tag and any-tag lookup tables
 */
const buildResolvedSources = (entries) => {
	/** @type {Record<string, Record<string, SourceItem>>} */
	const sourcesByTag = Object.create(null);
	/** @type {Record<string, SourceItem>} */
	const anyTagSources = Object.create(null);
	let hasAnyTagSource = false;
	for (const entry of entries) {
		const isSrcset = entry.type === "srcset";
		/** @type {SourceItem} */
		const item = {
			parse: isSrcset ? parseSrcset : parseSrc,
			type: isSrcset ? "src" : /** @type {SourceTypeOrResolver} */ (entry.type),
			filter: entry.filter
		};
		const attribute = entry.attribute.toLowerCase();
		if (entry.tag === undefined) {
			anyTagSources[attribute] = item;
			hasAnyTagSource = true;
		} else {
			const tag = entry.tag.toLowerCase();
			let bucket = sourcesByTag[tag];
			if (!bucket) sourcesByTag[tag] = bucket = Object.create(null);
			bucket[attribute] = item;
		}
	}
	return { sourcesByTag, anyTagSources, hasAnyTagSource };
};

// Precompute the defaults once at module load. Every parser created
// with the default `sources` config shares this instance — no
// re-allocation per parser and V8's inline caches stay hot across
// instances since the object shapes are identical.
const DEFAULT_RESOLVED_SOURCES = buildResolvedSources(DEFAULT_SOURCES);

/** @type {ResolvedSources} */
const EMPTY_RESOLVED_SOURCES = {
	sourcesByTag: Object.create(null),
	anyTagSources: Object.create(null),
	hasAnyTagSource: false
};
...
class HtmlParser extends Parser {
	 ...
	 * @param {HtmlParserOptions=} options parser options (from `module.parser.html`)
	 */
	constructor(hashFunction, context, outputModule, css, options) {
		super();
		...
		// Resolve `sources`: `true` / `undefined` reuses the precomputed
		// default tables (no per-parser allocation); `false` reuses the
		// shared empty tables; an array spreads `DEFAULT_SOURCES` in place
		// of any `"..."` literal so user entries can prepend, append, or
		// override per `(tag, attribute)` — later entries win.
		const sources = options && options.sources;
		const resolved =
			sources === false
				? EMPTY_RESOLVED_SOURCES
				: sources === undefined || sources === true
					? DEFAULT_RESOLVED_SOURCES
					: buildResolvedSources(
							sources.flatMap((e) => (e === "..." ? DEFAULT_SOURCES : [e]))
						);
		this.sourcesByTag = resolved.sourcesByTag;
		this.anyTagSources = resolved.anyTagSources;
		this.hasAnyTagSource = resolved.hasAnyTagSource;
	}
	...
	parse(source, state) {
		...
		const reconcileScriptTypeAttr = (typeAttr, nameEnd, type, input) => {
			if (this.outputModule && type === "script") {
			...
			} else if (!this.outputModule && type === "script-module" && typeAttr) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				// Per-tag source dictionary (or undefined when this tag has
				// none). A single property read on a null-prototype object
				// is the cheapest test we can do, and most elements
				// (`<div>`, `<p>`, …) miss here — they short-circuit out
				// without ever touching `pendingAttributes`. `<script>`
				// always falls through so the inline-script handler below
				// can pick it up.
				const tagSources = this.sourcesByTag[elementName];
				if (
					tagSources === undefined &&
					!this.hasAnyTagSource &&
					elementName !== "script"
				) {
					pendingAttributes.length = 0;
					return end;
				...
				const anyTagSources = this.anyTagSources;
				for (const attr of pendingAttributes) {
					const attributeName = input
						.slice(attr.nameStart, attr.nameEnd)
						.toLowerCase();
					// Per-tag wins; fall back to any-tag. Both are
					// null-prototype dictionaries, so a miss is a single
					// undefined read with no prototype-chain walk. The
					// any-tag table is empty for the defaults, so the second
					// read is a free no-op there.
					const sourceItem =
						(tagSources !== undefined && tagSources[attributeName]) ||
						anyTagSources[attributeName];
					...
					// `type` string set at construction time.
					const type =
						typeof sourceItem.type === "function"
							? sourceItem.type(getAttributesMap(), Boolean(this.css))
							: sourceItem.type;
