...
class HtmlParser extends Parser {
	...
	constructor(options) {
		...
		// Emit a `<script>` without an explicit `type` as an ES module
		// script. Combined with
		// `output.module` (which always needs module scripts) when parsing.
		/** @type {boolean} */
		this.scriptModule = options.scriptModule === true;
	...
	parse(source, state) {
		...
		// Whether a `<script>` without an explicit `type` is emitted as an
		// ES module script. ESM
		// output always requires module scripts; the `scriptModule` parser
		// option additionally opts in even for classic output.
		const moduleScripts = this.scriptModule || outputModule;
		...
		const doc = walkHtmlTokens.buildAst(source);

		/**
		 * @param {import("./walkHtmlTokens").HtmlAttribute | undefined} typeAttr type attribute
		...
		const reconcileScriptTypeAttr = (typeAttr, nameEnd, type, input) => {
			...
			} else if (!moduleScripts && type === "script-module" && typeAttr) {
		...
		/**
		 * @param {import("./walkHtmlTokens").HtmlNode[]} children children
		 * @returns {void}
		 */
		...
		/** @param {import("./walkHtmlTokens").HtmlNode} node AST node */
		const walkNode = (node) => {
			...
			for (const attr of tagSources ? attrs : []) {
				...
				for (const parsedSource of parsedAttributeValue) {
					...
					const willBeModuleScript =
						type === "script-module" || (moduleScripts && type === "script");
			...
			if (elementName === "script") {
				...
				if (!hasSrc && isExecutableJsScript(scriptAttrs)) {
					...
					if (jsContent.trim() !== "") {
						...
						// An explicit `type="module"` always means ESM; `scriptModule`
						// opts in inline scripts that have no `type` too. The entry follows
						// that, while the tag's reconciliation reflects the source
						// attribute so the `type="module"` upgrade still fires.
						const explicitModule = isModuleScript(scriptAttrs);
						const useEsmEntry = explicitModule || this.scriptModule;
						const entryName = `__html_${moduleHash}_${nextEntryIndex++}`;
						const { line: sl, column: sc } = locConverter.get(contentStart);
						const { line: el, column: ec } = locConverter.get(contentEnd);
						...
						reconcileScriptTypeAttr(
							typeAttr,
							node.nameEnd,
							explicitModule ? "script-module" : "script",
							source
						);

						const collection = useEsmEntry
							? scriptModuleEntries
							: scriptEntries;
						collection.push({
							request,
							entryName,
							type: useEsmEntry ? "script-module" : "script"
						});
					}
				}
			}
