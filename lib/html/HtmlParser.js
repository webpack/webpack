 ...
 * @param {import("./syntax").HtmlAst} api AST accessor
 * @param {string} source HTML source
 * @param {import("./syntax").HtmlAttributeRef} attr attribute ref
 * @returns {string} the attribute's source slice
 */
const attrSourceSpan = (api, source, attr) => {
	const valueStart = api.attributeValueStart(attr);
	const valueEnd = api.attributeValueEnd(attr);
	const end =
		valueStart === -1
			? api.attributeNameEnd(attr)
			: source.charCodeAt(valueStart - 1) === CC_QUOTATION ||
				  source.charCodeAt(valueStart - 1) === CC_APOSTROPHE
				? valueEnd + 1
				: valueEnd;
	return source.slice(api.attributeNameStart(attr) - 1, end);
};
...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		 ...
		 */
		const reconcileScriptTypeAttr = (api, typeAttr, nameEnd, type, input) => {
			const valueStart =
				typeAttr !== 0 ? api.attributeValueStart(typeAttr) : -1;
			const valueEnd = typeAttr !== 0 ? api.attributeValueEnd(typeAttr) : -1;
			if (outputModule && type === "script") {
				...
				let attrEnd;
				if (valueStart === -1) {
					attrEnd = api.attributeNameEnd(typeAttr);
				} else if (input[valueEnd] === '"' || input[valueEnd] === "'") {
					attrEnd = valueEnd + 1;
				} else {
					attrEnd = valueEnd;
				}
				let attrStart = api.attributeNameStart(typeAttr);
				if (
					attrStart > 0 &&
		...
		new SourceProcessor()
			.use({
				[NodeType.Comment]: (api, n) => {
					const start = api.start(n);
					const end = api.end(n);
				...
				[NodeType.Element]: {
					...
					enter: (api, n) => {
						...
						const elementName = api.tagName(n);
						const attributeCount = api.attributeCount(n);
						const elementStart = api.start(n);
						const tagEnd = api.tagEnd(n);
						const nameEnd = api.nameEnd(n);
						...
						const getAttributesMap = () => {
							...
							for (let i = 0; i < attributeCount; i++) {
								const attr = api.attributeAt(n, i);
								// Decoded values — filters and type resolvers compare what
								// the browser sees (e.g. `rel="&#105;con"` means `icon`)
								attributesMap.set(
									api.attributeName(attr),
									decodeHtmlEntities(api.attributeValue(attr), true)
								);
							}
						...
						for (let i = 0; i < attributeCount + 1; i++) {
							...
							if (content) {
								...
								const bodyEnd = api.contentEnd(n);
								...
								attr = api.attributeAt(n, i);
								const item = sources[api.attributeName(attr)];
								if (!item) continue;
								if (
									item.namespace !== undefined &&
									api.namespace(n) !== item.namespace
								) {
									continue;
								}
								// Adoption-agency clones carry no offsets; skip blank values.
								if (api.attributeValueStart(attr) === -1) {
									continue;
								}
								value = api.attributeValue(attr);
								...
								start = api.attributeValueStart(attr);
								end = api.attributeValueEnd(attr);
								type =
									...
									try {
										...
										module.addError(
											new ModuleDependencyError(
												module,
												new WebpackError(
													`Bad value for attribute "${api.attributeName(
														attr
										...
										if (
											...
											if (elementStart >= 0) {
												for (const copyableName of COPYABLE_SIBLING_ATTRS) {
													const copyableAttr = api.findAttribute(
														n,
														copyableName
													);
													if (copyableAttr !== 0) {
											...
											if (elementStart >= 0 && elementName === nativeTag) {
												const integrityAttr = api.findAttribute(n, "integrity");
												if (
													integrityAttr !== 0 &&
													api.attributeNameStart(integrityAttr) >= 0
												) {
													let rs = api.attributeNameStart(integrityAttr);
													while (
														rs > 0 &&
													...
													let re;
													const ivs = api.attributeValueStart(integrityAttr);
													if (ivs === -1) {
														re = api.attributeNameEnd(integrityAttr);
													} else {
														const q = source.charCodeAt(ivs - 1);
														const ive = api.attributeValueEnd(integrityAttr);
														re =
												...
												if (elementKind === "script-module") {
													const typeAttr = api.findAttribute(n, "type");
													if (
														typeAttr !== 0 &&
														api.attributeValueStart(typeAttr) >= 0
													) {
														typeValueRange = [
															api.attributeValueStart(typeAttr) - elementStart,
															api.attributeValueEnd(typeAttr) - elementStart
														];
													}
												}
											}
