...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		// HTML modules in the same compilation. `rawRequest` is the original
		// request string (e.g. `./page.html`) and is stable across machines.
		const moduleHash = createHash(this.hashFunction || "md4")
			.update(
				/** @type {string} */ (
					/** @type {EXPECTED_ANY} */ (module).rawRequest || module.identifier()
				)
			)
			.digest("hex")
			.slice(0, 8);
		...
		// entries with a chain-style `dependOn` so the first entry in each
		// group provides the runtime and the rest share it.
		/**
		 * @typedef {object} EntryScriptInfo
		 * @property {string} request
		 * @property {string} entryName
		 * @property {"classic" | "esm"} kind
		 */
				...
				for (const attr of pendingAttributes) {
					...
					for (const parsedSource of parsedAttributeValue) {
						...
						if (isEntry) {
							...
							const kind = useEsmEntry ? "esm" : "classic";
							...
							const collection =
								kind === "classic" ? classicEntries : esmEntries;
		...
		// Hand off the collected entries to HtmlModulesPlugin; it creates real
		// compilation entries (with chain-style dependOn within each group)
		// during the finishMake hook so seal sees them in `compilation.entries`.
		if (classicEntries.length > 0 || esmEntries.length > 0) {
