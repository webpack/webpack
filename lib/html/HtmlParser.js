...
/** @typedef {"src" | "srcset" | "css-url" | "msapplication-task" | "script" | "script-module" | "modulepreload" | "stylesheet" | "stylesheet-style" | "stylesheet-style-attribute"} SourceType */
...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * @param {ConstDependency | HtmlSourceDependency | HtmlScriptSrcDependency | HtmlInlineStyleDependency | HtmlInlineScriptDependency} dep dependency
				...
				[NodeType.Element]: {
					...
					enter: (n) => {
						...
						for (let i = 0; i <= attrs.length; i++) {
							// The final iteration is the element body, if any.
							...
							switch (type) {
								...
								case "stylesheet": {
									...
									try {
										...
										module.addError(
											new ModuleDependencyError(
												module,
												new WebpackError(
													`Bad value for attribute "${a.name}" on element "${elementName}": ${
														/** @type {Error} */ (err).message
													}`
												),
												{
									...
									for (const [url, us, ue] of parsed) {
										...
										// `HtmlScriptSrcDependency`, recorded for HtmlModulesPlugin.
										if (
											type === "script" ||
											type === "script-module" ||
											type === "modulepreload" ||
