...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		// Resolve `sources`: `true` / `undefined` → defaults; `false` →
		// nothing; an array spreads `DEFAULT_SOURCES` in place of any
		// `"..."` literal so user entries can prepend, append, or override
		// per (tag, attribute) — later entries win.
		const sources = options && options.sources;
		/** @type {readonly SourceEntry[]} */
		const entries =
			sources === false
				? []
		...
		// Build one merged lookup keyed by `${tag}\0${attribute}`. Tag-less
		// entries land under `\0${attribute}` and act as the fallback when
		// the tag-specific lookup misses (see the walk below).
		...
		for (const entry of entries) {
			...
			const item = {
				...
				type: entry.type === "srcset" ? "src" : entry.type,
				filter: entry.filter
			};
			if (entry.tag === undefined) {
				merged.set(`\0${attribute}`, item);
				hasAnyTagSource = true;
			} else {
				merged.set(`${entry.tag.toLowerCase()}\0${attribute}`, item);
			}
		}
		this.sources = merged;
		this.hasAnyTagSource = hasAnyTagSource;
	}
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				for (const attr of pendingAttributes) {
					const attributeName = input
						.slice(attr.nameStart, attr.nameEnd)
						.toLowerCase();
					// Per-tag sources win over any-tag (`\0attribute`) so a user
					// can shadow a tag-less entry with a tag-specific one. We
					// only consult the any-tag fallback when at least one
					// tag-less entry exists (it's empty for the defaults).
					const sourceItem =
						this.sources.get(`${elementName}\0${attributeName}`) ||
						(this.hasAnyTagSource
							? this.sources.get(`\0${attributeName}`)
							: undefined);
