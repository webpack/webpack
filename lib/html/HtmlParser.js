...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * that follow it.
		 * @param {import("./syntax").HtmlAttribute} hrefAttr the base's href attribute
		 */
		const resolveDocumentBase = (hrefAttr) => {
			documentBase = decodeHtmlEntities(hrefAttr.value, true).trim();
			...
			if (depth > 0) baseUrlPrefix = "../".repeat(depth);
		};
				...
				[NodeType.Element]: {
					...
					enter: (n) => {
						...
						if (
							...
							const hrefAttr = findAttr(attrs, "href");
							if (hrefAttr) resolveDocumentBase(hrefAttr);
						}
