...
/** @typedef {"src" | "srcset" | "css-url" | "msapplication-task" | "script" | "script-module" | "modulepreload" | "stylesheet" | "html" | "stylesheet-style" | "stylesheet-style-attribute" | "srcdoc"} SourceType */
/** Entry types: a `type` whose value is loaded as its own compilation entry chunk. */
/** @typedef {"script" | "script-module" | "modulepreload" | "stylesheet" | "html"} EntrySourceType */
...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		 ...
		 * @property {"script" | "script-module" | "modulepreload" | "stylesheet" | "html"} type
		 */
		/** @type {HtmlEntryInfo[]} */
		const scriptEntries = [];
		/** @type {HtmlEntryInfo[]} */
								...
								case "html": {
									...
									for (const [url, us, ue] of parsed) {
										...
										if (type === "html") {
											...
											setLoc(dep, s, e);
											module.addPresentationalDependency(dep);
											htmlLinkEntries.push({ request: url, entryName, type });
											continue;
										}
