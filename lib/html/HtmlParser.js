...
class HtmlParser extends Parser {
	...
	parse(source, state) {
		...
		/**
		 * Tracks the resource-hint values from the most recent comment that
		 * carried a `webpackPrefetch` / `webpackPreload` / `webpackFetchPriority`
		 * value, so that the next `HtmlSourceDependency` (the next asset URL
		 * attribute) can pick them up.
		...
		const walkNode = (node) => {
			if (node.type === "comment") {
				...
				if (options.webpackIgnore === undefined) {
					pendingWebpackIgnore = undefined;
				} else if (typeof options.webpackIgnore !== "boolean") {
					module.addWarning(
			...
			for (const attr of tagSources ? attrs : []) {
				...
				for (const parsedSource of parsedAttributeValue) {
					...
					if (type === "src" || type === "srcset") {
						...
						const hintDefaults = resolveResourceHints(value);
						if (hintDefaults.prefetch) dep.prefetch = true;
						if (hintDefaults.preload) dep.preload = true;
						if (hintDefaults.fetchPriority) {
							dep.fetchPriority = hintDefaults.fetchPriority;
						}
						// `<!-- webpackPrefetch: true -->` (etc.) immediately
						// before this tag wins over the project-wide default.
						if (pendingHints) {
							if (pendingHints.prefetch !== undefined) {
								dep.prefetch = pendingHints.prefetch;
							}
							if (pendingHints.preload !== undefined) {
								dep.preload = pendingHints.preload;
							}
							if (pendingHints.fetchPriority !== undefined) {
								dep.fetchPriority = pendingHints.fetchPriority;
							}
							pendingHints = undefined;
						}
						dep.setLoc(sl, sc, el, ec);
