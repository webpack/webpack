...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		this.css = css;
		this.sources = !options || options.sources !== false;
	}
	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				// those aren't URL extraction.
				const sources = this.sources
					? DEFAULT_SOURCES.get(elementName)
					: undefined;

				if (!sources && elementName !== "script") {
					pendingAttributes.length = 0;
					return end;
				}
				...
				const attributesToProcess = sources ? pendingAttributes : [];
				for (const attr of attributesToProcess) {
					const attributeName = input
						.slice(attr.nameStart, attr.nameEnd)
						.toLowerCase();
					const sourceItem =
						/** @type {NonNullable<typeof sources>} */
						(sources).get(attributeName);
