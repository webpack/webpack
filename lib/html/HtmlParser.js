...
/** @typedef {{ byTag: Record<string, Record<string, SourceItem>>, anyTag: Record<string, SourceItem> | undefined }} ResolvedSources */

/**
 * Builds the resolved source tables from a user-supplied array.
 * Returns two pieces: `byTag` (per-tag attribute dicts, with any-tag
 * entries already merged in as a fallback) and `anyTag` (the bare
 * any-tag dict, used as the fallback for tags missing from `byTag`).
 *
 * Pre-merging any-tag entries into each per-tag bucket means the
 * walk's per-attribute lookup is a single property read instead of a
 * `(tag && tag[attr]) || (any && any[attr])` chain.
 *
 * `"..."` expands `DEFAULT_SOURCES_BY_TAG` in place at its array
 * position; later entries override earlier ones per `(tag, attribute)`.
 * @param {readonly (SourceEntry | "...")[]} userSources user entries
 * @returns {ResolvedSources} resolved sources
 */
const buildSourcesByTag = (userSources) => {
	...
	for (const entry of userSources) {
		...
		if (entry.tag === undefined) {
			anyTag = { ...anyTag, [attr]: item };
		} else {
			const tag = entry.tag.toLowerCase();
			byTag[tag] = { ...byTag[tag], [attr]: item };
		}
	}
	// Fold the any-tag bucket into every per-tag bucket as a base
	// (tag-specific entries still win). One-time build cost so the walk
	// can do a single property read per attribute.
	if (anyTag) {
		for (const tag of Object.keys(byTag)) {
			byTag[tag] = { ...anyTag, ...byTag[tag] };
		}
	}
	return { byTag, anyTag };
};
...
const DEFAULT_RESOLVED = { byTag: DEFAULT_SOURCES_BY_TAG, anyTag: undefined };
/** @type {ResolvedSources} */
const EMPTY_RESOLVED = { byTag: EMPTY_SOURCES_BY_TAG, anyTag: undefined };
...
class HtmlParser extends Parser {
	...
	constructor(hashFunction, context, outputModule, css, options) {
		...
		// Resolve `sources`. `true`/`undefined` and `false` reuse shared
		// precomputed tables — zero allocation, zero work per parser.
		// Only a user array pays for the build.
		...
		this.sourcesByTag = resolved.byTag;
		this.anyTagSources = resolved.anyTag;
	}
