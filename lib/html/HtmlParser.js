	...
	parse(source, state) {
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				pendingWebpackIgnore = undefined;
				if (ignore) {
					// For `<style>` we still need to skip past the close tag
					// so the rawtext body isn't reparsed; we just don't emit
					// a dependency for it.
					const elName = input.slice(nameStart, nameEnd).toLowerCase();
					pendingAttributes.length = 0;
					if (elName === "style") {
						STYLE_END_REGEXP.lastIndex = end;
						const m = STYLE_END_REGEXP.exec(input);
						return m ? m.index + m[0].length : end;
					}
					return end;
				}
				const elementName = input.slice(nameStart, nameEnd).toLowerCase();

				// `<style>` is rawtext: capture the inline CSS, hand it to
				// the CSS pipeline as a virtual `data:text/css` module with
				// `exportType: "text"`, and jump the walker past `</style>`
				// so the body is never reparsed as HTML. Only `<style>` tags
				...
				if (elementName === "style") {
					...
					const afterClose = closeMatch.index + closeMatch[0].length;
					...
					// `experiments.css` is off, leave the body alone and just
					// skip past `</style>` so the rawtext isn't reparsed.
					if (!this.css) {
						return afterClose;
					}
						...
						if (isEntry) {
							...
							const willBeModuleScript =
								...
								(this.outputModule &&
									kind === "classic" &&
									elementName === "script");
							/** @type {"script-classic" | "script-module" | "modulepreload"} */
							const elementKind =
								kind === "modulepreload"
									? "modulepreload"
									: willBeModuleScript
