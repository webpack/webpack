 ...
 *
 * User-supplied sources are always pure URL rewrites — they never carry
 * `entry`/`filter`/`entryCategory` metadata. Users who want `<script
 * src>` to keep becoming a compilation entry should include `"..."` in
 * their list to inline the defaults (whose `SourceItem` references are
 * preserved by reference, so the entry-detection strict-equality check
		...
		walkHtmlTokens(source, 0, {
			...
			openTag: (input, start, end, nameStart, nameEnd) => {
				...
				for (const attr of hasMatchingSources ? pendingAttributes : []) {
					...
					// Identify a default by strict-equality with its
					// `DEFAULT_SOURCES` slot; `"..."` expansion preserves the
					// references so user lists that opt into defaults keep
					// entry behaviour.
