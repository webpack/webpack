...
const { decodeHtmlEntities } = require("./walkHtmlTokens");
...
const DEFAULT_SOURCES_BY_TAG = dict({
	...
	// SVG (`feImage` matches the tree builder's adjusted camelCase tag name)
		...
		const walkNode = (node) => {
			...
			if (css) {
				for (const attr of attrs) {
					...
					if (!value || !STYLE_ATTR_URL_REGEXP.test(value)) break;
					// The browser sees the decoded value as the CSS text; the
					// dependency template re-escapes on write-back.
			...
			for (const attr of tagSources ? attrs : []) {
				...
				for (const parsedSource of parsedAttributeValue) {
					const [rawValue, innerStart, innerEnd] = parsedSource;
					// Browsers decode character references at tokenize time; the
					// AST keeps attribute values raw so dependency spans stay
					// source offsets — decode just the extracted URL, then apply
					// the same cleanup `parseSrc` applies to raw values.
					const value = rawValue.includes("&")
						? decodeHtmlEntities(rawValue, true)
								.replace(IGNORE_CHARS_REGEXP, "")
								.trim()
						: rawValue;
