	...
	while (pos < len) {
		...
		switch (state) {
			...
			case STATE_RAWTEXT_LESS_THAN_SIGN:
				...
				// temporary buffer here; we track via byte ranges instead.)
				if (cc === CC_SOLIDUS) {
			...
			case STATE_SCRIPT_DATA_ESCAPED_LESS_THAN_SIGN:
				...
				// (Spec sets a temporary buffer; we track via byte ranges.)
				if (cc === CC_SOLIDUS) {
...
// 0x80\u20130x9F range. Per spec these C1 control codepoints decode to the
