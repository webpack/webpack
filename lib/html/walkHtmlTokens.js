...
const HTML_ENTITIES = require("./htmlEntities");
			...
			case STATE_NAMED_CHARACTER_REFERENCE: {
				...
				for (let n = runLen; n > 0; n--) {
					...
					if (n === runLen && hasSemicolon) {
						...
						if (Object.prototype.hasOwnProperty.call(HTML_ENTITIES, withSemi)) {
							namedEntityConsumed = n + 1;
							break;
						}
					}
					const bare = input.slice(pos, pos + n);
					if (Object.prototype.hasOwnProperty.call(HTML_ENTITIES, bare)) {
...
walkHtmlTokens.decodeHtmlEntities = (str) => {
	...
	return str.replace(/&(#[xX]?[0-9a-fA-F]+|#?[0-9a-zA-Z]+);?/g, (match) => {
		...
		// shorter prefixes \u2014 this both handles direct matches like `&amp;` and
		// the WHATWG longest-prefix semantics where e.g. `&notpre;` decodes as
		// `&not` (a legacy bare entity) followed by `pre;` as literal text.
		const name = match.slice(1);
		for (let i = name.length; i > 0; i--) {
			const prefix = name.slice(0, i);
			if (Object.prototype.hasOwnProperty.call(HTML_ENTITIES, prefix)) {
				return (
					/** @type {Record<string, string>} */ (HTML_ENTITIES)[prefix] +
					name.slice(i)
				);
			}
		}
