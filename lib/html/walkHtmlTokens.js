...
// cspell:ignore apos notpre noncharacter noncharacters DFFF FFFE
...
const walkHtmlTokens = (input, pos = 0, callbacks = {}) => {
	...
	// Offset of the `&` that opened the current character reference, and the
	// running numeric value accumulated by the hex/decimal reference states.
	// `charRefCode` is clamped just past the Unicode range so it can't
	// overflow on pathological inputs; the numeric-reference-end state uses it
	// to emit the WHATWG numeric-reference validation parse errors.
	let charRefStart = -1;
	let charRefCode = 0;
	...
	while (pos < len) {
		...
		// All WHATWG tokenizer states are handled and validated against the
		// html5lib-tests conformance suite. A few spec parse errors are
		// intentionally not reported because they need context this offset-only
		// scanner does not keep: `duplicate-attribute` (needs per-tag
		// attribute-name accumulation, against the allocation-free design),
		// `cdata-in-html-content` (needs the tree builder's adjusted current
		// node), and the `*-in-input-stream` errors (raw control/noncharacter/
		// surrogate code points in the source, which we don't scan for). The
		// scanner flags character-reference errors but never performs the spec's
		// substitutions — decoding is `decodeHtmlEntities`.
		switch (state) {
			...
			case STATE_ATTRIBUTE_NAME:
				...
				if (isSpace(cc) || cc === CC_SOLIDUS || cc === CC_GREATER_THAN) {
					...
					// U+0000 NULL: unexpected-null-character (append U+FFFD).
					// U+0022 ("), U+0027 ('), U+003C (<): unexpected-character-
					// in-attribute-name. Otherwise append to the attribute name.
					if (cc === CC_NULL) {
						reportError("unexpected-null-character", pos, pos + 1, "warning");
					} else if (
			...
			case STATE_ATTRIBUTE_VALUE_UNQUOTED:
				if (isSpace(cc)) {
					...
					// unexpected-character-in-unquoted-attribute-value. Otherwise
					// append to the attribute value.
					if (cc === CC_NULL) {
						reportError("unexpected-null-character", pos, pos + 1, "warning");
					} else if (
						cc === CC_QUOTATION_MARK ||
	...
	// If EOF caught us inside a character-reference state, run the spec's
	// end-of-reference processing (the numeric states are reached after their
	// terminator was consumed, so they would otherwise never run when the
	// reference ends exactly at EOF), then resume in the return state so any
	// in-progress tag/comment is handled correctly by the branches below.
	if (
		state >= STATE_CHARACTER_REFERENCE &&
		state <= STATE_NUMERIC_CHARACTER_REFERENCE_END
	) {
