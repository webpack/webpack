...
const walkHtmlTokens = (input, pos = 0, callbacks = {}) => {
	...
	while (pos < len) {
		...
		// All WHATWG tokenizer states are handled. Two spec parse errors are
		...
		// attribute-name accumulation, against the allocation-free design) and
		// `cdata-in-html-content` (needs the tree builder's adjusted current
		// node). The scanner also flags character-reference parse errors but
		// never performs the spec's substitutions — decoding is `decodeHtmlEntities`.
		switch (state) {
			...
			case STATE_NUMERIC_CHARACTER_REFERENCE_END:
				...
				// WHATWG validation parse error. The scanner does not perform
				// the spec's substitution here (that happens later in
				// `decodeHtmlEntities`); it only flags the error. The offset
				// range covers the whole reference.
				if (charRefCode === 0) {
					reportError("null-character-reference", charRefStart, pos, "warning");
