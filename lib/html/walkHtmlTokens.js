...
const walkHtmlTokens = (input, pos = 0, callbacks = {}) => {
	...
	// Content mode for the tag just opened. In foreign content (SVG/MathML)
	// the tree builder vetoes RAWTEXT/RCDATA/script switching via `isForeign`,
	// so e.g. an SVG `<title>` or `<style>` is parsed as normal markup.
...
// building an AST from the token stream above. Scripting is always disabled (webpack is a build tool).
 ...
 */
const buildAst = (source, fragmentContext) => {
	/** @type {HtmlDocument} */
...
walkHtmlTokens.buildAst = buildAst;
walkHtmlTokens.NS_HTML = NS_HTML;
walkHtmlTokens.NS_MATHML = NS_MATHML;
walkHtmlTokens.NS_SVG = NS_SVG;
