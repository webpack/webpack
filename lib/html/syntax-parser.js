...
const dispatch = () => {
	const token = /** @type {Token} */ (/** @type {unknown} */ (tok));
	if (xmlMode) processXml(token);
	else process(token);
...
const handleDoctypeToken = (
	...
	tok.name =
		nameStart === -1
			? ""
			: xmlMode
				? doctypePart(input, nameStart, nameEnd)
...
const handleCommentToken = (input, start, end, dataStart, dataEnd) => {
	...
	if (
		...
		(xmlMode || open.length > 0) &&
		adjustedCurrent() &&
		(xmlMode || _namespaceOf(adjustedCurrent()) !== NS_HTML)
	) {
	...
	if (xmlMode && input.charCodeAt(start + 1) === CC_QUESTION_MARK) {
...
const handleTextToken = (input, start, end) => {
	...
	if (xmlMode) {
		_dispatchXmlText(
			inputHasNul ? normalized.replace(/\0/g, "\uFFFD") : normalized,
			start,
			end
		);
		return end;
...
const _dispatchXmlText = (text, start, end) => {
	...
	for (let i = text.indexOf("&"); i !== -1; i = text.indexOf("&", i + 1)) {
		...
		_xmlEntityStack.push(name);
		tokenizeXml(replacement, 0, PARSE_CALLBACKS);
		_xmlEntityStack.pop();
...
const handleAttributeToken = (
	...
	const name = xmlMode
		...
		: internLowerName(ATTR_NAME_INTERN, input, nameStart, nameEnd);
	if (pendAttrStart === 0) {
...
const handleStartTagToken = (
	...
	const name = xmlMode
		? input.slice(nameStart, nameEnd)
		: internLowerName(TAG_NAME_INTERN, input, nameStart, nameEnd);
	if (!xmlMode && name === "selectedcontent") sawSelectedContent = true;
...
const handleEndTagToken = (input, start, end, nameStart, nameEnd) => {
	...
	if (open.length > 0) {
		...
		if (
			xmlMode
				? input.slice(nameStart, nameEnd) === topName
				: rangeEqualsLowerCase(input, nameStart, nameEnd, topName)
		) {
			name = topName;
		}
	}
...
const parseHtml = (input, pos = 0, options = {}) => {
	...
	if (xmlMode) {
		...
		tokenizeXml(_S.source, pos, PARSE_CALLBACKS);
	} else {
