...
const _sheetDecides = (css) => {
	...
	const joining = _S._mergeStyles;
	if (!emptying && !joining) return undefined;
	const clean = joining && _sheetEndsCleanly(css);
	return (answer) =>
		typeof answer === "string" &&
		((emptying && answer === "") ||
			(joining && _sheetEndsCleanly(answer) !== clean));
};
...
const _sheetPrintsNothing = (path, element) => {
	...
	if (css.trim() === "" || _renderedSheet(css) !== "") return false;
	if (!_S._mergeStyles) return true;
	// The run `_mergedStyleRun` builds, read with the same two breaks.
	for (
		let next = _mergeableStyleAfter(path, element);
		next !== 0;
		next = _mergeableStyleAfter(path, next)
	) {
		const text = _nodeStrings[_onlyTextChild(next)];
		if (_SHEET_LEADING_AT_RULE_REGEXP.test(text)) break;
		const printed = text.trim() === "" ? text : _renderedSheet(text);
		if (!_sheetEndsCleanly(printed)) break;
...
module.exports._sheetEndsCleanly = _sheetEndsCleanly;
