...
/** @typedef {import("./syntax-printer").CompiledVisitorMap} CompiledVisitorMap */
...
/** @typedef {import("./syntax-printer").PrintContext} PrintContext */
 ...
 */
const isAsciiWhitespace = (cc) =>
	cc < 0x80 && (CHAR_CLASS[cc] & CCLS_SPACE) !== 0;
 ...
 * @property {(input: string, start: number, end: number, nameStart: number, nameEnd: number, publicStart: number, publicEnd: number, systemStart: number, systemEnd: number, forceQuirks: boolean, subsetStart?: number, subsetEnd?: number) => number=} doctype `subsetStart` / `subsetEnd` delimit an XML internal subset (`tokenizeXml` only; -1 when absent)
...
/**
 * Whether a code unit is XML's `S` production (§2.3): space, tab, LF or CR.
 * @param {number} cc code unit
 * @returns {boolean} true for XML whitespace
 */
const isXmlWhitespace = (cc) =>
	cc === CC_SPACE || cc === CC_TAB || cc === CC_LF || cc === CC_CR;

/**
 * Where a name ends: the first code unit XML's `Name` cannot hold and the markup
 * around it uses as a delimiter. Lenient on purpose, as the HTML tokenizer is.
 * @param {string} input source
 * @param {number} pos where the name starts
 * @returns {number} offset past the name
 */
const _xmlNameEnd = (input, pos) => {
	const len = input.length;
	while (pos < len) {
		const cc = input.charCodeAt(pos);
		if (
...
/**
 * Whether a code unit may open a name (§2.3 `NameStartChar`), read leniently:
 * anything but ASCII that cannot, so a `<` before a digit or a space is text.
 * @param {number} cc code unit
 * @returns {boolean} true when a tag name can start with it
 */
const _isXmlNameStart = (cc) =>
	cc >= 0x80 ||
	(cc >= 0x41 && cc <= 0x5a) ||
	(cc >= 0x61 && cc <= 0x7a) ||
	cc === CC_LOW_LINE ||
 ...
 * @returns {number} offset of the first non-whitespace code unit at or past it
 */
const _skipXmlWhitespace = (input, pos) => {
	const len = input.length;
	while (pos < len && isXmlWhitespace(input.charCodeAt(pos))) pos++;
	return pos;
};

/**
 * Where a DOCTYPE's internal subset ends: its `]`, skipping what a declaration
 * may quote and the comments and processing instructions it may hold.
 * @param {string} input source
 * @param {number} pos offset past the `[`
 * @returns {number} offset of the closing `]`, or the input length when missing
 */
const _xmlInternalSubsetEnd = (input, pos) => {
	const len = input.length;
	while (pos < len) {
		const cc = input.charCodeAt(pos);
		if (cc === CC_RIGHT_SQUARE_BRACKET) return pos;
		if (cc === CC_QUOTATION_MARK || cc === CC_APOSTROPHE) {
			const close = input.indexOf(cc === CC_APOSTROPHE ? "'" : '"', pos + 1);
			pos = close === -1 ? len : close + 1;
		} else if (input.startsWith("<!--", pos)) {
			const close = input.indexOf("-->", pos + 4);
...
/**
 * Read a quoted DOCTYPE identifier.
 * @param {string} input source
 * @param {number} pos offset of the opening quote
 * @returns {number} offset of the closing quote, or -1 when there is no quote at `pos`
 */
const _xmlQuotedEnd = (input, pos) => {
	const cc = input.charCodeAt(pos);
	if (cc !== CC_QUOTATION_MARK && cc !== CC_APOSTROPHE) return -1;
	const close = input.indexOf(cc === CC_APOSTROPHE ? "'" : '"', pos + 1);
	return close === -1 ? input.length : close;
};

/**
 * Tokenize XML 1.0 markup (§2.4–§4.1) into the callbacks `tokenize` drives, so
 * one tree builder serves both. Never throws: a document that is not well-formed
 * reads on the way a lenient processor would, reporting nothing.
 * @param {string} input input string
 * @param {number} pos current position
 * @param {HtmlTokenCallbacks} callbacks callbacks
 * @returns {number} final position
 */
const tokenizeXml = (input, pos = 0, callbacks = {}) => {
	const len = input.length;
	if (callbacks.parseError !== undefined && input.includes("\0", pos)) {
	...
	while (pos < len) {
		...
		let end = -1;
		if (next === CC_EXCLAMATION_MARK) {
			if (input.startsWith("--", lt + 2)) {
				const close = input.indexOf("-->", lt + 4);
				const dataEnd = close === -1 ? len : close;
				end = close === -1 ? len : close + 3;
				if (textStart < lt && callbacks.text !== undefined) {
					callbacks.text(input, textStart, lt);
				}
				if (callbacks.comment !== undefined) {
					callbacks.comment(input, lt, end, lt + 4, dataEnd);
				}
			} else if (input.startsWith("[CDATA[", lt + 2)) {
				...
				if (callbacks.comment !== undefined) {
					callbacks.comment(input, lt, end, lt + 9, dataEnd);
				}
			} else if (rangeEqualsLowerCase(input, lt + 2, lt + 9, "doctype")) {
				end = _tokenizeXmlDoctype(input, lt, callbacks, textStart);
			} else {
				// A declaration outside the DTD is not XML: read it as a comment, the
				// way the HTML tokenizer reads a bogus one.
				...
				if (callbacks.comment !== undefined) {
					callbacks.comment(input, lt, end, lt + 2, close === -1 ? len : close);
				}
			}
		} else if (next === CC_QUESTION_MARK) {
			const close = input.indexOf("?>", lt + 2);
			end = close === -1 ? len : close + 2;
			if (textStart < lt && callbacks.text !== undefined) {
				callbacks.text(input, textStart, lt);
			}
			if (close !== -1 && callbacks.comment !== undefined) {
				callbacks.comment(input, lt, end, lt + 2, close);
			}
		} else if (next === CC_SOLIDUS) {
			const nameStart = lt + 2;
			const nameEnd = _xmlNameEnd(input, nameStart);
			if (nameEnd !== nameStart) {
				const close = input.indexOf(">", nameEnd);
				end = close === -1 ? len : close + 1;
				if (textStart < lt && callbacks.text !== undefined) {
					callbacks.text(input, textStart, lt);
				}
				if (close !== -1 && callbacks.closeTag !== undefined) {
					callbacks.closeTag(input, lt, end, nameStart, nameEnd);
				}
			}
		} else if (_isXmlNameStart(next)) {
			...
			if (nameEnd !== nameStart) {
				...
				end = _tokenizeXmlStartTag(input, lt, nameStart, nameEnd, callbacks);
			}
		}
		if (end === -1) {
			// A `<` that opens nothing stays in the text, as it does in HTML.
			pos = lt + 1;
		} else {
			pos = end;
			textStart = end;
 ...
 * @param {number} nameStart offset of the tag name
 * @param {number} nameEnd offset past the tag name
 * @param {HtmlTokenCallbacks} callbacks callbacks
 * @returns {number} offset past the tag
 */
const _tokenizeXmlStartTag = (input, start, nameStart, nameEnd, callbacks) => {
	const len = input.length;
	let pos = nameEnd;
	while (true) {
		pos = _skipXmlWhitespace(input, pos);
		if (pos >= len) {
			if (callbacks.parseError !== undefined) {
				callbacks.parseError(input, "eof-in-tag", len, len, "error");
			}
			if (callbacks.openTag !== undefined) {
				callbacks.openTag(input, start, len, nameStart, nameEnd, false);
			}
			return len;
		}
		const cc = input.charCodeAt(pos);
		if (cc === CC_GREATER_THAN) {
			if (callbacks.openTag !== undefined) {
				callbacks.openTag(input, start, pos + 1, nameStart, nameEnd, false);
			}
			return pos + 1;
		}
		// A `<` cannot sit in a tag, so the next one starts markup of its own.
		if (cc === CC_LESS_THAN) {
			if (callbacks.openTag !== undefined) {
				callbacks.openTag(input, start, pos, nameStart, nameEnd, false);
			}
			return pos;
		}
		if (cc === CC_SOLIDUS && input.charCodeAt(pos + 1) === CC_GREATER_THAN) {
			if (callbacks.openTag !== undefined) {
				callbacks.openTag(input, start, pos + 2, nameStart, nameEnd, true);
			}
			return pos + 2;
		}
		const attributeStart = pos;
		const attributeEnd = _xmlNameEnd(input, pos);
		// A stray `/`, quote or `=` is skipped so the tag still ends where it should.
		if (attributeEnd === attributeStart) {
			pos++;
			continue;
		}
		pos = _skipXmlWhitespace(input, attributeEnd);
		let valueStart = -1;
		let valueEnd = -1;
		let quoteType = QUOTE_NONE;
		if (input.charCodeAt(pos) === CC_EQUALS) {
			pos = _skipXmlWhitespace(input, pos + 1);
			const quote = input.charCodeAt(pos);
			if (quote === CC_QUOTATION_MARK || quote === CC_APOSTROPHE) {
				quoteType = quote === CC_QUOTATION_MARK ? QUOTE_DOUBLE : QUOTE_SINGLE;
				valueStart = pos + 1;
				...
				while (
					pos < len &&
					!isXmlWhitespace(input.charCodeAt(pos)) &&
					input.charCodeAt(pos) !== CC_GREATER_THAN
				) {
					pos++;
				}
				valueEnd = pos;
		...
		if (callbacks.attribute !== undefined) {
			callbacks.attribute(
				input,
				attributeStart,
				attributeEnd,
 ...
 * @param {number} start offset of the `<`
 * @param {HtmlTokenCallbacks} callbacks callbacks
 * @param {number} textStart start of the text still to flush before the declaration
 * @returns {number} offset past the declaration
 */
const _tokenizeXmlDoctype = (input, start, callbacks, textStart) => {
	const len = input.length;
	let pos = _skipXmlWhitespace(input, start + 9);
	const nameStart = pos;
	const nameEnd = _xmlNameEnd(input, pos);
	pos = _skipXmlWhitespace(input, nameEnd);
	let publicStart = -1;
	let publicEnd = -1;
	let systemStart = -1;
	let systemEnd = -1;
	if (input.startsWith("PUBLIC", pos)) {
		pos = _skipXmlWhitespace(input, pos + 6);
		const close = _xmlQuotedEnd(input, pos);
		if (close !== -1) {
			publicStart = pos + 1;
			publicEnd = close;
			pos = _skipXmlWhitespace(input, close + 1);
		}
	} else if (input.startsWith("SYSTEM", pos)) {
		pos = _skipXmlWhitespace(input, pos + 6);
	}
	const systemClose = _xmlQuotedEnd(input, pos);
	if (systemClose !== -1) {
		systemStart = pos + 1;
		systemEnd = systemClose;
		pos = _skipXmlWhitespace(input, systemClose + 1);
	}
	let subsetStart = -1;
	let subsetEnd = -1;
	if (input.charCodeAt(pos) === CC_LEFT_SQUARE_BRACKET) {
		subsetStart = pos + 1;
		subsetEnd = _xmlInternalSubsetEnd(input, subsetStart);
		pos = subsetEnd + 1;
	}
	const close = input.indexOf(">", pos);
	const end = close === -1 ? len : close + 1;
	if (textStart < start && callbacks.text !== undefined) {
		callbacks.text(input, textStart, start);
	}
	if (callbacks.doctype !== undefined) {
		callbacks.doctype(
			input,
			...
			nameEnd === nameStart ? -1 : nameStart,
			nameEnd === nameStart ? -1 : nameEnd,
			publicStart,
			publicEnd,
			systemStart,
			systemEnd,
...
const NS_MATHML = 1;
const NS_SVG = 2;
// XML elements do not participate in HTML, MathML or SVG tree construction.
// The fourth value fits in the existing two-bit namespace field.
const NS_XML = 3;
...
const _templateContentOf = (el) =>
	(_S._nodeFlags[el] & FLAG_HAS_TEMPLATE) !== 0 ? _nodeContentEnds[el] : 0;
/** @type {(el: HtmlNodeRef) => number} */
const _namespaceOf = (el) => _S._nodeFlags[el] & NS_MASK;
...
const _growNodeColumns = (need, exact) => {
	...
	ty.set(_S._nodeTypes);
	_S._nodeTypes = ty;
	const fl = new Uint8Array(cap);
	fl.set(_S._nodeFlags);
	_S._nodeFlags = fl;
	...
	first.set(_S._nodeFirstChildren);
	_S._nodeFirstChildren = first;
	const last = new Int32Array(cap);
	last.set(_nodeLastChildren);
	_nodeLastChildren = last;
	const next = new Int32Array(cap);
	next.set(_S._nodeNextSiblings);
	_S._nodeNextSiblings = next;
...
const _releaseAstColumns = () => {
	...
	if (_nodeCapacity > _COLUMN_SHRINK_CAPACITY) {
		_nodeCapacity = 0;
		_S._nodeTypes = new Uint8Array(0);
		_S._nodeFlags = new Uint8Array(0);
		...
		_S._nodeFirstChildren = new Int32Array(0);
		_nodeLastChildren = new Int32Array(0);
		_S._nodeNextSiblings = new Int32Array(0);
...
const _allocNode = (type, start, end) => {
	...
	_S._nodeTypes[i] = type;
	_S._nodeFlags[i] = 0;
	_nodeStarts[i] = start;
	_nodeEnds[i] = end;
	_nodeParents[i] = 0;
	_S._nodeFirstChildren[i] = 0;
	_nodeLastChildren[i] = 0;
	_S._nodeNextSiblings[i] = 0;
...
const _appendChild = (parent, node) => {
	...
	if (last === 0) _S._nodeFirstChildren[parent] = node;
	else _S._nodeNextSiblings[last] = node;
...
const mkEl = (tagName, ns, attrs, pos) => {
	...
	_S._nodeTypes[el] = NodeType.Element;
	...
	_nodeParents[el] = 0;
	_S._nodeFirstChildren[el] = 0;
	_nodeLastChildren[el] = 0;
	_S._nodeNextSiblings[el] = 0;
 ...
 * @returns {boolean} whether XML 1.0 permits the character
 */
...
// A character reference, or an entity reference by any name a lenient reader
// would take; which names resolve is up to the document's DTD.
const XML_REFERENCE_REGEXP =
	/&(?:#(?:x([\da-fA-F]+)|(\d+))|([^\s&;#<>"'=/%]+));/g;
const XML_CHARACTER_REFERENCE_REGEXP = /&#(?:x([\da-fA-F]+)|(\d+));/g;
// WHY: §3.3.3 normalizes each literal whitespace character of an attribute value
// to a space after §2.11 has folded CRLF, so a CRLF is one space; replacement
// text of an entity is already folded, so there each character counts alone.
const XML_ATTRIBUTE_LINE_END_REGEXP = /\r\n?|[\t\n]/g;
const XML_WHITESPACE_REGEXP = /[\t\n\r]/g;
const XML_LINE_END_REGEXP = /\r\n?/g;
// Entities an entity may name before the rest is left as written.
const XML_ENTITY_DEPTH_LIMIT = 16;
// Characters one parse may expand entities into beyond the source itself, so
// a declaration nesting its references ("billion laughs") stays bounded.
const XML_ENTITY_EXPANSION_BUDGET = 1 << 20;

/** @type {Map<string, string> | null} internal general entities the DOCTYPE declared */
let _xmlEntities = null;
// The source range of the text whose entity reference is being tokenized, -1
// outside one: what the replacement text builds is reported there.
let _xmlEntityRangeStart = -1;
/** @type {Map<string, boolean> | null} which entities expand to markup, as far as asked */
let _xmlMarkupEntities = null;
let _xmlEntityRangeEnd = -1;
/**
 * @typedef {object} XmlAttributeDeclaration
 * @property {boolean} tokenized whether the type is not CDATA, so §3.3.3 also trims and collapses spaces
 * @property {string | null} defaultValue the default as written, null for `#REQUIRED` / `#IMPLIED`
 */
/** @type {Map<string, Map<string, XmlAttributeDeclaration>> | null} attributes the DOCTYPE declared, per element */
let _xmlAttributeDeclarations = null;
/** @type {string[]} entities being expanded, innermost last */
const _xmlEntityStack = [];
let _xmlExpansionBudget = 0;

/**
 * @param {string} match the reference as written
 * @param {string | undefined} hex hexadecimal digits of a character reference
 * @param {string | undefined} decimal decimal digits of a character reference
 * @returns {string} the character, or U+FFFD for one XML does not allow
 */
const _xmlCharacterReference = (match, hex, decimal) => {
	const code = Number.parseInt(
		hex === undefined ? /** @type {string} */ (decimal) : hex,
		hex === undefined ? 10 : 16
	);
...
/**
 * Resolve character and entity references (§4.4): the five predefined names,
 * and the internal general entities the document's DTD declared.
 * @param {string} value source text, its line ends already folded
 * @param {boolean} attribute whether `value` is (part of) an attribute value
 * @returns {string} decoded text
 */
const _decodeXmlReferences = (value, attribute) =>
	value.replace(XML_REFERENCE_REGEXP, (match, hex, decimal, name) => {
		if (name === undefined) return _xmlCharacterReference(match, hex, decimal);
		if (Object.prototype.hasOwnProperty.call(XML_NAMED_REFERENCES, name)) {
			return XML_NAMED_REFERENCES[
				/** @type {keyof typeof XML_NAMED_REFERENCES} */ (name)
			];
		}
		const replacement =
			_xmlEntities === null ? undefined : _xmlEntities.get(name);
		// An undeclared or recursive entity, or one past the limits, stays as written.
		if (
			replacement === undefined ||
			_xmlEntityStack.length >= XML_ENTITY_DEPTH_LIMIT ||
			_xmlEntityStack.includes(name) ||
			_xmlExpansionBudget < replacement.length
		) {
			return match;
		}
		_xmlExpansionBudget -= replacement.length;
		_xmlEntityStack.push(name);
		const text = attribute
			? replacement.replace(XML_WHITESPACE_REGEXP, " ")
			: replacement;
		const decoded = text.includes("&")
			? _decodeXmlReferences(text, attribute)
			: text;
		_xmlEntityStack.pop();
		return decoded;
	});
...
/**
 * Decode the character data of XML content.
 * @param {string} value source text, its line ends already folded
 * @returns {string} decoded text
 */
const decodeXmlEntities = (value) =>
	value.includes("&") ? _decodeXmlReferences(value, false) : value;

/**
 * An attribute value as §3.3.3 normalizes a CDATA one: literal whitespace
 * becomes a space, then references are resolved.
 * @param {string} value the value as written
 * @returns {string} normalized value
 */
const decodeXmlAttribute = (value) => {
	const spaced = value.replace(XML_ATTRIBUTE_LINE_END_REGEXP, " ");
	return spaced.includes("&") ? _decodeXmlReferences(spaced, true) : spaced;
};

/**
 * Escape text for XML content (§2.4): `&`, `<` and `>`, and a CR, which §2.11
 * would otherwise read back as a LF. Only a `]]>` needs its `>` escaped, but a
 * bare one costs gzip what it saves raw where the text also spells `&lt;`.
 * @param {string} data decoded text
 * @returns {string} the text as character data
 */
const escapeXmlText = (data) =>
	data
		.replace(/&/g, "&amp;")
		.replace(/</g, "&lt;")
		.replace(/>/g, "&gt;")
...
/**
 * Escape an attribute value for XML (§3.1): `&`, `<` and the delimiter, plus the
 * whitespace §3.3.3 would otherwise normalize to a space.
 * @param {string} value decoded value
 * @param {string} quote the delimiter it is written between
 * @returns {string} the value as written between `quote`s
 */
...
/**
 * Read an attribute-list declaration (§3.3) into `_xmlAttributeDeclarations`.
 * @param {string} input source
 * @param {number} pos offset past `<!ATTLIST`
 * @param {number} end offset of the internal subset's `]`
 * @returns {number} offset past the declaration
 */
const _readXmlAttributeListDeclaration = (input, pos, end) => {
	pos = _skipXmlWhitespace(input, pos);
	const elementEnd = _xmlNameEnd(input, pos);
	const element = input.slice(pos, elementEnd);
	pos = elementEnd;
	while (pos < end) {
		pos = _skipXmlWhitespace(input, pos);
		if (pos >= end || input.charCodeAt(pos) === CC_GREATER_THAN) break;
		const nameEnd = _xmlNameEnd(input, pos);
		if (nameEnd === pos) break;
		const name = input.slice(pos, nameEnd);
		pos = _skipXmlWhitespace(input, nameEnd);
		// AttType: a keyword, an enumeration, or NOTATION and an enumeration.
		let tokenized = true;
		if (input.startsWith("CDATA", pos)) {
			tokenized = false;
			pos += 5;
		} else {
			if (input.startsWith("NOTATION", pos)) {
				pos = _skipXmlWhitespace(input, pos + 8);
			}
			if (input.charCodeAt(pos) === 0x28 /* ( */) {
				const close = input.indexOf(")", pos);
				pos = close === -1 || close > end ? end : close + 1;
			} else {
				pos = _xmlNameEnd(input, pos);
			}
		}
		pos = _skipXmlWhitespace(input, pos);
		let defaultValue = null;
		if (input.startsWith("#FIXED", pos)) {
			pos = _skipXmlWhitespace(input, pos + 6);
		} else if (input.charCodeAt(pos) === 0x23 /* # */) {
			pos = _xmlNameEnd(input, pos + 1);
		}
		const quote = input.charCodeAt(pos);
		if (quote === CC_QUOTATION_MARK || quote === CC_APOSTROPHE) {
			const close = input.indexOf(quote === CC_APOSTROPHE ? "'" : '"', pos + 1);
			if (close === -1 || close > end) return end;
			defaultValue = input.slice(pos + 1, close);
			pos = close + 1;
		}
		if (_xmlAttributeDeclarations === null) {
			_xmlAttributeDeclarations = new Map();
		}
		let declarations = _xmlAttributeDeclarations.get(element);
		if (declarations === undefined) {
			declarations = new Map();
			_xmlAttributeDeclarations.set(element, declarations);
		}
		// §3.3: the first declaration of an attribute binds it.
		if (!declarations.has(name)) {
			declarations.set(name, { tokenized, defaultValue });
		}
	}
	const close = input.indexOf(">", pos);
...
/**
 * Apply what the DTD declares to a start tag's attribute run: a tokenized type's
 * value is trimmed and its spaces collapsed, and a missing attribute with a
 * default gets one. Both are stored as a spelling `decodeXmlAttribute` reads.
 * @param {{ start: number, count: number }} attrs the tag's attribute run, updated in place
 * @param {Map<string, XmlAttributeDeclaration>} declarations the element's declared attributes
 * @returns {void}
 */
const _applyXmlAttributeDeclarations = (attrs, declarations) => {
	const start = attrs.start;
	const count = attrs.count;
	for (let i = start; i < start + count; i++) {
		const declaration = declarations.get(_attrNames[i]);
		if (declaration === undefined || !declaration.tokenized) continue;
		const value = decodeXmlAttribute(_attrValueOf(i));
		const collapsed = value.replace(/ +/g, " ").replace(/^ | $/g, "");
		if (collapsed !== value) {
			_attrValues[i] = escapeXmlAttribute(collapsed, '"');
		}
	}
	for (const [name, declaration] of declarations) {
		...
		if (present) continue;
		let value = decodeXmlAttribute(declaration.defaultValue);
		if (declaration.tokenized) {
			value = value.replace(/ +/g, " ").replace(/^ | $/g, "");
		}
		// No source offsets: the attribute is the DTD's, not the tag's.
		const index = _allocAttr(
			name,
		...
		if (attrs.count === 0) attrs.start = index;
		attrs.count++;
	}
};

/**
 * Read the general entities an internal DTD subset declares (§4.2). A literal's
 * character references are expanded as it is declared; its entity references
 * wait until the entity is used.
 * @param {string} input source
 * @param {number} start offset past the `[`
 * @param {number} end offset of the `]`
 * @returns {void}
 */
const _readXmlInternalSubset = (input, start, end) => {
	let pos = start;
	while (pos < end) {
		const cc = input.charCodeAt(pos);
		if (cc === 0x25 /* % */) {
			// §5.1: once a parameter entity went unread, a non-validating processor
			// must not process the declarations after it.
			return;
		}
		if (cc !== CC_LESS_THAN) {
			pos++;
		...
		if (input.startsWith("<!ATTLIST", pos)) {
			pos = _readXmlAttributeListDeclaration(input, pos + 9, end);
			continue;
		}
		const isEntity = input.startsWith("<!ENTITY", pos);
		let i = _skipXmlWhitespace(input, pos + (isEntity ? 8 : 2));
		const parameter = isEntity && input.charCodeAt(i) === 0x25;
		if (parameter) i = _skipXmlWhitespace(input, i + 1);
		const nameStart = i;
		const nameEnd = _xmlNameEnd(input, i);
		i = _skipXmlWhitespace(input, nameEnd);
		const quote = input.charCodeAt(i);
		if (
			isEntity &&
			!parameter &&
			nameEnd !== nameStart &&
			(quote === CC_QUOTATION_MARK || quote === CC_APOSTROPHE)
		) {
			const close = input.indexOf(quote === CC_APOSTROPHE ? "'" : '"', i + 1);
			if (close === -1 || close > end) return;
			const name = input.slice(nameStart, nameEnd);
			if (_xmlEntities === null) _xmlEntities = new Map();
			// §4.2: the first declaration of a name binds it.
			if (!_xmlEntities.has(name)) {
				_xmlEntities.set(
					name,
					input
						.slice(i + 1, close)
						.replace(XML_LINE_END_REGEXP, "\n")
						.replace(XML_CHARACTER_REFERENCE_REGEXP, _xmlCharacterReference)
				);
			}
			i = close + 1;
		}
		// The rest of the declaration: skip what it quotes, since a literal may
		// hold a `>`.
		while (i < end) {
			const c = input.charCodeAt(i);
			if (c === CC_GREATER_THAN) break;
			if (c === CC_QUOTATION_MARK || c === CC_APOSTROPHE) {
				const close = input.indexOf(c === CC_APOSTROPHE ? "'" : '"', i + 1);
...
const appendTo = (parent, node) => {
	...
	if (
		_S._nodeTypes[node] === NodeType.Text &&
		last !== 0 &&
		_S._nodeTypes[last] === NodeType.Text
	) {
...
const insertAtPlace = (place, node) => {
	...
	if (before !== 0) {
		...
		let prev = 0;
		let c = _S._nodeFirstChildren[p];
		while (c !== 0 && c !== before) {
			prev = c;
			c = _S._nodeNextSiblings[c];
		}
		if (c === 0) {
		...
		if (
			_S._nodeTypes[node] === NodeType.Text &&
			prev !== 0 &&
			_S._nodeTypes[prev] === NodeType.Text
		) {
		...
		_nodeParents[node] = p;
		_S._nodeNextSiblings[node] = before;
		if (prev === 0) _S._nodeFirstChildren[p] = node;
		else _S._nodeNextSiblings[prev] = node;
...
const insertCharacters = (data, start, end) => {
	...
	if (_S._nodeTypes[place.parent] === NodeType.Document) return; // never insert text into document
	...
	if (skipText) {
		...
		if (
			_S._nodeTypes[p] === NodeType.Element &&
	...
	if (before !== 0) {
		...
		if (prev !== 0 && _S._nodeTypes[prev] === NodeType.Text) {
		...
		const last = _nodeLastChildren[p];
		if (last !== 0 && _S._nodeTypes[last] === NodeType.Text) {
...
const resetInsertionMode = () => {
	...
	for (let i = open.length - 1; i >= 0; i--) {
		...
		if (_namespaceOf(node) === NS_HTML) {
			...
			if (tn === "html") {
				mode = _S.head ? MODE_AFTER_HEAD : MODE_BEFORE_HEAD;
				return;
			}
		}
		if (last) {
...
/**
 * XML has no insertion modes: elements are appended to the current element
 * and an end tag closes the element with the same case-sensitive name.
 * @param {Token} t token to process
 * @returns {void}
 */
const processXml = (t) => {
	const ty = t.type;
	if (_xmlEntityRangeStart !== -1) {
		if (ty === TOKEN_START_TAG || ty === TOKEN_END_TAG) {
			t.pos.start = _xmlEntityRangeStart;
			t.pos.end = _xmlEntityRangeEnd;
			t.pos.tagEnd = _xmlEntityRangeEnd;
			t.pos.nameEnd = 0;
		} else if (ty !== TOKEN_EOF) {
			const located = /** @type {CharToken} */ (t);
			located.start = _xmlEntityRangeStart;
			located.end = _xmlEntityRangeEnd;
		}
	}
	if (ty === TOKEN_START_TAG || ty === TOKEN_END_TAG) tokenEnd = t.pos.end;
	else if (ty !== TOKEN_EOF) tokenEnd = t.end;
	...
	if (isCommentOrProcessingInstruction(t)) {
		insertCommentOrProcessingInstruction(
			t,
			open.length === 0 ? { parent: doc, beforeNode: 0 } : undefined
		);
		return;
	}
	if (ty === TOKEN_DOCTYPE) {
	...
	if (ty === TOKEN_START_TAG) {
		if (_xmlAttributeDeclarations !== null) {
			const declarations = _xmlAttributeDeclarations.get(t.name);
			if (declarations !== undefined) {
				_applyXmlAttributeDeclarations(t.attrs, declarations);
			}
		}
		const el = mkEl(t.name, NS_XML, t.attrs, t.pos);
		if (t.selfClosing) _S._nodeFlags[el] |= FLAG_SELF_CLOSING;
		appendTo(open.length === 0 ? doc : cur(), el);
		if (!t.selfClosing) open.push(el);
		return;
	}
	if (ty === TOKEN_END_TAG) {
		// A misnested end tag closes the element it names and the ones opened
		// inside it; one naming no open element is dropped.
		for (let i = open.length - 1; i >= 0; i--) {
			if (_tagNameOf(open[i]) !== t.name) continue;
			while (open.length > i + 1) popOpen();
			_nodeEnds[popOpen()] = t.pos.end;
			return;
		}
	}
};
...
const adoptionAgency = (subject, pos) => {
	...
	while (outer < 8) {
		...
		let k = _S._nodeFirstChildren[furthest];
		_S._nodeFirstChildren[furthest] = 0;
		_nodeLastChildren[furthest] = 0;
		while (k !== 0) {
			const next = _S._nodeNextSiblings[k];
			_S._nodeNextSiblings[k] = 0;
			_nodeParents[k] = 0;
			appendTo(
				(_S._nodeFlags[k] & FLAG_SHADOW_ROOT) === 0 ? cloneFmt : furthest,
				k
			);
			k = next;
		}
...
const detach = (node) => {
	...
	if (c === 0) return;
	if (prev === 0) _S._nodeFirstChildren[p] = _S._nodeNextSiblings[node];
	else _S._nodeNextSiblings[prev] = _S._nodeNextSiblings[node];
	if (_nodeLastChildren[p] === node) _nodeLastChildren[p] = prev;
	_S._nodeNextSiblings[node] = 0;
	_nodeParents[node] = 0;
};
...
modes.beforeHead = (t) => {
	...
	if (t.type === TOKEN_START_TAG && t.name === "head") {
		_S.head = insertHtmlElement("head", t.attrs, t.pos);
		mode = MODE_IN_HEAD;
		return;
	}
	if (t.type === TOKEN_END_TAG && !HEAD_BODY_HTML_BR.has(t.name)) {
	...
	_S.head = insertHtmlElement("head", EMPTY_ATTRS, null);
	mode = MODE_IN_HEAD;
	process(t);
};
...
modes.inHead = (t) => {
	...
	if (t.type === TOKEN_START_TAG) {
		...
		if (t.name === "template") {
			...
			_S._nodeFlags[el] |= FLAG_HAS_TEMPLATE;
			...
			if (
				...
				host !== 0 &&
				host !== open[0] &&
				(_S._nodeFlags[host] & FLAG_SHADOW_HOST) === 0 &&
				_S._nodeTypes[host] === NodeType.Element &&
				_isShadowHost(host)
			) {
				const mode = _attrValueDecoded(shadowRootMode).toLowerCase();
				if (mode === "open" || mode === "closed") {
					_S._nodeFlags[el] |= FLAG_SHADOW_ROOT;
					_S._nodeFlags[host] |= FLAG_SHADOW_HOST;
				}
			}
			return;
		}
...
modes.afterHead = (t) => {
	...
	if (t.type === TOKEN_START_TAG) {
		...
		if (HEAD_ELEMENTS.has(t.name)) {
			const headEl = /** @type {HtmlElement} */ (_S.head);
			open.push(headEl);
			modes.inHead(t);
...
modes.text = (t) => {
	...
	if (t.type === TOKEN_END_TAG) {
		...
		if (
			...
			insertCharacters(
				_S.source.slice(t.pos.start, t.pos.end),
				t.pos.start,
				t.pos.end
			);
			return;
...
const _scanProcessingInstruction = (input, start, end) => {
	...
	while (i < contentEnd && isAsciiWhitespace(input.charCodeAt(i))) i++;
...
const handleTextToken = (input, start, end) => {
	...
	const normalized =
		_S.inputHasCr && raw.includes("\r") ? raw.replace(/\r\n?/g, "\n") : raw;
...
// XML's own token handlers: the same token, dispatched straight to `processXml`,
// so the HTML handlers above carry no XML branch on their per-token path.
const dispatchXml = () => {
	processXml(/** @type {Token} */ (/** @type {unknown} */ (tok)));
};

// A reference in the same shape as `XML_REFERENCE_REGEXP`, matched in place.
const XML_REFERENCE_STICKY_REGEXP =
	/&(?:#(?:x[\da-fA-F]+|\d+)|([^\s&;#<>"'=/%]+));/y;

/**
 * @param {string} data decoded character data
 * @param {number} start source offset the token is reported at
 * @param {number} end source offset past it
 * @returns {void}
 */
const _dispatchXmlCharacters = (data, start, end) => {
	if (data === "") return;
	tok.type = TOKEN_CHAR;
	tok.data = data;
	tok.start = start;
...
/**
 * Whether an entity's replacement text is markup once the entities it names are
 * expanded too. Memoized per parse, so nesting costs one visit per name.
 * @param {string} name declared entity name
 * @returns {boolean} true when expanding it can build a node other than text
 */
const _xmlEntityHoldsMarkup = (name) => {
	if (_xmlMarkupEntities === null) _xmlMarkupEntities = new Map();
	const known = _xmlMarkupEntities.get(name);
	if (known !== undefined) return known;
	// A cycle reads its own name as text-only, which is what it expands to.
	_xmlMarkupEntities.set(name, false);
	const replacement = /** @type {Map<string, string>} */ (_xmlEntities).get(
		name
	);
	let holds = replacement !== undefined && replacement.includes("<");
	if (replacement !== undefined && !holds) {
		for (
			let i = replacement.indexOf("&");
			i !== -1 && !holds;
			i = replacement.indexOf("&", i + 1)
		) {
			XML_REFERENCE_STICKY_REGEXP.lastIndex = i;
			const match = XML_REFERENCE_STICKY_REGEXP.exec(replacement);
			holds =
				match !== null &&
				match[1] !== undefined &&
				_xmlEntityHoldsMarkup(match[1]);
		}
	}
	_xmlMarkupEntities.set(name, holds);
	return holds;
};

/**
 * Dispatch XML character data, expanding a reference to an entity whose
 * replacement text is itself markup (§4.4.2 "included") by tokenizing it in
 * place. What it builds is reported at the range of the text that named it.
 * @param {string} text source text, line ends folded
 * @param {number} start source offset of the text
 * @param {number} end source offset past it
 * @returns {void}
 */
const _dispatchXmlText = (text, start, end) => {
	if (_xmlEntities === null || !text.includes("&")) {
		_dispatchXmlCharacters(decodeXmlEntities(text), start, end);
		return;
	}
	let pending = "";
	let last = 0;
	for (let i = text.indexOf("&"); i !== -1; i = text.indexOf("&", i + 1)) {
		XML_REFERENCE_STICKY_REGEXP.lastIndex = i;
		const match = XML_REFERENCE_STICKY_REGEXP.exec(text);
		if (match === null || match[1] === undefined) continue;
		const name = match[1];
		const replacement = _xmlEntities.get(name);
		if (
			replacement === undefined ||
			!_xmlEntityHoldsMarkup(name) ||
			_xmlEntityStack.length >= XML_ENTITY_DEPTH_LIMIT ||
			_xmlEntityStack.includes(name) ||
			_xmlExpansionBudget < replacement.length
		) {
		...
		pending += decodeXmlEntities(text.slice(last, i));
		_dispatchXmlCharacters(pending, start, end);
		pending = "";
		last = i + match[0].length;
		i = last - 1;
		_xmlExpansionBudget -= replacement.length;
		// The printer cannot rebuild a reference from what it expanded to, so a
		// document holding one prints as written.
		_S._xmlVerbatim = true;
		const outer = _xmlEntityRangeStart;
		if (outer === -1) {
			_xmlEntityRangeStart = start;
			_xmlEntityRangeEnd = end;
		}
		_xmlEntityStack.push(name);
		// A tag the replacement text ends inside is the entity's to drop, not the document's.
		const outerEofInTag = eofInTag;
		tokenizeXml(replacement, 0, XML_PARSE_CALLBACKS);
		eofInTag = outerEofInTag;
		_xmlEntityStack.pop();
		if (outer === -1) _xmlEntityRangeStart = -1;
	}
	pending += decodeXmlEntities(last === 0 ? text : text.slice(last));
	_dispatchXmlCharacters(pending, start, end);
};

/** @type {(input: string, start: number, end: number, nameStart: number, nameEnd: number, publicStart: number, publicEnd: number, systemStart: number, systemEnd: number, forceQuirks: boolean, subsetStart?: number, subsetEnd?: number) => number} */
const handleXmlDoctypeToken = (
	input,
	start,
	end,
	nameStart,
	...
	// §2.8: only the first DOCTYPE declares anything.
	if (
		subsetStart !== -1 &&
		_xmlEntities === null &&
		_xmlAttributeDeclarations === null
	) {
		_readXmlInternalSubset(input, subsetStart, subsetEnd);
	}
	tok.type = TOKEN_DOCTYPE;
	tok.name = nameStart === -1 ? "" : doctypePart(input, nameStart, nameEnd);
	tok.publicId =
		publicStart === -1 ? null : doctypePart(input, publicStart, publicEnd);
	tok.systemId =
		systemStart === -1 ? null : doctypePart(input, systemStart, systemEnd);
...
/** @type {(input: string, start: number, end: number, dataStart: number, dataEnd: number) => number} */
const handleXmlCommentToken = (input, start, end, dataStart, dataEnd) => {
	const cdata = input.startsWith("<![CDATA[", start);
	// Character data needs an element to go in; outside the root it is a comment.
	if (cdata && open.length > 0) {
		const data = input
	...
	if (input.charCodeAt(start + 1) === CC_QUESTION_MARK) {
		// §2.6: the target is a name, and the data starts past the whitespace after it.
		const targetEnd = _xmlNameEnd(input, dataStart);
		tok.type = TOKEN_PROCESSING_INSTRUCTION;
		tok.name = input.slice(dataStart, targetEnd);
		const piRaw = input.slice(_skipXmlWhitespace(input, targetEnd), dataEnd);
		tok.data = _S.inputHasCr ? piRaw.replace(XML_LINE_END_REGEXP, "\n") : piRaw;
	} else {
		tok.type = TOKEN_COMMENT;
		const commentRaw = input.slice(dataStart, dataEnd);
		tok.data = inputHasNul ? commentRaw.replace(/\0/g, "�") : commentRaw;
...
/** @type {(input: string, start: number, end: number) => number} */
const handleXmlTextToken = (input, start, end) => {
	let text = input.slice(start, end);
	if (_S.inputHasCr) text = text.replace(XML_LINE_END_REGEXP, "\n");
	if (inputHasNul) text = text.replace(/\0/g, "�");
	_dispatchXmlText(text, start, end);
	return end;
};

/** @type {(input: string, nameStart: number, nameEnd: number, valueStart: number, valueEnd: number, quoteType: number) => number} */
const handleXmlAttributeToken = (
	input,
	nameStart,
	nameEnd,
	valueStart,
	...
	// Names are case-sensitive and never interned: XML has no vocabulary to share.
	const name = input.slice(nameStart, nameEnd);
	...
	if (!duplicate) {
		if (_xmlEntityRangeStart !== -1) {
			// Inside an entity's replacement text, which no offset into the source reaches.
			_allocAttr(
				name,
				valueStart === -1 ? "" : input.slice(valueStart, valueEnd),
				-1,
				-1,
				-1,
				-1
...
const handleXmlStartTagToken = (
	...
	const attrStart = pendAttrStart;
	pendAttrStart = 0;
	// The input ended inside the tag, which drops it as HTML's eof-in-tag does.
	if (eofInTag) return end;
	tok.type = TOKEN_START_TAG;
	tok.name = input.slice(nameStart, nameEnd);
	tok.attrs.start = attrStart;
	tok.attrs.count = attrStart === 0 ? 0 : _attrCount + 1 - attrStart;
	tok.selfClosing = selfClosing;
	tok.pos.start = start;
	tok.pos.end = end;
	tok.pos.tagEnd = end;
...
/** @type {(input: string, start: number, end: number, nameStart: number, nameEnd: number) => number} */
const handleXmlEndTagToken = (input, start, end, nameStart, nameEnd) => {
	pendAttrStart = 0;
	tok.type = TOKEN_END_TAG;
	tok.name = input.slice(nameStart, nameEnd);
	tok.pos.start = start;
...
/** @type {HtmlTokenCallbacks} */
const XML_PARSE_CALLBACKS = {
	parseError: handleParseError,
	doctype: handleXmlDoctypeToken,
	comment: handleXmlCommentToken,
	text: handleXmlTextToken,
	attribute: handleXmlAttributeToken,
	openTag: handleXmlStartTagToken,
	closeTag: handleXmlEndTagToken
};
...
const parseHtml = (input, pos = 0, options = {}) => {
	_S.source = input;
	_S.inputHasCr = input.includes("\r");
	...
	xmlMode = options.xml === true;
	_S._xmlMode = xmlMode;
	_xmlEntities = null;
	_xmlAttributeDeclarations = null;
	_xmlEntityRangeStart = -1;
	_xmlMarkupEntities = null;
	_S._xmlVerbatim = false;
	_xmlEntityStack.length = 0;
	_xmlExpansionBudget = XML_ENTITY_EXPANSION_BUDGET;
	sourceShellEnds = 0;
	...
	_S._fosteredRuns = null;
	_S._fosteredNodes = null;
	_S._fosterHolders = null;
	// Node slots are reused between parses, so the marks come off with the sets.
	if (_S._fosterVerbatim !== null) {
		for (const n of _S._fosterVerbatim) _S._nodeFlags[n] &= ~FLAG_FOSTER_REGION;
	}
	_S._fosterVerbatim = null;
	...
	if (xmlMode) {
		// Everything outside the root element is the document's too, so it spans all.
		_nodeEnds[doc] = input.length;
		tokenizeXml(_S.source, pos, XML_PARSE_CALLBACKS);
	} else {
		tokenize(_S.source, pos, PARSE_CALLBACKS);
		tok.type = TOKEN_EOF;
		dispatch();
	}
...
const cloneSubtree = (node) => {
	...
	_S._nodeFlags[clone] = _S._nodeFlags[node];
	...
	for (
		let k = _S._nodeFirstChildren[node];
		k !== 0;
		k = _S._nodeNextSiblings[k]
	) {
	...
	if (tc !== 0) {
		...
		_S._nodeFlags[clone] |= FLAG_HAS_TEMPLATE;
		for (
			let k = _S._nodeFirstChildren[tc];
			k !== 0;
			k = _S._nodeNextSiblings[k]
		) {
...
const selectedOption = (select) => {
	...
	const collect = (el) => {
		for (
			let c = _S._nodeFirstChildren[el];
			c !== 0;
			c = _S._nodeNextSiblings[c]
		) {
			if (
				_S._nodeTypes[c] !== NodeType.Element ||
				_namespaceOf(c) !== NS_HTML
			) {
				continue;
			}
...
const mirrorSelectedContent = (node, select) => {
	...
	for (
		let child = _S._nodeFirstChildren[container];
		child !== 0;
		child = _S._nodeNextSiblings[child]
	) {
		if (_S._nodeTypes[child] !== NodeType.Element) continue;
		if (_namespaceOf(child) === NS_HTML && _nodeStrings[child] === "select") {
			...
			if (option !== 0) {
				...
				_S._nodeFirstChildren[child] = 0;
				_nodeLastChildren[child] = 0;
				for (
					let k = _S._nodeFirstChildren[option];
					k !== 0;
					k = _S._nodeNextSiblings[k]
				) {
...
const _startsOutsideLineBox = (path, node, parent) =>
	parent !== 0 &&
	_S._nodeFirstChildren[parent] === node &&
...
const _printsNothing = (path, node) => {
	...
	if (
		_S._collapseWhitespace === "none" ||
		_S._collapseWhitespace === "conservative"
	) {
		return false;
	}
	for (
...
const _renderEmbeddedOrNull = (source, type, as) => {
	...
	try {
		const rendered = _S._renderEmbeddedSource(source, {
			type,
			hostType: HTML_TYPE,
...
const _canOmitShellStart = (path, name, node) => {
	...
	if (type === NodeType.Text) {
		return !startsWithWs(path.data(first)) || _S._collapseWhitespace === "all";
	}
...
const _minifyStyleAttribute = (raw) => {
	if (_S._deferEmbeddedSource !== undefined || raw.trim() === "") return raw;
...
const _childrenPrintNothing = (path, element) => {
	for (
		let child = _S._nodeFirstChildren[element];
		child !== 0;
...
const _attributeFrequency = () => {
	...
	for (let node = 1; node <= _nodeCount; node++) {
		if (_S._nodeTypes[node] !== NodeType.Element) continue;
		...
		const markers =
			html && _S._removeRedundantAttributes !== "none"
				? REDUNDANT_TYPE_ATTRIBUTES[elementName]
				: undefined;
		const defaults =
			html && _S._removeRedundantAttributes === "all"
...
const _separates = (out, unquotedTail, separator) => {
	...
	return isAsciiWhitespace(cc) || (!unquotedTail && cc === CC_SOLIDUS);
};
	...
	for (let k = 0; k < count; k++) {
		...
		if (
			...
			(_S._renderEmbeddedSource !== undefined ||
				(_S._deferEmbeddedSource !== undefined && _deferSrcdoc)) &&
			...
			if (_S._deferEmbeddedSource !== undefined && _deferSrcdoc) {
		...
		if (quoted || html || attributeName === "style" || rawValue.includes("&")) {
			...
			if (handler !== null && _asciiTrim(handler) !== "") {
				...
				const holdQuoted = tailIsRead && _S._deferEmbeddedSource !== undefined;
			...
			if (isStyle && !verbatim && _S._deferEmbeddedSource !== undefined) {
...
// WHY: one object rather than one binding each, so the printer module reads
// the same fields the walk writes — every slot is initialized here, so the
// shape never transitions (see AGENTS.md "Keep instance shapes stable").
const _S = {
	...
	// Whether the body being rendered is written back escaped, set around a
	// foreign subtree's literal-text element: its answer arrives after the walk,
	// so the escaping rides with the deferred write rather than the printer.
	_escapeEmbedded: false,
	...
	// Whether the last parse read XML, which the printer then writes back as XML.
	_xmlMode: false,
	// Whether that XML expanded an entity into markup, so it prints as written.
	_xmlVerbatim: false,
...
const _openTagSource = (path) => {
	...
	const source =
		_mergedAttrNodes.size !== 0 && _mergedAttrNodes.has(path.node)
			...
			: path.openTag();
	_S._elementOpenSource = source;
	return source;
};

/**
 * Whether an XML element can print anything between its tags: minifying drops
 * the comments a `<a/>` then stands in for.
 * @param {HtmlPath} path positioned on the element
 * @param {boolean} minify whether minifying
 * @returns {boolean} true when a child may print
 */
const _xmlPrintsChildren = (path, minify) => {
	for (let c = path.firstChild(); c !== 0; c = path.nextSibling(c)) {
		if (!minify || _S._nodeTypes[c] !== NodeType.Comment) return true;
	}
	return false;
};

// The namespaces whose `<style>` and `style=""` hold CSS; in any other one the
// same names are data a stylesheet has nothing to do with.
const XML_STYLED_NAMESPACES = new Set([
	"http://www.w3.org/2000/svg",
	"http://www.w3.org/1999/xhtml"
]);

/**
 * Whether an XML element is in a namespace that styles with CSS, read off the
 * `xmlns` declarations in scope for its prefix (Namespaces in XML §6.2).
 * @param {HtmlElement} node the element
 * @returns {boolean} true for an SVG or XHTML element
 */
const _xmlElementIsStyled = (node) => {
	const name = _tagNameOf(node);
	const colon = name.indexOf(":");
	const declaration = colon === -1 ? "xmlns" : `xmlns:${name.slice(0, colon)}`;
	for (let n = node; n !== 0; n = _nodeParents[n]) {
		if (_S._nodeTypes[n] !== NodeType.Element) break;
		const start = _nodeAttrStarts[n];
		for (let i = start; i < start + _nodeAttrCounts[n]; i++) {
			if (_attrNames[i] === declaration) {
				return XML_STYLED_NAMESPACES.has(decodeXmlAttribute(_attrValueOf(i)));
			}
		}
	}
	return false;
...
/**
 * XML character data in its shortest well-formed spelling: as it stands where
 * nothing in it is markup, else a CDATA section, else escaped.
 * @param {string} text decoded text
 * @returns {string} the text as XML content
 */
const _xmlCharacterData = (text) => {
...
/**
 * Offer a `style` attribute of an SVG or XHTML element to the caller's renderer,
 * as the declaration list it is.
 * @param {string} value the decoded value
 * @returns {string | null} the value written between its quotes, the marker standing for it, or null when nothing renders it
 */
const _xmlStyleAttribute = (value) => {
	/**
	 * @param {string} css what to write
	 * @returns {string} it quoted
	 */
	const quote = (css) => {
		const delimiter = css.includes('"') && !css.includes("'") ? "'" : '"';
		return `${delimiter}${escapeXmlAttribute(css, delimiter)}${delimiter}`;
	};
	if (_S._deferEmbeddedSource !== undefined) {
		return _deferEmbedded(
			value,
			CSS_TYPE,
			(rendered) => quote(typeof rendered === "string" ? rendered : value),
			BLOCK_CONTENTS
		);
	}
	const rendered = _renderEmbeddedOrNull(value, CSS_TYPE, BLOCK_CONTENTS);
	return rendered === null ? null : quote(rendered);
};

/**
 * An XML element's start tag: as written, or when minifying rebuilt with one
 * space between attributes and each value in its shortest well-formed spelling.
 * @param {HtmlPath} path positioned on the element
 * @param {boolean} minify whether minifying
 * @returns {string} the start tag, ending in `>` even for an empty element
 */
const _xmlOpenTag = (path, minify) => {
	const node = path.node;
	if (!minify) {
		const source = path.openTag();
		return source.endsWith("/>") ? `${source.slice(0, -2)}>` : source;
	}
	let out = `<${_tagNameOf(node)}`;
	const start = _nodeAttrStarts[node];
	const rendering =
		_S._renderEmbeddedSource !== undefined ||
		_S._deferEmbeddedSource !== undefined;
	for (let i = start; i < start + _nodeAttrCounts[node]; i++) {
		// A default the DTD supplies is the DTD's to keep supplying.
		if (_attrNameStarts[i] === -1) continue;
		const raw = _attrValueOf(i);
		const decoded = decodeXmlAttribute(raw);
		if (
			rendering &&
			_attrNames[i] === "style" &&
			decoded.trim() !== "" &&
			_xmlElementIsStyled(node)
		) {
			const styled = _xmlStyleAttribute(decoded);
			if (styled !== null) {
				out += ` style=${styled}`;
				continue;
			}
		...
		const quote =
			decoded.split('"').length > decoded.split("'").length ? "'" : '"';
		let value = escapeXmlAttribute(decoded, quote);
		const valueStart = _attrValueStarts[i];
		// The spelling the source quoted is kept when shorter, an entity reference
		// the DTD declares being the usual reason.
		if (
			_attrValues[i] === null &&
			valueStart > 0 &&
			...
			const written = _htmlSource.charAt(valueStart - 1);
			if (written === '"' || written === "'") {
				out += ` ${_attrNames[i]}=${written}${raw}${written}`;
				continue;
			}
		}
		value = `${quote}${value}${quote}`;
		out += ` ${_attrNames[i]}=${value}`;
	}
	return `${out}>`;
};
...
const _contentOpensOutside = (node) => {
	const first = _S._nodeFirstChildren[node];
	if (first === 0) return false;
	const type = _S._nodeTypes[first];
...
const _streamTags = (path, minify) => {
	_streamImplied = "";
	if (xmlMode) {
		// Composed instead when it has no children, so it can print as `<a/>`.
		if (_S._xmlVerbatim || !_xmlPrintsChildren(path, minify)) return false;
		_streamOpen = _xmlOpenTag(path, minify);
		_streamClose = `</${path.tagName()}>`;
		return true;
	}
	...
	if (
		_S._fosterHolders !== null &&
		_S._fosterHolders.has(path.node) &&
		((_S._nodeFlags[path.node] & FLAG_FOSTER_REGION) === 0 ||
			!(/** @type {Set<HtmlNodeRef>} */ (_S._fosterVerbatim).has(path.node)))
	) {
		return false;
	}
...
const _formResetWouldPop = () => {
	...
	for (
		...
		if (_S._nodeTypes[n] !== NodeType.Element || _namespaceOf(n) !== NS_HTML) {
			continue;
		}
...
const _fosterRunMoves = (path, table) => {
	...
	if (blocked) {
		if (_S._fosterVerbatim === null) _S._fosterVerbatim = new Set();
		...
		for (let a = holder; a !== 0; a = path.parentOf(a)) {
			...
			_S._nodeFlags[a] |= FLAG_FOSTER_REGION;
		}
...
const _settleFosteredRuns = (path) => {
	...
	_S._fosteredNodes = nodes;
	_S._fosterHolders = holders;
};
	...
	while (stack.length !== 0) {
		...
		if (type === NodeType.Element) {
			...
			if (
				...
				(_S._absorbed !== null && _S._absorbed.has(node))
			) {
				continue;
			}
			flushText();
...
const _enterNode = (node) => {
	...
	const b = _visitors[_S._nodeTypes[node]];
...
const _firstInner = (node) => {
	...
	if (ty === NodeType.Element) {
		const tc = _templateContentOf(node);
		return tc !== 0 ? tc : _S._nodeFirstChildren[node];
	}
	if (ty === NodeType.Document || ty === NodeType.DocumentFragment) {
		return _S._nodeFirstChildren[node];
	}
	return 0;
};
	...
	descend: for (;;) {
		...
		for (;;) {
			...
			if (parent === container && _templateContentOf(container) !== node) {
				const next = _S._nodeNextSiblings[node];
			...
			if (_templateContentOf(parent) === node) {
				const inner = _S._nodeFirstChildren[parent];
				if (inner !== 0) {
					node = inner;
					continue descend;
				}
...
const _streamFlush = (parent, openChild, visit, hold) => {
	...
	const first = _S._nodeFirstChildren[container];
	...
	if (hold && openChild === 0) {
		...
		if (lastChild !== 0 && _S._nodeTypes[lastChild] === NodeType.Text) {
			child = lastChild;
		}
	}
	if (first === child) return;
	if (visit) _walkRun(container, first, child);
	_S._nodeFirstChildren[container] = child;
...
const _streamDetach = (el) => {
	...
	const next = _S._nodeNextSiblings[el];
	if (parent !== 0) {
		_S._nodeFirstChildren[parent] = next;
...
const grammar = (input, visitors, writer, options) => {
	...
	_S._mergeStyles = writer !== undefined && writer.options.mergeStyles === true;
	_S._mergeScripts =
		writer !== undefined && writer.options.mergeScripts === true;
	...
	try {
		...
		if (writer !== undefined) {
			...
			_walkRun(_nodeParents[root], root, _S._nodeNextSiblings[root]);
			...
			// An entity expanded into markup that no reference could be rebuilt from.
			if (_S._xmlVerbatim) writer.replaceAll(input);
			...
			if (verify) {
				...
				const printed =
					_S._deferEmbeddedSource === undefined ||
					_S._deferEmbeddedSource.length === 0
						? writer.result()
						: declineDeferredWrites(writer.result(), _S._deferEmbeddedSource);
...
// The CSS parser is loaded on first `parseCssUrls` call, not at module load —
// `HtmlGenerator` defers it the same way, and most documents carry no
// URL-bearing SVG presentation attribute.
/** @type {typeof import("../css/syntax-parser") | undefined} */
let _cssParser;
...
const parseCssUrls = (input) => {
	const {
		...
		TokenStream,
		equalsLowerCase
	} = _cssParser || (_cssParser = require("../css/syntax-parser"));
...
const A = {
	...
	namespace(n = _currentNode) {
		return _S._nodeFlags[n] & NS_MASK;
	},
	/**
	 * @param {HtmlElement=} n element
	...
	selfClosing(n = _currentNode) {
		return (_S._nodeFlags[n] & FLAG_SELF_CLOSING) !== 0;
	},
	...
	contentEnd(n = _currentNode) {
		return (_S._nodeFlags[n] & FLAG_HAS_TEMPLATE) !== 0
			? _nodeTagEnds[n]
			: _nodeContentEnds[n];
	},
	/**
	...
	firstChild(n = _currentNode) {
		return _S._nodeFirstChildren[n];
	},
	/**
	 * @param {HtmlNodeRef=} n node
...
module.exports.CC_APOSTROPHE = CC_APOSTROPHE;
module.exports.CC_LEFT_SQUARE_BRACKET = CC_LEFT_SQUARE_BRACKET;
module.exports.CC_LF = CC_LF;
module.exports.CC_NUMBER_SIGN = CC_NUMBER_SIGN;
module.exports.CC_QUOTATION_MARK = CC_QUOTATION_MARK;
module.exports.CC_RIGHT_SQUARE_BRACKET = CC_RIGHT_SQUARE_BRACKET;
module.exports.CC_SOLIDUS = CC_SOLIDUS;
module.exports.EMBEDDED_LANGUAGES = EMBEDDED_LANGUAGES;
module.exports.EVENT_HANDLER = EVENT_HANDLER;
module.exports.FLAG_FOSTER_REGION = FLAG_FOSTER_REGION;
...
module.exports.SVG_TYPE = SVG_TYPE;
module.exports._S = _S;
module.exports._asciiLowerCase = _asciiLowerCase;
module.exports._asciiTrim = _asciiTrim;
module.exports._canOmitEndTag = _canOmitEndTag;
module.exports._canOmitStartTag = _canOmitStartTag;
module.exports._contentOpensOutside = _contentOpensOutside;
module.exports._deferEmbedded = _deferEmbedded;
module.exports._elementCloseTag = _elementCloseTag;
module.exports._endTagOutsideSpan = _endTagOutsideSpan;
module.exports._endsOutsideLineBox = _endsOutsideLineBox;
module.exports._formNeedsPointerReset = _formNeedsPointerReset;
module.exports._fosterRunMoves = _fosterRunMoves;
module.exports._fosterSourceRegion = _fosterSourceRegion;
module.exports._impliedCloseTag = _impliedCloseTag;
module.exports._impliedTableGroupStartNeeded = _impliedTableGroupStartNeeded;
module.exports._isRedundantAttribute = _isRedundantAttribute;
module.exports._isRemovableEmptyAttribute = _isRemovableEmptyAttribute;
module.exports._isRemovableEmptyElement = _isRemovableEmptyElement;
module.exports._isSvgRoot = _isSvgRoot;
module.exports._keepComment = _keepComment;
module.exports._minifyOpenTag = _minifyOpenTag;
module.exports._nodeStrings = _nodeStrings;
module.exports._openTagSource = _openTagSource;
module.exports._printsNothing = _printsNothing;
module.exports._renderEmbeddedOrNull = _renderEmbeddedOrNull;
module.exports._startsCharacterReference = _startsCharacterReference;
module.exports._startsOutsideLineBox = _startsOutsideLineBox;
module.exports._synthesizeOpenTag = _synthesizeOpenTag;
module.exports._xmlCharacterData = _xmlCharacterData;
module.exports._xmlElementIsStyled = _xmlElementIsStyled;
module.exports._xmlOpenTag = _xmlOpenTag;
module.exports.baseTag = baseTag;
module.exports.buildHeadTags = buildHeadTags;
module.exports.collapseWhitespaceRuns = collapseWhitespaceRuns;
module.exports.decodeEntities = decodeEntities;
module.exports.decodeXmlAttribute = decodeXmlAttribute;
module.exports.decodeXmlEntities = decodeXmlEntities;
...
module.exports.escapeXmlText = escapeXmlText;
module.exports.grammar = grammar;
module.exports.isAllWs = isAllWs;
module.exports.isAsciiAlphanumeric = isAsciiAlphanumeric;
module.exports.isAsciiWhitespace = isAsciiWhitespace;
...
module.exports.startsWithWs = startsWithWs;
module.exports.tokenize = tokenize;
module.exports.tokenizeXml = tokenizeXml;
