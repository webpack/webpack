 ...
 */
const _attrValueDecoded = (i) =>
	xmlMode
		? decodeXmlEntities(_attrValueOf(i))
		: decodeEntities(_attrValueOf(i), true);
 ...
 * Unlike HTML, XML has no built-in entity table beyond those five names.
...
const decodeXmlEntities = (value) =>
	value.replace(
		...
		(match, hex, decimal, name) => {
			if (name !== undefined) {
				return Object.prototype.hasOwnProperty.call(XML_NAMED_REFERENCES, name)
...
const processXml = (t) => {
	...
	if (ty === TOKEN_END_TAG && open.length > 0 && _tagNameOf(cur()) === t.name) {
...
const _scanProcessingInstruction = (input, start, end) => {
	...
	if (
		(!xmlMode && rangeEqualsLowerCase(input, targetStart, i, "xml")) ||
		(!xmlMode && rangeEqualsLowerCase(input, targetStart, i, "xml-stylesheet"))
	) {
		return PI_SCAN_COMMENT;
	}
	_piScanTargetEnd = i;
