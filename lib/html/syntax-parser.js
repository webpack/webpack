...
const _sheetClosing = (css) => {
	...
	for (let i = 0; i < css.length && open.length === 0; i++) {
		...
		if (c === CC_REVERSE_SOLIDUS) {
			...
			else closers.push(CC_RIGHT_PARENTHESIS);
...
const _styleRunText = (path, element, css, absorb) => {
	// Printed as written, a closing would stay in the value a custom property
	// keeps as authored (`--x: f(` reads back `f()`); minified, it is closed anyway.
	const asWritten =
		_S._renderEmbeddedSource === undefined &&
		_S._deferEmbeddedSource === undefined;
	let text = css;
	let closing = _sheetClosing(css);
	if (asWritten && closing !== "") closing = null;
	for (
