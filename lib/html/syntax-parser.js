 ...
 * print again with it: the element going empty, or a join decided on its source.
 * @param {string} css the body
 * @returns {((answer: string | undefined) => boolean) | undefined} the test, or undefined where no choice rests on it
 */
const _sheetDecides = (css) => {
	const emptying = _removeEmptyElements;
	// A run is joined on its answer only where its text is left open.
	const joining = _S._mergeStyles && !_sheetEndsAtRule(css);
	if (!emptying && !joining) return undefined;
	return (answer) =>
		typeof answer === "string" &&
		((emptying && answer === "") || (joining && _sheetEndsAtRule(answer)));
};
...
/**
 * Whether a sheet ends where a rule does: outside every string, comment and
 * block, after a `}` or an at-rule's `;`. A sheet appended to it then starts
 * rules of its own (§5.4) rather than finishing this one's last.
 * @param {string} css the sheet's text
 * @returns {boolean} true when another sheet may follow it as written
 */
const _sheetEndsAtRule = (css) => {
	/** @type {number[]} */
	const closers = [];
 ...
 * source is appended while the text before it ends at a rule; where it is left
 * open, the renderer's answer for it is appended to instead, if that ends at one.
 * @param {HtmlPath} path the accessor
...
const _styleRunText = (path, element, css, absorb) => {
	...
	for (
		...
		if (!ends) {
			// Without an answer yet, `_sheetDecides` has the print run again.
			if (!_rendersNow()) break;
			text = _renderedSheet(text);
			if (!_sheetEndsAtRule(text)) break;
		}
		if (absorb) {
...
module.exports._sheetEndsAtRule = _sheetEndsAtRule;
