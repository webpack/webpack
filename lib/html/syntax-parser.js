...
// A declaration naming a custom property, past the whitespace and comments
// before it.
const _CUSTOM_PROPERTY_START_REGEXP = /^(?:\s|\/\*[\s\S]*?\*\/)*--/;
...
const _sheetClosing = (css) => {
	...
	for (let block = 0; block < closers.length; block++) {
		if (
			...
			_CUSTOM_PROPERTY_START_REGEXP.test(css.slice(declarations[block]))
		) {
			return null;
		}
	}
