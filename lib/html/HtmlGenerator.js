 ...
 * whether an `alterTags` plugin changed a tag, so unchanged tags stay untouched.
...
/** @typedef {{ tags: HtmlMutableTag[], spans: { start: number, openEnd: number, end: number }[], originals: string[] }} CollectedTags */
...
class HtmlGenerator extends Generator {
	 ...
	 * mutable descriptors (for the `alterTags` hook), each paired with its source
	...
	static collectTags(content) {
		/** @type {HtmlMutableTag[]} */
		const tags = [];
		/** @type {{ start: number, openEnd: number, end: number }[]} */
		const spans = [];
		/** @type {string[]} */
		const originals = [];
		new SourceProcessor()
			...
			.process(content, { skip: { text: true, doctype: true } });
		return { tags, spans, originals };
	}

	/**
	 * Applies the `alterTags` mutations back into `content`: a `remove`d tag is
	 * deleted, a tag whose `attrs` changed has its opening tag rewritten in place,
	 * and untouched tags are left byte-for-byte. Edits run right-to-left.
	 * @param {string} content the final HTML
	...
	static applyTagMutations(content, collected) {
		const { tags, spans, originals } = collected;
		/** @type {{ start: number, end: number, text: string }[]} */
		const edits = [];
		for (let i = 0; i < tags.length; i++) {
			const tag = tags[i];
			const { start, openEnd, end } = spans[i];
			if (tag.remove) {
				edits.push({ start, end, text: "" });
			} else if (snapshotAttrs(tag.attrs) !== originals[i]) {
				let attrStr = "";
				for (const name of Object.keys(tag.attrs)) {
					const value = tag.attrs[name];
				...
				edits.push({ start, end: openEnd, text: `<${tag.tag}${attrStr}>` });
			}
		}
