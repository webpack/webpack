...
class HtmlGenerator extends Generator {
	...
	_renderHmrShim(module, htmlLiteral, extracting) {
		...
		if (extracting) {
			// DOM patching uses regex extraction of the `<body>` and `<title>`
			// instead of `DOMParser` so the runtime stays small and works
			// uniformly across browsers and (just as importantly) any
			// document host the bundle is evaluated in. The patch only runs
			// on a re-evaluation (`module.hot.data` is set by the previous
			...
			lines.push(
				...
				"\t}",
				"\tmodule.hot.dispose(function (data) { data.__webpack_html_updated__ = true; });"
			);
		}
		lines.push("}");
		return lines.join("\n");
