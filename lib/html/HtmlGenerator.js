...
class HtmlGenerator extends Generator {
	...
	static renderHtml(content, model, csp) {
		...
		// replace/delete an existing tag's span; region groups hold placements for a
		// missing `<head>`/`<body>` (head tags prepended, body tags appended).
		...
		const edits = [];
		// CSP meta, placed first at the head anchor so it precedes a head-prepend tag.
		let cspNoHeadPrefix = "";
		for (let i = 0; i < tags.length; i++) {
			const tag = tags[i];
			const span = spans[i];
		...
		if (addCsp) {
			...
			if (anchors.headOpen >= 0) at(anchors.headOpen, meta);
			else cspNoHeadPrefix = meta;
		}
		// Emit region placements at the `<head>`/`<body>` anchors (after any CSP meta,
		// prepend before the plain name); a missing region wraps as prefix/suffix.
		let prefix = cspNoHeadPrefix;
		let suffix = "";
		if (anchors.headOpen >= 0) {
			at(anchors.headOpen, groups["head-prepend"]);
			at(anchors.headEnd, groups.head);
		} else {
			prefix += groups["head-prepend"] + groups.head;
		}
		if (anchors.bodyOpen >= 0) {
			at(anchors.bodyOpen, groups["body-prepend"]);
			at(anchors.bodyEnd, groups.body);
		} else {
			prefix += groups["body-prepend"];
			suffix += groups.body;
		}
		for (const [pos, text] of inserts) {
			edits.push({ start: pos, end: pos, text, insert: true });
		}
		if (edits.length === 0) return prefix + content + suffix;
		edits.sort(
