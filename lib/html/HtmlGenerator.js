...
const createHashModule = require("crypto").createHash;
...
const HtmlSyntax = require("./syntax");
...
const serializeAttrs = (attrs) => {
	let out = "";
	if (attrs) {
		const { escapeAttribute } = HtmlSyntax;
...
const rebaseCssUrls = (css, fromFile, toFile, outputPath) => {
	...
	const { readToken, TT_URL } = CssSyntax;
	const token = {
		type: 0,
		start: 0,
		end: 0,
...
class HtmlGenerator extends Generator {
	...
	static collectHtml(content) {
		...
		const { NodeType, SourceProcessor } = HtmlSyntax;
		new SourceProcessor()
			.use({
				[NodeType.Element]: {
					enter: (path) => {
