...
// Memoized rather than required at module scope: `HtmlModulesPlugin` reaches
// this file the same way, and neither may be half-loaded when the other lands.
const getHtmlModulesPlugin = memoize(() => require("./HtmlModulesPlugin"));
...
class HtmlGenerator extends Generator {
	...
	generate(module, generateContext) {
		...
		const generated = getHtmlModulesPlugin()
			.getCompilationHooks(generateContext.runtimeTemplate.compilation)
			.renderHtmlInJavascript.call(
				this._renderHtml(module, generateContext, ""),
				module
			);
	...
	updateHash(hash, updateHashContext) {
		...
		// A `renderHtmlInJavascript` tap has to reach the codegen cache key, or a
		// changed option replays the output from before it.
		const { runtimeTemplate } = updateHashContext;
		if (runtimeTemplate) {
			getHtmlModulesPlugin()
				.getCompilationHooks(runtimeTemplate.compilation)
				.htmlInJavascriptHash.call(updateHashContext.module, hash);
		}
	}
}
