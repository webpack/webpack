...
class HtmlGenerator extends Generator {
	...
	generate(module, generateContext) {
		...
		if (generateContext.concatenationScope && !module.hot) {
			...
			const htmlLiteral = JSON.stringify(generated);
			if (module.hot) {
				...
				sourceContent = `${module.moduleArgument}.exports = ${htmlLiteral};`;
			}
		}
	 ...
	 * @param {NormalModule} module module
	 * @param {string} htmlLiteral the HTML content as a JSON-encoded string literal
	 * @param {boolean} extracting whether the HTML module is being extracted as a real `.html` file
	 * @returns {string} JS shim source
	 */
	_renderHmrShim(module, htmlLiteral, extracting) {
		const lines = [
			`var __webpack_html__ = ${htmlLiteral};`,
			`${module.moduleArgument}.exports = __webpack_html__;`,
			"if (module.hot) {",
			"\tmodule.hot.accept();"
		];
		if (extracting) {
			// DOM patching uses regex extraction of the `<body>`, `<title>`
			// and `<head>` instead of `DOMParser` so the runtime stays
			// small and works uniformly across browsers and any document
			// host the bundle is evaluated in. The patch only runs on a
			// re-evaluation (`module.hot.data` is set by the previous
			// instance's dispose handler below), so the initial page load
			// — where `document.body` already matches the extracted HTML
			// — never triggers it.
			//
			// `<head>` changes beyond `<title>` (a new `<meta>`, a swapped
			// `<link rel=icon>`, an inline `<style>` block, …) can't be
			// safely DOM-patched: webpack injects its own runtime scripts
			// and stylesheet links into the head on initial load, and
			// blindly replacing `document.head.innerHTML` would tear them
			// down. So we fall back to a full reload — which is fine in a
			// dev-server context because the regular (non-hot-update)
			// `page.html` chunk is re-emitted on every rebuild, so the
			// reloaded page picks up the new head.
			lines.push(
				'\tvar __webpack_get_head__ = function (h) { var m = /<head[^>]*>([\\s\\S]*?)<\\/head>/i.exec(h); return m ? m[1].replace(/<title[^>]*>[\\s\\S]*?<\\/title>/i, "").trim() : ""; };',
				"\tif (module.hot.data && typeof document !== 'undefined') {",
				"\t\tvar __webpack_new_head__ = __webpack_get_head__(__webpack_html__);",
				...
				"\t\t} else {",
				"\t\t\tvar __webpack_body__ = /<body[^>]*>([\\s\\S]*?)<\\/body>/i.exec(__webpack_html__);",
				"\t\t\tif (__webpack_body__ && document.body) document.body.innerHTML = __webpack_body__[1];",
				"\t\t\tvar __webpack_title__ = /<title[^>]*>([\\s\\S]*?)<\\/title>/i.exec(__webpack_html__);",
				"\t\t\tif (__webpack_title__) document.title = __webpack_title__[1];",
				"\t\t}",
				"\t}",
				// Capture the current head (sans title) on every dispose so
				// the next module instance can diff against it.
				"\tmodule.hot.dispose(function (data) { data.__webpack_html_updated__ = true; data.__webpack_head__ = __webpack_get_head__(__webpack_html__); });"
			);
		}
		lines.push("}");
		return lines.join("\n");
