...
/** @typedef {{ tags: HtmlMutableTag[], spans: { start: number, openEnd: number, end: number, location: HtmlTagLocation }[], originals: string[], anchors: { headOpen: number, headEnd: number, bodyOpen: number, bodyEnd: number } }} CollectedTags */
...
class HtmlGenerator extends Generator {
	...
	/**
	 * Injects a `<meta http-equiv="Content-Security-Policy">` for
	 * `output.html.csp`. Hashes every inline `<script>`/`<style>` body into
	 * `script-src`/`style-src` over a strict baseline (`object-src 'none'`,
	 * `base-uri 'self'`); `options.policy` overrides directives,
	 * `options.hashFunction` picks the algorithm, and `options.nonce` adds a
	 * placeholder `nonce` to injected `<script>`/`<style>` tags plus a matching
	 * `'nonce-…'` source (for a server to rewrite per request). No-op when the
	 * page already declares a CSP. Runs after inline content is resolved, so the
	 * hashes cover the exact bytes the browser sees.
	 * @param {string} content final HTML (inline content already resolved)
	 * @param {CspOption} csp the `output.html.csp` option
	 * @returns {string} HTML with the CSP meta injected
	 */
	static injectCsp(content, csp) {
		if (!csp) return content;
		...
		let hasCsp = false;
		// Meta insertion point (just inside `<head>`) and the offsets, right after
		// each `<script>`/`<style>` tag name, where a nonce attribute is inserted.
		let headEnd = -1;
		/** @type {number[]} */
		const nonceAt = [];
		// Walk the *final* HTML with the same parser the rest of the pipeline uses.
		// Offsets from the original source parse no longer map here — inline content,
		// sentinels and sibling tags were all resolved after it — so this reads the
		// emitted bytes directly (`</script` stays escaped, so bodies hash exactly as
		// the browser sees them). `skip.text` populates `contentEnd` (the raw-text
		// body end) without materializing text nodes.
		new SourceProcessor()
			.use({
				[NodeType.Element]: (path) => {
					const name = path.tagName();
					if (name === "head") {
						if (headEnd < 0) headEnd = path.tagEnd();
						return;
					}
					if (name === "meta") {
						...
						if (
							httpEquiv !== 0 &&
							path.attributeValue(httpEquiv).trim().toLowerCase() ===
					...
					if (nonce && path.findAttribute("nonce") === 0) {
						nonceAt.push(path.nameEnd());
					}
					// External `<script src>` is covered by `'self'`; only an inline
					// (raw-text) body needs a hash.
					const isScript = name === "script";
					if (isScript && path.findAttribute("src") !== 0) return;
					if (path.selfClosing()) return;
					const body = content.slice(path.tagEnd(), path.contentEnd());
					if (body !== "") (isScript ? scriptSrc : styleSrc).add(hashOf(body));
				}
			})
			.process(content, { skip: { text: true, doctype: true } });
		// An author-declared policy wins — never override it.
		if (hasCsp) return content;
		if (nonce) {
			scriptSrc.add(`'nonce-${nonce}'`);
			styleSrc.add(`'nonce-${nonce}'`);
		}
		// Strict baseline; user `policy` replaces a directive; hashes/nonce append.
		...
		if (options.policy) {
			for (const [name, value] of Object.entries(options.policy)) {
		...
		// Apply insertions right-to-left so earlier offsets stay valid: the nonce
		// attributes, then the meta first inside `<head>` (prepended if there's none).
		/** @type {{ pos: number, text: string }[]} */
		const edits = nonce
			? nonceAt.map((pos) => ({
					pos,
					text: ` nonce="${escapeAttribute(nonce)}"`
				}))
			: [];
		if (headEnd >= 0) edits.push({ pos: headEnd, text: meta });
		edits.sort((a, b) => b.pos - a.pos);
		let result = content;
		for (const edit of edits) {
			result = result.slice(0, edit.pos) + edit.text + result.slice(edit.pos);
		}
		return headEnd >= 0 ? result : meta + result;
	}

	/**
	 * Places `injectTags` descriptors into the final HTML at their `injectTo`
	 * position. Locates the `<head>`/`<body>` open and content-end offsets with the
	 * same parser the rest of the pipeline uses (no regex); `*-prepend` goes right
	 * after the open tag, the plain names right before the close. A fragment without
	 * a `<head>`/`<body>` gets head tags prepended and body tags appended.
	 * @param {string} content the final HTML
	 * @param {HtmlTagDescriptor[]} tags descriptors to inject
	 * @returns {string} the HTML with the tags placed
	 */
	static injectTags(content, tags) {
		if (tags.length === 0) return content;
		// Open-tag ends (for `*-prepend`) and content ends (for append) of the
		// document's `<head>`/`<body>`; the content end tracks the max end of nodes
		// seen while inside the element (its last child's end ≈ before the close).
		let headOpen = -1;
		let headEnd = -1;
		let bodyOpen = -1;
		let bodyEnd = -1;
		...
		for (const tag of tags) {
			const to = tag.injectTo || "head";
			group[to in group ? to : "head"] += serializeTag(tag);
		}
		// Keyed by offset and appended in placement order, so when an empty
		// `<head>`/`<body>` makes the prepend and append offsets coincide the
		// prepend text still lands first.
		/** @type {Map<number, string>} */
		const byPos = new Map();
		/** @type {(pos: number, text: string) => void} */
		const at = (pos, text) => {
			if (text) byPos.set(pos, (byPos.get(pos) || "") + text);
		};
		let prefix = "";
		let suffix = "";
		if (headOpen >= 0) {
			at(headOpen, group["head-prepend"]);
			at(headEnd, group.head);
		} else {
			prefix += group["head-prepend"] + group.head;
		}
		if (bodyOpen >= 0) {
			at(bodyOpen, group["body-prepend"]);
			at(bodyEnd, group.body);
		} else {
			prefix += group["body-prepend"];
			suffix += group.body;
		}
		let result = content;
		for (const pos of [...byPos.keys()].sort((a, b) => b - a)) {
			result =
				result.slice(0, pos) +
				/** @type {string} */ (byPos.get(pos)) +
				result.slice(pos);
		}
	...
	/**
	 * Collects the `<script>`/`<link>`/`<style>`/`<meta>` tags in `content` as
	 * mutable descriptors (for the `transformTags` hook), each paired with its source
	 * span and a snapshot of its attributes so `applyTagMutations` can rewrite only
	 * the tags a plugin actually changed. Uses the pipeline's own parser (no regex).
	 * @param {string} content the final HTML
	 * @returns {CollectedTags} the descriptors plus internal span/snapshot data
	 */
	static collectTags(content) {
		/** @type {HtmlMutableTag[]} */
		const tags = [];
		/** @type {CollectedTags["spans"]} */
		const spans = [];
		/** @type {string[]} */
		const originals = [];
		// `<head>`/`<body>` open- and content-end offsets, so `applyTagMutations`
		// can move a tag between the two regions (matching each tag's `location`).
		let headOpen = -1;
		let headEnd = -1;
		let bodyOpen = -1;
		let bodyEnd = -1;
			...
			.use({
				[NodeType.Element]: {
					enter: (path) => {
						...
						const location = bodyDepth > 0 ? "body" : "head";
						tags.push({ tag: name, attrs, injectTo: location });
						spans.push({
							start: path.start(),
							openEnd: path.tagEnd(),
							end,
	...
	/**
	 * Applies the `transformTags` mutations back into `content`: a `remove`d tag is
	 * deleted, a tag whose `injectTo` no longer matches its `location` is moved to
	 * the other region (`<head>`↔`<body>`, honoring the `*-prepend` variants), a tag
	 * whose `attrs` changed has its opening tag rewritten, and untouched tags are
	 * left byte-for-byte. Edits run right-to-left; same-offset inserts keep order.
	 * @param {string} content the final HTML
	 * @param {CollectedTags} collected the value from `collectTags`, after the hook
	 * @returns {string} the HTML with the mutations applied
	 */
	static applyTagMutations(content, collected) {
		const { tags, spans, originals, anchors } = collected;
		/** @type {Record<string, number>} */
		const anchorPos = {
			head: anchors.headEnd,
			"head-prepend": anchors.headOpen,
			body: anchors.bodyEnd,
			"body-prepend": anchors.bodyOpen
		};
		// Range edits replace/delete an existing tag's span (distinct spans never
		// overlap); moved tags are re-inserted at their target region's anchor,
		// accumulated per offset in document order.
		...
		for (let i = 0; i < tags.length; i++) {
			const tag = tags[i];
			const { start, openEnd, end, location } = spans[i];
			if (tag.remove) {
				edits.push({ start, end, text: "", insert: false });
				continue;
			}
			const attrsChanged = snapshotAttrs(tag.attrs) !== originals[i];
			const opening = attrsChanged
				? `<${tag.tag}${serializeAttrs(tag.attrs)}>`
				: content.slice(start, openEnd);
			const target = tag.injectTo;
			if (
				target !== undefined &&
				target !== location &&
				anchorPos[target] >= 0
				...
				edits.push({ start, end, text: "", insert: false });
				const pos = anchorPos[target];
				inserts.set(
					pos,
					(inserts.get(pos) || "") + opening + content.slice(openEnd, end)
				);
			} else if (attrsChanged) {
				edits.push({ start, end: openEnd, text: opening, insert: false });
			}
		}
		...
		// so the insert's text stays intact after the range collapses.
		edits.sort(
