...
const getCssModulesPlugin = memoize(
	() =>
...
const getCssSyntax = memoize(() => require("../css/syntax.js"));

/** @typedef {import("webpack-sources").Source} Source */
/** @typedef {import("../../declarations/WebpackOptions.js").HtmlGeneratorOptions} HtmlGeneratorOptions */
...
const INLINE_SENTINEL_REGEXP =
	/__WEBPACK_HTML_INLINE__([0-9a-f]+)__([a-z]+)__END__/g;
...
class HtmlGenerator extends Generator {
	...
	static stripChunkIntegritySentinels(content) {
		...
		return content.replaceAll(INTEGRITY_SENTINEL_STRIP_REGEXP, "");
	}
			...
			(_, hexId, contentHashType) => {
				...
				for (const file of chunk.files) {
					...
					if (isCss) {
						...
						raw = rebaseCssUrls(raw, file, htmlFilename, outputPath).replaceAll(
							/<\/(style)/gi,
							"<\\/$1"
						);
					} else {
	 ...
	 * @param {import("../DependenciesBlock.js").default} block the dependencies block which will be processed
	 * @param {ReplaceSource} source the current replace source which can be modified
	...
	generateError(error, module, generateContext) {
		if (generateContext.type === HTML_TYPE) {
			...
			const safe = String(error.message)
				.replaceAll(/[<>]/g, "")
				.replaceAll(/-{2,}/g, (m) => `${"-".repeat(m.length - 1)} `);
...
export { HtmlGenerator as "module.exports" };
