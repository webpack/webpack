...
class HtmlGenerator extends Generator {
	...
	static isChunkInlined(htmlOption, chunk, contentType) {
		...
		// An extracted inline `<script>` is named after its page rather than its
		// entry, so the template it emits under is matched as well. A pattern the
		// caller made global carries `lastIndex` between chunks, so it is reset.
