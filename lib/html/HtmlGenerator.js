...
class HtmlGenerator extends Generator {
	 ...
	 * the HTML with regexes. HTML comments are stripped *first* via a
	 * `<!--…-->` regex, so a `<body>` (or `</body>` / `<title>` / etc.)
	 * that appears inside a comment can't fool the tag regexes — that was
	 * the one well-known sharp edge of HTML+regex.
	 *
	...
	_renderHmrShim(module, html, extracting) {
		...
		if (extracting) {
			lines.push(
				// Strip `<!-- … -->` comments before any tag-extraction
				// regex runs. Without this, a comment like
				// `<!-- <body>example</body> -->` would be picked up as
				// the actual `<body>` content (the non-greedy `*?` in the
				// tag regex stops at the FIRST `</body>` it sees, which
				// would be the one inside the comment).
				'\tvar __webpack_strip_comments__ = function (h) { return h.replace(/<!--[\\s\\S]*?-->/g, ""); };',
				"\tvar __webpack_extract_head__ = function (h) {",
				"\t\tvar m = /<head[^>]*>([\\s\\S]*?)<\\/head>/i.exec(__webpack_strip_comments__(h));",
				'\t\treturn m ? m[1].replace(/<title[^>]*>[\\s\\S]*?<\\/title>/i, "").trim() : "";',
				"\t};",
