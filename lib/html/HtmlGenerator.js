...
// entirely when the per-asset `integrity` function returns `false`. Minification
// runs first and may have unquoted the value (§13.1.2.3), so the quote is
// optional and back-referenced to keep the pair matched; the replacement always
// re-quotes, since an SRI list carries spaces and `=` padding.
		...
		new SourceProcessor()
			.use({
				[NodeType.Element]: {
					...
					// Collected on exit: an element's `end` is only final once it
					// closes, and the streamed walk enters it while it is still open.
					// `script` / `style` / `link` / `meta` never nest in each other, so
					// exiting keeps them in source order.
					exit: (path) => {
						const name = path.tagName();
						if (name === "head") {
			...
			? [
					...
					`${declare} __webpack_patch_head__ = ${runtimeTemplate.basicFunction(
						...
						[
							...
							// one edit still worth a reload — but only a script that ran. A
							// `type` that is neither a JavaScript MIME type nor `module` /
							// `importmap` / `speculationrules` makes the element a data block
							// (`application/ld+json`, `text/template`, …) the browser never
							// executed, so removing it is as safe as removing a `<meta>`.
							// Every JavaScript MIME essence ends in `script`, and a type
							// nothing here recognizes falls back to the reload.
