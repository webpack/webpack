...
class HtmlGenerator extends Generator {
	 ...
	 * known. Safe to assume `chunk.id` is set — every code path that
	 * embeds an HTML chunk URL runs during or after `codeGeneration`,
	 * which `Compilation#seal` guarantees runs after the
	 * `optimizeChunkIds` hook (i.e. after `chunkIds` plugins have
	 * assigned every chunk an id).
