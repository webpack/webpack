...
/**
 * Whether a text node is the whole body of an SVG or XHTML `<style>`, which a
 * caller's renderer is offered as the stylesheet it is.
 * @param {HtmlPath} path the accessor positioned on the text node
 * @returns {boolean} true when the text is a stylesheet to render
 */
const _isXmlStylesheet = (path) => {
	if (
	...
	if (name !== "style" && !name.endsWith(":style")) return false;
	// As in HTML, the attribute is optional and only `text/css` keeps it CSS.
	for (const attribute of path.attributes(parent)) {
		if (attribute.name !== "type") continue;
		const type = _asciiLowerCase(
			_asciiTrim(decodeXmlAttribute(attribute.value))
		);
		if (type !== "" && type !== "text/css") return false;
	}
	// A comment or an element beside it splits the sheet, which is left alone.
	return (
		path.firstChild(parent) === path.node &&
		path.nextSibling(path.node) === 0 &&
		_xmlElementIsStyled(parent)
	);
};
...
const _printXml = (path, writer, minify) => {
	switch (path.type()) {
		...
		case NodeType.Text: {
			...
			if (minify && _isXmlStylesheet(path)) {
				return data.trim() === ""
					? raw
					: _S._deferEmbeddedSource === undefined
						? _xmlCharacterData(_renderEmbedded(data, CSS_TYPE))
						: _deferEmbedded(data, CSS_TYPE, (rendered) =>
								_xmlCharacterData(
									typeof rendered === "string" ? rendered : data
								)
							);
			}
