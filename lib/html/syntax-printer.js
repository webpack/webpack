...
const _minifyStyleBody = (css) => {
	...
	return _renderEmbedded(css, CSS_TYPE, undefined, _sheetDecides(css));
};
...
const _mergedStyleRun = (path, element, css) => {
	...
	// renderer closes what a sheet left open. A marker says nothing yet.
	const deferred = !_rendersNow();
	if (!_sheetEndsCleanly(deferred ? css : out)) return out;
	for (
		...
		if (
			(deferred && !_sheetEndsCleanly(text)) ||
			_SHEET_LEADING_AT_RULE_REGEXP.test(text)
		) {
			break;
		}
		const printed = _minifyStyleBody(text);
		if (!deferred && !_sheetEndsCleanly(printed)) break;
 ...
 * part of it. A `<!--` one is rebuilt from its data, since text ending in `--`
 * would close differently; the rest end at their first `>` and get one.
...
const _closedCommentSource = (path) => {
	...
	if (path.end() !== _S.source.length || source.endsWith(">")) return source;
	return source.startsWith("<!--") ? `<!--${path.data()}-->` : `${source}>`;
};
