...
const {
	...
	_isSvgRoot,
	_keepComment,
	_mergeableStyleAfter,
	_minifyOpenTag,
	_nodeStrings,
	_onlyTextChild,
	_openTagSource,
	_printedAttributeCount,
	_printsNothing,
 ...
 * @property {((offer: Omit<DeferredEmbeddedSource, "build">) => string | undefined)=} embeddedAnswers what a print before this one was answered for each offer, which `processAsync` passes where an answer overturned a choice made without it: a body it answers is written from it rather than deferred
 ...
 * @param {((rendered: string | undefined) => boolean)=} decides whether a deferred answer overturns a choice this print made without it
 * @returns {string} the rendered body, or the original
 */
const _renderEmbedded = (source, type, as, decides) => {
 ...
 * @param {boolean=} joined whether it is one of several sheets a run prints as one, which is printed again once they are answered
 * @returns {string} the rendered text, the marker standing for it, or the text as written
 */
const _minifyStyleBody = (css, joined) => {
	if (css.trim() === "") return css;
	// Through the emptiness check's memo, which may have rendered it already.
	if (_rendersNow()) return _takeRenderedSheet(css);
	return _renderEmbedded(
		css,
		CSS_TYPE,
		undefined,
		joined ? _answerJoins : _sheetDecides(css)
	);
};

/**
 * The `decides` of a sheet a run joins: the run is rendered as the one sheet it
 * prints as, which only a print holding each sheet's answer can offer.
 * @returns {boolean} true
 */
const _answerJoins = () => true;
 ...
 * and text print as nothing. A sheet joins when what it printed ends cleanly —
 * its source where the answer is still a marker — and not after the first when it
 * carries `@import` (or `@charset` / `@namespace`), which would stop applying.
 * @param {HtmlPath} path the accessor
...
const _mergedStyleRun = (path, element, css) => {
	if (!_rendersNow()) return _deferredStyleRun(path, element, css);
	let out = _minifyStyleBody(css);
	// What is joined is what printed, so that is what has to end cleanly — the
	// renderer closes what a sheet left open.
	if (!_sheetEndsCleanly(out)) return out;
	let joined = false;
	for (
		...
		const printed = _minifyStyleBody(text);
	...
	// Joined, the sheets are one, and the renderer reads rules across them as a
	// second pass over this element would; a blank sheet joined is no longer one.
	return joined ? _minifyStyleBody(out) : out;
};

/**
 * `_mergedStyleRun` where the answers are still to come: a marker says nothing
 * of how its sheet ends, so the run is read off the sources, and a run of more
 * than one has `processAsync` print again with the answers in hand.
 * @param {HtmlPath} path the accessor
 * @param {HtmlElement} element the run's first `<style>`
 * @param {string} css its own text
 * @returns {string} the run's text
 */
const _deferredStyleRun = (path, element, css) => {
	/** @type {string[]} */
	const texts = [];
	if (_sheetEndsCleanly(css)) {
		for (
			let next = _mergeableStyleAfter(path, element);
			next !== 0;
			next = _mergeableStyleAfter(path, next)
			...
			const text =
				_nodeStrings[/** @type {HtmlNodeRef} */ (_onlyTextChild(next))];
			if (
				!_sheetEndsCleanly(text) ||
				_SHEET_LEADING_AT_RULE_REGEXP.test(text)
			) {
				break;
			}
			if (_S._absorbed === null) _S._absorbed = new Set();
	...
	let out = _minifyStyleBody(css, joined);
	for (const text of texts) out += _minifyStyleBody(text, joined);
	return out;
};
...
/**
 * A comment or processing instruction as written, closed where the input ran
 * out inside it (§13.2.5): left open, the end tags printed after it are read as
 * part of it. A `<!--` one reads a `>` as data unless `--` or `--!` precedes it,
 * so it is rebuilt from its data; the rest end at their first `>` and get one.
 * @param {HtmlPath} path the accessor positioned on the node
 * @returns {string} the node's source, closed
 */
const _closedCommentSource = (path) => {
	const source = path.source();
	if (path.end() !== _S.source.length) return source;
...
const printer = (path, writer) => {
	...
	switch (path.type()) {
		...
		case NodeType.ProcessingInstruction:
			...
			return _closedCommentSource(path);
		default: {
