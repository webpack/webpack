...
module.exports.runLoaders = function runLoaders(options, callback) {
	// Reuse a context `createLoaderContext` prepared, else augment the given one in
	// place. State survives the handoff, so a host hook can pre-add dependencies;
	// a caller re-running on one context should `clearDependencies()` first.
