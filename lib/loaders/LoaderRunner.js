...
/** @import { Environment } from "../../declarations/WebpackOptions" */
...
/**
 * @typedef {object} AdditionalDataKnown
 * @property {EXPECTED_OBJECT} webpackAST the AST webpack should parse instead of the source
 */

/** @typedef {AdditionalDataKnown & Record<string, EXPECTED_ANY>} AdditionalData */

/**
 * @callback WebpackLoaderContextCallback
 * @param {undefined | null | Error} err error object
 * @param {(string | Buffer)=} content the loader result
 * @param {(null | string | RawSourceMap)=} sourceMap a source map for the result
 * @param {AdditionalData=} additionalData data passed to the next loader
 * @returns {void}
 */
 ...
 * @property {EXPECTED_OBJECT | string | undefined} options
 * @property {string} ident
 * @property {EXPECTED_FUNCTION | undefined} normal
 * @property {EXPECTED_FUNCTION | undefined} pitch
 * @property {boolean | undefined} raw
 * @property {EXPECTED_OBJECT | undefined} data
 * @property {boolean} pitchExecuted
 * @property {boolean} normalExecuted
 ...
 * @typedef {{ readonly currentRequest: string, readonly data: EXPECTED_ANY, readonly previousRequest: string, readonly query: string | OptionsType, readonly remainingRequest: string, readonly request: string }} LoaderRunnerReadonlyContext
 */

/**
 * @template OptionsType
 * @typedef {object} LoaderRunnerMutableContext
 * @property {(context: string) => void} addContextDependency Add a directory as dependency of the loader result.
 * @property {(file: string) => void} addDependency Adds a file as dependency of the loader result in order to make them watchable.
 * For example, html-loader uses this technique as it finds src and src-set attributes.
 * Then, it sets the url's for those attributes as dependencies of the html file that is parsed.
 * @property {(context: string) => void} addMissingDependency
 * @property {() => WebpackLoaderContextCallback} async Make this loader async.
 * @property {(flag?: boolean) => void} cacheable Make this loader result cacheable. By default it's cacheable.
 * A cacheable loader must have a deterministic result, when inputs and dependencies haven't changed.
 * This means the loader shouldn't have other dependencies than specified with this.addDependency.
 * Most loaders are deterministic and cacheable.
 * @property {WebpackLoaderContextCallback} callback
 * @property {() => void} clearDependencies Remove all dependencies of the loader result. Even initial dependencies and these of other loaders.
 ...
 * @property {(file: string) => void} dependency alias of addDependency
 * Adds a file as dependency of the loader result in order to make them watchable.
 * For example, html-loader uses this technique as it finds src and src-set attributes.
 * Then, it sets the url's for those attributes as dependencies of the html file that is parsed.
 * @property {() => string[]} getContextDependencies
 * @property {() => string[]} getDependencies
 * @property {() => string[]} getMissingDependencies
 * @property {number} loaderIndex The index in the loaders array of the current loader.
 * In the example: in loader1: 0, in loader2: 1
 * @property {LoaderEntry[]} loaders An array of all the loaders. It is writeable in the pitch phase.
 * loaders = [{request: string, path: string, query: string, module: function}]
 * @property {string} resourcePath The resource path.
 * In the example: "/abc/resource.js"
 * @property {string} resourceQuery The resource query string.
 * Example: "?query"
 * @property {string} resourceFragment The resource fragment.
 * Example: "#frag"
 * @property {string} resource The resource inclusive query and fragment.
 * Example: "/abc/resource.js?query#frag"
 * @property {string} target Target of compilation.
 * Example: "web"
 * @property {Environment} environment Tell what kind of ES-features may be used in the generated runtime-code.
 ...
 * @template OptionsType
 * @typedef {LoaderRunnerMutableContext<OptionsType> & LoaderRunnerReadonlyContext<OptionsType>} LoaderRunnerLoaderContext
 */

/**
 * @template [OptionsType={}]
 * @template [ContextAdditions={}]
 * @typedef {(this: import("../module/NormalModule").LoaderContext<OptionsType> & ContextAdditions, remainingRequest: string, previousRequest: string, data: EXPECTED_OBJECT) => string | Buffer | Promise<string | Buffer> | void} PitchLoaderDefinitionFunction
 */

/**
 * @template [OptionsType={}]
 * @template [ContextAdditions={}]
 * @typedef {(this: import("../module/NormalModule").LoaderContext<OptionsType> & ContextAdditions, content: string, sourceMap?: string | RawSourceMap, additionalData?: AdditionalData) => string | Buffer | Promise<string | Buffer> | void} LoaderDefinitionFunction
 */

/**
 * @template [OptionsType={}]
 * @template [ContextAdditions={}]
 * @typedef {(this: import("../module/NormalModule").LoaderContext<OptionsType> & ContextAdditions, content: Buffer, sourceMap?: string | RawSourceMap, additionalData?: AdditionalData) => string | Buffer | Promise<string | Buffer> | void} RawLoaderDefinitionFunction
 */

/**
 * @template [OptionsType={}]
 * @template [ContextAdditions={}]
 * @typedef {LoaderDefinitionFunction<OptionsType, ContextAdditions> & { raw?: false, pitch?: PitchLoaderDefinitionFunction<OptionsType, ContextAdditions> }} LoaderDefinition
 */

/**
 * @template [OptionsType={}]
 * @template [ContextAdditions={}]
 * @typedef {RawLoaderDefinitionFunction<OptionsType, ContextAdditions> & { raw: true, pitch?: PitchLoaderDefinitionFunction<OptionsType, ContextAdditions> }} RawLoaderDefinition
 */
 ...
 * @property {(RawLoaderDefinitionFunction<OptionsType, ContextAdditions> | LoaderDefinitionFunction<OptionsType, ContextAdditions>)=} default
 * @property {false=} raw
 * @property {PitchLoaderDefinitionFunction<OptionsType, ContextAdditions>=} pitch
 */
 ...
 * @typedef {Omit<LoaderRunnerLoaderContext<EXPECTED_ANY>, "context" | "callback" | "async" | "loaders"> & { context: string | null, callback: LoaderCallback | null, async: (() => LoaderCallback | undefined) | null, loaders: LoaderObject[] }} LoaderContext
 */
 ...
 * @param {LoaderRunnerLoaderContext<EXPECTED_ANY>} loaderContext loader context
...
module.exports.runLoaders = function runLoaders(options, callback) {
	// WHY: reuse a context `createLoaderContext` already prepared, from
	// NormalModule say, else augment the caller-provided context, or a fresh
	// object, in place. State is intentionally preserved across the handoff, so a
	// host hook such as `beforeLoaders` can pre-add dependencies or mark the
	// request non-cacheable before the run; a caller re-running on the same context
	// should `clearDependencies()` first, to avoid accumulating stale ones.
