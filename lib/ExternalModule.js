...
/** @typedef {{ assertions?: Assertions }} ImportDependencyMeta */
...
const getSourceForImportExternal = (
	...
	const assertions = dependencyMeta.assertions
		? ` , ${JSON.stringify(dependencyMeta.assertions)}`
		: "";
	if (!Array.isArray(moduleAndSpecifiers)) {
		return {
	...
	return {
		expression: `${importName}(${JSON.stringify(
			moduleName
		)}${assertions}).then(${runtimeTemplate.returningFunction(
	...
	constructor(request, ident, dependencyMeta, hashFunction = "md4") {
		...
		super(
			`import * as ${identifier} from ${JSON.stringify(request)}${
				dependencyMeta && dependencyMeta.assertions
					? ` assert ${JSON.stringify(dependencyMeta.assertions)}`
					: ""
			};\n`,
			InitFragment.STAGE_HARMONY_IMPORTS,
			0,
