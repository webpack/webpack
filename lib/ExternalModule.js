	...
	if (url === "host") {
		return {
			...
			expression: `new Promise(${runtimeTemplate.basicFunction(
				...
				[
					`
						if(!${RuntimeGlobals.shareScopeMap}) reject(__webpack_error__);
						if(!${RuntimeGlobals.hasOwnProperty}(${RuntimeGlobals.shareScopeMap}, scope)) reject(__webpack_error__);
						if(!${RuntimeGlobals.hasOwnProperty}(${RuntimeGlobals.shareScopeMap}[scope], module)) reject(__webpack_error__);
						if(!${RuntimeGlobals.hasOwnProperty}(${RuntimeGlobals.shareScopeMap}[scope][module], '0')) reject(__webpack_error__);
						resolve()
					`
				]
			)}).then(${runtimeTemplate.returningFunction(
				`Promise.resolve().then(() => ${RuntimeGlobals.shareScopeMap}[scope][module][0].get())`
			)})`,
