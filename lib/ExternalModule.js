 ...
 * @returns {boolean} whether the request is a Node.js core module
 */
const isNodeCoreModule = (request) => NODE_CORE_MODULES.has(request);
...
class ModuleExternalInitFragment extends InitFragment {
	...
	getContent(context) {
		...
		// Emit `node:` for core modules so the output also runs on Deno/Bun, which
		// require the prefix; gated by the environment (opt-out via
		// `output.environment.nodePrefixForCoreModules`). The external identifier
		// stays derived from the raw request, so only the specifier changes.
		const source = JSON.stringify(
			runtimeTemplate &&
				runtimeTemplate.supportNodePrefixForCoreModules() &&
				typeof request === "string" &&
				!request.startsWith("node:") &&
				isNodeCoreModule(request)
				? `node:${request}`
				: request
		);
		const attributes =
		...
		if (imported === true) {
			...
			content = `import ${phaseKeyword}* as ${identifier} from ${source}${attributes};\n`;
		} else if (imported.length === 0) {
			// just import, no use
			content = `import ${source}${attributes};\n`;
		} else if (
			...
			content = `import source ${imported[0][1]} from ${source}${attributes};\n`;
		} else {
