...
const getExternalModuleNodeCommonjsInitFragment = (
	...
	// A static `import ... from "module"` breaks the bundle at load time in non-node
	// runtimes, so obtain `createRequire` via `process.getBuiltinModule()` instead:
	// guarded inline when the node version is known to support it, otherwise in a
	// try/catch so the bundle still loads where `process` is absent.
	const createRequire = `process.getBuiltinModule(${moduleId}).createRequire(${importMetaName}.url)`;
	const content = runtimeTemplate.supportsNodeBuiltinModuleGetter()
		? `${runtimeTemplate.renderConst()} __WEBPACK_EXTERNAL_createRequire_require = typeof process !== "undefined" && process.getBuiltinModule && ${createRequire};\n`
		: `${runtimeTemplate.renderLet()} __WEBPACK_EXTERNAL_createRequire_require;\ntry { __WEBPACK_EXTERNAL_createRequire_require = ${createRequire}; } catch (e) {}\n`;
 ...
 * @property {boolean=} universalExternal true when the external may load in both node and non-node runtimes (universal target)
 */
...
class ExternalModule extends Module {
	...
	build(options, compilation, resolver, fs, callback) {
		...
		const { node, web } = options.externalsPresets;
		this.buildInfo = {
			strict: true,
			topLevelDeclarations: new Set(),
			javascriptModule: compilation.outputOptions.module,
			// universal target (e.g. `["node", "web"]`) where node externals may also be loaded in the browser
			universalExternal: Boolean(node && web)
		};
			...
			case "node-commonjs":
				return /** @type {BuildInfo} */ (this.buildInfo).javascriptModule
					? getSourceForCommonJsExternalInNodeModule(
							request,
							runtimeTemplate,
							/** @type {ExternalModuleBuildInfo} */ (this.buildInfo)
								.universalExternal
						)
