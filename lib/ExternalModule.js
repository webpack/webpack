 ...
 * @returns {string} local variable name
 */
const getExternalModuleVariableName = (id) =>
	`__WEBPACK_EXTERNAL_MODULE_${Template.toIdentifier(`${id}`)}__`;

/**
 * @param {ModuleId | string} id the module id
 * @param {string} uniqueName the compilation unique name
 * @returns {string} global variable name
 */
const getExternalModuleGlobalVariableName = (id, uniqueName) =>
	`__WEBPACK_EXTERNAL_MODULE_${Template.toIdentifier(`${uniqueName}_${id}`)}__`;
 ...
 * @param {ModuleId | string} id the module id
 * @param {RuntimeTemplate} runtimeTemplate the runtime template
 * @returns {string} the external variable access
 */
const getGlobalAccessForAmdOrUmdExternal = (id, runtimeTemplate) =>
	`${runtimeTemplate.globalObject}[${JSON.stringify(getExternalModuleGlobalVariableName(id, runtimeTemplate.outputOptions.uniqueName))}]`;

/**
 * @param {string} externalVariable the external variable access
 * @param {boolean} optional true, if the module is optional
 * @param {string | string[]} request the request path
 * @param {RuntimeTemplate} runtimeTemplate the runtime template
 * @returns {SourceData} the generated source
 */
const getSourceForExternalVariable = (
	externalVariable,
	optional,
	request,
	runtimeTemplate
 ...
 * @returns {SourceData} the generated source
 */
const getSourceForUmdExternal = (id, request, runtimeTemplate) => {
	const localVariable = getExternalModuleVariableName(id);
	const registeredVariable = getGlobalAccessForAmdOrUmdExternal(
		id,
		runtimeTemplate
	);
	const globalVariable = getSourceForGlobalVariableExternal(
		request,
		runtimeTemplate.globalObject
	).expression;

	return {
		init: Template.asString([
			// Check local cache first (module-level), then runtime-registered, then global
			`var ${localVariable} = typeof ${localVariable} !== "undefined" ? ${localVariable} : typeof ${registeredVariable} !== "undefined" ? ${registeredVariable} : ${globalVariable};`,
			// Throw if still unresolved
			`if (typeof ${localVariable} === "undefined") throw new Error("External module not found: " + ${JSON.stringify(Array.isArray(request) ? request.join(".") : request)});`,
			// Register resolved value in global cache for subsequent lazy chunk loads
			`${registeredVariable} = ${localVariable};`
		]),
	...
	_getSourceData(
		...
		switch (externalType) {
			...
			case "amd-require": {
				...
				return getSourceForExternalVariable(
					getExternalModuleVariableName(id !== null ? id : this.identifier()),
			...
			case "umd2": {
				const id = chunkGraph.getModuleId(this);
				return getSourceForUmdExternal(
