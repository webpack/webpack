...
/**
 * An import phase is spelled with syntax the target has to read, and there is no
 * form to fall back to: a host module's evaluation cannot be deferred, nor its
 * source handed over, without the host doing it. Reported like the `import()`
 * check beside it, so a target that cannot parse it fails the build rather than
 * the bundle.
...
class ModuleExternalInitFragment extends InitFragment {
	...
	constructor(
		...
		if (ident === undefined) {
			...
			// One request imported in two phases is two imports binding two names, so the
			// phase has to reach both the key these are deduplicated by and the name each
			// introduces — otherwise one silently replaces the other.
			const phase = dependencyMeta && dependencyMeta.phase;
