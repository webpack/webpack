...
class ModuleExternalInitFragment extends InitFragment {
	 ...
	 * @param {ImportDependencyMeta=} dependencyMeta the dependency meta
	 * @param {string | HashConstructor=} hashFunction the hash function to use (HashConstructor currently not supported)
	 */
	constructor(
		request,
		imported,
		...
		// Validate hashFunction to prevent serialization issues
		// TODO: Support HashConstructor (function) type for hashFunction parameter
		if (typeof hashFunction !== "string") {
			throw new Error(
				`ModuleExternalInitFragment: Only string hash functions are currently supported with persistent caching. Received type: ${typeof hashFunction}.`
			);
		}
