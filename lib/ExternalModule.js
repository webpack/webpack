...
/** @typedef {CssImportDependencyMeta} DependencyMeta */
	...
	codeGeneration({
		...
		switch (externalType) {
			...
			case "css-import": {
				const sources = new Map();
				const layer =
					this.dependencyMeta.layer !== undefined
						? ` layer(${this.dependencyMeta.layer})`
						: "";
				const supports = this.dependencyMeta.supports
					? ` supports(${this.dependencyMeta.supports})`
					: "";
				const media = this.dependencyMeta.media
					? ` ${this.dependencyMeta.media}`
					: "";
				sources.set(
					"css-import",
					new RawSource(
