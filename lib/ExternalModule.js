...
/**
 * @param {string} strPath the raw path e.g "my-lib/alpha/beta/gamma"
 * @returns {string} the generated array chain '["my-lib"]["alpha"]["beta"][gamma]'
 */
const pathChain = strPath => {
	return strPath
		.split("/")
...
const getSourceForGlobalVariableExternal = (variableName, type) => {
	...
	const objectLookup = variableName.map(r => pathChain(r)).join("");
