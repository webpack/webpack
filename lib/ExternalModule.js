			...
			expression: `new Promise(${runtimeTemplate.basicFunction(
				...
				[
					`
						...
						if(!${RuntimeGlobals.hasOwnProperty}(${RuntimeGlobals.shareScopeMap}, 'default')) reject(__webpack_error__);
						if(!${RuntimeGlobals.hasOwnProperty}(${RuntimeGlobals.shareScopeMap}['default'], 'webpack/container/reference/${globalName}')) reject(__webpack_error__);
						resolve();
					`
				]
			)}).then(${runtimeTemplate.returningFunction(
				...
				{get: ${runtimeTemplate.basicFunction("module, getScope", [
					`
						debugger;
						var [[scope]] = getScope;
						return ${RuntimeGlobals.shareScopeMap}[scope][module][0].get();
					`
				])}}
				`
			)})`,
