...
/**
 * @typedef {{attributes?: Record<string,string|boolean|number | Record<string,any>>}} BaseDependencyMeta
 */

/**
 * @typedef {DependencyMeta & BaseDependencyMeta} EnhancedDependencyMeta
 * @property {boolean=} isLazyCompilationDependency Whether this dependency is for a lazy-compiled module
 */

/**
 * @typedef {{iife?: boolean,init?: string,expression: string,chunkInitFragments?: InitFragment<ChunkRenderContext>[],runtimeRequirements?: ReadOnlyRuntimeRequirements}} SourceData
...
class ExternalModule extends Module {
	...
	/**
	 * @returns {boolean=} whether this module is for a lazy-compilation module
	 */
	isLazy() {
		return (
			this.dependencyMeta &&
			(this.dependencyMeta.isLazyCompilationDependency === true ||
				(this.buildMeta && this.buildMeta.isLazyCompilationDependency === true))
		);
	}
	...
	codeGeneration({
		...
		if (isLazy) {
			...
			this.buildMeta.isLazyCompilationDependency = true;
		}
		const meta =
			/** @type {DependencyMeta & {isLazyCompilationDependency?: boolean}} */ (
				this.dependencyMeta
