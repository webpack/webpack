...
const getExternalModuleNodeCommonjsInitFragment = (
	...
	// guarded for the browser (no `process`) and old node (no `getBuiltinModule`).
	const getBuiltinModule = runtimeTemplate.optionalChaining(
		"process.getBuiltinModule",
		`(${moduleId}).createRequire(${importMetaName}.url)`
	);
	const content = `${runtimeTemplate.renderConst()} __WEBPACK_EXTERNAL_createRequire_require = typeof process !== "undefined" && ${getBuiltinModule};\n`;
	...
	_getSourceData(
		...
		switch (externalType) {
			...
			case "node-commonjs": {
				...
				// ESM output has no `require`, so load via `createRequire`. node-commonjs always
				// targets node; the generic commonjs family only does so when node is available.
