	...
	if (url === "host") {
		return {
			...
			expression: `new Promise(${runtimeTemplate.basicFunction(
				"resolve, reject",
				[
					`if(!${RuntimeGlobals.shareScopeMap} ||
						!${RuntimeGlobals.hasOwnProperty}(${RuntimeGlobals.shareScopeMap}, 'default') || 
						!${RuntimeGlobals.hasOwnProperty}(${RuntimeGlobals.shareScopeMap}['default'], 'webpack/container/reference/${globalName}')) {
				...
				{get: ${runtimeTemplate.basicFunction("module, getScope", [
					`var [[scope]] = getScope; return ${RuntimeGlobals.shareScopeMap}[scope][module][0].get();`
				])}}
				`
			)})`,
