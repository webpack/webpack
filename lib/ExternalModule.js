 ...
 * @property {[string, string][] | undefined} specifiers
 */
...
class ModuleExternalInitFragment extends InitFragment {
	 ...
	 * @param {boolean} forceNamespace whether to force use namespace import
	 */
	constructor(
		request,
		ident,
		...
		const attributes =
			dependencyMeta && dependencyMeta.attributes
				? dependencyMeta.attributes._isLegacyAssert
					...
					: ` with ${JSON.stringify(dependencyMeta.attributes)}`
				: "";
		if (imported === true || forceNamespace) {
			// namespace
...
const getSourceForModuleExternal = (
	...
	const initFragment = new ModuleExternalInitFragment(
		...
		// when no concatenationScope, fallback to namespace import
	...
	const useNamespace = imported === true || concatenationScope === undefined;
	let moduleRemapping = "";
	if (useNamespace) {
	...
	return {
		...
		init: moduleRemapping
			...
			: undefined,
		specifiers: useNamespace ? undefined : imported,
