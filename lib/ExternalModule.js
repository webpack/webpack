		...
		switch (externalType) {
			...
			default: {
				...
				if (specifiers) {
					...
					// Use `var` rather than `const` here so that when the outer
					// ConcatenatedModule is async-wrapped, an async external
					// (e.g. `import` / `script` / `promise` types — see
					// `buildMeta.async`) can be reassigned in place with the
					// awaited value via `__webpack_handle_async_dependencies__`.
					const decl =
