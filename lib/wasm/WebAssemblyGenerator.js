...
const rewriteImportedGlobals = state => bin => {
	const { additionalInitCode, additionalInitParams } = state;
	const newGlobals = [];
	...
	bin = editWithAST(state.ast, bin, {
		...
		Global(path) {
			...
			if (init.id === "get_global") {
				...
				additionalInitParams.push(t.funcParam(valtype, initialGlobalidx));
			}
 ...
 * @param {t.Instruction[]} state.additionalInitCode list of addition params for the init function
...
const addInitFunction = ({
	...
	// Add additional code and params
	funcBody.push(...additionalInitCode);
	funcParams.push(...additionalInitParams);
...
class WebAssemblyGenerator extends Generator {
	generate(module) {
		...
		const transform = compose(
			...
			rewriteImportedGlobals({ ast, additionalInitCode, additionalInitParams }),
