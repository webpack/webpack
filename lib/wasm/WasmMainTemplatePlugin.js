	...
	for (const dep of module.dependencies) {
		if (dep instanceof WebAssemblyImportDependency) {
			...
			if (usedName !== false) {
				...
				const propertyName = Template.numberToIdentifer(index);

				if (direct) {
					const instanceVar = `m${waitForPromises.size}`;
					waitForPromises.set(
						instanceVar,
						`installedWasmModules[${JSON.stringify(importedModule.id)}]`
					);
					properties.push(
					...
					Template.indent([
						...
						Template.indent([
							...
							this.supportsStreaming
								? Template.asString([
										...
										Template.indent([
											...
											Template.indent([
												"return WebAssembly.instantiate(items[0], {a:items[1]});"
											]),
											"});"
										]),
										"} else if(typeof WebAssembly.instantiateStreaming === 'function') {",
										Template.indent([
											"promise = WebAssembly.instantiateStreaming(req, {a:importObject});"
										])
								  ])
								: Template.asString([
							...
							Template.indent([
								...
								Template.indent([
									"return WebAssembly.instantiate(bytes, {a:importObject});"
								]),
								"});"
							]),
							"}",
