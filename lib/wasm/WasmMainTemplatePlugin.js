	...
	for (const pair of depsByRequest) {
		...
		for (const data of pair[1]) {
			if (data.direct) {
				...
				properties.push(
					`${JSON.stringify(data.exportName)}: ${instanceVar}` +
				...
				const mod = `installedModules[${JSON.stringify(data.module.id)}]`;
				const func = `${mod}.exports[${JSON.stringify(data.usedName)}]`;

				properties.push(
					Template.asString([
						`${JSON.stringify(data.exportName)}: ` +
							(data.module.type.startsWith("webassembly")
								? `${mod} ? ${func} : `
								: "") +
							`function(${params}) {`,
