...
class WebAssemblyJavascriptGenerator extends Generator {
	...
	generate(module, dependencyTemplates, runtimeTemplate, type) {
		const initIdentifier = Array.isArray(module.usedExports)
			? Template.numberToIdentifier(module.usedExports.length)
			: "__webpack_init__";
			...
			[
				...
				Array.isArray(module.usedExports) && !needExportsCopy
					...
					: "for(var name in wasmExports) " +
					  `if(name != ${JSON.stringify(initIdentifier)}) ` +
					  `${module.exportsArgument}[name] = wasmExports[name];`,
				"// exec imports from WebAssembly module (for esm order)",
				importsCode,
				"",
				"// exec wasm module",
				`wasmExports[${JSON.stringify(initIdentifier)}](${initParams.join(
					", "
				)})`
			].join("\n")
		);
