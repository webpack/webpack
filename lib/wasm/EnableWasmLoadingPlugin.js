...
/** @import { WasmLoadingType } from "../../declarations/WebpackOptions" */
		...
		if (typeof type === "string") {
			switch (type) {
				...
				case "async-node": {
					if (compiler.options.experiments.syncWebAssembly) {
						...
						new ReadFileCompileWasmPlugin({
							mangleImports: compiler.options.optimization.mangleWasmImports,
							// ESM output has no `require`, and any runtime that loads ESM has
							// `import()` — pairing them by `dynamicImport` emitted CJS into an
							// ES module, which the runtime refuses to load at all.
							import: compiler.options.output.module
						}).apply(compiler);
					}
				...
				case "universal": {
					...
					if (compiler.options.experiments.asyncWebAssembly) {
						const UniversalCompileAsyncWasmPlugin = require("./UniversalCompileAsyncWasmPlugin");
