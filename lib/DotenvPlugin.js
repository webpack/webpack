...
class DotenvPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.beforeCompile.tapPromise(PLUGIN_NAME, async () => {
			...
			const { env, fileDependencies, missingDependencies } = await this.loadEnv(
				inputFileSystem,
				mode,
				context
			);
	 ...
	 * @returns {Promise<{ env: Record<string, string>, fileDependencies: string[], missingDependencies: string[] }>} env with dependencies
	 */
	async loadEnv(fs, mode, context) {
		...
		const prefixes = Array.isArray(rawPrefix)
			? rawPrefix
			: [rawPrefix || "WEBPACK_"];
		const dir = typeof rawDir === "string" ? rawDir : context;
		...
		return { env, fileDependencies, missingDependencies };
	}
