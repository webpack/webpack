...
class DotenvPlugin {
	...
	apply(compiler) {
		...
		const prefix = Array.isArray(this.options.prefix)
			? this.options.prefix
	 ...
	 * @param {Prefix} prefixes prefixes
	 * @param {string | false} dir dir to load `.env` files
	 ...
	 */
	async _loadEnv(compiler, prefixes, dir, cache, fileSystemInfo) {
		const fs = /** @type {InputFileSystem} */ (compiler.inputFileSystem);
		...
		if (result) {
			...
			if (isSnapshotValid) {
				const env = this._getEnv(prefixes, result.parsed);
