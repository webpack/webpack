...
class ModuleChunkLoadingRuntimeModule extends RuntimeModule {
	...
	_generateBaseUri(chunk, rootOutputDir) {
		...
		const {
			outputOptions: { importMetaName, path }
		} = compilation;
		// Handle case where ESM output is used in a CommonJS context (e.g., tests)
		return Template.asString([
			"// Set base URI for resolving URLs",
			...
			"} else {",
			Template.indent([
				"// Try to use import.meta if available (wrapped to avoid syntax error)",
				"try {",
				Template.indent([
					`${RuntimeGlobals.baseURI} = new Function('return new URL(${JSON.stringify(
						rootOutputDir
					)}, ${importMetaName}.url)')();`
				]),
				"} catch(e) {",
	...
	generate() {
		...
		return Template.asString([
			...
			withHmr
				? Template.asString([
						"// HMR chunk loading for ESM",
						`${RuntimeGlobals.hmrDownloadUpdateHandlers} = ${
							RuntimeGlobals.hmrDownloadUpdateHandlers
						} || {};`,
						`${RuntimeGlobals.hmrDownloadUpdateHandlers}.module = ${runtimeTemplate.basicFunction(
							...
							[
								...
								Template.indent([
									...
									Template.indent([
										...
										Template.indent([
											`if (${RuntimeGlobals.hasOwnProperty}(currentUpdate, moduleId)) {`,
											Template.indent([
												`${RuntimeGlobals.moduleFactories}[moduleId] = currentUpdate[moduleId];`
											]),
											"}"
										]),
										"}",
										...
										Template.indent([
											`currentUpdateRuntime[i](${RuntimeGlobals.require});`
										]),
										"}"
									]),
									"}"
								...
								`chunkIds.map(${runtimeTemplate.basicFunction("chunkId", [
									...
									Template.indent([
										"// In Node.js, we need to use the file path, not the public path",
										`var filename = ${RuntimeGlobals.getChunkUpdateScriptFilename}(chunkId);`,
										"var importUrl;",
										"try {",
										Template.indent([
											"// Try to construct a file URL using baseURI",
											`importUrl = new URL(filename, ${RuntimeGlobals.baseURI}).href;`
										]),
										"} catch(e) {",
										Template.indent([
											"// Fallback to using URL with publicPath",
											"try {",
											Template.indent([
												`importUrl = new URL(url, ${RuntimeGlobals.baseURI}).href;`
											]),
											"} catch(e2) {",
											Template.indent("importUrl = url;"),
											"}"
										]),
										"}",
										`promises.push(${importFunctionName}(/* webpackIgnore: true */ importUrl).then(${runtimeTemplate.basicFunction(
											"update",
											[
												"var { __webpack_ids__, __webpack_modules__, __webpack_runtime__ } = update;",
												"for(var moduleId in __webpack_modules__) {",
												Template.indent([
													`if(${RuntimeGlobals.hasOwnProperty}(__webpack_modules__, moduleId)) {`,
													Template.indent([
														"currentUpdate[moduleId] = __webpack_modules__[moduleId];",
														"updatedModulesList.push(moduleId);"
													]),
													"}"
												]),
												"}",
												"if(__webpack_runtime__) currentUpdateRuntime.push(__webpack_runtime__);"
											]
										)}));`
									]),
									"} else {",
									Template.indent([
										`promises.push(${importFunctionName}(/* webpackIgnore: true */ url).then(${runtimeTemplate.basicFunction(
											"update",
											[
												"var { __webpack_ids__, __webpack_modules__, __webpack_runtime__ } = update;",
												"for(var moduleId in __webpack_modules__) {",
			...
			withHmrManifest
				? Template.asString([
						`${RuntimeGlobals.hmrDownloadManifest} = ${runtimeTemplate.basicFunction(
							"",
							[
								// Check if we're in a Node.js environment
								"if (typeof process !== 'undefined' && process.versions && process.versions.node) {",
								Template.indent([
									"// Node.js environment - use fs to read the manifest",
									"return new Promise(function(resolve, reject) {",
									Template.indent([
										`var filename = new URL(${RuntimeGlobals.publicPath} + ${RuntimeGlobals.getUpdateManifestFilename}(), ${RuntimeGlobals.baseURI}).pathname;`,
										"var fs = require('fs');",
								...
								Template.indent([
									...
									`return fetch(${RuntimeGlobals.publicPath} + ${RuntimeGlobals.getUpdateManifestFilename}()).then(${runtimeTemplate.basicFunction(
										"response",
										[
