...
const getAPIPlugin = memoize(() => require("../javascript/APIPlugin"));
...
class ModuleChunkLoadingRuntimeModule extends RuntimeModule {
	...
	getInstalledChunkHandlers() {
		...
		if (
			...
			(runtimeRequirements.has(RuntimeGlobals.ensureChunk) || withHmr)
		) {
			handlers.push([RuntimeGlobals.ensureChunkHandlers, "j"]);
		}
		// The hot update flow installs this one onto the same map while it runs.
		if (withHmr) {
			handlers.push([RuntimeGlobals.ensureChunkHandlers, "moduleHmr"]);
		}
		if (
			withHint &&
			...
			handlers.push([RuntimeGlobals.prefetchChunkHandlers, "j"]);
		}
		if (
			withHint &&
			...
			handlers.push([RuntimeGlobals.preloadChunkHandlers, "j"]);
		}
		if (runtimeRequirements.has(RuntimeGlobals.onChunksLoaded)) {
			handlers.push([RuntimeGlobals.onChunksLoaded, "j"]);
		}
		return handlers;
	}
