...
const ChunkImportMapPlugin = require("./ChunkImportMapPlugin");
...
/**
 * The document-resolvable URL of a chunk (`publicPath` + filename), used as an
 * import-map value. A relative `publicPath` (`""`/`"auto"`) falls back to a
 * document-relative `./name` (import-map values must start with `/`, `./`, `../`
 * or be absolute).
 * @param {Compilation} compilation the compilation instance
 * @param {Chunk} chunk the chunk
 * @returns {string} the URL
 */
const getChunkPublicUrl = (compilation, chunk) => {
	const filename = compilation
		.getPath(getChunkFilenameTemplate(chunk, compilation.outputOptions), {
			chunk,
			contentHashType: "javascript"
		})
	...
	const prefix =
		typeof publicPath === "string" && publicPath !== "auto" && publicPath !== ""
			? publicPath
			: "./";
...
/**
 * Renders chunk import. When the chunk import map is active the hashed relative
 * path is replaced by a stable `webpack/c/<id>` specifier (recorded in the map),
 * so the importing chunk's source is independent of the imported chunk's hash.
 * @param {Compilation} compilation the compilation instance
...
function renderChunkImport(compilation, chunk, namedImport, runtimeChunk) {
	const importMap = ChunkImportMapPlugin.getImportMap(compilation);
	let specifier;
	if (importMap !== undefined) {
		specifier = ChunkImportMapPlugin.getSpecifier(chunk);
		if (!importMap.has(specifier)) {
			importMap.set(specifier, getChunkPublicUrl(compilation, chunk));
		}
	} else {
		specifier = getRelativePath(compilation, chunk, runtimeChunk || chunk);
	}
	return `import ${
