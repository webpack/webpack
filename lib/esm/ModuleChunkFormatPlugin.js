 ...
 * path is replaced by a stable `webpack/c/<id>` specifier the map resolves, so
 * the importing chunk's source is independent of the imported chunk's hash.
 * @param {Compilation} compilation the compilation instance
...
function renderChunkImport(compilation, chunk, namedImport, runtimeChunk) {
	const specifier = ChunkImportMapPlugin.isEnabled(compilation)
