				...
				if (runtimeChunk) {
					...
					for (let i = 0; i < entries.length; i++) {
						...
						if (module === inlinedEntryModule && inlinedEntrySource) {
							...
							// `__webpack_exec__` assigns `entryModuleId` on the way in where a
							// reader needs it, which the annotation would license a minifier to
							// drop along with the call.
							const pure = chunkGraph
							...
							startupSource.add(
								...
								}${pure}__webpack_exec__(${JSON.stringify(moduleId)});\n`
							);
						}
					}
